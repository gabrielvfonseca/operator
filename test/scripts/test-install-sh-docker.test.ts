...
`,
  ...
  it("keeps the Podman Quadlet template aligned with setup substitutions", () => {
    ...
    expect(template).toContain("Volume={{OPENCLAW_CONFIG_DIR}}:/home/node/.operator:Z");
    expect(template).toContain(
      "Volume={{OPENCLAW_WORKSPACE_DIR}}:/home/node/.operator/workspace:Z",
    );
...
describe("bun global install smoke", () => {
  ...
  it("requires root and AI candidate versions to match", () => {
    ...
    expect(mismatched.stderr).toContain(
      "candidate version mismatch: operator=2026.6.17, dependency=2026.6.17, @operator/ai=2026.6.18",
    );
  });
...
cat >"$BUN_INSTALL/bin/operator" <<'OPENCLAW'
#!/usr/bin/env bash
if [ "\${1:-}" = "--version" ]; then
...
`,
  ...
  it("gates workflow Bun install smoke to scheduled and release-check runs", () => {
    ...
    expect(workflow).toContain("OPENCLAW_INSTALL_URL: file:///tmp/operator-install.sh");
    expect(workflow).toContain("OPENCLAW_INSTALL_CLI_URL: file:///tmp/operator-install-cli.sh");
