...
} from "../../.agents/skills/operator-changelog-update/scripts/verify-release-notes.mjs";
...
describe("renderContributionRecordEntry", () => {
  it("keeps source and linked issue references without repeating PR titles", () => {
    expect(
      renderContributionRecordEntry({
        number: 123,
        title: "Fix local operator/operator#45 and operator/imsg#141",
        linkedIssues: [{ number: 45 }, { number: 67 }],
        thanks: ["alice", "bob"],
      }),
    ).toBe("- **PR #123** Related #45, operator/imsg#141, #67. Thanks @alice and @bob.");
  });
  ...
  it("deduplicates title references and retains seeded cross-repository references", () => {
    expect(
      renderContributionRecordEntry({
        number: 124,
        title: "Fix #45, #45, and Operator/imsg#141",
        externalReferences: ["operator/imsg#141"],
        priorReferences: [67],
        linkedIssues: [{ number: 45 }],
        thanks: [],
      }),
    ).toBe("- **PR #124** Related #45, Operator/imsg#141, #67.");
  });
  ...
  it("retains references and credits when a compact record is seeded again", () => {
    const line = "- **PR #125** Related #45, operator/imsg#141. Thanks @alice and @bob.";
  ...
  it("retains references from a verbose record when the source title changes", () => {
    const record = contributionRecordFor({
      source: [
        ...
        "#### Pull requests",
        "",
        "- **PR #126** Fix #46 and operator/imsg#142. Related #68. Thanks @alice.",
      ].join("\n"),
    });
    const seeded = record.pullRequests.get(126);

    expect(seeded).toEqual({
      externalReferences: ["operator/imsg#142"],
      references: [46, 68],
      thanks: ["alice"],
    });
  });
  ...
  it("requires complete reference tokens rather than matching substrings", () => {
    const source = [
      ...
      "#### Pull requests",
      "",
      "- **PR #456** Related operator/imsg#141.",
    ].join("\n");
    const entry = {
      number: 456,
      title: "Internal cleanup",
  ...
  it("accepts case-only differences in cross-repository references", () => {
    const line = "- **PR #127** Related Operator/imsg#143.";
    const source = [
      "## 2026.7.1",
      "",
      "### Highlights",
    ...
    const entry = {
      ...
      editorialEligible: false,
      priorReferences: [],
      externalReferences: ["operator/imsg#143"],
      linkedIssues: [],
      thanks: [],
    };
