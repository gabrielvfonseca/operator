...
const WORKFLOW_PATH = ".github/workflows/operator-cross-os-release-checks-reusable.yml";
const RELEASE_CHECKS_PATH = ".github/workflows/operator-release-checks.yml";
const WRAPPER_PATH = "scripts/github/run-operator-cross-os-release-checks.sh";
const SCRIPT_PATH = "scripts/operator-cross-os-release-checks.ts";
const HARNESS = "bash workflow/scripts/github/run-operator-cross-os-release-checks.sh";
...
describe("cross-OS release checks workflow", () => {
  ...
  it("binds the prepared release package to an immutable artifact and package tuple", () => {
    ...
    expect(metadata.run).toContain("file_name=operator-current.tgz");
  ...
  it("downloads and re-exports exact candidate artifacts only by immutable id", () => {
    ...
    expect(resolve.run).toContain("resolve-operator-package-candidate.mjs");
    ...
    expect(upload.with?.name).toBe(
      "operator-cross-os-release-checks-candidate-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    ...
    expect(baselineUpload.with?.name).toBe(
      "operator-cross-os-release-checks-baseline-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    ...
    expect(binding.env).toMatchObject({
      ...
      ARTIFACT_NAME:
        "${{ format('operator-cross-os-release-checks-candidate-{0}-{1}', needs.prepare.outputs.candidate_artifact_run_id, needs.prepare.outputs.candidate_artifact_run_attempt) }}",
      ...
      BASELINE_ARTIFACT_NAME:
        "${{ format('operator-cross-os-release-checks-baseline-{0}-{1}', needs.prepare.outputs.baseline_artifact_run_id, needs.prepare.outputs.baseline_artifact_run_attempt) }}",
