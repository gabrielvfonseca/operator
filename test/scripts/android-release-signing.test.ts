...
function writeSigningSources(tempRoot: string) {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "OPENCLAW_ANDROID_KEY_ALIAS=operator-upload",
  ...
  it.runIf(commandAvailable("openssl"))(
    ...
    () => {
      ...
      const encryptedProperties = fs.readFileSync(
        path.join(remoteCheck, "android", "operator", "gradle.properties.enc"),
        "utf8",
      );
      ...
      expect(materializedProperties).toContain("OPENCLAW_ANDROID_KEY_ALIAS=operator-upload");
