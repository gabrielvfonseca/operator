// Operator NPM Publish tests cover publish wrapper argument safety.
...
const scriptPath = "scripts/operator-npm-publish.sh";
...
function makeReleaseCheckout(root: string, version: string): string {
  ...
  copyFileSync(
    "scripts/operator-npm-extended-stable-release.mjs",
    path.join(checkout, "scripts/operator-npm-extended-stable-release.mjs"),
  );
...
function makePackageTarball(root: string, packageJson?: string): string {
  ...
  const tarball = path.join(root, "operator.tgz");
...
describe("operator npm publish wrapper", () => {
  it("prints help without resolving release metadata", () => {
    const result = runPublishWrapper(["--help"]);

    expect(result.status).toBe(0);
    expect(result.stdout.trim()).toBe(
      "usage: bash scripts/operator-npm-publish.sh --publish [package.tgz]",
    );
  ...
  it("rejects extra publish arguments before npm publish", () => {
    ...
    const tarball = path.join(tempRoot, "operator.tgz");
  ...
  it.each([
    ...
    ["missing version", JSON.stringify({ name: "operator" }), "has no valid version"],
