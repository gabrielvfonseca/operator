...
describe("scripts/operator-cross-os-release-checks", () => {
  ...
  it("keeps cross-OS live smoke agent turns on GPT-5-safe timeouts and minimal context", () => {
    ...
    expect(source).toContain('agentRuntime: { id: "operator" }');
  ...
  it("resolves Linux npm package roots when the CLI is a user-local shim", () => {
    ...
    try {
      const packageRoot = join(homeDir, ".npm-global", "lib", "node_modules", "operator");
      const distDir = join(packageRoot, "dist");
      const cliDir = join(homeDir, ".local", "bin");
      mkdirSync(distDir, { recursive: true });
      mkdirSync(cliDir, { recursive: true });
      writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "operator" }));
      writeFileSync(join(distDir, "entry.js"), "#!/usr/bin/env node\n");

      expect(
        resolveInstalledPackageRootFromCliPath(join(cliDir, "operator"), "linux", {
          HOME: homeDir,
        }),
      ).toBe(packageRoot);

      rmSync(join(cliDir, "operator"), { force: true });
      symlinkSync(join(distDir, "entry.js"), join(cliDir, "operator"));
    ...
    try {
      const packageRoot =
        process.platform === "win32"
          ? join(prefixDir, "node_modules", "operator")
          : join(prefixDir, "lib", "node_modules", "operator");
