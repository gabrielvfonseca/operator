...
describe("package Telegram live Docker E2E", () => {
  ...
  it("keeps the installed Operator command as the package SUT", async () => {
    ...
    const command = path.join(prefix, "bin", "operator");
  ...
  it("keeps private QA harness imports local while using the installed package dist", () => {
    ...
    expect(gatewayRpcClient).toContain('from "operator/plugin-sdk/gateway-runtime"');
    expect(qaRuntimeApi).toContain('from "operator/plugin-sdk/gateway-runtime"');
    for (const source of qaHarnessSources) {
      expect(source).not.toContain('from "operator/plugin-sdk/qa-runtime"');
    }
  });
