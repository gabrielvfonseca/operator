...
describe("security-sensitive guard workflow", () => {
  ...
  it("keeps detection separate from the final required check", () => {
    ...
    expect(finalSteps.at(-1)?.env?.OPENCLAW_SECURITY_TEAM_SLUG).toBe("operator-secops");
  ...
  it("requires secops review for future workflow or guard changes", () => {
    ...
    expect(codeowners).toContain(
      "/.github/workflows/security-sensitive-guard.yml @operator/operator-secops",
    );
    expect(codeowners).toContain(
      "/test/scripts/security-sensitive-guard-workflow.test.ts @operator/operator-secops",
    );
    expect(codeowners).toContain(
      "/test/scripts/security-sensitive-guard-script.test.ts @operator/operator-secops",
    );
    expect(codeowners).toContain(
      "/scripts/github/security-sensitive-guard.mjs @operator/operator-secops",
    );
    expect(codeowners).toContain("/.gitignore @operator/operator-secops");
  });
});
