...
function makeRepository(): { root: string; commit: string } {
  const root = mkdtempSync(path.join(tmpdir(), "operator-apple-release-source-"));
  tempDirs.push(root);
  execFileSync("git", ["init", "--quiet"], { cwd: root });
  execFileSync("git", ["config", "user.email", "release-test@operator.test"], { cwd: root });
  execFileSync("git", ["config", "user.name", "Operator Release Test"], { cwd: root });
