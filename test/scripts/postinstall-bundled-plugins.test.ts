...
describe("bundled plugin postinstall", () => {
  ...
  it("prunes Node versioned compile cache dirs during package postinstall", () => {
    ...
    const readdirSync = vi.fn((value: string) => {
      if (value === configuredBase) {
        return [
          ...
          { name: "operator", isDirectory: () => true },
    ...
    expect(removed).not.toContain(path.join(configuredBase, "operator"));
  ...
  it("resolves legacy plugin runtime deps roots from Operator state env", () => {
    expect(
      collectLegacyPluginRuntimeDepsStateRoots({
        env: {
          HOME: "/users/alice",
          OPENCLAW_HOME: "/srv/operator-home",
          OPENCLAW_CONFIG_PATH: "~/profile/operator.json",
  ...
  it("prunes only bundled plugin package node_modules in source checkouts", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@operator/acpx" }),
    );
