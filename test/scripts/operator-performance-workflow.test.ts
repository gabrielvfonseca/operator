...
const WORKFLOW = ".github/workflows/operator-performance.yml";
...
describe("Operator performance workflow", () => {
  ...
  it("resolves each target once before benchmark and publication fan out", () => {
    ...
    const resolveTarget = findStep("Resolve Operator target ref", "resolve_target");
    const checkout = findStep("Checkout Operator");
    const record = findStep("Record tested revision");
    const sourceCheckout = findStep("Checkout Operator source target", "source_performance");
    ...
    expect(
      Object.values(workflow.jobs ?? {})
        ...
        .filter((step) => step.name === "Resolve Operator target ref"),
    ).toHaveLength(1);
  });
  ...
  it("isolates required publication in a fresh artifact-consuming job", () => {
    ...
    expect(kovaSteps.some((step) => step.name === "Run Operator source performance probes")).toBe(
      false,
    );
    expect(
      workflow.jobs?.source_performance?.steps?.some(
        (step) => step.name === "Run Operator source performance probes",
      ),
    ).toBe(true);
  ...
  it("reuses the producing artifact when only publisher jobs rerun", () => {
    const artifact = findStep("Resolve Kova artifact", "publish");
    const root = mkdtempSync(join(realpathSync(tmpdir()), "operator-artifact-resolver-"));
    ...
    writeFileSync(
      ...
      `#!/bin/sh
printf '%s\\n' \
  '101	operator-performance-mock-provider-9001-1' \
  '202	operator-performance-source-9001-2' \
  '303	operator-performance-mock-provider-9001-3'
`,
    );
  ...
  it("advertises a clawgrit URL only after a direct or remotely verified push", () => {
    const publish = findStep("Publish to clawgrit reports", "publish");
    const root = mkdtempSync(join(realpathSync(tmpdir()), "operator-publish-shell-"));
    const bin = join(root, "bin");
    const reportsRoot = join(root, "reports");
    const reportUrl =
      "https://github.com/operator/clawgrit-reports/tree/main/operator-performance/main/123-1/mock-provider";
    mkdirSync(bin);
...
case "$*" in
  ...
  *"remote get-url origin"*) echo https://github.com/operator/clawgrit-reports.git ;;
...
`,
    ...
    const execute = ({
      ...
      const result = spawnSync("bash", ["-c", publish.run ?? ""], {
        ...
        env: {
          ...
          DEST_REL: "operator-performance/main/123-1/mock-provider",
  ...
  it("preserves both reports when concurrent writers update one latest pointer", () => {
    const root = mkdtempSync(join(realpathSync(tmpdir()), "operator-report-race-"));
    ...
    const latest = "operator-performance/main/latest-mock-provider.json";
    const reportA = "operator-performance/main/100-1/mock-provider";
    const reportB = "operator-performance/main/200-1/mock-provider";
    ...
    const commitReport = (repo: string, reportPath: string, marker: string) => {
      ...
      runGit(repo, ["add", "--", "operator-performance"]);
