...
function entitlementTemps(dir: string): string[] {
  return readdirSync(dir).filter((name) => name.startsWith("operator-entitlements"));
}
...
describe("codesign-mac-app temp file hygiene", () => {
  ...
  it("does not allocate entitlement temp files for help output", () => {
    const tempRoot = makeTempDir("operator-codesign-help-");
  ...
  it("does not allocate entitlement temp files before app validation", () => {
    const tempRoot = makeTempDir("operator-codesign-missing-");
  ...
  it("rejects unknown options before app validation", () => {
    const tempRoot = makeTempDir("operator-codesign-unknown-");
  ...
  it("rejects extra app bundle arguments before signing", () => {
    const tempRoot = makeTempDir("operator-codesign-extra-");
  ...
  it("cleans entitlement temp files when signing fails", () => {
    const tempRoot = makeTempDir("operator-codesign-fail-");
    const app = path.join(tempRoot, "Fake.app");
    mkdirSync(path.join(app, "Contents", "MacOS"), { recursive: true });
  ...
  it("passes generated app entitlements to signing commands and cleans them", () => {
    const tempRoot = makeTempDir("operator-codesign-success-");
    const app = path.join(tempRoot, "Fake.app");
    ...
    writeFileSync(path.join(app, "Contents", "MacOS", "operator-mlx-tts"), "#!/bin/sh\n");
    writeFileSync(path.join(app, "Contents", "MacOS", "Operator"), "#!/bin/sh\n");
    ...
    expect(signLines[0]).toContain(`${path.join(app, "Contents", "MacOS", "operator-mlx-tts")}\t`);
    expect(signLines[1]).toContain(`${path.join(app, "Contents", "MacOS", "Operator")}\t`);
    ...
    for (const line of signLines) {
      ...
      expect(entitlementSource).toContain("operator-entitlements");
