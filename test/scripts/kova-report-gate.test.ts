...
function commandResult() {
  return {
    command: "ocm @env -- operator agent --local",
    status: 0,
    stderr: "",
    stdout: "",
    timedOut: false,
...
function partialReport(): JsonObject {
  return {
    ...
    target: "local-build:/workspace/operator",
...
function writeReport(report: unknown): string {
  const root = mkdtempSync(join(tmpdir(), "operator-kova-report-"));
  tempRoots.push(root);
...
describe("scripts/lib/kova-report-gate.mjs", () => {
  ...
  const profiledMutations: ReportMutation[] = [
    ...
    [
      ...
      (report) => (blockingCard(report).failedCommand = "operator agent"),
    ],
    [
    ...
    [
      ...
      (report) => {
        const cards = arrayAt(objectAt(report.gate).cards);
        cards.push({ ...infoCard(), kind: "operator-failure", status: "FAIL" });
