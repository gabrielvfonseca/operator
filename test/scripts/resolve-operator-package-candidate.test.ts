...
import {
  ...
  cleanPackedOperatorTarballsForTest,
  downloadUrl,
  ...
  validateOperatorPackageSpec,
} from "../../scripts/resolve-operator-package-candidate.mjs";
...
describe("resolve-operator-package-candidate", () => {
  it("allows Unreleased notes when packaging an exact ref candidate", () => {
    const script = readFileSync("scripts/resolve-operator-package-candidate.mjs", "utf8");
    const refPackageBuild = script.slice(
      script.indexOf('if (options.source === "ref")'),
      script.indexOf('} else if (options.source === "npm")'),
    );

    expect(refPackageBuild).toContain('"scripts/package-operator-for-docker.mjs"');
    expect(refPackageBuild).toContain('"--allow-unreleased-changelog"');
  });

  it("accepts only Operator release package specs for npm candidates", () => {
    for (const spec of [
      "operator@beta",
      "operator@alpha",
      "operator@extended-stable",
      "operator@latest",
      "operator@2026.4.27",
      "operator@2026.4.27-1",
      "operator@2026.4.27-beta.2",
      "operator@2026.4.27-alpha.2",
    ]) {
      expect(validateOperatorPackageSpec(spec), spec).toBeUndefined();
    }

    expect(() => validateOperatorPackageSpec("@evil/operator@1.0.0")).toThrow(
      "package_spec must be operator@alpha",
    );
    expect(() => validateOperatorPackageSpec("operator@canary")).toThrow(
      "package_spec must be operator@alpha",
    );
    expect(() => validateOperatorPackageSpec("operator@2026.04.27")).toThrow(
      "package_spec must be operator@alpha",
    );
    expect(() => validateOperatorPackageSpec("operator@npm:other-package")).toThrow(
      "package_spec must be operator@alpha",
    );
    expect(() => validateOperatorPackageSpec("operator@file:../other-package.tgz")).toThrow(
      "package_spec must be operator@alpha",
    );
  });
  ...
  it("parses optional empty workflow inputs without rejecting the command line", () => {
    expect(
      ...
      outputName: "operator-current.tgz",
      packageSha256: "",
      packageRef: "release/2026.4.27",
      packageSpec: "operator@beta",
      packageUrl: "",
      source: "npm",
      trustedSourceId: "",
  ...
  it("rejects duplicate package candidate CLI options", () => {
    ...
    const duplicateCases = [
      ...
      [
        "--package-spec",
        [...requiredArgs, "--package-spec", "operator@beta", "--package-spec", "operator@latest"],
      ],
      [
        "--package-url",
        [...requiredArgs, "--package-url", "", "--package-url", "https://example.com/operator.tgz"],
      ],
  ...
  it("rejects package candidate output names that escape the output directory", () => {
    for (const outputName of [
      "../operator-current.tgz",
      "nested/operator-current.tgz",
      "operator-current.zip",
      ".operator-current.tgz",
    ]) {
    ...
    expect(parseArgs(["--output-name", "operator-current.tar.gz"]).outputName).toBe(
      "operator-current.tar.gz",
    );
  });
  ...
  it("resolves npm package candidates through the Windows npm.cmd toolchain shim", () => {
    ...
    const runner = resolveNpmPackageCandidatePackRunner(
      "operator@2026.5.26-beta.1",
      "C:\\operator\\.artifacts\\docker-e2e-package",
      {
    ...
    expect(runner).toEqual({
      ...
      args: [
        ...
        "/s",
        "/c",
        `${npmCmdPath} pack operator@2026.5.26-beta.1 --ignore-scripts --json --pack-destination C:\\operator\\.artifacts\\docker-e2e-package`,
      ],
      shell: false,
  ...
  it("keeps npm pack filenames inside the package candidate output directory", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-npm-pack-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "operator-2026.6.17.tgz"), "package");

    await expect(
      moveNewestPackedTarballForTest(
        dir,
        JSON.stringify([{ filename: "operator-2026.6.17.tgz" }]),
        "operator-current.tgz",
      ),
    ).resolves.toBe(path.join(dir, "operator-current.tgz"));
    await expect(readFile(path.join(dir, "operator-current.tgz"), "utf8")).resolves.toBe("package");
  });
  ...
  it("rejects path-like npm pack filenames instead of renaming outside the output directory", async () => {
    ...
    const unsafeFilenames = [
      "../operator-2026.6.17.tgz",
      "nested/operator-2026.6.17.tgz",
      "nested\\operator-2026.6.17.tgz",
      "/tmp/operator-2026.6.17.tgz",
      "C:\\temp\\operator-2026.6.17.tgz",
      "operator-2026.6.17.tar.gz",
    ];

    for (const filename of unsafeFilenames) {
      await expect(
        moveNewestPackedTarballForTest(dir, JSON.stringify([{ filename }]), "operator-current.tgz"),
      ).rejects.toThrow("npm pack reported unsafe Operator tarball filename");
    }
  });
  ...
  it("rejects unsafe text npm pack filenames instead of using loose stdout fallback", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-npm-pack-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "operator-2026.6.17.tgz"), "safe fallback");

    for (const filename of ["../operator-2026.6.17.tgz", "C:operator-2026.6.17.tgz"]) {
      await expect(
        moveNewestPackedTarballForTest(
          dir,
  ...
  it("cleans stale package tarballs before npm fallback scanning", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-npm-pack-stale-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "operator-9999.1.1.tgz"), "stale");
    await writeFile(path.join(dir, "operator-C:evil.tgz"), "unsafe");

    await cleanPackedOperatorTarballsForTest(dir);
    await writeFile(path.join(dir, "operator-2026.6.17.tgz"), "current");

    await expect(
      moveNewestPackedTarballForTest(dir, "npm notice\n", "operator-current.tgz"),
    ).resolves.toBe(path.join(dir, "operator-current.tgz"));
    await expect(missing(path.join(dir, "operator-9999.1.1.tgz"))).resolves.toBe(true);
    await expect(readFile(path.join(dir, "operator-C:evil.tgz"), "utf8")).resolves.toBe("unsafe");
    await expect(readFile(path.join(dir, "operator-current.tgz"), "utf8")).resolves.toBe("current");
  });
  ...
  it("kills timed-out package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-runner-timeout-"));
    tempDirs.push(dir);
  ...
  it("clamps oversized package runner kill grace before scheduling", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-runner-grace-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
  ...
  it("rejects timed-out package runner commands when descendants exit cleanly", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-runner-timeout-clean-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
  ...
  it("forwards external termination to package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-runner-signal-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
    const scriptUrl = pathToFileURL(
      path.resolve("scripts/resolve-operator-package-candidate.mjs"),
    ).href;
    let childPid: number | undefined;
  ...
  it("fails successful ref candidates when package source worktree cleanup fails", async () => {
    await expect(
      cleanupPackageSourceWorktreeForTest("/tmp/operator-package-source-stuck", {
        runImpl: async () => {
  ...
  it("preserves original ref candidate failures when worktree cleanup also fails", async () => {
    ...
    expect(warnings).toEqual([
      "warning: failed to remove temporary package source worktree /tmp/operator-package-source-stuck: worktree remove denied",
    ]);
  });

  it("loads named trusted package URL source policies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "operator-trusted-package-source-"));
    tempDirs.push(dir);
      ...
      JSON.stringify({
        ...
        sources: {
          "enterprise-artifactory": {
            ...
            pathPrefixes: ["/artifactory/operator/"],
            ports: [443, 8443],
  ...
  it("allows private package_url downloads only through an explicit trusted source policy", async () => {
    ...
    await downloadUrl("https://packages.internal:8443/artifactory/operator/operator.tgz", target, {
    ...
    expect(requestedUrls).toEqual([
      "https://packages.internal:8443/artifactory/operator/operator.tgz",
    ]);
    await expect(readFile(target)).resolves.toEqual(Buffer.from([4, 5, 6]));

    await expect(
      downloadUrl("https://evil.internal:8443/artifactory/operator/operator.tgz", target, {
        fetchImpl: unexpectedFetch,
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/other/operator.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "203.0.113.8", family: 4 }]),
        trustedSource,
      }),
  ...
  it("matches trusted package_url path prefixes on path segment boundaries", async () => {
    ...
    await downloadUrl("https://packages.internal:8443/artifactory/operator/pkg.tgz", target, {
      fetchImpl: async (url: URL) => {
        requestedUrls.push(url.toString());
    ...
    expect(requestedUrls).toEqual(["https://packages.internal:8443/artifactory/operator/pkg.tgz"]);
    await expect(
      downloadUrl("https://packages.internal:8443/artifactory/operator-malicious/pkg.tgz", target, {
        fetchImpl: unexpectedFetch,
        lookupHost: lookupAddresses([{ address: "203.0.113.8", family: 4 }]),
        trustedSource,
      }),
  ...
  it("keeps trusted package_url redirects inside the named source policy", async () => {
    ...
    await expect(
      downloadUrl("https://packages.internal:8443/artifactory/operator/operator.tgz", target, {
        fetchImpl: async () =>
          new Response(null, {
            headers: { location: "https://metadata.internal:8443/artifactory/operator/pwn.tgz" },
            status: 302,
          }),
        lookupHost: lookupAddresses([{ address: "10.0.0.8", family: 4 }]),
        trustedSource,
          ...
          fetchImpl: async (_url: URL, init?: RequestInit) => {
            ...
            if (requestHeaders.length === 1) {
              return new Response(null, {
                headers: {
                  location: "https://mirror.internal:8443/artifactory/operator/operator.tgz",
                },
                status: 302,
              });
            }
  ...
  it("validates redirects for package_url downloads", async () => {
    ...
    expect(requestedUrls).toEqual(["https://packages.example/operator.tgz"]);
  });
  ...
  it("bounds package_url downloads and writes completed files atomically", async () => {
    ...
    await downloadUrl("https://packages.example/operator.tgz", target, {
      fetchImpl: async () =>
  ...
  it("times out stalled package_url response bodies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-download-timeout-"));
    tempDirs.push(dir);
    const target = path.join(dir, "operator.tgz");
    let bodyCancelled = false;
    const startedAt = Date.now();
    ...
    await expect(
      ...
      "package_url download timed out after 25ms: https://packages.example/operator.tgz",
    );
  ...
  it("reads package source metadata from package artifacts", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-candidate-"));
    tempDirs.push(dir);
    await writeFile(
  ...
  it("normalizes artifact package source SHAs before workflow output", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-candidate-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("normalizes whitespace-only artifact package source SHAs to absent", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-candidate-empty-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("rejects malformed artifact package source SHAs", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-candidate-bad-sha-"));
    tempDirs.push(dir);
    await writeFile(
      path.join(dir, "package-candidate.json"),
      JSON.stringify({
  ...
  it("accepts uppercase package artifact SHA-256 metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-sha-"));
    tempDirs.push(dir);
    const file = path.join(dir, "operator.tgz");
    await writeFile(file, "operator package bytes");
  ...
  it("rejects source artifact scans that exceed the filesystem entry limit", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-artifact-scan-"));
    tempDirs.push(dir);
    const maxEntries = 3;
  ...
  it("rejects source artifact directories with multiple tarballs", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-artifact-duplicates-"));
    tempDirs.push(dir);

    await writeFile(path.join(dir, "operator-a.tgz"), "a");
    ...
    expect(message).toContain("operator-a.tgz");
    expect(message).not.toContain(path.join(dir, "nested.tar.gz"));
    expect(message).not.toContain(path.join(dir, "operator-a.tgz"));
  });

  it("reads the source SHA from packed npm build metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "operator-package-build-info-"));
    tempDirs.push(dir);
    const root = path.join(dir, "package");
    await mkdir(path.join(root, "dist"), { recursive: true });
    await writeFile(path.join(root, "package.json"), JSON.stringify({ name: "operator" }));
    await writeFile(
      path.join(root, "dist", "build-info.json"),
      JSON.stringify({ commit: "66CE632B9B7C5C7FDD3E66C739687D51638AD6E2" }),
    );
    const tarball = path.join(dir, "operator.tgz");
