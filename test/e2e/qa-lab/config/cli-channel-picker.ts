...
async function runRealPicker(options: ProducerOptions, operatorHome: string) {
  ...
  const child = spawn(
    ...
    [
      ...
      path.join(operatorHome, "picker.raw.log"),
      process.execPath,
      "operator.mjs",
      "configure",
      "--section",
...
async function runCliChannelPickerProducer(options: ProducerOptions) {
  ...
  const operatorHome = path.join(workDir, "operator-home");

  try {
    await fs.rm(workDir, { force: true, recursive: true });
    await fs.mkdir(operatorHome, { recursive: true });
    buildCliStartup(options.repoRoot);
    const result = await runRealPicker(options, operatorHome);
    writer.appendLog(sanitizePickerTranscript(result.transcript));
    const configPath = path.join(operatorHome, ".operator", "operator.json");
