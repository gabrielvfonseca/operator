// Package Operator For Docker tests cover QA Lab package artifact evidence.
...
import {
  ...
  packOperatorPackageForDocker,
  parseArgs,
  prepareBundledAiRuntimePackage,
  runCommandForTest,
  writePackageInventoryForDocker,
} from "../../../../scripts/package-operator-for-docker.mjs";
...
describe("package-operator-for-docker", () => {
  ...
  it("loads from a trusted harness checkout without installed dependencies", async () => {
    const tempRoot = tempDirs.make("operator-package-harness-");
    const copiedFiles = [
    ...
    try {
      ...
      const result = await new Promise<{ status: number | null; stderr: string }>(
        (resolve, reject) => {
          const child = spawn(
            process.execPath,
            [path.join(tempRoot, "scripts/package-operator-for-docker.mjs"), "--invalid"],
  ...
  it("writes inventory for a frozen source checkout without the trusted helper", async () => {
    const sourceDir = tempDirs.make("operator-package-frozen-source-");
    fs.mkdirSync(path.join(sourceDir, "dist"), { recursive: true });
    fs.mkdirSync(path.join(sourceDir, "scripts"), { recursive: true });
    fs.writeFileSync(path.join(sourceDir, "package.json"), '{"name":"operator"}\n');
  ...
  it("uses build-all with declaration generation for package artifacts", async () => {
    ...
    const previousTimeout = process.env.OPERATOR_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
    const previousSkipDts = process.env.OPERATOR_RUN_NODE_SKIP_DTS_BUILD;
    const previousPackageExtensions = process.env.OPERATOR_EXTENSIONS;
    const previousDockerBuildExtensions = process.env.OPERATOR_DOCKER_BUILD_EXTENSIONS;
    const previousInternalPluginIds = process.env[DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV];
    process.env.OPERATOR_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = "1234";
    process.env.OPERATOR_RUN_NODE_SKIP_DTS_BUILD = "1";
    process.env.OPERATOR_EXTENSIONS = "clickclack";
    process.env.OPERATOR_DOCKER_BUILD_EXTENSIONS = "slack";
    ...
    try {
      await buildPackageArtifacts("/repo", {
        runImpl: async (
          ...
          calls.push({
            ...
            args,
            cwd,
            noPnpm: options.env?.OPERATOR_BUILD_ALL_NO_PNPM,
            packageExtensions: options.env?.OPERATOR_EXTENSIONS,
            dockerBuildExtensions: options.env?.OPERATOR_DOCKER_BUILD_EXTENSIONS,
            internalDockerBuildPluginIds: options.env?.[DOCKER_SELECTED_PLUGIN_BUILD_IDS_ENV],
            skipDts: options.env?.OPERATOR_RUN_NODE_SKIP_DTS_BUILD,
      ...
      if (previousTimeout === undefined) {
        delete process.env.OPERATOR_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
      } else {
        process.env.OPERATOR_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = previousTimeout;
      }
      if (previousSkipDts === undefined) {
        delete process.env.OPERATOR_RUN_NODE_SKIP_DTS_BUILD;
      } else {
        process.env.OPERATOR_RUN_NODE_SKIP_DTS_BUILD = previousSkipDts;
      }
      for (const [envName, previousValue] of [
        ["OPERATOR_EXTENSIONS", previousPackageExtensions],
        ["OPERATOR_DOCKER_BUILD_EXTENSIONS", previousDockerBuildExtensions],
  ...
  it("rejects loose package artifact timeout env values", async () => {
    ...
    try {
      for (const value of ["1e3", "123.9", "9007199254740993", "0"]) {
        process.env.OPERATOR_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = value;
        ...
        await expect(
          ...
          "OPERATOR_DOCKER_PACKAGE_BUILD_TIMEOUT_MS must be a positive timeout in milliseconds",
        );
      }
    } finally {
      if (previousTimeout === undefined) {
  ...
  it("bundles and restores the separately packed AI runtime", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-docker-ai-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-docker-ai-output-"));
    ...
    const installedAiPath = path.join(sourceDir, "node_modules", "@operator", "ai");
    ...
    try {
      const cleanup = await prepareBundledAiRuntimePackage(
        ...
        async (command: string, args: string[], cwd: string) => {
          ...
          fs.writeFileSync(path.join(outputDir, "operator-ai-2026.6.17.tgz"), "ai package");
          return "";
        },
        {
      ...
      expect(fs.existsSync(path.join(outputDir, "operator-ai-2026.6.17.tgz"))).toBe(false);
    } finally {
  ...
  it("leaves pre-AI-workspace package sources unchanged", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-docker-legacy-source-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-docker-legacy-output-"));
    const packageJsonPath = path.join(sourceDir, "package.json");
    const originalPackageJson = `${JSON.stringify({
      dependencies: { "dep-a": "1.2.3" },
      name: "operator",
  ...
  it("rejects incomplete AI workspace package sources", async () => {
    ...
    for (const testCase of cases) {
      const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-docker-invalid-source-"));
      const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-docker-invalid-output-"));
      fs.writeFileSync(
        path.join(sourceDir, "package.json"),
        `${JSON.stringify({ dependencies: testCase.dependencies, name: "operator" })}\n`,
      );
  ...
  it("trims and restores the changelog around ignore-scripts package artifacts", async () => {
    const calls: string[] = [];
    const tarball = await packOperatorPackageForDocker("/repo", "/out", {
      ...
      runCaptureImpl: async (
        ...
        return "operator-2026.5.28.tgz\n";
      },
    });

    expect(tarball).toBe(path.join("/out", "operator-2026.5.28.tgz"));
    expect(calls).toEqual([
      "prepare:/repo",
  ...
  it("packages Unreleased notes for explicitly non-publish stable artifacts", async () => {
    const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-unreleased-package-"));
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-unreleased-output-"));
    ...
    fs.writeFileSync(
      path.join(sourceDir, "package.json"),
      '{"name":"operator","version":"2026.5.29"}\n',
    );
    ...
    try {
      const tarball = await packOperatorPackageForDocker(sourceDir, outputDir, {
        ...
        runCaptureImpl: async () => {
          ...
          const packedPath = path.join(outputDir, "operator-2026.5.29.tgz");
          fs.writeFileSync(packedPath, "package");
          return "operator-2026.5.29.tgz\n";
        },
      });

      expect(tarball).toBe(path.join(outputDir, "operator-2026.5.29.tgz"));
  ...
  it("uses pnpm pack when requested", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-pnpm-pack-"));
    const calls: string[] = [];
    const packedPath = path.join(outputDir, "operator-2026.5.28.tgz");

    try {
      const tarball = await packOperatorPackageForDocker("/repo", outputDir, {
        pnpmPack: true,
        prepareBundledAiRuntime: skipBundledAiRuntime,
  ...
  it("writes npm pack metadata for renamed package artifacts", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-docker-pack-json-"));
    ...
    try {
      const tarball = await packOperatorPackageForDocker("/repo", outputDir, {
        ...
        runCaptureImpl: async (
          ...
          expect(options.deferForwardedSignalExit).toBe(true);
          fs.writeFileSync(path.join(outputDir, "operator-2026.5.28.tgz"), "package");
          return JSON.stringify([
            {
              entryCount: 1,
              filename: "operator-2026.5.28.tgz",
              size: 7,
              unpackedSize: 7,
              version: "2026.5.28",
            },
      ...
      expect(tarball).toBe(path.join(outputDir, "operator-current.tgz"));
      expect(JSON.parse(fs.readFileSync(packJsonPath, "utf8"))).toEqual([
        {
          entryCount: 1,
          filename: "operator-current.tgz",
          size: 7,
          unpackedSize: 7,
          version: "2026.5.28",
        },
  ...
  it("rejects path-like npm pack stdout before resolving Docker package tarballs", async () => {
    for (const filename of [
      ...
      String.raw`C:\temp\operator-2026.6.17.tgz`,
      "operator-nested/evil.tgz",
      String.raw`operator-nested\evil.tgz`,
      "operator-C:evil.tgz",
    ]) {
      await expect(
        packOperatorPackageForDocker("/repo", "/out", {
          prepareBundledAiRuntime: skipBundledAiRuntime,
          prepareChangelog: async () => {},
          restoreChangelog: async () => {},
  ...
  it("ignores unsafe output directory tarball names when npm stdout is not usable", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-docker-pack-"));
    try {
      fs.writeFileSync(path.join(outputDir, "operator-C:evil.tgz"), "");
      fs.writeFileSync(path.join(outputDir, String.raw`operator-nested\evil.tgz`), "");
      await expect(
        packOperatorPackageForDocker("/repo", outputDir, {
          prepareBundledAiRuntime: skipBundledAiRuntime,
          prepareChangelog: async () => {},
          restoreChangelog: async () => {},
          runCaptureImpl: async () => "npm notice\n",
        }),
      ).rejects.toThrow("missing packed Operator tarball");
      ...
      await expect(
        packOperatorPackageForDocker("/repo", outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "operator-2026.6.17.tgz"), "");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "operator-2026.6.17.tgz"));
    } finally {
      fs.rmSync(outputDir, { recursive: true, force: true });
    }
  });
  ...
  it("ignores stale package tarballs before fallback scanning npm output", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-docker-pack-stale-"));
    try {
      fs.writeFileSync(path.join(outputDir, "operator-9999.1.1.tgz"), "stale");
      ...
      await expect(
        packOperatorPackageForDocker("/repo", outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "operator-2026.6.17.tgz"), "current");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "operator-2026.6.17.tgz"));

      expect(fs.existsSync(path.join(outputDir, "operator-9999.1.1.tgz"))).toBe(false);
      expect(fs.readFileSync(path.join(outputDir, "operator-2026.6.17.tgz"), "utf8")).toBe(
        "current",
      );
    } finally {
      fs.rmSync(outputDir, { recursive: true, force: true });
  ...
  it("kills timed-out child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-package-timeout-"));
  ...
  it("clamps oversized kill grace before scheduling", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-package-grace-"));
  ...
  it("keeps fallback SIGKILL armed for descendants after the direct child exits", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-package-descendant-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid;
    try {
      const childScript = ["process.on('SIGTERM', () => {});", "setInterval(() => {}, 1000);"].join(
  ...
  it("forwards external termination to active child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-package-signal-"));
    const childPidPath = path.join(tempDir, "child.pid");
    const scriptUrl = pathToFileURL(path.resolve("scripts/package-operator-for-docker.mjs")).href;
    let childPid = 0;
    let runnerPid;
    try {
