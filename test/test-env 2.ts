...
const LIVE_TEST_TRIGGER_ENV_KEYS = ["LIVE", "OPERATOR_LIVE_TEST", "OPERATOR_LIVE_GATEWAY"] as const;
const HERMETIC_TEST_ENV_KEYS = [
  ...LIVE_TEST_TRIGGER_ENV_KEYS,
  "OPERATOR_LIVE_USE_REAL_HOME",
  "OPERATOR_BUNDLED_PLUGINS_DIR",
  "OPERATOR_TEST_TRUST_BUNDLED_PLUGINS_DIR",
...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.OPERATOR_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    ...
    { key: "OPERATOR_TEST_FAST", value: process.env.OPERATOR_TEST_FAST },
    {
      key: "OPERATOR_STRICT_FAST_REPLY_CONFIG",
      value: process.env.OPERATOR_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "OPERATOR_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.OPERATOR_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "OPERATOR_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.OPERATOR_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "OPERATOR_STATE_DIR", value: process.env.OPERATOR_STATE_DIR },
    { key: "OPERATOR_CONFIG_PATH", value: process.env.OPERATOR_CONFIG_PATH },
    { key: "OPERATOR_GATEWAY_PORT", value: process.env.OPERATOR_GATEWAY_PORT },
    { key: "OPERATOR_BRIDGE_ENABLED", value: process.env.OPERATOR_BRIDGE_ENABLED },
    { key: "OPERATOR_BRIDGE_HOST", value: process.env.OPERATOR_BRIDGE_HOST },
    { key: "OPERATOR_BRIDGE_PORT", value: process.env.OPERATOR_BRIDGE_PORT },
    { key: "OPERATOR_CANVAS_HOST_PORT", value: process.env.OPERATOR_CANVAS_HOST_PORT },
    { key: "OPERATOR_TEST_HOME", value: process.env.OPERATOR_TEST_HOME },
    { key: "OPERATOR_AGENT_DIR", value: process.env.OPERATOR_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "operator-test-home-"));

  setTestEnvValue("HOME", tempHome);
  setTestEnvValue("USERPROFILE", tempHome);
  setTestEnvValue("OPERATOR_TEST_HOME", tempHome);
  setTestEnvValue("OPERATOR_TEST_FAST", "1");
  setTestEnvValue("OPERATOR_STRICT_FAST_REPLY_CONFIG", "1");
  deleteTestEnvValue("OPERATOR_ALLOW_SLOW_REPLY_TESTS");
  ...
  deleteTestEnvValue("OPERATOR_AGENT_DIR");
  ...
  deleteTestEnvValue("OPERATOR_BRIDGE_ENABLED");
  deleteTestEnvValue("OPERATOR_BRIDGE_HOST");
  deleteTestEnvValue("OPERATOR_BRIDGE_PORT");
  deleteTestEnvValue("OPERATOR_CANVAS_HOST_PORT");
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.OPERATOR_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.OPERATOR_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".operator");
  const priorIsolatedHome = params.env.OPERATOR_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".operator")
  ) {
    realStateDir = path.join(params.realHome, ".operator");
  }
  const tempStateDir = path.join(params.tempHome, ".operator");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.OPERATOR_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.OPERATOR_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "operator.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "operator.json"),
...
export function installTestEnv(options?: InstallTestEnvOptions): {
  ...
  const allowRealHome = !hermetic && isTruthyEnvValue(process.env.OPERATOR_LIVE_USE_REAL_HOME);
  ...
  if (hermetic) {
    ...
    setTestEnvValue("OPERATOR_TEST_TRUST_BUNDLED_PLUGINS_DIR", "1");
    setTestEnvValue(
      "OPERATOR_BUNDLED_PLUGINS_DIR",
