...
import { createOperatorTestInstance, type OperatorTestInstance } from "./operator-test-instance.js";

export type GatewayInstance = OperatorTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createOperatorTestInstance({ name });
  try {
