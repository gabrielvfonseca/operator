...
describe("collectSourcePackWorkspaceDependencyErrors", () => {
  it("rejects the plain source pack that pnpm rewrites without bundling @operator/ai", () => {
    ...
    writeFileSync(
      ...
      `${JSON.stringify({ name: "@operator/ai", version }, null, 2)}\n`,
    );
    ...
    expect(packedPackageJson.dependencies?.["@operator/ai"]).toBe(version);
    ...
    expect(collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {})).toEqual([
      "plain root packing cannot safely resolve @operator/ai from workspace:*: pnpm rewrites the workspace dependency to an exact version without bundling the package",
      "use `node scripts/package-operator-for-docker.mjs --allow-unreleased-changelog` for a self-contained source package; official npm release automation prepares and publishes @operator/ai separately",
    ]);
    expect(
