  ...
  it("validates only selected package names when filters are provided", () => {
    ...
    expect(
      collectClawHubPublishablePluginPackages(repoDir, {
        packageNames: ["@operator/demo-plugin"],
      }).map((plugin) => plugin.packageName),
    ).toEqual(["@operator/demo-plugin"]);
  });
  ...
  it("collects exact release dependencies that must match npm latest", () => {
    ...
    expect(collectClawHubPublishablePluginPackages(repoDir)).toEqual([
      expect.objectContaining({
        packageName: "@operator/demo-plugin",
...
describe("Operator dual-published plugin metadata", () => {
  const dualPublishedPlugins = [
    {
      extensionId: "cohere",
      packageName: "@operator/cohere-provider",
      install: {
        clawhubSpec: "clawhub:@operator/cohere-provider",
        defaultChoice: "npm",
        minHostVersion: ">=2026.6.8",
        npmSpec: "@operator/cohere-provider",
      },
    },
    {
    ...
    },
    {
      ...
      install: {
        clawhubSpec: "clawhub:@operator/diagnostics-prometheus",
        defaultChoice: "npm",
        minHostVersion: ">=2026.4.25",
        npmSpec: "@operator/diagnostics-prometheus",
      },
    },
    {
      extensionId: "gmi",
      packageName: "@operator/gmi-provider",
      install: {
        clawhubSpec: "clawhub:@operator/gmi-provider",
        defaultChoice: "npm",
        minHostVersion: ">=2026.6.8",
...
describe("collectClawHubVersionGateErrors", () => {
  it("requires a version bump when a publishable plugin changes", () => {
    ...
    expect(errors).toEqual([
      "@operator/demo-plugin@2026.4.1: changed publishable plugin still has the same version in package.json.",
    ]);
  });
...
describe("collectPluginClawHubReleasePlan", () => {
  it("rejects stale required dependencies before querying ClawHub", async () => {
    ...
    await expect(
      collectPluginClawHubReleasePlan({
        rootDir: repoDir,
        selection: ["@operator/demo-plugin"],
      ...
      '@operator/demo-plugin@2026.4.1: demo-runtime must match npm latest for release; found "1.2.3", latest is "1.2.4".',
    );
  });
  ...
  it("accepts required dependencies matching npm latest", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@operator/demo-plugin@2026.4.1": 404,
      },
    });
    ...
    expect(plan.candidates.map((plugin) => plugin.packageName)).toEqual(["@operator/demo-plugin"]);
  });
  ...
  it("fails closed when npm latest cannot be resolved", async () => {
    ...
    await expect(
      ...
      "@operator/demo-plugin@2026.4.1: could not resolve npm latest for demo-runtime: registry unavailable",
    );
  });
  ...
  it("preserves ClawHub response details after package retries are exhausted", async () => {
    ...
    await expect(
      ...
      "Failed to query ClawHub package @operator/demo-plugin: 503 Rate limit temporarily unavailable [retry-after=1; x-request-id=request-123]",
    );
  ...
  it("skips versions that already exist on ClawHub", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@operator/demo-plugin@2026.4.1": 200,
      },
    });
...
describe("buildOperatorReleaseClawHubPlan", () => {
  it("emits a dispatch plan that keeps ClawHub children on the release tag", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@operator/demo-plugin@2026.4.1": 404,
        "@operator/demo-three@2026.4.1": 404,
      },
    });
    ...
    expect(plan.normal).toEqual({
      ...
      shouldDispatch: true,
      packages: ["@operator/demo-plugin"],
      inputs: {
        publish_scope: "selected",
        plugins: "@operator/demo-plugin",
    ...
    expect(plan.bootstrap).toEqual({
      ...
      shouldDispatch: true,
      packages: ["@operator/demo-two", "@operator/demo-three"],
      inputs: {
        bootstrap_workflow_sha: "d".repeat(40),
        ref: "a".repeat(40),
        release_tag: "v2026.4.1-beta.1",
        plugins: "@operator/demo-two,@operator/demo-three",
    ...
    expect(plan.summary).toEqual({
      ...
      normalPlugins: "@operator/demo-plugin",
      bootstrapPlugins: "@operator/demo-two,@operator/demo-three",
      missingTrustedPlugins: "@operator/demo-three",
    });
  ...
  it("routes already-published packages missing trusted publisher config to bootstrap repair", async () => {
    ...
    expect(plan.summary).toMatchObject({
      ...
      bootstrapCount: 1,
      missingTrustedPublisherCount: 1,
      bootstrapPlugins: "@operator/demo-plugin",
      missingTrustedPlugins: "@operator/demo-plugin",
    });
  });
...
describe("runPluginClawHubReleaseCheck", () => {
  it("rejects stale required dependencies", async () => {
    ...
    await expect(
      runPluginClawHubReleaseCheck(["--plugins", "@operator/demo-plugin"], {
        rootDir: repoDir,
        resolveLatestVersion: () => "1.2.4",
      }),
    ).rejects.toThrow(
...
describe("buildOperatorReleaseClawHubRuntimeState", () => {
  it("includes the normal ClawHub run in verifier args when the release waits for it", () => {
    ...
    expect(state.proofLines.normal).toBe(
      "- plugin ClawHub publish: https://github.com/operator/operator/actions/runs/111",
    );
  ...
  it("includes a completed bootstrap run even when there is no normal ClawHub run", () => {
    ...
    expect(state.proofLines.bootstrap).toBe(
      "- plugin ClawHub bootstrap: https://github.com/operator/operator/actions/runs/222",
    );
  });
  ...
  it("skips ClawHub verification for non-awaited incomplete runs while keeping proof links", () => {
    ...
    expect(state.proofLines.normal).toBe(
      "- plugin ClawHub publish: dispatched separately, not awaited by this proof: https://github.com/operator/operator/actions/runs/111",
    );
    expect(state.proofLines.bootstrap).toBe(
      "- plugin ClawHub bootstrap: dispatched separately, not awaited by this proof: https://github.com/operator/operator/actions/runs/222",
    );
  });
...
if [[ "\${1:-}" == "package" && "\${2:-}" == "pack" ]]; then
  ...
  printf 'fake tgz\\n' > "$pack_path"
  printf '{"path":"%s","name":"@operator/demo-plugin","version":"2026.4.1"}\\n' "$pack_path"
fi
exit 0
`,
    );
...
`,
  ...
  it("rejects duplicate normalized paths before invoking the ClawHub CLI", () => {
    ...
    const tgzBytes = createClawPackBytes("@operator/demo-plugin", "2026.4.1", {
      ...
      execFileSync(
        ...
        [join(process.cwd(), "scripts/plugin-clawhub-publish.sh"), "--validate-packed", tgzPath],
        {
          ...
          env: {
            ...
            EXPECTED_CLAWHUB_PACKAGE_NAME: "@operator/demo-plugin",
...
if [[ " $* " == *" --dry-run "* ]]; then
  printf '{"name":"@operator/demo-plugin","version":"2026.4.1"}\\n'
  exit 0
fi
attempts=0
...
function createTempPluginRepo(
  ...
  for (const currentExtensionId of extensionIds) {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          name: `@operator/${currentExtensionId}`,
          version: "2026.4.1",
          type: "module",
          repository: {
            type: "git",
          ...
          operator: {
            ...
            install: {
              npmSpec: `@operator/${currentExtensionId}`,
            },
            release: {
