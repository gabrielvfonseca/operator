...
export function resolveOperatorManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export function resolveOperatorManifestRequires(
  metadataObj: Record<string, unknown>,
): OperatorManifestRequires | undefined {
  const requiresRaw =
...
export function resolveOperatorManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveOperatorManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

type ParsedOperatorManifestInstallBase = {
...
export function parseOperatorManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedOperatorManifestInstallBase | undefined {
  ...
  const spec: ParsedOperatorManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyOperatorManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedOperatorManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
