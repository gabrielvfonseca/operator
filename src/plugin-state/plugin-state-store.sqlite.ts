...
type PluginStateEntriesTable = OperatorStateKyselyDatabase["plugin_state_entries"];
type PluginStateStoreDatabase = Pick<OperatorStateKyselyDatabase, "plugin_state_entries">;
...
function wrapPluginStateError(
  ...
  message: string,
  pathname = resolveOperatorStateSqlitePath(process.env),
): PluginStateStoreError {
...
function parseStoredJson(raw: string, operation: PluginStateStoreOperation): unknown {
  try {
    ...
    throw createPluginStateError({
      ...
      path: resolveOperatorStateSqlitePath(process.env),
      cause: error,
    });
  }
}
...
function envOptions(env?: NodeJS.ProcessEnv): OperatorStateDatabaseOptions {
  return env ? { env } : {};
}
...
function probePluginStateStore(): PluginStateStoreProbeResult {
  ...
  const stateWasOpen = isOperatorStateDatabaseOpen();
  ...
  try {
    ...
    openOperatorStateDatabase().walMaintenance.checkpoint();
    pushOk("checkpoint");
  } catch (error) {
