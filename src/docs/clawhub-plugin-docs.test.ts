...
describe("ClawHub plugin docs", () => {
  it("keeps the canonical plugin-publish snippets contract-valid", async () => {
    ...
    const pluginManifest = JSON.parse(
      await fs.readFile(
        path.join(DOCS_ROOT, "snippets", "plugin-publish", "minimal-operator.plugin.json"),
        "utf8",
      ),
  ...
  it("keeps the canonical package snippet embedded in the primary plugin docs", async () => {
    ...
    expect(extractNamedJsonBlock(sdkSetup, "operator-clawhub-package.json")).toEqual(snippet);
  });
});
