...
function listAgents(cfg: OperatorConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: OperatorConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<OperatorConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: OperatorConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: OperatorConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OperatorConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<OperatorConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  OperatorConfig,
  {
    bindingsRef: OperatorConfig["bindings"];
    agentsRef: OperatorConfig["agents"];
    sessionRef: OperatorConfig["session"];
...
function resolveRouteCacheForConfig(cfg: OperatorConfig): Map<string, ResolvedAgentRoute> {
