...
import type { DB as OperatorStateDatabase } from "../../state/operator-state-db.generated.js";
import {
...
type CuratorOptions = OperatorStateDatabaseOptions & {
  nowMs?: number;
};

function curatorDb(options: OperatorStateDatabaseOptions = {}) {
...
function registerSkillUsageTracking(options: OperatorStateDatabaseOptions = {}): () => void {
...
function writeSweepAttempt(nowMs: number, options: OperatorStateDatabaseOptions): void {
  runOperatorStateWriteTransaction(({ db }) => {
    const kysely = getNodeSqliteKysely<CuratorDatabase>(db);
    executeSqliteQuerySync(
      db,
...
async function runSkillCuratorSweep(
  ...
  try {
    ...
    const result = runOperatorStateWriteTransaction(({ db }) => {
      const kysely = getNodeSqliteKysely<CuratorDatabase>(db);
...
function updateLifecyclePin(skill: string, pinned: boolean, options: OperatorStateDatabaseOptions) {
  const skillKey = canonicalSkillKey(skill);
  const firstSkillFile = runOperatorStateWriteTransaction(({ db }) => {
    const kysely = getNodeSqliteKysely<CuratorDatabase>(db);
...
export function pinCuratedSkill(skill: string, options: OperatorStateDatabaseOptions = {}) {
  return updateLifecyclePin(skill, true, options);
}

export function unpinCuratedSkill(skill: string, options: OperatorStateDatabaseOptions = {}) {
