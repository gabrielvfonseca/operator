...
type OperatorSkillSecurityVerdictItem = Omit<
...
function projectClawHubVerdictItem(
  ...
  registry: string,
): OperatorSkillSecurityVerdictItem {
  const projected: OperatorSkillSecurityVerdictItem = {
    registry,
    ok: item.ok,
  ...
  if (item.error) {
    const error: OperatorSkillSecurityVerdictItem["error"] = {};
...
export async function fetchOperatorSkillSecurityVerdicts(
  targets: Array<{ registry: string; slug: string; version: string }>,
): Promise<OperatorSkillSecurityVerdictItem[]> {
  ...
  const items: OperatorSkillSecurityVerdictItem[] = [];
