...
import { AcpRuntimeError } from "@gabrielvfonseca/acp-core/runtime/errors";
...
export function isAcpEnabledByPolicy(cfg: OperatorConfig): boolean {
  return cfg.acp?.enabled !== false;
}

/** Resolves the effective dispatch policy state for inbound ACP routing. */
function resolveAcpDispatchPolicyState(cfg: OperatorConfig): AcpDispatchPolicyState {
...
export function resolveAcpDispatchPolicyMessage(cfg: OperatorConfig): string | null {
...
export function resolveAcpDispatchPolicyError(cfg: OperatorConfig): AcpRuntimeError | null {
...
export function resolveAcpExplicitTurnPolicyError(cfg: OperatorConfig): AcpRuntimeError | null {
  if (isAcpEnabledByPolicy(cfg)) {
    return null;
  }
...
function isAcpAgentAllowedByPolicy(cfg: OperatorConfig, agentId: string): boolean {
