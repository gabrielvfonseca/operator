...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-operator-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-operator-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-operator-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-operator-thread-id"));
