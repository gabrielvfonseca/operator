...
// Doctor/startup warning shape for shells where OPERATOR_GATEWAY_TOKEN would
...
export function resolveGatewayAuthTokenSourceConflict(params: {
  ...
  const envToken = normalizeOptionalString(params.env.OPERATOR_GATEWAY_TOKEN);
  if (!envToken) {
    return null;
  }

  if (params.env.OPERATOR_SERVICE_KIND?.trim() === GATEWAY_SERVICE_KIND) {
  ...
  const remediation =
    `Remove ${GATEWAY_ENV_TOKEN} from the shell, ~/.operator/.env, or launchctl env if gateway.auth.token is intended, ` +
