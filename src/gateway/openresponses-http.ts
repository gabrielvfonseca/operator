...
function createResponseSessionScope(params: {
  ...
  return normalizeResponseSessionScope({
    ...
    requestedSessionKey: getHeader(params.req, "x-operator-session-key"),
  });
}
  ...
  unsubscribe = onAgentEvent((evt) => {
    ...
    if (evt.stream === "lifecycle") {
      ...
      if (phase === "end" || phase === "error") {
        const finalText =
          accumulatedText || bufferedReplaceableAssistantContent || "No response from Operator.";
  ...
  void (async () => {
    try {
      ...
      if (!sawAssistantDelta) {
        const content =
          resultPayloadText || bufferedReplaceableAssistantContent || "No response from Operator.";
