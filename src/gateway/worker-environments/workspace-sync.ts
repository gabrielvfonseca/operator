...
// Relative to the $HOME/.operator-worker root owned by REMOTE_WORKSPACE_SETUP_SCRIPT;
...
> {
  ...
  const syncWorkspaceImpl = async (
    ...
    const temporaryDirectory = await fs.mkdtemp(
      path.join(os.tmpdir(), "operator-worker-workspace-sync-"),
    );
  ...
  const reconcileWorkspaceImpl = async (
    ...
    const prepared = requirePrepared();
    const temporaryDirectory = await fs.mkdtemp(
      path.join(os.tmpdir(), "operator-worker-workspace-reconcile-"),
    );
