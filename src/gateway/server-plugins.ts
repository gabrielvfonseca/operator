...
import { clearActivatedPluginRuntimeState, loadOperatorPlugins } from "../plugins/loader.js";
...
const PLUGIN_SUBAGENT_POLICY_STATE_KEY: unique symbol = Symbol.for(
  "operator.pluginSubagentOverridePolicyState",
);
...
export function setPluginSubagentOverridePolicies(cfg: OperatorConfig): void {
  ...
  if (!policy?.allowModelOverride) {
    return {
      ...
      reason:
        ...
        "See https://docs.operator.ai/plugins/sdk-runtime#api-runtime-subagent and search for: " +
...
export function loadGatewayPlugins(params: {
  ...
  const pluginRegistry = loadOperatorPlugins({
    config: resolvedConfig,
