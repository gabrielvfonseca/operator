...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.OPERATOR_CONFIG_PATH) {
    configPaths.add(process.env.OPERATOR_CONFIG_PATH);
  }
  if (process.env.OPERATOR_STATE_DIR) {
    configPaths.add(path.join(process.env.OPERATOR_STATE_DIR, "operator.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "operator-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.OPERATOR_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "operator-test-no-bundled-extensions")
    : "operator-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".operator-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".operator-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".operator-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>operator-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__operatorTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__operatorTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["OPERATOR_GATEWAY_TOKEN"]);
  const prev = process.env.OPERATOR_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPERATOR_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.OPERATOR_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
