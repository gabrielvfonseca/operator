...
function buildNodeApproveCommand(opts: DevicesRpcOpts, requestId: string): string {
  const args = ["@gabrielvfonseca/operator", "nodes", "approve", requestId];
...
function buildFallbackStateMismatchError(details: ConnectPairingRequiredDetails): Error {
  return new Error(
    [
      ...
      "The running gateway is probably using a different OPERATOR_PROFILE or OPERATOR_STATE_DIR than this CLI.",
...
function buildExplicitApproveCommand(opts: DevicesRpcOpts, requestId: string): string {
  const args = ["@gabrielvfonseca/operator", "devices", "approve", requestId];
...
export async function runDevicesApproveCommand(
  ...
  if (usingImplicitSelection) {
    ...
    switch (approval.kind) {
      ...
      case "re-approval":
        defaultRuntime.log(
          "  Note:   Already paired. Approval-bound device details changed, so Operator created a fresh request instead of silently reusing the old approval.",
        );
        break;
      case "new-pairing":
