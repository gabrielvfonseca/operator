...
export function mergeInstallInvocationEnv(params: {
  ...
  for (const [rawKey, rawValue] of Object.entries(params.existingServiceEnv)) {
    ...
    if (upper === OPERATOR_WRAPPER_ENV_KEY) {
      const value = rawValue.trim();
      if (value) {
        preservedServiceEnv[normalizeInstallEnvKey(OPERATOR_WRAPPER_ENV_KEY)] = value;
      }
      continue;
    }
    if (
    ...
    if (!opts.force) {
      ...
      if (autoRefreshMessage) {
        ...
        if (!json) {
          ...
          defaultRuntime.log(
            `Reinstall with: ${formatCliCommand("operator gateway install --force")}`,
          );
        }
        return;
      }
  ...
  try {
    ...
    if (currentEmbeddedToken) {
      ...
      const plannedEmbeddedToken = normalizeOptionalString(
        plannedInstall.environment.OPERATOR_GATEWAY_TOKEN,
      );
      if (currentEmbeddedToken !== plannedEmbeddedToken) {
        return "Gateway service OPERATOR_GATEWAY_TOKEN differs from the current install plan; refreshing the install.";
      }
    }
    const wrapperRequested = Boolean(
      params.wrapperPath || normalizeOptionalString(params.installEnv[OPERATOR_WRAPPER_ENV_KEY]),
    );
    if (wrapperRequested) {
      const plannedInstall = await buildGatewayInstallPlan({
        env: params.installEnv,
      ...
      const plannedWrapperPath = normalizeOptionalString(
        plannedInstall.environment[OPERATOR_WRAPPER_ENV_KEY],
      );
      const currentWrapperPath = normalizeOptionalString(
        currentCommand.environment?.[OPERATOR_WRAPPER_ENV_KEY],
      );
      if (plannedWrapperPath !== currentWrapperPath) {
        return `Gateway service ${OPERATOR_WRAPPER_ENV_KEY} differs from the current wrapper install plan; refreshing the install.`;
      }
    }
