    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["operator nodes status", "List known nodes with live status."],
          ["operator nodes pairing pending", "Show pending node pairing requests."],
          ["operator nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'operator nodes invoke --node <id> --command system.which --params \'{"bins":["uname"]}\'',
            "Invoke a node command directly.",
          ],
          ["operator nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.operator.ai/cli/nodes")}\n`,
    );
...
function shouldRegisterNodesPluginCommands(nodes: Command, argv: readonly string[]): boolean {
  ...
  if (!requestedSubcommand) {
    // Bare `operator nodes` listing should still surface plugin-provided subcommands.
    return true;
  }
