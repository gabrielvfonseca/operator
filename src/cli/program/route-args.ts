...
/** Parse `operator health` flags for the route-first status family. */
...
/** Parse `operator status` flags without registering the full command tree. */
...
/** Parse `operator gateway status` RPC-only flags accepted by the fast route. */
...
/** Parse `operator sessions` filters for JSON/list route execution. */
...
/** Parse `operator agents list` display switches for route-first execution. */
...
/** Parse `operator config get <path>` while preserving root option handling. */
...
/** Parse `operator config unset <path>` and its mutation guard flags. */
...
/** Parse `operator models list` filters for the lightweight model catalog route. */
...
/** Parse `operator models status` probe controls for the route-first status path. */
...
/** Parse `operator channels list` display flags for the route-first list path. */
...
/** Parse `operator channels status` probe flags without full CLI registration. */
...
/** Parse JSON-only `operator plugins list` flags for plugin inventory output. */
...
/** Parse both `operator tasks --json` and `operator tasks list --json` aliases. */
...
/** Parse JSON-only `operator tasks audit` filters for the route-first audit path. */
