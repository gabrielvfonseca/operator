...
describe("tryRouteCli", () => {
  ...
  it("keeps config guard for routed status --json commands", async () => {
    await expect(
      tryRouteCli(["node", "@gabrielvfonseca/operator", "status", "--json"]),
    ).resolves.toBe(true);
  ...
  it("keeps config guard for the parent tasks JSON list alias", async () => {
    await expect(
      tryRouteCli(["node", "@gabrielvfonseca/operator", "tasks", "--json"]),
    ).resolves.toBe(true);
  ...
  it("does not pass suppressDoctorStdout for routed non-json commands", async () => {
    await expect(tryRouteCli(["node", "@gabrielvfonseca/operator", "status"])).resolves.toBe(true);
  ...
  it("keeps logs routed to stderr for routed --json commands", async () => {
    ...
    await tryRouteCli(["node", "@gabrielvfonseca/operator", "agents", "--json"]);
  ...
  it("routes command-run logs to stderr for config-guard-skipping --json routes", async () => {
    ...
    await expect(
      tryRouteCli(["node", "@gabrielvfonseca/operator", "models", "status", "--json"]),
    ).resolves.toBe(true);
  ...
  it("does not route logs to stderr during plugin loading without --json", async () => {
    ...
    await tryRouteCli(["node", "@gabrielvfonseca/operator", "agents"]);
  ...
  it("routes status when root options precede the command", async () => {
    ...
    await expect(
      tryRouteCli(["node", "@gabrielvfonseca/operator", "--log-level", "debug", "status"]),
    ).resolves.toBe(true);

    expect(findRoutedCommandMock).toHaveBeenCalledWith(
      ["status"],
      ["node", "@gabrielvfonseca/operator", "--log-level", "debug", "status"],
    );
    expect(ensureConfigReadyMock).toHaveBeenCalledTimes(1);
    const configReadyCall = firstConfigReadyCall();
    expect(typeof configReadyCall?.runtime).toBe("object");
  ...
  it("applies routed log level options after the command", async () => {
    ...
    await expect(
      tryRouteCli(["node", "@gabrielvfonseca/operator", "status", "--log-level=trace"]),
    ).resolves.toBe(true);
  ...
  it.each([
    ["invalid value", ["node", "@gabrielvfonseca/operator", "status", "--log-level", "verbose"]],
    ["missing value", ["node", "@gabrielvfonseca/operator", "status", "--log-level"]],
    [
      "later invalid value",
      [
        "node",
  ...
  it("falls back before bootstrap when the route cannot parse the argv", async () => {
    ...
    await expect(tryRouteCli(["node", "@gabrielvfonseca/operator", "tasks", "list"])).resolves.toBe(
      false,
    );
