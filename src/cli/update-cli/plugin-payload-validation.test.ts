...
describe("runPluginPayloadSmokeCheck", () => {
  let tmpRoot: string;
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "operator-payload-smoke-"));
  });
  afterEach(async () => {
  ...
  async function linkOperatorPeerToHost(dir: string): Promise<void> {
    ...
    await fs.symlink(
      resolveTestHostRoot(),
      path.join(dir, "node_modules", "@gabrielvfonseca/operator"),
      "junction",
    );
  }
  ...
  it("reports ok for a record whose package.json + main file exist", async () => {
    ...
    await writePackage(
      dir,
      { name: "@gabrielvfonseca/discord", main: "dist/index.js" },
      "module.exports = {};",
    );
  ...
  it("reports a failure when the main entry file is missing on disk", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, { name: "@gabrielvfonseca/brave", main: "dist/index.js" });
    const result = await runPluginPayloadSmokeCheck({
      records: { brave: { source: "npm", installPath: dir } },
      env: {},
    });
  ...
  it("accepts a manifest with no main field (Operator plugins commonly use `exports` or `operator.extensions`)", async () => {
    const dir = path.join(tmpRoot, "matrix");
    await writePackage(dir, { name: "@gabrielvfonseca/plugin-matrix" });
    const result = await runPluginPayloadSmokeCheck({
  ...
  it("accepts a manifest that declares only `exports` and no `main`", async () => {
    ...
    await writePackage(dir, {
      name: "@gabrielvfonseca/qa-channel",
  ...
  it("accepts a manifest that declares an existing `operator.extensions` entry and no `main`", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
  ...
  it("reports a failure when `operator.extensions` contains invalid entries", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@gabrielvfonseca/brave-plugin",
      operator: { extensions: ["./index.js", " "] },
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        reason: "missing-extension-entry",
        detail:
          "Plugin extension entry validation failed: package.json operator.extensions[1] must be a non-empty string",
      },
    ]);
  });
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        reason: "missing-extension-entry",
        detail:
          "Plugin extension entry validation failed: package.json operator.extensions is empty",
      },
    ]);
  });
  ...
  it("reports a failure when an openclaw peer link is missing", async () => {
    ...
    await writePackage(
      dir,
      {
        name: "@gabrielvfonseca/codex",
        main: "dist/index.js",
        peerDependencies: { operator: ">=2026.5.18-beta.1" },
      },
      "export default {};\n",
    );
    ...
    expect(result.failures).toStrictEqual([
      {
        ...
        detail: `Plugin declares peerDependency "@gabrielvfonseca/operator" but peer link audit failed: missing ${path.join(
          dir,
          "node_modules",
  ...
  it("reports a failure when an openclaw peer link is a stale real directory", async () => {
    ...
    const stalePeerDir = path.join(dir, "node_modules", "@gabrielvfonseca/operator");
  ...
  it("reports a failure when an `operator.extensions` entry file is missing", async () => {
    const dir = path.join(tmpRoot, "brave");
    await writePackage(dir, {
      name: "@gabrielvfonseca/brave-plugin",
