...
const STEP_LABELS: Record<string, string> = {
  ...
  "operator doctor entry": "Checking doctor entrypoint",
  "operator doctor": "Running doctor checks",
...
function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g operator@latest");
    hints.push(
      "System install outline: operator gateway stop -> sudo <system-npm> i -g operator@latest -> operator gateway install --force -> operator gateway restart.",
    );
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g operator@latest --omit=optional");
  }
