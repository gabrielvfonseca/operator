...
export function rejectAgentScopedModelWrite(
  ...
  throw new Error(
    `operator models ${commandName} does not support --agent; it only updates global model defaults. Remove --agent, or run ${formatCliCommand("operator agents list")} and set the per-agent model in agent config.`,
  );
}
