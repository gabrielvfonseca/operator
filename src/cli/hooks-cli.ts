...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  if (hooks.length === 0) {
    const message = opts.eligible
      ? `No eligible hooks found. Run \`${formatCliCommand("operator hooks list")}\` to see all hooks.`
      : "No hooks found.";
    return message;
  }
...
export function formatHookInfo(
  ...
  if (!hook) {
    ...
    return `Hook "${hookName}" not found. Run \`${formatCliCommand("operator hooks list")}\` to see available hooks.`;
  }
...
export function registerHooksCli(program: Command): void {
  const hooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/hooks", "docs.operator.ai/cli/hooks")}\n`,
    );
  ...
  hooks
    .command("install")
    .description("Deprecated: install a hook pack via `operator plugins install`")
    ...
    .action(async (raw: string, opts: { force?: boolean; link?: boolean; pin?: boolean }) => {
      defaultRuntime.log(
        theme.warn("`operator hooks install` is deprecated; use `operator plugins install`."),
      );
  ...
  hooks
    .command("update")
    .description("Deprecated: update hook packs via `operator plugins update`")
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      defaultRuntime.log(
        theme.warn("`operator hooks update` is deprecated; use `operator plugins update`."),
      );
