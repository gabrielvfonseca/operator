...
export type StaleOperatorUpdateLaunchdJob = {
  label: string;
  pid?: number;
  lastExitStatus?: number;
};

type OperatorUpdateLaunchdLabelCandidate = {
  label: string;
  requiresMetadata: boolean;
};

function normalizeOperatorUpdateLaunchdLabel(label: unknown): string | null {
...
function normalizeOperatorUpdateLaunchdLabelCandidate(
  label: unknown,
): OperatorUpdateLaunchdLabelCandidate | null {
...
function resolveCurrentOperatorUpdateLaunchdJobLabel(
...
export function parseLaunchctlListOperatorUpdateJobs(
  output: string,
): StaleOperatorUpdateLaunchdJob[] {
  return parseLaunchctlListOperatorUpdateJobCandidates(output)
    .filter((job) => !job.requiresMetadata)
    .map(({ requiresMetadata: _requiresMetadata, ...job }) => job);
}

function parseLaunchctlListOperatorUpdateJobCandidates(
  output: string,
): Array<StaleOperatorUpdateLaunchdJob & OperatorUpdateLaunchdLabelCandidate> {
  const jobs: Array<StaleOperatorUpdateLaunchdJob & OperatorUpdateLaunchdLabelCandidate> = [];
...
function hasOperatorUpdateLaunchdMarker(env: Record<string, string | undefined> | undefined) {
  return env?.OPERATOR_UPDATE_RUN_HANDOFF?.trim() === "1";
}

function isOperatorUpdateCommandPrefix(programArguments: string[], updateIndex: number): boolean {
  if (updateIndex === 1) {
...
function isOperatorUpdateProgramArguments(programArguments: string[] | undefined): boolean {
  ...
  return (
    isOperatorUpdateCommandPrefix(programArguments, updateIndex) &&
...
async function isLaunchdJobConfirmedOperatorUpdater(params: {
  ...
  return (
    hasOperatorUpdateLaunchdMarker(command?.environment) ||
    isOperatorUpdateProgramArguments(command?.programArguments)
  );
}

export async function findStaleOperatorUpdateLaunchdJobs(
  env: NodeJS.ProcessEnv = process.env,
): Promise<StaleOperatorUpdateLaunchdJob[]> {
  ...
  const jobs: StaleOperatorUpdateLaunchdJob[] = [];
  for (const job of parseLaunchctlListOperatorUpdateJobCandidates(result.stdout)) {
    ...
    if (
      ...
      !(await isLaunchdJobConfirmedOperatorUpdater({ label: job.label, env }))
    ) {
      continue;
    }
    jobs.push({
...
async function disableOperatorUpdateLaunchdJobCandidate(params: {
  candidate: OperatorUpdateLaunchdLabelCandidate;
  env: NodeJS.ProcessEnv;
  ...
  if (
    ...
    !(
      (params.trustCurrentEnvMarker && hasOperatorUpdateLaunchdMarker(params.env)) ||
      (await isLaunchdJobConfirmedOperatorUpdater({
...
export async function disableOperatorUpdateLaunchdJob(
  label: string,
  env: NodeJS.ProcessEnv = process.env,
): Promise<boolean> {
  const candidate = normalizeOperatorUpdateLaunchdLabelCandidate(label);
...
export async function disableCurrentOperatorUpdateLaunchdJob(
  env: NodeJS.ProcessEnv = process.env,
): Promise<boolean> {
