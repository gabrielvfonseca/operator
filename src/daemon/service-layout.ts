...
async function resolveOperatorPackageRoot(entrypoint: string): Promise<string | undefined> {
...
export async function summarizeGatewayServiceLayout(
  ...
  const packageRoot = entrypointReal ? await resolveOperatorPackageRoot(entrypointReal) : undefined;
