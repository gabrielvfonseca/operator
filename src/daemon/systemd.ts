...
function sanitizeSystemdUnitBackupContent(params: {
  ...
  // Backups should not retain file-managed secrets that Operator moved into the
...
async function writeSystemdGatewayEnvironmentFile(params: {
  ...
  /** Operator-managed keys that must not be preserved from an old env file; stale file values
  ...
  fileManagedKeys?: ReadonlySet<string>;
  /** State-dir .env keys Operator previously managed but is now skipping (unresolved shell
  ...
  // Operator-managed keys (identified by inlineManagedKeys) are excluded: a stale
