...
const LIVE = isLiveTestEnabled(["OPERATOR_LIVE_CLI_MCP_GEMINI"]);
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an Operator-configured streamable-http server", async () => {
    const geminiCommand = process.env.OPERATOR_LIVE_GEMINI_COMMAND ?? "gemini";
