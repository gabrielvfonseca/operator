...
beforeEach(async () => {
  testState = await createOperatorTestState({
    layout: "state-only",
    prefix: "operator-workspace-state-",
  });
});
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OPERATOR_HOME for default workspace resolution", () => {
    ...
    expect(dir).toBe(path.join(path.resolve("/srv/operator-home"), ".operator", "workspace"));
  });

  it("prefers OPERATOR_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      OPERATOR_WORKSPACE_DIR: "/srv/operator-workspace",
      OPERATOR_HOME: "/srv/operator-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/operator-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = ["operator-workspace-state.json"] as const;
const LEGACY_WORKSPACE_STATE_PATH_SEGMENTS = [".operator", "workspace-state.json"] as const;
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-workspace-hardlink-"));
    try {
