...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", `operator.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  args.push("--label", `operator.createArgsEpoch=${SANDBOX_DOCKER_CREATE_ARGS_EPOCH}`);
  if (params.configHash) {
    args.push("--label", `operator.configHash=${params.configHash}`);
  }
