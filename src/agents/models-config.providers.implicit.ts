...
function resolveLiveProviderCatalogTimeoutMs(env: NodeJS.ProcessEnv): number | null {
  const live =
    env.OPERATOR_LIVE_TEST === "1" || env.OPERATOR_LIVE_GATEWAY === "1" || env.LIVE === "1";
  if (!live) {
    return null;
  }
...
function resolveProviderDiscoveryFilter(params: {
  ...
  const rawValues = [
    env.OPERATOR_LIVE_PROVIDERS?.trim(),
    env.OPERATOR_LIVE_GATEWAY_PROVIDERS?.trim(),
...
function buildPluginCatalogConfig(ctx: ImplicitProviderContext): OperatorConfig {
...
): Promise<NonNullable<OperatorConfig["models"]>["providers"]> {
