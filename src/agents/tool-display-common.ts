...
function parseToolSearchCall(code: string): { target: string; args?: string } | undefined {
  ...
  const prefixMatch = code.match(/operator\.tools\.call\s*\(\s*/s);
...
function normalizeToolSearchDisplayToolName(toolName: string | undefined): string | undefined {
  ...
  const catalogIdMatch = value.match(/^(?:operator|mcp|client):[^:]+:(.+)$/s);
...
function collectToolSearchDescribeBindings(code: string): Map<string, string> {
  ...
  const bindingPattern =
    /\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:await\s+)?operator\.tools\.describe\s*\(\s*("[^"]{1,240}"|'[^']{1,240}')\s*(?:,|\))/gs;
...
export function resolveToolSearchCodeDisplayTarget(
  ...
  const describeMatch = code.match(/operator\.tools\.describe\s*\(\s*([^)]+?)\s*(?:,|\))/s);
  if (describeMatch) {
  ...
  const searchMatch = code.match(/operator\.tools\.search\s*\(\s*([^)]+?)\s*(?:,|\))/s);
  if (searchMatch) {
