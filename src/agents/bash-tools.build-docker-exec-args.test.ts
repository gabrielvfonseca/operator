...
describe("buildDockerExecArgs", () => {
  ...
  it("does not interpolate PATH into the shell command", () => {
    const injectedPath = "$(touch /tmp/operator-path-injection)";
    const args = buildDockerExecArgs({
      containerName: "test-container",
      command: "echo hello",
      env: {
    ...
    const commandArg = args[args.length - 1];
    expect(args).toContain(`OPERATOR_PREPEND_PATH=${injectedPath}`);
    expect(commandArg).not.toContain(injectedPath);
    expect(commandArg).toContain("OPERATOR_PREPEND_PATH");
  });
