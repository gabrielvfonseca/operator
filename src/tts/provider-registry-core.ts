...
export type SpeechProviderRegistryResolver = {
  getProvider: (providerId: string, cfg?: OperatorConfig) => SpeechProviderPlugin | undefined;
  listProviders: (cfg?: OperatorConfig) => SpeechProviderPlugin[];
};
...
export function createSpeechProviderRegistry(resolver: SpeechProviderRegistryResolver) {
  const buildResolvedProviderMaps = (cfg?: OperatorConfig) =>
    buildCapabilityProviderMaps(resolver.listProviders(cfg));

  const listProviders = (cfg?: OperatorConfig): SpeechProviderPlugin[] => [
