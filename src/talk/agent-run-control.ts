/**
 * Runtime adapter for realtime voice control of active Operator agent runs.
 *
...
export async function controlRealtimeVoiceAgentRun(
  ...
  if (mode === "cancel") {
    ...
    const message = aborted
      ? "Cancelled the active Operator run."
      : "Operator could not cancel the active run.";
    return {
      ok: aborted,
      mode,
      sessionKey,
  ...
  return {
    ...
    message:
      mode === "followup"
        ? "Queued that follow-up for the active Operator run."
