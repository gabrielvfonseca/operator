...
type ThemeEnvOverrides = {
  OPERATOR_THEME?: string | undefined;
...
async function importThemeWithEnv(env: ThemeEnvOverrides) {
  if (Object.hasOwn(env, "OPERATOR_THEME")) {
    if (env.OPERATOR_THEME === undefined) {
      delete process.env.OPERATOR_THEME;
    } else {
      process.env.OPERATOR_THEME = env.OPERATOR_THEME;
    }
  }
...
describe("light background detection", () => {
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      OPERATOR_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when OPERATOR_THEME=light", async () => {
    const mod = await importThemeWithEnv({ OPERATOR_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when OPERATOR_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ OPERATOR_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats OPERATOR_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ OPERATOR_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("OPERATOR_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      OPERATOR_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ OPERATOR_THEME: "light" });
    const darkMod = await importThemeWithEnv({ OPERATOR_THEME: "dark" });
