...
const SUPPORT_EXPORT_PREFIX = "operator-diagnostics-";
...
function resolveBonjourEnvOverride(
  ...
  const raw = env.OPERATOR_DISABLE_BONJOUR?.trim().toLowerCase();
  if (!raw) {
    return "unset";
  }
  switch (raw) {
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$OPERATOR_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    ...
    "- `logs/operator-sanitized.jsonl`: sanitized log summaries and metadata",
