...
function isGatewayRemotelyExposed(cfg: OperatorConfig): boolean {
...
function listKnownNodeCommands(cfg: OperatorConfig): Set<string> {
  const baseCfg: OperatorConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function listOpenInboundPolicies(cfg: OperatorConfig): string[] {
  const out: string[] = [];
...
function listPotentialMultiUserSignals(cfg: OperatorConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function listAuditAgentToolContexts(cfg: OperatorConfig): AuditAgentToolContext[] {
...
function collectRiskyToolExposureContexts(cfg: OperatorConfig): {
...
function collectControlPlaneToolExposureContexts(cfg: OperatorConfig): string[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep OPERATOR_STATE_DIR on a local-only volume and re-run "${formatCliCommand("openclaw security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: OperatorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDockerNoopFindings(cfg: OperatorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: OperatorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: OperatorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: OperatorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: OperatorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: OperatorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: OperatorConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
