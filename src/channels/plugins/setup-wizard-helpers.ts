...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: OperatorConfig) => Array<string | number> | undefined;
}): OperatorConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export function createTopLevelChannelDmPolicy(params: {
  ...
  policyKey: string;
  allowFromKey: string;
  getCurrent: (cfg: OperatorConfig) => DmPolicy;
...
export function createTopLevelChannelDmPolicySetter(params: {
  channel: string;
  getAllowFrom?: (cfg: OperatorConfig) => Array<string | number> | undefined;
}): (cfg: OperatorConfig, dmPolicy: DmPolicy) => OperatorConfig {
  return (cfg, dmPolicy) =>
...
export function createTopLevelChannelAllowFromSetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: OperatorConfig, allowFrom: string[]) => OperatorConfig {
...
export function createTopLevelChannelGroupPolicySetter(params: {
  channel: string;
  enabled?: boolean;
}): (cfg: OperatorConfig, groupPolicy: "open" | "allowlist" | "disabled") => OperatorConfig {
  ...
  const promptToken = async (): Promise<string> =>
    (
      await params.prompter.text({
        ...
        // Credential input: masked in terminal prompts, and the Operator
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: OperatorConfig) => OperatorConfig | Promise<OperatorConfig>;
  applySet?: (
    cfg: OperatorConfig,
    value: SecretInput,
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const resolved = await promptSecretRefForSetup({
    ...
    copy: {
      ...
      envVarFormatError:
        'Use an env var name like "OPERATOR_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:
...
export async function promptParsedAllowFromForAccount<TConfig extends OperatorConfig>(params: {
  cfg: TConfig;
  accountId?: string;
  defaultAccountId: string;
...
export function createPromptParsedAllowFromForAccount<TConfig extends OperatorConfig>(params: {
...
export function createTopLevelChannelParsedAllowFromPrompt(params: {
  channel: string;
  defaultAccountId: string | ((cfg: OperatorConfig) => string);
  enabled?: boolean;
  noteTitle?: string;
  noteLines?: string[];
  message: string;
  placeholder: string;
  parseEntries: (raw: string) => ParsedAllowFromResult;
  getExistingAllowFrom?: (cfg: OperatorConfig) => Array<string | number>;
  mergeEntries?: (params: { existing: Array<string | number>; parsed: string[] }) => string[];
}): NonNullable<ChannelSetupDmPolicy["promptAllowFrom"]> {
  ...
  const sharedParams = {
    ...
    placeholder: params.placeholder,
    parseEntries: params.parseEntries,
    getExistingAllowFrom: ({ cfg }: { cfg: OperatorConfig }) =>
      params.getExistingAllowFrom?.(cfg) ??
      (cfg.channels?.[params.channel] as { allowFrom?: Array<string | number> } | undefined)
        ?.allowFrom ??
      [],
    ...(params.mergeEntries ? { mergeEntries: params.mergeEntries } : {}),
    applyAllowFrom: ({ cfg, allowFrom }: { cfg: OperatorConfig; allowFrom: string[] }) =>
...
export async function promptLegacyChannelAllowFromForAccount<TAccount>(params: {
  ...
  accountId?: string;
  defaultAccountId: string;
  resolveAccount: (cfg: OperatorConfig, accountId: string) => TAccount;
  resolveExisting: (account: TAccount, cfg: OperatorConfig) => Array<string | number>;
