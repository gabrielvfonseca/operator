...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): OperatorConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as OperatorConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: OperatorConfig) => string[];
...
function expectResolvedDefaultAccountCase(input: OperatorConfig, expected: string) {
