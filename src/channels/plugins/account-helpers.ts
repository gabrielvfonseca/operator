...
export function createAccountListHelpers(
  ...
  options?: {
    ...
    hasImplicitDefaultAccount?: (cfg: OperatorConfig) => boolean;
  },
) {
  function hasImplicitDefaultAccount(cfg: OperatorConfig): boolean {
  ...
  function resolveConfiguredDefaultAccountId(cfg: OperatorConfig): string | undefined {
    const channel = cfg.channels?.[channelKey] as Record<string, unknown> | undefined;
  ...
  function listAccountIds(cfg: OperatorConfig): string[] {
  ...
  function resolveDefaultAccountId(cfg: OperatorConfig): string {
