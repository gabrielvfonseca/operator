...
function resolveLegacyDebugProxyCapturePaths(
  ...
  return {
    sourcePath: env.OPERATOR_DEBUG_PROXY_DB_PATH?.trim() || path.join(rootDir, "capture.sqlite"),
    blobDir: env.OPERATOR_DEBUG_PROXY_BLOB_DIR?.trim() || path.join(rootDir, "blobs"),
  };
}
