...
import { isHttpUrl, isWebSocketUrl } from "@gabrielvfonseca/net-policy/url-protocol";
...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    NO_PROXY: process.env["NO_PROXY"],
    OPERATOR_PROXY_ACTIVE: process.env["OPERATOR_PROXY_ACTIVE"],
    OPERATOR_PROXY_LOOPBACK_MODE: process.env["OPERATOR_PROXY_LOOPBACK_MODE"],
    OPERATOR_PROXY_CA_FILE: process.env["OPERATOR_PROXY_CA_FILE"],
  };
}
...
function applyProxyEnv(
  ...
  process.env["OPERATOR_PROXY_LOOPBACK_MODE"] = loopbackMode;
  if (proxyCaFile) {
    process.env["OPERATOR_PROXY_CA_FILE"] = proxyCaFile;
  } else {
    delete process.env["OPERATOR_PROXY_CA_FILE"];
  }
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["OPERATOR_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or OPERATOR_PROXY_URL to an http:// or https:// forward proxy.",
    );
  }
...
export function ensureInheritedManagedProxyRoutingActive(): void {
  if (process.env["OPERATOR_PROXY_ACTIVE"] !== "1") {
    return;
  }
 ...
 * loopback CDP probe to a Chromium instance Operator spawned itself.
 *
