...
// update.
//
// `runGatewayUpdate` (git mode) runs `operator doctor --fix` with
// `OPERATOR_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE=1`, which makes the doctor
// pass DEFER configured-plugin repair to a later convergence step (see
// `shouldDeferConfiguredPluginInstallRepair`). The `operator update` CLI resumes
...
// binary's hidden `operator update finalize` entrypoint — the designed
...
function buildFinalizeEnv(
  ...
  if (compatHostVersion) {
    env.OPERATOR_COMPATIBILITY_HOST_VERSION = compatHostVersion;
  }
  if (sourceConfigPath) {
    env[POST_CORE_UPDATE_SOURCE_CONFIG_PATH_ENV] = sourceConfigPath;
  }
  if (serviceRepairPolicy) {
    env.OPERATOR_SERVICE_REPAIR_POLICY = serviceRepairPolicy;
  }
  return env;
}
...
// re-enters the full `operator update` CLI. Re-run convergence on no-op retries:
...
export async function runPostCoreFinalizeAfterGatewayUpdate(params: {
  ...
  try {
    ...
    if (params.preUpdateConfig) {
      sourceConfigDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-update-post-core-"));
  ...
  return {
    ...
    steps: [
      ...result.steps,
      {
        ...
        command: "operator update finalize",
