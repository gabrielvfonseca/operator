...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OperatorConfig | null = null;
   ...
   */
  get(key: string, cfg: OperatorConfig): T | undefined {
   ...
   */
  set(key: string, value: T, cfg: OperatorConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: OperatorConfig): void {
