...
describe("runMessageAction media behavior", () => {
  ...
  it("rejects plugin-declined attachment actions before loading media", async () => {
    ...
    await expect(
      runMessageAction({
        cfg: { channels: { textonly: { enabled: true } } } as OperatorConfig,
        action: "upload-file",
        params: {
          channel: "textonly",
          target: "room-1",
  ...
  it("does not stage buffer-only send attachments before target validation passes", async () => {
    ...
    await withTempOperatorStateDir(async (stateDir) => {
      await expect(
        runMessageAction({
          cfg: workspaceConfig,
          action: "send",
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Operator tmp root", async () => {
