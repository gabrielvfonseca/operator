...
export function triggerOperatorRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.OPERATOR_SYSTEMD_UNIT,
      process.env.OPERATOR_PROFILE,
    );
  ...
  const label =
    process.env.OPERATOR_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPERATOR_PROFILE);
