// Covers Operator's default fs-safe Python helper configuration.
...
vi.mock("@operator/fs-safe/config", () => ({
...
describe("fs-safe defaults", () => {
  afterEach(() => {
    ...
    delete process.env.OPERATOR_FS_SAFE_PYTHON_MODE;
  });

  it("disables the Python helper by default in Operator", async () => {
  ...
  it("honors the Operator-specific env mode override", async () => {
    process.env.OPERATOR_FS_SAFE_PYTHON_MODE = "auto";
