...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "operator.plugin.json");
...
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "operator.plugin.json");
...
const BASE_PROJECT_FILES = {
  [ROOT_TSCONFIG]: "{}\n",
  [ROOT_PACKAGE]: '{"name":"operator-test"}\n',
...
async function writeRuntimePostBuildScaffold(tmp: string): Promise<void> {
  ...
  await writeProjectFiles(tmp, {
    ...
    [DIST_OPERATOR_ALIAS_PACKAGE]:
      '{"name":"@gabrielvfonseca/operator","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js"}}\n',
  });
...
function statusCommandSpawn() {
  return [process.execPath, "operator.mjs", "status"];
}
...
describe("run-node script", () => {
    ...
    async () => {
      await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "operator.mjs", "--version"],
        ]);
      });
    },
  );
  ...
  it("skips DTS generation only for launcher-triggered local builds", async () => {
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      ...
      expect(spawnCalls[2]?.args).toEqual(["operator.mjs", "status"]);
    ...
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[], options?: unknown) => {
        ...
        return createPipedExitedProcess({
          stdout: args[0] === "operator.mjs" ? "child stdout\n" : "",
          stderr: args[0] === "operator.mjs" ? "child stderr\n" : "",
        });
      };
      const mutedStream = {
        write: () => true,
    ...
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[]) =>
        createPipedExitedProcess({
          stderr: args[0] === "operator.mjs" ? childStderr : "",
        });
      const stderrChunks: string[] = [];
      const stderr = {
        write: (chunk: string | Buffer) => {
  ...
  it("adds Node CPU profiling flags to the launched Operator child when requested", async () => {
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      ...
      expect(childArgs[2]).toMatch(
        /^--cpu-prof-name=operator-status-4242-\d{4}-\d{2}-\d{2}T.*\.cpuprofile$/,
      );
      expect(childArgs.slice(3)).toEqual(["operator.mjs", "status"]);
  ...
  it("rotates old Node CPU profiles when a retention cap is set", async () => {
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      ...
      const oldProfiles = [
        "operator-status-oldest.cpuprofile",
        "operator-status-middle.cpuprofile",
        "operator-status-newest.cpuprofile",
      ];
      ...
      fsSync.writeFileSync(path.join(profileDir, "operator-models-old.cpuprofile"), "{}");
      ...
      expect(fsSync.existsSync(path.join(profileDir, "operator-models-old.cpuprofile"))).toBe(true);
    });
  });
  ...
  it("adds Node sync I/O tracing flag to the launched Operator child when requested", async () => {
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      ...
      expect(exitCode).toBe(0);
      expect(spawnCalls.at(-1)).toEqual(["--trace-sync-io", "operator.mjs", "gateway", "--force"]);
    });
  });
  ...
  it("does not mutate Matrix QA args when no generic output log is requested", async () => {
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      ...
      expect(exitCode).toBe(0);
      const childArgs = spawnCalls.at(-1)?.args ?? [];
      expect(childArgs).toEqual(["operator.mjs", "qa", "matrix"]);
  ...
  it("reruns runtime postbuild for dirty extension package metadata in watch mode", async () => {
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [EXTENSION_PACKAGE]: '{"@gabrielvfonseca/operator":{"extensions":["./index.ts"]}}\n',
  ...
  it("forwards wrapper SIGTERM to the active openclaw child and returns 143", async () => {
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      ...
      expect(spawnCall?.[1]).toEqual(["operator.mjs", "status"]);
    ...
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]:
            '{"name":"demo","@gabrielvfonseca/operator":{"extensions":["./index.ts"]}}\n',
          [ROOT_TSDOWN]: "export default {};\n",
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_PACKAGE]:
            '{"name":"demo","@gabrielvfonseca/operator":{"extensions":["./stale.js"]}}\n',
        },
  ...
  it.each([
    ...
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      ...
      expect(exitCode).toBe(0);
      expect(spawnCalls).toEqual([[process.execPath, "operator.mjs", ...args]]);
      expect(runRuntimePostBuild).not.toHaveBeenCalled();
    });
  });
  ...
  it("rechecks a dirty dashboard client after waiting for an active build", async () => {
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      ...
      expect(spawnCalls).toEqual([
        [process.execPath, "operator.mjs", "dashboard", "--no-open", "--yes"],
      ]);
      expect(runRuntimePostBuild).not.toHaveBeenCalled();
    });
  });
    ...
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]:
            '{"@gabrielvfonseca/operator":{"extensions":["./src/index.ts","./src/extra.ts"]}}\n',
    ...
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [EXTENSION_SRC]: "export default {};\n",
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"@gabrielvfonseca/operator":{"extensions":["./src/index.ts"]}}\n',
        },
        buildPaths: [
          ROOT_SRC,
          EXTENSION_SRC,
    ...
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_PACKAGE]:
            '{"@gabrielvfonseca/operator":{"extensions":["./src/index.js"]}}\n',
    ...
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_EXTENSION_INDEX]: "export default {};\n",
          [DIST_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_EXTENSION_PACKAGE]: '{"@gabrielvfonseca/operator":{"extensions":["./index.js"]}}\n',
          [DIST_RUNTIME_EXTENSION_INDEX]: "export default {};\n",
          [DIST_RUNTIME_EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [DIST_RUNTIME_EXTENSION_PACKAGE]:
            '{"@gabrielvfonseca/operator":{"extensions":["./index.js"]}}\n',
          [RUNTIME_POSTBUILD_STAMP]: '{"head":"abc123"}\n',
        },
        buildPaths: [
          ROOT_SRC,
    ...
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          ...
          [DIST_OPERATOR_ALIAS_PACKAGE]:
            '{"name":"@gabrielvfonseca/operator","type":"module","exports":{"./plugin-sdk":"./plugin-sdk/index.js","./plugin-sdk/string-coerce-runtime":"./plugin-sdk/string-coerce-runtime.js"}}\n',
          [DIST_OPERATOR_ALIAS_PLUGIN_SDK_INDEX]:
            "export * from '../../../../plugin-sdk/index.js';\n",
    ...
    await withTempDir({ prefix: "operator-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
          [DIFFS_PACKAGE]:
            '{"@gabrielvfonseca/operator":{"build":{"staticAssets":[{"source":"./assets/viewer-runtime.js","output":"assets/viewer-runtime.js"}]}}}\n',
