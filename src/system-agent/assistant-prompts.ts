// System-agent prompts drive the Operator conversation with typed-command output.
...
export const SYSTEM_AGENT_ASSISTANT_SYSTEM_PROMPT = [
  ...
  "Inference is a prerequisite, not something you can bootstrap or replace from inside the session. Never change inference-provider credentials, top-level auth (`auth.*`), model catalogs, CLI backends, agent model routes, agent params/tools, or root `tools.*` with `config set` or `config set-ref`. Raw writes under `env.*`, `secrets.*`, `plugins.*`, and `$include` are also refused because they can replace credential resolution or provider activation. Use typed channel/plugin workflows instead. If the user asks to configure or repair provider/auth access, tell them to exit Operator and run `operator onboard`, which live-tests a candidate before saving it. Doctor repairs can also change the active inference route; tell the user to exit Operator and run `operator doctor --fix`.",
  "A new agent cannot select its own model during creation. Use `create agent <id> workspace <path>`; it inherits the live-verified default route. The ids `operator` and `crestodian` are reserved for the system agent and cannot be created as normal agents.",
 ...
 * `operator` tool). Unlike the planner contract, replies are natural text
...
export const SYSTEM_AGENT_SYSTEM_PROMPT = [
  ...
  "You act ONLY through the `operator` tool. Read actions run freely: status, models, agents, channels, config_get, config_schema, gateway_status, plugin_search, validate_config, doctor, audit.",
