  ...
  switch (operation.kind) {
    ...
    case "model-setup":
      runtime.log(
        [
          ...
          "Exit Operator and run `openclaw onboard`; it stages credentials, live-tests the candidate route, and saves only a passing setup.",
        ].join("\n"),
      );
      return { applied: false };
    case "open-setup": {
    ...
    case "plugin-uninstall": {
      const message = [
        ...
        `Exit Operator and run \`openclaw plugins uninstall ${operation.pluginId}\` from a terminal.`,
      ].join("\n");
      runtime.log(message);
    ...
    case "doctor-fix":
      runtime.log(
        "Doctor repairs can change the inference route that powers this session. Exit Operator and run `openclaw doctor --fix` in a terminal.",
      );
      return { applied: false };
    case "status": {
