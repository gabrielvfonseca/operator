import { isOperatorTrustedPluginInstallSpec } from "../plugins/install-provenance.js";
...
export function validateSystemAgentPluginInstallSpec(spec: string): string | null {
  ...
  if (/\s/.test(trimmed)) {
    return "Operator plugin install accepts one npm or ClawHub package spec.";
  }
  if (/^(?:\.{1,2}\/|\/|~\/|file:|git(?:\+ssh|\+https)?:|https?:)/i.test(trimmed)) {
    // Operator does not install local paths or URLs; those can execute arbitrary package code.
