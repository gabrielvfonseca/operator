...
/** CLI options accepted by `operator reset`. */
type ResetOptions = {
  scope?: ResetScope;
  yes?: boolean;
...
async function stopGatewayIfRunning(runtime: RuntimeEnv) {
  if (isNixMode) {
    // Nix mode owns service lifecycle outside Operator-managed launchd/systemd
...
export async function resetCommand(runtime: RuntimeEnv, opts: ResetOptions) {
  ...
  if (scope === "config+creds+sessions") {
    ...
    runtime.log(`Next: ${formatCliCommand("operator onboard --install-daemon")}`);
    return;
  }
