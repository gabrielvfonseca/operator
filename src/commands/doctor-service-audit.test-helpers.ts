...
export function readEmbeddedGatewayTokenForTest(
  ...
  return isEnvironmentFileOnlySource(command?.environmentValueSources?.OPERATOR_GATEWAY_TOKEN)
    ? undefined
    : normalizeOptionalString(command?.environment?.OPERATOR_GATEWAY_TOKEN);
}
