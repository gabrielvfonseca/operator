// Builds the data model for the standard `operator status` text report.
...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("operator security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("operator status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("operator tasks maintenance --apply")}`,
