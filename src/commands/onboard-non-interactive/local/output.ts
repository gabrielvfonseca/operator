...
function recoveryHintForGatewayHealthFailure(
  ...
  switch (classification) {
    case "auth-mismatch":
      return "Fix: run `operator doctor --fix`.";
    case "module-missing":
      return "Fix: run `operator doctor --fix`.";
    case "service-missing":
      return "Fix: run `operator gateway install --force`.";
    case "service-stopped":
      return "Fix: run `operator gateway restart`.";
    case "startup-blocked":
      return "Fix: run `operator gateway status --deep`.";
    case "not-listening":
      return "Fix: start `operator gateway run`, or run `operator gateway restart` for a managed gateway.";
    default:
      return undefined;
  }
}
