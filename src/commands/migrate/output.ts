...
function formatItemMessage(item: MigrationItem, mode: FormatMode): string | undefined {
  if (mode === "preview") {
    ...
    if (item.kind === "skill" && item.action === "copy") {
      return "Copy Codex skill into Operator";
    }
    if (item.kind === "plugin" && item.action === "install") {
      return "Install Codex plugin into Operator";
    }
...
export function assertConflictFreePlan(plan: MigrationPlan, providerId: string): void {
  if (plan.summary.conflicts > 0) {
    throw new Error(
      `Migration has ${formatCount(plan.summary.conflicts, "conflict")}. Re-run with --overwrite after reviewing operator migrate plan ${providerId}.`,
    );
  }
}
...
export function writeApplyResult(
  ...
  if (result.backupPath) {
    ...
    runtime.log("Backup: skipped (no existing Operator state found)");
  }
  if (result.reportDir) {
