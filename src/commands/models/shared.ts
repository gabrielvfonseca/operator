...
export async function loadValidConfigOrThrow(): Promise<OperatorConfig> {
...
export async function updateConfig(
  mutator: (cfg: OperatorConfig, context: UpdateConfigContext) => OperatorConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OperatorConfig }): {
  provider: string;
  model: string;
} {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("operator agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
