...
type StatusScanSharedMocks = {
  ...
  resolveGatewayPort: Mock<(cfg?: OperatorConfig) => number>;
...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/operator-${configPathLabel}-missing-${process.pid}.json`),
    resolveGatewayPort: vi.fn((cfg?: OperatorConfig) => cfg?.gateway?.port ?? 18789),
...
export function createStatusScanConfig<T extends object = OperatorConfig>(
  overrides: T = {} as T,
): OperatorConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): OperatorConfig {
