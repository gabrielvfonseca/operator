...
function resolveBundledWorkspaceDir(cfg: OperatorConfig): string | undefined {
  return resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg)) ?? undefined;
}

function isOperatorNodeModulesPackageRoot(packageRoot: string): boolean {
  ...
  for (const rawPath of rawPaths) {
    ...
    if (!match) {
      ...
      const oldPackageMatch =
        // Only rewrite paths rooted in the installed Operator package; user plugin paths stay intact.
        oldPackageRoot && oldBundledLeaf && isOperatorNodeModulesPackageRoot(oldPackageRoot)
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; Operator loads the packaged bundled plugin from "${hit.toPath}".`,
  );
