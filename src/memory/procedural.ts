// Neo4j + Temporal procedural memory for reusable procedures and workflow execution.
import type { MemoryProceduralConfig } from "./config.js";
...
export function createProceduralClient(config: MemoryProceduralConfig): ProceduralClient {
  ...
  return {
    ...
    async executeProcedure(params) {
      ...
      try {
        const run = await (t as any).start({
