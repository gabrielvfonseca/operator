...
export function listQaRunnerCliContributions(): readonly QaRunnerCliContribution[] {
  ...
  for (const plugin of listDeclaredQaRunnerPlugins(env)) {
    ...
    for (const runner of plugin.qaRunners) {
      ...
      if (!registration) {
        throw new Error(
          `QA runner plugin "${plugin.id}" declared "${runner.commandName}" in operator.plugin.json but did not export a matching CLI registration`,
        );
      }
    ...
    for (const commandName of runtimeRegistrationByCommandName?.keys() ?? []) {
      if (!declaredCommandNames.has(commandName)) {
        throw new Error(
          `QA runner plugin "${plugin.id}" exported "${commandName}" from runtime-api.js but did not declare it in operator.plugin.json`,
        );
      }
    }
  }
