...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "@gabrielvfonseca/operator/plugin-sdk/core";
import {
  readFileWithinRoot,
...
} from "@gabrielvfonseca/operator/plugin-sdk/secret-file-runtime";
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTempDir({ prefix: "operator-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps root-bounded file-access helpers on file-access-runtime", async () => {
    await withTempDir({ prefix: "operator-sdk-file-access-compat-" }, async (root) => {
