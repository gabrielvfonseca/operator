// Temp home test helpers create isolated Operator home directories for plugin tests.
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked OPERATOR_HOME.
  deleteTestEnvValue("OPERATOR_HOME");
  setTestEnvValue("OPERATOR_STATE_DIR", path.join(base, ".operator"));
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "operator-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".operator", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
