...
describe("oauth paths", () => {
  it("prefers OPERATOR_OAUTH_DIR over OPERATOR_STATE_DIR", () => {
    const env = {
      OPERATOR_OAUTH_DIR: "/custom/oauth",
      OPERATOR_STATE_DIR: "/custom/state",
  ...
  it("derives oauth path from OPERATOR_STATE_DIR when unset", () => {
    const env = {
      OPERATOR_STATE_DIR: "/custom/state",
    } as NodeJS.ProcessEnv;
...
describe("gateway port resolution", () => {
  it("prefers numeric env values over config", () => {
    expect(
      resolveGatewayPort({ gateway: { port: 19002 } }, envWith({ OPERATOR_GATEWAY_PORT: "19001" })),
    ).toBe(19001);
  });
  ...
  it("accepts Compose-style IPv4 host publish values from env", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19002 } },
        envWith({ OPERATOR_GATEWAY_PORT: "127.0.0.1:18789" }),
      ),
    ).toBe(18789);
  });
  ...
  it("accepts Compose-style IPv6 host publish values from env", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19002 } },
        envWith({ OPERATOR_GATEWAY_PORT: "[::1]:28789" }),
      ),
    ).toBe(28789);
  });
  ...
  it("falls back to config when the Compose-style suffix is invalid", () => {
    expect(
      resolveGatewayPort(
        { gateway: { port: 19003 } },
        envWith({ OPERATOR_GATEWAY_PORT: "127.0.0.1:not-a-port" }),
      ),
    ).toBe(19003);
  });
  ...
  it("falls back to config when env ports exceed TCP bounds", () => {
    expect(
      resolveGatewayPort({ gateway: { port: 19003 } }, envWith({ OPERATOR_GATEWAY_PORT: "65536" })),
    ).toBe(19003);
    expect(
      resolveGatewayPort(
        { gateway: { port: 19004 } },
        envWith({ OPERATOR_GATEWAY_PORT: "127.0.0.1:65536" }),
      ),
    ).toBe(19004);
    expect(
      resolveGatewayPort(
        { gateway: { port: 19005 } },
        envWith({ OPERATOR_GATEWAY_PORT: "[::1]:65536" }),
      ),
    ).toBe(19005);
  });
  ...
  it("falls back when malformed IPv6 inputs do not provide an explicit port", () => {
    expect(
      resolveGatewayPort({ gateway: { port: 19003 } }, envWith({ OPERATOR_GATEWAY_PORT: "::1" })),
    ).toBe(19003);
    expect(resolveGatewayPort({}, envWith({ OPERATOR_GATEWAY_PORT: "2001:db8::1" }))).toBe(
      DEFAULT_GATEWAY_PORT,
    );
  });

  it("falls back to the default port when env is invalid and config is unset", () => {
    expect(resolveGatewayPort({}, envWith({ OPERATOR_GATEWAY_PORT: "127.0.0.1:not-a-port" }))).toBe(
      DEFAULT_GATEWAY_PORT,
    );
  });
});
...
describe("state + config path candidates", () => {
  function expectOperatorHomeDefaults(env: NodeJS.ProcessEnv): void {
    const configuredHome = env.OPERATOR_HOME;
    if (!configuredHome) {
  ...
  it("normalizes relative OPERATOR_STATE_DIR overrides to absolute paths", () => {
    ...
    expect(env.OPERATOR_STATE_DIR).toBe(path.resolve("."));
  });
  ...
  it("pins a relative state-dir override before later resolution", () => {
    ...
    normalizeStateDirEnv(env);
    const normalized = env.OPERATOR_STATE_DIR;
  ...
  it("re-pins exported runtime paths after startup environment selection", () => {
    ...
    try {
      ...
      pinRuntimePaths({
        OPERATOR_CONFIG_PATH: originalConfigPath,
        OPERATOR_NIX_MODE: originalNixMode ? "1" : undefined,
        OPERATOR_STATE_DIR: originalStateDir,
        OPERATOR_TEST_FAST: "1",
      });
    }
  });
...
describe("resolveIncludeRoots", () => {
  ...
  it("returns an empty list when OPERATOR_INCLUDE_ROOTS is unset or blank", () => {
    ...
    expect(resolveIncludeRoots(envWith({ OPERATOR_INCLUDE_ROOTS: "" }), () => HOME)).toStrictEqual(
      [],
    );
    expect(
      resolveIncludeRoots(envWith({ OPERATOR_INCLUDE_ROOTS: "   " }), () => HOME),
    ).toStrictEqual([]);
  });
  ...
  it("splits on the platform path delimiter and resolves each entry to an absolute path", () => {
    ...
    const env = envWith({ OPERATOR_INCLUDE_ROOTS: [a, b].join(path.delimiter) });
  ...
  it("drops empty entries and preserves de-duplicated order for repeated roots", () => {
    const a = path.resolve(path.parse(process.cwd()).root, "shared", "a");
    const env = envWith({
      OPERATOR_INCLUDE_ROOTS: ["", a, "  ", a].join(path.delimiter),
    });
