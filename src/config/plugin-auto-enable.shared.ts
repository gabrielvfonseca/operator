...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: OperatorConfig): boolean {
...
function isProviderConfigured(cfg: OperatorConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: OperatorConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: OperatorConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: OperatorConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: OperatorConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: OperatorConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: OperatorConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredSpeechProviderSelection(cfg: OperatorConfig): boolean {
  return collectConfiguredSpeechProviderIds(cfg).size > 0;
}

function hasConfiguredPluginConfigEntry(cfg: OperatorConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: OperatorConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: OperatorConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: OperatorConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: OperatorConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: OperatorConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: OperatorConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: OperatorConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: OperatorConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: OperatorConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: OperatorConfig): boolean {
  return (
...
function hasPluginEntries(cfg: OperatorConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: OperatorConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: OperatorConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: OperatorConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: OperatorConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: OperatorConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: OperatorConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: OperatorConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: OperatorConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
