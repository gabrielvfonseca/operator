...
describe("Dockerfile", () => {
  ...
  it("uses full bookworm for build stages and slim bookworm for runtime", async () => {
    ...
    expect(dockerfile).not.toContain("OPERATOR_VARIANT");
  });
  ...
  it("installs optional browser dependencies after pnpm install", async () => {
    ...
    const browserArgIndex = dockerfile.indexOf("ARG OPERATOR_INSTALL_BROWSER");
  ...
  it("does not let pnpm resync the full source workspace during Docker build scripts", async () => {
    ...
    expect(collapsed).not.toContain(
      'OPERATOR_DOCKER_BUILD_EXTENSIONS="$OPERATOR_EXTENSIONS" OPERATOR_RUN_NODE_SKIP_DTS_BUILD=',
    );
  ...
  it("prunes runtime dependencies and omitted plugin packages after the build stage", async () => {
    ...
    expect(dockerfile).toContain("ARG OPERATOR_EXTENSIONS");
    expect(dockerfile).toContain(
    ...
    expect(dockerfile).toContain("ARG OPERATOR_BUNDLED_PLUGIN_DIR");
    expect(dockerfile).toContain(
  ...
  it("keeps the Codex plugin in official Docker release images", async () => {
    ...
    const releaseKeepList = "OPERATOR_EXTENSIONS=diagnostics-otel,codex";

    expect(workflow.match(new RegExp(releaseKeepList, "g"))).toHaveLength(4);
    expect(workflow).not.toContain("OPERATOR_EXTENSIONS=diagnostics-otel\n");
  });
  ...
  it("does not override bundled plugin discovery in runtime images", async () => {
    const dockerfile = collapseDockerContinuations(await readFile(dockerfilePath, "utf8"));
    expect(dockerfile).toContain(`ARG OPERATOR_BUNDLED_PLUGIN_DIR=${BUNDLED_PLUGIN_ROOT_DIR}`);
    expect(dockerfile).not.toMatch(/^\s*ENV\b[^\n]*\bOPERATOR_BUNDLED_PLUGINS_DIR\b/m);
  });
  ...
  it("Docker GPG fingerprint awk uses correct quoting for OPERATOR_SANDBOX=1 build", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
