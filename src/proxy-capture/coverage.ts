...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[operator proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[operator proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`operator proxy coverage\` for details.`,
  );
}
