...
export const zh_TW = {
  ...
  wizard: {
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "遠端主機請使用 wss://，或透過 SSH 通道使用 ws://127.0.0.1/localhost。緊急繞過：可信私有網路可設定 OPERATOR_ALLOW_INSECURE_PRIVATE_WS=1。",
    ...
    guided: {
      ...
      ttyRequired:
        "Onboarding 需要互動式 TTY。自動化請使用 `operator onboard --non-interactive --accept-risk ...`。",
      welcomeTitle: "設定選項",
    },
    setup: {
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作區設定 → Integrations → Operator → 建立 bot → 複製 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 執行：./occ talk:bot:install "Operator" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    finalize: {
      ...
      gatewayTokenStored:
        "儲存位置：$OPERATOR_CONFIG_PATH（預設 ~/.operator/operator.json）中的 gateway.auth.token，或 OPERATOR_GATEWAY_TOKEN。",
      ...
      containerSystemdUnavailable:
        "此容器內沒有 systemd 使用者服務。Operator 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
        "在你的電腦上執行 agent 存在風險，請加固設定：https://docs.operator.ai/security",
