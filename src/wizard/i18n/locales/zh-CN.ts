...
export const zh_CN = {
  ...
  wizard: {
    ...
    remote: {
      ...
      insecureRemoteUrl:
        "远程主机请使用 wss://，或通过 SSH 隧道使用 ws://127.0.0.1/localhost。紧急绕过：可信私有网络可设置 OPERATOR_ALLOW_INSECURE_PRIVATE_WS=1。",
    ...
    guided: {
      ...
      ttyRequired:
        "Onboarding 需要交互式 TTY。自动化请使用 `operator onboard --non-interactive --accept-risk ...`。",
      welcomeTitle: "设置选项",
    },
    setup: {
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作区设置 → Integrations → Operator → 创建 bot → 复制 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 运行：./occ talk:bot:install "Operator" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    finalize: {
      ...
      gatewayTokenStored:
        "存储位置：$OPERATOR_CONFIG_PATH（默认 ~/.operator/operator.json）中的 gateway.auth.token，或 OPERATOR_GATEWAY_TOKEN。",
      ...
      containerSystemdUnavailable:
        "此容器内没有 systemd 用户服务。Operator 只会跳过后台服务安装；请以前台方式运行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
        "在你的电脑上运行 agent 存在风险，请加固设置：https://docs.operator.ai/security",
