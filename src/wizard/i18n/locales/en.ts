...
export const en = {
  ...
  wizard: {
    ...
    security: {
      ...
      hardeningRequired:
        "If you're not comfortable with security hardening and access control, don't run Operator.",
    ...
    clickclack: {
      ...
      helpCreateToken:
        "In ClickClack: Workspace settings → Integrations → Operator → create bot → copy token",
      invalidToken:
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) Run: ./occ talk:bot:install "Operator" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    finalize: {
      ...
      daemonRuntimeNodeHint:
        "Required because Operator state uses node:sqlite; Bun cannot run the Gateway.",
      ...
      outroDashboardOpened:
        "Onboarding complete. Dashboard opened; keep that tab to control Operator.",
      outroSeeded:
      ...
      containerSystemdUnavailable:
        "Systemd user services are not available inside this container. Operator is skipping only the background service install; run the Gateway in the foreground or use your container supervisor.",
      securityReminder:
