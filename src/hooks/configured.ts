...
export function hasConfiguredInternalHooks(config: OperatorConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: OperatorConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
