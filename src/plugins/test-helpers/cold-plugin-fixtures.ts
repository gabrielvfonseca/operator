...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/operator-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "operator.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): OperatorConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    OPERATOR_HOME: path.join(homeDir, "home"),
    OPERATOR_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
