...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before Operator consumes it. */
...
export type CliBackendPrepareExecutionContext = {
  ...
  provider: string;
  modelId: string;
  /** Effective Operator context budget selected for this run. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Operator should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume Operator's bundle
   ...
   *
   * Use this when the backend should consume a canonical Operator auth profile
   ...
   * `profile-only` treats the selected Operator auth profile as the sole auth
   ...
   * hooks that do not consume Operator auth profiles.
   */
   ...
   * as static config, such as mapping Operator thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside Operator's tool
