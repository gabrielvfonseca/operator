...
function resolveMemoryRuntimePluginIds(config: OperatorConfig): string[] {
...
function resolveMemoryRuntimeWorkspaceDir(cfg: OperatorConfig): string | undefined {
...
function ensureMemoryRuntime(cfg?: OperatorConfig) {
...
export function resolveActiveMemoryBackendConfig(params: { cfg: OperatorConfig; agentId: string }) {
  return ensureMemoryRuntime(params.cfg)?.resolveMemoryBackendConfig(params) ?? null;
}

/** Closes all active plugin-backed memory search managers. */
export async function closeActiveMemorySearchManagers(cfg?: OperatorConfig): Promise<void> {
  void cfg;
