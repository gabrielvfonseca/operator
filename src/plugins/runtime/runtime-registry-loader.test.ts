...
function loadOptions(index = 0) {
  return requireRecord(mocks.loadOperatorPlugins.mock.calls[index]?.[0], `load options ${index}`);
}
...
vi.mock("../loader.js", () => ({
  loadOperatorPlugins: (...args: Parameters<typeof mocks.loadOperatorPlugins>) =>
    mocks.loadOperatorPlugins(...args),
  resolveCompatibleRuntimePluginRegistry: (
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadOperatorPlugins.mockReturnValue(createEmptyPluginRegistry());
    mocks.resolveRuntimePluginRegistry.mockImplementation(
      (...args: Parameters<typeof mocks.loadOperatorPlugins>) => mocks.loadOperatorPlugins(...args),
    );
  ...
  it("does not reuse non-empty all-scope registries without loader compatibility", () => {
    ...
    mocks.loadOperatorPlugins.mockClear();
