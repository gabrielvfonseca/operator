...
/**
 * Provider-owned stream wrapper hook after Operator applies its generic
 ...
 *
 * Return `true` or `false` to override Operator's built-in provider cache TTL
 ...
 *
 * Runs only after Operator exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Operator should append to the generic `Unknown model`
 * error.
 */
