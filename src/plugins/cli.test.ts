...
const mocks = vi.hoisted(() => ({
  ...
  loadOperatorPluginCliRegistry: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadOperatorPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadOperatorPluginCliRegistry(...args),
  loadOperatorPlugins: (...args: unknown[]) => mocks.loadOperatorPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  ...
  autoEnabledConfig: OperatorConfig;
  ...
  const loadOptions = getMockCallObject(mocks.loadOperatorPlugins);
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadOperatorPluginCliRegistry.mockReset();
    mocks.loadOperatorPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadOperatorPlugins.mockReset();
    mocks.loadOperatorPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as OperatorConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as OperatorConfig);
  ...
  it("injects gateway-backed node runtime into plugin CLI commands", async () => {
    await registerPluginCliCommands(createProgram(), {} as OperatorConfig);

    const loadOptions = getMockCallObject(mocks.loadOperatorPlugins) as {
  ...
  it("reuses loaded plugin CLI entries on repeat calls for the same program", async () => {
    ...
    expect(mocks.loadOperatorPlugins).toHaveBeenCalledTimes(1);
  });

  it("reloads plugin CLI entries when the requested primary command changes", async () => {
    const program = createProgram();

    await registerPluginCliCommands(program, {} as OperatorConfig, undefined, undefined, {
      primary: "memory",
    });
    await registerPluginCliCommands(program, {} as OperatorConfig);

    expect(mocks.loadOperatorPlugins).toHaveBeenCalledTimes(2);
  });

  it("reloads plugin CLI entries when config or environment identity changes", async () => {
    const program = createProgram();
    const configA = {} as OperatorConfig;
    const configB = { plugins: {} } as OperatorConfig;
    const envA = { OPERATOR_HOME: "/tmp/a" } as NodeJS.ProcessEnv;
    const envB = { OPERATOR_HOME: "/tmp/b" } as NodeJS.ProcessEnv;
    ...
    expect(mocks.loadOperatorPlugins).toHaveBeenCalledTimes(3);
  });
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadOperatorPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    const registryOptions = getMockCallObject(mocks.loadOperatorPluginCliRegistry);
  ...
  it("keeps root-help descriptor load failures quiet", async () => {
    ...
    mocks.loadOperatorPluginCliRegistry.mockImplementationOnce((options: { logger?: unknown }) => {
    ...
    await expect(
      getPluginCliCommandDescriptors({ plugins: { entries: { stale: {} } } } as OperatorConfig),
    ).resolves.toEqual([]);
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadOperatorPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as OperatorConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("scopes full CLI loading through CLI metadata when manifest planning finds no plugin match", async () => {
    ...
    expect(mocks.loadOperatorPluginCliRegistry).toHaveBeenCalled();
    const loadOptions = getMockCallObject(mocks.loadOperatorPlugins);
    expect(loadOptions.onlyPluginIds).toEqual(["memory-core"]);
  });
  ...
  it("scopes nested CLI loading through CLI metadata parent paths", async () => {
    ...
    mocks.loadOperatorPluginCliRegistry.mockResolvedValue(nestedRegistry);
    mocks.loadOperatorPlugins.mockReturnValue(nestedRegistry);
    const program = createProgram("nodes");
    program.exitOverride();
  ...
  it("skips full plugin runtime loading when no metadata owns the requested primary", async () => {
    ...
    expect(mocks.loadOperatorPluginCliRegistry).toHaveBeenCalled();
    expect(mocks.loadOperatorPlugins).not.toHaveBeenCalled();
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as OperatorConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
