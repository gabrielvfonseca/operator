...
describe("installPluginFromArchive", () => {
  ...
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@operator/voice-call" });
  ...
  it("rejects native plugin zip archives without openclaw.plugin.json", async () => {
    ...
    expect(fs.existsSync(resolvePluginInstallDir("@operator/zipper", extensionsDir))).toBe(false);
  });
  ...
  it("rejects packages without openclaw.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@operator/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
  ...
  it("rejects legacy plugin package shape when openclaw.extensions is missing", async () => {
    ...
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@operator/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
...
describe("installPluginFromDir", () => {
  ...
  it("emits a redacted security event after installing a plugin directory", async () => {
    ...
    expect(captured.events[0]).toMatchObject({
      ...
      severity: "medium",
      actor: { kind: "operator" },
      target: { kind: "plugin", name: "@operator/test-plugin" },
  ...
  it("allows plugins when a beta host is on the package plugin API floor", async () => {
    ...
    expect(result.pluginId).toBe("@operator/test-plugin");
  });
  ...
  it("uses openclaw.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@operator/cognee-openclaw"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedPluginId: "@operator/test-plugin",
      install: (pluginDir: string, extensionsDir: string) =>
        installPluginFromDir({
          dirPath: pluginDir,
          extensionsDir,
