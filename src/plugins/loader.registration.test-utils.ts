...
describe("loadOperatorPlugins", () => {
  ...
  it("allows bundled plugins to supply system.notify without opening the command to external plugins", () => {
    ...
    const bundledRegistry = loadOperatorPlugins({
      cache: false,
    ...
    const externalRegistry = loadOperatorPlugins({
      cache: false,
  ...
  it("restores cached memory capability public artifacts on cache hits", async () => {
    ...
    const first = loadOperatorPlugins(options);
    ...
    const second = loadOperatorPlugins(options);
  ...
  it("uses discovery registration mode for non-activating loads", () => {
    ...
    const snapshot = loadOperatorPlugins({
      activate: false,
      cache: false,
      workspaceDir: plugin.dir,
      config,
  ...
  it("reuses a gateway-bindable cache entry for later default-mode loads", () => {
    ...
    const gatewayBindable = loadOperatorPlugins({
      ...options,
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
      },
    });
    const defaultMode = loadOperatorPlugins(options);
  ...
  it("keeps pinned gateway hooks and later default-mode hooks dispatchable together", () => {
    ...
    const gatewayRegistry = loadOperatorPlugins({
    ...
    try {
      ...
      const defaultRegistry = loadOperatorPlugins({
