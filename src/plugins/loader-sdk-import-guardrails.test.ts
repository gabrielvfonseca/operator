...
const ALLOWED_PLUGIN_SDK_FIXTURE_IMPORTS = new Set([
  ...
  'src/plugins/loader.test.ts:configSchema: (require("operator/plugin-sdk").emptyPluginConfigSchema)(),',
...
function findLoaderFixtureSdkImports(): string[] {
  ...
  for (const file of LOADER_FIXTURE_TEST_FILES) {
    ...
    for (const line of source.split("\n")) {
      if (
        line.includes('require("operator/plugin-sdk') ||
        (line.includes("import ") && line.includes('"operator/plugin-sdk'))
      ) {
