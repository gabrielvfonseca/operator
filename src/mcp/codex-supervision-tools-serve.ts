...
function withCodexSupervisionEnabled(config: OperatorConfig): OperatorConfig {
  const next = structuredClone(normalizePluginTargetConfig(config, "codex")) as OperatorConfig &
  ...
  if (process.env.OPERATOR_CODEX_SUPERVISOR_ALLOW_RAW_TRANSCRIPTS === "1") {
    supervision.allowRawTranscripts = true;
  }
  if (process.env.OPERATOR_CODEX_SUPERVISOR_ALLOW_WRITE_CONTROLS === "1") {
    supervision.allowWriteControls = true;
  }
  return next;
}

function resolveCodexSupervisionTools(config: OperatorConfig): AnyAgentTool[] {
  const context = {
    config,
...
export function createCodexSupervisionToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "operator-codex-supervisor", tools });
}
