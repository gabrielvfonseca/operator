...
vi.mock("./channel-bridge.js", () => ({
  OperatorChannelBridge: class MockOperatorChannelBridge {
...
describe("serveOperatorChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveOperatorChannelMcp } = await import("./channel-server.js");

    const servePromise = serveOperatorChannelMcp({ verbose: false });
