...
import { isParentOwnedBackgroundAcpSession } from "@gabrielvfonseca/acp-core/session-interaction-mode";
import {
...
if (process.env.VITEST || process.env.NODE_ENV === "test") {
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("operator.dispatchFromConfigTestApi")] = {
...
async function dispatchReplyFromConfigInner(
  ...
  try {
    ...
    const wrapProgressCallback = <
      Args extends unknown[],
      Result extends undefined | false | undefined,
    >(
    ...
    const replyConfig = withFullRuntimeReplyConfig(
      params.configOverride ? (applyMergePatch(cfg, params.configOverride) as OperatorConfig) : cfg,
    );
