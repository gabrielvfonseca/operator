---
name: node-connect
description: "Diagnose Operator Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Operator is advertising that route, then fix pairing/auth.
...
- whether `operator devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `operator qr --json`. It uses the same setup-code payload Android scans.

```bash
operator config get gateway.mode
operator config get gateway.bind
operator config get gateway.tailscale.mode
operator config get gateway.remote.url
operator config get gateway.auth.mode
operator config get gateway.auth.allowTailscale
operator config get plugins.entries.device-pair.config.publicUrl
operator qr --json
operator devices list
operator nodes status
```

If this Operator instance is pointed at a remote gateway, also run:

```bash
operator qr --remote --json
```
...
`operator qr --json` success means:
...
If `operator qr --json` says `Gateway is only bound to loopback`:
...
- `operator devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run operator qr again, rescan, then approve the pending device pairing.`
