...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${OPERATOR_CONFIG_PATH:-${OPERATOR_STATE_DIR:-$HOME/.operator}/operator.json}"
...
```text
${OPERATOR_STATE_DIR:-$HOME/.operator}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with operator message send if route provided.
```
