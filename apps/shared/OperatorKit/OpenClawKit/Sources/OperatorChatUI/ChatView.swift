...
@MainActor
public struct OperatorChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    private let talkControl: OperatorChatTalkControl?
    private let voiceNoteControl: OperatorChatVoiceNoteControl?
    private let speech: OperatorChatSpeechController?
    ...
    public init(
        ...
        talkControl: OperatorChatTalkControl? = nil,
        voiceNoteControl: OperatorChatVoiceNoteControl? = nil,
        speech: OperatorChatSpeechController? = nil)
    {
    ...
    private func isListenable(_ msg: OperatorChatMessage) -> Bool {
    ...
    private func speechChipIsPreparing(
        _ speech: OperatorChatSpeechController,
        messageID: UUID) -> Bool?
    {
        switch speech.phase {
    ...
    @ViewBuilder
    private func copyMessageButton(for message: OperatorChatMessage) -> some View {
    ...
    private var visibleMessages: [OperatorChatMessage] {
        let base: [OperatorChatMessage]
    ...
    private var jumpToLatestButton: some View {
        Button {
            ...
            Label("Jump to latest", systemImage: "arrow.down")
                .font(OperatorChatTypography.body(size: 16, weight: .semibold, relativeTo: .callout))
    ...
    private func mergeToolResults(in messages: [OperatorChatMessage]) -> [OperatorChatMessage] {
        var result: [OperatorChatMessage] = []
        ...
        for message in messages {
            ...
            let merged = OperatorChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: OperatorChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: OperatorChatMessage) -> Bool {
    ...
    private func primaryText(in message: OperatorChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: OperatorChatMessage) -> Bool {
    ...
    private func toolCalls(in message: OperatorChatMessage) -> [OperatorChatMessageContent] {
    ...
    private func inlineToolResults(in message: OperatorChatMessage) -> [OperatorChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: OperatorChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: OperatorChatMessage) -> String {
...
private struct ChatAssistantIntroCard: View {
    let text: String
    let prompts: [OperatorChatView.StarterPrompt]
    let onPrompt: (OperatorChatView.StarterPrompt) -> Void
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            ForEach(self.prompts) { prompt in
                Button {
                    ...
                    HStack(spacing: 8) {
                        Text(prompt.title)
                            .font(OperatorChatTypography.body(size: 15, weight: .semibold, relativeTo: .callout))
                            .multilineTextAlignment(.leading)
                        Spacer(minLength: 8)
    ...
    var body: some View {
        ...
        ContentUnavailableView {
            Label(self.title, systemImage: self.systemImage)
                .font(OperatorChatTypography.headline)
        } description: {
            Text(self.message)
                .font(OperatorChatTypography.body)
        } actions: {
            if let actionTitle, let action {
                Button(action: action) {
                    Text(actionTitle)
                        .font(OperatorChatTypography.body(size: 15, weight: .semibold, relativeTo: .subheadline))
                }
