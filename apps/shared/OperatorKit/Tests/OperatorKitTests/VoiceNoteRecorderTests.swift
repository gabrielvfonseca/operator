...
final class VoiceNoteRecorderTests: XCTestCase {
    ...
    func testDurationLabelBoundsMalformedHistoryValues() {
        XCTAssertEqual(operatorVoiceNoteDurationLabel(.infinity), "0:00")
        XCTAssertEqual(operatorVoiceNoteDurationLabel(.nan), "0:00")
        XCTAssertEqual(operatorVoiceNoteDurationLabel(1e100), "3:00")
        XCTAssertEqual(operatorVoiceNoteDurationLabel(-1), "0:00")
    }
    ...
    func testRecordButtonRequiresAttachmentInput() {
        let recorder = operatorVoiceNoteRecorder(capture: FakeVoiceNoteAudioCapture())
        let control = operatorChatVoiceNoteControl(recorder: recorder, isTalkActive: false)
        let button = operatorVoiceNoteButton(
            control: control,
            compact: false,
