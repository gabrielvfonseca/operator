...
public struct operatorVoiceNoteRecording: Equatable, Sendable {
...
@MainActor
@Observable
public final class operatorVoiceNoteRecorder {
    public enum State: Equatable {
        case idle
        case requestingPermission
        case recording(startedAt: Date, fileURL: URL)
        case finished(recording: operatorVoiceNoteRecording)
        case staging(recording: operatorVoiceNoteRecording)
    ...
    public convenience init() {
        self.init(capture: operatorVoiceNoteAudioCapture())
    }

    /// Creates a recorder with an injectable audio capture seam.
    public init(
        capture: any VoiceNoteAudioCapture,
        durationLimit: TimeInterval = operatorVoiceNoteRecorder.maximumDurationSeconds,
    ...
    public var completedRecording: operatorVoiceNoteRecording? {
    ...
    func claimCompletedRecording() -> operatorVoiceNoteRecording? {
        guard case let .finished(recording) = self.state else { return nil }
    ...
    func completeStaging(_ recording: operatorVoiceNoteRecording) {
    ...
    @discardableResult
    public func finish() -> operatorVoiceNoteRecording? {
        guard case let .recording(_, fileURL) = self.state else { return nil }

        self.timerTask?.cancel()
        self.timerTask = nil
        let duration = max(0, self.capture.stop())
        let recording = operatorVoiceNoteRecording(fileURL: fileURL, durationSeconds: duration)
...
@MainActor
public final class operatorVoiceNoteAudioCapture: NSObject, VoiceNoteAudioCapture, AVAudioRecorderDelegate {
    ...
    public func start(url: URL) throws {
        do {
            ...
            guard recorder.record() else {
                throw NSError(
                    domain: "operatorVoiceNoteAudioCapture",
                    code: 1,
...
func operatorVoiceNoteDurationLabel(_ durationSeconds: TimeInterval) -> String {
