...
struct ChatAgentAvatar: View {
    ...
    var body: some View {
        Text(self.displayText)
            .font(operatorChatTypography.avatar(size: self.fontSize))
            ...
            .background(
                Circle()
                    .fill(
                        LinearGradient(
                            colors: [
                                (self.tint ?? operatorChatTheme.accent).opacity(0.95),
            ...
            .shadow(color: (self.tint ?? operatorChatTheme.accent).opacity(0.18), radius: 8, y: 4)
...
private struct ChatMessageBody: View {
    @Environment(\.operatorAssistantBubblesInCleanChrome) private var assistantBubblesInClean
    let message: operatorChatMessage
    let isUser: Bool
    let style: operatorChatView.Style
    let markdownVariant: ChatMarkdownVariant
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? operatorChatTheme.userText : operatorChatTheme.assistantText
    ...
    private var primaryText: String {
        ...
        return operatorChatMessage.displayText(
    ...
    private var inlineAttachments: [operatorChatMessageContent] {
    ...
    private var toolCalls: [operatorChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [operatorChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func usageTint(_ pressure: ChatMessageUsagePresentation.Pressure) -> Color {
        switch pressure {
        case .normal:
            operatorChatTheme.muted
        case .warning:
            operatorChatTheme.warning
        case .danger:
            operatorChatTheme.danger
        }
    }
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? operatorChatTheme.userBubble
        }
        if self.style == .onboarding {
            return operatorChatTheme.onboardingAssistantBubble
        }
        return operatorChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return operatorChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: operatorChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.isAudio ? "Voice note" : (self.att.fileName ?? "Attachment"))
                ...
                .lineLimit(1)
                .foregroundStyle(self.isUser ? operatorChatTheme.userText : operatorChatTheme.assistantText)
            if self.isAudio, let durationSeconds = self.att.durationSeconds {
                Text(operatorVoiceNoteDurationLabel(durationSeconds))
                    .font(operatorChatTypography.footnote)
                    .foregroundStyle(
                        self.isUser
                            ? operatorChatTheme.userText.opacity(0.72)
                            : operatorChatTheme.assistantText.opacity(0.72))
            }
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: operatorChatMessageContent
...
struct ChatOutboxStatusLabel: View {
    let state: operatorChatOutboxMessageState
    ...
    var body: some View {
        ...
        .foregroundStyle(self.state.isFailed ? AnyShapeStyle(operatorChatTheme.danger) : AnyShapeStyle(.secondary))
    ...
    private var title: LocalizedStringResource {
        ...
        case .confirming:
            "Confirming…"
        case let .failed(reason) where reason == operatorChatSQLiteTranscriptCache.outboxUnconfirmedError:
            "Delivery unknown"
        case .failed:
            "Not sent"
        }
...
private struct operatorAssistantBubblesInCleanChromeKey: EnvironmentKey {
...
extension EnvironmentValues {
    ...
    public var operatorAssistantBubblesInCleanChrome: Bool {
        get { self[operatorAssistantBubblesInCleanChromeKey.self] }
        set { self[operatorAssistantBubblesInCleanChromeKey.self] = newValue }
    }
}
...
private struct AssistantBubbleContainerStyle: ViewModifier {
    ...
    @Environment(\.operatorAssistantBubblesInCleanChrome) private var bubblesInClean
                ...
                // Clean call sites pre-pad only ~4pt; bubbles need room to breathe.
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                            .fill(operatorChatTheme.assistantBubble))
                    .overlay(
                        RoundedRectangle(cornerRadius: self.cornerRadius, style: .continuous)
                            .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
        }
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [operatorChatPendingToolCall]
    let isClean: Bool
        ...
        return VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                let font = segment.kind == .thinking
                    ? operatorChatTypography.callout.italic()
                    : operatorChatTypography.body
