...
struct OperatorMacCLI {
    static func main() async {
        ...
        case let .unknown(exitCode):
            fputs("operator-mac: unknown command\n", stderr)
            printUsage()
            exit(exitCode)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      ...
      operator-mac discover [--timeout <ms>] [--json] [--include-local]
      operator-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      operator-mac connect
      operator-mac configure-remote --ssh-target user@gateway.local --remote-port 18789
      operator-mac connect --url ws://127.0.0.1:18789 --json
      operator-mac discover --timeout 3000 --json
      operator-mac wizard --mode local
    """)
}
