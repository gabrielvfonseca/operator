...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OPERATOR_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/operator")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("operator-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("operator-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/operator/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        let prefixes = ["@gabrielvfonseca/operator"]
        return files
            .filter { file in
