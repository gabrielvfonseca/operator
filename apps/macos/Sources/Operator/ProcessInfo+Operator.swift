...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.operator.mac`) even if the shipped
    ...
    {
        if environment["operator_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "operator.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "operator.nixMode") { return true }
