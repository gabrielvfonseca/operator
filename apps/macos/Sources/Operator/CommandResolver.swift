...
enum CommandResolver {
    ...
    private static let helperName = "operator"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let operatorEntry = root.appendingPathComponent("operator.mjs").path
        if FileManager().isReadableFile(atPath: operatorEntry) { return operatorEntry }
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/operator")
    ...
    private static func operatorManagedPaths(home: URL) -> [String] {
        let bases = [
    ...
    static func operatorExecutable(searchPaths: [String]? = nil) -> String? {
    ...
    static func operatorNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let operatorPath = projectoperatorExecutable(projectRoot: root) {
            return [operatorPath, subcommand] + extraArgs
        }
        if let operatorPath = operatorExecutable(searchPaths: searchPaths) {
            return [operatorPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "operator", subcommand] + extraArgs
        }
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/operator"
