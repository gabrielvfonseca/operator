...
struct VoiceWakeRuntimeTests {
    @Test func `trims after trigger keeps post speech`() {
        let triggers = ["claude", "@gabrielvfonseca/operator"]
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["@gabrielvfonseca/operator"]
        let text = "hey operator"
    ...
    @Test func `trigger only rejects trailing wake word mentions in ordinary speech`() {
        let triggers = ["@gabrielvfonseca/operator"]
        let text = "tell me about operator"
        #expect(!VoiceWakeRuntime._testIsTriggerOnly(text, triggers: triggers))
    }
    ...
    @Test func `matched trigger finds trigger not at transcript start`() {
        let triggers = ["@gabrielvfonseca/operator"]
        let text = "uh operator"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "@gabrielvfonseca/operator")
    }
    ...
    @Test func `matched trigger prefers most specific overlapping phrase`() {
        let triggers = ["@gabrielvfonseca/operator", "hey operator"]
        let text = "hey operator"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "hey operator")
    }
    ...
    @Test func `text only fallback populates matched trigger`() {
        let transcript = "hey operator do thing"
        let config = WakeWordGateConfig(triggers: ["@gabrielvfonseca/operator"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["@gabrielvfonseca/operator"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "@gabrielvfonseca/operator")
    }

    @Test func `text only fallback keeps the first trigger phrase when later words match another trigger`() {
        let transcript = "operator tell me about computer vision"
        let config = WakeWordGateConfig(triggers: ["@gabrielvfonseca/operator", "computer"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
    ...
    @Test func `trims after chinese trigger keeps post speech`() {
        let triggers = ["小爪", "@gabrielvfonseca/operator"]
    ...
    @Test func `gate requires gap between trigger and command`() {
        ...
        let segments = makeWakeWordSegments(
            ...
            words: [
                ("hey", 0.0, 0.1),
                ("@gabrielvfonseca/operator", 0.2, 0.1),
                ("do", 0.35, 0.1),
                ("thing", 0.5, 0.1),
            ])
        let config = WakeWordGateConfig(triggers: ["@gabrielvfonseca/operator"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "@gabrielvfonseca/operator", start: 0.2, duration: 0.1, range: transcript.range(of: "@gabrielvfonseca/operator")),
