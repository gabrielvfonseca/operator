...
@Suite(.serialized)
struct OperatorConfigFileTests {
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["OPERATOR_CONFIG_PATH": override]) {
            #expect(OperatorConfigFile.url().path == override)
        }
    }
    ...
    func `browser control enabled reads config flag`() async {
        ...
        await TestIsolation.withEnvValues(["OPERATOR_CONFIG_PATH": override]) {
            #expect(OperatorConfigFile.browserControlEnabled() == true)
            OperatorConfigFile.saveDict(["browser": ["enabled": false]])
            #expect(OperatorConfigFile.browserControlEnabled() == false)
            OperatorConfigFile.setBrowserControlEnabled(true)
            #expect(OperatorConfigFile.browserControlEnabled() == true)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["OPERATOR_CONFIG_PATH": override]) {
            ...
            #expect(OperatorConfigFile.remoteGatewayPort() == 19999)
            #expect(OperatorConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(OperatorConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(OperatorConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(OperatorConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(OperatorConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["OPERATOR_CONFIG_PATH": override]) {
            ...
            OperatorConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:18789")
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["OPERATOR_CONFIG_PATH": override]) {
            ...
            OperatorConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = OperatorConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["OPERATOR_CONFIG_PATH": override]) {
            ...
            OperatorConfigFile.clearRemoteGatewayUrl()
            let root = OperatorConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        ...
        await TestIsolation.withEnvValues([
            "OPERATOR_CONFIG_PATH": nil,
            "OPERATOR_STATE_DIR": dir,
        ]) {
            #expect(OperatorConfigFile.stateDirURL().path == dir)
            #expect(OperatorConfigFile.url().path == "\(dir)/operator.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            #expect(auditRoot?["source"] as? String == "macos-operator-config-file")
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let allowedRoot = OperatorConfigFile.loadDict()
    ...
    func `load dict ignores legacy config health sidecar`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try OperatorConfigFile.withTestingFileLock {
                let loaded = OperatorConfigFile.loadDict()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = OperatorConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [
