...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: OperatorProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OperatorProtocol.AnyCodable] = [
            "presence": .init([OperatorProtocol.AnyCodable(entry)]),
        ]
        let payload = OperatorProtocol.AnyCodable(payloadMap)
