...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: OperatorWatchNotifyParams) -> OperatorWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [OperatorWatchAction]?,
        kind: String?,
        promptId: String?) -> [OperatorWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> OperatorWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return OperatorWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                OperatorWatchAction(id: "approve", label: "Approve"),
                OperatorWatchAction(id: "decline", label: "Decline", style: "destructive"),
                OperatorWatchAction(id: "open_phone", label: "Open iPhone"),
                OperatorWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            OperatorWatchAction(id: "done", label: "Done"),
            OperatorWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            OperatorWatchAction(id: "open_phone", label: "Open iPhone"),
            OperatorWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: OperatorWatchRisk?,
        priority: OperatorNotificationPriority?) -> OperatorWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: OperatorNotificationPriority?,
        risk: OperatorWatchRisk?) -> OperatorNotificationPriority?
    {
