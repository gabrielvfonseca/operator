...
final class ContactsService: ContactsServicing {
    ...
    func search(params: OperatorContactsSearchParams) async throws -> OperatorContactsSearchPayload {
        ...
        return OperatorContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: OperatorContactsAddParams) async throws -> OperatorContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return OperatorContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return OperatorContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> OperatorContactPayload {
        OperatorContactPayload(
