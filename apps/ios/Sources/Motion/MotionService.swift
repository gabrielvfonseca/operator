...
final class MotionService: MotionServicing {
    func activities(params: OperatorMotionActivityParams) async throws -> OperatorMotionActivityPayload {
        ...
        let mapped: [OperatorMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return OperatorMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = OperatorPedometerPayload(
