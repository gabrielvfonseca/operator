    ...
    var body: some View {
        VStack(alignment: .leading, spacing: self.style == .sheet ? 16 : 12) {
            HStack(alignment: .top, spacing: 12) {
                Image(systemName: self.iconSystemName)
                    ...
                    .foregroundStyle(self.requestIsPending ? OperatorBrand.warn : OperatorBrand.accent)
                    .frame(width: 28, height: 28)

                VStack(alignment: .leading, spacing: 6) {
                    Text(self.titleText)
                        .font(self.style == .sheet ? OperatorType.title3SemiBold : OperatorType.headline)
        ...
        .background {
            if self.style == .card || self.style == .sheet {
                RoundedRectangle(cornerRadius: OperatorRadius.md, style: .continuous)
                    .fill(.thinMaterial)
            }
        }
        .overlay {
            if self.style == .card || self.style == .sheet {
                RoundedRectangle(cornerRadius: OperatorRadius.md, style: .continuous)
                    .stroke(OperatorBrand.accent.opacity(0.20), lineWidth: 1)
            }
        }
