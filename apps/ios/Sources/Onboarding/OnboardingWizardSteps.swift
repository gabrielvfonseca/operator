...
struct OnboardingActivationCanvas<Content: View>: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .background(OperatorBrand.activationCanvasGradient.ignoresSafeArea())
        }
    }
}
...
private struct OnboardingHeroGlyph: View {
    var body: some View {
        OperatorActivationGlyph(size: 78, interactive: true)
    }
}
...
private typealias OnboardingPrimaryButtonStyle = OperatorPrimaryActionButtonStyle
...
private enum OnboardingIntroPanelStyle {
    ...
    static let panelFill = OperatorBrand.activationNeutralSurface
    static let iconFill = OperatorBrand.activationNeutralInsetSurface
    static let stroke = OperatorBrand.activationNeutralStroke
}
            ...
            HStack(alignment: .top, spacing: OnboardingIntroPanelStyle.contentSpacing) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text("Security notice")
                        ...
                        .foregroundStyle(.primary)
                    (
                        Text("The connected Operator agent can use device capabilities you enable.")
                            + Text(verbatim: " ")
                            + Text(
...
private struct OnboardingCommandChip: View {
    ...
    private let command = "operator qr"
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Button {
                ...
                Image(systemName: self.didCopy ? "checkmark" : "doc.on.doc")
                    .font(OperatorType.subheadSemiBold)
                    .foregroundStyle(
                        self.didCopy ? OperatorBrand.activationPrimaryAction : Color.secondary.opacity(0.56))
        ...
        .foregroundStyle(OperatorBrand.activationPrimaryAction)
        ...
        .overlay {
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .stroke(OperatorBrand.activationNeutralStroke, lineWidth: 0.5)
        }
    }
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                VStack(spacing: 36) {
                            ...
                            if self.isConnecting {
                                HStack(spacing: 8) {
                                    ProgressView()
                                        ...
                                        .tint(OperatorBrand.activationPrimaryActionText)
                                    Text("Connecting…")
                                        .font(OperatorType.subheadSemiBold)
                                }
                            } else {
                    ...
                    VStack(spacing: 14) {
                        ...
                        .buttonStyle(OperatorSecondaryActionButtonStyle(height: 54, shadowOpacity: 0.018))
                        .disabled(self.isConnecting)
                    }
                }
                .padding(.top, 46)
        ...
        OnboardingActivationCanvas {
            VStack(spacing: 0) {
                ...
                ZStack(alignment: .bottomTrailing) {
                    OperatorActivationGlyph(size: 86, interactive: true)
                        .shadow(color: OperatorBrand.activationGlow.opacity(0.18), radius: 12, x: 0, y: 6)
                    ...
                    Image(systemName: "checkmark")
                        ...
                        .background {
                            Circle()
                                .fill(OperatorBrand.ok)
                        }
                        .overlay {
                            Circle()
                                .stroke(OperatorBrand.activationCanvas, lineWidth: 3)
                        }
                }
                .padding(.bottom, 22)
...
struct OnboardingModeIcon: View {
    ...
    var body: some View {
        Image(systemName: self.symbol)
            .font(OperatorType.subheadSemiBold)
            .foregroundStyle(self.selected ? OperatorBrand.activationPrimaryActionText : .secondary)
            .frame(width: 34, height: 34)
            .background {
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .fill(self.selected ? OperatorBrand.activationPrimaryGradient : OperatorBrand
                        .activationNeutralGradient)
                    .shadow(
                        color: self.selected ? OperatorBrand.activationGlow.opacity(0.18) : .clear,
                        radius: 5,
                        x: 0,
                        y: 2)
            }
            .overlay {
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .stroke(
                        self.selected ? Color.white.opacity(0.30) : OperatorBrand.activationNeutralStroke,
                        lineWidth: 0.5)
            }
    }
}
