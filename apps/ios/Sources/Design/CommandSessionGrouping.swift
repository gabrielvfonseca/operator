...
struct CommandSessionSection: Identifiable {
    ...
    let id: ID
    let title: String
    let entries: [OperatorChatSessionEntry]
    let showsHeader: Bool
}

enum CommandSessionGrouping {
    static func sections(
        from entries: [OperatorChatSessionEntry],
    ...
    static func previewOrder(_ entries: [OperatorChatSessionEntry]) -> [OperatorChatSessionEntry] {
    ...
    static func previewSelection(
        _ entries: [OperatorChatSessionEntry],
        currentKey: String,
        limit: Int = 3) -> [OperatorChatSessionEntry]
    {
    ...
    static func members(
        of group: String,
        in lists: [[OperatorChatSessionEntry]]) -> [OperatorChatSessionEntry]
    {
    ...
    static func activityTimestamp(_ entry: OperatorChatSessionEntry) -> Double {
        entry.lastActivityAt ?? entry.updatedAt ?? 0
    }

    private static func sortedByActivity(
        _ entries: [OperatorChatSessionEntry]) -> [OperatorChatSessionEntry]
    {
        entries.sorted { lhs, rhs in
            let left = self.activityTimestamp(lhs)
            let right = self.activityTimestamp(rhs)
