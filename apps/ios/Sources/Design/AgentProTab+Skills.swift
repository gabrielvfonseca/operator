...
extension AgentProTab {
    ...
    var clawHubSearchCard: some View {
        ProCard(radius: AgentLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 10) {
                    ProIconBadge(systemName: "square.and.arrow.down", color: OperatorBrand.accent)
    ...
    var missingSkillEditorSheet: some View {
        NavigationStack {
            ZStack {
                ...
                .padding(OperatorSpacing.space6)
            }
        ...
        ProCard(radius: AgentLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if let message = self.skillConfigMessages[skill.effectiveSkillKey] {
                    Text(message.text)
                        .font(OperatorType.caption2)
                        .foregroundStyle(message.kind == .success ? OperatorBrand.accent : OperatorBrand.warn)
                }
            }
        }
        .padding(.horizontal, OperatorProMetric.pagePadding)
        ...
        Button {
            ...
            HStack {
                Text(title)
                    .font(OperatorType.subhead)
                Spacer(minLength: 8)
                OperatorToggleIndicator(isOn: isOn)
            }
    ...
    func skillStatus(_ skill: SkillStatusEntryLite) -> (text: String, color: Color) {
        ...
        if skill.hasMissingRequirements {
            return ("setup", OperatorBrand.warn)
        }
        return ("enabled", OperatorBrand.accent)
    }
