...
struct AgentProDreamingDestination: View {
    ...
    private var header: some View {
        if let headerLeadingAction {
            OperatorAdaptiveHeaderRow(
                ...
                subtitleFont: OperatorType.subheadMedium)
            {
            ...
            ProCard(padding: 0) {
                if let diary = self.overview?.dreamDiary {
                    if diary.found, let content = self.normalizedMultiline(diary.content) {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            HStack {
                                ProIconBadge(systemName: "book.pages", color: OperatorBrand.accent)
    ...
    private func dreamingEntriesList(
        ...
        emptyTitle: OperatorTextValue,
        emptyDetail: OperatorTextValue) -> some View
    {
        VStack(alignment: .leading, spacing: 8) {
    ...
    private func dreamingEntryRow(_ entry: DreamingEntryLite) -> some View {
        HStack(alignment: .top, spacing: 12) {
            ProIconBadge(systemName: "text.page", color: OperatorBrand.accent)
    ...
    private func dreamingPhaseRow(_ phase: DreamingPhaseRow) -> some View {
        HStack(alignment: .top, spacing: 12) {
            ProIconBadge(
                ...
                color: phase.status.enabled == false ? .secondary : OperatorBrand.accent)
            VStack(alignment: .leading, spacing: 4) {
                Text(phase.title)
                    .font(OperatorType.subheadSemiBold)
            ...
            Text(self.dreamingPhaseState(phase.status))
                .font(OperatorType.caption2SemiBold)
                .foregroundStyle(phase.status.managedCronPresent == true ? OperatorBrand.accent : .secondary)
                .lineLimit(1)
        }
        .padding(.vertical, 10)
        .padding(.horizontal, 14)
    ...
    private func emptyDetailRow(
        icon: String,
        title: OperatorTextValue,
        detail: OperatorTextValue) -> some View
    {
        HStack(spacing: 12) {
    ...
    private static func dreamDiaryInnerContent(_ content: String) -> String {
        let start = "<!-- operator:dreaming:diary:start -->"
        let end = "<!-- operator:dreaming:diary:end -->"
