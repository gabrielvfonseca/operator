...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    private func batteryStatus() -> OperatorBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: OperatorBatteryState = switch device.batteryState {
        ...
        return OperatorBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> OperatorThermalStatusPayload {
        let state: OperatorThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return OperatorThermalStatusPayload(state: state)
    }

    private func storageStatus() -> OperatorStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return OperatorStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
