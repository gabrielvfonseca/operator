...
final class RemindersService: RemindersServicing {
    ...
    func list(params: OperatorRemindersListParams) async throws -> OperatorRemindersListPayload {
        ...
        let payload: [OperatorReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return OperatorReminderPayload(
        ...
        return OperatorRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OperatorRemindersAddParams) async throws -> OperatorRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = Self.date(fromDueComponents: reminder.dueDateComponents)
        let payload = OperatorReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OperatorRemindersAddPayload(reminder: payload)
    }
