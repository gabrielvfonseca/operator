...
Operator iOS is the officially released iPhone app. It connects to an Operator Gateway as a `role: node` for chat, voice, approvals, sharing, and device-aware automation.
...
- `xcodegen`
- `fastlane`
- Apple account signed into Xcode for the canonical Operator team (`FWJYW4S8P8`)
- Fastlane Apple Developer Portal session for the canonical Operator team when creating bundle IDs or enabling services
- Release-owner access to the encrypted signing repo password (`MATCH_PASSWORD`)
- App Store Connect app already created for `ai.operatorfoundation.app`
...
- Local development uses the canonical `ai.operatorfoundation.app*` bundle IDs when the Operator team is available, and unique `ai.operatorfoundation.app.test.*` bundle IDs only for non-canonical fallback teams.
- App Store release uses canonical `ai.operatorfoundation.app*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/AppStoreRelease.xcconfig`.
...
- App Store release also switches the app to `OperatorPushMode=appStore`, which derives relay transport, official distribution, the canonical production relay, production APNs, production relay profile, `appleStrict` proof, and the App-Attest-capable entitlement file.
...
- App Store release builds use the canonical hosted relay at `https://ios-push-relay.operator.ai`.
...
3. Confirm the App Store Connect app and Apple Developer identifiers/capabilities exist for:
   - `ai.operatorfoundation.app`
   - `ai.operatorfoundation.app.share`
   - `ai.operatorfoundation.app.activitywidget`
   - `ai.operatorfoundation.app.watchkitapp`
...
8. Expected outputs after a successful run:
   - `apps/ios/build/app-store/Operator-<version>.ipa`
   - `apps/ios/build/app-store/Operator-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/Operator.entitlements` derives `aps-environment` from the active build configuration/signing override.
- App Attest relay builds use `apps/ios/Sources/OperatorAppAttest.entitlements`; local/direct builds do not require App Attest provisioning.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual Debug builds default to `OperatorPushMode=localSandbox`, direct APNs registration, and a development `aps-environment` entitlement. Local/manual Release builds default to `OperatorPushMode=localProduction` and direct production APNs registration.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `OPERATOR_APNS_TEAM_ID`, `OPERATOR_APNS_KEY_ID`, and either `OPERATOR_APNS_PRIVATE_KEY_P8` or `OPERATOR_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.operator/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `OPERATOR_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to sandbox APNs through `OperatorPushMode=localSandbox`; Release builds default to production APNs through `OperatorPushMode=localProduction`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `operator.json`. `OPERATOR_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Operator official builds and to ensure a
