...
fun requireOperatorAndroidVersionProperty(name: String): String =
...
val openClawAndroidVersionName = requireOperatorAndroidVersionProperty("OPERATOR_ANDROID_VERSION_NAME")
val openClawAndroidVersionCode =
  requireOperatorAndroidVersionProperty("OPERATOR_ANDROID_VERSION_CODE").toIntOrNull()
    ?: error("OPERATOR_ANDROID_VERSION_CODE must be an integer in Config/Version.properties.")

fun optionalOperatorBuildProperty(name: String): String? =
  providers
    .gradleProperty(name)
    .orNull
    ?.trim()
...
val repositoryBuildCommit =
  if (explicitOperatorBuildCommit == null) {
    runCatching {
      providers
        .exec {
...
val openClawBuildCommit = explicitOperatorBuildCommit ?: repositoryBuildCommit ?: "unknown"
...
val openClawBuildTimestamp = explicitOperatorBuildTimestamp ?: invocationBuildTimestamp

val androidStoreFile = providers.gradleProperty("OPERATOR_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("OPERATOR_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("OPERATOR_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("OPERATOR_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
val missingAndroidBuildMetadata =
  explicitOperatorBuildCommit == null || explicitOperatorBuildTimestamp == null

if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
  error(
    "Missing Android release signing properties. Set OPERATOR_ANDROID_STORE_FILE, " +
      "OPERATOR_ANDROID_STORE_PASSWORD, OPERATOR_ANDROID_KEY_ALIAS, and " +
      "OPERATOR_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
  )
}
...
android {
  namespace = "com.gabrielvfonseca.operator.app"
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/OperatorKit/Sources/OperatorKit/Resources")
      assets.directories.add(thirdPartyLicensesDir.path)
    }
  }

  defaultConfig {
    applicationId = "com.gabrielvfonseca.operator.app"
    minSdk = 31
    targetSdk = 36
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "operator-$versionName-$buildType.apk"
          } else {
            "operator-$versionName-$flavorName-$buildType.apk"
          }
...
val validateOperatorReleaseBuildMetadata =
  tasks.register("validateOperatorReleaseBuildMetadata") {
    doLast {
...
androidComponents {
  onVariants(selector().withBuildType("release")) { variant ->
    ...
    tasks.matching { task -> task.name == preBuildTaskName }.configureEach {
      dependsOn(validateOperatorReleaseBuildMetadata)
    }
