...
class VoiceWakePreferencesTest {
  ...
  fun matcherRequiresWordBoundariesAndCommand() {
    assertNull(VoiceWakePhraseMatcher.match("reoperator show status", listOf("operator")))
    assertNull(VoiceWakePhraseMatcher.match("operator", listOf("operator")))
    assertNull(VoiceWakePhraseMatcher.match("tell operator show status", listOf("operator")))
    assertEquals(
      VoiceWakeMatch(trigger = "operator", command = "show status"),
      VoiceWakePhraseMatcher.match("Hey operator, show status", listOf("operator")),
    )
  }
