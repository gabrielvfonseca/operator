# fastlane setup (Operator Android)
...
Create a Google Play service account JSON key with Google Play Developer API access, then grant that service account access to the Operator app in Play Console.
...
```bash
GOOGLE_PLAY_PACKAGE_NAME=ai.operator.app
```
...
OPERATOR_ANDROID_UPLOAD_KEYSTORE=<path-to-upload-keystore.jks> \
OPERATOR_ANDROID_SIGNING_PROPERTIES=<path-to-android-signing.properties> \
...
```properties
OPERATOR_ANDROID_STORE_PASSWORD=<store-password>
OPERATOR_ANDROID_KEY_ALIAS=<upload-key-alias>
OPERATOR_ANDROID_KEY_PASSWORD=<key-password>
```
...
The script creates and boots a retained `Operator_Screenshots_API36` AVD from
...
- Stable GitHub Release APK publication is separate from Google Play: `Operator Release Publish` dispatches `.github/workflows/android-release.yml`, whose protected `android-release` environment provides `MATCH_PASSWORD`; the repository GitHub App reads the encrypted signing repo.
