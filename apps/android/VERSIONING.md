...
```text
refs/operator/mobile-releases/android/<versionName>-<versionCode>
```

Example:

```text
refs/operator/mobile-releases/android/2026.6.10-2026061008
```
...
core Operator release machinery.
...
If `MATCH_PASSWORD` is not set, the existing manual Gradle-property signing path still works: provide `OPERATOR_ANDROID_STORE_FILE`, `OPERATOR_ANDROID_STORE_PASSWORD`, `OPERATOR_ANDROID_KEY_ALIAS`, and `OPERATOR_ANDROID_KEY_PASSWORD` through your local Gradle user properties before running release tasks.
