...
PACKAGE_NAME="com.gabrielvfonseca.operator.app"
RECEIVER="$PACKAGE_NAME/.VoiceE2eReceiver"
RUN_ACTION="com.gabrielvfonseca.operator.app.debug.RUN_VOICE_E2E"
OPEN_ACTION="com.gabrielvfonseca.operator.app.debug.OPEN_VOICE_E2E"
PORT=18789
HOST="127.0.0.1"
MODE="both"
...
ARTIFACT_DIR="/tmp/operator-android-voice-e2e-$(date +%Y%m%d-%H%M%S)"
...
if [[ "$START_GATEWAY" -eq 1 ]]; then
  if command -v bws_get_secret >/dev/null 2>&1; then
    OPERATOR_OPENAI_API_KEY="$(bws_get_secret OPERATOR_OPENAI_API_KEY)"
  else
    OPERATOR_OPENAI_API_KEY="$(zsh -ic 'bws_get_secret OPERATOR_OPENAI_API_KEY')"
  fi
  (
    cd "$ROOT_DIR"
...
adb logcat -d -v time |
  rg -i 'Operator|TalkMode|MicCapture|AudioRecord|SpeechRecognizer|realtime|talk.session|appendAudio|transcript|Talk failed|Transcription failed|Speech network|VoiceE2E' |
