...
PACKAGE="com.gabrielvfonseca.operator.app"
...
Default package/activity target Operator Android startup.
EOF
}
...
if [[ -z "$OUTPUT_PERF_DATA" ]]; then
  OUTPUT_PERF_DATA="/tmp/operator-startup-$(date +%Y%m%d-%H%M%S).perf.data"
fi
...
tmp_dir="$(mktemp -d -t operator-android-hotspots.XXXXXX)"
