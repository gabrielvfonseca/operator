// Operator SDK helper module supports normalize behavior.
import type { GatewayEvent, JsonObject, OperatorEvent, OperatorEventType } from "./types.js";
...
function normalizeLifecycleEndEventType(data: JsonObject): OperatorEventType {
  const status = readLowerString(data.status);
  const stopReason = readLowerString(data.stopReason);
...
function normalizeAgentEventType(payload: JsonObject): OperatorEventType {
...
function normalizeNamedEventType(event: GatewayEvent): OperatorEventType {
...
export function normalizeGatewayEvent(event: GatewayEvent): OperatorEvent {
  const payload = asRecord(event.payload);
