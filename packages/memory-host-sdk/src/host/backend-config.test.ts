...
import type { operatorConfig } from "./config-utils.js";
...
const rootMemoryConfig = (workspaceDir: string): operatorConfig =>
  ({
    agents: {
...
describe("resolveMemoryBackendConfig", () => {
  it("defaults to builtin backend when config missing", () => {
    const cfg = { agents: { defaults: { workspace: "/tmp/memory-test" } } } as operatorConfig;
  ...
  it("preserves unquoted Windows absolute qmd command paths", () => {
    ...
    const cfg = {
      agents: { defaults: { workspace: String.raw`C:\Users\penny\.operator\workspace` } },
      memory: {
        backend: "qmd",
        qmd: {
          command,
