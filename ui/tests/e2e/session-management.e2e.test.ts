...
const collapsedSessionSectionsStorageKey = "operator:sidebar:sessions:collapsed-sections";
...
describeControlUiE2e("Control UI session management mocked Gateway E2E", () => {
  beforeAll(async () => {
    if (!chromiumAvailable) {
      throw new Error(
        `Playwright Chromium is not installed or cannot start at ${chromiumExecutablePath}. Run \`pnpm --dir ui exec playwright install --with-deps chromium\`, or set OPERATOR_UI_E2E_ALLOW_MISSING_CHROMIUM=1 only when intentionally skipping this lane.`,
      );
    }
  ...
  it("archives a session from the Sessions page context menu and kebab", async () => {
    ...
    try {
      ...
      const menuHost = page.locator("operator-session-menu");
      await menuHost
  ...
  it("keeps a session row when the Gateway reports no deletion", async () => {
    ...
    try {
      ...
      await activateMenuItem(
        page.locator("operator-session-menu").getByRole("menuitem", { name: "Delete…" }),
      );
      ...
      return [
        ...
        sessionRow(
          ...
          withWork
            ? {
                execNode: nodeHash,
                worktree: { branch: "operator/wt-1", repoRoot: "/Users/dev/Projects/clawdbot" },
              }
            : {},
        ),
    ...
    const rows = [
      ...Array.from({ length: 8 }, (_, index) =>
        sessionRow(`agent:main:work-${index}`, `Work session ${index}`, baseTime - index * 60_000, {
          worktree: { branch: `operator/wt-${index}`, repoRoot: "/Users/dev/Projects/clawdbot" },
        }),
      ),
