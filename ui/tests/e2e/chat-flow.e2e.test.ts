...
describeControlUiE2e("Control UI mocked Gateway E2E", () => {
  beforeAll(async () => {
    if (!chromiumAvailable) {
      throw new Error(
        `Playwright Chromium is not installed or cannot start at ${chromiumExecutablePath}. Run \`pnpm --dir ui exec playwright install --with-deps chromium\`, set PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH to a compatible browser, or set OPERATOR_UI_E2E_ALLOW_MISSING_CHROMIUM=1 only when intentionally skipping this lane.`,
      );
    }
  ...
  it("steers ordinary follow-ups into the active run by default", async () => {
    const artifactDir = process.env.OPERATOR_UI_E2E_ARTIFACT_DIR?.trim();
    const context = await newBrowserContext({
      locale: "en-US",
      serviceWorkers: "block",
      viewport: { height: 900, width: 1280 },
  ...
  it("stores new input while offline and sends it after reconnect", async () => {
    ...
    try {
      ...
      if (process.env.OPERATOR_BEHAVIOR_PROOF === "1") {
