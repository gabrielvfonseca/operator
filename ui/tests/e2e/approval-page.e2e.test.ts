...
} from "../helpers/control-ui-e2e.ts";

const chromiumExecutablePath = resolvePlaywrightChromiumExecutablePath(chromium.executablePath());
const chromiumAvailable = canRunPlaywrightChromium(chromiumExecutablePath);
const allowMissingChromium = process.env.OPERATOR_UI_E2E_ALLOW_MISSING_CHROMIUM === "1";
...
async function waitForApprovalPage(page: Page): Promise<void> {
  await page.locator("operator-approval-page").waitFor();
...
async function expectStandaloneApprovalPage(page: Page): Promise<void> {
  expect(await page.locator("operator-approval-page").count()).toBe(1);
  expect(await page.locator(".approval-page").count()).toBe(1);
  expect(await page.locator("operator-app-shell, .shell").count()).toBe(0);
  expect(await page.locator("operator-app-topbar, operator-app-sidebar").count()).toBe(0);
  expect(await page.locator("operator-exec-approval").count()).toBe(0);
}
...
describeControlUiE2e("Control UI standalone approval page", () => {
  ...
  it("keeps one canonical winner across conflicting surfaces and terminal reload", async () => {
    ...
    try {
      ...
      expect(await mobile.page.title()).toBe("Command approval — Operator");
      ...
      expect(await mobile.page.title()).toBe("Approved here — Operator");
      await waitForStableApprovalPaint(mobile.page);
      await mobile.page.screenshot({
      ...
      expect(await mobile.page.title()).toBe("Approved — Operator");
      expect(new URL(mobile.page.url()).pathname).toBe(approvalPath(""));
  ...
  it("preserves a mounted deep link across the authentication gate", async () => {
    ...
    await surface.page.locator("operator-login-gate").waitFor();
  ...
  it("uses the page Gateway without replacing a saved remote selection", async () => {
    ...
    const selectionKey = `operator.control.currentGateway.v1:${pageGatewayScope}`;
    const pageGateway = `ws://${appUrl.hostname}:18789`;
    const pageSettingsKey = `operator.control.settings.v1:${pageGateway}`;
    ...
    const remoteSettingsKey = `operator.control.settings.v1:${remoteGateway}`;
