// Shipped apps stamp `operator-native-nav`; current apps advertise web chrome
// at document start and stamp `operator-native-web-chrome` at document end.
...
describeControlUiE2e("Control UI native-nav sidebar toggle E2E", () => {
  ...
  async function openPage(options: { nativeNav?: boolean; webChrome?: boolean; width?: number }) {
    ...
    if (options.nativeNav) {
      ...
      await page.addInitScript(() => {
        ...
        const stamp = () =>
          document.documentElement.classList.add("operator-native-macos", "operator-native-nav");
    ...
    if (options.webChrome) {
      await page.addInitScript(() => {
        const nativeWindow = window as Window & {
          ...
          __OPERATOR_NATIVE_HISTORY__?: { canGoBack: boolean; canGoForward: boolean };
        };
        nativeWindow["__OPERATOR_NATIVE_WEB_CHROME__"] = true;
        nativeWindow["__OPERATOR_NATIVE_HISTORY__"] = {
          canGoBack: false,
          canGoForward: false,
        };
        const stamp = () =>
          document.documentElement.classList.add(
            "operator-native-macos",
            "operator-native-web-chrome",
          );
        if (document.documentElement) {
          stamp();
        } else {
  ...
  it("hides both web toggles when the native titlebar toggle is present", async () => {
    ...
    await page.evaluate(() => {
      window.dispatchEvent(new CustomEvent("operator:native-toggle-sidebar"));
    });
    await expect
    ...
    await page.evaluate(() => {
      window.dispatchEvent(new CustomEvent("operator:native-open-search"));
    });
    await expect.poll(() => page.locator(".cmd-palette-overlay").isVisible()).toBe(true);

    await page.evaluate(() => {
      window.dispatchEvent(new CustomEvent("operator:native-new-session"));
    });
