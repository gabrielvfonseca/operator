...
const proofDir = process.env.OPERATOR_UI_E2E_ARTIFACT_DIR?.trim();
...
async function selectGatewayOnNextLoad(
  ...
  const settingsKey = `operator.control.settings.v1:${normalizeGatewayTokenScope(gatewayUrl)}`;
  const selectionKey =
    `operator.control.currentGateway.v1:` +
...
async function openGatewayPage(params: {
  ...
  await page.locator("operator-app-shell").waitFor();
...
describeControlUiE2e("Control UI device-token reconnect E2E", () => {
  ...
  it("isolates device tokens across gateways, origins, and revocation", async () => {
    ...
    expect(await rositaReconnect.page.locator("operator-login-gate").count()).toBe(0);
    ...
    expect(await wilfredReconnect.page.locator("operator-login-gate").count()).toBe(0);

    const identity = await wilfredSource.page.evaluate(() => {
      const raw = localStorage.getItem("operator-device-identity-v1");
