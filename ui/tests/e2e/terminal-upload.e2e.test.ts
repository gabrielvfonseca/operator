...
const expectUploadSurface = process.env.OPERATOR_TERMINAL_UPLOAD_EXPECT_PRESENT !== "0";
const screenshotPath = process.env.OPERATOR_TERMINAL_UPLOAD_SCREENSHOT?.trim();
const progressScreenshotPath = process.env.OPERATOR_TERMINAL_UPLOAD_PROGRESS_SCREENSHOT?.trim();
const errorScreenshotPath = process.env.OPERATOR_TERMINAL_UPLOAD_ERROR_SCREENSHOT?.trim();
const videoDir = process.env.OPERATOR_TERMINAL_UPLOAD_VIDEO_DIR?.trim();
...
describeControlUiE2e("Control UI terminal file upload", () => {
  ...
  it("uploads picked and dropped files, then pastes staged paths without Enter", async () => {
    ...
    await page.addInitScript(() => {
      (
        window as Window & {
          ["__OPERATOR_NATIVE_CONTROL_AUTH__"]?: { gatewayUrl: string; token: string };
        }
      )["__OPERATOR_NATIVE_CONTROL_AUTH__"] = {
