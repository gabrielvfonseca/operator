...
import { CONNECTOR_SUGGESTIONS } from "../../../src/pages/plugins/presentation.ts";
import { pluginRowKey, renderPlugins } from "../../../src/pages/plugins/view.ts";
...
describe("renderPlugins", () => {
  ...
  it("opens the detail overlay from a row and renders actions and metadata", () => {
    ...
    expect(detail.closest("operator-modal-dialog")?.getAttribute("label")).toBe("Workboard");
  ...
  it("appends live ClawHub results below the discover shelves while searching", () => {
        ...
        searchResults: [
          {
            score: 0.9,
            package: {
              name: "@gabrielvfonseca/calendar-plus",
    ...
    const result = container.querySelector<HTMLElement>(
      '[data-package-name="@gabrielvfonseca/calendar-plus"]',
    );
    ...
    expect(onInstall).toHaveBeenCalledWith(clawHubKey("@gabrielvfonseca/calendar-plus"), {
      source: "clawhub",
      packageName: "@gabrielvfonseca/calendar-plus",
    });
  });
  ...
  it("renders row-local risk acknowledgement and busy state", () => {
    const packageName = "@gabrielvfonseca/calendar-plus";
