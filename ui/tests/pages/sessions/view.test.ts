...
import { renderSessions, type SessionsProps } from "../../../src/pages/sessions/view.ts";
...
describe("sessions view", () => {
  ...
  it("assigns custom groups from the group cell and header drop targets", async () => {
    ...
    dropWithPayload(["application/x-operator-session-key"], {
      "application/x-operator-session-key": "agent:main:main",
    });
  ...
  it("uses the shared tooltip component for session filters", async () => {
    ...
    const tooltips = Array.from(
      filters?.querySelectorAll<HTMLElement>("operator-tooltip") ?? [],
