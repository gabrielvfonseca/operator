...
} from "../../../src/pages/chat/chat-gateway.ts";
import { loadChatHistory, type ChatState } from "../../../src/pages/chat/chat-history.ts";
import { readChatMessagesFromCache } from "../../../src/pages/chat/session-message-cache.ts";
...
describe("handleChatGatewayEvent", () => {
  ...
  it("keeps repeated assistant final text within the same turn", () => {
    ...
    const secondAssistant = {
      ...
      content: [
        { type: "text", text: "OK" },
        { type: "canvas", url: "/__operator__/canvas/documents/repeat/index.html" },
      ],
      timestamp: 3,
    };
    const state = createState({
...
describe("loadChatHistory filtering", () => {
  ...
  it("keeps image-only user messages that carry transcript media paths", async () => {
    const messages = [
      { role: "user", content: "", MediaPath: "/tmp/operator/user-upload.png" },
      {
        role: "user",
        content: "",
