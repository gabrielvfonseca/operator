...
describeBrowserLayout.concurrent("chat responsive browser layout", () => {
  ...
  it("reveals message context on timestamp hover and keeps click-to-open", {
    // the UI runner's cold-browser budget instead of imposing a flaky 20s cap. // This full-app case shares Chromium with concurrent layout pages; match
    timeout: 60_000,
  }, async () => {
    if (!realChatServer) {
