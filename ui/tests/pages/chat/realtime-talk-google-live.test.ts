...
import { GoogleLiveRealtimeTalkTransport } from "../../../src/pages/chat/realtime-talk-google-live.ts";
import {
  REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME,
  REALTIME_VOICE_AGENT_CONTROL_TOOL_NAME,
} from "../../../src/pages/chat/realtime-talk-shared.ts";
import type {
...
describe("GoogleLiveRealtimeTalkTransport", () => {
  ...
  it("sends spoken active-control acknowledgements through Google Live", async () => {
    ...
    expect(sent).toContainEqual({
      realtimeInput: {
        text: expect.stringContaining('Status: "Operator is working in read (running)."'),
      },
    });
    transport.stop();
  });
