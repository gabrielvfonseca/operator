...
} from "../../../../src/pages/chat/components/chat-pull-requests.ts";
...
function pullRequest(
  ...
  return {
    ...
    url: "https://github.com/gabrielvfonseca/operator/pull/103469",
    state: "open",
    additions: 4,
    deletions: 3,
    checks: { state: "passing", passed: 5, failed: 0, skipped: 1, running: 0 },
    checksUrl: "https://github.com/gabrielvfonseca/operator/pull/103469/checks",
    ...overrides,
  };
}
...
function sessionBranch(overrides: Partial<ControlUiSessionBranch> = {}): ControlUiSessionBranch {
  return {
    ...
    deletions: 205,
    createUrl:
      "https://github.com/gabrielvfonseca/operator/pull/new/claude/cloud-workers-live-events",
    ...overrides,
  };
}
...
describe("renderChatPullRequests", () => {
  ...
  it("renders an open PR chip with diff counts and CI state", () => {
    ...
    expect(chip?.querySelector(".chat-pr__repo")?.textContent).toBe("@gabrielvfonseca/operator");
    ...
    expect(chip?.querySelector(".chat-pr__link")?.getAttribute("href")).toBe(
      "https://github.com/gabrielvfonseca/operator/pull/103469",
    );
  ...
  it("shows per-state check counts and a checks link in the CI popover", () => {
    ...
    expect(menu?.querySelector<HTMLAnchorElement>("a")?.href).toBe(
      "https://github.com/gabrielvfonseca/operator/pull/103469/checks",
    );
  ...
  it("renders a Create PR branch row with locale-formatted diff stats", () => {
    ...
    expect(row?.querySelector(".chat-pr__repo")?.textContent).toBe("@gabrielvfonseca/operator");
...
describe("dismissed pull request storage", () => {
  ...
  it("ignores malformed stored payloads", () => {
    localStorage.setItem("operator.chat.dismissedPullRequests", "not json");
