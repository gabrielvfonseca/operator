...
import {
  renderMessageGroup,
  renderStreamGroup,
} from "../../../../src/pages/chat/components/chat-message.ts";
...
function createAssistantCanvasBlock(params: {
  ...
  const url = params.url ?? `/__operator__/canvas/documents/${viewId}/index.html`;
...
function clearDeleteConfirmSkip() {
  localStorageValues.delete("operator:skipDeleteConfirm");
}
...
describe("grouped chat rendering", () => {
  ...
  it.each([
    ...
    ({ preference, expected }) => {
      ...
      expect(container.querySelector("operator-tooltip")?.getAttribute("content")).toMatch(
        expected,
      );
    },
  );
  ...
  it("renders verified local assistant attachments through the authenticated media route", async () => {
    const source = `/tmp/operator/${crypto.randomUUID()} test image.png`;
    ...
    const renderMessage = () =>
      renderAssistantMessage(
        ...
        },
        {
          ...
          localMediaPreviewRoots: ["/tmp/operator"],
    ...
    const expectedMetaUrl = `/operator/__operator__/assistant-media?source=${encodeURIComponent(source).replaceAll("%20", "+")}&meta=1`;
  ...
  it("refreshes local assistant media tickets before expiry without another render", async () => {
    ...
    const source = `/tmp/operator/${crypto.randomUUID()}-refresh.png`;
    const fetchMock = vi
      .fn<
  ...
  it("rechecks local assistant media when its auth token changes", async () => {
    const source = `/tmp/operator/${crypto.randomUUID()}-auth.png`;
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      if (!url.includes("meta=1")) {
        throw new Error(`Unexpected fetch: ${url}`);
      }
  ...
  it("retries unavailable local assistant media after the retry window", async () => {
    vi.useFakeTimers();
    const source = `/tmp/operator/${crypto.randomUUID()}-retry.png`;
    const fetchMock = vi
  ...
  it("preserves same-origin assistant attachments without local preview rewriting", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        role: "assistant",
        content:
          "Inline\nMEDIA:/media/inbound/test-image.png\nMEDIA:/__operator__/media/test-doc.pdf",
        timestamp: Date.now(),
      },
      {
        showToolCalls: false,
    ...
    expect(
      container
        ...
        ?.getAttribute("href"),
    ).toBe("/__operator__/media/test-doc.pdf");
  ...
  it("renders transcript image variants and structured image blocks", async () => {
    const firstSource = `/tmp/operator/${crypto.randomUUID()}-first.png`;
    const secondSource = `/tmp/operator/${crypto.randomUUID()}-second.jpg`;
    const fetchMock = vi.fn(async (url: string, init?: RequestInit) => {
      const mediaUrl = new URL(url, "http://control.test");
      expect(mediaUrl.pathname).toBe("/operator/__operator__/assistant-media");
  ...
  it("renders canonical inbound transcript images through the authenticated media route", async () => {
    ...
    expect(
      ...
      `/operator/__operator__/assistant-media?source=${encodeURIComponent(source)}&mediaTicket=ticket-inbound`,
    );
  });
  ...
  it("expires pairing QR images and requests a refresh at the expiry boundary", async () => {
    ...
    expect(image?.getAttribute("alt")).toBe("Operator pairing QR code");
  ...
  it("fetches managed outgoing chat images with auth and requester scope", async () => {
    ...
    const fetchMock = vi.fn(async (_url: string, init?: RequestInit) => {
      ...
      expect(headers.get("x-operator-requester-session-key")).toBe("agent:main:main");
  ...
  it("routes inline canvas blocks through the scoped canvas host when available", () => {
    ...
    renderAssistantMessage(
        ...
        content: [
          { type: "text", text: "Rendered inline." },
          {
            ...
            preview: {
              ...
              url: "/__operator__/canvas/documents/cv_inline_scoped/index.html",
              preferredHeight: 320,
            },
          },
        ],
      ...
      {
        canvasPluginSurfaceUrl: "http://127.0.0.1:19003/__operator__/cap/cap_123",
      },
    );

    const iframe = container.querySelector(".chat-tool-card__preview-frame");
    expect(iframe?.getAttribute("src")).toBe(
      "http://127.0.0.1:19003/__operator__/cap/cap_123/__operator__/canvas/documents/cv_inline_scoped/index.html",
    );
  });
  ...
  it("renders server-history canvas blocks for the live toolResult sequence after history reload", () => {
        ...
        content: [
          ...
          { type: "text", text: "This item is ready." },
          {
            ...
            preview: {
              ...
              url: "/__operator__/canvas/documents/cv_canvas_live_history/index.html",
              preferredHeight: 420,
            },
            rawText: JSON.stringify({
              kind: "canvas",
    ...
    expect(iframe.getAttribute("src")).toBe(
      "/__operator__/canvas/documents/cv_canvas_live_history/index.html",
    );
  ...
  it("keeps lifted assistant canvas previews beside flat tool rows", () => {
    ...
    const iframe = expectElement(container, ".chat-tool-card__preview-frame", HTMLIFrameElement);
    expect(iframe.getAttribute("src")).toBe(
      "/__operator__/canvas/documents/cv_inline_tool_canvas/index.html",
    );
  ...
  it("renders hidden assistant_message canvas results with the configured sandbox", () => {
    ...
    expect(iframe.getAttribute("src")).toBe(
      "/__operator__/canvas/documents/cv_inline_default/index.html",
    );
  ...
  it("renders assistant_message canvas results in the assistant bubble even when tool rows are visible", () => {
        ...
        createMessageGroup(
          {
            ...
            content: JSON.stringify({
              ...
              view: {
                ...
                url: "/__operator__/canvas/documents/cv_inline_visible/index.html",
                title: "Inline demo",
    ...
    const iframe = expectElement(bubble, ".chat-tool-card__preview-frame", HTMLIFrameElement);
    expect(iframe.getAttribute("src")).toBe(
      "/__operator__/canvas/documents/cv_inline_visible/index.html",
    );
  ...
  it("adds a full-message request when opening a truncated assistant message", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        __operator: { id: "msg-truncated-1", seq: 1 },
      },
      {
        sessionKey: "global",
        agentId: "work",
  ...
  it("does not add a full-message request for non-truncated assistant messages", () => {
    ...
    renderAssistantMessage(
      container,
      {
        ...
        __operator: { id: "msg-visible-1", seq: 1 },
      },
      {
        sessionKey: "global",
        agentId: "work",
