...
import { renderWorkboard } from "../../../src/pages/workboard/view.ts";
import type { GatewayBrowserClient } from "../../api/gateway.ts";
import { getRenderedModalDialog } from "../../helpers/modal-dialog.ts";
...
describe("renderWorkboard", () => {
  ...
  it("keeps focus inside the card modal and restores focus on Escape", async () => {
    ...
    try {
      ...
      const modal = container.querySelector("operator-modal-dialog");
  ...
  it("opens a modal for new cards", () => {
    ...
    expect(container.querySelector("operator-modal-dialog")?.textContent).toContain("New card");
  ...
  it("opens an edit modal and submits card updates", async () => {
    ...
    expect(container.querySelector("operator-modal-dialog")?.textContent).toContain("Edit card");
    expect(container.querySelector("operator-modal-dialog")?.textContent).toContain(
      "Needs owner check",
    );
    ...
    render(renderWorkboard(props), container);
    expect(container.querySelector("operator-modal-dialog")).toBeNull();
    container
      .querySelector<HTMLButtonElement>('button[aria-label="Edit card"]')
      ?.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    render(renderWorkboard(props), container);
