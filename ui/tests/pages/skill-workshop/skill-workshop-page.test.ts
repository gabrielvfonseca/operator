...
} from "../../../src/pages/skill-workshop/proposals.ts";
import type {
  SkillWorkshopRouteData,
  SkillWorkshopState,
} from "../../../src/pages/skill-workshop/proposals.ts";
...
function createContext(
  ...
  return {
    ...
    config: {
      current: { assistantIdentity: { name: "Operator" } },
      subscribe,
    },
    agentSelection: {
...
describe("SkillWorkshopPage lifecycle", () => {
  it("renders revisions in the shared modal and handles modal cancellation", async () => {
    ...
    const modal = page.querySelector("operator-modal-dialog");
    ...
    await page.updateComplete;
    expect(page.querySelector("operator-modal-dialog")).toBeNull();
  });
