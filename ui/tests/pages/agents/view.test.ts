...
import { renderAgentFiles } from "../../../src/pages/agents/panels-status-files.ts";
import { renderAgents } from "../../../src/pages/agents/view.ts";
...
describe("renderAgents", () => {
  ...
  it("renders the custom agent select with the provided agents and selected label", async () => {
    ...
    try {
      ...
      const select = container.querySelector("operator-agent-select") as
        | (HTMLElement & {
...
describe("renderAgentFiles", () => {
  ...
  it("resets the expanded preview button state when the dialog closes", () => {
    ...
    const dialog = container.querySelector("operator-modal-dialog");
