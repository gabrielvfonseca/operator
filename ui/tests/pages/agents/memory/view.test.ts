...
} from "../../../../src/pages/agents/memory/view.ts";
...
describe("dreaming view", () => {
  ...
  it("shows a truncation hint when the wiki preview only contains the first chunk", async () => {
    ...
    expect(container.querySelector("operator-modal-dialog")).not.toBeNull();
    ...
    const container = renderInto(
      buildProps({
        dreamDiaryContent: [
          ...
          "*January 1, 2026*",
          "",
          "<!-- operator:dreaming:backfill-entry day=2026-01-01 source=memory/2026-01-01.md -->",
          "",
          "What Happened",
