...
import { getLobsterdex, getLobsterdexEntries } from "../../src/components/lobster-dex.ts";
import {
...
} from "../../src/components/lobster-pet.ts";
...
function createPet(seed: number, mode: LobsterPetMode = "idle"): LobsterPetElement {
  const element = document.createElement("operator-lobster-pet") as LobsterPetElement;
  element.seed = seed;
  element.mode = mode;
...
describe("lobster pet element", () => {
  ...
  it("old friends wave hello on their first arrival of the load", async () => {
    ...
    localStorage.setItem(
      "operator.control.lobsterpet.familiarity.v1",
  ...
  it("shooing is remembered in the familiarity counters", async () => {
    ...
    const raw = JSON.parse(
      localStorage.getItem("operator.control.lobsterpet.familiarity.v1") ?? "{}",
    );
  ...
  it("carries a bindle on the first load after a gateway upgrade", async () => {
    ...
    vi.setSystemTime(new Date("2026-07-09T12:00:00"));
    vi.stubGlobal("localStorage", window.localStorage);
    localStorage.setItem("operator.control.lobsterpet.gatewayVersion.v1", "2026.6.1");
    const element = createPet(42);
    ...
    expect(localStorage.getItem("operator.control.lobsterpet.gatewayVersion.v1")).toBe("2026.7.1");
  });
  ...
  it("wears the party hat on its first-visit anniversary", async () => {
    ...
    const look = createLobsterPetLook(42, new Date("2026-07-09T12:00:00"));
    localStorage.setItem(
      "operator.control.lobsterdex.v1",
      JSON.stringify({
        [look.palette.id]: {
