...
import { createApplicationContextProvider } from "../helpers/application-context.ts";
import { installDialogPolyfill } from "../helpers/modal-dialog.ts";
...
async function mountPalette(context: ApplicationContext<RouteId>) {
  ...
  const palette = document.createElement("operator-command-palette") as CommandPalette;
...
describe("CommandPalette lifecycle", () => {
  ...
  it("closes and clears its query before a retained element reconnects", async () => {
    ...
    palette.remove();
    provider.append(palette);
    const modal = palette.querySelector("operator-modal-dialog");
