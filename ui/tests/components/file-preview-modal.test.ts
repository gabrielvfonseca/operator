...
import { OperatorFilePreviewModal } from "../../src/components/file-preview-modal.ts";
...
const FILE_PREVIEW_MODAL_ELEMENT_NAME = `test-operator-file-preview-modal-${crypto.randomUUID()}`;
...
describe("operator-file-preview-modal", () => {
  beforeEach(() => {
  ...
  it("handles file navigation from the modal dialog event path", async () => {
    ...
    const dialog = modal.shadowRoot?.querySelector<HTMLElement>("operator-modal-dialog");
    const arrowDown = new KeyboardEvent("keydown", {
      key: "ArrowDown",
      bubbles: true,
      cancelable: true,
  ...
  it("rerenders default copy when the locale changes", async () => {
    ...
    expect(
      modal.shadowRoot?.querySelector<HTMLElement & { label: string }>("operator-modal-dialog")
        ?.label,
