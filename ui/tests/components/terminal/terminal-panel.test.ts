...
import type { TerminalGatewayClient } from "../../../src/components/terminal/terminal-connection.ts";
...
import { OperatorTerminalPanel } from "../../../src/components/terminal/terminal-panel.ts";
...
describe("OperatorTerminalPanel", () => {
  beforeEach(async () => {
  ...
  it("restores persisted open state when a mounted tag upgrades lazily", async () => {
    ...
    const panel = element as unknown as OperatorTerminalPanel;
  ...
  it("opens new sessions for the selected agent", async () => {
    ...
    const styleResults = Array.isArray(OperatorTerminalPanel.styles)
      ? OperatorTerminalPanel.styles
      : [OperatorTerminalPanel.styles];
  ...
  it("discards buffered startup input when open fails", async () => {
    ...
    await vi.waitFor(() => {
      const panel = document.querySelector(TERMINAL_PANEL_ELEMENT_NAME) as OperatorTerminalPanel;
