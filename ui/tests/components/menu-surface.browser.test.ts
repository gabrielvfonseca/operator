...
// popover top layer via operator-menu-surface.
//
...
describe.skipIf(!hasPopoverApi)("sidebar menu stacking", () => {
  ...
  it("paints a menu hosted in operator-menu-surface above the resizer divider", async () => {
    await useDesktopViewport();
    const { nav, divider } = mountShell();
    const surface = document.createElement("operator-menu-surface");
    const menu = createSortMenu();
    surface.append(menu);
    nav.append(surface);
