...
import {
  ...
  recordLobsterShoo,
  recordLobsterVisit,
} from "../../src/components/lobster-dex.ts";
...
describe("lobsterdex", () => {
  ...
  it("migrates v1 array entries and backfills memories on the next visit", () => {
    localStorage.setItem("operator.control.lobsterdex.v1", JSON.stringify(["crimson"]));
  ...
  it("tolerates corrupt storage", () => {
    localStorage.setItem("operator.control.lobsterdex.v1", "{not json");
    expect(getLobsterdex().size).toBe(0);
