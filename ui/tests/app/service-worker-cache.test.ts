...
describe("Control UI service worker cache versioning", () => {
  it("registers the service worker with a build id and bounds prior build caches", () => {
    ...
    expect(serviceWorkerSource).toContain(
      'const EMBEDDED_CACHE_VERSION = "__OPERATOR_CONTROL_UI_BUILD_ID__"',
    );
    ...
    expect(viteConfigSource).not.toContain(
      "OPERATOR_CONTROL_UI_BUILD_ID: JSON.stringify(controlUiBuildId)",
    );
    expect(serviceWorkerSource).not.toContain('const CACHE_NAME = "operator-control-v1"');
  });
  ...
  it("broadcasts updated versions to uncontrolled window clients during activation", async () => {
    ...
    const caches = {
      delete: cacheDelete,
      keys: vi.fn(async () => [
        "operator-control-oldest",
        "operator-control-older",
        "operator-control-previous",
        "operator-control-new-build",
        "other-cache",
      ]),
      open: vi.fn(),
    };
    ...
    expect(cacheDelete).toHaveBeenCalledWith("operator-control-oldest");
