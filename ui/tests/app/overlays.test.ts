...
import type { ApplicationGateway, ApplicationGatewaySnapshot } from "../../src/app/gateway.ts";
import { createApplicationOverlays } from "../../src/app/overlays.ts";
...
function createGatewayHarness(
  ...
  return {
    ...
    emitSystemApproval(id: string, createdAtMs: number) {
      const event: GatewayEventFrame = {
        event: "operator.approval.requested",
        payload: {
          id,
          createdAtMs,
          expiresAtMs: Date.now() + 60_000,
...
describe("application approval overlays", () => {
  it("resolves Operator changes through unified human approval", async () => {
  ...
  it("reloads pending approvals for each connected epoch", async () => {
    ...
    expect(request).toHaveBeenCalledWith("operator.approval.list", {});
