...
import { createApplicationGateway } from "../../src/app/gateway-store.ts";
import { loadSettings } from "../../src/app/settings.ts";
import type {
...
describe("createApplicationGateway reconnecting snapshot", () => {
  ...
  it("does not copy selected-remote settings into an ephemeral document Gateway", () => {
    ...
    const selectionKey = `operator.control.currentGateway.v1:${pageGateway}`;
