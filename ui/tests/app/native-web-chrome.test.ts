...
import { isNativeWebChromeHost, readNativeHistoryState } from "../../src/app/native-web-chrome.ts";
...
afterEach(() => {
  Reflect.deleteProperty(window, "__OPERATOR_NATIVE_WEB_CHROME__");
  Reflect.deleteProperty(window, "__OPERATOR_NATIVE_HISTORY__");
});
...
describe("native web chrome capability", () => {
  it("requires the document-start capability flag", () => {
    ...
    (window as TestNativeWindow)["__OPERATOR_NATIVE_WEB_CHROME__"] = true;
  ...
  it("reads native history state and defaults safely", () => {
    ...
    (window as TestNativeWindow)["__OPERATOR_NATIVE_HISTORY__"] = {
      canGoBack: true,
      canGoForward: false,
    };
