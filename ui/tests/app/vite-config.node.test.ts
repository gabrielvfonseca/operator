...
describe("Control UI Vite config", () => {
    ...
    expect(
      resolveControlUiBuildInfo({
        env: {
          ...
          OPERATOR_BUILD_TIMESTAMP: "2026-07-10T12:34:56Z",
        },
        readGitCommit,
        readGitBranch: () => null,
        readGitDirty: () => null,
    ...
    expect(
      resolveControlUiBuildInfo({
        env: {
          ...
          OPERATOR_BUILD_TIMESTAMP: "2026-07-10T13:14:15.000Z",
        },
        readGitCommit: () => "a".repeat(40),
        readPackageVersion: () => "2026.7.10",
      }).buildId,
  ...
  it("fails closed for nonempty invalid explicit build inputs", () => {
    ...
    expect(() =>
      resolveControlUiBuildInfo({
        env: { OPERATOR_BUILD_TIMESTAMP: "2026-07-10 12:34:56" },
        readGitCommit: () => "a".repeat(40),
        readPackageVersion: () => "2026.7.10",
      }),
    ).toThrow("OPERATOR_BUILD_TIMESTAMP must be a valid UTC ISO-8601 timestamp ending in Z");
  });

  it("resolves root tsconfig package aliases for source imports", () => {
    expect(findStringAlias("@gabrielvfonseca/net-policy/ip")?.replacement).toBe(
  ...
  it("resolves Control UI dev-server source aliases for internal packages", () => {
    ...
    expect(
      aliases.find((alias) => alias.find === "@gabrielvfonseca/normalization-core/string-coerce"),
    )?.toEqual({
  ...
  it("keeps specific tsconfig aliases ahead of broad package aliases", () => {
    ...
    const netPolicyIpIndex = aliases.findIndex(
      (alias) => alias.find === "@gabrielvfonseca/net-policy/ip",
    );
    const netPolicyPackageIndex = aliases.findIndex(
      (alias) => alias.find === "@gabrielvfonseca/net-policy",
    );
