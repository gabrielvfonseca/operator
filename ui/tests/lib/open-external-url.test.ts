...
import { openExternalUrlSafe, resolveSafeExternalUrl } from "../../src/lib/open-external-url.ts";
...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://operator.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://operator.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://operator.ai/abc-123", baseHref)).toBe(
      "blob:https://operator.ai/abc-123",
    );
  });
...
describe("openExternalUrlSafe", () => {
  it("nulls opener when window.open returns a proxy-like object", () => {
    ...
    const opened = openExternalUrlSafe("https://example.com/safe.png", {
      baseHref: "https://operator.ai/chat",
    });
