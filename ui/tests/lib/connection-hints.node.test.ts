...
} from "../../src/lib/connection-hints.ts";
...
describe("resolveGatewayTokenForUrlEdit", () => {
  ...
  it("loads a scoped token when the normalized gateway endpoint changes", () => {
    ...
    sessionStorage.setItem(
      "operator.control.token.v1:wss://other-gateway.example/operator",
      "other-token",
    );
    ...
    expect(
      resolveGatewayTokenForUrlEdit(
        "wss://gateway.example/operator",
        "wss://other-gateway.example/operator/",
        "abc123",
      ),
    ).toBe("other-token");
  });
  ...
  it("does not restore legacy durable tokens when the gateway endpoint changes", () => {
    ...
    localStorage.setItem(
      "operator.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://other-gateway.example/operator",
        token: "gateway-token",
      }),
    );
