...
function engineBlockedByRuntime(
  ...
  if (normalized === "@gabrielvfonseca/operator" || normalized === "pi") {
    return null;
  }
...
function renderCardActionButton(params: {
  ...
  return params.iconOnly
    ? html`<operator-tooltip .content=${params.label}>${button}</operator-tooltip>`
    : button;
}
...
function renderCardModal(props: WorkboardProps) {
  ...
  return html`
    <operator-modal-dialog
      ...
      style="--operator-modal-width: min(1120px, calc(100vw - 56px)); --operator-modal-max-height: calc(100dvh - 56px);"
    ...
    >
      ...
      >
        <div class="workboard-modal__header">
          ...
          <operator-tooltip .content=${t("common.cancel")}>
            <button
...
function renderStartExecutionButton(
  ...
  return options.iconOnly
    ? html`<operator-tooltip .content=${title}>${button}</operator-tooltip>`
    : button;
}
...
function renderCard(props: WorkboardProps, card: WorkboardCard) {
  ...
  const detailAction = html`
    <operator-tooltip .content=${t("workboard.viewDetails")}>
      <button
        class="btn btn--icon workboard-card__icon"
          ...
          <div class="workboard-toolbar__filters">
            ...
            <div class="workboard-layout-controls">
              <div class="workboard-layout-toggle" role="group" aria-label=${t("workboard.layout")}>
                <operator-tooltip .content=${t("workboard.layoutCompact")}>
                  <button
                ...
                <operator-tooltip .content=${t("workboard.layoutComfortable")}>
                  <button
