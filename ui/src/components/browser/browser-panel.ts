...
const panelLayout = createDockPanelLayout({
  storageKey: "operator.browser.panel.v1",
  minHeight: 240,
  minWidth: 380,
  defaultDock: "right",
  defaultHeight: 420,
...
/** `<operator-browser-panel>` — the dockable gateway browser surface. */
class OperatorBrowserPanel extends OperatorLitElement {
...
if (!customElements.get("operator-browser-panel")) {
  customElements.define("operator-browser-panel", OperatorBrowserPanel);
}

declare global {
  interface HTMLElementTagNameMap {
    "operator-browser-panel": OperatorBrowserPanel;
  }
}
