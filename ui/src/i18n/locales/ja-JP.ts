...
export const ja_JP: TranslationMap = {
  ...
  lazyView: {
    ...
    staleSubtitle:
      "Operator がバックグラウンドで更新されました。最新のパネルを表示するには再読み込みしてください。",
    retry: "再試行",
  },
  nodes: {
    pairing: {
      ...
      qrAlt: "Operator mobile ペアリングQRコード",
      ...
      review: "レビュー",
      waiting: "公式の Operator mobile アプリはスキャン後に自動的に接続します。",
      help: "ペアリングのヘルプ",
  ...
  worktrees: {
    ...
    title: "管理対象ワークツリー",
    subtitle: "Operator が所有する分離されたリポジトリのチェックアウト。",
    cleanNow: "今すぐクリーンアップ",
    cleanupTitle: "クリーンアップ",
    cleanupSubtitle:
      "Operator は、ディスク容量を確保するために古い worktree を定期的に削除します。クリーンアップの実行基準を調整できます。",
  ...
  configView: {
    ...
    retry: "再試行",
    applyBannerText: "operator.json に保存しました — Gateway を再起動して適用してください。",
  ...
  memoryImport: {
    ...
    confirmBackup:
      "Operator は、メモリをコピーする前に検証済みのインポート前バックアップを作成します。",
    confirmImport: "メモリをインポート",
    disconnected: "メモリをインポートするには Gateway に接続してください。",
    title: "アシスタントのメモリをインポート",
    subtitle:
      "Codex の統合メモリと Claude Code の自動メモリを確認してから、Operator にコピーします。",
    agent: "コピー先エージェント",
  ...
  pluginsPage: {
    ...
    noInstalledBody: "注目のプラグインを見つけるか、ClawHub を検索して Operator を拡張しましょう。",
    ...
    optionalCapability: "任意のOperator機能です。",
    enabled: "有効",
    disabled: "無効",
    available: "利用可能",
  ...
  skillWorkshop: {
    ...
    selfLearning: {
      ...
      pitchBody:
        "Operator は修正内容や完了した重要な実行をレビューし、このボード向けのスキル提案の下書きを作成します。追加のバックグラウンドトークンを消費し、下書きは保留中の提案として追加されます。",
      enable: "自己学習を有効にする",
      enabling: "有効化中…",
  ...
  login: {
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste:
          "operator dashboard --no-open のトークンを貼り付けるか、構成済みのパスワードを入力します。",
        stepGenerate:
          "トークンが構成されていない場合は、Gateway ホストで operator doctor --generate-gateway-token を実行します。",
      ...
      authFailed: {
        ...
        stepDashboard:
          "operator dashboard --no-open を実行し、新しい URL を開くか、そのトークンを貼り付けます。",
        stepReplace:
      ...
      pairing: {
        ...
        stepList: "Gateway ホストで operator devices list を実行します。",
        stepApproveId: "このリクエストを承認します: operator devices approve {requestId}.",
      ...
      protocol: {
        ...
        stepDashboard:
          "operator dashboard で提供元の dashboard を開き直し、UI と Gateway が同じインストールから来るようにします。",
        stepDevUi:
          "pnpm ui:dev を使用している場合は、現在の checkout に対して開発 UI を再ビルドまたは再起動します。",
        stepRestart: "Operator 更新後に Gateway を再起動し、現在のプロトコルを提供させます。",
      },
      network: {
        title: "接続できません",
        summary:
        ...
        stepGateway:
          "operator status または operator gateway run で Gateway が実行中であることを確認します。",
        stepUrl:
          "WebSocket URL を確認し、Gateway が HTTPS/Tailscale Serve の背後にある場合は wss:// を使用します。",
        stepDashboard:
          "operator dashboard --no-open で dashboard を開き直し、現在の URL と認証詳細を再コピーします。",
      },
    },
  },
  chat: {
  ...
  cron: {
    ...
    list: {
      ...
      emptyHint: "Operator に何をいつ実行させるかを記述すると、スケジュールに従って実行されます。",
    ...
    detail: {
      ...
      newSubtitle: "Operator に何をさせるかを記述し、実行するタイミングを選択します。",
      back: "すべての自動化",
      settingsTab: "設定",
      historyTitle: "実行履歴",
      generalSection: "一般",
    ...
    form: {
      ...
      promptPlaceholder: "Operator に実行させたいことを記述...",
      repeat: "繰り返し",
      repeatInterval: "間隔",
      repeatOnce: "1回",
      repeatOnExit: "終了時",
