...
type OperatorModalDialog = HTMLElement & { updateComplete: Promise<boolean> };
...
export async function getRenderedModalDialog(container: HTMLElement) {
  const modal = container.querySelector<OperatorModalDialog>("operator-modal-dialog");
  expect(modal).toBeInstanceOf(HTMLElement);
  if (!modal) {
    throw new Error("Expected operator-modal-dialog");
  }
