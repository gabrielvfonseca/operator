  ...
  const guidance =
    {
      ...
      "not-git-install":
        "Not a git checkout. Run `operator update` from the CLI for a global reinstall.",
      "not-operator-root":
      ...
      "doctor-failed": "Doctor repair failed. Run `operator doctor --non-interactive` and retry.",
...
function resolvePendingUpdateHandoffTimeoutBanner(): ApplicationStatusBanner {
  return {
    tone: "danger",
    text: "Update handoff started, but completion was not reported after reconnect. Run `operator update status` for the final result.",
  };
}
