...
} from "../../src/infra/windows-install-roots.js";
import { decodeWindowsLauncherScript } from "../../src/infra/windows-launcher-encoding.js";
import "./test-helpers/schtasks-base-mocks.js";
import type { GatewayServiceRuntime } from "../../src/daemon/service-runtime.js";
import {
  inspectPortUsage,
  killProcessTree,
  ...
  withWindowsEnv,
  writeGatewayScript,
} from "../../src/daemon/test-helpers/schtasks-fixtures.js";
...
function resolveStartupEntryPath(env: Record<string, string>, extension = "cmd") {
  const taskName = env.OPERATOR_WINDOWS_TASK_NAME ?? "Operator Gateway";
  return path.join(
...
async function writeNodeScript(env: Record<string, string>, port = "18789") {
  ...
  await fs.writeFile(
    ...
    [
      "@echo off",
      `set "OPERATOR_SERVICE_KIND=node"`,
      `set "OPERATOR_GATEWAY_PORT=${port}"`,
      `"C:\\bin\\operator.cmd" node run --host 127.0.0.1 --port ${port}`,
      "",
    ].join("\r\n"),
    "utf8",
  );
...
function makeNodeServiceEnv(env: Record<string, string>): Record<string, string> {
  return {
    ...
    OPERATOR_WINDOWS_TASK_NAME: "Operator Node",
  };
}
        ...
        stdout: JSON.stringify(
          processAlive
            ? [
                {
                  ProcessId: processId,
                  CommandLine: "C:\\bin\\operator.cmd node run --host 127.0.0.1 --port 18789",
                },
...
function expectStartupFallbackSpawn() {
  ...
  expect((options.env as Record<string, string> | undefined)?.OPERATOR_GATEWAY_PORT).toBe("18789");
...
function installGatewayScheduledTask(
  ...
  return installScheduledTask({
    ...
    environment: { OPERATOR_GATEWAY_PORT: port },
...
describe("Windows startup fallback", () => {
  it("skips task ownership probes when no Startup fallback exists", async () => {
    await withWindowsEnv("operator-win-startup-", async ({ env }) => {
  ...
  it("migrates an exact persisted wrapper that owns the replacement port", async () => {
    await withWindowsEnv("operator-win-startup-", async ({ env }) => {
      ...
      await fs.writeFile(
        ...
        [
          "@echo off",
          'set "OPERATOR_GATEWAY_PORT=18789"',
          '"C:\\bin\\operator-doppler.exe" gateway --port 18789',
          "",
        ].join("\r\n"),
        "utf8",
      );
            ...
            stdout: JSON.stringify(
              processAlive
                ? [
                    {
                      ProcessId: 4242,
                      CommandLine: '"C:\\bin\\operator-doppler.exe" gateway --port 18789',
                    },
                    { ProcessId: 9999, CommandLine: "powershell.exe" },
                  ]
                : [{ ProcessId: 9999, CommandLine: "powershell.exe" }],
        ...
        if (
          ...
          return makeSpawnSyncResult({
            stdout: JSON.stringify([
              {
                ProcessId: 3131,
                CommandLine: "C:\\manual\\operator.cmd gateway --port 18789",
              },
              {
                ProcessId: 4242,
                CommandLine:
  ...
  it("probes the old fallback port before replacing a drifted task script", async () => {
    ...
    await withWindowsEnv("operator-win-startup-", async ({ env }) => {
      ...
      env.OPERATOR_GATEWAY_PORT = "19433";
      inspectPortUsage.mockImplementation(async (port) => ({
        port,
  ...
  it("accepts a clean hidden-launcher exit when its gateway listener is running", async () => {
    await withWindowsEnv("operator-win-startup-", async ({ env }) => {
      const hiddenEnv = { ...env, OPERATOR_WINDOWS_TASK_HIDDEN_LAUNCHER: "1" };
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 4242,
                CommandLine: "C:\\manual\\operator.cmd node run --host 127.0.0.1 --port 18789",
              },
              {
                ProcessId: 5151,
                CommandLine: "C:\\bin\\operator.cmd node run --host 127.0.0.1 --port 18789",
  ...
  it("kills the Startup fallback runtime even when the CLI env omits the gateway port", async () => {
    ...
    await withWindowsEnv("operator-win-startup-", async ({ env }) => {
      ...
      delete envWithoutPort.OPERATOR_GATEWAY_PORT;
