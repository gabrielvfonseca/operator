...
import { isSystemdUnavailableDetail, renderSystemdUnavailableHints } from "../../src/daemon/systemd-hints.js";
...
describe("renderSystemdUnavailableHints", () => {
  ...
  it("renders generic Linux recovery hints outside WSL", () => {
    expect(renderSystemdUnavailableHints({ kind: "generic_unavailable" })).toEqual([
      ...
      `If you're in a container, run the gateway in the foreground instead of \`${formatCliCommand("operator gateway")}\`.`,
    ]);
  });
