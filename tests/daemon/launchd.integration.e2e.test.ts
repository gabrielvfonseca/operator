...
import type { GatewayServiceEnv } from "../../src/daemon/service-types.js";
import { resolveGatewayService, startGatewayService } from "../../src/daemon/service.js";
...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `operator-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      OPERATOR_LAUNCHD_LABEL: `ai.operator.launchd-int-${testId}`,
      OPERATOR_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
