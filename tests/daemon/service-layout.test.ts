...
import { summarizeGatewayServiceLayout } from "../../src/daemon/service-layout.js";
    ...
    expect(
      (
        await summarizeGatewayServiceLayout({
          ...
          workingDirectory: "/repo/operator",
        })
      )?.entrypoint,
    ).toBe(path.join("/repo/operator", "dist", "index.js"));
  });
    ...
    expect(
      (
        await summarizeGatewayServiceLayout({
          ...
          workingDirectory: "C:\\operator",
        })
      )?.entrypoint,
    ).toBe("C:\\operator\\dist\\index.js");
  });
