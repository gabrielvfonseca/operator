...
import {
  ...
  buildOperatorReleaseClawHubRuntimeState,
  parseOperatorReleaseClawHubPlanArgs,
} from "../scripts/lib/operator-release-clawhub-plan.ts";
import {
...
import {
  ...
  OPERATOR_PLUGIN_NPM_REPOSITORY_URL,
...
function createClawPackBytes(
  ...
  return gzipSync(
    Buffer.concat([
      ...packageJsonEntries,
      entry("package/operator.plugin.json", JSON.stringify({ id: "demo-plugin" })),
      Buffer.alloc(1024),
    ]),
  );
}
...
describe("resolveChangedClawHubPublishablePluginPackages", () => {
  const publishablePlugins: PublishablePluginPackage[] = [
    ...
    },
    {
      ...
      packageName: "@gabrielvfonseca/zalo",
...
describe("collectClawHubPublishablePluginPackages", () => {
  it("requires the ClawHub external plugin contract", () => {
    ...
    expect(() => collectClawHubPublishablePluginPackages(repoDir)).toThrow(
      "operator.compat.pluginApi is required for external code plugin packages.",
    );
  });
  ...
  it("validates only selected package names when filters are provided", () => {
    ...
    expect(
      collectClawHubPublishablePluginPackages(repoDir, {
        packageNames: ["@gabrielvfonseca/demo-plugin"],
      }).map((plugin) => plugin.packageName),
    ).toEqual(["@gabrielvfonseca/demo-plugin"]);
  });
  ...
  it("collects exact release dependencies that must match npm latest", () => {
    ...
    expect(collectClawHubPublishablePluginPackages(repoDir)).toEqual([
      expect.objectContaining({
        packageName: "@gabrielvfonseca/demo-plugin",
...
describe("Operator dual-published plugin metadata", () => {
  const dualPublishedPlugins = [
    {
      extensionId: "cohere",
      packageName: "@gabrielvfonseca/cohere-provider",
      install: {
    ...
    {
      extensionId: "gmi",
      packageName: "@gabrielvfonseca/gmi-provider",
      install: {
  ...
  it("keeps dual-published plugins selectable through both ClawHub and npm release paths", () => {
    ...
    for (const plugin of dualPublishedPlugins) {
      ...
      expect(packageJson.operator?.install).toEqual(plugin.install);
...
describe("collectClawHubVersionGateErrors", () => {
  it("requires a version bump when a publishable plugin changes", () => {
    ...
    expect(errors).toEqual([
      "@gabrielvfonseca/demo-plugin@2026.4.1: changed publishable plugin still has the same version in package.json.",
    ]);
  });
  ...
  it("does not require a version bump for the first ClawHub opt-in", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@gabrielvfonseca/demo-plugin",
          version: "2026.4.1",
          type: "module",
          repository: {
            type: "git",
            url: OPERATOR_PLUGIN_NPM_REPOSITORY_URL,
          },
          operator: {
            extensions: ["./index.ts"],
            compat: {
...
describe("collectPluginClawHubReleasePlan", () => {
  it("rejects stale required dependencies before querying ClawHub", async () => {
    ...
    await expect(
      collectPluginClawHubReleasePlan({
        rootDir: repoDir,
        selection: ["@gabrielvfonseca/demo-plugin"],
      ...
      '@gabrielvfonseca/demo-plugin@2026.4.1: demo-runtime must match npm latest for release; found "1.2.3", latest is "1.2.4".',
    );
  });
  ...
  it("accepts required dependencies matching npm latest", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@gabrielvfonseca/demo-plugin@2026.4.1": 404,
      },
    });
  ...
  it("fails closed when npm latest cannot be resolved", async () => {
    ...
    await expect(
      ...
      "@gabrielvfonseca/demo-plugin@2026.4.1: could not resolve npm latest for demo-runtime: registry unavailable",
    );
  });
  ...
  it("preserves ClawHub response details after package retries are exhausted", async () => {
    ...
    await expect(
      ...
      "Failed to query ClawHub package @gabrielvfonseca/demo-plugin: 503 Rate limit temporarily unavailable [retry-after=1; x-request-id=request-123]",
    );
  ...
  it("routes missing package rows to bootstrap candidates instead of normal candidates", async () => {
    ...
    expect(plan.bootstrapCandidates[0]).toMatchObject({
      ...
      artifactName: "clawhub-package-operator-demo-plugin-2026.4.1",
      packageName: "@gabrielvfonseca/demo-plugin",
      version: "2026.4.1",
    });
    expect(plan.missingTrustedPublisher).toStrictEqual([]);
  ...
  it("skips versions that already exist on ClawHub", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      ...
      versions: {
        "@gabrielvfonseca/demo-plugin@2026.4.1": 200,
      },
    });
  ...
  it("plans selected packages without validating unrelated publishable packages", async () => {
    ...
    expect(plan.candidates.map((plugin) => plugin.artifactName)).toEqual([
      "clawhub-package-operator-demo-plugin-2026.4.1",
    ]);
  });
});

describe("buildOperatorReleaseClawHubPlan", () => {
  it("emits a dispatch plan that keeps ClawHub children on the release tag", async () => {
    ...
    const { fetchImpl } = createClawHubPlanFetch({
      packages: {
        ...
        "@gabrielvfonseca/demo-two": {
          status: 404,
        },
        "@gabrielvfonseca/demo-three": {
          status: 200,
          body: {
            package: {},
            owner: {},
      ...
      versions: {
        "@gabrielvfonseca/demo-plugin@2026.4.1": 404,
        "@gabrielvfonseca/demo-three@2026.4.1": 404,
      },
    });

    const plan = await buildOperatorReleaseClawHubPlan(
      {
    ...
    expect(plan.normal).toEqual({
      ...
      shouldDispatch: true,
      packages: ["@gabrielvfonseca/demo-plugin"],
      inputs: {
        publish_scope: "selected",
        plugins: "@gabrielvfonseca/demo-plugin",
    ...
    expect(plan.bootstrap).toEqual({
      ...
      shouldDispatch: true,
      packages: ["@gabrielvfonseca/demo-two", "@gabrielvfonseca/demo-three"],
      inputs: {
        bootstrap_workflow_sha: "d".repeat(40),
        ref: "a".repeat(40),
        release_tag: "v2026.4.1-beta.1",
        plugins: "@gabrielvfonseca/demo-two,@gabrielvfonseca/demo-three",
    ...
    expect(plan.summary).toEqual({
      ...
      normalPlugins: "@gabrielvfonseca/demo-plugin",
      bootstrapPlugins: "@gabrielvfonseca/demo-two,@gabrielvfonseca/demo-three",
      missingTrustedPlugins: "@gabrielvfonseca/demo-three",
    });
  ...
  it("routes already-published packages missing trusted publisher config to bootstrap repair", async () => {
    ...
    expect(plan.summary).toMatchObject({
      ...
      bootstrapCount: 1,
      missingTrustedPublisherCount: 1,
      bootstrapPlugins: "@gabrielvfonseca/demo-plugin",
      missingTrustedPlugins: "@gabrielvfonseca/demo-plugin",
    });
  });
  ...
  it("requires an exact lowercase release SHA for bootstrap targeting", () => {
    ...
    expect(() => parseOperatorReleaseClawHubPlanArgs(baseArgs)).toThrow(
      "--release-sha is required.",
    );
    expect(() =>
      parseOperatorReleaseClawHubPlanArgs([...baseArgs, "--release-sha", "ABCDEF"]),
  ...
  it("requires an exact parent release run attempt for bootstrap approval binding", () => {
    ...
    expect(() => parseOperatorReleaseClawHubPlanArgs(args)).toThrow(
      "--release-publish-run-attempt is required.",
    );
    expect(() =>
      parseOperatorReleaseClawHubPlanArgs([...args, "--release-publish-run-attempt", "0"]),
...
describe("runPluginClawHubReleaseCheck", () => {
  it("rejects stale required dependencies", async () => {
    ...
    await expect(
      runPluginClawHubReleaseCheck(["--plugins", "@gabrielvfonseca/demo-plugin"], {
        rootDir: repoDir,
        resolveLatestVersion: () => "1.2.4",
      }),
    ).rejects.toThrow(
...
describe("buildOperatorReleaseClawHubRuntimeState", () => {
  it("includes the normal ClawHub run in verifier args when the release waits for it", () => {
    const state = buildOperatorReleaseClawHubRuntimeState({
      repository: "operator/operator",
      waitForClawHub: true,
    ...
    expect(state.proofLines.normal).toBe(
      "- plugin ClawHub publish: https://github.com/gabrielvfonseca/operator/actions/runs/111",
    );
  ...
  it("includes a completed bootstrap run even when there is no normal ClawHub run", () => {
    ...
    expect(state.proofLines.bootstrap).toBe(
      "- plugin ClawHub bootstrap: https://github.com/gabrielvfonseca/operator/actions/runs/222",
    );
  });
  ...
  it("skips ClawHub verification for non-awaited incomplete runs while keeping proof links", () => {
    ...
    expect(state.proofLines.normal).toBe(
      "- plugin ClawHub publish: dispatched separately, not awaited by this proof: https://github.com/gabrielvfonseca/operator/actions/runs/111",
    );
    expect(state.proofLines.bootstrap).toBe(
      "- plugin ClawHub bootstrap: dispatched separately, not awaited by this proof: https://github.com/gabrielvfonseca/operator/actions/runs/222",
    );
  });
...
if [[ "\${1:-}" == "package" && "\${2:-}" == "pack" ]]; then
  ...
  pack_path="$pack_destination/operator-demo-plugin-2026.4.1.tgz"
  printf 'fake tgz\\n' > "$pack_path"
  printf '{"path":"%s","name":"@gabrielvfonseca/demo-plugin","version":"2026.4.1"}\\n' "$pack_path"
fi
exit 0
`,
    );
...
`,
    ...
    execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          OPERATOR_CLAWHUB_MANUAL_OVERRIDE_REASON:
...
`,
    ...
    const output = execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          OPERATOR_CLAWHUB_PACK_OUTPUT_DIR: outputDir,
          PATH: `${binDir}${delimiter}${process.env.PATH ?? ""}`,
        },
      },
    );
    ...
    expect(existsSync(join(outputDir, "operator-demo-plugin-2026.4.1.tgz"))).toBe(true);
    const invocations = readFileSync(markerPath, "utf8");
  ...
  it("rejects duplicate normalized paths before invoking the ClawHub CLI", () => {
    ...
    const tgzBytes = createClawPackBytes("@gabrielvfonseca/demo-plugin", "2026.4.1", {
      ...
      execFileSync(
        ...
        [join(process.cwd(), "scripts/plugin-clawhub-publish.sh"), "--validate-packed", tgzPath],
        {
          ...
          env: {
            ...
            EXPECTED_CLAWHUB_PACKAGE_NAME: "@gabrielvfonseca/demo-plugin",
...
if [[ " $* " == *" --dry-run "* ]]; then
  printf '{"name":"@gabrielvfonseca/demo-plugin","version":"2026.4.1"}\\n'
  exit 0
fi
attempts=0
    ...
    execFileSync(
      ...
      [join(process.cwd(), "scripts/plugin-clawhub-publish.sh"), "--publish-packed", tgzPath],
      {
        ...
        env: {
          ...
          EXPECTED_CLAWHUB_PACKAGE_NAME: "@gabrielvfonseca/demo-plugin",
          EXPECTED_CLAWHUB_PACKAGE_VERSION: "2026.4.1",
          OPERATOR_CLAWHUB_PUBLISH_ATTEMPTS: "2",
          OPERATOR_CLAWHUB_PUBLISH_RETRY_DELAY_SECONDS: "1",
          PACKAGE_DIR: "extensions/demo-plugin",
          PATH: `${binDir}${delimiter}${process.env.PATH ?? ""}`,
        },
      },
    ...
    execFileSync(
      ...
      [join(process.cwd(), "scripts/plugin-clawhub-publish.sh"), "--publish-packed", tgzPath],
      {
        ...
        env: {
          ...
          EXPECTED_CLAWHUB_PACKAGE_VERSION: "2026.4.1",
          OPERATOR_CLAWHUB_PUBLISH_ATTEMPTS: "2",
          OPERATOR_CLAWHUB_PUBLISH_ATTEMPT_TIMEOUT_SECONDS: "1",
          OPERATOR_CLAWHUB_PUBLISH_RETRY_DELAY_SECONDS: "1",
          PACKAGE_DIR: "extensions/demo-plugin",
          PATH: `${binDir}${delimiter}${process.env.PATH ?? ""}`,
        },
...
function createTempPluginRepo(
  ...
  const repoDir = makeTempRepoRoot(tempDirs, "operator-clawhub-release-");
  const extensionId = options.extensionId ?? "demo-plugin";
  const extensionIds = [extensionId, ...(options.extraExtensionIds ?? [])];

  writeFileSync(
    join(repoDir, "package.json"),
    JSON.stringify({ name: "operator-test-root", type: "module" }, null, 2),
  );
  ...
  for (const currentExtensionId of extensionIds) {
    ...
    writeFileSync(
      ...
      JSON.stringify(
        {
          name: `@gabrielvfonseca/${currentExtensionId}`,
          version: "2026.4.1",
          type: "module",
          repository: {
            type: "git",
          ...
          operator: {
            ...
            install: {
              npmSpec: `@gabrielvfonseca/${currentExtensionId}`,
            },
            release: {
