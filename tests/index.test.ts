...
import { applyTemplate, runLegacyCliEntry } from "../src/index.js";
...
describe("legacy root entry", () => {
  ...
  it("does not run CLI bootstrap when imported as a library dependency", async () => {
    ...
    await runLegacyCliEntry(["@gabrielvfonseca/operator", "status"], { runCli });
    expect(runCli).toHaveBeenCalledWith(["@gabrielvfonseca/operator", "status"]);
  });
});
