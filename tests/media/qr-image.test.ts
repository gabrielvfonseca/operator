...
import { renderQrPngBase64, renderQrPngDataUrl, writeQrPngTempFile } from "../../src/media/qr-image.ts";

describe("renderQrPngBase64", () => {
  const tmpRoot = path.join(os.tmpdir(), "operator-qr-image-tests");
  ...
  it("delegates PNG rendering to qrcode", async () => {
    await expect(
      renderQrPngBase64("@gabrielvfonseca/operator", { scale: 8, marginModules: 2 }),
    ).resolves.toBe(MOCK_PNG_BASE64);
    expect(toBuffer).toHaveBeenCalledWith("@gabrielvfonseca/operator", {
      margin: 2,
      scale: 8,
    });
  });
  ...
  it("uses the default PNG rendering options", async () => {
    await renderQrPngBase64("@gabrielvfonseca/operator");
    expect(toBuffer).toHaveBeenCalledWith("@gabrielvfonseca/operator", {
      margin: 4,
      scale: 6,
    });
  ...
  it("floors finite PNG rendering options before delegating", async () => {
    await renderQrPngBase64("@gabrielvfonseca/operator", { scale: 8.9, marginModules: 2.9 });
    expect(toBuffer).toHaveBeenCalledWith("@gabrielvfonseca/operator", {
      margin: 2,
      scale: 8,
    });
  ...
  it.each([
    ...
    await expect(
      renderQrPngBase64("@gabrielvfonseca/operator", { scale, marginModules }),
    ).rejects.toThrow(message);
    expect(toBuffer).not.toHaveBeenCalled();
  });

  it("formats QR PNG data URLs", async () => {
    await expect(renderQrPngDataUrl("@gabrielvfonseca/operator")).resolves.toBe(
  ...
  it("writes QR PNGs to a scoped temp file", async () => {
    ...
    const result = await writeQrPngTempFile("@gabrielvfonseca/operator", {
      tmpRoot,
      dirPrefix: "pair-",
  ...
  it.each([
    ...
    await expect(
      writeQrPngTempFile("@gabrielvfonseca/operator", {
        tmpRoot,
