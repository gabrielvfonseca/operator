...
import { listRunningSessions } from "../../src/agents/bash-process-registry.js";
...
import { WorkerAdmissionDeadlineExceededError } from "../../src/worker/worker-connection-contract.js";
import { createWorkerConnection, WorkerConnectionStoppedError } from "../../src/worker/worker-connection.js";
import {
  WorkerInferenceProxyClient,
  WorkerLiveEventClient,
  WorkerTranscriptCommitClient,
} from "../../src/worker/worker-rpc-clients.js";
...
describe("worker runtime", () => {
  it("runs a full embedded turn through remote inference, live events, and transcript commits", async () => {
    ...
    const terminalIndex = gateway.applicationOrder.indexOf("live:lifecycle:end");
  ...
  it("executes coding tools locally without reading the preexisting auth profile", async () => {
    ...
    process.env.OPERATOR_STATE_DIR = trapStateDir;
    process.env.OPERATOR_CONFIG_PATH = configTrap;
    try {
      await expect(runWorkerDescriptor(launch)).resolves.toMatchObject({ status: "completed" });
    } finally {
      ...
      if (previousConfigPath === undefined) {
        ...
        process.env.OPERATOR_CONFIG_PATH = previousConfigPath;
      }
    }
