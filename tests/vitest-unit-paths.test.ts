...
describe("isUnitConfigTestFile", () => {
  ...
  it("rejects files excluded from the unit config", () => {
    ...
    expect(isUnitConfigTestFile("src/infra/operator-exec-env.test.ts")).toBe(false);
    expect(
      isUnitConfigTestFile(bundledPluginFile("matrix", "src/migration-snapshot.test.ts")),
    ).toBe(false);
    expect(isUnitConfigTestFile("src/infra/operator-root.test.ts")).toBe(false);
    ...
    expect(isUnitConfigTestFile("ui/tests/ui/views/channels.test.ts")).toBe(false);
    expect(isUnitConfigTestFile("ui/tests/ui/views/chat.test.ts")).toBe(false);
    expect(isUnitConfigTestFile("ui/tests/ui/views/other.test.ts")).toBe(false);
