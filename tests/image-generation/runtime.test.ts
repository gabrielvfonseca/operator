...
} from "../../src/image-generation/runtime.js";
...
describe("image-generation runtime", () => {
  ...
  it("lists runtime image-generation providers through the provider registry", () => {
    ...
    expect(
      listRuntimeImageGenerationProviders({ config: {} as OperatorConfig }, runtimeDeps),
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    await expect(
      runGenerateImage({ cfg: {} as OperatorConfig, prompt: "draw a cat" }),
    ).rejects.toThrow(
