...
import { recordPersistedRuntimeToolSchemaQuarantine } from "../../src/agents/tool-schema-quarantine-health.js";
import { resolveReadOnlyChannelPluginsForConfig } from "../../src/channels/plugins/read-only.js";
import { recordPersistedContextEngineQuarantine } from "../../src/context-engine/quarantine-health.js";
import { resetContextEngineRuntimeQuarantineForTests } from "../../src/context-engine/registry.test-support.js";
import {
  createCorePluginStateSyncKeyedStore,
  resetPluginStateStoreForTests,
} from "../../src/plugin-state/plugin-state-store.js";
import { createRuntimeHealthRecordEnvelope } from "../../src/plugin-state/runtime-health-store.js";
...
import { collectRuntimePluginHealthSnapshot } from "../../src/status/status-plugin-health.runtime.js";
...
describe("runtime plugin health snapshot", () => {
  it("includes persisted context-engine quarantines", async () => {
    await withStateDirEnv("operator-status-plugin-health-", async () => {
  ...
  it("includes persisted runtime tool-schema quarantines", async () => {
    await withStateDirEnv("operator-status-tool-quarantine-", async () => {
  ...
  it("includes core-owned runtime tool quarantines from this process", async () => {
    await withStateDirEnv("operator-status-tool-quarantine-core-", async () => {
  ...
  it("drops runtime tool quarantines from dead or unverifiable recorder processes", async () => {
    await withStateDirEnv("operator-status-tool-quarantine-liveness-", async () => {
      setActivePluginRegistry(createEmptyPluginRegistry(), "empty", "default", "/tmp/ws");
  ...
  it("drops runtime tool quarantines from a previous incarnation of this PID", async () => {
    await withStateDirEnv("operator-status-tool-quarantine-pid-reuse-", async () => {
      setActivePluginRegistry(createEmptyPluginRegistry(), "empty", "default", "/tmp/ws");
      seedPersistedToolQuarantineForTest({
  ...
  it("suppresses persisted plugin-owned runtime tool quarantines after the owner plugin is gone", async () => {
    await withStateDirEnv("operator-status-tool-quarantine-owner-", async () => {
      recordPersistedRuntimeToolSchemaQuarantine({
        toolName: "bad_tool",
        owner: "plugin:bad-tools",
        reason: "unsupported anyOf",
