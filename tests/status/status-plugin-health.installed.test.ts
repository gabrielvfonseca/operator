...
import { resolveGatewayStartupPluginActivationConfig } from "../../src/gateway/plugin-activation-runtime-config.js";
...
} from "../../src/plugins/gateway-startup-plugin-ids.js";
import { createEmptyPluginRegistry } from "../../src/plugins/registry-empty.js";
import { resetPluginRuntimeStateForTest, setActivePluginRegistry } from "../../src/plugins/runtime.js";
import { withStateDirEnv } from "../../src/test-helpers/state-dir-env.js";
import { formatDetailedPluginHealth } from "../../src/status/status-plugin-health.js";
import { collectInstalledPluginHealthSnapshot } from "../../src/status/status-plugin-health.runtime.js";
...
describe("installed plugin health should-run drift", () => {
  it("excludes deferred channel plugins and flags the not-loaded remainder as drift", async () => {
    await withStateDirEnv("operator-status-should-run-drift-", async () => {
  ...
  it("builds the plan via the shared gateway helper using source + runtime config", async () => {
    await withStateDirEnv("operator-status-should-run-source-cfg-", async () => {
      resolveReadOnlyChannelPluginsForConfigMock.mockReturnValue({
        loadFailures: [],
        missingConfiguredChannelIds: [],
      } as never);
  ...
  it("omits the should-run set entirely when no config is provided", async () => {
    await withStateDirEnv("operator-status-should-run-no-config-", async () => {
      resolveReadOnlyChannelPluginsForConfigMock.mockReturnValue({
        loadFailures: [],
        missingConfiguredChannelIds: [],
      } as never);
...
describe("installed plugin health unregistered memory embedding providers", () => {
  it("surfaces configured memory embedding providers the runtime registry does not register", async () => {
    await withStateDirEnv("operator-status-memory-embed-", async () => {
      resolveReadOnlyChannelPluginsForConfigMock.mockReturnValue({
        loadFailures: [],
        missingConfiguredChannelIds: [],
      } as never);
  ...
  it("skips the check and renders no line when no runtime registry is active", async () => {
    await withStateDirEnv("operator-status-memory-embed-no-registry-", async () => {
  ...
  it("omits the check when no config is provided", async () => {
    await withStateDirEnv("operator-status-memory-embed-no-config-", async () => {
      resolveReadOnlyChannelPluginsForConfigMock.mockReturnValue({
        loadFailures: [],
        missingConfiguredChannelIds: [],
      } as never);
