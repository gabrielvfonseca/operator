...
import { buildTrajectoryArtifacts, buildTrajectoryRunMetadata } from "../../src/trajectory/metadata.js";
...
describe("trajectory metadata", () => {
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/operator.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.operator/operator.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          OPERATOR_STATE_DIR: "/Users/tester/.operator",
        },
      ...
      expect(harness.invocation).toEqual([
        ...
        "--api-key",
        "<redacted>",
        "--config=$OPERATOR_STATE_DIR/operator.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/operator.js");
  ...
  it("captures redacted config plus active plugin and skill inventory", () => {
    ...
    registry.plugins.push({
      ...
      agentHarnessIds: ["@gabrielvfonseca/operator"],
      cliCommands: [],
      services: [],
  ...
  it("redactPathForSupport returns empty string for null/undefined input", () => {
    const ctx: SupportRedactionContext = { env: {}, stateDir: "/tmp/.operator" };
