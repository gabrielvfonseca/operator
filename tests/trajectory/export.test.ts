...
import type { Message, Usage } from "@gabrielvfonseca/operator/plugin-sdk/llm";
import { afterAll, describe, expect, it } from "vitest";
import { replaceTranscriptEvents } from "../../src/config/sessions/session-accessor.js";
...
import { exportTrajectoryBundle, resolveDefaultTrajectoryExportDir } from "../../src/trajectory/export.js";
import { TRAJECTORY_RUNTIME_FILE_MAX_BYTES, resolveTrajectoryPointerFilePath } from "../../src/trajectory/paths.js";
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "operator-trajectory-"));
let tempDirId = 0;
...
describe("exportTrajectoryBundle", () => {
  it("sanitizes session ids in default export directory names", () => {
    ...
    expect(outputDir).toBe(
      path.join(
        ...
        ".operator",
        "trajectory-exports",
        "operator-trajectory-___evil_-2026-04-22T08-00-00",
      ),
    );
  });
    ...
    fs.writeFileSync(
      ...
      `${[
        ...
        },
        {
          ...
          sessionId: "session-1",
          data: {
            harness: { type: "@gabrielvfonseca/operator", token: rawSecrets[3] },
            metadata: {
  ...
  it("uses the recorded runtime pointer before current environment overrides", async () => {
    ...
    const previous = process.env.OPERATOR_TRAJECTORY_DIR;
    process.env.OPERATOR_TRAJECTORY_DIR = envRuntimeDir;
    try {
      const bundle = await exportTrajectoryBundle({
        outputDir,
        sessionFile,
      ...
      if (previous === undefined) {
        ...
        process.env.OPERATOR_TRAJECTORY_DIR = previous;
      }
    }
  });
  ...
  it("exports merged runtime and transcript events plus convenience files", async () => {
    ...
    const runtimeEvents: TrajectoryEvent[] = [
      ...
      },
      {
        ...
        sessionId: "session-1",
        data: {
          harness: { type: "@gabrielvfonseca/operator", version: "0.1.0" },
