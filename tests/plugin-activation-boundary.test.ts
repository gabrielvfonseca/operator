...
import { normalizeModelRef } from "../src/agents/model-selection-normalize.js";
import { isStaticallyChannelConfigured } from "../src/config/channel-configured-shared.js";
import { parseBrowserMajorVersion } from "../src/plugin-sdk/browser-host-inspection.js";
    ...
    plugins: [
      {
        ...
        hooks: [],
        origin: "bundled",
        rootDir: "/tmp/operator-test-channel-fixture",
        source: "bundled",
        manifestPath: "/tmp/operator-test-channel-fixture/operator.plugin.json",
      },
    ],
  })),
);
  ...
  it("keeps generic boundaries cold and loads only narrow browser helper surfaces on use", () => {
    ...
    expect(
      isStaticallyChannelConfigured({}, "irc", {
        ...
        IRC_NICK: "@gabrielvfonseca/operator",
      }),
    ).toBe(true);
