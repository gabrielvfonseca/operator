...
import * as archive from "../../src/infra/archive.js";
import { resolveExistingInstallPath, withExtractedArchiveRoot } from "../../src/infra/install-flow.js";
import * as installSource from "../../src/infra/install-source-utils.js";

async function runExtractedArchiveFailureCase(configureArchive: () => void) {
  vi.spyOn(installSource, "withTempDir").mockImplementation(
    async (_prefix, fn) => await fn("/tmp/operator-install-flow"),
  );
  configureArchive();
...
describe("resolveExistingInstallPath", () => {
  it("returns resolved path and stat for existing files", async () => {
    await withTempDir({ prefix: "operator-install-flow-" }, async (fixtureRoot) => {
...
describe("withExtractedArchiveRoot", () => {
  ...
  it("extracts archive and passes root directory to callback", async () => {
    const tmpRoot = path.join(path.sep, "tmp", "operator-install-flow");
    ...
    expect(withTempDirCall?.[0]).toBe("operator-plugin-");
