...
import { createSafeNpmInstallArgs, createSafeNpmInstallEnv } from "../../src/infra/safe-package-install.js";
...
describe("safe npm install helpers", () => {
  ...
  it("forces project-local script-free npm install env", () => {
    const env = createSafeNpmInstallEnv(
      ...
      {
        cacheDir: "/tmp/operator-npm-cache",
        ignoreWorkspaces: true,
        legacyPeerDeps: true,
        packageLock: false,
        quiet: true,
    ...
    expect(env.npm_config_before).toBe("");
    expect(env.npm_config_cache).toBe("/tmp/operator-npm-cache");
