...
import { resolveSessionKey } from "../../src/config/sessions/session-key.js";
import { resolveCronAgentSessionKey } from "../../src/cron/isolated-agent/session-key.js";
import { resolveSessionStoreKey } from "../../src/gateway/session-store-key.js";
import { normalizeMainKey } from "../../src/routing/session-key.js";

function makeNonDefaultAgentCfg(overrides?: Partial<OperatorConfig>): OperatorConfig {
  return {
...
describe("session key write/read round-trip (#29683)", () => {
  ...
  describe("no-op when default agent is main", () => {
    it("write and gateway canonical keys match when agent is main", () => {
      const cfg = { session: { scope: "per-sender" } } as OperatorConfig;
      const mainKey = normalizeMainKey(cfg.session?.mainKey);
