...
import { loadCliDotEnv } from "../../src/cli/dotenv.js";
import {
...
import { loadDotEnv, loadWorkspaceDotEnvFile } from "../../src/infra/dotenv.js";
...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "OPERATOR_GATEWAY_SECRET",
] as const;

const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "OPERATOR_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "OPERATOR_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "OPERATOR_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "OPERATOR_BUNDLED_SKILLS_DIR=./attacker-skills",
...
describe("loadDotEnv", () => {
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "OPERATOR_STATE_DIR=./evil-state",
            "OPERATOR_CONFIG_PATH=./evil-config.json",
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "OPERATOR_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "OPERATOR_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "OPERATOR_LIVE_GEMINI_KEY=sk-gemini-live",
            "OPERATOR_LIVE_OPENAI_KEY=sk-openai-live",
            "OPERATOR_GATEWAY_TOKEN=attacker-token",
            "OPERATOR_GATEWAY_PASSWORD=attacker-password",
            "OPERATOR_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
      ...
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          ...
          [
            ...
            "OPERATOR_AGENT_DIR=./evil-agent",
            `OPERATOR_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "OPERATOR_OAUTH_DIR=./evil-oauth",
  ...
  it("blocks OPERATOR_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "OPERATOR_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.OPERATOR_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.OPERATOR_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            'OPERATOR_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );
        ...
        expect(process.env.OPERATOR_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.OPERATOR_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "OPERATOR_PINNED_PYTHON=./attacker-python",
            "OPERATOR_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.OPERATOR_PINNED_PYTHON;
        delete process.env.OPERATOR_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.OPERATOR_PINNED_PYTHON).toBeUndefined();
        expect(process.env.OPERATOR_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "OPERATOR_PINNED_PYTHON=/trusted/python",
            "OPERATOR_PINNED_WRITE_PYTHON=/trusted/write-python",
        ...
        expect(process.env.OPERATOR_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.OPERATOR_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "OPERATOR_LIVE_GEMINI_KEY=sk-gemini-live",
            "OPERATOR_LIVE_OPENAI_KEY=sk-openai-live",
            "OPERATOR_GATEWAY_TOKEN=trusted-token",
            "OPERATOR_GATEWAY_PASSWORD=trusted-password",
            "OPERATOR_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.OPERATOR_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.OPERATOR_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.OPERATOR_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.OPERATOR_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.OPERATOR_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.OPERATOR_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.OPERATOR_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "OPERATOR_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks OPERATOR_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "OPERATOR_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when OPERATOR_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        expect(process.env.FOO).toBeUndefined();
        expect(process.env.OPERATOR_STATE_DIR).toBe(customStateDir);
        expect(process.env.BAR).toBeUndefined();
      });
    });
  });
  ...
  it("blocks workspace .env takeover vars before loading the global fallback", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        expect(process.env.OPERATOR_STATE_DIR).toBe(stateDir);
        expect(process.env.OPERATOR_CONFIG_PATH).toBeUndefined();
        expect(process.env.OPERATOR_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.NODE_OPTIONS).toBeUndefined();
        expect(process.env.NODE_REDIRECT_WARNINGS).toBeUndefined();
        expect(process.env.NODE_REPL_EXTERNAL_MODULE).toBeUndefined();
        expect(process.env.NODE_REPL_HISTORY).toBeUndefined();
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "OPERATOR_MDNS_HOSTNAME",
          "OPERATOR_SESSION_CACHE_TTL_MS",
          "OPERATOR_UPDATE_PACKAGE_SPEC",
          "OPERATOR_GATEWAY_PORT",
          "OPERATOR_GATEWAY_URL",
          "OPERATOR_CLAWHUB_URL",
          "CLAWHUB_URL",
          "CLAWHUB_TOKEN",
          "CLAWHUB_AUTH_TOKEN",
          "CLAWHUB_CONFIG_PATH",
          ...
          "OPERATOR_WHATSAPP_WEB_SOCKET_URL",
          "EXAMPLE_API_HOST",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "IRC_HOST",
          ...
          "OPERATOR_SKIP_CRON",
          "OPERATOR_RAW_STREAM",
          "OPERATOR_RAW_STREAM_PATH",
          "OPERATOR_CACHE_TRACE",
          "OPERATOR_CACHE_TRACE_FILE",
          "OPERATOR_CACHE_TRACE_MESSAGES",
          "OPERATOR_CACHE_TRACE_PROMPT",
          "OPERATOR_CACHE_TRACE_SYSTEM",
          "OPERATOR_SHOW_SECRETS",
          "OPERATOR_PLUGIN_CATALOG_PATHS",
          "OPERATOR_MPM_CATALOG_PATHS",
          "OPERATOR_NODE_EXEC_HOST",
          "OPERATOR_NODE_EXEC_FALLBACK",
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("operator/operator");
