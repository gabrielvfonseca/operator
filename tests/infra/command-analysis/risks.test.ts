...
} from "../../../src/infra/command-analysis/risks.js";
...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "OPERATOR_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("operator channels login --channel whatsapp");
  });
