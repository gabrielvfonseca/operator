...
import { matchAllowlist, type ExecAllowlistEntry } from "../../src/infra/exec-approvals.js";
...
describe("exec allowlist matching", () => {
  ...
  it("does not let bare command-name patterns match path-selected executables", () => {
    const relativeResolution = {
      rawExecutable: "./rg",
      resolvedPath: "/tmp/operator-workspace/rg",
      executableName: "rg",
    };
    const absoluteResolution = {
      rawExecutable: "/tmp/operator-workspace/rg",
      resolvedPath: "/tmp/operator-workspace/rg",
      executableName: "rg",
    };
