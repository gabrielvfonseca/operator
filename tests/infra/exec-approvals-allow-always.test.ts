...
import { resolveAllowAlwaysPatternEntries } from "../../src/infra/exec-approvals-allowlist.js";
import {
  makeExecutable,
...
import { matchAllowlist } from "../../src/infra/exec-command-resolution.js";
...
describe("resolveAllowAlwaysPatterns", () => {
  ...
  it("returns direct executable paths for non-shell segments", () => {
    const exe = path.join("/tmp", "operator-tool");
  ...
  it("rejects startup shell inline payloads for allow-always and inline-chain allowlist fallback", async () => {
    ...
    const dir = makeTempDir();
    const tool = makeExecutable(dir, "operator-ok");
    const env = { PATH: `${dir}${path.delimiter}${process.env.PATH ?? ""}` };
    const safeBins = resolveSafeBins(undefined);

    for (const command of [
      `bash --login -c "operator-ok && operator-ok"`,
      `bash -i -c "operator-ok && operator-ok"`,
      `bash -lc "operator-ok && operator-ok"`,
