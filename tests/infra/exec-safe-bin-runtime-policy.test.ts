...
} from "../../src/infra/exec-safe-bin-runtime-policy.js";
import { isTrustedSafeBinPath } from "../../src/infra/exec-safe-bin-trust.js";
...
describe("exec safe-bin runtime policy", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "operator-safe-bin-trusted-symlink-" }, async (root) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "operator-safe-bin-trusted-retarget-" }, async (root) => {
        const trustedDir = path.join(root, "bin");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "operator-safe-bin-trusted-nonexec-" }, async (root) => {
        const trustedDir = path.join(root, "bin");
  ...
  it("emits runtime warning when explicitly trusted dir is writable", async () => {
    ...
    await withTempDir({ prefix: "operator-safe-bin-runtime-" }, async (dir) => {
      try {
