...
async function makeTempDir(label: string): Promise<string> {
  return await tempDirs.make(`operator-${label}-`);
}
...
describe("git commit resolution", () => {
  ...
  it("resolves commit metadata from the caller module root instead of the caller cwd", async () => {
    ...
    const repoRoot = path.join(temp, "@gabrielvfonseca/operator");
  ...
  it("does not walk out of the operator package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "@gabrielvfonseca/operator");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "@gabrielvfonseca/operator", version: "2026.3.10" }),
      "utf-8",
    );
