...
import { findGitRoot, resolveGitHeadPath } from "../../src/infra/git-root.js";
...
async function expectGitRootResolution(params: {
  ...
  await withTempDir({ prefix: `operator-${params.label}-` }, async (temp) => {
...
describe("git-root", () => {
  ...
  it("respects maxDepth traversal limit", async () => {
    await withTempDir({ prefix: "operator-git-root-depth-" }, async (temp) => {
      const repoRoot = path.join(temp, "repo");
