...
import { packNpmSpecToArchive, withTempDir } from "../../src/infra/install-source-utils.js";
import type { NpmIntegrityDriftPayload } from "../../src/infra/npm-integrity.js";
import {
  finalizeNpmSpecArchiveInstall,
  installFromNpmSpecArchiveWithInstaller,
} from "../../src/infra/npm-pack-install.js";
...
vi.mock("./install-source-utils.js", async () => {
  ...
  return {
    ...
    withTempDir: vi.fn(async (_prefix: string, fn: (tmpDir: string) => Promise<unknown>) => {
      return await fn("/tmp/operator-npm-pack-install-test");
    }),
...
describe("installFromNpmSpecArchiveWithInstaller", () => {
  const baseSpec = "@gabrielvfonseca/test@1.0.0";
  const baseArchivePath = "/tmp/operator-test.tgz";
  ...
  it("returns pack errors without invoking installer", async () => {
    ...
    expect(withTempDir).toHaveBeenCalledWith("operator-test-", expect.any(Function));
  });
  ...
  it("returns resolution metadata and installer result on success", async () => {
    mockPackedSuccess({ name: "@gabrielvfonseca/test", version: "1.0.0" });
  ...
  it("aborts when integrity drift callback rejects drift", async () => {
    ...
    expect(result).toEqual({
      ok: false,
      error: "aborted: npm package integrity drift detected for @gabrielvfonseca/test@1.0.0",
    });
    expect(installFromArchive).not.toHaveBeenCalled();
  });
  ...
  it("warns and aborts on drift when no callback is configured", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "Integrity drift detected for @gabrielvfonseca/test@1.0.0: expected sha512-old, got sha512-new",
    );
    expect(installFromArchive).not.toHaveBeenCalled();
  });
  ...
  it("rejects prerelease resolutions unless explicitly requested", async () => {
    mockPackedSuccess({
      resolvedSpec: "@gabrielvfonseca/test@latest",
      version: "1.1.0-beta.1",
    });
    const installFromArchive = vi.fn(async () => ({ ok: true as const }));

    const result = await runInstall({
      spec: "@gabrielvfonseca/test@latest",
      installFromArchive,
    });
  ...
  it("passes archive path and installer params to installFromArchive", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ok: true,
      archivePath: "/tmp/operator-plugin.tgz",
      metadata: {
        resolvedSpec: "@gabrielvfonseca/voice-call@1.0.0",
...
describe("finalizeNpmSpecArchiveInstall", () => {
  ...
  it("returns install errors unchanged", () => {
    const result = finalizeNpmSpecArchiveInstall<{ ok: true } | { ok: false; error: string }>({
      ...
      npmResolution: {
        resolvedSpec: "@gabrielvfonseca/test@1.0.0",
        integrity: "sha512-same",
