...
import { withMockedPlatform, withRestoredMocks } from "../../src/test-utils/vitest-spies.js";
import { createNpmFreshnessBypassArgs, createNpmProjectInstallEnv } from "../../src/infra/npm-install-env.js";
...
describe("npm project install env", () => {
      ...
      withRestoredMocks([existsSyncSpy], () => {
        expect(
          createNpmProjectInstallEnv(
            {
              PATH: "/tmp/operator-npm-global/bin",
            },
            {},
            FROZEN_NOW,
          ),
  ...
  it("uses a current before override for explicit npm before policy", () => {
    const dir = fsSync.mkdtempSync(path.join(os.tmpdir(), "operator-npmrc-"));
    try {
  ...
  it("uses before args for expanded npm userconfig paths", () => {
    const dir = fsSync.mkdtempSync(path.join(os.tmpdir(), "operator-home-npmrc-"));
    try {
      const baseEnv = createIsolatedNpmConfigEnv(dir);
  ...
  it("uses before args for npm default globalconfig before policies", () => {
    const dir = fsSync.mkdtempSync(path.join(os.tmpdir(), "operator-npm-prefix-"));
    try {
      const home = path.join(dir, "home");
  ...
  it("uses before args for command project npmrc before policies", () => {
    const dir = fsSync.mkdtempSync(path.join(os.tmpdir(), "operator-project-npmrc-"));
    try {
      const baseEnv = createIsolatedNpmConfigEnv(dir);
      fsSync.writeFileSync(path.join(dir, ".npmrc"), "before=2026-01-01T00:00:00.000Z\n", "utf-8");
  ...
  it("uses before args for the current project npmrc by default", () => {
    const dir = fsSync.mkdtempSync(path.join(os.tmpdir(), "operator-current-npmrc-"));
    try {
      const baseEnv = createIsolatedNpmConfigEnv(dir);
      fsSync.writeFileSync(path.join(dir, ".npmrc"), "before=2026-01-01T00:00:00.000Z\n", "utf-8");
  ...
  it("uses before args for scoped npm prefix before policies", () => {
    const dir = fsSync.mkdtempSync(path.join(os.tmpdir(), "operator-prefix-npmrc-"));
    try {
      const baseEnv = createIsolatedNpmConfigEnv(dir);
      const npmrcDir = path.join(dir, "etc");
      fsSync.mkdirSync(npmrcDir, { recursive: true });
