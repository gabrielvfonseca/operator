...
} from "../../src/infra/package-dist-inventory.js";

describe("package dist inventory", () => {
  it("tracks missing and stale dist files", async () => {
    await withTempDir({ prefix: "operator-dist-inventory-" }, async (packageRoot) => {
  ...
  it("keeps the pending install guard outside the expected inventory", async () => {
    await withTempDir({ prefix: "operator-dist-install-guard-" }, async (packageRoot) => {
  ...
  it("keeps npm-omitted dist artifacts out of the inventory", async () => {
    await withTempDir({ prefix: "operator-dist-inventory-pack-" }, async (packageRoot) => {
  ...
  it("honors package files exclusions when writing the dist inventory", async () => {
    await withTempDir({ prefix: "operator-dist-inventory-package-files-" }, async (packageRoot) => {
  ...
  it("keeps transient plugin dependency trees out of the inventory", async () => {
    await withTempDir({ prefix: "operator-dist-inventory-plugin-deps-" }, async (packageRoot) => {
  ...
  it("omits packaged extension node_modules while keeping extension runtime files", async () => {
    await withTempDir(
      { prefix: "operator-dist-inventory-extension-node-modules-" },
      async (packageRoot) => {
  ...
  it("keeps publishable externalized bundled plugin dist trees out of the inventory", async () => {
    await withTempDir({ prefix: "operator-dist-inventory-externalized-" }, async (packageRoot) => {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          name: "@gabrielvfonseca/external-chat",
          operator: {
            release: {
      ...
      await fs.writeFile(
        bundledPackageJson,
        JSON.stringify({
          name: "@gabrielvfonseca/bundled-chat",
          operator: {},
        }),
        "utf8",
      );
  ...
  it("keeps publishable core-package runtime plugin dist trees in the inventory", async () => {
    await withTempDir({ prefix: "operator-dist-inventory-core-runtime-" }, async (packageRoot) => {
      ...
      await fs.writeFile(
        corePackageJson,
        JSON.stringify({
          name: "@gabrielvfonseca/core-chat",
          operator: {
            release: {
              publishToClawHub: true,
              publishToNpm: true,
  ...
  it("matches install-stage paths case-insensitively across path segments", () => {
    expect(
      isLegacyPluginDependencyInstallStagePath(
        "dist/extensions/brave/.operator-install-stage/node_modules/typebox/package.json",
      ),
    ).toBe(true);
    expect(
      isLegacyPluginDependencyInstallStagePath(
    ...
    expect(
      isLegacyPluginDependencyInstallStagePath(
        "dist/extensions/browser/.operator-runtime-deps-copy-AbC123/package.json",
      ),
    ).toBe(false);
    expect(
      isLegacyPluginDependencyInstallStagePath("dist/extensions/.operator-install-stage"),
    ).toBe(false);
  });

  it("rejects pre-populated install-stage debris before writing an inventory", async () => {
    await withTempDir({ prefix: "operator-dist-inventory-stage-" }, async (packageRoot) => {
      for (const relativePath of [
        "dist/extensions/brave/.operator-install-stage/package.json",
  ...
  it("rejects mixed-case install-stage debris on case-sensitive builders", async () => {
    await withTempDir({ prefix: "operator-dist-inventory-stage-case-" }, async (packageRoot) => {
  ...
  it("returns null when the inventory is missing", async () => {
    await withTempDir({ prefix: "operator-dist-inventory-missing-" }, async (packageRoot) => {
  ...
  it("rejects symlinked dist entries", async () => {
    await withTempDir({ prefix: "operator-dist-inventory-symlink-" }, async (packageRoot) => {
