...
} from "../../src/infra/exec-approvals-test-helpers.js";
import type { ExecApprovalsFile } from "../../src/infra/exec-approvals.js";
...
function expectMalformedAgentAskUsesDefaults(agentAsk: unknown): void {
  ...
  expectFields(summary.ask, {
    requested: "off",
    host: "always",
    hostSource: "~/.operator/exec-approvals.json defaults.ask",
    effective: "always",
...
describe("exec approvals policy helpers", () => {
  ...
  it("explains stricter host security and ask precedence", () => {
    ...
    expectFields(summary.security, {
      ...
      host: "allowlist",
      effective: "allowlist",
      hostSource: "~/.operator/exec-approvals.json defaults.security",
    ...
    expect(summary.askFallback).toEqual({
      effective: "deny",
      source: "~/.operator/exec-approvals.json defaults.askFallback",
    });
  });
  ...
  it("uses OPERATOR_STATE_DIR when reporting default host sources", () => {
    ...
    const stateDir = path.join(process.cwd(), ".tmp-operator-state");
  ...
  it("attributes host policy to wildcard agent entries before defaults", () => {
    ...
    expectFields(summary.security, {
      host: "allowlist",
      hostSource: "~/.operator/exec-approvals.json agents.*.security",
    });
    expectFields(summary.ask, {
      host: "always",
      hostSource: "~/.operator/exec-approvals.json agents.*.ask",
    });
    expect(summary.askFallback).toEqual({
      effective: "deny",
      source: "~/.operator/exec-approvals.json agents.*.askFallback",
    });
  });
  ...
  it("avoids a duplicate default-agent scope when main only appears in approvals", () => {
    ...
    expectFields(snapshots[0]?.security, {
      host: "allowlist",
      hostSource: "~/.operator/exec-approvals.json agents.main.security",
    });
    expectFields(snapshots[0]?.ask, {
      host: "always",
      hostSource: "~/.operator/exec-approvals.json agents.main.ask",
    });
  });
