...
} from "../../../src/channels/message/types.js";
import type { ChannelOutboundAdapter } from "../../../src/channels/plugins/types.public.js";
import type { OperatorConfig } from "../../../src/config/config.js";
import type { SessionTranscriptAppendResult } from "../../../src/config/sessions/transcript.js";
import * as mediaCapabilityModule from "../../../src/media/read-capability.js";
...
import { createInternalHookEventPayload } from "../../../src/test-utils/internal-hook-event-payload.js";
import {
...
const matrixChunkConfig: OperatorConfig = {
  channels: { matrix: { textChunkLimit: 4000 } } as OperatorConfig["channels"],
};

const expectedPreferredTmpRoot = resolvePreferredOperatorTmpDir();
...
async function runChunkedMatrixDelivery(params?: {
  ...
  const cfg: OperatorConfig = {
    channels: { matrix: { textChunkLimit: 2 } } as OperatorConfig["channels"],
  };
...
describe("deliverOutboundPayloads", () => {
  ...
  it("chunks direct adapter text and preserves delivery overrides across sends", async () => {
    ...
    const results = await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 2 } } } as OperatorConfig,
      channel: "matrix",
      to: "!room",
      accountId: "default",
  ...
  it("passes delivery config and account context to adapter payload normalization", async () => {
    ...
    const cfg = { channels: { matrix: { enabled: true } } } as unknown as OperatorConfig;
    setActivePluginRegistry(
      createTestRegistry([
        {
          pluginId: "matrix",
  ...
  it("uses adapter-provided formatted senders and scoped media roots when available", async () => {
    ...
    const textResults = await deliverOutboundPayloads({
      cfg: { channels: { line: {} } } as OperatorConfig,
      channel: "line",
      to: "U123",
      accountId: "default",
    ...
    const cfg = { channels: { line: {} } } as OperatorConfig;
    await deliverOutboundPayloads({
      cfg,
      channel: "line",
      to: "U123",
  ...
  it("includes Operator tmp root in plugin mediaLocalRoots", async () => {
    const sendMatrix = vi.fn().mockResolvedValue({ messageId: "m-media", roomId: "!room" });

    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as OperatorConfig,
      channel: "matrix",
      to: "!room:example",
  ...
  it("lets explicit formatting options override configured chunking", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 4000 } } } as OperatorConfig,
      channel: "matrix",
      to: "!room",
      payloads: [{ text: "abcd" }],
  ...
  it("queues a spool copy while the live send keeps the producer's path", async () => {
    ...
    // Operator temp root that TTS actually writes to. Staging must resolve the
  ...
  it("stages agent-workspace media that only the agent-scoped capability can read", async () => {
    ...
    // Deliberately outside the Operator temp root: that root is itself a default
  ...
  it("bails out without sending when a concurrent drain already claimed the queue entry", async () => {
    // Regression for operator/operator#70386: if a reconnect or startup drain
