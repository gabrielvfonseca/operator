...
import { ensureInstallTargetAvailable, resolveCanonicalInstallTarget } from "../../src/infra/install-target.js";
...
describe("resolveCanonicalInstallTarget", () => {
  it("creates the base dir and returns early for invalid install ids", async () => {
    await withTempDir({ prefix: "operator-install-target-" }, async (root) => {
  ...
  it("returns canonical boundary errors for Error and non-Error throws", async () => {
    await withTempDir({ prefix: "operator-install-target-" }, async (baseDir) => {
