...
import { createLowDiskSpaceWarning, formatDiskSpaceBytes, tryReadDiskSpace } from "../../src/infra/disk-space.js";
...
describe("disk-space helpers", () => {
  ...
  it("reads disk space from the nearest existing ancestor", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-disk-space-"));
    try {
