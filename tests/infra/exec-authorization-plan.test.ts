...
import { analyzeArgvCommand } from "../../src/infra/exec-approvals-analysis.js";
import { planExecAuthorization, planShellAuthorization } from "../../src/infra/exec-authorization-plan.js";
...
describe("exec authorization planner", () => {
  ...
  it("keeps eval as prompt-only", async () => {
    await expectSingleShellCandidate('eval "$OPERATOR_CMD"', {
      sourceSegment: expect.objectContaining({ argv: ["eval", "$OPERATOR_CMD"] }),
