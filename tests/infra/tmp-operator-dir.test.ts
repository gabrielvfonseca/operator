// Covers preferred Operator temp directory resolution.
import { constants as fsConstants } from "node:fs";
import path from "node:path";
import { describe, expect, it, vi } from "vitest";
import { POSIX_OPERATOR_TMP_DIR, resolvePreferredOperatorTmpDir } from "../../src/infra/tmp-operator-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredOperatorTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `operator-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredOperatorTmpDir({
    ...
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_OPERATOR_TMP_DIR) {
...
describe("resolvePreferredOperatorTmpDir", () => {
  it("prefers /tmp/operator when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_OPERATOR_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/operator when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_OPERATOR_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(tmpdir).not.toHaveBeenCalled();
  ...
  it.each([
    {
      name: "falls back to os.tmpdir()/operator when /tmp/operator is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/operator when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/operator exists but is not writable",
    ...
    {
      name: "falls back when /tmp/operator is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/operator is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/operator is group/other writable",
  ...
  it("repairs existing /tmp/operator permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_OPERATOR_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_OPERATOR_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_OPERATOR_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[operator] tightened permissions on temp dir: ${POSIX_OPERATOR_TMP_DIR}`,
    );
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/operator after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_OPERATOR_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Operator temp dir/);
  });
  ...
  it("repairs existing fallback directory when permissions are too broad", () => {
    ...
    expect(chmodSync).toHaveBeenCalledWith(fallbackPath, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[operator] tightened permissions on temp dir: ${fallbackPath}`,
    );
  });

  it("uses /tmp/operator when another process tightened permissions before repair", () => {
  ...
  it("uses /tmp/operator when chmod loses a concurrent repair race", () => {
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_OPERATOR_TMP_DIR && mode === 0o700) {
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredOperatorTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_OPERATOR_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback Operator temp dir/);
  });
  ...
  it("skips the POSIX preferred path on Windows even when /tmp is accessible (#60713)", () => {
    ...
    // create it), the previous code path returned `/tmp/operator` and routed
    // log files / TTS temp files there instead of `%TEMP%\operator`. The
    // platform: "win32" branch must skip the POSIX path entirely.
    const winFallback = path.win32.join("C:\\Users\\u\\AppData\\Local\\Temp", "operator-501");
    const accessSync = vi.fn();
    const lstatSync = vi.fn((target: string) => {
      if (target === POSIX_OPERATOR_TMP_DIR || target === winFallback) {
        return secureDirStat();
      }
      throw nodeErrorWithCode("ENOENT");
    });
    ...
    const result = resolvePreferredOperatorTmpDir({
      platform: "win32",
      accessSync,
      lstatSync,
      mkdirSync,
    ...
    expect(result).not.toBe(POSIX_OPERATOR_TMP_DIR);
    expect(tmpdir).toHaveBeenCalled();
  });
  ...
  it("still uses the POSIX preferred path on non-Windows platforms when available", () => {
    ...
    expect(result).toBe(POSIX_OPERATOR_TMP_DIR);
  });
});
