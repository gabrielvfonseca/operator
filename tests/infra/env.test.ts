...
import { isTruthyEnvValue, logAcceptedEnvOption, normalizeEnv, normalizeZaiEnv } from "../../src/infra/env.js";
...
describe("logAcceptedEnvOption", () => {
  it("logs accepted env options once with redaction and formatting", async () => {
    ...
    withEnv(
      {
        ...
        OPERATOR_TEST_ENV: "  line one\nline two  ",
      },
      () => {
        logAcceptedEnvOption({
          key: "OPERATOR_TEST_ENV",
    ...
    expect(loggerMocks.info).toHaveBeenCalledWith(
      "env: OPERATOR_TEST_ENV=<redacted> (test option)",
    );
  });
    ...
    withEnv(
      {
        VITEST: "1",
        NODE_ENV: "development",
        OPERATOR_BLANK_ENV: "value",
      },
      () => {
        logAcceptedEnvOption({
          key: "OPERATOR_BLANK_ENV",
  ...
  it("keeps bounded non-secret values UTF-16 well-formed", async () => {
    withEnv(
      {
        VITEST: "",
        NODE_ENV: "development",
        OPERATOR_UTF16_TEST_ENV: `${"x".repeat(159)}🚀tail`,
      },
      () => {
        logAcceptedEnvOption({
          key: "OPERATOR_UTF16_TEST_ENV",
    ...
    expect(loggerMocks.info).toHaveBeenCalledWith(
      `env: OPERATOR_UTF16_TEST_ENV=${"x".repeat(159)}… (UTF-16 test)`,
    );
  });
});
