...
import { resetFatalErrorHooksForTest } from "../../src/infra/fatal-error-hooks.js";
import {
...
describe("installUnhandledRejectionHandler - fatal detection", () => {
  ...
  describe("fatal errors", () => {
    it("exits on fatal runtime codes", () => {
      ...
      expectConsoleLogWithMessage(
        consoleErrorSpy,
        "[operator] FATAL unhandled rejection:",
        "Out of memory",
      );
    });
  });
  ...
  describe("configuration errors", () => {
    it("uses exit 78 only for invalid configuration", () => {
      ...
      expectConsoleLogWithMessage(
        consoleErrorSpy,
        "[operator] CONFIGURATION ERROR - requires fix:",
        "Invalid config",
      );
    });
  });
  ...
  describe("non-fatal errors", () => {
    ...
    it("exits on generic errors without code", () => {
      ...
      expectConsoleLogWithMessage(
        consoleErrorSpy,
        "[operator] Unhandled promise rejection:",
    ...
    it("does not exit on AbortError and logs suppression warning", () => {
      ...
      expectConsoleLogWithMessage(
        consoleWarnSpy,
        "[operator] Suppressed AbortError:",
