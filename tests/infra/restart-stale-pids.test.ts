...
import { resolveLsofCommandSync } from "../../src/infra/ports-lsof.js";
...
function createOperatorBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "operator-gateway" }]),
    ...overrides,
  });
}
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses operator-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "operator-gateway" },
          { pid: process.pid, cmd: "operator-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: operator-weixin sidecar (child of the gateway) invoked
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "operator-gateway" },
          { pid: unrelatedStalePid, cmd: "operator-gateway" },
        ]),
        stderr: "",
      });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "operator-gateway" },
            { pid: grandparentPid, cmd: "operator-gateway" },
            { pid: benignStalePid, cmd: "operator-gateway" },
          ]),
          stderr: "",
        });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "operator-gateway" },
            { pid: gatewayGrandparentPid, cmd: "operator-gateway" },
          ]),
          stderr: "",
        });
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            ...
            status: 0,
            stdout: lsofOutput([
              { pid: toolHostPid, cmd: "operator-gateway" },
              { pid: gatewayGrandparentPid, cmd: "operator-gateway" },
              { pid: benignStalePid, cmd: "operator-gateway" },
            ]),
            stderr: "",
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: process.pid + 3152, cmd: "operator-gateway" }]),
            stderr: "",
          };
        });
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["@gabrielvfonseca/operator", "gateway"]);
        expect(findGatewayPidsOnPortSync(18789)).toEqual([stalePid]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(18789, undefined);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple operator pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "operator-gateway" },
          { pid: pid2, cmd: "operator-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "operator-gateway" }]),
          stderr: "",
        };
      });
    ...
    it("does not kill a protected gateway pid after reparenting", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        return lsofCall === 1
          ? createLsofResult({
              stdout: lsofOutput([
                { pid: protectedPid, cmd: "operator-gateway" },
                { pid: stalePid, cmd: "operator-gateway" },
              ]),
            })
          : createLsofResult({ status: 1 });
    ...
    it("treats failed Windows port probes as inconclusive, not free", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgsResult.mockReturnValue({
          ok: true,
          args: ["@gabrielvfonseca/operator", "gateway"],
        });
        mockSpawnSync.mockReturnValue({
          error: null,
          status: 0,
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          if (command === "lsof") {
            ...
            return lsofCall === 2
              ? createLsofResult({
                  stdout: lsofOutput([{ pid: gatewayParentPid, cmd: "operator-gateway" }]),
                })
              : createLsofResult({ status: 1 });
          }
          return createLsofResult();
