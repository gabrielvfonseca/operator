...
import { makeTempDir } from "../../src/infra/exec-approvals-test-helpers.js";
import {
  isSafeBinUsage,
  matchAllowlist,
...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    const prevOperatorHome = process.env.OPERATOR_HOME;

    try {
      process.env.OPERATOR_HOME = dir;
      const approvalsPath = path.join(dir, ".operator", "exec-approvals.json");
      ...
      if (prevOperatorHome === undefined) {
        delete process.env.OPERATOR_HOME;
      } else {
        process.env.OPERATOR_HOME = prevOperatorHome;
      }
    }
  });
});
