...
import { seedSessionStore } from "../../src/infra/heartbeat-runner.test-utils.js";
import {
  resolveHeartbeatDeliveryTarget,
  resolveHeartbeatDeliveryTargetWithSessionRoute,
  resolveHeartbeatSenderContext,
} from "../../src/infra/outbound/targets.js";
import { telegramMessagingForTest } from "../../src/infra/outbound/targets.test-helpers.js";
import { enqueueSystemEvent, resetSystemEventsForTest } from "../../src/infra/system-events.js";
...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "operator-heartbeat-suite-"));
});
...
describe("resolveHeartbeatPrompt", () => {
  it.each([
    { name: "default prompt", cfg: {} as OperatorConfig, expected: HEARTBEAT_PROMPT },
    {
...
describe("runHeartbeatOnce", () => {
  ...
  it.each([
    ["the heartbeat main session", (cfg: OperatorConfig) => resolveMainSessionKey(cfg)],
  ...
  it.each([
    {
      ...
      message: "Group alert",
      applyOverride: ({ cfg, sessionKey }: { cfg: OperatorConfig; sessionKey: string }) => {
  ...
  it("loads the default agent session from templated stores", async () => {
    const tmpDir = await createCaseDir("operator-hb");
...
`,
  ...
  it("keeps non-task HEARTBEAT.md context while stripping blank-line-separated task blocks", async () => {
    const tmpDir = await createCaseDir("operator-hb-tasks-context");
    const storePath = path.join(tmpDir, "sessions.json");
    const workspaceDir = path.join(tmpDir, "workspace");
    await fs.mkdir(workspaceDir, { recursive: true });
    await fs.writeFile(
...
`,
  ...
  it("strips documented unindented task entries while keeping following top-level bullets", async () => {
    const tmpDir = await createCaseDir("operator-hb-unindented-tasks-context");
    const storePath = path.join(tmpDir, "sessions.json");
    const workspaceDir = path.join(tmpDir, "workspace");
    await fs.mkdir(workspaceDir, { recursive: true });
    await fs.writeFile(
