...
import { startProxy, stopProxy, type ProxyHandle } from "../../src/infra/net/proxy/proxy-lifecycle.js";
import { appendApnsResponseBodyCapture, createApnsResponseBodyCapture } from "../../src/infra/push-apns-http2.js";
import {
  sendApnsAlert,
...
function createRelayApnsSendFixture(params: {
  ...
  return {
    ...
    relayConfig: {
      baseUrl: "https://relay.operator.test",
      timeoutMs: 2_500,
    },
    gatewayIdentity: {
...
describe("push APNs send semantics", () => {
  ...
  it("routes direct APNs HTTP/2 requests through the active managed proxy", async () => {
    ...
    try {
      ...
      expect(request?.headers["apns-topic"]).toBe("ai.operator.ios");
  ...
  it("sends exec approval alert pushes with generic modal-only metadata", async () => {
    ...
    expect(payload.aps).toEqual({
      alert: {
        ...
        body: "Open Operator to review this request.",
      },
      sound: "default",
  ...
  it("defaults background wake reason when not provided", async () => {
    ...
    const payload = requirePayload(requireSendRequest(send));
    expectRecordFields(requireRecord(payload.operator, "operator payload"), {
      kind: "node.wake",
      reason: "node.invoke",
      nodeId: "ios-node-wake-default-reason",
    });
