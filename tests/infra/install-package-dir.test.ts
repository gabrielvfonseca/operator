...
import { runCommandWithTimeout, type CommandOptions, type SpawnResult } from "../../src/process/exec.js";
import { createSuiteTempRootTracker } from "../../src/test-helpers/temp-dir.js";
import { installPackageDir } from "../../src/infra/install-package-dir.js";
...
describe("installPackageDir", () => {
  const fixtureRootTracker = createSuiteTempRootTracker({
    prefix: "operator-install-package-dir-",
  });
  ...
  it("keeps the existing install in place when staged validation fails", async () => {
    ...
    await expect(
      listMatchingDirs(installBaseDir, ".operator-install-stage-"),
    ).resolves.toHaveLength(0);
    await expect(
      listMatchingDirs(installBaseDir, ".operator-install-backups"),
    ).resolves.toHaveLength(0);
  });
  ...
  it("restores the original install if publish rename fails", async () => {
    ...
    const backupRoot = path.join(installBaseDir, ".operator-install-backups");
  ...
  it("publishes the staged install through the copy fallback when rename crosses devices", async () => {
    ...
    vi.spyOn(fs, "rename").mockImplementation(async (...args: Parameters<typeof fs.rename>) => {
      ...
      if (
        exdevMoves === 0 &&
        path.basename(fromPath).startsWith(".operator-install-stage-") &&
  ...
  it("warns and leaves the backup in place when the install base changes before backup cleanup", async () => {
    ...
    await expectMissingPath(path.join(outsideInstallRoot, "demo", "marker.txt"));
    const backupRoot = path.join(preservedInstallRoot, ".operator-install-backups");
  ...
  it("installs peer dependencies for isolated plugin package installs", async () => {
    ...
    expect(installOptions.cwd).toContain(".operator-install-stage-");
  });
  ...
  it("hides the staged project .npmrc while npm install runs and restores it afterward", async () => {
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (_argv, optionsOrTimeout) => {
      ...
      await expect(
        listMatchingEntries(cwd, ".operator-install-hidden-npmrc-"),
    ...
    await expect(
      listMatchingEntries(targetDir, ".operator-install-hidden-npmrc-"),
    ).resolves.toHaveLength(0);
  });
