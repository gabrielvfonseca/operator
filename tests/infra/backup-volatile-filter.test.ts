...
import { isVolatileBackupPath } from "../../src/infra/backup-volatile-filter.js";

const stateDir = "/opt/operator/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/operator/state/sessions/../config.jsonl` resolves to
    // `/opt/operator/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("normalizes Windows-style separators before anchor checks", () => {
    const winStateDir = "C:\\operator\\state";
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/operator/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
