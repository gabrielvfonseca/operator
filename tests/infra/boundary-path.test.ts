...
import { resolveRootPath, resolveRootPathSync } from "../../src/infra/boundary-path.js";
...
describe("resolveRootPath", () => {
  it("resolves symlink parents with non-existent leafs inside root", async () => {
    ...
    await withTempDir({ prefix: "operator-boundary-path-" }, async (base) => {
  ...
  it("maintains containment invariant across randomized alias cases", async () => {
    ...
    await withTempDir({ prefix: "operator-boundary-path-fuzz-" }, async (base) => {
      const root = path.join(base, "workspace");
      const outside = path.join(base, "outside");
