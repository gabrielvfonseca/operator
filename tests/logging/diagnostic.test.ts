...
import { withDiagnosticPhase } from "../../src/logging/diagnostic-phase.js";
import {
...
} from "../../src/logging/diagnostic-run-activity.test-support.js";
import type { SessionAttentionClassification } from "../../src/logging/diagnostic-session-attention.js";
import {
  requestStuckSessionRecovery,
  resetDiagnosticSessionRecoveryCoordinatorForTest,
} from "../../src/logging/diagnostic-session-recovery-coordinator.js";
import type { StuckSessionRecoveryOutcome } from "../../src/logging/diagnostic-session-recovery.js";
import {
...
describe("stuck session diagnostics threshold", () => {
  ...
  it("threads session files from heartbeat state into stuck-session recovery", () => {
    const recoverStuckSession = vi.fn();
    const sessionFile = "/tmp/operator-heartbeat-session.jsonl";
  ...
  it("uses diagnostics.stuckSessionAbortMs for stalled active-work recovery", () => {
    ...
    logSessionStateChange({
      sessionId: "s1",
      sessionKey: "main",
      sessionFile: "/tmp/operator-active-abort-session.jsonl",
      state: "processing",
    });
    markDiagnosticEmbeddedRunStarted({ sessionId: "s1", sessionKey: "main" });
