...
import { emitDiagnosticEvent, resetDiagnosticEventsForTest } from "../../src/infra/diagnostic-events.js";
import {
...
import { writeDiagnosticSupportExport } from "../../src/logging/diagnostic-support-export.js";
import {
  redactSupportString,
...
import type { LogTailPayload } from "../../src/logging/log-tail.js";
...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "operator.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["@gabrielvfonseca/operator", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
    ...
    const sanitizedLogs = entries["logs/operator-sanitized.jsonl"];
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/operator.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/operator-sanitized.jsonl");
