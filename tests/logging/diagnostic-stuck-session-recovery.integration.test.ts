...
import { resolveEmbeddedSessionLane } from "../../src/agents/embedded-agent-runner/lanes.js";
import {
...
import { enqueueCommandInLane, getQueueSize, resetCommandLane } from "../../src/process/command-queue.js";
import { resetCommandQueueStateForTest } from "../../src/process/command-queue.test-support.js";
import {
  testing as recoveryTesting,
  recoverStuckDiagnosticSession,
} from "../../src/logging/diagnostic-stuck-session-recovery.runtime.js";
...
describe("stuck session recovery integration", () => {
  ...
  it("does not reset sibling-key lane work while the same session file has an active embedded run", async () => {
    ...
    const sessionFile = "/tmp/operator-diagnostic-shared-session.jsonl";
