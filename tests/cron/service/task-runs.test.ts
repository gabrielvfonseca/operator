...
import { getDetachedTaskLifecycleRuntime } from "../../../src/tasks/detached-task-runtime.js";
import * as taskExecutor from "../../../src/tasks/task-executor.js";
import { finalizeTaskRunByRunId } from "../../../src/tasks/task-executor.js";
import * as taskRegistry from "../../../src/tasks/task-registry.js";
import { markTaskLostById } from "../../../src/tasks/task-registry.js";
import { listTaskRegistryRecordsByRuntimeSourceIdFromSqlite } from "../../../src/tasks/task-registry.store.sqlite.js";
import {
...
import { timeoutErrorMessage } from "../../../src/cron/service/execution-errors.js";
...
describe("cron task run terminal records", () => {
  it("persists canonical history directly when a detached runtime is registered", async () => {
    await withOperatorTestState(
      { layout: "state-only", prefix: "operator-cron-core-ledger-runtime-" },
      async () => {
        resetTaskRegistryForTests();
  ...
  it("creates an immediately terminal task row for a skipped-only event", async () => {
    await withOperatorTestState(
      { layout: "state-only", prefix: "operator-cron-skipped-task-" },
      async () => {
        resetTaskRegistryForTests();
  ...
  it("keeps same-millisecond cron executions as distinct task rows", async () => {
    await withOperatorTestState(
      { layout: "state-only", prefix: "operator-cron-distinct-task-runs-" },
      async () => {
        resetTaskRegistryForTests();
  ...
  it("keeps operator cancellation while attaching terminal run history", async () => {
    await withOperatorTestState(
      { layout: "state-only", prefix: "operator-cron-cancelled-task-" },
      async () => {
        resetTaskRegistryForTests();
  ...
  it("retries the original outcome after an empty finalization result", async () => {
    await withOperatorTestState(
      { layout: "state-only", prefix: "operator-cron-task-retry-" },
      async () => {
        resetTaskRegistryForTests();
  ...
  it("overwrites a lost canonical row with restart terminal history", async () => {
    await withOperatorTestState(
      { layout: "state-only", prefix: "operator-cron-task-lost-recovery-" },
      async () => {
        resetTaskRegistryForTests();
  ...
  it("overwrites a provisional timeout with restart terminal history", async () => {
    await withOperatorTestState(
      { layout: "state-only", prefix: "operator-cron-task-timeout-recovery-" },
      async () => {
        resetTaskRegistryForTests();
  ...
  it("recovers pre-discriminator task rows written by older releases", async () => {
    await withOperatorTestState(
      { layout: "state-only", prefix: "operator-cron-task-legacy-runid-" },
      async () => {
        resetTaskRegistryForTests();
  ...
  it("keeps suffixed recovery identities scoped to the current cron store", async () => {
    await withOperatorTestState(
      { layout: "state-only", prefix: "operator-cron-task-store-recovery-" },
      async (fixture) => {
        resetTaskRegistryForTests();
