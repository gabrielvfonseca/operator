...
import { makeIsolatedAgentJobFixture, makeIsolatedAgentParamsFixture } from "../../../src/cron/isolated-agent/job-fixtures.js";
...
describe("runCronIsolatedAgentTurn isolated session identity", () => {
  ...
  it("uses a run-scoped key for embedded isolated cron execution", async () => {
    ...
    expect(runRequest.promptCacheKey).toMatch(/^operator-cron-[a-f0-9]{32}$/u);
  ...
  it("keeps embedded isolated cron prompt-cache affinity stable across run sessions", async () => {
    ...
    expect(requests[0]?.promptCacheKey).toMatch(/^operator-cron-[a-f0-9]{32}$/u);
  });
