...
import { getCliSessionBinding } from "../../../src/config/sessions/cli-session-binding.js";
...
describe("resolveCronSession provider-owned daily reset", () => {
  ...
  it("still rotates a provider-owned session when reset is explicitly configured", () => {
    ...
    const result = resolveCronSession({
      cfg: { session: { reset: { mode: "daily" } } } as OperatorConfig,
      sessionKey,
      agentId: "main",
      nowMs: NOW_MS,
      forceNew: false,
