...
import { createMockCronStateForJobs } from "../../src/cron/service.test-harness.js";
import { listPage } from "../../src/cron/service/ops.js";
...
describe("cron listPage sort guards", () => {
  ...
  it("applies schedule and last-run status filters before paging", async () => {
    ...
    const state = createMockCronStateForJobs({ jobs });
    const storeDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-cron-list-page-"));
    try {
