...
import { resolveBundledSkillsDir } from "../../../src/skills/loading/bundled-dir.js";
...
describe("resolveBundledSkillsDir", () => {
  ...
  it("returns OPERATOR_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-bundled-override-"));
    process.env.OPERATOR_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.OPERATOR_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "operator-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "operator" }));
