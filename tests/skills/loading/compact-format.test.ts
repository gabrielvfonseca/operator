...
import { formatSkillsForPrompt as upstreamFormatSkillsForPrompt } from "operator/plugin-sdk/agent-sessions";
...
} from "../../../src/skills/test-support/home-env.test-support.js";
...
import { formatSkillsForPrompt, type Skill } from "../../../src/skills/loading/skill-contract.js";
import {
  formatSkillsCompact,
...
const COMPACT_OMITTED_NOTICE =
  "⚠️ Skills catalog using compact format (descriptions omitted). Run `operator skills check` to audit.";
const COMPACT_SHORTENED_NOTICE =
  "⚠️ Skills catalog using compact format (descriptions shortened). Run `operator skills check` to audit.";
...
describe("applySkillsPromptLimits (via buildWorkspaceSkillsPrompt)", () => {
  ...
  beforeEach(() => {
    envSnapshot = setMockSkillsHomeEnv("/Users/operator-test-user");
  });
  ...
  it("count truncation + compact: shows included X of Y with compact note", () => {
    ...
    const truncatedNotice =
      "⚠️ Skills truncated: included 10 of 30 (compact format, descriptions shortened). Run `operator skills check` to audit.";
  ...
  it("budget check uses compacted home-dir paths, not canonical paths", () => {
    ...
    const skills = Array.from({ length: 30 }, (_, i) =>
      makeSkill(
        `skill-${i}`,
        "A".repeat(800),
        `${home}/.operator/workspace/skills/skill-${i}/SKILL.md`,
      ),
    );
  ...
  it("skills are sorted alphabetically regardless of entry insertion order", () => {
    ...
    const prompt = buildWorkspaceSkillsPrompt("/fake", {
      entries,
      config: { skills: { limits: { maxSkillsPromptChars: 50_000 } } } satisfies OperatorConfig,
    });
  ...
  it("resolvedSkills in snapshot keeps canonical paths, not compacted", () => {
    ...
    const skills = Array.from({ length: 5 }, (_, i) =>
      makeSkill(`skill-${i}`, "A skill", `${home}/.operator/workspace/skills/skill-${i}/SKILL.md`),
    );
