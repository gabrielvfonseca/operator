...
import { writeSkill } from "../../../src/skills/test-support/test-helpers.js";
...
async function createWorkspaceSkillDirs() {
  const workspaceDir = await createTempDir("operator-");
  return {
    workspaceDir,
...
describe("buildWorkspaceSkillsPrompt — .agents/skills/ directories", () => {
  ...
  beforeEach(async () => {
    fakeHome = await createTempDir("operator-home-");
