...
import { buildWorkspaceSkillStatus } from "../../../src/skills/discovery/status.js";
...
describe("installSkillFromSource", () => {
  it("installs a local skill directory using the SKILL.md frontmatter name", async () => {
    await withTempDir({ prefix: "operator-skill-source-local-" }, async (root) => {
  ...
  it("uses --as slug override for local skill directories", async () => {
    await withTempDir({ prefix: "operator-skill-source-as-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
      const sourceDir = path.join(root, "source");
      await writeSkill(sourceDir, { name: "frontmatter-skill" });
  ...
  it("ignores oversized source-origin metadata while loading skill keys", async () => {
    await withTempDir({ prefix: "operator-skill-source-origin-cap-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
      const sourceDir = path.join(root, "source");
      await writeSkill(sourceDir, { name: "frontmatter-skill" });
      ...
      await fs.writeFile(
        path.join(workspaceDir, "skills", "custom-name", ".operator", "source-origin.json"),
        "x".repeat(20 * 1024),
      );
  ...
  it("installs git: file repositories and records the resolved commit", async () => {
    await withTempDir({ prefix: "operator-skill-source-git-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
  ...
  it("isolates git commands from inherited Git hook environment", async () => {
    await withTempDir({ prefix: "operator-skill-source-git-env-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
      const repoDir = path.join(root, "repo");
  ...
  it("disables system git config while preserving sanitized git command env", async () => {
    await withTempDir({ prefix: "operator-skill-source-git-system-config-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
      const repoDir = path.join(root, "repo");
      const poisonRepoDir = path.join(root, "poison");
      await fs.mkdir(repoDir, { recursive: true });
  ...
  it("installs slash-containing git branch refs from fresh clones", async () => {
    await withTempDir({ prefix: "operator-skill-source-git-ref-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
      const repoDir = path.join(root, "repo");
      await fs.mkdir(repoDir, { recursive: true });
      await initGitSkillRepo(repoDir);
  ...
  it.each([
    ...
    async (entry) => {
      await withTempDir({ prefix: "operator-skill-source-git-policy-" }, async (root) => {
        const workspaceDir = path.join(root, "workspace");
        const repoDir = path.join(root, "repo");
        await fs.mkdir(repoDir, { recursive: true });
        await initGitSkillRepo(repoDir);
  ...
  it("removes stale ClawHub lock tracking after source installs", async () => {
    await withTempDir({ prefix: "operator-skill-source-untrack-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
      const sourceDir = path.join(root, "source");
      await writeSkill(sourceDir, { name: "frontmatter-skill" });
  ...
  it("rejects missing local skill roots before treating them as ClawHub slugs", async () => {
    await withTempDir({ prefix: "operator-skill-source-missing-" }, async (root) => {
      const result = await installSkillFromSource({
  ...
  it("refuses git specs whose url would be consumed as a git clone option", async () => {
    await withTempDir({ prefix: "operator-skill-source-opt-inject-" }, async (root) => {
      const workspaceDir = path.join(root, "workspace");
