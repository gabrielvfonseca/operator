...
import { withExtractedArchiveRoot } from "../../../src/infra/install-flow.js";
import {
...
} from "../../../src/skills/lifecycle/archive-install.js";
...
async function expectFlatRootMarkerRejected(params: {
  ...
  const result = await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "operator-skill-clawhub-test-",
    timeoutMs: 120_000,
...
describe("skill archive install", () => {
  it.each(["skill.md", "skills.md", "SKILL.MD"])(
    ...
    async (marker) => {
      const root = await tempDirs.make("operator-skill-archive-install-");
