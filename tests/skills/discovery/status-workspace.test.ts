...
import { loadWorkspaceSkillEntries } from "../../../src/skills/loading/workspace.js";
...
function makeEntry(params: {
  ...
  return {
    skill: createCanonicalFixtureSkill({
      ...
      filePath,
      baseDir,
      source: params.source ?? "operator-workspace",
    }),
    frontmatter: {},
    metadata: {
...
describe("buildWorkspaceSkillStatus", () => {
  ...
  it("does not mark an overridden workspace skill as bundled by bundled name alone", async () => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-bundled-"));
    ...
    await withEnvAsync({ OPERATOR_BUNDLED_SKILLS_DIR: bundledDir }, async () => {
      const report = buildWorkspaceSkillStatus("/tmp/ws", {
        entries: [
          makeEntry({
            name: "peekaboo",
      ...
      expect(skill.source).toBe("operator-workspace");
