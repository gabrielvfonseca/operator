...
import { buildRandomTempFilePath, withTempDownloadPath } from "../../src/plugin-sdk/temp-path.js";

function expectPathInsideTmpRoot(resultPath: string) {
  const tmpRoot = fsSync.realpathSync(resolvePreferredOperatorTmpDir());
...
describe("withTempDownloadPath", () => {
  it.each([
    ...
    if (expectedBasename) {
      ...
      expect(capturedPath).toContain(path.join(resolvePreferredOperatorTmpDir(), "line-media-"));
    }
    if (expectCleanup) {
