...
import { OperatorChannelBridge } from "../../src/mcp/channel-bridge.js";
import type { QueueEvent, WaitFilter } from "../../src/mcp/channel-shared.js";
...
function makeBridge(verbose = false): BridgeInternals {
  return new OperatorChannelBridge({} as never, {
    claudeChannelMode: "off",
    verbose,
  }) as unknown as BridgeInternals;
}

describe("OperatorChannelBridge — Claude permission authorization", () => {
  test.each([
...
describe("OperatorChannelBridge — pendingClaudePermissions / pendingApprovals memory bounds", () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(0);
  });
  ...
  test("a failed notification still emits exactly one diagnostic record with verbose off", async () => {
    ...
    try {
      ...
      expect(writes[0]).toBe("operator mcp: notification channel/event failed\n");
  ...
  test("a rejected gateway event still emits exactly one diagnostic record with verbose off", async () => {
    ...
    try {
      ...
      expect(writes).toHaveLength(1);
      expect(writes[0]).toBe("operator mcp: gateway event exec.approval.requested failed\n");
  ...
  test("a rejected gateway event includes error detail with verbose on", async () => {
    ...
    try {
      ...
      expect(writes[1]).toBe(
        "operator mcp: gateway event exec.approval.requested error: Error: handler boom\n",
      );
    } finally {
      writeSpy.mockRestore();
      await bridge.close();
