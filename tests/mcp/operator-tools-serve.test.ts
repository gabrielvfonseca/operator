...
import { hashSystemAgentOperation } from "../../src/agents/tools/system-agent-tool.js";
import {
...
} from "../../src/mcp/operator-tools-serve-config.js";
import {
  OPERATOR_TOOLS_MCP_AGENT_SESSION_KEY_ENV,
  resolveOperatorToolsForMcp,
  resolveOperatorToolsMcpAgentSessionKey,
} from "../../src/mcp/operator-tools-serve.js";
import { createPluginToolsMcpHandlers } from "../../src/mcp/plugin-tools-handlers.js";
...
describe("Operator tools MCP server", () => {
  ...
  it("serves the ring-zero operator tool without an agent session key", async () => {
    const handlers = createPluginToolsMcpHandlers(
      resolveOperatorToolsForMcp({ tools: ["operator"], systemAgentSurface: "cli" }),
    );

    const listed = await handlers.listTools();
    expect(listed.tools.map((tool) => tool.name)).toEqual(["operator"]);
  });
  ...
  it("parses the served tool selection from env and defaults to cron", () => {
    ...
    expect(
      resolveOperatorToolsMcpToolSelection({
        [OPERATOR_TOOLS_MCP_TOOLS_ENV]: " operator , cron ",
      }),
    ).toEqual(["operator", "cron"]);
    expect(() =>
      resolveOperatorToolsMcpToolSelection({ [OPERATOR_TOOLS_MCP_TOOLS_ENV]: "exec" }),
    ).toThrow(OPERATOR_TOOLS_MCP_TOOLS_ENV);
  });

  it("parses the operator surface from env and defaults to cli", () => {
  ...
  it("builds a operator-only stdio server config under the operator name", () => {
    const config = buildSystemAgentToolsMcpServerConfig({ surface: "gateway" });

    expect(Object.keys(config.mcpServers)).toEqual(["operator"]);
    ...
    expect(server.args?.at(-1)).toMatch(/operator-tools-serve\.(js|ts)$/);
    expect(server.env).toEqual({
      [OPERATOR_TOOLS_MCP_TOOLS_ENV]: "operator",
      [OPERATOR_TOOLS_MCP_SYSTEM_AGENT_SURFACE_ENV]: "gateway",
    });
  });
});
