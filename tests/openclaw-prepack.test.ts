// Operator prepack tests validate package prepack output.
...
import {
  ...
  runPrepackCommand,
} from "../scripts/operator-prepack.ts";
import { useAutoCleanupTempDirTracker } from "./helpers/temp-dir.js";

const tempDirs = useAutoCleanupTempDirTracker(afterEach);

describe("collectSourcePackWorkspaceDependencyErrors", () => {
  it("rejects the plain source pack that pnpm rewrites without bundling @gabrielvfonseca/ai", () => {
    const rootDir = tempDirs.make("operator-source-pack-workspace-");
    ...
    const rootPackageJson = {
      ...
      name: "operator-source-pack-regression",
      version,
    };
    ...
    writeFileSync(
      ...
      `${JSON.stringify({ name: "@gabrielvfonseca/ai", version }, null, 2)}\n`,
    );
    ...
    expect(packedPackageJson.dependencies?.["@gabrielvfonseca/ai"]).toBe(version);
    expect(packedPackageJson.bundleDependencies).toBeUndefined();
    expect(existsSync(path.join(extractDir, "package", "node_modules", "@operator", "ai"))).toBe(
      false,
    );
    expect(collectSourcePackWorkspaceDependencyErrors(rootPackageJson, {})).toEqual([
      "plain root packing cannot safely resolve @gabrielvfonseca/ai from workspace:*: pnpm rewrites the workspace dependency to an exact version without bundling the package",
      "use `node scripts/package-operator-for-docker.mjs --allow-unreleased-changelog` for a self-contained source package; official npm release automation prepares and publishes @gabrielvfonseca/ai separately",
    ]);
    expect(
