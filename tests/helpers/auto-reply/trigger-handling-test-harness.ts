...
const embeddedAgentMocks = getSharedMocks("operator.trigger-handling.embedded-agent-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("operator.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("operator.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("operator.trigger-handling.web-session-mocks", () => ({
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("OPERATOR_HOME", snapshot.operatorHome);
  restoreKey("OPERATOR_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.OPERATOR_STATE_DIR = join(home, ".operator");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "operator-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".operator", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): OperatorConfig {
