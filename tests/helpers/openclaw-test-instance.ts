// Operator test instance helper spawns isolated Operator processes.
...
type OperatorTestStateOptions = NonNullable<Parameters<typeof createOperatorTestState>[0]>;

type OperatorTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<OperatorTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

type OperatorTestInstanceCommandResult = {
  code: number | null;
...
type OperatorTestProcess = ChildProcessByStdio<null, Readable, Readable>;

export type OperatorTestInstance = {
  name: string;
  port: number;
  url: string;
  hookToken: string;
  ...
  stdout: string[];
  stderr: string[];
  child?: OperatorTestProcess;
  env: NodeJS.ProcessEnv;
  entrypoint: () => Promise<string[]>;
  cli: (
    args: string[],
    options?: { timeoutMs?: number },
  ) => Promise<OperatorTestInstanceCommandResult>;
...
type OperatorTestChildProcess = Pick<OperatorTestProcess, "kill" | "pid">;
...
async function prepareGatewayEntrypoint(cwd: string): Promise<string[]> {
  ...
  const child = spawn("node", ["scripts/run-node.mjs", "--help"], {
    ...
    detached: shouldUseOperatorTestProcessGroup(),
  });
  ...
  if (completed === null) {
    signalOperatorTestProcess(child, "SIGKILL");
...
async function waitForPortOpen(
  proc: Pick<OperatorTestProcess, "exitCode" | "signalCode">,
  chunksOut: string[],
  chunksErr: string[],
  port: number,
  timeoutMs: number,
...
async function waitForGatewayExit(child: OperatorTestProcess, timeoutMs: number): Promise<boolean> {
...
function hasChildExited(child: Pick<OperatorTestProcess, "exitCode" | "signalCode">) {
...
export async function createOperatorTestInstance(
  options: OperatorTestInstanceOptions,
): Promise<OperatorTestInstance> {
  ...
  let child: OperatorTestProcess | undefined;
  let cleaned = false;

  const instance: OperatorTestInstance = {
    name: options.name,
    port,
    ...
    stopGateway: async () => {
      ...
      if (!hasChildExited(child) && !child.killed) {
        try {
          signalOperatorTestProcess(child, "SIGTERM");
        } catch {
          // ignore
        }
      }
...
function shouldUseOperatorTestProcessGroup(): boolean {
  return process.platform !== "win32";
}

function signalOperatorTestProcess(
  child: OperatorTestChildProcess,
  signal: NodeJS.Signals,
  killProcess: (pid: number, signal: NodeJS.Signals) => boolean = (pid, nextSignal) =>
    process.kill(pid, nextSignal),
): void {
  if (shouldUseOperatorTestProcessGroup() && typeof child.pid === "number") {
    try {
