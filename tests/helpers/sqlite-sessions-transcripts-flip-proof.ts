...
const FULL_TURN_ASSISTANT_TEXT = "OPERATOR_E2E_OK_12";
  ...
  const child = spawn(
    ...
    [
      ...
      `
        ...
        const db = new DatabaseSync(process.env.OPERATOR_E2E_BUSY_DB_PATH);
        db.exec("PRAGMA busy_timeout = 30000; BEGIN IMMEDIATE;");
        fs.writeFileSync(process.env.OPERATOR_E2E_BUSY_READY_PATH, "ready");
        setTimeout(() => {
          db.exec("COMMIT");
          db.close();
        }, Number(process.env.OPERATOR_E2E_BUSY_HOLD_MS));
      `,
    ],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        OPERATOR_E2E_BUSY_DB_PATH: context.agentDbPath,
        OPERATOR_E2E_BUSY_HOLD_MS: String(holdMs),
        OPERATOR_E2E_BUSY_READY_PATH: readyPath,
      },
      stdio: ["ignore", "pipe", "pipe"],
    },
  );
