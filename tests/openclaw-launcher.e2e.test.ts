// Operator launcher E2E tests validate launcher process behavior.
...
async function makeLauncherFixture(fixtureRoots: string[]): Promise<string> {
  const fixtureRoot = makeTempDir(fixtureRoots, "operator-launcher-");
  await fs.copyFile(
    path.resolve(process.cwd(), "operator.mjs"),
    path.join(fixtureRoot, "operator.mjs"),
  );
...
async function makeLauncherProbeFixture(
  ...
  const launcherPath = path.join(fixtureRoot, "operator.mjs");
  const launcher = await fs.readFile(launcherPath, "utf8");
  const bootstrapStart = "\nif (!waitingForCompileCacheRespawn) {";
  const bootstrapIndex = launcher.indexOf(bootstrapStart);
  if (bootstrapIndex < 0) {
    throw new Error("operator launcher bootstrap block was not found");
  }
  await fs.writeFile(
    launcherPath,
...
async function addGitMarker(fixtureRoot: string): Promise<void> {
  await fs.writeFile(path.join(fixtureRoot, ".git"), "gitdir: .git/worktrees/operator\n", "utf8");
}
...
describe("operator launcher", () => {
    ...
    for (const testCase of cases) {
      ...
      if (testCase.supported) {
        ...
        expect(result.stderr, testCase.version).toContain(
          `operator: Node.js >=22.22.3 <23, >=24.15.0 <25, or >=25.9.0 is required (current: v${testCase.version}).`,
        );
      }
    }
  });
  ...
  it("rejects Bun even when its Node compatibility version is new enough", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      ["--import", pathToFileURL(mockRuntime).href, path.join(fixtureRoot, "operator.mjs")],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
    ...
    expect(result.stderr).toContain(
      "the Bun runtime is unsupported because Operator requires node:sqlite",
    );
  });
  ...
  it("surfaces transitive entry import failures instead of masking them as missing dist", async () => {
    ...
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      'import "missing-operator-launcher-dep";\nexport {};\n',
      "utf8",
    );
    ...
    expect(result.stderr).toContain("missing-operator-launcher-dep");
  ...
  it("prints root version without importing the runtime entry", async () => {
    ...
    expect(result.stdout).toBe("Operator 1.2.3-test (abcdef0)\n");
  ...
  it("defers container-targeted root version to the runtime entry", async () => {
    ...
    await fs.writeFile(
      path.join(fixtureRoot, "package.json"),
      JSON.stringify({ name: "@gabrielvfonseca/operator", version: "1.2.3-test" }),
      "utf8",
    );
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "operator.mjs"), "--container", "demo", "--version"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it("treats Bun direct optional import misses as direct launcher misses", async () => {
    const fixtureRoot = await makeLauncherProbeFixture(
      ...
      [
        ...
        "    { message: \"Cannot find module './nested.js' from '/pkg/operator/dist/entry.js'\" },",
        "    './dist/entry.js',",
        "  ),",
        "  sameSpecifierTransitive: isDirectModuleNotFoundError(",
        "    { message: \"Cannot find module './dist/entry.js' from '/pkg/operator/dist/entry.js'\" },",
        "    './dist/entry.js',",
        "  ),",
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "operator.mjs")], {
      cwd: fixtureRoot,
      env: launcherEnv(),
      encoding: "utf8",
    });
  ...
  it.each([
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "operator.mjs"), params.command, "--help"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it.each(["doctor", "gateway", "models", "plugins", "sessions", "tasks"])(
    ...
    async (command) => {
      ...
      const result = spawnSync(
        process.execPath,
        [path.join(fixtureRoot, "operator.mjs"), command, "--help"],
        {
          cwd: fixtureRoot,
          env: launcherEnv(),
          encoding: "utf8",
  ...
  it("uses precomputed subcommand help with leading root options", async () => {
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "operator.mjs"), "--profile", "work", "--no-color", "models", "-h"],
      {
        cwd: fixtureRoot,
        env: launcherEnv(),
        encoding: "utf8",
  ...
  it.each([
    ...
    const result = spawnSync(
      process.execPath,
      [path.join(fixtureRoot, "operator.mjs"), ...params.args],
      {
        cwd: fixtureRoot,
  ...
  it("defers root help to the runtime entry when plugin config can change help", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const configPath = path.join(fixtureRoot, "operator.json");
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "cli-startup-metadata.json"),
  ...
  it("checks the OPERATOR_HOME default config path before using precomputed root help", async () => {
    ...
    const configDir = path.join(openclawHome, ".operator");
    ...
    await fs.writeFile(
      path.join(configDir, "operator.json"),
      JSON.stringify({ plugins: { slots: { memory: "memory-lancedb" } } }),
      "utf8",
    );
    ...
    const result = spawnSync(process.execPath, [path.join(fixtureRoot, "operator.mjs"), "--help"], {
      cwd: fixtureRoot,
      env: launcherEnv({ OPERATOR_HOME: openclawHome }),
      encoding: "utf8",
    });
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          'process.title = "operator-launcher-sigterm-test-child";',
      ...
      const launcher = spawn(process.execPath, [path.join(fixtureRoot, "operator.mjs")], {
        cwd: fixtureRoot,
        env: launcherEnv({
          NODE_COMPILE_CACHE: path.join(fixtureRoot, ".node-compile-cache"),
        }),
    ...
    async () => {
      ...
      await fs.writeFile(
        ...
        [
          'import { writeFileSync } from "node:fs";',
          `writeFileSync(${JSON.stringify(childInfoPath)}, JSON.stringify({ pid: process.pid }) + "\\n");`,
          'process.title = "operator-launcher-sigterm-ignore-test-child";',
          'process.on("SIGTERM", () => {});',
          "setInterval(() => {}, 1000);",
          "",
        ].join("\n"),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await addGitMarker(fixtureRoot);
      await addCompileCacheProbe(fixtureRoot);
      const linkParent = makeTempDir(fixtureRoots, "operator-launcher-link-");
      const linkedRoot = path.join(linkParent, "operator-linked");
      await fs.symlink(fixtureRoot, linkedRoot, "dir");

      const result = spawnSync(process.execPath, [path.join(linkedRoot, "operator.mjs")], {
        cwd: linkParent,
        env: launcherEnv({
  ...
  it("scopes packaged launcher compile cache inside configured cache roots", async () => {
    ...
    expect(result.stdout).toContain(
      path.join(".node-compile-cache", "@gabrielvfonseca/operator", "2026.4.29"),
    );
  });

  it("falls back to the default packaged launcher compile cache when NODE_COMPILE_CACHE is empty", async () => {
    const fixtureRoot = await makeLauncherFixture(fixtureRoots);
    const runCwd = makeTempDir(fixtureRoots, "operator-launcher-cwd-");
    const tmpRoot = makeTempDir(fixtureRoots, "operator-launcher-tmp-");
    await fs.writeFile(path.join(fixtureRoot, "package.json"), '{"version":"2026.4.29"}\n');
    await fs.writeFile(
      path.join(fixtureRoot, "dist", "entry.js"),
      [
    ...
    expect(result.stdout).not.toContain(path.join(runCwd, "@gabrielvfonseca/operator"));
  });
