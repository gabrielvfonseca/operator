...
import { resolvePluginRoutePathContext } from "../../../../src/gateway/server/plugins-http/path-context.js";
import {
  findMatchingPluginNodeCapabilityRoute,
  listPluginNodeCapabilities,
} from "../../../../src/gateway/server/plugins-http/route-capability.js";
...
describe("plugin node capability route metadata", () => {
  ...
  it("adds plugin ownership to matched capability route metadata", () => {
    ...
    expect(
      findMatchingPluginNodeCapabilityRoute(
        registry,
        resolvePluginRoutePathContext("/__operator__/canvas/ws"),
      )?.nodeCapability,
