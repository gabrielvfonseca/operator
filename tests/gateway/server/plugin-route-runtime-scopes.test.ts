...
import { resolvePluginRouteRuntimeOperatorScopes } from "../../../src/gateway/server/plugin-route-runtime-scopes.js";
...
describe("resolvePluginRouteRuntimeOperatorScopes", () => {
  it("preserves declared trusted-proxy scopes when the header is present", () => {
    expect(
      resolvePluginRouteRuntimeOperatorScopes(createReq({ "x-operator-scopes": "operator.read" }), {
  ...
  it("does not trust caller-declared admin scopes on plugin routes for mode=none requests", () => {
    expect(
      resolvePluginRouteRuntimeOperatorScopes(
        createReq({ "x-operator-scopes": "operator.admin,operator.write" }),
