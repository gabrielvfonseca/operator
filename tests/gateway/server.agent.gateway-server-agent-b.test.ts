...
import { readAgentCommandCall } from "../../src/gateway/agent-command.test-helpers.js";
...
async function useTempSessionStorePath() {
  const dir = makeTempDir(gwSessionTempDirs, "operator-gw-");
...
describe("gateway server agent", () => {
  ...
  test("agent reuses the last plugin delivery route when channel=last", {
    timeout: 20_000,
  }, async () => {
