...
import { approveDevicePairing, requestDevicePairing } from "../../src/infra/device-pairing.js";
import { approveNodePairing, listNodePairing, requestNodePairing } from "../../src/infra/node-pairing.js";
...
describe("node pairing rate limit", () => {
  test("limits concurrent first-time node pairing requests before the pairing lock", async () => {
    ...
    await withGatewayServer(async ({ port }) => {
      const identityPrefix = path.join(os.tmpdir(), `operator-node-pairing-${randomUUID()}`);
  ...
  test("records paired reconnect reapproval despite first-time pairing limits", async () => {
    ...
    await withGatewayServer(async ({ port }) => {
      const identityPrefix = path.join(
        os.tmpdir(),
        `operator-node-pairing-upgrade-${randomUUID()}`,
      );
  ...
  test("reuses identical paired reapproval without rejecting the node", async () => {
    ...
    await withGatewayServer(async ({ port }) => {
      const identityPath = path.join(os.tmpdir(), `operator-node-reapproval-${randomUUID()}.json`);
