...
describe("live-agent-probes", () => {
  ...
  it("builds a retryable cron prompt with provider-specific fallback wording", () => {
    ...
    expect(claudeRetryPrompt).toContain("search/load MCP tools for `operator cron` or `cron`");
    ...
    expect(claudeRetryPrompt).not.toContain("operator-tools");
    expect(
      buildLiveCronProbeMessage({
        agent: "future-agent",
        argsJson: spec.argsJson,
    ...
    ).toContain("previous Operator cron MCP tool call was cancelled");
