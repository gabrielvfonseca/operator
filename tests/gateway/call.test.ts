...
import type { DeviceAuthEntry } from "../../src/shared/device-auth.js";
...
} from "../../src/gateway/gateway-connection.test-mocks.js";
...
function resetGatewayCallMocks() {
  ...
  connectError = null;
  const loadConfigForTests = getRuntimeConfig as unknown as () => OperatorConfig;
...
describe("callGateway url resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot.restore();
    deleteTestEnvValue("OPERATOR_ALLOW_INSECURE_PRIVATE_WS");
  ...
  it("keeps CLI device identity when an ambient token is inactive under auth mode none", async () => {
    ...
    setGatewayNetworkDefaults();
    process.env.OPERATOR_GATEWAY_TOKEN = "inactive-env-token";
  ...
  it("uses OPERATOR_GATEWAY_URL env override in remote mode when remote URL is missing", async () => {
    getRuntimeConfig.mockReturnValue({
      gateway: { mode: "remote", bind: "loopback", remote: {} },
    });
    resolveGatewayPort.mockReturnValue(18789);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.OPERATOR_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
    process.env.OPERATOR_GATEWAY_TOKEN = "env-token";
  ...
  it("lets an explicit local port override bypass gateway env URL and port", async () => {
    ...
    resolveGatewayPort.mockImplementation((_config?: unknown, env?: unknown) => {
      ...
      return Number(candidateEnv?.OPERATOR_GATEWAY_PORT ?? 18789);
    });
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.OPERATOR_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
    process.env.OPERATOR_GATEWAY_PORT = "19001";
    process.env.OPERATOR_GATEWAY_TOKEN = "env-token";
...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("lets probe details local port override bypass gateway env URL and port", async () => {
    ...
    testing.setDepsForTests({
      ...
      resolveGatewayPort: (_config?: unknown, env?: NodeJS.ProcessEnv) =>
        Number(env?.OPERATOR_GATEWAY_PORT ?? 18789),
    });
    const prevUrl = process.env.OPERATOR_GATEWAY_URL;
    const prevPort = process.env.OPERATOR_GATEWAY_PORT;
    try {
      process.env.OPERATOR_GATEWAY_URL = "wss://env-gateway.example/ws";
      process.env.OPERATOR_GATEWAY_PORT = "19001";
      ...
      if (prevUrl === undefined) {
        ...
        process.env.OPERATOR_GATEWAY_URL = prevUrl;
      }
      if (prevPort === undefined) {
        delete process.env.OPERATOR_GATEWAY_PORT;
      } else {
        process.env.OPERATOR_GATEWAY_PORT = prevPort;
      }
    }
  });
  ...
  it("lets a bound remote call keep its config URL over the gateway env override", async () => {
    ...
    const prevUrl = process.env.OPERATOR_GATEWAY_URL;
    try {
      process.env.OPERATOR_GATEWAY_URL = "wss://unrelated-gateway.example/ws";
  ...
  it("uses env OPERATOR_GATEWAY_URL when set", () => {
    getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
    resolveGatewayPort.mockReturnValue(18800);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    const prevUrl = process.env.OPERATOR_GATEWAY_URL;
    try {
      process.env.OPERATOR_GATEWAY_URL = "wss://browser-gateway.local:9443/ws";

      const details = buildGatewayConnectionDetails();

      expect(details.url).toBe("wss://browser-gateway.local:9443/ws");
      expect(details.urlSource).toBe("env OPERATOR_GATEWAY_URL");
      expect(details.bindDetail).toBeUndefined();
    } finally {
      if (prevUrl === undefined) {
        delete process.env.OPERATOR_GATEWAY_URL;
  ...
  it("allows ws:// hostname remote URLs when OPERATOR_ALLOW_INSECURE_PRIVATE_WS=1", () => {
...
describe("callGateway error details", () => {
  ...
  it("keeps the default wrapper timeout aligned with env handshake timeout", async () => {
    const envSnapshot = captureEnv(["OPERATOR_HANDSHAKE_TIMEOUT_MS"]);
    try {
  ...
  it("disables the request and wrapper deadline when timeout is null", async () => {
    ...
    testing.setDepsForTests({
      ...
      getRuntimeConfig: getRuntimeConfig as unknown as () => OperatorConfig,
...
describe("callGateway url override auth requirements", () => {
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.OPERATOR_GATEWAY_TOKEN = "env-token";
    process.env.OPERATOR_GATEWAY_PASSWORD = "env-password";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
  ...
  it("throws when env URL override is set without env credentials", async () => {
    process.env.OPERATOR_GATEWAY_URL = "wss://override.example/ws";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        auth: { token: "local-token", password: "local-password" },
    ...
    await expect(callGateway({ method: "health" })).rejects.toThrow(
      /OPERATOR_GATEWAY_TOKEN or OPERATOR_GATEWAY_PASSWORD/i,
    );
  });
});
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      ...
      envKey: "OPERATOR_GATEWAY_PASSWORD",
      envValue: "from-env",
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.OPERATOR_GATEWAY_PASSWORD = envPassword;
    }
  ...
  it("does not resolve local password ref when env password takes precedence", async () => {
    process.env.OPERATOR_GATEWAY_PASSWORD = "from-env";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
