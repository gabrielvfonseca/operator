...
import { getRuntimeAuthProfileStoreCredentialsRevision } from "../../src/agents/auth-profiles/runtime-snapshots.js";
import { addSession, markBackgrounded, markExited } from "../../src/agents/bash-process-registry.js";
import { createProcessSessionFixture } from "../../src/agents/bash-process-registry.test-helpers.js";
import { resetProcessRegistryForTests } from "../../src/agents/bash-process-registry.test-support.js";
import { prepareConfigRuntimeEnv } from "../../src/config/config-env-vars.js";
import type { ConfigWriteNotification } from "../../src/config/config.js";
import {
...
import { createEmptyRuntimeWebToolsMetadata } from "../../src/secrets/runtime-fast-path.js";
import {
...
import { createChannelTestPluginBase, createTestRegistry } from "../../src/test-utils/channel-plugins.js";
import { diffConfigPaths } from "../../src/gateway/config-diff.js";
import {
  buildGatewayReloadPlan,
  type ChannelKind,
  type GatewayReloadPlan,
} from "../../src/gateway/config-reload-plan.js";
import type { GatewayPluginReloadResult } from "../../src/gateway/server-reload-handlers.js";
import {
...
} from "../../src/gateway/server-reload-handlers.js";
import { enforceSharedGatewaySessionGenerationForConfigWrite } from "../../src/gateway/server-shared-auth-generation.js";
import { createTerminalLaunchPolicy } from "../../src/gateway/terminal/launch.js";
...
const hoisted = vi.hoisted(() => ({
  ...
  runtimeConfig: { value: { session: { store: "/tmp/active-sessions.json" } } as OperatorConfig },
  reloadEvents: [] as string[],
  loadModelCatalog: vi.fn(async (_params: { config: OperatorConfig }) => []),
  resetModelCatalogCache: vi.fn(() => {}),
  refreshContextWindowCache: vi.fn(async (_cfg: OperatorConfig) => {}),
  clearCurrentProviderAuthState: vi.fn(() => {}),
  warmCurrentProviderAuthStateOffMainThread: vi.fn(async (_cfg: OperatorConfig) => {}),
...
vi.mock("../agents/model-catalog.js", () => ({
  loadModelCatalog: (params: { config: OperatorConfig }) => {
...
vi.mock("../agents/context.js", () => ({
  refreshContextWindowCache: async (cfg: OperatorConfig) => {
...
function createManagedRestartSequenceHarness(
  ...
  const activateRuntimeSecrets = vi.fn(async (config: OperatorConfig) => {
    const secretInputs = [
...
async function runManagedOwnershipScenario(params: {
  ...
  const snapshot = (config: OperatorConfig): PreparedSecretsRuntimeSnapshot => ({
    sourceConfig: config,
    config,
    authStores: [],
    authStoreCredentialsRevision: getRuntimeAuthProfileStoreCredentialsRevision(),
...
describe("gateway hot reload model state", () => {
  it("stops old cron exit watchers and reconciles rebuilt ones after cron restart", async () => {
    ...
    const nextConfig = { cron: { enabled: true } } as OperatorConfig;
  ...
  it("completes reload reconciliation when the replacement scheduler is disabled", async () => {
    ...
    const nextConfig = { cron: { enabled: false } } as OperatorConfig;
  ...
  it("applies an in-place heartbeat update without a recovery restart owner", async () => {
    ...
    const nextConfig = { agents: { defaults: { heartbeat: { every: "1h" } } } } as OperatorConfig;
    ...
    try {
      await expect(
        applyHotReload(
          ...
          { agents: { defaults: { maxConcurrent: 1 } } } as OperatorConfig,
  ...
  it("resets prepared model runtime state for every hot reload and rewarms after plugin reload", async () => {
    ...
    const nextConfig = { plugins: { enabled: true } } as OperatorConfig;
  ...
  it("disposes cached MCP runtimes on MCP config hot reloads", async () => {
    ...
    const nextConfig = { mcp: { servers: {} } } as OperatorConfig;
  ...
  it("rejects an ownerless context cache reload before runtime commit", async () => {
    ...
    await expect(
      applyHotReload(
        ...
        { agents: { defaults: { workspace: "/tmp/next-workspace" } } } as OperatorConfig,
      ),
    ).rejects.toThrow(
      "config reload requires a managed gateway restart owner for irreversible hot reload",
    );
  ...
  it.each([
    ...
    const previousConfig = testCase.previousConfig as OperatorConfig;
    const nextConfig = testCase.nextConfig as OperatorConfig;
...
describe("gateway hot reload superseded tail recovery", () => {
  it("rearms detached stale-tail recovery against an already accepted config", async () => {
    ...
    const prepareRuntimeConfig = vi.fn(
      async (): Promise<OperatorConfig> => ({ logging: { level: "debug" } }),
    );
  ...
  it("pauses stale-target recovery until a newer valid config is accepted", async () => {
    ...
    const configA = { logging: { level: "info" as const } } satisfies OperatorConfig;
    const configC = { logging: { level: "debug" as const } } satisfies OperatorConfig;
  ...
  it.each(["mcp", "gmail", "channel", "context"] as const)(
    ...
    async (surface) => {
      ...
      let pendingConfig: OperatorConfig | null = null;
...
describe("gateway hot reload commit policy", () => {
  it("retires the old health monitor before publishing its replacement", async () => {
    ...
    await applyHotReload(createHotTailPlan({ restartHealthMonitor: true }), {} as OperatorConfig);
...
describe("gateway Gmail hot reload handlers", () => {
  ...
  function createGmailConfig(account: string): OperatorConfig {
    return {
  ...
  it("rejects ownerless irreversible plans but applies safe hot plans", async () => {
    ...
    const terminalPolicy = createTerminalLaunchPolicy(initialConfig);
    const prepareTerminalConfig = vi.fn((plan: GatewayReloadPlan, nextConfig: OperatorConfig) => {
      terminalPolicy.prepareConfig(nextConfig, { restartPending: plan.restartGateway });
    });
    const reconcileTerminalSessions = vi.fn();
    const setState = vi.fn();
    ...
    let revision = 0;
    const writeConfig = (config: OperatorConfig, hash: string) => {
      const listener = writeListenerRef.current;
      if (!listener) {
        throw new Error("Expected config write listener to be registered");
      }
    ...
    try {
      ...
      ] satisfies Array<{ label: string; config: OperatorConfig; surface: string }>;
      ...
      const safeConfig: OperatorConfig = {
        ...initialConfig,
        logging: { level: "debug" },
      };
  ...
  it("does not emit a prepared config restart after managed shutdown starts", async () => {
    ...
    harness.activateRuntimeSecrets.mockImplementationOnce(async (config: OperatorConfig) => {
  ...
  it("retries managed hot reload when secrets change before publication", async () => {
    ...
    const activateRuntimeSecrets = Object.assign(
      vi.fn(async (config: OperatorConfig) => {
        preparationCount += 1;
...
describe("gateway plugin hot reload handlers", () => {
  it("restarts channels when the candidate env removes an active skip flag", async () => {
    const envKey = "OPERATOR_SKIP_CHANNELS";
    const previousValue = process.env[envKey];
    process.env[envKey] = "1";
    const targetEnv: NodeJS.ProcessEnv = { [envKey]: "1" };
    const previousConfig = { env: { vars: { [envKey]: "1" } } } satisfies OperatorConfig;
  ...
  it("skips channel work when the candidate env adds a skip flag", async () => {
    const envKey = "OPERATOR_SKIP_PROVIDERS";
    const previousValue = process.env[envKey];
    delete process.env[envKey];
    const targetEnv: NodeJS.ProcessEnv = {};
    const nextConfig = { env: { vars: { [envKey]: "1" } } } satisfies OperatorConfig;
    const runtimeEnv = prepareConfigRuntimeEnv({
      previousConfig: {},
      nextConfig,
      env: targetEnv,
  ...
  it("publishes candidate env before cron, plugin, and channel replacements start", async () => {
    vi.useFakeTimers();
    const envKey = "OPERATOR_TEST_HOT_RELOAD_SERVICE_ENV";
