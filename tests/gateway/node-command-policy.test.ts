...
import { filterLegacyNodeProtocolFeatures } from "../../src/gateway/node-legacy-protocol-filter.js";
...
describe("gateway/node-command-policy", () => {
  ...
  it("keeps canvas commands out of core defaults when the canvas plugin is not active", () => {
    const allowlist = resolveNodeCommandAllowlist({} as OperatorConfig, {
      platform: "windows",
  ...
  it("allows approved node-host MCP calls while denyCommands still wins", () => {
    ...
    const allowlist = resolveNodeCommandAllowlist({} as OperatorConfig, node);
    expect(
      resolveNodePairingCommandAllowlist({} as OperatorConfig, node).has("mcp.tools.call.v1"),
    ).toBe(true);
    ...
    const denied = resolveNodeCommandAllowlist(
      { gateway: { nodes: { denyCommands: ["mcp.tools.call.v1"] } } } as OperatorConfig,
      node,
    );
  ...
  it("does not grandfather approved non-default commands after config removal", () => {
    const staleApproval = resolveNodeCommandAllowlist({} as OperatorConfig, {
      platform: "macos",
      deviceFamily: "Mac",
  ...
  it("keeps computer.act out of the runtime allowlist until explicitly allowed", () => {
    ...
    const unarmed = resolveNodeCommandAllowlist({} as OperatorConfig, macNode);
    expect(unarmed.has("computer.act")).toBe(false);
    expect(
      resolveNodeCommandAllowlist({} as OperatorConfig, {
        ...macNode,
    ...
    const armed = resolveNodeCommandAllowlist(
      { gateway: { nodes: { allowCommands: ["computer.act"] } } } as OperatorConfig,
      macNode,
    );
  ...
  it("keeps computer.act declarable through the macOS pairing allowlist only", () => {
    const pairing = resolveNodePairingCommandAllowlist({} as OperatorConfig, {
      platform: "macos",
      deviceFamily: "Mac",
    ...
    const windowsPairing = resolveNodePairingCommandAllowlist({} as OperatorConfig, {
      platform: "windows",
      deviceFamily: "Windows",
      commands: ["computer.act"],
    });
  ...
  it("requires explicit gateway opt-in for iOS health summaries", () => {
    ...
    expect(resolveNodeCommandAllowlist({} as OperatorConfig, node).has("health.summary")).toBe(
      false,
    );
