...
} from "../../src/gateway/probe-auth.js";
...
function configWithDefaultEnvProvider(gateway: NonNullable<OperatorConfig["gateway"]>) {
  return {
    gateway,
    secrets: {
      providers: {
...
function resolveSafeProbeAuth(cfg: OperatorConfig, mode: "local" | "remote" = "local") {
...
function expectUnresolvedProbeTokenWarning(cfg: OperatorConfig) {
  ...
  it("resolves env SecretRef token via async secret-inputs path", async () => {
    const result = await resolveGatewayProbeAuthSafeWithSecretInputs({
      cfg: configWithDefaultEnvProvider({
        auth: tokenAuthConfig("OPERATOR_GATEWAY_TOKEN"),
      }),
      mode: "local",
      env: {
        OPERATOR_GATEWAY_TOKEN: "test-token-from-env",
