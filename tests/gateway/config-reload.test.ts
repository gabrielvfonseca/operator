...
import { hashRuntimeConfigValue } from "../../src/config/runtime-snapshot.js";
...
import { diffConfigPaths, diffGatewayReloadPaths } from "../../src/gateway/config-diff.js";
import { buildGatewayReloadPlan, resolveConfigReloadMetadata } from "../../src/gateway/config-reload-plan.js";
import { resolveGatewayReloadSettings } from "../../src/gateway/config-reload-settings.js";
import {
  type GatewayConfigReloadTransactionOwnership,
  type GatewayReloadPlan,
  startGatewayConfigReloader,
} from "../../src/gateway/config-reload.js";
...
function createReloaderHarness(
  ...
  options: {
    ...
    onConfigApplied?: (plan: GatewayReloadPlan, nextConfig: OperatorConfig) => void | Promise<void>;
    onConfigRevisionApplied?: (hash: string) => void;
    onConfigChange?: (plan: GatewayReloadPlan, nextConfig: OperatorConfig) => void | Promise<void>;
    onNoopConfigCommit?: (
  ...
  const onConfigChange = vi.fn(
    options.onConfigChange ?? (async (_plan: GatewayReloadPlan, _nextConfig: OperatorConfig) => {}),
  );
  const onConfigApplied = vi.fn(
    options.onConfigApplied ??
      (async (_plan: GatewayReloadPlan, _nextConfig: OperatorConfig) => {}),
  );
  ...
  const onRestart = vi.fn(
    options.onRestart ?? ((_plan: GatewayReloadPlan, _nextConfig: OperatorConfig) => {}),
  );
...
function getOnlyRestartCall(harness: ReloaderHarness): [GatewayReloadPlan, OperatorConfig] {
...
function getOnlyHotReloadCall(harness: ReloaderHarness): [GatewayReloadPlan, OperatorConfig] {
...
describe("startGatewayConfigReloader", () => {
  ...
  it("prepares a superseding config against the env owner committed at the runtime edge", async () => {
    const envKey = "OPERATOR_TEST_COMMITTED_ENV_SOURCE";
  ...
  it("does not restart stale external config A before rejecting invalid SecretRef config B", async () => {
    ...
    const restartRequests: OperatorConfig[] = [];
    const harness = createReloaderHarness(readSnapshot, {
      initialConfig,
  ...
  it("plans one immutable runtime override snapshot per candidate", async () => {
    ...
    const prepareConfigCandidate = vi.fn(({ runtimeConfig, sourceConfig }) => {
      ...
      const applyCapturedOverride = (config: OperatorConfig): OperatorConfig =>
        override
    ...
    const overrideSource: OperatorConfig = {
      ...initialConfig,
    ...
    const resetSource: OperatorConfig = {
      ...initialConfig,
  ...
  it("does not publish a restart-only hot-mode candidate through a later safe edit", async () => {
    ...
    const restartOnlyConfig: OperatorConfig = {
      ...initialConfig,
      gateway: {
  ...
  it("skips plugin-local invalid reloads without degraded mode", async () => {
    const activeConfig: OperatorConfig = {
      gateway: { reload: { debounceMs: 0 } },
    ...
    const promoteSnapshot = vi.fn(async (_snapshot: ConfigFileSnapshot, _reason: string) => true);
    const previousConfig: OperatorConfig = {
      ...activeConfig,
      plugins: {
        entries: {
          "lossless-claw": {
  ...
  it.each([
    ...
    async (testCase) => {
      const envKey = "OPERATOR_TEST_RELOAD_TRANSACTION_ENV";
      const targetEnv: NodeJS.ProcessEnv = { [envKey]: "old" };
      const initialConfig = {
  ...
  it.each([
    ...
    const envKey = "OPERATOR_TEST_RELOAD_ENV_COMMIT_EDGE";
    const targetEnv: NodeJS.ProcessEnv = { [envKey]: "old" };
    const initialConfig = {
      gateway: { reload: { debounceMs: 0 } },
  ...
  it("keeps a deferred config env candidate isolated when a watcher supersedes it", async () => {
    const envKey = "OPERATOR_TEST_SUPERSEDED_RELOAD_ENV";
    const targetEnv: NodeJS.ProcessEnv = { [envKey]: "old" };
    const initialConfig = {
      gateway: { reload: { debounceMs: 0 } },
      env: { vars: { [envKey]: "old" } },
  ...
  it("reprepares a stale managed-write env candidate after another transaction accepts", async () => {
    const envKey = "OPERATOR_TEST_INTERLEAVED_RELOAD_ENV";
    const targetEnv: NodeJS.ProcessEnv = { [envKey]: "a" };
    const makeConfig = (value: string, port: number): OperatorConfig => ({
  ...
  it("retains the accepted candidate overlay when a watcher echoes the same hash", async () => {
    ...
    const applyDebugOverride = (config: OperatorConfig): OperatorConfig => ({
      ...config,
      logging: { level: "debug" },
    });
  ...
  it("does not suppress functional install changes that collide with timestamp paths", async () => {
    ...
    const nextSourceConfig: OperatorConfig = {
      gateway: { reload: { debounceMs: 0 } },
      plugins: {
        installs: {
          lossless: {
