...
import { buildSessionHistorySnapshot, SessionHistorySseState } from "../../src/gateway/session-history-state.js";
import * as sessionTranscriptReaders from "../../src/gateway/session-transcript-readers.js";
...
describe("SessionHistorySseState", () => {
  ...
  test("carries inline user idempotency keys into history metadata", () => {
    ...
    expect(
      (
        appended!.message as {
          __operator?: { id?: string; idempotencyKey?: string; seq?: number };
        }
      )["__operator"],
    ).toMatchObject({
      id: "message-user-2",
  ...
  test("reuses one canonical array for items and messages", () => {
    ...
    expect(snapshot.history.messages[0]?.["__operator"]?.seq).toBe(2);
  ...
  test("uses carried sequence for inline SSE appends", () => {
    ...
    expect(state.snapshot().messages.at(-1)?.["__operator"]?.seq).toBe(9);
  });
  ...
  test("keeps cursors when a paginated history page starts with a message-tool mirror", () => {
    ...
    expect(snapshot.history.messages[0]?.["__operator"]?.seq).toBe(3);
    expect(
  ...
  test("does not coerce partial cursor values", () => {
    ...
    expect(snapshot.history.messages.map((message) => message["__operator"]?.seq)).toEqual([1, 2]);
  });
  ...
  test("requests refresh when inline TTS supplement merges into an existing assistant message", () => {
    ...
    const appended = state.appendInlineMessage({
      message: {
        ...
        operatorTtsSupplement: { textSha256, spokenText: visibleText },
      },
      messageSeq: 3,
    });
  ...
  test("requests refresh for non-monotonic carried inline sequence", () => {
    ...
    expect(appended).toEqual({ shouldRefresh: true });
    expect(state.snapshot().messages).toHaveLength(1);
    expect(state.snapshot().messages.at(-1)?.["__operator"]?.seq).toBe(5);
  });
  ...
  test("carries a hidden heartbeat boundary into the next visible SSE append", () => {
    ...
    expect(compaction?.message?.["__operator"]?.turnBoundary).toBeUndefined();
