...
import { cleanupArchivedSessionTranscripts } from "../../src/gateway/session-transcript-files.fs.js";
...
describe("cleanupArchivedSessionTranscripts", () => {
  ...
  beforeEach(async () => {
    dir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "operator-archive-cleanup-"));
  });
