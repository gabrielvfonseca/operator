...
import { resolveTailnetDnsHint } from "../../src/gateway/server-discovery.js";

describe("resolveTailnetDnsHint", () => {
  const prevTailnetDns = { value: undefined as string | undefined };

  beforeEach(() => {
    prevTailnetDns.value = process.env.OPERATOR_TAILNET_DNS;
    delete process.env.OPERATOR_TAILNET_DNS;
  ...
  afterEach(() => {
    if (prevTailnetDns.value === undefined) {
      ...
      process.env.OPERATOR_TAILNET_DNS = prevTailnetDns.value;
    }
  });

  test("returns env hint when disabled", async () => {
    process.env.OPERATOR_TAILNET_DNS = "studio.tailnet.ts.net.";
