...
import { resetContainerEnvironmentCacheForTest } from "../../src/infra/container-environment.js";
import { resolveGatewayRuntimeConfig } from "../../src/gateway/server-runtime-config.js";
...
describe("resolveGatewayRuntimeConfig", () => {
  ...
  describe("token/password auth modes", () => {
    ...
    beforeEach(() => {
      originalToken = process.env.OPERATOR_GATEWAY_TOKEN;
      delete process.env.OPERATOR_GATEWAY_TOKEN;
    });

    afterEach(() => {
      if (originalToken !== undefined) {
        process.env.OPERATOR_GATEWAY_TOKEN = originalToken;
      } else {
        delete process.env.OPERATOR_GATEWAY_TOKEN;
      }
    });
    ...
    it.each([
      {
        ...
        expectedMessage:
          "gateway auth mode is token, but no token was configured (set gateway.auth.token or OPERATOR_GATEWAY_TOKEN)",
      },
      {
