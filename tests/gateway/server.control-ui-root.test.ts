...
import { installGatewayTestHooks, testState, withGatewayServer } from "../../src/gateway/test-helpers.js";

installGatewayTestHooks({ scope: "suite" });

async function withGlobalControlUiHardlinkFixture<T>(run: (rootPath: string) => Promise<T>) {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "operator-gateway-ui-hardlink-"));
  try {
...
describe("gateway.controlUi.root", () => {
  test("rejects hardlinked index.html when configured root points at global Operator package control-ui", async () => {
