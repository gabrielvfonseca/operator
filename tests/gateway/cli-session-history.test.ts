...
import { readClaudeCliSessionMessages } from "../../src/gateway/cli-session-history.claude.js";
import {
  augmentChatHistoryWithCliSessionImports,
  readClaudeCliFallbackSeed,
  resolveChatHistoryWithCliSessionImports,
} from "../../src/gateway/cli-session-history.js";
import { mergeImportedChatHistoryMessages } from "../../src/gateway/cli-session-history.merge.js";
import { expectRecordFields, requireRecord } from "../../src/gateway/test-helpers.assertions.js";
...
async function withClaudeProjectsDir<T>(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "operator-claude-history-"));
  ...
  it("suppresses only the first user row with a trusted omission receipt", async () => {
    await withClaudeProjectsDir(async ({ homeDir, sessionId, filePath }) => {
      ...
      const messages = readClaudeCliSessionMessages({
        cliSessionId: sessionId,
        homeDir,
        localSessionId: "operator-session",
        reseedReceipt: {
          version: 1,
  ...
  it("fails open when the receipt belongs to a different local session", async () => {
    await withClaudeProjectsDir(async ({ homeDir, sessionId, filePath }) => {
      ...
      const messages = readClaudeCliSessionMessages({
        cliSessionId: sessionId,
        homeDir,
        localSessionId: "new-operator-session",
        reseedReceipt: {
          version: 1,
          promptHash: hashCliReseedPrompt(transformedPrompt),
          localSessionId: "old-operator-session",
          userTurnDisposition: "persisted",
        },
      });
...
describe("readClaudeCliFallbackSeed", () => {
  ...
  beforeEach(async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), "operator-fallback-seed-"));
