...
import type { normalizeProviderModelIdWithRuntime } from "../../src/agents/provider-model-normalization.runtime.js";
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...
    listOperatorPluginManifestMetadata: (
      params?: Parameters<typeof actual.listOperatorPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listOperatorPluginManifestMetadata(params);
      return actual.listOperatorPluginManifestMetadata(params);
    },
  };
});
...
} from "../../src/gateway/model-pricing-cache-state.js";
import {
  getCachedGatewayModelPricing,
  startGatewayModelPricingRefresh,
} from "../../src/gateway/model-pricing-cache.js";
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listOperatorPluginManifestMetadata.mockClear();
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listOperatorPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/operator.plugin.json",
    ...overrides,
  };
}
