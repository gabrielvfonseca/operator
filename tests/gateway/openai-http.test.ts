...
import { createClientToolNameConflictError } from "../../src/agents/agent-tool-definition-adapter.js";
import {
  createStubSessionHarness,
  emitAssistantTextDelta,
} from "../../src/agents/embedded-agent-subscribe.e2e-harness.js";
...
import { buildAssistantDeltaResult } from "../../src/gateway/test-helpers.agent-results.js";
import {
  agentCommand,
  getFreePort,
...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("OPERATOR_CONFIG_PATH is required for gateway config tests");
  }
  ...
  it("handles request validation and routing", async () => {
    ...
    try {
      ...
      await expectAgentSessionKeyMatch({
        body: { model: "@gabrielvfonseca/operator", messages: [{ role: "user", content: "hi" }] },
        headers: { "x-operator-agent-id": "beta" },
        matcher: /^agent:beta:/,
      });
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "@gabrielvfonseca/operator", messages: [{ role: "user", content: "hi" }] },
...

      {
        ...
        const res = await postChatCompletions(
          ...
          { model: "@gabrielvfonseca/operator", messages: [{ role: "user", content: "hi" }] },
          {
            "x-operator-session-key": "agent:main:harness:codex:supervision:spoofed-native-thread",
          },
        );
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        ...
        expect(json.error?.message).toBe(
          "`x-operator-session-key` cannot use reserved internal session namespaces.",
        );
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }

      {
        ...
        const res = await postChatCompletions(
          ...
          { model: "@gabrielvfonseca/operator", messages: [{ role: "user", content: "hi" }] },
          {
            ...
            "x-operator-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "@gabrielvfonseca/operator", messages: [{ role: "user", content: "hi" }] },
          { "x-operator-session-key": "agent:main:subagent:spoofed" },
        );
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
...

      {
        ...
        const res = await postChatCompletions(
          ...
          { "x-operator-message-channel": "custom-client-channel" },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Invalid `x-operator-model`.");
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from Operator.");
      }
