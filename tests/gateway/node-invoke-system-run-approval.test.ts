...
import { ExecApprovalManager, type ExecApprovalRecord } from "../../src/gateway/exec-approval-manager.js";
import { sanitizeSystemRunParamsForForwarding } from "../../src/gateway/node-invoke-system-run-approval.js";
...
describe("sanitizeSystemRunParamsForForwarding", () => {
  ...
  function makeNoDeviceUiRecord(
    ...
    record.requestedByClientId = overrides.requestedByClientId ?? "operator-control-ui";
  ...
  test("rejects no-device approval replay from a non-backend client on a different connection", () => {
    const result = sanitizeApprovedRun({
      ...
      client: {
        ...
        connect: {
          scopes: ["operator.write", "operator.approvals"],
          client: { id: "operator-control-ui", mode: "ui" },
          device: null,
        },
      },
      record: makeNoDeviceUiRecord(),
