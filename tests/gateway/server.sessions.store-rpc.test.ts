...
import { agentDiscoveryMock, rpcReq, testState, writeSessionStore } from "../../src/gateway/test-helpers.js";
import {
...
test("lists and patches session store via sessions.* RPC", async () => {
  ...
  expect(mainAfterModelPatch?.agentRuntime).toEqual({
    id: "@gabrielvfonseca/operator",
    source: "implicit",
  });
...
test("sessions.list configuredAgentsOnly keeps configured-agent children and hides unrelated stores", async () => {
  ...
  if (!stateDir) {
    throw new Error("OPERATOR_STATE_DIR is required for gateway session tests");
  }
  testState.agentsConfig = { list: [{ id: "main", default: true }] };
  const configPath = process.env.OPERATOR_CONFIG_PATH;
  if (!configPath) {
    throw new Error("OPERATOR_CONFIG_PATH is required for gateway session tests");
  }
  await fs.writeFile(
    configPath,
