...
import { tryListenOnPort } from "../../src/infra/ports-probe.js";
import { getFreePort, installGatewayTestHooks, startGatewayServer } from "../../src/gateway/test-helpers.js";
import { createGatewayRuntimeStateForTest } from "../../src/gateway/test-helpers.server-runtime-state.js";
...
describe("gateway startup websocket readiness", () => {
  ...
  it("accepts an immediate websocket connection once startup resolves", async () => {
    const previousMinimal = process.env.OPERATOR_TEST_MINIMAL_GATEWAY;
    process.env.OPERATOR_TEST_MINIMAL_GATEWAY = "0";
    ...
    try {
      ...
      if (previousMinimal === undefined) {
        ...
        process.env.OPERATOR_TEST_MINIMAL_GATEWAY = previousMinimal;
      }
    }
  });
