...
import { tryBeginGatewaySuspendAdmission } from "../../src/process/gateway-work-admission.js";
...
import { createPreauthConnectionBudget } from "../../src/gateway/server/preauth-connection-budget.js";
import {
  GATEWAY_WS_CONNECTION_KIND_PROPERTY,
  GATEWAY_WS_PREAUTH_BUDGET_PROPERTY,
  type GatewayIngressWebSocket,
  type GatewayWsClient,
} from "../../src/gateway/server/ws-types.js";
import { testState } from "../../src/gateway/test-helpers.runtime-state.js";
import {
  createGatewaySuiteHarness,
  installGatewayTestHooks,
  readConnectChallengeNonce,
} from "../../src/gateway/test-helpers.server.js";
import { readClientResponseBody } from "../../src/gateway/test-http-response.js";
...
describe("gateway pre-auth hardening", () => {
  ...
  it("closes idle unauthenticated sockets after the handshake timeout", async () => {
    setEnvForTest("OPERATOR_TEST_HANDSHAKE_TIMEOUT_MS", "200");
  ...
  it("uses gateway.handshakeTimeoutMs for idle unauthenticated sockets", async () => {
    ...
    if (!configPath) {
      throw new Error("OPERATOR_CONFIG_PATH missing in gateway preauth test");
    }
    await writeFile(
      configPath,
      JSON.stringify(
  ...
  it("rejects excess simultaneous unauthenticated sockets from the same client ip", async () => {
    setEnvForTest("OPERATOR_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP", "1");
