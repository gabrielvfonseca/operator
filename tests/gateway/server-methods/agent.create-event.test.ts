...
} from "../../../src/gateway/active-sessions-shutdown-tracker.js";
  ...
  getRuntimeConfig: vi.fn(() => ({
    agents: {
      defaults: {
        ...
        workspace: configMocks.workspaceDir || "/tmp/operator-agent-create-event",
      },
    },
    session: {
      mainKey: "main",
...
describe("agent handler session create events", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-agent-create-event-"));
