...
import { resolveAttachGrant } from "../../../src/gateway/mcp-grant-store.js";
import { closeMcpLoopbackServer } from "../../../src/gateway/mcp-http.js";
import { attachHandlers } from "../../../src/gateway/server-methods/attach.js";
...
describe("attach gateway methods", () => {
  ...
  it("attach.grant mints a session-bound grant and returns loopback config + token env", async () => {
    ...
    expect(body.env.OPERATOR_MCP_TOKEN).toBe(body.token);
    expect(Object.keys(body.env)).toEqual(["OPERATOR_MCP_TOKEN"]);
