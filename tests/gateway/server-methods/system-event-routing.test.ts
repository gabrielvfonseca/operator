...
import { systemHandlers } from "../../../src/gateway/server-methods/system.js";
...
describe("system-event routing", () => {
  ...
  it("queues and immediately wakes the requested session", async () => {
    ...
    const request = {
      params: {
        text: "Operator updated. Welcome the user back.",
        sessionKey,
        wake: true,
      },
      respond,
    ...
    expect(peekSystemEvents(sessionKey)).toEqual(["Operator updated. Welcome the user back."]);
