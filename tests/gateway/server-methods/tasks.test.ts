...
import { reloadTaskRegistryFromStore } from "../../../src/tasks/task-registry.js";
import { saveTaskRegistryStateToSqlite } from "../../../src/tasks/task-registry.store.sqlite.js";
...
import { tasksHandlers } from "../../../src/gateway/server-methods/tasks.js";
...
beforeEach(async () => {
  stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-gateway-tasks-"));
...
describe("tasks gateway handlers", () => {
  ...
  it("sanitizes task text before exposing SDK summaries", async () => {
    const task = createTaskRecord({
      ...
      runId: "run-sanitized",
      label:
        "Compile artifact\nOperator runtime context (internal): Keep internal details private.",
    ...
    expect(JSON.stringify(calls[0]?.[1])).not.toContain("Operator runtime context");
  });
