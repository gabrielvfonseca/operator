...
import { installGatewayTestHooks, testState, writeSessionStore } from "../../../src/gateway/test-helpers.js";
import { getGatewayConfigModule, sessionStoreEntry } from "../../../src/gateway/test/server-sessions.test-helpers.js";
import { testing, toolsEffectiveHandlers } from "../../../src/gateway/server-methods/tools-effective.js";
...
describe("tools.effective global agent integration", () => {
  ...
  async function seedSelectedGlobalStores() {
    ...
    if (!stateDir) {
      throw new Error("OPERATOR_STATE_DIR is required");
    }
