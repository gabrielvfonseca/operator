...
import { usageHandlers } from "../../../src/gateway/server-methods/usage.js";
...
async function runSessionsUsage(
  ...
  config: OperatorConfig = TEST_RUNTIME_CONFIG,
) {
  const respond = vi.fn();
  await expectDefined(
...
async function withUsageState(
  ...
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-usage-test-"));
