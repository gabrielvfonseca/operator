...
import type { GatewayClient, GatewayRequestContext, RespondFn } from "../../../src/gateway/server-methods/types.js";
...
import { sessionsHandlers } from "../../../src/gateway/server-methods/sessions.js";
...
describe("sessions.abort agent scope", () => {
  ...
  it("marks listed sessions active when the embedded or channel reply run registry owns the session id", async () => {
    ...
    listSessionsFromStoreAsyncMock.mockResolvedValue({
      sessions: [{ key: "agent:main:operator-weixin:direct:user", sessionId: "sess-weixin" }],
    });
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        sessions: [
          expect.objectContaining({
            key: "agent:main:operator-weixin:direct:user",
