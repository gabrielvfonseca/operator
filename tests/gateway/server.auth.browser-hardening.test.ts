...
import { CONTROL_UI_CLIENT, TEST_OPERATOR_CLIENT } from "../../src/gateway/server.auth.test-helpers.js";
import {
  connectReq,
  connectOk,
...
async function createSignedBrowserDevice(
  ...
  return createSignedDevice({
    ...
    identityPath: path.join(os.tmpdir(), `operator-${identityName}-device-${randomUUID()}.json`),
    nonce: nonce ?? "",
  });
}
