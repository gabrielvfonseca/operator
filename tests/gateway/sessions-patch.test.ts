...
import { resetProviderAuthAliasMapCacheForTest } from "../../src/agents/provider-auth-aliases.test-support.js";
...
import { withAgentSessionModelPatchOrigin } from "../../src/gateway/session-model-patch-origin.js";
import { applySessionsPatchToStore } from "../../src/gateway/sessions-patch.js";
...
async function applySubagentModelPatch(cfg: OperatorConfig) {
  return expectPatchOk(
    await runPatch({
      cfg,
...
function createAllowlistedAnthropicModelCfg(): OperatorConfig {
  return {
    agents: {
      defaults: {
...
describe("gateway sessions patch", () => {
  ...
  test("persists Operator Luna Ultra through the runtime-aware provider profile", async () => {
    const entry = expectPatchOk(
      await runPatch({
        cfg: {
          agents: {
  ...
  test("honors an explicit Operator session runtime override for Luna Ultra", async () => {
    const entry = expectPatchOk(
      await runPatch({
        cfg: {
