...
import { runSessionsSendA2AFlow } from "../../src/agents/tools/sessions-send-tool.a2a.js";
import {
  loadSessionEntry,
...
describe("sessions_send gateway loopback", () => {
  ...
  it("announces through gateway send using external deliveryContext over stale webchat session fields", {
  ...
  it("does not re-announce a trailing message-tool delivery mirror after a waited A2A run", {
    timeout: SESSION_SEND_E2E_TIMEOUT_MS,
  }, async () => {
...
describe("sessions_send label lookup", () => {
  it("finds session by label and sends message", {
    timeout: SESSION_SEND_E2E_TIMEOUT_MS,
  }, async () => {
...
describe("sessions_send agent targeting", () => {
  it("starts configured agent main session by agentId before sending", {
    timeout: SESSION_SEND_E2E_TIMEOUT_MS,
  }, async () => {
    const configPath = process.env.OPERATOR_CONFIG_PATH;
    if (!configPath) {
