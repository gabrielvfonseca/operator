...
import { STARTUP_UNAVAILABLE_GATEWAY_METHODS } from "../../src/gateway/methods/core-descriptors.js";
...
describe("gateway control-plane write rate limit", () => {
  ...
  it("allows the Operator inference ladder to probe more than 3 candidates", async () => {
    const handlerCalls = vi.fn();
    const handler: GatewayRequestHandler = (opts) => {
      handlerCalls(opts);
