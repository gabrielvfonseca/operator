...
describe("resolveVitestIsolation", () => {
  it("aliases private QA plugin SDK subpaths for source tests only", () => {
    for (const subpath of PRIVATE_PLUGIN_SDK_SUBPATHS) {
      ...
      expect(() =>
        findAlias(sharedVitestConfig.resolve.alias, `@gabrielvfonseca/plugin-sdk/${subpath}`),
      ).toThrow(`missing alias @gabrielvfonseca/plugin-sdk/${subpath}`);
    }
  });
  ...
  it("aliases private core packages to source for clean checkout tests", () => {
    ...
    expect(
      findAlias(sharedVitestConfig.resolve.alias, "@gabrielvfonseca/acp-core/runtime/types"),
    ).toEqual({
      find: "@gabrielvfonseca/acp-core/runtime/types",
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ OPERATOR_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ OPERATOR_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ OPERATOR_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  ...
  it("loads scoped include overrides from OPERATOR_VITEST_INCLUDE_FILE", () => {
  ...
  it("keeps shared gateway include files inside their actual child projects", () => {
    ...
    try {
      ...
      const env = { OPERATOR_VITEST_INCLUDE_FILE: includeFile };
...
describe("scoped vitest configs", () => {
  ...
  it("loads channel include overrides from OPERATOR_VITEST_INCLUDE_FILE", () => {
  ...
  it("normalizes ui include patterns relative to the scoped dir", () => {
    ...
    expect(testConfig.include).toEqual(["ui/tests/**/*.test.ts"]);
    expect(testConfig.exclude).toContain("ui/tests/**/*.e2e.test.ts");
  });
