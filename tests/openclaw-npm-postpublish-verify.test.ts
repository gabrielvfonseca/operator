...
// Operator npm postpublish tests validate postpublish verification behavior.
...
} from "../scripts/operator-npm-postpublish-verify.ts";

const INSTALLED_ROOT_DIST_JS_FILE_SCAN_LIMIT = 10_000;

describe("parseOperatorNpmPostpublishVerifyArgs", () => {
  it("supports help and package-manager separators", () => {
    expect(parseOperatorNpmPostpublishVerifyArgs(["--help"])).toEqual({
      help: true,
      version: "",
    });
    expect(parseOperatorNpmPostpublishVerifyArgs(["--", "2026.3.23"])).toEqual({
      help: false,
      version: "2026.3.23",
    });
  });
  ...
  it("rejects missing, option-like, and extra arguments before verification", () => {
    ...
    expect(() => parseOperatorNpmPostpublishVerifyArgs(["2026.3.23", "extra"])).toThrow(
      "Unexpected operator npm postpublish verifier argument: extra",
    );
  });
});
...
describe("buildPublishedInstallScenarios", () => {
  it("uses a single fresh scenario for plain stable releases", () => {
    expect(buildPublishedInstallScenarios("2026.3.23")).toEqual([
      {
        name: "fresh-exact",
        installSpecs: ["operator@2026.3.23"],
  ...
  it("adds a stable-to-correction upgrade scenario for correction releases", () => {
    expect(buildPublishedInstallScenarios("2026.3.23-2")).toEqual([
      {
        name: "fresh-exact",
        installSpecs: ["operator@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
      {
        name: "upgrade-from-base-stable",
        installSpecs: ["operator@2026.3.23", "operator@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
    ]);
  });
...
describe("npm registry provenance verification", () => {
  ...
  const provenancePayload = {
    ...
    predicate: {
      buildDefinition: {
        externalParameters: {
          workflow: {
            repository: "https://github.com/operator/operator",
            path: ".github/workflows/operator-npm-release.yml",
  ...
  it("fetches npm registry JSON with bounded response handling", async () => {
    ...
    await expect(
      fetchRegistryJson("https://registry.example/operator", {
  ...
  it("bounds oversized npm registry response bodies", async () => {
    ...
    await expect(
      ...
      "npm registry https://registry.example/operator response body exceeded 64 bytes",
    );
  });
  ...
  it("keeps npm registry timeouts active while reading response bodies", async () => {
    ...
    await expect(
      ...
      "npm registry request timed out after 5ms: https://registry.example/operator",
    );
  });
  ...
  it("requires a trusted GitHub release identity for the exact SLSA provenance attestation", async () => {
    ...
    expect(verificationPolicy).toEqual({
      ...
      certificateIdentityURI:
        "https://github.com/gabrielvfonseca/operator/.github/workflows/operator-npm-release.yml@refs/heads/release/2026.3.23",
    });
                    ...
                    predicate: {
                      ...
                      buildDefinition: {
                        ...
                        resolvedDependencies: [
                          {
                            uri: `git+https://github.com/operator/operator@${protectedWorkflowRef}`,
    ...
    expect(verificationPolicy).toEqual({
      certificateIssuer: "https://token.actions.githubusercontent.com",
      certificateIdentityURI: `https://github.com/gabrielvfonseca/operator/.github/workflows/operator-npm-release.yml@${protectedWorkflowRef}`,
    });
            ...
            bundle: {
              dsseEnvelope: {
                payload: Buffer.from(
                  JSON.stringify({
                    ...provenancePayload,
                    subject: [{ name: "pkg:npm/operator@2026.3.24", digest: {} }],
                  }),
                  "utf8",
                ).toString("base64"),
              },
  ...
  it("rejects matching provenance from an untrusted source before Sigstore verification", async () => {
    ...
    ).rejects.toThrow("does not bind 2026.3.23 to the trusted Operator GitHub release workflow");
    ...
    await expect(
      retryNpmRegistryProvenanceRead(
        async () => {
          ...
          if (attempts === 1) {
            throw new Error(
              "npm provenance attestation does not bind 2026.3.23 to the trusted Operator GitHub release workflow.",
            );
          }
          if (attempts === 2) {
            throw new Error(
              "npm registry provenance metadata is incomplete for operator@2026.3.23.",
            );
          }
          return "verified";
        },
...
describe("buildPublishedInstallCommandArgs", () => {
  it("runs lifecycle scripts for published install verification", () => {
    const args = buildPublishedInstallCommandArgs("/tmp/operator-prefix", "operator@2026.4.10");
...
describe("collectInstalledPackageErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "operator-postpublish-package-"));
  }
  ...
  it("flags version mismatches", () => {
    const errors = collectInstalledPackageErrors({
      ...
      packageRoot: "/tmp/empty-operator",
    });
...
describe("collectInstalledAlwaysAllowedRuntimeFacadeErrors", () => {
  function withInstalledPackageRoot(run: (packageRoot: string) => void): void {
    const packageRoot = mkdtempSync(join(tmpdir(), "operator-postpublish-facade-runtime-"));
    try {
      run(packageRoot);
    } finally {
      rmSync(packageRoot, { recursive: true, force: true });
...
describe("collectInstalledContextEngineRuntimeErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "operator-postpublish-context-engine-"));
  }
...
describe("collectInstalledPluginSdkZodArtifactErrors", () => {
  function withInstalledPackageRoot(run: (packageRoot: string) => void): void {
    const packageRoot = mkdtempSync(join(tmpdir(), "operator-postpublish-zod-sdk-"));
    try {
      run(packageRoot);
    } finally {
      rmSync(packageRoot, { recursive: true, force: true });
...
describe("normalizeInstalledBinaryVersion", () => {
  it("accepts decorated CLI version output", () => {
    expect(normalizeInstalledBinaryVersion("Operator 2026.4.8 (9ece252)")).toBe("2026.4.8");
    expect(normalizeInstalledBinaryVersion("Operator 2026.4.8-beta.1 (9ece252)")).toBe(
      "2026.4.8-beta.1",
    );
    expect(normalizeInstalledBinaryVersion("Operator 2026.4.8-alpha.1 (9ece252)")).toBe(
      "2026.4.8-alpha.1",
    );
  });
});
...
describe("resolveInstalledBinaryPath", () => {
  it("uses the Unix global bin path on non-Windows platforms", () => {
    expect(resolveInstalledBinaryPath("/tmp/operator-prefix", "darwin")).toBe(
      "/tmp/operator-prefix/bin/operator",
    );
  });

  it("uses the Windows npm shim path on win32", () => {
    expect(resolveInstalledBinaryPath("C:/operator-prefix", "win32")).toBe(
      "C:\\operator-prefix\\operator.cmd",
    );
  });
});
...
describe("resolveInstalledBinaryCommandInvocation", () => {
  it("runs the Unix installed binary directly", () => {
    expect(
      resolveInstalledBinaryCommandInvocation("/tmp/operator-prefix", ["--version"], {
        platform: "linux",
      }),
    ).toEqual({
      command: "/tmp/operator-prefix/bin/operator",
      args: ["--version"],
    });
  });
  ...
  it("wraps the Windows installed npm shim without Node shell argv", () => {
    expect(
      ...
      args: [
        ...
        "/s",
        "/c",
        '""C:\\operator prefix\\operator.cmd" agent --message "hello world""',
      ],
...
describe("collectInstalledRootDependencyManifestErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "operator-postpublish-root-deps-"));
  }
