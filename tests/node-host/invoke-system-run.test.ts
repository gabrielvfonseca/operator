...
import type { SystemRunApprovalPlan } from "../../src/infra/exec-approvals.js";
import {
...
import type { ExecAutoReviewer } from "../../src/infra/exec-auto-review.js";
import type { ExecHostResponse } from "../../src/infra/exec-host.js";
import { withEnvAsync } from "../../src/test-utils/env.js";
import { buildSystemRunApprovalPlan } from "../../src/node-host/invoke-system-run-plan.js";
import { handleSystemRunInvoke } from "../../src/node-host/invoke-system-run.js";
...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  ...
  beforeAll(() => {
    sharedFixtureRoot = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), "operator-node-host-fixtures-")),
    );
    sharedOperatorHome = path.join(sharedFixtureRoot, "operator-home");
  ...
  it("uses auto reviewer for system.run approval misses when exec mode is auto", async () => {
    const tmp = createFixtureDir("operator-system-run-auto-review-");
  ...
  it("does not auto-review direct system.run approval misses without an approval plan", async () => {
    const tmp = createFixtureDir("operator-system-run-auto-review-no-plan-");
    const executablePath = createTempExecutable({ dir: tmp, name: "read-info" });
    setRuntimeConfigSnapshot({
      tools: {
        exec: {
  ...
  it("does not auto-review direct system.run security audit suppression edits", async () => {
    const tmp = createFixtureDir("operator-system-run-auto-review-suppression-");
    const executablePath = createTempExecutable({ dir: tmp, name: "@gabrielvfonseca/operator" });
    setRuntimeConfigSnapshot({
      tools: {
        exec: {
          mode: "auto",
  ...
  it("defers to human approval when system.run auto reviewer asks", async () => {
    const tmp = createFixtureDir("operator-system-run-auto-review-ask-");
    const executablePath = createTempExecutable({ dir: tmp, name: "read-info" });
    setRuntimeConfigSnapshot({
      tools: {
        exec: {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      for (const testCase of approvedEnvShellWrapperCases) {
        const tmp = createFixtureDir("operator-approved-wrapper-");
  ...
  it("denies abbreviated PowerShell encoded payloads even when the wrapper is allowlisted", async () => {
    const binDir = createFixtureDir("operator-pwsh-allowlist-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withPathTokenCommand({
        tmpPrefix: "operator-approval-path-pin-",
        run: async ({ expected }) => {
          const { runCommand, sendInvokeResult } = await runSystemInvoke({
            preferMacAppExecHost: false,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const sendInvokeResult = vi.fn(async () => {});
      await withPathTokenCommand({
        tmpPrefix: "operator-allowlist-path-pin-",
      ...
      for (const testCase of [
        {
          label: "cwd symlink",
          setup: () => {
            const tmp = createFixtureDir("operator-approval-cwd-link-");
        ...
        },
        {
          ...
          setup: () => {
            const tmp = createFixtureDir("operator-approval-cwd-parent-link-");
  ...
  it("uses canonical executable path for approval-based relative command execution", async () => {
    const tmp = createFixtureDir("operator-approval-cwd-real-");
  ...
  it("denies approval-based execution when cwd identity drifts before execution", async () => {
    const tmp = createFixtureDir("operator-approval-cwd-drift-");
    const fallback = createFixtureDir("operator-approval-cwd-drift-alt-");
    const script = path.join(tmp, "run.sh");
    fs.writeFileSync(script, "#!/bin/sh\necho SAFE\n");
    fs.chmodSync(script, 0o755);
    const canonicalCwd = fs.realpathSync(tmp);
  ...
  it("validates approved script operand bindings at dispatch", async () => {
    for (const mutate of [true, false]) {
      const tmp = createFixtureDir(
        mutate ? "operator-approval-script-drift-" : "operator-approval-script-stable-",
      );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tmp = createFixtureDir("operator-approval-cwd-post-commit-drift-");
  ...
  it("revalidates approved script operands after authorization commit", async () => {
    const tmp = createFixtureDir("operator-approval-script-post-commit-drift-");
    const fixture = createMutableScriptOperandFixture(tmp);
    fs.writeFileSync(fixture.scriptPath, fixture.initialBody);
    if (process.platform !== "win32") {
      fs.chmodSync(fixture.scriptPath, 0o755);
  ...
  it("validates approved runtime script operand bindings at dispatch", async () => {
    await withFakeRuntimeOnPath({
      runtime: "tsx",
      run: async () => {
        const tmp = createFixtureDir("operator-approval-tsx-script-drift-");
        ...
        const missingBindingTmp = createFixtureDir("operator-approval-tsx-missing-binding-");
  ...
  it("denies ./sh wrapper spoof in allowlist on-miss mode before execution", async () => {
    const marker = path.join(os.tmpdir(), `operator-wrapper-spoof-${process.pid}-${Date.now()}`);
    const runCommand = vi.fn(async () => {
  ...
  it("fails closed when allow-always approval persistence fails", async () => {
    await withTempApprovalsHome({
      approvals: createAllowlistOnMissApprovals(),
      run: async () => {
        const tempDir = createFixtureDir("operator-allow-always-write-failure-");
  ...
  it("does not restore a revoked allowlist rule during explicit allow-always persistence", async () => {
    const tempDir = createFixtureDir("operator-allow-always-revoked-rule-");
    const executablePath = createTempExecutable({ dir: tempDir, name: "approved-tool" });
  ...
  it("fails closed when allowlist usage persistence fails", async () => {
    const tempDir = createFixtureDir("operator-allowlist-usage-write-failure-");
  ...
  it("does not let forwarded auto-review authorize security audit suppression edits", async () => {
    const tmp = createFixtureDir("operator-forwarded-auto-review-suppression-");
    const executablePath = createTempExecutable({ dir: tmp, name: "@gabrielvfonseca/operator" });
    const prepared = buildSystemRunApprovalPlan({
      command: [executablePath, "config", "set", "security.audit.suppressions", "[]"],
      cwd: tmp,
  ...
  it("preserves exact-plan forwarded auto-review for strict inline eval", async () => {
    const plan = createStrictInlineEvalApprovalPlan("operator-forwarded-inline-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = createFixtureDir("operator-fallback-durable-");
      const prepared = buildSystemRunApprovalPlan({
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = createFixtureDir("operator-fallback-durable-revoked-");
      const prepared = buildSystemRunApprovalPlan({
        command: ["/bin/sh", "-c", "/bin/ls"],
        cwd: tempDir,
        sessionKey: "agent:main:main",
  ...
  it("does not let timeout fallback satisfy strict inline review", async () => {
    const plan = createStrictInlineEvalApprovalPlan("operator-fallback-inline-");
    setRuntimeConfigSnapshot({ tools: { exec: { strictInlineEval: true } } });
    try {
      await withTempApprovalsHome({
        approvals: {
  ...
  it("does not let timeout fallback authorize security audit suppression edits", async () => {
    const tmp = createFixtureDir("operator-timeout-fallback-suppression-");
    const executablePath = createTempExecutable({ dir: tmp, name: "@gabrielvfonseca/operator" });
    const prepared = buildSystemRunApprovalPlan({
      command: [executablePath, "config", "set", "security.audit.suppressions", "[]"],
      cwd: tmp,
  ...
  it("keeps audit suppression edits approval-gated under allowlist fallback from full/off", async () => {
    const tmp = createFixtureDir("operator-timeout-fallback-full-off-suppression-");
    const executablePath = createTempExecutable({ dir: tmp, name: "@gabrielvfonseca/operator" });
    const prepared = buildSystemRunApprovalPlan({
      command: [executablePath, "config", "set", "security.audit.suppressions", "[]"],
      cwd: tmp,
  ...
  it("rejects unbindable strict inline-eval carriers before delayed approval", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("operator-inline-eval-bin-");
  ...
  it("persists benign awk allow-always approvals in strict inline-eval mode without reopening inline carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("operator-inline-eval-awk-");
          const executablePath = createTempExecutable({
            dir: tempDir,
            name: "gawk",
          });
  ...
  it("does not persist allow-always approvals for strict inline-eval make carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("operator-inline-eval-make-");
          const executablePath = createTempExecutable({
            dir: tempDir,
            name: "make",
          });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = createFixtureDir("operator-shell-wrapper-inner-");
  ...
  it("keeps cmd.exe transport wrappers approval-gated on Windows", async () => {
    ...
    try {
      for (const testCase of [
        ...
        const tempDir = createFixtureDir("operator-cmd-wrapper-allow-");
  ...
  it("fails closed when cmd.exe wrapper trust is downgraded before execution", async () => {
    const platformSpy = vi.spyOn(process, "platform", "get").mockReturnValue("win32");
    try {
      const tempDir = createFixtureDir("operator-cmd-wrapper-downgraded-");
  ...
  it("reuses exact-command durable trust for shell-wrapper reruns", async () => {
    ...
    const tempDir = createFixtureDir("operator-shell-wrapper-allow-");
    const prepared = buildSystemRunApprovalPlan({
      command: ["/bin/sh", "-c", "/bin/ls"],
      cwd: tempDir,
    });
  ...
  it("does not bind safe builtin policy to a redundant exact-command grant", async () => {
    ...
    const tempDir = createFixtureDir("operator-shell-wrapper-redundant-grant-");
    const prepared = buildSystemRunApprovalPlan({
      command: ["/bin/sh", "-c", "cd ."],
      cwd: tempDir,
    });
  ...
  it("fails closed when an exact-command grant is revoked before execution", async () => {
    ...
    const tempDir = createFixtureDir("operator-shell-wrapper-revoked-");
    const prepared = buildSystemRunApprovalPlan({
      command: ["/bin/sh", "-c", "/bin/ls"],
      cwd: tempDir,
    });
