...
import { withTempDir, withTempDirSync } from "../../src/test-helpers/temp-dir.js";

const parentRoots: string[] = [];

async function makeParentRoot(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "operator-temp-dir-helper-test-"));
...
describe("withTempDir", () => {
  it("removes the cached async prefix root when the case finishes", async () => {
    ...
    await withTempDir({ prefix: "operator-leak-check-", parentDir }, async (dir) => {
  ...
  it("keeps the cached async prefix root while another case is active", async () => {
    ...
    const first = withTempDir({ prefix: "operator-shared-root-", parentDir }, async (dir) => {
      await fs.writeFile(path.join(dir, "first.txt"), "ok");
      await firstCanFinish;
    });

    await withTempDir({ prefix: "operator-shared-root-", parentDir }, async (dir) => {
  ...
  it("removes the cached sync prefix root when the case finishes", async () => {
    ...
    withTempDirSync({ prefix: "operator-leak-check-sync-", parentDir }, (dir) => {
