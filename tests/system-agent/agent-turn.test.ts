...
import { fingerprintResolvedProviderAuth } from "../../src/agents/execution-auth-binding.js";
import type { CliBackendConfig, OperatorConfig } from "../../src/config/types.js";
import {
  cleanupSystemAgentSession,
  createSystemAgentSession,
  type SystemAgentSession,
} from "../../src/system-agent/agent-turn.js";
import { runSystemAgentTurnWithDeps, type SystemAgentTurnDeps } from "../../src/system-agent/agent-turn.test-support.js";
...
import { createSystemAgentVerifiedInferenceTestFixture } from "../../src/system-agent/system-agent.test-helpers.js";
import { createSystemAgentVerifiedInferenceBinding } from "../../src/system-agent/verified-inference.js";
...
async function createVerifiedSession(config: OperatorConfig) {
...
describe("runSystemAgentTurn", () => {
  ...
  it("uses the default agent CLI route while keeping Operator session identity", async () => {
  ...
  it("rejects an always-on CLI backend before launching Operator", async () => {
    useTempStateDir();
    const config = {
      agents: {
        defaults: {
    ...
    expect((failure as SystemAgentInferenceUnavailableError).failures).toEqual([
      expect.objectContaining({
        message: expect.stringContaining(
          "CLI backend google-gemini-cli cannot enforce Operator's exact tool availability",
        ),
      }),
    ]);
          ...
          list: [
            {
              id: "ops",
              default: true,
              // Keep the model owner's policy stable. Operator executes with
  ...
  it("uses the default agent embedded model, auth directory, profile, and runtime", async () => {
    ...
    expect(requireValue(call.systemAgentTool, "missing embedded Operator tool").proposalRef).toBe(
      session.proposalRef,
    );
  });
