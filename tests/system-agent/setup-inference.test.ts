...
import { resolveAgentDir } from "../../src/agents/agent-scope-config.js";
import {
...
import { updateAuthProfileStoreWithLock } from "../../src/agents/auth-profiles/store.js";
import {
  fingerprintAuthProfileCredential,
  fingerprintResolvedProviderAuth,
  type AgentExecutionAuthBinding,
} from "../../src/agents/execution-auth-binding.js";
import { detectInferenceBackends } from "../../src/commands/onboard-inference.js";
...
import { withoutPluginInstallRecords } from "../../src/plugins/installed-plugin-index-records.js";
import { hasRetainedManagedNpmInstallMarker } from "../../src/plugins/managed-npm-retention.js";
import type { ProviderAuthChoiceMetadata } from "../../src/plugins/provider-auth-choices.js";
...
import { applySystemAgentModelSelection } from "../../src/system-agent/setup-apply.js";
import { resolveSetupInferenceProbeStreamParams } from "../../src/system-agent/setup-inference-probe.js";
import {
...
} from "../../src/system-agent/setup-inference.js";
import {
  createSystemAgentVerifiedInferenceBinding,
  type SystemAgentVerifiedInferenceBinding,
} from "../../src/system-agent/verified-inference.js";
...
function successfulAgentHarnessBinding(params?: SuccessfulRunParams): AgentExecutionAuthBinding {
  ...
  const agentHarnessId =
    !requestedHarnessId || requestedHarnessId === "auto"
...
describe("applySystemAgentModelSelection", () => {
  ...
  it("overrides higher-priority runtime metadata on an inheriting default agent", async () => {
    ...
    expect(config.agents.list[0]?.models["openai/gpt-5.5"]?.agentRuntime?.id).toBe(
      "@gabrielvfonseca/operator",
    );
  });
});
...
describe("detectSetupInference", () => {
  ...
  it("surfaces an invalid existing config instead of treating it as fresh", async () => {
    ...
    await expect(detectSetupInference()).rejects.toThrow(
      "Operator config /tmp/operator.json is invalid (agents.defaults.model: Expected a model reference)",
    );
  });
...
describe("activateSetupInference", () => {
  it("omits the token cap when harness selection is automatic", () => {
    ...
    expect(resolveSetupInferenceProbeStreamParams("@gabrielvfonseca/operator")).toEqual({
  ...
  it("surfaces an invalid existing config without probing or persisting", async () => {
    ...
    await expect(
      ...
      "Operator config /tmp/operator.json is invalid (gateway.port: Expected a number). Fix it before running setup.",
    );
  ...
  it("persists inference only after the live test succeeds", async () => {
        ...
        list: [
          {
            id: "ops",
            default: true,
            agentDir: "/tmp/operator-ops-agent",
    ...
    expect(
      probeConfig?.agents?.list?.find((agent) => agent.id === "@gabrielvfonseca/operator"),
    ).toEqual({
      id: "@gabrielvfonseca/operator",
      params: { temperature: 0.2 },
      tools: { allow: ["read"], deny: ["exec"] },
    ...
    expect(
      configHarness
        .current()
        .agents?.list?.find((agent) => agent.id === "@gabrielvfonseca/operator"),
    ).toEqual({
      id: "@gabrielvfonseca/operator",
      params: { temperature: 1.7 },
      tools: { allow: ["exec"] },
  ...
  it("keeps a committed success when temporary cleanup fails", async () => {
    ...
    const result = await activateSetupInference({
      ...
      deps: {
        runCliAgent: vi.fn(successfulRunner("claude-cli", "claude-opus-4-8")) as never,
        transformConfigWithPendingPluginInstalls: configHarness.transform as never,
        createTempDir: async () => "/tmp/operator-setup-cleanup-fixture",
        removeTempDir: async () => {
  ...
  it("disposes the temporary auth database before Windows-style removal", async () => {
    ...
    const databasePath = path.join(tempDir, "agent", "operator-agent.sqlite");
    let disposed = false;
  ...
  it("scopes provider setup to the selected inference route and one credential", async () => {
    ...
    try {
      ...
      expect(probeConfig.agents?.list?.[0]?.models).toMatchObject({
        "groq/llama-3.3-70b-versatile": { agentRuntime: { id: "@gabrielvfonseca/operator" } },
      });
  ...
  it("commits only the refreshed codex record when authored install metadata is stale", async () => {
    const staleAuthoredRecords = {
      ...
      unrelated: {
        source: "npm" as const,
        spec: "@gabrielvfonseca/unrelated@1.0.0",
    ...
    const canonicalRecords = {
      ...
      unrelated: {
        source: "npm" as const,
        spec: "@gabrielvfonseca/unrelated@2.0.0",
        installPath: "/tmp/plugins/unrelated-v2",
      },
    };
    const refreshedCodexRecord = {
      source: "npm" as const,
      spec: "@gabrielvfonseca/codex@3.0.0",
  ...
  it("restores the active registry from persisted config after a failed Codex probe", async () => {
    ...
    try {
      ...
      expect(getActivePluginRegistryWorkspaceDir()).toBe("/tmp/operator-workspace");
    } finally {
      resetPluginRuntimeStateForTest();
    }
  });
  ...
  it("clears transient Codex install caches before and after a failed probe", async () => {
    const installRecords = [
      {
        source: "npm" as const,
        spec: "@gabrielvfonseca/codex@generation-1",
        installPath: "/tmp/plugins/codex-generation-1",
      },
      {
        source: "npm" as const,
        spec: "@gabrielvfonseca/codex@generation-2",
  ...
  it.each([
    { name: "missing", installRecords: {} as Record<string, PluginInstallRecord>, succeeds: false },
    {
      ...
      installRecords: {
        codex: {
          source: "npm" as const,
          spec: "@gabrielvfonseca/codex@other",
...
describe("resolvePersistentApplyInference", () => {
  ...
  it("rejects a liveness turn that used a different actual harness", async () => {
    ...
    changedBinding.execution.agentHarnessRuntimeOverride = "@gabrielvfonseca/operator";
    const resolveVerifiedInferenceRoute = vi.fn(async () => binding.execution);
