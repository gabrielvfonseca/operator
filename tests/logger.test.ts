...
import { isVerbose, isYes, logVerbose, setVerbose, setYes } from "../src/globals.js";
import { logDebug, logError, logInfo, logSuccess, logWarn } from "../src/logger.js";
import {
  resetLogger,
  setLoggerOverride,
...
describe("logger helpers", () => {
  ...
  it("writes to configured log file at configured level", () => {
    withTempDirSync({ prefix: "operator-log-test-" }, (dir) => {
      const logPath = path.join(dir, "operator.log");
  ...
  it("uses daily rolling log files and prunes old ones", () => {
    withTempDirSync({ prefix: "operator-log-test-" }, (dir) => {
      ...
      const todayPath = path.join(dir, `operator-${today}.log`);
      setLoggerOverride({ level: "info", file: todayPath });

      // create an old file to be pruned
      const oldPath = path.join(dir, "operator-2000-01-01.log");
