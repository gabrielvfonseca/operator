...
import { REALTIME_VOICE_AGENT_CONSULT_TOOL } from "../../src/talk/agent-consult-tool.js";
import {
...
function requireEmbeddedAgentCall(runEmbeddedAgent: {
  ...
  if (!call) {
    throw new Error("Expected embedded Operator agent call");
  }
  const [params] = call;
  if (typeof params !== "object" || params === null || Array.isArray(params)) {
    throw new Error("Expected embedded Operator agent params to be an object");
  }
  ...
  it("runs an embedded agent using the shared session and prompt contract", async () => {
    ...
    expect(call.prompt).toBe(
      [
        ...
        "Act as the configured Operator agent on behalf of this user. Use available tools when the request asks you to do work.",
    ...
    expect(call.extraSystemPrompt).toBe(
      "You are the configured Operator agent receiving delegated requests from a live voice bridge. Act on behalf of the user, use available tools when appropriate, and return a brief speakable result.",
    );
  });
