...
import type { RealtimeVoiceAgentRunActivity } from "../../src/talk/agent-run-control-shared.js";
import {
...
} from "../../src/talk/agent-run-control.js";
...
describe("controlRealtimeVoiceAgentRun", () => {
  ...
  it("answers status from recent Talk tool events", async () => {
    ...
    expect(result).toMatchObject({
      ...
      mode: "status",
      active: true,
      message: "Operator is working in read (running).",
    });
    expect(deps.queueEmbeddedAgentMessageWithOutcomeAsync).not.toHaveBeenCalled();
  });
  ...
  it("answers status from diagnostic run activity when Talk events are absent", async () => {
    ...
    expect(result).toMatchObject({
      ...
      mode: "status",
      active: true,
      message: "Operator is running exec_command.",
    });
    expect(deps.queueEmbeddedAgentMessageWithOutcomeAsync).not.toHaveBeenCalled();
  });
  ...
  it("skips control tool progress when reporting active run status", async () => {
    ...
    expect(result).toMatchObject({
      ...
      mode: "status",
      active: true,
      message: "Operator is working in exec_command (running).",
    });
    expect(deps.queueEmbeddedAgentMessageWithOutcomeAsync).not.toHaveBeenCalled();
  });
