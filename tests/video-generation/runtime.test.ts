...
} from "../../src/video-generation/runtime.js";
import type { VideoGenerationProvider, VideoGenerationProviderOptionType } from "../../src/video-generation/types.js";

let providers: VideoGenerationProvider[] = [];
let listedConfigs: Array<OperatorConfig | undefined> = [];
...
describe("video-generation runtime", () => {
  ...
  it("lists runtime video-generation providers through the provider registry", () => {
    ...
    expect(
      listRuntimeVideoGenerationProviders({ config: {} as OperatorConfig }, runtimeDeps),
    ).toEqual(registryProviders);
    expect(listedConfigs).toEqual([{} as OperatorConfig]);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    await expect(
      runGenerateVideo({ cfg: {} as OperatorConfig, prompt: "animate a cat" }),
    ).rejects.toThrow(
