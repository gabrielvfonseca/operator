...
import { pickTagline } from "../../src/cli/tagline.js";

const EXPECTED_DEFAULT_TAGLINE = "All your chats, one Operator.";
...
describe("pickTagline", () => {
  ...
  it("keeps OPERATOR_TAGLINE_INDEX behavior in random mode", () => {
    const value = pickTagline({
      mode: "random",
      env: { OPERATOR_TAGLINE_INDEX: "0" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(
  ...
  it("ignores partial OPERATOR_TAGLINE_INDEX values", () => {
    expect(
      pickTagline({
        mode: "random",
        env: { OPERATOR_TAGLINE_INDEX: "1abc" } as NodeJS.ProcessEnv,
        random: () => 0,
      }),
