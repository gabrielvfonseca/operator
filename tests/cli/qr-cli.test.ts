...
import { createCliRuntimeCapture, mockRuntimeModule } from "../../src/cli/test-runtime-capture.js";
...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    resetRuntimeCapture();
    vi.stubEnv("OPERATOR_GATEWAY_TOKEN", "");
    vi.stubEnv("OPERATOR_GATEWAY_PASSWORD", "");
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("operator devices approve <requestId>");
  });
  ...
  it("uses OPERATOR_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("OPERATOR_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
