...
import { IOS_NODE, createIosNodeListResponse } from "../../src/cli/program.nodes-test-helpers.js";
import { callGateway, runtime } from "../../src/cli/program.test-mocks.js";
...
describe("cli program (nodes media)", () => {
  ...
  it("runs nodes camera snap and prints two MEDIA paths", async () => {
    ...
    expect(mediaPaths[0]).toContain("operator-camera-snap-");
    expect(mediaPaths[1]).toContain("operator-camera-snap-");
  ...
  it("runs one unknown-position camera snap for a Linux node", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /operator-camera-snap-unknown-.*\.jpg$/,
    });
  });
  ...
  it("runs nodes camera clip and prints one MEDIA path", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /operator-camera-clip-front-.*\.mp4$/,
    });
  });
  ...
  it("runs an unknown-position camera clip for a Linux node", async () => {
    ...
    await expectLoggedSingleMediaFile({
      expectedPathPattern: /operator-camera-clip-unknown-.*\.mp4$/,
    });
  });
    ...
    it.each([
      {
        ...
        expectedPathPattern: /operator-camera-snap-front-.*\.jpg$/,
      },
      {
