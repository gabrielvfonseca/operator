...
import { applyParentDefaultHelpAction, isParentDefaultHelpAction } from "../../../src/cli/program/parent-default-help.js";
...
describe("applyParentDefaultHelpAction (#73077)", () => {
  ...
  it("invokes parent help and exits 0 when invoked without subcommand", async () => {
    ...
    const helpSpy = vi.spyOn(parent, "outputHelp").mockImplementation(() => {
      suppressHelpBannerValues.push(process.env.OPERATOR_SUPPRESS_HELP_BANNER);
    });
    ...
    expect(process.env.OPERATOR_SUPPRESS_HELP_BANNER).toBeUndefined();
