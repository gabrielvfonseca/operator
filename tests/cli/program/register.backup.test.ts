...
import { registerBackupCommand } from "../../../src/cli/program/register.backup.js";
...
describe("registerBackupCommand", () => {
  ...
  it("runs backup verify with forwarded options", async () => {
    await runCli(["backup", "verify", "/tmp/operator-backup.tar.gz", "--json"]);

    const options = expectForwardedOptions(backupVerifyCommand);
    expect(options.archive).toBe("/tmp/operator-backup.tar.gz");
    expect(options.json).toBe(true);
  });
