import { repoInstallSpec } from "@gabrielvfonseca/operator/plugin-sdk/test-fixtures";
...
import { setCommandJsonMode } from "../../../src/cli/program/json-mode.js";
import { applyParentDefaultHelpAction } from "../../../src/cli/program/parent-default-help.js";
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const programLocal = new Command().name("@gabrielvfonseca/operator");
    programLocal
      .command("agent")
  ...
  it("handles debug mode and config-only command preaction", async () => {
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "@gabrielvfonseca/operator", "status", "--debug"],
    });
    ...
    expect(processTitleSetSpy).toHaveBeenCalledWith("operator-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "@gabrielvfonseca/operator", "agents", "list"],
    });
  ...
  it("passes the gateway config recheck to the state migration boundary", async () => {
    await runPreAction({
      parseArgv: ["gateway", "run"],
      processArgv: ["node", "@gabrielvfonseca/operator", "gateway", "run"],
    });
  ...
  it("loads plugins for text local agent runs", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "@gabrielvfonseca/operator", "agent", "--local", "--message", "hi"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "@gabrielvfonseca/operator", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "@gabrielvfonseca/operator", "channels", "add"],
    });
  ...
  it("skips startup bootstrap for parent default help actions", async () => {
    await runPreAction({
      parseArgv: ["channels"],
      processArgv: ["node", "@gabrielvfonseca/operator", "channels"],
    });
  ...
  it("lets configure own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["configure"],
      processArgv: ["node", "@gabrielvfonseca/operator", "configure"],
    });
  ...
  it("lets bare config own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["config"],
      processArgv: ["node", "@gabrielvfonseca/operator", "config"],
    });
  ...
  it("lets guided config sections own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["config"],
      processArgv: ["node", "@gabrielvfonseca/operator", "config", "--section", "models"],
    });
  ...
  it("skips the config guard and plugin loading for doctor lint", async () => {
    await runPreAction({
      parseArgv: ["doctor"],
      processArgv: ["node", "@gabrielvfonseca/operator", "doctor", "--lint"],
    });
  ...
  it("only allows invalid config for explicit official recovery reinstall requests", async () => {
    await runPreAction({
      parseArgv: ["plugins", "install", "@gabrielvfonseca/discord"],
      processArgv: [
        "node",
        "@gabrielvfonseca/operator",
        "plugins",
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@gabrielvfonseca/discord@2026.5.22"],
      processArgv: [
        "node",
        "@gabrielvfonseca/operator",
        "plugins",
        "install",
        "@gabrielvfonseca/discord@2026.5.22",
      ],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@gabrielvfonseca/brave-plugin"],
      processArgv: [
        "node",
        "@gabrielvfonseca/operator",
        "plugins",
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@gabrielvfonseca/slack"],
      processArgv: [
        "node",
        "@gabrielvfonseca/operator",
        "plugins",
        "install",
        "@gabrielvfonseca/slack",
      ],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "@gabrielvfonseca/operator", "plugins", "install", "alpha"],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@gabrielvfonseca/discord", "--marketplace", "local/repo"],
      processArgv: [
        "node",
        "@gabrielvfonseca/operator",
        "plugins",
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "@gabrielvfonseca/operator", "--version"],
    });
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "@gabrielvfonseca/operator", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "@gabrielvfonseca/operator", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "@gabrielvfonseca/operator", "update", "status", "--json"],
    });
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "@gabrielvfonseca/operator", "channels", "send", "--json"],
    });
  ...
  it("uses the Commander action path for protocol stdout ownership", async () => {
    await runPreAction({
      parseArgv: ["acp"],
      processArgv: ["node", "@gabrielvfonseca/operator", "acp", "--token", "-secret"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "@gabrielvfonseca/operator", "acp", "--verbose", "client"],
    });
    ...
    await runPreAction({
      parseArgv: ["mcp", "serve"],
      processArgv: ["node", "@gabrielvfonseca/operator", "mcp", "serve"],
    });
  ...
  it("does not preload plugins for agents list JSON output", async () => {
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "@gabrielvfonseca/operator", "agents", "list", "--json"],
    });
  ...
  it("does not preload plugins for remote agent JSON output", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "@gabrielvfonseca/operator", "agent", "--message", "hi", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "@gabrielvfonseca/operator", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "@gabrielvfonseca/operator", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for config schema", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "@gabrielvfonseca/operator", "config", "schema"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "@gabrielvfonseca/operator", "backup", "create", "--json"],
    });
