...
import { renderRootHelpText } from "../../../src/cli/program/root-help.js";
...
describe("root help", () => {
  ...
  it("passes isolated config and env through to plugin CLI descriptor loading", async () => {
    const config = {
      agents: {
        defaults: {
          workspace: "/tmp/operator-root-help-workspace",
        },
      },
    };
    const env = { OPERATOR_STATE_DIR: "/tmp/operator-root-help-state" } as NodeJS.ProcessEnv;
