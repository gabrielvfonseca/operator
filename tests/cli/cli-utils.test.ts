...
import { runCommandWithRuntime } from "../../src/cli/cli-utils.js";
import { registerDnsCli } from "../../src/cli/dns-cli.js";
import { parseByteSize } from "../../src/cli/parse-bytes.js";
import { parseDurationMs } from "../../src/cli/parse-duration.js";
import {
  shouldSkipRespawnForArgv,
  shouldSkipStartupEnvironmentRespawnForArgv,
} from "../../src/cli/respawn-policy.js";
...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "@gabrielvfonseca/operator", "terminal"] },
    { argv: ["node", "@gabrielvfonseca/operator", "chat"] },
    { argv: ["node", "@gabrielvfonseca/operator", "gateway"] },
    {
      argv: [
        "node",
  ...
  it.each([
    ...
    { argv: ["node", "@gabrielvfonseca/operator", "gateway", "status"] },
    { argv: ["node", "@gabrielvfonseca/operator", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "@gabrielvfonseca/operator", "--help"] },
    { argv: ["node", "@gabrielvfonseca/operator", "gateway"] },
    { argv: ["node", "@gabrielvfonseca/operator", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "operator.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("operator.internal");
    } finally {
      log.mockRestore();
    }
  });
