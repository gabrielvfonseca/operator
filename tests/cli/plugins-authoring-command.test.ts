...
import { defineToolPlugin, getToolPluginMetadata } from "../../src/plugin-sdk/tool-plugin.js";
...
} from "../../src/cli/plugins-authoring-command.js";
...
function writeSourceToolPluginProject(params: {
  ...
  fs.writeFileSync(
    entryPath,
    `import { defineToolPlugin } from "@gabrielvfonseca/operator/plugin-sdk/tool-plugin";
...
describe("plugin authoring commands", () => {
  beforeAll(async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-plugin-source-warm-"));
    try {
      const entryPath = writeSourceToolPluginProject({
        tmpDir,
        packageName: "operator-plugin-source-warm",
  ...
  it("preserves manifest-owned metadata while updating generated fields", () => {
    ...
    expect(
      validateToolPluginProject({
        ...
        entry: "./src/index.ts",
        manifest,
        packageManifest: { version: "1.2.3", operator: { extensions: ["./src/index.ts"] } },
      }),
    ).toEqual([]);
  });
  ...
  it("drops stale manifest-owned tool metadata when no generated metadata remains", () => {
    const metadata = createDemoMetadata();
    const packageManifest = { version: "1.2.3", operator: { extensions: ["./src/index.ts"] } };
    const manifest = buildToolPluginManifest({
      metadata,
      packageManifest,
      existingManifest: {
  ...
  it("aligns package metadata with the selected runtime extension entry", () => {
    expect(
      buildToolPluginPackageManifest({
        packageManifest: {
          name: "demo",
          operator: { setupEntry: "./setup.ts", extensions: ["./src/other.ts"] },
        },
        entry: "./src/index.ts",
      }),
    ).toEqual({
  ...
  it("reports missing entries with an author-facing path", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-plugin-missing-"));
  ...
  it("loads source entries that import the Operator plugin SDK package subpath", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-plugin-source-"));
    const entryPath = writeSourceToolPluginProject({
      tmpDir,
      packageName: "operator-plugin-source-demo",
  ...
  it("finishes a build from an absolute root after the launch directory is removed", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-plugin-deleted-cwd-build-"));
    const packagePath = path.join(tmpDir, "package.json");
    const entryPath = writeSourceToolPluginProject({
      tmpDir,
      packageName: "operator-plugin-deleted-cwd-build",
    ...
    try {
      ...
      expect(fs.existsSync(path.join(tmpDir, "operator.plugin.json"))).toBe(true);
      expect(log).toHaveBeenCalledWith(`Wrote ${path.join(tmpDir, "operator.plugin.json")}`);
  ...
  it("finishes init with an absolute directory after the launch directory is removed", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-plugin-deleted-cwd-init-"));
  ...
  it("scaffolds a dist-entry tool plugin project", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-plugin-init-"));
    ...
    expect(
      JSON.parse(fs.readFileSync(path.join(projectDir, "operator.plugin.json"), "utf8")),
    ).toMatchObject({
      id: "stock-quotes",
      name: 'Stock "Quotes"',
      configSchema: {
  ...
  it("scaffolds a provider plugin project with ClawHub validation and release metadata", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-provider-init-"));
    ...
    expect(packageManifest).toMatchObject({
      name: "operator-plugin-plugin-init-test",
      scripts: {
      ...
      operator: {
        extensions: ["./dist/index.js"],
        install: {
          clawhubSpec: "clawhub:operator-plugin-plugin-init-test",
    ...
    const manifest = JSON.parse(
      fs.readFileSync(path.join(projectDir, "operator.plugin.json"), "utf8"),
    );
    expect(manifest).toMatchObject({
      id: "plugin-init-test",
      name: "Plugin Init Test",
