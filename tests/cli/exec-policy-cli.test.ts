...
import { SESSION_EXEC_OVERRIDES_NOTE } from "../../src/infra/exec-approvals-effective.js";
import type { ExecApprovalsFile, ExecApprovalsSnapshot } from "../../src/infra/exec-approvals.js";
import { registerExecPolicyCli } from "../../src/cli/exec-policy-cli.js";
...
const mocks = vi.hoisted(() => {
  ...
  let configState: OperatorConfig = {
    tools: {
      exec: {
        host: "auto",
        security: "allowlist",
  ...
  return {
    ...
    defaultRuntime,
    runtimeErrors,
    mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: OperatorConfig) => void }) => {
...
describe("exec-policy CLI", () => {
  ...
  beforeEach(() => {
    ...
    mocks.mutateConfigFile.mockImplementation(
      async ({ mutate }: { mutate: (draft: OperatorConfig) => void }) => {
