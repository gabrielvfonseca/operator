...
import { registerDirectoryCli } from "../../src/cli/directory-cli.js";
...
describe("registerDirectoryCli", () => {
  ...
  it("reports unsupported directory capability instead of continuing setup for installed plugins", async () => {
    mocks.resolveInstallableChannelPlugin.mockResolvedValue({
      cfg: { channels: { "operator-weixin": {} } },
      channelId: "operator-weixin",
      plugin: {
        id: "operator-weixin",
      },
      configChanged: false,
    ...
    await expect(
      program.parseAsync(["directory", "peers", "list", "--channel", "operator-weixin"], {
        from: "user",
      }),
    ...
    expect(mocks.resolveInstallableChannelPlugin).toHaveBeenCalledTimes(1);
    const installArgs = firstRecordArg(mocks.resolveInstallableChannelPlugin);
    expect(installArgs.rawChannel).toBe("operator-weixin");
    expect(installArgs.allowInstall).toBe(true);
    expect(mocks.replaceConfigFile).not.toHaveBeenCalled();
    expect(
      runtimeErrors().some((message) =>
        message.includes("Channel operator-weixin does not support directory peers"),
      ),
    ).toBe(true);
  });
