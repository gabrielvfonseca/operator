...
import {
  ...
  mockCall,
  mockFirstObjectArg,
} from "../../src/test-utils/mock-call-assertions.js";
import { registerSecretsCli } from "../../src/cli/secrets-cli.js";
...
describe("secrets CLI", () => {
  ...
  it("explains Gateway reload failures without duplicate doctor noise", async () => {
    callGatewayFromCli.mockRejectedValue(
      new Error(
        "gateway closed (1006 abnormal closure). Gateway target: ws://127.0.0.1:18789 Source: local loopback Config: /tmp/operator.json Bind: loopback Possible causes: - Gateway not yet ready. Run `operator doctor` for diagnostics.",
      ),
    );
    ...
    expect(runtimeErrors.at(-1)).toContain("operator gateway status --deep");
