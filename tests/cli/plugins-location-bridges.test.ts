...
import type { InstalledPluginStartupInfo } from "../../src/plugins/installed-plugin-index-types.js";
    ...
    plugins: [
      {
        ...
        startup: {},
        packageInstall: {
          clawhubSpec: `clawhub:@gabrielvfonseca/${pluginId}`,
...
describe("listPersistedBundledPluginLocationBridges", () => {
  ...
  it("keeps persisted bundled relocations npm-first for launch", async () => {
    readPersistedInstalledPluginIndexMock.mockResolvedValue(
      makeIndex({
        ...
        manifestPath: "/app/dist/extensions/diagnostics-otel/operator.plugin.json",
        manifestHash: "hash",
  ...
  it("does not create a relocation bridge without persisted or official install metadata", async () => {
    readPersistedInstalledPluginIndexMock.mockResolvedValue(
      makeIndex({
        pluginId: "local-only",
        manifestPath: "/app/dist/extensions/local-only/operator.plugin.json",
        manifestHash: "hash",
...
describe("listPersistedBundledPluginRecoveryLocations", () => {
  ...
  it("does not use a relative persisted bundled root as ownership proof", async () => {
    readPersistedInstalledPluginIndexMock.mockResolvedValue(
      makeIndex({
        pluginId: "diagnostics-otel",
        manifestPath: "extensions/diagnostics-otel/operator.plugin.json",
        manifestHash: "hash",
