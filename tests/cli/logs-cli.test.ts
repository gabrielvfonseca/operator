...
import { GatewayTransportError } from "../../src/gateway/call.js";
import { runRegisteredCli } from "../../src/test-utils/command-runner.js";
import { formatLogTimestamp, registerLogsCli } from "../../src/cli/logs-cli.js";
...
describe("logs cli", () => {
  ...
  describe("--follow retry behavior", () => {
    it("uses the active systemd journal for implicit local follow failures", async () => {
      ...
      expect(execFileUtf8Tail).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([
          "--user",
          "--boot",
          "--user-unit=operator-gateway.service",
          "_PID=2557",
          "--output=cat",
          "--show-cursor",
        ]),
      ...
      expect(stdoutWrites.join("")).toContain("Service Unit: operator-gateway.service");
    ...
    it("switches back to Gateway logs.tail after temporary journal fallback", async () => {
      ...
      expect(output).toContain("Log file: /tmp/operator.log");
    ...
    it("emits source meta records in --follow --json when fallback recovers", async () => {
      ...
      expect(metaRecords).toEqual([
        ...
        expect.objectContaining({
          type: "meta",
          source: "journalctl --user --boot --user-unit=operator-gateway.service _PID=2557",
          sourceKind: "journal",
          service: { pid: 2557, unit: "operator-gateway.service" },
          cursor: "s=abc",
          localFallback: true,
        }),
        expect.objectContaining({
