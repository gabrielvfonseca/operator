...
import { runChannelLogin, runChannelLogout } from "../../src/cli/channel-auth.js";
...
describe("channel-auth", () => {
  ...
  it("throws when channel does not support login", async () => {
    ...
    await expect(runChannelLogin({ channel: "whatsapp" }, runtime)).rejects.toThrow(
      'Channel "whatsapp" does not support login. Run `operator channels status --channel whatsapp` to inspect supported actions.',
    );
  });
  ...
  it("installs a catalog-backed channel plugin on demand for login", async () => {
    const catalogEntry = {
      id: "whatsapp",
      pluginId: "@gabrielvfonseca/whatsapp",
      meta: {
        id: "whatsapp",
        label: "WhatsApp",
  ...
  it("throws when channel does not support logout", async () => {
    ...
    await expect(runChannelLogout({ channel: "whatsapp" }, runtime)).rejects.toThrow(
      'Channel "whatsapp" does not support logout. Run `operator channels status --channel whatsapp` to inspect supported actions.',
    );
  });
});
