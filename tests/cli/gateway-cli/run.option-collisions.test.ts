...
import type { ConfigFileSnapshot } from "../../../src/config/types.js";
...
import { withTempSecretFiles } from "../../../src/test-utils/secret-file-fixture.js";
import { createCliRuntimeCapture } from "../../../src/cli/test-runtime-capture.js";
import { installGatewayRunRuntimeHooks } from "../../../src/cli/gateway-cli/runtime-hooks.js";
...
const resolveShellEnvExpectedKeys = vi.fn((_env?: NodeJS.ProcessEnv) => ["OPERATOR_GATEWAY_TOKEN"]);
...
describe("gateway run option collisions", () => {
  ...
  it("loads configured shell env fallback before final proxy refresh and gateway startup", async () => {
    await withEnvAsync({ OPERATOR_GATEWAY_TOKEN: undefined }, async () => {
      const finalConfig = {
        env: {
      ...
      readConfigFileSnapshotWithPluginMetadata
        ...
        .mockImplementationOnce(async (options) => {
          expect(options?.lowerPrecedenceEnv).toEqual({
            OPERATOR_GATEWAY_TOKEN: "shell-token",
          });
          expect(process.env.OPERATOR_GATEWAY_TOKEN).toBe("shell-token");
          return {
            snapshot: {
              ...configState.snapshot,
              config: {
      ...
      loadShellEnvFallback.mockImplementationOnce((opts?: unknown) => {
        callOrder.push("shell-env");
        (opts as { env: NodeJS.ProcessEnv }).env.OPERATOR_GATEWAY_TOKEN = "shell-token";
      });
      const uninstall = installGatewayRunRuntimeHooks({ refreshManagedProxy });
      try {
        await runGatewayCli(["gateway"]);
      ...
      expect(loadShellEnvFallback).toHaveBeenCalledWith({
        enabled: true,
        env: process.env,
        expectedKeys: ["OPERATOR_GATEWAY_TOKEN"],
        logger: expect.any(Object),
        timeoutMs: 1234,
      });
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledWith(
        expect.objectContaining({
          lowerPrecedenceEnv: { OPERATOR_GATEWAY_TOKEN: "shell-token" },
        }),
      );
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledTimes(2);
      expect(process.env.OPERATOR_GATEWAY_TOKEN).toBe("config-token");
      expect(clearShellEnvAppliedKeys).toHaveBeenCalledWith(["OPERATOR_GATEWAY_TOKEN"]);
    ...
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: {
            vars: {
              ...
              OPERATOR_SHELL_ENV_TIMEOUT_MS: "4321",
            },
          },
          gateway: { auth: { mode: "none" }, mode: "local" },
        };
        ...
        shouldEnableShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.OPERATOR_LOAD_SHELL_ENV === "1",
        );
        resolveShellEnvFallbackTimeoutMs.mockImplementationOnce((env?: NodeJS.ProcessEnv) =>
          Number(env?.OPERATOR_SHELL_ENV_TIMEOUT_MS),
        );
        ...
        expect(process.env.OPERATOR_LOAD_SHELL_ENV).toBe("1");
        expect(process.env.OPERATOR_SHELL_ENV_TIMEOUT_MS).toBe("4321");
      },
    );
  });
    ...
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: {
            vars: {
              OPERATOR_DEFER_SHELL_ENV_FALLBACK: "1",
              OPERATOR_LOAD_SHELL_ENV: "1",
            },
          },
          gateway: { auth: { mode: "none" }, mode: "local" },
        ...
        shouldDeferShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.OPERATOR_DEFER_SHELL_ENV_FALLBACK === "1",
        );
  ...
  it("ignores shell fallback controls from invalid config", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        const invalidConfig = {
          env: { vars: { OPERATOR_DEFER_SHELL_ENV_FALLBACK: "1" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
  ...
  it("replaces config-derived env when the final startup snapshot changes in place", async () => {
    await withEnvAsync(
      {
        OPERATOR_GATEWAY_TOKEN: undefined,
        OPERATOR_PROXY_URL: undefined,
        OPERATOR_RAW_STREAM: undefined,
      },
      async () => {
        const oldConfig = {
          env: {
            vars: {
              ...
              OPERATOR_PROXY_URL: "http://127.0.0.1:19876",
        ...
        const newConfig = {
          env: { vars: { OPERATOR_GATEWAY_TOKEN: "new-token" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
          config: oldConfig,
        ...
        expect(process.env.OPERATOR_GATEWAY_TOKEN).toBe("old-token");
        expect(process.env.OPERATOR_PROXY_URL).toBe("http://127.0.0.1:19876");
        ...
        readConfigFileSnapshotWithPluginMetadata.mockImplementationOnce(async () => {
          expect(process.env.OPERATOR_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.OPERATOR_PROXY_URL).toBeUndefined();
          return { snapshot: configState.snapshot };
        });
        await runGatewayCli(["gateway", "--raw-stream"]);

        expect(process.env.OPERATOR_GATEWAY_TOKEN).toBe("new-token");
        expect(process.env.OPERATOR_PROXY_URL).toBeUndefined();
        expect(process.env.OPERATOR_RAW_STREAM).toBe("1");
      },
    );
  });
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    process.env.OPERATOR_SERVICE_MARKER = "gateway";
    try {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        ensureDevGatewayConfig.mockImplementationOnce(async () => {
          ...
          expect(process.env.OPERATOR_HOME).toBeUndefined();
  ...
  it.each([
    ...
    delete process.env.OPERATOR_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.OPERATOR_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
