...
function getSharedTestHome(): string | undefined {
  ...
  return globalState[SHARED_TEST_SETUP]?.tempHome ?? process.env.OPERATOR_TEST_HOME;
}
...
function restoreSharedTestHomeAfterEnvUnstub(testHomeRaw: string | undefined): void {
  ...
  process.env.OPERATOR_TEST_HOME = testHome;
...
export default class OperatorNonIsolatedRunner extends TestRunner {
  override onCollectStart(file: RunnerTestFile) {
    ...
    const orderLogPath = process.env.OPERATOR_VITEST_FILE_ORDER_LOG?.trim();
    if (orderLogPath) {
