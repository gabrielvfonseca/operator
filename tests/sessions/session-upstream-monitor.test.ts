...
import type { SessionCatalogProvider, SessionUpstreamProbe } from "../../src/plugins/session-catalog.js";
import {
...
describe("session upstream monitor", () => {
  it("records watched activity once and advances its marker", async () => {
    ...
    const dedupeRow = openOperatorStateDatabase(database)
  ...
  it("clamps skewed upstream event times without touching bookkeeping clocks", async () => {
    ...
    const cursor = openOperatorStateDatabase(database)
  ...
  it("skips recording and marker writes when the link was refreshed mid-scan", async () => {
    ...
    const row = openOperatorStateDatabase(database)
  ...
  it("records an external prompt five seconds after Operator activity", async () => {
    const database = createDatabaseOptions();
    ...
    await runSessionUpstreamMonitorTick({
      ...
      isRunActive: () => false,
      loadOwnRecentUserTexts: async () => ["Operator prompt"],
    });
