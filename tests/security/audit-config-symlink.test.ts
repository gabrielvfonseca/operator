...
import { AsyncTempCaseFactory } from "../../src/security/test-temp-cases.js";

const isWindows = process.platform === "win32";

describe("security audit config symlink findings", () => {
  const tempCases = new AsyncTempCaseFactory("operator-security-audit-config-");
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-operator.json");
