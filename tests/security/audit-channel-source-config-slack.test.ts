...
import { stubAuditChannelPlugin } from "../../src/security/audit-channel-test-helpers.js";
import { collectChannelSecurityFindings } from "../../src/security/audit-channel.js";

function stubSlackPlugin(params: {
  resolveAccount: (cfg: OperatorConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: OperatorConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: OperatorConfig) => boolean;
}) {
...
function makeSlackHttpConfig(): OperatorConfig {
  return {
    channels: {
      slack: {
        enabled: true,
    ...
    const cases = [
      {
        ...
        plugin: (sourceConfig: OperatorConfig) =>
          stubSlackPlugin({
            inspectAccount: (cfg) => {
