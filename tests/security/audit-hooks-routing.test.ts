...
import { collectHooksHardeningFindings } from "../../src/security/audit-extra.sync.js";
...
describe("security audit hooks ingress findings", () => {
  it("evaluates hooks ingress auth and routing findings", () => {
    ...
    } satisfies NonNullable<OperatorConfig["hooks"]>;
    ...
    const cases = [
      ...
      },
      {
        ...
        cfg: { hooks: unrestrictedBaseHooks } satisfies OperatorConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "warn" as const,
      },
      {
        name: "scores unrestricted hooks.allowedAgentIds by remote exposure",
        cfg: { gateway: { bind: "lan" }, hooks: unrestrictedBaseHooks } satisfies OperatorConfig,
        expectedFinding: "hooks.allowed_agent_ids_unrestricted",
        expectedSeverity: "critical" as const,
      },
      {
        name: "scores hooks request sessionKey override by local exposure",
        cfg: { hooks: requestSessionKeyHooks } satisfies OperatorConfig,
  ...
  it("flags hooks token reuse of gateway password auth as critical", () => {
    ...
    expect(finding?.remediation).toContain("operator doctor --fix");
  });
