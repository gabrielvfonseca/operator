...
import { resolveGatewayProbeAuthSafe, resolveGatewayProbeTarget } from "../../src/gateway/probe-auth.js";
import { collectDeepProbeFindings } from "../../src/security/audit-deep-probe-findings.js";
  ...
  it("applies gateway auth precedence across local and remote modes", async () => {
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.OPERATOR_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.OPERATOR_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
