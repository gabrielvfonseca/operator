...
import { listSetupMigrationOptions } from "../../src/wizard/setup.migration-import.js";
import {
  inspectSetupMigrationFreshness,
  preserveSetupMigrationSecurityAcknowledgement,
} from "../../src/wizard/setup.migration-snapshot.js";
...
describe("setup migration import options", () => {
  ...
  it("offers official installable Codex when bundled plugins are unavailable", async () => {
    const previousDisableBundled = process.env.OPERATOR_DISABLE_BUNDLED_PLUGINS;
    const previousDisablePersisted = process.env.OPERATOR_DISABLE_PERSISTED_PLUGIN_REGISTRY;
    process.env.OPERATOR_DISABLE_BUNDLED_PLUGINS = "1";
    process.env.OPERATOR_DISABLE_PERSISTED_PLUGIN_REGISTRY = "1";
    try {
      ...
      if (previousDisableBundled === undefined) {
        ...
        process.env.OPERATOR_DISABLE_BUNDLED_PLUGINS = previousDisableBundled;
      }
      if (previousDisablePersisted === undefined) {
        delete process.env.OPERATOR_DISABLE_PERSISTED_PLUGIN_REGISTRY;
      } else {
        process.env.OPERATOR_DISABLE_PERSISTED_PLUGIN_REGISTRY = previousDisablePersisted;
      }
    }
  });
});
