...
import { resolveCompletionProfilePath } from "../../src/cli/completion-runtime.js";
import { setupWizardShellCompletion } from "../../src/wizard/setup.completion.js";

async function withLocale(locale: string, run: () => Promise<void>): Promise<void> {
  const previousLocale = process.env.OPERATOR_LOCALE;
  process.env.OPERATOR_LOCALE = locale;
  try {
    await run();
  } finally {
