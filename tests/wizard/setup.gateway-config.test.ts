...
import { DEFAULT_DANGEROUS_NODE_COMMANDS } from "../../src/gateway/node-command-policy.js";
...
import { configureGatewayForSetup } from "../../src/wizard/setup.gateway-config.js";
...
describe("configureGatewayForSetup", () => {
  ...
  it("prefers OPERATOR_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.OPERATOR_GATEWAY_TOKEN;
    process.env.OPERATOR_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("keeps OPERATOR_GATEWAY_TOKEN in advanced flow when user confirms keeping existing", async () => {
    const prevToken = process.env.OPERATOR_GATEWAY_TOKEN;
    process.env.OPERATOR_GATEWAY_TOKEN = "advanced-env-token";
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    ...
    process.env.OPERATOR_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "OPERATOR_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
  ...
  it("stores gateway token as SecretRef when secretInputMode=ref", async () => {
    const previous = process.env.OPERATOR_GATEWAY_TOKEN;
    process.env.OPERATOR_GATEWAY_TOKEN = "token-from-env";
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "token", "off", "env"],
        textQueue: ["18789", "OPERATOR_GATEWAY_TOKEN"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.OPERATOR_GATEWAY_TOKEN = previous;
      }
    }
  });
