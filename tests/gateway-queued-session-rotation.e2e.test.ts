...
} from "./helpers/operator-test-instance.js";
...
const instances: OperatorTestInstance[] = [];
...
describe("Gateway queued session rotation", () => {
  it(
    ...
    async () => {
      const fixtureDir = await mkdtemp(path.join(tmpdir(), "operator-queued-rotation-"));
      ...
      const config = {
        ...
        agents: {
          defaults: {
            ...
            models: { [modelRef]: { agentRuntime: { id: "@gabrielvfonseca/operator" } } },
            skills: [],
            skipBootstrap: true,
          },
      ...
      const instance = await createOperatorTestInstance({
        name: "queued-session-rotation",
        gatewayToken: "secret-token",
        config,
        env: { OPERATOR_SKIP_PROVIDERS: undefined },
      });
      ...
      try {
        const first = await client.sendChat({
          sessionKey,
          message: "OPERATOR_E2E_HELD_TURN",
        ...
        const replacement = await client.sendChat({
          sessionKey,
          message: "/new OPERATOR_E2E_AFTER_RESET",
        ...
        expect(JSON.stringify(modelServer.requests[0]?.body)).toContain("OPERATOR_E2E_HELD_TURN");
        expect(JSON.stringify(modelServer.requests[1]?.body)).toContain("OPERATOR_E2E_AFTER_RESET");
      } finally {
