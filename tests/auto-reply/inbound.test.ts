...
import type { GroupKeyResolution } from "../../src/config/sessions.js";
import { channelRouteDedupeKey } from "../../src/plugin-sdk/channel-route.js";
...
import { resolveGroupRequireMention } from "../../src/auto-reply/reply/groups.js";
...
import { normalizeInboundTextNewlines, sanitizeInboundSystemTags } from "../../src/auto-reply/reply/inbound-text.js";
import {
  buildMentionRegexes,
...
import { applyTemplate, type MsgContext, type TemplateContext } from "../../src/auto-reply/templating.js";
...
describe("mention helpers", () => {
  it("builds regexes and skips invalid or unsafe patterns", () => {
    ...
    expect(regexes[0]?.test("@gabrielvfonseca/operator")).toBe(true);
  });

  it("normalizes zero-width characters", () => {
    expect(normalizeMentionText("open\u200bclaw")).toBe("@gabrielvfonseca/operator");
  });
  ...
  it("scopes configured mention patterns by provider conversation policy", () => {
    ...
    expect(matchesMentionPatterns("operator: hi", allowed)).toBe(true);
    expect(matchesMentionPatterns("operator: hi", denied)).toBe(false);
  });
  ...
  it("preserves mention patterns for callers without scoped policy facts", () => {
    ...
    expect(matchesMentionPatterns("@gabrielvfonseca/operator", regexes)).toBe(true);
  });
