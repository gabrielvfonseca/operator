...
import { PROVIDER_CONVERSATION_STATE_ERROR_USER_MESSAGE } from "../../../src/auto-reply/reply/provider-request-error-classifier.js";
...
describe("runAgentTurnWithFallback: conversation failures", () => {
  ...
  it("keeps raw generic errors on internal control surfaces", async () => {
    ...
    if (result.kind === "final") {
      ...
      expect(result.payload.text).toContain("Logs: operator logs --follow");
    }
  });
});
