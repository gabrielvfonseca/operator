...
import { persistAbortTargetEntry, persistSessionEntry } from "../../../src/auto-reply/reply/commands-session-store.js";

async function withTempStore<T>(run: (storePath: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-command-session-store-"));
  try {
