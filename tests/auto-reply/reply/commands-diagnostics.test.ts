...
import type { OperatorPluginCommandDefinition, PluginCommandContext } from "../../../src/plugins/types.js";

type PluginCommandHandler = OperatorPluginCommandDefinition["handler"];
import type { MsgContext } from "../../../src/auto-reply/templating.js";
import { handleDiagnosticsCommand as defaultDiagnosticsCommandHandler } from "../../../src/auto-reply/reply/commands-diagnostics.js";
...
function buildDiagnosticsParams(
  ...
  return {
    cfg: { commands: { text: true } } as OperatorConfig,
    ctx: {
      Provider: "whatsapp",
      Surface: "whatsapp",
      CommandSource: "text",
...
describe("diagnostics command", () => {
  ...
  it("routes group diagnostics confirmations privately", async () => {
    const commandHandler = vi.fn(async () => ({
      text: [
        "Codex diagnostics sent to OpenAI servers:",
        "- channel whatsapp, Operator session session-1, Codex thread codex-thread-1",
      ].join("\n"),
    }));
    registerHostTrustedReservedCommandForTest({
      name: "codex",
