...
import { applyReplyThreading } from "../../../src/auto-reply/reply/reply-payloads.js";
import {
  formatRunLabel,
  formatRunStatus,
  resolveSubagentLabel,
  sortSubagentRuns,
} from "../../../src/auto-reply/reply/subagents-utils.js";
...
describe("buildThreadingToolContext", () => {
  ...
  it("uses raw Slack channel ids without implicit thread context", () => {
    ...
    const result = buildThreadingToolContext({
      sessionCtx,
      config: { channels: { slack: { replyToMode: "all" } } } as OperatorConfig,
      hasRepliedRef: undefined,
    });
  ...
  it("lets plugin threading adapters suppress the generic message-id fallback", () => {
    ...
    const result = buildThreadingToolContext({
      sessionCtx,
      config: { channels: { googlechat: { replyToMode: "all" } } } as OperatorConfig,
      hasRepliedRef: undefined,
    });
