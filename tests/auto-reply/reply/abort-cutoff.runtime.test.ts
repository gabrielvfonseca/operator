...
import { clearAbortCutoffInSessionRuntime } from "../../../src/auto-reply/reply/abort-cutoff.runtime.js";

async function withTempStore<T>(run: (storePath: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-abort-cutoff-"));
  try {
