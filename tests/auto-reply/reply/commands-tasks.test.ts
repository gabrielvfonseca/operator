...
import { handleTasksCommand } from "../../../src/auto-reply/reply/commands-tasks.js";
import {
...
describe("handleTasksCommand task board", () => {
  ...
  it("sanitizes inline internal runtime fences from visible task titles", async () => {
    createRunningTaskRun({
      ...
      task: [
        "[Mon 2026-04-06 02:42 GMT+1] <<<BEGIN_OPERATOR_INTERNAL_CONTEXT>>>",
        "Operator runtime context (internal):",
        "This context is runtime-generated, not user-authored. Keep internal details private.",
      ].join("\n"),
    ...
    expect(reply.text).not.toContain("BEGIN_OPERATOR_INTERNAL_CONTEXT");
    expect(reply.text).not.toContain("Operator runtime context (internal):");
  });
