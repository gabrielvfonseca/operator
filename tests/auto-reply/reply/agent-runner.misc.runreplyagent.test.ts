...
import { testing as embeddedRunTesting } from "../../../src/agents/embedded-agent-runner/runs.test-support.js";
import { clearRuntimeConfigSnapshot } from "../../../src/config/config.js";
import type { OperatorConfig } from "../../../src/config/config.js";
import * as sessionTypesModule from "../../../src/config/sessions.js";
...
} from "../../../src/plugins/memory-state.test-fixtures.js";
import { GatewayDrainingError } from "../../../src/process/command-queue.js";
import { getReplyPayloadMetadata, type ReplyPayload } from "../../../src/auto-reply/reply-payload.js";
import type { TemplateContext } from "../../../src/auto-reply/templating.js";
import type { VerboseLevel } from "../../../src/auto-reply/thinking.shared.js";
...
import { enqueueFollowupRun, scheduleFollowupDrain } from "../../../src/auto-reply/reply/queue.js";
import { createReplyOperation, replyRunRegistry } from "../../../src/auto-reply/reply/reply-run-registry.js";
import { testing as replyRunRegistryTesting } from "../../../src/auto-reply/reply/reply-run-registry.test-support.js";
...
vi.mock("../../cron/store.js", () => {
  const resolveCronPath = (storePath?: string) => storePath ?? "/tmp/operator-cron-store.json";
  return {
...
describe("runReplyAgent auto-compaction token update", () => {
  ...
  it("keeps an unarmed preflight drain visible instead of dropping the reply", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "operator-preflight-drain-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
    const sessionEntry = {
      sessionId: "session",
  ...
  it("reports live diagnostic context from promptTokens, not provider usage totals", async () => {
    const { usageEvent } = await runBaseReplyWithAgentMeta({
      tmpPrefix: "operator-usage-diagnostic-",
  ...
  it("falls back to last-call prompt usage for live diagnostic context", async () => {
    const { usageEvent } = await runBaseReplyWithAgentMeta({
      tmpPrefix: "operator-usage-diagnostic-last-",
      collectDiagnostics: true,
      agentMeta: {
        usage: { input: 75_000, output: 5_000, cacheRead: 25_000, total: 105_000 },
        lastCallUsage: {
  ...
  it("does not treat diagnostic compaction metadata as a context-refresh trigger", async () => {
    const workspaceDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "operator-post-compaction-workspace-"),
    );
    await fs.writeFile(
    ...
    const { sessionKey } = await runBaseReplyWithAgentMeta({
      tmpPrefix: "operator-post-compaction-workspace-root-",
      workspaceDir,
      config: {
        agents: {
          defaults: {
...
describe("runReplyAgent Active Memory inline debug", () => {
  ...
  it("appends inline Active Memory status payload when verbose is enabled", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "operator-active-memory-inline-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionKey = "main";
  ...
  it("appends raw trace payloads when trace raw is enabled", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "operator-trace-raw-usage-"));
    const storePath = path.join(tmp, "sessions.json");
  ...
  it("does not emit persisted trace output to an unauthorized sender", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "operator-trace-raw-unauthorized-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionFile = path.join(tmp, "session.jsonl");
    const sessionKey = "main";
    const sessionEntry: SessionEntry = {
  ...
  it("escapes markdown fence delimiters inside raw trace blocks", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "operator-trace-raw-fence-"));
    const storePath = path.join(tmp, "sessions.json");
    const sessionFile = path.join(tmp, "session.jsonl");
    const sessionKey = "main";
    const sessionEntry: SessionEntry = {
...
describe("runReplyAgent private message_tool_only final warning (#85714)", () => {
  ...
  async function runPrivateFinalCase(params: {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "operator-stranded-"));
    const storePath = path.join(tmp, "sessions.json");
