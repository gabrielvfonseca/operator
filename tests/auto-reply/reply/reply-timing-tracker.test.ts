...
import { createReplyTimingTracker, isReplyProfilerEnabled } from "../../../src/auto-reply/reply/reply-timing-tracker.js";

describe("isReplyProfilerEnabled", () => {
  it("matches global and reply profiler diagnostic flags", () => {
    const cfg = { diagnostics: { flags: ["reply.profiler"] } } as OperatorConfig;
    expect(isReplyProfilerEnabled({ config: cfg, env: {} as NodeJS.ProcessEnv })).toBe(true);
    expect(
      isReplyProfilerEnabled({
        env: { OPERATOR_DIAGNOSTICS: "profiler" } as NodeJS.ProcessEnv,
      }),
    ).toBe(true);
  });
});
...
describe("createReplyTimingTracker", () => {
  ...
  it("records and logs spans when the profiler flag is enabled", () => {
    ...
    const tracker = createReplyTimingTracker({
      log: { warn },
      env: { OPERATOR_DIAGNOSTICS: "reply.profiler" } as NodeJS.ProcessEnv,
      totalWarnMs: 0,
      stageWarnMs: 0,
    });
