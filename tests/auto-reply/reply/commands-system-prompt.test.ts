...
import { resolveReusableWorkspaceSkillSnapshot } from "../../../src/skills/runtime/session-snapshot.js";
import { resolveCommandsSystemPromptBundle } from "../../../src/auto-reply/reply/commands-system-prompt.js";
import type { HandleCommandsParams } from "../../../src/auto-reply/reply/commands-types.js";

const { createOperatorCodingToolsMock } = vi.hoisted(() => ({
  createOperatorCodingToolsMock: vi.fn(() => []),
}));
...
vi.mock("../../agents/agent-tools.js", () => ({
  createOperatorCodingTools: createOperatorCodingToolsMock,
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    createOperatorCodingToolsMock.mockClear();
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    const toolParams = requireFirstArg(
      vi.mocked(createOperatorCodingTools),
  ...
  it("uses config-backed prompt settings for the target agent", async () => {
    ...
    createOperatorCodingToolsMock.mockReturnValue([{ name: "sessions_spawn" }] as never);
    const params = makeParams();
    params.cfg = {
      agents: {
        defaults: {
