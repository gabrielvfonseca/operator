...
} from "../../../src/plugin-sdk/allowlist-config-edit.js";
...
import { handleAllowlistCommand } from "../../../src/auto-reply/reply/commands-allowlist.js";
...
async function withTempConfigPath<T>(
  ...
  try {
    ...
    if (previous === undefined) {
      ...
      setTestEnvValue("OPERATOR_CONFIG_PATH", previous);
    }
        ...
        run: async () => {
          await withTempConfigPath(
            ...
            async (configPath) => {
              ...
              const written = await readJsonFile<OperatorConfig>(configPath);
      ...
      createTestRegistry([
        {
          ...
          plugin: {
            ...
            config: {
              ...
              defaultAccountId: (cfg: OperatorConfig) =>
                (cfg.channels?.telegram as TelegramTestSectionConfig | undefined)?.defaultAccount ??
                DEFAULT_ACCOUNT_ID,
            },
          },
