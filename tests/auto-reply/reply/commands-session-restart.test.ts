...
import type { RestartSentinelPayload } from "../../../src/infra/restart-sentinel.js";
import type { scheduleGatewaySigusr1Restart } from "../../../src/infra/restart.js";
...
const mocks = vi.hoisted(() => ({
  ...
  triggerOperatorRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerOperatorRestart: mocks.triggerOperatorRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerOperatorRestart.mockReset();
    mocks.triggerOperatorRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(mocks.triggerOperatorRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerOperatorRestart).not.toHaveBeenCalled();
  ...
  it("clears the success sentinel when fallback restart fails", async () => {
    mocks.triggerOperatorRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
