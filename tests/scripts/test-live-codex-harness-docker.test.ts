...
describe("scripts/test-live-codex-harness-docker.sh", () => {
  ...
  it("fails before Docker build when codex-auth has no host auth file", () => {
    ...
    expect(script).toContain(
      "OPERATOR_LIVE_CODEX_HARNESS_AUTH=codex-auth requires ~/.codex/auth.json before building the live Docker image",
    );
    expect(script).toContain(
      "If this is a Testbox/API-key run, set OPERATOR_LIVE_CODEX_HARNESS_AUTH=api-key and run through operator-testbox-env.",
    );
    expect(script.indexOf("requires ~/.codex/auth.json before building")).toBeLessThan(
      script.indexOf('OPERATOR_LIVE_DOCKER_REPO_ROOT="$ROOT_DIR"'),
    );
  });
  ...
  it("forwards the live Codex bind provider override into Docker", () => {
    ...
    expect(script).toContain(
      '-e OPERATOR_LIVE_CODEX_BIND_PROVIDER="${OPERATOR_LIVE_CODEX_BIND_PROVIDER:-}"',
    );
  });
  ...
  it("installs the plugin-pinned Codex CLI package for app-server proof", () => {
    ...
    expect(script).toContain('-e OPERATOR_LIVE_CODEX_CLI_PACKAGE_SPEC="$CODEX_CLI_PACKAGE_SPEC"');
    expect(script).toContain(
  ...
  it("fails instead of skipping when Codex auth cannot identify an account", () => {
    ...
    expect(script).toContain(
      "ERROR: Codex auth cannot extract accountId from the available token; refresh OPERATOR_CODEX_AUTH_JSON or use OPERATOR_LIVE_CODEX_HARNESS_AUTH=api-key.",
    );
  ...
  it("rejects invalid setup timeout values before auth or Docker setup", () => {
    const result = spawnSync("bash", [SCRIPT_PATH], {
      ...
      env: {
        ...process.env,
        OPERATOR_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS: "180s",
      },
    });

    expect(result.status).toBe(2);
    expect(result.stderr).toContain(
      "invalid OPERATOR_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS: 180s",
    );
