...
function runDockerRunArgs(pathPrefix: string) {
  const script = [
    ...
    "unset OPERATOR_LIVE_DOCKER_DISABLE_RESOURCE_LIMITS OPERATOR_DOCKER_E2E_DISABLE_RESOURCE_LIMITS",
    "unset OPERATOR_LIVE_DOCKER_MEMORY OPERATOR_DOCKER_E2E_MEMORY",
    "unset OPERATOR_LIVE_DOCKER_CPUS OPERATOR_DOCKER_E2E_CPUS",
    "unset OPERATOR_LIVE_DOCKER_PIDS_LIMIT OPERATOR_DOCKER_E2E_PIDS_LIMIT",
    "ARGS=()",
...
describe("scripts/lib/live-docker-auth.sh", () => {
  it("reads positive integer env values before live Docker setup", () => {
    const result = spawnSync(
      ...
      [
        ...
        [
          "source scripts/lib/live-docker-auth.sh",
          'fallback="$(operator_live_read_positive_int_env OPERATOR_LIVE_SAMPLE_SECONDS 180)"',
          'leading_zero="$(OPERATOR_LIVE_SAMPLE_SECONDS=008 operator_live_read_positive_int_env OPERATOR_LIVE_SAMPLE_SECONDS 180)"',
    ...
    const invalid = spawnSync(
      ...
      [
        ...
        [
          "source scripts/lib/live-docker-auth.sh",
          "OPERATOR_LIVE_SAMPLE_SECONDS=30s operator_live_read_positive_int_env OPERATOR_LIVE_SAMPLE_SECONDS 180",
        ].join("\n"),
      ],
      { cwd: process.cwd(), encoding: "utf8" },
    );
    ...
    expect(invalid.stderr).toContain("invalid OPERATOR_LIVE_SAMPLE_SECONDS: 30s");
  });
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          "source scripts/lib/live-docker-auth.sh",
          "ARGS=()",
          "OPERATOR_LIVE_DOCKER_AVAILABLE_CPUS=8 operator_live_init_docker_run_args ARGS 42s",
          "printf '%s\\n' \"${ARGS[@]}\"",
        ].join("\n"),
      ],
      {
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          "source scripts/lib/live-docker-auth.sh",
          "ARGS=()",
          "OPERATOR_LIVE_DOCKER_DISABLE_RESOURCE_LIMITS=1 operator_live_init_docker_run_args ARGS 42s",
          "printf '%s\\n' \"${ARGS[@]}\"",
        ].join("\n"),
      ],
      {
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          "source scripts/lib/live-docker-auth.sh",
          "ARGS=()",
          "OPERATOR_LIVE_DOCKER_PIDS_LIMIT=0008 operator_live_init_docker_run_args ARGS 42s",
          "printf '%s\\n' \"${ARGS[@]}\"",
        ].join("\n"),
      ],
      {
  ...
  it.each([
    ["live", "OPERATOR_LIVE_DOCKER_PIDS_LIMIT"],
    ["shared", "OPERATOR_DOCKER_E2E_PIDS_LIMIT"],
    ...
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          OPERATOR_DOCKER_E2E_PIDS_LIMIT:
            envName === "OPERATOR_DOCKER_E2E_PIDS_LIMIT" ? "many" : "",
          OPERATOR_LIVE_DOCKER_PIDS_LIMIT:
            envName === "OPERATOR_LIVE_DOCKER_PIDS_LIMIT" ? "many" : "",
          PATH: binDir,
        },
      },
    );
