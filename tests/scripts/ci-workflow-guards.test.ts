  ...
  try {
    ...
    const run = spawnSync("bash", ["-c", manifestStep.run], {
      ...
      env: {
        ...
        OPERATOR_CI_CHANGED_PATHS_JSON: JSON.stringify(options.changedPaths ?? null),
        OPERATOR_CI_CHECKOUT_REVISION: "a".repeat(40),
        OPERATOR_CI_DOCS_CHANGED: "true",
        OPERATOR_CI_DOCS_ONLY: "false",
        OPERATOR_CI_EVENT_NAME: options.eventName ?? "workflow_dispatch",
        OPERATOR_CI_HISTORICAL_TARGET:
          (options.historicalCompatibility ?? true) &&
          (options.eventName ?? "workflow_dispatch") === "workflow_dispatch"
            ? "true"
            : "false",
        OPERATOR_CI_RELEASE_CANDIDATE_TARGET:
          options.releaseCandidateCompatibility === true ? "true" : "false",
        OPERATOR_CI_REPOSITORY: "operator/operator",
        OPERATOR_CI_RUN_ANDROID: "true",
        OPERATOR_CI_RUN_CONTROL_UI_I18N: "true",
        OPERATOR_CI_RUN_IOS_BUILD: "true",
        OPERATOR_CI_RUN_MACOS: "true",
        OPERATOR_CI_RUN_NATIVE_I18N: "true",
        OPERATOR_CI_RUN_NODE: String(options.runNode ?? true),
        OPERATOR_CI_RUN_NODE_FAST_CI_ROUTING: String(options.nodeFastCiRouting ?? false),
        OPERATOR_CI_RUN_NODE_FAST_ONLY: String(options.nodeFastOnly ?? false),
        OPERATOR_CI_RUN_NODE_FAST_PLUGIN_CONTRACTS: String(
          options.nodeFastPluginContracts ?? false,
        ),
        OPERATOR_CI_RUN_SKILLS_PYTHON: "true",
        OPERATOR_CI_RUN_WINDOWS: "true",
        OPERATOR_CI_WORKFLOW_REVISION: "b".repeat(40),
      },
    });
...
function runGeneratedPublisherScenario(
  ...
  try {
    ...
    writeExecutable(path.join(fakeBin, "gh"), [
      ...
      "      fi",
      '      printf "https://github.com/gabrielvfonseca/operator/pull/1\\t%s\\n" "$head"',
      "    fi",
      "    ;;",
      "  pr:create)",
      '    : > "$FAKE_PR_STATE"',
      '    printf "%s\\n" "https://github.com/gabrielvfonseca/operator/pull/1"',
      "    ;;",
      "  pr:edit) exit 0 ;;",
    ...
    const publish = spawnSync("bash", ["-c", publishRun], {
      ...
      env: {
        ...
        GITHUB_REPOSITORY_OWNER: "@gabrielvfonseca/operator",
...
describe("ci workflow guards", () => {
  ...
  it("makes the hosted release-gate fallback explicit and exact-SHA only", () => {
    ...
    expect(readFileSync(".github/workflows/ci.yml", "utf8")).toContain(
      "OPERATOR_CI_RUN_ANDROID: ${{ github.event_name == 'workflow_dispatch' && (inputs.release_gate || inputs.include_android) && 'true' || steps.changed_scope.outputs.run_android || 'false' }}",
    );
  ...
  it("keeps locale refresh matrices alive and publishes each aggregate through a PR", () => {
    ...
    expect(refreshStep.env.OPERATOR_DOCS_I18N_OPENAI_API_KEY).toBe(
      "${{ secrets.OPERATOR_DOCS_I18N_OPENAI_API_KEY }}",
    );
    ...
    expect(controlUiRefreshStep.env.OPERATOR_DOCS_I18N_OPENAI_API_KEY).toBe(
      "${{ secrets.OPERATOR_DOCS_I18N_OPENAI_API_KEY }}",
    );
    expect(controlUiRefreshStep.env.OPENAI_API_KEY).toBe("${{ secrets.OPENAI_API_KEY }}");
    expect(controlUiRefreshStep.env.OPERATOR_CONTROL_UI_I18N_AUTH_OPTIONAL).toBe("0");
  ...
  it.skipIf(process.platform === "win32")(
    ...
    () => {
      ...
      expect(result.summary).toContain("https://github.com/gabrielvfonseca/operator/pull/1");
    },
  );
  ...
  it("debounces canonical main pushes before Blacksmith admission", () => {
    ...
    expect(admission.steps[0].run).toContain('sleep "${OPERATOR_MAIN_CI_DEBOUNCE_SECONDS}"');
    expect(admission.env.OPERATOR_MAIN_CI_DEBOUNCE_SECONDS).toBe("90");
  ...
  it("keeps sticky dependency snapshots on trusted Blacksmith Node shards", () => {
    ...
    expect(bindStep.run).toContain('echo "OPERATOR_BUILD_ALL_NO_PNPM=1"');
  ...
  it("uses target-owned CI plans and capabilities for older release checkouts", () => {
    ...
    expect(uiInstall.run).not.toContain("OPERATOR_UI_E2E_ALLOW_MISSING_CHROMIUM");
  ...
  it("keeps Control UI locale parity advisory until release CI", () => {
    ...
    expect(readFileSync("ui/tests/i18n/test/translate.test.ts", "utf8")).not.toContain(
  ...
  it("keeps the hosted plugin-list memory allowance scoped to GitHub-hosted runners", () => {
    ...
    expect(startupMemoryStep.env.OPERATOR_STARTUP_MEMORY_PLUGINS_LIST_MB).toBe(
  ...
  it("fails and retries quiet Node test shard stalls quickly", () => {
    ...
    expect(runStep.env.OPERATOR_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("300000");
    expect(runStep.env.OPERATOR_VITEST_NO_OUTPUT_RETRY).toBe("1");
    expect(runStep.env.OPERATOR_NODE_TEST_ENV_JSON).toBe("${{ toJson(matrix.env) }}");
    expect(runStep.env.OPERATOR_NODE_TEST_TARGETS_JSON).toBe("${{ toJson(matrix.targets) }}");
  ...
  it("keeps maturity scorecard generated QA evidence handoff strict", () => {
    ...
    expect(
      maturityWorkflow.on.workflow_call.secrets.OPERATOR_MATURITY_SCORECARD_AGENT_OPENAI_API_KEY
        .required,
    ).toBe(false);
    expect(Object.keys(maturityWorkflow.on.workflow_call.secrets).toSorted()).toEqual([
      ...
      "OPENAI_API_KEY",
      "OPERATOR_MATURITY_SCORECARD_AGENT_OPENAI_API_KEY",
    ]);
    for (const secret of [
      "CLAWSWEEPER_APP_PRIVATE_KEY",
      "MANTIS_GITHUB_APP_ID",
  ...
  it("keeps workflow guards in fast CI-routing checks", () => {
    ...
    expect(smokeBuildStep.env.OPERATOR_BUILD_PRIVATE_QA).toBe("1");
    ...
    expect(smokeRunStep.env.OPERATOR_QA_SUITE_WORKER_START_STAGGER_MS).toContain(
      "github.event_name != 'workflow_dispatch'",
    );
    expect(smokeRunStep.env.OPERATOR_QA_SUITE_WORKER_START_STAGGER_MS).toContain(
      "github.repository == 'operator/operator'",
    );
    expect(smokeRunStep.env.OPERATOR_QA_SUITE_WORKER_START_STAGGER_MS).toContain("'0'");
    expect(smokeRunStep.env.OPERATOR_QA_SUITE_WORKER_START_STAGGER_MS).toContain("'1500'");
    ...
    expect(smokeRunStep.run).toContain("OPERATOR_CURRENT_PACKAGE_TGZ");
  ...
  it("keeps push docs validation ClawHub-backed", () => {
    ...
    expect(workflow).toContain(
      "OPERATOR_DOCS_SYNC_CLAWHUB_REPO: ${{ github.workspace }}/clawhub-source",
    );
  });
  ...
  it("keeps network CodeQL off unrelated source-only refactors", () => {
    ...
    expect(workflow).toContain(
      "grep -En 'HTTP_PROXY|HTTPS_PROXY|NO_PROXY|GLOBAL_AGENT_|OPERATOR_PROXY_' \"$added_lines\"",
    );
