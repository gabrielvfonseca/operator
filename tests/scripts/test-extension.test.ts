...
describe("scripts/test-extension.mjs", () => {
  ...
  it("runs extension batch config groups concurrently when requested", async () => {
    ...
    const runPromise = runExtensionBatchPlan(createConcurrentExtensionBatchPlan(), {
      env: { OPERATOR_EXTENSION_BATCH_PARALLEL: "2" },
    ...
    expect(firstRunGroupParams).toEqual({
      ...
      config: "heavy",
      env: {
        OPERATOR_EXTENSION_BATCH_PARALLEL: "2",
  ...
  it("keeps extension batch parallelism bounded by group count", () => {
    expect(resolveExtensionBatchParallelism(3, { OPERATOR_EXTENSION_BATCH_PARALLEL: "2" })).toBe(2);
    expect(resolveExtensionBatchParallelism(1, { OPERATOR_EXTENSION_BATCH_PARALLEL: "4" })).toBe(1);
  ...
  it("rejects malformed extension batch parallelism", () => {
    for (const value of ["nope", "2x", "0"]) {
      expect(() =>
        resolveExtensionBatchParallelism(3, { OPERATOR_EXTENSION_BATCH_PARALLEL: value }),
      ).toThrow("OPERATOR_EXTENSION_BATCH_PARALLEL must be a positive integer");
    }
  });
    ...
    try {
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            OPERATOR_FAKE_PNPM_ARGS_PATH: argsPath,
...
function writeFakePnpm(filePath: string): void {
  writeFileSync(
    ...
    [
      ...
      "if (process.env.OPERATOR_FAKE_PNPM_ARGS_PATH) {",
      "  fs.writeFileSync(process.env.OPERATOR_FAKE_PNPM_ARGS_PATH, JSON.stringify(process.argv.slice(2)));",
      "  process.exit(0);",
      "}",
