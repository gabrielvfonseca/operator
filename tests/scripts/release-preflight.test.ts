...
function makeFakePnpm(): { binDir: string; eventsPath: string; logPath: string } {
  const root = makeTempDir(tempDirs, "operator-release-preflight-");
...
appendFileSync(process.env.OPERATOR_RELEASE_PREFLIGHT_PNPM_LOG, command + "\\n");
appendFileSync(process.env.OPERATOR_RELEASE_PREFLIGHT_PNPM_EVENTS, "start " + command + "\\n");
const delayMs = Number(process.env.OPERATOR_RELEASE_PREFLIGHT_DELAY_MS ?? "0");
if (delayMs > 0) {
  Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, delayMs);
}
appendFileSync(process.env.OPERATOR_RELEASE_PREFLIGHT_PNPM_EVENTS, "end " + command + "\\n");
const failures = new Set((process.env.OPERATOR_RELEASE_PREFLIGHT_FAIL_COMMANDS ?? "").split(";").filter(Boolean));
...
function runPreflight(
  ...
  return spawnSync(process.execPath, [SCRIPT, ...args], {
    ...
    env: {
      ...
      ...(fakePnpm
        ? {
            OPERATOR_RELEASE_PREFLIGHT_PNPM_LOG: fakePnpm.logPath,
            OPERATOR_RELEASE_PREFLIGHT_PNPM_EVENTS: fakePnpm.eventsPath,
...
function makeReleaseFixture(
  ...
  const root = makeTempDir(tempDirs, "operator-release-preflight-fixture-");
  const plistDir = join(root, "apps", "macos", "Sources", "Operator", "Resources");
...
describe("scripts/release-preflight.mjs", () => {
  ...
  it("runs every check command and reports all failed release artifact checks", () => {
    ...
    const result = runPreflight(["--check"], fakePnpm, {
      OPERATOR_RELEASE_PREFLIGHT_FAIL_COMMANDS:
  ...
  it("serializes the root package writer before generated-artifact readers", () => {
    ...
    const result = runPreflight(
      ...
      fakePnpm,
      {
        OPERATOR_RELEASE_PREFLIGHT_DELAY_MS: "40",
      },
      root,
    );
  ...
  it("checks non-version scopes without requiring macOS source metadata", () => {
    const fakePnpm = makeFakePnpm();
    const root = makeTempDir(tempDirs, "operator-release-preflight-config-");
  ...
  it("uses bounded parallelism for independent checks", () => {
    const fakePnpm = makeFakePnpm();
    const root = makeReleaseFixture();
    const env = { OPERATOR_RELEASE_PREFLIGHT_DELAY_MS: "120" };
  ...
  it("fails closed when required macOS plist values are missing", () => {
    const fakePnpm = makeFakePnpm();
    const root = makeReleaseFixture();
    const plistPath = join(root, "apps", "macos", "Sources", "Operator", "Resources", "Info.plist");
    writeFileSync(
      plistPath,
