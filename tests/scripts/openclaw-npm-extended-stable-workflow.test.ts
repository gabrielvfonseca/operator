...
const workflowPath = ".github/workflows/operator-npm-release.yml";
...
describe("minimal npm extended-stable workflow", () => {
  ...
  it("reuses the v1 preflight tarball and guards all three extended-stable gates", () => {
    ...
    expect(step(parsed.jobs?.preflight_operator_npm, "Validate npm release request").run).toContain(
      "operator-npm-extended-stable-release.mjs validate-request",
    );
    expect(
    ...
    ).toContain("operator-npm-extended-stable-release.mjs validate-request");
    expect(step(parsed.jobs?.publish_operator_npm, "Recheck npm release request").run).toContain(
      "operator-npm-extended-stable-release.mjs validate-request",
    );
    expect(
  ...
  it("threads an explicit, default-off extended-stable bypass through every policy gate", () => {
    ...
    const policySteps = [
      step(parsed.jobs?.preflight_operator_npm, "Validate npm release request"),
      step(parsed.jobs?.validate_publish_request, "Validate npm release request"),
      step(parsed.jobs?.publish_operator_npm, "Recheck npm release request"),
      step(parsed.jobs?.publish_operator_npm, "Publish"),
    ];
    ...
    const summary = step(
      parsed.jobs?.publish_operator_npm,
  ...
  it("authenticates exact extended-stable run and Full Validation identities", () => {
    ...
    expect(raw.match(/operator-npm-extended-stable-release\.mjs verify-run/g)).toHaveLength(3);
    expect(raw).toContain("operator-npm-extended-stable-release.mjs verify-manifest");
  });
  ...
  it("requires and authenticates the plugin npm run before an extended-stable core publish", () => {
    ...
    expect(verify.run).toContain("operator-npm-extended-stable-release.mjs verify-run");
  });

  it("captures selector fail closed, publishes extended-stable, retries, and summarizes", () => {
    const parsed = workflow();
    const publish = parsed.jobs?.publish_operator_npm;
    ...
    expect(capture.run).toContain("operator-npm-extended-stable-release.mjs capture-selector");
    expect(step(publish, "Publish").run).toContain("operator-npm-publish.sh");
    expect(readback.run).toContain("operator-npm-extended-stable-release.mjs verify-readback");
    expect(summary.if).toContain("always()");
    expect(summary.run).toContain("operator-npm-extended-stable-release.mjs repair-command");
  ...
  it("publishes only the tarball path verified from the preflight manifest", () => {
    const publish = workflow().jobs?.publish_operator_npm;
