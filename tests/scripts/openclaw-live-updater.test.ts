...
} from "../../.agents/skills/operator-live-updater/scripts/update-main.mjs";
import {
  BUILD_STAMP_FILE,
...
const script = path.join(repoRoot, ".agents/skills/operator-live-updater/scripts/update-main.mjs");
...
function initializeFixture(root: string) {
  ...
  const canonicalOrigin = "https://github.com/operator/operator.git";
...
function makeFixture(options?: { includeSeed?: boolean }) {
  ...
  const root = realpathSync(mkdtempSync(path.join(tmpdir(), "operator-live-updater-")));
  const origin = path.join(root, "origin.git");
  const seed = path.join(root, "seed");
  const mirror = path.join(root, "mirror");
...
describe("operator live updater", () => {
  beforeAll(() => {
    const root = realpathSync(mkdtempSync(path.join(tmpdir(), "operator-live-updater-template-")));
  ...
  test("audits only error and warning logs emitted after Gateway restart", () => {
    const output = [
      { type: "meta", file: "/tmp/operator.log" },
  ...
  test("parses the loaded launchd ProgramArguments block", () => {
    expect(
      parseLaunchctlArguments(`gui/501/ai.operator.gateway = {
\tprogram = /bin/sh
\targuments = {
\t\t/bin/sh
\t\t/Users/test/.operator/service-env/ai.operator.gateway-env-wrapper.sh
\t\t/Users/test/.operator/service-env/ai.operator.gateway.env
\t\t/opt/homebrew/bin/node
\t\t/Users/test/operator/dist/index.js
\t\tgateway
\t\t--port
\t\t18789
\t}
\tpid = 123
  ...
  test("ignores restart-window logs emitted by a foreign Operator checkout", () => {
    const root = mkdtempSync(path.join(tmpdir(), "operator-log-attribution-"));
    const sourceRoot = path.join(root, "managed/operator/dist");
    const foreignRoot = path.join(root, "worktree/operator");
    mkdirSync(path.join(foreignRoot, ".git"), { recursive: true });
    writeFileSync(path.join(foreignRoot, "package.json"), '{"name":"@gabrielvfonseca/operator"}\n');
    const output = [
      {
        "0": '{"subsystem":"gateway"}',
        "1": "managed warning",
  ...
  test("keeps managed restart logs when the deployment root is symlinked", () => {
    const root = mkdtempSync(path.join(tmpdir(), "operator-log-symlink-attribution-"));
    ...
    writeFileSync(path.join(releaseRoot, "package.json"), '{"name":"@gabrielvfonseca/operator"}\n');
  ...
  test("scopes embedded RPC records without dropping unattributed errors", () => {
    const root = mkdtempSync(path.join(tmpdir(), "operator-rpc-log-attribution-"));
    const sourceRoot = path.join(root, "managed/operator/dist");
    const foreignRoot = path.join(root, "worktree/operator");
    mkdirSync(path.join(foreignRoot, ".git"), { recursive: true });
    writeFileSync(path.join(foreignRoot, "package.json"), '{"name":"@gabrielvfonseca/operator"}\n');
    ...
    expect(
      resolveManagedPluginSourceRoots({
        plugins: [
          ...
          { id: "workspace", rootDir: "/srv/workspace/.operator/extensions/workspace" },
          { id: "global", rootDir: "/Users/test/.operator/extensions/global" },
        ],
      }),
    ).toEqual([
      "/opt/configured-plugin",
      "/srv/workspace/.operator/extensions/workspace",
      "/Users/test/.operator/extensions/global",
    ]);
  ...
  test("scopes restart logs to the effective managed runtime", () => {
    expect(
      resolveManagedGatewaySourceRoot("/srv/operator", {
    ...
    expect(
      prepareGatewaySuspension(
        ...
        (
          ...
          expect(params.requestId).toMatch(/^operator-live-updater-/u);
  ...
  test("pins managed Gateway calls with a backward-compatible local overlay", () => {
    const root = realpathSync(mkdtempSync(path.join(tmpdir(), "operator-gateway-call-")));
  ...
  test("accepts supported Operator GitHub origins", () => {
    expect(originMatches("https://github.com/operator/operator.git")).toBe(true);
    expect(originMatches("git@github.com:operator/operator.git")).toBe(true);
    expect(originMatches("https://github.com/example/operator.git")).toBe(false);
  });
  ...
  test("accepts only immutable canonical runtime snapshots owned by the checkout", () => {
    ...
    const runtimeRoot = path.join(home, ".operator/runtime");
    const snapshot = path.join(runtimeRoot, `gateway-${head.slice(0, 7)}`);
    mkdirSync(runtimeRoot, { recursive: true });
    git(runtimeRoot, "clone", origin, snapshot);
    git(snapshot, "remote", "set-url", "origin", "https://github.com/operator/operator.git");
  ...
  test("accepts owned entrypoints only in supported LaunchAgent command layouts", () => {
    ...
    const entrypoint = "/Users/test/.operator/runtime/gateway-1234567/dist/index.js";
    const wrapper = `${home}/.operator/service-env/ai.operator.gateway-env-wrapper.sh`;
    const envFile = `${home}/.operator/service-env/ai.operator.gateway.env`;
    ...
    const customWrapper = `${customState}/service-env/ai.operator.gateway-env-wrapper.sh`;
    const customEnvFile = `${customState}/service-env/ai.operator.gateway.env`;
    expect(
      resolveManagedGatewayEntrypoint(
  ...
  test("retargets an accepted snapshot to the exact source build", () => {
    const checkout = "/Users/test/operator";
    const snapshot = "/Users/test/.operator/runtime/gateway-1234567/dist/index.js";
    const source = path.join(checkout, "dist/index.js");
    const replacements: string[] = [];
    const deployment = {
      configPath: "/Users/test/config/operator.json",
      entrypoint: snapshot,
      entrypointIndex: 1,
      label: "ai.operator.gateway",
      plistPath: "/Users/test/Library/LaunchAgents/ai.operator.gateway.plist",
      port: 18789,
    };
  ...
  test("replaces a wrapped LaunchAgent entrypoint without inserting another argument", () => {
    const snapshot = "/Users/test/.operator/runtime/gateway-1234567/dist/index.js";
    const source = "/Users/test/operator/dist/index.js";
    const original = [
      "/bin/sh",
      "/Users/test/.operator/service-env/ai.operator.gateway-env-wrapper.sh",
      "/Users/test/.operator/service-env/ai.operator.gateway.env",
  ...
  test("fails closed when Gateway service retargeting does not stick", () => {
    ...
    expect(() =>
      repointManagedGatewayDeployment(
        checkout,
        {
          configPath: "/Users/test/.operator/operator.json",
          entrypoint: snapshot,
          label: "ai.operator.gateway",
          port: 18789,
        },
  ...
  test("rejects Git URL rewrites that change the effective fetch source", () => {
    ...
    git(mirror, "config", `url.${origin}.insteadOf`, "https://github.com/operator/operator.git");
  ...
  test("classifies exact-head build, install, macOS rebuild, and native UI proof", () => {
    ...
    expect(
      classifyActions(["package.json", "apps/macos/Sources/Operator/AppDelegate.swift"], {
        buildProvenanceKnown: true,
        buildRequired: true,
        nodeModulesPresent: true,
      }),
    ...
    expect(
      classifyActions(["apps/shared/OperatorKit/Sources/OperatorProtocol/GatewayModels.swift"], {
        buildProvenanceKnown: true,
        buildRequired: true,
        nodeModulesPresent: true,
      }),
  ...
  test("accepts only the delayed exact target bundle process", () => {
    const executable = "/Users/steipete/operator/dist/Operator.app/Contents/MacOS/Operator";
    const foreign = "41 /tmp/agent/Operator.app/Contents/MacOS/Operator";
  ...
  test("fast-forwards, builds exact SHA, restarts Gateway, then proves exact Mac target", () => {
    ...
    mkdirSync(path.join(seed, "apps/macos/Sources/Operator"), { recursive: true });
    writeFileSync(path.join(seed, "apps/macos/Sources/Operator/App.swift"), "// changed\n");
    git(seed, "add", ".");
    ...
    const output = maintainFixture(
      ...
      },
      {
        ...
        verifyMacTarget: () => ({
          executable: path.join(mirror, "dist/Operator.app/Contents/MacOS/Operator"),
          pid: 123,
        }),
      },
    );
    ...
    expect(output.buildChangedPaths).toEqual(["apps/macos/Sources/Operator/App.swift"]);
    ...
    expect(output.macTarget?.executable).toBe(
      path.join(mirror, "dist/Operator.app/Contents/MacOS/Operator"),
    );
  });
  ...
  test("preserves the signed Mac bundle while a Gateway build replaces dist", () => {
    const { root, mirror } = makeFixture();
    mkdirSync(path.join(mirror, "node_modules"));
    const appBundle = path.join(mirror, "dist/Operator.app");
    ...
    expect(
      readdirSync(path.join(mirror, ".git")).filter((entry) =>
        entry.startsWith(".operator-live-mac-"),
      ),
    ).toEqual([]);
  });
  ...
  test("restores the Mac bundle when the Gateway build fails", () => {
    const { root, mirror } = makeFixture();
    mkdirSync(path.join(mirror, "node_modules"));
    const appMarker = path.join(mirror, "dist/Operator.app/Contents/signature-marker");
    mkdirSync(path.dirname(appMarker), { recursive: true });
    writeFileSync(appMarker, "signed\n");
  ...
  test("accepts a delayed external restore of the exact preserved Mac bundle", () => {
    ...
    writeFileSync(appMarker, "signed\n");
    const commands = fakeCommands(mirror);
    const delayedBundle = path.join(root, "delayed-operator.app");
    let restored = false;
  ...
  test("repoints an ancestor snapshot across the next source update", () => {
    ...
    const configPath = path.join(root, "operator.json");
    const plistPath = path.join(root, "ai.operator.gateway.plist");
    writeFileSync(configPath, "{}\n");
  ...
  test("bootstraps an owned LaunchAgent left unloaded by a failed restart", () => {
    ...
    writeFileSync(plistPath, "plist\n", { mode: 0o600 });
    const deployment = {
      configPath: path.join(root, "operator.json"),
      entrypoint: source,
      entrypointIndex: 1,
      executable: process.execPath,
      invocationPrefix: [source],
    ...
    expect(() =>
      maintainMain(
        { checkout: mirror, remote: "origin", lockPath: path.join(root, "maintenance.lock") },
        {
          fetchMain: fetchFixtureMain,
          runCommand(command: string, args: string[]) {
            if (command === "pnpm" && args.slice(0, 3).join(" ") === "operator gateway status") {
              statusCalls += 1;
              throw new Error("RPC unavailable");
            }
          },
