...
const helperPath = path.resolve("scripts/lib/operator-e2e-instance.sh");
const hostPath = [
...
function writePackageFixture(packagePath: string): void {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-package-"));
  try {
    const packageDir = path.join(root, "package");
    fs.mkdirSync(packageDir);
    fs.writeFileSync(
      path.join(packageDir, "package.json"),
      JSON.stringify({ name: "operator-e2e-fixture", version: "0.0.0" }),
    );
...
function expectNpmInstallObserved(argsPath: string, expectedArgs: string, prefix: string): void {
  ...
  expect(
    fs.existsSync(path.join(prefix, "lib/node_modules/operator-e2e-fixture/package.json")),
  ).toBe(true);
}
...
describe("scripts/lib/operator-e2e-instance.sh", () => {
  ...
  it("fails when the test-state payload is not valid base64", () => {
    ...
    expect(result.stderr).toContain("Invalid Operator test-state base64 payload");
  });
  ...
  it("probes default and explicit mock OpenAI base URLs", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-mock-openai-url-"));
    try {
      const probePath = path.join(tempDir, "probe-url.txt");
      const result = runSourcedHelper(
        [
          `operator_e2e_probe_http() { printf "%s\\n" "$1" >>${shellQuote(probePath)}; return 0; }`,
          "operator_e2e_wait_mock_openai 44080 1 400",
          "operator_e2e_wait_mock_openai 443 1 400 https://api.openai.com:443",
        ].join("; "),
      );
  ...
  it("requires /readyz after the gateway ready log", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-readyz-required-"));
    try {
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            "operator_e2e_probe_http() { return 1; }",
            "sleep 30 &",
            'gateway_pid="$!"',
            "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
            `printf '[gateway] ready ws://127.0.0.1:23456\\n' >${shellQuote(logPath)}`,
            `if operator_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2; then`,
  ...
  it("probes /readyz on the explicit gateway port", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-readyz-port-"));
    try {
      const logPath = path.join(tempDir, "gateway.log");
      const probePath = path.join(tempDir, "probe-url.txt");
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `operator_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2 23456`,
          ].join("\n"),
        ],
        {
          encoding: "utf8",
  ...
  it("allows explicit legacy ready-log mode without /readyz", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-readyz-legacy-"));
    try {
      const logPath = path.join(tempDir, "gateway.log");
      const result = spawnSync(
        "/bin/bash",
        [
          ...
          [
            ...
            "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
            `printf '[gateway] ready\\n' >${shellQuote(logPath)}`,
            `operator_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2 23456 legacy-ready-log-ok`,
          ].join("\n"),
        ],
        {
          encoding: "utf8",
  ...
  it("wraps package installs with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-instance-"));
    try {
      ...
      const packagePath = path.join(tempDir, "operator.tgz");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `operator_e2e_install_package ${shellQuote(logPath)} ${shellQuote("fixture package")} ${shellQuote(prefixPath)}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
  ...
  it("falls back to plain timeout when kill-after is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-instance-plain-timeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
  ...
  it("uses gtimeout when GNU timeout is not on PATH", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-instance-gtimeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
  ...
  it("uses the Node watchdog when timeout is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-instance-no-timeout-"));
    try {
      const npmArgsPath = path.join(tempDir, "npm-args.txt");
      const logPath = path.join(tempDir, "install.log");
      const packagePath = path.join(tempDir, "operator.tgz");
  ...
  it("bounds npm install failure logs to the configured tail", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-instance-install-log-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const logPath = path.join(tempDir, "install.log");
      const packagePath = path.join(tempDir, "operator.tgz");
  ...
  it.each([
    ...
    try {
      ...
      const result = runSourcedHelper(`operator_e2e_print_log ${shellQuote(logPath)}`, {
        [envName]: value,
      });
  ...
  it("bounds commands with the Node watchdog when timeout is unavailable", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-instance-node-watchdog-"));
    try {
      writeNodeShim(tempDir);
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `operator_e2e_maybe_timeout 200ms ${shellQuote(process.execPath)} -e ${shellQuote("setInterval(() => {}, 1000)")}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
      ...
      expect(result.stderr).toContain("Operator E2E command timed out after 200ms");
    } finally {
      fs.rmSync(tempDir, { force: true, recursive: true });
    }
  });
  ...
  for (const [shellSignal, expectedStatus] of [
    ...
    it(`escalates Node watchdog children that ignore parent SIG${shellSignal}`, () => {
      const tempDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "operator-e2e-instance-node-watchdog-signal-"),
      );
      try {
        writeNodeShim(tempDir);
...
`;
  ...
  it("terminates only the tracked gateway process", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-gateway-terminate-"));
    try {
...
set -euo pipefail
source ${shellQuote(helperPath)}
operator_e2e_terminate_gateways ""
/bin/sleep 30 &
tracked_pid="$!"
operator_e2e_terminate_gateways "$tracked_pid"
...
`;
  ...
  it("terminates descendants in the tracked process group", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-process-group-"));
...
set -euo pipefail
source ${shellQuote(helperPath)}
tracked_pid="$(operator_e2e_start_tracked_process ${shellQuote(logPath)} ${shellQuote(process.execPath)} ${shellQuote(parentPath)} ${shellQuote(childPath)} ${shellQuote(parentPidPath)} ${shellQuote(childPidPath)} ${shellQuote(childTermPath)})"
for ((i = 0; i < 100; i += 1)); do
...
operator_e2e_stop_process "$tracked_pid"
for ((i = 0; i < 100; i += 1)); do
...
`;
  ...
  it("bounds HTTP readiness probes when a server accepts connections but never responds", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-http-probe-"));
    try {
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            `source ${shellQuote(helperPath)}`,
            'operator_e2e_probe_http_status "http://127.0.0.1:${port}/health" 200 100',
          ].join("; "),
        ],
  ...
  it("does not repeatedly grep the full gateway log while waiting for readiness", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-readyz-incremental-"));
    try {
      const logPath = path.join(tempDir, "gateway.log");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            "operator_e2e_probe_http() { return 0; }",
            "grep() {",
            ...
            `printf 'old log line\\n' >${shellQuote(logPath)}`,
            `printf '[gateway] ready ws://127.0.0.1:23456\\n' >>${shellQuote(logPath)}`,
            `operator_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 2`,
          ].join("\n"),
        ],
        {
          encoding: "utf8",
  ...
  it("detects gateway ready markers split across incremental log reads", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-readyz-split-marker-"));
    try {
      const logPath = path.join(tempDir, "gateway.log");
      const result = spawnSync(
        "/bin/bash",
        [
          ...
          [
            ...
            `operator_e2e_wait_gateway_ready "$gateway_pid" ${shellQuote(logPath)} 8`,
          ].join("\n"),
        ],
        {
          encoding: "utf8",
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `operator_e2e_probe_http() { printf '%s' "$1" >${shellQuote(probePath)}; [[ "$1" = "http://127.0.0.1:23456/readyz" ]]; }`,
            "sleep 30 &",
            'gateway_pid="$!"',
            "trap 'kill \"$gateway_pid\" >/dev/null 2>&1 || true' EXIT",
  ...
  it("wraps logged Operator E2E commands with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-instance-run-logged-"));
    ...
    try {
      ...
      expect(path.basename(logPath)).toMatch(new RegExp(`^operator-${logLabel}\\..+\\.log$`, "u"));
  ...
  it("bounds logged command failure output to the configured tail", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-instance-run-log-tail-"));
    const logLabel = path.basename(tempDir);
    const logDir = path.join(tempDir, "logs");
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `operator_e2e_run_logged ${shellQuote(logLabel)} fixture-command`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
      ...
      const logFile = expectDefined(fs.readdirSync(logDir)[0], "Operator E2E command log file");
  ...
  it("installs the trash shim under isolated test state", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-trash-shim-"));
    try {
      ...
      expect(binDir).not.toBe("/tmp/operator-bin");
  ...
  it("wraps package-installed Operator CLI calls with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-instance-operator-cli-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      const commandArgsPath = path.join(tempDir, "operator-args.txt");
      fs.writeFileSync(
        path.join(tempDir, "timeout"),
        [
          "#!/usr/bin/env bash",
          ...
          'if [ "${1:-}" = "--kill-after=1s" ]; then exit 0; fi',
          'printf "%s\\n" "$*" >"$OPERATOR_TEST_TIMEOUT_ARGS"',
          `while [ "$#" -gt 0 ] && [ "$1" != ${shellQuote(path.join(tempDir, "@gabrielvfonseca/operator"))} ]; do shift; done`,
          '[ "$#" -gt 0 ] || exit 127',
          "shift",
      ...
      fs.writeFileSync(
        path.join(tempDir, "@gabrielvfonseca/operator"),
        [
          "#!/usr/bin/env bash",
          "set -euo pipefail",
          'printf "%s\\n" "$*" >"$OPERATOR_TEST_COMMAND_ARGS"',
      ...
      fs.chmodSync(path.join(tempDir, "timeout"), 0o755);
      fs.chmodSync(path.join(tempDir, "@gabrielvfonseca/operator"), 0o755);
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: shellTestEnv({
            ...
            OPERATOR_TEST_TIMEOUT_ARGS: timeoutArgsPath,
            OPERATOR_TEST_COMMAND_ARGS: commandArgsPath,
            OPERATOR_TEST_OPERATOR_BIN: path.join(tempDir, "@gabrielvfonseca/operator"),
          }),
        },
      );
      ...
      expect(result.status).toBe(0);
      expect(fs.readFileSync(timeoutArgsPath, "utf8").trim()).toBe(
        `--kill-after=30s 23s ${path.join(tempDir, "@gabrielvfonseca/operator")} plugins list --json`,
      );
  ...
  it("wraps interactive PTY scripts with the configured timeout", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-e2e-instance-pty-timeout-"));
    try {
      const timeoutArgsPath = path.join(tempDir, "timeout-args.txt");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            "set -euo pipefail",
            `source ${shellQuote(helperPath)}`,
            `operator_e2e_run_script_with_pty ${shellQuote("node /tmp/entry onboard")} ${shellQuote(logPath)}`,
          ].join("; "),
        ],
        {
          encoding: "utf8",
