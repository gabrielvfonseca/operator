...
function runGatewayPortCheck(fakeLsof: string) {
  const root = mkdtempSync(join(tmpdir(), "operator-restart-mac-test-"));
  tempRoots.push(root);
...
function runCleanupFunction(fakePs: string) {
  ...
  writeFileSync(
    ...
    [
      ...
      'DEBUG_PROCESS_PATTERN="/worktree/apps/macos/.build/debug/Operator"',
      'LOCAL_PROCESS_PATTERN="/worktree/apps/macos/.build-local/debug/Operator"',
      'RELEASE_PROCESS_PATTERN="/worktree/apps/macos/.build/release/Operator"',
      "kill() {",
  ...
  const result = spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      OPERATOR_TEST_KILL_CALLS: killCallsPath,
      PATH: `${binDir}:${process.env.PATH ?? ""}`,
    },
  });
...
function runManagedSupervisorClassifier(
  ...
  const root = mkdtempSync(join(tmpdir(), "operator-restart-mac-supervisor-test-"));
  tempRoots.push(root);
  ...
  writeFileSync(
    ...
    [
      ...
      classifierFunctions,
      "loaded_launch_jobs() {",
      '  [[ "${OPERATOR_TEST_FAIL_ENUMERATION:-0}" != "1" ]] || return 1',
      "  cut -d'|' -f1,2 \"$OPERATOR_TEST_LOADED_JOBS\"",
      "}",
      "launch_job_snapshot() {",
      '  grep "^$1|$2|" "$OPERATOR_TEST_LOADED_JOBS" |',
      "    awk -F'|' '{ print \"program = \" $3; print \"properties = \" $4 }'",
      "}",
      'TARGET_EXECUTABLE="/worktree/dist/Operator.app/Contents/MacOS/Operator"',
      'INSTALLED_EXECUTABLE="/Applications/Operator.app/Contents/MacOS/Operator"',
  ...
  return spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      OPERATOR_TEST_FAIL_ENUMERATION: options.failEnumeration ? "1" : "0",
      OPERATOR_TEST_LOADED_JOBS: recordsPath,
    },
  });
}
...
function runForeignProcessClassifier(fakePs: string) {
  ...
  writeFileSync(
    ...
    [
      ...
      functions,
      'APP_EXECUTABLE_RELATIVE_PATH="Contents/MacOS/Operator"',
      'TARGET_EXECUTABLE="/Users/steipete/operator/dist/Operator.app/Contents/MacOS/Operator"',
      'INSTALLED_EXECUTABLE="/Applications/Operator.app/Contents/MacOS/Operator"',
...
describe("scripts/restart-mac.sh", () => {
  ...
  it("keeps the default restart log scoped to the current worktree lock", () => {
    ...
    expect(script).toContain(
      'LOG_PATH="${OPERATOR_RESTART_LOG:-${TMPDIR:-/tmp}/operator-restart-${LOCK_KEY}.log}"',
    );
    expect(script).not.toContain('LOG_PATH="${OPERATOR_RESTART_LOG:-/tmp/operator-restart.log}"');
  });

  it("does not remove a live restart lock it did not acquire", () => {
    const root = mkdtempSync(join(tmpdir(), "operator-restart-mac-test-"));
    tempRoots.push(root);
    const lockDir = join(root, "operator-restart-lock");
    mkdirSync(lockDir);
  ...
  it("prefers the freshly packaged app unless an explicit app bundle is set", () => {
    ...
    expect(script).toContain('fail "OPERATOR_APP_BUNDLE does not exist: ${APP_BUNDLE}"');
    expect(chooseBlock).toContain("canonicalize_app_bundle");
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/Operator.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("/Applications/Operator.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/Operator.app")).toBeLessThan(
      chooseBlock.indexOf("/Applications/Operator.app"),
    );
  });
  ...
  it("keeps restart cleanup scoped to known Operator app and build paths", () => {
    ...
    expect(cleanupBlock).toContain('"/Applications/Operator.app/${APP_EXECUTABLE_RELATIVE_PATH}"');
    ...
    expect(cleanupBlock).not.toContain('pkill -x "Operator"');
    expect(cleanupBlock).not.toContain("pgrep");
    expect(cleanupBlock).not.toContain('pgrep -x "Operator"');
  });
  ...
  it("finds persistent launchd supervisors across explicit domains", () => {
    const result = runManagedSupervisorClassifier([
      {
        domain: "gui/501",
        label: "ai.operator.mac.custom",
        program: "/Applications/Operator.app/Contents/MacOS/Operator",
        properties: "keepalive | runatload",
      },
      {
        domain: "user/501",
        label: "ai.operator.mac.target",
        program: "/worktree/dist/Operator.app/Contents/MacOS/Operator",
        properties: "keepalive",
      },
      {
        domain: "gui/501",
        label: "application.ai.operator.mac.123",
        program: "/Applications/Operator.app/Contents/MacOS/Operator",
      },
      {
        domain: "system",
    ...
    expect(result.stdout.trim().split("\n").toSorted()).toEqual([
      "ai.operator.mac.custom",
      "ai.operator.mac.target",
    ]);
    expect(result.stderr).toBe("");
  });
  ...
  it("checks managed launchd supervisors before starting the Swift package build", () => {
    ...
    const supervisorIndex = script.indexOf(
      'managed_supervisors="$(managed_operator_supervisor_labels',
    );
  ...
  it("keeps the managed app alive until the signed replacement is ready", () => {
    ...
    expect(script).toContain('OPERATOR_PACKAGE_APP_ROOT="${STAGED_APP_BUNDLE}"');
  ...
  it("escalates only exact managed app processes when graceful shutdown stalls", () => {
    ...
    expect(managedKillBlock).toContain("managed_operator_process_pids");
  ...
  it("treats the canonical installed app as managed but temp bundles as foreign", () => {
    const result = runForeignProcessClassifier(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  101 /Applications/Operator.app/Contents/MacOS/Operator --attach-only'",
        "printf '%s\\n' '  102 /Users/steipete/operator/dist/Operator.app/Contents/MacOS/Operator --attach-only'",
        "printf '%s\\n' '  103 /tmp/agent/Operator.app/Contents/MacOS/Operator --attach-only'",
        "printf '%s\\n' '  104 /bin/sh test.sh /Applications/Operator.app/Contents/MacOS/Operator'",
      ].join("\n"),
    );
  ...
  it("normalizes custom app bundle paths before process matching", () => {
    const root = mkdtempSync(join(tmpdir(), "operator-restart-mac-test-"));
    tempRoots.push(root);
    const appBundle = join(root, "dist", "Operator.app");
    mkdirSync(appBundle, { recursive: true });

    const { result } = runCanonicalizeAppBundle(`${appBundle}/../Operator.app/`);
  ...
  it("fails restart cleanup when scoped processes survive every kill attempt", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  321 /worktree/dist/Operator.app/Contents/MacOS/Operator --attach-only'",
      ].join("\n"),
    );
  ...
  it("passes restart cleanup when the final kill attempt clears the process", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        'kill_count="$(wc -l < "$OPERATOR_TEST_KILL_CALLS" 2>/dev/null || echo 0)"',
  ...
  it("does not kill unrelated Operator app bundles", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  654 /tmp/Other/Operator.app/Contents/MacOS/Operator'",
      ].join("\n"),
    );
