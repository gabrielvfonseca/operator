...
describe("scripts/operator-cross-os-release-checks", () => {
  ...
  it("keeps gateway status RPC probing when help probing is unavailable", () => {
    ...
    expect(buildGatewayStatusArgsFromHelpText("Usage: operator gateway status")).toEqual([
      "gateway",
      "status",
    ]);
    expect(
  ...
  it("gives the Windows packaged updater wrapper enough headroom for Operator timeout output", () => {
  ...
  it("accepts OK agent output from the captured log when stdout is empty", () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-agent-output-"));
    try {
  ...
  it("ignores stale OK markers outside the recent agent log tail", () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-agent-output-tail-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("detects embedded fallback agent turns as non-gateway proof", () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-agent-fallback-"));
    const logPath = join(dir, "agent.log");
    expect(
      agentTurnUsedEmbeddedFallback({
  ...
  it("skips optional live agent turns only for model availability failures", () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-agent-skip-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("does not classify stale timeout logs as current optional agent-turn failures", () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-agent-skip-tail-"));
    try {
      const logPath = join(dir, "agent.log");
      writeFileSync(
        logPath,
  ...
  it("only skips opted-in cross-OS live agent turns after retry exhaustion", () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-agent-skip-retry-"));
    try {
      const logPath = join(dir, "agent.log");
  ...
  it("can stage packaged-upgrade baselines without npm lifecycle scripts", () => {
    expect(buildNpmGlobalInstallArgs("operator@2026.5.2", { ignoreScripts: true })).toEqual([
      "install",
      "-g",
      "operator@2026.5.2",
      "--omit=dev",
  ...
  it("rejects unsafe npm pack tarball filenames before staging release artifacts", () => {
    expect(resolveNpmPackTarballFileName("operator-2026.6.17.tgz")).toBe("operator-2026.6.17.tgz");
  ...
  it("accepts pnpm pack tarballs reported under the requested destination", () => {
    const packDir = resolvePath("/tmp/operator-pack");

    expect(resolvePackDestinationTarball("operator-2026.6.17.tgz", packDir, "pnpm pack")).toEqual({
      fileName: "operator-2026.6.17.tgz",
      path: resolvePath(packDir, "operator-2026.6.17.tgz"),
    });
    expect(
      resolvePackDestinationTarball(
        resolvePath(packDir, "operator-2026.6.17.tgz"),
        packDir,
        "pnpm pack",
      ),
    ).toEqual({
  ...
  it("rejects pnpm pack tarballs outside the requested destination", () => {
    ...
    const unsafeFilenames = [
      ...
      "nested/operator.tgz",
      "nested\\operator.tgz",
      resolvePath(dirname(packDir), "operator.tgz"),
      resolvePath(packDir, "nested", "operator.tgz"),
      "operator\u0000.tgz",
      "operator.tar.gz",
    ];
  ...
  it("falls back to pnpm pack for historical refs without the Docker package helper", () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-pack-command-"));
    try {
      ...
      const helperPath = join(dir, "scripts", "package-operator-for-docker.mjs");
  ...
  it("keeps packaged-upgrade release updates out of service restart flow", () => {
    const args = buildPackagedUpgradeUpdateArgs("http://127.0.0.1:49152/operator-current.tgz");
    expect(args.slice(0, 6)).toEqual([
      "update",
      "--tag",
      "http://127.0.0.1:49152/operator-current.tgz",
      "--yes",
      "--json",
      "--no-restart",
    ]);
  ...
  it("keeps cross-OS live smoke agent turns on GPT-5-safe timeouts and minimal context", () => {
    ...
    expect(source).toContain('agentRuntime: { id: "@gabrielvfonseca/operator" }');
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'operator.cmd')");
    expect(script).toContain("$cmd = Get-Command operator -ErrorAction Stop");
  });
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("operator-2026.4.14.tgz")).toBe("application/octet-stream");
  });

  it("streams release artifacts from the static file server", async () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-static-server-"));
    const filePath = join(dir, "operator-2026.4.14.tgz");
  ...
  it("closes static release artifact sockets left by aborted clients", async () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-static-server-close-"));
    const filePath = join(dir, "operator-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("flushes static release artifact logs before close resolves", async () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-static-server-log-flush-"));
    const filePath = join(dir, "operator-2026.4.14.tgz");
    const logPath = join(dir, "server.log");
    let server: Awaited<ReturnType<typeof startStaticFileServer>> | undefined;
  ...
  it("uses the published installer URLs for native installer lanes", () => {
    expect(resolvePublishedInstallerUrl("darwin")).toBe("https://operator.ai/install.sh");
    expect(resolvePublishedInstallerUrl("linux")).toBe("https://operator.ai/install.sh");
    expect(resolvePublishedInstallerUrl("win32")).toBe("https://operator.ai/install.ps1");
  });
  ...
  it("runs the installed browser override import smoke only on native Windows", () => {
    ...
    expect(script).toContain('from "@gabrielvfonseca/operator/plugin-sdk/plugin-runtime"');
    ...
    const installedScript = buildInstalledBrowserOverrideImportProbeScript(
      "file:///C:/Users/runner/AppData/Roaming/npm/node_modules/operator/dist/plugin-sdk/plugin-runtime.js",
    );
  ...
  it("wraps Windows cmd shims without Node shell argv", () => {
    expect(
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Program Files\nodejs\npm.cmd" view operator@latest version"`,
      ],
      shell: false,
  ...
  it("wraps installed Windows CLI cmd fallbacks without Node shell argv", () => {
    expect(
      resolveInstalledCliInvocation(
        win32.join(String.raw`C:\Operator Prefix`, "operator.cmd"),
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\Operator Prefix\operator.cmd" gateway run --port 1234"`,
      ],
      shell: false,
      windowsVerbatimArguments: true,
    });
  ...
  it("runs resolved command invocations and writes command logs", async () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-run-command-"));
    try {
  ...
  it("bounds retained command output while preserving full command logs", async () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-run-command-output-"));
    try {
      const logPath = join(dir, "command.log");
  ...
  it("flushes command logs before resolving", async () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-run-command-flush-"));
    try {
  ...
  it("reads npm debug logs from the Windows cache root", () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-npm-debug-"));
    try {
  ...
  it("prefers npm configured log directories over cache defaults", () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-npm-logs-dir-"));
    try {
      const homeDir = join(dir, "home");
  ...
  it("keeps npm debug log collection best-effort", () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-npm-debug-best-effort-"));
    try {
      const homeDir = join(dir, "home");
      const logPath = join(dir, "install.log");
  ...
  it("resolves relative npm log config from the install working directory", () => {
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-npm-relative-logs-"));
    try {
      const homeDir = join(dir, "home");
  ...
  it("kills timed-out command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-run-command-timeout-"));
  ...
  it("forwards external termination to command process groups", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-run-command-signal-"));
    const childPidPath = join(dir, "child.pid");
  ...
  it("exits promptly after externally signaled commands close", async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "operator-cross-os-run-command-signal-exit-"));
    const childPidPath = join(dir, "child.pid");
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/operator", "darwin"),
    ).toBe("/Users/runner/.npm-global");
  });

  it("resolves Linux npm package roots when the CLI is a user-local shim", () => {
    const homeDir = mkdtempSync(join(tmpdir(), "operator-cross-os-linux-home-"));
    try {
      ...
      expect(
        resolveInstalledPackageRootFromCliPath(join(cliDir, "@gabrielvfonseca/operator"), "linux", {
          HOME: homeDir,
        }),
      ).toBe(packageRoot);

      rmSync(join(cliDir, "@gabrielvfonseca/operator"), { force: true });
      symlinkSync(join(distDir, "entry.js"), join(cliDir, "@gabrielvfonseca/operator"));
      ...
      verifyPackagedUpgradeUpdateResult(
        {
          ...
          stderr:
            "[operator] Failed to start CLI: Error [ERR_MODULE_NOT_FOUND]: Cannot find module '/tmp/prefix/lib/node_modules/operator/dist/memory-state-old.js'",
        },
  ...
  it("recognizes the shipped Windows updater packaged-upgrade timeout", () => {
    const error = new Error(
      "Command timed out: C:\\hostedtoolcache\\windows\\node\\24.15.0\\x64\\node.exe C:\\Users\\RUNNER~1\\AppData\\Local\\Temp\\operator-upgrade-q9DsA7\\prefix\\node_modules\\operator\\operator.mjs update --tag http://127.0.0.1:49951/operator-2026.5.4-beta.1.tgz --yes --json --no-restart --timeout 1500",
    );
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error(
          "Command timed out: C:\\prefix\\node_modules\\operator\\operator.mjs update --tag http://127.0.0.1:49951/operator-current.tgz --yes --json --timeout 1500",
        ),
        "win32",
      ),
    ).toBe(true);
    ...
    expect(
      isRecoverableWindowsPackagedUpgradeTimeoutError(
        new Error("Command timed out: node operator.mjs update --tag operator@beta"),
        "win32",
      ),
    ).toBe(false);
  });
      ...
      isRecoverableWindowsPackagedUpgradeSwapCleanupFailure(
        {
          exitCode: 1,
          stdout:
            "EPERM: operation not permitted, unlink '/tmp/prefix/node_modules/.operator-1-2/native.node'",
          stderr: "",
        },
        "linux",
      ),
  ...
  it("only treats pinned baseline specs as exact installer version assertions", () => {
    ...
    expect(resolveExplicitBaselineVersion("operator@latest")).toBe("");
    expect(resolveExplicitBaselineVersion("operator@2026.4.10")).toBe("2026.4.10");
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    const prefixDir = mkdtempSync(join(tmpdir(), "operator-cross-os-installed-version-"));
    try {
      const packageRoot =
        process.platform === "win32"
          ? join(prefixDir, "node_modules", "@gabrielvfonseca/operator")
          : join(prefixDir, "lib", "node_modules", "@gabrielvfonseca/operator");
  ...
  it("treats missing package scripts as optional in older refs", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "operator-cross-os-scripts-"));
    try {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
  ...
  it("rejects legacy plugin dependency staging debris before candidate inventory generation", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "operator-cross-os-stage-debris-"));
    try {
      mkdirSync(
        join(packageRoot, "dist", "Extensions", "demo", ".Operator-Install-Stage", "node_modules"),
        { recursive: true },
      );
      writeFileSync(
        join(packageRoot, "dist", "Extensions", "demo", ".Operator-Install-Stage", "package.json"),
        "{}\n",
        "utf8",
      );
  ...
  it("omits local build metadata from candidate package inventories", async () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "operator-cross-os-local-stamps-"));
    try {
      mkdirSync(join(packageRoot, "dist"), { recursive: true });
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({ name: "operator-fixture", version: "0.0.0", files: ["dist/"] }),
        "utf8",
      );
