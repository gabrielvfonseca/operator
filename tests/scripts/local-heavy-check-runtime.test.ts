...
function makeEnv(overrides: Record<string, string | undefined> = {}) {
  ...
  if (!Object.hasOwn(overrides, "OPERATOR_LOCAL_CHECK_MODE")) {
    delete env.OPERATOR_LOCAL_CHECK_MODE;
  }
...
describe("local-heavy-check-runtime", () => {
  it("reenables local heavy-check policy for local wrapper entrypoints", () => {
    expect(resolveLocalHeavyCheckEnv({ OPERATOR_LOCAL_CHECK: "0", PATH: "/usr/bin" })).toEqual({
      OPERATOR_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
    expect(resolveLocalHeavyCheckEnv({ OPERATOR_LOCAL_CHECK: "false", PATH: "/usr/bin" })).toEqual({
      OPERATOR_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
  });
  ...
  it("skips declaration transforms for no-emit tsgo checks", () => {
    const { args } = applyLocalTsgoPolicy([], makeEnv({ OPERATOR_LOCAL_CHECK: "0" }), ROOMY_HOST);
  ...
  it("keeps explicit tsgo flags and Go env overrides intact when throttled", () => {
    const { args, env } = applyLocalTsgoPolicy(
      ...
      makeEnv({
        GOGC: "80",
        GOMEMLIMIT: "5GiB",
        OPERATOR_TSGO_PPROF_DIR: "/tmp/profile",
      }),
      CONSTRAINED_HOST,
    );
  ...
  it("keeps explicit tsgo declaration flags intact", () => {
    const env = makeEnv({ OPERATOR_LOCAL_CHECK_MODE: "full" });
  ...
  it("uses the configured local tsgo build info file", () => {
    const { args } = applyLocalTsgoPolicy(
      [],
      makeEnv({
        OPERATOR_LOCAL_CHECK_MODE: "full",
        OPERATOR_TSGO_BUILD_INFO_FILE: ".artifacts/custom/tsgo.tsbuildinfo",
      }),
      ROOMY_HOST,
    );
  ...
  it("avoids incremental cache reuse for ad hoc tsgo runs", () => {
    const { args } = applyLocalTsgoPolicy(
      ["--extendedDiagnostics"],
      makeEnv({ OPERATOR_LOCAL_CHECK_MODE: "full" }),
      ROOMY_HOST,
    );
  ...
  it("allows forcing the tsgo lock back on", () => {
    expect(
      shouldAcquireLocalHeavyCheckLockForTsgo(
        ["--help"],
        makeEnv({ OPERATOR_TSGO_FORCE_LOCK: "1" }),
      ),
    ).toBe(true);
  });
  ...
  it("allows forcing the oxlint lock back on", () => {
    ...
    expect(
      shouldAcquireLocalHeavyCheckLockForOxlint(["--type-aware", "--", "sample.ts"], {
        cwd,
        env: makeEnv({ OPERATOR_OXLINT_FORCE_LOCK: "1" }),
      }),
    ).toBe(true);
  });
  ...
  it("uses a worktree-local heavy-check lock when explicitly requested", () => {
    ...
    const release = acquireLocalHeavyCheckLockSync({
      cwd,
      env: makeEnv({ OPERATOR_HEAVY_CHECK_LOCK_SCOPE: "worktree" }),
  ...
  it("rejects malformed heavy-check lock timing env values", () => {
    ...
    expect(() =>
      acquireLocalHeavyCheckLockSync({
        cwd,
        env: makeEnv({ OPERATOR_HEAVY_CHECK_LOCK_TIMEOUT_MS: "10ms" }),
        toolName: "oxlint",
      }),
    ).toThrow("OPERATOR_HEAVY_CHECK_LOCK_TIMEOUT_MS must be a positive integer; got: 10ms");
    expect(() =>
      acquireLocalHeavyCheckLockSync({
        cwd,
        env: makeEnv({ OPERATOR_HEAVY_CHECK_LOCK_POLL_MS: "0" }),
        toolName: "oxlint",
      }),
    ).toThrow("OPERATOR_HEAVY_CHECK_LOCK_POLL_MS must be a positive integer; got: 0");
  });
