...
async function withPluginAssetFixture(run: (rootDir: string) => Promise<void>) {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-plugin-assets-"));
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@gabrielvfonseca/canvas-plugin",
          operator: {
            assetScripts: {
    ...
    fs.writeFileSync(
      path.join(rootDir, "extensions", "canvas", "operator.plugin.json"),
  ...
  it("discovers plugin-owned asset scripts by manifest id", async () => {
    await withPluginAssetFixture(async (rootDir) => {
      ...
      expect(hooks).toEqual([
        {
          aliases: ["@gabrielvfonseca/canvas-plugin", "canvas", "canvas-plugin"],
          command: "node scripts/bundle-a2ui.mjs",
          packageName: "@gabrielvfonseca/canvas-plugin",
          phase: "build",
