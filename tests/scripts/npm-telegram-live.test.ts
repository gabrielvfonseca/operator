...
function mkTempRoot() {
  const root = mkdtempSync(path.join(tmpdir(), "operator-npm-telegram-live-"));
  tempRoots.push(root);
  return root;
}
...
describe("package Telegram live Docker E2E", () => {
  it("supports npm-specific Convex credential aliases", () => {
    ...
    expect(script).toContain("OPERATOR_NPM_TELEGRAM_CREDENTIAL_SOURCE");
    expect(script).toContain("OPERATOR_NPM_TELEGRAM_CREDENTIAL_ROLE");
    expect(script).toContain('docker_env+=(-e OPERATOR_QA_CREDENTIAL_SOURCE="$credential_source")');
    expect(script).toContain('docker_env+=(-e OPERATOR_QA_CREDENTIAL_ROLE="$credential_role")');
  });
  ...
  it("defaults CI runs to Convex when broker credentials are present", () => {
    ...
    expect(script).toContain(
      'if [ -n "${CI:-}" ] && [ -n "${OPERATOR_QA_CONVEX_SITE_URL:-}" ]; then',
    );
    expect(script).toContain("OPERATOR_QA_CONVEX_SECRET_CI");
    expect(script).toContain("OPERATOR_QA_CONVEX_SECRET_MAINTAINER");
  ...
  it("installs the package candidate before forwarding runtime secrets", () => {
    ...
    expect(installRun).toContain(
      '-e OPERATOR_E2E_NPM_INSTALL_TIMEOUT="${OPERATOR_E2E_NPM_INSTALL_TIMEOUT:-600s}"',
    );
    expect(installRun).toContain(
    ...
    expect(installRun).toContain(
      'echo "timeout or gtimeout is required for OPERATOR_E2E_NPM_INSTALL_TIMEOUT=$npm_install_timeout" >&2',
    );
    ...
    expect(installRun).not.toContain(
      "running package install without OPERATOR_E2E_NPM_INSTALL_TIMEOUT",
    );
  ...
  it("bounds installed-package hot path Operator commands", () => {
    const script = readFileSync(DOCKER_SCRIPT_PATH, "utf8");
    ...
    expect(script).toContain(
      '-e OPERATOR_E2E_COMMAND_TIMEOUT="${OPERATOR_E2E_COMMAND_TIMEOUT:-300s}"',
    );
    expect(runtimeRun).toContain("source scripts/lib/operator-e2e-instance.sh");
    expect(runtimeRun).toContain("operator_e2e_run_command operator --version");
    expect(runtimeRun).toContain("operator_e2e_run_command operator onboard");
    ...
    expect(runtimeRun).toContain("operator_e2e_run_command operator channels add");
    expect(runtimeRun).toContain("operator_e2e_run_command operator doctor --fix");
    expect(runtimeRun).toContain("operator_e2e_run_command operator doctor --non-interactive");
    expect(runtimeRun).toContain('operator_e2e_print_log "$file"');
    expect(runtimeRun).not.toContain("sed -n '1,220p'");
    expect(runtimeRun).not.toMatch(/^\s*operator (onboard|channels add|doctor )/mu);
  });

  it("can install a resolved package tarball instead of a registry spec", () => {
    const script = readFileSync(DOCKER_SCRIPT_PATH, "utf8");

    expect(script).toContain("OPERATOR_NPM_TELEGRAM_PACKAGE_TGZ");
    expect(script).toContain("OPERATOR_CURRENT_PACKAGE_TGZ");
    expect(script).toContain('-e OPERATOR_QA_PACKAGE_SOURCE="$package_install_source"');
    expect(script).toContain('-e OPERATOR_QA_PACKAGE_SOURCE_KIND="$package_source_kind"');
    expect(script).toContain("OPERATOR_QA_PACKAGE_SOURCE_SHA");
    expect(script).toContain(
      'package_mount_args=(-v "$resolved_package_tgz:$package_install_source:ro")',
    );
    expect(script).toContain('validate_operator_package_spec "$PACKAGE_SPEC"');
    expect(script.indexOf('if [ -n "$resolved_package_tgz" ]; then')).toBeLessThan(
      script.indexOf('validate_operator_package_spec "$PACKAGE_SPEC"'),
    );
  });
  ...
  it("installs prepared root and companion tarballs through an exact local registry", () => {
    ...
    expect(script).toContain("OPERATOR_NPM_TELEGRAM_PACKAGE_DIR");
    expect(script).toContain('package_source_kind="prepared-package-set"');
    expect(script).toContain('package_install_source="operator@$(read_package_version');
    ...
    expect(script).toContain("OPERATOR_NPM_TELEGRAM_PACKAGE_SET");
    expect(script).toContain("node /tmp/operator-npm-registry-server.mjs");
  ...
  it("keeps live Docker artifacts isolated by default", () => {
    ...
    expect(script).toContain(
      'RUN_ID="${OPERATOR_NPM_TELEGRAM_RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-$$}"',
    );
    expect(script).toContain(
      'OUTPUT_DIR="${OPERATOR_NPM_TELEGRAM_OUTPUT_DIR:-.artifacts/qa-e2e/npm-telegram-live/$RUN_ID}"',
    );
    expect(script).toContain(
    ...
    expect(script).toContain(
      '-e OPERATOR_NPM_TELEGRAM_OUTPUT_DIR="$OUTPUT_DIR_CONTAINER_RELATIVE"',
    );
    expect(script).not.toContain(
      'OUTPUT_DIR="${OPERATOR_NPM_TELEGRAM_OUTPUT_DIR:-.artifacts/qa-e2e/npm-telegram-live}"',
    );
  });
  ...
  it("uses unique direct-run output dirs by default", () => {
    ...
    expect(
      testing.resolvePackageTelegramOutputDir(
        { OPERATOR_NPM_TELEGRAM_OUTPUT_DIR: ".artifacts/custom" },
        repoRoot,
      ),
    ).toBe(".artifacts/custom");
  });

  it("keeps the installed Operator command as the package SUT", async () => {
    const prefix = mkTempRoot();
    const command = path.join(prefix, "bin", "@gabrielvfonseca/operator");
    mkdirSync(path.dirname(command), { recursive: true });
    writeFileSync(command, "#!/bin/sh\n");

    await expect(
      testing.resolveTrustedOperatorCommand(command, {
  ...
  it("mounts configured output paths before entering the container", () => {
    ...
    expect(dockerEnv).not.toContain('-e OPERATOR_NPM_TELEGRAM_OUTPUT_DIR="$OUTPUT_DIR_CONTAINER"');
    expect(dockerEnv).not.toContain('-e OPERATOR_NPM_TELEGRAM_OUTPUT_DIR="$OUTPUT_DIR"');
    expect(script).toContain('-v "$OUTPUT_DIR_HOST:$OUTPUT_DIR_CONTAINER"');
  });

  it("uses the container temp root for Operator runtime scratch files", () => {
    const script = readFileSync(DOCKER_SCRIPT_PATH, "utf8");
    const dockerEnvStart = script.indexOf("docker_env=(");
    const dockerEnvEnd = script.indexOf(")\n\nforward_env_if_set", dockerEnvStart);
    const dockerEnv = script.slice(dockerEnvStart, dockerEnvEnd);
  ...
  it("forwards repeated RTT controls to the package Telegram live lane", () => {
    ...
    expect(script).toContain("OPERATOR_NPM_TELEGRAM_RTT_SAMPLES");
    expect(script).toContain("OPERATOR_NPM_TELEGRAM_RTT_TIMEOUT_MS");
    expect(script).toContain("OPERATOR_NPM_TELEGRAM_RTT_MAX_FAILURES");
    expect(script).toContain("OPERATOR_NPM_TELEGRAM_RTT_CHECKS");
  });
  ...
  it("keeps private QA harness imports local while using the installed package dist", () => {
    ...
    expect(script).toContain('ln -sfnT "$operator_package_dir/dist" /app/dist');
    expect(script).toContain('cp "$operator_package_dir/package.json" /app/package.json');
    ...
    expect(script).not.toContain('ln -sfnT /app/extensions "$operator_package_dir/extensions"');
    expect(script).toContain("node scripts/e2e/lib/npm-telegram-live/prepare-package.mjs");
    expect(script).toContain("/app/node_modules/operator/package.json");
    ...
    expect(gatewayRpcClient).toContain(
      'from "@gabrielvfonseca/operator/plugin-sdk/gateway-runtime"',
    );
    expect(qaRuntimeApi).toContain('from "@gabrielvfonseca/operator/plugin-sdk/gateway-runtime"');
    for (const source of qaHarnessSources) {
      expect(source).not.toContain('from "@gabrielvfonseca/operator/plugin-sdk/qa-runtime"');
    }
  });
  ...
  it("lets npm-specific credential aliases override shared QA env", () => {
    expect(
      testing.resolveCredentialSource({
        OPERATOR_NPM_TELEGRAM_CREDENTIAL_SOURCE: "convex",
        OPERATOR_QA_CREDENTIAL_SOURCE: "env",
      }),
    ).toBe("convex");
    expect(
      testing.resolveCredentialRole({
        OPERATOR_NPM_TELEGRAM_CREDENTIAL_ROLE: "ci",
        OPERATOR_QA_CREDENTIAL_ROLE: "maintainer",
      }),
    ).toBe("ci");
  });
  ...
  it("maps repeated RTT env onto package Telegram live options", () => {
    expect(
      testing.resolveRttOptions({
        OPERATOR_NPM_TELEGRAM_RTT_SAMPLES: "7",
        OPERATOR_NPM_TELEGRAM_RTT_TIMEOUT_MS: "45000",
        OPERATOR_NPM_TELEGRAM_RTT_MAX_FAILURES: "2",
        OPERATOR_NPM_TELEGRAM_RTT_CHECKS: "channel-canary",
      }),
    ).toEqual({
      scenarioId: "channel-canary",
      count: 7,
  ...
  it("rejects retired RTT scenario ids", () => {
    expect(() =>
      testing.resolveRttOptions({
        OPERATOR_NPM_TELEGRAM_RTT_CHECKS: "telegram-mentioned-message-reply",
      }),
  ...
  it("rejects invalid repeated RTT env", () => {
    expect(() =>
      testing.resolveRttOptions({
        OPERATOR_NPM_TELEGRAM_RTT_SAMPLES: "7samples",
      }),
    ).toThrow("invalid OPERATOR_NPM_TELEGRAM_RTT_SAMPLES: 7samples");
  });
  ...
  it("gates package Telegram status on the summary artifact", async () => {
    ...
    await expect(
      testing.shouldFailPackageTelegramRun(
        { summaryPath },
        { OPERATOR_NPM_TELEGRAM_ALLOW_FAILURES: "" },
      ),
    ).resolves.toBe(true);
  });
  ...
  it("does not read package Telegram summaries when failures are allowed", async () => {
    await expect(
      testing.shouldFailPackageTelegramRun(
        ...
        { OPERATOR_NPM_TELEGRAM_ALLOW_FAILURES: "1" },
      ),
    ).resolves.toBe(false);
  });
});
