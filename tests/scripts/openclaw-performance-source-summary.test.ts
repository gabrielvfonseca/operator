...
import { buildMarkdown, parseArgs } from "../../scripts/operator-performance-source-summary.mjs";

const tmpRoots: string[] = [];

function mkTmpRoot() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "operator-source-summary-"));
  tmpRoots.push(root);
  return root;
}
...
function runCli(...args: string[]) {
  return spawnSync(process.execPath, ["scripts/operator-performance-source-summary.mjs", ...args], {
    cwd: path.resolve("."),
    encoding: "utf8",
  });
}
