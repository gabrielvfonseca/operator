...
function makeTempRoot(): string {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "operator-android-signing-"));
    ...
    `${JSON.stringify(
      {
        ...
        gradlePropertyNames: [
          "OPERATOR_ANDROID_STORE_FILE",
          "OPERATOR_ANDROID_STORE_PASSWORD",
...
function writeSigningSources(tempRoot: string) {
  ...
  fs.writeFileSync(
    propertiesPath,
    [
      `OPERATOR_ANDROID_STORE_PASSWORD=${STORE_PASSWORD}`,
      "OPERATOR_ANDROID_KEY_ALIAS=operator-upload",
      `OPERATOR_ANDROID_KEY_PASSWORD=${KEY_PASSWORD}`,
      "",
    ].join("\n"),
  );
...
describe("scripts/android-release-signing.mjs", () => {
  ...
  it("documents the canonical Android release signing plan", () => {
    ...
    expect(result.stdout).toContain("Signing repo: git@github.com:operator/apps-signing.git");
    expect(result.stdout).toContain("Signing assets: android/operator");
  ...
  it.runIf(commandAvailable("openssl"))(
    ...
    () => {
      ...
      const encryptedProperties = fs.readFileSync(
        path.join(remoteCheck, "android", "@gabrielvfonseca/operator", "gradle.properties.enc"),
        "utf8",
      );
      ...
      expect(materializedProperties).toContain(
        `OPERATOR_ANDROID_STORE_FILE=${path.join(materializedDir, "upload-keystore.jks")}`,
      );
      expect(materializedProperties).toContain(`OPERATOR_ANDROID_STORE_PASSWORD=${STORE_PASSWORD}`);
      expect(materializedProperties).toContain("OPERATOR_ANDROID_KEY_ALIAS=operator-upload");
      expect(materializedProperties).toContain(`OPERATOR_ANDROID_KEY_PASSWORD=${KEY_PASSWORD}`);
