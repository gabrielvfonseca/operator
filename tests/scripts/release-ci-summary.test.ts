...
function trustedMainPackageFixture({
  ...
  const parentRun = {
    ...
    html_url: `https://github.com/gabrielvfonseca/operator/actions/runs/${runId}`,
    id: Number(runId),
    path: parentPath,
  ...
  const childRun = {
    ...
    head_branch: workflowRef,
    head_sha: workflowSha,
    html_url: `https://github.com/gabrielvfonseca/operator/actions/runs/${childRunId}`,
    id: Number(childRunId),
    path: ".github/workflows/operator-release-checks.yml",
    repository: { full_name: "operator/operator" },
    run_attempt: 1,
    status: "completed",
  ...
  const client = {
    ...
    getJobLog(jobId: number) {
      ...
      return [
        ...
        `Dispatched operator-release-checks.yml: ${childRun.html_url}`,
      ].join("\n");
    },
...
describe("release CI summary child correlation", () => {
  ...
  it("binds the parent to the exact Full Release Validation REST run", () => {
    ...
    expect(() =>
      validateParentRunBinding(
        parentView,
        { ...parentRest, path: ".github/workflows/operator-release-checks.yml" },
        "29090000000",
      ),
  ...
  it("derives every child title from the exact parent run and attempt", () => {
    expect(expectedChildDispatches("29090000000", 3, "release/2026.7.1")).toEqual([
      ...
      {
        displayTitle:
          "Operator Release Checks full-release-validation-29090000000-3-release-checks",
        headBranch: "release/2026.7.1",
        manifestKey: "releaseChecks",
        name: "Operator Release Checks",
        parentJobName: "Run release/live/Docker/QA validation",
        suffix: "-release-checks",
        trustedRef: "parent",
        workflow: "operator-release-checks.yml",
      },
      {
      ...
      {
        displayTitle: "Operator Performance full-release-validation-29090000000-3",
        headBranch: "release/2026.7.1",
        ...
        suffix: "",
        trustedRef: "parent",
        workflow: "operator-performance.yml",
      },
    ]);
  });

  it("ignores same-SHA and nearby-name runs without the exact parent dispatch binding", () => {
    const expected = "Operator Performance full-release-validation-29090000000-3";
    const exact = {
    ...
    expect(
      selectExactChildRun(
        [
          {
            display_title: "Operator Performance",
            event: "workflow_dispatch",
            head_branch: "main",
  ...
  it("validates manifest child workflow, dispatch tuple, branch, and attempt", () => {
    ...
    const parentLog = [
      ...
      "Dispatched ci.yml: https://github.com/gabrielvfonseca/operator/actions/runs/101",
    ].join("\n");
    const run = {
      actor: { login: "github-actions[bot]" },
      display_title: child.displayTitle,
  ...
  it("accepts strongly bound legacy and correlated children across parent attempts", () => {
    ...
    const fixtures = new Map([
      ...
      [
        "productPerformance",
        {
          originAttempt: 1,
          runId: 28717802171,
          title: "Operator Performance full-release-validation-28717729503-1",
        },
      ],
      ["releaseChecks", { originAttempt: 1, runId: 28717802397, title: "Operator Release Checks" }],
    ]);
    const fingerprint = {
    ...
    for (const child of children.filter((entry) => fixtures.has(entry.manifestKey))) {
      ...
      const parentLog = [
        ...
        `Dispatched ${child.workflow}: https://github.com/gabrielvfonseca/operator/actions/runs/${runId}`,
      ].join("\n");
