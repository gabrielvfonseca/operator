...
describe("scripts/lib/vitest-shard-timings.mjs", () => {
  ...
  it("uses the CI shard name for include-pattern timing keys", () => {
    expect(
      resolveShardTimingKey({
        ...
        env: { OPERATOR_VITEST_SHARD_NAME: "auto-reply/reply agent dispatch" },
  ...
  it("persists include-pattern timing metadata", () => {
    ...
    const env = {
      OPERATOR_TEST_PROJECTS_TIMINGS_PATH: path.join(tempDir, "timings.json"),
      OPERATOR_VITEST_SHARD_NAME: "auto-reply-reply-agent-runner",
    };
    ...
    const persistedTiming = JSON.parse(
      fs.readFileSync(env.OPERATOR_TEST_PROJECTS_TIMINGS_PATH, "utf8"),
