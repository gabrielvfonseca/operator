...
describe("plain gh helpers", () => {
  it("prefers OPERATOR_GH_BIN over PATH shims", () => {
    const ghPath = makeFakeGh();

    expect(
      resolvePlainGhBin({
        HOME: path.dirname(path.dirname(ghPath)),
        OPERATOR_GH_BIN: ghPath,
        PATH: "",
      }),
    ).toBe(ghPath);
  });
  ...
  it("routes explicit GET reads through the PATH shim", () => {
    ...
    const output = execGhApiRead("repos/operator/operator/pulls/1", {
      ...
      env: {
        ...process.env,
        OPERATOR_GH_BIN: "/identity-sensitive/plain-gh",
    ...
    expect(output).toContain("OPERATOR_GH_BIN_SET=");
  });
  ...
  it("runs the shell helper with color disabled", () => {
    ...
    const script = [
      ...
      `OPERATOR_GH_BIN=${JSON.stringify(ghPath)}`,
      "export OPERATOR_GH_BIN",
