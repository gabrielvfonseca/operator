...
function git(cwd: string, args: string[]): string {
  return execFileSync("git", args, {
    ...
    env: {
      ...
      GIT_AUTHOR_EMAIL: "test@operator.invalid",
      GIT_COMMITTER_NAME: "Operator Test",
      GIT_COMMITTER_EMAIL: "test@operator.invalid",
    },
  }).trim();
}
...
describe("release-note verification", () => {
  ...
  it("reuses exact-range GitHub GraphQL snapshots without caching REST reads", () => {
    const cwd = mkdtempSync(join(tmpdir(), "operator-release-notes-snapshot-"));
    try {
      ...
      expect(
        githubApiWithSnapshot(["repos/operator/operator/releases/tags/v1"], fetchApi, first),
      ).toEqual({
        data: {
          request: ["repos/operator/operator/releases/tags/v1"],
          fetches: 2,
        },
      });
  ...
  it("records a canonical target SHA when --target is symbolic", () => {
    const cwd = mkdtempSync(join(tmpdir(), "operator-release-notes-"));
    try {
      git(cwd, ["init", "-q"]);
      writeFileSync(
