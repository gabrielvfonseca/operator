...
function extractWindowsBackgroundControlMarkers(decoded: string): {
  ...
  return {
    done: marker("__OPERATOR_BACKGROUND_DONE__", false),
    exitPrefix: marker("__OPERATOR_BACKGROUND_EXIT__", true),
  };
}
...
`,
  ...
  it("guards beta validation against cross-version harness checkouts", () => {
    ...
    expect(script).toContain("OPERATOR_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH");
  ...
  it("bounds POSIX guest failure logs", () => {
    ...
    expect(scripts).toContain("OPERATOR_PARALLELS_NPM_UPDATE_LOG_TAIL_BYTES");
  ...
  it("passes platform model timeouts to POSIX update agent turns", () => {
    ...
    withEnv(
      {
        OPERATOR_PARALLELS_LINUX_MODEL_TIMEOUT_S: undefined,
        OPERATOR_PARALLELS_MACOS_MODEL_TIMEOUT_S: undefined,
        OPERATOR_PARALLELS_MODEL_TIMEOUT_S: undefined,
      },
      () => {
    ...
    withEnv(
      {
        OPERATOR_PARALLELS_LINUX_MODEL_TIMEOUT_S: "321",
        OPERATOR_PARALLELS_MACOS_MODEL_TIMEOUT_S: "654",
      },
      () => {
  ...
  it("sets platform-aware fresh lane timeouts", () => {
    withEnv({ OPERATOR_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: undefined }, () => {
    ...
    withEnv({ OPERATOR_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: "3" }, () => {
      expect(freshLaneTimeoutMs("macos")).toBe(3000);
    });

    withEnv(
      { OPERATOR_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: String(Number.MAX_SAFE_INTEGER) },
      () => {
...
`,
  ...
  it("scrubs future plugin entries before invoking old same-guest updaters", () => {
    ...
    expect(script).toContain("Invoke-WithScopedEnv @{ OPERATOR_DISABLE_BUNDLED_PLUGINS = '1'");
    expect(macosScript).toContain('OPERATOR_BIN="$(resolve_required_command operator)"');
    expect(macosScript).toContain("/usr/local/bin:/usr/local/sbin");
    expect(macosScript).toContain(
      'OPERATOR_DISABLE_BUNDLED_PLUGINS=1 "$OPERATOR_BIN" update --tag',
    );
    expect(macosScript).not.toContain("/opt/homebrew/bin/operator");
    expect(script).toContain("OPERATOR_DISABLE_BUNDLED_PLUGINS=1 operator update --tag");
    expect(macosScript).toContain(
      'OPERATOR_DISABLE_BUNDLED_PLUGINS=1 "$OPERATOR_BIN" gateway stop',
    );
    expect(script).toContain(
      "OPERATOR_DISABLE_BUNDLED_PLUGINS=1 OPERATOR_ALLOW_ROOT=1 operator gateway stop",
    );
  });
  ...
  it("reenables bundled plugins before Windows post-update verification", () => {
    ...
    const scopedIndex = script.indexOf("Invoke-WithScopedEnv @{ OPERATOR_DISABLE_BUNDLED_PLUGINS");
    ...
    expect(script).not.toContain("$env:OPERATOR_DISABLE_BUNDLED_PLUGINS = '1'");
  });
