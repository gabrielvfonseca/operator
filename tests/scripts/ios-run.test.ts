...
function makeFixture(bundleId: string): { root: string; script: string; logFile: string } {
  const root = mkdtempSync(path.join(os.tmpdir(), "operator-ios-run-"));
  tempDirs.push(root);
  ...
  writeExecutable(
    ...
    `#!/usr/bin/env bash
set -euo pipefail
if [[ -n "\${OPERATOR_SIMULATOR_PUSH_PROOF_SECRET:-}" || -n "\${CUSTOM_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
if (process.env.OPERATOR_SIMULATOR_PUSH_PROOF_SECRET || process.env.CUSTOM_SIMULATOR_PUSH_PROOF_SECRET) {
...
set -euo pipefail
printf 'xcodegen %s\\n' "$*" >>"${logFile}"
if [[ -n "\${OPERATOR_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
app_dir="$derived/Build/Products/$configuration-iphonesimulator/Operator.app"
mkdir -p "$app_dir"
...
printf 'simctl %s\\n' "$*" >>"${logFile}"
if [[ "$1" == "launch" ]]; then
  if [[ -n "\${SIMCTL_CHILD_OPERATOR_SIMULATOR_PUSH_PROOF_SECRET:-}" ]]; then
...
describe("scripts/ios-run.sh", () => {
  ...
  it("installs and launches the configured app bundle identifier", () => {
    const fixture = makeFixture("ai.operatorfoundation.app");

    runIosRun(fixture, { SIMCTL_BOOT_MODE: "booted" });

    expect(readFileSync(fixture.logFile, "utf8")).toContain(
      "simctl launch iPhone 17 ai.operatorfoundation.app",
    );
  });
  ...
  it("builds simulator sandbox relay mode and injects proof secret only at launch", () => {
    ...
    runIosRun(
      fixture,
      {
        OPERATOR_SIMULATOR_PUSH_PROOF_SECRET: proofSecret,
    ...
    expect(log).toContain("OPERATOR_PUSH_MODE=simulatorSandbox");
    expect(log).toContain(
      "OPERATOR_PUSH_RELAY_BASE_URL=https://ios-push-relay-sandbox.operator.ai",
    );
    expect(log).toContain("simctl launch iPhone 17 ai.operatorfoundation.app");
  ...
  it("scrubs exported simulator proof secrets from normal build helpers", () => {
    ...
    runIosRun(fixture, {
      OPERATOR_SIMULATOR_PUSH_PROOF_SECRET: proofSecret,
      OPERATOR_SIMULATOR_PUSH_PROOF_SECRET_ENV: "CUSTOM_SIMULATOR_PUSH_PROOF_SECRET",
