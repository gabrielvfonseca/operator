...
describe("release-beta-smoke", () => {
  ...
  it("rejects malformed positive integer environment limits", () => {
    expect(readPositiveInt(undefined, 60, "OPERATOR_RELEASE_BETA_SMOKE_COMMAND_MS")).toBe(60);
    expect(readPositiveInt("", 60, "OPERATOR_RELEASE_BETA_SMOKE_COMMAND_MS")).toBe(60);
    expect(readPositiveInt("25", 60, "OPERATOR_RELEASE_BETA_SMOKE_COMMAND_MS")).toBe(25);

    for (const raw of ["1e3", "25ms", "1.5", "0", "-1", String(Number.MAX_SAFE_INTEGER + 1)]) {
      expect(() => readPositiveInt(raw, 60, "OPERATOR_RELEASE_BETA_SMOKE_COMMAND_MS")).toThrow(
        "OPERATOR_RELEASE_BETA_SMOKE_COMMAND_MS must be a positive integer",
      );
    }
  });
  ...
  it("parses workflow run urls when gh includes them in dispatch output", () => {
    expect(
      parseWorkflowRunIdFromOutput(
        "Dispatched: https://github.com/gabrielvfonseca/operator/actions/runs/1234567890",
      ),
    ).toBe("1234567890");
  });
  ...
  it("replaces stale Telegram proof placeholders", () => {
    ...
    const merged = mergeTelegramProofIntoReleaseBody(
      body,
      "- npm Telegram beta E2E: https://github.com/gabrielvfonseca/operator/actions/runs/123",
    );
  ...
  it("stops polling Telegram workflow runs after the timeout budget", async () => {
    ...
    await expect(
      pollRun("operator/operator", "123", {
        ...
        readRun: () => ({
          conclusion: null,
          html_url: "https://github.com/gabrielvfonseca/operator/actions/runs/123",
          status: "queued",
