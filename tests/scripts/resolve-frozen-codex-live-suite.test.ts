...
function runResolver(params: {
  ...
  const result = spawnSync(process.execPath, ["scripts/resolve-frozen-codex-live-suite.mjs"], {
    ...
    env: {
      ...
      OPERATOR_ALLOW_FROZEN_TARGET_SCENARIO_OMISSIONS: params.allow === false ? "0" : "1",
      OPERATOR_FROZEN_CODEX_SUITE_ID: params.suiteId,
      OPERATOR_FROZEN_TARGET_ROOT: root,
      OPERATOR_SELECTED_SHA: params.selectedSha ?? "a".repeat(40),
      OPERATOR_WORKFLOW_SHA: "b".repeat(40),
    },
  });
  return {
    ...result,
...
describe("frozen Codex live-suite resolver", () => {
  it("selects the newest model exposed by an older frozen target", () => {
    ...
    expect(result.envFile).toBe("OPERATOR_LIVE_CODEX_HARNESS_MODEL=openai/gpt-5.5\n");
  ...
  it("keeps the current GPT-5.6 cohort and generic default unchanged", () => {
    ...
    expect(generic.envFile).toBe("OPERATOR_LIVE_CODEX_HARNESS_MODEL=openai/gpt-5.6-luna\n");
  });
