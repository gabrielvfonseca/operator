...
function expectedArchiveEnv(fixture: ReturnType<typeof createFixture>): NodeJS.ProcessEnv {
  ...
  return {
    ...fixture.env,
    OPERATOR_SHARED_IMAGE_ARCHIVE_SHA256: manifest.archive.sha256,
    OPERATOR_SHARED_IMAGE_RUN_ATTEMPT: String(manifest.runAttempt),
    OPERATOR_SHARED_IMAGE_RUN_ID: String(manifest.runId),
  };
}
...
describe("shared Docker image artifacts", () => {
      ...
      for (const variant of [
        {
          env: {
            ...
            OPERATOR_SHARED_IMAGE_RUN_ID: "654321",
          },
        ...
        },
        {
          env: {
            ...expectedArchiveEnv(fixture),
            OPERATOR_SHARED_IMAGE_PACKAGE_SHA256: "d".repeat(64),
          },
          imageRefs: IMAGE_REFS,
  ...
  it("requires an external expected archive digest before loading", () => {
    ...
    try {
      ...
      delete missingRunEnv.OPERATOR_SHARED_IMAGE_RUN_ID;
      delete missingRunEnv.OPERATOR_SHARED_IMAGE_RUN_ATTEMPT;
      ...
      expect(missingRun.stderr).toContain("OPERATOR_SHARED_IMAGE_RUN_ID");
      expect(readFileSync(fixture.dockerLog, "utf8")).not.toContain("image load");
      ...
      const missing = runHelper({
        ...
        env: {
          ...fixture.env,
          OPERATOR_SHARED_IMAGE_RUN_ATTEMPT: "2",
          OPERATOR_SHARED_IMAGE_RUN_ID: "123456",
        },
      });
      ...
      const mismatched = runHelper({
        ...
        env: {
          ...expectedArchiveEnv(fixture),
          OPERATOR_SHARED_IMAGE_ARCHIVE_SHA256: "d".repeat(64),
        },
      });
