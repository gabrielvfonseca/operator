...
describe("Docker E2E client scripts", () => {
  ...
  it("keeps session runtime-context checks wired to packaged transcript behavior", () => {
    ...
    expect(source).toContain("operator/plugin-sdk/agent-sessions");
    ...
    expect(source).toContain("<<<BEGIN_OPERATOR_INTERNAL_CONTEXT>>>");
    expect(source).toContain("operator.runtime-context");
