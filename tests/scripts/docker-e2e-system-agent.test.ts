// Operator Docker E2E tests cover packaged-dist harness wiring.
...
describe("Operator Docker E2E scripts", () => {
  it("keeps first-run checks wired to packaged CLI and Operator behavior", () => {
    ...
    expect(source).toContain("Operator did not fail closed without inference");
    ...
    expect(source).toContain("[operator] interpreted: ${plannerCommand}");
    expect(source).toContain("expected one fuzzy setup planner prompt");
    expect(source).toContain("Operator did not enable Discord");
    expect(source).toContain("Operator did not write Discord token SecretRef");
    expect(source).toContain("Operator first-run Docker E2E passed");
    expect(spec).toContain('"auditOperations"');
    expect(spec).toContain('"operator.setup"');
  ...
  it("keeps rescue checks wired through auto-reply command handling", () => {
    ...
    expect(shell).toContain("OPERATOR_GATEWAY_TOKEN=system-agent-rescue-token");
    ...
    expect(source).toContain("[operator] done: gateway.restart");
    ...
    expect(source).not.toContain("[operator] done: doctor.fix");
    expect(source).toContain("Operator rescue Docker E2E passed");
  });
});
