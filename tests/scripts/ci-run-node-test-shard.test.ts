...
describe("scripts/ci-run-node-test-shard.mjs", () => {
  ...
  it("prefers explicit targets and keeps one target per child", () => {
    const plans = resolveShardPlans({
      OPERATOR_NODE_TEST_TARGETS_JSON: JSON.stringify(["a.test.ts", "b.test.ts"]),
      OPERATOR_NODE_TEST_GROUPS_JSON: JSON.stringify([{ configs: ["c.config.ts"] }]),
    });
  ...
  it("falls back from groups to the single-shard matrix envelope", () => {
    const groupPlans = resolveShardPlans({
      OPERATOR_NODE_TEST_GROUPS_JSON: JSON.stringify([
    ...
    const singlePlans = resolveShardPlans({
      OPERATOR_NODE_TEST_CONFIGS_JSON: JSON.stringify(["solo.config.ts"]),
      OPERATOR_VITEST_SHARD_NAME: "solo",
    });
  ...
  it("builds child env with per-plan cache isolation, includes, and env overlays", () => {
    ...
    const childEnv = buildChildEnv(
      entry,
      { BASE: "1", OPERATOR_VITEST_INCLUDE_FILE: "stale.json" },
      scratchDir,
      3,
    );
    ...
    expect(childEnv.OPERATOR_VITEST_SHARD_NAME).toBe("g");
    expect(childEnv.OPERATOR_TEST_PROJECTS_PARALLEL).toBe("1");
    expect(childEnv.OPERATOR_TEST_HEAVY_CHECK_LOCK_HELD).toBe("1");
    expect(childEnv.OPERATOR_VITEST_FS_MODULE_CACHE_PATH).toBe(
      path.join(scratchDir, "vitest-cache-3"),
    );
    expect(childEnv.OPERATOR_VITEST_INCLUDE_FILE).toBe(
      path.join(scratchDir, "node-test-include-3.json"),
    );
    expect(JSON.parse(readFileSync(childEnv.OPERATOR_VITEST_INCLUDE_FILE ?? "", "utf8"))).toEqual([
      "src/a.test.ts",
    ]);

    const bare = buildChildEnv(
      { kind: "group" as const, name: "bare", plan: { configs: ["cfg.ts"] } },
      { OPERATOR_VITEST_INCLUDE_FILE: "stale.json" },
      scratchDir,
      0,
    );
    expect(bare.OPERATOR_VITEST_INCLUDE_FILE).toBeUndefined();
  });
    ...
    const exitCode = await runShardPlans(
      ...
      }),
      {
        ...
        runChild: async (
          ...
          seen.push({ args, cache: childEnv.OPERATOR_VITEST_FS_MODULE_CACHE_PATH, label });
          active -= 1;
          return 0;
        },
        scratchDir,
