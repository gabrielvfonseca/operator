  ...
  it("rejects loose numeric env limits instead of parsing prefixes", () => {
    expect(() =>
      readTelegramBotApiLimits({
        OPERATOR_TELEGRAM_USER_BOT_API_TIMEOUT_MS: "1e3",
      }),
    ).toThrow("invalid OPERATOR_TELEGRAM_USER_BOT_API_TIMEOUT_MS: 1e3");
    expect(() =>
      readTelegramBotApiLimits({
        OPERATOR_TELEGRAM_USER_BOT_API_BODY_MAX_BYTES: "1000ms",
      }),
    ).toThrow("invalid OPERATOR_TELEGRAM_USER_BOT_API_BODY_MAX_BYTES: 1000ms");
    expect(
      readTelegramBotApiLimits({
        OPERATOR_TELEGRAM_USER_BOT_API_BODY_MAX_BYTES: "2048",
        OPERATOR_TELEGRAM_USER_BOT_API_TIMEOUT_MS: "15000",
      }),
    ).toEqual({
      bodyMaxBytes: 2048,
      timeoutMs: 15000,
