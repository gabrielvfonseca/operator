...
function runStopExistingLocalApp(params: { fakeLsof?: string; fakePgrep: string }) {
  const root = mkdtempSync(join(tmpdir(), "operator-build-run-mac-test-"));
  tempRoots.push(root);
  ...
  writeFileSync(
    ...
    [
      "#!/usr/bin/env bash",
      "set -euo pipefail",
      'BIN_ABS="/worktree/apps/macos/.build-local/debug/Operator"',
      'BIN=".build-local/debug/Operator"',
      'APP_CWD="/worktree/apps/macos"',
      "kill() {",
      '  printf "%s\\n" "$*" >> "$OPERATOR_TEST_KILL_CALLS"',
      '  touch "$OPERATOR_TEST_KILLED_MARKER"',
      "  return 0",
      "}",
      stopFunction,
  ...
  const result = spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      OPERATOR_TEST_KILLED_MARKER: join(root, "killed"),
      OPERATOR_TEST_KILL_CALLS: killCallsPath,
      OPERATOR_TEST_PGREP_CALLS: pgrepCallsPath,
      OPERATOR_TEST_PGREP_COUNT: join(root, "pgrep-count.txt"),
...
describe("scripts/build-and-run-mac.sh", () => {
  ...
  it("keeps launch logs isolated unless an explicit log path is provided", () => {
    ...
    expect(script).toContain(
      'LOG_PATH="${OPERATOR_MAC_RUN_LOG:-$(mktemp "${TMPDIR:-/tmp}/operator-${PRODUCT}.XXXXXX.log")}"',
    );
    expect(script).toContain('nohup "$BIN_ABS" >"$LOG_PATH" 2>&1 &');
    expect(script).toContain('printf "Started $PRODUCT (PID $PID). Logs: $LOG_PATH\\n"');
    expect(script).not.toContain("/tmp/operator.log");
  });
  ...
  it("stops only the local debug app binary before relaunching", () => {
    ...
    const { killCalls, pgrepCalls, result } = runStopExistingLocalApp({
      fakePgrep: [
        "#!/usr/bin/env bash",
        `printf '%s\\n' "$*" >> "$OPERATOR_TEST_PGREP_CALLS"`,
        'count="$(cat "$OPERATOR_TEST_PGREP_COUNT" 2>/dev/null || echo 0)"',
        'next="$((count + 1))"',
        'printf "%s\\n" "$next" > "$OPERATOR_TEST_PGREP_COUNT"',
        'if [[ "$2" == "/worktree/apps/macos/.build-local/debug/Operator" ]]; then exit 1; fi',
        'if [[ "$2" == ".build-local/debug/Operator" && "$count" == "1" ]]; then echo 321; exit 0; fi',
        "exit 1",
      ].join("\n"),
    });
    ...
    expect(pgrepCalls).toContain("-f /worktree/apps/macos/.build-local/debug/Operator");
    expect(pgrepCalls).toContain("-f .build-local/debug/Operator");
  ...
  it("fails when the scoped local app process survives cleanup", () => {
    const { result } = runStopExistingLocalApp({
      fakePgrep: [
        "#!/usr/bin/env bash",
        'if [[ "$2" == ".build-local/debug/Operator" ]]; then echo 321; exit 0; fi',
        "exit 1",
      ].join("\n"),
    });
