...
describe("scripts/plan-release-workflow-matrix.mjs", () => {
  it("declares every job input for both workflow entry points", () => {
    ...
    expect(definition.env.OPERATOR_DOCKER_E2E_ALLOW_UNRELEASED_CHANGELOG).toBe(
  ...
  it("wires filtered matrices into the reusable live and E2E workflow", () => {
    ...
    expect(liveModels.env.OPERATOR_LIVE_MODELS).toBe("${{ matrix.models || 'modern' }}");
    expect(liveModels.env.OPERATOR_LIVE_MAX_MODELS).toBe("${{ matrix.max_models || '6' }}");
  });
