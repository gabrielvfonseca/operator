...
const CHECK_SCRIPT = "scripts/check-operator-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "operator-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(
      join(packageRoot, "package.json"),
      JSON.stringify({ name: "@gabrielvfonseca/operator", version, ...options.packageJson }),
    );
    ...
    const controlUiFiles =
      options.includeControlUi === false
        ? {}
        : {
            "dist/control-ui/index.html": "<!doctype html><operator-app></operator-app>",
      ...
      options.includeInstallGuard === false
        ...
        : {
            [PACKAGE_INSTALL_GUARD_RELATIVE_PATH]:
              "Operator package preinstall has not completed.\n",
          };
    ...
    const tarball = join(root, "operator.tgz");
...
describe("check-operator-package-tarball", () => {
  it("prints help before touching tarball state", () => {
    const result = spawnSync("node", [CHECK_SCRIPT, "--help"], { encoding: "utf8" });

    expect(result.status, result.stderr).toBe(0);
    expect(result.stdout).toContain(
      "Usage: node scripts/check-operator-package-tarball.mjs [--require-bundled-workspace-deps] <operator.tgz>",
    );
  ...
  it("rejects option-like and extra arguments before tar inspection", () => {
    ...
    expect(unknown.stderr).toContain("Unknown Operator package tarball check option: --tag");
    expect(unknown.stderr).not.toContain("Operator package tarball does not exist");

    const extra = spawnSync("node", [CHECK_SCRIPT, "operator.tgz", "extra"], {
      encoding: "utf8",
    });

    expect(extra.status).not.toBe(0);
    expect(extra.stderr).toContain("Unexpected Operator package tarball check argument: extra");
    expect(extra.stderr).not.toContain("Operator package tarball does not exist");
  });

  it.runIf(process.platform !== "win32")(
    "removes the extract dir when tar extraction fails",
    () => {
      const root = mkdtempSync(join(tmpdir(), "operator-package-tarball-extract-fail-"));
      try {
