...
function runProbe(root: string, env: Record<string, string | undefined> = {}) {
  ...
  childEnv.OPERATOR_ENTRY = path.join(root, "dist", "index.js");
...
function runRuntimeSmoke(root: string, args: string[]) {
  return spawnSync(process.execPath, [runtimeSmokePath, ...args], {
    ...
    env: {
      ...process.env,
      OPERATOR_ENTRY: path.join(root, "dist", "index.js"),
    },
  });
}
...
describe("bundled plugin install/uninstall probe", () => {
  ...
  it("bounds bundled plugin package lifecycle commands", () => {
    ...
    expect(sweep).toContain("OPERATOR_BUNDLED_PLUGIN_SWEEP_COMMAND_TIMEOUT:-300s");
  ...
  it("rejects unsafe bundled plugin runtime limit env values", async () => {
    await expect(
      importRuntimeSmokeWithEnv({
        OPERATOR_BUNDLED_PLUGIN_RUNTIME_READY_MS: String(Number.MAX_SAFE_INTEGER + 1),
      }),
    ).rejects.toThrow("invalid OPERATOR_BUNDLED_PLUGIN_RUNTIME_READY_MS: 9007199254740992");
  });

  it("rejects bundled plugin runtime ports outside the TCP range", async () => {
    const runtimeSmoke = await import(pathToFileURL(runtimeSmokePath).href);
    const env = {
      OPERATOR_BUNDLED_PLUGIN_RUNTIME_PORT_BASE: "65533",
    };
    ...
    expect(() => runtimeSmoke.resolveRuntimeSmokePort(1, 0, env)).toThrow(
      "OPERATOR_BUNDLED_PLUGIN_RUNTIME_PORT_BASE with bundled plugin runtime index 1 and offset 0 must resolve to a TCP port from 1 to 65535. Got: 65536",
    );
  });

  it("caps noisy runtime gateway logs", async () => {
    const runtimeSmoke = await importRuntimeSmokeWithEnv({
      OPERATOR_BUNDLED_PLUGIN_RUNTIME_GATEWAY_LOG_BYTES: "64",
    });
    const root = makePackageRoot();
  ...
  it("rejects loose runtime output limit env values instead of parsing prefixes", async () => {
    await expect(
      importRuntimeSmokeWithEnv({
        OPERATOR_BUNDLED_PLUGIN_RUNTIME_OUTPUT_CHARS: "5chars",
      }),
    ).rejects.toThrow("invalid OPERATOR_BUNDLED_PLUGIN_RUNTIME_OUTPUT_CHARS: 5chars");
  });
  ...
  it("rejects loose runtime log scan byte env values instead of parsing prefixes", async () => {
    await expect(
      importRuntimeSmokeWithEnv({
        OPERATOR_BUNDLED_PLUGIN_RUNTIME_LOG_SCAN_BYTES: "64bytes",
      }),
    ).rejects.toThrow("invalid OPERATOR_BUNDLED_PLUGIN_RUNTIME_LOG_SCAN_BYTES: 64bytes");
  });
  ...
  it.runIf(process.platform !== "win32")("stops runtime gateway process groups", async () => {
    const runtimeSmoke = await importRuntimeSmokeWithEnv({
      OPERATOR_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_GRACE_MS: "50",
  ...
  it("cleans per-call RPC state directories", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        "import fs from 'node:fs';",
        "fs.writeFileSync(process.env.OPERATOR_TEST_RPC_STATE_PATH, process.env.OPERATOR_STATE_DIR);",
    ...
    await expect(
      runtimeSmoke.rpcCall(
        ...
        {
          entrypoint,
          env: { OPERATOR_TEST_RPC_STATE_PATH: statePath },
          port: 19001,
        },
      ),
  ...
  it("allows degraded runtime readiness only for expected channel failures", async () => {
    const runtimeSmoke = await importRuntimeSmokeWithEnv({
      OPERATOR_BUNDLED_PLUGIN_RUNTIME_HTTP_MS: "100",
      OPERATOR_BUNDLED_PLUGIN_RUNTIME_RPC_READY_MS: "50",
    });
    const server = createHttpServer((_request, response) => {
  ...
  it("keeps stalled runtime readiness probes inside the ready deadline", async () => {
    const runtimeSmoke = await importRuntimeSmokeWithEnv({
      OPERATOR_BUNDLED_PLUGIN_RUNTIME_HTTP_MS: "1000",
      OPERATOR_BUNDLED_PLUGIN_RUNTIME_READY_MS: "50",
    });
    const sockets = new Set<Socket>();
    const server = createNetServer((socket) => {
      sockets.add(socket);
  ...
  it("creates runtime smoke state with OPERATOR_HOME at the test home", async () => {
    const runtimeSmoke = await import(pathToFileURL(runtimeSmokePath).href);
  ...
  it("selects packaged installable bundled sources instead of raw dist extension dirs", () => {
    ...
    const result = runProbe(root, {
      OPERATOR_BUNDLED_PLUGIN_SWEEP_IDS: undefined,
    });
  ...
  it("does not select source-only bundled plugins for package-backed sweeps", () => {
    ...
    const result = runProbe(root, {
      OPERATOR_BUNDLED_PLUGIN_SWEEP_IDS: "qa-channel",
    });

    expect(result.status).toBe(1);
    expect(result.stderr).toContain(
      "OPERATOR_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: qa-channel",
    );
  ...
  it("rejects loose packaged plugin list limit env values", () => {
    ...
    const timeout = runProbe(root, {
      OPERATOR_BUNDLED_PLUGIN_LIST_TIMEOUT_MS: "100ms",
    });
    expect(timeout.status).toBe(1);
    expect(timeout.stderr).toContain("invalid OPERATOR_BUNDLED_PLUGIN_LIST_TIMEOUT_MS: 100ms");

    const maxBuffer = runProbe(root, {
      OPERATOR_BUNDLED_PLUGIN_LIST_MAX_BUFFER_BYTES: "64bytes",
    });
    expect(maxBuffer.status).toBe(1);
    expect(maxBuffer.stderr).toContain(
      "invalid OPERATOR_BUNDLED_PLUGIN_LIST_MAX_BUFFER_BYTES: 64bytes",
    );
  });
  ...
  it("rejects loose bundled plugin sweep shard env values", () => {
    ...
    const total = runProbe(root, {
      OPERATOR_BUNDLED_PLUGIN_SWEEP_TOTAL: "2shards",
    });
    expect(total.status).toBe(1);
    expect(total.stderr).toContain("invalid OPERATOR_BUNDLED_PLUGIN_SWEEP_TOTAL: 2shards");

    const index = runProbe(root, {
      OPERATOR_BUNDLED_PLUGIN_SWEEP_INDEX: "0of2",
    });
    expect(index.status).toBe(1);
    expect(index.stderr).toContain("invalid OPERATOR_BUNDLED_PLUGIN_SWEEP_INDEX: 0of2");
  });
  ...
  it("bounds plugin list selection when the CLI hangs", () => {
    ...
    const startedAt = Date.now();
    const result = runProbe(root, {
      OPERATOR_BUNDLED_PLUGIN_LIST_TIMEOUT_MS: "100",
    });
