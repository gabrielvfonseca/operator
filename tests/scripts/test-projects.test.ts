...
async function loadRawVitestConfig(configPath: string): Promise<VitestConfig> {
  ...
  const previousIncludeFile = process.env.OPERATOR_VITEST_INCLUDE_FILE;
  process.argv = [previousArgv[0] ?? "node", previousArgv[1] ?? "vitest"];
  delete process.env.OPERATOR_VITEST_INCLUDE_FILE;
  try {
    ...
    if (previousIncludeFile === undefined) {
      ...
      process.env.OPERATOR_VITEST_INCLUDE_FILE = previousIncludeFile;
    }
  }
}
...
describe("scripts/test-projects changed-target routing", () => {
  ...
  it("keeps the broad changed run available for Vitest wiring edits", () => {
    expect(
      resolveChangedTargetArgs(
        ...
        { env: { OPERATOR_TEST_CHANGED_BROAD: "1" } },
      ),
    ).toBeNull();
  });
  ...
  it("explains changed paths that need explicit broad fallback before skipping", () => {
    expect(formatNoChangedTestTargetLines(["unknown-root-surface.txt"])).toEqual([
      ...
      "[test] run `OPERATOR_TEST_CHANGED_BROAD=1 pnpm test:changed` for broad coverage.",
    ]);
  });
  ...
  it("routes changed ui support files to the ui lane without dead include globs", () => {
    const plans = buildVitestRunPlans(["--changed", "origin/main"], process.cwd(), () => [
      ...
      "ui/tests/helpers/lit-warnings.setup.ts",
    ]);
    ...
    expect(plans).toEqual([
      {
        config: "test/vitest/vitest.ui.config.ts",
        forwardedArgs: [],
        includePatterns: ["ui/tests/app/control-ui-chunking.test.ts"],
        watchMode: false,
      },
    ]);
  });
  ...
  it("keeps explicit non-renderer ui test targets scoped", () => {
    expect(
      buildVitestRunPlans([
        "ui/tests/i18n/test/translate.test.ts",
        "test/scripts/control-ui-i18n.test.ts",
      ]),
    ).toEqual([
      {
      ...
      },
      {
        config: "test/vitest/vitest.ui.config.ts",
        forwardedArgs: [],
        includePatterns: ["ui/tests/i18n/test/translate.test.ts"],
        watchMode: false,
      },
    ]);
  });
  ...
  it("routes control ui e2e tests to the ui e2e lane", () => {
    expect(buildVitestRunPlans(["ui/tests/e2e/chat-flow.e2e.test.ts"])).toEqual([
      {
        config: "test/vitest/vitest.ui-e2e.config.ts",
        forwardedArgs: [],
        includePatterns: ["ui/tests/e2e/chat-flow.e2e.test.ts"],
        watchMode: false,
      },
    ]);

    expect(buildVitestRunPlans(["ui/tests/helpers/control-ui-e2e.ts"])).toEqual([
      {
        config: "test/vitest/vitest.ui-e2e.config.ts",
        forwardedArgs: [],
        includePatterns: null,
    ...
    expect(buildVitestRunPlans(["ui/tests/e2e"])).toEqual([
      {
        config: "test/vitest/vitest.ui-e2e.config.ts",
        forwardedArgs: [],
        includePatterns: ["ui/tests/e2e/**/*.test.ts"],
        watchMode: false,
      },
    ]);

    expect(buildVitestArgs(["ui/tests/e2e"])).toContain("--configLoader");
  });
  ...
  it("preflights targeted UI E2E specs with Playwright browser assets", () => {
    const [spec] = createVitestRunSpecs(["ui/tests/pages/tasks/tasks.e2e.test.ts"], {
      baseEnv: {},
    });
...
describe("scripts/test-projects local heavy-check lock", () => {
  const localCheckEnv = () => ({
    ...process.env,
    OPERATOR_TEST_HEAVY_CHECK_LOCK_HELD: undefined,
    OPERATOR_TEST_PROJECTS_FORCE_LOCK: undefined,
  });
  ...
  it("allows forcing the lock back on", () => {
    expect(
      shouldAcquireLocalHeavyCheckLock(
        ...
        {
          ...localCheckEnv(),
          OPERATOR_TEST_PROJECTS_FORCE_LOCK: "1",
        },
      ),
    ).toBe(true);
  });
...
describe("scripts/test-projects full-suite sharding", () => {
  ...
  beforeAll(async () => {
    ...
    const previous = process.env.OPERATOR_TEST_PROJECTS_LEAF_SHARDS;
    const gatewayServerConfig = "test/vitest/vitest.gateway-server.config.ts";
    process.env.OPERATOR_TEST_PROJECTS_LEAF_SHARDS = "1";
    try {
      ...
      if (previous === undefined) {
        delete process.env.OPERATOR_TEST_PROJECTS_LEAF_SHARDS;
      } else {
        process.env.OPERATOR_TEST_PROJECTS_LEAF_SHARDS = previous;
      }
    }
  });
  ...
  it("keeps explicit parallel overrides ahead of the host-aware profile", () => {
    expect(
      resolveParallelFullSuiteConcurrency(
        61,
        {
          OPERATOR_TEST_PROJECTS_PARALLEL: "3",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
  ...
  it("rejects malformed parallel full-suite overrides", () => {
    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          OPERATOR_TEST_PROJECTS_PARALLEL: "3x",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("OPERATOR_TEST_PROJECTS_PARALLEL must be a positive integer; got: 3x");

    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          OPERATOR_TEST_PROJECTS_PARALLEL: "0",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("OPERATOR_TEST_PROJECTS_PARALLEL must be a positive integer; got: 0");
  });
  ...
  it("rejects malformed conservative worker budget values", () => {
    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          OPERATOR_VITEST_MAX_WORKERS: "1e0",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("OPERATOR_VITEST_MAX_WORKERS must be a positive integer; got: 1e0");

    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          OPERATOR_TEST_WORKERS: "1 worker",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("OPERATOR_TEST_WORKERS must be a positive integer; got: 1 worker");
  });

  it("keeps serial untargeted local runs on leaf project configs", () => {
    const previousParallel = process.env.OPERATOR_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.OPERATOR_TEST_PROJECTS_SERIAL;
    const previousCi = process.env.CI;
    const previousActions = process.env.GITHUB_ACTIONS;
    delete process.env.OPERATOR_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.OPERATOR_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    delete process.env.OPERATOR_TEST_PROJECTS_PARALLEL;
    ...
    try {
      ...
      if (previousParallel === undefined) {
        ...
        process.env.OPERATOR_TEST_PROJECTS_PARALLEL = previousParallel;
      }
      if (previousSerial === undefined) {
        delete process.env.OPERATOR_TEST_PROJECTS_SERIAL;
      } else {
        process.env.OPERATOR_TEST_PROJECTS_SERIAL = previousSerial;
      }
  ...
  it("expands untargeted local runs to leaf project configs by default", () => {
    const previousLeafShards = process.env.OPERATOR_TEST_PROJECTS_LEAF_SHARDS;
    const previousParallel = process.env.OPERATOR_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.OPERATOR_TEST_PROJECTS_SERIAL;
    const previousCi = process.env.CI;
    const previousActions = process.env.GITHUB_ACTIONS;
    const previousVitestMaxWorkers = process.env.OPERATOR_VITEST_MAX_WORKERS;
    const previousTestWorkers = process.env.OPERATOR_TEST_WORKERS;
    delete process.env.OPERATOR_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.OPERATOR_TEST_PROJECTS_PARALLEL;
    delete process.env.OPERATOR_TEST_PROJECTS_SERIAL;
    delete process.env.CI;
    delete process.env.GITHUB_ACTIONS;
    delete process.env.OPERATOR_VITEST_MAX_WORKERS;
    delete process.env.OPERATOR_TEST_WORKERS;
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previousLeafShards === undefined) {
        ...
        process.env.OPERATOR_TEST_PROJECTS_LEAF_SHARDS = previousLeafShards;
      }
      if (previousParallel === undefined) {
        delete process.env.OPERATOR_TEST_PROJECTS_PARALLEL;
      } else {
      ...
      if (previousVitestMaxWorkers === undefined) {
        ...
        process.env.OPERATOR_VITEST_MAX_WORKERS = previousVitestMaxWorkers;
      }
      if (previousTestWorkers === undefined) {
        delete process.env.OPERATOR_TEST_WORKERS;
      } else {
        process.env.OPERATOR_TEST_WORKERS = previousTestWorkers;
      }
    }
  });
  ...
  it("can skip the aggregate extension shard when CI runs dedicated extension shards", () => {
    const previous = process.env.OPERATOR_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    const previousParallel = process.env.OPERATOR_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.OPERATOR_TEST_PROJECTS_SERIAL;
    const previousCi = process.env.CI;
    delete process.env.OPERATOR_TEST_PROJECTS_PARALLEL;
    process.env.OPERATOR_TEST_PROJECTS_SERIAL = "1";
    process.env.CI = "true";
    process.env.OPERATOR_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previous === undefined) {
        ...
        process.env.OPERATOR_TEST_SKIP_FULL_EXTENSIONS_SHARD = previous;
      }
      if (previousParallel === undefined) {
        delete process.env.OPERATOR_TEST_PROJECTS_PARALLEL;
      } else {
  ...
  it("skips extension project configs when leaf sharding and the aggregate extension shard is disabled", () => {
    const previousLeafShards = process.env.OPERATOR_TEST_PROJECTS_LEAF_SHARDS;
    const previousSkipExtensions = process.env.OPERATOR_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    process.env.OPERATOR_TEST_PROJECTS_LEAF_SHARDS = "1";
    process.env.OPERATOR_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previousSkipExtensions === undefined) {
        ...
        process.env.OPERATOR_TEST_SKIP_FULL_EXTENSIONS_SHARD = previousSkipExtensions;
      }
    }
  });
  ...
  it("expands full-suite shards before running them in parallel", () => {
    ...
    const previousParallel = process.env.OPERATOR_TEST_PROJECTS_PARALLEL;
    delete process.env.OPERATOR_TEST_PROJECTS_LEAF_SHARDS;
    process.env.OPERATOR_TEST_PROJECTS_PARALLEL = "6";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
  ...
  it("rejects malformed full-suite expansion parallel overrides", () => {
    ...
    const previousParallel = process.env.OPERATOR_TEST_PROJECTS_PARALLEL;
    delete process.env.OPERATOR_TEST_PROJECTS_LEAF_SHARDS;
    process.env.OPERATOR_TEST_PROJECTS_PARALLEL = "6x";
    try {
      expect(() => buildFullSuiteVitestRunPlans([], process.cwd())).toThrow(
        "OPERATOR_TEST_PROJECTS_PARALLEL must be a positive integer; got: 6x",
      );
    } finally {
      if (previousLeafShards === undefined) {
        delete process.env.OPERATOR_TEST_PROJECTS_LEAF_SHARDS;
...
describe("scripts/test-projects parallel cache paths", () => {
  it("assigns isolated Vitest fs-module cache paths per parallel shard", () => {
    ...
    expect(specs.map((spec) => spec.env)).toEqual([
      {
        OPERATOR_VITEST_FS_MODULE_CACHE_PATH: path.join(
          "/repo",
          "node_modules",
  ...
  it("keeps an explicit global cache path", () => {
    const [spec] = applyParallelVitestCachePaths(
      [{ config: "test/vitest/vitest.gateway.config.ts", env: {}, pnpmArgs: [] }],
      { cwd: "/repo", env: { OPERATOR_VITEST_FS_MODULE_CACHE_PATH: "/tmp/cache" } },
    );

    expect(spec?.env.OPERATOR_VITEST_FS_MODULE_CACHE_PATH).toBeUndefined();
  });
});
...
describe("scripts/test-projects Vitest stall watchdog", () => {
  it("adds default no-output watchdog settings to non-watch specs", () => {
    ...
    expect(spec?.env.OPERATOR_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_TIMEOUT_MS,
    );
    expect(spec?.env.OPERATOR_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_HEARTBEAT_MS,
    );
  });
  ...
  it("extends the no-output watchdog for slow silent full-suite configs", () => {
    ...
    expect(specs[0]?.env.OPERATOR_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("2400000");
    expect(specs[1]?.env.OPERATOR_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("2400000");
    expect(specs[2]?.env.OPERATOR_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("2400000");
    expect(specs[3]?.env.OPERATOR_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("2400000");
    expect(specs[4]?.env.OPERATOR_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_TIMEOUT_MS,
    );
  });
  ...
  it("keeps explicit watchdog settings and watch mode untouched", () => {
    const specs = applyDefaultVitestNoOutputTimeout(
      [
        ...
        },
        {
          ...
          env: {
            OPERATOR_VITEST_NO_OUTPUT_HEARTBEAT_MS: "25000",
            OPERATOR_VITEST_NO_OUTPUT_TIMEOUT_MS: "0",
            PATH: "/usr/bin",
          },
          includeFilePath: null,
          includePatterns: null,
    ...
    expect(specs[0]?.env.OPERATOR_VITEST_NO_OUTPUT_TIMEOUT_MS).toBeUndefined();
    expect(specs[0]?.env.OPERATOR_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBeUndefined();
    expect(specs[1]?.env.OPERATOR_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("0");
    expect(specs[1]?.env.OPERATOR_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBe("25000");
  });
  ...
  it("allows changed checks to disable automatic silent-run retries", () => {
    ...
    expect(shouldRetryVitestNoOutputTimeout({ OPERATOR_VITEST_NO_OUTPUT_RETRY: "1" })).toBe(true);
    expect(shouldRetryVitestNoOutputTimeout({ OPERATOR_VITEST_NO_OUTPUT_RETRY: "0" })).toBe(false);
    expect(shouldRetryVitestNoOutputTimeout({ OPERATOR_VITEST_NO_OUTPUT_RETRY: "false" })).toBe(
      false,
    );
  });
});
...
describe("scripts/test-projects Vitest cache isolation", () => {
  it("assigns isolated fs-module caches to multi-spec non-watch runs", () => {
    ...
    expect(specs.map((spec) => spec.env.OPERATOR_VITEST_FS_MODULE_CACHE_PATH)).toEqual([
      path.join(
        "/repo",
        "node_modules",
        ".experimental-vitest-cache",
