...
describe("Mantis Telegram Desktop proof workflow", () => {
  ...
  it("releases Telegram Desktop proof leases left by interrupted agents", () => {
    ...
    expect(codexStep.env?.OPERATOR_QA_CREDENTIAL_OWNER_ID).toContain(
      "mantis-telegram-desktop-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    expect(workflowStep("Prepare Codex user").run).toContain("OPERATOR_QA_CREDENTIAL_OWNER_ID");
    ...
    expect(cleanupStep.env?.OPERATOR_QA_CONVEX_SECRET_CI).toContain(
      "secrets.OPERATOR_QA_CONVEX_SECRET_CI",
    );
    expect(cleanupStep.env?.OPERATOR_QA_CONVEX_SITE_URL).toContain(
      "secrets.OPERATOR_QA_CONVEX_SITE_URL",
    );
  ...
  it("keeps Telegram Desktop proof credentials out of the generic qa-lab API", () => {
    ...
    expect(readFileSync(TELEGRAM_PROOF_SKILL, "utf8")).toContain(
      "OPERATOR_TELEGRAM_USER_PROOF_CMD",
    );
  ...
  it("installs local proof tools before the Codex agent runs", () => {
    ...
    expect(agent.env?.OPERATOR_TELEGRAM_USER_DRIVER_SCRIPT).toBe(
      "${{ github.workspace }}/scripts/e2e/telegram-user-driver.py",
    );
    expect(agent.env?.OPERATOR_TELEGRAM_USER_PROOF_CMD).toBe(
      "/usr/local/bin/operator-telegram-user-crabbox-proof",
    );
    expect(agent.env?.OPERATOR_TELEGRAM_USER_CRABBOX_BIN).toBe("/usr/local/bin/crabbox");
    expect(agent.env?.CRABBOX_COORDINATOR).toContain(
      "secrets.CRABBOX_COORDINATOR || secrets.OPERATOR_QA_MANTIS_CRABBOX_COORDINATOR",
    );
    expect(agent.env?.CRABBOX_COORDINATOR_TOKEN).toContain(
      "secrets.CRABBOX_COORDINATOR_TOKEN || secrets.OPERATOR_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN",
    );

    const prepare = workflowStep("Prepare Codex user");
    expect(prepare.run).toContain(
      "OPERATOR_TELEGRAM_USER_CRABBOX_BIN OPERATOR_TELEGRAM_USER_CRABBOX_PROVIDER OPERATOR_TELEGRAM_USER_DRIVER_SCRIPT OPERATOR_TELEGRAM_USER_PROOF_CMD",
    );
    expect(prepare.run).toContain("MANTIS_CANDIDATE_TRUST");

    const prompt = readFileSync(PROMPT, "utf8");
    expect(prompt).toContain("$OPERATOR_TELEGRAM_USER_PROOF_CMD");
