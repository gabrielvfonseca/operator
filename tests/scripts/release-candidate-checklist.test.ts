...
async function withGithubApiTimeoutEnv<T>(value: string, fn: () => Promise<T>): Promise<T> {
  const previous = process.env.OPERATOR_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS;
  process.env.OPERATOR_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS = value;
  try {
    return await fn();
  } finally {
    if (previous === undefined) {
      delete process.env.OPERATOR_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS;
    } else {
      process.env.OPERATOR_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS = previous;
    }
  }
}
...
describe("release candidate checklist", () => {
  ...
  it("passes scoped environment overrides to release child commands", () => {
    const output = run(
      process.execPath,
      ["-e", "process.stdout.write(process.env.OPERATOR_RELEASE_TEST_VALUE ?? '')"],
      { capture: true, env: { OPERATOR_RELEASE_TEST_VALUE: "passed" } },
    );
  ...
  it("extracts a workflow run id from gh dispatch output", () => {
    expect(
      parseRunIdFromDispatchOutput(
        "https://github.com/gabrielvfonseca/operator/actions/runs/25922042055\n",
      ),
    ).toBe("25922042055");
  });
    ...
    async (raw) => {
      ...
      await withGithubApiTimeoutEnv(raw, async () => {
        await expect(
          ...
          "OPERATOR_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS must be a positive integer",
        );
      });
