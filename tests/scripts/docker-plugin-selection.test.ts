...
describe("Docker plugin selection", () => {
  ...
  it("fails closed for unknown, invalid, and ambiguous ids", () => {
    ...
    for (const [selection, message] of [
      ["missing-plugin", "unknown OPERATOR_EXTENSIONS plugin id: missing-plugin"],
      ["../invalid", "invalid OPERATOR_EXTENSIONS plugin id: ../invalid"],
      ["shared", "ambiguous OPERATOR_EXTENSIONS plugin id: shared"],
    ] as const) {
