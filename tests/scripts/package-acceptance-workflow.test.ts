...
const LIVE_MEDIA_RUNNER_IMAGE = "ghcr.io/operator/operator-live-media-runner:ubuntu-24.04";
...
const RELEASE_CHECKS_WORKFLOW = ".github/workflows/operator-release-checks.yml";
const RELEASE_TELEGRAM_QA_WORKFLOW = ".github/workflows/operator-release-telegram-qa.yml";
const RELEASE_PUBLISH_WORKFLOW = ".github/workflows/operator-release-publish.yml";
const OPERATOR_NPM_RELEASE_WORKFLOW = ".github/workflows/operator-npm-release.yml";
...
const STABLE_MAIN_CLOSEOUT_WORKFLOW = ".github/workflows/operator-stable-main-closeout.yml";
...
const SCHEDULED_LIVE_CHECKS_WORKFLOW = ".github/workflows/operator-scheduled-live-checks.yml";
const CI_HYDRATE_LIVE_AUTH_SCRIPT = "scripts/ci-hydrate-live-auth.sh";
const VERIFY_PROVIDER_SECRETS_SCRIPT =
  ".agents/skills/release-operator-ci/scripts/verify-provider-secrets.mjs";
...
function runNpmTelegramInputValidation(overrides: Record<string, string>) {
  ...
  return spawnSync("bash", ["-c", script], {
    encoding: "utf8",
    env: {
      OPERATOR_QA_CONVEX_SECRET_CI: "test-secret",
      OPERATOR_QA_CONVEX_SITE_URL: "https://example.invalid",
...
function runOperatorNpmTrustedRefGuard(overrides: Record<string, string>) {
  const job = workflowJob(OPERATOR_NPM_RELEASE_WORKFLOW, "validate_publish_request");
  const script = workflowStep(job, "Require trusted workflow ref for publish").run;
  if (!script) {
    throw new Error("Expected Operator npm trusted ref guard");
  }
  const binDir = tempDirs.make("operator-npm-trusted-ref-");
...
function runReleaseChecksSummary(params: {
  ...
  const workdir = tempDirs.make("operator-release-check-status-");
  return spawnSync("bash", ["-c", script], {
    cwd: workdir,
    encoding: "utf8",
    env: {
...
describe("package acceptance workflow", () => {
  it("requires selected plugin names or complete immutable evidence for broad publication", () => {
    const selected = runReleasePublishInputValidation({
      ...
      PLUGINS: "@gabrielvfonseca/meta",
      PLUGIN_PUBLISH_SCOPE: "selected",
      PREFLIGHT_RUN_ID: "",
      PUBLISH_OPERATOR_NPM: "false",
    });
    ...
    expect(runReleasePublishInputValidation({ PUBLISH_OPERATOR_NPM: "false" }).status).toBe(0);

    const invalidResumeRun = runReleasePublishInputValidation({
      OPERATOR_NPM_RESUME_RUN_ID: "not-a-run-id",
    });
    expect(invalidResumeRun.status).toBe(1);
    expect(invalidResumeRun.stderr).toContain(
      "operator_npm_resume_run_id must be a positive GitHub Actions run id",
    );
  });
  ...
  it("allows protected SHA-pinned tooling tags through the core npm publish guard", () => {
    ...
    const valid = runOperatorNpmTrustedRefGuard({
      WORKFLOW_REF: protectedRef,
      WORKFLOW_SHA: workflowSha,
    });
    expect(valid.status, valid.stderr).toBe(0);

    const mismatchedName = runOperatorNpmTrustedRefGuard({
      WORKFLOW_REF: `refs/tags/release-publish/${"b".repeat(12)}-123`,
      WORKFLOW_SHA: workflowSha,
    });
    expect(mismatchedName.status).toBe(1);
    expect(mismatchedName.stderr).toContain(
      "SHA-pinned release-publish tag does not match the Operator npm workflow SHA",
    );

    const unreachable = runOperatorNpmTrustedRefGuard({
    ...
    expect(unreachable.status).toBe(1);
    expect(unreachable.stderr).toContain(
      "SHA-pinned Operator npm workflow revision is not reachable from current main",
    );
  });
  ...
  it("resolves broad release evidence and exact-binds every publish child", () => {
    ...
    for (const stepName of [
      ...
      expect(workflowStep(publishJob, stepName).if).toContain("inputs.publish_operator_npm");
    }
    ...
    expectTextToIncludeAll(publishOrchestration.run, [
      ...
      'wait_for_run_background operator-npm-release.yml "${operator_npm_run_id}" "${PARENT_WORKFLOW_SHA}"',
  ...
  it("verifies immutable postpublish evidence before stable closeout reads it", () => {
    ...
    expect(workflow).toContain("verify_checksum_manifest Operator-Android-SHA256SUMS.txt");
    expect(workflow).toContain("verify_checksum_manifest OperatorCompanion-SHA256SUMS.txt");
    ...
    expect(workflow).toContain(
      'awk -v asset="operator-${release_version}-stable-main-closeout.json"',
    );
    ...
    expect(workflow).toContain(
      'release_manifest_asset="operator-${evidence_version}-release-manifest.json"',
    );
    ...
    expect(evidenceStep.run).toContain(
      'manifest_asset="operator-${evidence_version}-release-manifest.json"',
    );
  ...
  it("keeps Crabbox hydration compatible with local Actions replay", () => {
    ...
    expect(hydrateWindowsPnpm.run).toContain('"@gabrielvfonseca/operator",');
    expect(hydrateWindowsPnpm.run).toContain(
    ...
    expect(workflowText).toContain("OPERATOR_CRABBOX_HYDRATE_DOWNLOAD_TIMEOUT_SECONDS:-300");
    expect(workflowText).toContain("OPERATOR_CRABBOX_HYDRATE_DOWNLOAD_RETRIES:-3");
  ...
  it("resolves candidate package sources before reusing Docker E2E lanes", () => {
    ...
    expect(workflow).toContain("scripts/resolve-operator-package-candidate.mjs");
    ...
    expect(workflow).toContain('node scripts/check-operator-package-tarball.mjs "$package"');
  ...
  it("keeps ref packaging independent of workflow-checkout dependencies", () => {
    ...
    expect(resolveJob).toContain("scripts/resolve-operator-package-candidate.mjs");
  ...
  it("offers bounded product profiles and can run Telegram against the resolved artifact", () => {
    ...
    expect(workflow).toContain(
      "package_label: operator@${{ needs.resolve_package.outputs.package_version }}",
    );
  ...
  it("requires full release child workflows to run at the parent workflow SHA", () => {
    ...
    expect(releaseChecksWorkflow).toContain(
      'codex_plugin_spec="npm:@gabrielvfonseca/codex@${BASH_REMATCH[1]}"',
    );
    expect(releaseChecksWorkflow).toContain(
    ...
    expect(releaseChecksWorkflow).toContain(
      "package_spec: ${{ needs.resolve_target.outputs.package_acceptance_package_spec || needs.resolve_target.outputs.release_package_spec || 'operator@beta' }}",
    );
  });
  ...
  it("adopts exact full-release child runs without retrying ambiguous dispatch posts", () => {
    ...
    for (const message of [
      ...
      "unexpected EOF",
      'Post "https://api.github.com/repos/operator/operator/actions/workflows/ci.yml/dispatches": EOF',
      "EOF",
      "ETIMEDOUT",
      "ECONNRESET",
      "EAI_AGAIN",
    ...
    expect(readFileSync(RELEASE_CHECKS_WORKFLOW, "utf8")).toContain(
      "format('Operator Release Checks {0}', inputs.dispatch_id)",
    );
...
describe("package artifact reuse", () => {
  ...
  it("lets reusable Docker E2E consume an already resolved package artifact", () => {
    ...
    expect(workflow).toContain("OPERATOR_DOCKER_E2E_SELECTED_SHA:");
    expect(workflow).toContain(
      "OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC: ${{ inputs.published_upgrade_survivor_baseline }}",
    );
    expect(workflow).toContain(
      "OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ matrix.group.published_upgrade_survivor_baselines || inputs.published_upgrade_survivor_baselines }}",
    );
    expect(workflow).toContain(
      "OPERATOR_UPGRADE_SURVIVOR_SCENARIOS: ${{ inputs.published_upgrade_survivor_scenarios }}",
    );
    expect(workflow).toContain("OPERATOR_UPGRADE_SURVIVOR_TARGET_ROOT: ${{ github.workspace }}");
    expect(workflow).toContain(
      "OPERATOR_ALLOW_FROZEN_TARGET_SCENARIO_OMISSIONS: ${{ inputs.allow_frozen_target_scenario_omissions && '1' || '0' }}",
    );
    expect(workflow).toContain("Download current-run Operator Docker E2E package");
    expect(workflow).toContain("Download previous-run Operator Docker E2E package");
    ...
    expect(workflow).toContain("OPERATOR_DOCKER_E2E_REPO_ROOT:");
    ...
    expect(workflowStep(prepareDockerImage, "Plan Docker E2E images").env).toEqual({
      ...
      OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC: "${{ inputs.published_upgrade_survivor_baseline }}",
      OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPECS:
    ...
    expect(workflow).toContain(
      "OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ inputs.published_upgrade_survivor_baselines }}",
    );
    ...
    expect(scheduler).toContain(
      "published_upgrade_survivor_baseline=${shellQuote(env.OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC)}",
    );
    expect(scheduler).toContain(
      "published_upgrade_survivor_baselines=${shellQuote(env.OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPECS)}",
    );
    expect(scheduler).toContain(
      '["OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC", baseEnv.OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC]',
    );
    expect(scheduler).toContain('["OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPECS",');
    expect(scheduler).toContain('["OPERATOR_UPGRADE_SURVIVOR_SCENARIOS",');
    expect(packageJson).toContain("OPERATOR_UPGRADE_SURVIVOR_PUBLISHED_BASELINE=1");
    expect(packageJson).toContain("test:docker:update-restart-auth");
    expect(packageJson).toContain("OPERATOR_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE=auto-auth");
    expect(publishedUpgradeSurvivor).toContain("validate_baseline_package_spec");
    expect(publishedUpgradeSurvivor).toContain("OPERATOR_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE");
    ...
    expect(publishedUpgradeSurvivor).toContain(
      "env -u OPERATOR_GATEWAY_TOKEN -u OPERATOR_GATEWAY_PASSWORD operator",
    );
    expect(publishedUpgradeSurvivor).toContain("phase prepare-update-restart-probe");
    expect(publishedUpgradeSurvivor).toContain("operator@(alpha|beta|latest|");
    ...
    expect(publishedUpgradeSurvivor).toContain('"id": "opik-operator"');
  ...
  it("bounds shared Docker image pulls so package acceptance cannot stall forever", () => {
    ...
    expect(pullHelper).toContain("OPERATOR_DOCKER_PULL_ATTEMPTS");
    expect(pullHelper).toContain("OPERATOR_DOCKER_PULL_TIMEOUT_SECONDS");
    expect(pullHelper).toContain('timeout_seconds="${OPERATOR_DOCKER_PULL_TIMEOUT_SECONDS:-180}"');
    expect(pullHelper).toContain(
      'retry_delay_seconds="${OPERATOR_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"',
    );
    expect(pullHelper).toContain('source "$SCRIPT_DIR/lib/host-timeout.sh"');
    expect(pullHelper).toContain("operator_host_timeout_bin");
    ...
    expect(dockerE2ePlanAction).not.toContain('docker pull "${OPERATOR_DOCKER_E2E_');
  });
  ...
  it("shards broad native live tests instead of one serial live-all job", () => {
    ...
    expect(workflow).toContain("OPERATOR_LIVE_COMMAND: ${{ matrix.command }}");
    ...
    expect(workflow).toContain("OPERATOR_LIVE_GATEWAY_PROVIDERS=deepseek,fireworks");
    expect(workflow).toContain("OPERATOR_LIVE_GATEWAY_PROVIDERS=opencode-go,openrouter");
    expect(workflow).toContain("OPERATOR_LIVE_GATEWAY_PROVIDERS=xai,zai");
    expect(workflow).toContain("inputs.live_suite_filter == matrix.suite_group");
    expect(workflow).toContain("OPERATOR_LIVE_CLI_BACKEND_MODEL=claude-cli/claude-sonnet-4-6");
    expect(workflow).toContain("OPERATOR_LIVE_CLI_BACKEND_AUTH=api-key");
    expect(workflow).not.toContain("OPERATOR_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG=1");
    expect(workflow).not.toContain('service_tier=\\"fast\\"');
    expect(workflow).not.toContain("OPERATOR_LIVE_CLI_BACKEND_ARGS=");
    expect(workflow).not.toContain("OPERATOR_LIVE_CLI_BACKEND_RESUME_ARGS=");
    expect(workflow).not.toContain(
      'OPERATOR_LIVE_CLI_BACKEND_ARGS=["exec","--json","--color","never","--sandbox","danger-full-access","--skip-git-repo-check"]',
    );
    ...
    expect(workflow).toContain(
      "command: OPERATOR_LIVE_CODEX_HARNESS=1 OPERATOR_LIVE_CODEX_HARNESS_AUTH=api-key node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-core",
    );
    expect(workflow).toContain(
      "command: OPERATOR_LIVE_CODEX_HARNESS=1 OPERATOR_LIVE_CODEX_HARNESS_AUTH=api-key node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-backends",
    );
    expect(workflow).toContain("suite_id: native-live-src-infra");
    expect(workflow).toContain(
      "command: OPERATOR_LIVE_APNS_REACHABILITY=1 node .release-harness/scripts/test-live-shard.mjs native-live-src-infra",
    );
    ...
    expect(workflow).toContain("OPERATOR_LIVE_GATEWAY_MODELS=anthropic/claude-opus-4-8");
    ...
    expect(workflow).toContain(
      "command: OPERATOR_LIVE_GATEWAY_THINKING=off OPERATOR_LIVE_GATEWAY_PROVIDERS=openai OPERATOR_LIVE_GATEWAY_MODELS=openai/gpt-5.6-luna OPERATOR_LIVE_GATEWAY_STEP_TIMEOUT_MS=180000 OPERATOR_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    );
    expect(workflow).toContain(
      "OPERATOR_LIVE_GATEWAY_MODELS=google/gemini-3.1-pro-preview node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-profiles",
    );
    expect(workflow).toContain(
      "OPERATOR_LIVE_GATEWAY_MODELS=minimax/MiniMax-M2.7,minimax-portal/MiniMax-M2.7 OPERATOR_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).not.toContain(
      "OPERATOR_LIVE_GATEWAY_PROVIDERS=deepseek,opencode-go,openrouter,xai,zai",
    );
    expect(workflow).toContain("suite_id: live-gateway-anthropic-docker");
    expect(workflow).toContain("OPERATOR_LIVE_GATEWAY_MAX_MODELS=2");
    expect(workflow).toContain(
      "OPERATOR_LIVE_GATEWAY_THINKING=off OPERATOR_LIVE_GATEWAY_PROVIDERS=openai OPERATOR_LIVE_GATEWAY_MODELS=openai/gpt-5.6-luna OPERATOR_LIVE_GATEWAY_MAX_MODELS=1 OPERATOR_LIVE_GATEWAY_STEP_TIMEOUT_MS=90000 OPERATOR_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    );
    expect(workflow).toContain(
      "OPERATOR_LIVE_GATEWAY_MODELS=anthropic/claude-sonnet-4-6,anthropic/claude-haiku-4-5 OPERATOR_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toContain("OPERATOR_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000");
    ...
    expect(workflow).toContain("OPERATOR_LIVE_SUITE_ADVISORY: ${{ matrix.advisory }}");
    expect(workflow).toContain("Advisory live suite failed with exit code");
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?OPERATOR_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}[\s\S]*?shell: bash[\s\S]*?Advisory live suite failed with exit code/u,
    );
    expect(workflow).toMatch(
      /suite_id: live-gateway-advisory-docker-deepseek-fireworks[\s\S]*?advisory: true/u,
    );
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?OPERATOR_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}/u,
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).toContain("OPERATOR_LIVE_VIDEO_GENERATION_PROVIDERS=google,minimax");
    expect(workflow).toContain("OPERATOR_LIVE_VIDEO_GENERATION_PROVIDERS=openai,openrouter,xai");
    expect(workflow).toContain(
    ...
    expect(retryHelper).toContain("OPERATOR_LIVE_COMMAND_ATTEMPTS:-2");
    ...
    expect(retryHelper).toContain("OPERATOR_LIVE_COMMAND_RATE_LIMIT_RETRY_DELAY_SECONDS:-60");
  ...
  it("pins DeepSeek live profiles to both current V4 model refs", () => {
    ...
    expect(deepSeek).toMatchObject({
      advisory: true,
      command:
        "OPERATOR_LIVE_GATEWAY_PROVIDERS=deepseek OPERATOR_LIVE_GATEWAY_MODELS=deepseek/deepseek-v4-flash,deepseek/deepseek-v4-pro node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-profiles",
      profiles: "full",
    });
    expect(openCodeGo.command).toContain(
      "OPERATOR_LIVE_GATEWAY_MODELS=opencode-go/deepseek-v4-flash,opencode-go/deepseek-v4-pro",
    );
  });
  ...
  it("pins OpenCode Go MiMo live profiles to both current V2.5 model refs", () => {
    ...
    expect(mimo).toMatchObject({
      advisory: true,
      command:
        "OPERATOR_LIVE_GATEWAY_PROVIDERS=opencode-go OPERATOR_LIVE_GATEWAY_MODELS=opencode-go/mimo-v2.5,opencode-go/mimo-v2.5-pro node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-profiles",
      profiles: "full",
  ...
  it("runs the fresh OpenAI API-key default without hard-coding a model filter", () => {
    ...
    expect(openaiDefault.command).toContain("OPERATOR_LIVE_GATEWAY_OPENAI_API_DEFAULT=1");
    expect(openaiDefault.command).toContain("OPERATOR_LIVE_GATEWAY_PROVIDERS=openai");
    expect(openaiDefault.command).not.toContain("OPERATOR_LIVE_GATEWAY_MODELS=");
  });
  ...
  it("runs Docker live harnesses from trusted helper scripts", () => {
    ...
    expect(workflow).toContain(
      'run: OPERATOR_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-models-docker.sh',
    );
    expect(workflow).toContain(
      "command: OPERATOR_LIVE_GATEWAY_THINKING=off OPERATOR_LIVE_GATEWAY_PROVIDERS=openai OPERATOR_LIVE_GATEWAY_MODELS=openai/gpt-5.6-luna OPERATOR_LIVE_GATEWAY_MAX_MODELS=1",
    );
    expect(workflow).toContain(
      "command: OPERATOR_LIVE_GATEWAY_PROVIDERS=minimax,minimax-portal OPERATOR_LIVE_GATEWAY_MODELS=minimax/MiniMax-M2.7,minimax-portal/MiniMax-M2.7 OPERATOR_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toContain(
      'command: OPERATOR_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-cli-backend-docker.sh',
    );
    expect(workflow).toContain(
      'command: OPERATOR_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-acp-bind-docker.sh',
    );
    expect(workflow).toContain(
      'command: OPERATOR_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-codex-harness-docker.sh',
    );
    ...
    expect(codexCompatibility).toMatchObject({
      id: "codex_compat",
      env: {
        OPERATOR_FROZEN_CODEX_SUITE_ID: "${{ matrix.suite_id }}",
        OPERATOR_FROZEN_TARGET_ROOT: "${{ github.workspace }}",
        OPERATOR_SELECTED_SHA: "${{ needs.validate_selected_ref.outputs.selected_sha }}",
        OPERATOR_WORKFLOW_SHA: "${{ needs.validate_selected_ref.outputs.workflow_sha }}",
      },
    ...
    for (const [model, thinking] of [
      ...
      expect(workflow).toContain(
        `OPERATOR_LIVE_CODEX_HARNESS_TARGETS=openai/gpt-5.6-${model}=${thinking}`,
      );
    }
    ...
    expect(workflow).toContain(
      'command: OPERATOR_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 20m bash .release-harness/scripts/test-live-subagent-announce-docker.sh',
    );
    ...
    expect(scenarios).toContain("process.env.OPERATOR_DOCKER_E2E_REPO_ROOT");
    expect(scenarios).toContain(
      'harness="\\${OPERATOR_DOCKER_E2E_TRUSTED_HARNESS_DIR:-${LIVE_DOCKER_DEFAULT_HARNESS_DIR}}"',
    );
    ...
    expect(liveDockerAuth).toContain("operator_live_init_docker_run_args()");
    expect(liveDockerAuth).toContain("operator_live_stage_profile_into_home()");
    expect(liveDockerAuth).toContain("operator_live_chown_bind_dirs_for_container_user()");
    expect(liveDockerAuth).toContain("operator_live_uses_managed_bind_dirs()");
    expect(liveDockerAuth).toContain('operator_live_truthy "${OPERATOR_TESTBOX:-}"');
    expect(liveDockerAuth).toContain('[[ -n "${OPERATOR_DOCKER_CACHE_HOME_DIR:-}" ]]');
    expect(liveDockerAuth).toContain(
      'timeout_value="${2:-${OPERATOR_LIVE_DOCKER_RUN_TIMEOUT:-2700s}}"',
    );
    ...
    expect(harness).toContain(
      '-e OPERATOR_LIVE_DOCKER_SCRIPTS_DIR="${DOCKER_TRUSTED_HARNESS_CONTAINER_DIR}/scripts"',
    );
    ...
    for (const script of [harness, ...sharedLiveScripts]) {
      ...
      expect(script).toContain("operator_live_init_docker_run_args DOCKER_RUN_ARGS");
      expect(script).toContain("operator_live_prepare_bind_dir_for_container_user");
    ...
    for (const script of [
      ...
      expect(script).not.toContain('timeout --kill-after=30s "${OPERATOR_LIVE_');
    }
    expect(readFileSync("scripts/test-live-models-docker.sh", "utf8")).toContain(
      "OPERATOR_LIVE_MODELS_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-gateway-models-docker.sh", "utf8")).toContain(
      "OPERATOR_LIVE_GATEWAY_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      "OPERATOR_LIVE_CLI_BACKEND_DOCKER_RUN_TIMEOUT:-2700s",
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      'CLI_SETUP_TIMEOUT_SECONDS="$(operator_live_read_positive_int_env OPERATOR_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS 180)"',
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      'timeout_value="${OPERATOR_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:?missing live CLI backend setup timeout seconds}s"',
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      "OPERATOR_LIVE_ACP_BIND_DOCKER_RUN_TIMEOUT:-2700s",
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      'ACP_SETUP_TIMEOUT_SECONDS="$(operator_live_read_positive_int_env OPERATOR_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS 180)"',
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      'timeout_value="${OPERATOR_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS:?missing live ACP bind setup timeout seconds}s"',
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      '-e OPERATOR_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS="$ACP_SETUP_TIMEOUT_SECONDS"',
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      '-e OPERATOR_LIVE_ACP_BIND_REQUIRE_CRON="${OPERATOR_LIVE_ACP_BIND_REQUIRE_CRON:-}"',
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
    ...
    expect(acpBindScript).toContain(
      "OPERATOR_LIVE_ACP_BIND_CLAUDE_AUTH must be one of: auto, api-key, subscription.",
    );
    expect(acpBindScript).toContain(
    ...
    expect(workflow.match(/OPERATOR_LIVE_ACP_BIND_CLAUDE_AUTH=subscription/g)).toHaveLength(2);
    expect(workflow.match(/OPERATOR_LIVE_ACP_BIND_CLAUDE_AUTH=api-key/g)).toHaveLength(2);
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      "OPERATOR_LIVE_CODEX_HARNESS_DOCKER_RUN_TIMEOUT:-$((2100 * CODEX_HARNESS_TARGET_COUNT))s",
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      'CODEX_HARNESS_SETUP_TIMEOUT_SECONDS="$(operator_live_read_positive_int_env OPERATOR_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS 180)"',
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      'timeout_value="${OPERATOR_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS:?missing live Codex harness setup timeout seconds}s"',
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      '-e OPERATOR_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS="$CODEX_HARNESS_SETUP_TIMEOUT_SECONDS"',
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      'echo "timeout command not found; cannot bound live Codex harness setup after ${timeout_value}"',
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      'run_setup_command npm install -g "$OPERATOR_LIVE_CODEX_CLI_PACKAGE_SPEC"',
    );
    expect(readFileSync("scripts/test-live-subagent-announce-docker.sh", "utf8")).toContain(
      "OPERATOR_LIVE_SUBAGENT_DOCKER_RUN_TIMEOUT:-1200s",
    );
    expect(build).toContain('ROOT_DIR="${OPERATOR_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"');
    ...
    expect(build).toContain('LIVE_IMAGE_PULL_ATTEMPTS="${OPERATOR_LIVE_DOCKER_PULL_ATTEMPTS:-3}"');
    ...
    expect(stage).toContain(
      'local scripts_dir="${OPERATOR_LIVE_DOCKER_SCRIPTS_DIR:-/src/scripts}"',
    );
  ...
  it("fails Droid ACP Docker live proof when Factory auth is missing", () => {
    ...
    expect(script).toContain("operator_live_acp_bind_load_factory_api_key_from_profile");
    expect(script).not.toContain('source "$PROFILE_FILE"');
    expect(script.indexOf("operator_live_acp_bind_load_factory_api_key_from_profile")).toBeLessThan(
  ...
  it("plumbs live credentials through planned Docker E2E live lanes", () => {
    ...
    const testboxProviderSecretKeys = [
      ...
      "OPERATOR_LIVE_BROWSER_CDP_URL",
      "OPERATOR_LIVE_SETUP_TOKEN",
      "OPERATOR_LIVE_SETUP_TOKEN_MODEL",
      "OPERATOR_LIVE_SETUP_TOKEN_PROFILE",
      "OPERATOR_LIVE_SETUP_TOKEN_VALUE",
      "GEMINI_API_KEY",
      "GOOGLE_API_KEY",
      "OPENROUTER_API_KEY",
      "QWEN_API_KEY",
      ...
      "OPERATOR_CODEX_AUTH_JSON",
      "OPERATOR_CODEX_CONFIG_TOML",
      "OPERATOR_CLAUDE_JSON",
      "OPERATOR_CLAUDE_CREDENTIALS_JSON",
      "OPERATOR_CLAUDE_SETTINGS_JSON",
      "OPERATOR_CLAUDE_SETTINGS_LOCAL_JSON",
      "OPERATOR_GEMINI_SETTINGS_JSON",
      "FIREWORKS_API_KEY",
    ];
    ...
    expectTextToIncludeAll(dockerPlanAction, [
      ...
      "require_any Codex OPERATOR_CODEX_AUTH_JSON",
      'if [[ "$credentials" == *",anthropic,"* ]]; then',
      "require_any Anthropic ANTHROPIC_API_TOKEN ANTHROPIC_API_KEY OPERATOR_CLAUDE_CREDENTIALS_JSON OPERATOR_CLAUDE_JSON",
      'if [[ "$credentials" == *",factory,"* ]]; then',
      "require_any Factory FACTORY_API_KEY",
      'if [[ "$credentials" == *",gemini,"* ]]; then',
      "require_any Gemini GEMINI_API_KEY GOOGLE_API_KEY OPERATOR_GEMINI_SETTINGS_JSON",
    ...
    expect(reusableWorkflow.match(/OPERATOR_LIVE_CLI_BACKEND_AUTH=subscription/g)).toHaveLength(2);
    expect(
      reusableWorkflow.match(
        /if \[\[ -n "\$\{OPERATOR_CLAUDE_CREDENTIALS_JSON:-\}" \|\| -n "\$\{CLAUDE_CODE_OAUTH_TOKEN:-\}" \]\]; then/g,
      ),
    ).toHaveLength(4);
  });
  ...
  it("finalizes dispatched Testbox delegation even when setup or the remote command fails", () => {
    ...
    expect(workflow).toContain('PNPM_CONFIG_STORE_DIR: "/tmp/operator-pnpm-store"');
  ...
  it("allows the Telegram lane to run from reusable package acceptance artifacts", () => {
    ...
    expect(workflow).toContain("OPERATOR_NPM_TELEGRAM_PACKAGE_TGZ");
  ...
  it("includes package acceptance in release checks", () => {
    ...
    expect(workflow).toContain(
      "OPERATOR_QA_CONVEX_SITE_URL: ${{ secrets.OPERATOR_QA_CONVEX_SITE_URL }}",
    );
    expect(workflow).toContain(
      "OPERATOR_QA_CONVEX_SECRET_CI: ${{ secrets.OPERATOR_QA_CONVEX_SECRET_CI }}",
    );
    ...
    expect(workflow).toContain("OPERATOR_RELEASE_QA_*_LIVE_CI_ENABLED");
    expect(workflow).not.toContain(
  ...
  it("routes release Matrix through the QA Lab selector", () => {
    ...
    expect(releaseWorkflow).not.toContain("pnpm operator qa matrix");
    expect(qaWorkflow).toContain("pnpm operator qa matrix");
    ...
    expect(workflowStep(matrixJob, "Run Matrix live lane").env).toMatchObject({
      ...
      MATRIX_PRIMARY_MODEL:
        "${{ inputs.expected_sha != '' && inputs.matrix_primary_model || github.event_name == 'workflow_dispatch' && 'mock-openai/gpt-5.6-luna' || env.OPERATOR_CI_OPENAI_MODEL }}",
      MATRIX_ALTERNATE_MODEL:
        "${{ inputs.expected_sha != '' && inputs.matrix_alternate_model || github.event_name == 'workflow_dispatch' && 'mock-openai/gpt-5.6-luna-alt' || env.OPERATOR_CI_OPENAI_FALLBACK_MODEL }}",
    });
    ...
    expect(qaWorkflow).not.toContain("OPERATOR_QA_MATRIX_CANARY_TIMEOUT_MS");
  ...
  it("runs live transport lanes nightly while release checks stay gated", () => {
    ...
    for (const channel of ["DISCORD", "WHATSAPP", "SLACK"]) {
      ...
      expect(releaseWorkflow).toContain(
        `RELEASE_QA_${channel}_LIVE_CI_ENABLED: \${{ vars.OPERATOR_RELEASE_QA_${channel}_LIVE_CI_ENABLED || 'false' }}`,
      );
      ...
      expect(releaseWorkflow).not.toContain(
        `vars.OPERATOR_RELEASE_QA_${channel}_LIVE_CI_ENABLED == 'true'`,
      );
      expect(qaWorkflow).not.toContain(`OPERATOR_QA_${channel}_LIVE_CI_ENABLED`);
    }
  });
  ...
  it("isolates Open WebUI release coverage on a lean large-disk runner", () => {
    ...
    expect(job.env?.OPERATOR_DOCKER_ALL_RELEASE_PROFILE).toBe("${{ inputs.release_test_profile }}");
  ...
  it("documents the full-release Telegram package path in operator summaries", () => {
    ...
    expectTextToIncludeAll(workflow, [
      ...
      "Package Telegram E2E: Operator Release Checks Package Acceptance",
  ...
  it("lets npm Telegram consume current-run or release-run package artifacts", () => {
    ...
    expectTextToIncludeAll(validateStep.run, [
      ...
      "package_spec must be operator@alpha",
    ...
    expectTextToIncludeAll(runStep.run, [
      ...
      "package candidate manifest does not match the Operator tarball",
      ...
      'export OPERATOR_NPM_TELEGRAM_PACKAGE_DIR="${package_dir}"',
      'export OPERATOR_NPM_TELEGRAM_PACKAGE_TGZ="${package_tgz}"',
    ]);
  });
  ...
  it("uses bounded Convex lease waits instead of GitHub concurrency for CI Telegram consumers", () => {
    ...
    for (const [workflowPath, jobName, stepName, acquireTimeoutMs] of telegramJobs) {
      ...
      expect(step.env?.OPERATOR_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS).toBe(acquireTimeoutMs);
    }
  });
  ...
  it("keeps release QA and repo E2E lanes off scarce 32-core runners", () => {
    ...
    expectTextToIncludeAll(liveE2eWorkflow, [
      "OPERATOR_LIVE_GATEWAY_STEP_TIMEOUT_MS=180000",
      "OPERATOR_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    ]);
  });
  ...
  it("keeps release QA status artifacts blocking in the verifier", () => {
    ...
    expect(telegramDispatch.run).toContain('workflow="operator-release-telegram-qa.yml"');
  ...
  it("keeps release publish creation compatible with gh api and prerelease notes", () => {
    ...
    const npmWorkflow = readFileSync(".github/workflows/operator-npm-release.yml", "utf8");
    const maintainerSkill = readFileSync(
      ".agents/skills/release-operator-maintainer/SKILL.md",
      "utf8",
    );
    ...
    const npmPublishJob = workflowJob(
      ".github/workflows/operator-npm-release.yml",
      "publish_operator_npm",
    );
    ...
    expect(workflow).toContain("Download Operator npm preflight manifest");
    expect(workflow).toContain("Validate Operator npm preflight manifest");
    ...
    expect(workflow).toContain(
      "publish_operator_npm=true requires plugin_publish_scope=all-publishable",
    );
    ...
    expect(fullReleaseWorkflow).not.toContain("node /app/operator.mjs agent");
    expect(fullReleaseWorkflow).toContain('OPERATOR_EXTENSIONS="diagnostics-otel,codex"');
    ...
    expect(npmWorkflow).toContain("does not match operator");
    ...
    expect(npmTelegramWorkflow).toContain("OPERATOR_NPM_TELEGRAM_PACKAGE_DIR");
    ...
    expect(publishSteps[notesIndex]?.if).toBe("${{ inputs.publish_operator_npm }}");
  ...
  it("keeps Operator npm release pack tarball paths local before preflight upload", () => {
    const npmWorkflow = readFileSync(".github/workflows/operator-npm-release.yml", "utf8");
  ...
  it("gates stable GitHub publication on the Windows Hub release asset contract", () => {
    ...
    expect(releaseWorkflow).toContain(
      "Stable Operator publish requires an explicit windows_node_tag.",
    );
    expect(releaseWorkflow).toContain(
      "Stable Operator publish requires candidate-approved windows_node_installer_digests.",
    );
    ...
    expect(releaseWorkflow).toContain(
      "Stable release OperatorCompanion asset names do not exactly match the current contract",
    );
    expect(releaseWorkflow).toContain('select(.name | startswith("OperatorCompanion-"))');
    expect(releaseWorkflow).toContain(
    ...
    expect(windowsWorkflow).toContain("must be an explicit operator-windows-node release tag");
    ...
    expect(windowsWorkflow).toContain(
      "--repo operator/operator-windows-node --json tagName,isDraft,isPrerelease,assets,url",
    );
    expect(windowsWorkflow).toContain(
    ...
    expect(windowsWorkflow).toContain(
      "CN=Operator Foundation, O=Operator Foundation, L=Mill Valley, S=California, C=US",
    );
    expect(windowsWorkflow).toContain("has unexpected signer subject");
    expect(windowsWorkflow).toContain("OperatorCompanion-SHA256SUMS.txt");
    expect(windowsWorkflow).toContain("Verify promoted release asset contract");
    expect(windowsWorkflow).toContain(
      "Promoted OperatorCompanion asset names do not exactly match the current contract",
    );
    expect(windowsWorkflow).toContain(
  ...
  it("gates stable GitHub publication on the signed Android APK contract", () => {
    ...
    expect(androidWorkflow).toContain(
      '--signer-workflow "${GITHUB_REPOSITORY}/.github/workflows/operator-release-publish.yml"',
    );
    ...
    expect(androidWorkflow).toContain("Operator-Android.apk");
    expect(androidWorkflow).toContain("Operator-Android-SHA256SUMS.txt");
    ...
    expect(androidWorkflow).toContain(
      "OPERATOR_BUILD_TIMESTAMP: ${{ steps.release_approval.outputs.build_timestamp }}",
    );
    ...
    expect(androidWorkflow).toContain("requires a final or correction Operator release tag");
    ...
    expect(androidDocs).toContain("github.com/operator/operator/releases");
    expect(androidDocs).not.toContain("releases/latest/download/Operator-Android.apk");
    expect(androidDocs).toContain("gh attestation verify Operator-Android.apk");
  ...
  it("rejects unsafe direct Windows recovery before uploading assets", () => {
    ...
    const rejectUnexpectedTargetAssetsIndex = windowsWorkflow.indexOf(
      "Target Operator release contains unexpected OperatorCompanion assets before upload",
    );
  ...
  it("keeps beta release verification and ClawHub publish repair hooks wired", () => {
    ...
    const clawHubReleasePlanScript = readFileSync(
      "scripts/lib/operator-release-clawhub-plan.ts",
      "utf8",
    );
    ...
    expect(clawHubWorkflow).toContain(
      "uses: operator/clawhub/.github/workflows/package-publish.yml@d8096dfc039e86ab942ddf9ef117d04849fd84c1",
    );
    expect(clawHubWorkflow).toContain(
      'family: ${{ contains(fromJson(\'["@gabrielvfonseca/acpx","@gabrielvfonseca/diffs","@gabrielvfonseca/feishu","@gabrielvfonseca/qqbot"]\'), matrix.plugin.packageName) && \'bundle-plugin\' || \'\' }}',
    );
    ...
    expect(releaseWorkflow).toContain("scripts/operator-release-clawhub-plan.ts");
    expect(releaseWorkflow).toContain("scripts/operator-release-clawhub-runtime-state.ts");
    expect(isExecutable("scripts/operator-release-clawhub-plan.ts")).toBe(true);
    expect(isExecutable("scripts/operator-release-clawhub-runtime-state.ts")).toBe(true);
    expect(releaseWorkflow).toContain("operator-release-clawhub-plan.json");
    ...
    expect(releaseWorkflow).toContain(
      '"${GITHUB_WORKSPACE}/.release-harness/scripts/operator-release-clawhub-plan.ts"',
    );
    ...
    expect(releaseWorkflow).toContain(
      '"${GITHUB_WORKSPACE}/.release-harness/scripts/operator-release-clawhub-runtime-state.ts"',
    );
    expect(releaseWorkflow).toContain(
      '"${GITHUB_WORKSPACE}/.release-harness/scripts/release-verify-beta.ts"',
    );
    expect(releaseWorkflow).toContain("operator-release-clawhub-runtime-state");
    ...
    expect(releaseWorkflow).toContain("Operator npm run ID");
    ...
    expect(releaseWorkflow).toContain("resolve_operator_npm_publish_state");
    expect(releaseWorkflow).toContain(
    ...
    expect(
      releaseWorkflow.match(/npm view "operator@\$\{release_version\}" dist\.tarball/g),
    ).toHaveLength(1);
    ...
    expect(releaseWorkflow).toContain("operator_npm_resume_run_id");
    expect(releaseWorkflow).toContain(
      '"${GITHUB_WORKSPACE}/.release-harness/scripts/operator-npm-resume-run.mjs"',
    );
    expect(releaseWorkflow).toContain('--run-id "${OPERATOR_NPM_RESUME_RUN_ID}"');
    expect(releaseWorkflow).toContain("operator_npm_expected_workflow_ref=\"$(printf '%s'");
    expect(releaseWorkflow).toContain("operator_npm_expected_workflow_sha=\"$(printf '%s'");
    expect(releaseWorkflow).toContain(
      '"${GITHUB_WORKSPACE}/.release-harness/scripts/operator-npm-postpublish-verify.ts"',
    );
    expect(releaseWorkflow).toContain(
      '"${GITHUB_WORKSPACE}/.release-harness/scripts/operator-npm-postpublish-verify.ts"',
    );
    ...
    expect(releaseWorkflow).toContain(
      "Operator Release Publish must use trusted main workflow tooling",
    );
    ...
    expect(releaseWorkflow).toContain('operator_npm_expected_workflow_ref="${GITHUB_REF}"');
    expect(releaseWorkflow).toContain(
      'operator_npm_expected_workflow_sha="${PARENT_WORKFLOW_SHA}"',
    );
    expect(releaseWorkflow).toContain(
      'OPERATOR_NPM_EXPECTED_WORKFLOW_REF="${operator_npm_expected_workflow_ref}"',
    );
    expect(releaseWorkflow).toContain('if [[ "${PUBLISH_OPERATOR_NPM}" == "true" ]]');
    ...
    expect(releaseWorkflow).toContain("operator-release-postpublish-evidence");
    ...
    expect(postpublishEvidenceUpload.if).toContain("inputs.publish_operator_npm");
    ...
    expect(clawHubNewWorkflow).not.toContain(
      "uses: operator/clawhub/.github/workflows/package-publish.yml",
    );
    ...
    expect(clawHubNewWorkflow).toContain(
      "CLAW-277 03 - Split Operator plugin ClawHub publishing into OIDC release and token bootstrap workflows",
    );
  ...
  it("bounds the npm registry tarball download used for release resume", () => {
    ...
    const resolvePublishState = shellFunctionSource(
      publishRun,
      "resolve_operator_npm_publish_state",
    );
...
`,
  ...
  it("keeps release workflow setup and timeout budgets bounded", () => {
    ...
    const crossOs = readWorkflow(".github/workflows/operator-cross-os-release-checks-reusable.yml");
    const liveE2e = readWorkflow(LIVE_E2E_WORKFLOW);
    const releaseWorkflowPaths = [
      FULL_RELEASE_VALIDATION_WORKFLOW,
      RELEASE_CHECKS_WORKFLOW,
      RELEASE_TELEGRAM_QA_WORKFLOW,
      ".github/workflows/operator-cross-os-release-checks-reusable.yml",
      LIVE_E2E_WORKFLOW,
      NPM_TELEGRAM_WORKFLOW,
      ".github/workflows/operator-release-publish.yml",
