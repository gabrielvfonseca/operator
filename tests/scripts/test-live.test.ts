...
describe("scripts/test-live", () => {
  ...
  posixIt("signals the live pnpm child when the wrapper is terminated", async () => {
    ...
    const runner = spawn(process.execPath, ["scripts/test-live.mjs", "--", "fake.live.test.ts"], {
      env: {
        ...process.env,
        OPERATOR_FAKE_PNPM_PID_PATH: childPidPath,
        OPERATOR_FAKE_PNPM_DESCENDANT_PID_PATH: descendantPidPath,
        OPERATOR_FAKE_PNPM_SIGNALED_PATH: signaledPath,
  ...
  posixIt("kills the live pnpm process group after the no-output timeout", async () => {
    ...
    const runner = spawn(process.execPath, ["scripts/test-live.mjs", "--", "fake.live.test.ts"], {
      env: {
        ...
        OPERATOR_FAKE_PNPM_PID_PATH: childPidPath,
        OPERATOR_FAKE_PNPM_DESCENDANT_PID_PATH: descendantPidPath,
        OPERATOR_LIVE_WRAPPER_HEARTBEAT_MS: "25",
        OPERATOR_VITEST_NO_OUTPUT_TIMEOUT_MS: "100",
        npm_execpath: fakePnpmPath,
      },
  ...
  it("rejects loose heartbeat intervals instead of parsing prefixes", () => {
    ...
    expect(resolveTestLiveHeartbeatMs({ OPERATOR_LIVE_WRAPPER_HEARTBEAT_MS: "2500" })).toBe(2500);
    expect(() => resolveTestLiveHeartbeatMs({ OPERATOR_LIVE_WRAPPER_HEARTBEAT_MS: "1e3" })).toThrow(
      "invalid OPERATOR_LIVE_WRAPPER_HEARTBEAT_MS: 1e3",
    );
    expect(() =>
      resolveTestLiveHeartbeatMs({ OPERATOR_LIVE_WRAPPER_HEARTBEAT_MS: "1000ms" }),
    ).toThrow("invalid OPERATOR_LIVE_WRAPPER_HEARTBEAT_MS: 1000ms");
    expect(() => resolveTestLiveHeartbeatMs({ OPERATOR_LIVE_WRAPPER_HEARTBEAT_MS: "0" })).toThrow(
      "invalid OPERATOR_LIVE_WRAPPER_HEARTBEAT_MS: 0",
    );
  });
...
function writeFakePnpm(filePath: string): void {
  writeFileSync(
    ...
    [
      ...
      "if (process.env.OPERATOR_FAKE_PNPM_DESCENDANT_PID_PATH) {",
      "  const child = spawn(process.execPath, [",
      '    "-e",',
      "    \"process.on('SIGTERM', () => {}); setInterval(() => {}, 1000);\",",
      "  ], { stdio: 'ignore' });",
      "  fs.writeFileSync(process.env.OPERATOR_FAKE_PNPM_DESCENDANT_PID_PATH, String(child.pid));",
      "}",
      "fs.writeFileSync(process.env.OPERATOR_FAKE_PNPM_PID_PATH, String(process.pid));",
      'process.on("SIGTERM", () => {',
      '  fs.writeFileSync(process.env.OPERATOR_FAKE_PNPM_SIGNALED_PATH, "SIGTERM");',
      "  process.exit(0);",
      "});",
      "setInterval(() => {}, 1000);",
      "",
