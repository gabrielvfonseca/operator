...
describe("scripts/test-group-report grouping", () => {
  it("groups repo files by stable product area", () => {
    ...
    expect(resolveTestArea("ui/tests/ui/views/chat.test.ts")).toBe("ui/views");
...
describe("scripts/test-group-report run plans", () => {
  ...
  beforeAll(() => {
    withEnv(
      {
        OPERATOR_TEST_PROJECTS_PARALLEL: undefined,
        OPERATOR_TEST_PROJECTS_LEAF_SHARDS: undefined,
      },
      () => {
        serialFullSuitePlans = resolveRunPlans(parseTestGroupReportArgs(["--full-suite"]));
      },
    );
    withEnv({ OPERATOR_TEST_PROJECTS_PARALLEL: "6" }, () => {
