...
`),
  ...
  it("enables both startup and restart trace in the child gateway environment", () => {
    ...
    expect(env.OPERATOR_GATEWAY_STARTUP_TRACE).toBe("1");
    expect(env.OPERATOR_GATEWAY_RESTART_TRACE).toBe("1");
    expect(env.OPERATOR_NO_RESPAWN).toBe("1");
    expect(env.OPERATOR_LOCAL_CHECK).toBeUndefined();
  });
  ...
  it("can pin ACPX startup probe policy per benchmark case", () => {
    const probeOffEnv = testing.sanitizedEnv(
      ...
      {
        config: {},
        env: { OPERATOR_ACPX_RUNTIME_STARTUP_PROBE: "0" },
        id: "skipChannelsNoAcpxProbe",
        name: "gateway restart, skip channels, ACPX startup probe off",
      },
    );

    expect(probeOffEnv.OPERATOR_ACPX_RUNTIME_STARTUP_PROBE).toBe("0");
  });
  ...
  it("writes restart intent files for the target gateway pid", () => {
    ...
    try {
      const env = { OPERATOR_STATE_DIR: path.join(root, "state") };
