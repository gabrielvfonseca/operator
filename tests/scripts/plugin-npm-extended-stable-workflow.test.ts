...
describe("plugin npm extended-stable workflow", () => {
  ...
  it("uses one override for check, plan, preview, pack, and publish", () => {
    ...
    for (const name of [
      ...
      ).toMatchObject({ OPERATOR_PLUGIN_NPM_PUBLISH_TAG: expectedOverride });
    }
  });
  ...
  it("attests the canonical Meta provider package and install route", () => {
    ...
    expect(packageJson.name).toBe("@gabrielvfonseca/meta-provider");
    expect(packageJson.operator?.install?.npmSpec).toBe("@gabrielvfonseca/meta-provider");
  ...
  it("publishes extended-stable with OIDC only and verifies every package tag", () => {
    ...
    expect(publish.env).toMatchObject({
      OPERATOR_NPM_PUBLISH_AUTH_MODE: "trusted-publisher",
    });
    ...
    expect(bootstrap.run).not.toContain("@gabrielvfonseca/meta-provider");
