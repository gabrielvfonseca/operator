...
function runRetryHelper(commandPath: string, counterPath: string) {
  ...
  delete env.OPERATOR_LIVE_COMMAND_RETRY_PATTERN;
  delete env.OPERATOR_LIVE_COMMAND_RATE_LIMIT_PATTERN;
  return spawnSync("/bin/bash", [SCRIPT_PATH], {
    cwd: process.cwd(),
    encoding: "utf8",
    env: {
      ...env,
      OPERATOR_LIVE_COMMAND: `/bin/bash ${JSON.stringify(commandPath)}`,
      OPERATOR_LIVE_COMMAND_ATTEMPTS: "2",
      OPERATOR_LIVE_COMMAND_RETRY_DELAY_SECONDS: "0",
      OPERATOR_LIVE_COMMAND_RATE_LIMIT_RETRY_DELAY_SECONDS: "0",
      OPERATOR_RETRY_TEST_COUNTER: counterPath,
    },
  });
}
...
describe("scripts/ci-live-command-retry.sh", () => {
  it("retries a provider-internal RPC timeout", () => {
    const { commandPath, counterPath } = writeCommand("operator-ci-live-rpc-timeout-", [
      'attempts="$(cat "$OPERATOR_RETRY_TEST_COUNTER" 2>/dev/null || printf 0)"',
      'attempts="$((attempts + 1))"',
      'printf "%s" "$attempts" > "$OPERATOR_RETRY_TEST_COUNTER"',
