...
describe("scripts/check-deprecated-api-usage", () => {
  ...
  it("bans the scoped @gabrielvfonseca/plugin-sdk spelling of every deprecated specifier", () => {
    const specifiers = new Set(buildDeprecatedPluginSdkModuleSpecifiers());
  ...
  it("flags internal facade imports across static, relative, scoped, and dynamic forms", () => {
    const result = runFacadeImportRule({
      "src/channels/probe.ts": [
        'import { createChannelReplyPipeline } from "@gabrielvfonseca/operator/plugin-sdk/channel-reply-pipeline";',
        ...
        'import { formatInboundEnvelope } from "@gabrielvfonseca/plugin-sdk/channel-envelope";',
      ].join("\n"),
    ...
    expect(result.stderr).toContain(
      "src/channels/probe.ts:4: @gabrielvfonseca/plugin-sdk/channel-envelope",
    );
    expect(result.stderr).toContain(
