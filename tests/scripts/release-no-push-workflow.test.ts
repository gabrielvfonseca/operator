...
describe("release validation no-push transport", () => {
  it("builds planned live images locally without entering pull fallback", () => {
    ...
    for (const jobName of [
      ...
      expect(runStep?.run, jobName).toContain("OPERATOR_SKIP_DOCKER_BUILD=0");
      expect(runStep?.run, jobName).not.toContain("OPERATOR_DOCKER_BUILD_ON_MISSING=1");
    }
  });
  ...
  it("builds shared images locally, verifies artifacts, and cannot fall back to a registry", () => {
    ...
    expect(packDockerArtifact.run).toContain(
      'OPERATOR_SHARED_IMAGE_PACKAGE_SHA256="$PACKAGE_SHA256"',
    );
    ...
    for (const name of [
      ...
      expect(consumer.env?.OPERATOR_DOCKER_E2E_REQUIRE_LOCAL_IMAGE).toContain("no-push-artifact");
      ...
      expect(loadArtifact.env?.OPERATOR_SHARED_IMAGE_RUN_ATTEMPT).toBe(
        "${{ needs.prepare_docker_e2e_image.outputs.image_artifact_run_attempt }}",
      );
      expect(loadArtifact.env?.OPERATOR_SHARED_IMAGE_RUN_ID).toBe(
        "${{ needs.prepare_docker_e2e_image.outputs.image_artifact_run_id }}",
      );
      expect(loadArtifact.run).toContain("shared-image-artifact.sh");
      expect(loadArtifact.run).toContain('OPERATOR_SHARED_IMAGE_ARCHIVE_SHA256="$ARCHIVE_SHA256"');
      expect(loadArtifact.run).toContain('OPERATOR_SHARED_IMAGE_PACKAGE_SHA256="$PACKAGE_SHA256"');
    ...
    for (const name of [
      ...
      expect(consumer.env?.OPERATOR_LIVE_REQUIRE_LOCAL_IMAGE).toContain("no-push-artifact");
    ...
    const requireLocalIndex = liveBuild.indexOf("OPERATOR_LIVE_REQUIRE_LOCAL_IMAGE");
...
`,
      ...
      const result = spawnSync("bash", [resolve(LIVE_BUILD)], {
        ...
        env: {
          ...
          OPERATOR_LIVE_IMAGE: "operator-live-test:required-local",
          OPERATOR_LIVE_REQUIRE_LOCAL_IMAGE: "1",
...
docker_e2e_build_or_reuse "operator-e2e:required-local" "required local image test"`,
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            FAKE_DOCKER_LOG: calls,
            OPERATOR_DOCKER_BUILD_ON_MISSING: "1",
            OPERATOR_DOCKER_E2E_REQUIRE_LOCAL_IMAGE: "1",
            OPERATOR_SKIP_DOCKER_BUILD: "1",
            PATH: `${bin}:${process.env.PATH ?? ""}`,
          },
        },
