...
const originalOperatorCodexRepo = process.env.OPERATOR_CODEX_REPO;

afterEach(() => {
  if (originalOperatorCodexRepo === undefined) {
    delete process.env.OPERATOR_CODEX_REPO;
  } else {
    process.env.OPERATOR_CODEX_REPO = originalOperatorCodexRepo;
  }
});
...
`),
  ...
  it("allows an explicit local disk headroom override", () => {
    expect(resolveCodexProtocolMinFreeBytes({ OPERATOR_CODEX_PROTOCOL_MIN_FREE_BYTES: "0" })).toBe(
      0,
    );
    expect(() =>
      validateCodexProtocolGenerationHeadroom({
  ...
  it("rejects malformed local disk headroom overrides", () => {
    expect(() =>
      resolveCodexProtocolMinFreeBytes({ OPERATOR_CODEX_PROTOCOL_MIN_FREE_BYTES: "nope" }),
  ...
  it("uses OPERATOR_CODEX_REPO when provided", async () => {
    ...
    process.env.OPERATOR_CODEX_REPO = codexRepo;
  ...
  it("finds the primary checkout sibling from a git worktree", async () => {
    ...
    const primaryOperator = path.join(parentDir, "@gabrielvfonseca/operator");
