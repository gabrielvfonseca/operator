...
const scrubbedEnvKeys = [
  ...
  "MOCK_TLS_CERT",
  "MOCK_TLS_KEY",
  "OPERATOR_CONFIG_RELOAD_LOG_MAX_READ_BYTES",
  "OPERATOR_CONFIG_RELOAD_LOG_PATH",
  "OPERATOR_CONFIG_RELOAD_LOG_TIMEOUT_MS",
  "OPERATOR_MOCK_OPENAI_PORT",
  "RAW_SCHEMA_ERROR",
  "SUCCESS_MARKER",
];
...
describe("mock OpenAI response markers", () => {
  it("echoes dynamic Operator E2E markers", async () => {
    await withMockServer(mockOpenAiPath, {}, async (baseUrl) => {
      for (const marker of ["OPERATOR_E2E_SEED_0_123", "OPERATOR_E2E_ANDROID_OK"]) {
...
describe("e2e mock and config helper numeric limits", () => {
  it("rejects loose mock OpenAI port env values", () => {
    ...
    const fallbackPort = runScript(mockOpenAiPath, {
      OPERATOR_MOCK_OPENAI_PORT: "44080http",
    });
    expect(fallbackPort.status).not.toBe(0);
    expect(fallbackPort.stderr).toContain("invalid OPERATOR_MOCK_OPENAI_PORT: 44080http");
  });
  ...
  it("rejects out-of-range mock OpenAI port env values", () => {
    ...
    const fallbackPort = runScript(mockOpenAiPath, {
      OPERATOR_MOCK_OPENAI_PORT: "65536",
    });
    expect(fallbackPort.status).not.toBe(0);
    expect(fallbackPort.stderr).toContain("invalid OPERATOR_MOCK_OPENAI_PORT: 65536");
  });
  ...
  it("rejects loose config-reload log timeout env values", () => {
    const result = runScript(configReloadAssertPath, {
      OPERATOR_CONFIG_RELOAD_LOG_TIMEOUT_MS: "30000ms",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid OPERATOR_CONFIG_RELOAD_LOG_TIMEOUT_MS: 30000ms");
  });

  it("rejects loose config-reload log read caps", () => {
    const result = runScript(configReloadAssertPath, {
      OPERATOR_CONFIG_RELOAD_LOG_MAX_READ_BYTES: "256kb",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid OPERATOR_CONFIG_RELOAD_LOG_MAX_READ_BYTES: 256kb");
  });
      ...
      await withMockServer(
        ...
        async (baseUrl, output) => {
          const response = await fetch(`${baseUrl}/v1/responses`, {
            method: "POST",
            headers: { "content-type": "application/json" },
            body: JSON.stringify({ input: "OPERATOR_E2E_OK" }),
          });
          const body = await response.json();
    ...
    try {
      await withMockServer(
        webSearchMockPath,
        {
          ...
          SUCCESS_MARKER: "OPERATOR_SCHEMA_E2E_OK",
        },
        async (baseUrl, output) => {
          const response = await fetch(`${baseUrl}/v1/responses`, {
            method: "POST",
