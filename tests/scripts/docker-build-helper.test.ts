...
describe("docker build helper", () => {
  it("allows deployments to build an immutable sandbox image tag", () => {
    ...
    expect(script).toContain(
      'IMAGE_NAME="${OPERATOR_SANDBOX_IMAGE:-operator-sandbox:bookworm-slim}"',
    );
  });
  ...
  it("forces BuildKit for centralized Docker builds", () => {
    ...
    expect(helper).toContain("OPERATOR_DOCKER_BUILD_RETRIES");
    expect(helper).toContain("OPERATOR_DOCKER_BUILD_TIMEOUT");
    expect(helper).toContain('docker_build_run_logged "$label" "$timeout_value" "$log_file"');
    expect(helper).toContain("OPERATOR_DOCKER_BUILD_REQUIRE_TIMEOUT");
  ...
  it("treats Docker registry auth 5xx failures as transient build failures", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-build-transient-"));
...
`;
  ...
  it("detects Docker builder memory exhaustion failures", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-build-memory-"));
...
`;
  ...
  it("routes standalone Docker smoke runs through the timeout-aware helper", () => {
    ...
    expect(cleanupSmoke).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${OPERATOR_CLEANUP_SMOKE_DOCKER_TIMEOUT:-600s}}"',
    );
    expect(cleanupSmoke).toContain(
    ...
    expect(installE2eSmoke).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${OPERATOR_INSTALL_E2E_DOCKER_TIMEOUT:-2700s}}"',
    );
  ...
  it("bounds cleanup-smoke failure log output", () => {
    ...
    expect(cleanupRun).toContain("OPERATOR_CLEANUP_SMOKE_LOG_PRINT_BYTES");
    expect(cleanupRun).toContain(
      "read_positive_int_env OPERATOR_CLEANUP_SMOKE_LOG_PRINT_BYTES 65536 >/dev/null",
    );
    expect(cleanupRun.match(/print_log_tail \/tmp\/operator-cleanup-/g)).toHaveLength(3);
    expect(cleanupRun).not.toContain("cat /tmp/operator-cleanup-");
  });
  ...
  it("gives cleanup-smoke builds enough Node heap while preserving explicit callers", () => {
    ...
    expect(cleanupRun.indexOf("ensure_cleanup_smoke_node_options")).toBeLessThan(
      cleanupRun.indexOf("pnpm build >/tmp/operator-cleanup-build.log"),
    );
  });

  it("rejects invalid cleanup-smoke log byte limits", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-cleanup-smoke-log-invalid-"));
...
set -euo pipefail
LOG_PATH=${shellQuote(logPath)}
export OPERATOR_CLEANUP_SMOKE_LOG_PRINT_BYTES=64kb
...
`;
      ...
      expect(result.stderr).toContain("invalid OPERATOR_CLEANUP_SMOKE_LOG_PRINT_BYTES: 64kb");
  ...
  it("normalizes zero-padded cleanup-smoke log byte limits", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-cleanup-smoke-log-tail-"));
...
set -euo pipefail
LOG_PATH=${shellQuote(logPath)}
export OPERATOR_CLEANUP_SMOKE_LOG_PRINT_BYTES=0008
...
`;
  ...
  it("prints in-container Docker client logs through bounded helpers", () => {
    for (const scriptPath of [CODEX_MEDIA_PATH_SCENARIO_PATH, OPENAI_CHAT_TOOLS_SCENARIO_PATH]) {
      ...
      expect(script, scriptPath).toContain("source scripts/lib/operator-e2e-instance.sh");
      expect(script, scriptPath).toContain('operator_e2e_print_log "$CLIENT_LOG"');
      expect(script, scriptPath).not.toContain('cat "$CLIENT_LOG"');
    }
  });
  ...
  it("runs cleanup smoke on the native ARM platform instead of pulling an amd64 tag", () => {
    ...
    expect(
      runCleanupDefaultPlatform({ OPERATOR_CLEANUP_SMOKE_PLATFORM: "linux/s390x" }, "x86_64"),
    ).toBe("linux/s390x");
  });
  ...
  it("lets Testbox fall back to building when a reused Docker image is missing", () => {
    ...
    expect(helper).toContain("OPERATOR_DOCKER_BUILD_ON_MISSING");
    expect(helper).toContain("OPERATOR_TESTBOX");
    ...
    expect(liveBuild).toContain(
      'DOCKER_COMMAND_TIMEOUT="${DOCKER_COMMAND_TIMEOUT:-${OPERATOR_LIVE_DOCKER_PULL_TIMEOUT:-600s}}"',
    );
    expect(liveBuild).toContain(
      'LIVE_IMAGE_PULL_ATTEMPTS="${OPERATOR_LIVE_DOCKER_PULL_ATTEMPTS:-3}"',
    );
    ...
    expect(liveCliBackend).toContain(
      'OPERATOR_LIVE_DOCKER_REPO_ROOT="$ROOT_DIR" "$TRUSTED_HARNESS_DIR/scripts/test-live-build-docker.sh"',
    );
    ...
    expect(liveCliBackend).not.toContain(
      'echo "==> Reuse live-test image: $LIVE_IMAGE_NAME (OPERATOR_SKIP_DOCKER_BUILD=1)"',
    );
  });
  ...
  it("rejects malformed Docker E2E resource limits before a suite starts", () => {
    ...
    const runProbe = (value: string) => {
      const script = [
        ...
        "docker_e2e_read_nonnegative_decimal_env OPERATOR_SAMPLE_RESOURCE_LIMIT 2048",
      ].join("\n");
      return spawnSync("bash", ["-c", script], {
        ...
        env: {
          ...process.env,
          OPERATOR_SAMPLE_RESOURCE_LIMIT: value,
        },
      });
    };
    ...
    expect(invalid.stderr).toContain("invalid OPERATOR_SAMPLE_RESOURCE_LIMIT: 12mb");
    expect(overlarge.status).toBe(2);
    expect(overlarge.stderr).toContain("invalid OPERATOR_SAMPLE_RESOURCE_LIMIT: 9999999999");
    expect(overprecise.status).toBe(2);
    expect(overprecise.stderr).toContain("invalid OPERATOR_SAMPLE_RESOURCE_LIMIT: 12.1234567");
  ...
  it("keeps Testbox image-build fallback before isolating live MCP code-mode runtime flags", () => {
    ...
    const unsetIndex = script.indexOf("unset OPERATOR_TESTBOX");
  ...
  it("wraps centralized Docker builds with the timeout helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-build-timeout-"));
...
export OPERATOR_DOCKER_BUILD_TIMEOUT=17s
...
`;
  ...
  it("prints heartbeat progress for long successful centralized Docker builds", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-build-heartbeat-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin:$PATH"
export OPERATOR_DOCKER_BUILD_HEARTBEAT_SECONDS=1
...
`;
  ...
  it("stops the tracked build command without retrying when interrupted", async () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-build-signal-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin:$PATH"
export OPERATOR_DOCKER_BUILD_RETRIES=3
source "$ROOT_DIR/scripts/lib/docker-build.sh"
docker_build_run e2e-build -t demo-image .
`,
      );
  ...
  it("does not delay fast successful centralized Docker builds until the next heartbeat", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-build-fast-heartbeat-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin:$PATH"
export OPERATOR_DOCKER_BUILD_HEARTBEAT_SECONDS=30
...
ROOT_DIR=${shellQuote(rootDir)}
export ROOT_DIR
export OPERATOR_DOCKER_BUILD_HEARTBEAT_SECONDS=08
...
ROOT_DIR=${shellQuote(rootDir)}
export ROOT_DIR
export OPERATOR_DOCKER_BUILD_RETRIES=08
...
`;
  ...
  it.each([
    [
      "retry count",
      "OPERATOR_DOCKER_BUILD_RETRIES",
      "2x",
      "invalid OPERATOR_DOCKER_BUILD_RETRIES: 2x",
    ],
    [
      "heartbeat interval",
      "OPERATOR_DOCKER_BUILD_HEARTBEAT_SECONDS",
      "soon",
      "invalid OPERATOR_DOCKER_BUILD_HEARTBEAT_SECONDS: soon",
    ],
  ])(
    "rejects invalid centralized Docker build %s before invoking docker",
    (_label, envName, value, expectedError) => {
      const workDir = mkdtempSync(join(tmpdir(), "operator-docker-build-config-"));
...
`;
  ...
  it("fails centralized Docker builds fast when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-build-timeout-required-"));
...
export OPERATOR_DOCKER_BUILD_TIMEOUT=19s
...
`;
  ...
  it("keeps setup-style Docker builds compatible when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-build-timeout-optional-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export OPERATOR_DOCKER_BUILD_TIMEOUT=23s
...
`;
  ...
  it("keeps reused Docker image probes behind the timeout-aware helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-image-reuse-timeout-"));
...
export OPERATOR_SKIP_DOCKER_BUILD=1
...
grep -q '^image inspect operator-reuse-image$' "$TMPDIR/docker-seen"
grep -q '^pull operator-reuse-image$' "$TMPDIR/docker-seen"
`;
  ...
  it("derives the browser CDP image from the shared functional image", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-browser-cdp-shared-image-"));
...
export PATH="$TMPDIR/bin:$PATH"
export OPERATOR_SKIP_DOCKER_BUILD=1
export OPERATOR_DOCKER_E2E_IMAGE=shared-functional
export OPERATOR_DOCKER_ALL_LANE_NAME=browser-cdp-snapshot

bash "$ROOT_DIR/scripts/e2e/browser-cdp-snapshot-docker.sh"

grep -q '^image inspect shared-functional$' "$TMPDIR/docker-seen"
grep -Fq 'build -t operator-browser-cdp-snapshot-e2e:browser-cdp-snapshot' "$TMPDIR/docker-seen"
grep -Fq ' operator-browser-cdp-snapshot-e2e:browser-cdp-snapshot ' "$TMPDIR/docker-seen"
...
`;
  ...
  it("fails fast on invalid browser CDP snapshot byte limits", () => {
    const result = spawnSync("bash", [BROWSER_CDP_SNAPSHOT_DOCKER_E2E_PATH], {
      ...
      env: {
        ...process.env,
        OPERATOR_BROWSER_CDP_SNAPSHOT_MAX_BYTES: "64kb",
        OPERATOR_SKIP_DOCKER_BUILD: "1",
      },
    });

    expect(result.status).toBe(2);
    expect(result.stderr).toContain("invalid OPERATOR_BROWSER_CDP_SNAPSHOT_MAX_BYTES: 64kb");
  });
  ...
  it("forwards browser CDP snapshot byte limits into the Docker runner", () => {
    ...
    expect(runner).toContain(
      "docker_e2e_read_positive_int_env OPERATOR_BROWSER_CDP_SNAPSHOT_MAX_BYTES 524288",
    );
    expect(runner).toContain('-e "OPERATOR_BROWSER_CDP_SNAPSHOT_MAX_BYTES=$SNAPSHOT_MAX_BYTES"');
  });
  ...
  it("opens the browser CDP fixture before snapshotting", () => {
    const runner = readFileSync(BROWSER_CDP_SNAPSHOT_DOCKER_E2E_PATH, "utf8");
    const quarantineIndex = runner.indexOf("mkdir -p /tmp/operator-browser-cdp");
  ...
  it("fails Docker commands fast when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-timeout-required-"));
...
`;
  ...
  it("uses a Node watchdog for Docker commands when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-node-timeout-"));
...
export PATH="$TMPDIR/bin"
export DOCKER_COMMAND_TIMEOUT=7s
unset OPERATOR_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset OPERATOR_DOCKER_E2E_MEMORY OPERATOR_DOCKER_E2E_CPUS OPERATOR_DOCKER_E2E_PIDS_LIMIT
...
`;
  ...
  it("adds default Docker run resource limits without overriding explicit limits", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-resource-limits-"));
...
unset OPERATOR_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset OPERATOR_DOCKER_E2E_MEMORY OPERATOR_DOCKER_E2E_CPUS OPERATOR_DOCKER_E2E_PIDS_LIMIT
export OPERATOR_DOCKER_E2E_AVAILABLE_CPUS=32
...
OPERATOR_DOCKER_E2E_MEMORY=12g OPERATOR_DOCKER_E2E_CPUS=4 OPERATOR_DOCKER_E2E_PIDS_LIMIT=512 docker_e2e_docker_cmd run demo
OPERATOR_DOCKER_E2E_AVAILABLE_CPUS=8 OPERATOR_DOCKER_E2E_MEMORY=12g OPERATOR_DOCKER_E2E_CPUS=16 OPERATOR_DOCKER_E2E_PIDS_LIMIT=512 docker_e2e_docker_cmd run demo
docker_e2e_docker_cmd run --memory 2g --cpus 3 --pids-limit 99 demo
OPERATOR_DOCKER_E2E_DISABLE_RESOURCE_LIMITS=1 docker_e2e_docker_cmd run demo
...
`;
  ...
  it("rejects invalid Docker run pids limits before invoking docker", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-resource-pids-"));
...
set +e
OPERATOR_DOCKER_E2E_PIDS_LIMIT=many docker_e2e_docker_cmd run demo 2>"$TMPDIR/stderr"
status="$?"
set -e

[[ "$status" = "2" ]]
[[ "$(<"$TMPDIR/stderr")" = *"invalid OPERATOR_DOCKER_E2E_PIDS_LIMIT: many"* ]]
[[ ! -e "$TMPDIR/docker-seen" ]]
`;
  ...
  for (const [shellSignal, expectedStatus] of [
    ...
    it(`escalates Docker watchdog children that ignore parent SIG${shellSignal}`, () => {
      const workDir = mkdtempSync(join(tmpdir(), "operator-docker-node-signal-"));
...
export OPERATOR_DOCKER_TIMEOUT_KILL_GRACE_MS=100
...
`;
  ...
  it("uses plain timeout when kill-after is unsupported", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-plain-timeout-"));
...
`;
  ...
  it("uses gtimeout when timeout is unavailable", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-gtimeout-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export OPERATOR_DOCKER_E2E_RUN_TIMEOUT=13s
export OPERATOR_DOCKER_E2E_AVAILABLE_CPUS=8
unset OPERATOR_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset OPERATOR_DOCKER_E2E_MEMORY OPERATOR_DOCKER_E2E_CPUS OPERATOR_DOCKER_E2E_PIDS_LIMIT
...
`;
  ...
  it("keeps package-backed Docker runs bounded without the shared timeout helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-package-timeout-required-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export OPERATOR_DOCKER_E2E_RUN_TIMEOUT=11s
...
`;
  ...
  it("rejects invalid package-backed Docker run pids limits before invoking docker", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-package-pids-"));
...
set +e
OPERATOR_DOCKER_E2E_PIDS_LIMIT=many docker_e2e_docker_run_cmd run demo 2>"$TMPDIR/stderr"
status="$?"
set -e
...
`;
  ...
  it("uses gtimeout for package-backed Docker runs without the shared timeout helper", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-package-gtimeout-"));
...
export ROOT_DIR TMPDIR
export PATH="$TMPDIR/bin"
export OPERATOR_DOCKER_E2E_RUN_TIMEOUT=15s
export OPERATOR_DOCKER_E2E_AVAILABLE_CPUS=8
unset OPERATOR_DOCKER_E2E_DISABLE_RESOURCE_LIMITS
unset OPERATOR_DOCKER_E2E_MEMORY OPERATOR_DOCKER_E2E_CPUS OPERATOR_DOCKER_E2E_PIDS_LIMIT
...
`;
  ...
  it("removes functional Docker build package inputs after the build", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-build-cleanup-"));
...
node() {
  local script="$1"
  shift
  if [[ "$script" != "$ROOT_DIR/scripts/package-operator-for-docker.mjs" ]]; then
    command node "$script" "$@"
    return
  fi
...
docker_build_run() {
  ...
  for arg in "$@"; do
    case "$arg" in
      operator_package=*)
        build_context="\${arg#operator_package=}"
        ;;
    esac
  done

  test -n "$build_context"
  test -f "$build_context/operator-current.tgz"
...
leftovers="$(find "$TMPDIR" -maxdepth 1 \\( \\
  -name 'operator-docker-e2e-pack.*' \\
  -o -name 'operator-docker-e2e-package-context.*' \\
\\) -print)"
if [[ -n "$leftovers" ]]; then
...
`;
  ...
  it("keeps caller-provided functional Docker build packages", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-build-external-package-"));
...
printf fixture >"$external_dir/operator-current.tgz"
OPERATOR_CURRENT_PACKAGE_TGZ="$external_dir/operator-current.tgz"
export OPERATOR_CURRENT_PACKAGE_TGZ
...
test -f "$TMPDIR/build-context-seen"
test -f "$OPERATOR_CURRENT_PACKAGE_TGZ"
leftovers="$(find "$TMPDIR" -maxdepth 1 -name 'operator-docker-e2e-package-context.*' -print)"
if [[ -n "$leftovers" ]]; then
...
`;
  ...
  it("cleans generated package mounts after harness Docker runs", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-package-mount-cleanup-"));
...
test ! -e "$pack_dir"
test -z "$(find "$TMPDIR" -maxdepth 1 -name 'operator-docker-e2e-container.*' -print)"

external_dir="$TMPDIR/external-package"
mkdir -p "$external_dir"
printf fixture >"$external_dir/operator-current.tgz"
docker_e2e_package_mount_args "$external_dir/operator-current.tgz"
...
test -f "$external_dir/operator-current.tgz"
`;
  ...
  it("propagates shared E2E command timeouts into package-backed containers", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-package-timeout-env-"));
...
package="$TMPDIR/operator-current.tgz"
printf fixture >"$package"
export OPERATOR_E2E_NPM_INSTALL_TIMEOUT=42s
export OPERATOR_E2E_COMMAND_TIMEOUT=23s
docker_e2e_package_mount_args "$package"
printf "%s\\n" "\${DOCKER_E2E_PACKAGE_ARGS[@]}" >"$TMPDIR/package-args"

grep -qx -- "-e" "$TMPDIR/package-args"
grep -qx -- "OPERATOR_CURRENT_PACKAGE_TGZ=/tmp/operator-current.tgz" "$TMPDIR/package-args"
grep -qx -- "OPERATOR_E2E_NPM_INSTALL_TIMEOUT=42s" "$TMPDIR/package-args"
grep -qx -- "OPERATOR_E2E_COMMAND_TIMEOUT=23s" "$TMPDIR/package-args"
`;
  ...
  it("passes plugin lifecycle sampler timeout overrides into Docker", () => {
    ...
    expect(runner).toContain("append_positive_int_env OPERATOR_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS");
    expect(runner).toContain(
      "append_positive_int_env OPERATOR_PLUGIN_LIFECYCLE_TIMEOUT_KILL_GRACE_MS",
    );
    expect(runner).toContain("append_positive_int_env OPERATOR_PLUGIN_LIFECYCLE_METRIC_POLL_MS");
    expect(runner).toContain("append_positive_int_env OPERATOR_PLUGIN_LIFECYCLE_MAX_RSS_KB");
    expect(runner).toContain("append_positive_int_env OPERATOR_PLUGIN_LIFECYCLE_MAX_WALL_MS");
    expect(runner).toContain(
      "append_positive_number_env OPERATOR_PLUGIN_LIFECYCLE_MAX_CPU_CORE_RATIO",
    );
    expect(runner).toContain('docker_e2e_run_with_harness \\\n  "${DOCKER_ENV_ARGS[@]}"');
  });

  it.each([
    ["phase timeout", "OPERATOR_PLUGIN_LIFECYCLE_PHASE_TIMEOUT_MS", "150ms"],
    ["CPU ratio", "OPERATOR_PLUGIN_LIFECYCLE_MAX_CPU_CORE_RATIO", "0"],
  ])(
    ...
    (_label, envName, value) => {
      const result = spawnSync("bash", [PLUGIN_LIFECYCLE_MATRIX_DOCKER_E2E_PATH], {
        ...
        env: {
          ...process.env,
          OPERATOR_CURRENT_PACKAGE_TGZ: "/tmp/operator-missing-package.tgz",
          [envName]: value,
        },
      });
      ...
      expect(result.stderr).not.toContain("Operator package tarball does not exist");
    },
  );
  ...
  it("wraps direct Docker E2E npm installs with the shared timeout helper", () => {
    ...
    expect(multiNode).toContain(
      'operator_e2e_install_package "$ARTIFACTS/install-a.log" "Operator package under node-A prefix" "$NPM_PREFIX_A"',
    );
    expect(updateChannel).toContain(
      'operator_e2e_maybe_timeout "${OPERATOR_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit',
    );
    expect(updateChannel).toContain(
      'operator_e2e_maybe_timeout "${OPERATOR_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$pkg_tgz_path"',
    );
    expect(updateChannel).toContain("operator_e2e_print_log /tmp/operator-git-install.log");
    expect(updateChannel).toContain('operator_e2e_print_log "$package_install_log"');
    expect(updateChannel).not.toContain("cat /tmp/operator-git-install.log");
    expect(updateChannel).not.toContain('cat "$package_install_log"');
    expect(doctorSwitch).toContain(
      'operator_e2e_maybe_timeout "${OPERATOR_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit',
    );
    expect(doctorSwitch).toContain(
      'operator_e2e_maybe_timeout "${OPERATOR_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$package_tgz"',
    );
    for (const script of [releaseUpgrade, upgradeSurvivor, pluginCorrupt]) {
      expect(script).toContain(
        'operator_e2e_maybe_timeout "${OPERATOR_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g',
      );
    }
  });
  ...
  it("keeps upgrade survivor mutable state off the host-mounted artifact tree", () => {
    ...
    for (const script of [runner, publishedRunner]) {
      expect(script).toContain("operator-upgrade-survivor-runtime");
      expect(script).toContain("OPERATOR_UPGRADE_SURVIVOR_TMPDIR");
      expect(script).toContain("OPERATOR_UPGRADE_SURVIVOR_TEST_STATE_TMPDIR");
      expect(script).toContain(
        'export npm_config_cache="${OPERATOR_UPGRADE_SURVIVOR_NPM_CACHE:-$OPERATOR_UPGRADE_SURVIVOR_RUNTIME_ROOT/npm-cache}"',
      );
      ...
      expect(script).not.toContain('export TMPDIR="$OPERATOR_UPGRADE_SURVIVOR_ARTIFACT_ROOT/tmp"');
      expect(script).not.toContain('export npm_config_cache="$ARTIFACT_ROOT/npm-cache"');
      expect(script).not.toContain(
        'export npm_config_cache="$OPERATOR_UPGRADE_SURVIVOR_ARTIFACT_ROOT/npm-cache"',
      );
    }
  });
  ...
  it("lets upgrade survivor fixture registries resolve transitive public packages", () => {
    ...
    for (const script of [runner, publishedRunner]) {
      expect(script).toContain("OPERATOR_NPM_REGISTRY_UPSTREAM=https://registry.npmjs.org");
      expect(script).toContain("node scripts/e2e/lib/plugins/npm-registry-server.mjs");
    }
  });

  it("wraps package-backed scenario Operator CLI calls with the shared timeout helper", () => {
    const paths = [
    ...
    for (const path of paths) {
      ...
      expect(script, path).toContain("operator_e2e_enable_operator_cli_timeout");
    }
    expect(readFileSync(RELEASE_UPGRADE_USER_JOURNEY_SCENARIO_PATH, "utf8")).toContain(
      'operator_e2e_run_command node "$baseline_entry" onboard',
    );
  });
  ...
  it("keeps append-only mock E2E state under per-run scratch roots", () => {
    const scripts = [
      {
        ...
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/operator-release-typed-onboarding.XXXXXX")"',
        ...
        removed: [
          "/tmp/operator-release-typed-onboarding-openai.jsonl",
          "/tmp/operator-release-typed-onboarding-install.log",
          "/tmp/operator-release-typed-onboarding.log",
          "/tmp/operator-release-typed-onboarding-openai.log",
          "/tmp/operator-release-typed-onboarding-agent.log",
          'mktemp -d "/tmp/operator-release-typed-onboarding.XXXXXX"',
        ],
      },
      {
        path: RELEASE_USER_JOURNEY_SCENARIO_PATH,
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/operator-release-user-journey.XXXXXX")"',
        logDir: 'LOG_DIR="$scenario_tmp/logs"',
        requestLog: 'MOCK_REQUEST_LOG="$scenario_tmp/openai-requests.jsonl"',
        ...
        removed: [
          "/tmp/operator-release-user-journey-openai.jsonl",
          "/tmp/operator-release-user-journey-clickclack.json",
          "/tmp/operator-release-user-journey-install.log",
          "/tmp/operator-release-user-journey-onboard.log",
          "/tmp/operator-release-user-journey-agent.log",
          "/tmp/operator-release-user-journey-plugin-a-install-path.txt",
          "/tmp/operator-release-user-journey-plugin-a-source-path.txt",
          'mktemp -d "/tmp/operator-release-journey-plugin-a.XXXXXX"',
          'mktemp -d "/tmp/operator-release-journey-plugin-b.XXXXXX"',
        ],
      },
      {
        path: RELEASE_UPGRADE_USER_JOURNEY_SCENARIO_PATH,
        scratch:
          'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/operator-release-upgrade-user-journey.XXXXXX")"',
        logDir: 'LOG_DIR="$scenario_tmp/logs"',
        requestLog: 'MOCK_REQUEST_LOG="$scenario_tmp/openai-requests.jsonl"',
        extraState: 'CLICKCLACK_STATE="$scenario_tmp/clickclack.json"',
        expectedPaths: [
        ...
        removed: [
          "/tmp/operator-release-upgrade-user-journey-openai.jsonl",
          "/tmp/operator-release-upgrade-user-journey-clickclack.json",
          "/tmp/operator-release-upgrade-baseline-install.log",
          "/tmp/operator-release-upgrade-candidate-install.log",
          "/tmp/operator-release-upgrade-onboard.log",
          "/tmp/operator-release-upgrade-agent.log",
          'mktemp -d "/tmp/operator-release-upgrade-plugin.XXXXXX"',
        ],
      },
      {
        ...
        removed: ["/tmp/operator-mock-openai-requests.jsonl"],
      },
    ];
    ...
    for (const {
      ...
      expect(script, path).not.toMatch(/\/tmp\/operator-release-[\w-]+\.(?:log|json|err|txt)/u);
    }
  });
  ...
  it("keeps multi-node update Docker artifacts isolated by default", () => {
    ...
    expect(multiNode).toContain(
      'RUN_ID="${OPERATOR_MULTI_NODE_RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-$$}"',
    );
    expect(multiNode).toContain(
      'ARTIFACT_DIR="${OPERATOR_MULTI_NODE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/multi-node-update/$RUN_ID}"',
    );
    expect(multiNode).toContain('-v "$ARTIFACT_DIR:/tmp/artifacts"');
    expect(multiNode).not.toContain(
      'ARTIFACT_DIR="${OPERATOR_MULTI_NODE_ARTIFACT_DIR:-$ROOT_DIR/.artifacts/multi-node-update}"',
    );
  });

  it("reuses the shared bare image for multi-node update targeted runs", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-multi-node-shared-image-"));

    try {
      const rootDir = process.cwd();
      mkdirSync(join(workDir, "bin"));
      writeFileSync(join(workDir, "operator-current.tgz"), "fake package");
      writeFileSync(
        join(workDir, "bin", "docker"),
        `#!/usr/bin/env bash
printf "%s\\n" "$*" >>"$TMPDIR/docker-seen"
...
export PATH="$TMPDIR/bin:$PATH"
export OPERATOR_SKIP_DOCKER_BUILD=1
export OPERATOR_DOCKER_E2E_IMAGE=shared-bare
export OPERATOR_CURRENT_PACKAGE_TGZ="$TMPDIR/operator-current.tgz"
export OPERATOR_MULTI_NODE_ARTIFACT_DIR="$TMPDIR/artifacts"
...
if grep -Fq 'operator-multi-node-update-e2e' "$TMPDIR/docker-seen"; then
...
`;
  ...
  it("bounds upgrade survivor foreground Operator CLI calls", () => {
    const runner = readFileSync(UPGRADE_SURVIVOR_DOCKER_E2E_PATH, "utf8");
    const publishedRunner = readFileSync(UPGRADE_SURVIVOR_RUN_SCRIPT, "utf8");
    const updateRestartAuth = readFileSync(UPGRADE_SURVIVOR_UPDATE_RESTART_AUTH_PATH, "utf8");

    expect(runner).toContain('source "$ROOT_DIR/scripts/lib/operator-e2e-instance.sh"');
    expect(runner).toContain(
      'START_BUDGET_SECONDS="$(operator_e2e_read_positive_int_env OPERATOR_UPGRADE_SURVIVOR_START_BUDGET_SECONDS 90)"',
    );
    expect(runner).toContain(
      'STATUS_BUDGET_SECONDS="$(operator_e2e_read_positive_int_env OPERATOR_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS 30)"',
    );
    expect(runner).toContain(
      '-e OPERATOR_UPGRADE_SURVIVOR_START_BUDGET_SECONDS="$START_BUDGET_SECONDS"',
    );
    expect(runner).toContain(
      '-e OPERATOR_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS="$STATUS_BUDGET_SECONDS"',
    );
    expect(runner).toContain(
      'START_BUDGET="$(operator_e2e_read_positive_int_env OPERATOR_UPGRADE_SURVIVOR_START_BUDGET_SECONDS 90)"',
    );
    expect(runner).toContain(
      'STATUS_BUDGET="$(operator_e2e_read_positive_int_env OPERATOR_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS 30)"',
    );
    expect(runner).toContain(
      'COMMAND_TIMEOUT="${OPERATOR_UPGRADE_SURVIVOR_COMMAND_TIMEOUT:-900s}"',
    );
    expect(runner).toContain('-e OPERATOR_UPGRADE_SURVIVOR_COMMAND_TIMEOUT="$COMMAND_TIMEOUT"');
    expect(runner).toContain(
      'command_timeout="${OPERATOR_UPGRADE_SURVIVOR_COMMAND_TIMEOUT:-900s}"',
    );
    expect(runner).toContain(
      'operator_e2e_maybe_timeout "$command_timeout" env -u OPERATOR_GATEWAY_TOKEN',
    );
    expect(runner).toContain(
      'operator_e2e_maybe_timeout "$command_timeout" operator doctor --fix --non-interactive',
    );
    expect(runner).toContain(
      'operator_e2e_maybe_timeout "$command_timeout" operator config validate',
    );
    expect(runner).toContain(
      'operator_e2e_maybe_timeout "$command_timeout" operator gateway status',
    );
    expect(runner).toContain(
      'operator gateway --port "$PORT" --bind loopback --allow-unconfigured',
    );
    expect(runner).toContain(
      'PROBE_TIMEOUT_MS="$(operator_e2e_read_nonnegative_int_env OPERATOR_UPGRADE_SURVIVOR_PROBE_TIMEOUT_MS 60000)"',
    );
    expect(runner).toContain(
      "operator_e2e_read_positive_int_env OPERATOR_UPGRADE_SURVIVOR_PROBE_ATTEMPT_TIMEOUT_MS 5000",
    );
    expect(runner).toContain(
      "operator_e2e_read_positive_int_env OPERATOR_UPGRADE_SURVIVOR_PROBE_MAX_BODY_BYTES 1048576",
    );
    expect(runner).toContain('-e OPERATOR_UPGRADE_SURVIVOR_PROBE_TIMEOUT_MS="$PROBE_TIMEOUT_MS"');
    expect(runner).toContain(
      '-e OPERATOR_UPGRADE_SURVIVOR_PROBE_ATTEMPT_TIMEOUT_MS="$PROBE_ATTEMPT_TIMEOUT_MS"',
    );
    expect(runner).toContain(
      '-e OPERATOR_UPGRADE_SURVIVOR_PROBE_MAX_BODY_BYTES="$PROBE_MAX_BODY_BYTES"',
    );
    expect(runner).toContain("readyz_probe_args=(");
    expect(runner).toContain(
      'readyz_probe_args+=(--allow-failing "$OPERATOR_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING")',
    );
    ...
    expect(runner).toContain("OPERATOR_UPGRADE_SURVIVOR_READYZ_ALLOW_FAILING");
    expect(runner).toContain("OPERATOR_UPGRADE_SURVIVOR_READYZ_ALLOW_DEGRADED");

    expect(publishedRunner).toContain(
      'COMMAND_TIMEOUT="${OPERATOR_UPGRADE_SURVIVOR_COMMAND_TIMEOUT:-900s}"',
    );
    expect(publishedRunner).toContain(
      'budget="$(operator_e2e_read_positive_int_env OPERATOR_UPGRADE_SURVIVOR_START_BUDGET_SECONDS 90)"',
    );
    expect(publishedRunner).toContain(
      'budget="$(operator_e2e_read_positive_int_env OPERATOR_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS 30)"',
    );
    expect(publishedRunner).toContain(
      'operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" env -u OPERATOR_GATEWAY_TOKEN',
    );
    expect(publishedRunner).toContain(
      'operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" operator --version',
    );
    expect(publishedRunner).toContain(
      'operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" operator config validate >"$BASELINE_CONFIG_VALIDATE_LOG"',
    );
    expect(publishedRunner).toContain(
      'operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${update_env[@]}" operator',
    );
    expect(publishedRunner).toContain(
      'operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" operator',
    );
    expect(publishedRunner).toContain(
      'operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" operator doctor --fix --non-interactive',
    );
    ...
    expect(publishedRunner).toContain('operator gateway --port "$port" --bind loopback');
    expect(publishedRunner).toContain("start_gateway legacy-ready-log-ok");
    expect(publishedRunner).toContain(
      'operator_e2e_wait_gateway_ready "$gateway_pid" "$GATEWAY_LOG" 360 "$port" "${1:-strict}"',
    );
    ...
    expect(updateRestartAuth).toContain('operator gateway --port "$port" --bind loopback');
    expect(updateRestartAuth).toContain(
  ...
  it.each([
    ["start budget", "OPERATOR_UPGRADE_SURVIVOR_START_BUDGET_SECONDS", "90s"],
    ["status budget", "OPERATOR_UPGRADE_SURVIVOR_STATUS_BUDGET_SECONDS", "30s"],
    ["probe timeout", "OPERATOR_UPGRADE_SURVIVOR_PROBE_TIMEOUT_MS", "soon"],
    ["probe attempt timeout", "OPERATOR_UPGRADE_SURVIVOR_PROBE_ATTEMPT_TIMEOUT_MS", "0"],
    ["probe body cap", "OPERATOR_UPGRADE_SURVIVOR_PROBE_MAX_BODY_BYTES", "64bytes"],
    ...
    const result = spawnSync("bash", [UPGRADE_SURVIVOR_DOCKER_E2E_PATH], {
      ...
      env: {
        ...process.env,
        OPERATOR_UPGRADE_SURVIVOR_E2E_SKIP_BUILD: "1",
        [envName]: value,
      },
    });
  ...
  it("bounds upgrade survivor failure log diagnostics", () => {
    ...
    expect(runner).toContain("operator_e2e_print_log /tmp/operator-upgrade-survivor-update.err");
    expect(runner).toContain("operator_e2e_print_log /tmp/operator-upgrade-survivor-update.json");
    expect(runner).toContain("operator_e2e_print_log /tmp/operator-upgrade-survivor-doctor.log");
    expect(runner).toContain("operator_e2e_print_log /tmp/operator-upgrade-survivor-status.err");
    expect(runner).toContain("operator_e2e_print_log /tmp/operator-upgrade-survivor-status.json");
    expect(runner).toContain('operator_e2e_print_log "$GATEWAY_LOG"');
    expect(runner).toContain('operator_e2e_print_log "$SYSTEMCTL_SHIM_DAEMON_LOG"');
    expect(runner).toContain('operator_e2e_print_log "$log_file"');
    expect(runner).not.toContain("cat /tmp/operator-upgrade-survivor-update.err");
    expect(runner).not.toContain("cat /tmp/operator-upgrade-survivor-update.json");
    expect(runner).not.toContain("cat /tmp/operator-upgrade-survivor-doctor.log");
    expect(runner).not.toContain("cat /tmp/operator-upgrade-survivor-status.err");
    expect(runner).not.toContain("cat /tmp/operator-upgrade-survivor-status.json");
    ...
    expect(publishedRunner).toContain('operator_e2e_print_log "$BASELINE_INSTALL_LOG"');
    expect(publishedRunner).toContain('operator_e2e_print_log "$BASELINE_CONFIG_VALIDATE_LOG"');
    expect(publishedRunner).toContain('operator_e2e_print_log "$BASELINE_SERVICE_INSTALL_ERR"');
    expect(publishedRunner).toContain('operator_e2e_print_log "$BASELINE_SERVICE_INSTALL_JSON"');
    expect(publishedRunner).toContain('operator_e2e_print_log "$UPDATE_ERR"');
    expect(publishedRunner).toContain('operator_e2e_print_log "$UPDATE_JSON"');
    expect(publishedRunner).toContain('operator_e2e_print_log "$DOCTOR_LOG"');
    expect(publishedRunner).toContain('operator_e2e_print_log "$GATEWAY_LOG"');
    expect(publishedRunner).toContain('operator_e2e_print_log "$STATUS_ERR"');
    expect(publishedRunner).toContain('operator_e2e_print_log "$STATUS_JSON"');
    expect(publishedRunner).toContain('operator_e2e_print_log "$log_file"');
  ...
  it("keeps the harness run wrapper available with pre-sourced Docker command helpers", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-package-helper-guard-"));
...
`;
  ...
  it("forwards harness stdin to backgrounded Docker runs", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-harness-stdin-"));
...
`;
  ...
  it("preserves caller-owned file descriptors around harness runs", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-harness-fd-"));
    try {
      const rootDir = process.cwd();
...
`;
  ...
  it("bounds Codex npm plugin live assertion file and transcript reads", () => {
    ...
    expect(assertions).toContain("OPERATOR_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES");
    expect(assertions).toContain("OPERATOR_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES");
    expect(assertions).toContain("OPERATOR_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES");
    expect(assertions).toContain("OPERATOR_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES");
    expect(assertions).toContain("OPERATOR_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES");
    expect(assertions).toContain("const AGENT_TURN_TIMEOUT_SECONDS = readPositiveIntEnv(");
    expect(assertions).toContain('"OPERATOR_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS"');
    ...
    expect(runner).toContain("docker_e2e_print_log /tmp/operator-codex-plugin-pack.log");
    expect(runner).not.toContain("cat /tmp/operator-codex-plugin-pack.log");
    expect(assertions).toContain(
      'readTextFileTail(\n        "/tmp/operator-codex-agent-after-uninstall.err",',
    );
    expect(runner).toContain('assert-agent-error "$post_uninstall_status"');
    expect(runner).not.toContain("tail -n 120 /tmp/operator-codex-agent-after-uninstall.err");
    expect(runner).not.toContain("cat /tmp/operator-codex-agent-after-uninstall.err");
    const earlyAgentTimeoutEnvIndex = runner.indexOf(
      "docker_e2e_read_positive_int_env OPERATOR_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS 420",
    );
    const profileSourceIndex = runner.indexOf('source "$PROFILE_FILE"');
    const finalAgentTimeoutEnvIndex = runner.lastIndexOf(
      "docker_e2e_read_positive_int_env OPERATOR_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS",
    );
    ...
    expect(runner).toContain(
      'docker_e2e_read_positive_int_env OPERATOR_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS "$AGENT_TURN_TIMEOUT_SECONDS"',
    );
    expect(runner).toContain(
      'COMMAND_TIMEOUT="${OPERATOR_E2E_COMMAND_TIMEOUT:-$((10#$AGENT_TURN_TIMEOUT_SECONDS + 60))s}"',
    );
    expect(runner).toContain(
      '-e "OPERATOR_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS"',
    );
    expect(runner).toContain('-e "OPERATOR_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT"');
    expect(runner).toContain(
      'AGENT_TURN_TIMEOUT_SECONDS="${OPERATOR_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS:-420}"',
    );
  ...
  it.each([
    ...
    [
      ...
      NPM_ONBOARD_CHANNEL_AGENT_DOCKER_E2E_PATH,
      "OPERATOR_NPM_ONBOARD_JSON_ARTIFACT_MAX_BYTES",
      "64kb",
    ],
    [
      "plugins",
      PLUGINS_DOCKER_E2E_PATH,
      "OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS",
      "soon",
    ],
    [
  ...
  it("forwards package assertion env limits into Docker runners", () => {
    const expectations = [
      [
        CODEX_NPM_PLUGIN_LIVE_DOCKER_E2E_PATH,
        [
          ["OPERATOR_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES", "1048576"],
          ["OPERATOR_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES", "65536"],
          ["OPERATOR_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES", "64"],
          ["OPERATOR_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES", "4096"],
          ["OPERATOR_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES", "2097152"],
          ["OPERATOR_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS", "420"],
        ],
      ],
      [
        NPM_ONBOARD_CHANNEL_AGENT_DOCKER_E2E_PATH,
        [
          ["OPERATOR_NPM_ONBOARD_JSON_ARTIFACT_MAX_BYTES", "1048576"],
          ["OPERATOR_NPM_ONBOARD_STATUS_TEXT_MAX_BYTES", "1048576"],
        ],
      ],
      [
        PLUGINS_DOCKER_E2E_PATH,
        [
          ["OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES", "1048576"],
          ["OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS", "30000"],
        ],
      ],
      [
        RELEASE_USER_JOURNEY_DOCKER_E2E_PATH,
        [
          ["OPERATOR_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS", "5000"],
          ["OPERATOR_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES", "1048576"],
        ],
      ],
    ] as const;
  ...
  it("gives Codex on-demand package installs enough time to reach Codex assertions", () => {
    ...
    expect(runner).toContain(
      'export OPERATOR_E2E_NPM_INSTALL_TIMEOUT="${OPERATOR_E2E_NPM_INSTALL_TIMEOUT:-1200s}"',
    );
  });
  ...
  it("threads the live plugin tool output cap into the Docker harness", () => {
    ...
    const earlyTimeoutEnvIndex = runner.indexOf(
      "operator_e2e_read_positive_int_env OPERATOR_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS 300",
    );
    const profileSourceIndex = runner.indexOf('source "$PROFILE_FILE"');
    const finalTimeoutEnvIndex = runner.lastIndexOf(
      "operator_e2e_read_positive_int_env OPERATOR_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS",
    );
    const dockerBuildIndex = runner.indexOf("docker_e2e_build_or_reuse");
    ...
    expect(runner).toContain(
      'AGENT_TURN_TIMEOUT_SECONDS="$(operator_e2e_read_positive_int_env OPERATOR_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS 300)"',
    );
    expect(runner).toContain(
      'AGENT_TURN_TIMEOUT_SECONDS="$(operator_e2e_read_positive_int_env OPERATOR_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS "$AGENT_TURN_TIMEOUT_SECONDS")"',
    );
    expect(runner).toContain(
      'COMMAND_TIMEOUT="${OPERATOR_E2E_COMMAND_TIMEOUT:-$((10#$AGENT_TURN_TIMEOUT_SECONDS + 60))s}"',
    );
    expect(runner).toContain(
      'AGENT_OUTPUT_MAX_BYTES="$(operator_e2e_read_positive_int_env OPERATOR_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES 1048576)"',
    );
    expect(runner).toContain(
      'AGENT_OUTPUT_DUMP_BYTES="$(operator_e2e_read_nonnegative_int_env OPERATOR_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES 16384)"',
    );
    expect(runner).toContain(
      'SESSION_SCAN_MAX_ENTRIES="$(operator_e2e_read_positive_int_env OPERATOR_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES 50000)"',
    );
    expect(runner).toContain(
      '-e "OPERATOR_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES=$AGENT_OUTPUT_DUMP_BYTES"',
    );
    expect(runner).toContain(
      '-e "OPERATOR_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES=$AGENT_OUTPUT_MAX_BYTES"',
    );
    expect(runner).toContain(
      '-e "OPERATOR_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES=$SESSION_SCAN_MAX_ENTRIES"',
    );
    expect(runner).toContain('-e "OPERATOR_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT"');
    expect(runner).not.toContain(
      'AGENT_OUTPUT_MAX_BYTES="${OPERATOR_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES:-1048576}"',
    );
    expect(runner).toContain("OPERATOR_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES");
    expect(runner).toContain('tail -c "$agent_output_dump_bytes" /tmp/operator-agent.json');
    const dumpLogsStart = runner.indexOf("operator_e2e_dump_logs \\");
    const dumpLogsEnd = runner.indexOf("\n}", dumpLogsStart);
    expect(runner.slice(dumpLogsStart, dumpLogsEnd)).not.toContain("/tmp/operator-agent.json");
  });

  it.each([
    ["timeout", "OPERATOR_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS", "1e3"],
    ["output cap", "OPERATOR_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES", "64kb"],
    ["output dump cap", "OPERATOR_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_DUMP_BYTES", "64kb"],
    ["session scan cap", "OPERATOR_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES", "0"],
  ])(
    ...
    (_label, envName, value) => {
      const result = spawnSync("bash", [LIVE_PLUGIN_TOOL_DOCKER_E2E_PATH], {
        ...
        env: {
          ...process.env,
          OPERATOR_LIVE_PLUGIN_TOOL_HOST_BUILD: "0",
          OPERATOR_SKIP_DOCKER_BUILD: "1",
          [envName]: value,
        },
      });
  ...
  it("keeps live plugin tool npm pack tarball paths inside the fixture directory", () => {
    ...
    expect(runner).toContain("/tmp/operator-live-plugin-tool-pack.log");
    ...
    expect(runner).toContain("operator_e2e_dump_logs /tmp/operator-live-plugin-tool-pack.log");
  ...
  it("prints plugins Docker E2E logs on successful runs", () => {
    ...
    expect(helper).toContain("OPERATOR_DOCKER_E2E_LOG_HEARTBEAT_SECONDS");
    expect(readFileSync("scripts/lib/docker-e2e-logs.sh", "utf8")).toContain(
      "OPERATOR_DOCKER_E2E_LOG_PRINT_BYTES",
    );
    expect(runner).toContain(
      "docker_e2e_read_positive_int_env OPERATOR_DOCKER_E2E_LOG_PRINT_BYTES 65536",
    );
    expect(runner).toContain(
      '-e "OPERATOR_DOCKER_E2E_LOG_PRINT_BYTES=$OPERATOR_DOCKER_E2E_LOG_PRINT_BYTES"',
    );
  ...
  it("bounds printed Docker E2E logs to the configured tail", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-e2e-log-print-tail-"));
...
TMPDIR=${shellQuote(workDir)}
export ROOT_DIR TMPDIR
export OPERATOR_DOCKER_E2E_LOG_PRINT_BYTES=64
...
`;
  ...
  it.each([
    ["printed log bytes", "OPERATOR_DOCKER_E2E_LOG_PRINT_BYTES", "64kb"],
    ["heartbeat termination grace", "OPERATOR_DOCKER_E2E_HEARTBEAT_TERM_GRACE_SECONDS", "soon"],
  ])("rejects invalid Docker E2E %s before setup", (_label, envName, value) => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-e2e-log-invalid-"));
...
`;
  ...
  it("rejects invalid Docker E2E log heartbeat env before harness setup", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-e2e-log-heartbeat-invalid-"));
...
TMPDIR=${shellQuote(workDir)}
export ROOT_DIR TMPDIR
export OPERATOR_DOCKER_E2E_LOG_HEARTBEAT_SECONDS=1e3
...
`;
      ...
      expect(result.status).toBe(2);
      expect(result.stderr).toContain("invalid OPERATOR_DOCKER_E2E_LOG_HEARTBEAT_SECONDS: 1e3");
      expect(result.stdout).toBe("");
    } finally {
      rmSync(workDir, { recursive: true, force: true });
    }
  ...
  it("prints heartbeat progress for long successful Docker E2E log captures", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-e2e-log-heartbeat-"));
...
`;
  ...
  it("cleans the heartbeat command when the wrapper is terminated", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-e2e-log-term-cleanup-"));
...
TMPDIR=${shellQuote(workDir)}
export ROOT_DIR TMPDIR
export OPERATOR_DOCKER_E2E_HEARTBEAT_TERM_GRACE_SECONDS=1
...
`;
  ...
  it("cleans harness containers when heartbeat-wrapped Docker runs are terminated", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-e2e-harness-term-cleanup-"));
...
`;
  ...
  it("does not delay fast successful Docker E2E log captures until the next heartbeat", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-e2e-log-fast-heartbeat-"));
...
`;
  ...
  it("normalizes zero-padded Docker E2E stats heartbeat intervals", () => {
    const workDir = mkdtempSync(join(tmpdir(), "operator-docker-e2e-stats-zero-heartbeat-"));
...
`;
  ...
  it("keeps onboarding Docker E2E resource-guarded", () => {
    ...
    expect(runner).toContain("OPERATOR_ONBOARD_MAX_MEMORY_MIB");
    expect(runner).toContain("OPERATOR_ONBOARD_MAX_CPU_PERCENT");
    expect(runner).toContain(
      'COMMAND_TIMEOUT="${OPERATOR_ONBOARD_COMMAND_TIMEOUT:-${OPERATOR_E2E_COMMAND_TIMEOUT:-300s}}"',
    );
    expect(runner).toContain(
      'GATEWAY_WAIT_ATTEMPTS="$(operator_e2e_read_positive_int_env OPERATOR_ONBOARD_GATEWAY_WAIT_ATTEMPTS 20)"',
    );
    expect(runner).toContain(
      'GATEWAY_WAIT_INTERVAL_S="$(docker_e2e_read_nonnegative_decimal_env OPERATOR_ONBOARD_GATEWAY_WAIT_INTERVAL_S 1)"',
    );
    expect(runner).toContain('-e "OPERATOR_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT"');
    expect(runner).toContain('-e "OPERATOR_ONBOARD_GATEWAY_WAIT_ATTEMPTS=$GATEWAY_WAIT_ATTEMPTS"');
    expect(runner).toContain(
      '-e "OPERATOR_ONBOARD_GATEWAY_WAIT_INTERVAL_S=$GATEWAY_WAIT_INTERVAL_S"',
    );
  ...
  it("cleans resource-sampled Docker E2E temp logs on every exit path", () => {
    for (const { path, label } of [
      ...
      expect(runner, path).toContain('DOCKER_RUN_TIMEOUT="${OPERATOR_');
  ...
  it("keeps captured Docker E2E run log replay bounded", () => {
    ...
    expect(pluginBinding).not.toContain("process.env.OPERATOR_DOCKER_E2E_LOG_PRINT_BYTES");
  ...
  it("keeps Open WebUI Docker E2E resource-guarded", () => {
    ...
    expect(runner).toContain(
      'validate_positive_int OPERATOR_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS "$PROVIDER_TIMEOUT_SECONDS"',
    );
    expect(runner).toContain(
      'validate_positive_int OPERATOR_OPENWEBUI_FETCH_TIMEOUT_MS "$PROBE_FETCH_TIMEOUT_MS"',
    );
    expect(runner).toContain("docker_e2e_read_tcp_port_env OPERATOR_OPENWEBUI_GATEWAY_PORT 18789");
    expect(runner).toContain("docker_e2e_read_tcp_port_env OPERATOR_OPENWEBUI_PORT 8080");
    expect(runner).toContain("OPERATOR_OPENWEBUI_MAX_MEMORY_MIB");
    expect(runner).toContain("OPERATOR_OPENWEBUI_MAX_CPU_PERCENT");
  ...
  it.each([
    ["gateway", "OPERATOR_OPENWEBUI_GATEWAY_PORT", "1e3"],
    ["webui", "OPERATOR_OPENWEBUI_PORT", "65536"],
  ...
  it.each([
    ["provider", "OPERATOR_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS", "300s"],
    ["fetch", "OPERATOR_OPENWEBUI_FETCH_TIMEOUT_MS", "8000ms"],
  ])(
  ...
  it("accepts decimal Open WebUI Docker numeric inputs with leading zeroes", () => {
    const result = spawnSync("bash", [OPENWEBUI_DOCKER_E2E_PATH], {
      ...
      env: {
        ...process.env,
        OPERATOR_OPENWEBUI_FETCH_TIMEOUT_MS: "09000",
        OPERATOR_OPENWEBUI_GATEWAY_PORT: "018789",
        OPERATOR_OPENWEBUI_PORT: "08080",
        OPERATOR_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS: "08",
      },
    });
  ...
  it.each([
    [MCP_CODE_MODE_GATEWAY_DOCKER_E2E_PATH, "OPERATOR_MCP_CODE_MODE_GATEWAY_PORT", "1e3"],
    [MCP_CODE_MODE_GATEWAY_DOCKER_E2E_PATH, "OPERATOR_MCP_CODE_MODE_MOCK_PORT", "65536"],
    [MCP_CODE_MODE_GATEWAY_LIVE_DOCKER_E2E_PATH, "OPERATOR_MCP_CODE_MODE_LIVE_GATEWAY_PORT", "0"],
    [CODEX_MEDIA_PATH_DOCKER_E2E_PATH, "OPERATOR_CODEX_MEDIA_PATH_PORT", "18790tcp"],
    [OPENAI_CHAT_TOOLS_DOCKER_E2E_PATH, "OPERATOR_OPENAI_CHAT_TOOLS_PORT", "0"],
    [OPENAI_WEB_SEARCH_MINIMAL_E2E_PATH, "OPERATOR_OPENAI_WEB_SEARCH_MINIMAL_PORT", "18789tcp"],
  ...
  it.each([
    ["timeout", "OPERATOR_CODEX_MEDIA_PATH_TIMEOUT_SECONDS", "180s"],
    ["log tail cap", "OPERATOR_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES", "64kb"],
  ...
  it("forwards Codex media path client limits into Docker", () => {
    ...
    expect(runner).toContain(
      'LOG_TAIL_MAX_BYTES="$(docker_e2e_read_positive_int_env OPERATOR_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES 2097152)"',
    );
    expect(runner).toContain(
      '-e "OPERATOR_CODEX_MEDIA_PATH_LOG_TAIL_MAX_BYTES=$LOG_TAIL_MAX_BYTES"',
    );
  });

  it.each([
    [MCP_CODE_MODE_GATEWAY_DOCKER_E2E_PATH, "OPERATOR_MCP_CODE_MODE_CLIENT_TIMEOUT_MS", "1e3"],
    [
      MCP_CODE_MODE_GATEWAY_DOCKER_E2E_PATH,
      "OPERATOR_MCP_CODE_MODE_CLIENT_BODY_MAX_BYTES",
      "64bytes",
    ],
    [MCP_CODE_MODE_GATEWAY_LIVE_DOCKER_E2E_PATH, "OPERATOR_MCP_CODE_MODE_CLIENT_TIMEOUT_MS", "1e3"],
    [
  ...
  it.each([MCP_CODE_MODE_GATEWAY_DOCKER_E2E_PATH, MCP_CODE_MODE_GATEWAY_LIVE_DOCKER_E2E_PATH])(
    ...
    (scriptPath) => {
      ...
      expect(runner).toContain(
        'CLIENT_TIMEOUT_MS="$(docker_e2e_read_positive_int_env OPERATOR_MCP_CODE_MODE_CLIENT_TIMEOUT_MS 300000)"',
      );
      expect(runner).toContain(
        'CLIENT_BODY_MAX_BYTES="$(docker_e2e_read_positive_int_env OPERATOR_MCP_CODE_MODE_CLIENT_BODY_MAX_BYTES 1048576)"',
      );
      expect(runner).toContain('-e "OPERATOR_MCP_CODE_MODE_CLIENT_TIMEOUT_MS=$CLIENT_TIMEOUT_MS"');
      expect(runner).toContain(
        '-e "OPERATOR_MCP_CODE_MODE_CLIENT_BODY_MAX_BYTES=$CLIENT_BODY_MAX_BYTES"',
      );
    },
  );

  it.each([
    ["timeout", "OPERATOR_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS", "180s"],
    ["body cap", "OPERATOR_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES", "64kb"],
  ...
  it("forwards every OpenAI chat tools runtime env knob into Docker", () => {
    ...
    const consumed = new Set(
      [...`${client}\n${writer}`.matchAll(/["`](OPERATOR_OPENAI_CHAT_TOOLS_[A-Z0-9_]+)["`]/gu)].map(
        (match) => match[1],
      ),
    );
    const forwarded = new Set(
      [...runner.matchAll(/-e\s+"(OPERATOR_OPENAI_CHAT_TOOLS_[A-Z0-9_]+)=/gu)].map(
        (match) => match[1],
      ),
    );
  ...
  it("forwards every kitchen-sink RPC runtime env knob into Docker", () => {
    ...
    const consumed = new Set(
      [...walk.matchAll(/\b(?:env|process\.env)\.(OPERATOR_KITCHEN_SINK_[A-Z0-9_]+)/gu)].map(
        (match) => match[1],
      ),
    );
    const forwarded = new Set(
      [...runner.matchAll(/\b(OPERATOR_KITCHEN_SINK_[A-Z0-9_]+)\b/gu)].map((match) => match[1]),
    );
    const missing = [...consumed].filter((envName) => !forwarded.has(envName)).toSorted();
  ...
  it("keeps the kitchen-sink RPC Docker watchdog above the internal walk budgets", () => {
    ...
    expect(runner).toContain(
      'DOCKER_RUN_TIMEOUT="${OPERATOR_KITCHEN_SINK_RPC_DOCKER_RUN_TIMEOUT:-1500s}"',
    );
  });
  ...
  it("bounds kitchen-sink plugin CLI commands inside the Docker sweep", () => {
    ...
    expect(runner).toContain(
      'KITCHEN_SINK_CLI_TIMEOUT="${OPERATOR_KITCHEN_SINK_PLUGIN_CLI_TIMEOUT:-${KITCHEN_SINK_CLI_TIMEOUT:-180s}}"',
    );
    expect(runner).toContain(
      "docker_e2e_read_positive_int_env OPERATOR_DOCKER_E2E_LOG_PRINT_BYTES 65536",
    );
    expect(runner).toContain(
      "docker_e2e_read_positive_int_env OPERATOR_CLAWHUB_FIXTURE_WAIT_ATTEMPTS 600",
    );
    expect(runner).toContain(
      '-e "OPERATOR_CLAWHUB_FIXTURE_WAIT_ATTEMPTS=$CLAW_HUB_FIXTURE_WAIT_ATTEMPTS"',
    );
    expect(runner).toContain(
      '-e "OPERATOR_DOCKER_E2E_LOG_PRINT_BYTES=$OPERATOR_DOCKER_E2E_LOG_PRINT_BYTES"',
    );
    ...
    expect(sweep).toContain("run_kitchen_sink_operator_logged()");
    expect(sweep).toContain("run_kitchen_sink_operator_capture()");
    expect(sweep).toContain(
      'operator_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$OPERATOR_ENTRY" "$@" >"$log_file" 2>&1',
    );
    expect(sweep).toContain('local log_file="${KITCHEN_SINK_TMP_DIR}/${safe_label}.log"');
    for (const line of sweep.split("\n")) {
      if (!line.includes('node "$OPERATOR_ENTRY" plugins')) {
        continue;
      }

      expect(line).toContain("operator_e2e_maybe_timeout");
    }
  });
  ...
  it("routes named Docker E2E container cleanup through the timeout-aware helper", () => {
    ...
    expect(packageRunner).toContain(
      'DOCKER_RUN_TIMEOUT="${OPERATOR_DOCKER_PACKAGE_INSTALL_RUN_TIMEOUT:-120s}"',
    );
    expect(packageRunner).toContain(
  ...
  it("proves gateway suspension across a same-container process restart", () => {
    ...
    expect(runner).toContain("operator_e2e_probe_http http://127.0.0.1:$PORT/readyz ok 400");
  ...
  it.each([
    ["connect", "OPERATOR_GATEWAY_NETWORK_CLIENT_CONNECT_TIMEOUT_MS", "100ms"],
    ["ready", "OPERATOR_GATEWAY_NETWORK_CONNECT_READY_TIMEOUT_MS", "1e3"],
  ])(
  ...
  it("forwards gateway network client timeout env into the Docker client", () => {
    ...
    expect(runner).toContain(
      "docker_e2e_read_positive_int_env OPERATOR_GATEWAY_NETWORK_CLIENT_CONNECT_TIMEOUT_MS 80000",
    );
    expect(runner).toContain(
      "docker_e2e_read_positive_int_env OPERATOR_GATEWAY_NETWORK_CONNECT_READY_TIMEOUT_MS 80000",
    );
    expect(runner).toContain(
      '-e "OPERATOR_GATEWAY_NETWORK_CLIENT_CONNECT_TIMEOUT_MS=$CLIENT_CONNECT_TIMEOUT_MS"',
    );
    expect(runner).toContain(
      '-e "OPERATOR_GATEWAY_NETWORK_CONNECT_READY_TIMEOUT_MS=$CONNECT_READY_TIMEOUT_MS"',
    );
  ...
  it("requires TCP readiness for the gateway network runner", () => {
    ...
    expect(runner).toContain("operator_e2e_probe_tcp 127.0.0.1 $PORT");
    expect(runner).not.toMatch(/operator_e2e_probe_tcp[^\n]*\|\|[^\n]*gateway-net-e2e\.log/u);
  });
  ...
  it("preserves pnpm lookup paths for scheduled Docker child lanes", () => {
    ...
    expect(scheduler).toContain("OPERATOR_DOCKER_ALL_PNPM_COMMAND");
  });
  ...
  it("runs release installer E2E against the npm beta tag", () => {
    ...
    expect(scenarios).toContain(
      '"OPERATOR_INSTALL_TAG=beta OPERATOR_E2E_MODELS=openai OPERATOR_INSTALL_E2E_IMAGE=operator-install-e2e-openai:local OPERATOR_INSTALL_E2E_AGENT_TOOL_SMOKE=0 OPERATOR_INSTALL_E2E_OPENAI_MODEL=openai/gpt-5.4-mini OPERATOR_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS=120 OPERATOR_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS=120"',
    );
    expect(scenarios).toContain(
      '"OPERATOR_INSTALL_TAG=beta OPERATOR_E2E_MODELS=anthropic OPERATOR_INSTALL_E2E_IMAGE=operator-install-e2e-anthropic:local"',
    );
    expect(scenarios).toContain('"test-install-sh-e2e-docker.sh"');
    expect(scenarios).not.toContain("pnpm test:install:e2e");
    expect(scenarios).toContain(
      '"OPERATOR_OPENWEBUI_MODEL=openai/gpt-5.4-mini OPERATOR_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=300 OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui"',
    );
    expect(scenarios).not.toContain("OPENWEBUI_SMOKE_MODE=models");
    expect(openWebUiRunner).toContain(
      'SMOKE_MODE="${OPENWEBUI_SMOKE_MODE:-${OPERATOR_OPENWEBUI_SMOKE_MODE:-chat}}"',
    );
  ...
  it("times and parallelizes release installer E2E agent turns after gateway startup", () => {
    ...
    expect(runner).toContain(
      'AGENT_TURNS_PARALLEL="$(read_boolean_env OPERATOR_INSTALL_E2E_AGENT_TURNS_PARALLEL 1)"',
    );
    expect(runner).toContain(
      'AGENT_TOOL_SMOKE="$(read_boolean_env OPERATOR_INSTALL_E2E_AGENT_TOOL_SMOKE 1)"',
    );
    ...
    expect(wrapper).toContain("OPERATOR_INSTALL_E2E_AGENT_TURNS_PARALLEL");
    expect(wrapper).toContain("OPERATOR_INSTALL_E2E_AGENT_TOOL_SMOKE");
    expect(wrapper).toContain("OPERATOR_INSTALL_E2E_OPENAI_MODEL");
    expect(wrapper).toContain("OPERATOR_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS");
    expect(wrapper).toContain(
      "docker_e2e_read_positive_int_env OPERATOR_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS 300",
    );
    expect(wrapper).toContain(
      'docker_e2e_read_positive_int_env OPERATOR_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS "$AGENT_TURN_TIMEOUT_SECONDS"',
    );
    expect(wrapper).toContain(
      '-e OPERATOR_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS="$AGENT_TURN_TIMEOUT_SECONDS"',
    );
    expect(wrapper).toContain("OPERATOR_INSTALL_E2E_PROFILE_FILE");
    expect(wrapper).toContain("OPERATOR_PROFILE_FILE");
    expect(wrapper).toContain("OPERATOR_TESTBOX_PROFILE_FILE");
    ...
    expect(runner).toContain("OPERATOR_INSTALL_E2E_OPENAI_MODEL");
    expect(runner).toContain("OPERATOR_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS");
    expect(runner).toContain(
      'AGENT_TURN_TIMEOUT_SECONDS="$(read_positive_int_env OPERATOR_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS 300)"',
    );
  });
  ...
  it("keeps package acceptance plugin coverage offline-capable", () => {
    ...
    expect(scenarios).toContain("OPERATOR_PLUGINS_E2E_CLAWHUB=0");
  });
  ...
  it("fails the multi-node update probe on update or restart regressions", () => {
    ...
    expect(runner).toContain("OPERATOR_NO_RESPAWN=1");
  ...
  it("keeps a stalled multi-node health request inside the probe deadline", () => {
    ...
    const workDir = tempDirs.make("operator-multi-node-health-timeout-");
  ...
  it("caps package acceptance legacy compatibility at 2026.4.25", () => {
    ...
    expect(doctorScenario).toContain("OPERATOR_UPDATE_PARENT_ALLOWS_GATEWAY_SERVICE_REPAIR=1");
    ...
    expect(scripts.join("\n")).toContain("OPERATOR_PACKAGE_ACCEPTANCE_LEGACY_COMPAT");
    expect(scripts.join("\n")).toContain(
  ...
  it("routes doctor install switch commands through the E2E timeout helper", () => {
    ...
    expect(runner).toContain('NPM_INSTALL_TIMEOUT="${OPERATOR_E2E_NPM_INSTALL_TIMEOUT:-600s}"');
    expect(runner).toContain(
      'COMMAND_TIMEOUT="${OPERATOR_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"',
    );
    expect(runner).toContain('-e "OPERATOR_E2E_NPM_INSTALL_TIMEOUT=$NPM_INSTALL_TIMEOUT"');
    expect(runner).toContain('-e "OPERATOR_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT=$COMMAND_TIMEOUT"');
    expect(scenario).toContain(
      'command_timeout="${OPERATOR_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"',
    );
    expect(scenario).toContain(
      'operator_e2e_maybe_timeout "$command_timeout" bash -c "$install_cmd"',
    );
    expect(scenario).toContain(
      'operator_e2e_maybe_timeout "$command_timeout" bash -c "$doctor_cmd"',
    );
    expect(scenario).toContain(
      'operator_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" --force',
    );
    expect(scenario).toContain(
      'operator_e2e_maybe_timeout "$command_timeout" node "$git_cli" doctor --repair --force --yes',
    );
  ...
  it("bounds doctor install switch command log diagnostics", () => {
    ...
    expect(scenario).toContain('operator_e2e_print_log "$npm_log"');
    expect(scenario).toContain('operator_e2e_print_log "$install_log"');
    expect(scenario).toContain('operator_e2e_print_log "$doctor_log"');
    expect(scenario).toContain('operator_e2e_print_log "$reinstall_log"');
    expect(scenario).toContain('operator_e2e_print_log "$env_repair_log"');
    expect(scenario).toContain('operator_e2e_print_log "$clear_log"');
  ...
  it("prepares pnpm workspace package fixtures without package dependencies", () => {
    const root = mkdtempSync(join(tmpdir(), "operator-update-channel-fixture-"));
    try {
      mkdirSync(join(root, "patches"));
      writeFileSync(
        join(root, "package.json"),
        `${JSON.stringify({ name: "@gabrielvfonseca/operator", version: "2026.5.6", scripts: {} }, null, 2)}\n`,
        "utf8",
      );
      writeFileSync(
  ...
  it("keeps bundled plugin install/uninstall sweep chunkable", () => {
    ...
    const forwardedRuntimeEnv = [
      "OPERATOR_BUNDLED_PLUGIN_LIST_TIMEOUT_MS",
      "OPERATOR_BUNDLED_PLUGIN_LIST_MAX_BUFFER_BYTES",
      "OPERATOR_BUNDLED_PLUGIN_RUNTIME_OUTPUT_CHARS",
      "OPERATOR_BUNDLED_PLUGIN_RUNTIME_LOG_SCAN_BYTES",
      "OPERATOR_BUNDLED_PLUGIN_RUNTIME_GATEWAY_LOG_BYTES",
      "OPERATOR_BUNDLED_PLUGIN_RUNTIME_READY_MS",
      "OPERATOR_BUNDLED_PLUGIN_RUNTIME_RPC_MS",
      "OPERATOR_BUNDLED_PLUGIN_RUNTIME_RPC_READY_MS",
      "OPERATOR_BUNDLED_PLUGIN_RUNTIME_COMMAND_MS",
      "OPERATOR_BUNDLED_PLUGIN_RUNTIME_HTTP_MS",
      "OPERATOR_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_GRACE_MS",
      "OPERATOR_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_KILL_GRACE_MS",
      "OPERATOR_BUNDLED_PLUGIN_RUNTIME_WATCHDOG_MS",
    ] as const;

    expect(runner).toContain("OPERATOR_BUNDLED_PLUGIN_SWEEP_TOTAL");
    expect(runner).toContain("OPERATOR_BUNDLED_PLUGIN_SWEEP_INDEX");
    expect(runner).toContain("OPERATOR_BUNDLED_PLUGIN_SWEEP_COMMAND_TIMEOUT");
    ...
    expect(runner).toContain("OPERATOR_PLUGIN_LIFECYCLE_TRACE");
    for (const [envName, fallback] of [
      ["OPERATOR_BUNDLED_PLUGIN_LIST_TIMEOUT_MS", "30000"],
      ["OPERATOR_BUNDLED_PLUGIN_LIST_MAX_BUFFER_BYTES", "4194304"],
      ["OPERATOR_BUNDLED_PLUGIN_RUNTIME_OUTPUT_CHARS", "1048576"],
      ["OPERATOR_BUNDLED_PLUGIN_RUNTIME_LOG_SCAN_BYTES", "262144"],
      ["OPERATOR_BUNDLED_PLUGIN_RUNTIME_GATEWAY_LOG_BYTES", "16777216"],
      ["OPERATOR_BUNDLED_PLUGIN_RUNTIME_READY_MS", "900000"],
      ["OPERATOR_BUNDLED_PLUGIN_RUNTIME_RPC_MS", "60000"],
      ["OPERATOR_BUNDLED_PLUGIN_RUNTIME_RPC_READY_MS", "210000"],
      ["OPERATOR_BUNDLED_PLUGIN_RUNTIME_WATCHDOG_MS", "1000"],
      ["OPERATOR_BUNDLED_PLUGIN_RUNTIME_COMMAND_MS", "120000"],
      ["OPERATOR_BUNDLED_PLUGIN_RUNTIME_HTTP_MS", "5000"],
      ["OPERATOR_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_GRACE_MS", "10000"],
      ["OPERATOR_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_KILL_GRACE_MS", "1000"],
    ] as const) {
    ...
    expect(runner).toContain(
      "docker_e2e_read_tcp_port_env OPERATOR_BUNDLED_PLUGIN_RUNTIME_PORT_BASE 19000",
    );
    expect(runner).toContain('-e "OPERATOR_BUNDLED_PLUGIN_RUNTIME_PORT_BASE=$RUNTIME_PORT_BASE"');
    ...
    expect(probe).toContain('"operator.plugin.json"');
    expect(runtimeSmoke).toContain(
      'readPositiveIntEnv("OPERATOR_BUNDLED_PLUGIN_RUNTIME_READY_MS", 900000)',
    );
    expect(sweep).toContain("read -r plugin_id plugin_dir requires_config");
    expect(sweep).toContain('node "$OPERATOR_ENTRY" plugins install "$plugin_id"');
    expect(sweep).toContain('node "$OPERATOR_ENTRY" plugins uninstall "$plugin_id" --force');
  ...
  it.each([
    ["list timeout", "OPERATOR_BUNDLED_PLUGIN_LIST_TIMEOUT_MS", "100ms"],
    ["runtime port base", "OPERATOR_BUNDLED_PLUGIN_RUNTIME_PORT_BASE", "99999"],
    ["runtime log scan", "OPERATOR_BUNDLED_PLUGIN_RUNTIME_LOG_SCAN_BYTES", "64bytes"],
    ["runtime command timeout", "OPERATOR_BUNDLED_PLUGIN_RUNTIME_COMMAND_MS", "soon"],
    ["runtime teardown grace", "OPERATOR_BUNDLED_PLUGIN_RUNTIME_TEARDOWN_GRACE_MS", "0"],
  ])(
  ...
  it("passes installer tag env to bash, not curl", () => {
    ...
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | OPERATOR_BETA=1 bash');
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | OPERATOR_VERSION="$INSTALL_TAG" bash');
    expect(runner).not.toContain('OPERATOR_BETA=1 curl -fsSL "$INSTALL_URL" | bash');
    expect(runner).not.toContain(
      'OPERATOR_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash',
    );
  });
  ...
  it("bounds installer E2E session transcript tool scans", () => {
    ...
    expect(start).toBeGreaterThanOrEqual(0);
    expect(end).toBeGreaterThan(start);
    expect(helper).toContain("OPERATOR_INSTALL_E2E_SESSION_SCAN_BYTES");
    expect(helper).toContain("OPERATOR_INSTALL_E2E_SESSION_LINE_BYTES");
    expect(helper).toContain("OPERATOR_INSTALL_E2E_SESSION_SCAN_DEPTH");
    expect(helper).toContain("OPERATOR_INSTALL_E2E_SESSION_SCAN_NODES");
  ...
  it("exports SQLite-backed installer E2E sessions before scanning tools", () => {
    ...
    expect(helper).toContain('operator --profile "$profile" sessions export-trajectory');
    expect(helper).toContain('--session-key "agent:main:explicit:${session_id}"');
    expect(helper).toContain('--workspace "$export_workspace"');
    expect(helper).toContain(
      'jsonl="$export_workspace/.operator/trajectory-exports/scan/events.jsonl"',
    );
  ...
  it("keeps OpenAI web search smoke on one gateway agent connection", () => {
    ...
    expect(runner).toContain(
      'PORT="$(docker_e2e_read_tcp_port_env OPERATOR_OPENAI_WEB_SEARCH_MINIMAL_PORT 18789)"',
    );
    expect(runner).toContain('MOCK_PORT="443"');
    expect(runner).not.toContain("OPERATOR_OPENAI_WEB_SEARCH_MINIMAL_MOCK_PORT");
    ...
    expect(scenario).toContain(
      'operator_e2e_wait_mock_openai "$MOCK_PORT" 80 400 "https://api.openai.com:$MOCK_PORT"',
    );
  ...
  it("cleans OpenAI web search smoke processes through the E2E helpers", () => {
    ...
    expect(scenario).toContain('operator_e2e_terminate_gateways "${gateway_pid:-}"');
    expect(scenario).toContain('operator_e2e_stop_process "${mock_pid:-}"');
    expect(scenario).toContain(
      'gateway_pid="$(operator_e2e_start_gateway "$entry" "$PORT" "$GATEWAY_LOG")"',
    );
    expect(scenario).toContain(
      'operator_e2e_wait_mock_openai "$MOCK_PORT" 80 400 "https://api.openai.com:$MOCK_PORT"',
    );
  ...
  it("keeps OpenAI web search smoke logs isolated per run", () => {
    ...
    expect(scenario).toContain(
      'scenario_tmp="$(mktemp -d "${TMPDIR:-/tmp}/operator-openai-web-search-minimal.XXXXXX")"',
    );
    ...
    expect(scenario).toContain('operator_e2e_print_log "$file"');
    expect(scenario).toContain('rm -rf "$scenario_tmp"');
    expect(scenario).not.toContain("sed -n '1,260p'");
    expect(scenario).not.toContain("/tmp/operator-openai-web-search-minimal-requests.jsonl");
    expect(scenario).not.toContain("/tmp/operator-openai-web-search-minimal-client-success.log");
    expect(scenario).not.toContain("/tmp/operator-openai-web-search-minimal-client-reject.log");
  });
  ...
  it("keeps ClawHub plugin Docker smoke hermetic by default", () => {
    ...
    expect(runner).toContain("OPERATOR_PLUGINS_E2E_LIVE_CLAWHUB");
    expect(sweep).toContain("scripts/e2e/lib/plugins/clawhub.sh");
    expect(clawhub).toContain("start_clawhub_fixture_server()");
    expect(clawhub).toContain('OPERATOR_CLAWHUB_URL="http://127.0.0.1:');
    expect(clawhub).toContain("OPERATOR_PLUGINS_E2E_LIVE_CLAWHUB");
    expect(clawhub).toContain("OPERATOR_PLUGINS_E2E_LIVE_NPM_REGISTRY");
    expect(runner).toContain("OPERATOR_PLUGINS_E2E_LIVE_NPM_REGISTRY");
    expect(clawhub).toContain("live ClawHub can rate-limit CI");
    expect(clawhub).toContain('[[ -n "${OPERATOR_CLAWHUB_URL:-}" || -n "${CLAWHUB_URL:-}" ]]');
    expect(clawhub).toContain("Ignoring ambient ClawHub URL for fixture-mode plugin E2E");
    expect(clawhub).toContain("unset OPERATOR_CLAWHUB_URL CLAWHUB_URL");
  });
  ...
  it("keeps the plugin binding command escape Docker smoke focused", () => {
    ...
    expect(runner).toContain(
      'DOCKER_RUN_TIMEOUT="${OPERATOR_PLUGIN_BINDING_COMMAND_ESCAPE_DOCKER_RUN_TIMEOUT:-900s}"',
    );
    expect(runner).toContain(
    ...
    expect(dockerfile).toContain("OPERATOR_DISABLE_BUNDLED_PLUGIN_POSTINSTALL=1");
    expect(dockerfile).toContain(
      "pnpm install --frozen-lockfile --ignore-scripts --filter operator",
    );
  });
  ...
  it("covers plugin CLI sources in the Docker plugin sweep", () => {
    ...
    expect(sweep).toContain('OPERATOR_PLUGINS_CLI_TIMEOUT="${OPERATOR_PLUGINS_CLI_TIMEOUT:-180s}"');
    expect(runner).toContain('PLUGINS_CLI_TIMEOUT="${OPERATOR_PLUGINS_CLI_TIMEOUT:-180s}"');
    expect(runner).toContain('-e "OPERATOR_PLUGINS_CLI_TIMEOUT=$PLUGINS_CLI_TIMEOUT"');
    expect(sweep).toContain(
      'run_logged "$label" operator_e2e_maybe_timeout "$OPERATOR_PLUGINS_CLI_TIMEOUT" node "$OPERATOR_ENTRY" "$@"',
    );
    expect(sweep).toContain("run_plugins_operator_capture()");
    expect(sweep).toContain(
      'operator_e2e_maybe_timeout "$OPERATOR_PLUGINS_CLI_TIMEOUT" node "$OPERATOR_ENTRY" "$@" >"$output_file"',
    );
    expect(sweep).not.toContain('run_logged install-npm node "$OPERATOR_ENTRY"');
    for (const [path, script] of [
      ...
      const unboundedPluginCliLines = script
        .split("\n")
        .filter((line) => line.includes('node "$OPERATOR_ENTRY" plugins'))
        .filter((line) => !line.includes("operator_e2e_maybe_timeout"));
    ...
    expect(sweep).toContain('plugins install "npm:@gabrielvfonseca/demo-plugin-npm@0.0.1" --force');
    ...
    expect(clawhub).toContain("run_plugins_operator_logged install-clawhub");
    expect(clawhub).toContain('operator_e2e_maybe_timeout "$OPERATOR_PLUGINS_CLI_TIMEOUT"');
    expect(clawhub).toContain("clawhub:@gabrielvfonseca/kitchen-sink");
