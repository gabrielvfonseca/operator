...
describe("Plugin ClawHub New workflow", () => {
  ...
  it("rehashes and validates tgz identity before exposing the token", () => {
    ...
    expect(step(publish, "Publish exact ClawHub bootstrap artifacts").run).toContain(
      "OPERATOR_CLAWHUB_RELEASE_GIT_DIR",
    );
    expect(step(publish, "Publish exact ClawHub bootstrap artifacts").run).toContain(
      "OPERATOR_CLAWHUB_RELEASE_TAG",
    );
    expect(step(publish, "Publish exact ClawHub bootstrap artifacts").run).toContain(
      "OPERATOR_CLAWHUB_TARGET_SHA",
    );
  });
  ...
  it("preserves configure-only repair and exact registry byte readback", () => {
    ...
    expect(publishRun).toContain('"${OPERATOR_CLAWHUB_CLI}" package trusted-publisher set');
  ...
  it("uses one lockfile-only ClawHub CLI graph and absolute binary path", () => {
    ...
    expect(source).toContain("OPERATOR_CLAWHUB_CLI: ${{ steps.clawhub_cli.outputs.cli }}");
    expect(source).toContain('"${OPERATOR_CLAWHUB_CLI}" package trusted-publisher set');
  });
