  ...
  try {
    ...
    writeJson(join(stateDir, "plugins", "installs.json"), {
      installRecords: {
        matrix: {
          source: "clawhub",
          spec: "clawhub:@gabrielvfonseca/matrix",
          installPath: matrixInstallDir,
          clawhubPackage: "@gabrielvfonseca/matrix",
    ...
    execFileSync(process.execPath, [ASSERTIONS_PATH, "assert-state"], {
      env: {
        ...
        OPERATOR_TEST_WORKSPACE_DIR: workspace,
        OPERATOR_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON: coveragePath,
        OPERATOR_UPGRADE_SURVIVOR_SCENARIO: "configured-plugin-installs",
      },
      stdio: "pipe",
    });
  } finally {
