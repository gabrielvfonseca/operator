...
async function extractClawhubSkillInstallVerifier(): Promise<string> {
  ...
  const marker =
    'node --input-type=module - "$OPERATOR_CONFIG_PATH" "$skill_dir" "$origin_json" "$lock_json" "$info_json" "$slug" <<\'NODE\'\n';
...
export OPERATOR_ONBOARD_SCENARIO_SOURCE_ONLY=1
export OPERATOR_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
OPERATOR_ENTRY=node
...
`,
  ...
  it("checks local onboarding logs for systemd noise", async () => {
    ...
    expect(contents).toContain('OPERATOR_E2E_LOG_DIR="$ONBOARD_TMP_DIR/logs"');
    ...
    expect(contents).toContain('validate_local_basic_log "$OPERATOR_E2E_LAST_LOG_PATH"');
...
export OPERATOR_ONBOARD_SCENARIO_SOURCE_ONLY=1
export OPERATOR_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
export OPERATOR_ONBOARD_GATEWAY_WAIT_ATTEMPTS=2
export OPERATOR_ONBOARD_GATEWAY_WAIT_INTERVAL_S=0.1
OPERATOR_ENTRY=node
source scripts/e2e/lib/onboard/scenario.sh
...
export OPERATOR_ONBOARD_SCENARIO_SOURCE_ONLY=1
export OPERATOR_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
export OPERATOR_ONBOARD_GATEWAY_WAIT_ATTEMPTS=2x
OPERATOR_ENTRY=node
source scripts/e2e/lib/onboard/scenario.sh
...
`,
    ...
    try {
      ...
      expect(result.stderr).toContain("invalid OPERATOR_ONBOARD_GATEWAY_WAIT_ATTEMPTS: 2x");
...
`,
    ...
    try {
      const result = spawnSync("bash", ["scripts/e2e/lib/skills/clawhub-install-proof.sh"], {
        ...
        env: {
          ...process.env,
          OPERATOR_CURRENT_PACKAGE_TGZ: "",
          OPERATOR_TEST_STATE_SCRIPT_B64: "",
