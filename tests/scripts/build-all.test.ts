...
describe("resolveBuildAllStep", () => {
  it("pins one generated timestamp across every child build", () => {
    ...
    expect(uiInvocation.options.env).toMatchObject({
      FOO: "bar",
      GIT_COMMIT: commit,
      OPERATOR_BUILD_TIMESTAMP: "2026-07-10T12:34:56.789Z",
    });
    expect(buildInfoInvocation.options.env.OPERATOR_BUILD_TIMESTAMP).toBe(
      uiInvocation.options.env.OPERATOR_BUILD_TIMESTAMP,
    );
  });
  ...
  it("preserves an explicit build timestamp after trimming outer whitespace", () => {
    expect(
      resolveBuildAllEnvironment({
        OPERATOR_BUILD_TIMESTAMP: " 2026-07-10T01:02:03.000Z ",
  ...
  it.each([
    {
      ...
      expectedEnv: { FOO: "bar", OPERATOR_PLUGIN_SDK_CANONICAL_DTS: "1" },
    },
    {
...
describe("resolveBuildAllSteps", () => {
  ...
  it("skips bundled tsdown declarations for runtime-only profiles", () => {
    for (const profile of [
      ...
      expect(
        resolveBuildAllStep(tsdown, { env: { OPERATOR_RUN_NODE_SKIP_DTS_BUILD: "0" } }).options.env,
      ).toMatchObject({
        OPERATOR_RUN_NODE_SKIP_DTS_BUILD: "1",
      });
    }
  ...
  it("keeps canonical declarations enabled for package artifact builds", () => {
    ...
    expect(
      resolveBuildAllStep(tsdown, { env: { OPERATOR_RUN_NODE_SKIP_DTS_BUILD: "1" } }).options.env,
    ).toMatchObject({
  ...
  it("preserves startup metadata only for profiles that regenerate it", () => {
    ...
    for (const profile of ["gatewayWatch", "qaRuntime"]) {
      ...
      expect(resolveBuildAllStep(tsdown, { env: {} }).options.env).not.toHaveProperty(
        "OPERATOR_PRESERVE_CLI_STARTUP_METADATA",
      );
    }
  });
    ...
    for (const profile of ["gatewayWatch", "cliStartup"] as const) {
      ...
      expect(
        resolveBuildAllStep(runtimePostbuild, {
          env: { OPERATOR_RUNTIME_POSTBUILD_STATIC_ASSETS: "1" },
        }).options.env,
      ).toMatchObject({
        OPERATOR_RUNTIME_POSTBUILD_STATIC_ASSETS: "0",
      });
  ...
  it("keeps generated static plugin assets enabled for QA-backed profiles", () => {
    for (const profile of ["qaRuntime", "sourcePerformance"] as const) {
      ...
      expect(
        ...
        OPERATOR_RUNTIME_POSTBUILD_STATIC_ASSETS: "1",
      });
    }
  });
  ...
  it("does not cache ui:build because Vite reads package.json, git HEAD, and env metadata", () => {
    ...
    // git HEAD, and OPERATOR_CONTROL_UI_BUILD_ID env, so a file-input
  ...
  it("caches plugin-sdk entry declarations without restoring compiled JS", () => {
    ...
    expect(step.env).toEqual({ OPERATOR_PLUGIN_SDK_CANONICAL_DTS: "1" });
  ...
  it("marks cacheable steps stale when a tracked env input changes", () => {
    withBuildCacheFixture(({ rootDir, step }) => {
      const envStep = {
        ...
        cache: {
          ...step.cache,
          env: ["OPERATOR_BUILD_PRIVATE_QA"],
        },
      };
      const cacheState = resolveBuildAllStepCacheState(envStep, {
        rootDir,
        env: { OPERATOR_BUILD_PRIVATE_QA: "0" },
      });
