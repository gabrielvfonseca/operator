...
function writeFixtureServerShims(binDir: string, pidPath: string): void {
  ...
  writeFileSync(
    ...
    [
      "#!/bin/bash",
      'printf "%s\\n" "$$" >"$OPERATOR_TEST_FIXTURE_SERVER_PID"',
      "trap 'exit 0' TERM",
...
describe("plugins Docker assertions", () => {
  it("rejects loose ClawHub preflight limits instead of parsing prefixes", () => {
    const timeoutResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "clawhub-preflight"], {
      ...
      env: {
        ...process.env,
        CLAWHUB_PLUGIN_SPEC: "clawhub:@gabrielvfonseca/kitchen-sink",
        OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
      },
    });
    expect(timeoutResult.status).not.toBe(0);
    expect(timeoutResult.stderr).toContain(
      "invalid OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: 1e3",
    );
    ...
    const bodyLimitResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "clawhub-preflight"], {
      ...
      env: {
        ...process.env,
        CLAWHUB_PLUGIN_SPEC: "clawhub:@gabrielvfonseca/kitchen-sink",
        OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: "1000bytes",
      },
    });
    expect(bodyLimitResult.status).not.toBe(0);
    expect(bodyLimitResult.stderr).toContain(
      "invalid OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: 1000bytes",
    );
  });
  ...
  it("keeps sweep artifact paths aligned with the assertion scratch root", () => {
    ...
    for (const scriptPath of scripts) {
      ...
      expect(script).toContain("OPERATOR_PLUGINS_TMP_DIR");
  ...
  it("cleans the default plugin sweep scratch root", () => {
    ...
    try {
      const result = runPluginsSweepShell(
        `
set -euo pipefail
export OPERATOR_PLUGINS_SWEEP_SOURCE_ONLY=1
source scripts/e2e/lib/plugins/sweep.sh
printf '%s\\n' "$OPERATOR_PLUGINS_TMP_DIR" > "$MARKER"
test -d "$OPERATOR_PLUGINS_TMP_DIR"
cleanup_operator_plugins_sweep
test ! -e "$OPERATOR_PLUGINS_TMP_DIR"
`,
        { MARKER: marker },
      );
...
set -euo pipefail
export OPERATOR_PLUGINS_SWEEP_SOURCE_ONLY=1
export OPERATOR_PLUGINS_TMP_DIR="$SCRATCH_ROOT"
source scripts/e2e/lib/plugins/sweep.sh
test -d "$OPERATOR_PLUGINS_TMP_DIR"
cleanup_operator_plugins_sweep
test -d "$OPERATOR_PLUGINS_TMP_DIR"
  ...
  it("scans plugin assertion logs without echoing whole files on failure", async () => {
    ...
    try {
      ...
      const pass = await runAssertionAsync(["plugin-dir-update-skipped"], {
        OPERATOR_PLUGINS_TMP_DIR: passRoot,
      });
      ...
      const fail = await runAssertionAsync(["plugin-dir-update-skipped"], {
        OPERATOR_PLUGINS_TMP_DIR: failRoot,
      });
      ...
      const invalid = await runAssertionAsync(["invalid-operator-extensions"], {
        HOME: invalidHome,
        OPERATOR_PLUGINS_TMP_DIR: invalidRoot,
      });
    ...
    try {
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            OPERATOR_TEST_FIXTURE_SERVER_PID: pidPath,
    ...
    try {
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            OPERATOR_PLUGINS_FIXTURE_STOP_ATTEMPTS: "2",
            OPERATOR_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS: "0.05",
            OPERATOR_TEST_FIXTURE_SERVER_PID: pidPath,
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
  ...
  it("rejects invalid fixture stop attempts before cleanup polling", () => {
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          OPERATOR_PLUGINS_FIXTURE_STOP_ATTEMPTS: "2x",
        },
      },
    );

    expect(result.status).toBe(2);
    expect(result.stderr).toContain("invalid OPERATOR_PLUGINS_FIXTURE_STOP_ATTEMPTS: 2x");
    ...
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          OPERATOR_PLUGINS_FIXTURE_STOP_ATTEMPTS: "2",
          OPERATOR_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS: "soon",
        },
      },
    );

    expect(result.status).toBe(2);
    expect(result.stderr).toContain("invalid OPERATOR_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS: soon");
    expect(result.stdout).not.toContain("signal");
    expect(result.stdout).not.toContain("probe");
  });
    ...
    try {
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            OPERATOR_DOCKER_E2E_LOG_PRINT_BYTES: "80",
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
      );
  ...
  it("keeps npm fixture registry alive after malformed package paths", async () => {
    ...
    const child = spawn(
      ...
      [
        ...
        portFile,
        "@gabrielvfonseca/demo-plugin-npm",
        "1.0.0",
        tarballPath,
      ],
      {
    ...
    try {
      ...
      expect(JSON.parse(valid.body)).toMatchObject({
        name: "@gabrielvfonseca/demo-plugin-npm",
  ...
  it("recomputes proxied content length after fetch decodes the response", async () => {
    ...
    const child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          OPERATOR_NPM_REGISTRY_UPSTREAM: `http://127.0.0.1:${upstreamAddress.port}`,
        },
        stdio: ["ignore", "pipe", "pipe"],
      },
    );
  ...
  it("does not let absolute-form request targets escape the configured upstream", async () => {
    ...
    const child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          OPERATOR_NPM_REGISTRY_UPSTREAM: `http://127.0.0.1:${configuredAddress.port}`,
        },
        stdio: ["ignore", "pipe", "pipe"],
      },
    );
    ...
    try {
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            OPERATOR_DOCKER_E2E_LOG_PRINT_BYTES: "64kb",
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
      );
    ...
    try {
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            OPERATOR_PLUGINS_E2E_LIVE_CLAWHUB: "0",
            OPERATOR_PLUGINS_TMP_DIR: tmpDir,
            OPERATOR_TEST_FIXTURE_SERVER_PID: pidPath,
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
    ...
    try {
      ...
      const result = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "plugin-tgz"], {
        ...
        env: {
          ...
          HOME: home,
          OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
          OPERATOR_PLUGINS_TMP_DIR: scratchRoot,
        },
      });
      ...
      writeJson(path.join(home, ".operator", "plugins", "installs.json"), {
        installRecords: {
          "operator-kitchen-sink-fixture": {
            ...
            source: "clawhub",
            spec: "clawhub:@gabrielvfonseca/kitchen-sink",
          },
        },
      });
  ...
  it("times out stalled ClawHub package metadata requests", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        CLAWHUB_PLUGIN_ID: "operator-kitchen-sink-fixture",
        CLAWHUB_PLUGIN_SPEC: "clawhub:@gabrielvfonseca/kitchen-sink",
        OPERATOR_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "25",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ClawHub package preflight for @gabrielvfonseca/kitchen-sink timed out after 25ms",
      );
    } finally {
      await new Promise<void>((resolve) => {
  ...
  it("times out stalled ClawHub package metadata bodies", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        ...
        CLAWHUB_PLUGIN_SPEC: "clawhub:@gabrielvfonseca/kitchen-sink",
        OPERATOR_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "75",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ClawHub package preflight response for @gabrielvfonseca/kitchen-sink timed out after 75ms",
      );
    } finally {
      await new Promise<void>((resolve) => {
        server.close(() => resolve());
  ...
  it("bounds ClawHub package metadata response bodies", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["clawhub-preflight"], {
        ...
        CLAWHUB_PLUGIN_SPEC: "clawhub:@gabrielvfonseca/kitchen-sink",
        OPERATOR_CLAWHUB_URL: `http://127.0.0.1:${address.port}`,
        OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES: "16",
        OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS: "1000",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "ClawHub package preflight response for @gabrielvfonseca/kitchen-sink response body exceeded 16 bytes",
      );
