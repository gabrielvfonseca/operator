...
function runCodexOnDemandAssertions(root: string) {
  return spawnSync(process.execPath, [CODEX_ON_DEMAND_ASSERTIONS_SCRIPT], {
    ...
    env: {
      ...
      OPERATOR_CONFIG_PATH: path.join(root, "state", "operator.json"),
  ...
  return spawnSync(
    ...
    ],
    {
      ...
      env: {
        ...
        NODE_OPTIONS: nodeOptionsWithoutExperimentalWarnings(),
        OPERATOR_STATE_DIR: path.join(params.root, "state"),
        OPERATOR_CODEX_NPM_PLUGIN_SESSION_STORE_CONTRACT: params.sessionStoreContract ?? "sqlite",
      },
    },
  );
}
...
function createCodexNpmPluginLiveFixture(root: string, storedSessionId?: string) {
  ...
  const marker = "OPERATOR-CODEX-NPM-PLUGIN-LIVE-OK";
...
function createCodexInstallFixture(root: string) {
  ...
  const projectRoot = npmProjectRootForInstalledPackage(installPath, "@gabrielvfonseca/codex");
  writeJson(path.join(installPath, "package.json"), { name: "@gabrielvfonseca/codex" });
    ...
    plugins: {
      installs: {
        codex: {
          installPath,
          source: "npm",
          spec: "npm:@gabrielvfonseca/codex",
        },
      },
    },
  });
...
describe("Codex install helpers", () => {
  ...
  it("resolves package roots and package manifests inside managed npm installs", () => {
    ...
    const projectRoot = npmProjectRootForInstalledPackage(packageRoot, "@gabrielvfonseca/codex");
