...
function runProofHarness(
  ...
  return spawnSync(process.execPath, [harnessPath, proofScriptPath, root, mode], {
    ...
    env: {
      ...process.env,
      OPERATOR_ENTRY: fakeOperator,
      OPERATOR_SECRET_PROOF_READY_MS: "60",
      OPERATOR_SECRET_PROOF_RPC_MS: "1000",
      ...envOverrides,
    },
    timeout: 5_000,
  });
...
describe("secret provider integration proof harness", () => {
  it("runs pnpm-backed Operator commands through the repo pnpm runner", async () => {
    ...
    const command = await proof.resolveOperatorCommand(
      ["gateway", "status"],
      { ...process.env, OPERATOR_SECRET_PROOF_SENTINEL: "1" },
      {
        nodeExecPath: "/opt/node/bin/node",
        npmExecPath: fakePnpm,
        runner: { pnpm: true, baseArgs: ["@gabrielvfonseca/operator"], label: "pnpm operator" },
      },
    );

    expect(command.command).toBe("/opt/node/bin/node");
    expect(command.args).toEqual([fakePnpm, "@gabrielvfonseca/operator", "gateway", "status"]);
    expect(command.options.env.OPERATOR_SECRET_PROOF_SENTINEL).toBe("1");
  ...
  it("fails fast when startup exits by signal", () => {
    ...
    const result = runProofHarness(root, fakeOperator, "start", {
      OPERATOR_SECRET_PROOF_READY_MS: "2000",
    });
  ...
  it("kills a stalled startup gateway before returning a readiness failure", async () => {
    ...
    const result = runProofHarness(root, fakeOperator, "start", {
      OPERATOR_SECRET_PROOF_TEARDOWN_GRACE_MS: "100",
    });
  ...
  it("bounds captured command output", async () => {
    const previousLimit = process.env.OPERATOR_SECRET_PROOF_OUTPUT_BYTES;
    process.env.OPERATOR_SECRET_PROOF_OUTPUT_BYTES = "1024";
    try {
      ...
      if (previousLimit === undefined) {
        delete process.env.OPERATOR_SECRET_PROOF_OUTPUT_BYTES;
      } else {
        process.env.OPERATOR_SECRET_PROOF_OUTPUT_BYTES = previousLimit;
      }
    }
  });

  it("clamps oversized command timeout env values before scheduling timers", async () => {
    const previousTimeout = process.env.OPERATOR_SECRET_PROOF_COMMAND_MS;
    process.env.OPERATOR_SECRET_PROOF_COMMAND_MS = String(Number.MAX_SAFE_INTEGER);
    try {
      const proof = await import(
      ...
      if (previousTimeout === undefined) {
        delete process.env.OPERATOR_SECRET_PROOF_COMMAND_MS;
      } else {
        process.env.OPERATOR_SECRET_PROOF_COMMAND_MS = previousTimeout;
      }
    }
  });
  ...
  it("blocks skipped secret proofs unless local rehearsals explicitly allow skips", async () => {
    const previousAllowSkips = process.env.OPERATOR_SECRET_PROOF_ALLOW_SKIPS;
    const proof = await import(
    ...
    try {
      delete process.env.OPERATOR_SECRET_PROOF_ALLOW_SKIPS;
      expect(proof.collectBlockingProofResults(entries)).toEqual(entries);

      process.env.OPERATOR_SECRET_PROOF_ALLOW_SKIPS = "1";
      expect(proof.collectBlockingProofResults(entries)).toEqual([]);
    } finally {
      if (previousAllowSkips === undefined) {
        delete process.env.OPERATOR_SECRET_PROOF_ALLOW_SKIPS;
      } else {
        process.env.OPERATOR_SECRET_PROOF_ALLOW_SKIPS = previousAllowSkips;
      }
    }
  });
  ...
  it.runIf(process.platform !== "win32")("bounds captured PTY configure output", async () => {
    ...
    const previousLimit = process.env.OPERATOR_SECRET_PROOF_OUTPUT_BYTES;
    const previousEntry = process.env.OPERATOR_ENTRY;
    process.env.OPERATOR_SECRET_PROOF_OUTPUT_BYTES = "128";
    process.env.OPERATOR_ENTRY = fakeOperator;
    try {
      const proof = await import(
      ...
      if (previousEntry === undefined) {
        delete process.env.OPERATOR_ENTRY;
      } else {
        process.env.OPERATOR_ENTRY = previousEntry;
      }
    }
  });
  ...
  it.each([
    ["OPERATOR_SECRET_PROOF_COMMAND_MS", "150ms"],
    ["OPERATOR_SECRET_PROOF_READY_MS", "0"],
    ["OPERATOR_SECRET_PROOF_OUTPUT_BYTES", "4mb"],
    ["OPERATOR_SECRET_PROOF_RESOLVER_STDIN_BYTES", "4mb"],
  ...
  it("bounds generated resolver stdin before reading the secret store", async () => {
    ...
    const previousLimit = process.env.OPERATOR_SECRET_PROOF_RESOLVER_STDIN_BYTES;
    process.env.OPERATOR_SECRET_PROOF_RESOLVER_STDIN_BYTES = "64";
    ...
    try {
      ...
      if (previousLimit === undefined) {
        delete process.env.OPERATOR_SECRET_PROOF_RESOLVER_STDIN_BYTES;
      } else {
        process.env.OPERATOR_SECRET_PROOF_RESOLVER_STDIN_BYTES = previousLimit;
      }
    }
  });
  ...
  it("detects startup secret leaks after the retained output cap", () => {
    ...
    const result = runProofHarness(root, fakeOperator, "startup-fails", {
      OPERATOR_SECRET_PROOF_OUTPUT_BYTES: "128",
    });
