...
describe("telegram user Crabbox proof log polling", () => {
  it("starts the local gateway through the repo pnpm runner", () => {
    ...
    const spec = createOperatorGatewaySpawnSpec({
      env: { ...process.env, OPERATOR_TELEGRAM_PROOF_SENTINEL: "1" },
      gatewayPort: 19042,
    ...
    expect(spec.options.env?.OPERATOR_TELEGRAM_PROOF_SENTINEL).toBe("1");
  ...
  it("rejects loose numeric log tail limits instead of parsing prefixes", () => {
    expect(() =>
      readTelegramUserProofLogTailBytes({
        OPERATOR_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "1e3",
      }),
    ).toThrow("invalid OPERATOR_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: 1e3");
    expect(() =>
      readTelegramUserProofLogTailBytes({
        OPERATOR_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "1000bytes",
      }),
    ).toThrow("invalid OPERATOR_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: 1000bytes");
    expect(
      readTelegramUserProofLogTailBytes({
        OPERATOR_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "4096",
      }),
    ).toBe(4096);
  });
  ...
  posixIt("does not expand generated remote probe arguments in the shell", () => {
    ...
    writeExecutable(
      ...
      `#!/usr/bin/env node
import fs from "node:fs";
fs.writeFileSync(process.env.OPERATOR_TEST_ARGV_PATH, JSON.stringify(process.argv.slice(1)));
`,
    );
    writeExecutable(
      scriptPath,
    ...
    const result = spawnSync("bash", [scriptPath], {
      ...
      env: {
        ...process.env,
        OPERATOR_TEST_ARGV_PATH: argvPath,
