...
function publishedUpgradeSurvivorLane(
  ...
  return {
    command: `OPERATOR_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/${name}" OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC='${baselineSpec}' ${
      scenario ? `OPERATOR_UPGRADE_SURVIVOR_SCENARIO='${scenario}' ` : ""
...
function updateMigrationLane(name: string, baselineSpec: string): ReturnType<typeof summarizeLane> {
  return {
    command: `OPERATOR_UPGRADE_SURVIVOR_ARTIFACT_DIR="$PWD/.artifacts/upgrade-survivor/${name}" OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC='${baselineSpec}' OPERATOR_UPGRADE_SURVIVOR_SCENARIO='plugin-deps-cleanup' OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-migration`,
    imageKind: "bare",
    live: false,
    name,
    resources: ["docker", "npm"],
...
function bundledPluginSweepLane(index: number): ReturnType<typeof summarizeLane> {
  return {
    command: `OPERATOR_BUNDLED_PLUGIN_SWEEP_TOTAL=24 OPERATOR_BUNDLED_PLUGIN_SWEEP_INDEX=${index} OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:bundled-plugin-install-uninstall`,
...
describe("scripts/lib/docker-e2e-plan", () => {
  ...
  it("routes live Docker scripts through the nested trusted release harness", () => {
    ...
    const tempRoot = tempDirs.make("operator-release-harness-");
    ...
    expect(sourceLane?.command).toContain(
      'harness="${OPERATOR_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.}"',
    );
    ...
    const laneJson = execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          OPERATOR_DOCKER_E2E_REPO_ROOT: tempRoot,
        },
      },
    );
    ...
    expect(lane.command).toContain(
      'harness="${OPERATOR_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.release-harness}"',
    );
  ...
  it("plans package-backed Compose and package artifact proofs", () => {
    ...
    expect(plan.lanes.map(summarizeLane)).toEqual([
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:compose-setup",
        imageKind: "functional",
        live: false,
        name: "compose-setup",
      ...
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:package-install",
        imageKind: "bare",
        live: false,
  ...
  it("splits release-path package and plugin chunks across shorter CI jobs", () => {
    ...
    expect(packageUpdateCore.lanes.map(summarizeLane)).toEqual([
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:doctor-switch",
        imageKind: "bare",
        live: false,
        name: "doctor-switch",
        resources: ["docker", "npm"],
      ...
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-channel-switch",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:skill-install",
        imageKind: "bare",
        live: false,
        name: "skill-install",
        resources: ["docker", "npm"],
      ...
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:upgrade-survivor",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:published-upgrade-survivor",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:root-managed-vps-upgrade",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-restart-auth",
        imageKind: "bare",
        live: false,
    ...
    expect(pluginsRuntimeServices.lanes.map(summarizeLane)).toEqual([
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:cron-mcp-cleanup",
        imageKind: "functional",
        live: false,
      ...
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-rpc",
        imageKind: "functional",
        live: false,
      ...
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-web-search-minimal",
        imageKind: "functional",
        live: false,
  ...
  it("expands the published upgrade survivor lane across deduped baselines", () => {
    const plan = planFor({
      ...
      upgradeSurvivorBaselines:
        "operator@2026.4.29 2026.4.23 operator@2026.4.23 operator@2026.3.13-1",
    });

    expect(plan.lanes.map(summarizeLane)).toEqual([
      publishedUpgradeSurvivorLane("published-upgrade-survivor-2026.4.29", "operator@2026.4.29"),
      publishedUpgradeSurvivorLane("published-upgrade-survivor-2026.4.23", "operator@2026.4.23"),
      publishedUpgradeSurvivorLane(
        "published-upgrade-survivor-2026.3.13-1",
        "operator@2026.3.13-1",
      ),
    ]);
  });
  ...
  it("expands reported upgrade issue scenarios", () => {
    ...
    expect(plan.lanes.map((lane) => lane.name)).toEqual([
      "published-upgrade-survivor-2026.4.29",
      "published-upgrade-survivor-2026.4.29-acpx-operator-tools-bridge",
      "published-upgrade-survivor-2026.4.29-feishu-channel",
  ...
  it("omits trusted-current scenarios unsupported by a frozen target harness", () => {
    const targetRoot = tempDirs.make("operator-frozen-upgrade-harness-");
    ...
    expect(plan.omittedUnsupportedLanes).toEqual([
      "published-upgrade-survivor-2026.6.11-acpx-operator-tools-bridge",
    ]);
  });

  it("reads content-addressed scenario catalogs from pre-command frozen targets", () => {
    const targetRoot = tempDirs.make("operator-legacy-frozen-upgrade-harness-");
    const assertionsFile = join(targetRoot, "scripts/e2e/lib/upgrade-survivor/assertions.mjs");
  ...
  it("omits survivor lanes when the target exposes none of the requested scenarios", () => {
    const targetRoot = tempDirs.make("operator-frozen-empty-upgrade-harness-");
  ...
  it("omits baseline-only survivor lanes when the target lacks the implicit base scenario", () => {
    const targetRoot = tempDirs.make("operator-frozen-no-base-upgrade-harness-");
    writeFrozenScenarioContract(targetRoot, ["unrelated"]);
  ...
  it("omits an unconfigured survivor lane when the target lacks the implicit base scenario", () => {
    const targetRoot = tempDirs.make("operator-frozen-default-base-harness-");
    writeFrozenScenarioContract(targetRoot, ["unrelated"]);
  ...
  it("reports an unsupported survivor lane beside runnable selected lanes", () => {
    const targetRoot = tempDirs.make("operator-frozen-mixed-upgrade-harness-");
    writeFrozenScenarioContract(targetRoot, ["unrelated"]);
  ...
  it("reports an explicitly selected expanded survivor lane as unsupported", () => {
    const targetRoot = tempDirs.make("operator-frozen-expanded-upgrade-harness-");
    writeFrozenScenarioContract(targetRoot, ["unrelated"]);
  ...
  it("omits unsupported scenario-only survivor lanes without explicit baselines", () => {
    const targetRoot = tempDirs.make("operator-frozen-scenario-only-harness-");
    writeFrozenScenarioContract(targetRoot, ["unrelated"]);
  ...
  it("does not fall back to base when an unsupported scenario is baseline-incompatible", () => {
    const targetRoot = tempDirs.make("operator-frozen-incompatible-scenario-harness-");
    writeFrozenScenarioContract(targetRoot, ["unrelated"]);

    const plan = planFor({
      selectedLaneNames: ["published-upgrade-survivor"],
      upgradeSurvivorBaselines: "2026.4.21",
      upgradeSurvivorScenarios: "acpx-operator-tools-bridge",
      upgradeSurvivorTargetRoot: targetRoot,
    });
  ...
  it("fails closed when an unknown legacy scenario catalog lacks the command", () => {
    const targetRoot = tempDirs.make("operator-frozen-failed-scenario-harness-");
    const assertionsFile = join(targetRoot, "scripts/e2e/lib/upgrade-survivor/assertions.mjs");
    mkdirSync(dirname(assertionsFile), { recursive: true });
    writeFileSync(
      assertionsFile,
  ...
  it("fails closed when a frozen target scenario command returns non-JSON output", () => {
    const targetRoot = tempDirs.make("operator-frozen-non-json-scenario-harness-");
    const assertionsFile = join(targetRoot, "scripts/e2e/lib/upgrade-survivor/assertions.mjs");
    mkdirSync(dirname(assertionsFile), { recursive: true });
  ...
  it("fails closed when a frozen target scenario command returns an invalid catalog", () => {
    const targetRoot = tempDirs.make("operator-frozen-invalid-scenario-harness-");
    const assertionsFile = join(targetRoot, "scripts/e2e/lib/upgrade-survivor/assertions.mjs");
    mkdirSync(dirname(assertionsFile), { recursive: true });
  ...
  it("does not inspect a frozen survivor contract for unrelated selected lanes", () => {
    const targetRoot = tempDirs.make("operator-frozen-unrelated-lane-harness-");
    const assertionsFile = join(targetRoot, "scripts/e2e/lib/upgrade-survivor/assertions.mjs");
    mkdirSync(dirname(assertionsFile), { recursive: true });
  ...
  it("skips plugin dependency cleanup for baselines without packaged plugin dirs", () => {
    ...
    expect(plan.lanes.map((lane) => lane.name)).toEqual([
      ...
      "published-upgrade-survivor-2026.4.22-acpx-operator-tools-bridge",
  ...
  it("expands update migration across baselines and cleanup scenarios", () => {
    ...
    expect(plan.lanes.map(summarizeLane)).toEqual([
      updateMigrationLane("update-migration-2026.4.29-plugin-deps-cleanup", "operator@2026.4.29"),
      updateMigrationLane("update-migration-2026.4.23-plugin-deps-cleanup", "operator@2026.4.23"),
    ]);
  });
  ...
  it("marks the aggregate Gemini CLI backend lane advisory for auth drift", () => {
    ...
    expect(lane.command).toContain("OPERATOR_LIVE_CLI_BACKEND_ADVISORY=1");
    expect(lane.command).toContain("OPERATOR_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP=1");
    expect(lane.command).toContain(
      "OPERATOR_LIVE_CLI_BACKEND_MODEL=google-gemini-cli/gemini-3-flash-preview",
    );
  });
  ...
  it("plans Codex harness Docker-all lanes for API-key Testbox auth", () => {
    for (const name of ["live-codex-harness", "live-codex-bind"]) {
      ...
      expect(lane.command, name).toContain("OPERATOR_LIVE_CODEX_HARNESS_AUTH=api-key");
  ...
  it("plans the Codex npm plugin live lane as package-backed OpenAI proof", () => {
    ...
    expect(plan.lanes.map(summarizeLane)).toEqual([
      {
        command:
          'OPERATOR_SKIP_DOCKER_BUILD=1 bash -c \'harness="${OPERATOR_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.}"; OPERATOR_LIVE_DOCKER_REPO_ROOT="${OPERATOR_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/e2e/codex-npm-plugin-live-docker.sh"\'',
        imageKind: "bare",
        live: true,
  ...
  it("plans the Codex on-demand onboarding lane as package-backed npm proof", () => {
    ...
    const lane = requireFirstLane(plan);
    expect(lane.command).toBe("OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:codex-on-demand");
  ...
  it("plans the Droid ACP bind live lane as Factory-auth proof", () => {
    ...
    const lane = requireFirstLane(plan);
    expect(lane.command).toBe(
      'OPERATOR_LIVE_ACP_BIND_AGENT=droid OPERATOR_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1 OPERATOR_SKIP_DOCKER_BUILD=1 bash -c \'harness="${OPERATOR_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.}"; OPERATOR_LIVE_DOCKER_REPO_ROOT="${OPERATOR_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/test-live-acp-bind-docker.sh"\'',
    );
    expect(lane.imageKind).toBeUndefined();
    expect(lane.live).toBe(true);
  ...
  it("plans Docker package scripts that were previously only directly runnable", () => {
    ...
    expect(plan.lanes.map(summarizeLane)).toEqual([
      ...
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:multi-node-update",
        imageKind: "bare",
        live: false,
      ...
      {
        command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-telegram-live",
        imageKind: "bare",
        live: true,
  ...
  it("maps installer E2E to provider-specific package install lanes", () => {
    ...
    expect(plan.lanes.map(summarizeLane)).toEqual([
      {
        command:
          'OPERATOR_INSTALL_TAG=beta OPERATOR_E2E_MODELS=openai OPERATOR_INSTALL_E2E_IMAGE=operator-install-e2e-openai:local OPERATOR_INSTALL_E2E_AGENT_TOOL_SMOKE=0 OPERATOR_INSTALL_E2E_OPENAI_MODEL=openai/gpt-5.4-mini OPERATOR_INSTALL_E2E_AGENT_TURN_TIMEOUT_SECONDS=120 OPERATOR_INSTALL_E2E_OPENAI_PROVIDER_TIMEOUT_SECONDS=120 bash -c \'harness="${OPERATOR_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.}"; OPERATOR_LIVE_DOCKER_REPO_ROOT="${OPERATOR_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/test-install-sh-e2e-docker.sh"\'',
        imageKind: "bare",
        live: true,
      ...
      {
        command:
          'OPERATOR_INSTALL_TAG=beta OPERATOR_E2E_MODELS=anthropic OPERATOR_INSTALL_E2E_IMAGE=operator-install-e2e-anthropic:local bash -c \'harness="${OPERATOR_DOCKER_E2E_TRUSTED_HARNESS_DIR:-.}"; OPERATOR_LIVE_DOCKER_REPO_ROOT="${OPERATOR_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/test-install-sh-e2e-docker.sh"\'',
        imageKind: "bare",
        live: true,
  ...
  it("rejects unknown selected lanes with the available lane names", () => {
    expect(() => planFor({ selectedLaneNames: ["missing-lane"] })).toThrow(
      /OPERATOR_DOCKER_ALL_LANES unknown lane\(s\): missing-lane/u,
    );
  });
});
