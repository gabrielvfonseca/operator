...
describe("resolveTsdownBuildInvocation", () => {
  ...
  it("honors OPERATOR_TSDOWN_MAX_OLD_SPACE_MB over platform and memory defaults", () => {
    const result = resolveTsdownBuildInvocation({
      nodeExecPath: "/usr/bin/node",
      npmExecPath: "/tmp/pnpm.cjs",
      env: { OPERATOR_TSDOWN_MAX_OLD_SPACE_MB: "3072" },
      cgroupMemoryLimitBytes: 7 * 1024 * 1024 * 1024,
    });
  ...
  it("keeps memory detection when OPERATOR_TSDOWN_MAX_OLD_SPACE_MB is blank", () => {
    const result = resolveTsdownBuildInvocation({
      nodeExecPath: "/usr/bin/node",
      npmExecPath: "/tmp/pnpm.cjs",
      env: { OPERATOR_TSDOWN_MAX_OLD_SPACE_MB: "  " },
      cgroupMemoryLimitBytes: 7 * 1024 * 1024 * 1024,
    });
  ...
  it("uses OPERATOR_TSDOWN_MAX_OLD_SPACE_MB to normalize inherited NODE_OPTIONS", () => {
    const result = resolveTsdownBuildInvocation({
      platform: "win32",
      nodeExecPath: "C:\\Program Files\\nodejs\\node.exe",
      npmExecPath: "C:\\repo\\pnpm.cjs",
      env: {
        ...
        OPERATOR_TSDOWN_MAX_OLD_SPACE_MB: "4096",
      },
      ...NO_MEMORY_LIMIT,
    });
  ...
  it("rejects malformed OPERATOR_TSDOWN_MAX_OLD_SPACE_MB values", () => {
    for (const value of ["0", "-1", "1.5", "1e3", "4096mb", "9007199254740992"]) {
      expect(() =>
        resolveTsdownBuildInvocation({
          nodeExecPath: "/usr/bin/node",
          npmExecPath: "/tmp/pnpm.cjs",
          env: { OPERATOR_TSDOWN_MAX_OLD_SPACE_MB: value },
          ...NO_MEMORY_LIMIT,
        }),
      ).toThrow("OPERATOR_TSDOWN_MAX_OLD_SPACE_MB must be");
    }
  });
  ...
  it("can run tsdown without invoking pnpm", () => {
    const result = resolveTsdownBuildInvocation({
      platform: "linux",
      nodeExecPath: "/usr/bin/node",
      env: { OPERATOR_BUILD_ALL_NO_PNPM: "1" },
      ...NO_MEMORY_LIMIT,
    });
  ...
  it("preserves CLI startup metadata across opted-in build-all tsdown clean", async () => {
    ...
    cleanTsdownOutputRoots({
      cwd: rootDir,
      env: { OPERATOR_PRESERVE_CLI_STARTUP_METADATA: "1" },
    });
  ...
  it("preserves existing package declarations when tsdown DTS output is skipped", async () => {
    ...
    cleanTsdownOutputRoots({
      cwd: rootDir,
      env: { OPERATOR_RUN_NODE_SKIP_DTS_BUILD: "1" },
    });
...
describe("runTsdownBuildInvocation", () => {
  ...
  it("streams child output while preserving diagnostics for post-run checks", async () => {
    ...
    const result = await runTsdownBuildInvocation(
      ...
      },
      {
        stdout: output.sink,
        stderr: output.sink,
        env: { ...process.env, OPERATOR_TSDOWN_HEARTBEAT_MS: "0" },
      },
    );
  ...
  it("rejects malformed OPERATOR_TSDOWN_TIMEOUT_MS values", async () => {
    const invocation = {
      command: process.execPath,
    ...
    for (const value of ["1.5", "1e3", "10ms", "0"]) {
      await expect(
        runTsdownBuildInvocation(invocation, {
          env: {
            ...process.env,
            OPERATOR_TSDOWN_TIMEOUT_MS: value,
          },
        }),
      ).rejects.toThrow("OPERATOR_TSDOWN_TIMEOUT_MS must be");
    }
  });

  it("rejects malformed OPERATOR_TSDOWN_HEARTBEAT_MS values", async () => {
    const invocation = {
      command: process.execPath,
      args: ["-e", "process.exit(0)"],
      options: {
    ...
    for (const value of ["1.5", "1e3", "10ms", "-1"]) {
      await expect(
        runTsdownBuildInvocation(invocation, {
          env: {
            ...process.env,
            OPERATOR_TSDOWN_HEARTBEAT_MS: value,
          },
        }),
      ).rejects.toThrow("OPERATOR_TSDOWN_HEARTBEAT_MS must be");
    }
  });

  it("terminates the child when OPERATOR_TSDOWN_TIMEOUT_MS elapses", async () => {
    const output = createWriteSink();
    const result = await runTsdownBuildInvocation(
      {
        command: process.execPath,
      ...
      },
      {
        ...
        env: {
          ...process.env,
          OPERATOR_TSDOWN_HEARTBEAT_MS: "0",
          OPERATOR_TSDOWN_TIMEOUT_MS: "50",
        },
      },
    );
        ...
        const runPromise = runTsdownBuildInvocation(
          ...
          },
          {
            ...
            env: {
              ...process.env,
              OPERATOR_TSDOWN_HEARTBEAT_MS: "0",
              OPERATOR_TSDOWN_TIMEOUT_MS: String(timeoutMs),
            },
          },
        );
        ...
        const runPromise = runTsdownBuildInvocation(
          ...
          },
          {
            ...
            env: {
              ...process.env,
              OPERATOR_TSDOWN_HEARTBEAT_MS: "0",
              OPERATOR_TSDOWN_TIMEOUT_MS: "250",
            },
          },
        );
