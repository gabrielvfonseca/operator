...
function makeEnvMutationFixture(): string {
  ...
  writeRepoFile(
    ...
    "src/runtime.ts",
    `
process.env.OPERATOR_STATE_DIR = "ignored because this is not a test file";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/example.test.ts",
    `
process.env.OPERATOR_STATE_DIR = "state";
delete process.env["OPERATOR_CONFIG_PATH"];
vi.stubEnv("HOME", "home");
vi.stubEnv("OPERATOR_WORKSPACE_DIR", "workspace");
process.env = { ...process.env, OPERATOR_HOME: "home", OTHER_KEY: "ignored" };
const dynamicEnvKey = "OPERATOR_STATE_DIR";
process.env[dynamicEnvKey] = "dynamic";
delete process.env[dynamicEnvKey];
const fixture = 'process.env.OPERATOR_HOME = "not code"';
...
`,
  ...
  writeRepoFile(
    ...
    `
process.env.OPERATOR_CONFIG_PATH = "script-harness";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.OPERATOR_AGENT_DIR = "agent";
`,
  );
  writeRepoFile(
    repoRoot,
...
describe("collectTestEnvMutationReport", () => {
  it("reports active Operator env mutations while ignoring strings and non-test files", () => {
    ...
    expect(
      ...
      },
      {
        ...
        key: "OPERATOR_AGENT_DIR",
        operation: "assign",
      },
      { file: "src/example.test.ts", key: "OPERATOR_STATE_DIR", operation: "assign" },
      { file: "src/example.test.ts", key: "OPERATOR_CONFIG_PATH", operation: "delete" },
      { file: "src/example.test.ts", key: "HOME", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "OPERATOR_WORKSPACE_DIR", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "OPERATOR_HOME", operation: "replace" },
  ...
  it("renders a non-blocking text report for active findings", () => {
    ...
    expect(rendered).toContain("L2 OPERATOR_STATE_DIR assign process.env");
