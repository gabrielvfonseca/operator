...
describe("plugin gateway gauntlet helpers", () => {
  ...
  it("rejects duplicate repeatable selectors", () => {
    ...
    vi.stubEnv("OPERATOR_PLUGIN_GATEWAY_GAUNTLET_IDS", "telegram,discord");
  ...
  it("keeps manifest ids separate from bounded build entry ids", async () => {
    ...
    expect(buildGauntletPrebuildEnv({}, { buildIds: [kimi.buildId] })).toEqual({
      OPERATOR_BUNDLED_PLUGIN_BUILD_IDS: "kimi-coding",
  ...
  it("prebuilds private QA dist when QA chunks are enabled", () => {
    expect(buildGauntletPrebuildEnv({ EXISTING: "1" }, { includePrivateQa: true })).toEqual({
      ...
      OPERATOR_BUNDLED_PLUGIN_BUILD_IDS: "qa-channel,qa-lab",
  ...
  it("marks gauntlet prebuilds as runtime-only when requested", () => {
    expect(
      ...
      EXISTING: "1",
      OPERATOR_BUNDLED_PLUGIN_BUILD_IDS: "acpx",
  ...
  it("prebuilds only selected plugin dist entries for bounded gauntlet runs", () => {
    expect(
      ...
      EXISTING: "1",
      OPERATOR_BUILD_PRIVATE_QA: "1",
      OPERATOR_BUNDLED_PLUGIN_BUILD_IDS: "acpx,active-memory,qa-channel,qa-lab",
      OPERATOR_ENABLE_PRIVATE_QA_CLI: "1",
      PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
    });
  });
...
`,
  ...
  it("documents gauntlet guardrail options and env defaults in help", () => {
    ...
    for (const text of [
      ...
      "--build-timeout-ms",
      "--qa-timeout-ms",
      "OPERATOR_PLUGIN_GATEWAY_GAUNTLET_IDS",
      "OPERATOR_PLUGIN_GATEWAY_GAUNTLET_TOTAL",
      "OPERATOR_PLUGIN_GATEWAY_GAUNTLET_INDEX",
      "OPERATOR_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION",
      "OPERATOR_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT",
      "OPERATOR_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES",
    ]) {
  ...
  it("parses observation failure mode from CLI and env", () => {
    ...
    vi.stubEnv("OPERATOR_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION", "1");
  ...
  it("does not parse QA summary limit env when QA is skipped", () => {
    ...
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          OPERATOR_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES: "not-a-number",
        },
      },
    );
  ...
  it.each([
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const stateDir = process.env.OPERATOR_STATE_DIR ?? process.cwd();",
  ...
  it("carries required plugin build ids and enables dependencies in QA chunks", async () => {
    ...
    await fs.writeFile(
      ...
      [
        ...
        'fs.writeFileSync(path.join(outputDir, "env.txt"), process.env.OPERATOR_BUNDLED_PLUGIN_BUILD_IDS ?? "", "utf8");',
  ...
  it("fails successful QA chunks that write oversized summary JSON", async () => {
    ...
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          OPERATOR_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES: "64",
        },
      },
    );
