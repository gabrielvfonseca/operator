...
import { OPERATOR_PLUGIN_NPM_REPOSITORY_URL } from "../../scripts/lib/plugin-npm-release.ts";
import {
...
describe("scripts/plugin-release-pretag-pack-check.ts", () => {
  ...
  it("runs runtime build, npm pack, and ClawHub pack commands for selected targets", () => {
    ...
    expect(callOptions(1)).toMatchObject({
      cwd: repoDir,
      env: { OPERATOR_PLUGIN_NPM_RUNTIME_BUILD: "0" },
    ...
    expect(callOptions(2).env?.OPERATOR_CLAWHUB_PACK_OUTPUT_DIR).toContain("clawhub-0");
  });
});
