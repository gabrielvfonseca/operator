...
function metadataResponse(artifact: Uint8Array, body?: BodyInit) {
  ...
  return new Response(
    body ??
      JSON.stringify({
        package: { name: "@gabrielvfonseca/meta" },
        version: "2026.7.1-beta.3",
        artifact: {
          kind: "npm-pack",
          sha256: artifactIdentity.sha256,
...
describe("ClawHub published artifact verification", () => {
  ...
  it("rejects a missing configure-only tag before artifact or publisher requests", async () => {
    ...
    await expect(
      ...
      "@gabrielvfonseca/meta@2026.7.1-beta.3 ClawHub artifact did not stabilize after 1 attempts; last failure @gabrielvfonseca/meta ClawHub tag beta mismatch",
    );
