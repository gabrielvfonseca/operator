...
function fixture() {
  ...
  writeFileSync(
    ...
    JSON.stringify([
      ...
      {
        packageName: "@gabrielvfonseca/existing",
        version: "2026.7.1-beta.3",
...
function common(paths: ReturnType<typeof fixture>) {
  return {
    ...
    plugins: "@gabrielvfonseca/meta,@gabrielvfonseca/existing",
...
describe("ClawHub bootstrap artifact manifest", () => {
  it("binds the exact package set and packed file identity", async () => {
    ...
    const meta = created.entries.find((entry) => entry.packageName === "@gabrielvfonseca/meta");
  ...
  it("binds exact target bytes to configure-only repairs", async () => {
    ...
    const existing = manifest.entries.find(
      (entry) => entry.packageName === "@gabrielvfonseca/existing",
    );
...
describe("ClawHub packed artifact identity", () => {
  const expectedIdentity = {
    ...
    expectedName: "@gabrielvfonseca/meta-provider",
