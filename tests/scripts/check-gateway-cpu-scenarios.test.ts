...
describe("gateway CPU scenario guard", () => {
  ...
  it("prebuilds private QA dist before running QA scenarios when it is missing", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        ...
        if (args.includes("@gabrielvfonseca/operator") && args.includes("qa")) {
    ...
    expect(calls[0]?.env).toMatchObject({
      ...
      OPERATOR_CONFIG_PATH: path.join(outputDir, "qa-state-root", "state", "operator.json"),
      OPERATOR_ENABLE_PRIVATE_QA_CLI: "1",
      OPERATOR_HOME: path.join(outputDir, "qa-state-root", "home"),
      OPERATOR_RUN_NODE_SKIP_DTS_BUILD: "1",
      OPERATOR_STATE_DIR: path.join(outputDir, "qa-state-root", "state"),
      OPERATOR_TEST_DISABLE_UPDATE_CHECK: "1",
      PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
      USERPROFILE: path.join(outputDir, "qa-state-root", "home"),
    });
    expect(calls[0]?.env?.OPERATOR_BUNDLED_PLUGIN_BUILD_IDS).toBeUndefined();
  });
  ...
  it("does not prebuild private QA dist when the required entries already exist", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      env: {
        HOME: "/real/user/home",
        OPERATOR_CONFIG_PATH: "/real/user/.operator/operator.json",
        OPERATOR_HOME: "/real/user/home",
        OPERATOR_STATE_DIR: "/real/user/.operator",
      },
      silent: true,
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        calls.push({ args, env: opts?.env });
