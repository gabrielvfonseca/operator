...
const RELEASE_CHECKS_PATH = ".github/workflows/operator-release-checks.yml";
const WORKFLOW_PATH = ".github/workflows/operator-release-telegram-qa.yml";
...
function runIdentityVerification(params: {
  ...
  const trustedWorkflowRef = `${repository}/.github/workflows/operator-release-telegram-qa.yml@refs/heads/main`;
  const invocation = params.invocation ?? "dispatch";
  const workflowRef =
    invocation === "dispatch"
      ? trustedWorkflowRef
      : `${repository}/.github/workflows/operator-release-checks.yml@refs/heads/release-ci/test`;
  const workflowRefName =
    invocation === "dispatch" ? "refs/heads/main" : "refs/heads/release-ci/test";
  const targetSha = "a".repeat(40);
  const workdir = tempDirs.make("operator-telegram-identity-");
  ...
  const payload = {
    aud: "operator-release-telegram-qa",
...
function runAdvisoryStatus(overrides: Record<string, string> = {}) {
  ...
  const workdir = tempDirs.make("operator-telegram-advisory-status-");
  const githubOutput = join(workdir, "github-output");
...
describe("release Telegram QA workflow", () => {
  ...
  it("dispatches one accepted trusted-main child from release checks", () => {
    ...
    const dispatchers = readdirSync(".github/workflows")
      ...
      .flatMap((name) => {
        ...
        return readFileSync(path, "utf8").includes('workflow="operator-release-telegram-qa.yml"')
          ? [path]
          : [];
      });
    expect(dispatchers).toEqual([RELEASE_CHECKS_PATH]);
    expect(reusableSource).toContain(
      "operator/operator/.github/workflows/operator-release-telegram-qa.yml@refs/heads/main",
    );
    expect(reusableSource).toContain(
      '--cert-identity "https://github.com/gabrielvfonseca/operator/.github/workflows/operator-release-telegram-qa.yml@refs/heads/main"',
    );
  ...
  it("bounds the OIDC identity request below the job timeout", () => {
    ...
    const oidcRequest = identityStep.run?.match(
      /curl --fail --silent --show-error[\s\S]*?audience=operator-release-telegram-qa/u,
    )?.[0];
  ...
  it("keeps the isolated SUT lifetime below the credential lease TTL", () => {
    ...
    expect(captureStep?.run).not.toContain("-name operator.json");
    expect(
      job?.steps?.findIndex(
        (step) => step.name === "Capture isolated Telegram runtime diagnostics",
      ),
  ...
  it.runIf(process.platform === "linux")("keeps only the newest eight gateway logs", () => {
    ...
    const workdir = tempDirs.make("operator-telegram-log-selector-");
    ...
    const logPaths = Array.from({ length: 12 }, (_, index) => {
      ...
      const logPath = join(logDir, `operator-${index}.log`);
      mkdirSync(logDir);
  ...
  it("retains only allowlisted verbose runtime diagnostics", () => {
    ...
    const workdir = tempDirs.make("operator-telegram-log-filter-");
  ...
  it.runIf(process.platform === "linux")(
    ...
    () => {
      ...
      const workdir = tempDirs.make("operator-telegram-model-proof-");
      const runtimeRoot = join(workdir, "runtime");
      const evidenceRoot = join(workdir, "evidence");
      const configPath = join(workdir, "operator.json");
      const duplicateConfigPath = join(workdir, "operator-duplicate.json");
  ...
  it("arms the boundary preload only in the gateway main thread", () => {
    ...
    const workdir = tempDirs.make("operator-telegram-preload-");
  ...
  it("reports the persisted inner launcher stage when namespace supervision fails", () => {
    ...
    const workdir = tempDirs.make("operator-telegram-launcher-stage-");
