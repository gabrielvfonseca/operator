...
function approvalRun(overrides: Record<string, unknown> = {}) {
  return {
    ...
    status: "in_progress",
    url: "https://github.com/gabrielvfonseca/operator/actions/runs/123",
...
function writeClawHubApproval(overrides: Record<string, unknown> = {}) {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify({
      ...
      targetSha: "a".repeat(40),
      packages: ["@gabrielvfonseca/meta-provider", "@gabrielvfonseca/voice-call"],
      ...overrides,
    })}\n`,
  );
  return approvalPath;
...
describe("scripts/validate-release-publish-approval.mjs", () => {
  it("accepts an in-progress release publish workflow run for approval", () => {
    ...
    expect(result.stdout).toContain(
      "Using release publish approval run 123: https://github.com/gabrielvfonseca/operator/actions/runs/123",
    );
  ...
  it("accepts an exact attested ClawHub bootstrap parent tuple", () => {
    ...
    const result = runApprovalScript(
      ...
      }),
      {
        ...
        RELEASE_PACKAGES: "@gabrielvfonseca/voice-call,@gabrielvfonseca/meta-provider",
  ...
  it("accepts a child workflow SHA that differs from the approving parent tooling", () => {
    ...
    const result = runApprovalScript(
      ...
      }),
      {
        ...
        EXPECTED_RUN_ATTEMPT: "2",
        RELEASE_APPROVAL_KIND: "clawhub-bootstrap",
        RELEASE_PACKAGES: "@gabrielvfonseca/meta-provider,@gabrielvfonseca/voice-call",
        RELEASE_TAG: "v2026.7.1-beta.3",
      },
    );
  ...
  it.each([
    ...
    ["package set", { packages: ["@gabrielvfonseca/meta-provider"] }, {}],
  ...
  it("accepts completed success or failure runs for direct recovery", () => {
    for (const conclusion of ["success", "failure"]) {
      ...
      expect(result.status).toBe(0);
      expect(result.stdout).toContain(
        `Using completed release publish run 123 (${conclusion}) for direct recovery: https://github.com/gabrielvfonseca/operator/actions/runs/123`,
      );
      expect(result.stderr).toBe("");
    }
  });
