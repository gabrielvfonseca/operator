  ...
  it("collects stable Android and Apple UI entries", async () => {
    ...
    expect(
      entries.some(
        (entry) =>
          entry.source ===
          "Paste the token configured on the gateway host. On the gateway host, run `operator config get gateway.auth.token`. If the gateway uses an environment variable instead, use `OPERATOR_GATEWAY_TOKEN`.",
      ),
    ).toBe(true);
    expect(
      entries.some((entry) =>
