...
describe("CLI startup benchmark script spawners", () => {
      ...
      expect(() =>
        execFileSync(
          ...
          ],
          {
            ...
            env: {
              ...process.env,
              OPERATOR_STARTUP_BENCH_ENFORCE_NONCANONICAL_ARCH: "1",
            },
            stdio: "pipe",
          },
        ),
  ...
  it("rejects malformed startup budget env vars before reading reports", () => {
    const result = spawnSync(process.execPath, ["scripts/test-cli-startup-bench-budget.mjs"], {
      ...
      env: {
        ...process.env,
        OPERATOR_STARTUP_BENCH_MAX_RSS_REGRESSION_PCT: "20pct",
      },
    });
    ...
    expect(result.stderr).toContain(
      "OPERATOR_STARTUP_BENCH_MAX_RSS_REGRESSION_PCT must be a non-negative number",
    );
