...
describe("dependency guard workflow", () => {
  ...
  it("keeps contents write scoped to the conditional autoscrub job", () => {
    ...
    expect(detectRunStep.env?.OPERATOR_DEPENDENCY_GUARD_MODE).toBe("detect");
    ...
    expect(autoscrubRunStep.env?.OPERATOR_DEPENDENCY_GUARD_AUTOSCRUB_TOKEN).toBe(
      "${{ steps.app-token.outputs.token || steps.app-token-fallback.outputs.token }}",
    );
    expect(autoscrubRunStep.env?.OPERATOR_DEPENDENCY_GUARD_MODE).toBe("autoscrub");
    expect(finalRunStep.env?.OPERATOR_DEPENDENCY_GUARD_MODE).toBe("enforce");
  });
  ...
  it("uses a dedicated checked-in script and bounded sticky comments", () => {
    ...
    expect(runStep.env?.OPERATOR_SECURITY_TEAM_SLUG).toBe("operator-secops");
    expect(runStep.env?.OPERATOR_SECURITY_APPROVERS).toBe("vincentkoc,steipete,joshavant");
  ...
  it("requires secops review for future workflow or guard changes", () => {
    ...
    expect(codeowners).toContain(
      "/.github/workflows/dependency-guard.yml @gabrielvfonseca/operator-secops",
    );
    expect(codeowners).toContain(
      "/test/scripts/dependency-guard-workflow.test.ts @gabrielvfonseca/operator-secops",
    );
    expect(codeowners).toContain(
      "/scripts/github/dependency-guard.mjs @gabrielvfonseca/operator-secops",
    );
    expect(codeowners).toContain("/package-lock.json @gabrielvfonseca/operator-secops");
    expect(codeowners).toContain("/npm-shrinkwrap.json @gabrielvfonseca/operator-secops");
    expect(codeowners).toContain(
      "/extensions/*/package-lock.json @gabrielvfonseca/operator-secops",
    );
    expect(codeowners).toContain(
      "/extensions/*/npm-shrinkwrap.json @gabrielvfonseca/operator-secops",
    );
  });
});
