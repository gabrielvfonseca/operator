...
function sanitizedEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  ...
  delete env.OPERATOR_PR_GATES_REMOTE;
  delete env.OPERATOR_TESTBOX;
  delete env.OPERATOR_TEST_HEAVY_CHECK_LOCK_HELD;
  delete env.OPERATOR_TSGO_HEAVY_CHECK_LOCK_HELD;
  delete env.OPERATOR_OXLINT_SKIP_LOCK;
  delete env.OPERATOR_HEAVY_CHECK_LOCK_TIMEOUT_MS;
  delete env.OPERATOR_HEAVY_CHECK_LOCK_POLL_MS;
...
describe("resolve_pr_gates_remote_mode", () => {
  ...
  ])("resolves OPERATOR_PR_GATES_REMOTE=$value to $expected", ({ value, expected }) => {
    const env: NodeJS.ProcessEnv = {};
    if (value !== undefined) {
      env.OPERATOR_PR_GATES_REMOTE = value;
    }
  ...
  it("rejects unsupported values", () => {
    const result = runGatesBash("resolve_pr_gates_remote_mode", {
      env: { OPERATOR_PR_GATES_REMOTE: "azure" },
    });
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("Unsupported OPERATOR_PR_GATES_REMOTE=azure");
  });

  it("rejects the hosted-gates conflict before touching the worktree", () => {
    const result = runGatesBash("prepare_gates 424242", {
      env: { OPERATOR_PR_GATES_REMOTE: "testbox", OPERATOR_TESTBOX: "1" },
    });
    expect(result.status).toBe(2);
    expect(result.stdout).toContain("conflicts with OPERATOR_TESTBOX=1");
  });
});
  ...
  it("extracts the last successful blacksmith-testbox timing stamp", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "GitHub Actions run: https://github.com/gabrielvfonseca/operator/actions/runs/1234",
        '{"not":"a stamp"}',
        "not json at all",
        ...
        "GitHub Actions run: https://github.com/gabrielvfonseca/operator/actions/runs/9999",
    ...
    expect(result.stdout.trim()).toBe(
      "tbx_final\thttps://github.com/gabrielvfonseca/operator/actions/runs/1234",
    );
  });
    ...
    const result = runGatesBash(
      ...
      ].join("\n"),
      {
        ...
        env: {
          ...
          OPERATOR_PR_GATES_REMOTE: "testbox",
        },
      },
    );
...
describe("prepare sync-head transitions", () => {
  it("publishes only appended fixups when main advances", () => {
    ...
    const result = runGatesBash(
      ...
      { cwd: repoDir, env: { OPERATOR_TESTBOX: "1" }, sourcePrepareCore: true },
    );
...
describe("prepare gate stamp transitions", () => {
  ...
  it("clears remote stamps when hosted gates replace remote proof", () => {
    ...
    const result = runGatesBash(
      ...
      { cwd: repoDir, env: { OPERATOR_TESTBOX: "1" } },
    );
...
describe("pr-gates-lock helper", () => {
  ...
  it("queues behind an existing holder and acquires after it exits", async () => {
    ...
    const second = spawnGateLockHolder(repoDir, secondStatus, {
      OPERATOR_HEAVY_CHECK_LOCK_POLL_MS: "50",
    });
  ...
  it("fails instead of holding when the wait times out", async () => {
    ...
    const statusFile = join(repoDir, "status");
    const holder = spawnGateLockHolder(repoDir, statusFile, {
      OPERATOR_HEAVY_CHECK_LOCK_TIMEOUT_MS: "200",
      OPERATOR_HEAVY_CHECK_LOCK_POLL_MS: "50",
    });
    await waitForExit(holder);
...
describe("gates.sh gate lock plumbing", () => {
  ...
  it("exports the held-lock contract while holding and clears it on release", () => {
    ...
    const result = runGatesBash(
      [
        ...
        'echo "held=${OPERATOR_TEST_HEAVY_CHECK_LOCK_HELD:-unset},${OPERATOR_TSGO_HEAVY_CHECK_LOCK_HELD:-unset},${OPERATOR_OXLINT_SKIP_LOCK:-unset}"',
        "jq -r .tool .git/operator-local-checks/heavy-check.lock/owner.json",
        "release_pr_gates_lock",
        'echo "released=${OPERATOR_TEST_HEAVY_CHECK_LOCK_HELD:-unset}"',
  ...
  it("skips acquisition when a parent already holds the lock", () => {
    ...
    const result = runGatesBash(
      ...
      { cwd: repoDir, env: { OPERATOR_TEST_HEAVY_CHECK_LOCK_HELD: "1" } },
    );
