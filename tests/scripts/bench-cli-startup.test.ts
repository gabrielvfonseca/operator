...
describe("bench-cli-startup", () => {
      ...
      try {
        ...
        const result = spawnSync(
          ...
          ],
          {
            ...
            env: {
              ...process.env,
              OPERATOR_TEST_CLI_STARTUP_TIMEOUT_KILL_GRACE_MS: "50",
              VITEST: "1",
            },
            timeout: 8_000,
          },
  ...
  it("parses config fixture gateway ports strictly from env", () => {
    ...
    expect(
      withEnv({ OPERATOR_GATEWAY_PORT: "45678" }, () =>
    ...
    for (const invalid of ["45678abc", "127.0.0.1:45678abc"]) {
      expect(() =>
        withEnv({ OPERATOR_GATEWAY_PORT: invalid }, () =>
          testing.buildConfigFixture({
            id: "gatewayHealthJson",
            name: "gateway health --json",
            args: ["gateway", "health", "--json"],
      ...
      ).toThrow("OPERATOR_GATEWAY_PORT must be an integer >= 1");
    }
  });
});
