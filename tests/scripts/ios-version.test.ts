...
describe("resolveIosVersion", () => {
  it("writes shared full commit and UTC timestamp settings for iOS builds", () => {
    ...
    expect(script).toContain("OPERATOR_GIT_COMMIT = ${RESOLVED_GIT_COMMIT}");
    expect(script).toContain("OPERATOR_BUILD_TIMESTAMP = ${RESOLVED_BUILD_TIMESTAMP}");
    expect(script).toContain('operator_resolve_git_commit "${ROOT_DIR}"');
    expect(script).toContain("operator_resolve_build_timestamp");
  });
  ...
  it("prints selected fields from the CLI", () => {
    const rootDir = writeIosFixture({
      ...
      changelog: "# Operator iOS Changelog\n\n## 2026.4.6\n\nStable notes.\n",
    });
    const result = spawnSync(
      process.execPath,
      [
