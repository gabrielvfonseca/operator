...
describe("scripts/test-docker-all scheduler", () => {
  ...
  it("prints CLI help without a stack trace", () => {
    ...
    expect(result.stdout).toContain("OPERATOR_DOCKER_ALL_* env vars");
  });
  ...
  it("plans from an isolated release harness without installed dependencies", () => {
    ...
    const result = spawnSync(
      ...
      [path.join(scriptsDir, "test-docker-all.mjs"), "--plan-json"],
      {
        ...
        env: {
          ...process.env,
          OPERATOR_DOCKER_ALL_PLAN_RELEASE_ALL: "1",
          OPERATOR_DOCKER_ALL_PROFILE: "release-path",
          OPERATOR_UPGRADE_SURVIVOR_TARGET_ROOT: process.cwd(),
        },
      },
    );
  ...
  it("rejects loose numeric runner env vars without a stack trace", () => {
    const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs", "--plan-json"], {
      ...
      env: {
        ...process.env,
        OPERATOR_DOCKER_ALL_PARALLELISM: "1e3",
      },
    });

    expect(result.status).toBe(1);
    expect(result.stdout).toBe("");
    expect(result.stderr).toContain("OPERATOR_DOCKER_ALL_PARALLELISM must be a positive integer");
    expect(result.stderr).not.toContain("at ");
  });
  ...
  it("reuses only registry-backed images in generated workflow reruns", () => {
    const localCommand = githubWorkflowRerunCommand(["install-e2e"], "a".repeat(40), {
      ...
      GITHUB_RUN_ID: "12345",
      OPERATOR_DOCKER_E2E_BARE_IMAGE: "operator-docker-e2e-bare:local",
      OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE: "operator-docker-e2e-functional:local",
      OPERATOR_DOCKER_E2E_PACKAGE_ARTIFACT_NAME: "docker-e2e-package",
    });
    ...
    const registryCommand = githubWorkflowRerunCommand(["install-e2e"], "b".repeat(40), {
      OPERATOR_DOCKER_E2E_BARE_IMAGE: "ghcr.io/operator/operator-docker-e2e-bare:test",
      OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE: "ghcr.io/operator/operator-docker-e2e-functional:test",
      OPERATOR_DOCKER_E2E_ALLOW_UNRELEASED_CHANGELOG: "true",
      OPERATOR_DOCKER_E2E_WORKFLOW_REF: "main",
      OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC: "operator@2026.5.3",
      OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPECS: "operator@2026.5.3 operator@2026.5.2",
      OPERATOR_UPGRADE_SURVIVOR_SCENARIOS: "plugin-dependency-cleanup",
    });
    ...
    try {
      ...
      await writeRunSummary(
        ...
        },
        {
          ...process.env,
          OPERATOR_DOCKER_E2E_ALLOW_UNRELEASED_CHANGELOG: "true",
          OPERATOR_DOCKER_E2E_SELECTED_SHA: selectedSha,
        },
      );
    ...
    try {
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...
          OPERATOR_DOCKER_ALL_DOCKER_LIMIT: "1e3",
          OPERATOR_DOCKER_ALL_DRY_RUN: "1",
          OPERATOR_DOCKER_ALL_LOG_DIR: logDir,
          OPERATOR_DOCKER_ALL_PREFLIGHT: "0",
          OPERATOR_DOCKER_ALL_TIMINGS: "0",
        },
      });

      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        "OPERATOR_DOCKER_ALL_DOCKER_LIMIT must be a positive integer",
      );
      expect(result.stderr).not.toContain("at ");
    } finally {
      rmSync(logDir, { force: true, recursive: true });
  ...
  it("rejects release-path configs that schedule zero Docker lanes", () => {
    ...
    try {
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...process.env,
          OPERATOR_DOCKER_ALL_CHUNK: "openwebui",
          OPERATOR_DOCKER_ALL_DRY_RUN: "1",
          OPERATOR_DOCKER_ALL_INCLUDE_OPENWEBUI: "0",
          OPERATOR_DOCKER_ALL_LOG_DIR: logDir,
          OPERATOR_DOCKER_ALL_PREFLIGHT: "0",
          OPERATOR_DOCKER_ALL_PROFILE: "release-path",
          OPERATOR_DOCKER_ALL_TIMINGS: "0",
  ...
  it("rejects candidate-controlled survivor omissions without trusted opt-in", () => {
    ...
    try {
      ...
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...process.env,
          OPERATOR_ALLOW_FROZEN_TARGET_SCENARIO_OMISSIONS: "0",
          OPERATOR_DOCKER_ALL_DRY_RUN: "1",
          OPERATOR_DOCKER_ALL_LANES: "published-upgrade-survivor",
          OPERATOR_DOCKER_ALL_TIMINGS: "0",
          OPERATOR_UPGRADE_SURVIVOR_TARGET_ROOT: root,
        },
      });
  ...
  it("writes a passing summary when a frozen target cannot run selected survivor lanes", () => {
    ...
    try {
      ...
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...process.env,
          OPERATOR_ALLOW_FROZEN_TARGET_SCENARIO_OMISSIONS: "1",
          OPERATOR_DOCKER_ALL_BUILD: "0",
          OPERATOR_DOCKER_ALL_LANES: "published-upgrade-survivor",
          OPERATOR_DOCKER_ALL_LOG_DIR: logDir,
          OPERATOR_DOCKER_ALL_TIMINGS: "0",
          OPERATOR_UPGRADE_SURVIVOR_SCENARIOS: "reported-issues",
          OPERATOR_UPGRADE_SURVIVOR_TARGET_ROOT: root,
        },
      });
    ...
    try {
      ...
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...
          OPERATOR_ALLOW_FROZEN_TARGET_SCENARIO_OMISSIONS: "1",
          OPERATOR_DOCKER_ALL_DRY_RUN: "1",
          OPERATOR_DOCKER_ALL_LANES: "published-upgrade-survivor,plugin-binding-command-escape",
          OPERATOR_DOCKER_ALL_TIMINGS: "0",
          OPERATOR_UPGRADE_SURVIVOR_SCENARIOS: "reported-issues",
          OPERATOR_UPGRADE_SURVIVOR_TARGET_ROOT: root,
        },
...
`,
    ...
    try {
      const baseEnv = {
        ...process.env,
        OPERATOR_DOCKER_E2E_IMAGE: "operator-test-image",
