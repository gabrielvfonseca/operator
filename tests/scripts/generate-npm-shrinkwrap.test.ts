...
describe("generate-npm-shrinkwrap", () => {
  ...
  it("validates shrinkwrap worker counts from flags and environment", () => {
    ...
    expect(resolveShrinkwrapJobs(undefined, { OPERATOR_NPM_SHRINKWRAP_JOBS: "2" })).toBe(2);
    expect(() => resolveShrinkwrapJobs("0", {})).toThrow("invalid OPERATOR_NPM_SHRINKWRAP_JOBS: 0");
  ...
  it("accepts strict npm shrinkwrap command timeout and buffer overrides", () => {
    expect(
      createNpmShrinkwrapExecOptions({ command: "npm", args: ["install"] }, "/tmp/package", {
        OPERATOR_NPM_SHRINKWRAP_COMMAND_MAX_BUFFER_BYTES: "1048576",
        OPERATOR_NPM_SHRINKWRAP_COMMAND_TIMEOUT_MS: "30000",
      }),
    ).toMatchObject({
      maxBuffer: 1024 * 1024,
      timeout: 30000,
  ...
  it("rejects loose npm shrinkwrap command timeout and buffer overrides", () => {
    expect(() =>
      createNpmShrinkwrapExecOptions({ command: "npm", args: ["install"] }, "/tmp/package", {
        OPERATOR_NPM_SHRINKWRAP_COMMAND_TIMEOUT_MS: "30s",
      }),
    ).toThrow("invalid OPERATOR_NPM_SHRINKWRAP_COMMAND_TIMEOUT_MS: 30s");
    expect(() =>
      createNpmShrinkwrapExecOptions({ command: "npm", args: ["install"] }, "/tmp/package", {
        OPERATOR_NPM_SHRINKWRAP_COMMAND_MAX_BUFFER_BYTES: "64mb",
      }),
    ).toThrow("invalid OPERATOR_NPM_SHRINKWRAP_COMMAND_MAX_BUFFER_BYTES: 64mb");
  });
  ...
  it("parses nested scoped package paths", () => {
    expect(
      parseLockPackagePath("node_modules/@gabrielvfonseca/codex/node_modules/@anthropic-ai/sdk"),
    ).toEqual([
      {
        name: "@gabrielvfonseca/codex",
        path: "node_modules/@gabrielvfonseca/codex",
      },
      {
        name: "@anthropic-ai/sdk",
        path: "node_modules/@gabrielvfonseca/codex/node_modules/@anthropic-ai/sdk",
      },
    ]);
  });
  ...
  it("disables embedded shrinkwraps that hide workspace overrides", () => {
    const lockfile = {
      packages: {
        ...
        "node_modules/@gabrielvfonseca/codex": {
          version: "0.75.4",
          hasShrinkwrap: true,
        },
        "node_modules/@gabrielvfonseca/codex/node_modules/protobufjs": {
          version: "7.5.9",
        },
        "node_modules/@gabrielvfonseca/codex/node_modules/fetch-blob": {
          version: "4.0.0",
        },
        "node_modules/@gabrielvfonseca/codex/node_modules/fetch-blob/node_modules/node-domexception":
          {
            version: "1.0.0",
          },
      },
    ...
    expect(lockfile.packages["node_modules/@gabrielvfonseca/codex"]).not.toHaveProperty(
      "hasShrinkwrap",
    );
    expect(
      lockfile.packages["node_modules/@gabrielvfonseca/codex/node_modules/protobufjs"],
    ).toBeUndefined();
  });
