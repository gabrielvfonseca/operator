...
describe("scripts/lib/plugin-prerelease-test-plan.mjs", () => {
  ...
  it("uses kitchen-sink npm and ClawHub scenarios as the registry install canary", () => {
    ...
    expect(lane).toEqual({
      command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin",
    ...
    expect(script).toContain("npm:@gabrielvfonseca/kitchen-sink@latest");
    expect(script).toContain("npm-latest-conformance");
    expect(script).toContain("npm-latest-adversarial");
    expect(script).toContain("npm:@gabrielvfonseca/kitchen-sink@beta");
    expect(script).toContain("clawhub:@gabrielvfonseca/kitchen-sink@latest");
    expect(script).toContain("clawhub:@gabrielvfonseca/kitchen-sink@beta");
    expect(script).toContain("OPERATOR_KITCHEN_SINK_PLUGIN_MAX_MEMORY_MIB");
    expect(script).toContain(
      "npm-to-clawhub|clawhub:@gabrielvfonseca/kitchen-sink@latest|operator-kitchen-sink-fixture|clawhub|success|basic||${KITCHEN_SINK_NPM_SPEC}",
    );
    ...
    expect(sweepScript).toContain("OPERATOR_KITCHEN_SINK_PERSONALITY");
  ...
  it("keeps kitchen-sink RPC coverage package-backed and resource-guarded", () => {
    ...
    expect(script).toContain("OPERATOR_ENTRY=/app/operator.mjs");
    expect(script).toContain("OPERATOR_KITCHEN_SINK_COMMAND_MAX_RSS_MIB");
    expect(script).toContain("docker_e2e_sample_stats_until_exit");
    expect(script).toContain("scripts/e2e/lib/docker-stats/assert-resource-ceiling.mjs");
  ...
  it("keeps the generic plugin Docker lane as an external install contract canary", () => {
    ...
    expect(lane).toEqual({
      command: "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
      e2eImageKind: "functional",
      live: false,
      name: "plugins",
  ...
  it("wires the full plugin prerelease plan into its release workflow", () => {
    ...
    expect(manifestEnv).toEqual({
      OPERATOR_CI_CHANGED_PATHS_JSON:
        "${{ steps.changed_scope.outputs.changed_paths_json || 'null' }}",
      OPERATOR_CI_CHECKOUT_REVISION: "${{ steps.checkout_ref.outputs.sha }}",
      OPERATOR_CI_DOCS_CHANGED:
        "${{ github.event_name == 'workflow_dispatch' && 'true' || steps.docs_scope.outputs.docs_changed }}",
      OPERATOR_CI_DOCS_ONLY:
        "${{ github.event_name == 'workflow_dispatch' && 'false' || steps.docs_scope.outputs.docs_only }}",
      OPERATOR_CI_EVENT_NAME: "${{ github.event_name }}",
      OPERATOR_CI_HISTORICAL_TARGET: "${{ steps.historical_target.outputs.eligible || 'false' }}",
      OPERATOR_CI_RELEASE_CANDIDATE_TARGET:
        "${{ steps.release_candidate_target.outputs.eligible || 'false' }}",
      OPERATOR_CI_REPOSITORY: "${{ github.repository }}",
      OPERATOR_CI_RUN_ANDROID:
    ...
    expect(inspectorRun.env).toEqual({
      OPERATOR_PLUGIN_INSPECTOR_ROOT: ".artifacts/plugin-inspector",
      OPERATOR_PLUGIN_INSPECTOR_VERSION: "0.3.10",
    });
    expect(inspectorRun.run).toContain("extensions/");
    expect(inspectorRun.run).toContain(
      'npm exec --yes "@gabrielvfonseca/plugin-inspector@${OPERATOR_PLUGIN_INSPECTOR_VERSION}" -- ci',
    );
  ...
  it("keeps release-check reruns independent while cancelling superseded umbrella runs", () => {
    ...
    expect(dockerPreflightStep?.run).toContain(
      '--build-arg OPERATOR_EXTENSIONS="diagnostics-otel,codex"',
    );
    expect(
  ...
  it("keeps the live-ish availability check redacted", () => {
    ...
    expect(output).toContain("channel-discord: present (DISCORD_TOKEN, OPERATOR_DISCORD_TOKEN)");
