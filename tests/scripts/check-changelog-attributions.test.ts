...
function createRepoWithPrChangelogDiff(entry: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "operator-changelog-credit-"));
...
function createRepoWithChangelog(content: string): string {
  const repo = mkdtempSync(path.join(os.tmpdir(), "operator-changelog-policy-"));
...
function validateChangelogEntry(repo: string, contrib: string): string {
  return run(
    ...
    [
      "-c",
      'source "$OPERATOR_PR_CHANGELOG_SH"; validate_changelog_entry_for_pr 123 "$OPERATOR_TEST_CONTRIB"',
    ],
    {
      OPERATOR_PR_CHANGELOG_SH: changelogScriptPath,
      OPERATOR_TEST_CONTRIB: contrib,
    },
  );
}
...
function validateChangelogAttributionPolicy(repo: string): string {
  return run(
    repo,
    "bash",
    ["-c", 'source "$OPERATOR_PR_CHANGELOG_SH"; validate_changelog_attribution_policy'],
    {
      OPERATOR_PR_CHANGELOG_SH: changelogScriptPath,
    },
  );
...
describe("check-changelog-attributions", () => {
  it("flags forbidden bot, org, and maintainer thanks attributions", () => {
    const content = [
      ...
      "- Org-owned fix. Thanks @operator.",
      "- Maintainer-owned fix. Thanks @steipete.",
      "- Mixed credit. Thanks @contributor and @Operator.",
    ...
    expect(findForbiddenChangelogThanks(content)).toEqual([
      ...
      { line: 2, handle: "@gabrielvfonseca/operator", text: "- Org-owned fix. Thanks @operator." },
      { line: 3, handle: "steipete", text: "- Maintainer-owned fix. Thanks @steipete." },
      {
        line: 4,
        handle: "@gabrielvfonseca/operator",
        text: "- Mixed credit. Thanks @contributor and @Operator.",
      },
  ...
  it("checks every thanked handle on a changelog line", () => {
    expect(
      findForbiddenChangelogThanks("- Mixed credit (#123). Thanks @operator and @alice."),
    ).toEqual([
      {
        line: 1,
        handle: "@gabrielvfonseca/operator",
        text: "- Mixed credit (#123). Thanks @operator and @alice.",
      },
    ]);
  });
  ...
  it("uses one attribution predicate for scanner and shell checks", () => {
    ...
    expect(isForbiddenChangelogThanksHandle("@gabrielvfonseca/operator")).toBe(true);
    ...
    expect(isForbiddenChangelogThanksHandle("operator-clawsweeper")).toBe(true);
    expect(isForbiddenChangelogThanksHandle("operator-clawsweeper[bot]")).toBe(true);
      ...
      try {
        run(
          ...
          [
            ...
            `
set -euo pipefail
source "$OPERATOR_PR_COMMON_SH"
source "$OPERATOR_PR_CHANGELOG_SH"
source "$OPERATOR_PR_GATES_SH"
...
validate_changelog_entry_for_pr() { printf 'entry:%s:%s\\n' "$1" "$2" >>"$OPERATOR_TEST_CALLS"; }
run_quiet_logged() { printf 'gate:%s\\n' "$1" >>"$OPERATOR_TEST_CALLS"; }

prepare_gates 123
`,
          ],
          {
            OPERATOR_PR_COMMON_SH: commonScriptPath,
            OPERATOR_PR_CHANGELOG_SH: changelogScriptPath,
            OPERATOR_PR_GATES_SH: gatesScriptPath,
            OPERATOR_TEST_CALLS: callsPath,
            OPERATOR_TESTBOX: "0",
          },
        );
      } catch (error) {
...
`,
        ...
        {
          OPERATOR_ALLOW_ROOT_CHANGELOG_PR: "1",
          OPERATOR_PR_COMMON_SH: commonScriptPath,
          OPERATOR_PR_CHANGELOG_SH: changelogScriptPath,
          OPERATOR_PR_GATES_SH: gatesScriptPath,
          OPERATOR_TEST_CALLS: callsPath,
