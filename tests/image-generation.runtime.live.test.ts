...
import { loadConfig, type OperatorConfig } from "../src/config/config.js";
import {
...
const REQUIRE_PROFILE_KEYS =
  isLiveProfileKeyModeEnabled() || isTruthyEnvValue(process.env.OPERATOR_LIVE_REQUIRE_PROFILE_KEYS);
const describeLive = LIVE ? describe : describe.skip;
const providerFilter = parseCsvFilter(process.env.OPERATOR_LIVE_IMAGE_GENERATION_PROVIDERS);
const caseFilter = parseCaseFilter(process.env.OPERATOR_LIVE_IMAGE_GENERATION_CASES);
const envModelMap = parseProviderModelMap(process.env.OPERATOR_LIVE_IMAGE_GENERATION_MODELS);
const DEFAULT_LIVE_IMAGE_GENERATION_TIMEOUT_MS = 120_000;
const LIVE_IMAGE_GENERATION_TIMEOUT_MS = resolvePositiveIntegerEnv(
  process.env.OPERATOR_LIVE_IMAGE_GENERATION_TIMEOUT_MS,
...
function withPluginsEnabled(cfg: OperatorConfig): OperatorConfig {
  return {
    ...cfg,
    plugins: {
      ...cfg.plugins,
