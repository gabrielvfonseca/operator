...
} from "../../src/acp/client-helpers.js";
import {
...
describe("resolveAcpClientSpawnEnv", () => {
  ...
  it("preserves OPERATOR_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        OPERATOR_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["OPERATOR_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default Operator bridge", () => {
    ...
    expect(env.OPERATOR_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.OPERATOR_SHELL).toBe("acp-client");
  });
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("OPERATOR_API_KEY")).toBe(false);
  });
});
