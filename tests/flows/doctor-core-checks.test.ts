...
import type { SkillStatusEntry } from "../../src/skills/discovery/status.js";
...
} from "../../src/flows/doctor-core-checks.js";
import { clearHealthChecksForTest } from "../../src/flows/health-check-registry.js";
import type { HealthCheck, HealthFinding, HealthRepairEffect } from "../../src/flows/health-checks.js";
...
function createSkill(overrides: Partial<SkillStatusEntry> = {}): SkillStatusEntry {
  return {
    ...
    source: "workspace",
    bundled: false,
    filePath: "/tmp/operator-test-workspace/skills/missing-tool/SKILL.md",
    baseDir: "/tmp/operator-test-workspace/skills/missing-tool",
    ...
    requirements: {
      bins: ["operator-test-missing-skill-bin"],
      anyBins: [],
      env: [],
      config: [],
      os: [],
...
describe("CORE_HEALTH_CHECKS", () => {
  ...
  it("converts unavailable skills into repair-capable health findings", async () => {
    ...
    const cfg: OperatorConfig = {
      agents: {
        defaults: {
          workspace: "/tmp/operator-test-workspace",
          skills: ["missing-tool"],
        },
      },
    };
    ...
    const findings = await check.detect({
      ...
      runtime,
      cfg,
      cwd: "/tmp/operator-test-workspace",
    });
  ...
  it("does not execute or warn for valid exec SecretRefs during default gateway auth lint checks", async () => {
    tmp = await fs.mkdtemp(join(tmpdir(), "operator-health-exec-ref-"));
    ...
    const check = getCheck(
      createCoreHealthChecks(
        createDeps({
          async collectWorkspaceSuggestionNotes(): Promise<readonly string[]> {
            return [
              "- Tip: back up the agent workspace in a private git repo; keep ~/.operator out of git (credentials, sessions). Details: /concepts/agent-workspace#git-backup-recommended",
...
describe("core/doctor/bootstrap-size", () => {
  ...
  it("honors the per-agent bootstrapMaxChars override in health findings", async () => {
    tmp = await fs.mkdtemp(join(tmpdir(), "operator-health-bootstrap-"));
