...
import { listAuditEvents, recordAuditEvent } from "../../src/audit/audit-event-store.js";
import type { AuditEventInput, MessageAuditEventInput } from "../../src/audit/audit-event-types.js";
...
function createDatabaseOptions() {
  return { env: { OPERATOR_STATE_DIR: makeTempDir(tempDirs, "operator-message-audit-") } };
}
...
describe("message audit persistence", () => {
  ...
  it("fails closed when retained message references lose their identity key", () => {
    ...
    const reopened = openOperatorStateDatabase(database).db;
    expect(
      (
