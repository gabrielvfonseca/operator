...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    setTestEnvValue("OPERATOR_LIVE_TEST", "1");
    setTestEnvValue("OPERATOR_LIVE_TEST_QUIET", "1");
    setTestEnvValue("OPERATOR_CONFIG_PATH", "~/custom-operator.json5");
    setTestEnvValue("OPERATOR_TEST_HOME", priorIsolatedHome);
    setTestEnvValue("OPERATOR_STATE_DIR", path.join(priorIsolatedHome, ".operator"));
    ...
    expect(process.env.OPERATOR_TEST_HOME).toBe(testEnv.tempHome);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    setTestEnvValue("USERPROFILE", realHome);
    setTestEnvValue("OPERATOR_LIVE_TEST", "1");
    setTestEnvValue("OPERATOR_LIVE_USE_REAL_HOME", "1");
    setTestEnvValue("OPERATOR_LIVE_TEST_QUIET", "1");
  ...
  it("keeps hermetic mode isolated when live flags request the real HOME", () => {
    ...
    expect(process.env.OPERATOR_LIVE_TEST).toBeUndefined();
    expect(process.env.OPERATOR_LIVE_GATEWAY).toBeUndefined();
    expect(process.env.OPERATOR_LIVE_USE_REAL_HOME).toBeUndefined();
    expect(process.env.OPERATOR_BUNDLED_PLUGINS_DIR).not.toBe(callerPluginDir);
    expect(path.basename(process.env.OPERATOR_BUNDLED_PLUGINS_DIR ?? "")).toBe("extensions");
    expect(process.env.OPERATOR_TEST_TRUST_BUNDLED_PLUGINS_DIR).toBe("1");
    expect(process.env.OPERATOR_DISABLE_BUNDLED_PLUGINS).toBeUndefined();
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    deleteTestEnvValue("OPERATOR_LIVE_TEST");
    deleteTestEnvValue("OPERATOR_LIVE_GATEWAY");
    deleteTestEnvValue("OPERATOR_LIVE_USE_REAL_HOME");
    deleteTestEnvValue("OPERATOR_LIVE_TEST_QUIET");
