...
import { runPostUpgradeProbes } from "../../src/commands/doctor-post-upgrade.js";
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          ...
          plugins: [
            {
              pluginId: "ghost",
              manifestPath: path.join(pluginDir, "operator.plugin.json"),
              rootDir: pluginDir,
              enabled: true,
    ...
    try {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({
          ...
          version: "0.0.1",
          type: "module",
          operator: { extensions: ["../outside/leak.js"] },
        }),
        "utf-8",
      );
      await fs.writeFile(
