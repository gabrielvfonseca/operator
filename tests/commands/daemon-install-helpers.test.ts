...
import { writeStateDirDotEnv } from "../../src/config/test-helpers.js";
...
const mocks = vi.hoisted(() => ({
  ...
  resolveOperatorWrapperPath: vi.fn(),
...
vi.mock("../daemon/program-args.js", () => ({
  ...
  resolveOperatorWrapperPath: mocks.resolveOperatorWrapperPath,
}));
...
import { buildGatewayInstallPlan, gatewayInstallErrorHint } from "../../src/commands/daemon-install-helpers.js";
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { OPERATOR_PORT: "3000" },
  } = params;
  const workingDirectory = Object.hasOwn(params, "workingDirectory")
    ? params.workingDirectory
    : "/Users/me";
  mocks.resolvePreferredNodePath.mockResolvedValue("/opt/node");
  mocks.resolveOperatorWrapperPath.mockImplementation(async (value: string | undefined) =>
...
async function buildPluginConfigExecSecretRefPlan(home: string) {
  mockNodeGatewayPlanFixture({ serviceEnvironment: { OPERATOR_PORT: "3000" } });
...
describe("buildGatewayInstallPlan", () => {
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ OPERATOR_PORT: "3000" });
  ...
  it("passes OPERATOR_WRAPPER through program args and managed service env", async () => {
    ...
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
    expect(
      firstMockArg(mocks.buildServiceEnvironment, "buildServiceEnvironment").env?.OPERATOR_WRAPPER,
    ).toBe(wrapperPath);
    expect(plan.environment.OPERATOR_WRAPPER).toBe(wrapperPath);
  });
  ...
  it("clears a Windows wrapper env that points at the generated gateway.cmd script", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        OPERATOR_WRAPPER: selfWrapperPath,
      }),
      port: 3000,
      runtime: "node",
      platform: "win32",
    ...
    expect(plan.environment.OPERATOR_WRAPPER).toBeUndefined();
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Ignoring OPERATOR_WRAPPER because it points to the Windows task script",
      ),
    );
  });
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.OPERATOR_PORT).toBe("3000");
    expect(plan.environment.OPERATOR_SERVICE_MANAGED_ENV_KEYS).toBe(
  ...
  it("renders config env SecretRefs as file-backed managed values on Linux", async () => {
    ...
    expect(plan.environment.OPERATOR_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
  ...
  it("keeps config env SecretRefs managed when auth profiles reuse the key", async () => {
    ...
    expect(plan.environment.OPERATOR_SERVICE_MANAGED_ENV_KEYS).toBe("OPENAI_API_KEY");
  });
  ...
  it("includes passEnv values for configured exec SecretRef providers", async () => {
    ...
    expect(plan.environment.OPERATOR_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        OPERATOR_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
        gateway: {
          auth: {
            token: { source: "env", provider: "default", id: "OPERATOR_GATEWAY_TOKEN" },
          },
        },
      },
    });

    expect(plan.environment.OPERATOR_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.OPERATOR_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("retains .env values for macOS LaunchAgent env SecretRefs", async () => {
    ...
    expect(plan.environment.OPERATOR_SERVICE_MANAGED_ENV_KEYS).toBe("MINIMAX_API_KEY");
  });
  ...
  it("retains config SecretRef env values for macOS LaunchAgent env files", async () => {
    ...
    expect(plan.environment.OPERATOR_SERVICE_MANAGED_ENV_KEYS).toBe("TELEGRAM_DEFAULT_BOTTOKEN");
  });
  ...
  it("retains existing generated env-file SecretRef values for macOS LaunchAgent regeneration", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        OPERATOR_SERVICE_MANAGED_ENV_KEYS:
  ...
  it("does not retain config env values for macOS LaunchAgent env files", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        OPERATOR_SERVICE_MANAGED_ENV_KEYS: "BRAVE_API_KEY",
      },
      existingEnvironmentValueSources: {
        BRAVE_API_KEY: "file",
        OPERATOR_SERVICE_MANAGED_ENV_KEYS: "inline",
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.OPERATOR_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        OPERATOR_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.OPERATOR_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    expect(plan.environment.OPERATOR_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.OPERATOR_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("collectPreservedExistingServiceEnvVars — operator opt-in allowlist", () => {
  ...
  it("continues to drop stale OPERATOR_ALLOW_ROOT", async () => {
    const result = await buildEnvironment({ OPERATOR_ALLOW_ROOT: "1" });
    expect(result.OPERATOR_ALLOW_ROOT).toBeUndefined();
  });

  it("preserves OPERATOR_CLI_CONTAINER_BYPASS and OPERATOR_CONTAINER_HINT", async () => {
    const result = await buildEnvironment({
      OPERATOR_CLI_CONTAINER_BYPASS: "1",
      OPERATOR_CONTAINER_HINT: "ci",
    });
    expect(result.OPERATOR_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.OPERATOR_CONTAINER_HINT).toBe("ci");
  });

  it("still drops arbitrary OPERATOR_FOO", async () => {
    const result = await buildEnvironment({ OPERATOR_FOO: "bar" });
    expect(result.OPERATOR_FOO).toBeUndefined();
  });

  it("preserves container opt-ins while dropping unrelated OPERATOR_* keys", async () => {
    const result = await buildEnvironment({
      OPERATOR_CLI_CONTAINER_BYPASS: "1",
      OPERATOR_CONTAINER_HINT: "ci",
      OPERATOR_BAZ: "qux",
    ...
    expect(result.OPERATOR_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.OPERATOR_CONTAINER_HINT).toBe("ci");
    expect(result.OPERATOR_BAZ).toBeUndefined();
  });
});
