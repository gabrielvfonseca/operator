...
} from "../../src/commands/doctor-plugin-registry.js";
import { DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV } from "../../src/commands/doctor/shared/plugin-registry-migration.js";
...
function makeTempDir() {
  return makeTrackedTempDir("operator-doctor-plugin-registry", tempDirs);
}
...
function createBundledCandidate(params: {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "operator.plugin.json"),
    JSON.stringify({
      id: params.id,
      name: params.id,
      configSchema: { type: "object" },
...
function expectedPluginIndexRecord(params: {
  ...
  return {
    ...
    manifestPath: path.join(params.rootDir, "operator.plugin.json"),
...
describe("maybeRepairPluginRegistryState", () => {
  ...
  it("warns about stale managed npm packages that shadow bundled plugins", async () => {
    ...
    expect(vi.mocked(note).mock.calls.join("\n")).toContain(
      "@gabrielvfonseca/google-meet@2026.5.2",
    );
  ...
  it("removes stale managed npm packages from the package lock during repair", async () => {
    ...
    expect(packageLock.packages).not.toHaveProperty("node_modules/@gabrielvfonseca/google-meet");
    expect(packageLock.dependencies).not.toHaveProperty("@gabrielvfonseca/google-meet");
    expect(packageLock.dependencies).toHaveProperty("other-plugin");
  });

  it("repairs managed npm operator peer links during registry repair", async () => {
    const stateDir = makeTempDir();
    const managed = createManagedNpmPlugin({
      stateDir,
      id: "codex",
    ...
    const linkPath = path.join(managed.packageDir, "node_modules", "@gabrielvfonseca/operator");
    expect(fs.lstatSync(linkPath).isSymbolicLink()).toBe(true);
    expect(fs.realpathSync(linkPath)).toBe(fs.realpathSync(process.cwd()));
    expect(vi.mocked(note).mock.calls.join("\n")).toContain("Repaired Operator host peer link");
  });

  it("warns about broken managed npm operator peer links without repairing them", async () => {
    const stateDir = makeTempDir();
    const managed = createManagedNpmPlugin({
      stateDir,
      id: "codex",
    ...
    expect(notes).toContain("operator doctor --fix");
