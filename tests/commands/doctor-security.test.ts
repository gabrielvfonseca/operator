...
import { noteSecurityWarnings } from "../../src/commands/doctor-security.js";
...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  beforeEach(() => {
    ...
    pluginRegistry.list = [];
    prevToken = process.env.OPERATOR_GATEWAY_TOKEN;
    prevPassword = process.env.OPERATOR_GATEWAY_PASSWORD;
    prevHome = process.env.HOME;
    prevServiceKind = process.env.OPERATOR_SERVICE_KIND;
  ...
  afterEach(() => {
    ...
    if (prevPassword === undefined) {
      ...
      process.env.OPERATOR_GATEWAY_PASSWORD = prevPassword;
    }
    ...
    if (prevServiceKind === undefined) {
      ...
      process.env.OPERATOR_SERVICE_KIND = prevServiceKind;
    }
  });
  ...
  it("warns when exposed without auth", async () => {
    const cfg = { gateway: { bind: "lan" } } as OperatorConfig;
  ...
  it("uses env token to avoid critical warning", async () => {
    process.env.OPERATOR_GATEWAY_TOKEN = "token-123";
    const cfg = { gateway: { bind: "lan" } } as OperatorConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
  ...
  it("warns when OPERATOR_GATEWAY_TOKEN env conflicts with gateway.auth.token config (#74271)", async () => {
  ...
  it("does not warn when only env token is set without config token", async () => {
    process.env.OPERATOR_GATEWAY_TOKEN = "env-token-only";
    const cfg = { gateway: { bind: "lan" } } as OperatorConfig;
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).not.toContain("OPERATOR_GATEWAY_TOKEN overrides");
  });
  ...
  it("does not warn inside the managed gateway service credential context", async () => {
    ...
    await noteSecurityWarnings(cfg);
    const message = lastMessage();
    expect(message).not.toContain("OPERATOR_GATEWAY_TOKEN conflicts");
  });
  ...
  it("skips warning for loopback bind", async () => {
    const cfg = { gateway: { bind: "loopback" } } as OperatorConfig;
    await noteSecurityWarnings(cfg);
    expect(note).not.toHaveBeenCalled();
  });

  it("treats unset bind as loopback for host-side doctor checks", async () => {
    const cfg = { gateway: {} } as OperatorConfig;
    await noteSecurityWarnings(cfg);
    expect(note).not.toHaveBeenCalled();
  });
  ...
  it("shows explicit dmScope config command for multi-user DMs", async () => {
    ...
    const cfg = { session: { dmScope: "main" } } as OperatorConfig;
    await noteSecurityWarnings(cfg);
  ...
  it("warns when normalized tools.exec mode is broader than host exec defaults", async () => {
    ...
    expect(message).toContain('defaults.security="allowlist"');
    expect(message).not.toContain("Operator default");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    await noteSecurityWarnings({} as OperatorConfig);
