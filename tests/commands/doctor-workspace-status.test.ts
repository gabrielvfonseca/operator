...
import type { PluginVersionDriftReport } from "../../src/plugins/plugin-version-drift.js";
import {
  createPluginLoadResult,
  createPluginRecord,
  createTypedHook,
} from "../../src/plugins/status.test-fixtures.js";
import {
  collectWorkspaceStatusHealthFindings,
  noteWorkspaceStatus,
} from "../../src/commands/doctor-workspace-status.js";
...
describe("noteWorkspaceStatus", () => {
  ...
  it("collects plugin version drift as structured findings", async () => {
    ...
    expect(findings).toEqual([
      expect.objectContaining({
        ...
        fixHint: expect.stringContaining("operator plugins update codex"),
      }),
    ]);
  });
  ...
  it("collects compatibility warnings, plugin diagnostics, and TaskFlow recovery findings", async () => {
    ...
    expect(findings).toEqual([
      ...
      expect.objectContaining({
        ...
        fixHint: expect.stringContaining("operator tasks flow show flow-123"),
      }),
    ]);
  });
  ...
  it("surfaces active official managed plugin version drift", async () => {
    ...
    try {
      ...
      expect(body).toContain("operator plugins update codex");
      expect(body).toContain("operator gateway restart");
    } finally {
      noteSpy.mockRestore();
    }
  });
  ...
  it("uses package-version update commands for exact npm plugin drift", async () => {
    ...
    try {
      ...
      expect(body).not.toContain("operator plugins update brave");
      expect(body).toContain("operator gateway restart");
    } finally {
      noteSpy.mockRestore();
    }
  ...
  it("adds TaskFlow recovery hints for broken blocked flows", async () => {
    ...
    try {
      ...
      expect(body).toContain("operator tasks flow show <flow-id>");
    } finally {
      noteSpy.mockRestore();
    }
  });
