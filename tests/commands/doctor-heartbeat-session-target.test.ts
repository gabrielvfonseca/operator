...
import { describeHeartbeatSessionTargetIssues } from "../../src/commands/doctor-heartbeat-session-target.js";

describe("describeHeartbeatSessionTargetIssues", () => {
  let tmpDir: string;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-heartbeat-doctor-"));
  });
  ...
  function cfgWithSession(session: string, target: string | null = "slack"): OperatorConfig {
  ...
  function writeStore(cfg: OperatorConfig, entries: Record<string, unknown>) {
