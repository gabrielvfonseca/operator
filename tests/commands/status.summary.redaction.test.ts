...
import { redactSensitiveStatusSummary } from "../../src/commands/status.summary.js";
import type { SessionStatus, StatusSummary } from "../../src/commands/status.types.js";
...
describe("redactSensitiveStatusSummary", () => {
  it("removes sensitive session and path details while preserving summary structure", () => {
    const input: StatusSummary = {
      ...
      sessions: {
        paths: ["/tmp/operator/sessions.json"],
        count: 1,
        ...
        byAgent: [
          {
            agentId: "main",
            path: "/tmp/operator/main-sessions.json",
            count: 1,
            recent: [createRecentSessionRow()],
          },
        ],
