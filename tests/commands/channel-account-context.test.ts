...
import { resolveDefaultChannelAccountContext } from "../../src/commands/channel-account-context.js";
...
describe("resolveDefaultChannelAccountContext", () => {
  it("uses enabled/configured defaults when hooks are missing", async () => {
    ...
    const result = await resolveDefaultChannelAccountContext(plugin, {} as OperatorConfig);
  ...
  it("keeps strict mode fail-closed and degrades read_only mode when resolveAccount throws", async () => {
    ...
    await expect(resolveDefaultChannelAccountContext(plugin, {} as OperatorConfig)).rejects.toThrow(
      /missing secret/i,
    );
