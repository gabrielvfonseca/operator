...
import { configureCommandFromSectionsArg } from "../../src/commands/configure.commands.js";
...
describe("configureCommandFromSectionsArg", () => {
  ...
  it("fails closed on a non-interactive terminal before reaching the wizard (#93953)", async () => {
    ...
    expect(message).toContain(formatCliCommand("operator config set"));
    expect(message).toContain(formatCliCommand("operator config validate"));
