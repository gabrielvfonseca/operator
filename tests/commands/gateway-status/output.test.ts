...
import type { GatewayProbeResult } from "../../../src/gateway/probe.js";
import type { RuntimeEnv } from "../../../src/runtime.js";
import type { GatewayStatusProbedTarget } from "../../../src/commands/gateway-status/probe-run.js";
...
describe("gateway status output", () => {
  ...
  it("warns with diagnostic next steps when no probes or Bonjour discovery find a gateway", () => {
    ...
    expect(warnings.find((entry) => entry.code === "no_gateway_reachable")).toStrictEqual({
      ...
      message:
        "No gateway answered any probe and Bonjour discovery returned no local gateways. Run `operator gateway status --deep --require-rpc` to inspect service state, config paths, listener owners, and logs; include `ss -ltnp` or `lsof -nP -iTCP:<port> -sTCP:LISTEN` for the configured port when filing a report.",
