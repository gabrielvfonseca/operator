...
import { createTempHomeEnv, type TempHomeEnv } from "../../src/test-utils/temp-home.js";
import * as backupShared from "../../src/commands/backup-shared.js";
import {
  buildBackupArchivePath,
  buildBackupArchiveRoot,
  type BackupAsset,
  resolveBackupPlanFromDisk,
} from "../../src/commands/backup-shared.js";
import {
  ...
  tarCreateMock,
} from "../../src/commands/backup.test-support.js";
...
describe("backup commands", () => {
  ...
  beforeAll(async () => {
    tempHome = await createTempHomeEnv("operator-backup-test-");
  });
  ...
  async function withInvalidWorkspaceBackupConfig<T>(fn: (runtime: RuntimeEnv) => Promise<T>) {
    const stateDir = path.join(tempHome.home, ".operator");
    const configPath = path.join(tempHome.home, "custom-config.json");
    await fs.writeFile(path.join(stateDir, "operator.json"), JSON.stringify({}), "utf8");
  ...
  it("formats backup archive timestamps in local time", () => {
    ...
    try {
      ...
      expect(buildBackupArchiveRoot(Date.UTC(2026, 2, 14, 1, 2, 3, 456))).toBe(
        "2026-03-14T09-02-03.456+08-00-operator-backup",
      );
      setTestEnvValue("TZ", "America/New_York");
      expect(buildBackupArchiveRoot(Date.UTC(2026, 2, 14, 1, 2, 3, 456))).toBe(
        "2026-03-13T21-02-03.456-04-00-operator-backup",
      );
    } finally {
      envSnapshot.restore();
    }
  ...
  it("orders coverage checks by canonical path so symlinked workspaces do not duplicate state", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".operator");
    const workspaceDir = path.join(stateDir, "workspace");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-workspace-link-"));
  ...
  it("creates an archive with a manifest and external workspace payload", async () => {
    const stateDir = path.join(tempHome.home, ".operator");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "operator-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-backups-"));
  ...
  it("keeps volatile-skip notices out of json output", async () => {
    const stateDir = path.join(tempHome.home, ".operator");
    const backupDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-backups-json-"));
    try {
      const runtime = createBackupTestRuntime();
  ...
  it("rejects symlinked output paths even when intermediate directories do not exist yet", async () => {
    ...
    const stateDir = path.join(tempHome.home, ".operator");
    const symlinkDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-backup-link-"));
  ...
  it("falls back to the home directory when cwd is inside a backed-up source tree", async () => {
    ...
    if (process.platform !== "win32") {
      const linkParent = await fs.mkdtemp(path.join(os.tmpdir(), "operator-backup-cwd-link-"));
