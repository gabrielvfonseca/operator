...
} from "../../../../src/commands/doctor/cron/runtime-policy-migration.js";
import {
  collectStoredCronCodexRuntimePolicyTargets,
  cronCodexRuntimePolicyTargetKey,
  normalizeStoredCronJobs,
} from "../../../../src/commands/doctor/cron/store-migration.js";
...
describe("normalizeStoredCronJobs", () => {
  ...
  it("converts legacy agent command prompts into command cron payloads", () => {
    const command =
      "cd /home/operator/.razor/quant && ./scripts/system/run_position_control.sh --write-card --silent-token NO_REPLY";
    const { job, result } = normalizeOneJob(
      makeLegacyJob({
        ...
        payload: {
          ...
          message: [
            ...
            "Command to run:",
            `- command: ${command}`,
            "- workdir: /home/operator/.razor/quant",
            "- background: false",
            "- timeout: 840",
            "",
