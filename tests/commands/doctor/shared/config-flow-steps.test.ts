...
import type { DoctorConfigPreflightResult } from "../../../../src/commands/doctor-config-preflight.js";
...
import { applyLegacyCompatibilityStep, applyUnknownConfigKeyStep } from "../../../../src/commands/doctor/shared/config-flow-steps.js";
...
describe("doctor config flow steps", () => {
  ...
  it("collects legacy compatibility issue lines and preview fix hints", () => {
    ...
    expect(result.state.fixHints).toStrictEqual([
      'Run "operator doctor --fix" to migrate legacy config keys.',
    ]);
  ...
  it("removes unknown keys and adds preview hint", () => {
    ...
    expect(result.state.fixHints).toStrictEqual([
      'Run "operator doctor --fix" to remove these keys.',
    ]);
  });
