...
import { collectStalePluginRuntimeSymlinkHealthFindings } from "../../../../src/commands/doctor/shared/plugin-runtime-symlinks.js";
...
describe("plugin runtime symlink health findings", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-plugin-runtime-symlinks-"));
  });
  ...
  it("reports symlinks that point inside classified stale roots", async () => {
    ...
    const nodeModulesRoot = path.dirname(packageRoot);
    const legacyRoot = path.join(tempDir, "state", "plugin-runtime-deps");
    const existingTarget = path.join(legacyRoot, "operator-demo", "node_modules", "left-pad");
