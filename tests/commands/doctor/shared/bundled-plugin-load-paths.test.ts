...
import * as bundledSources from "../../../../src/plugins/bundled-sources.js";
import {
  collectBundledPluginLoadPathWarnings,
  maybeRepairBundledPluginLoadPaths,
  scanBundledPluginLoadPathMigrations,
} from "../../../../src/commands/doctor/shared/bundled-plugin-load-paths.js";

function bundled(pluginId: string, localPath: string): BundledPluginSource {
  return {
    pluginId,
    localPath,
    npmSpec: `@gabrielvfonseca/${pluginId}`,
  };
}
...
describe("bundled plugin load path repair", () => {
  ...
  it("removes stale bundled paths from old versioned Operator package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "@gabrielvfonseca/operator");
  ...
  it("removes stale legacy bundled paths from old versioned Operator package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "@gabrielvfonseca/operator");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
  ...
  it("ignores bundled plugins that already resolve to source extensions", () => {
    const sourcePath = path.resolve("repo", "@gabrielvfonseca/operator", "extensions", "feishu");
    vi.spyOn(bundledSources, "resolveBundledPluginSources").mockReturnValue(
