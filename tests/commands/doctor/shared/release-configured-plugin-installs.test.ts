...
import { maybeRunConfiguredPluginInstallReleaseStep } from "../../../../src/commands/doctor/shared/release-configured-plugin-installs.js";
...
describe("configured plugin install release step", () => {
  ...
  it("does not stamp config during update-time deferred install repair", async () => {
    mocks.repairMissingPluginInstallsForIds.mockResolvedValue({
      changes: [
        'Skipped package-manager repair for configured plugin "codex" during package update; rerun "operator doctor --fix" after the update completes.',
      ],
      warnings: [],
  ...
  it("includes allow-only official plugin ids in the repair set", async () => {
    mocks.getOfficialExternalPluginCatalogEntry.mockImplementation((pluginId: string) => {
      if (pluginId === "lobster") {
        return { name: "@gabrielvfonseca/lobster" };
      }
      return undefined;
    });
    const result = await collectReleaseConfiguredPluginIdsThroughDoctor({
