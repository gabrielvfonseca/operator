...
import { maybeRepairCodexRoutes } from "../../src/commands/doctor/shared/codex-route-warnings.js";
import { normalizeCompatibilityConfigValues } from "../../src/commands/doctor/shared/legacy-config-core-migrate.js";
import { LEGACY_CONFIG_MIGRATIONS } from "../../src/commands/doctor/shared/legacy-config-migrations.js";
import { collectBlockedLegacyOpenAICodexProviderPlan } from "../../src/commands/doctor/shared/legacy-config-migrations.runtime.models.js";
...
vi.mock("../plugins/setup-registry.js", () => ({
  ...
  runPluginSetupConfigMigrations: ({ config }: { config: OperatorConfig }) => ({
    config,
    changes: [],
  }),
}));
...
vi.mock("./doctor/shared/channel-legacy-config-migrate.js", () => ({
  applyChannelDoctorCompatibilityMigrations: (cfg: OperatorConfig) => ({
    next: cfg,
    changes: [],
  }),
}));
...
vi.mock("../secrets/target-registry.js", () => {
  ...
  return {
    discoverConfigSecretTargets: (cfg: OperatorConfig) => {
      const targets: Array<{
        entry: typeof entry;
        path: string;
        pathSegments: string[];
...
describe("normalizeCompatibilityConfigValues", () => {
  ...
  afterAll(() => {
    if (previousOauthDir === undefined) {
      delete process.env.OPERATOR_OAUTH_DIR;
    } else {
      process.env.OPERATOR_OAUTH_DIR = previousOauthDir;
    }
    fs.rmSync(tempOauthDir, { recursive: true, force: true });
  });
  ...
  it("migrates a 2026.6 wizard-shaped Codex config into gateway-loadable canonical state", () => {
    ...
    const normalized = normalizeCompatibilityConfigValues(migrated as OperatorConfig);
  ...
  it("does not report talk provider normalization for realtime voice aliases", () => {
    ...
    const res = normalizeCompatibilityConfigValues(input as OperatorConfig);
