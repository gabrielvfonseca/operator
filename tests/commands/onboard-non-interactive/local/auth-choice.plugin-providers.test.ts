...
import { applyNonInteractivePluginProviderChoice } from "../../../../src/commands/onboard-non-interactive/local/auth-choice.plugin-providers.js";
...
const ensureCodexRuntimePluginForModelSelection = vi.hoisted(() =>
  vi.fn(
    async ({ cfg }: { cfg: OperatorConfig }): Promise<RuntimePluginInstallResult> => ({
      cfg,
      required: false,
      installed: false,
    }),
...
describe("applyNonInteractivePluginProviderChoice", () => {
  it("loads plugin providers for provider-plugin auth choices", async () => {
    ...
    const result = await applyNonInteractivePluginProviderChoice({
      nextConfig: { agents: { defaults: {} } } as OperatorConfig,
      authChoice: "provider-plugin:vllm:custom",
      opts: {} as never,
      runtime: runtime as never,
      baseConfig: { agents: { defaults: {} } } as OperatorConfig,
      resolveApiKey: vi.fn(),
  ...
  it("installs an official catalog provider before applying a cold auth choice", async () => {
    const runtime = createRuntime();
    const runNonInteractive = vi.fn(async ({ config }: { config: OperatorConfig }) => ({
      ...config,
      agents: {
        defaults: {
