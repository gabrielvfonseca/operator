...
import { readFileTailLines, summarizeLogTail } from "../../../src/commands/status-all/gateway.js";
...
describe("readFileTailLines", () => {
  it("returns complete recent lines from a bounded large-file tail", async () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-status-log-tail-"));
    tempDirs.push(dir);
