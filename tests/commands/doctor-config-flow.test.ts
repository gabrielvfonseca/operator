...
import { writeChannelPairingStateSnapshot } from "../../src/pairing/pairing-store-sqlite.test-helpers.js";
...
} from "../../src/commands/doctor-config-flow.test-utils.js";
  ...
  it("warns when hooks transformsDir points outside the hook transforms root", async () => {
    const doctorWarnings = await collectDoctorWarnings({
      hooks: {
        enabled: true,
        token: "hook-secret",
        transformsDir: "/virtual/.operator/workspace/skills/linear-webhook",
        mappings: [
          {
            match: { path: "linear" },
            action: "agent",
            messageTemplate: "Linear event",
            transform: { module: "./operator-linear-transform.js" },
          },
        ],
      },
    });
    ...
    expect(warning).toContain("/virtual/.operator/workspace/skills/linear-webhook");
    expect(warning).toContain("/virtual/.operator/hooks/transforms");
