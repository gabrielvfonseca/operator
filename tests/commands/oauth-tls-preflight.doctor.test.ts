...
import { noteOpenAIOAuthTlsPrerequisites } from "../../src/plugins/provider-openai-chatgpt-oauth-tls.js";

function buildOpenAICodexOAuthConfig(): OperatorConfig {
  return {
    auth: {
      profiles: {
...
function buildOpenAIOAuthConfig(): OperatorConfig {
  return {
    auth: {
      profiles: {
        "openai:user@example.com": {
