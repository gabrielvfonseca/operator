...
import type { MigrationItem, MigrationPlan } from "../../../src/plugins/types.js";
import { formatMigrationPreview, formatMigrationResult } from "../../../src/commands/migrate/output.js";
...
describe("formatMigrationPreview", () => {
  ...
  it("renders migration warnings with a warning glyph", () => {
    const output = formatMigrationPreview({
      ...plan([skillItem(1)]),
      warnings: [
        "Some Codex plugins could not be migrated. Run `operator migrate codex` after onboarding.",
      ],
    })
      .map(stripAnsi)
      .join("\n");
    ...
    expect(output).toContain(
      "⚠️  Some Codex plugins could not be migrated. Run `operator migrate codex` after onboarding.",
    );
  });
...
describe("formatMigrationResult", () => {
  ...
  it("renders warning-backed next steps with a warning glyph", () => {
    ...
    const output = formatMigrationResult({
      ...
      warnings: [warning],
      nextSteps: [warning, "Run operator doctor after applying the migration."],
    })
      .map(stripAnsi)
      .join("\n");

    expect(output).toContain(`⚠️  ${warning}`);
    expect(output).toContain("• Run operator doctor after applying the migration.");
  });
