...
import { noteInstallPolicyHealth } from "../../src/commands/doctor-install-policy.js";
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-doctor-install-policy-"));
  tempDirs.push(dir);
  return dir;
}
...
function configWithPolicy(scriptPath: string): OperatorConfig {
  return {
    security: {
      installPolicy: {
        enabled: true,
