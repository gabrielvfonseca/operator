...
import type { PluginWebSearchProviderEntry } from "../../src/plugins/types.js";
import type { RuntimeEnv } from "../../src/runtime.js";
import type { WizardPrompter } from "../../src/wizard/prompts.js";
import { listSearchProviderOptions, setupSearch } from "../../src/commands/onboard-search.js";
...
function getWebSearchConfig(config: OperatorConfig | undefined, pluginId: string) {
  return (config as WebSearchConfigRecord | undefined)?.plugins?.entries?.[pluginId]?.config
    ?.webSearch;
}

function ensureWebSearchConfig(config: OperatorConfig, pluginId: string) {
...
function createSearchProviderEntry(id: string): PluginWebSearchProviderEntry {
  ...
  const entry: PluginWebSearchProviderEntry = {
    ...
    applySelectionConfig: (config) => {
      const next: OperatorConfig = { ...config, plugins: { ...config.plugins } };
      const entries = { ...next.plugins?.entries } as NonNullable<
        NonNullable<OperatorConfig["plugins"]>["entries"]
      >;
...
function createPerplexityConfig(apiKey: string, enabled?: boolean): OperatorConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: OperatorConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): OperatorConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: OperatorConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: OperatorConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
