...
import { resetCoreHealthChecksForTest } from "../../src/flows/doctor-core-checks.js";
import { clearHealthChecksForTest, registerHealthCheck } from "../../src/flows/health-check-registry.js";
import { runDoctorLintCli } from "../../src/commands/doctor-lint.js";
...
describe("runDoctorLintCli", () => {
  ...
  it("reports disabled Codex plugin routes through doctor lint", async () => {
    ...
    try {
      ...
      expect(payload.findings[0].fixHint).toContain("operator doctor --fix");
    } finally {
      stdout.mockRestore();
    }
  });
