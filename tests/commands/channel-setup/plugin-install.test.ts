...
const discoverOperatorPlugins = vi.fn((_args?: unknown) => ({ candidates: [], diagnostics: [] }));
vi.mock("../../plugins/discovery.js", () => ({
  discoverOperatorPlugins: (args: unknown) => discoverOperatorPlugins(args),
}));
...
import { loadOperatorPlugins } from "../../../src/plugins/loader.js";
...
import { makePrompter, makeRuntime } from "../../../src/commands/setup/__tests__/test-utils.js";
import {
...
function expectSetupSnapshotDoesNotScopeToPlugin(params: {
  ...
  expect(requireMockCallArg(vi.mocked(loadOperatorPlugins), 0).onlyPluginIds).toStrictEqual([]);
}
...
beforeEach(() => {
  ...
  discoverOperatorPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
...
afterEach(() => {
  clearPluginMetadataLifecycleCaches();
  if (ORIGINAL_OPERATOR_STATE_DIR === undefined) {
    delete process.env.OPERATOR_STATE_DIR;
  } else {
    process.env.OPERATOR_STATE_DIR = ORIGINAL_OPERATOR_STATE_DIR;
  }
});
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const select = vi.fn((async <T extends string>() => "skip" as T) as WizardPrompter["select"]);
  const prompter = makePrompter({ select: select as unknown as WizardPrompter["select"] });
  const cfg: OperatorConfig = { update: { channel } };
...
function expectLoadOperatorPluginFields(expected: Record<string, unknown>, callIndex = 0) {
  expectRecordFields(
    requireMockCallArg(vi.mocked(loadOperatorPlugins), callIndex),
    "loadOperatorPlugins args",
    expected,
  );
}
...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: OperatorConfig = { plugins: { allow: ["bundled-chat"] } };
  ...
  it("installs npm beta on the beta channel without persisting the beta tag", async () => {
    ...
    const cfg: OperatorConfig = { update: { channel: "beta" } };
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
