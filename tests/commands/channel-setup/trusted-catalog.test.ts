...
} from "../../../src/commands/channel-setup/trusted-catalog.js";
  ...
  it("forwards caller env when resolving load paths", () => {
    ...
    getTrustedChannelPluginCatalogEntry("e2e-load-paths", {
      cfg: {
        plugins: {
          load: {
            paths: ["$OPERATOR_HOME/custom-plugin"],
          },
        },
      },
      env: {
        ...process.env,
        OPERATOR_HOME: "/tmp/custom-home",
      },
      workspaceDir: "/tmp/workspace",
    });
    ...
    expect(getChannelPluginCatalogEntry).toHaveBeenCalledWith("e2e-load-paths", {
      ...
      extraPaths: ["$OPERATOR_HOME/custom-plugin"],
      workspaceDir: "/tmp/workspace",
    });
  });
});
