...
import { buildNodeInstallPlan } from "../../src/commands/node-daemon-install-helpers.js";
...
describe("buildNodeInstallPlan", () => {
  it("passes the selected node bin directory into the node service environment", async () => {
    ...
    mocks.buildNodeServiceEnvironment.mockReturnValue({
      OPERATOR_SERVICE_VERSION: "2026.3.22",
    });
  ...
  it("marks node gateway credentials as file-backed service env", async () => {
    ...
    mocks.renderSystemNodeWarning.mockReturnValue(undefined);
    mocks.buildNodeServiceEnvironment.mockReturnValue({
      OPERATOR_GATEWAY_TOKEN: "node-token",
      OPERATOR_GATEWAY_PASSWORD: "node-password",
      OPERATOR_SERVICE_VERSION: "2026.3.22",
    });
    ...
    expect(plan.environment.OPERATOR_GATEWAY_TOKEN).toBe("node-token");
    expect(plan.environment.OPERATOR_GATEWAY_PASSWORD).toBe("node-password");
    expect(plan.environmentValueSources).toEqual({
      OPERATOR_GATEWAY_TOKEN: "file",
      OPERATOR_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
