...
import type { OAuthCredentials } from "../../src/llm/utils/oauth/types.js";
import {
  applyAuthProfileConfig,
  upsertApiKeyProfile,
  writeOAuthCredentials,
} from "../../src/plugins/provider-auth-helpers.js";
import { setTestEnvValue } from "../../src/test-utils/env.js";
import {
  createAuthTestLifecycle,
  readAuthProfilesForAgent,
  setupAuthTestEnv,
} from "../../src/commands/test-wizard-helpers.js";
...
vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.OPERATOR_STATE_DIR ?? "/tmp/operator-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  const upsert = (params: { profileId: string; credential: unknown; agentDir?: string }) => {
    const stateDir = process.env.OPERATOR_STATE_DIR ?? "/tmp/operator-state";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under the default agent dir", async () => {
    const env = await setupAuthTestEnv("operator-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-oauth-sync-"));
    ...
    setTestEnvValue("OPERATOR_AGENT_DIR", kidAgentDir);
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-oauth-nosync-"));
    lifecycle.setStateDir(tempStateDir);
    setTestEnvValue("OPERATOR_STATE_DIR", tempStateDir);
  ...
  it("syncs siblings from explicit agentDir outside OPERATOR_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-oauth-external-"));
    lifecycle.setStateDir(tempStateDir);
    setTestEnvValue("OPERATOR_STATE_DIR", tempStateDir);

    // Create standard-layout agents tree *outside* OPERATOR_STATE_DIR
...
describe("upsertApiKeyProfile secret refs", () => {
  ...
  it("handles plaintext, ref mode, and inline env-ref provider keys", async () => {
    const env = await setupAuthTestEnv("operator-onboard-auth-credentials-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("stores provider-specific env refs and metadata in ref mode", async () => {
    const env = await setupAuthTestEnv("operator-onboard-auth-credentials-provider-ref-");
    lifecycle.setStateDir(env.stateDir);
...
describe("upsertApiKeyProfile", () => {
  const lifecycle = createAuthTestLifecycle(["OPERATOR_STATE_DIR", "OPERATOR_AGENT_DIR"]);
  ...
  it("writes to the default agent dir", async () => {
    const env = await setupAuthTestEnv("operator-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
    const defaultAgentDir = path.join(env.stateDir, "agents", "main", "agent");
