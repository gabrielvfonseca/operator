...
import type { PluginEnableResult } from "../../src/plugins/enable.js";
import { resolveNpmInstallSpecsForUpdateChannel } from "../../src/plugins/install-channel-specs.js";
...
import { ensureOnboardingPluginInstalled } from "../../src/commands/onboarding-plugin-install.js";
import { testing } from "../../src/commands/onboarding-plugin-install.test-support.js";
...
describe("ensureOnboardingPluginInstalled", () => {
    ...
    try {
      await ensureOnboardingPluginInstalled({
        ...
        entry: {
          ...
          label: "QQ Bot",
          install: {
            npmSpec: "@gabrielvfonseca/qqbot@beta",
          },
        },
        prompter: {
      ...
      expect(captured?.options).toEqual([
        { value: "npm", label: "从 npm 下载（@gabrielvfonseca/qqbot@beta）" },
      ...
      await expect(
        ensureOnboardingPluginInstalled({
          ...
          entry: {
            ...
            label: "Demo Provider",
            install: {
              npmSpec: "@gabrielvfonseca/demo-plugin@1.2.3",
            },
          },
          promptInstall: false,
          prompter: {
  ...
  it("uses a guarded npm install override without official-trust flags", async () => {
    process.env.OPERATOR_ALLOW_PLUGIN_INSTALL_OVERRIDES = "1";
    process.env.OPERATOR_PLUGIN_INSTALL_OVERRIDES = JSON.stringify({
      codex: "npm:@gabrielvfonseca/codex@2026.5.8",
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ...
      npmResolution: {
        ...
        version: "2026.5.8",
        resolvedSpec: "@gabrielvfonseca/codex@2026.5.8",
      },
    });
    ...
    expect(npmCall.spec).toBe("@gabrielvfonseca/codex@2026.5.8");
    ...
    const result = await ensureOnboardingPluginInstalled({
      ...
      entry: {
        ...
        install: {
          ...
          npmSpec: "@gabrielvfonseca/demo-plugin@2026.5.2",
  ...
  it("passes npm specs and optional expected integrity to npm installs with progress", async () => {
    ...
    const npmResolution = {
      ...
      version: "1.2.3",
      resolvedSpec: "@wecom/wecom-operator-plugin@1.2.3",
    ...
    expect(npmCall.spec).toBe("@wecom/wecom-operator-plugin@1.2.3");
    ...
    expect(recordUpdate.spec).toBe("@wecom/wecom-operator-plugin@1.2.3");
    expect(recordUpdate.installPath).toBe("/tmp/demo-plugin");
    ...
    expect(installed?.spec).toBe("@wecom/wecom-operator-plugin@1.2.3");
    expect(clearLoadInstalledPluginIndexInstallRecordsCache).toHaveBeenCalledOnce();
    expect(clearPluginMetadataLifecycleCaches).toHaveBeenCalledOnce();
    expect(invalidatePluginRuntimeDiscoveryAfterConfigMutation).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("installs trusted official plugins at the exact extended-stable core version", async () => {
    installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        ...
        version: VERSION,
        resolvedSpec: `@gabrielvfonseca/discord@${VERSION}`,
      },
    });
    ...
    expect(npmCall.spec).toBe(`@gabrielvfonseca/discord@${VERSION}`);
    const [, recordUpdate] = readFirstMockCall(recordPluginInstall, "recordPluginInstall") as [
      OperatorConfig,
      PluginInstallRecord,
    ];
    expect(recordUpdate.spec).toBe("@gabrielvfonseca/discord");
  ...
  it("preserves default intent for trusted official stable installs", async () => {
    installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        name: "@gabrielvfonseca/discord",
        version: "2026.7.21",
        resolvedSpec: "@gabrielvfonseca/discord@2026.7.21",
      },
    });
  ...
  it("logs npm install warnings once while shortening the progress label", async () => {
    ...
    const result = await ensureOnboardingPluginInstalled({
      ...
      entry: {
        ...
        label: "Codex",
        install: {
          npmSpec: "@gabrielvfonseca/codex@beta",
        },
      },
      prompter: {
        select: vi.fn(async () => "npm"),
  ...
  it("defaults dual-source remote installs to npm unless ClawHub is explicit", async () => {
    ...
    expect(captured?.options).toEqual([
      ...
      { value: "npm", label: "Download from npm (@gabrielvfonseca/demo-plugin@2026.5.2)" },
      { value: "skip", label: "Skip for now" },
    ]);
    expect(captured?.initialValue).toBe("npm");
    expect(installPluginFromClawHub).not.toHaveBeenCalled();
  ...
  it("falls back from ClawHub to npm when the ClawHub artifact is unavailable", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValueOnce({
      ...
      npmResolution: {
        ...
        version: "2026.5.2",
        resolvedSpec: "@gabrielvfonseca/demo-plugin@2026.5.2",
    ...
    expect(npmCall.spec).toBe("@gabrielvfonseca/demo-plugin@2026.5.2");
    expect(npmCall.expectedPluginId).toBe("demo-plugin");
    expect(result.installed).toBe(true);
  });
  ...
  it("does not offer local installs when the workspace only has a spoofed .git marker", async () => {
    await withTempDir({ prefix: "operator-onboarding-install-spoofed-git-" }, async (temp) => {
  ...
  it("allows local installs for real gitdir checkouts and sanitizes prompt text", async () => {
    await withTempDir({ prefix: "operator-onboarding-install-gitdir-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(pluginDir, { recursive: true });
  ...
  it("does not add local plugin paths when enablement is blocked by policy", async () => {
    await withTempDir({ prefix: "operator-onboarding-install-blocked-enable-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(pluginDir, { recursive: true });
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
  ...
  it("allows local installs for linked git worktrees", async () => {
    await withTempDir({ prefix: "operator-onboarding-install-worktree-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
  ...
  it("records local install source metadata when a local path is selected", async () => {
    await withTempDir({ prefix: "operator-onboarding-install-local-record-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
      await fs.mkdir(pluginDir, { recursive: true });
  ...
  it("hides the npm download option for bundled plugins so the menu matches non-npm channels", async () => {
    await withTempDir({ prefix: "operator-onboarding-install-bundled-prompt-" }, async (temp) => {
      ...
      const prompt = requireCapturedPrompt(captured);
      // "Download from npm (@gabrielvfonseca/tlon)" must NOT appear: the bundled
  ...
  it("enables bundled plugins without adding their bundled directory as a local install", async () => {
    await withTempDir({ prefix: "operator-onboarding-install-bundled-record-" }, async (temp) => {
  ...
  it("records local install source metadata when npm install falls back to local", async () => {
    await withTempDir(
      { prefix: "operator-onboarding-install-npm-fallback-record-" },
      async (temp) => {
        const workspaceDir = path.join(temp, "workspace");
        const pluginDir = path.join(workspaceDir, "plugins", "demo");
        await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
  ...
  it("records absolute local catalog paths as workspace-relative source metadata", async () => {
    await withTempDir({ prefix: "operator-onboarding-install-portable-record-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
      await fs.mkdir(pluginDir, { recursive: true });
  ...
  it("keeps local installs available when cwd is a git repo but workspaceDir is not", async () => {
    await withTempDir({ prefix: "operator-onboarding-install-cwd-git-" }, async (temp) => {
  ...
  it("rejects local install paths outside the trusted workspace roots", async () => {
    await withTempDir({ prefix: "operator-onboarding-install-outside-root-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
  ...
  it("rejects local install paths when relative resolution looks cross-drive", async () => {
    await withTempDir({ prefix: "operator-onboarding-install-cross-drive-" }, async (temp) => {
      const workspaceDir = path.join(temp, "workspace");
      const pluginDir = path.join(workspaceDir, "plugins", "demo");
      await fs.mkdir(path.join(workspaceDir, ".git"), { recursive: true });
      await fs.mkdir(pluginDir, { recursive: true });
