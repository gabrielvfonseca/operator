...
import { setupSkills } from "../../src/commands/onboard-skills.js";
import { testing } from "../../src/commands/onboard-skills.test-support.js";
...
describe("setupSkills", () => {
  ...
  it("hides brew-only installs in Linux containers when brew is missing", async () => {
    await withPlatform("linux", async () => {
      ...
      await setupSkills({} as OperatorConfig, "/tmp/ws", runtime, prompter);
  ...
  it("uses the requested node manager for node-backed auto installs", async () => {
    ...
    const next = await setupSkills({} as OperatorConfig, "/tmp/ws", runtime, prompter, {
      nodeManager: "pnpm",
    });
