...
describe("detectLinuxVolatileStateDir", () => {
  const TMPFS_MOUNT_INFO = [
    ...
    "35 22 0:35 / /home/user/.operator rw - tmpfs tmpfs rw,size=1048576k",
  ].join("\n");
  const RAMFS_MOUNT_INFO = [
    "22 1 0:21 / / rw,relatime - ext4 /dev/sda1 rw",
    "35 22 0:35 / /home/user/.operator rw - ramfs ramfs rw",
  ].join("\n");
  ...
  it.each([
    ...
    const result = detectLinuxVolatileStateDir("/home/user/.operator", {
      platform: "linux",
      mountInfo,
    ...
    expect(result).toMatchObject({
      path: "/home/user/.operator",
      mountPoint: "/home/user/.operator",
      fsType,
    });
  });
  ...
  it("uses the most specific matching mount", () => {
    const mountInfo = [
      ...
      "35 30 0:35 / /home/user/.operator rw - tmpfs tmpfs rw",
    ].join("\n");

    expect(
      detectLinuxVolatileStateDir("/home/user/.operator", {
        platform: "linux",
        mountInfo,
        resolveRealPath: (targetPath) => targetPath,
      }),
  ...
  it("detects a missing state directory through an existing symlink", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "operator-doctor-volatile-"));
    try {
      ...
      const result = detectLinuxVolatileStateDir(
        path.join(stateLink, "@gabrielvfonseca/operator"),
        {
          platform: "linux",
          mountInfo: [
            "22 1 0:21 / / rw,relatime - ext4 /dev/sda1 rw",
      ...
      expect(result).toMatchObject({
        path: path.join(resolvedVolatileMount, "@gabrielvfonseca/operator"),
...
describe("formatLinuxVolatileStateDirWarning", () => {
  it("covers all SQLite state and sidecar files under the volatile state directory", () => {
    const warning = formatLinuxVolatileStateDirWarning("~/.operator", {
      path: "/home/user/.operator",
      mountPoint: "/home/user/.operator",
      fsType: "tmpfs",
    });
