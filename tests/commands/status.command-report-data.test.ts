...
import { buildStatusCommandReportData } from "../../src/commands/status.command-report-data.ts";
import { createStatusCommandReportDataParams } from "../../src/commands/status.test-support.ts";
...
describe("buildStatusCommandReportData", () => {
  it("builds report inputs from shared status surfaces", async () => {
    ...
    expect(result.taskMaintenanceHint).toBe(
      "Task maintenance: cmd:operator tasks maintenance --apply",
    );
    ...
    expect(result.footerLines.at(-1)).toBe("  Need to test channels? cmd:operator status --deep");
  });
  ...
  it("shows skipped audit text when fast status omits the security audit", async () => {
    ...
    expect(result.securityAuditLines).toEqual([
      "muted(Skipped in fast status. Full report: cmd:operator security audit)",
      "muted(Deep probe: cmd:operator status --deep)",
    ]);
  });
