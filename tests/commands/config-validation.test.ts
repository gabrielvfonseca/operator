...
import type { PluginCompatibilityNotice } from "../../src/plugins/status.js";
import { createCompatibilityNotice } from "../../src/plugins/status.test-fixtures.js";
import { requireValidConfigSnapshot } from "../../src/commands/config-validation.js";
...
describe("requireValidConfigSnapshot", () => {
  ...
  it("replaces doctor fix advice for plugin packaging compiled-output failures", async () => {
    ...
    expect(runtime.error).not.toHaveBeenCalledWith("Fix: operator doctor --fix");
    expect(runtime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it("keeps doctor fix advice for normal invalid config failures", async () => {
    ...
    expect(config).toBeNull();
    expect(runtime.error).toHaveBeenCalledWith("Fix: operator doctor --fix");
    expect(runtime.exit).toHaveBeenCalledWith(1);
  });
});
