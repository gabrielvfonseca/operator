...
} from "../../src/plugin-state/plugin-state-store.test-helpers.js";
import {
  readPersistedInstalledPluginIndex,
  writePersistedInstalledPluginIndex,
} from "../../src/plugins/installed-plugin-index-store.js";
import type { InstalledPluginInstallRecordInfo } from "../../src/plugins/installed-plugin-index.js";
import {
...
} from "../../src/commands/doctor-state-migrations.js";
...
async function runTelegramAllowFromMigration(params: { root: string; cfg: OperatorConfig }) {
  const oauthDir = ensureCredentialsDir(params.root);
  writeLegacyTelegramAllowFromStore(oauthDir);
  const env = { OPERATOR_STATE_DIR: params.root } as NodeJS.ProcessEnv;
...
async function detectAndRunMigrations(params: {
  ...
  const detected = await detectLegacyStateMigrations({
    cfg: params.cfg,
    env: { OPERATOR_STATE_DIR: params.root } as NodeJS.ProcessEnv,
  });
...
describe("doctor legacy state migrations", () => {
  ...
  it("does not repair newer shared state schemas", async () => {
    ...
    seededDb.exec(`PRAGMA user_version = ${OPERATOR_STATE_SCHEMA_VERSION + 1};`);
    seededDb.close();
    ...
    expect(result.warnings[0]).toContain(
      `uses newer schema version ${OPERATOR_STATE_SCHEMA_VERSION + 1}`,
    );
  ...
  it("imports debug proxy capture storage from shipped environment overrides", async () => {
    ...
    const env = {
      ...
      OPERATOR_DEBUG_PROXY_DB_PATH: sourcePath,
      OPERATOR_DEBUG_PROXY_BLOB_DIR: blobDir,
  ...
  it("ignores a legacy debug proxy override that points at shared state", async () => {
    ...
    const detected = await detectLegacyStateMigrations({
      ...
      env: {
        OPERATOR_STATE_DIR: root,
        OPERATOR_DEBUG_PROXY_DB_PATH: sharedStatePath,
      } as NodeJS.ProcessEnv,
    });
  ...
  it("auto-migrates the plugin-state sidecar when custom agent dirs skip session migration", async () => {
    ...
    const result = await autoMigrateLegacyState({
      ...
      env: {
        OPERATOR_STATE_DIR: root,
        OPERATOR_AGENT_DIR: path.join(root, "custom-agent"),
      } as NodeJS.ProcessEnv,
      log: { info: vi.fn(), warn: vi.fn() },
    });
  ...
  it("normalizes obsolete task delivery status before archiving the legacy sidecar", async () => {
    ...
    const shared = openOperatorStateDatabase({
      env: { OPERATOR_STATE_DIR: root } as NodeJS.ProcessEnv,
    });
    expect(
      shared.db
  ...
  it("honors session.mainKey when seeding the direct-chat bucket", async () => {
    const root = await makeTempRoot();
    const cfg: OperatorConfig = { session: { mainKey: "work" } };
    writeLegacySessionsFixture({
      root,
      sessions: {
        "+1555": { sessionId: "a", updatedAt: 10 },
