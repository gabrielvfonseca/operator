...
import type { DaemonStatus } from "../../src/cli/daemon-cli/status.gather.js";
import { ensureGatewayReadyForOperation } from "../../src/commands/gateway-readiness.js";
...
describe("ensureGatewayReadyForOperation", () => {
  ...
  it("prints diagnosis and skips recovery when an interactive user declines", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      "Gateway is not installed. Install and start it now so Operator can open the dashboard?",
      true,
    );
  ...
  it("installs a missing service and waits for the gateway before returning ready", async () => {
    ...
    const running = createStatus({
      service: {
        ...
        loadedText: "enabled",
        notLoadedText: "disabled",
        command: { programArguments: ["@gabrielvfonseca/operator", "gateway", "run"] },
