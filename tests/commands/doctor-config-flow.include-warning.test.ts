...
import { noteIncludeConfinementWarning } from "../../src/commands/doctor-config-analysis.js";
...
describe("doctor include warning", () => {
  it("surfaces include confinement hint for escaped include paths", () => {
    noteIncludeConfinementWarning({
      path: "/tmp/operator-config/operator.json",
      issues: [
        {
    ...
    expect(noteSpy).toHaveBeenCalledWith(
      [
        "- $include paths must stay under: /tmp/operator-config",
