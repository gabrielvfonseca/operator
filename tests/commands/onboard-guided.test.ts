...
import type { CallGatewayCliOptions } from "../../src/gateway/call.js";
import { createSuiteLogPathTracker } from "../../src/logging/log-test-helpers.js";
...
import { runGuidedOnboarding, type GuidedOnboardingDeps } from "../../src/commands/onboard-guided.js";
import { runRemoteGatewayInferenceOnboarding } from "../../src/commands/onboard-remote-gateway.js";
...
describe("runGuidedOnboarding", () => {
  ...
  it("shows copyable repair commands without opening AI when config is invalid", async () => {
    ...
    expect(prompter.outro).toHaveBeenCalledWith(expect.stringContaining("operator doctor --fix"));
    expect(prompter.outro).toHaveBeenCalledWith(
      expect.stringContaining("operator config validate"),
    );
    expect(runtime.exit).toHaveBeenCalledWith(1);
    expect(deps.runSystemAgentChat).not.toHaveBeenCalled();
    expect(deps.detect).not.toHaveBeenCalled();
