...
import { maybeRepairCanonicalApiKeyFieldAlias } from "../../src/commands/doctor-auth-flat-profiles.js";
...
async function makeTestState(): Promise<OperatorTestState> {
  const state = await createOperatorTestState({
    layout: "state-only",
    prefix: "operator-doctor-canonical-api-key-",
    env: {
...
describe("maybeRepairCanonicalApiKeyFieldAlias", () => {
  ...
  it("repairs auth profiles from OPERATOR_AGENT_DIR", async () => {
    const state = await makeTestState();
