...
import type { SpawnProcessAdapter } from "../../../src/process/supervisor/types.js";
...
describe("process supervisor", () => {
  ...
  it("bounds retained output on UTF-16 boundaries while streaming full chunks", async () => {
    ...
    const stdoutMarker = `[operator: captured stdout truncated to last ${maxCapturedOutputChars} chars]\n`;
    const stderrMarker = `[operator: captured stderr truncated to last ${maxCapturedOutputChars} chars]\n`;
