...
} from "../../../../src/process/supervisor/adapters/test-support.js";
...
describe("createChildAdapter", () => {
  const originalServiceMarker = process.env.OPERATOR_SERVICE_MARKER;
  ...
  afterAll(() => {
    if (originalServiceMarker === undefined) {
      ...
      process.env.OPERATOR_SERVICE_MARKER = originalServiceMarker;
    }
  });
  ...
  it("uses process-tree kill for default SIGKILL", async () => {
    ...
    const expectedDetached = process.platform !== "win32" && !process.env.OPERATOR_SERVICE_MARKER;
  ...
  it("passes detached:false in service-managed mode where useDetached is false from the start (#71662)", async () => {
    process.env.OPERATOR_SERVICE_MARKER = "1";
    try {
