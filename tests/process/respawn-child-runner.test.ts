...
import { runRespawnChildWithSignalBridge } from "../../src/process/respawn-child-runner.js";
...
describe("runRespawnChildWithSignalBridge", () => {
  ...
  it("spawns POSIX respawn children detached for process-group cleanup", () => {
    ...
    expect(spawnChild).toHaveBeenCalledWith("/usr/bin/node", ["/repo/operator/dist/entry.js"], {
      stdio: "inherit",
      env: { OPERATOR_NODE_OPTIONS_READY: "1" },
  ...
  it("keeps terminal stdio respawn children attached", () => {
    ...
    runRespawnChildWithSignalBridge({
      command: "/usr/bin/node",
      args: ["/repo/operator/dist/entry.js", "configure"],
      env: {},
      detachForProcessTree: true,
    ...
    expect(spawnChild).toHaveBeenCalledWith(
      "/usr/bin/node",
      ["/repo/operator/dist/entry.js", "configure"],
      {
        stdio: "inherit",
        env: {},
        detached: false,
