...
import { setVerbose } from "../../src/global-state.js";
import { attachChildProcessBridge } from "../../src/process/child-process-bridge.js";
import {
  resolveCommandEnv,
...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        OPERATOR_BASE_ENV: "base",
        OPERATOR_CHILD_ENV_REMOVE: "base",
        OPERATOR_TO_REMOVE: undefined,
      },
      env: {
        OPERATOR_CHILD_ENV_REMOVE: undefined,
        OPERATOR_TEST_ENV: "ok",
      },
    });

    expect(resolved.OPERATOR_BASE_ENV).toBe("base");
    expect(resolved.OPERATOR_CHILD_ENV_REMOVE).toBeUndefined();
    expect(resolved.OPERATOR_TEST_ENV).toBe("ok");
    expect(resolved.OPERATOR_TO_REMOVE).toBeUndefined();
    expect(resolved.OPERATOR_CLI).toBe(OPERATOR_CLI_ENV_VALUE);
  });
  ...
  it("does not restore parent variables excluded from the child environment", async () => {
    const key = "OPERATOR_EXECA_PARENT_ONLY_TEST";
...
describe("runExec", () => {
  ...
  it("supports stdin and an explicit base environment", async () => {
    const { stdout, stderr } = await runExec(
      ...
      [
        "-e",
        "process.stdin.pipe(process.stdout); process.stderr.write(process.env.OPERATOR_RUN_EXEC_TEST ?? 'missing')",
      ],
      {
        baseEnv: { OPERATOR_RUN_EXEC_TEST: "base" },
