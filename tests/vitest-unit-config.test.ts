...
const patternFiles = createPatternFileHelper("operator-vitest-unit-config-");
...
describe("loadExtraExcludePatternsFromEnv", () => {
  ...
  it("loads extra exclude patterns from a JSON file", () => {
    const filePath = patternFiles.writePatternFile("extra-exclude.json", [
      ...
      42,
      "",
      "ui/tests/ui/views/chat.test.ts",
    ]);

    expect(
      loadExtraExcludePatternsFromEnv({
        OPERATOR_VITEST_EXTRA_EXCLUDE_FILE: filePath,
      }),
    ).toEqual(["src/infra/update-runner.test.ts", "ui/tests/ui/views/chat.test.ts"]);
  });
...
describe("unit vitest config", () => {
  ...
  it("keeps acp and ui tests out of the generic unit lane", () => {
    ...
    for (const pattern of [
      "ui/tests/ui/app-chat.test.ts",
      "ui/tests/ui/chat/**/*.test.ts",
      "ui/tests/ui/views/chat.test.ts",
    ]) {
  ...
  it("adds the Operator runtime setup hooks on top of the base setup", () => {
    const unitConfig = createUnitVitestConfig({});
    const testConfig = requireTestConfig(unitConfig);
