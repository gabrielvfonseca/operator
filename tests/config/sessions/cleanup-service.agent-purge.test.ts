...
import { purgeAgentSessionStoreEntries } from "../../../src/config/sessions/cleanup-service.js";
...
describe("purgeAgentSessionStoreEntries", () => {
  ...
  it("purges deleted-agent entries through the storage boundary", async () => {
    const cfg = {
      session: { store: "/tmp/operator-agent-purge-sessions.json" },
      agents: {
        list: [
    ...
    expect(sessionAccessorMocks.purgeDeletedAgentSessionEntries).toHaveBeenCalledWith({
      ...
      agentId: "ops",
      storeAgentId: "main",
      storePath: "/tmp/operator-agent-purge-sessions.json",
    });
