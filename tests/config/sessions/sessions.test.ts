...
import { upsertAcpSessionMeta } from "../../../src/acp/runtime/session-meta.js";
import * as jsonFiles from "../../../src/infra/json-files.js";
import { createSuiteTempRootTracker, withTempDirSync } from "../../../src/test-helpers/temp-dir.js";
import type { OperatorConfig } from "../../../src/config/config.js";
import type { SessionConfig } from "../../../src/config/types.base.js";
import { resolveSessionLifecycleTimestamps, resolveSessionWorkStartError } from "../../../src/config/sessions/lifecycle.js";
import {
...
import { evaluateSessionFreshness, resolveSessionResetPolicy } from "../../../src/config/sessions/reset.js";
import { mergeRestartRecoveryTerminalRunIds } from "../../../src/config/sessions/restart-recovery-state.js";
...
import { readSessionStoreCache, writeSessionStoreCache } from "../../../src/config/sessions/store-cache.js";
import {
...
import { mergeSessionEntry, type SessionEntry } from "../../../src/config/sessions/types.js";
...
describe("session path safety", () => {
  ...
  it("accepts symlink-alias session paths that resolve under the sessions dir", () => {
    ...
    withTempDirSync({ prefix: "operator-symlink-session-" }, (tmpDir) => {
  ...
  it("falls back when sessionFile is a symlink that escapes sessions dir", () => {
    ...
    withTempDirSync({ prefix: "operator-symlink-escape-" }, (tmpDir) => {
...
describe("session lifecycle timestamps", () => {
  it("falls back to the JSONL session header for legacy session start time", async () => {
    const dir = await fsPromises.mkdtemp("/tmp/operator-lifecycle-test-");
    try {
...
describe("session store writer queue", () => {
  const writerFixtureRootTracker = createSuiteTempRootTracker({ prefix: "operator-writer-test-" });
  ...
  it("clones session store cache hits from cached serialized JSON", () => {
    ...
    const storePath = "/tmp/operator-serialized-cache-test.json";
    const store = {
      [key]: {
