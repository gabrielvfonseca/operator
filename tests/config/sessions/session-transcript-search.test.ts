...
import { listSessionsNeedingTranscriptIndexReconcile } from "../../../src/config/sessions/session-transcript-index.js";
import { searchSessionTranscripts } from "../../../src/config/sessions/session-transcript-search.js";
...
function env(): NodeJS.ProcessEnv {
  return { ...process.env, OPERATOR_STATE_DIR: paths.stateDir };
}
...
function agentKysely() {
  const database = openOperatorAgentDatabase({ agentId: "main", env: env() });
  return {
    db: database.db,
