...
} from "../../src/agents/sandbox/config.js";
...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("operator-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "operator-sandbox-browser-global" },
      agentBrowser: { network: "operator-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("operator-sandbox-browser-agent");
  });
