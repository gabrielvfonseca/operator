...
} from "../../src/config/io.write-prepare.js";
...
describe("config io write prepare", () => {
        ...
        sourceConfig: {
          plugins: {
            ...
            installs: {
              "operator-web-search": {
                source: "npm",
                spec: "@ollama/operator-web-search",
                installPath: "/tmp/operator-web-search",
                resolvedName: "@ollama/operator-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "operator-web-search": {
                source: "npm",
                spec: "@ollama/operator-web-search@0.2.2",
                installPath: "/tmp/operator-web-search",
                resolvedName: "@ollama/operator-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("operator config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('operator config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/operator.json",
  ...
  it("keeps runtime-only channel defaults out of the persisted candidate", () => {
    ...
    const nextConfig: OperatorConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://operator.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
