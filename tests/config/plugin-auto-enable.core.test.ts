...
import type { PluginCandidate, PluginDiscoveryResult } from "../../src/plugins/discovery.js";
import {
  ...
  it("does not load plugin manifests for disabled plugin entries under a restrictive allowlist", () => {
    ...
    expect(
      readFileSync.mock.calls.some(
        ([filePath]) => typeof filePath === "string" && filePath.endsWith("operator.plugin.json"),
      ),
    ).toBe(false);
  });
