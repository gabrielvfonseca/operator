...
import { findLegacyConfigIssues } from "../../src/config/legacy.js";
import { buildWebSearchProviderConfig, withTempHome, writeOperatorConfig } from "../../src/config/test-helpers.js";
import { validateConfigObject, validateConfigObjectRaw } from "../../src/config/validation.js";
import { OperatorSchema } from "../../src/config/zod-schema.js";
...
describe("boolean config validation", () => {
  it.each(nonBooleanConfigCases)("rejects non-boolean values for $name", ({ config }) => {
    const result = OperatorSchema.safeParse(config);
...
describe("$schema key in config (#14998)", () => {
  ...
  it("rejects non-string $schema", () => {
    const result = OperatorSchema.safeParse({ $schema: 123 });
    expect(result.success).toBe(false);
  });
...
describe("mcp.apps.enabled", () => {
  it.each([true, false])("accepts %s", (enabled) => {
    expect(OperatorSchema.safeParse({ mcp: { apps: { enabled } } }).success).toBe(true);
  });

  it("rejects non-boolean values", () => {
    expect(OperatorSchema.safeParse({ mcp: { apps: { enabled: "yes" } } }).success).toBe(false);
  });
  ...
  it("accepts only a bare HTTP(S) sandbox origin", () => {
    ...
    expect(OperatorSchema.safeParse({ mcp: { apps: { sandboxPort: 65536 } } }).success).toBe(false);
    for (const sandboxOrigin of [
      ...
      expect(OperatorSchema.safeParse({ mcp: { apps: { sandboxOrigin } } }).success).toBe(false);
    }
  });
});
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = OperatorSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("rejects resolved-only gateway.bind aliases as invalid schema values, not legacy", async () => {
    await withTempHome(async (home) => {
      ...
      const prev = process.env.OPERATOR_BIND;
      process.env.OPERATOR_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.OPERATOR_BIND;
        } else {
          process.env.OPERATOR_BIND = prev;
        }
      }
    });
  });
