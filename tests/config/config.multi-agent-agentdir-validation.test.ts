...
import { withTempHomeConfig } from "../../src/config/test-helpers.js";
...
describe("multi-agent agentDir validation", () => {
  it("rejects shared agents.list agentDir", () => {
    const shared = path.join(tmpdir(), "operator-shared-agentdir");
    ...
    await withTempHomeConfig(
      {
        agents: {
          list: [
            { id: "a", agentDir: "~/.operator/agents/shared/agent" },
            { id: "b", agentDir: "~/.operator/agents/shared/agent" },
          ],
        },
