...
import { normalizeExecSafeBinProfilesInConfig } from "../../src/config/normalize-exec-safe-bin.js";
import { withTempHome } from "../../src/config/test-helpers.js";
...
describe("config io paths", () => {
  ...
  it("uses OPERATOR_HOME for default config path", async () => {
    await withTempHome(async (home) => {
  ...
  it("does not warn about newer config during internal update handoff reads", async () => {
    await withTempHome(async (home) => {
      ...
      const io = createConfigIO({
        configPath,
        env: { HOME: home, OPERATOR_UPDATE_POST_CORE: "1" } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger,
      });
      io.loadConfig();
