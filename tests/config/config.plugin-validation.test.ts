...
import { clearLoadInstalledPluginIndexInstallRecordsCache } from "../../src/plugins/installed-plugin-index-records.js";
import { writePersistedInstalledPluginIndex } from "../../src/plugins/installed-plugin-index-store.js";
...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "operator.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      OPERATOR_STATE_DIR: path.join(suiteHome, ".operator"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "operator-config-plugin-validation-"));
  ...
  describe("missing Codex plugin diagnostics", () => {
    ...
    it.each([
      ...
      },
      {
        ...
        config: {
          agents: {
            list: [{ id: "@gabrielvfonseca/operator" }],
            defaults: {
              models: {
            ...
            diagnostics: [
              {
                ...
                source: path.join(suiteHome, "extensions", "broken-local", "operator.plugin.json"),
  ...
  it("uses persisted installed-plugin records as stale channel evidence", async () => {
    const stateDir = path.join(suiteHome, ".operator");
  ...
  it("ignores standalone helper scripts in auto-discovered global extensions", async () => {
    const helperPath = path.join(suiteHome, ".operator", "extensions", "my-helper.mjs");
  ...
  it("accepts known plugin ids and valid channel/heartbeat enums", () => {
    const res = validateInSuite({
      agents: {
        ...
        list: [{ id: "@gabrielvfonseca/operator", heartbeat: { directPolicy: "allow" } }],
      },
      channels: {
        modelByChannel: {
          openai: {
  ...
  it("accepts plugin heartbeat targets", () => {
    const res = validateInSuite({
      agents: {
        defaults: { heartbeat: { target: "chat" } },
        list: [{ id: "@gabrielvfonseca/operator" }],
      },
