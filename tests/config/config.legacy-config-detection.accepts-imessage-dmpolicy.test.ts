...
import {
  ...
  expectSchemaValid,
} from "../../src/config/legacy-config-detection.test-support.js";
import { AudioSchema, BindingsSchema } from "../../src/config/zod-schema.agents.js";
import { OperatorSchema } from "../../src/config/zod-schema.js";

function expectOperatorSchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = OperatorSchema.safeParse(params.config);
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectOperatorSchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = OperatorSchema.safeParse(config);
