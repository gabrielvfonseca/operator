...
import { createConfigIO, resetConfigRuntimeState, writeConfigFile } from "../../src/config/io.js";
...
describe("config io EACCES handling", () => {
  it("returns a helpful error message when config file is not readable (EACCES)", async () => {
    const configPath = "/data/.operator/operator.json";
    const errors: string[] = [];
  ...
  it("includes configPath in the chown hint for the correct remediation command", async () => {
    const configPath = "/home/myuser/.operator/operator.json";
    const io = createConfigIO({
      configPath,
      fs: makeEaccesFs(configPath),
      logger: { error: () => {}, warn: () => {} },
...
describe("config write guard after unreadable config", () => {
  ...
  it.each([
    ...
    async ({ writeOptions }) => {
      const home = fsNode.mkdtempSync(path.join(os.tmpdir(), "operator-unreadable-"));
      tempRoots.push(home);
      ...
      const rejectedArtifacts = fsNode
        ...
        .filter((name) => name.startsWith("operator.json.rejected."));
