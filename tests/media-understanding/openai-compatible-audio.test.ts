...
import { transcribeOpenAiCompatibleAudio } from "../../src/media-understanding/openai-compatible-audio.js";
...
describe("transcribeOpenAiCompatibleAudio", () => {
  it("adds hidden attribution headers on the native OpenAI audio host", async () => {
    ...
    expect(headers.get("originator")).toBe("@gabrielvfonseca/operator");
    expect(headers.get("version")).toBe(VERSION);
    expect(headers.get("user-agent")).toBe(`operator/${VERSION}`);
  });
