...
import { buildModelDecision, formatDecisionSummary, runProviderEntry } from "../../src/media-understanding/runner.entries.js";
import type { MediaUnderstandingDecision } from "../../src/media-understanding/types.js";
...
describe("media-understanding missing provider errors", () => {
  it("includes the catalog repair hint for a media provider contract", async () => {
    ...
    expect(message).toMatch(/^Media provider not available: groq .*operator plugins install/);
    expect(message).toContain("@gabrielvfonseca/groq-provider");
    expect(message).toContain("operator plugins registry --refresh");
    expect(message).toContain("stop and start the gateway service");
    expect(message).toContain("operator doctor --fix");
  });
