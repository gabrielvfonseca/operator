...
import { resolveFileExtractionLimits } from "../../src/media-understanding/file-extraction-limits.js";
...
describe("resolveFileExtractionLimits", () => {
  it("sizes inbound extraction to managed-media defaults, not the 5MB/4-page input_file defaults", () => {
    ...
    const limits = resolveFileExtractionLimits({} as OperatorConfig);
  ...
  it("derives the byte cap from agents.defaults.mediaMaxMb", () => {
    const cfg = { agents: { defaults: { mediaMaxMb: 12 } } } as OperatorConfig;
