...
const targetableIncludes = [
  ...
  "tests/tui/tui-pty-local.e2e.test.ts",
...
function createTuiPtyVitestConfig(env?: Record<string, string | undefined>) {
  ...
  const includeLocal = configEnv.OPERATOR_TUI_PTY_INCLUDE_LOCAL === "1";
  const include = [
    "tui/tui-pty-harness.e2e.test.ts",
    ...(includeLocal ? ["tui/tui-pty-local.e2e.test.ts"] : []),
  ];
  const includeFromEnv = toTuiPtyIncludePatterns(
    loadPatternListFromEnv("OPERATOR_VITEST_INCLUDE_FILE", configEnv),
  );
  ...
  return defineConfig({
    ...
    test: {
      ...
      setupFiles: [
        ...new Set(
          [...(baseTest.setupFiles ?? []), "test/setup-operator-runtime.ts"].map(
            resolveRepoRootPath,
          ),
        ),
      ],
