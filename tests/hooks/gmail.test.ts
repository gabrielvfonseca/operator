...
import { type OperatorConfig, DEFAULT_GATEWAY_PORT } from "../../src/config/config.js";
import {
  buildDefaultHookUrl,
...
describe("gmail hook config", () => {
  function resolveWithGmailOverrides(
    overrides: Partial<NonNullable<OperatorConfig["hooks"]>["gmail"]>,
  ) {
  ...
  it("resolves runtime config with defaults", () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.account).toBe("operator@gmail.com");
