...
import { MANIFEST_KEY } from "../../src/compat/legacy-names.js";
import { loadWorkspaceHookEntries } from "../../src/hooks/workspace.js";
...
function setupHardlinkHookWorkspace(hookName: string): {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "operator-hooks-workspace-hardlink-"));
...
describe("hooks workspace", () => {
  it("ignores package.json hook paths that traverse outside package directory", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "operator-hooks-workspace-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
  ...
  it("accepts package.json hook paths within package directory", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "operator-hooks-workspace-ok-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
  ...
  it("ignores package.json hook paths that escape via symlink", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "operator-hooks-workspace-link-"));
    const hooksRoot = path.join(root, "hooks");
    fs.mkdirSync(hooksRoot, { recursive: true });
  ...
  it("does not let workspace hooks override managed hooks with the same name", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "operator-hooks-collision-"));
    ...
    expect(entries[0]?.hook.source).toBe("operator-managed");
  });

  it("treats configured extraDirs as managed hook sources", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "operator-hooks-extra-"));
    const workspaceDir = path.join(root, "workspace");
