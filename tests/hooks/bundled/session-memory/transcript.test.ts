...
import { getRecentSessionContentWithResetFallback } from "../../../../src/hooks/bundled/session-memory/transcript.js";
...
async function writeTranscript(content: string): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "operator-session-memory-transcript-"));
  tempRoots.push(root);
