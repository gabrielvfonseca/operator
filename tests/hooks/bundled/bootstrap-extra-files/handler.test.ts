...
import handler from "../../../../src/hooks/bundled/bootstrap-extra-files/handler.js";
...
describe("bootstrap-extra-files hook", () => {
  it("appends extra bootstrap files from configured patterns", async () => {
    const tempDir = await makeTempWorkspace("operator-bootstrap-extra-");
  ...
  it("re-applies subagent bootstrap allowlist after extras are added", async () => {
    const tempDir = await makeTempWorkspace("operator-bootstrap-extra-subagent-");
