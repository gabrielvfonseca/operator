  ...
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [clientPath], {
      env: {
        ...
        OPERATOR_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: "1",
        PORT: String(port),
        ...env,
      },
...
function runWriteConfig(root: string, env: Record<string, string> = {}) {
  return spawnSync(process.execPath, [writeConfigPath], {
    ...
    env: {
      ...
      OPERATOR_GATEWAY_TOKEN: "test-token",
      OPERATOR_OPENAI_CHAT_TOOLS_MODEL: "openai/gpt-5.6-luna",
      OPERATOR_STATE_DIR: path.join(root, "state"),
      OPERATOR_TEST_WORKSPACE_DIR: path.join(root, "workspace"),
      PORT: "18789",
      ...env,
    },
  });
...
function runDockerRunnerAuthPreflight(root: string, env: Record<string, string> = {}) {
  return spawnSync("bash", [dockerRunnerPath], {
    ...
    env: {
      ...
      OPENAI_API_KEY: "",
      OPENAI_BASE_URL: "",
      OPERATOR_OPENAI_CHAT_TOOLS_PROFILE_FILE: path.join(root, "missing.profile"),
      ...env,
    },
  });
}
...
describe("scripts/e2e/lib/openai-chat-tools/client.mjs", () => {
  ...
  it("treats placeholder profile auth as missing before Docker build work starts", () => {
    ...
    try {
      ...
      const result = runDockerRunnerAuthPreflight(root, {
        OPERATOR_OPENAI_CHAT_TOOLS_PROFILE_FILE: profile,
      });
      const output = `${result.stdout}\n${result.stderr}`;
  ...
  it.each([
    ["timeout", "OPERATOR_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS", "1e3"],
    ["body limit", "OPERATOR_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES", "64bytes"],
  ])(
  ...
  it("passes normalized timeout and body limits into the Docker runner", () => {
    ...
    expect(runner).toContain(
      "docker_e2e_read_positive_int_env OPERATOR_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS 180",
    );
    expect(runner).toContain(
      "docker_e2e_read_positive_int_env OPERATOR_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES 1048576",
    );
    expect(runner).toContain('-e "OPERATOR_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS=$TIMEOUT_SECONDS"');
    expect(runner).toContain('-e "OPERATOR_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES=$MAX_BODY_BYTES"');
  });

  it("rejects loose timeout env values instead of parsing numeric prefixes", async () => {
    const result = await runClient(1, {
      OPERATOR_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: "1e3",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid OPERATOR_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: 1e3");
  });

  it("rejects loose body limit env values instead of parsing numeric prefixes", async () => {
    const result = await runClient(1, {
      OPERATOR_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES: "64bytes",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid OPERATOR_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES: 64bytes");
  });
  ...
  it("writes strict positive timeout and port values into generated config", () => {
    ...
    try {
      const result = runWriteConfig(root, {
        OPERATOR_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: "240",
        PORT: "19001",
      });
  ...
  it("caps chat completion response bodies before JSON parsing", async () => {
    ...
    const port = await listen(server);
    try {
      const result = await runClient(port, { OPERATOR_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES: "64" });
