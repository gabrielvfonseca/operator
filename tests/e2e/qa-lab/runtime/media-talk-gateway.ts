  ...
  register(api) {
    api.registerSpeechProvider({
      ...
      async synthesize(request) {
        fs.appendFileSync(process.env.OPERATOR_QA_SPEECH_CALLS_PATH, JSON.stringify({ text: request.text, target: request.target }) + "\\n");
        return {
    ...
    api.registerRealtimeVoiceProvider({
      ...
      async createBrowserSession(request) {
        fs.appendFileSync(process.env.OPERATOR_QA_REALTIME_CALLS_PATH, JSON.stringify({ tools: request.tools?.map((tool) => tool.name) ?? [] }) + "\\n");
        return {
...
async function runWebchatAutoTtsProof(options: ProducerOptions): Promise<string> {
  ...
  try {
    gateway = await startQaGatewayChild({
      ...
      runtimeEnvPatch: {
        OPERATOR_QA_SPEECH_CALLS_PATH: fixture.speechCallsPath,
        OPERATOR_QA_REALTIME_CALLS_PATH: fixture.realtimeCallsPath,
      },
      mutateConfig: (config) => {
