...
async function runScheduler(options: ProducerOptions, appendLog: (chunk: unknown) => void) {
  ...
  let packageTgz = process.env.OPERATOR_CURRENT_PACKAGE_TGZ;
  if (packageTgz) {
  ...
  return await new Promise<{
    ...
    const child = spawn(process.execPath, ["scripts/test-docker-all.mjs"], {
      ...
      env: {
        ...process.env,
        OPERATOR_DOCKER_ALL_BUILD: "1",
        OPERATOR_DOCKER_ALL_DRY_RUN: "0",
        OPERATOR_DOCKER_ALL_LANES: options.lane,
        OPERATOR_DOCKER_ALL_LOG_DIR: dockerRunDir,
        OPERATOR_DOCKER_ALL_PARALLELISM: "1",
        OPERATOR_DOCKER_ALL_PREFLIGHT: "1",
        OPERATOR_DOCKER_ALL_TIMINGS_FILE: path.join(dockerRunDir, "lane-timings.json"),
        OPERATOR_DOCKER_ARTIFACT_IDENTITY_PATH: identityPath,
        ...(packageTgz ? { OPERATOR_CURRENT_PACKAGE_TGZ: packageTgz } : {}),
      },
