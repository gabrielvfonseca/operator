...
const QA_DOCKER_E2E_LANES = {
  ...
  "update-migration": {
    env: (env) => ({
      OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC:
        env.OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC ?? "operator@2026.4.23",
      OPERATOR_UPGRADE_SURVIVOR_PUBLISHED_BASELINE: "1",
      OPERATOR_UPGRADE_SURVIVOR_SCENARIO:
        env.OPERATOR_UPGRADE_SURVIVOR_SCENARIO ?? "plugin-deps-cleanup",
    }),
    script: "scripts/e2e/upgrade-survivor-docker.sh",
  },
  "update-restart-auth": {
    env: (env) => ({
      OPERATOR_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT:
        env.OPERATOR_UPGRADE_SURVIVOR_DOCKER_RUN_TIMEOUT ?? "1500s",
      OPERATOR_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE: "auto-auth",
    }),
    script: "scripts/e2e/upgrade-survivor-docker.sh",
  },
  "upgrade-survivor": {
