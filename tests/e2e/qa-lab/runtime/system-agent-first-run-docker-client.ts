...
const DISCORD_CREDENTIAL_FIXTURE = ["@gabrielvfonseca/operator", "discord", "fixture"].join("-");
...
async function runPackagedCli(args: string[]): Promise<PackagedCommandResult> {
  const child = spawn("@gabrielvfonseca/operator", args, {
    env: process.env,
...
async function main() {
  ...
  setEnvValue("OPERATOR_CONFIG_PATH", configPath);
  ...
  clearConfigCache();
  assert(
    await shouldStartOnboardingForFreshInstall(["node", "@gabrielvfonseca/operator"]),
