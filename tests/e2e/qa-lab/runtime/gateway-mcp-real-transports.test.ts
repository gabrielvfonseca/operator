...
function createRepoRoot() {
  return tempDirs.make("operator-qalab-cli-entry-");
}
...
describe("gateway MCP real transport producer", () => {
  it("uses the source channel MCP module when build output is absent", async () => {
    ...
    expect(mcp.args[3]).toContain("serveOperatorChannelMcp");
    expect(mcp.cwd).toBe(root);
    expect(mcp.envPatch).toStrictEqual({
      OPERATOR_QA_GATEWAY_TOKEN: "secret-token",
      OPERATOR_QA_GATEWAY_URL: "ws://127.0.0.1:12345",
    });
  });
  ...
  it("isolates the source plugin-tools MCP invocation", () => {
    ...
    const invocation = testing.resolvePluginToolsMcpInvocation({
      configPath: "/tmp/plugin-tools/operator.json",
    ...
    expect(invocation).toStrictEqual({
      ...
      env: {
        ...
        OPERATOR_CONFIG_PATH: "/tmp/plugin-tools/operator.json",
        OPERATOR_DISABLE_BUNDLED_PLUGINS: "1",
        OPERATOR_HOME: "/tmp/plugin-tools/home",
        OPERATOR_STATE_DIR: "/tmp/plugin-tools/state",
      },
    });
  });
