// QA OTEL Smoke runtime supports Operator repository automation.
...
const DEFAULT_DOCKER_COLLECTOR_IMAGE =
  process.env.OPERATOR_QA_OTEL_COLLECTOR_IMAGE || "otel/opentelemetry-collector:0.104.0";
...
const REQUIRED_METRIC_NAMES = ["operator.harness.duration_ms"] as const;
...
const MAX_OTLP_COMPRESSED_BODY_BYTES = readPositiveIntegerEnv(
  "OPERATOR_QA_OTEL_MAX_COMPRESSED_BODY_BYTES",
  2 * 1024 * 1024,
);
const MAX_OTLP_DECODED_BODY_BYTES = readPositiveIntegerEnv(
  "OPERATOR_QA_OTEL_MAX_DECODED_BODY_BYTES",
  8 * 1024 * 1024,
);
const MAX_CAPTURED_BODY_TEXT_BYTES = readPositiveIntegerEnv(
  "OPERATOR_QA_OTEL_MAX_CAPTURED_BODY_TEXT_BYTES",
  512 * 1024,
);
const MAX_STDOUT_DIAGNOSTIC_LINE_BYTES = readPositiveIntegerEnv(
  "OPERATOR_QA_OTEL_MAX_STDOUT_DIAGNOSTIC_LINE_BYTES",
  512 * 1024,
);
...
function isStdoutDiagnosticLogRecord(value: unknown): value is StdoutDiagnosticLogRecord {
  return (
    ...
    value !== null &&
    !Array.isArray(value) &&
    objectValue(value, "signal") === "operator.diagnostic.log"
  );
}
...
async function startDockerOtelCollector(
  ...
  const tempDir = await makeTempDir(path.join(osTmpdir(), "operator-otel-collector-"));
  const configPath = path.join(tempDir, "collector.yaml");
  const containerName = `operator-otel-smoke-${makeUuid()}`;
...
function isLatestGenAiModelCallSpan(span: CapturedSpan): boolean {
  ...
  return (
    ...
    typeof span.attributes["operator.provider"] === "string" &&
    typeof span.attributes["operator.model"] === "string"
  );
}
...
async function runDirectTelemetryProducer(params: {
  ...
  process.env.OTEL_SERVICE_NAME = "operator-qa-lab-otel-smoke";
...
function assertSmoke(params: {
  ...
  if (spanNames.has("operator.model.call")) {
    failures.push("legacy operator.model.call span exported with GenAI semconv opt-in");
  }
  ...
  const contentKeys = [...attributeKeys].filter((key) => key.startsWith("operator.content."));
  ...
  const modelErrorSpans = modelSpans.filter((span) => {
    ...
    return (
      ...
      Object.hasOwn(span.attributes, "operator.errorCategory") ||
  ...
  const failedRunSpans = params.spans.filter(
    (span) =>
      (span.name === "operator.run" || span.name === "operator.harness.run") &&
      span.attributes["operator.error"] === `${DIRECT_ERROR_MESSAGE} OPENAI_API_KEY=***`,
  );
  if (failedRunSpans.length !== 2) {
    const observed = params.spans
      .filter((span) => span.name === "operator.run" || span.name === "operator.harness.run")
      .map((span) => ({ name: span.name, error: span.attributes["operator.error"] }));
    failures.push(
