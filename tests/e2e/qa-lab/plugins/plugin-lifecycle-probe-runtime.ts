...
function stateDir(env: ProbeEnv = process.env) {
  return env.OPERATOR_STATE_DIR || path.join(env.HOME ?? os.homedir(), ".operator");
}

function configPath(env: ProbeEnv = process.env) {
  return env.OPERATOR_CONFIG_PATH || path.join(stateDir(env), "operator.json");
}
...
function createMatrixStateEnv(resourceDir: string): MatrixEnv {
  ...
  return {
    ...
    OPERATOR_CONFIG_PATH: configFile,
    OPERATOR_TEST_WORKSPACE_DIR: workspaceDir,
    OPERATOR_AUTH_PROFILE_SECRET_KEY: randomBytes(32).toString("hex"),
  };
}
...
async function installOperatorPackage(prefix: string, env: MatrixEnv) {
  const packageTgz = env.OPERATOR_CURRENT_PACKAGE_TGZ;
  assertProbe(packageTgz, "OPERATOR_CURRENT_PACKAGE_TGZ is required");
  ...
  await runCommand(
    ...
    ["install", "-g", "--prefix", prefix, packageTgz, "--no-fund", "--no-audit"],
    {
      ...
      timeoutMs: parseDurationMs(env.OPERATOR_E2E_NPM_INSTALL_TIMEOUT, "600s"),
    },
  );
}
...
async function runPluginLifecycleMatrix() {
  ...
  const packageName = "@gabrielvfonseca/lifecycle-claw";
