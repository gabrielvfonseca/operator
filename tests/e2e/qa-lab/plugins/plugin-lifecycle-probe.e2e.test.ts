...
function makeTempDir(): string {
  return tempDirs.make("operator-plugin-lifecycle-probe-");
}
...
describe("plugin lifecycle matrix probe", () => {
  ...
  it("rejects unreadable config during uninstall proof", async () => {
    const dir = makeTempDir();
    const configFile = path.join(dir, ".operator", "operator.json");
  ...
  it("keeps fallback SIGKILL armed for ignored-stdio descendants", async () => {
    ...
    try {
      ...
      const parentScript = [
        ...
        "child.unref();",
        "writeFileSync(process.env.OPERATOR_TEST_DESCENDANT_PID, String(child.pid));",
      ...
      const run = probeTesting.runCommand(
        ...
        {
          env: { ...process.env, OPERATOR_TEST_DESCENDANT_PID: descendantPidPath },
          timeoutKillGraceMs: 100,
          timeoutMs: 500,
        },
      );
