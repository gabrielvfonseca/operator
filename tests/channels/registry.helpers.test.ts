...
import { listChatChannels } from "../../src/channels/chat-meta.js";
import { normalizeAnyChannelId as normalizeAnyChannelIdLight } from "../../src/channels/registry-normalize.js";
import {
...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://operator.ai");
  });

  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    const startupRegistry = createRegistryWithRegisteredChannel("operator-weixin", ["weixin"]);
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["operator-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("operator-weixin");
