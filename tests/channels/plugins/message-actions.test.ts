...
} from "../../../src/channels/plugins/message-action-discovery.js";
import type { ChannelMessageCapability } from "../../../src/channels/plugins/message-capabilities.js";
...
describe("message action capability checks", () => {
  ...
  it("aggregates capabilities across plugins", () => {
    ...
    expect(channelSupportsMessageCapability({} as OperatorConfig, "presentation")).toBe(true);
    expect(channelSupportsMessageCapability({} as OperatorConfig, "delivery-pin")).toBe(true);
  });
  ...
  it("checks per-channel capabilities", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as OperatorConfig, channel: "demo-buttons" },
        "presentation",
      ),
    ).toBe(true);
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as OperatorConfig, channel: "demo-cards" },
        "presentation",
      ),
    ).toBe(false);
    expect(
    ...
    expect(
      channelSupportsMessageCapabilityForChannel({ cfg: {} as OperatorConfig }, "delivery-pin"),
    ).toBe(false);
  });
  ...
  it("normalizes channel aliases for per-channel capability checks", () => {
    ...
    expect(
      channelSupportsMessageCapabilityForChannel(
        { cfg: {} as OperatorConfig, channel: "demo-cards-alias" },
        "delivery-pin",
      ),
    ).toBe(true);
  });
  ...
  it("skips crashing action/capability discovery paths and logs once", () => {
    ...
    expect(channelSupportsMessageCapability({} as OperatorConfig, "presentation")).toBe(false);
