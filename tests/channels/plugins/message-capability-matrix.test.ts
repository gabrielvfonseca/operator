...
import type { ChannelMessageActionAdapter, ChannelPlugin } from "../../../src/channels/plugins/types.public.js";
...
describe("channel action capability matrix", () => {
  ...
  function getCapabilities(plugin: Pick<ChannelPlugin, "actions">, cfg: OperatorConfig) {
  ...
  it("forwards Telegram action capabilities through the channel wrapper", () => {
    ...
    const result = getCapabilities(telegramPlugin, {} as OperatorConfig);
    ...
    const discordResult = getCapabilities(discordPlugin, {} as OperatorConfig);
