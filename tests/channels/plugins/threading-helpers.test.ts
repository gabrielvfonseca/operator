...
} from "../../../src/channels/plugins/threading-helpers.js";

describe("createStaticReplyToModeResolver", () => {
  it.each(["off", "all"] as const)("always returns the configured mode %s", (mode) => {
    expect(createStaticReplyToModeResolver(mode)({ cfg: {} as OperatorConfig })).toBe(mode);
  });
});
  ...
  it.each([
    {
      ...
      cfg: { channels: { "demo-top-level": { replyToMode: "first" } } } as OperatorConfig,
      expected: "first",
    },
    {
...
describe("createScopedAccountReplyToModeResolver", () => {
  ...
  it("passes chatType through", () => {
    ...
    expect(resolver({ cfg: {} as OperatorConfig, chatType: "group" })).toBe("first");
