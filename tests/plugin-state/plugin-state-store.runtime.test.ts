...
import type { PluginRecord } from "../../src/plugins/registry-types.js";
...
describe("plugin runtime state proxy", () => {
  it("binds openKeyedStore to the bundled plugin id and keeps resolveStateDir", async () => {
    await withOperatorTestState({ label: "plugin-state-runtime" }, async (state) => {
  ...
  it("allows trusted official global plugins to use keyed state", async () => {
    await withOperatorTestState({ label: "plugin-state-trusted-global" }, async () => {
      const registry = createTestPluginRegistry();
