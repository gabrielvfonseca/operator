...
import { evaluateGatewayAuthSurfaceStates } from "../../src/secrets/runtime-gateway-auth-surfaces.js";
...
function evaluate(config: OperatorConfig, env: NodeJS.ProcessEnv = EMPTY_ENV) {
...
describe("evaluateGatewayAuthSurfaceStates", () => {
  ...
  it("marks gateway.auth.token inactive when env token is configured", () => {
    const states = evaluate(
      ...
      { OPERATOR_GATEWAY_TOKEN: "env-token" } as NodeJS.ProcessEnv,
    );
