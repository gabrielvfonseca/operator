...
import { setRuntimeAuthProfileStoreSnapshot } from "../../src/agents/auth-profiles/runtime-snapshots.js";
import { getRuntimeConfigSnapshotRefreshHandler } from "../../src/config/runtime-snapshot.js";
import { activateSecretsRuntimeSnapshot, getActiveSecretsRuntimeSnapshot } from "../../src/secrets/runtime.js";
import {
  asConfig,
  loadAuthStoreWithProfiles,
  setupSecretsRuntimeSnapshotTestHooks,
} from "../../src/secrets/runtime.test-support.ts";
...
describe("secrets runtime snapshot request secret refs", () => {
  it("can skip auth-profile SecretRef resolution when includeAuthStoreRefs is false", async () => {
    const missingEnvVar = `OPERATOR_MISSING_AUTH_PROFILE_SECRET_${Date.now()}`;
  ...
  it("can skip auth-profile SecretRef resolution during active runtime refresh", async () => {
    const initialEnvVar = `OPERATOR_INITIAL_AUTH_PROFILE_SECRET_${Date.now()}`;
    const missingEnvVar = `OPERATOR_MISSING_AUTH_PROFILE_SECRET_${Date.now()}`;
    delete process.env[missingEnvVar];
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "operator-runtime-exec-preflight-"));
      try {
