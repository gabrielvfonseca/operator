...
import { registerResolvedAgentDir } from "../../src/agents/agent-dir-registry.js";
import { getRuntimeAuthProfileStoreCredentialMutationToken } from "../../src/agents/auth-profiles/runtime-snapshots.js";
import {
...
import type { SecretsApplyPlan } from "../../src/secrets/plan.js";
...
async function writeJsonFile(filePath: string, value: unknown): Promise<void> {
  if (path.basename(filePath) === "operator-agent.sqlite") {
...
async function createApplyFixture(): Promise<ApplyFixture> {
  const paths = buildFixturePaths(
    await fs.mkdtemp(path.join(os.tmpdir(), "operator-secrets-apply-")),
  );
