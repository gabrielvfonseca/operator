...
} from "../../src/secrets/provider-integrations.js";
import { resolveSecretRefString } from "../../src/secrets/resolve.js";

const tempDirs: string[] = [];

function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-secret-provider-integrations-"));
...
describe("secret provider integration presets", () => {
  ...
  it("skips presets whose persisted plugin integration IDs would violate config schema limits", () => {
    ...
    fs.writeFileSync(
      path.join(longPluginRootDir, "operator.plugin.json"),
      JSON.stringify({
        id: longPluginId,
        secretProviderIntegrations: {
          vault: {
  ...
  it.skipIf(process.platform === "win32")("allows node presets from symlinked plugin roots", () => {
    ...
    fs.writeFileSync(
      path.join(realRoot, "operator.plugin.json"),
      JSON.stringify({
