...
import { getRuntimeConfig, writeConfigFile } from "../../src/config/config.js";
...
} from "../../src/secrets/runtime.integration.test-helpers.js";
import {
...
describe("secrets runtime snapshot gateway-auth integration", () => {
  ...
  it(
    ...
    async () => {
      await withTempHome("operator-secrets-runtime-gateway-auth-reload-lkg-", async (home) => {
        ...
        const persistedConfig = JSON.parse(
          await fs.readFile(path.join(home, ".operator", "operator.json"), "utf8"),
        ) as OperatorConfig;
