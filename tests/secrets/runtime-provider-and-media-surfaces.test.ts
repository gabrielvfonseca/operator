...
function createOpenAiFileModelsConfig(): NonNullable<OperatorConfig["models"]> {
  return {
    providers: {
      openai: {
...
describe("secrets runtime provider and media surfaces", () => {
  ...
  it("resolves file refs via configured file provider", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "operator-secrets-file-provider-"));
  ...
  it("refreshes provider auth without resolving or republishing gateway state", async () => {
    ...
    const root = autoCleanupTempDirs.make("operator-provider-auth-refresh-");
    const secretsPath = path.join(root, "secrets.json");
  ...
  it("fails when file provider payload is not a JSON object", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "operator-secrets-file-provider-bad-"));
    const secretsPath = path.join(root, "secrets.json");
    try {
