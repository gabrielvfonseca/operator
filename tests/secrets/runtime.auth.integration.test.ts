...
} from "../../src/secrets/runtime-auth.integration.test-helpers.js";
import {
...
describe("secrets runtime snapshot auth integration", () => {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("operator-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".operator", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".operator", "agents", "ops", "agent");
