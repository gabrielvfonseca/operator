...
} from "../../src/agents/provider-attribution.js";
...
describe("provider attribution", () => {
  it("resolves the canonical Operator product and runtime version", () => {
  ...
  it("returns a documented NVIDIA attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      version: "2026.3.22",
      headers: {
        "X-BILLING-INVOKE-ORIGIN": "Operator",
      },
    });
  });
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { OPERATOR_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
  ...
  it("returns a hidden-spec xAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("xai", { OPERATOR_VERSION: "2026.3.22" })).toEqual({
      provider: "xai",
      enabledByDefault: true,
      verification: "vendor-hidden-api-spec",
      hook: "request-headers",
  ...
  it("lists the current attribution support matrix", () => {
    ...
    expect(
      listProviderAttributionPolicies({ OPERATOR_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
