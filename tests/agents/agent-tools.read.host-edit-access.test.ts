 ...
 * Ensures Operator lets the real guarded read path report escape errors
...
import { createHostWorkspaceEditTool } from "../../src/agents/agent-tools.read.js";
...
describe("createHostWorkspaceEditTool host access mapping", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-edit-access-test-"));
