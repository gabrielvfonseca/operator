...
import { coerceSecretRef, type SecretInput } from "../../src/config/types.secrets.js";
import { parseLiveCsvFilter } from "../../src/media-generation/live-test-helpers.js";
import { withBundledPluginEnablementCompat } from "../../src/plugins/bundled-compat.js";
...
} from "../../src/agents/agent-model-discovery.js";
import { resolveDefaultAgentDir } from "../../src/agents/agent-scope.js";
import { externalCliDiscoveryForProviders } from "../../src/agents/auth-profiles/external-cli-discovery.js";
import { ensureCustomApiRegistered } from "../../src/agents/custom-api-registry.js";
import { isRateLimitErrorMessage } from "../../src/agents/embedded-agent-helpers/errors.js";
import { extractAssistantText } from "../../src/agents/embedded-agent-utils.js";
import { collectProviderApiKeys } from "../../src/agents/live-auth-keys.js";
import { appendPrioritizedDynamicLiveModels } from "../../src/agents/live-model-dynamic-candidates.js";
...
} from "../../src/agents/live-model-filter.js";
import {
...
} from "../../src/agents/live-model-turn-probes.js";
import { createLiveTargetMatcher } from "../../src/agents/live-target-matcher.js";
import {
...
import { shouldSkipLiveProviderDrift } from "../../src/agents/live-test-provider-drift.js";
import {
  isLiveBillingDrift,
  isLiveRateLimitDrift,
} from "../../src/agents/live-test-provider-drift.test-support.js";
import {
  getApiKeyForModel,
  requireApiKey,
  resolveUsableCustomProviderApiKey,
} from "../../src/agents/model-auth.js";
import { shouldSuppressBuiltInModel } from "../../src/agents/model-suppression.js";
import { ensureOperatorModelsJson } from "../../src/agents/models-config.js";
...
const DIRECT_ENABLED = Boolean(process.env.OPERATOR_LIVE_MODELS?.trim());
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_HEARTBEAT_MS = Math.max(1_000, toInt(process.env.OPERATOR_LIVE_HEARTBEAT_MS, 30_000));
const LIVE_SETUP_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.OPERATOR_LIVE_SETUP_TIMEOUT_MS, 45_000),
);
const LIVE_TEST_TIMEOUT_MS = Math.max(
  1_000,
  toInt(process.env.OPERATOR_LIVE_TEST_TIMEOUT_MS, 60 * 60 * 1000),
);
const DEFAULT_LIVE_MODEL_CONCURRENCY = 20;
const LIVE_MODEL_CONCURRENCY = resolveLiveModelConcurrency(
  process.env.OPERATOR_LIVE_MODEL_CONCURRENCY,
);
const LIVE_MODELS_JSON_TIMEOUT_MS = resolveLiveModelsJsonTimeoutMs(
  process.env.OPERATOR_LIVE_MODELS_JSON_TIMEOUT_MS,
);
...
let activeLiveCompletionConfig: OperatorConfig | undefined;
...
function applyLiveOllamaProviderEnvCompat(params: {
  ...
  const liveBaseUrl = params.env?.OPERATOR_LIVE_OLLAMA_BASE_URL?.trim();
...
function resolveLiveOllamaBaseUrl(model: Pick<Model, "baseUrl">, config?: OperatorConfig): string {
  return (
...
describe("explicit live model discovery scope", () => {
  ...
  it("merges explicit model providers with OPERATOR_LIVE_PROVIDERS", () => {
  ...
  it("uses the local Ollama auth marker for self-hosted live env URLs", () => {
    ...
    for (const baseUrl of [
      ...
      const result = applyLiveProviderDiscoveryPluginCompat({
        ...
        providers: ["ollama"],
        env: {
          OPERATOR_LIVE_OLLAMA_BASE_URL: baseUrl,
        },
      });
  ...
  it("replaces configured Ollama auth when live env redirects to a different local endpoint", () => {
    ...
    const result = applyLiveProviderDiscoveryPluginCompat({
      ...
      providers: ["ollama"],
      env: {
        OPERATOR_LIVE_OLLAMA_BASE_URL: "http://127.0.0.1:11434",
      },
    });
...
describeLive("live models (profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.OPERATOR_LIVE_MODELS?.trim();
      ...
      const providers = parseProviderFilter(process.env.OPERATOR_LIVE_PROVIDERS);
      ...
      if (!DIRECT_ENABLED) {
        logProgress(
          "[live-models] skipping (set OPERATOR_LIVE_MODELS=modern|small|all|<list>; all=modern)",
        );
        return;
      }
      ...
      const perModelTimeoutMs = toInt(process.env.OPERATOR_LIVE_MODEL_TIMEOUT_MS, 30_000);
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[live-models] capped to ${selectedCandidates.length}/${candidates.length} via OPERATOR_LIVE_MAX_MODELS=${maxModels}`,
        );
      }
