...
import { clearCurrentProviderAuthState } from "../../src/agents/model-provider-auth.js";
import { runProviderAuthWarmWorkerInput } from "../../src/agents/model-provider-auth.worker.js";
...
describe("provider auth warm worker", () => {
  ...
  it("preserves runtime-only auth profile snapshots in the worker warm input", async () => {
    ...
    const root = mkdtempSync(path.join(tmpdir(), "operator-provider-auth-worker-"));
    tempDirs.push(root);
