...
import { createParameterFreeTool } from "@gabrielvfonseca/operator/plugin-sdk/agent-runtime-test-contracts";
import { afterEach, describe, expect, it, vi } from "vitest";
import { resetConfigRuntimeState, setRuntimeConfigSnapshot } from "../../../src/config/config.js";
import {
  resolveProviderRuntimePluginHandle,
  prepareProviderExtraParams,
  resolveProviderFollowupFallbackRoute,
  type ProviderRuntimePluginHandle,
} from "../../../src/plugins/provider-hook-runtime.js";
import { buildAgentRuntimeDeliveryPlan, buildAgentRuntimePlan } from "../../../src/agents/runtime-plan/build.js";
...
describe("AgentRuntimePlan", () => {
  ...
  it("defers default transport extra params until they are read", () => {
    ...
    const plan = buildAgentRuntimePlan({
      ...
      config: {},
      workspaceDir: "/tmp/operator-runtime-plan",
      model: gpt54Model,
    });
  ...
  it("keeps Operator-owned tool-schema normalization reachable from the plan", () => {
    const plan = buildAgentRuntimePlan({
      provider: "openai",
      modelId: "gpt-5.4",
      modelApi: "openai-responses",
  ...
  it("forwards OpenAI Codex profiles for explicit OpenAI Operator runs", () => {
    const plan = buildAgentRuntimePlan({
      provider: "openai",
      modelId: "gpt-5.4",
      modelApi: "openai-responses",
