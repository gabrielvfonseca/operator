...
import type { Model, SimpleStreamOptions } from "../../../src/llm/types.js";
...
import { takeRuntimeUserTurnTranscriptContext } from "../../../src/sessions/user-turn-transcript-runtime-context.js";
...
import type { LoadExtensionsResult, ToolDefinition } from "../../../src/agents/sessions/extensions/types.js";
  ...
  it("carries prepared transcript context on the exact steered message", async () => {
    ...
    expect(takeRuntimeUserTurnTranscriptContext(runtimeMessage)).toMatchObject({
      message: {
        ...
        __operator: { senderId: "user-42", senderName: "Ada" },
      },
      recorder,
    });
  });
...
describe("createAgentSession attribution headers", () => {
  ...
  it("keeps Cloudflare attribution headers for provider and endpoint matches", async () => {
    ...
    expect(providerOptions.headers).toMatchObject({ "User-Agent": "@gabrielvfonseca/operator" });
    expect(endpointOptions.headers).toMatchObject({ "User-Agent": "@gabrielvfonseca/operator" });
  });
});
...
describe("createAgentSession tool defaults", () => {
  ...
  it("preserves an exact base system prompt when active tools change", async () => {
    ...
    const systemPrompt = "You are a personal assistant running inside Operator.";
