...
} from "../../src/agents/anthropic-payload-policy.js";
...
describe("anthropic payload policy", () => {
  ...
  it("keeps implicit env-driven long retention conservative for custom hosts", () => {
    const previous = process.env.OPERATOR_CACHE_RETENTION;
    process.env.OPERATOR_CACHE_RETENTION = "long";
    try {
      const policy = resolveAnthropicPayloadPolicy({
        provider: "anthropic",
        api: "anthropic-messages",
      ...
      if (previous === undefined) {
        delete process.env.OPERATOR_CACHE_RETENTION;
      } else {
        process.env.OPERATOR_CACHE_RETENTION = previous;
      }
    }
  });
