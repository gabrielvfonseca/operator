...
import type { AuthProfileFailureReason } from "../../src/agents/auth-profiles.js";
import { ensureAuthProfileStore, saveAuthProfileStore } from "../../src/agents/auth-profiles/store.js";
...
} from "../../src/agents/test-helpers/embedded-agent-runner-e2e-fixtures.js";
import {
  installEmbeddedRunnerBackoffE2eMocks,
  installEmbeddedRunnerBaseE2eMocks,
  installEmbeddedRunnerFastRunE2eMocks,
} from "../../src/agents/test-helpers/embedded-agent-runner-e2e-mocks.js";
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): OperatorConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): OperatorConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  try {
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `operator-auth-rotation-${Date.now()}.log`),
    });
