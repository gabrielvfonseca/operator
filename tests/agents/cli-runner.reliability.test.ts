...
import { getReplyPayloadMetadata } from "../../src/auto-reply/reply-payload.js";
...
import { loadTranscriptEvents, upsertSessionEntry } from "../../src/config/sessions/session-accessor.js";
...
import { resetDiagnosticEventsForTest } from "../../src/infra/diagnostic-events.js";
...
import { runSkillResearchAutoCapture } from "../../src/skills/research/autocapture.js";
...
import { resetClaudeLiveSessionsForTest } from "../../src/agents/cli-runner/claude-live-session.test-support.js";
import { executePreparedCliRun } from "../../src/agents/cli-runner/execute.js";
import {
  resolveCliNoOutputTimeoutMs,
  resolveCliRunTimeoutOverrideMs,
} from "../../src/agents/cli-runner/helpers.js";
import { prepareCliRunContext } from "../../src/agents/cli-runner/prepare.js";
import { hashCliReseedPrompt } from "../../src/agents/cli-runner/reseed-envelope.js";
import * as sessionHistoryModule from "../../src/agents/cli-runner/session-history.js";
...
import { MAX_AGENT_HOOK_HISTORY_MESSAGES } from "../../src/agents/harness/hook-history.js";
...
function createSessionFile(params?: { history?: Array<{ role: "user"; content: string }> }) {
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-cli-hooks-"));
...
describe("runCliAgent reliability", () => {
  ...
  it("preserves fresh retry for direct CLI callers without a pre-clear hook", async () => {
    ...
    const stateDir = autoCleanupTempDirs.make("operator-cli-retry-images-");
  ...
  it("keeps non-capture live-session artifacts through fresh recovery retry", async () => {
    ...
    const artifactDir = autoCleanupTempDirs.make("operator-live-retry-artifacts-");
  ...
  it("passes cwd to approved CLI user-turn persistence", async () => {
    ...
    const { dir, sessionFile } = createSessionFile();
    const taskDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-cli-persist-cwd-"));
  ...
  it("does not execute the CLI when approved user turn persistence fails", async () => {
    supervisorSpawnMock.mockClear();
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-cli-persist-fail-"));
    const onUserMessagePersisted = vi.fn();
  ...
  it("blocks CLI runs before llm_input and model execution when before_agent_run blocks", async () => {
    ...
    try {
      ...
      expect(blockedLine.message["__operator"].beforeAgentRunBlocked.blockedBy).toBe(
        "policy-plugin",
      );
      expect(blockedLine.message["__operator"].beforeAgentRunBlocked).not.toHaveProperty("reason");
      expect(Object.hasOwn(blockedLine.message["__operator"], "beforeAgentRunBlocked")).toBe(true);
    } finally {
      fs.rmSync(dir, { recursive: true, force: true });
    }
  });
