...
import { PLUGIN_MODEL_CATALOG_GENERATED_BY } from "../../../src/agents/plugin-model-catalog.js";
import { resetModelDiscoveryCacheForTest } from "../../../src/agents/embedded-agent-runner/model-discovery-cache.test-support.js";
import { createProviderRuntimeTestMock } from "../../../src/agents/embedded-agent-runner/model.provider-runtime.test-support.js";
...
vi.mock("../model-suppression.js", () => {
  ...
  // extensions/qwen/operator.plugin.json and src/plugins/manifest-model-suppression.ts.
...
import type { OperatorConfig, OperatorConfigInput } from "../../../src/config/config.js";
import { COPILOT_INTEGRATION_ID, buildCopilotIdeHeaders } from "../../../src/agents/copilot-dynamic-headers.js";
import { getModelProviderLocalService } from "../../../src/agents/provider-local-service.js";
import { getModelProviderRequestTransport } from "../../../src/agents/provider-request-config.js";
import { buildForwardCompatTemplate } from "../../../src/agents/embedded-agent-runner/model.forward-compat.test-support.js";
import { buildInlineProviderModels } from "../../../src/agents/embedded-agent-runner/model.inline-provider.js";
import { resolveModel, resolveModelAsync, resolveModelWithRegistry } from "../../../src/agents/embedded-agent-runner/model.js";
import {
...
} from "../../../src/agents/embedded-agent-runner/model.test-harness.js";
...
describe("resolveModel", () => {
  ...
  it("invalidates agent discovery stores when generated plugin catalogs change", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-model-cache-plugin-"));
  ...
  it("invalidates agent discovery stores when inherited default auth changes", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-model-cache-"));
    const agentDir = path.join(rootDir, "agent");
  ...
  it("uses the resolved default agent workspace for cached model discovery", () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-model-workspace-"));
    const agentDir = path.join(rootDir, "agent");
  ...
  it("invalidates agent discovery stores when implicit main auth changes without config", async () => {
    const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-model-cache-state-"));
  ...
  it("skips Operator auth and model discovery during dynamic model resolution", async () => {
  ...
  it("prefers user operator.json config over the Fireworks manifest for the same id", () => {
  ...
  it("does not build a configured fallback for unsupported xAI multi-agent models", () => {
    ...
    expect(result.model).toBeUndefined();
    expect(result.error).toBe(
      "Unknown model: xai/grok-4.20-multi-agent-0309. Operator does not currently support xAI multi-agent models; choose another xAI model. See https://docs.operator.ai/providers/xai.",
    );
  });
