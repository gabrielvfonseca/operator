...
import { resolveHeartbeatToolResponseFromReplyResult } from "../../../../src/auto-reply/heartbeat-tool-response.js";
import { getReplyPayloadMetadata } from "../../../../src/auto-reply/reply-payload.js";
import type { InteractiveReply, MessagePresentation } from "../../../../src/interactive/payload.js";
import {
  buildPayloads,
  expectSinglePayloadText,
  expectSingleToolErrorPayload,
} from "../../../../src/agents/embedded-agent-runner/run/payloads.test-helpers.js";
...
describe("buildEmbeddedRunPayloads tool-error warnings", () => {
  ...
  it("does not replay raw-looking accumulated tool output when final answer text is available", () => {
    const payloads = buildPayloads({
      assistantTexts: [
        "/root/operator/packages/gateway-protocol/src/schema/protocol-schemas.ts:181:  PluginControlUiDescriptorSchema,",
  ...
  it("strips NO_REPLY text but keeps voice media directives", () => {
    const payloads = buildPayloads({
      assistantTexts: ["NO_REPLY\nMEDIA:/tmp/operator/tts-a/voice-a.opus\n[[audio_as_voice]]"],
    });

    expect(payloads).toHaveLength(1);
    expect(payloads[0]?.mediaUrl).toBe("/tmp/operator/tts-a/voice-a.opus");
    expect(payloads[0]?.mediaUrls).toEqual(["/tmp/operator/tts-a/voice-a.opus"]);
