// Runtime-context prompt tests keep hidden Operator context separate from the
...
} from "../../../../src/agents/embedded-agent-runner/run/runtime-context-prompt.js";
...
describe("runtime context prompt submission", () => {
  ...
  it("ignores repeated inline marker mentions without recursive stack growth", () => {
    ...
    const inlineMarkers = Array.from(
      { length: 250 },
      () => "inline <<<BEGIN_OPERATOR_INTERNAL_CONTEXT>>> marker",
    ).join("\n");
    const effectivePrompt = [
      inlineMarkers,
      "",
    ...
    expect(parts.modelPrompt).toContain("inline <<<BEGIN_OPERATOR_INTERNAL_CONTEXT>>> marker");
  ...
  it("uses a marker prompt for runtime-only events", () => {
    ...
    expect(parts).toEqual({
      prompt: "Continue the Operator runtime event.",
  ...
  it("submits empty-transcript model prompts when persistence is suppressed separately", () => {
    expect(
      resolveRuntimeContextPromptParts({
        effectivePrompt: "[Operator room event]",
        transcriptPrompt: "",
  ...
  it("labels runtime-only events as system context", () => {
    ...
    expect(parts.runtimeSystemContext).toContain("Operator runtime event.");
