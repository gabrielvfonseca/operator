...
import type { ContextEngineRuntimeContext } from "../../../src/context-engine/types.js";
import { peekSystemEvents, resetSystemEventsForTest } from "../../../src/infra/system-events.js";
import { enqueueCommandInLane, markGatewayDraining } from "../../../src/process/command-queue.js";
import * as commandQueueModule from "../../../src/process/command-queue.js";
import { resetCommandQueueStateForTest } from "../../../src/process/command-queue.test-support.js";
import { createQueuedTaskRun as createQueuedTaskRunOrNull } from "../../../src/tasks/task-executor.js";
import { getTaskFlowById } from "../../../src/tasks/task-flow-registry.js";
import { getTaskById, listTasksForOwnerKey } from "../../../src/tasks/task-registry.js";
...
import { castAgentMessage } from "../../../src/agents/test-helpers/agent-message-fixtures.js";
import { resolveSessionLane } from "../../../src/agents/embedded-agent-runner/lanes.js";
...
describe("runContextEngineMaintenance", () => {
  ...
  it("defers turn maintenance to a hidden background task when enabled", async () => {
    await withStateDirEnv("operator-turn-maintenance-", async () => {
      vi.useFakeTimers();
      try {
  ...
  it("queues a follow-up maintenance run when a new turn finishes during an active deferred run", async () => {
    await withStateDirEnv("operator-turn-maintenance-rerun-", async () => {
      vi.useFakeTimers();
      try {
        resetCommandQueueStateForTest();
        resetTaskRegistryForTests({ persist: false });
  ...
  it("disposes owned deferred engines only after their maintenance run finishes", async () => {
    await withStateDirEnv("operator-turn-maintenance-dispose-", async () => {
      resetCommandQueueStateForTest();
      resetTaskRegistryForTests({ persist: false });
      resetTaskFlowRegistryForTests({ persist: false });
  ...
  it("reports deferred maintenance schedule failure while gateway is draining", async () => {
    await withStateDirEnv("operator-turn-maintenance-draining-", async () => {
      resetCommandQueueStateForTest();
      resetTaskRegistryForTests({ persist: false });
      resetTaskFlowRegistryForTests({ persist: false });
  ...
  it("rejects coalesced deferred maintenance requests while gateway is draining", async () => {
    await withStateDirEnv("operator-turn-maintenance-draining-coalesced-", async () => {
      vi.useFakeTimers();
      try {
        resetCommandQueueStateForTest();
        resetTaskRegistryForTests({ persist: false });
