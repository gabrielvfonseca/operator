...
} from "../../../src/agents/embedded-agent-runner/compaction-runtime-context.js";
...
describe("buildEmbeddedCompactionRuntimeContext", () => {
  ...
  it("preserves sender and current message routing for compaction", () => {
    const result = buildEmbeddedCompactionRuntimeContext({
      ...
      cwd: "/tmp/task-repo",
      agentDir: "/tmp/agent",
      config: {} as unknown as OperatorConfig,
      senderIsOwner: true,
      senderId: "user-123",
      provider: "openai",
      modelId: "gpt-5.4",
  ...
  it("preserves direct OpenAI compaction for the Operator runtime", () => {
    const result = resolveEmbeddedCompactionTarget({
      config: {
        models: {
          providers: {
