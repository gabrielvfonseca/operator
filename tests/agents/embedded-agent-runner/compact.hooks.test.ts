...
import {
  ...
  sessionCompactImpl,
  triggerInternalHook,
} from "../../../src/agents/embedded-agent-runner/compact.hooks.harness.js";
import {
...
describe("compactEmbeddedAgentSession hooks (ownsCompaction engine)", () => {
  ...
  it("keeps unbound api-key queued compaction on operator without native harness compaction", async () => {
    resolveAgentHarnessPolicyMock.mockReturnValue({
