...
import { OperatorStdioClientTransport } from "../../src/agents/mcp-stdio-transport.js";
...
describe("OperatorStdioClientTransport", () => {
  afterEach(() => {
    vi.useRealTimers();
    spawnMock.mockReset();
  ...
  it("starts stdio MCP servers in a disposable process group on POSIX", async () => {
    // Detached POSIX process groups let Operator clean up child tool servers
  ...
  it("kills the process tree when graceful stdio close does not exit", async () => {
    ...
    const transport = new OperatorStdioClientTransport({ command: "npx" });
    const started = transport.start();
    child.emit("spawn");
    await started;
  ...
  it("reports stderr pipe errors without an unhandled error crash", async () => {
    ...
    const transport = new OperatorStdioClientTransport({ command: "npx", stderr: "pipe" });
    const onerror = vi.fn();
