...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "OPERATOR_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { OPERATOR_LIVE_MODEL_IMAGE_PROBE: "false" },
        "OPERATOR_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { OPERATOR_LIVE_MODEL_IMAGE_PROBE: "1" },
        "OPERATOR_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
