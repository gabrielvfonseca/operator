...
import { consumeEmbeddedToolSendReceipt } from "../../src/agents/embedded-agent-runner/tool-send-receipts.js";
import { cleanupTempPluginTestEnvironment } from "../../src/agents/test-helpers/temp-plugin-extension-fixtures.js";
...
describe("buildEmbeddedExtensionFactories", () => {
  it("bridges middleware mutations with unique fallback tool call ids", async () => {
    ...
    registry.agentToolResultMiddlewares.push({
      ...
      runtimes: ["@gabrielvfonseca/operator"],
      source: "test",
    });
    ...
    expect(seenToolCallIds[0]).toMatch(/^operator-/);
    expect(seenToolCallIds[1]).toMatch(/^operator-/);
