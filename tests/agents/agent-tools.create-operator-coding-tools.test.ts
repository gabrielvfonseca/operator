...
} from "../../src/plugin-sdk/provider-tools.js";
import {
...
import { isPluginToolAllowed } from "../../src/plugins/tool-grant-allowlist.js";
...
import { stubTool } from "../../src/agents/test-helpers/fast-tool-stubs.js";
import { createHostSandboxFsBridge } from "../../src/agents/test-helpers/host-sandbox-fs-bridge.js";
import { buildEmptyExplicitToolAllowlistError } from "../../src/agents/tool-allowlist-guard.js";
import { DEFAULT_PLUGIN_TOOLS_ALLOWLIST_ENTRY, normalizeToolName } from "../../src/agents/tool-policy.js";
import { replaceWithEffectiveCronCreatorToolAllowlist } from "../../src/agents/tools/cron-tool.js";
import { getGatewayToolCallerIdentity } from "../../src/agents/tools/gateway-caller-context.js";
...
describe("createOperatorCodingTools", () => {
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:operator"], ["*"]]) {
      const tools = createOperatorCodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("operator-tools:test-helper")).toBeLessThan(
      stages.indexOf("operator-tools"),
    );
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from operator read tool.";
