...
import { FILE_LOCK_TIMEOUT_ERROR_CODE, type FileLockTimeoutError } from "../../../src/infra/file-lock.js";
import {
  buildRefreshContentionError,
  isGlobalRefreshLockTimeoutError,
} from "../../../src/agents/auth-profiles/oauth-refresh-lock-errors.js";
import { resolveAuthStorePath, resolveOAuthRefreshLockPath } from "../../../src/agents/auth-profiles/paths.js";
...
describe("OAuth refresh lock timeout classification", () => {
  it("matches only the global refresh lock path", () => {
    ...
    const authStoreLockPath = resolveAuthStorePath("/tmp/operator-oauth-lock-timeout/agent");
