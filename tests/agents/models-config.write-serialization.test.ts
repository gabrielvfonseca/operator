...
import { readGeneratedModelsJson } from "../../src/agents/models-config.test-utils.js";
import {
...
const planOperatorModelsJsonMock = vi.fn();
...
function planParamsAt(callIndex: number): {
  ...
  // Planner call shape is the contract between ensureOperatorModelsJson and planning.
  const call = planOperatorModelsJsonMock.mock.calls[callIndex];
  if (!call) {
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planOperatorModelsJson: (...args: unknown[]) => planOperatorModelsJsonMock(...args),
  }));
...
describe("models-config write serialization", () => {
  it("does not reuse default workspace plugin metadata for explicit agent dirs without workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureOperatorModelsJson({}, agentDir);
  ...
  it("reuses current plugin metadata for explicit agent dirs with matching workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureOperatorModelsJson({}, agentDir, { workspaceDir });
  ...
  it("writes plugin-owned model catalogs beside the agent plugin state", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planOperatorModelsJsonMock.mockImplementation(async () => ({
        action: "write",
  ...
  it("removes stale plugin-owned model catalogs", async () => {
    await withModelsTempHome(async (home) => {
      ...
      const result = await ensureOperatorModelsJson({}, agentDir);
  ...
  it("keeps generated plugin catalogs on non-authoritative skip plans", async () => {
    await withModelsTempHome(async (home) => {
      ...
      planOperatorModelsJsonMock.mockImplementation(async () => ({ action: "skip" }));
  ...
  it("does not reuse scoped startup discovery cache for a different provider scope", async () => {
    await withModelsTempHome(async (home) => {
      ...
      expect(planOperatorModelsJsonMock).toHaveBeenCalledTimes(2);
  ...
  it("keeps the ready cache warm after models.json is written", async () => {
    await withModelsTempHome(async () => {
      await ensureOperatorModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
      await ensureOperatorModelsJson(CUSTOM_PROXY_MODELS_CONFIG);

      expect(planOperatorModelsJsonMock).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it("keeps distinct config fingerprints cached without evicting each other", async () => {
    await withModelsTempHome(async () => {
      planOperatorModelsJsonMock.mockImplementation(async () => ({ action: "noop" }));
      ...
      await ensureOperatorModelsJson(first);
      await ensureOperatorModelsJson(second);
      await ensureOperatorModelsJson(first);
  ...
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      const writes = Promise.all([
        ensureOperatorModelsJson(first),
        ensureOperatorModelsJson(second),
      ]);
