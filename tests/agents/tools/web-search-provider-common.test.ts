...
describe("web_search shared cache", () => {
  it("keeps cache entries module-local instead of exposing them on a global symbol", async () => {
    ...
    vi.resetModules();
    delete (globalThis as Record<PropertyKey, unknown>)[Symbol.for("operator.web-search.cache")];
    ...
    expect(
      (globalThis as Record<PropertyKey, unknown>)[Symbol.for("operator.web-search.cache")],
    ).toBeUndefined();
  });
});
