...
} from "../../../src/plugin-sdk/session-visibility.js";
import { resolveSandboxedSessionToolContext } from "../../../src/agents/tools/sessions-access.js";
...
describe("resolveSessionToolsVisibility", () => {
  it("defaults to tree when unset or invalid", () => {
    expect(resolveSessionToolsVisibility({} as unknown as OperatorConfig)).toBe("tree");
    expect(
      resolveSessionToolsVisibility({
...
describe("sandbox session-tools context", () => {
  it("defaults sandbox visibility clamp to spawned", () => {
    expect(resolveSandboxSessionToolsVisibility({} as unknown as OperatorConfig)).toBe("spawned");
  });
...
describe("createAgentToAgentPolicy", () => {
  it("denies cross-agent access when disabled", () => {
    const policy = createAgentToAgentPolicy({} as unknown as OperatorConfig);
...
describe("createSessionVisibilityGuard", () => {
  it("allows cross-agent spawned child rows in list results with tree visibility", () => {
    const guard = createSessionVisibilityRowChecker({
      ...
      visibility: "tree",
      a2aPolicy: createAgentToAgentPolicy({} as unknown as OperatorConfig),
    });
