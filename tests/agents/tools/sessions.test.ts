...
import { clearRuntimeConfigSnapshot, setRuntimeConfigSnapshot } from "../../../src/config/io.js";
import { parseSessionThreadInfo } from "../../../src/config/sessions/thread-info.js";
import { createTestRegistry } from "../../../src/test-utils/channel-plugins.js";
import { withEnvAsync } from "../../../src/test-utils/env.js";
import { extractAssistantText, sanitizeTextContent } from "../../../src/agents/tools/chat-history-text.js";
...
async function withStubbedStateDir<T>(
  ...
  return await withEnvAsync({ OPERATOR_STATE_DIR: stateDir }, async () => await run(stateDir));
}
...
describe("sessions_list transcriptPath resolution", () => {
  ...
  it("resolves cross-agent transcript paths from agent defaults when gateway store path is relative", async () => {
    await withStubbedStateDir("operator-state-relative", async () => {
      callGatewayMock.mockResolvedValueOnce({
  ...
  it("resolves transcriptPath even when sessions.list does not return a store path", async () => {
    await withStubbedStateDir("operator-state-no-path", async () => {
      callGatewayMock.mockResolvedValueOnce({
        sessions: [
          {
            key: "agent:worker:main",
  ...
  it("falls back to agent defaults when gateway path is non-string", async () => {
    await withStubbedStateDir("operator-state-non-string-path", async () => {
      callGatewayMock.mockResolvedValueOnce({
  ...
  it("falls back to agent defaults when gateway path is '(multiple)'", async () => {
    await withStubbedStateDir("operator-state-multiple", async (stateDir) => {
      callGatewayMock.mockResolvedValueOnce({
        path: "(multiple)",
        sessions: [
          {
  ...
  it("resolves absolute {agentId} template paths per session agent", async () => {
    const templateStorePath = "/tmp/operator/agents/{agentId}/sessions/sessions.json";
