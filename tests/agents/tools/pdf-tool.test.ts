...
import * as pdfExtractModule from "../../../src/media/pdf-extract.js";
import * as webMedia from "../../../src/media/web-media.js";
import { withEnvAsync } from "../../../src/test-utils/env.js";
import * as modelDiscovery from "../../../src/agents/agent-model-discovery.js";
import type { AuthProfileStore } from "../../../src/agents/auth-profiles/types.js";
import * as modelAuth from "../../../src/agents/model-auth.js";
import * as modelsConfig from "../../../src/agents/models-config.js";
import * as pdfNativeProviders from "../../../src/agents/tools/pdf-native-providers.js";
import * as pdfModelConfigModule from "../../../src/agents/tools/pdf-tool.model-config.js";
import { resetPdfToolAuthEnv, withTempPdfAgentDir } from "../../../src/agents/tools/pdf-tool.test-support.js";
...
function withPdfModel(primary: string): OperatorConfig {
  return {
...
async function stubPdfToolInfra(
  ...
  vi.spyOn(modelsConfig, "ensureOperatorModelsJson").mockResolvedValue({
    agentDir,
    wrote: false,
  });
...
async function withManagedInboundPdf(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-pdf-managed-inbound-"));
...
describe("createPdfTool", () => {
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox pdf paths", async () => {
    await withTempPdfAgentDir(async (agentDir) => {
      const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-pdf-ws-"));
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-pdf-out-"));
      try {
        const cfg = withPdfModel(ANTHROPIC_PDF_MODEL);
        const tool = requirePdfTool(
  ...
  it("uses native PDF path without eager extraction", async () => {
    ...
    await withTempPdfAgentDir(async (agentDir) => {
      ...
      const ensureModelsJsonMock = vi.mocked(modelsConfig.ensureOperatorModelsJson);
