...
import { isInboundPathAllowed } from "@gabrielvfonseca/media-core/inbound-path-policy";
...
} from "../../../src/plugin-sdk/media-understanding.js";
import { withEnvAsync } from "../../../src/test-utils/env.js";
import { withFetchPreconnect } from "../../../src/test-utils/fetch-mock.js";
import type { AuthProfileCredential, AuthProfileStore } from "../../../src/agents/auth-profiles/types.js";
...
import { makeZeroUsageSnapshot } from "../../../src/agents/usage.js";
import { createImageTool } from "../../../src/agents/tools/image-tool.js";
import { testing, resolveImageModelConfigForTool } from "../../../src/agents/tools/image-tool.test-support.js";
import { resolveMediaToolInboundRoots } from "../../../src/agents/tools/media-tool-shared.js";
...
vi.mock("../auth-profiles.js", () => ({
  ...
  ensureAuthProfileStore: (agentDir?: string) => {
    ...
    if (process.env.OPERATOR_TEST_CODEX_CLI_OAUTH === "1") {
...
vi.mock("../auth-profiles/external-cli-sync.js", () => ({
  resolveExternalCliAuthProfiles: (
    ...
    if (
      process.env.OPERATOR_TEST_CODEX_CLI_OAUTH !== "1" ||
...
vi.mock("../model-auth.js", () => ({
  ...
  hasUsableCustomProviderApiKey: (cfg?: OperatorConfig, provider?: string) => {
...
async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-image-"));
  try {
...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(parentDir, "operator-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): OperatorConfig {
  return {
    agents: {
      defaults: {
...
async function withTempSandboxState(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-image-sandbox-"));
...
describe("image tool implicit imageModel config", () => {
  ...
  it("lets external CLI Codex OAuth survive the candidate auth filter", async () => {
    await withTempAgentDir(async (agentDir) => {
      vi.stubEnv("OPERATOR_TEST_CODEX_CLI_OAUTH", "1");
      installImageUnderstandingProviderStubs(minimaxProvider, moonshotProvider, codexMediaProvider);
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox image paths", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      ...
      await withTempAgentDir(async (agentDir) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-outside-"));
  ...
  it("still rejects non-workspace local image paths when workspaceOnly is disabled", async () => {
    ...
    await withTempAgentDir(async (agentDir) => {
      const cfg = createMinimaxImageConfig();
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-image-outside-"));
      const outsideImage = path.join(outsideDir, "secret.png");
      await fs.writeFile(outsideImage, Buffer.from(ONE_PIXEL_PNG_B64, "base64"));
      try {
        const tool = createRequiredImageTool({
  ...
  it("allows image paths from the current iMessage account attachment roots", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const attachmentRoot = await fs.mkdtemp(path.join(os.tmpdir(), "operator-imessage-root-"));
  ...
  it("allows image paths from current iMessage wildcard attachment roots", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const attachmentRootParent = await fs.mkdtemp(
        path.join(os.tmpdir(), "operator-imessage-wildcard-root-"),
      );
  ...
  it("rewrites inbound absolute paths into sandbox media/inbound", async () => {
    await withTempSandboxState(async ({ agentDir, sandboxRoot }) => {
      ...
      const res = await tool.execute("t1", {
        prompt: "Describe the image.",
        image: "@/Users/steipete/.operator/media/inbound/photo.png",
      });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  async function createMinimaxVlmFixture(baseResp: { status_code: number; status_msg: string }) {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg = createMinimaxImageConfig();
    const tool = createRequiredImageTool({ config: cfg, agentDir });
    return { fetch, tool };
...
describe("image tool managed inbound media", () => {
  ...
  async function withManagedInboundPng(
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-image-managed-inbound-"));
