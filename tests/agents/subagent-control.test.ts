...
} from "../../src/agents/subagent-control.test-support.js";
import {
...
beforeAll(() => {
  tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "operator-subagent-control-"));
});
...
function cfgWithSessionStore(storePath = nextSessionStorePath("sessions")): OperatorConfig {
  return {
