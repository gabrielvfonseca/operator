...
import { resolvePreferredSessionKeyForSessionIdMatches } from "../../src/sessions/session-id-resolution.js";
import type { TaskRecord } from "../../src/tasks/task-registry.types.js";
import { buildTaskStatusSnapshot } from "../../src/tasks/task-status.js";

const loadSessionStoreMock = vi.fn();
const updateSessionStoreMock = vi.fn();
const callGatewayMock = vi.fn();
const buildStatusMessageMock = vi.hoisted(() =>
  vi.fn((_params?: unknown) => "Operator\n🧠 Model: GPT-5.4"),
);
...
describe("session_status tool", () => {
  ...
  it("returns a status card for the current session", async () => {
    ...
    expect(details.statusText).toContain("Operator");
