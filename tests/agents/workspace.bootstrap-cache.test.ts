...
import { getOrLoadBootstrapFiles } from "../../src/agents/bootstrap-cache.js";
import { loadWorkspaceBootstrapFiles, DEFAULT_AGENTS_FILENAME } from "../../src/agents/workspace.js";

describe("workspace bootstrap file caching", () => {
  let workspaceDir: string;

  beforeEach(async () => {
    workspaceDir = await makeTempWorkspace("operator-bootstrap-cache-test-");
  });
  ...
  it("caches files independently by path", async () => {
    ...
    const workspace1 = await makeTempWorkspace("operator-cache-test1-");
    const workspace2 = await makeTempWorkspace("operator-cache-test2-");
