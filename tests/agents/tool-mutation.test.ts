...
} from "../../src/agents/tool-mutation.js";
...
describe("tool mutation helpers", () => {
  ...
  it.each([
    ...
    ["exec", "gh search prs --repo operator/operator tool-mutation --json number,title,state"],
    ["bash", "gh pr view 123 --repo operator/operator --json title,state"],
  ...
  it.each([
    ...
    ["exec", "gh api --method POST repos/operator/operator/issues"],
