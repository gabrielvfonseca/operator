...
} from "../../src/agents/agent-tools.read.js";
...
import { resolveReadOnlyWorkspaceSkillMounts } from "../../src/agents/sandbox/workspace-mounts.js";
import {
  expectReadWriteEditTools,
  expectReadWriteTools,
  getTextContent,
} from "../../src/agents/test-helpers/agent-tools-fs-helpers.js";
import { withUnsafeMountedSandboxHarness } from "../../src/agents/test-helpers/unsafe-mounted-sandbox.js";
  ...
  it("allows read-only materialized sandbox skills for sandbox reads only", async () => {
    await withUnsafeMountedSandboxHarness(
      async ({ sandbox, skillsWorkspaceDir }) => {
        ...
        const containerSkillPath = "/workspace/.operator/sandbox-skills/skills/demo/SKILL.md";
        ...
        const fileUrlReadResult = await readTool?.execute("t3", {
          path: "file:///workspace/.operator/sandbox-skills/skills/demo/SKILL.md",
        });
