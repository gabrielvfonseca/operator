...
import { attachRuntimeUserTurnTranscriptContext } from "../../src/sessions/user-turn-transcript-runtime-context.js";
import {
...
describe("guardSessionManager integration", () => {
  ...
  it("commits queued group sender metadata to JSONL and completes its recorder", () => {
    const dir = tempDirs.make("operator-queued-group-turn-");
  ...
  it("does not consume prepared user persistence for before-agent-run blocked messages", () => {
    ...
    appendMessage({
      ...
      timestamp: 124,
      __operator: { beforeAgentRunBlocked: { blockedBy: "test", blockedAt: 123 } },
    } as AgentMessage);
