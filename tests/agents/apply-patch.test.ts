...
} from "../../src/test-utils/symlink-rebind-race.js";
import { createApplyPatchTool } from "../../src/agents/apply-patch.js";
import { applyPatch } from "../../src/agents/apply-patch.test-support.js";
import type { SandboxFsBridge } from "../../src/agents/sandbox/fs-bridge.js";

async function withTempDir<T>(fn: (dir: string) => Promise<T>) {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-patch-"));
  try {
    return await fn(dir);
  } finally {
...
async function withWorkspaceTempDir<T>(fn: (dir: string) => Promise<T>) {
  const dir = await fs.mkdtemp(path.join(process.cwd(), "operator-patch-workspace-"));
  try {
    return await fn(dir);
  } finally {
    await fs.rm(dir, { recursive: true, force: true });
...
*** End Patch`;
  ...
  it("rejects absolute paths outside cwd by default", async () => {
    await withTempDir(async (dir) => {
      const escapedPath = path.join(os.tmpdir(), `operator-apply-patch-${Date.now()}.txt`);
...
*** End Patch`;
  ...
  it("allows deleting a symlink itself even if it points outside cwd", async () => {
    await withTempDir(async (dir) => {
      const outsideDir = await fs.mkdtemp(path.join(path.dirname(dir), "operator-patch-outside-"));
      try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (dir) => {
        ...
        const outside = await fs.mkdtemp(path.join(os.tmpdir(), "operator-patch-outside-"));
