...
import { SANDBOX_DOCKER_CREATE_ARGS_EPOCH } from "../../../src/agents/sandbox/constants.js";
import { collectDockerFlagValues } from "../../../src/agents/sandbox/test-args.js";
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-docker-mounts-"));
  tmpDirs.push(dir);
  return dir;
}
...
async function spawnDockerProcess(commandAndArgs: string[]) {
  ...
  if (command !== "docker") {
    ...
    args[0] === "inspect" &&
    args[1] === "-f" &&
    args[2]?.includes('index .Config.Labels "operator.configHash"')
  ) {
...
describe("ensureSandboxContainer config-hash recreation", () => {
  ...
  it("recreates shared container when array-order change alters hash", async () => {
    ...
    expect(createCall.args).toContain(`operator.configHash=${newHash}`);
  ...
  it("keeps a hot pre-init container running and emits the recreate hint", async () => {
    ...
    expect(runtimeMocks.log).toHaveBeenCalledWith(
      expect.stringContaining("Recreate to apply: operator sandbox recreate --all"),
    );
  ...
  it("applies custom binds after workspace mounts so overlapping binds can override", async () => {
    ...
    const createCall = await ensureSandboxCreateCallForTest({ cfg, workspaceDir });
    expect(createCall.args).toContain(`operator.configHash=${expectedHash}`);
  ...
  it("stamps the mount format version label on created containers", async () => {
    ...
    const createCall = await ensureSandboxCreateCallForTest({ cfg, workspaceDir });
    expect(createCall.args).toContain(
      `operator.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`,
    );
  });
});
