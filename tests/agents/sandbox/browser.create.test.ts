...
} from "../../../src/agents/sandbox/config-hash.js";
import { resolveSandboxBrowserDockerCreateConfig } from "../../../src/agents/sandbox/config.js";
import {
  SANDBOX_BROWSER_IMAGE_CONTRACT_EPOCH,
  SANDBOX_BROWSER_SECURITY_HASH_EPOCH,
  SANDBOX_DOCKER_CREATE_ARGS_EPOCH,
} from "../../../src/agents/sandbox/constants.js";
import { collectDockerFlagValues, findDockerArgsCall } from "../../../src/agents/sandbox/test-args.js";
import type { SandboxConfig } from "../../../src/agents/sandbox/types.js";
import { SANDBOX_MOUNT_FORMAT_VERSION } from "../../../src/agents/sandbox/workspace-mounts.js";
...
describe("ensureSandboxBrowser create args", () => {
  ...
  it("publishes noVNC on loopback and injects noVNC password env", async () => {
    ...
    expect(envEntries).toContain("OPERATOR_BROWSER_NO_SANDBOX=1");
    const passwordEntry = envEntries.find((entry) =>
      entry.startsWith("OPERATOR_BROWSER_NOVNC_PASSWORD="),
    );
    expect(passwordEntry).toMatch(/^OPERATOR_BROWSER_NOVNC_PASSWORD=[A-Za-z0-9]{8}$/);
  ...
  it("keeps a hot pre-init browser running and emits the recreate hint", async () => {
    ...
    expect(runtimeMocks.log).toHaveBeenCalledWith(
      expect.stringContaining(
        "Recreate to apply: operator sandbox recreate --browser --session session:test",
      ),
    );
  ...
  it("does not inject noVNC password env when noVNC is disabled", async () => {
    ...
    const envEntries = collectDockerFlagValues(createArgs ?? [], "-e");
    expect(
      envEntries.filter((entry) => entry.startsWith("OPERATOR_BROWSER_NOVNC_PASSWORD=")),
    ).toStrictEqual([]);
  ...
  it("requires auth for the sandbox CDP relay without auto-derived source ranges", async () => {
    ...
    const authEntry = envEntries.find((entry) =>
      entry.startsWith("OPERATOR_BROWSER_CDP_AUTH_TOKEN="),
    );
    expect(authEntry).toMatch(/^OPERATOR_BROWSER_CDP_AUTH_TOKEN=[0-9a-f]{48}$/);
    expect(envEntries).not.toContain("OPERATOR_BROWSER_CDP_SOURCE_RANGE=172.21.0.1/32");

    const token = requireValue(authEntry, "CDP auth env").slice(
      "OPERATOR_BROWSER_CDP_AUTH_TOKEN=".length,
    );
  ...
  it("passes explicit cdpSourceRange as an additional relay filter", async () => {
    ...
    const createArgs = findDockerArgsCall(dockerMocks.execDocker.mock.calls, "create");
    const envEntries = collectDockerFlagValues(createArgs ?? [], "-e");
    expect(envEntries).toContain("OPERATOR_BROWSER_CDP_SOURCE_RANGE=10.0.0.0/24");
  });
  ...
  it("does not inject a source range for network=none by default", async () => {
    ...
    const createArgs = requireDockerCreateArgs();
    const envEntries = collectDockerFlagValues(createArgs, "-e");
    expect(envEntries.some((entry) => entry.startsWith("OPERATOR_BROWSER_CDP_SOURCE_RANGE="))).toBe(
      false,
    );
  });
});
