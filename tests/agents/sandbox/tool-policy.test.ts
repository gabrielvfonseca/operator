...
} from "../../../src/agents/sandbox/runtime-status.js";
import { isToolAllowed, resolveSandboxToolPolicyForAgent } from "../../../src/agents/sandbox/tool-policy.js";
...
describe("sandbox/tool-policy", () => {
  ...
  it("keeps blocked-tool guidance glob-aware and shell-safe", () => {
    ...
    expect(message).toContain(
      "operator sandbox explain --session 'agent:main:weird session;rm -rf /'",
    );
  });
  ...
  it.each([
    ...
    ({ sessionKey, expectedLabel }) => {
      ...
      expect(message).toContain(`operator sandbox explain --session '${sessionKey}'`);
    },
  );
  ...
  it("avoids terminal injection for control-character session keys", () => {
    ...
    expect(message).toContain("operator sandbox explain --agent main");
