...
import { DEFAULT_AGENTS_FILENAME } from "../../../src/agents/workspace.js";
import { ensureSandboxWorkspace } from "../../../src/agents/sandbox/workspace.js";

const tempRoots: string[] = [];

async function makeTempRoot(): Promise<string> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "operator-sandbox-workspace-"));
  tempRoots.push(root);
  return root;
}
