...
} from "../../../src/agents/sandbox/fs-bridge-mutation-helper.js";
...
function runWritePlan(args: string[], input?: string) {
  ...
  return spawnSync("/bin/sh", ["-c", plan.script, "operator-sandbox-fs", ...(plan.args ?? [])], {
    input,
    encoding: "utf8",
    stdio: ["pipe", "pipe", "pipe"],
  });
...
describe("sandbox pinned mutation helper", () => {
  it("writes through a pinned directory fd", async () => {
    await withTempDir({ prefix: "operator-mutation-helper-" }, async (root) => {
