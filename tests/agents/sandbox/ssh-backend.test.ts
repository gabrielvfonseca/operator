...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "operator-test-ssh-config"),
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "operator-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "operator-ssh-worker-abcd1234",
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        browser: {
          enabled: false,
          image: "operator-browser",
          containerPrefix: "operator-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(skillsUploadParams.remoteDir).toContain("/workspace/.operator/sandbox-skills");
  ...
  it("clears stale remote materialized skills when the local copy is missing", async () => {
    const tmpDir = await createTempDir("operator-ssh-skills-");
    ...
    expect(commandParams.remoteCommand).toContain("operator-sandbox-clear");
    expect(commandParams.remoteCommand).toContain("/workspace/.operator/sandbox-skills");
    await backend.finalizeExec?.({
      status: "completed",
      exitCode: 0,
      timedOut: false,
