...
} from "../../../src/agents/sandbox/backend-handle.types.js";
...
async function runLocalShellCommand(
  ...
  return await new Promise<SandboxBackendCommandResult>((resolve, reject) => {
    const child = spawn(
      "sh",
      ["-c", params.script, "operator-sandbox-fs", ...(params.args ?? [])],
      {
...
describe("sandbox fs bridge local backend e2e", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-fsbridge-e2e-"));
