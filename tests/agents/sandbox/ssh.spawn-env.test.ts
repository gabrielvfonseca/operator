...
import { SANDBOX_COMMAND_MAX_BUFFER_BYTES } from "../../../src/agents/sandbox/constants.js";
...
describe("ssh subprocess env sanitization", () => {
  ...
  it("filters blocked secrets before spawning ssh uploads", async () => {
    ...
    const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-ssh-upload-env-"));
