...
} from "../../../src/agents/sandbox/ssh.js";
...
describe("sandbox ssh helpers", () => {
  it("materializes inline ssh auth data into a temp config", async () => {
    ...
    expect(config).toContain("Host operator-sandbox");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = await fs.mkdtemp(path.join(os.tmpdir(), "operator-ssh-real-"));
      ...
      const target = path.join(root, "workspace", ".operator", "sandbox-skills");
      ...
      await expect(
        fs.stat(path.join(realParent, "runtime", "workspace", ".operator", "sandbox-skills")),
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const realParent = makeTempDir(tempDirs, "operator-ssh-real-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = makeTempDir(tempDirs, "operator-ssh-workdir-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = makeTempDir(tempDirs, "operator-ssh-root-");
  ...
  it("builds remote workdir validation commands with quoted literal paths", () => {
    ...
    expect(command).toContain("operator-validate-workdir");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await fs.symlink(realParent, path.join(root, "workspace", ".operator"));
      ...
      await expect(
        execFileAsync("/bin/sh", [
          ...
          ENSURE_REMOTE_REAL_DIRECTORY_SCRIPT,
          "operator-remote-dir",
          path.join(root, "workspace", ".operator", "sandbox-skills"),
          root,
        ]),
      ).rejects.toThrow(/unsafe remote directory symlink/);
    },
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-ssh-upload-"));
      ...
      await expect(
        uploadDirectoryToSshTarget({
          session: {
            command: "ssh",
            configPath: "/tmp/operator-test-ssh-config",
            host: "operator-sandbox",
          },
          localDir,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-ssh-upload-safe-"));
      tempDirs.push(localDir);
