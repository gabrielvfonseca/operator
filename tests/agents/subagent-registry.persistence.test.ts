...
import { persistSubagentSessionTiming } from "../../src/agents/subagent-registry-helpers.js";
import { getSubagentRunsSnapshotForRead } from "../../src/agents/subagent-registry-state.js";
import {
  createSubagentRegistryTestDeps,
  readSubagentSessionStore,
  removeSubagentSessionEntry,
  writeSubagentSessionEntry,
} from "../../src/agents/subagent-registry.persistence.test-support.js";
import {
  loadSubagentRegistryFromSqlite,
  saveSubagentRegistryToSqlite,
} from "../../src/agents/subagent-registry.store.sqlite.js";
import {
  testing,
...
} from "../../src/agents/subagent-registry.test-helpers.js";
...
describe("subagent registry persistence", () => {
  ...
  const writePersistedRegistry = async (
    ...
    return path.join(tempStateDir, "state", "operator.sqlite");
  };
  ...
  it("reuses the persisted registry cache on hot internal read snapshots", async () => {
    ...
    const previousFlag = process.env.OPERATOR_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE;
    let cloneSpy: { mockRestore(): void } | undefined;
    try {
      process.env.OPERATOR_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE = "1";
      ...
      if (previousFlag === undefined) {
        delete process.env.OPERATOR_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE;
      } else {
        process.env.OPERATOR_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE = previousFlag;
      }
    }
  });
  ...
  it("prefers active runs and can resolve them from persisted registry snapshots", async () => {
    ...
    const resolved = withEnv({ OPERATOR_TEST_READ_SUBAGENT_RUNS_FROM_SQLITE: "1" }, () =>
