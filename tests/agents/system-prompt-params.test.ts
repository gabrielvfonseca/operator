...
import { setActiveNodeContext } from "../../src/infra/active-node-context.js";
import { buildSystemPromptParams } from "../../src/agents/system-prompt-params.js";

async function makeTempDir(label: string): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), `operator-${label}-`));
}
...
function buildParams(params: { config?: OperatorConfig; workspaceDir?: string; cwd?: string }) {
