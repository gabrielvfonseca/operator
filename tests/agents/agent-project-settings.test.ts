...
} from "../../src/agents/agent-project-settings-snapshot.js";
import { createPreparedEmbeddedAgentSettingsManager } from "../../src/agents/agent-project-settings.js";
...
describe("createPreparedEmbeddedAgentSettingsManager", () => {
  it("keeps trusted file-backed settings runtime-scoped after preparation", async () => {
    const baseDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-agent-settings-"));
    try {
      const cwd = path.join(baseDir, "workspace");
      const agentDir = path.join(baseDir, "agent");
      const projectSettingsDir = path.join(cwd, ".operator");
