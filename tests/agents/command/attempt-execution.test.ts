...
import { cliBackendLog } from "../../../src/agents/cli-runner/log.js";
import {
...
} from "../../../src/agents/command/attempt-execution.helpers.js";
import {
  claudeCliSessionTranscriptPath,
  formatClaudeCliFallbackPrelude,
} from "../../../src/agents/command/attempt-execution.helpers.test-support.js";
...
describe("buildClaudeCliFallbackContextPrelude", () => {
  ...
  it("returns empty string when the Claude session file does not exist", async () => {
    const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "operator-fallback-prelude-"));
    try {
      expect(
        buildClaudeCliFallbackContextPrelude({
  ...
  it("uses the canonical Claude project dir resolver", () => {
    expect(
      claudeCliSessionTranscriptPath({
        ...
        workspaceDir: "/home/faris/.operator/workspace",
        homeDir: "/home/faris",
      }),
    ).toBe(
      path.join(
