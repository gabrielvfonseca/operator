...
import { writeClaudeBundleManifest } from "../../../src/plugins/bundle-mcp.test-support.js";
import { prepareCliBundleMcpCaptureAttempt, prepareCliBundleMcpConfig } from "../../../src/agents/cli-runner/bundle-mcp.js";
import {
  cliBundleMcpHarness,
...
describe("prepareCliBundleMcpConfig", () => {
  ...
  it("serves only the exclusive config, ignoring user and plugin servers", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "operator-cli-bundle-mcp-exclusive-",
    );
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      config: { plugins: { enabled: false } },
      exclusiveConfig: {
        mcpServers: { operator: { command: "node", args: ["operator.mjs"] } },
      },
    });
    ...
    expect(raw.mcpServers?.operator?.args).toEqual(["operator.mjs"]);
  ...
  it("strips variadic Claude --mcp-config values and merges every listed config", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "operator-cli-bundle-mcp-variadic-",
    );
  ...
  it("merges and strips Claude --mcp-config equals form", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "operator-cli-bundle-mcp-equals-",
    );
  ...
  it("loads workspace bundle MCP plugins from the configured workspace root", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
      "operator-cli-bundle-mcp-workspace-root-",
    );
    const pluginRoot = path.join(workspaceDir, ".operator", "extensions", "workspace-probe");
  ...
  it("merges loopback overlay config with bundle MCP servers", async () => {
    ...
    expect(Object.keys(raw.mcpServers ?? {}).toSorted()).toEqual([
      "bundleProbe",
      "@gabrielvfonseca/operator",
    ]);
    expect(raw.mcpServers?.operator?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.operator?.headers?.Authorization).toBe("Bearer lb-tk-123");
    expect(raw.mcpServers?.operator?.headers?.["x-operator-cli-capture-key"]).toBe("");
    ...
    expect(attemptRaw.mcpServers?.operator?.headers?.Authorization).toBe("Bearer lb-tk-123");
    expect(attemptRaw.mcpServers?.operator?.headers?.["x-operator-cli-capture-key"]).toBe(
      "attempt-123",
    );
  ...
  it("leaves args untouched when bundle MCP is disabled", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/operator-bundle-mcp-disabled",
    });
