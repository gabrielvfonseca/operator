...
import { resolveAgentCredentialsForDiscovery } from "../../src/agents/agent-auth-discovery.js";
...
describe("resolveAgentCredentialsForDiscovery external CLI scoping", () => {
  ...
  it("threads scoped external CLI discovery into writable auth store loading", () => {
    ...
    resolveAgentCredentialsForDiscovery("/tmp/operator-agent", {
      config: cfg,
      env: {},
      externalCli,
    });

    expect(storeMocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/operator-agent", {
  ...
  it("preserves scoped external CLI discovery for read-only auth store loading", () => {
    ...
    expect(storeMocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/operator-agent", {
      allowKeychainPrompt: false,
      config: cfg,
      externalCli,
      readOnly: true,
