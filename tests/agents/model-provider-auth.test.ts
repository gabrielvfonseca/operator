...
} from "../../src/agents/model-auth-availability.js";
import type { ModelCatalogEntry } from "../../src/agents/model-catalog.types.js";
import { publishProviderAuthWarmSnapshot } from "../../src/agents/model-provider-auth-state.js";
...
describe("prepared provider auth state", () => {
  ...
  it("caches OpenAI auth by the complete route tuple", async () => {
    const hasAuth = createProviderAuthChecker({ cfg: {} as OperatorConfig });
    const platformRef = {
      modelId: "gpt-5.5",
      api: "openai-responses",
      baseUrl: "https://api.openai.com/v1",
  ...
  it("hasAuthForModelProvider uses the prepared answer for equivalent runtime config clones", async () => {
    const cfg = { gateway: { port: 18789 } } as OperatorConfig;
  ...
  it("terminates the off-main-thread warm worker when cancellation fires", async () => {
    ...
    try {
      const warmPromise = warmCurrentProviderAuthStateOffMainThread(
        { markerPath } as unknown as OperatorConfig,
        {
          isCancelled: () => cancelled,
          timeoutMs: 5_000,
