...
import { normalizeProviders } from "../../src/agents/models-config.providers.normalize.js";
import { resolveApiKeyFromProfiles } from "../../src/agents/models-config.providers.secret-helpers.js";
...
describe("normalizeProviders", () => {
  const createModel = (
    overrides: Partial<
      NonNullable<NonNullable<OperatorConfig["models"]>["providers"]>[string]["models"][number]
    > = {},
  ) => ({
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    } as unknown as NonNullable<NonNullable<OperatorConfig["models"]>["providers"]>;

    const sourceProviders: NonNullable<NonNullable<OperatorConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
