...
} from "../../src/agents/agent-tools.before-tool-call.state.js";
import type { MessagingToolSend } from "../../src/agents/embedded-agent-messaging.types.js";
import { buildEmbeddedRunPayloads } from "../../src/agents/embedded-agent-runner/run/payloads.js";
import {
  handleToolExecutionEnd,
  handleToolExecutionStart,
  handleToolExecutionUpdate,
} from "../../src/agents/embedded-agent-subscribe.handlers.tools.js";
import type {
  ToolCallSummary,
  ToolHandlerContext,
} from "../../src/agents/embedded-agent-subscribe.handlers.types.js";
...
describe("handleToolExecutionEnd cron mutation tracking", () => {
  ...
  it.each([
    ["exec", "operator cron add --at +1h --message 'follow up' --name reminder"],
    ["exec", "npx operator cron add --at=+1h --message 'follow up'"],
    ["exec", "bunx operator cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm exec operator cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm dlx operator cron add --at +1h --message 'follow up'"],
    ["exec", "npx -y operator cron add --at +1h --message 'follow up'"],
    ["exec", "bunx --bun operator cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm dlx operator@latest cron add --at +1h --message 'follow up'"],
    ["exec", "npx operator@latest cron add --at +1h --message 'follow up'"],
    ["exec", "bunx operator@latest cron add --at +1h --message 'follow up'"],
    ["exec", "/usr/local/bin/operator cron add --at +1h --message 'follow up'"],
    ["bash", "corepack pnpm exec operator cron add --at +1h --message 'follow up'"],
    ["exec", "env OPERATOR_PROFILE=test operator cron add --at +1h --message 'follow up'"],
    ["exec", "operator cron create --at +1h --message 'follow up'"],
    ["exec", "operator --profile work cron create --at +1h --message 'follow up'"],
    ["exec", "operator --dev cron add --at +1h --message 'follow up'"],
    ["exec", "operator --log-level debug --no-color cron add --at +1h --message 'follow up'"],
    ["exec", "operator --container helper cron add --at +1h --message 'follow up'"],
    ["exec", "operator cron add --at +1h --message 'follow up || wait'"],
    ["exec", "operator cron add --at +1h --message 'follow up' 2>&1"],
  ...
  it("does not increment successfulCronAdds when shell cron add fails", async () => {
    ...
    await handleToolExecutionStart(
      ctx as never,
      {
        ...
        args: {
          command: "operator cron add --at +1h --message 'follow up' --name reminder",
        },
      } as never,
    );
  ...
  it.each([
    ["operator cron list --json", "a different cron action"],
    ["echo operator cron add --at +1h", "a command that only mentions cron add"],
    ["operator cron add --at '+1h", "an unterminated shell argument"],
    ["cd /tmp && operator cron add --at +1h", "a compound command"],
    ["operator cron add --help", "the add command help"],
    ["operator cron create -h", "the create alias help"],
    ["operator cron add --bad||true", "a masked cron failure"],
    ["operator cron add --at +1h; true", "a semicolon suffix"],
    ["operator cron add --at +1h | cat", "a pipeline suffix"],
    ["operator cron add --at +1h & true", "a background suffix"],
    ["operator cron add --at +1h\ntrue", "a newline-separated suffix"],
    ["operator cron add --bad # ignored\ntrue", "a comment-masked cron failure"],
    ["npx -y echo operator cron add --at +1h", "a package runner for another executable"],
    ["pnpm operator cron add --at +1h", "a bare pnpm package script"],
    ["corepack pnpm operator cron add --at +1h", "a corepack pnpm package script"],
    ["operator@latest cron add --at +1h", "a package spec without a package runner"],
    ["pnpm exec operator@latest cron add --at +1h", "a package spec passed to pnpm exec"],
    ["operator cron add --bad &>/tmp/cron.log", "a bash-only combined redirection"],
    ["operator cron add --bad &>>/tmp/cron.log", "a bash-only append redirection"],
...
describe("handleToolExecutionEnd exec approval prompts", () => {
  ...
  it("emits a deterministic unavailable payload when the initiating surface cannot approve", async () => {
    ...
    expect(text).toContain(
      "Print the Control UI URL with `operator dashboard --no-open`, open it in a browser, then use the approval inbox.",
    );
    expect(text).toContain(
      "Inspect the node's effective exec policy with `operator approvals get --node node-mac-1`.",
    );
