...
import type { Api, Context, Model } from "@gabrielvfonseca/operator/plugin-sdk/llm";
import { describe, expect, it } from "vitest";
import { transformTransportMessages } from "../../src/agents/transport-message-transform.js";
...
describe("transformTransportMessages synthetic tool-result policy", () => {
  ...
  it.each([
    {
      ...
      targetApi: "operator-anthropic-messages-transport" as const,
  ...
  it("preserves real OpenAI transport results and aborts missing parallel siblings", () => {
    ...
    const result = transformTransportMessages(
      messages,
      makeModel("operator-openai-responses-transport" as Api, "openai", "gpt-5.4"),
    );
  ...
  it("still synthesizes missing tool results for transport alias apis that own replay repair", () => {
    ...
    const anthropicAlias = transformTransportMessages(
      messages,
      makeModel("operator-anthropic-messages-transport" as Api, "anthropic", "claude-opus-4-6"),
    );
    expect(anthropicAlias.map((msg) => msg.role)).toEqual(["assistant", "toolResult", "user"]);

    const googleAlias = transformTransportMessages(
      messages,
      makeModel("operator-google-generative-ai-transport" as Api, "google", "gemini-2.5-pro"),
    );
