...
import { isPrimaryBootstrapRun, resolveWorkspaceBootstrapRouting } from "../../src/agents/bootstrap-routing.js";
...
describe("resolveWorkspaceBootstrapRouting", () => {
  it("resolves bootstrap pending from the canonical workspace instead of a copied sandbox", async () => {
    ...
    // canonical workspace.
    const sandboxWorkspace = "/tmp/operator-sandbox-copy";
    const canonicalWorkspace = "/tmp/operator-canonical-workspace";
  ...
  it("falls back to limited bootstrap wording when a primary run cannot read files", async () => {
    const routing = await resolveWorkspaceBootstrapRouting({
      ...
      isPrimaryRun: true,
      isCanonicalWorkspace: true,
      effectiveWorkspace: "/tmp/operator-workspace",
      resolvedWorkspace: "/tmp/operator-workspace",
  ...
  it("treats hook-provided BOOTSTRAP.md content as pending bootstrap context", async () => {
    ...
    const routing = await resolveWorkspaceBootstrapRouting({
      ...
      bootstrapFiles: [
        {
          name: "BOOTSTRAP.md",
          path: "/tmp/operator-workspace/BOOTSTRAP.md",
