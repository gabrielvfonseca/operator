...
import { getBashShellConfig, getShellConfig } from "../../src/agents/shell-utils.js";
...
describe("exec shell snapshots", () => {
  ...
  it("leaves commands unchanged when trusted process env disables snapshots", async () => {
    ...
    expect(wrapped).toBe(command);
    expect(fs.existsSync(resolveShellSnapshotDirForTest({ OPERATOR_STATE_DIR: stateDir }))).toBe(
      false,
    );
  });
  ...
  it("does not honor per-call env for selecting the snapshot state dir", async () => {
    ...
    const wrapped = await maybeWrapCommandWithShellSnapshot({
      ...
      env: {
        ...
        OPERATOR_STATE_DIR: untrustedStateDir,
    ...
    expect(
      fs.existsSync(resolveShellSnapshotDirForTest({ OPERATOR_STATE_DIR: untrustedStateDir })),
    ).toBe(false);
    expect(
      fs.existsSync(resolveShellSnapshotDirForTest({ OPERATOR_STATE_DIR: trustedStateDir })),
    ).toBe(true);
  ...
  it("captures bash startup aliases, functions, and safe environment without secrets", async () => {
    ...
    fs.writeFileSync(
      ...
      [
        ...
        '[ "$OPERATOR_SHELL" = exec ] && alias oc_snap_exec_alias="printf marker-ok"',
