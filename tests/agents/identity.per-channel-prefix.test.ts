...
import { resolveResponsePrefix, resolveEffectiveMessagesConfig } from "../../src/agents/identity.js";

const makeConfig = <T extends OperatorConfig>(cfg: T) => cfg;
...
describe("resolveResponsePrefix with per-channel override", () => {
  ...
  describe("backward compatibility (no channel param)", () => {
    ...
    it("returns global prefix when set", () => {
      const cfg: OperatorConfig = { messages: { responsePrefix: "[Bot] " } };
    ...
    it("returns empty string when global prefix is explicitly empty", () => {
      const cfg: OperatorConfig = { messages: { responsePrefix: "" } };
