...
import { HEARTBEAT_RESPONSE_TOOL_NAME } from "../../src/auto-reply/heartbeat-tool-response.js";
import * as agentEvents from "../../src/infra/agent-events.js";
import { resetLogger, setLoggerOverride } from "../../src/logging/logger.js";
import { parseLogLine } from "../../src/logging/parse-log-line.js";
import {
  THINKING_TAG_CASES,
...
describe("subscribeEmbeddedAgentSession", () => {
  ...
  async function captureToolLifecycleLogSubsystems(messageChannel?: string): Promise<string[]> {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-tool-log-attribution-"));
    const logFile = path.join(tempDir, "operator.log");
    try {
      setLoggerOverride({
        level: "debug",
        consoleLevel: "silent",
  ...
  it.each([
    ...
    ["@gabrielvfonseca/operator", "agent/embedded"],
