...
import { resolveAgentHarnessPolicy } from "../../src/agents/harness/policy.js";
import { isModelKeyAllowedBySet } from "../../src/agents/model-selection-shared.js";
import {
...
import { createModelVisibilityPolicy } from "../../src/agents/model-visibility-policy.js";
...
function resolveAnthropicOpusThinking(cfg: OperatorConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: OperatorConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveAnthropicOpus48Thinking(cfg: OperatorConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveAnthropicVertexOpus48Thinking(cfg: OperatorConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic-vertex",
    model: "claude-opus-4-8",
...
function resolveClaudeCliOpus48Thinking(cfg: OperatorConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "claude-cli",
    model: "claude-opus-4-8",
...
function createProviderWithModelsConfig(provider: string, models: Array<Record<string, unknown>>) {
  ...
  } as Partial<OperatorConfig>;
}

function resolveConfiguredRefForTest(cfg: Partial<OperatorConfig>) {
