...
import { resolveToolSearchCodeDisplayTarget } from "../../src/agents/tool-display-common.js";
import {
  scanTopLevelChars,
  splitTopLevelPipes,
  splitTopLevelStages,
} from "../../src/agents/tool-display-exec-shell.js";
import { resolveExecDetail } from "../../src/agents/tool-display-exec.js";
import { formatToolDetail, formatToolSummary, resolveToolDisplay } from "../../src/agents/tool-display.js";
...
describe("tool display details", () => {
  ...
  it("formats web_search query with quotes", () => {
    const detail = formatToolDetail(
      resolveToolDisplay({
        name: "web_search",
        args: { query: "Operator docs", count: 3 },
      }),
    );

    expect(detail).toBe('for "Operator docs" (top 3)');
  });
  ...
  it("omits bash and exec names from compact tool summaries", () => {
    ...
    expect(
      formatToolSummary(
        resolveToolDisplay({
          name: "web_search",
          args: { query: "Operator docs" },
        }),
      ),
    ).toBe('🔎 Web Search: for "Operator docs"');
  });
