...
import type { runAgentAttempt } from "../../src/agents/command/attempt-execution.runtime.js";
import type { EmbeddedAgentRunResult } from "../../src/agents/embedded-agent.js";
import type { loadManifestModelCatalog } from "../../src/agents/model-catalog.js";
...
vi.mock("./agent-scope.js", async () => {
  ...
  return {
    ...
    resolveAgentDir: () => state.agentDir ?? "/tmp/operator-agent",
    resolveDefaultAgentId: () => "main",
    resolveEffectiveModelFallbacks: () => undefined,
    resolveSessionAgentId: () => "main",
    resolveAgentWorkspaceDir: () => state.workspaceDir ?? "/tmp/operator-workspace",
  };
});
...
beforeEach(async () => {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-rotation-e2e-"));
