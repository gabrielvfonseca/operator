...
import type { AgentRouteBinding } from "../../src/config/types.agents.js";
import { emitAgentEvent } from "../../src/infra/agent-events.js";
import { testing as bundleMcpRuntimeTesting } from "../../src/agents/agent-bundle-mcp-runtime.js";
...
} from "../../src/agents/operator-tools.subagents.sessions-spawn.test-harness.js";
import {
...
const fastModeEnv = vi.hoisted(() => {
  const previous = process.env.OPERATOR_TEST_FAST;
...
describe("operator-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(async () => {
  ...
  afterAll(() => {
    ...
    process.env.OPERATOR_TEST_FAST = fastModeEnv.previous;
  });
