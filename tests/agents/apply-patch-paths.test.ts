...
import { extractApplyPatchTargetPaths } from "../../src/agents/apply-patch-paths.js";
...
describe("extractApplyPatchTargetPaths", () => {
  ...
  it("can resolve paths with the same cwd semantics as apply_patch execution", () => {
    const cwd = path.join(os.tmpdir(), "operator-derived-paths");
    const patch = [
      "*** Begin Patch",
      "*** Add File: @src/../resolved.ts",
      "+x",
      "*** Update File: ~/renamed-source.ts",
      "*** Move to: /tmp/operator-target.ts",
      "@@",
      "+y",
      "*** End Patch",
    ].join("\n");
    expect(extractApplyPatchTargetPaths(patch, { cwd })).toEqual([
      ...
      path.join("/tmp", "operator-target.ts"),
    ]);
  });
