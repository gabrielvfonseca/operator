...
} from "../../src/agents/shell-utils.js";
...
function createTempCommandDir(
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-shell-"));
  tempDirs.push(dir);
...
describe("getShellConfig", () => {
  ...
  it("rejects a missing explicit custom shell path", () => {
    expect(() => getShellConfig(path.join(os.tmpdir(), "missing-operator-shell"))).toThrow(
...
describe("getBashShellConfig", () => {
  ...
  it("finds Git Bash under ProgramFiles", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "operator-git-bash-"));
...
describe("getShellEnv", () => {
  ...
  it("returns an env object with the Operator bin dir on PATH", () => {
    ...
    expect(env.PATH).toContain(".operator");
  });
});
...
describe("getShellConfig on Windows", () => {
  ...
  it("prefers PowerShell 7 in ProgramFiles", () => {
    const base = fs.mkdtempSync(path.join(os.tmpdir(), "operator-pfiles-"));
    tempDirs.push(base);
  ...
  it("prefers ProgramW6432 PowerShell 7 when ProgramFiles lacks pwsh", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "operator-pfiles-"));
    const programW6432 = fs.mkdtempSync(path.join(os.tmpdir(), "operator-pw6432-"));
  ...
  it("finds pwsh on PATH when not in standard install locations", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "operator-pfiles-"));
    const binDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-bin-"));
  ...
  it("falls back to Windows PowerShell 5.1 path when pwsh is unavailable", () => {
    const programFiles = fs.mkdtempSync(path.join(os.tmpdir(), "operator-pfiles-"));
    const sysRoot = fs.mkdtempSync(path.join(os.tmpdir(), "operator-sysroot-"));
