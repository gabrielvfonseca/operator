...
import type { ExecuteNodeHostCommandParams } from "../../src/agents/bash-tools.exec-host-node.types.js";
...
const CHANNEL_CONTEXT_ENV_KEY = "OPERATOR_CHANNEL_CONTEXT";
...
describe("exec resolve_exec_env hook wiring", () => {
  ...
  it("merges filtered plugin env into gateway execution and approval-visible requested env", async () => {
    ...
    expect(mocks.gatewayParams[0]?.env.OPERATOR_CLI).toBe(OPERATOR_CLI_ENV_VALUE);
  ...
  it("lets lazy before_tool_call see invalid workdirs before failing unchanged params", async () => {
    ...
    const exec = createOperatorCodingTools({
      agentId: "main",
      sessionKey: "agent:main:telegram:chat-1",
      cwd: process.cwd(),
