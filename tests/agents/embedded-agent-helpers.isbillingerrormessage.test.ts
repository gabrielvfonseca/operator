...
import { isCloudflareOrHtmlErrorPage } from "../../src/shared/assistant-error-format.js";
import {
  ...
  parseImageSizeError,
} from "../../src/agents/embedded-agent-helpers.js";
import { isAuthPermanentErrorMessage } from "../../src/agents/embedded-agent-helpers/failover-matches.js";
...
describe("classifyFailoverReason provider messages", () => {
  ...
  it("classifies internal and compatibility error messages", () => {
    ...
    expect(
      classifyFailoverReason(
        'No API key found for provider "openai". Auth store: /tmp/operator-agent-abc/auth-profiles.json (agentDir: /tmp/operator-agent-abc).',
      ),
    ).toBe("auth");
...
describe("classifyProviderRuntimeFailureKind", () => {
  ...
  it("classifies OAuth refresh timeouts and lock contention distinctly", () => {
    ...
    expect(
      classifyProviderRuntimeFailureKind("file lock timeout for /tmp/operator-oauth-refresh.lock"),
