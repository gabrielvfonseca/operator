...
import { classifyFailoverSignal } from "../../src/agents/embedded-agent-helpers/errors.js";
import {
...
import { SessionWriteLockTimeoutError } from "../../src/agents/session-write-lock-error.js";
...
// https://github.com/operator/operator/issues/33785
...
describe("failover-error", () => {
  ...
  it("403 OpenRouter 'Key limit exceeded' returns billing (model fallback trigger)", () => {
    // GitHub: operator/operator#53849 — OpenRouter returns 403 with "Key limit exceeded"
  ...
  describe("isNonProviderRuntimeCoordinationError", () => {
    ...
    const makeEmbeddedTakeoverError = () => {
      const err = new Error(
        "session file changed while embedded prompt lock was released: /tmp/operator/session.jsonl",
      );
...
describe("buildFailoverRemediationHint", () => {
  it("returns a copy-pasteable login command for auth failures", () => {
    ...
    expect(buildFailoverRemediationHint(err)).toBe(
      "Re-authenticate with: operator models auth login --provider 'anthropic' --force",
    );
  });
  ...
  it("returns a hint for auth_permanent as well", () => {
    ...
    expect(buildFailoverRemediationHint(err)).toBe(
      "Re-authenticate with: operator models auth login --provider 'google-gemini-cli' --force",
    );
  });

  it("quotes provider ids that contain shell metacharacters", () => {
    expect(buildProviderReauthCommand("custom;touch /tmp/pwned")).toBe(
      "operator models auth login --provider 'custom;touch /tmp/pwned' --force",
    );
    expect(buildProviderReauthCommand("custom'provider")).toBe(
      "operator models auth login --provider 'custom'\\''provider' --force",
    );
  });
  ...
  it("wraps rendered provider commands in the standard CLI formatter", () => {
    expect(buildProviderReauthCommand("anthropic", { OPERATOR_PROFILE: "work" })).toBe(
      "operator --profile work models auth login --provider 'anthropic' --force",
    );
    expect(buildProviderReauthCommand("anthropic", { OPERATOR_CONTAINER_HINT: "dev" })).toBe(
      "operator --container dev models auth login --provider 'anthropic' --force",
    );
  });
