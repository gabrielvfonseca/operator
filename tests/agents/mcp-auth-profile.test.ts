...
import { resolveMcpBearerBundleConfig, withMcpAuthProfileBearer } from "../../src/agents/mcp-auth-profile.js";
...
describe("mcp auth profile bearer projection", () => {
  ...
  it("projects existing MCP-native OAuth credentials without an auth profile", async () => {
    ...
    expect(resolved.config.mcpServers.docs).toMatchObject({
      url: "https://mcp.example.com/mcp",
      headers: {
        Authorization: expect.stringMatching(/^Bearer \$\{OPERATOR_MCP_AUTH_[A-F0-9]{12}_TOKEN}$/),
      },
    });
