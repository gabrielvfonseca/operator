...
import { resolveImageSanitizationLimits } from "../../src/agents/image-sanitization.js";
...
describe("image sanitization config", () => {
  it("defaults when no config value exists", () => {
    ...
    expect(
      resolveImageSanitizationLimits({ agents: { defaults: {} } } as unknown as OperatorConfig),
    ).toStrictEqual({});
  });
