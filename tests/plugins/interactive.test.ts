...
import * as conversationBinding from "../../src/plugins/conversation-binding.js";
import { createInteractiveConversationBindingHelpers } from "../../src/plugins/interactive-binding-helpers.js";
import type {
...
} from "../../src/plugins/interactive-contract.test-helpers.js";
import { registerRegistryPluginInteractiveHandler } from "../../src/plugins/interactive-registry.js";
import {
...
type InteractiveDispatchParams =
  | {
      ...
      afterInvoke?: (result: { handled?: boolean } | undefined) => Promise<void> | void;
      ctx: Omit<
        TelegramInteractiveHandlerContext,
        | "callback"
        | "respond"
...
describe("plugin interactive handlers", () => {
  ...
  it("hydrates legacy interactive state shapes before clearing handlers", async () => {
    ...
    const stateKey = Symbol.for("operator.pluginInteractiveState");
  ...
  it("resolves active registry handlers without retaining them after retirement", async () => {
    ...
    registry.plugins.push({
      id: "operator-code-agent",
      name: "Operator Code Agent",
      status: "loaded",
    } as never);
    registry.interactiveHandlers = [
      {
        ...
        pluginId: "operator-code-agent",
        pluginName: "Operator Code Agent",
        pluginRoot: "/plugins/operator-code-agent",
        handler: handler as never,
      },
    ];
    expect(
