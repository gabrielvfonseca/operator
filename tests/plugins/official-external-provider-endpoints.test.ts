...
import { parseJsonWithJson5Fallback } from "../../src/utils/parse-json-compat.js";
import { listOfficialExternalProviderEndpointManifests } from "../../src/plugins/official-external-provider-endpoints.js";
...
function listExtensionManifests(): ExtensionManifestRecord[] {
  ...
  for (const entry of fs.readdirSync(extensionsDir, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(extensionsDir, entry.name, "operator.plugin.json");
...
describe("official external provider endpoint catalog mirror", () => {
  ...
  it("mirrors providerEndpoints for every dist-excluded plugin manifest that declares them", () => {
    ...
    for (const { dirName, manifest } of extensionManifests) {
      ...
      expect(
        ...
        `catalog providerEndpoints for plugin "${pluginId}" must mirror extensions/${dirName}/operator.plugin.json`,
  ...
  it("keeps catalog providerEndpoints in sync with local plugin manifests", () => {
    ...
    for (const [pluginId, catalogManifest] of catalogManifestsByPluginId) {
      ...
      expect(
        catalogManifest.providerEndpoints,
        `catalog providerEndpoints for plugin "${pluginId}" must mirror extensions/${local.dirName}/operator.plugin.json`,
