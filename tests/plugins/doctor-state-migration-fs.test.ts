...
import { archiveLegacyStateSource } from "../../src/plugins/doctor-state-migration-fs.js";

describe("archiveLegacyStateSource", () => {
  let dir: string;

  beforeEach(async () => {
    dir = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "operator-doctor-fs-")));
  });
