...
import { collectBundledChannelConfigs } from "../../src/plugins/bundled-channel-config-metadata.js";
import {
  listBundledPluginMetadata,
  resolveBundledPluginGeneratedPath,
  resolveBundledPluginRepoEntryPath,
} from "../../src/plugins/bundled-plugin-metadata.js";

type BundledPluginMetadata = ReturnType<typeof listBundledPluginMetadata>[number];
import { resolveGatewayStartupPluginIdsFromRegistry } from "../../src/plugins/gateway-startup-plugin-ids.js";
import {
  createGeneratedPluginTempRoot,
  installGeneratedPluginTempRootCleanup,
  pluginTestRepoRoot as repoRoot,
  writeJson,
} from "../../src/plugins/generated-plugin-test-helpers.js";
import type { InstalledPluginIndex, InstalledPluginIndexRecord } from "../../src/plugins/installed-plugin-index.js";
import type { PluginManifestRecord, PluginManifestRegistry } from "../../src/plugins/manifest-registry.js";
import {
...
function listGitRepoBundledPluginManifestFiles(): string[] | null {
  return listGitTrackedFiles({ repoRoot, pathspecs: "extensions/*/operator.plugin.json" });
}
...
function createRepoBundledManifestRegistry(): PluginManifestRegistry {
  return {
    plugins: listRepoBundledPluginManifests().map(({ manifest, dirName }) => ({
      ...
      manifestPath: path.join(repoRoot, "extensions", dirName, "operator.plugin.json"),
...
describe("bundled plugin metadata", () => {
  ...
  it("prefers built generated paths when present and falls back to source paths", () => {
    const tempRoot = createGeneratedPluginTempRoot("operator-bundled-plugin-metadata-");
  ...
  it("uses dist-runtime generated paths before source fallback when packaged dist is absent", () => {
    const tempRoot = createGeneratedPluginTempRoot("operator-bundled-plugin-runtime-metadata-");
    const pluginRoot = path.join(tempRoot, "extensions", "plugin");
  ...
  it("resolves plugin-local generated entry paths when the plugin dir is provided", () => {
    const tempRoot = createGeneratedPluginTempRoot("operator-bundled-plugin-metadata-local-");
  ...
  it("keeps generated entry path resolution inside bundled plugin roots", () => {
    const tempRoot = createGeneratedPluginTempRoot("operator-bundled-plugin-path-contained-");
  ...
  it("scans direct plugin-tree overrides and resolves generated paths from that scan dir", () => {
    const tempRoot = createGeneratedPluginTempRoot("operator-bundled-plugin-direct-tree-");
    const pluginsDir = path.join(tempRoot, "bundled-plugins");
    const pluginRoot = path.join(pluginsDir, "alpha");

    writeJson(path.join(pluginRoot, "package.json"), {
      name: "@gabrielvfonseca/alpha",
      version: "0.0.1",
      operator: {
        extensions: ["./index.ts"],
      },
  ...
  it("reflects bundled manifest edits on the next metadata read", () => {
    const tempRoot = createGeneratedPluginTempRoot("operator-bundled-plugin-fresh-");
    const pluginRoot = path.join(tempRoot, "extensions", "alpha");
  ...
  it("prefers direct scan-dir overrides over nested dist artifacts within the same override root", () => {
    const pluginsDir = createGeneratedPluginTempRoot("operator-bundled-plugin-direct-priority-");
    const pluginRoot = path.join(pluginsDir, "alpha");
  ...
  it("resolves bundled repo entry paths from dist before workspace source", () => {
    const tempRoot = createGeneratedPluginTempRoot("operator-bundled-plugin-repo-entry-");
    const pluginRoot = path.join(tempRoot, "extensions", "alpha");
    const distPluginRoot = path.join(tempRoot, "dist", "extensions", "alpha");
  ...
  it("keeps bundled repo entry path resolution inside the plugin directory", () => {
    const tempRoot = createGeneratedPluginTempRoot("operator-bundled-plugin-repo-contained-");
    const pluginRoot = path.join(tempRoot, "extensions", "alpha");
  ...
  it("merges runtime channel schema metadata with manifest-owned channel config fields", () => {
    const tempRoot = createGeneratedPluginTempRoot("operator-bundled-plugin-channel-configs-");
    ...
    writeJson(path.join(tempRoot, "extensions", "alpha", "operator.plugin.json"), {
      id: "alpha",
      channels: ["alpha"],
      configSchema: { type: "object" },
      channelConfigs: {
  ...
  it("captures top-level public surface artifacts without duplicating the primary entrypoints", () => {
    const tempRoot = createGeneratedPluginTempRoot("operator-bundled-plugin-public-artifacts-");
  ...
  it("loads channel config metadata from built public surfaces in dist-only roots", () => {
    const tempRoot = createGeneratedPluginTempRoot("operator-bundled-plugin-dist-config-");
    ...
    writeJson(path.join(distRoot, "extensions", "alpha", "operator.plugin.json"), {
      id: "alpha",
      configSchema: {
        type: "object",
        properties: {},
  ...
  it("does not probe broad runtime public surfaces for channel config metadata", () => {
    const tempRoot = createGeneratedPluginTempRoot("operator-bundled-plugin-dist-config-runtime-");
    const distRoot = path.join(tempRoot, "dist");
