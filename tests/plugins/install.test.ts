...
import * as installSecurityScan from "../../src/plugins/install-security-scan.js";
import {
...
import { markRetainedManagedNpmInstall } from "../../src/plugins/managed-npm-retention.js";
import { packToArchive } from "../../src/plugins/test-helpers/archive-fixtures.js";
...
} from "../../src/plugins/test-helpers/install-fixtures.js";
...
const suiteTempRootTracker = createSuiteTempRootTracker("operator-plugin-install");
...
process.stdin.on("end", () => {
  fs.appendFileSync(process.env.OPERATOR_POLICY_LOG, input + "\\n");
...
function configWithInstallPolicy(scriptPath: string, logPath: string): OperatorConfig {
  return {
    security: {
      installPolicy: {
        enabled: true,
        exec: {
          ...
          args: [scriptPath],
          env: { OPERATOR_POLICY_LOG: logPath },
...
async function ensureDynamicArchiveTemplate(params: {
  ...
  if (params.writePluginManifest !== false) {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "operator.plugin.json"),
      JSON.stringify({
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@gabrielvfonseca/test-plugin",
      version: "0.0.1",
      operator: { extensions: ["./dist/index.js"] },
      dependencies: { "left-pad": "1.3.0" },
    }),
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "operator.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs package archive runtime dependencies", async () => {
    ...
    expect(commandOptions.cwd).toContain(".operator-install-stage-");
  });
  ...
  it("rejects native plugin zip archives without operator.plugin.json", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package missing valid operator.plugin.json");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_PLUGIN_MANIFEST);
    }
    expect(fs.existsSync(resolvePluginInstallDir("@gabrielvfonseca/zipper", extensionsDir))).toBe(
      false,
    );
  });
  ...
  it("rejects packages without operator.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@gabrielvfonseca/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("operator.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_OPERATOR_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when operator.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@gabrielvfonseca/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing operator.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_OPERATOR_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without operator.extensions");
  });

  it("rejects package installs when operator.extensions entries escape the package", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_OPERATOR_EXTENSIONS);
  ...
  it("allows missing TypeScript source entries when an inferred built runtime entry exists", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        operator: { extensions: ["./src/index.ts"] },
      }),
    );
    fs.writeFileSync(path.join(pluginDir, "dist", "index.js"), "export {};\n");
  ...
  it("rejects package installs when operator.extensions contains a blank entry", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.mkdirSync(path.join(pluginDir, "dist"), { recursive: true });
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_OPERATOR_EXTENSIONS);
      expect(result.error).toContain("operator.extensions[1]");
  ...
  it("rejects package installs when runtimeExtensions contains a blank entry", async () => {
    ...
    if (!result.ok) {
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.INVALID_OPERATOR_EXTENSIONS);
      expect(result.error).toContain("operator.runtimeExtensions[0]");
      expect(result.error).toContain("non-empty string");
    }
  });
  ...
  it("rejects package installs when an extension entry is a symlink escape", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        operator: { extensions: ["./linked/escape.js"] },
      }),
    );
  ...
  it("allows declared package entrypoints with dangerous code under test-looking paths", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        operator: { extensions: ["tests/runtime.test.js"] },
      }),
    );
    fs.mkdirSync(path.join(pluginDir, "tests"), { recursive: true });
    fs.writeFileSync(
...
describe("installPluginFromDir", () => {
  ...
  it("emits a redacted security event after installing a plugin directory", async () => {
    ...
    expect(captured.events[0]).toMatchObject({
      ...
      severity: "medium",
      actor: { kind: "operator" },
      target: { kind: "plugin", name: "@gabrielvfonseca/test-plugin" },
  ...
  it("preserves local package manifests without dependency surgery", async () => {
    ...
    expect(manifest.devDependencies?.operator).toBe("workspace:*");
  ...
  it.each([
    {
      ...
      expectedMessageIncludes: ["requires Operator >=2026.3.22, but this host is 2026.3.21"],
    },
    {
      name: "rejects plugins with invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedCode: PLUGIN_INSTALL_ERROR_CODE.INVALID_MIN_HOST_VERSION,
      expectedMessageIncludes: ["invalid package.json operator.install.minHostVersion"],
    },
    {
  ...
  it("rejects plugins whose package plugin API metadata is malformed", async () => {
    ...
    expectFailedInstallResult({
      ...
      messageIncludes: ["operator.compat.pluginApi", "must be a string"],
    });
    expect(vi.mocked(runCommandWithTimeout)).not.toHaveBeenCalled();
  });
  ...
  it("checks package plugin API before current-host extension shape validation", async () => {
    ...
    if (!result.ok) {
      expect(result.error).not.toContain("operator.extensions");
    }
    expect(vi.mocked(runCommandWithTimeout)).not.toHaveBeenCalled();
  });
  ...
  it("rejects bundle package installs whose package plugin API range is newer than the current host", async () => {
    ...
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@gabrielvfonseca/future-bundle",
        version: "2026.5.27",
        operator: { compat: { pluginApi: ">=2026.5.27" } },
      }),
      "utf-8",
    );
  ...
  it("allows plugins when a beta host is on the package plugin API floor", async () => {
    ...
    expect(result.pluginId).toBe("@gabrielvfonseca/test-plugin");
  });

  it("uses operator.plugin.json id as install key when it differs from package name", async () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedPluginId: "@gabrielvfonseca/test-plugin",
      install: (pluginDir: string, extensionsDir: string) =>
        installPluginFromDir({
          dirPath: pluginDir,
          extensionsDir,
...
describe("linkOperatorPeerDependencies (via installPluginFromDir)", () => {
  const resolveRootMock = vi.mocked(resolveOperatorPackageRootSync);
  ...
  it("creates a node_modules/openclaw symlink when peerDependencies declares openclaw", async () => {
    ...
    writePluginWithPeerDeps(pluginDir, { operator: "*" });
    ...
    const symlinkPath = path.join(result.targetDir, "node_modules", "@gabrielvfonseca/operator");
  ...
  it("keeps the openclaw peer symlink when a local plugin already has dependencies", async () => {
    ...
    writePluginWithPeerDeps(pluginDir, { operator: "*" }, { "is-number": "7.0.0" });
  ...
  it("replaces a copied local openclaw package with the host peer symlink", async () => {
    ...
    writePluginWithPeerDeps(pluginDir, { operator: "*" });
    fs.mkdirSync(path.join(pluginDir, "node_modules", "@gabrielvfonseca/operator"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(pluginDir, "node_modules", "@gabrielvfonseca/operator", "package.json"),
      JSON.stringify({ name: "@gabrielvfonseca/operator", version: "2026.5.31" }),
      "utf-8",
    );
  ...
  it("does not create a symlink when peerDependencies is empty", async () => {
    ...
    const symlinkPath = path.join(nodeModulesDir, "@gabrielvfonseca/operator");
  ...
  it("is idempotent - re-installing replaces an existing symlink without error", async () => {
    ...
    const symlinkPath = path.join(second.targetDir, "node_modules", "@gabrielvfonseca/operator");
    expect(fs.lstatSync(symlinkPath).isSymbolicLink()).toBe(true);
  });

  it("rejects when resolveOperatorPackageRootSync returns null", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
