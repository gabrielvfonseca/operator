...
} from "../../src/plugins/plugin-cache-primitives.js";
...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as OperatorConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as OperatorConfig;
    const load = vi.fn(async (config?: OperatorConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: OperatorConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
