...
import type { OperatorPluginChannelRegistration } from "../../src/plugins/types.js";
...
describe("plugin registry channel guard", () => {
  ...
  it("rejects disabled workspace registration before reading channel data", () => {
    ...
    const registration = {} as OperatorPluginChannelRegistration;
  ...
  it.each(["bundled", "global", "workspace", "config"] as const)(
    ...
    (origin) => {
      ...
      pluginRegistry.registry.plugins.push(record);
      pluginRegistry
        .createApi(record, { config: {} as OperatorConfig, registrationMode: "full" })
        .registerChannel({
