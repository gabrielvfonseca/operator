...
import type { EmbeddingProviderCreateOptions } from "../../src/plugins/embedding-providers.js";
import { getRegisteredEmbeddingProvider } from "../../src/plugins/embedding-providers.js";
import { openAICompatibleEmbeddingProviderAdapter } from "../../src/plugins/openai-compatible-embedding-provider.js";
...
describe("openai-compatible generic embedding provider", () => {
  ...
  it("resolves env SecretRef API keys on the memory search secret surface", async () => {
    ...
    const envVar = "OPERATOR_TEST_OPENAI_COMPATIBLE_EMBEDDING_API_KEY";
  ...
  it("enforces configured env SecretRef allowlists for API keys", async () => {
    const envVar = "OPERATOR_TEST_OPENAI_COMPATIBLE_BLOCKED_API_KEY";
    const server = await startEmbeddingServer();
        ...
        createOpenAICompatibleEmbeddingProvider(
          createOptions({
            config: {
              secrets: {
                providers: {
                  default: { source: "env", allowlist: ["OPERATOR_ALLOWED_ONLY"] },
                },
              },
            } as EmbeddingProviderCreateOptions["config"],
            model: "text-embedding-bge-m3",
  ...
  it("enforces configured env SecretRef allowlists for custom headers", async () => {
    const envVar = "OPERATOR_TEST_OPENAI_COMPATIBLE_BLOCKED_HEADER";
    const server = await startEmbeddingServer();
  ...
  it("resolves env-template API key strings before treating them as inline secrets", async () => {
    ...
    const envVar = "OPERATOR_TEST_OPENAI_COMPATIBLE_EMBEDDING_TEMPLATE_KEY";
    const server = await startEmbeddingServer({ token });
  ...
  it("does not treat missing env-template API key strings as inline secrets", async () => {
    const envVar = "OPERATOR_TEST_OPENAI_COMPATIBLE_EMBEDDING_MISSING_TEMPLATE_KEY";
    const server = await startEmbeddingServer();
