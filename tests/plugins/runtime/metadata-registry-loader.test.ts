...
import type { PluginLoadOptions } from "../../../src/plugins/loader.js";
...
vi.mock("../loader.js", () => ({
  loadOperatorPlugins: (...args: unknown[]) => loadOperatorPluginsMock(...args),
}));
...
function getOnlyLoadOperatorPluginsOptions(): PluginLoadOptions {
  expect(loadOperatorPluginsMock).toHaveBeenCalledTimes(1);
  const options = loadOperatorPluginsMock.mock.calls[0]?.[0];
  if (!options || typeof options !== "object") {
    throw new Error("expected loadOperatorPlugins to receive plugin load options");
  }
...
describe("loadPluginMetadataRegistrySnapshot", () => {
  ...
  beforeEach(() => {
    ...
    loadOperatorPluginsMock.mockReturnValue({ plugins: [], diagnostics: [] });
  });
  ...
  it("defaults to a non-activating validate snapshot", () => {
    loadPluginMetadataRegistrySnapshot({
      ...
      env: { HOME: "/tmp/operator-home" } as NodeJS.ProcessEnv,
      workspaceDir: "/workspace",
      onlyPluginIds: ["demo"],
    });

    const loadOptions = getOnlyLoadOperatorPluginsOptions();
    expect(loadOptions).toMatchObject({
      config: { plugins: {} },
      activationSourceConfig: { plugins: { allow: ["demo"] } },
      autoEnabledReasons: {},
      workspaceDir: "/workspace",
      env: { HOME: "/tmp/operator-home" },
  ...
  it("forwards an explicit logger through metadata snapshots", () => {
    ...
    expect(getOnlyLoadOperatorPluginsOptions()).toMatchObject({
      config: { plugins: {} },
      activationSourceConfig: { plugins: {} },
      autoEnabledReasons: {},
      workspaceDir: "/workspace",
  ...
  it("honors explicit load options when reusing a resolved runtime context", () => {
    ...
    expect(getOnlyLoadOperatorPluginsOptions()).toEqual({
      config: { plugins: { allow: ["compat-provider"] } },
      activationSourceConfig: { plugins: { allow: ["raw-plugin"] } },
      autoEnabledReasons: {},
      workspaceDir: "/compat-workspace",
