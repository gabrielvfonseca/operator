...
import { loadEnabledBundleLspConfig } from "../../src/plugins/bundle-lsp.js";
import { loadEnabledBundleMcpConfig } from "../../src/plugins/bundle-mcp.js";
import {
...
} from "../../src/plugins/bundle-mcp.test-support.js";
...
describe("loadEnabledBundleMcpConfig", () => {
  ...
  it("uses a provided manifest registry instead of rediscovering bundle plugins", async () => {
    const homeDir = await tempHarness.createTempDir("operator-bundle-mcp-home-");
    const workspaceDir = await tempHarness.createTempDir("operator-bundle-mcp-workspace-");
  ...
  it("resolves inline Claude MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    await withBundleHomeEnv(
      tempHarness,
      "operator-bundle-inline-placeholder",
      async ({ homeDir, workspaceDir }) => {
