...
import type { PluginKind } from "../../src/plugins/plugin-kind.types.js";
import {
...
describe("applyExclusiveSlotSelection", () => {
  const createMemoryConfig = (plugins?: OperatorConfig["plugins"]): OperatorConfig => ({
    plugins: {
      ...plugins,
      entries: {
        ...plugins?.entries,
  ...
  const runMemorySelection = (config: OperatorConfig, selectedId = "memory") =>
    applyExclusiveSlotSelection({
      config,
      selectedId,
      selectedKind: "memory",
