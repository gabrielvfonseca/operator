...
import { APPROVALS_SCOPE, READ_SCOPE, WRITE_SCOPE } from "../../../src/gateway/operator-scopes.js";
import { pluginHostHookHandlers } from "../../../src/gateway/server-methods/plugin-host-hooks.js";
import { buildGatewaySessionRow } from "../../../src/gateway/session-utils.js";
import { withTempConfig } from "../../../src/gateway/test-temp-config.js";
import { emitAgentEvent, resetAgentEventsForTest } from "../../../src/infra/agent-events.js";
import { resolvePreferredOperatorTmpDir } from "../../../src/infra/tmp-operator-dir.js";
import { withEnvAsync } from "../../../src/test-utils/env.js";
import { validatePluginCommandDefinition } from "../../../src/plugins/command-registration.js";
import { executePluginCommand } from "../../../src/plugins/commands.js";
import { createHookRunner } from "../../../src/plugins/hooks.js";
import { cleanupReplacedPluginHostRegistry, runPluginHostCleanup } from "../../../src/plugins/host-hook-cleanup.js";
import {
  clearPluginHostRuntimeState,
  getPluginRunContext,
  setPluginRunContext,
} from "../../../src/plugins/host-hook-runtime.js";
import { listPluginSessionSchedulerJobs } from "../../../src/plugins/host-hook-runtime.test-fixtures.js";
import {
  drainPluginNextTurnInjectionContext,
  enqueuePluginNextTurnInjection,
  patchPluginSessionExtension,
  projectPluginSessionExtensionsSync,
} from "../../../src/plugins/host-hook-state.js";
import { buildPluginAgentTurnPrepareContext, isPluginJsonValue } from "../../../src/plugins/host-hooks.js";
import { createEmptyPluginRegistry } from "../../../src/plugins/registry-empty.js";
import { createPluginRegistry } from "../../../src/plugins/registry.js";
import { setActivePluginRegistry } from "../../../src/plugins/runtime.js";
import type { PluginRuntime } from "../../../src/plugins/runtime/types.js";
import { createPluginRecord } from "../../../src/plugins/status.test-helpers.js";
import { runTrustedToolPolicies } from "../../../src/plugins/trusted-tool-policy.js";
import { registerHostHookFixture, registerTrustedHostHookFixture } from "../../../src/plugins/contracts/host-hook-fixture.js";
...
describe("host-hook fixture plugin contract", () => {
  ...
  it("allows the official ClawHub Codex plugin to keep /codex command ownership", () => {
    const { config, registry } = createPluginRegistryFixture();
    const codexRoot = path.join("/tmp", ".operator", "extensions", "codex");
    registerTestPlugin({
      registry,
      config,
      record: createPluginRecord({
  ...
  it("requires explicit unset to remove plugin session extension state", async () => {
    ...
    await withHostHookState("operator-host-hooks-patch-", async ({ storePath, tempConfig }) => {
  ...
  it("reports duplicate next-turn injections as not newly enqueued", async () => {
    await withHostHookState("operator-host-hooks-injection-", async ({ storePath, tempConfig }) => {
      await updateSessionStore(storePath, (store) => {
        store["agent:main:main"] = {
          sessionId: "session-1",
          updatedAt: Date.now(),
  ...
  it("preserves global enqueue order when draining live next-turn injections", async () => {
    ...
    setActivePluginRegistry(registry);
    await withHostHookState("operator-host-hooks-order-", async ({ storePath, tempConfig }) => {
      await updateSessionStore(storePath, (store) => {
        store["agent:main:main"] = {
          sessionId: "session-1",
          updatedAt: Date.now(),
  ...
  it("cleans plugin-owned session state and lifecycle resources on reset/disable", async () => {
    ...
    await withHostHookState("operator-host-hooks-state-", async ({ tempConfig }) => {
  ...
  it("removes persistent plugin-owned session state and pending injections during cleanup", async () => {
    ...
    await withHostHookState("operator-host-hooks-store-", async ({ storePath, tempConfig }) => {
      await updateSessionStore(storePath, (store) => {
        store["agent:main:main"] = {
          sessionId: "session-1",
          updatedAt: Date.now(),
  ...
  it("does not clear unrelated run context during session-scoped cleanup", async () => {
    ...
    await withHostHookState("operator-host-hooks-run-context-", async ({ tempConfig }) => {
      await runPluginHostCleanup({
        cfg: tempConfig,
        registry,
        reason: "reset",
  ...
  it("preserves durable plugin session state during plugin restart cleanup", async () => {
    ...
    await withHostHookState(
      "operator-host-hooks-restart-state-",
      async ({ storePath, tempConfig }) => {
        await updateSessionStore(storePath, (store) => {
          store["agent:main:main"] = {
            sessionId: "session-1",
  ...
  it("cleans pending injections for plugins that registered no host-hook callbacks", async () => {
    ...
    await withHostHookState(
      "operator-host-hooks-injection-only-",
      async ({ storePath, tempConfig }) => {
        await updateSessionStore(storePath, (store) => {
          store["agent:main:main"] = {
            sessionId: "session-1",
