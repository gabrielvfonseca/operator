...
import { loadInstalledPluginIndexInstallRecordsSync } from "../../src/plugins/installed-plugin-index-records.js";
import { writePersistedInstalledPluginIndexSync } from "../../src/plugins/installed-plugin-index-store.js";
import {
...
import { loadPluginManifestRegistryForInstalledIndex } from "../../src/plugins/manifest-registry-installed.js";
import type { PluginMetadataSnapshot } from "../../src/plugins/plugin-metadata-snapshot.types.js";
import { loadPluginRegistrySnapshotWithMetadata } from "../../src/plugins/plugin-registry-snapshot.js";
...
function createHermeticEnv(rootDir: string): NodeJS.ProcessEnv {
  return {
    OPERATOR_BUNDLED_PLUGINS_DIR: path.join(rootDir, "bundled"),
    OPERATOR_STATE_DIR: path.join(rootDir, "state"),
...
describe("loadPluginRegistrySnapshotWithMetadata", () => {
  ...
  it("keeps persisted manifestless Claude bundles on the fast path", () => {
    ...
    const stateDir = path.join(tempRoot, "state");
    const env = { ...createHermeticEnv(tempRoot), OPERATOR_DISABLE_BUNDLED_PLUGINS: "1" };
    const config = {
      plugins: {
        load: { paths: [rootDir] },
      },
