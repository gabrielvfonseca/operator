...
import type { PluginMemoryEmbeddingProviderRegistration } from "../../src/plugins/registry.test-fixtures.js";
import {
...
function setPluginLoadResult(overrides: Partial<ReturnType<typeof createPluginLoadResult>>) {
  ...
  loadOperatorPluginsMock.mockReturnValue(result);
...
function expectPluginLoaderCall(params: {
  ...
  expectMockCalledWithFields(loadOperatorPluginsMock, {
      ...
      snapshot: createInstalledPluginIndexSnapshot([
        {
          ...
          manifestPath: "/tmp/provider-env-plugin/operator.plugin.json",
