...
} from "../../src/test-utils/session-state-cleanup.js";
...
describe("cleanupSessionStateForTest", () => {
  ...
  it("waits for in-flight session store writer queues before clearing test state", async () => {
    const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "operator-session-cleanup-"));
    const storePath = path.join(fixtureRoot, "operator-sessions.json");
