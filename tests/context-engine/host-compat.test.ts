...
import type { ContextEngine, ContextEngineHostCapability } from "../../src/context-engine/types.js";
    ...
    info: {
      ...
      hostRequirements: {
        "agent-run": {
          ...
          unsupportedMessage:
            "Use the native Codex or Operator embedded runtime, or switch contextEngine to legacy.",
        },
      },
    },
    async ingest() {
...
describe("context engine host compatibility", () => {
  ...
  it("allows native Codex and Operator embedded hosts to satisfy pre-prompt assembly", () => {
    ...
    assertContextEngineHostSupport({
      contextEngine: engine,
      operation: "agent-run",
      host: OPERATOR_EMBEDDED_CONTEXT_ENGINE_HOST,
    });
  });
