...
- Dependency-touching work: direct dependency inspection is mandatory when feasible; do not rely on assumptions, wrappers, or memory. Most dependencies are OSS, so read their source/docs/types. Codex-related work has a hard gate: the acting agent must personally inspect sibling `../codex` source for the exact protocol/runtime behavior before any verdict, comment, approval, merge recommendation, code change, or `proof sufficient` claim. If missing, clone `https://github.com/openai/codex.git` there first. Subagent reports, PR text, Operator wrappers, generated schemas, memory, and prior bot reviews do not satisfy this gate. No direct `../codex` check means no Codex verdict. Cite Codex files/lines checked in final/review/comment.
...
- Public ClawSweeper comments prefer `https://docs.operator.ai/...` when a public docs page exists; structured evidence still cites repo files, lines, SHAs.
...
- Installers: sibling `../operator.ai`.
...
- New binary fallible-operation results use `Result` from `@gabrielvfonseca/normalization-core/result`; domain-rich outcomes keep named discriminated unions.
...
- State/storage migrations are database-first. Runtime reads/writes the canonical store only. Old file stores, sidecars, aliases, and fallback readers belong in `operator doctor --fix` migration code only, never steady-state runtime.
- Storage default: SQLite only. Do not add JSON/JSONL/TXT/sidecar files for Operator-owned runtime state, caches, queues, registries, indexes, cursors, checkpoints, or plugin scratch data.
...
- Use the shared state DB (`state/operator.sqlite`) for global runtime state and plugin KV data. Use the per-agent DB (`agents/<agentId>/agent/operator-agent.sqlite`) for agent-scoped state/cache. Use a dedicated SQLite DB only when schema, volume, or lifecycle clearly does not fit those stores.
...
- Use `$operator-testing` for test/CI choice and `$crabbox` for remote/full/E2E proof.
...
- PR artifacts/screenshots: attach to PR/comment/external artifact store. Never push screenshots, videos, proof images, or proof assets to Operator or any product repo branch, including temp artifact branches. Use Crabbox artifact publishing plus the manifest URL. Do not commit `.github/pr-assets`.
...
- Agent PR landing to `main`: use only the repo-native `scripts/pr` wrapper: run `scripts/pr review-init <PR>`, follow its emitted checkout/guard guidance, initialize and complete review artifacts with `scripts/pr review-artifacts-init <PR>`, validate them with `scripts/pr review-validate-artifacts <PR>`, then run `OPERATOR_TESTBOX=1 scripts/pr prepare-run <PR>` and `scripts/pr merge-run <PR>`. The Testbox flag is mandatory for agents so prepare verifies hosted CI/Testbox on the current head or reuses a patch-identical pre-rebase run green within 24 hours instead of running full gates locally. `prepare-run` fails fast; invoke only after exact-head CI is complete and green. For owner-approved reviewed fork code without hosted Testbox, use `OPERATOR_PR_GATES_REMOTE=testbox` instead. Do not rebase only because `main` advanced; merge drift is advisory unless strict drift is explicitly enabled, while GitHub still blocks conflicts. Do not idle on `auto-response` or `check-docs`.
...
- Secrets: channel/provider creds in `~/.operator/credentials/`; model auth profiles in `~/.operator/agents/<agentId>/agent/auth-profiles.json`.
...
- Releases/publish/version bumps need explicit approval. Use `$release-operator-maintainer`.
...
- GHSA/advisories: `$operator-ghsa-maintainer` / `$security-triage`. Secret scanning: `$operator-secret-scanning-maintainer`.
...
- Mac gateway: dev watch = `pnpm gateway:watch`; managed installs = `operator gateway restart/status --deep`; logs = `./scripts/clawlog.sh`. No launchd/ad-hoc tmux.
...
- Version bump surfaces live in `$release-operator-maintainer`.
- Parallels: `$operator-parallels-smoke`; Discord roundtrip: `$parallels-discord-roundtrip`.
...
- Rebrand/migration/config warnings: run `operator doctor`.
