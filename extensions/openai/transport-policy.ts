...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-operator-turn-id": turnId,
      "x-operator-turn-attempt": attempt,
    },
    metadata: {
      operator_session_id: sessionHeaders["x-operator-session-id"] ?? "",
      operator_turn_id: turnId,
      operator_turn_attempt: attempt,
      operator_transport: ctx.transport,
    },
  };
}
