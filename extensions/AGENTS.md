...
- Extension production code should import from `operator/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `operator.plugin.json` and the package
  `operator` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `operator/plugin-sdk/*`.
