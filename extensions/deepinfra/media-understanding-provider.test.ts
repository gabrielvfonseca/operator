...
vi.mock("operator/plugin-sdk/media-understanding", async () => {
  const actual = await vi.importActual<typeof import("operator/plugin-sdk/media-understanding")>(
    "operator/plugin-sdk/media-understanding",
  );
  return {
    ...actual,
...
afterAll(() => {
  vi.doUnmock("operator/plugin-sdk/media-understanding");
  vi.resetModules();
});
