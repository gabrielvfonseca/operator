...
vi.mock("operator/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("operator/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("operator/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("operator/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("operator/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("operator/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/operator/synology-chat-sessions.json"),
