...
function changedProfileInvariants(
  ...
  const currentUsesLocalManagedLaunch =
    current.driver === "@gabrielvfonseca/operator" && !current.attachOnly && current.cdpIsLoopback;
  const nextUsesLocalManagedLaunch =
    next.driver === "@gabrielvfonseca/operator" && !next.attachOnly && next.cdpIsLoopback;
