...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<OperatorConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: OperatorConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): OperatorConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: OperatorConfig) {
