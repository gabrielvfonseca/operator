 ...
 * not exercise: launchOperatorChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopOperatorChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildOperatorChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveOperatorUserDataDir", () => {
    ...
    it("respects an explicit profile name", () => {
      const dir = resolveOperatorUserDataDir("my-profile");
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchOperatorChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchOperatorChrome(resolved, profile);
  ...
  describe("launchOperatorChrome", () => {
    ...
    const captureFailedLaunchStderr = async (params: {
      ...
      const result = await launchOperatorChrome(
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchOperatorChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it("returns structured no-display details before spawning headed Chrome", async () => {
      ...
      const error = await launchOperatorChrome(makeResolved(), profile, {
        platform: "linux",
    ...
    it("rejects a runtime spawn error before polling CDP", async () => {
      ...
      await expect(
        launchOperatorChrome(makeResolved(), makeProfile(51112), {
    ...
    it("rejects a bootstrap spawn error without attempting the runtime launch", async () => {
      ...
      await expect(launchOperatorChrome(makeResolved(), makeProfile(51113))).rejects.toBe(
        spawnError,
      );
    ...
    it("keeps handling process errors after a successful spawn", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          const running = await launchOperatorChrome(
            makeResolved(),
    ...
    it("aborts a deferred managed launch and proves its exact child exited", async () => {
      ...
      const launch = launchOperatorChrome(makeResolved(), makeProfile(51114), {
        signal: controller.signal,
      });
    ...
    it("aborts bootstrap immediately and never reaches the runtime launch", async () => {
      ...
      const launch = launchOperatorChrome(makeResolved(), makeProfile(51115), {
        signal: controller.signal,
      });
    ...
    it("returns the exact child when abort cleanup cannot prove process exit", async () => {
      ...
      const launch = launchOperatorChrome(makeResolved(), makeProfile(51116), {
        signal: controller.signal,
      });
      await probeEntered.promise;
    ...
    it.each([
      ...
      async ({ cdpUrl, configuredProbeHost }) => {
        ...
        await expect(launchOperatorChrome(makeResolved(), profile)).rejects.toThrow(portBusy);
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchOperatorChrome(makeResolved(), profile);
          expect(running.pid).toBe(4242);
    ...
    it("preserves the exact surviving child when a singleton retry cleanup fails", async () => {
      ...
      try {
        const error = await launchOperatorChrome(
          makeResolved({ localLaunchTimeoutMs: 20 }),
          profile,
        ).catch((err: unknown) => err);
        ...
        for (const testCase of [
          ...
          await withMockChromeCdpServer({
            ...
            run: async (baseUrl) => {
              ...
              const userDataDir = resolveOperatorUserDataDir(`${profile.name}-${testCase.pid}`);
              ...
              try {
                await expect(
                  launchOperatorChrome(makeResolved(), profileWithUniqueName),
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchOperatorChrome(resolved, profile)).rejects.toThrow(
  ...
  describe("launchOperatorChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      try {
        ...
        const userDataDir = path.join(resolveOperatorUserDataDir(profileName));
        ...
        const staged = resolveOperatorUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_OPERATOR_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      let message = "";
      try {
        await launchOperatorChrome(resolved, profile);
      } catch (err) {
        message = err instanceof Error ? err.message : String(err);
      }
    ...
    it("logs a warning when decorateOperatorProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateOperatorProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateOperatorProfile }, "decorateOperatorProfile")
        .mockImplementation(() => {
  ...
  describe("launchOperatorChrome managed-proxy CDP bypass", () => {
    ...
    it("surfaces loopbackMode=block as BrowserProfileUnavailableError without spawning Chrome", async () => {
      ...
      await expect(launchOperatorChrome(makeResolved(), profile)).rejects.toBeInstanceOf(
    ...
    it("does not register a bypass for a remote attachOnly CDP URL (loopback gate)", async () => {
      stubExecutableAndPrefsExist();
      // For this test we want launchOperatorChrome to reject before any
      ...
      await expect(launchOperatorChrome(makeResolved(), remoteProfile)).rejects.toThrow(
        /is remote; cannot launch local Chrome/,
      );
