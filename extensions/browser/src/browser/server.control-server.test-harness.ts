...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.OPERATOR_GATEWAY_PORT = prevGatewayPort;
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchOperatorChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.OPERATOR_GATEWAY_PORT;
  process.env.OPERATOR_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.OPERATOR_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.OPERATOR_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.OPERATOR_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.OPERATOR_GATEWAY_PASSWORD;
  } else {
    process.env.OPERATOR_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
