...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        operator: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.OPERATOR_GATEWAY_PORT;
    process.env.OPERATOR_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.OPERATOR_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.OPERATOR_GATEWAY_PASSWORD;
