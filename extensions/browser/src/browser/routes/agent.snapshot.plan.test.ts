...
function profile(driver: "existing-session" | "@gabrielvfonseca/operator"): ResolvedBrowserProfile {
  return {
    name: driver === "existing-session" ? "user" : "@gabrielvfonseca/operator",
    driver,
...
describe("resolveSnapshotPlan", () => {
  ...
  it("keeps ai snapshots for managed browsers when Playwright is available", () => {
    const plan = resolveSnapshotPlan({
      profile: profile("@gabrielvfonseca/operator"),
      query: {},
      hasPlaywright: true,
    });
