...
import { DEFAULT_OPERATOR_BROWSER_PROFILE_NAME, resolveBrowserConfig } from "./browser/config.js";
...
function collectChromeMcpProfiles(cfg: OperatorConfig): ExistingSessionProfile[] {
...
function collectManagedProfiles(cfg: OperatorConfig): ManagedProfile[] {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return [];
  }
...
function isLegacyClawdProfileConfigured(cfg: OperatorConfig, legacyProfileDir: string): boolean {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return false;
  }
...
export function detectLegacyClawdBrowserProfileResidue(
  ...
  if (
    resolved.defaultProfile !== DEFAULT_OPERATOR_BROWSER_PROFILE_NAME ||
...
export async function noteChromeMcpBrowserReadiness(
  ...
  if (!browserExecutable && managedProfiles.length > 0) {
    noteFn(
      [
        `- Operator-managed browser profile(s) are configured: ${managedProfileLabel}.`,
  ...
  if (missingDisplay || shouldWarnRootNoSandbox) {
    const lines = [`- Operator-managed browser profile(s) are configured: ${managedProfileLabel}.`];
    if (missingDisplay) {
      lines.push(
  ...
  if (autoConnectProfiles.length === 0) {
    noteFn(
      [
        ...
        "- Keep the browser running and accept the attach consent prompt the first time Operator connects.",
      ].join("\n"),
      "Browser",
    );
    return;
  ...
  if (!chrome) {
    const lines = [
      `- Chrome MCP existing-session is configured for profile(s): ${profileLabel}.`,
      `- Google Chrome was not found on this host for auto-connect profile(s): ${autoProfileLabel}. Operator does not bundle Chrome.`,
