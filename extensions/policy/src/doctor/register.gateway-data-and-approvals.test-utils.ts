...
describe("registerPolicyDoctorChecks", () => {
    ...
    expect(evidence.dataHandling).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          source: "oc://operator.config/logging/redactSensitive",
          value: false,
        }),
        expect.objectContaining({
          kind: "telemetryContentCapture",
          source: "oc://operator.config/diagnostics/otel/captureContent",
          value: true,
        }),
        expect.objectContaining({
          kind: "sessionRetentionMode",
          source: "oc://operator.config/session/maintenance/mode",
          value: "warn",
        }),
        expect.objectContaining({
          kind: "memorySessionTranscriptIndexing",
          source: "oc://operator.config/memory/qmd/sessions/enabled",
          value: true,
        }),
      ]),
    );
    expect(result.findings).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          ocPath: "oc://operator.config/logging/redactSensitive",
        ...
        expect.objectContaining({
          ...
          ocPath: "oc://operator.config/diagnostics/otel/captureContent",
        ...
        expect.objectContaining({
          ...
          ocPath: "oc://operator.config/session/maintenance/mode",
        ...
        expect.objectContaining({
          ...
          ocPath: "oc://operator.config/memory/qmd/sessions/enabled",
  ...
  it("supports agent-scoped session transcript memory conformance", async () => {
    ...
    expect(result.findings).toEqual([
      expect.objectContaining({
        checkId: "policy/data-handling-session-transcript-memory-enabled",
        ocPath: "oc://operator.config/agents/defaults/memorySearch/experimental/sessionMemory",
        requirement:
  ...
  it("reports exec approvals file conformance findings", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        version: 1,
        socket: { path: "/tmp/operator.sock", token: "secret-token" },
