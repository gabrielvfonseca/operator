...
function previewGatewayLoopbackBind(
  ...
  if (
    finding.ocPath !== "oc://operator.config/gateway/bind" &&
    finding.ocPath !== "oc://operator.config/gateway/customBindHost"
  ) {
    return [];
  }
  return [
...
function previewGatewayNodeDenyCommand(
  ...
  if (
    command === undefined ||
    command === "" ||
    finding.ocPath !== "oc://operator.config/gateway/nodes/denyCommands"
  ) {
    return [];
  }
  return [
