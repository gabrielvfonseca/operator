/**
 * Persistent lease store for ACPX wrapper processes. Leases let Operator attach
...
export const OPERATOR_ACPX_LEASE_ID_ENV = "OPERATOR_ACPX_LEASE_ID";
...
export function withAcpxLeaseEnvironment(params: {
  ...
  return [
    "env",
    `${OPERATOR_ACPX_LEASE_ID_ENV}=${quoteEnvValue(params.leaseId)}`,
    `${OPERATOR_GATEWAY_INSTANCE_ID_ENV}=${quoteEnvValue(params.gatewayInstanceId)}`,
