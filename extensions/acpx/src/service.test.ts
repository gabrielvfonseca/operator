...
const { cleanupOperatorOwnedAcpxProcessTreeMock } = vi.hoisted(() => ({
  cleanupOperatorOwnedAcpxProcessTreeMock: vi.fn(
    async (): Promise<{
...
const { reapStaleOperatorOwnedAcpxOrphansMock } = vi.hoisted(() => ({
  reapStaleOperatorOwnedAcpxOrphansMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
vi.mock("./process-reaper.js", () => ({
  cleanupOperatorOwnedAcpxProcessTree: cleanupOperatorOwnedAcpxProcessTreeMock,
  reapStaleOperatorOwnedAcpxOrphans: reapStaleOperatorOwnedAcpxOrphansMock,
}));

import { getAcpRuntimeBackend } from "../runtime-api.js";
import type { OperatorPluginServiceContext } from "../runtime-api.js";
...
const previousEnv = {
  OPERATOR_ACPX_RUNTIME_STARTUP_PROBE: process.env.OPERATOR_ACPX_RUNTIME_STARTUP_PROBE,
  OPERATOR_SKIP_ACPX_RUNTIME: process.env.OPERATOR_SKIP_ACPX_RUNTIME,
  OPERATOR_SKIP_ACPX_RUNTIME_PROBE: process.env.OPERATOR_SKIP_ACPX_RUNTIME_PROBE,
};
...
afterEach(async () => {
  ...
  cleanupOperatorOwnedAcpxProcessTreeMock.mockClear();
  reapStaleOperatorOwnedAcpxOrphansMock.mockClear();
  acpxRuntimeConstructorMock.mockClear();
  createAgentRegistryMock.mockClear();
  createFileSessionStoreMock.mockClear();
  restoreEnv("OPERATOR_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("OPERATOR_SKIP_ACPX_RUNTIME");
  restoreEnv("OPERATOR_SKIP_ACPX_RUNTIME_PROBE");
...
function createOpenKeyedStore(ctx: OperatorPluginServiceContext) {
  const env = { ...process.env, OPERATOR_STATE_DIR: ctx.stateDir };
...
function openGatewayInstanceStore(ctx: OperatorPluginServiceContext) {
...
function openProcessLeaseStore(ctx: OperatorPluginServiceContext) {
...
describe("createAcpxRuntimeService", () => {
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.OPERATOR_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.OPERATOR_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = await makeTempDir();
  ...
  it("waits for the embedded runtime startup probe before resolving by default", async () => {
    delete process.env.OPERATOR_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
  ...
  it("reaps stale ACPX process leases from the generated wrapper root at startup", async () => {
    ...
    cleanupOperatorOwnedAcpxProcessTreeMock.mockResolvedValueOnce({
    ...
    expect(cleanupOperatorOwnedAcpxProcessTreeMock).toHaveBeenCalledWith({
      rootPid: 101,
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale Operator-owned ACPX processes");
  ...
  it("runs wrapper-root orphan cleanup before dropping pending ACPX leases", async () => {
    ...
    await openProcessLeaseStore(ctx).register(lease.leaseId, lease);
    reapStaleOperatorOwnedAcpxOrphansMock.mockResolvedValueOnce({
    ...
    expect(cleanupOperatorOwnedAcpxProcessTreeMock).not.toHaveBeenCalled();
    expect(reapStaleOperatorOwnedAcpxOrphansMock).toHaveBeenCalledWith({
      wrapperRoot,
      deps: processCleanupDeps,
    });
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale Operator-owned ACPX processes");
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.OPERATOR_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.OPERATOR_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.OPERATOR_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.OPERATOR_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (OPERATOR_SKIP_ACPX_RUNTIME=1)",
    );
  });
});
