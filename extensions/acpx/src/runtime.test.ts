...
const DOCUMENTED_OPERATOR_BRIDGE_COMMAND =
  "env OPERATOR_HIDE_BANNER=1 OPERATOR_SUPPRESS_NOTES=1 operator acp --url ws://127.0.0.1:18789 --token-file ~/.operator/gateway.token --session agent:main:main";
const CODEX_ACP_COMMAND = "npx @zed-industries/codex-acp@0.13.0";
const CODEX_ACP_WRAPPER_COMMAND = `node "/tmp/operator/acpx/codex-acp-wrapper.mjs"`;
const CODEX_ACP_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_ACP_WRAPPER_COMMAND} ${OPERATOR_ACPX_LEASE_ID_ARG} lease-close ${OPERATOR_GATEWAY_INSTANCE_ID_ARG} gateway-test`;
  ...
  const runtime = new AcpxRuntime(
    {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "@gabrielvfonseca/operator" ? "operator acp" : agentName,
        list: () => ["codex", "@gabrielvfonseca/operator"],
      },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("adds the Operator session key to the managed Operator tools MCP bridge", () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
      ...
      mcpServers: [
        {
          name: "operator-tools",
          command: "node",
          args: ["dist/mcp/operator-tools-serve.js"],
          env: [],
        },
      ],
    });
    ...
    const readScopedMcpEnv = (sessionKey: string) => {
      ...
      ).resolveOperatorToolsDelegateForSession(sessionKey) as {
        options: {
          mcpServers?: Array<{
      ...
      return delegate.options.mcpServers?.find((server) => server.name === "operator-tools")?.env;
    };

    expect(readScopedMcpEnv("agent:worker:main")).toContainEqual({
      name: "OPERATOR_TOOLS_MCP_AGENT_SESSION_KEY",
  ...
  it("keeps managed Operator tools MCP delegates reachable for fresh sessions", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const firstDelegate =
      exposedRuntime.resolveOperatorToolsDelegateForSession("agent:worker:main");
    expect(exposedRuntime.operatorToolsSessionDelegates.has("agent:worker:main")).toBe(true);

    await runtime.prepareFreshSession({ sessionKey: "agent:worker:main" });

    expect(exposedRuntime.operatorToolsSessionDelegates.has("agent:worker:main")).toBe(true);
    expect(exposedRuntime.resolveOperatorToolsDelegateForSession("agent:worker:main")).toBe(
      firstDelegate,
    );
  });

  it("uses the no-MCP delegate for startup probes when the Operator tools bridge is enabled", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("normalizes Operator Codex model ids for ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("strips the Operator Anthropic provider prefix for Claude ACP startup", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
      agentRegistry: {
        ...
        list: () => ["claude", "@gabrielvfonseca/operator"],
      },
    });
    const ensure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:claude:acp:test",
  ...
  it("keeps Claude ACP model ids intact after stripping the Operator provider prefix", () => {
  ...
  it.each([
    ...
    const wrapperRoot = await fs.mkdtemp(path.join(os.tmpdir(), "operator-acpx-runtime-"));
    ...
    const { runtime, delegate } = makeRuntime(baseStore, {
      ...
      operatorWrapperRoot: wrapperRoot,
      agentRegistry: {
  ...
  it("adds Codex wrapper stderr tail to generic terminal turn error events", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        operatorLeaseId: "lease-turn-event",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("adds Codex wrapper stderr tail to generic startTurn failure results", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        operatorLeaseId: "lease-start-turn",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("adds Codex wrapper stderr tail when startTurn creation throws", async () => {
    ...
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
        agentCommand: CODEX_ACP_WRAPPER_COMMAND,
        operatorLeaseId: "lease-start-turn-create",
      })),
      save: vi.fn(async () => {}),
    };
    const { runtime, delegate } = makeRuntime(baseStore, {
  ...
  it("disables delegate prompt timeout for Operator-managed turns", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:test",
  ...
  it("passes model startup through sessionOptions for non-Codex ACP agents", async () => {
    ...
    const { runtime, delegate } = makeRuntime(baseStore, {
      agentRegistry: {
        ...
        list: () => ["main", "codex", "@gabrielvfonseca/operator"],
      },
    });
    const ensure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("injects Codex ACP startup config into the scoped registry", () => {
    ...
    expect(testing.isCodexAcpCommand("operator acp")).toBe(false);
  });
  ...
  it("recognizes claude-agent-acp commands", () => {
    ...
    expect(
      testing.isClaudeAcpCommand(`node "/tmp/operator/acpx/claude-agent-acp-wrapper.mjs"`),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
        `node.exe "C:/Users/runner/AppData/Local/Temp/operator/acpx/claude-agent-acp-wrapper.mjs"`,
      ),
    ).toBe(true);
    expect(
      testing.isClaudeAcpCommand(
    ...
    expect(testing.isClaudeAcpCommand("operator acp")).toBe(false);
  ...
  it("releases managed Operator tools MCP delegates after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const scopedDelegate =
      exposedRuntime.resolveOperatorToolsDelegateForSession("agent:codex:main");
    ...
    expect(exposedRuntime.operatorToolsSessionDelegates.has("agent:codex:main")).toBe(false);
  });

  it("cleans up Operator-owned ACPX process trees after close", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:codex:acp:binding:test",
        agentCommand: 'node "/tmp/operator/acpx/codex-acp-wrapper.mjs"',
        pid: 900,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate } = makeRuntime(
      baseStore,
      {
        operatorWrapperRoot: "/tmp/operator/acpx",
      },
      {
        operatorProcessCleanup: {
  ...
  it("records ACPX process leases without persisting lease-specific agent commands", async () => {
    ...
    expect(lease?.wrapperPath).toBe("/tmp/operator/acpx/codex-acp-wrapper.mjs");
    expect(launchCommands[0]).toContain("OPERATOR_ACPX_LEASE_ID=");
    expect(launchCommands[0]).toContain("OPERATOR_GATEWAY_INSTANCE_ID=gateway-test");
    expect(savedRecords[0]?.agentCommand).toBe(CODEX_ACP_WRAPPER_COMMAND);
    expect(savedRecords[0]?.operatorGatewayInstanceId).toBe("gateway-test");
    expect(savedRecords[0]?.operatorLeaseId).toBe(lease?.leaseId);
  });
  ...
  it("merges sidecar lease ids into loaded ACPX session records", async () => {
    ...
    expect(loadedRecord?.operatorGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.operatorLeaseId).toBe("lease-loaded");
  });
  ...
  it("merges the lease for the current ACPX session process when old leases exist", async () => {
    ...
    const loadedRecord = await wrappedStore.load("agent:codex:acp:binding:test");
    expect(loadedRecord?.operatorGatewayInstanceId).toBe("gateway-test");
    expect(loadedRecord?.operatorLeaseId).toBe("lease-current");
  });
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 930,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${OPERATOR_ACPX_LEASE_ID_ARG} lease-old ${OPERATOR_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
            {
              pid: 940,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${OPERATOR_ACPX_LEASE_ID_ARG} lease-current ${OPERATOR_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
          ...
          listProcesses: vi.fn(async () => [
            {
              pid: 920,
              ppid: 1,
              command: `${CODEX_ACP_WRAPPER_COMMAND} ${OPERATOR_ACPX_LEASE_ID_ARG} other-lease ${OPERATOR_GATEWAY_INSTANCE_ID_ARG} gateway-test`,
            },
          ]),
          killProcess: vi.fn((pid, signal) => {
            killed.push({ pid, signal });
  ...
  it("routes operator ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:operator:acp:test",
      backend: "acpx",
    ...
    const result = await runtime.ensureSession({
      sessionKey: "agent:operator:acp:test",
      agent: "@gabrielvfonseca/operator",
      mode: "persistent",
    });
  ...
  it("routes non-operator sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for operator sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:operator:acp:test",
      backend: "acpx",
      runtimeSessionName: "operator-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the operator agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "@gabrielvfonseca/operator" ? "codex" : agentName,
        list: () => ["codex", "@gabrielvfonseca/operator"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the operator bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "operator acp" : agentName),
        list: () => ["codex", "@gabrielvfonseca/operator"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped operator bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "@gabrielvfonseca/operator"
            ? DOCUMENTED_OPERATOR_BRIDGE_COMMAND
            : agentName,
        list: () => ["codex", "@gabrielvfonseca/operator"],
      },
    });
  ...
  it("uses the bridge-safe delegate for local node operator entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "@gabrielvfonseca/operator"
            ? "env OPERATOR_HIDE_BANNER=1 node operator.mjs acp"
            : agentName,
        list: () => ["codex", "@gabrielvfonseca/operator"],
      },
    });
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:operator:acp:test",
        agentCommand: DOCUMENTED_OPERATOR_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:operator:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:operator:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to operator bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
