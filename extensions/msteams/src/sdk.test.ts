...
describe("createMSTeamsApp", () => {
  ...
  it("preserves both Teams SDK and Operator User-Agent fragments", async () => {
    const creds: MSTeamsCredentials = {
      type: "secret",
      appId: "test-app-id",
      appPassword: "test-secret",
    ...
    expect(headers?.["User-Agent"]).toMatch(/^teams\.ts\[apps\]\/\S+ Operator\/\S+$/);
  });
  ...
  it("passes China cloud to the SDK App without requiring a configured serviceUrl", async () => {
    ...
    // default. Operator proactive sends use stored reference serviceUrls instead.
