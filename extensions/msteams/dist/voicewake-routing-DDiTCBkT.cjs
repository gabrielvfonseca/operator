...
function normalizeRouteTarget(value) {
	...
	const rec = value;
	if ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(rec.mode) === "current") return { mode: "current" };
	const agentId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(rec.agentId);
	const sessionKey = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(rec.sessionKey);
	if (agentId && !sessionKey) return { agentId: (0, _gabrielvfonseca_normalization_core_agent_id.normalizeAgentId)(agentId) };
...
function normalizeRouteRule(value) {
	if (!value || typeof value !== "object") return null;
	const rec = value;
	const triggerRaw = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(rec.trigger);
...
function isCanonicalAgentSessionKey(value) {
	...
	if (require_session_key.classifySessionKeyShape(trimmed) !== "agent") return false;
...
function validateRouteTargetInput(value, label) {
	...
	const rec = value;
	const mode = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(rec.mode);
	const agentId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(rec.agentId);
	const sessionKey = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(rec.sessionKey);
	if (mode !== void 0) {
	...
	if (agentId !== void 0) {
		if (!(0, _gabrielvfonseca_normalization_core_agent_id.isValidAgentId)(agentId)) return {
			ok: false,
...
function validateVoiceWakeRoutingConfigInput(input) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(input)) return {
		ok: false,
	...
	if (Array.isArray(rec.routes)) {
		...
		for (const [index, route] of rec.routes.entries()) {
			if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(route)) return {
				ok: false,
				message: `config.routes[${index}] must be an object`
			};
			const trigger = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(route.trigger);
...
async function loadVoiceWakeRoutingConfig(baseDir) {
	...
	const routingDb = require_state_migrations_cron_run_logs.getNodeSqliteKysely(database.db);
	const configRow = require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(database.db, routingDb.selectFrom("voicewake_routing_config").selectAll().where("config_key", "=", VOICEWAKE_ROUTING_CONFIG_KEY));
	if (!configRow) return { ...DEFAULT_ROUTING };
	const routeRows = require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, routingDb.selectFrom("voicewake_routing_routes").selectAll().where("config_key", "=", VOICEWAKE_ROUTING_CONFIG_KEY).orderBy("position", "asc")).rows;
	return {
		version: 1,
...
async function setVoiceWakeRoutingConfig(config, baseDir) {
	...
	require_openclaw_state_db.runOperatorStateWriteTransaction(({ db }) => {
		const routingDb = require_state_migrations_cron_run_logs.getNodeSqliteKysely(db);
		require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, routingDb.deleteFrom("voicewake_routing_routes").where("config_key", "=", VOICEWAKE_ROUTING_CONFIG_KEY));
		require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, routingDb.deleteFrom("voicewake_routing_config").where("config_key", "=", VOICEWAKE_ROUTING_CONFIG_KEY));
		const defaultTarget = targetColumns(next.defaultTarget);
		require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, routingDb.insertInto("voicewake_routing_config").values({
		...
		if (next.routes.length > 0) require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, routingDb.insertInto("voicewake_routing_routes").values(next.routes.map((route, position) => {
...
function resolveVoiceWakeRouteByTrigger(params) {
	const normalizedTrigger = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.trigger) ? normalizeVoiceWakeTriggerWord(params.trigger) : "";
...
//#endregion
Object.defineProperty(exports, "loadVoiceWakeRoutingConfig", {
	enumerable: true,
	get: function() {
		return loadVoiceWakeRoutingConfig;
	}
});
Object.defineProperty(exports, "normalizeVoiceWakeRoutingConfig", {
	enumerable: true,
	get: function() {
		return normalizeVoiceWakeRoutingConfig;
	}
});
Object.defineProperty(exports, "resolveVoiceWakeRouteByTrigger", {
	enumerable: true,
	get: function() {
		return resolveVoiceWakeRouteByTrigger;
	}
});
Object.defineProperty(exports, "setVoiceWakeRoutingConfig", {
	enumerable: true,
	get: function() {
		return setVoiceWakeRoutingConfig;
	}
});
Object.defineProperty(exports, "validateVoiceWakeRoutingConfigInput", {
	enumerable: true,
	get: function() {
		return validateVoiceWakeRoutingConfigInput;
	}
});
