...
function resolveProviderConfig(cfg, provider) {
	...
	return Object.entries(providers).find(([key]) => (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(key) === normalized)?.[1];
}
...
function normalizeProbeApi(providerConfig) {
	const api = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(providerConfig.api);
...
function isPrivateIpv4Host(host) {
	...
	const [a, b] = octets;
	return a === 10 || a === 172 && (0, _gabrielvfonseca_normalization_core.expectDefined)(b, "model preflight.runtime b") >= 16 && (0, _gabrielvfonseca_normalization_core.expectDefined)(b, "model preflight.runtime b") <= 31 || a === 192 && b === 168;
}
...
function formatUnavailableReason(params) {
	return [
		...
		`Skipping this cron run; Operator will retry the provider preflight on a later scheduled run.`,
...
//#endregion
exports.preflightCronModelProvider = preflightCronModelProvider;
exports.resetCronModelProviderPreflightCacheForTest = resetCronModelProviderPreflightCacheForTest;
