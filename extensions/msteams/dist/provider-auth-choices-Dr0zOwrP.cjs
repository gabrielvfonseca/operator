...
var provider_auth_choices_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	resolveManifestDeprecatedProviderAuthChoice: () => resolveManifestDeprecatedProviderAuthChoice,
	resolveManifestProviderAuthChoice: () => resolveManifestProviderAuthChoice,
	resolveManifestProviderAuthChoices: () => resolveManifestProviderAuthChoices,
	resolveProviderOnboardAuthFlags: () => resolveProviderOnboardAuthFlags
});
...
function formatDescriptorLabel(value) {
	return require_ansi.sanitizeForLog(value).trim().split(/[-_\s]+/gu).filter(Boolean).map((part) => {
...
function normalizeManifestAuthDescriptorId(value) {
	return require_ansi.sanitizeForLog(value).trim();
}
...
function resolveManifestProviderAuthChoiceCandidates(params) {
	...
	return registry.plugins.flatMap((plugin) => {
		...
		if (plugin.origin === "workspace" && params?.includeUntrustedWorkspacePlugins === false && !require_config_state.resolveEffectiveEnableState({
			id: plugin.id,
...
//#endregion
Object.defineProperty(exports, "provider_auth_choices_exports", {
	enumerable: true,
	get: function() {
		return provider_auth_choices_exports;
	}
});
Object.defineProperty(exports, "resolveManifestDeprecatedProviderAuthChoice", {
	enumerable: true,
	get: function() {
		return resolveManifestDeprecatedProviderAuthChoice;
	}
});
Object.defineProperty(exports, "resolveManifestProviderAuthChoice", {
	enumerable: true,
	get: function() {
		return resolveManifestProviderAuthChoice;
	}
});
Object.defineProperty(exports, "resolveManifestProviderAuthChoices", {
	enumerable: true,
	get: function() {
