...
function normalizeClawHubTrustToken(value) {
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(value)?.toLowerCase() ?? "";
}
...
function buildClawHubTrustInstallRecordFields(params) {
	...
	const reasons = params.trust.reasons.map((reason) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(reason)).filter((reason) => Boolean(reason));
	return {
...
function resolveClawHubSubjectUrl(params) {
	if (params.subject.kind === "skill" && params.subject.ownerHandle) return `${require_clawhub.resolveClawHubBaseUrl(params.baseUrl)}/${encodeURIComponent(params.subject.ownerHandle)}/skills/${encodeURIComponent(params.subject.packageName)}`;
	const pathRoot = params.subject.kind === "skill" ? "skills" : "plugins";
	return `${require_clawhub.resolveClawHubBaseUrl(params.baseUrl)}/${pathRoot}/${encodeClawHubPackagePath(params.subject.packageName)}`;
}
function resolveClawHubSecurityLinks(params) {
	const subjectUrl = resolveClawHubSubjectUrl(params);
	if (params.subject.kind === "skill") {
		const resolvedSubjectUrl = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.links?.subject) ?? subjectUrl;
		return {
			subject: resolvedSubjectUrl,
			security: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.links?.security) ?? `${resolvedSubjectUrl}/security-audit?version=${encodeURIComponent(params.version)}`
		};
	}
	return {
		subject: subjectUrl,
...
function wrapWords(text, width) {
	if (require_ansi.visibleWidth(text) <= width) return [text];
	const words = text.split(/\s+/).filter(Boolean);
	const lines = [];
	let line = "";
	for (const word of words) {
		const next = line ? `${line} ${word}` : word;
		if (require_ansi.visibleWidth(next) > width && line) {
			lines.push(line);
			line = word;
		} else line = next;
	}
...
function resolveClawHubTrustAccent(disposition) {
	switch (disposition) {
		case "blocked": return require_theme.theme.error;
		case "review-required": return require_theme.theme.warn;
		case "review-recommended": return require_theme.theme.info;
		case "clean": return require_theme.theme.success;
	}
	return require_theme.theme.info;
}
function formatClawHubEvidenceLine(params) {
	const label = require_safe_text.sanitizeTerminalText(params.label).replace(/:$/u, "");
	return `${require_theme.theme.muted(`• ${padRight(label, CLAWHUB_EVIDENCE_LABEL_WIDTH)}`)} ${params.accent(params.value)}`;
}
function renderClawHubTrustBox(title, lines, disposition) {
	...
	const titleFillWidth = Math.max(0, borderWidth - require_ansi.visibleWidth(titleSegment));
...
function formatLinkedClawHubValue(params) {
	const label = require_safe_text.sanitizeTerminalText(params.label);
	return require_terminal_link.formatTerminalLink(label, require_safe_text.sanitizeTerminalText(params.url), {
		fallback: label,
...
function formatClawHubTrustEvidenceLines(params) {
	...
	const moderationState = normalizeClawHubTrustToken(params.trust.moderationState);
	if (moderationState && !CLAWHUB_SAFE_MODERATION_STATES.has(moderationState)) addLine("Moderation:", require_safe_text.sanitizeTerminalText(moderationState));
	for (const reason of params.trust.reasons) {
		...
		switch (normalized) {
			...
			case "payload_strings":
				...
				break;
			default:
				addLine("Finding:", require_safe_text.sanitizeTerminalText(formatClawHubReasonCode(reason)));
				break;
		}
	}
	if (params.assessment.disposition === "review-recommended") for (const notice of params.assessment.notices) addLine("Status:", require_safe_text.sanitizeTerminalText(notice));
	if (params.trust.blockedFromDownload) addLine("Finding:", "Download disabled by ClawHub for this release");
	if (lines.length === 0) for (const reason of params.assessment.riskReasons) addLine("Finding:", require_safe_text.sanitizeTerminalText(reason));
	return lines;
}
function formatClawHubRawLinkLine(label, url) {
	return `  ${require_theme.theme.muted(padRight(label, CLAWHUB_RAW_LINK_LABEL_WIDTH))} ${require_theme.theme.info(require_safe_text.sanitizeTerminalText(url))}`;
}
function formatClawHubRawLinks(params) {
	const subjectUrl = require_safe_text.sanitizeTerminalText(params.links.subject);
...
function formatClawHubTrustWarning(params) {
	...
	if (params.assessment.disposition === "blocked") {
		...
		const blockedActionLines = params.mode === "update" ? malicious ? [`Latest ${noun} version is marked malicious; Operator will not download it.`, `Uninstall the installed ${noun} unless you have independently reviewed it.`] : [`Latest ${noun} version is blocked by ClawHub; Operator will not download it.`] : [`Operator will not install this ${noun} release from ClawHub.`];
...
function validateClawHubSecurityIdentity(params) {
	...
	const responsePackageName = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.security.package?.name);
	if (responsePackageName !== params.packageName) return {
		ok: false,
		error: `ClawHub release trust check for "${formatClawHubReleaseLabel(packageLabel, params.version)}" returned package "${require_safe_text.sanitizeTerminalText(responsePackageName ?? "unknown")}".`,
		code: CLAWHUB_TRUST_ERROR_CODE.CLAWHUB_SECURITY_UNAVAILABLE,
		version: params.version
	};
	const responseVersion = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.security.release?.version);
	if (responseVersion !== params.version) return {
		ok: false,
		error: `ClawHub release trust check for "${formatClawHubReleaseLabel(packageLabel, params.version)}" returned version "${require_safe_text.sanitizeTerminalText(responseVersion ?? "unknown")}".`,
		code: CLAWHUB_TRUST_ERROR_CODE.CLAWHUB_SECURITY_UNAVAILABLE,
		version: params.version
	};
	return null;
...
function mapSkillSecurityVerdictToPackageSecurity(params) {
	const responseSlug = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.item.slug ?? params.item.requestedSlug);
	if (responseSlug !== params.packageName) throw new Error(`ClawHub skill trust check for "${formatClawHubReleaseLabel(params.packageName, params.version)}" returned skill "${require_safe_text.sanitizeTerminalText(responseSlug ?? "unknown")}".`);
	const responsePublisher = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.item.publisherHandle);
	if (params.ownerHandle && responsePublisher !== params.ownerHandle) throw new Error(`ClawHub skill trust check for "${formatClawHubReleaseLabel(params.packageName, params.version)}" returned publisher "${require_safe_text.sanitizeTerminalText(responsePublisher ?? "unknown")}", expected "${require_safe_text.sanitizeTerminalText(params.ownerHandle)}".`);
	const responseVersion = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.item.version);
	if (responseVersion !== params.version) {
		const reason = params.item.error?.message ? `: ${require_safe_text.sanitizeTerminalText(params.item.error.message)}` : "";
		throw new Error(`ClawHub skill trust check for "${formatClawHubReleaseLabel(params.packageName, params.version)}" returned version "${require_safe_text.sanitizeTerminalText(responseVersion ?? "unknown")}"${reason}.`);
	}
	...
	const reasons = params.item.reasons.map((reason) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(reason)).filter((reason) => Boolean(reason));
	...
	const displayName = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.item.displayName);
	return {
		package: {
...
function resolveSkillSecurityLinks(item) {
	const subject = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(item.skillUrl);
	const security = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(item.securityAuditUrl);
...
function readOptionalStringField(value, field) {
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(readObject(value)?.[field]);
}
...
function mapSkillVerificationSecurityForVerdict(verification, opts) {
	...
	const status = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(security.status) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(security.rawStatus);
...
function mapSkillVerificationToSecurityVerdictItem(params) {
	...
	const pageUrl = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.verification.pageUrl);
	const reasons = params.verification.reasons.map((reason) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(reason)).filter((reason) => Boolean(reason));
	...
	const verifiedVersion = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.verification.version) ?? readOptionalStringField(versionRecord, "version");
	return {
		...
		slug: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.verification.slug) ?? readOptionalStringField(skill, "slug"),
		version: verifiedVersion ?? (cardOnlyCleanFailure ? params.version : null),
		displayName: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.verification.displayName) ?? readOptionalStringField(skill, "displayName"),
		publisherHandle: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.verification.publisherHandle) ?? readOptionalStringField(publisher, "handle") ?? params.ownerHandle,
		publisherDisplayName: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.verification.publisherDisplayName) ?? readOptionalStringField(publisher, "displayName"),
...
async function fetchOwnerQualifiedSkillSecurityFallback(params) {
	...
	const item = mapSkillVerificationToSecurityVerdictItem({
		verification: await require_clawhub.fetchClawHubSkillVerification({
...
async function fetchClawHubSubjectSecurity(params) {
	if (params.subject.kind === "plugin") return { security: await require_clawhub.fetchClawHubPackageSecurity({
...
async function ensureClawHubPackageTrustAcknowledged(params) {
	...
	try {
		...
		return {
			ok: false,
			error: `ClawHub release trust check failed for "${releaseLabel}": ${require_safe_text.sanitizeTerminalText(require_errors.formatErrorMessage(error))}`,
			code: CLAWHUB_TRUST_ERROR_CODE.CLAWHUB_SECURITY_UNAVAILABLE,
			version: params.version
		};
	}
	...
	const warning = require_ansi.stripAnsi(formatClawHubTrustWarning({
		baseUrl: params.baseUrl,
		subject: params.subject,
		version: params.version,
		trust,
...
//#endregion
Object.defineProperty(exports, "ensureClawHubPackageTrustAcknowledged", {
	enumerable: true,
	get: function() {
		return ensureClawHubPackageTrustAcknowledged;
	}
});
