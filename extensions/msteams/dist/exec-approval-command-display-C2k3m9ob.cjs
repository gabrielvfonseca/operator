...
function truncateForDisplay(text) {
	...
	return {
		text: (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(text, EXEC_APPROVAL_MAX_OUTPUT) + EXEC_APPROVAL_TRUNCATION_MARKER,
		truncated: true,
		oversized: false
	};
}
...
function sanitizeExecApprovalDisplayTextInternal(commandText, options) {
	...
	const rawRedacted = require_redact.redactSensitiveText(commandText, { mode: "tools" });
	const { stripped, strippedToOrig } = buildStrippedView(commandText);
	if (require_redact.redactSensitiveText(stripped, { mode: "tools" }) === stripped) return truncateForDisplay(escapeInvisibles(rawRedacted, options));
	const redaction = require_redact.resolveRedactOptions({ mode: "tools" });
	const rawMask = require_redact.computeSensitiveRedactionBitmap(commandText, redaction);
	const strippedMask = require_redact.computeSensitiveRedactionBitmap(stripped, redaction);
	let bypassDetected = false;
	for (let i = 0; i < strippedMask.length; i++) if (strippedMask[i] && !rawMask[(0, _gabrielvfonseca_normalization_core.expectDefined)(strippedToOrig[i], "stripped to orig entry at i")]) {
		bypassDetected = true;
		break;
	}
	if (!bypassDetected) return truncateForDisplay(escapeInvisibles(rawRedacted, options));
	const unionMask = rawMask.slice();
	for (let i = 0; i < strippedMask.length; i++) if (strippedMask[i]) unionMask[(0, _gabrielvfonseca_normalization_core.expectDefined)(strippedToOrig[i], "stripped to orig entry at i")] = true;
	let out = "";
	let i = 0;
...
//#endregion
Object.defineProperty(exports, "resolveExecApprovalCommandDisplay", {
	enumerable: true,
	get: function() {
		return resolveExecApprovalCommandDisplay;
	}
});
Object.defineProperty(exports, "sanitizeExecApprovalDisplayText", {
	enumerable: true,
	get: function() {
		return sanitizeExecApprovalDisplayText;
	}
});
Object.defineProperty(exports, "sanitizeExecApprovalDisplayTextWithStatus", {
	enumerable: true,
	get: function() {
		return sanitizeExecApprovalDisplayTextWithStatus;
	}
});
Object.defineProperty(exports, "sanitizeExecApprovalWarningText", {
	enumerable: true,
	get: function() {
		return sanitizeExecApprovalWarningText;
	}
});
