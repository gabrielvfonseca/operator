...
const require_chat_queued_turns = require("./chat-queued-turns-DVyzjtDW.cjs");
const require_session_reset_service = require("./session-reset-service-BJLBYKkE.cjs");
const require_restart_trace = require("./restart-trace-N9sMqXGV.cjs");
let _gabrielvfonseca_normalization_core_string_coerce = require("@gabrielvfonseca/normalization-core/string-coerce");
//#region src/gateway/server-close.ts
const shutdownLog = require_subsystem.createSubsystemLogger("gateway/shutdown");
...
function abortActiveRunsForRestart(params) {
	...
	for (const [runId, entry] of listUnabortedRestartRuns(params.chatAbortControllers)) {
		if (!require_chat_abort.isChatAbortControllerEntryAbortable(entry)) continue;
		if (entry.projectSessionActive === false) {
			entry.abortStopReason = "restart";
			entry.controller.abort(require_run_termination.createAgentRunRestartAbortError());
			require_chat_abort.removeChatAbortControllerEntry(params.chatAbortControllers, runId, entry);
			params.chatRunState.abortedRuns.set(runId, require_server_chat_state.createChatAbortMarker());
		...
		if (require_chat_abort.abortTrackedChatRunById({
			...params,
...
function abortQueuedTurnsForRestart(params) {
	...
	return require_chat_queued_turns.abortQueuedChatTurns(params.chatQueuedTurns, matches, "restart").length;
}
...
async function triggerGatewayLifecycleHookWithTimeout(params) {
	let timeout;
	const hookPromise = require_internal_hooks.triggerInternalHook(params.event);
...
async function disposeAllBundleLspRuntimesOnDemand() {
	const { disposeAllBundleLspRuntimes } = await Promise.resolve().then(() => require("./agent-bundle-lsp-runtime-DtCWr2fk.cjs")).then((n) => n.agent_bundle_lsp_runtime_exports);
	await disposeAllBundleLspRuntimes();
}
async function stopGmailWatcherOnDemand() {
	const { stopGmailWatcher } = await Promise.resolve().then(() => require("./gmail-watcher-BhEHZ6Qi.cjs"));
...
function createGatewayCloseHandler(params) {
	return async (opts) => {
		...
		const warnings = [];
		const reason = ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(opts?.reason) ?? "") || "gateway stopping";
		...
		const measureCloseStep = (name, run) => require_restart_trace.measureGatewayRestartTrace(`restart.close.${name}`, run, [["reason", reason]]);
		try {
			require_session_suspension.clearSessionSuspensionTimers();
			...
			await measureCloseStep("gateway-shutdown-hook", () => shutdownStep("gateway:shutdown", async () => {
				if (await triggerGatewayLifecycleHookWithTimeout({
					event: require_internal_hooks.createInternalHookEvent("gateway", "shutdown", "gateway:shutdown", {
						reason,
						restartExpectedMs
					}),
			...
			if (restartExpectedMs !== null) await measureCloseStep("gateway-pre-restart-hook", () => shutdownStep("gateway:pre-restart", async () => {
				if (await triggerGatewayLifecycleHookWithTimeout({
					event: require_internal_hooks.createInternalHookEvent("gateway", "pre-restart", "gateway:pre-restart", {
						reason,
						restartExpectedMs
					}),
			...
			await measureCloseStep("channels", async () => {
				const channelIds = params.channelIds ?? require_registry$1.listChannelPlugins().map((plugin) => plugin.id);
				for (const channelId of channelIds) await shutdownStep(`channel/${channelId}`, () => params.stopChannel(channelId), warnings);
			});
			await shutdownStep("agent-harnesses", () => require_registry.disposeRegisteredAgentHarnesses(), warnings);
			await measureCloseStep("bundle-runtimes", async () => {
				await Promise.all([disposeRuntimeWithShutdownGrace({
					label: "bundle-mcp",
					dispose: params.disposeSessionMcpRuntimes ?? require_agent_bundle_mcp_runtime.disposeAllSessionMcpRuntimes,
			...
			await shutdownStep("plugin-state-store", () => require_plugin_state_store.closePluginStateDatabase(), warnings);
		...
		require_restart_trace.recordGatewayRestartTrace("restart.close.total", durationMs, [
			["reason", reason],
			["restartExpectedMs", restartExpectedMs ?? "none"],
			...require_restart_trace.collectGatewayProcessMemoryUsageMb()
		]);
		return {
			durationMs,
			warnings
...
exports.drainActiveSessionsForShutdown = require_session_reset_service.drainActiveSessionsForShutdown;
exports.runGatewayClosePrelude = runGatewayClosePrelude;
