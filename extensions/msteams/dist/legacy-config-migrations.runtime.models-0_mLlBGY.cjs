...
const getRecord = (value) => (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value) ? value : null;
const ensureRecord = (root, key) => {
	const existing = root[key];
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(existing)) return existing;
	const next = {};
	root[key] = next;
	return next;
};
const mergeMissing = (target, source) => {
	for (const [key, value] of Object.entries(source)) {
		if (value === void 0 || require_prototype_keys.isBlockedObjectKey(key)) continue;
		const existing = target[key];
		if (existing === void 0) {
			target[key] = value;
			continue;
		}
		if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(existing) && (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value)) mergeMissing(existing, value);
	}
};
const mapLegacyAudioTranscription = (value) => {
	...
	if (!require_exec_safety.isSafeExecutableValue(rawExecutable)) return null;
...
var legacy_config_migrations_runtime_models_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	LEGACY_CONFIG_MIGRATIONS_RUNTIME_MODELS: () => LEGACY_CONFIG_MIGRATIONS_RUNTIME_MODELS,
	collectBlockedLegacyOpenAICodexProviderPlan: () => collectBlockedLegacyOpenAICodexProviderPlan
});
...
function hasInvalidThinkingFormat(providers) {
	...
	for (const provider of Object.values(providersRecord)) {
		...
		for (const model of models) {
			...
			if (typeof thinkingFormat === "string" && !require_types_models.isModelThinkingFormat(thinkingFormat)) return true;
		}
	}
	return false;
}
...
function parseVllmAgentModelKey(key) {
	const trimmed = require_model_ref_profile.splitTrailingAuthProfile(key).model.trim();
	const slashIndex = trimmed.indexOf("/");
	if (slashIndex <= 0) return;
	if ((0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(trimmed.slice(0, slashIndex)) !== "vllm") return;
...
function findVllmProvider(providers) {
	if (!providers) return;
	const key = Object.keys(providers).find((entry) => (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(entry) === "vllm");
...
function applyLegacyVllmQwenThinkingFormat(params) {
	...
	if (typeof currentThinkingFormat === "string" && require_types_models.isModelThinkingFormat(currentThinkingFormat)) {
		removeLegacyVllmQwenThinkingParams(params.legacyParams);
...
function upgradeRetiredModelRef(value) {
	...
	const split = require_model_ref_profile.splitTrailingAuthProfile(trimmed);
...
function sanitizeModelRefMapEntry(value) {
	...
	const sanitized = {};
	for (const [field, child] of Object.entries(record)) if (!require_prototype_keys.isBlockedObjectKey(field)) setRecordEntry(sanitized, field, sanitizeModelRefMapEntry(child));
	return sanitized;
}
function modelRefValuesAreEqual(existing, incoming, path) {
	if ((0, node_util.isDeepStrictEqual)(existing, incoming)) return true;
	...
	return (0, node_util.isDeepStrictEqual)(normalizedExisting, normalizedIncoming);
}
function mergeModelRefMapEntries(existing, incoming, path) {
	...
	for (const [field, incomingValue] of Object.entries(incomingRecord)) {
		if (incomingValue === void 0 || require_prototype_keys.isBlockedObjectKey(field)) continue;
...
function hasCanonicalOpenAIProvider(providers) {
	return Object.keys(providers).some((providerId) => (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(providerId) === OPENAI_PROVIDER_ID);
}
...
function getCanonicalOpenAIProviderEntry(providers) {
	const key = Object.keys(providers).find((k) => (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(k) === OPENAI_PROVIDER_ID);
...
function hasAutoFixableLegacyOpenAICodexProvider(providersValue) {
	...
	for (const [providerId, providerValue] of Object.entries(providers)) {
		...
		if (!provider || !require_codex_route_model_ref.isLegacyCodexProviderId(providerId)) continue;
...
function collectBlockedLegacyOpenAICodexProviderPlan(raw) {
	...
	if (!providers || !canonicalEntry) return { blockedModelIdentities: [] };
	...
	for (const [providerId, providerValue] of Object.entries(providers)) {
		...
		if (modelCollisions.length > 0) {
			const identity = require_codex_route_model_ref.legacyCodexProviderIdentityKey(providerId);
...
function resolveMovedCodexModelRuntime(params) {
	if ((0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(params.legacyProviderId) !== "codex") return;
	const modelRuntime = getRecord(params.model.agentRuntime);
	const modelRuntimeId = require_openai_routing.normalizeOptionalAgentRuntimeId(modelRuntime?.id);
	...
	const providerRuntimeId = require_openai_routing.normalizeOptionalAgentRuntimeId(providerRuntime?.id);
...
function collectNonEquivalentLegacyOpenAIModelCollisions(params) {
	...
	for (const legacyModel of legacyModels) {
		...
		if (!collisions.every((canonicalModel) => {
			if (!(0, node_util.isDeepStrictEqual)(buildMergedLegacyOpenAIModel(canonicalModel, params.canonical, OPENAI_PROVIDER_ID), legacyEffective)) return false;
			return MODEL_UNSCOPED_PROVIDER_DEFAULT_KEYS.every((key) => (0, node_util.isDeepStrictEqual)(params.canonical[key], params.legacy[key]));
		})) conflicts.add(legacyId ?? legacyName ?? "unknown");
	}
	return [...conflicts];
}
function prepareLegacyCodexProviderForCanonicalMove(providerId, provider) {
	if ((0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(providerId) !== "codex" || !Array.isArray(provider.models)) return provider;
	return {
		...provider,
...
function migrateLegacyOpenAICodexProvider(raw, changes) {
	...
	for (const [providerId, providerValue] of Object.entries({ ...providers })) {
		...
		if (!require_codex_route_model_ref.isLegacyCodexProviderId(providerId)) {
		...
		apply: (raw, changes) => {
			...
			for (const [providerId, provider] of Object.entries(providers)) {
				...
				for (const [index, model] of models.entries()) {
					...
					if (typeof thinkingFormat !== "string" || require_types_models.isModelThinkingFormat(thinkingFormat)) continue;
...
//#endregion
Object.defineProperty(exports, "LEGACY_CONFIG_MIGRATIONS_RUNTIME_MODELS", {
	enumerable: true,
	get: function() {
		return LEGACY_CONFIG_MIGRATIONS_RUNTIME_MODELS;
	}
});
Object.defineProperty(exports, "collectBlockedLegacyOpenAICodexProviderPlan", {
	enumerable: true,
	get: function() {
		return collectBlockedLegacyOpenAICodexProviderPlan;
	}
});
Object.defineProperty(exports, "defineLegacyConfigMigration", {
	enumerable: true,
	get: function() {
		return defineLegacyConfigMigration;
	}
});
...
Object.defineProperty(exports, "isLegacyModelsAddCodexMetadataModel", {
	enumerable: true,
	get: function() {
		return isLegacyModelsAddCodexMetadataModel;
	}
});
Object.defineProperty(exports, "legacy_config_migrations_runtime_models_exports", {
	enumerable: true,
	get: function() {
		return legacy_config_migrations_runtime_models_exports;
	}
});
Object.defineProperty(exports, "mapLegacyAudioTranscription", {
	enumerable: true,
	get: function() {
		return mapLegacyAudioTranscription;
	}
});
Object.defineProperty(exports, "mergeMissing", {
	enumerable: true,
	get: function() {
		return mergeMissing;
	}
