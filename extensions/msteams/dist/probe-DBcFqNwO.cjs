...
var probe_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	MAX_TIMER_DELAY_MS: () => MAX_TIMER_DELAY_MS,
	clampProbeTimeoutMs: () => clampProbeTimeoutMs,
	probeGateway: () => probeGateway
});
const MIN_PROBE_TIMEOUT_MS = 250;
const MAX_TIMER_DELAY_MS = require_timeouts.MAX_SAFE_TIMEOUT_DELAY_MS;
...
function clampProbeTimeoutMs(timeoutMs) {
	return require_timeouts.resolveSafeTimeoutDelayMs(timeoutMs, { minMs: MIN_PROBE_TIMEOUT_MS });
}
...
async function probeGateway(opts) {
	...
	const deviceIdentity = await (async () => {
		try {
			...
			const { loadDeviceIdentityIfPresent } = await Promise.resolve().then(() => require("./device-identity-C6ZGDbLx.cjs")).then((n) => n.device_identity_exports);
			const stateDir = require_paths.resolveStateDir(opts.env);
			const identity = loadDeviceIdentityIfPresent(node_path.default.join(stateDir, "identity", "device.json"));
			if (!identity) return null;
			return require_client_start_readiness.loadDeviceAuthToken({
	...
	return await new Promise((resolve) => {
		...
		const armProbeTimer = (onTimeout, timeoutMs = initialProbeTimeoutMs) => {
			clearProbeTimer();
			timer = setTimeout(onTimeout, require_timeouts.resolveSafeTimeoutDelayMs(timeoutMs));
		};
		const settle = (result) => {
			if (settled) return;
			settled = true;
		...
		const client = new require_client_start_readiness.GatewayClient({
			url: opts.url,
			...
			env: opts.env,
			scopes: [require_operator_scopes.READ_SCOPE],
			clientName: require_client_info.GATEWAY_CLIENT_NAMES.CLI,
			clientVersion: "dev",
			mode: require_client_info.GATEWAY_CLIENT_MODES.PROBE,
			instanceId,
			deviceIdentity,
			onConnectError: (err) => {
				connectError = require_errors.formatErrorMessage(err);
				connectErrorDetails = err instanceof require_src.GatewayClientRequestError ? err.details : null;
			},
...
//#endregion
Object.defineProperty(exports, "probeGateway", {
	enumerable: true,
	get: function() {
		return probeGateway;
	}
