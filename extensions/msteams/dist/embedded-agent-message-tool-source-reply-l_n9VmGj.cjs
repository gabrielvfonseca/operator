...
require("./embedded-agent-subscribe.tools-uz-J6wM0.cjs");
...
function isMessageToolSourceReplyActionName(action) {
	if (require_embedded_agent_messaging.isMessageToolSendActionName(action)) return true;
...
function parseJsonRecord(value) {
	return (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)((0, _gabrielvfonseca_normalization_core.safeParseJson)(value));
}
...
function isDeliveredMessagingToolResult(params) {
	...
	if (action && require_embedded_agent_messaging.isMessageToolConversationCreateActionName(action) && (deliveryEnvelopeHasCreatedConversationId(params.result) || deliveryEnvelopeHasCreatedConversationId(params.hookResult))) return true;
	...
	if (params.isError || require_tool_result_error.isToolResultError(params.result) || require_tool_result_error.isToolResultError(params.hookResult)) return false;
	const normalizedToolName = require_tool_policy.normalizeToolName(params.toolName ?? MESSAGE_TOOL_NAME);
	if (require_embedded_agent_messaging.isMessagingToolDeliveryAction(normalizedToolName, args) && action !== "broadcast" && (deliveryEnvelopeIndicatesOk(params.result) || deliveryEnvelopeIndicatesOk(params.hookResult)) && !deliveryEnvelopeIndicatesNonDelivery(params.result) && !deliveryEnvelopeIndicatesNonDelivery(params.hookResult) && !deliveryEnvelopeIndicatesNoOp(params.result) && !deliveryEnvelopeIndicatesNoOp(params.hookResult)) return true;
...
function isDeliveredMessageToolOnlySourceReplyResult(params) {
	...
	if (require_tool_policy.normalizeToolName(params.toolName) !== MESSAGE_TOOL_NAME) return false;
	const args = asRecord(params.args);
	const sourceRouteReplyAction = params.allowExplicitSourceRoute === true && isMessageToolSourceReplyActionName(args.action);
	if (!require_embedded_agent_messaging.isMessageToolSendActionName(args.action) && !sourceRouteReplyAction) return false;
...
//#endregion
Object.defineProperty(exports, "hasMessagingDeliveryReceipt", {
	enumerable: true,
	get: function() {
		return hasMessagingDeliveryReceipt;
	}
});
Object.defineProperty(exports, "isDeliveredMessageToolOnlySourceReplyResult", {
	enumerable: true,
	get: function() {
		return isDeliveredMessageToolOnlySourceReplyResult;
	}
});
Object.defineProperty(exports, "isDeliveredMessagingToolResult", {
	enumerable: true,
	get: function() {
		return isDeliveredMessagingToolResult;
	}
});
