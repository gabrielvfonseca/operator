...
const log = require_subsystem.createSubsystemLogger("env-overrides");
/**
...
function isAlwaysBlockedSkillEnvKey(key) {
	return require_host_env_security.isDangerousHostEnvVarName(key) || require_host_env_security.isDangerousHostEnvOverrideVarName(key) || matchesAnyPattern(key, SKILL_ALWAYS_BLOCKED_ENV_PATTERNS);
}
function sanitizeSkillEnvOverrides(params) {
	...
	const result = require_sanitize_env_vars.sanitizeEnvVars(params.overrides);
	const allowed = {};
	...
	for (const key of result.blocked) {
		...
		if (!value) continue;
		const warning = require_sanitize_env_vars.validateEnvVarValue(value);
		if (warning) {
...
function applySkillConfigEnvOverrides(params) {
	...
	if (normalizedPrimaryEnv && (process.env[normalizedPrimaryEnv] === void 0 || activeSkillEnvEntries.has(normalizedPrimaryEnv)) && !pendingOverrides[normalizedPrimaryEnv]) {
		const resolvedApiKey = require_types_secrets.normalizeResolvedSecretInputString({
...
function applySkillEnvOverridesFromSnapshot(params) {
	...
	for (const skill of snapshot.skills) {
		const skillConfig = require_config.resolveSkillConfig(config, skill.name);
		if (!skillConfig) continue;
		if (!shouldApplySkillConfigEnvOverrides(skillConfig)) continue;
		applySkillConfigEnvOverrides({
			updates,
...
Object.defineProperty(exports, "applySkillEnvOverrides", {
	enumerable: true,
	get: function() {
		return applySkillEnvOverrides;
	}
});
Object.defineProperty(exports, "applySkillEnvOverridesFromSnapshot", {
	enumerable: true,
	get: function() {
		return applySkillEnvOverridesFromSnapshot;
	}
});
