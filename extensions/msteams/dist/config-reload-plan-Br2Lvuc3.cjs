...
function listReloadRules() {
	const registry = require_runtime.getActivePluginHttpRouteRegistry();
	const gatewayRegistryVersion = require_runtime.getActivePluginHttpRouteRegistryVersion();
	...
	const channelReloadRules = require_registry.listChannelPlugins().flatMap((plugin) => (plugin.reload?.configPrefixes ?? []).map((prefix) => ({
		prefix,
		kind: "hot",
	...
	const channelPluginStateRules = require_registry.listChannelPlugins().flatMap((plugin) => [{
...
function getPluginInstallRecords(config) {
	if (!require_plain_object.isPlainObject(config)) return {};
	const plugins = config.plugins;
	if (!require_plain_object.isPlainObject(plugins)) return {};
	const installs = plugins.installs;
	return require_plain_object.isPlainObject(installs) ? installs : {};
}
...
function listPluginInstallTimestampMetadataPaths(prevConfig, nextConfig) {
	return listPluginInstallRecordDiffPaths(prevConfig, nextConfig, ({ id, prevRecord, nextRecord, paths }) => {
		if (!require_plain_object.isPlainObject(prevRecord) || !require_plain_object.isPlainObject(nextRecord)) return;
...
function listPluginInstallWholeRecordPaths(prevConfig, nextConfig) {
	return listPluginInstallRecordDiffPaths(prevConfig, nextConfig, ({ id, prevRecord, nextRecord, paths }) => {
		if (!require_plain_object.isPlainObject(prevRecord) || !require_plain_object.isPlainObject(nextRecord)) paths.push(`plugins.installs.${id}`);
	});
}
...
//#endregion
Object.defineProperty(exports, "buildGatewayReloadPlan", {
	enumerable: true,
	get: function() {
		return buildGatewayReloadPlan;
	}
});
Object.defineProperty(exports, "listPluginInstallTimestampMetadataPaths", {
	enumerable: true,
	get: function() {
		return listPluginInstallTimestampMetadataPaths;
	}
});
Object.defineProperty(exports, "listPluginInstallWholeRecordPaths", {
	enumerable: true,
	get: function() {
		return listPluginInstallWholeRecordPaths;
	}
});
Object.defineProperty(exports, "resolveConfigReloadMetadata", {
	enumerable: true,
	get: function() {
		return resolveConfigReloadMetadata;
	}
});
