...
function buildCronEventPrompt(pendingEvents, opts) {
	...
	if (!eventText) {
		if (useHeartbeatResponseTool) return "A scheduled cron event was triggered, but no event content was found. " + require_heartbeat.HEARTBEAT_RESPONSE_TOOL_INSTRUCTIONS;
...
function buildExecEventPrompt(pendingEvents, opts) {
	...
	const eventText = rawEventText.length > MAX_EXEC_EVENT_PROMPT_CHARS ? `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(rawEventText, MAX_EXEC_EVENT_PROMPT_CHARS)}\n\n[truncated]` : rawEventText;
	if (!eventText) {
		if (useHeartbeatResponseTool) return `An async command completion event was triggered, but no command output was found. ${require_heartbeat.HEARTBEAT_RESPONSE_TOOL_INSTRUCTIONS} Do not mention, summarize, or reuse output from any earlier run.`;
	...
	if (!deliverToUser) {
		if (useHeartbeatResponseTool) return `An async command completion event was triggered, but user delivery is disabled for this run. Handle the result internally. ${require_heartbeat.HEARTBEAT_RESPONSE_TOOL_INSTRUCTIONS} Do not mention, summarize, or reuse command output.`;
...
const HEARTBEAT_OK_PREFIX = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(require_tokens.HEARTBEAT_TOKEN);
...
function isHeartbeatNoiseEvent(evt) {
	const lower = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(evt);
...
//#endregion
Object.defineProperty(exports, "buildCronEventPrompt", {
	enumerable: true,
	get: function() {
		return buildCronEventPrompt;
	}
});
Object.defineProperty(exports, "buildExecEventPrompt", {
	enumerable: true,
	get: function() {
		return buildExecEventPrompt;
	}
});
Object.defineProperty(exports, "isCronSystemEvent", {
	enumerable: true,
	get: function() {
		return isCronSystemEvent;
	}
});
Object.defineProperty(exports, "isExecCompletionEvent", {
	enumerable: true,
	get: function() {
		return isExecCompletionEvent;
	}
});
Object.defineProperty(exports, "isRelayableExecCompletionEvent", {
	enumerable: true,
	get: function() {
		return isRelayableExecCompletionEvent;
	}
});
