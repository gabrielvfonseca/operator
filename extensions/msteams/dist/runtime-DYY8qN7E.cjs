...
require("./worker-admission-DNxVcwiA.cjs");
...
function normalizePortableMode(mode, relativePath) {
	return relativePath === "operator.mjs" || (mode & 73) !== 0 ? 448 : 384;
}
async function stageFileEntry(stagingRoot, source) {
	const { sourcePath, expectedRealPath, stagedPath } = source;
	if (await node_fs_promises.default.realpath(sourcePath) !== expectedRealPath) throw new Error(`Unsafe worker bundle path: ${stagedPath}`);
	const stats = await node_fs_promises.default.lstat(sourcePath);
	if (stats.isSymbolicLink() || !stats.isFile()) throw new Error(`Unsafe worker bundle path: ${stagedPath}`);
	const handle = await node_fs_promises.default.open(sourcePath, node_fs.constants.O_RDONLY | (node_fs.constants.O_NOFOLLOW ?? 0));
	let contents;
	let mode;
	try {
		const openedStats = await handle.stat();
		const currentStats = await node_fs_promises.default.lstat(sourcePath);
		const currentRealPath = await node_fs_promises.default.realpath(sourcePath);
	...
	const stagedFilePath = node_path.default.join(stagingRoot, ...stagedPath.split("/"));
	await node_fs_promises.default.mkdir(node_path.default.dirname(stagedFilePath), { recursive: true });
	await node_fs_promises.default.writeFile(stagedFilePath, contents, { mode });
	await node_fs_promises.default.chmod(stagedFilePath, mode);
	return {
		entry: {
			path: stagedPath,
			mode,
			size: contents.byteLength,
			sha256: (0, node_crypto.createHash)("sha256").update(contents).digest("hex")
		},
		contents
	};
}
async function stageManifestEntry(sourceRoot, sourceRootRealPath, stagingRoot, relativePath, transform) {
	return await stageFileEntry(stagingRoot, {
		sourcePath: node_path.default.join(sourceRoot, relativePath),
		expectedRealPath: node_path.default.resolve(sourceRootRealPath, ...relativePath.split("/")),
		stagedPath: relativePath,
		transform
	});
}
const OPERATOR_IMPORT_SPECIFIER_PATTERN = /["'`](@openclaw\/[a-z0-9-]+)(?:\/[A-Za-z0-9./_-]+)?["'`]/gu;
...
async function readWorkspaceDependencyNames(sourceRoot) {
	const raw = await node_fs_promises.default.readFile(node_path.default.join(sourceRoot, "package.json"), "utf8");
...
async function collectVendoredPackageFiles(packageName, vendorRealRoot) {
	...
	if ((await node_fs_promises.default.lstat(node_path.default.join(vendorRealRoot, "npm-shrinkwrap.json")).catch(() => void 0))?.isFile()) files.push("npm-shrinkwrap.json");
	const walk = async (relativeDir) => {
		const dirents = await node_fs_promises.default.readdir(node_path.default.join(vendorRealRoot, ...relativeDir.split("/")), { withFileTypes: true });
...
async function stageVendoredWorkspacePackages(params) {
	...
	for (const packageName of [...params.packageNames].toSorted(comparePaths)) {
		const linkedPath = node_path.default.join(params.sourceRoot, "node_modules", ...packageName.split("/"));
		let vendorRealRoot;
		try {
			vendorRealRoot = await node_fs_promises.default.realpath(linkedPath);
		} catch (error) {
		...
		for (const relativePath of await collectVendoredPackageFiles(packageName, vendorRealRoot)) {
			const { entry } = await stageFileEntry(params.stagingRoot, {
				sourcePath: node_path.default.join(vendorRealRoot, ...relativePath.split("/")),
				expectedRealPath: node_path.default.resolve(vendorRealRoot, ...relativePath.split("/")),
...
async function collectWorkerBundleManifest(sourceRoot, stagingRoot) {
	const sourceRootRealPath = await node_fs_promises.default.realpath(sourceRoot);
	const distFiles = await require_package_dist_inventory.collectPackageDistInventory(sourceRoot);
	if (distFiles.length === 0) throw new Error(`Operator worker bundle has no packaged dist files; build the running package at ${sourceRoot}`);
...
const WORKER_BUNDLE_MANIFEST_VERSION = "operator-worker-bundle-v1";
const OPERATOR_NPM_REGISTRY = "https://registry.npmjs.org/";
...
function resolvePackageRoot(packageRoot) {
	if (packageRoot) return node_path.default.resolve(packageRoot);
	const resolved = require_openclaw_root.resolveOperatorPackageRootSync({
		moduleUrl: require("url").pathToFileURL(__filename).href,
		argv1: process.argv[1],
		cwd: process.cwd()
	});
	if (!resolved) throw new Error("Unable to locate the running Operator package root for worker bundling");
	return resolved;
}
async function isReleasedPackageInstall(packageRoot) {
	const entries = new Set(await node_fs_promises.default.readdir(packageRoot));
...
async function hashNpmTarballIntegrity(tarballPath) {
	const hash = (0, node_crypto.createHash)("sha512");
...
async function verifyPublishedNpmRelease(params) {
	...
	const temporaryRoot = await node_fs_promises.default.mkdtemp(node_path.default.join(node_os.default.tmpdir(), "operator-worker-npm-proof-"));
	try {
		const published = parseNpmPackageIdentity(await runNpmProofCommand({
			argv: [
				...
				"dist.integrity",
				"--json",
				`--registry=${OPERATOR_NPM_REGISTRY}`
			],
			cwd: temporaryRoot,
			failureMessage: `Operator ${params.version} is not published; use the worker bundle install`,
			runCommand
		}));
		if (published?.name !== "@gabrielvfonseca/operator" || published.version !== params.version || !NPM_SHA512_INTEGRITY_PATTERN.test(published.integrity)) throw new Error(`Cannot verify exact public npm release openclaw@${params.version}; use the worker bundle install`);
		const packedValue = await runNpmProofCommand({
			...
			cwd: temporaryRoot,
			failureMessage: "Unable to verify the installed Operator package; use the worker bundle install",
			runCommand
		});
		const packed = Array.isArray(packedValue) ? parseNpmPackageIdentity(packedValue[0]) : void 0;
		if (!packed?.filename || node_path.default.basename(packed.filename) !== packed.filename) throw new Error("npm pack returned incomplete worker package metadata");
		const packedTarballPath = node_path.default.join(temporaryRoot, packed.filename);
		let packedTarballIntegrity;
		try {
			packedTarballIntegrity = await hashNpmTarballIntegrity(packedTarballPath);
		} catch {
			throw new Error("Unable to verify the installed Operator package; use the worker bundle install");
		}
		if (packed.name !== published.name || packed.version !== published.version || packed.integrity !== published.integrity || packedTarballIntegrity !== published.integrity) throw new Error(`Installed Operator ${params.version} does not match the published package; use the worker bundle install`);
		const extractedRoot = node_path.default.join(temporaryRoot, "package");
		await node_fs_promises.default.mkdir(extractedRoot);
		await tar.extract({
			cwd: extractedRoot,
		...
		if ((await prepareWorkerBundle({
			...
			cacheDir: node_path.default.join(temporaryRoot, "bundle-cache"),
			operatorVersion: params.version
		})).bundleHash !== params.bundleHash) throw new Error(`Published Operator ${params.version} does not match the prepared worker bundle; use the worker bundle install`);
		return published.integrity;
	} finally {
		await node_fs_promises.default.rm(temporaryRoot, {
			recursive: true,
			force: true
		});
	}
...
async function isCachedTarball(filePath) {
	try {
		const stats = await node_fs_promises.default.lstat(filePath);
...
async function writeTarball(params) {
	const temporaryPath = `${params.tarballPath}.${process.pid}.${(0, node_crypto.randomUUID)()}.tmp`;
	try {
		await tar.create({
		...
		try {
			await node_fs_promises.default.rename(temporaryPath, params.tarballPath);
		} catch (error) {
			if (error.code !== "EEXIST") throw error;
			if (!await cachedTarballMatches(params.tarballPath, params.entries)) {
				await node_fs_promises.default.rm(params.tarballPath, { force: true });
				try {
					await node_fs_promises.default.rename(temporaryPath, params.tarballPath);
				} catch (publishError) {
		...
		await node_fs_promises.default.rm(temporaryPath, { force: true });
	}
}
async function prepareWorkerBundle(options) {
	const packageRoot = resolvePackageRoot(options.packageRoot);
	const cacheDir = options.cacheDir ? node_path.default.resolve(options.cacheDir) : node_path.default.join(require_paths.resolveStateDir(), "cache", "worker-bundles");
	const operatorVersion = (options.operatorVersion ?? require_version.VERSION).trim();
	if (!operatorVersion) throw new Error("Worker bundle requires a non-empty Operator version");
	const protocolFeatures = normalizeProtocolFeatures(options.protocolFeatures ?? []);
	await node_fs_promises.default.mkdir(cacheDir, { recursive: true });
	const stagingRoot = await node_fs_promises.default.mkdtemp(node_path.default.join(cacheDir, ".staging-"));
	try {
		...
		const tarballPath = node_path.default.join(cacheDir, `${bundleHash}.tgz`);
		...
		await node_fs_promises.default.rm(stagingRoot, {
			recursive: true,
			force: true
		});
	}
...
async function resolveWorkerNpmInstallationArtifact(params) {
	const version = params.bundle.operatorVersion.trim();
	if (!require_npm_registry_spec.isExactSemverVersion(version)) throw new Error(`Worker npm install requires the exact published gateway version; expected ${version}`);
...
const BOOTSTRAP_ROOT = ".operator-worker";
...
const NODE_MISSING_MARKER = "OPERATOR_WORKER_NODE_MISSING";
const NODE_UNSUPPORTED_MARKER = "OPERATOR_WORKER_NODE_UNSUPPORTED";
const NPM_MISSING_MARKER = "OPERATOR_WORKER_NPM_MISSING";
const BOOTSTRAP_OUTPUT_TAG = "OPERATOR_WORKER_BOOTSTRAP_V1";
...
try {
  ...
  process.exit(
    shapeMatches &&
      ...
      actual.operatorVersion === expected.operatorVersion &&
      featuresMatch
      ? 0
      : 1,
  );
...
function addFile(relative) {
  ...
  const mode = relative === "operator.mjs" || (stats.mode & 0o111) !== 0 ? 0o700 : 0o600;
...
find "$incoming" -type f -name 'operator-upload-*.tgz.*' -mmin +60 -exec rm -f -- {} + 2>/dev/null || true
upload=$(mktemp "$incoming/operator-upload-$hash.tgz.XXXXXXXX")
...
case "$install" in
  ...
  npm)
    ...
    OPERATOR_DISABLE_PLUGIN_REGISTRY_MIGRATION=1 npm pack "$package_spec" --pack-destination "$staging" --ignore-scripts --json --registry=https://registry.npmjs.org/ > "$npm_pack_json"
...
if [ "$install" = bundle ]; then
  ...
  OPERATOR_DISABLE_PLUGIN_REGISTRY_MIGRATION=1 npm install --prefix "$staging" --ignore-scripts --omit=dev --no-audit --no-fund >&2
fi
...
function normalizeHandshake(artifact) {
	...
	if (!operatorVersion) throw new Error("Worker Operator version must be non-empty");
	...
	if (artifact.install === "npm") {
		if (!require_npm_registry_spec.isExactSemverVersion(operatorVersion) || artifact.packageSpec !== `openclaw@${operatorVersion}`) throw new Error(`Worker npm install must use exact package openclaw@${operatorVersion}`);
...
function parseReceiptJson(value, expected) {
	...
	if (!require_src.validateWorkerAdmissionHandshake(parsed)) throw new Error("Worker bootstrap returned an invalid receipt");
	if (parsed.bundleHash !== expected.bundleHash || parsed.operatorVersion !== expected.operatorVersion || parsed.protocolFeatures.length !== expected.protocolFeatures.length || parsed.protocolFeatures.some((feature, index) => feature !== expected.protocolFeatures[index])) throw new Error("Worker bootstrap receipt does not match the requested artifact");
	return parsed;
}
function commandFailure(phase, result) {
	const output = (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(require_redact.redactSensitiveText(result.stderr.trim() || result.stdout.trim(), { mode: "tools" }).replace(/\s+/gu, " "), 512);
...
async function runSshScript(params) {
	return await params.runCommand([
		"ssh",
		...require_ssh.workerSshOptions(params.prepared, { forwarding: "disabled" }),
		"-a",
		"-x",
		"-T",
		"-p",
...
function parsePreflight(result, expected) {
	...
	const normalizedPath = require_scp_host.normalizeScpRemotePath(output?.action === "install" ? output.payload : void 0);
...
async function bootstrapWorker(request, dependencies) {
	...
	const runCommand = dependencies.runCommand ?? require_exec.runCommandWithTimeout;
	const prepared = await require_ssh.prepareWorkerSsh({
		ssh: request.ssh,
		pinnedHostKey: request.pinnedHostKey,
		resolveIdentity: dependencies.resolveIdentity,
		temporaryDirectoryPrefix: "operator-worker-bootstrap-"
	});
	try {
...
exports.createWorkerBundleProducer = createWorkerBundleProducer;
exports.resolveSecretRefString = require_resolve.resolveSecretRefString;
exports.resolveWorkerNpmInstallationArtifact = resolveWorkerNpmInstallationArtifact;
exports.resolveWorkerSshIdentity = resolveWorkerSshIdentity;
