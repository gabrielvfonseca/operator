...
function extractProviderFromModelRef(modelRef) {
	const { model } = require_model_ref_profile.splitTrailingAuthProfile(modelRef);
...
function collectLegacyConfigAuthProfileProviderHints(cfg) {
	...
	const addModelHints = (models) => {
		...
		for (const [modelRef, rawModel] of Object.entries(models)) {
			...
			if (!provider || !isSafeLegacyProviderKey(provider) || !(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(rawModel)) continue;
			const agentRuntime = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(rawModel.agentRuntime) ? rawModel.agentRuntime : null;
	...
	for (const { value } of (0, _gabrielvfonseca_model_catalog_core_configured_model_refs.collectConfiguredModelRefs)(cfg)) {
	...
	const root = cfg;
	const auth = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(root.auth) ? root.auth : null;
	const order = auth && (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(auth.order) ? auth.order : null;
	...
	const agents = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(root.agents) ? root.agents : null;
	addModelHints((agents && (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(agents.defaults) ? agents.defaults : null)?.models);
	const agentList = agents && Array.isArray(agents.list) ? agents.list : [];
	for (const agent of agentList) if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(agent)) addModelHints(agent.models);
	return hints;
}
...
function coerceLegacyFlatAuthProfileStore(raw) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw) || "profiles" in raw) return null;
	const store = {
		version: 1,
		profiles: {}
	};
...
function listExistingAgentDirsFromState(env) {
	...
	return entries.filter((entry) => entry.isDirectory()).map((entry) => node_path.default.join(root, entry.name, "agent")).filter((agentDir) => {
		try {
...
function listAuthProfileRepairCandidates(cfg, env) {
	...
	const envAgentDir = readNonEmptyString(env.OPERATOR_AGENT_DIR) ?? readNonEmptyString(env.PI_CODING_AGENT_DIR);
...
function listAuthProfileSqliteMigrationCandidates(cfg, env) {
	...
	for (const candidate of listAuthProfileRepairCandidates(cfg, env)) candidates.push({
		...
		statePath: require_path_resolve.resolveAuthStatePath(candidate.agentDir),
		legacyPath: require_path_resolve.resolveLegacyAuthStorePath(candidate.agentDir)
	});
	return candidates;
}
...
function normalizeLegacyApiKeyAliasesForImport(raw) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw) || !(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw.profiles)) return;
	for (const profile of Object.values(raw.profiles)) {
		...
		if (readNonEmptyString(profile.key) !== void 0 || require_types_secrets.coerceSecretRef(profile.keyRef) !== null || profile["api_key"] === void 0) continue;
...
function inferLegacyConfigAuthProfileMode(raw) {
	...
	if (explicit === "api_key" || explicit === "token" || explicit === "oauth") return explicit;
	if (readNonEmptyString(raw.key) || readNonEmptyString(raw.apiKey) || readNonEmptyString(raw["api_key"]) || require_types_secrets.coerceSecretRef(raw.keyRef) || require_types_secrets.coerceSecretRef(raw.key) || require_types_secrets.coerceSecretRef(raw.apiKey) || require_types_secrets.coerceSecretRef(raw["api_key"])) return "api_key";
	if (readNonEmptyString(raw.token) || require_types_secrets.coerceSecretRef(raw.tokenRef) || require_types_secrets.coerceSecretRef(raw.token)) return "token";
...
function coerceLegacyConfigAuthProfileStore(cfg) {
	const cfgRecord = cfg;
	const auth = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(cfgRecord.auth) ? cfgRecord.auth : null;
	const profiles = auth && (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(auth.profiles) ? auth.profiles : null;
	...
	for (const [profileId, raw] of Object.entries(profiles)) {
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw)) continue;
		...
		if (mode === "api_key") {
			const keyRef = require_types_secrets.coerceSecretRef(raw.keyRef) ?? require_types_secrets.coerceSecretRef(raw.key) ?? require_types_secrets.coerceSecretRef(raw.apiKey) ?? require_types_secrets.coerceSecretRef(raw["api_key"]);
			...
			const tokenRef = require_types_secrets.coerceSecretRef(raw.tokenRef) ?? require_types_secrets.coerceSecretRef(raw.token);
			const token = readNonEmptyString(raw.token);
			if (tokenRef) {
	...
	const canonicalStore = require_persisted.coercePersistedAuthProfileStore(store);
...
function isDefaultAgentCandidate(candidate, cfg, env) {
	return node_path.default.resolve(candidate.agentDir ?? "") === node_path.default.resolve(require_agent_scope_config.resolveDefaultAgentDir(cfg, env));
}
...
function filterRawAuthProfileState(raw, shouldKeepProfileId) {
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw.order)) {
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw.lastGood)) {
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw.usageStats)) {
...
function collectUnresolvedLegacyOAuthSidecarProfileIds(raw) {
	...
	for (const [profileId, profile] of Object.entries(raw.profiles)) {
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(profile) || profile.type !== "oauth" || !(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(profile.oauthRef)) continue;
...
function hasLegacyAuthProfileSource(candidate) {
	return node_fs.default.existsSync(candidate.authPath) || node_fs.default.existsSync(candidate.statePath) || node_fs.default.existsSync(candidate.legacyPath);
}
function backupAuthProfileJson(pathname, suffix, now) {
	const backupPath = `${pathname}.${suffix}.${now()}.bak`;
	node_fs.default.copyFileSync(pathname, backupPath);
	return backupPath;
}
function backupAndRemoveAuthProfileJson(pathname, suffix, now) {
	const backupPath = backupAuthProfileJson(pathname, suffix, now);
	node_fs.default.unlinkSync(pathname);
	return backupPath;
}
function writeJsonFile(pathname, value) {
	node_fs.default.writeFileSync(pathname, `${JSON.stringify(value, null, 2)}\n`, "utf8");
}
/**
...
async function maybeMigrateAuthProfileJsonStoresToSqlite(params) {
	...
	const loadMigratedStore = params.deps?.loadPersistedAuthProfileStore ?? require_persisted.loadPersistedAuthProfileStore;
	...
	const result = {
		...
		].filter((pathname, index, entries) => entries.indexOf(pathname) === index).filter((pathname) => node_fs.default.existsSync(pathname) || configStore && isDefaultAgentCandidate(candidate, params.cfg, env) && pathname === candidate.authPath)),
		changes: [],
		warnings: []
	};
	if (detected.length === 0) return result;
	require_note.note([...detected.map((candidate) => `- ${require_utils.shortenHomePath(candidate.authPath)} / ${require_utils.shortenHomePath(candidate.statePath)}`), `- ${require_command_format.formatCliCommand("openclaw doctor --fix")} imports legacy auth profile JSON into the per-agent SQLite database and removes the old files after backup.`].join("\n"), "Auth profile SQLite migration");
	...
	for (const candidate of detected) try {
		const rawStore = node_fs.default.existsSync(candidate.authPath) ? require_json_file.loadJsonFile(candidate.authPath) : null;
		...
		if (unresolvedSidecarProfileIds.size > 0) {
			...
			result.warnings.push(`Left ${unresolvedSidecarProfileIds.size} legacy OAuth sidecar profile${unresolvedSidecarProfileIds.size === 1 ? "" : "s"} in ${require_utils.shortenHomePath(candidate.authPath)}; rerun ${require_command_format.formatCliCommand("openclaw doctor --fix")} after sidecar migration or re-authenticate those profiles.`);
		}
		const awsSdkMarkerStore = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(rawStore) && (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(rawStore.profiles) ? resolveAwsSdkAuthProfileMarkerStore(candidate) : null;
		if (awsSdkMarkerStore && (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(rawStore)) {
		...
		const maybeCanonicalStore = require_persisted.coercePersistedAuthProfileStore(rawStore) ?? coerceLegacyFlatAuthProfileStore(rawStore) ?? null;
		...
		const legacyStore = require_persisted.loadLegacyAuthProfileStore(candidate.agentDir);
		const state = require_persisted.coerceAuthProfileState(node_fs.default.existsSync(candidate.statePath) ? require_json_file.loadJsonFile(candidate.statePath) : null);
		if (!canonicalStore && !configCanonicalStore && !legacyStore && !hasAuthProfileState(state) && !awsSdkMarkerStore) {
			result.warnings.push(`Left auth profile JSON in place for ${require_utils.shortenHomePath(candidate.authPath)} because no importable auth profiles or state were found.`);
			continue;
		}
		...
		const existingState = require_persisted.coerceAuthProfileState(existing);
		let next = { ...existing };
		...
		if (legacyStore) {
			...
			require_persisted.applyLegacyAuthStore(legacyAsStore, legacyStore);
		...
		if (canonicalStore) {
			...
			next = mergeImportedAuthProfileState({
				store: next,
				state: require_persisted.coerceAuthProfileState(canonicalStore),
				existingState
			});
		}
		...
		if (canonicalStore || configCanonicalStore || legacyStore || hasAuthProfileState(state)) {
			const stateProfileIds = [
				...
				...canonicalStore ? collectAuthProfileStateProfileIds(require_persisted.coerceAuthProfileState(canonicalStore)) : [],
				...configCanonicalStore ? collectAuthProfileStateProfileIds(require_persisted.coerceAuthProfileState(configCanonicalStore)) : []
			];
			require_store.saveAuthProfileStore(next, candidate.agentDir, {
			...
			if (verificationFailure) {
				result.warnings.push(`Left auth profile JSON in place for ${require_utils.shortenHomePath(candidate.authPath)} because SQLite verification did not find ${verificationFailure}.`);
				continue;
			}
		...
		const backups = [];
		if (node_fs.default.existsSync(candidate.authPath)) if (unresolvedSidecarRawStore) {
		...
		if (node_fs.default.existsSync(candidate.statePath)) backups.push(backupAndRemoveAuthProfileJson(candidate.statePath, "sqlite-import", now));
		if (node_fs.default.existsSync(candidate.legacyPath)) backups.push(backupAndRemoveAuthProfileJson(candidate.legacyPath, "sqlite-import", now));
		const backupText = backups.length > 0 ? `backup${backups.length === 1 ? "" : "s"}: ${backups.map(require_utils.shortenHomePath).join(", ")}` : "no legacy JSON backup needed";
		result.changes.push(`Migrated auth profile JSON for ${require_utils.shortenHomePath(candidate.authPath)} into SQLite (${backupText}).`);
		if (awsSdkMarkerStore) result.changes.push(`Moved aws-sdk profile metadata from ${require_utils.shortenHomePath(candidate.authPath)} to auth.profiles before removing the legacy auth profile JSON.`);
	} catch (err) {
		result.warnings.push(`Failed to migrate auth profile JSON for ${require_utils.shortenHomePath(candidate.authPath)}: ${String(err)}`);
	}
	require_store.clearRuntimeAuthProfileStoreSnapshots();
	if (result.changes.length > 0) require_note.note(result.changes.map((change) => `- ${change}`).join("\n"), "Doctor changes");
	if (result.warnings.length > 0) require_note.note(result.warnings.map((warning) => `- ${warning}`).join("\n"), "Doctor warnings");
	return result;
}
...
function removeAwsSdkProfileMarkers(raw, profileIds) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw.profiles)) return;
	for (const profileId of profileIds) delete raw.profiles[profileId];
}
/**
...
async function maybeRepairLegacyFlatAuthProfileStores(params) {
	...
	const noteLines = [...legacyStores.map((entry) => `- ${require_utils.shortenHomePath(entry.authPath)} uses the legacy flat auth profile format.`), ...awsSdkMarkerStores.map((entry) => `- ${require_utils.shortenHomePath(entry.authPath)} contains aws-sdk profile markers that belong in operator.json auth.profiles.`)];
	if (legacyStores.length > 0) noteLines.push(`- The gateway expects the canonical version/profiles store; ${require_command_format.formatCliCommand("openclaw doctor --fix")} rewrites this legacy shape with a backup.`);
	if (awsSdkMarkerStores.length > 0) noteLines.push(`- AWS SDK profile markers are routing metadata, not stored credentials; ${require_command_format.formatCliCommand("openclaw doctor --fix")} moves them to config with a backup.`);
	require_note.note(noteLines.join("\n"), "Auth profiles");
	if (!await params.prompter.confirmAutoFix({
	...
	for (const entry of legacyStores) try {
		...
		require_store.saveAuthProfileStore(entry.store, entry.agentDir, { syncExternalCli: false });
		node_fs.default.unlinkSync(entry.authPath);
		result.changes.push(`Migrated ${require_utils.shortenHomePath(entry.authPath)} to the SQLite auth profile store (backup: ${require_utils.shortenHomePath(backupPath)}).`);
	} catch (err) {
		result.warnings.push(`Failed to rewrite ${require_utils.shortenHomePath(entry.authPath)}: ${String(err)}`);
	}
	for (const entry of awsSdkMarkerStores) try {
		...
		node_fs.default.writeFileSync(entry.authPath, `${JSON.stringify(entry.raw, null, 2)}\n`);
		result.changes.push(`Moved aws-sdk profile metadata from ${require_utils.shortenHomePath(entry.authPath)} to auth.profiles (backup: ${require_utils.shortenHomePath(backupPath)}).`);
	} catch (err) {
		result.warnings.push(`Failed to migrate aws-sdk profile markers from ${require_utils.shortenHomePath(entry.authPath)}: ${String(err)}`);
	}
	require_store.clearRuntimeAuthProfileStoreSnapshots();
	if (result.changes.length > 0) require_note.note(result.changes.map((change) => `- ${change}`).join("\n"), "Doctor changes");
	if (result.warnings.length > 0) require_note.note(result.warnings.map((warning) => `- ${warning}`).join("\n"), "Doctor warnings");
...
function resolveCanonicalApiKeyAliasRepair(candidate) {
	...
	for (const [profileId, value] of Object.entries(raw.profiles)) {
		...
		const hasApiKeyField = readNonEmptyString(value["api_key"]) !== void 0 || require_types_secrets.coerceSecretRef(value["api_key"]) !== null;
		const hasCanonicalKey = readNonEmptyString(value.key) !== void 0 || require_types_secrets.coerceSecretRef(value.key) !== null;
		const hasCanonicalKeyRef = require_types_secrets.coerceSecretRef(value.keyRef) !== null;
...
async function maybeRepairCanonicalApiKeyFieldAlias(params) {
	...
	const noteLines = repairs.map((entry) => `- ${require_utils.shortenHomePath(entry.authPath)} has ${entry.profileIds.length} profile(s) using the non-canonical "api_key" field; the canonical field is "key".`);
	noteLines.push(`- Runtime auth parsing only reads canonical "key" and "keyRef" fields, so these profiles are silently skipped; ${require_command_format.formatCliCommand("openclaw doctor --fix")} rewrites "api_key" to "key" with a backup.`);
	require_note.note(noteLines.join("\n"), "Auth profiles");
	if (!await params.prompter.confirmAutoFix({
	...
	for (const entry of repairs) try {
		...
		node_fs.default.writeFileSync(entry.authPath, `${JSON.stringify(entry.raw, null, 2)}\n`);
		result.changes.push(`Rewrote ${entry.profileIds.length} "api_key" field(s) to "key" in ${require_utils.shortenHomePath(entry.authPath)} (backup: ${require_utils.shortenHomePath(backupPath)}).`);
	} catch (err) {
		result.warnings.push(`Failed to rewrite "api_key" fields in ${require_utils.shortenHomePath(entry.authPath)}: ${String(err)}`);
	}
	require_store.clearRuntimeAuthProfileStoreSnapshots();
	if (result.changes.length > 0) require_note.note(result.changes.map((change) => `- ${change}`).join("\n"), "Doctor changes");
	if (result.warnings.length > 0) require_note.note(result.warnings.map((warning) => `- ${warning}`).join("\n"), "Doctor warnings");
...
function canonicalizeOpenAIAuthOrder(auth, profileIdMap) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(auth.order)) return false;
...
function maybeRepairOpenAICodexAuthConfig(cfg, options) {
	...
	const root = config;
	const auth = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(root.auth) ? root.auth : void 0;
	const profileIdMap = new Map(options?.profileIdMap);
	let changed = false;
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(auth?.profiles)) {
...
function resolveOpenAICodexAuthStoreRepair(candidate, profileIdMap) {
	...
	const usageChanged = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw.usageStats) ? renameMappedProfileIdKeys(raw.usageStats, rewrite.profileIdMap) : false;
	const lastGoodChanged = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw.lastGood) ? canonicalizeOpenAILastGood(raw.lastGood, rewrite.profileIdMap) : false;
...
function collectOpenAICodexAuthProfileStoreIdMap(params) {
	...
	for (const candidate of listAuthProfileRepairCandidates(params.cfg, env)) {
		if (!node_fs.default.existsSync(candidate.authPath)) continue;
		const raw = require_json_file.loadJsonFile(candidate.authPath);
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw) || !(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw.profiles)) continue;
...
async function maybeRepairOpenAICodexAuthProfileStores(params) {
	...
	for (const entry of repairs) try {
		...
		result.changes.push(`Migrated ${movedCount} OpenAI Codex auth profile(s) in ${require_utils.shortenHomePath(entry.authPath)} to provider "openai" (backup: ${require_utils.shortenHomePath(backupPath)}).`);
	} catch (err) {
		result.warnings.push(`Failed to migrate OpenAI Codex auth profiles in ${require_utils.shortenHomePath(entry.authPath)}: ${String(err)}`);
	}
	require_store.clearRuntimeAuthProfileStoreSnapshots();
	return result;
}
//#endregion
exports.collectOpenAICodexAuthProfileStoreIdMap = collectOpenAICodexAuthProfileStoreIdMap;
exports.maybeMigrateAuthProfileJsonStoresToSqlite = maybeMigrateAuthProfileJsonStoresToSqlite;
exports.maybeRepairCanonicalApiKeyFieldAlias = maybeRepairCanonicalApiKeyFieldAlias;
exports.maybeRepairLegacyFlatAuthProfileStores = maybeRepairLegacyFlatAuthProfileStores;
exports.maybeRepairOpenAICodexAuthConfig = maybeRepairOpenAICodexAuthConfig;
exports.maybeRepairOpenAICodexAuthProfileStores = maybeRepairOpenAICodexAuthProfileStores;
