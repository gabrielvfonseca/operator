...
function getNestedRecord(record, key) {
	const value = record?.[key];
	return (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(value);
}
...
function extractCanvasFromDetails(value) {
	return coerceCanvasPreview((0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)((0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(value)?.mcpAppPreview));
}
...
function extractCanvasFromText(outputText, _toolName) {
	return coerceCanvasPreview(outputText ? (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)((0, _gabrielvfonseca_normalization_core.safeParseJson)(outputText)) : void 0);
}
//#endregion
...
function truncateChatHistoryText(text, maxChars = DEFAULT_CHAT_HISTORY_TEXT_MAX_CHARS) {
	...
	return {
		text: `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(text, maxChars)}\n...(truncated)...`,
		truncated: true
	};
}
...
function projectToolResultDetails(details, maxChars) {
	const record = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(details);
	if (!record) return;
	const projected = {};
...
function extractChatHistoryCanvasPreview(message) {
	...
	for (const block of message.content) {
		const preview = extractCanvasFromDetails((0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(block)?.details);
...
function sanitizeChatHistoryContentBlock(block, opts) {
	...
	if (typeof entry.text === "string") {
		const stripped = require_directive_tags.stripInlineDirectiveTagsForDisplay(entry.text);
	...
	if (typeof entry.content === "string") {
		const stripped = require_directive_tags.stripInlineDirectiveTagsForDisplay(entry.content);
		if (preserveExactToolPayload) {
	...
	if (type === "image" && typeof entry.data === "string") {
		const bytes = (0, _gabrielvfonseca_media_core_base64.estimateBase64DecodedBytes)(entry.data);
		delete entry.data;
		entry.omitted = true;
		entry.bytes = bytes;
		changed = true;
	...
	if (type === "audio" && entry.source && typeof entry.source === "object") {
		...
		if (source.type === "base64" && typeof source.data === "string") {
			const bytes = (0, _gabrielvfonseca_media_core_base64.estimateBase64DecodedBytes)(source.data);
			delete source.data;
			source.omitted = true;
			source.bytes = bytes;
			entry.source = source;
...
function sanitizeAssistantPhasedContentBlocks(content) {
	if (!content.some((block) => {
		if (!block || typeof block !== "object") return false;
		const entry = block;
		return entry.type === "text" && Boolean(require_chat_message_content.parseAssistantTextSignature(entry.textSignature)?.phase);
	})) return {
		content,
		changed: false
	};
	const filtered = content.filter((block) => {
		...
		return require_chat_message_content.parseAssistantTextSignature(entry.textSignature)?.phase === "final_answer";
	});
	return {
		content: filtered,
...
function projectAssistantTextFromMixedToolContent(content, maxChars) {
	...
	for (const block of content) {
		...
		const truncated = truncateChatHistoryText(require_directive_tags.stripInlineDirectiveTagsForDisplay(entry.text).text, maxChars);
...
function toFiniteNumber(x) {
	return (0, _gabrielvfonseca_normalization_core_number_coercion.asFiniteNumber)(x);
}
...
function parseJsonRecord(value) {
	try {
		return (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(JSON.parse(value));
	} catch {
		return;
	}
}
...
function readToolBlockName(block) {
	const direct = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(block.name) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(block.toolName) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(block.tool_name) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(block.tool);
	if (direct) return direct;
	const fn = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(block.function);
	return fn ? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(fn.name) : void 0;
}
function readToolBlockCallId(block) {
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(block.id) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(block.toolCallId) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(block.tool_call_id) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(block.callId) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(block.call_id);
}
...
function readMessageToolVisibleText(args) {
	for (const field of [
		...
		if (typeof value === "string" && value.trim()) return require_directive_tags.stripInlineDirectiveTagsForDisplay(value).text;
	}
}
function isDryRunMessageToolRecord(record) {
	if (record.dryRun === true || record.dry_run === true) return true;
	return ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(record.deliveryStatus) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(record.delivery_status) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(record.status))?.toLowerCase() === "dry_run";
}
function extractMessageToolVisibleReplies(message) {
	if (message.role !== "assistant" || !Array.isArray(message.content)) return [];
	const replies = [];
	for (const block of message.content) {
		const record = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(block);
		if (!record) continue;
		...
		if ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(args.action)?.toLowerCase() !== "send") continue;
...
function isAssistantSilentControlReplyOnly(message) {
	...
	return text !== void 0 && require_control_reply_text.isSuppressedControlReplyText(text) && !hasAssistantNonTextContent(message);
}
function isRenderableAssistantDisplayMessage(message) {
	if (message.role !== "assistant") return false;
	const text = extractAssistantTextForSilentCheck(message);
	return text !== void 0 && !require_control_reply_text.isSuppressedControlReplyText(text);
}
function readMessageToolResultName(message) {
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(message.toolName) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(message.tool_name) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(message.name) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(message.tool);
}
function readMessageToolResultCallId(message) {
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(message.toolCallId) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(message.tool_call_id) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(message.callId) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(message.call_id) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(message.id);
}
function readToolResultOkValue(value) {
	...
	if (Array.isArray(value)) for (const block of value) {
		...
		const recordBlock = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(block);
...
function readMessageToolSourceReplySink(message) {
	return (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(message.details)?.sourceReplySink === "internal-ui" ? "internal-ui" : void 0;
}
function buildMessageToolVisibleReplyMirror(pending) {
	const sourceMessageSeq = (0, _gabrielvfonseca_normalization_core_number_coercion.asPositiveSafeInteger)((0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(pending.anchor["__openclaw"])?.seq);
	const mirror = {
		role: "assistant",
		content: [{
			type: "text",
	...
	const transcriptMeta = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)((pending.completionAnchor ?? pending.anchor)["__openclaw"]);
	if (transcriptMeta) mirror["__openclaw"] = { ...transcriptMeta };
	return mirror;
}
function readMessageToolDeliveryMirrorText(message) {
	if (!require_transcript_only_openclaw_assistant.isOperatorDeliveryMirrorAssistantMessage(message)) return;
	return displayTextForDuplicateCheck(message);
}
function mirrorMessageToolVisibleReplies(messages) {
	if (messages.length === 0) return messages;
	if (!messages.some((message) => (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(message))) return messages;
	let changed = false;
	const next = [];
	const pending = [];
	const clearPending = () => {
...
function shouldDropAssistantHistoryMessage(message) {
	...
	if (require_chat_message_content.resolveAssistantMessagePhase(message) === "commentary") return !hasAssistantMixedToolVisibleText(message);
	const text = extractAssistantTextForSilentCheck(message);
	if (text === void 0 || !require_control_reply_text.isSuppressedControlReplyText(text)) return false;
...
function digestTtsSupplementText(text) {
	return (0, node_crypto.createHash)("sha256").update(text.trim()).digest("hex");
}
function readTtsSupplementMarker(message) {
	const marker = message.operatorTtsSupplement;
...
function mergeTtsSupplementMessages(messages) {
	...
	for (const message of messages) {
		...
		if (marker && isAssistantTtsSupplementMessage(message)) {
			...
			if (targetIndex >= 0) {
				merged[targetIndex] = mergeTtsSupplementContent((0, _gabrielvfonseca_normalization_core.expectDefined)(merged[targetIndex], "merged entry at target index"), message);
				changed = true;
				continue;
			}
		}
...
function readChatHistoryRecordTimestampMs(message) {
	const value = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)((0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(message)?.["__openclaw"])?.recordTimestampMs;
	if (typeof value === "number" && Number.isFinite(value)) return value;
	const timestamp = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(message)?.timestamp;
...
function isSubagentAnnounceInterSessionUserChatHistoryMessage(message) {
	...
	const provenance = require_input_provenance.normalizeInputProvenance(record.provenance);
	if (provenance?.kind === "inter_session" && provenance.sourceTool === "subagent_announce") return true;
...
function isChatHistoryAssistantMessage(message) {
	return (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(message)?.role === "assistant";
}
...
function isDisplayHiddenProjectedMessage(message) {
	...
	return message.role === "custom" && message.customType === "operator.runtime-context";
}
function shouldHideProjectedHistoryMessage(message) {
	...
	if (require_heartbeat_filter.isHeartbeatUserMessage(roleContent, require_heartbeat.HEARTBEAT_PROMPT)) return true;
	return require_heartbeat_filter.isHeartbeatOkResponse(roleContent);
}
...
function isHeartbeatHistoryTurnBoundaryMessage(message) {
	...
	return roleContent?.role === "user" && require_heartbeat_filter.isHeartbeatUserMessage(roleContent, require_heartbeat.HEARTBEAT_PROMPT);
}
function attachProjectedTurnBoundary(message) {
	const metadata = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(message["__openclaw"]);
...
function openclawAssistantModel(message) {
	return message.role === "assistant" && message.provider === "@gabrielvfonseca/operator" && typeof message.model === "string" ? message.model : void 0;
}
...
function isDuplicateChannelFinalDeliveryMirror(current, previousVisible) {
	if (!previousVisible || !require_transcript_only_openclaw_assistant.isOperatorDeliveryMirrorAssistantMessage(current)) return false;
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(current.operatorDeliveryMirror)?.kind !== "channel-final") return false;
	if (asRoleContentMessage(previousVisible)?.role !== "assistant") return false;
	if (require_transcript_only_openclaw_assistant.isOperatorDeliveryMirrorAssistantMessage(previousVisible)) return false;
	if (isProjectedSessionsSendForwardedMessage(previousVisible)) return false;
	const previousMeta = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(previousVisible["__openclaw"]);
...
function filterVisibleProjectedHistoryMessages(messages, turnBoundaryPending = false) {
	...
	for (let i = 0; i < messages.length; i++) {
		...
		if (currentRoleContent && next && nextRoleContent && require_heartbeat_filter.isHeartbeatUserMessage(currentRoleContent, require_heartbeat.HEARTBEAT_PROMPT) && require_heartbeat_filter.isHeartbeatOkResponse(nextRoleContent) && !isProjectedSessionsSendForwardedMessage(next)) {
			changed = true;
...
function stripInterSessionPromptPrefixFromContent(content) {
	if (typeof content === "string") return require_input_provenance.stripInterSessionPromptPrefixForDisplay(content);
	if (!Array.isArray(content)) return content;
	return content.map((block) => {
		if (!block || typeof block !== "object" || Array.isArray(block)) return block;
		const record = block;
		if (typeof record.text !== "string") return block;
		const stripped = require_input_provenance.stripInterSessionPromptPrefixForDisplay(record.text);
...
function extractPromptPrefixField(text, field) {
	const prefixIndex = text.indexOf(require_input_provenance.INTER_SESSION_PROMPT_PREFIX_BASE);
	if (prefixIndex === -1) return;
	const lineEnd = text.indexOf("\n", prefixIndex);
	const header = lineEnd === -1 ? text.slice(prefixIndex) : text.slice(prefixIndex, lineEnd);
	const escapedField = field.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(new RegExp(`(?:^|\\s)${escapedField}=([^\\s]+)`).exec(header)?.[1]);
}
function resolveSessionsSendForwardedSenderLabel(message) {
	const provenance = require_input_provenance.normalizeInputProvenance(message.provenance);
	const text = extractProjectedText(message.content ?? message.text);
	const agentId = require_session_key.parseAgentSessionKey(provenance?.sourceSessionKey ?? extractPromptPrefixField(text, "sourceSession"))?.agentId;
...
function projectSessionsSendInterSessionMessages(messages) {
	...
	const projected = messages.map((message) => {
		...
		if (typeof next.text === "string") next.text = require_input_provenance.stripInterSessionPromptPrefixForDisplay(next.text);
		return next;
	});
...
function projectEmptyAssistantErrorMessages(messages) {
	...
	const projected = messages.map((message) => {
		...
		const hasVisibleReplyText = visibleTexts.map((text) => text.trim()).filter(Boolean).some((text) => text !== "[assistant turn failed before producing content]" && !require_control_reply_text.isSuppressedControlReplyText(text));
...
//#endregion
Object.defineProperty(exports, "DEFAULT_CHAT_HISTORY_TEXT_MAX_CHARS", {
	enumerable: true,
	get: function() {
		return DEFAULT_CHAT_HISTORY_TEXT_MAX_CHARS;
	}
});
Object.defineProperty(exports, "augmentChatHistoryWithCanvasBlocks", {
	enumerable: true,
	get: function() {
		return augmentChatHistoryWithCanvasBlocks;
	}
});
Object.defineProperty(exports, "dropPreSessionStartAnnouncePairs", {
	enumerable: true,
	get: function() {
		return dropPreSessionStartAnnouncePairs;
	}
});
Object.defineProperty(exports, "isHeartbeatHistoryTurnBoundaryMessage", {
	enumerable: true,
	get: function() {
		return isHeartbeatHistoryTurnBoundaryMessage;
	}
});
Object.defineProperty(exports, "projectChatDisplayMessage", {
	enumerable: true,
	get: function() {
		return projectChatDisplayMessage;
	}
});
Object.defineProperty(exports, "projectChatDisplayMessages", {
	enumerable: true,
	get: function() {
		return projectChatDisplayMessages;
	}
});
Object.defineProperty(exports, "projectChatDisplayMessagesWithState", {
	enumerable: true,
	get: function() {
		return projectChatDisplayMessagesWithState;
	}
});
Object.defineProperty(exports, "projectRecentChatDisplayMessages", {
	enumerable: true,
	get: function() {
		return projectRecentChatDisplayMessages;
	}
});
Object.defineProperty(exports, "resolveEffectiveChatHistoryMaxChars", {
	enumerable: true,
	get: function() {
		return resolveEffectiveChatHistoryMaxChars;
	}
});
Object.defineProperty(exports, "resolveTranscriptPathForComparison", {
	enumerable: true,
	get: function() {
		return resolveTranscriptPathForComparison;
	}
});
Object.defineProperty(exports, "sanitizeChatHistoryMessages", {
	enumerable: true,
	get: function() {
