...
function isCurrentProcessLaunchdServiceLabel(label, env = process.env, options = {}) {
	...
	const configuredLabel = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(env.OPERATOR_LAUNCHD_LABEL);
	if (!configuredLabel || configuredLabel !== label) return false;
	if ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(env.OPERATOR_SERVICE_MARKER) === "@gabrielvfonseca/operator" && (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(env.OPERATOR_SERVICE_KIND)) return true;
...
async function readLaunchAgentEnvironmentFile(programArguments, options) {
	...
	for (const candidate of candidateEnvFilePaths) try {
		content = await node_fs_promises.default.readFile(candidate, "utf8");
		break;
	} catch {}
	if (!content) return {};
	const environment = {};
...
async function readLaunchAgentProgramArgumentsFromFile(plistPath, options) {
	try {
		const plist = await node_fs_promises.default.readFile(plistPath, "utf8");
...
function assertValidLaunchAgentLabel$1(label) {
	...
	if (!/^[A-Za-z0-9._-]+$/.test(trimmed)) throw new Error(`Invalid launchd label: ${require_ansi.sanitizeForLog(trimmed)}`);
	return trimmed;
}
...
function resolveLaunchAgentLabel$1(env) {
	const envLabel = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(env?.OPERATOR_LAUNCHD_LABEL);
	if (envLabel) return assertValidLaunchAgentLabel$1(envLabel);
	return assertValidLaunchAgentLabel$1(require_paths$1.resolveGatewayLaunchAgentLabel(env?.OPERATOR_PROFILE));
}
function resolveLaunchdRestartTarget(env = process.env) {
	...
	const home = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(env.HOME) || node_os.default.homedir();
	return {
		domain,
		label,
...
label="$5"
${require_restart_logs.renderPosixRestartLogSetup(restartLogEnv)}
...
function scheduleDetachedLaunchdRestartHandoff(params) {
	...
	const restartEnv = require_host_env_security.sanitizeHostExecEnv({
		baseEnv: process.env,
		overrides: collectStringEnvOverrides(params.env)
	});
	try {
		const child = (0, node_child_process.spawn)("/bin/sh", [
			"-c",
		...
		child.unref();
		return (0, _gabrielvfonseca_normalization_core_result.ok)(child.pid ?? void 0);
	} catch (error) {
		return (0, _gabrielvfonseca_normalization_core_result.err)(require_errors.formatErrorMessage(error));
	}
}
//#endregion
//#region src/daemon/launchd.ts
/** macOS LaunchAgent installer, runtime inspection, and lifecycle controls. */
var launchd_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	disableCurrentOperatorUpdateLaunchdJob: () => disableCurrentOperatorUpdateLaunchdJob,
	disableOperatorUpdateLaunchdJob: () => disableOperatorUpdateLaunchdJob,
	findStaleOperatorUpdateLaunchdJobs: () => findStaleOperatorUpdateLaunchdJobs,
	formatLaunchAgentGuiSessionError: () => formatLaunchAgentGuiSessionError,
	...
	parseLaunchctlListOperatorUpdateJobs: () => parseLaunchctlListOperatorUpdateJobs,
	parseLaunchctlPrint: () => parseLaunchctlPrint,
	readLaunchAgentProgramArguments: () => readLaunchAgentProgramArguments,
	readLaunchAgentRuntime: () => readLaunchAgentRuntime,
	repairLaunchAgentBootstrap: () => repairLaunchAgentBootstrap,
	resolveLaunchAgentPlistPath: () => resolveLaunchAgentPlistPath,
	restartLaunchAgent: () => restartLaunchAgent,
	stageLaunchAgent: () => stageLaunchAgent,
	stopLaunchAgent: () => stopLaunchAgent,
...
const OPERATOR_PROFILE_UPDATE_LAUNCHD_LABEL_PATTERN = /^ai\.operator\.[A-Za-z0-9._-]+\.update\.[A-Za-z0-9._-]+$/;
const OPERATOR_DIRECT_CLI_NAMES = /* @__PURE__ */ new Set(["@gabrielvfonseca/operator", "operator.mjs"]);
const OPERATOR_NODE_RUNTIME_NAMES = /* @__PURE__ */ new Set([
	"bun",
	"bun.exe",
	"node",
	"node.exe"
]);
const OPERATOR_SCRIPT_NAMES = /* @__PURE__ */ new Set(["operator.mjs"]);
const LAUNCH_AGENT_STOP_PORT_RELEASE_TIMEOUT_MS = 20 * 1e3;
const LAUNCH_AGENT_STOP_PORT_RELEASE_POLL_MS = 100;
function normalizeOperatorUpdateLaunchdLabel(label) {
	if (typeof label !== "string") return null;
	const trimmed = label.trim();
	if (trimmed.startsWith(OPERATOR_UPDATE_LAUNCHD_LABEL_PREFIX)) return trimmed;
	return OPERATOR_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN.test(trimmed) ? trimmed : null;
}
function normalizeOperatorUpdateLaunchdLabelCandidate(label) {
	const normalized = normalizeOperatorUpdateLaunchdLabel(label);
...
function isCurrentGatewayLaunchdLabel(label, env) {
	if (label === require_paths$1.resolveGatewayLaunchAgentLabel(env.OPERATOR_PROFILE)) return true;
	if (env.OPERATOR_SERVICE_MARKER?.trim() !== "@gabrielvfonseca/operator" || env.OPERATOR_SERVICE_KIND?.trim() !== "gateway") return false;
	const configuredLabel = env.OPERATOR_LAUNCHD_LABEL?.trim();
	return Boolean(configuredLabel && label === configuredLabel);
}
function resolveCurrentOperatorUpdateLaunchdJobLabel(env = process.env) {
	for (const label of [
		env.LAUNCH_JOB_LABEL,
		env.LAUNCH_JOB_NAME,
		env.XPC_SERVICE_NAME,
		...
		const candidate = normalizeOperatorUpdateLaunchdLabelCandidate(label);
		if (candidate) {
...
function resolveLaunchAgentLabel(args) {
	...
	return assertValidLaunchAgentLabel(require_paths$1.resolveGatewayLaunchAgentLabel(args?.env?.OPERATOR_PROFILE));
}
function resolveLaunchAgentPlistPathForLabel(env, label) {
	...
	return node_path.default.posix.join(home, "Library", "LaunchAgents", `${label}.plist`);
}
function resolveLaunchAgentEnvDir(env) {
	return node_path.default.join(require_paths$1.resolveGatewayStateDir(env), LAUNCH_AGENT_ENV_DIR_NAME);
}
function resolveLaunchAgentEnvFilePath(env, label) {
	return node_path.default.join(resolveLaunchAgentEnvDir(env), `${label}.env`);
}
function resolveLaunchAgentEnvWrapperPath(env, label) {
	return node_path.default.join(resolveLaunchAgentEnvDir(env), `${label}-env-wrapper.sh`);
}
...
function buildLaunchAgentEnvironmentFile(entries) {
	return [
		"# Generated by Operator. Do not edit while the gateway service is installed.",
...
async function resolveLaunchAgentEnvironmentWrapperOverwriteWarnings(params) {
	const existingWrapper = await node_fs_promises.default.readFile(params.wrapperPath, "utf8").catch(() => null);
	if (existingWrapper === null || existingWrapper === params.generatedWrapper) return [];
	return [`Existing generated LaunchAgent env wrapper at ${params.wrapperPath} contains custom behavior and will be overwritten; move custom behavior to openclaw gateway install --wrapper <path> or OPERATOR_WRAPPER.`];
}
function writeLaunchAgentOverwriteWarnings(stdout, warn, warnings) {
	for (const warning of warnings) {
		...
		if (!stdout) continue;
		stdout.write(`${require_runtime_parse.formatLine("Warning", warning)}\n`);
	}
}
...
async function prepareLaunchAgentProgramArguments(params) {
	...
	await node_fs_promises.default.writeFile(envFilePath, buildLaunchAgentEnvironmentFile(entries), {
		encoding: "utf8",
		mode: LAUNCH_AGENT_ENV_FILE_MODE
	});
	await node_fs_promises.default.chmod(envFilePath, LAUNCH_AGENT_ENV_FILE_MODE).catch(() => void 0);
	...
	await node_fs_promises.default.writeFile(wrapperPath, generatedWrapper, {
		encoding: "utf8",
		mode: LAUNCH_AGENT_ENV_WRAPPER_MODE
	});
	await node_fs_promises.default.chmod(wrapperPath, LAUNCH_AGENT_ENV_WRAPPER_MODE).catch(() => void 0);
...
function buildLaunchAgentPlist({ label = require_paths$1.GATEWAY_LAUNCH_AGENT_LABEL, comment, programArguments, workingDirectory, stdoutPath, stderrPath, environment }) {
...
async function execLaunchctl(args) {
	...
	return await require_exec_file.execFileUtf8(isWindows ? require_windows_install_roots.getWindowsCmdExePath() : "launchctl", isWindows ? [
		"/d",
		"/s",
		"/c",
		"launchctl",
...
function parseLaunchctlListOperatorUpdateJobs(output) {
	return parseLaunchctlListOperatorUpdateJobCandidates(output).filter((job) => !job.requiresMetadata).map(({ requiresMetadata: _requiresMetadata, ...job }) => job);
}
function parseLaunchctlListOperatorUpdateJobCandidates(output) {
	const jobs = [];
	for (const rawLine of output.split(/\r?\n/)) {
		const line = rawLine.trim();
		if (!line) continue;
		const [pidRaw, statusRaw, ...labelParts] = line.split(/\s+/);
		const candidate = normalizeOperatorUpdateLaunchdLabelCandidate(labelParts.join(" "));
		if (!candidate) continue;
		const pid = pidRaw === "-" ? void 0 : require_parse_finite_number.parseStrictPositiveInteger(pidRaw ?? "");
		const lastExitStatus = require_parse_finite_number.parseStrictInteger(statusRaw ?? "");
		jobs.push({
...
function hasOperatorUpdateLaunchdMarker(env) {
	return env?.OPERATOR_UPDATE_RUN_HANDOFF?.trim() === "1";
}
function isOperatorUpdateCommandPrefix(programArguments, updateIndex) {
	if (updateIndex === 1) {
		const cliName = node_path.default.basename(programArguments[0] ?? "").toLowerCase();
		return OPERATOR_DIRECT_CLI_NAMES.has(cliName);
	}
	if (updateIndex !== 2) return false;
	const runtimeName = node_path.default.basename(programArguments[0] ?? "").toLowerCase();
	const entryName = node_path.default.basename(programArguments[1] ?? "").toLowerCase();
	return OPERATOR_NODE_RUNTIME_NAMES.has(runtimeName) && OPERATOR_SCRIPT_NAMES.has(entryName);
}
function isOperatorUpdateProgramArguments(programArguments) {
	...
	return isOperatorUpdateCommandPrefix(programArguments, updateIndex) && !programArguments.some((arg) => arg.trim() === "gateway");
}
async function isLaunchdJobConfirmedOperatorUpdater(params) {
	const command = await readLaunchAgentProgramArgumentsFromFile(resolveLaunchAgentPlistPathForLabel(params.env, params.label));
	return hasOperatorUpdateLaunchdMarker(command?.environment) || isOperatorUpdateProgramArguments(command?.programArguments);
}
async function findStaleOperatorUpdateLaunchdJobs(env = process.env) {
	if (process.platform !== "darwin") return [];
	const result = await execLaunchctl(["list"]);
	if (result.code !== 0) return [];
	const jobs = [];
	for (const job of parseLaunchctlListOperatorUpdateJobCandidates(result.stdout)) {
		if (isCurrentGatewayLaunchdLabel(job.label, env)) continue;
		if (job.requiresMetadata && !await isLaunchdJobConfirmedOperatorUpdater({
			label: job.label,
			env
		})) continue;
		jobs.push({
...
async function disableOperatorUpdateLaunchdJobCandidate(params) {
	if (process.platform !== "darwin") return false;
	if (params.candidate.requiresMetadata && !(params.trustCurrentEnvMarker && hasOperatorUpdateLaunchdMarker(params.env) || await isLaunchdJobConfirmedOperatorUpdater({
...
async function disableOperatorUpdateLaunchdJob(label, env = process.env) {
	const candidate = normalizeOperatorUpdateLaunchdLabelCandidate(label);
	if (!candidate) return false;
	return await disableOperatorUpdateLaunchdJobCandidate({
		candidate,
		env,
		trustCurrentEnvMarker: false
	});
}
async function disableCurrentOperatorUpdateLaunchdJob(env = process.env) {
	const candidate = resolveCurrentOperatorUpdateLaunchdJobLabel(env);
	if (!candidate) return false;
	return await disableOperatorUpdateLaunchdJobCandidate({
		candidate,
		env,
...
function parseGatewayPortFromProgramArguments(programArguments) {
	...
	for (let index = 0; index < programArguments.length; index += 1) {
		...
		if (!current) continue;
		if (current === "--port") {
			const next = require_paths.parseTcpPort(programArguments[index + 1] ?? "");
			if (next !== null) return next;
			continue;
		}
		if (current.startsWith("--port=")) {
			const value = require_paths.parseTcpPort(current.slice(7));
...
async function resolveLaunchAgentGatewayPort(env) {
	...
	const fromServiceEnv = require_paths.parseTcpPort(command?.environment?.OPERATOR_GATEWAY_PORT ?? "");
...
function writeLaunchAgentActionLine(stdout, label, value) {
	try {
		stdout.write(`${require_runtime_parse.formatLine(label, value)}\n`);
	} catch (err) {
...
async function ensureLaunchAgentPlistReadable(plistPath) {
	await node_fs_promises.default.chmod(plistPath, LAUNCH_AGENT_PLIST_MODE).catch(() => void 0);
}
async function ensureSecureDirectory(targetPath, dirMode = LAUNCH_AGENT_DIR_MODE) {
	await node_fs_promises.default.mkdir(targetPath, {
		recursive: true,
		mode: dirMode
	});
	try {
		const mode = (await node_fs_promises.default.stat(targetPath)).mode & 511;
		const tightenedMode = mode & ~(dirMode === LAUNCH_AGENT_PRIVATE_DIR_MODE ? 63 : 18);
		if (tightenedMode !== mode) await node_fs_promises.default.chmod(targetPath, tightenedMode);
	} catch {}
}
...
function parseLaunchctlPrint(output) {
	...
	if (pidValue) {
		const pid = require_parse_finite_number.parseStrictPositiveInteger(pidValue);
		if (pid !== void 0) info.pid = pid;
	}
	const exitStatusValue = entries["last exit status"];
	if (exitStatusValue) {
		const status = require_parse_finite_number.parseStrictInteger(exitStatusValue);
...
async function launchAgentPlistExists(env) {
	try {
		...
		await node_fs_promises.default.access(plistPath);
		return true;
	} catch {
		return false;
	}
...
async function readLaunchAgentRuntime(env) {
	...
	const plistExists = await launchAgentPlistExists(env);
	const state = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(parsed.state);
	return {
...
function isLaunchctlAlreadyLoaded(res) {
	const detail = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(res.stderr || res.stdout);
...
async function repairLaunchAgentBootstrap(args) {
	...
	await rewriteLaunchAgentPlistForRestart({
		...
		label,
		plistPath,
		warn: args.warn ?? ((message) => process.stderr.write(`${require_runtime_parse.formatLine("Warning", message)}\n`))
	});
...
async function uninstallLaunchAgent({ env, stdout }) {
	...
	const home = require_runtime_parse.toPosixPath(require_paths$1.resolveHomeDir(env));
	const trashDir = node_path.default.posix.join(home, ".Trash");
	const dest = node_path.default.join(trashDir, `${label}.plist`);
	try {
		await node_fs_promises.default.mkdir(trashDir, { recursive: true });
		await node_fs_promises.default.rename(plistPath, dest);
		stdout.write(`${require_runtime_parse.formatLine("Moved LaunchAgent to Trash", dest)}\n`);
	} catch {
...
function formatLaunchctlResultDetail(res) {
	return (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(require_ansi.sanitizeForLog((res.stderr || res.stdout).replace(/[\r\n\t]+/g, " ")).replace(/\s+/g, " ").trim(), 1e3);
}
async function bootoutLaunchAgentOrThrow(params) {
	...
	params.stdout.write(`${require_runtime_parse.formatLine("Warning", params.warning)}\n`);
}
async function probeLaunchAgentState(serviceTarget) {
	...
	if ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(runtime.state) === "running" || typeof runtime.pid === "number" && runtime.pid > 1) return { state: "running" };
...
async function waitForGatewayPortRelease(port) {
	...
	while (Date.now() < deadline) {
		await require_sleep.sleep(Math.min(LAUNCH_AGENT_STOP_PORT_RELEASE_POLL_MS, deadline - Date.now()));
		if (await require_ports.probePortUsage(port) === "free") return true;
	}
	return false;
}
async function assertGatewayPortReleasedAfterStop(env) {
	...
	if (port === null) return;
	require_restart_stale_pids.cleanStaleGatewayProcessesSync(port);
	...
	throw new Error([`gateway port ${port} is still busy after LaunchAgent stop`, ...require_ports.formatPortDiagnostics(diagnostics)].join("\n"));
}
async function stopLaunchAgent({ stdout, env, disable: persistDisable }) {
	...
	if (!persistDisable) {
		...
		stdout.write(`${require_runtime_parse.formatLine("Stopped LaunchAgent", serviceTarget)}\n`);
		return;
	}
	...
	if (disableResult.code !== 0) {
		...
		await assertGatewayPortReleasedAfterStop(serviceEnv);
		stdout.write(`${require_runtime_parse.formatLine("Stopped LaunchAgent (degraded)", serviceTarget)}\n`);
		return;
	}
...
async function writeLaunchAgentPlist({ env, programArguments, workingDirectory, environment, description, stdout, warn }) {
	const { logDir, stdoutPath } = require_restart_logs.resolveGatewaySupervisorLogPaths(env, { platform: "darwin" });
	await ensureSecureDirectory(logDir);
	const domain = resolveGuiDomain();
	const label = resolveLaunchAgentLabel({ env });
	for (const legacyLabel of require_paths$1.resolveLegacyGatewayLaunchAgentLabels(env.OPERATOR_PROFILE)) {
		...
		try {
			await node_fs_promises.default.unlink(legacyPlistPath);
		} catch {}
	}
	const plistPath = resolveLaunchAgentPlistPathForLabel(env, label);
	const home = require_runtime_parse.toPosixPath(require_paths$1.resolveHomeDir(env));
	const libraryDir = node_path.default.posix.join(home, "Library");
	await ensureSecureDirectory(home);
	await ensureSecureDirectory(libraryDir);
	await ensureSecureDirectory(node_path.default.dirname(plistPath));
	...
	const plist = buildLaunchAgentPlist({
		label,
		comment: require_paths$1.resolveGatewayServiceDescription({
			env,
			environment,
			description
		}),
	...
	await node_fs_promises.default.writeFile(plistPath, plist, {
		encoding: "utf8",
...
async function rewriteLaunchAgentPlistForRestart({ env, label, plistPath, stdout, warn }) {
	...
	if (await node_fs_promises.default.readFile(plistPath, "utf8").catch(() => "") === plist) {
		await ensureLaunchAgentPlistReadable(plistPath);
		return false;
	}
	await node_fs_promises.default.writeFile(plistPath, plist, {
...
async function restartLaunchAgent({ stdout, env, warn }) {
	...
	if (cleanupPort !== null) {
		require_restart_stale_pids.cleanStaleGatewayProcessesSync(cleanupPort);
		const diagnostics = await require_ports.inspectPortUsage(cleanupPort).catch(() => null);
		if (diagnostics?.status === "busy") throw new Error([`gateway port ${cleanupPort} is still busy before LaunchAgent restart`, ...require_ports.formatPortDiagnostics(diagnostics)].join("\n"));
	}
...
//#endregion
Object.defineProperty(exports, "findStaleOperatorUpdateLaunchdJobs", {
	enumerable: true,
	get: function() {
		return findStaleOperatorUpdateLaunchdJobs;
	}
});
Object.defineProperty(exports, "formatLaunchAgentGuiSessionError", {
	enumerable: true,
	get: function() {
		return formatLaunchAgentGuiSessionError;
	}
});
Object.defineProperty(exports, "installLaunchAgent", {
	enumerable: true,
	get: function() {
		return installLaunchAgent;
	}
});
Object.defineProperty(exports, "isLaunchAgentLoaded", {
	enumerable: true,
	get: function() {
		return isLaunchAgentLoaded;
	}
});
Object.defineProperty(exports, "launchAgentPlistExists", {
	enumerable: true,
	get: function() {
		return launchAgentPlistExists;
	}
});
Object.defineProperty(exports, "launchd_exports", {
	enumerable: true,
	get: function() {
		return launchd_exports;
	}
});
Object.defineProperty(exports, "readLaunchAgentProgramArguments", {
	enumerable: true,
	get: function() {
		return readLaunchAgentProgramArguments;
	}
});
Object.defineProperty(exports, "readLaunchAgentRuntime", {
	enumerable: true,
	get: function() {
		return readLaunchAgentRuntime;
	}
});
Object.defineProperty(exports, "repairLaunchAgentBootstrap", {
	enumerable: true,
	get: function() {
		return repairLaunchAgentBootstrap;
	}
});
Object.defineProperty(exports, "resolveLaunchAgentPlistPath", {
	enumerable: true,
	get: function() {
		return resolveLaunchAgentPlistPath;
	}
});
Object.defineProperty(exports, "restartLaunchAgent", {
	enumerable: true,
	get: function() {
		return restartLaunchAgent;
	}
});
Object.defineProperty(exports, "stageLaunchAgent", {
	enumerable: true,
	get: function() {
		return stageLaunchAgent;
	}
});
Object.defineProperty(exports, "stopLaunchAgent", {
	enumerable: true,
	get: function() {
		return stopLaunchAgent;
	}
});
Object.defineProperty(exports, "uninstallLaunchAgent", {
	enumerable: true,
	get: function() {
