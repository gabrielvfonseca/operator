...
require("./provider-attribution-CIUHVFNx.cjs");
require("./operation-retry-DQKBakBo.cjs");
require("./provider-request-config-BmGl8zwP.cjs");
require("./shared-BAXHm3k7.cjs");
...
async function createMSTeamsHttpError(response, label, options) {
	return await require_provider_http_errors.createProviderHttpError(response, label, options);
}
//#endregion
...
function resolveMSTeamsTokenExpiresAt(value) {
	return require_parse_finite_number.resolveExpiresAtMsFromDurationSeconds(value, { bufferMs: EXPIRY_BUFFER_MS });
}
...
async function fetchMSTeamsTokens(params) {
	...
	try {
		...
		return parseMSTeamsTokenResponse(await require_provider_http_errors.readProviderJsonResponse(response, `MSTeams ${params.failureLabel} failed`), params.failureLabel);
	} finally {
		await release();
	}
}
...
Object.defineProperty(exports, "refreshMSTeamsDelegatedTokens", {
	enumerable: true,
	get: function() {
