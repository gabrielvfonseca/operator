...
function truncateHintText(text, limit) {
	...
	const truncated = (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(text, limit);
...
function schemaResourceRefKey(resourceRoot, ref, baseId) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(resourceRoot)) return `boolean:${String(resourceRoot)}:${baseId ?? ""}:${ref}`;
...
function normalizeAdditionalPropertiesSchema(schema) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.additionalProperties) || (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.properties) || (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.patternProperties)) return schema;
...
function normalizeJsonSchemaNode(schema) {
	...
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema)) return schema;
	...
	return Object.fromEntries(Object.entries(normalizedSchema).map(([key, value]) => {
		...
		if (key === "patternProperties" && (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value)) return [key, normalizePatternProperties(value)];
...
function resolveLocalAnchor(schema, anchor, isRoot = true) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema)) return;
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.dependencies)) for (const entry of Object.values(schema.dependencies)) {
	...
	for (const key of schemaValueKeywords) {
		const value = schema[key];
		if (typeof value === "boolean" || (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value)) {
...
function resolveLocalRef(resourceRoot, ref, resourceBaseId) {
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(resourceRoot) && typeof resourceRoot.$id === "string" && resourceRoot.$id !== "") {
	...
	if (ref.startsWith("#/")) {
		...
		for (const segment of ref.slice(2).split("/").map(decodePointerSegment)) {
			...
			} else if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(current)) current = current[segment];
			else return { found: false };
			if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(current) && typeof current.$id === "string") {
				currentResourceRoot = current;
				currentResourceBaseId = resolveSchemaId(current.$id, currentResourceBaseId);
			}
		}
		return typeof current === "boolean" || (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(current) ? {
			found: true,
			schema: current,
...
function resolveSchemaResourceRef(schema, ref, baseId) {
	...
	const visit = (current, baseIdLocal) => {
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(current) || seen.has(current)) return { found: false };
		seen.add(current);
		...
		if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(current.dependencies)) for (const entry of Object.values(current.dependencies)) {
			if (isStringArray(entry)) continue;
			const resolved = visit(entry, currentBaseId);
			if (resolved.found) return resolved;
		}
...
function validateSchemaKeywordShapes(schema, path) {
	...
	if (schema.dependentRequired !== void 0) {
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.dependentRequired)) return `${path}.dependentRequired: expected string array map`;
	...
	if (schema.dependencies !== void 0) {
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.dependencies)) return `${path}.dependencies: expected schema or string array map`;
		for (const [key, value] of Object.entries(schema.dependencies)) if (!isStringArray(value) && typeof value !== "boolean" && !(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value)) return `${path}.dependencies.${key}: expected schema or string array`;
	}
}
function findJsonSchemaNodeError(schema, path, root, resourceRoot, resourceBaseId) {
	if (typeof schema === "boolean") return;
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema)) return `${path}: schema must be an object or boolean`;
	...
	for (const key of schemaMapKeywords) {
		...
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value)) return `${path}.${key}: expected schema map`;
		for (const [entryKey, entry] of Object.entries(value)) {
			const error = findJsonSchemaNodeError(entry, `${path}.${key}.${entryKey}`, root, currentResourceRoot, currentResourceBaseId);
			if (error) return error;
		}
	}
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.dependencies)) for (const [key, value] of Object.entries(schema.dependencies)) {
...
function getDefault(schema) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema) || !Object.hasOwn(schema, "default")) return;
	return cloneDefault(schema.default);
}
function schemaWithResourceContext(schema, resourceRoot) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema) || !(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(resourceRoot)) return schema;
	return {
		...schema,
		...typeof resourceRoot.$id === "string" && schema.$id === void 0 ? { $id: resourceRoot.$id } : {},
		...(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(resourceRoot.$defs) ? { $defs: resourceRoot.$defs } : {},
		...(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(resourceRoot.definitions) ? { definitions: resourceRoot.definitions } : {}
	};
}
function inlineLocalRefsForMatch(schema, root, resourceRoot, resourceBaseId, resolvingRefs = /* @__PURE__ */ new Set()) {
	...
	const currentResourceRoot = typeof schema.$id === "string" ? schema : resourceRoot;
	const currentResourceBaseId = typeof schema.$id === "string" ? resolveSchemaId(schema.$id, resourceBaseId) : resourceBaseId;
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema) && typeof schema.$ref === "string") {
	...
	return Object.fromEntries(Object.entries(schema).map(([key, value]) => {
		if (schemaMapKeywords.has(key) && (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value)) return [key, Object.fromEntries(Object.entries(value).map(([entryKey, entry]) => [entryKey, inlineLocalRefsForMatch(entry, root, currentResourceRoot, currentResourceBaseId, resolvingRefs)]))];
		if (key === "dependencies" && (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value)) return [key, Object.fromEntries(Object.entries(value).map(([entryKey, entry]) => [entryKey, isStringArray(entry) ? entry : inlineLocalRefsForMatch(entry, root, currentResourceRoot, currentResourceBaseId, resolvingRefs)]))];
...
function schemaMatches(schema, value, root, resourceRoot, resourceBaseId) {
	try {
		return (0, typebox_compile.Compile)(normalizeJsonSchemaForTypeBox(schemaWithResourceContext(inlineLocalRefsForMatch(schema, root, resourceRoot, resourceBaseId), resourceRoot))).Check(value);
	} catch {
		return false;
	}
}
function applyObjectPropertyDefaults(schema, value, root, resolvingRefs, currentResourceRoot, currentResourceBaseId) {
	const properties = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.properties) ? schema.properties : {};
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.patternProperties)) for (const [pattern, propertySchema] of Object.entries(schema.patternProperties)) {
		let regex;
		try {
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.additionalProperties)) {
...
function applyObjectDependencyDefaults(schema, value, root, resolvingRefs, currentResourceRoot, currentResourceBaseId) {
	let nextValue = value;
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.dependencies)) for (const [key, dependencySchema] of Object.entries(schema.dependencies)) {
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.dependentSchemas)) for (const [key, dependentSchema] of Object.entries(schema.dependentSchemas)) {
...
function applyObjectConditionalDefaults(schema, value, root, resolvingRefs, currentResourceRoot, currentResourceBaseId) {
	if (!(typeof schema.if === "boolean" || (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.if))) return value;
	const branch = schemaMatches(schema.if, value, root, currentResourceRoot, currentResourceBaseId) ? schema.then : schema.else;
	if (!(typeof branch === "boolean" || (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(branch))) return value;
...
function countSchemaNodes(schema, seen = /* @__PURE__ */ new Set()) {
	if (typeof schema === "boolean" || !(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema) || seen.has(schema)) return 1;
	seen.add(schema);
	let count = 1;
	for (const key of schemaMapKeywords) {
		const value = schema[key];
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.dependencies)) {
...
function applySchemaDefaults(schema, valueInput, root = schema, resolvingRefs = /* @__PURE__ */ new Set(), resourceRoot = root, resourceBaseId) {
	...
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema)) return value;
	const currentResourceRoot = typeof schema.$id === "string" ? schema : resourceRoot;
	const currentResourceBaseId = typeof schema.$id === "string" ? resolveSchemaId(schema.$id, resourceBaseId) : resourceBaseId;
	let nextValue = value;
	...
	const hasObjectApplicators = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.properties) || (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.patternProperties) || (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.additionalProperties) || (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.dependencies) || (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.dependentSchemas) || typeof schema.if === "boolean" || (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.if);
	if ((schemaTypeIncludes(schema, "object") || hasObjectApplicators) && (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(nextValue)) {
	...
	if ((schemaTypeIncludes(schema, "array") || schema.items !== void 0 || schema.prefixItems !== void 0) && Array.isArray(nextValue)) {
		...
		if (tupleSchemas) {
			...
			const restSchema = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.items) ? schema.items : (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.additionalItems) ? schema.additionalItems : null;
		...
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(schema.items)) return nextValue;
...
const schemaCache = new require_plugin_cache_primitives.PluginLruCache(512);
...
function compileSchema(schema) {
	return (0, typebox_compile.Compile)(normalizeJsonSchemaForTypeBox(schema));
}
...
function withPluginFormatSemantics(callback) {
	const previousFormats = typebox_format.Format.Entries();
	typebox_format.Format.Set("uri", (value) => URL.canParse(value));
	for (const format of annotationOnlyFormats) typebox_format.Format.Set(format, () => true);
	try {
		return callback();
	} finally {
		typebox_format.Format.Clear();
		for (const [format, check] of previousFormats) typebox_format.Format.Set(format, check);
	}
}
...
function formatValidationErrors(errors) {
	...
	return errors.map((error) => {
		...
		return {
			path,
			message,
			text: `${require_safe_text.sanitizeTerminalText(path)}: ${require_safe_text.sanitizeTerminalText(message)}`,
...
function validateJsonSchemaValue(params) {
	...
	if (schemaError) throw new Error(require_safe_text.sanitizeTerminalText(`invalid schema: ${schemaError}`));
...
//#endregion
Object.defineProperty(exports, "appendAllowedValuesHint", {
	enumerable: true,
	get: function() {
		return appendAllowedValuesHint;
	}
});
Object.defineProperty(exports, "findJsonSchemaShapeError", {
	enumerable: true,
	get: function() {
		return findJsonSchemaShapeError;
	}
});
Object.defineProperty(exports, "normalizeJsonSchemaForTypeBox", {
	enumerable: true,
	get: function() {
		return normalizeJsonSchemaForTypeBox;
	}
});
Object.defineProperty(exports, "summarizeAllowedValues", {
	enumerable: true,
	get: function() {
		return summarizeAllowedValues;
	}
});
Object.defineProperty(exports, "validateJsonSchemaValue", {
	enumerable: true,
	get: function() {
