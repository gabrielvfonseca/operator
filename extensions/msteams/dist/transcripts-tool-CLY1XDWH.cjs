...
function resolveAutoStart(raw) {
	...
	return raw.map((entry) => {
		...
		const providerId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(config.providerId);
		if (!providerId) return;
		return {
			providerId,
			sessionId: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(config.sessionId),
			title: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(config.title),
			accountId: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(config.accountId),
			guildId: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(config.guildId),
			channelId: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(config.channelId),
			meetingUrl: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(config.meetingUrl)
		};
...
function buildProviderMaps(cfg) {
	return require_worker_provider_registry.buildCapabilityProviderMaps(resolveTranscriptsSourceProviderEntries(cfg));
}
...
function firstSentences(utterances, limit) {
	return (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(((0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(utterances.map((utterance) => utterance.text)).join(" ").match(/[^.!?]+[.!?]?/g) ?? []).slice(0, limit)).join(" ");
}
...
async function readJsonFile(filePath) {
	try {
		return JSON.parse(await node_fs_promises.default.readFile(filePath, "utf8"));
	} catch (err) {
...
var TranscriptsStore = class {
	...
	sessionDir(session) {
		return node_path.default.join(this.rootDir, dateSegment(session.startedAt), safeSegment(session.sessionId));
	}
	async hasSessionMetadata(dir) {
		return await readJsonFile(node_path.default.join(dir, "metadata.json")) !== void 0;
	}
	async findSessionDirForSession(session) {
		const datedDir = this.sessionDir(session);
		const datedSession = await readJsonFile(node_path.default.join(datedDir, "metadata.json"));
	...
	async findSessionDir(selector) {
		...
		if (qualified?.[1] && qualified[2]) {
			const directDir = node_path.default.join(this.rootDir, qualified[1], safeSegment(qualified[2]));
		...
		if (idDate) {
			const directDir = node_path.default.join(this.rootDir, idDate, safeSessionId);
			return await this.hasSessionMetadata(directDir) ? directDir : void 0;
		}
		let entries;
		try {
			entries = await node_fs_promises.default.readdir(this.rootDir, { withFileTypes: true });
		} catch (err) {
			if (err && typeof err === "object" && "code" in err && err.code === "ENOENT") return;
			throw err;
		}
		...
		for (const entry of datedEntries) {
			const candidate = node_path.default.join(this.rootDir, entry.name, safeSessionId);
			if ((await readJsonFile(node_path.default.join(candidate, "metadata.json")))?.sessionId === selector) matches.push(candidate);
		}
	...
	async writeSession(session) {
		...
		await node_fs_promises.default.writeFile(node_path.default.join(dir, "metadata.json"), `${JSON.stringify(session, null, 2)}\n`);
	}
	...
	async readSessionEntry(sessionId) {
		...
		if (!dir) return;
		const session = await readJsonFile(node_path.default.join(dir, "metadata.json"));
		return session ? {
			session,
			sessionDir: dir
		} : void 0;
	...
	async appendUtteranceToDir(dir, sessionId, utterance) {
		await node_fs_promises.default.mkdir(dir, { recursive: true });
		await node_fs_promises.default.appendFile(node_path.default.join(dir, "transcript.jsonl"), `${JSON.stringify({
			...utterance,
			sessionId
		})}\n`);
	}
	...
	async readUtterancesFromDir(dir, options = {}) {
		const transcriptPath = node_path.default.join(dir, "transcript.jsonl");
		const maxUtterances = (0, _gabrielvfonseca_normalization_core_number_coercion.resolveOptionalIntegerOption)(options.maxUtterances, { min: 1 });
		if (maxUtterances !== void 0) return await new Promise((resolve, reject) => {
			const utterances = [];
			const stream = (0, node_fs.createReadStream)(transcriptPath, { encoding: "utf8" });
			const lines = (0, node_readline.createInterface)({
				input: stream,
				crlfDelay: Infinity
			});
			let settled = false;
		...
		let raw;
		try {
			raw = await node_fs_promises.default.readFile(transcriptPath, "utf8");
		} catch (err) {
			if (err && typeof err === "object" && "code" in err && err.code === "ENOENT") return [];
			throw err;
		}
	...
	async updateStopped(sessionId, stoppedAt) {
		...
		const session = await readJsonFile(node_path.default.join(dir, "metadata.json"));
		if (!session) return;
		await node_fs_promises.default.writeFile(node_path.default.join(dir, "metadata.json"), `${JSON.stringify({
			...session,
			stoppedAt
		}, null, 2)}\n`);
	}
	...
	async writeSummary(summary, session) {
		const dir = session !== void 0 ? await this.findSessionDirForSession(session) : await this.findSessionDir(summary.sessionId) ?? node_path.default.join(this.rootDir, dateSegment(summary.sessionId), safeSegment(summary.sessionId));
	...
	async writeSummaryToDir(summary, dir) {
		await node_fs_promises.default.mkdir(dir, { recursive: true });
		await node_fs_promises.default.writeFile(node_path.default.join(dir, "summary.json"), `${JSON.stringify(summary, null, 2)}\n`);
		const markdown = renderTranscriptsMarkdown(summary);
		const markdownPath = node_path.default.join(dir, "summary.md");
		await node_fs_promises.default.writeFile(markdownPath, `${markdown}\n`);
		return markdownPath;
	}
};
//#endregion
...
function createSessionId() {
	return `transcript-${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${(0, node_crypto.randomUUID)().slice(0, 8)}`;
}
...
*/
var transcripts_tool_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	createTranscriptsAutoStartService: () => createTranscriptsAutoStartService,
	createTranscriptsTool: () => createTranscriptsTool
});
...
const TranscriptsSchema = typebox.Type.Object({
	action: typebox.Type.String({ description: "start, stop, status, import, or summarize." }),
	sessionId: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	title: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	providerId: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	accountId: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	guildId: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	channelId: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	meetingUrl: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	transcript: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	speakerLabel: typebox.Type.Optional(typebox.Type.String({ minLength: 1 }))
}, { additionalProperties: false });
function createStore(ctx) {
	return new TranscriptsStore(node_path.default.join(ctx.stateDir, "transcripts"));
}
...
async function statusTranscripts(ctx) {
	const uniqueProviders = (0, _gabrielvfonseca_normalization_core_string_normalization.uniqueStrings)([manualTranscriptSourceProvider.id, ...listTranscriptSourceProviders(ctx.config).map((provider) => provider.id)]);
...
function createTranscriptsTool(options) {
	const ctx = {
		...
		stateDir: options?.stateDir ?? require_paths.resolveStateDir(),
...
function createTranscriptsAutoStartService(ctx) {
	...
	return {
		start() {
			...
			const store = new TranscriptsStore(node_path.default.join(ctx.stateDir, "transcripts"));
...
//#endregion
Object.defineProperty(exports, "createTranscriptsTool", {
	enumerable: true,
	get: function() {
		return createTranscriptsTool;
	}
});
Object.defineProperty(exports, "resolveTranscriptsConfig", {
	enumerable: true,
	get: function() {
		return resolveTranscriptsConfig;
	}
});
Object.defineProperty(exports, "transcripts_tool_exports", {
	enumerable: true,
	get: function() {
		return transcripts_tool_exports;
	}
});
