...
function truncateTaskStatusText(value, maxChars) {
	...
	return `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(trimmed, Math.max(0, maxChars - 1)).trimEnd()}…`;
}
function stripInlineLeakedInternalContext(value) {
	const beginIndex = value.indexOf(require_internal_runtime_context.INTERNAL_RUNTIME_CONTEXT_BEGIN);
	if (beginIndex !== -1 && (value.includes("<<<END_OPERATOR_INTERNAL_CONTEXT>>>") || value.includes("Operator runtime context (internal):") || value.includes("[Internal task completion event]"))) return value.slice(0, beginIndex);
	const legacyHeaderIndex = value.indexOf("Operator runtime context (internal):");
...
function sanitizeTaskStatusValue(value, errorContext) {
	if (typeof value === "string") return require_sanitize_user_facing_text.sanitizeUserFacingText(stripInlineLeakedInternalContext(value), { errorContext }).replace(/\s+/g, " ").trim() || void 0;
...
//#endregion
Object.defineProperty(exports, "buildTaskStatusSnapshot", {
	enumerable: true,
	get: function() {
		return buildTaskStatusSnapshot;
	}
});
Object.defineProperty(exports, "formatTaskStatusDetail", {
	enumerable: true,
	get: function() {
		return formatTaskStatusDetail;
	}
});
Object.defineProperty(exports, "formatTaskStatusTitle", {
	enumerable: true,
	get: function() {
		return formatTaskStatusTitle;
	}
});
Object.defineProperty(exports, "formatTaskStatusTitleText", {
	enumerable: true,
	get: function() {
		return formatTaskStatusTitleText;
	}
});
Object.defineProperty(exports, "sanitizeTaskStatusText", {
	enumerable: true,
	get: function() {
		return sanitizeTaskStatusText;
	}
});
