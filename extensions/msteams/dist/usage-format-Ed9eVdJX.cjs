...
require("./model-pricing-cache-Cs3Yqj4R.cjs");
...
*/
var usage_format_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	estimateUsageCost: () => estimateUsageCost,
	formatTokenCount: () => require_token_format.formatTokenCount,
	formatUsd: () => formatUsd,
	resetUsageFormatCachesForTest: () => resetUsageFormatCachesForTest,
	resolveModelCostConfig: () => resolveModelCostConfig,
	resolveModelCostConfigFingerprint: () => resolveModelCostConfigFingerprint
});
...
function toResolvedModelKey(params) {
	...
	const key = require_model_selection_normalize.modelKey(normalized.provider, normalized.model);
...
function toDirectModelKey(params) {
	...
	if (modelKeyCache.has(cacheKey)) return modelKeyCache.get(cacheKey) ?? null;
	const provider = require_model_selection_normalize.normalizeProviderId((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.provider) ?? "");
	const model = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.model);
	if (!provider || !model) {
		cacheModelKey(cacheKey, null);
		return null;
	}
	const key = require_model_selection_normalize.modelKey(provider, model);
	cacheModelKey(cacheKey, key);
	return key;
}
...
function buildProviderCostIndexBundle(providers, options) {
	...
	for (const [providerKey, providerConfig] of Object.entries(providers)) {
		const normalizedProvider = require_model_selection_normalize.normalizeProviderId(providerKey);
		for (const model of providerConfig?.models ?? []) {
			const normalized = require_model_selection_normalize.normalizeModelRef(normalizedProvider, model.id, {
...
function loadModelsJsonCostIndex(options) {
	...
	const modelsPath = node_path.default.join(require_agent_scope_config.resolveDefaultAgentDir({}), "models.json");
	try {
		if (!modelsJsonCostCache || modelsJsonCostCache.path !== modelsPath) {
			const parsed = (0, _openclaw_fs_safe_json.tryReadJsonSync)(modelsPath);
...
function isProviderCostSourceCurrent(providers, source, key, options) {
	...
	const normalized = require_model_selection_normalize.normalizeModelRef(require_model_selection_normalize.normalizeProviderId(source.providerKey), source.model.id, {
		allowManifestNormalization: options?.allowManifestNormalization ?? (options?.allowPluginNormalization === false ? false : void 0),
		allowPluginNormalization: options?.allowPluginNormalization
	});
	return require_model_selection_normalize.modelKey(normalized.provider, normalized.model) === key;
}
...
function hasProviderCostSourceForKey(providers, key, options) {
	for (const [providerKey, providerConfig] of Object.entries(providers)) {
		...
		for (const model of providerConfig?.models ?? []) {
			...
			if (require_model_selection_normalize.modelKey(normalized.provider, normalized.model) === key) return true;
		}
	}
	return false;
}
...
function resolveModelCostConfigFingerprint(config) {
	return stableCostFingerprintValue({
		...
		gatewayPricing: require_model_pricing_config.getGatewayModelPricingCacheFingerprint()
	});
}
/**
...
function resolveModelCostConfig(params) {
	...
	return require_model_pricing_config.getCachedGatewayModelPricing(params);
}
...
function selectPricingTier(tiers, input) {
	...
	for (let index = sortedTiers.length - 1; index >= 0; index -= 1) {
		const tier = (0, _gabrielvfonseca_normalization_core.expectDefined)(sortedTiers[index], "sorted tiers entry at index");
...
//#endregion
Object.defineProperty(exports, "estimateUsageCost", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "resolveModelCostConfig", {
	enumerable: true,
	get: function() {
		return resolveModelCostConfig;
	}
});
Object.defineProperty(exports, "resolveModelCostConfigFingerprint", {
	enumerable: true,
	get: function() {
		return resolveModelCostConfigFingerprint;
	}
});
Object.defineProperty(exports, "usage_format_exports", {
	enumerable: true,
	get: function() {
		return usage_format_exports;
	}
});
