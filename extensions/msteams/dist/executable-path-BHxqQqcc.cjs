...
function resolveExecutablePathCandidate(rawExecutable, options) {
	const expanded = rawExecutable.startsWith("~") ? require_home_dir.expandHomePrefix(rawExecutable, { env: options?.env }) : rawExecutable;
	...
	if (node_path.default.isAbsolute(expanded)) return node_path.default.resolve(expanded);
...
function resolveWindowsExecutableExtensions(executable, env, includeExtensionless = true) {
	if (process.platform !== "win32") return [""];
	if (node_path.default.extname(executable).length > 0) return [""];
	const extensions = (resolveEnvironmentValue(env, "PATHEXT") ?? resolveEnvironmentValue(process.env, "PATHEXT") ?? ".EXE;.CMD;.BAT;.COM").split(";").map((ext) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(ext));
	return includeExtensionless ? ["", ...extensions] : extensions;
}
function resolveWindowsExecutableExtSet(env) {
	return new Set((resolveEnvironmentValue(env, "PATHEXT") ?? resolveEnvironmentValue(process.env, "PATHEXT") ?? ".EXE;.CMD;.BAT;.COM").split(";").map((ext) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(ext)).filter(Boolean));
}
...
function resolveExecutableFromPathEnv(executable, pathEnv, env, options) {
	...
	const hasNativeWindowsExtension = process.platform === "win32" && WINDOWS_NATIVE_EXECUTABLE_EXTENSIONS.has((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(node_path.default.extname(executable)));
	for (const entry of entries) for (const ext of extensions) {
		const candidate = node_path.default.join(entry, executable + ext);
...
function resolveExecutable(cmd) {
	...
	if (WINDOWS_NATIVE_EXECUTABLE_EXTENSIONS.has((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(node_path.default.extname(cmd)))) return cmd;
	...
	const matches = [];
	for (const entry of entries) for (const ext of extensions) {
		const candidate = node_path.default.join(entry, cmd + ext);
		if (isExecutableFile(candidate, { env: process.env })) matches.push(candidate);
	}
	const cmdMatch = matches.find((match) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(node_path.default.extname(match)) === ".cmd");
	if (cmdMatch) return cmdMatch;
	const exeMatch = matches.find((match) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(node_path.default.extname(match)) === ".exe");
...
Object.defineProperty(exports, "resolveExecutable", {
	enumerable: true,
	get: function() {
		return resolveExecutable;
	}
});
Object.defineProperty(exports, "resolveExecutableFromPathEnv", {
	enumerable: true,
	get: function() {
		return resolveExecutableFromPathEnv;
	}
});
Object.defineProperty(exports, "resolveExecutablePath", {
	enumerable: true,
	get: function() {
		return resolveExecutablePath;
	}
});
Object.defineProperty(exports, "resolveExecutablePathCandidate", {
	enumerable: true,
	get: function() {
		return resolveExecutablePathCandidate;
	}
});
