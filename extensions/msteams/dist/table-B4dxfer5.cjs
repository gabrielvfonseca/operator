...
/** Resolve the underlying OS home before applying Operator overrides. */
function resolveRawOsHomeDir(env, homedir) {
	return normalize(env.HOME) ?? normalize(env.USERPROFILE) ?? resolveTermuxHome(env) ?? normalizeSafe(homedir);
}
/** Resolve raw home with OPERATOR_HOME tilde expansion. */
function resolveRawHomeDir(env = process.env, homedir = node_os.default.homedir) {
...
function resolveHomeDisplayPrefix() {
	...
	if (!home) return;
	return process.env.OPERATOR_HOME?.trim() ? {
		home,
...
/** Replace the effective home path with "~" or "$OPERATOR_HOME" for terminal display. */
...
var table_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	getTerminalTableWidth: () => getTerminalTableWidth,
...
function padCell(text, width, align) {
	const content = require_ansi.visibleWidth(text) > width ? require_ansi.truncateToVisibleWidth(text, width) : text;
	const w = require_ansi.visibleWidth(content);
...
function wrapLine(text, width) {
	...
	const tokens = [];
	for (const segment of require_ansi.splitAnsiSegments(text)) {
		if (segment.kind === "ansi") {
			tokens.push({
				kind: "ansi",
				value: segment.value,
				width: require_ansi.visibleWidth(segment.controls.join(""))
			});
			continue;
		}
		for (const grapheme of require_ansi.splitGraphemes(segment.value)) tokens.push({
			kind: "char",
			value: grapheme
		});
	}
	...
	const bufVisibleWidth = (slice) => slice.reduce((acc, token) => acc + (token.kind === "char" ? require_ansi.visibleWidth(token.value) : token.width), 0);
	const pushLine = (value) => {
		const cleaned = value.replace(/\s+$/, "");
		if (require_ansi.visibleWidth(cleaned) === 0) return;
		lines.push(cleaned);
	};
	...
	for (const token of tokens) {
		...
		const charWidth = require_ansi.visibleWidth(ch);
...
function renderTable(opts) {
	...
	const metrics = columns.map((c) => {
		return {
			headerW: require_ansi.visibleWidth(c.header),
			cellW: Math.max(0, ...rows.map((r) => require_ansi.visibleWidth(r[c.key] ?? "")))
		};
	});
...
//#endregion
Object.defineProperty(exports, "getTerminalTableWidth", {
	enumerable: true,
	get: function() {
		return getTerminalTableWidth;
	}
});
