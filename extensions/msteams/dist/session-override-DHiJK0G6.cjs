...
const sessionAccessorLoader = require_lazy_promise.createLazyImportLoader(() => Promise.resolve().then(() => require("./session-accessor-D_W4fZCX.cjs")).then((n) => n.session_accessor_exports));
...
function isProfileForProvider(params) {
	...
	if (entry) {
		...
		return params.providers.some((provider) => require_order.isStoredCredentialCompatibleWithAuthProvider({
			cfg: params.cfg,
			provider,
			credential: entry
		}));
	}
	return params.providers.some((provider) => require_order.isConfiguredAwsSdkAuthProfileForProvider({
		cfg: params.cfg,
		provider,
...
async function resolveSessionAuthProfileOverride(params) {
	...
	if (!sessionEntry.authProfileOverride?.trim() && !hasConfiguredAuthProfiles && !require_source_check.hasAnyAuthProfileStoreSource(agentDir)) return;
	...
	const order = [...new Set(providers.flatMap((candidateProvider) => require_order.resolveAuthProfileOrder({
		cfg,
		store,
	...
	if (currentProfileId && !store.profiles[currentProfileId] && !providers.some((candidateProvider) => require_order.isConfiguredAwsSdkAuthProfileForProvider({
		cfg,
		provider: candidateProvider,
	...
	const pickFirstAvailable = () => order.find((profileId) => !require_usage_state.isProfileInCooldown(store, profileId)) ?? order[0];
	const pickNextAvailable = (active) => {
		const startIndex = order.indexOf(active);
		if (startIndex < 0) return pickFirstAvailable();
		for (let offset = 1; offset <= order.length; offset += 1) {
			const candidate = order[(startIndex + offset) % order.length];
			if (candidate && !require_usage_state.isProfileInCooldown(store, candidate)) return candidate;
		}
	...
	const replacementForUnusableCurrent = current && require_usage_state.isProfileInCooldown(store, current) ? order.find((profileId) => profileId !== current && !require_usage_state.isProfileInCooldown(store, profileId)) : void 0;
	...
	else if (!current || require_usage_state.isProfileInCooldown(store, current)) next = pickFirstAvailable();
...
//#endregion
Object.defineProperty(exports, "clearSessionAuthProfileOverride", {
	enumerable: true,
	get: function() {
		return clearSessionAuthProfileOverride;
	}
});
Object.defineProperty(exports, "resolveSessionAuthProfileOverride", {
	enumerable: true,
	get: function() {
		return resolveSessionAuthProfileOverride;
	}
});
