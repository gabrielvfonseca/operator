...
async function persistInboundImagesForTranscript(params) {
	...
	for (const image of params.images) try {
		inline.push(await require_store.saveMediaBuffer(Buffer.from(image.data, "base64"), image.mimeType, "inbound"));
	} catch (err) {
		params.log.warn(`${params.logContext}: failed to persist inbound image (${image.mimeType}): ${require_errors.formatErrorMessage(err)}`);
	}
...
function normalizeMime(mime) {
	if (!mime) return;
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(mime.split(";")[0]) || void 0;
}
...
function normalizeAttachment(att, idx, opts) {
	...
	if (opts.stripDataUrlPrefix) {
		...
		if (dataUrlMatch) base64 = (0, _gabrielvfonseca_normalization_core.expectDefined)(dataUrlMatch[1], "data url match capture group 1");
	}
	return {
		label,
		mime,
...
async function parseMessageWithAttachments(message, attachments, opts) {
	...
	try {
		for (const [idx, att] of attachments.entries()) {
			...
			const sizeBytes = (0, _gabrielvfonseca_media_core_base64.estimateBase64DecodedBytes)(b64);
			if (sizeBytes > maxBytes) throw new Error(`attachment ${label}: exceeds size limit (${sizeBytes} > ${maxBytes} bytes)`);
			const providedMime = normalizeMime(mime);
			const sniffedMime = normalizeMime(await require_web_tools.sniffMimeFromBase64(b64));
			const finalMime = resolveAttachmentMime({
				sniffedMime,
				providedMime,
				labelMime: normalizeMime((0, _gabrielvfonseca_media_core_mime.mimeTypeFromFilePath)(label))
			});
			...
			if (isImage && sizeBytes > _gabrielvfonseca_media_core_constants.MAX_IMAGE_BYTES) throw new Error(`attachment ${label}: image exceeds size limit (${sizeBytes} > ${_gabrielvfonseca_media_core_constants.MAX_IMAGE_BYTES} bytes)`);
			...
			let savedMedia;
			try {
				savedMedia = assertSavedMedia(await require_store.saveMediaBuffer(buffer, finalMime, "inbound", maxBytes, ensureExtension(label, finalMime)), label);
			} catch (err) {
				throw new MediaOffloadError(`[Gateway Error] Failed to save intercepted media to disk: ${require_errors.formatErrorMessage(err)}`, { cause: err });
			}
		...
		if (savedMediaIds.length > 0) await Promise.allSettled(savedMediaIds.map((id) => require_store.deleteMediaBuffer(id, "inbound")));
		throw err;
	}
	return {
...
//#endregion
Object.defineProperty(exports, "MediaOffloadError", {
	enumerable: true,
	get: function() {
		return MediaOffloadError;
	}
});
Object.defineProperty(exports, "UnsupportedAttachmentError", {
	enumerable: true,
	get: function() {
		return UnsupportedAttachmentError;
	}
});
Object.defineProperty(exports, "normalizeRpcAttachmentsToChatAttachments", {
	enumerable: true,
	get: function() {
		return normalizeRpcAttachmentsToChatAttachments;
	}
});
Object.defineProperty(exports, "parseMessageWithAttachments", {
	enumerable: true,
	get: function() {
		return parseMessageWithAttachments;
	}
});
Object.defineProperty(exports, "persistInboundImagesForTranscript", {
	enumerable: true,
	get: function() {
		return persistInboundImagesForTranscript;
	}
});
Object.defineProperty(exports, "resolveChatAttachmentMaxBytes", {
	enumerable: true,
	get: function() {
		return resolveChatAttachmentMaxBytes;
	}
});
