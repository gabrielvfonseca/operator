...
const require_startup_migration_checkpoint = require("./startup-migration-checkpoint-Cw2amDJ3.cjs");
...
function looksLikeAuthClose(code, reason) {
	...
	const normalized = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(reason);
...
async function confirmGatewayReachable(params) {
	const token = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.auth?.token ?? process.env.OPERATOR_GATEWAY_TOKEN);
	const password = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.auth?.password ?? process.env.OPERATOR_GATEWAY_PASSWORD);
...
async function resolveGatewayRestartProbeAuth(env) {
	...
	return (await require_probe_auth.resolveGatewayProbeAuthSafeWithSecretInputs({
		cfg: await require_io.createConfigIO({
			env: mergedEnv,
...
async function inspectGatewayPortHealth(params) {
	let portUsage;
	try {
		portUsage = await require_ports.inspectPortUsage(params.port);
	} catch (err) {
		portUsage = {
			port: params.port,
			status: "unknown",
...
async function inspectGatewayRestart(params) {
	...
	const expectedVersion = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.expectedVersion);
	...
	const gatewayListeners = portUsage.status === "busy" ? portUsage.listeners.filter((listener) => require_ports.classifyPortListener(listener, params.port) === "gateway") : [];
	const fallbackListenerPids = params.includeUnknownListenersAsStale && process.platform === "win32" && runtime.status !== "running" && portUsage.status === "busy" ? portUsage.listeners.filter((listener) => require_ports.classifyPortListener(listener, params.port) === "unknown").map((listener) => listener.pid).filter((pid) => Number.isFinite(pid)) : [];
...
async function waitForGatewayHealthyRestart(params) {
	...
	for (let attempt = 0;; attempt += 1) {
		...
		if (snapshot.runtime.status !== "running") migrationActive = false;
			...
			migrationActive = (() => {
				try {
					return (params.isStartupMigrationActive ?? require_startup_migration_checkpoint.hasActiveStartupMigrationLease)({ env: params.env });
				} catch {
					return false;
				}
			})();
			nextMigrationActivityPollMs = elapsedMs + STARTUP_MIGRATION_ACTIVITY_POLL_MS;
			if (migrationActive && migrationDeadlineMs === void 0) migrationDeadlineMs = elapsedMs + require_startup_migration_checkpoint.STARTUP_MIGRATION_LEASE_TTL_MS;
...
function renderPortUsageDiagnostics(snapshot) {
	const lines = [];
	if (snapshot.portUsage.status === "busy") lines.push(...require_ports.formatPortDiagnostics(snapshot.portUsage));
...
async function terminateStaleGatewayPids(pids) {
	...
	for (const pid of targets) require_kill_tree.killProcessTree(pid, { graceMs: 300 });
	if (targets.length > 0) await require_sleep.sleep(500);
	return targets;
}
//#endregion
exports.DEFAULT_RESTART_HEALTH_ATTEMPTS = DEFAULT_RESTART_HEALTH_ATTEMPTS;
exports.DEFAULT_RESTART_HEALTH_DELAY_MS = DEFAULT_RESTART_HEALTH_DELAY_MS;
exports.inspectGatewayRestart = inspectGatewayRestart;
exports.renderGatewayPortHealthDiagnostics = renderGatewayPortHealthDiagnostics;
exports.renderRestartDiagnostics = renderRestartDiagnostics;
exports.terminateStaleGatewayPids = terminateStaleGatewayPids;
exports.waitForGatewayHealthyListener = waitForGatewayHealthyListener;
exports.waitForGatewayHealthyRestart = waitForGatewayHealthyRestart;
