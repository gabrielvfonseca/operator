...
async function noteBootstrapFileSize(cfg) {
	...
	const workspaceDir = require_agent_scope_config.resolveAgentWorkspaceDir(cfg, defaultAgentId);
	const bootstrapMaxChars = require_embedded_agent_helpers.resolveBootstrapMaxChars(cfg, defaultAgentId);
	const bootstrapTotalMaxChars = require_embedded_agent_helpers.resolveBootstrapTotalMaxChars(cfg, defaultAgentId);
	const { bootstrapFiles, contextFiles } = await require_bootstrap_files.resolveBootstrapContextForRun({
		workspaceDir,
		config: cfg,
		agentId: defaultAgentId
	});
	const analysis = require_bootstrap_budget.analyzeBootstrapBudget({
...
//#endregion
exports.noteBootstrapFileSize = noteBootstrapFileSize;
