...
function normalizeTextField(value) {
	...
	return require_system_tags.sanitizeInboundSystemTags(require_inbound_text.normalizeInboundTextNewlines(value));
}
function normalizeTrustedTextField(value) {
	if (typeof value !== "string") return;
	return require_inbound_text.normalizeInboundTextNewlines(value);
}
...
function finalizeInboundContext(ctx, opts = {}) {
	...
	normalized.Body = require_system_tags.sanitizeInboundSystemTags(require_inbound_text.normalizeInboundTextNewlines(typeof normalized.Body === "string" ? normalized.Body : ""));
	...
	if (Array.isArray(normalized.UntrustedContext)) normalized.UntrustedContext = normalized.UntrustedContext.map((entry) => require_system_tags.sanitizeInboundSystemTags(require_inbound_text.normalizeInboundTextNewlines(entry))).filter((entry) => Boolean(entry));
	const chatType = require_chat_type.normalizeChatType(normalized.ChatType);
	if (chatType && (opts.forceChatType || normalized.ChatType !== chatType)) normalized.ChatType = chatType;
	normalized.BodyForAgent = require_system_tags.sanitizeInboundSystemTags(require_inbound_text.normalizeInboundTextNewlines(opts.forceBodyForAgent ? normalized.Body : normalized.BodyForAgent ?? normalized.CommandBody ?? normalized.RawBody ?? normalized.Body));
	normalized.BodyForCommands = require_system_tags.sanitizeInboundSystemTags(require_inbound_text.normalizeInboundTextNewlines(opts.forceBodyForCommands ? normalized.CommandBody ?? normalized.RawBody ?? normalized.Body : normalized.BodyForCommands ?? normalized.CommandBody ?? normalized.RawBody ?? normalized.Body));
	const explicitLabel = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(normalized.ConversationLabel);
	if (opts.forceConversationLabel || !explicitLabel) {
		const resolved = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(require_session_accessor.resolveConversationLabel(normalized));
		if (resolved) normalized.ConversationLabel = resolved;
	} else normalized.ConversationLabel = explicitLabel;
	normalized.CommandAuthorized = normalized.CommandAuthorized === true;
	normalized.CommandTurn = require_command_turn_context.resolveCommandTurnContext(normalized);
...
//#endregion
Object.defineProperty(exports, "finalizeInboundContext", {
	enumerable: true,
	get: function() {
