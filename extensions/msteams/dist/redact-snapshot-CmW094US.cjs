...
function replaceSensitiveValuesInRaw(params) {
	const values = (0, _gabrielvfonseca_normalization_core_string_normalization.uniqueStrings)(params.sensitiveValues).filter((value) => value !== "").toSorted((a, b) => b.length - a.length);
...
function shouldFallbackToStructuredRawRedaction(params) {
	try {
		const parsed = json5.default.parse(params.redactedRaw);
		const restored = params.restoreParsed(parsed);
		if (!restored.ok) return true;
		return !(0, node_util.isDeepStrictEqual)(restored.result, params.originalConfig);
	} catch {
		return true;
	}
}
...
const log = require_subsystem.createSubsystemLogger("config/redaction");
const ENV_VAR_PLACEHOLDER_PATTERN = /^\$\{[^}]*\}$/;
function isSensitivePath(path) {
	if (path.endsWith("[]")) return require_sensitive_paths.isSensitiveConfigPath(path.slice(0, -2));
	return require_sensitive_paths.isSensitiveConfigPath(path);
}
function isEnvVarPlaceholder(value) {
	return ENV_VAR_PLACEHOLDER_PATTERN.test(value.trim());
}
function isWholeObjectSensitivePath(path) {
	const lowered = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(path);
	return lowered.endsWith("serviceaccount") || lowered.endsWith("serviceaccountref");
}
function isSensitiveUrlPath(path) {
	return (0, _gabrielvfonseca_net_policy_redact_sensitive_url.isSensitiveUrlConfigPath)(path);
}
function hasSensitiveUrlHintPath(hints, paths) {
	if (!hints) return false;
	return paths.some((path) => (0, _gabrielvfonseca_net_policy_redact_sensitive_url.hasSensitiveUrlHintTag)(hints[path]));
}
...
function redactObjectWithLookup(obj, lookup, prefix, values, hints) {
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(obj)) {
		const result = {};
		for (const [key, value] of Object.entries(obj)) {
			...
			for (const candidate of [path, wildcardPath]) {
				...
				if (lookup.has(candidate)) {
					...
					if (typeof value === "string" && !isEnvVarPlaceholder(value)) {
						...
						path,
						wildcardPath
					]) || isSensitiveUrlPath(path))) if ((0, _gabrielvfonseca_net_policy_redact_sensitive_url.redactSensitiveUrlLikeString)(value) !== value) {
						values.push(value);
						result[key] = REDACTED_SENTINEL;
			...
			if (!matched) {
				...
				if (typeof value === "string" && !markedNonSensitive && isSensitivePath(path) && !isEnvVarPlaceholder(value)) {
					result[key] = REDACTED_SENTINEL;
					values.push(value);
				} else if (typeof value === "string" && (hasSensitiveUrlHintPath(hints, [path, wildcardPath]) || isSensitiveUrlPath(path))) if ((0, _gabrielvfonseca_net_policy_redact_sensitive_url.redactSensitiveUrlLikeString)(value) !== value) {
					values.push(value);
					result[key] = REDACTED_SENTINEL;
				} else result[key] = value;
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(obj)) {
		...
		for (const [key, value] of Object.entries(obj)) {
			...
			if (!isExplicitlyNonSensitivePath(hints, [dotPath, wildcardPath]) && isSensitivePath(dotPath) && typeof value === "string" && !isEnvVarPlaceholder(value)) {
				...
				result[key] = REDACTED_SENTINEL;
			} else if (typeof value === "string" && (hasSensitiveUrlHintPath(hints, [dotPath, wildcardPath]) || isSensitiveUrlPath(dotPath))) if ((0, _gabrielvfonseca_net_policy_redact_sensitive_url.redactSensitiveUrlLikeString)(value) !== value) {
				values.push(value);
				result[key] = REDACTED_SENTINEL;
			} else result[key] = value;
...
var RedactionError = class extends Error {
	constructor(key, humanReadableMessage) {
		...
		this.key = key;
		this.humanReadableMessage = humanReadableMessage ?? `Sentinel value "__OPERATOR_REDACTED__" in key ${key} is not valid as real data`;
...
function assertNoRedactedSentinel(value, path) {
	if (typeof value === "string" && value === "__OPERATOR_REDACTED__") {
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value)) for (const [key, item] of Object.entries(value)) assertNoRedactedSentinel(item, path ? `${path}.${key}` : key);
}
function maybeRestoreSecretRefId(params) {
	const incomingObj = toObjectRecord(params.incoming);
	if (!isSecretRefShape(incomingObj) || incomingObj.id !== "__OPERATOR_REDACTED__") return { handled: false };
...
function toObjectRecord(value) {
	return (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value) ? value : {};
}
...
function restoreArrayItemWithLookup(params) {
	if (params.item === "__OPERATOR_REDACTED__") return params.originalArray[params.index];
...
function restoreArrayItemWithGuessing(params) {
	if (!isExplicitlyNonSensitivePath(params.hints, [params.path]) && isSensitivePath(params.path) && params.item === "__OPERATOR_REDACTED__") return params.originalArray[params.index];
...
function restoreRedactedEntryGuessing(params) {
	...
	if (params.value === "__OPERATOR_REDACTED__" && canRestoreSecretRef) return restoreOriginalValueOrThrow({
		key: params.key,
		path: params.path,
...
function restoreRedactedValuesWithLookup(incoming, original, lookup, prefix, hints) {
	...
	for (const [key, value] of Object.entries(toObjectRecord(incoming))) {
		...
		for (const candidate of [path, wildcardPath]) if (lookup.has(candidate)) {
			matched = true;
			if (value === "__OPERATOR_REDACTED__" && (hints[candidate]?.sensitive === true || hasSensitiveUrlHintPath(hints, [
				candidate,
				path,
				wildcardPath
...
//#endregion
Object.defineProperty(exports, "REDACTED_SENTINEL", {
	enumerable: true,
	get: function() {
		return REDACTED_SENTINEL;
	}
});
Object.defineProperty(exports, "redactConfigObject", {
	enumerable: true,
	get: function() {
		return redactConfigObject;
	}
});
Object.defineProperty(exports, "redactConfigSnapshot", {
	enumerable: true,
	get: function() {
		return redactConfigSnapshot;
	}
});
Object.defineProperty(exports, "restoreRedactedValues", {
	enumerable: true,
	get: function() {
