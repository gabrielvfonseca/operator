...
const require_runtime$1 = require("./runtime-B6pBPYCa.cjs");
...
async function listMSTeamsDirectoryPeersLive(params) {
	const query = require_graph_users.normalizeQuery(params.query);
	if (!query) return [];
	return (await require_graph_users.searchGraphUsers({
		token: await require_graph_users.resolveGraphToken(params.cfg),
		query,
...
async function listMSTeamsDirectoryGroupsLive(params) {
	const rawQuery = require_graph_users.normalizeQuery(params.query);
	if (!rawQuery) return [];
	const token = await require_graph_users.resolveGraphToken(params.cfg);
	const limit = typeof params.limit === "number" && params.limit > 0 ? params.limit : 20;
	const [teamQuery, channelQuery] = rawQuery.includes("/") ? require_string_normalization.normalizeStringEntries(rawQuery.split("/", 2)) : [rawQuery, null];
	const teams = await require_graph_users.listTeamsByName(token, teamQuery);
	const results = [];
	for (const team of teams) {
		...
		const channels = await require_graph_users.listChannelsForTeam(token, teamId);
		for (const channel of channels) {
			const name = channel.displayName?.trim();
			if (!name) continue;
			if (!require_string_coerce.normalizeLowercaseStringOrEmpty(name).includes(require_string_coerce.normalizeLowercaseStringOrEmpty(channelQuery))) continue;
			results.push({
				kind: "group",
...
async function getMessageMSTeams(params) {
	...
	const msg = await require_graph_users.fetchGraphJson({
		token,
...
async function pinMessageMSTeams(params) {
	...
	return {
		ok: true,
		pinnedMessageId: (await require_graph_users.postGraphJson({
			token,
...
async function unpinMessageMSTeams(params) {
	...
	if (conv.kind === "channel") throw new Error("Pin/unpin is not supported for channel messages on Graph v1.0. Only chat conversations support pinned messages.");
	await require_graph_users.deleteGraphRequest({
		token,
...
async function listPinsMSTeams(params) {
	...
	const allPins = [];
	let res = await require_graph_users.fetchGraphJson({
		token,
		path
	});
	let pages = 1;
	while (true) {
		...
		res = await require_graph_users.fetchGraphAbsoluteUrl({
			token,
			url: nextLink
		});
		pages++;
...
async function reactMessageMSTeams(params) {
	const reactionType = require_messenger.resolveMSTeamsReactionEmoji(params.reactionType);
	const token = await require_graph_users.resolveGraphToken(params.cfg, { preferDelegated: true });
	const { basePath } = resolveConversationPath(await resolveGraphConversationId(params.to));
	await require_graph_users.postGraphBetaJson({
		token,
...
async function listReactionsMSTeams(params) {
	...
	return { reactions: Array.from(grouped.entries()).map(([type, group]) => ({
		reactionType: type,
		name: type,
		emoji: require_messenger.getMSTeamsReactionEmoji(type),
		count: group.count,
		users: group.users
	})) };
}
...
function normalizeSearchText(message) {
	...
	return message.body?.contentType?.toLowerCase() === "html" ? require_messenger.stripHtmlFromTeamsMessage(content) : content.trim();
}
...
async function searchMessagesMSTeams(params) {
	...
	for (let page = 0; page < SEARCH_MAX_PAGES; page++) {
		const response = nextUrl ? await require_graph_users.fetchGraphAbsoluteUrl({
			token,
			url: nextUrl
		}) : await require_graph_users.fetchGraphJson({
			token,
...
async function findMSTeamsConversationMember(params) {
	...
	while (nextPath && pages < MAX_CONVERSATION_MEMBER_PAGES && !member) {
		const response = await require_graph_users.fetchGraphJson({
			token: params.token,
			path: nextPath
		});
...
async function addParticipantMSTeams(params) {
	...
	const body = {
		...
		"user@odata.bind": `https://graph.microsoft.com/v1.0/users('${require_graph_users.escapeOData(params.userId)}')`
	};
	await require_graph_users.postGraphJson({
		token,
...
async function renameGroupMSTeams(params) {
	...
	await require_graph_users.patchGraphJson({
		token,
		path: conv.basePath,
		body
	});
...
async function findStandardChannelMember(params) {
	...
	if ((await require_graph_users.fetchGraphJson({
		token: params.token,
...
async function listChannelsMSTeams(params) {
	...
	while (nextPath && page < MAX_PAGES) {
		const res = await require_graph_users.fetchGraphJson({
			token,
			path: nextPath
		});
...
async function getChannelInfoMSTeams(params) {
	const ch = await require_graph_users.fetchGraphJson({
		token: await require_graph_users.resolveGraphToken(params.cfg),
...
function resolveMSTeamsProactiveReplyStyle(params) {
	...
	const routeConfig = require_html_entity_runtime.resolveMSTeamsRouteConfig({
		cfg: params.cfg,
		teamId: params.ref.teamId,
		conversationId: params.conversationId,
		allowNameMatching: false
	});
	const { replyStyle } = require_html_entity_runtime.resolveMSTeamsReplyPolicy({
...
async function resolveMSTeamsSendContext(params) {
	...
	const log = require_runtime$1.getMSTeamsRuntime().logging.getChildLogger({ name: "msteams:send" });
	if (ref.serviceUrl && !require_graph_users.isAllowedBotFrameworkServiceUrl(ref.serviceUrl)) {
		try {
		...
		throw new Error(`Stored Microsoft Teams conversation reference has blocked serviceUrl host: ${require_graph_users.describeBotFrameworkServiceUrlHost(ref.serviceUrl)}. The bot must receive a new message from this conversation before it can send proactively.`);
	}
	const safeRef = ref.serviceUrl ? {
		...ref,
		serviceUrl: require_graph_users.normalizeBotFrameworkServiceUrl(ref.serviceUrl)
	} : ref;
	if (recipient.type === "user") {
		const resolvedType = require_string_coerce.normalizeLowercaseStringOrEmpty(safeRef.conversation?.conversationType ?? "");
	...
	const sdkCloudOptions = require_graph_users.resolveMSTeamsSdkCloudOptions(msteamsCfg);
	const { app } = await require_graph_users.loadMSTeamsSdkWithAuth(creds, sdkCloudOptions);
	require_graph_users.validateMSTeamsProactiveServiceUrlBoundary({
	...
	const storedConversationType = require_string_coerce.normalizeLowercaseStringOrEmpty(safeRef.conversation?.conversationType ?? "");
...
function createMSTeamsSendReceipt(params) {
	const receipt = require_runtime.createMessageReceiptFromOutboundResults({
		kind: params.kind,
...
async function sendMessageMSTeams(params) {
	...
	const tableMode = require_markdown_tables.resolveMarkdownTableMode({
		cfg,
		channel: "msteams"
	});
	const messageText = require_tables.convertMarkdownTables(text ?? "", tableMode);
	...
	if (mediaUrl) {
		const media = await require_runtime_api.loadOutboundMediaFromUrl(mediaUrl, {
		...
		try {
			...
			throw new Error(`msteams file send failed${status}: ${require_errors.formatUnknownError(err)}${hint ? ` (${hint})` : ""}`, { cause: err });
		}
	}
...
async function sendTextWithMedia(ctx, text, mediaUrl) {
	...
	try {
		...
		const hint = require_errors.formatMSTeamsSendErrorHint(classification);
		const status = classification.statusCode ? ` (HTTP ${classification.statusCode})` : "";
		throw new Error(`msteams send failed${status}: ${require_errors.formatUnknownError(err)}${hint ? ` (${hint})` : ""}`, { cause: err });
	}
...
async function sendProactiveActivityRaw({ app, ref, activity, serviceUrlBoundary }) {
	return require_messenger.extractMessageId(await require_messenger.sendMSTeamsActivityWithReference(app, require_messenger.buildConversationReference(ref), activity, { serviceUrlBoundary })) ?? "unknown";
}
async function sendProactiveActivity({ app, ref, activity, errorPrefix, serviceUrlBoundary }) {
	try {
		...
		const hint = require_errors.formatMSTeamsSendErrorHint(classification);
		const status = classification.statusCode ? ` (HTTP ${classification.statusCode})` : "";
		throw new Error(`${errorPrefix} failed${status}: ${require_errors.formatUnknownError(err)}${hint ? ` (${hint})` : ""}`, { cause: err });
	}
}
/**
...
async function editMessageMSTeams(params) {
	...
	try {
		await require_messenger.updateMSTeamsActivityWithReference(app, require_messenger.buildConversationReference(ref), activityId, {
			type: "message",
			id: activityId,
			text
		}, { serviceUrlBoundary: sdkCloudOptions });
		...
		const hint = require_errors.formatMSTeamsSendErrorHint(classification);
		const status = classification.statusCode ? ` (HTTP ${classification.statusCode})` : "";
		throw new Error(`msteams edit failed${status}: ${require_errors.formatUnknownError(err)}${hint ? ` (${hint})` : ""}`, { cause: err });
	}
...
async function deleteMessageMSTeams(params) {
	...
	try {
		await require_messenger.deleteMSTeamsActivityWithReference(app, require_messenger.buildConversationReference(ref), activityId, { serviceUrlBoundary: sdkCloudOptions });
	} catch (err) {
		const classification = require_errors.classifyMSTeamsSendError(err);
		const hint = require_errors.formatMSTeamsSendErrorHint(classification);
		const status = classification.statusCode ? ` (HTTP ${classification.statusCode})` : "";
		throw new Error(`msteams delete failed${status}: ${require_errors.formatUnknownError(err)}${hint ? ` (${hint})` : ""}`, { cause: err });
	}
...
function resolveMSTeamsTextSend(params) {
	return require_send_deps.resolveOutboundSendDep(params.deps, "msteams") ?? ((to, text) => sendMessageMSTeams({
		cfg: params.cfg,
		to,
		text
	}));
...
function resolveMSTeamsMediaSend(params) {
	return require_send_deps.resolveOutboundSendDep(params.deps, "msteams") ?? ((to, text, opts) => sendMessageMSTeams({
		cfg: params.cfg,
		to,
		text,
...
const msteamsOutbound = {
	...
	sendPayload: async ({ cfg, to, text, mediaUrl, mediaLocalRoots, mediaReadFile, payload, deps, onDeliveryResult }) => {
		...
		const mediaUrls = require_string_normalization.normalizeStringEntries(require_reply_payload.resolvePayloadMediaUrls({
			...payload,
		...
		if (mediaUrls.length > 0) {
			...
			const result = await require_reply_payload.sendPayloadMediaSequence({
				text,
				mediaUrls,
		...
		if (text.trim()) {
			...
			const chunks = require_reply_payload.resolveTextChunksWithFallback(text, require_runtime_api.chunkTextForOutbound(text, MSTEAMS_TEXT_CHUNK_LIMIT));
			let result;
...
function readStringArray(value) {
	...
	const out = require_string_normalization.normalizeStringEntries(value);
	return out.length > 0 ? out : void 0;
}
function readScopes(value) {
	if (typeof value !== "string") return;
	const out = require_string_normalization.normalizeStringEntries(value.split(/\s+/));
	return out.length > 0 ? out : void 0;
}
async function probeMSTeams(cfg) {
	const creds = require_graph_users.resolveMSTeamsCredentials(cfg);
	if (!creds) return {
		ok: false,
		error: "missing credentials (appId, appPassword, tenantId)"
	};
	try {
		const { app } = await require_graph_users.loadMSTeamsSdkWithAuth(creds, require_graph_users.resolveMSTeamsSdkCloudOptions(cfg));
		const tokenProvider = require_graph_users.createMSTeamsTokenProvider(app);
		if (!await tokenProvider.getAccessToken("https://api.botframework.com")) throw new Error("Failed to acquire bot token");
		let graph;
		try {
			const accessToken = require_graph_users.readAccessToken(await tokenProvider.getAccessToken("https://graph.microsoft.com"));
		...
		if (cfg?.delegatedAuth?.enabled) try {
			const tokens = require_graph_users.loadDelegatedTokens();
			if (tokens) {
				const isExpired = !require_parse_finite_number.isFutureDateTimestampMs(tokens.expiresAt);
				delegatedAuth = {
					ok: !isExpired,
