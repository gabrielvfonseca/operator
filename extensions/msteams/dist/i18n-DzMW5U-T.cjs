	...
	en: {
		...
		wizard: {
			...
			gateway: {
				...
				plaintextPasswordHint: "Stores the password directly in Operator config",
			...
			remote: {
				...
				insecureRemoteUrl: "Use wss:// for remote hosts, or ws://127.0.0.1/localhost via SSH tunnel. Break-glass: OPERATOR_ALLOW_INSECURE_PRIVATE_WS=1 for trusted private networks.",
				...
				plaintextPasswordHint: "Stores the password directly in Operator config",
				plaintextPasswordLabel: "Enter password now",
				plaintextTokenHint: "Stores the token directly in Operator config",
			...
			guided: {
				...
				complete: "Operator is ready.",
				completeWithoutAi: "Operator setup is saved. Connect AI before opening chat.",
				...
				invalidConfigDetails: "Operator config {path} is invalid:\n{issues}",
				...
				ttyRequired: "Onboarding needs an interactive TTY. Use `operator onboard --non-interactive --accept-risk ...` for automation.",
			...
			security: {
				...
				beta: "Operator is a hobby project and still in beta. Expect sharp edges.",
				confirm: "I understand this is personal-by-default and shared/multi-user use requires lock-down. Continue?",
				hardeningRequired: "If you're not comfortable with security hardening and access control, don't run Operator.",
				learnMore: "Learn more",
				notMultitenant: "Operator is not a hostile multi-tenant boundary by default.",
				personalAgent: "By default, Operator is a personal agent: one trusted operator boundary.",
			...
			skills: {
				...
				manualPrereqsDoctorHint: "Install those tools first, then run `operator doctor` to review skills + requirements.",
			...
			clickclack: {
				...
				helpCreateToken: "In ClickClack: Workspace settings → Integrations → Operator → create bot → copy token",
			...
			nextcloudTalk: {
				...
				helpInstallCommand: "2) Run: ./occ talk:bot:install \"Operator\" \"<shared-secret>\" \"<webhook-url>\" --feature webhook --feature response --feature reaction",
			...
			whatsapp: {
				...
				ownerNumberNote: "We need the sender/owner number so Operator can allowlist you.",
				...
				separatePhoneLabel: "Separate phone just for Operator",
			...
			synologyChat: {
				...
				incomingWebhookHelpReplies: "This is the URL Operator uses to send replies back to Chat.",
			...
			signal: {
				...
				multipleEntries: "Multiple entries: comma-separated.",
				nextLinkDevice: "Link device with: signal-cli link -n \"Operator\"",
			...
			channelsPrimer: {
				...
				blurbs: {
					...
					synologyChat: "Connect your Synology NAS Chat to Operator with full agent capabilities.",
			...
			finalize: {
				...
				dashboardCopyPaste: "Copy/paste this URL in a browser on this machine to control Operator.",
				dashboardLinkWithToken: "Dashboard link (with token): {url}",
				dashboardOpened: "Opened in your browser. Keep that tab to control Operator.",
				...
				daemonRuntimeNodeHint: "Required because Operator state uses node:sqlite; Bun cannot run the Gateway.",
				...
				gatewayTokenStored: "Stored in: $OPERATOR_CONFIG_PATH (default: ~/.operator/operator.json) under gateway.auth.token, or in OPERATOR_GATEWAY_TOKEN.",
				...
				outroDashboardLink: "Onboarding complete. Use the dashboard link above to control Operator.",
				outroDashboardOpened: "Onboarding complete. Dashboard opened; keep that tab to control Operator.",
				...
				containerSystemdUnavailable: "Systemd user services are not available inside this container. Operator is skipping only the background service install; run the Gateway in the foreground or use your container supervisor.",
				securityReminder: "Running agents on your computer is risky — harden your setup: https://docs.operator.ai/security",
	...
	"zh-CN": {
		...
		wizard: {
			...
			gateway: {
				...
				plaintextPasswordHint: "直接把密码存入 Operator 配置",
			...
			migration: {
				...
				appliedTitle: "迁移已应用",
				cancelled: "迁移已取消",
				complete: "迁移完成。下一步运行 `operator doctor`。",
			...
			remote: {
				...
				insecureRemoteUrl: "远程主机请使用 wss://，或通过 SSH 隧道使用 ws://127.0.0.1/localhost。紧急绕过：可信私有网络可设置 OPERATOR_ALLOW_INSECURE_PRIVATE_WS=1。",
				...
				plaintextPasswordHint: "直接把密码存入 Operator 配置",
				plaintextPasswordLabel: "现在输入密码",
				plaintextTokenHint: "直接把令牌存入 Operator 配置",
			...
			guided: {
				...
				appliedTitle: "推理已就绪",
				complete: "Operator 已准备就绪。",
				completeWithoutAi: "Operator 设置已保存。连接 AI 后再打开聊天。",
				detected: "AI 检测完成。",
				detectedCandidate: "{label} — {detail}{recommended}",
				...
				escapeHatches: "如需完整的分步向导，请运行 `operator onboard --classic`。",
				...
				intro: "连接你的 AI",
				invalidConfigDetails: "Operator 配置 {path} 无效：\n{issues}",
				...
				nextSteps: "工作区：{workspace}\n添加频道：`operator channels add`\n更喜欢聊天？运行 `operator setup`，然后说 `connect telegram`（或 `connect slack`）。\n打开 dashboard：`operator dashboard`\n稍后聊天：`operator`",
				nextStepsWithoutAi: "工作区：{workspace}\n稍后添加 AI：重新运行 `operator onboard`\n连接 AI 后添加频道：`operator channels add`\n打开 dashboard：`operator dashboard`",
				nextStepsTitle: "下一步",
				...
				ttyRequired: "Onboarding 需要交互式 TTY。自动化请使用 `operator onboard --non-interactive --accept-risk ...`。",
				welcomeTitle: "设置选项"
			},
			setup: {
			...
			security: {
				...
				beta: "Operator 是一个爱好项目，仍处于 beta 阶段。请预期会有边缘问题。",
				confirm: "我理解 Operator 默认面向个人使用；共享/多用户使用需要加固。继续？",
				hardeningRequired: "如果你不熟悉安全加固和访问控制，请不要运行 Operator。",
				learnMore: "了解更多",
				notMultitenant: "Operator 默认不是敌对多租户安全边界。",
				personalAgent: "默认情况下，Operator 是个人 agent：一个可信操作者边界。",
			...
			skills: {
				...
				docsLine: "文档：https://docs.operator.ai/skills",
				enterEnv: "输入 {env}",
				...
				manualPrereqsDoctorHint: "请先安装这些工具，然后运行 `operator doctor` 检查技能和要求。",
			...
			clickclack: {
				...
				helpCreateToken: "在 ClickClack 中：工作区设置 → Integrations → Operator → 创建 bot → 复制 token",
			...
			nextcloudTalk: {
				...
				helpInstallCommand: "2) 运行：./occ talk:bot:install \"Operator\" \"<shared-secret>\" \"<webhook-url>\" --feature webhook --feature response --feature reaction",
			...
			whatsapp: {
				...
				ownerNumberNote: "需要发送者/所有者号码，以便 Operator 将你加入允许列表。",
				...
				separatePhoneLabel: "Operator 专用独立手机",
			...
			irc: {
				...
				nextStatusCommand: "命令：operator channels status --probe",
			...
			synologyChat: {
				...
				incomingWebhookHelpReplies: "这是 Operator 用来向 Chat 发送回复的 URL。",
			...
			signal: {
				...
				multipleEntries: "多个条目请用逗号分隔。",
				nextLinkDevice: "绑定设备：signal-cli link -n \"Operator\"",
			...
			twitch: {
				...
				envPrompt: "检测到 Twitch 环境变量 OPERATOR_TWITCH_ACCESS_TOKEN。使用环境 token？",
				helpCopyToken: "3. 复制 token（以 'oauth:' 开头）和 Client ID",
				helpCreateApp: "1. 在 https://dev.twitch.tv/console 创建 Twitch 应用",
				helpEnvVars: "支持环境变量：OPERATOR_TWITCH_ACCESS_TOKEN",
			...
			channelsPrimer: {
				...
				blurbs: {
					...
					synologyChat: "将 Synology NAS Chat 连接到 Operator，并启用完整 agent 能力。",
			...
			search: {
				...
				configured: "已配置",
				configureLaterHint: "稍后可用 operator configure --section web 配置",
				docsLine: "文档：{url}",
			...
			finalize: {
				...
				controlUiTitle: "Control UI",
				controlUiDocs: "文档：https://docs.operator.ai/web/control-ui",
				dashboardCopyPaste: "在本机浏览器中复制/粘贴这个 URL 来控制 Operator。",
				dashboardLinkWithToken: "Dashboard 链接（含令牌）：{url}",
				dashboardOpened: "已在浏览器中打开。保留该标签页以控制 Operator。",
				...
				daemonRuntimeNodeHint: "Operator 状态使用 node:sqlite，因此必须使用 Node；Bun 无法运行网关。",
				...
				gatewayTokenStored: "存储位置：$OPERATOR_CONFIG_PATH（默认 ~/.operator/operator.json）中的 gateway.auth.token，或 OPERATOR_GATEWAY_TOKEN。",
				...
				nodeMac: "macOS app（system + notifications）",
				optionalApps: "可选应用",
				outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 链接控制 Operator。",
				outroDashboardOpened: "Onboarding 完成。Dashboard 已打开；保留该标签页以控制 Operator。",
				...
				containerSystemdUnavailable: "此容器内没有 systemd 用户服务。Operator 只会跳过后台服务安装；请以前台方式运行 Gateway，或使用你的容器 supervisor。",
				securityReminder: "在你的电脑上运行 agent 存在风险，请加固设置：https://docs.operator.ai/security",
				...
				webDocs: "文档：https://docs.operator.ai/tools/web",
				...
				whatNow: "下一步：https://operator.ai/showcase（\"What People Are Building\"）。",
				whatNowTitle: "下一步",
				workspaceBackupTitle: "工作区备份",
				workspaceDocs: "文档：https://docs.operator.ai/concepts/agent-workspace",
	...
	"zh-TW": {
		...
		wizard: {
			...
			gateway: {
				...
				plaintextPasswordHint: "直接把密碼存入 Operator 設定",
			...
			migration: {
				...
				appliedTitle: "遷移已套用",
				cancelled: "遷移已取消",
				complete: "遷移完成。下一步執行 `operator doctor`。",
			...
			remote: {
				...
				insecureRemoteUrl: "遠端主機請使用 wss://，或透過 SSH 通道使用 ws://127.0.0.1/localhost。緊急繞過：可信私有網路可設定 OPERATOR_ALLOW_INSECURE_PRIVATE_WS=1。",
				...
				plaintextPasswordHint: "直接把密碼存入 Operator 設定",
				plaintextPasswordLabel: "現在輸入密碼",
				plaintextTokenHint: "直接把權杖存入 Operator 設定",
			...
			guided: {
				...
				apiKeyPrompt: "{label} 的 API key 或 token",
				appliedTitle: "推理已就緒",
				complete: "Operator 已準備就緒。",
				completeWithoutAi: "Operator 設定已儲存。連接 AI 後再開啟聊天。",
				detected: "AI 偵測完成。",
				detectedCandidate: "{label} — {detail}{recommended}",
				detectedTitle: "找到的 AI",
				...
				escapeHatches: "如需完整的分步精靈，請執行 `operator onboard --classic`。",
				...
				intro: "連接你的 AI",
				invalidConfigDetails: "Operator 設定 {path} 無效：\n{issues}",
				...
				nextSteps: "工作區：{workspace}\n新增頻道：`operator channels add`\n偏好聊天？執行 `operator setup`，然後說 `connect telegram`（或 `connect slack`）。\n開啟 dashboard：`operator dashboard`\n稍後聊天：`operator`",
				nextStepsWithoutAi: "工作區：{workspace}\n稍後新增 AI：重新執行 `operator onboard`\n連接 AI 後新增頻道：`operator channels add`\n開啟 dashboard：`operator dashboard`",
				nextStepsTitle: "下一步",
				...
				ttyRequired: "Onboarding 需要互動式 TTY。自動化請使用 `operator onboard --non-interactive --accept-risk ...`。",
				welcomeTitle: "設定選項"
			},
			setup: {
			...
			security: {
				...
				beta: "Operator 是一個興趣專案，仍處於 beta 階段。請預期會有邊緣問題。",
				confirm: "我理解 Operator 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
				hardeningRequired: "如果你不熟悉安全加固和存取控制，請不要執行 Operator。",
				learnMore: "了解更多",
				notMultitenant: "Operator 預設不是敵對多租戶安全邊界。",
				personalAgent: "預設情況下，Operator 是個人 agent：一個可信操作者邊界。",
			...
			skills: {
				...
				docsLine: "文件：https://docs.operator.ai/skills",
				enterEnv: "輸入 {env}",
				...
				manualPrereqsDoctorHint: "請先安裝這些工具，然後執行 `operator doctor` 檢查技能和需求。",
			...
			clickclack: {
				...
				helpCreateToken: "在 ClickClack 中：工作區設定 → Integrations → Operator → 建立 bot → 複製 token",
			...
			nextcloudTalk: {
				...
				helpInstallCommand: "2) 執行：./occ talk:bot:install \"Operator\" \"<shared-secret>\" \"<webhook-url>\" --feature webhook --feature response --feature reaction",
			...
			whatsapp: {
				...
				ownerNumberNote: "需要傳送者/擁有者號碼，以便 Operator 將你加入允許清單。",
				...
				separatePhoneLabel: "Operator 專用獨立手機",
			...
			synologyChat: {
				...
				incomingWebhookHelpReplies: "這是 Operator 用來向 Chat 傳送回覆的 URL。",
			...
			signal: {
				...
				multipleEntries: "多個項目請用逗號分隔。",
				nextLinkDevice: "綁定裝置：signal-cli link -n \"Operator\"",
			...
			twitch: {
				...
				envPrompt: "偵測到 Twitch 環境變數 OPERATOR_TWITCH_ACCESS_TOKEN。使用環境 token？",
				helpCopyToken: "3. 複製 token（以 'oauth:' 開頭）和 Client ID",
				helpCreateApp: "1. 在 https://dev.twitch.tv/console 建立 Twitch 應用",
				helpEnvVars: "支援環境變數：OPERATOR_TWITCH_ACCESS_TOKEN",
			...
			channelsPrimer: {
				...
				blurbs: {
					...
					synologyChat: "將 Synology NAS Chat 連接到 Operator，並啟用完整 agent 能力。",
			...
			search: {
				...
				configured: "已設定",
				configureLaterHint: "稍後可用 operator configure --section web 設定",
				docsLine: "文件：{url}",
			...
			finalize: {
				...
				controlUiTitle: "Control UI",
				controlUiDocs: "文件：https://docs.operator.ai/web/control-ui",
				dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 Operator。",
				dashboardLinkWithToken: "Dashboard 連結（含權杖）：{url}",
				dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 Operator。",
				...
				daemonRuntimeNodeHint: "Operator 狀態使用 node:sqlite，因此必須使用 Node；Bun 無法執行閘道。",
				...
				gatewayTokenStored: "儲存位置：$OPERATOR_CONFIG_PATH（預設 ~/.operator/operator.json）中的 gateway.auth.token，或 OPERATOR_GATEWAY_TOKEN。",
				...
				nodeMac: "macOS app（system + notifications）",
				optionalApps: "可選應用",
				outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 Operator。",
				outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 Operator。",
				...
				containerSystemdUnavailable: "此容器內沒有 systemd 使用者服務。Operator 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
				securityReminder: "在你的電腦上執行 agent 存在風險，請加固設定：https://docs.operator.ai/security",
				...
				webDocs: "文件：https://docs.operator.ai/tools/web",
				...
				workspaceDocs: "文件：https://docs.operator.ai/concepts/agent-workspace",
...
//#endregion
Object.defineProperty(exports, "createSetupTranslator", {
	enumerable: true,
	get: function() {
		return createSetupTranslator;
	}
});
...
Object.defineProperty(exports, "wizardT", {
	enumerable: true,
	get: function() {
		return wizardT;
	}
