...
async function resolveCandidates(params) {
	...
	const [{ ensureStandaloneMigrationProviderRegistryLoaded, resolvePluginMigrationProviders }, { resolveManifestContractRuntimePluginResolution }, { createMigrationLogger }, { resolveStateDir }] = await Promise.all([
		Promise.resolve().then(() => require("./migration-provider-runtime-BfZcEKfA.cjs")).then((n) => n.migration_provider_runtime_exports),
		Promise.resolve().then(() => require("./manifest-contract-runtime-x2ZEDLx5.cjs")).then((n) => n.manifest_contract_runtime_exports),
	...
	for (const provider of providers) {
		...
		try {
			...
			logger.debug?.(`Post-install migration detect for ${provider.id} failed: ${require_errors.formatErrorMessage(error)}`);
		}
	}
	return candidates;
}
...
function logMigrationHint(runtime, candidate) {
	const command = require_command_format.formatCliCommand(`operator migrate ${candidate.provider.id} --dry-run`);
...
function applyMigrationConfigPatches(config, result) {
	const patches = (result?.items ?? []).filter((item) => Boolean(item && typeof item === "object" && "kind" in item && item.kind === "config" && "action" in item && item.action === "merge" && "status" in item && item.status === "migrated")).map(require_migration.readMigrationConfigPatchDetails).filter((patch) => patch !== void 0);
	if (patches.length === 0) return config;
	const nextConfig = structuredClone(config);
	for (const patch of patches) require_migration.writeMigrationConfigPath(nextConfig, patch.path, patch.value);
	return nextConfig;
}
/**
...
async function offerPostInstallMigrations(params) {
	...
	for (const candidate of candidates) {
		...
		try {
			...
			params.runtime.log(`Skipping ${candidate.provider.label} migration prompt: ${require_errors.formatErrorMessage(error)}`);
			logMigrationHint(params.runtime, candidate);
			continue;
		}
		if (!accepted) {
		...
		try {
			const [{ migrateDefaultCommand }, { createMigrationLogger }, { resolveStateDir }] = await Promise.all([
				Promise.resolve().then(() => require("./migrate-F376ET1M.cjs")),
				loadMigrationContextModule(),
				loadConfigPathsModule()
			]);
			...
			params.runtime.log(`${candidate.provider.label} migration failed: ${require_errors.formatErrorMessage(error)}. Re-run with ${require_command_format.formatCliCommand(`operator migrate ${candidate.provider.id} --dry-run`)} to inspect.`);
		} finally {
...
//#endregion
exports.offerPostInstallMigrations = offerPostInstallMigrations;
