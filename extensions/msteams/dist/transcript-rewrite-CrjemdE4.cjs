...
const RAW_APPEND_MESSAGE = Symbol("operator.session.rawAppendMessage");
...
const invalidJsonlSlotType = "__operator_invalid_jsonl_slot";
...
function isTextContent(value) {
	return (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value) && value.type === "text" && typeof value.text === "string" && isOptionalString(value.textSignature);
}
function isThinkingContent(value) {
	return (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value) && value.type === "thinking" && typeof value.thinking === "string" && isOptionalString(value.thinkingSignature) && (value.redacted === void 0 || typeof value.redacted === "boolean");
}
function isImageContent(value) {
	return (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value) && value.type === "image" && typeof value.data === "string" && typeof value.mimeType === "string";
}
...
function isToolCallPayload(value) {
	return value === null || (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value) || typeof value === "string";
}
function isToolCallContent(value) {
	return (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value) && typeof value.type === "string" && repairableToolCallContentTypes.has(value.type) && hasToolCallId(value) && isString(value.name) && (value.arguments === void 0 || isToolCallPayload(value.arguments)) && (value.input === void 0 || isToolCallPayload(value.input)) && isOptionalString(value.thoughtSignature);
}
function isPersistedContentBlock(value) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value) || !isString(value.type)) return false;
	switch (value.type) {
...
function parseLeafControlEntry(entry) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(entry) || entry.type !== "leaf") return;
	const candidate = entry;
	...
	for (const rawEntry of fileEntries) {
		...
		if (!isSessionEntry(entry)) {
			if (isString(id)) {
				rejectedIds.add(id);
				rejectedParentById.set(id, isString(rawParentId) ? rawParentId : null);
				if (typeof rawType === "string" && rawType !== "session" && !id.startsWith("__operator_invalid_jsonl_slot_") && !sessionEntryTypes.has(rawType) && Object.hasOwn(rawRecord, "parentId") && (rawParentId === null || isString(rawParentId))) effectiveAppendParentId = id;
			}
			continue;
		}
		...
		if (!Object.hasOwn(rawRecord, "parentId") || !require_transcript_tree.isSessionTranscriptSideAppendEntry(rawRecord) && entry.parentId === effectiveAppendParentId && effectiveLeafId !== effectiveAppendParentId) logicalParentsById.set(entry.id, effectiveLeafId);
		...
		if (require_transcript_tree.isSessionTranscriptSideAppendEntry(rawRecord)) effectiveAppendMode = "side";
		else {
...
function fileEntryOrMigrationSlot(value, index) {
	...
	return {
		...
		id: `__operator_invalid_jsonl_slot_${index}`,
		parentId: null,
...
var TranscriptFileState = class {
	...
	buildSessionContext() {
		...
		return require_session_manager.buildSessionContext(entries, entries.at(-1)?.id ?? null, new Map(entries.map((entry) => [entry.id, entry])));
	}
	...
	appendEntry(entry) {
		...
		this.byId.set(entry.id, entry);
		this.appendParentId = entry.id;
		if (require_transcript_tree.isSessionTranscriptSideAppendEntry(entry)) this.appendMode = "side";
		else {
			this.leafId = entry.id;
			this.appendMode = void 0;
		}
...
async function readTranscriptFileState(sessionFile) {
	const fileEntries = require_session_manager.parseSessionEntries(await node_fs_promises.default.readFile(sessionFile, "utf-8")).map(fileEntryOrMigrationSlot);
	const migrated = sessionHeaderVersion(fileEntries.find((entry) => entry.type === "session") ?? null) < 3;
	require_session_manager.migrateSessionEntries(fileEntries);
	return createReadableTranscriptFileState({
		fileEntries,
		header: fileEntries.find((entry) => entry.type === "session") ?? null,
		migrated
...
async function writeTranscriptFileAtomic(filePath, entries) {
	await require_private_file_store.privateFileStore(node_path.default.dirname(filePath)).writeText(node_path.default.basename(filePath), serializeTranscriptFileEntries(entries));
}
...
async function resolveRuntimeTranscriptReadTarget(scope) {
	return await require_session_accessor.resolveSessionTranscriptRuntimeReadTarget(scope);
}
/**
...
async function rewriteSqliteRuntimeTranscript(params) {
	const marker = require_sqlite_marker.parseSqliteSessionFileMarker(params.target.sessionFile);
	if (!marker) return {
		changed: false,
		bytesFreed: 0,
		rewrittenEntries: 0,
	...
	const nextEvents = (await require_session_accessor.loadTranscriptEvents({
	...
	await require_session_accessor.replaceTranscriptEvents({
		agentId: marker.agentId,
		sessionId: marker.sessionId,
		sessionKey: params.target.sessionKey,
		storePath: marker.storePath
...
function rewriteTranscriptEntriesInSessionManager(params) {
	...
	if (firstMatchedEntry?.type !== "message") return {
		changed: false,
		bytesFreed: 0,
		rewrittenEntries: 0,
...
async function rewriteTranscriptEntriesInRuntimeTranscript(params) {
	...
	try {
		...
		if (require_sqlite_marker.parseSqliteSessionFileMarker(target.sessionFile)) return await rewriteSqliteRuntimeTranscript({
			target,
			request: params.request
		});
		sessionLock = await require_session_write_lock.acquireSessionWriteLock({
			sessionFile: target.sessionFile,
			...require_session_write_lock.resolveSessionWriteLockOptions(params.config)
		});
		...
		if (result.changed) {
			...
			require_logger.log.info(`[transcript-rewrite] rewrote ${result.rewrittenEntries} entr${result.rewrittenEntries === 1 ? "y" : "ies"} bytesFreed=${result.bytesFreed} sessionKey=${target.sessionKey}`);
		}
		return result;
	} catch (err) {
		const reason = require_errors.formatErrorMessage(err);
		require_logger.log.warn(`[transcript-rewrite] failed: ${reason}`);
		return {
			changed: false,
			bytesFreed: 0,
			rewrittenEntries: 0,
...
//#endregion
Object.defineProperty(exports, "TranscriptFileState", {
	enumerable: true,
	get: function() {
		return TranscriptFileState;
	}
});
Object.defineProperty(exports, "createTranscriptFileStateFromPersistedEntries", {
	enumerable: true,
	get: function() {
		return createTranscriptFileStateFromPersistedEntries;
	}
});
Object.defineProperty(exports, "getRawSessionAppendMessage", {
	enumerable: true,
	get: function() {
		return getRawSessionAppendMessage;
	}
});
Object.defineProperty(exports, "persistTranscriptStateMutation", {
	enumerable: true,
	get: function() {
		return persistTranscriptStateMutation;
	}
});
Object.defineProperty(exports, "readTranscriptFileState", {
	enumerable: true,
	get: function() {
		return readTranscriptFileState;
	}
});
Object.defineProperty(exports, "resolveRuntimeTranscriptReadTarget", {
	enumerable: true,
	get: function() {
		return resolveRuntimeTranscriptReadTarget;
	}
});
Object.defineProperty(exports, "rewriteTranscriptEntriesInRuntimeTranscript", {
	enumerable: true,
	get: function() {
		return rewriteTranscriptEntriesInRuntimeTranscript;
	}
});
Object.defineProperty(exports, "rewriteTranscriptEntriesInSessionManager", {
	enumerable: true,
	get: function() {
		return rewriteTranscriptEntriesInSessionManager;
	}
});
Object.defineProperty(exports, "rewriteTranscriptEntriesInState", {
	enumerable: true,
	get: function() {
		return rewriteTranscriptEntriesInState;
	}
});
Object.defineProperty(exports, "setRawSessionAppendMessage", {
	enumerable: true,
	get: function() {
		return setRawSessionAppendMessage;
	}
});
Object.defineProperty(exports, "writeTranscriptFileAtomic", {
	enumerable: true,
	get: function() {
		return writeTranscriptFileAtomic;
	}
});
