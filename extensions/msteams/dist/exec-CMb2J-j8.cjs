...
function appendCapturedOutput(capture, chunk, maxBytes, mode) {
	...
	while (capture.bytes > maxBytes && capture.chunks.length > 0) {
		const first = (0, _gabrielvfonseca_normalization_core.expectDefined)(capture.chunks[0], "chunks entry at 0");
...
function trimTruncatedUtf8Boundary(buffer, mode, truncatedBytes) {
	if (truncatedBytes === 0 || buffer.length === 0 || node_process.default.platform === "win32") return buffer;
	if (mode === "tail") {
		let start = 0;
		while (start < buffer.length && ((0, _gabrielvfonseca_normalization_core.expectDefined)(buffer[start], "buffer byte") & 192) === 128) start += 1;
...
function finalizeCapturedOutput(capture, mode) {
...
function trimPreservedPendingLine(value, maxBytes) {
	return require_utf8_truncate.truncateUtf8Suffix(value, maxBytes);
}
...
function spawnCommandWithInvocation(argv, options = {}) {
	...
	const invocation = require_windows_command.resolveSafeChildProcessInvocation({
		argv,
	...
	return {
		child: (0, execa.execa)(invocation.command, invocation.args, {
			...execaOptions,
			env: commandEnv,
			extendEnv: false,
			shell: false,
...
function resolveCommandEnv(params) {
	const baseEnv = params.baseEnv ?? node_process.default.env;
	const platform = params.platform ?? node_process.default.platform;
	const argv = params.argv;
	const shouldSuppressNpmFund = (() => {
		const cmd = node_path.default.basename(argv[0] ?? "");
	...
	return require_openclaw_exec_env.markOperatorExecEnv(resolvedEnv);
}
//#endregion
...
function createCommandTerminationController(params) {
	...
	const spawnTaskkillOrFallback = (args, onSpawnError) => {
		try {
			return spawnCommand([require_windows_install_roots.getWindowsSystem32ExePath("taskkill.exe"), ...args], {
	...
	const terminate = () => {
		...
		if (node_process.default.platform === "win32" && !directChildAlive) return false;
		if (params.killProcessTree && typeof childPid === "number") {
			...
			require_kill_tree.killProcessTree(childPid, { graceMs: 300 });
			return false;
		}
		if (!directChildAlive) return false;
		if (node_process.default.platform === "win32" && typeof childPid === "number") {
	...
	const settle = async () => {
		...
		if (node_process.default.platform !== "win32") require_kill_tree.killProcessTree(params.child.pid, { force: true });
	};
	return {
		terminate,
		settle
...
async function runCommandWithTimeout(argv, optionsOrTimeout) {
	...
	const resolvedTimeoutMs = typeof timeoutMs === "number" ? (0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(timeoutMs, 1) : void 0;
	const hasInput = input !== void 0;
	const stdio = require_spawn_utils.resolveCommandStdio({
		hasInput,
		preferInherit: true
	});
	...
	const windowsEncoding = require_spawn_utils.resolveWindowsConsoleEncoding();
	...
	const { child, invocation } = spawnCommandWithInvocation(argv, {
		...
		cwd,
		detached: Boolean(killProcessTree && node_process.default.platform !== "win32"),
		encoding: "buffer",
		baseEnv,
		env,
		forceKillAfterDelay: 300,
	...
	const ownsExitedProcessTree = Boolean(killProcessTree && node_process.default.platform !== "win32");
	const cancel = (reason) => {
	...
	const resolvedNoOutputTimeoutMs = typeof noOutputTimeoutMs === "number" && Number.isFinite(noOutputTimeoutMs) && noOutputTimeoutMs > 0 ? (0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(noOutputTimeoutMs, 1) : void 0;
	...
	const captureOutput = (capture, chunk, maxBytes, stream, captureMode) => {
		...
		if (usesCombinedTailCapture) {
			...
			const removeCapturedBytes = (index, requestedBytes) => {
				const entry = (0, _gabrielvfonseca_normalization_core.expectDefined)(combinedTailChunks[index], "combined tail chunk");
	...
	if (outputObserverError !== void 0) throw (0, _gabrielvfonseca_normalization_core_error_coercion.toErrorObject)(outputObserverError, "Command output observer failed");
	...
	return {
		pid: child.pid,
		stdout: require_spawn_utils.decodeWindowsOutputBuffer({
			buffer: finalizeCapturedOutput(stdoutCapture, stdoutCaptureMode),
			windowsEncoding
		}),
		stderr: require_spawn_utils.decodeWindowsOutputBuffer({
...
async function runExec(command, args, opts = 1e4) {
	const timeout = typeof opts === "number" ? (0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(opts, 1) : typeof opts.timeoutMs === "number" ? (0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(opts.timeoutMs, 1) : void 0;
	...
	try {
		...
		const windowsEncoding = require_spawn_utils.resolveWindowsConsoleEncoding();
		const decodedStdout = require_spawn_utils.decodeWindowsOutputBuffer({
			buffer: Buffer.from(stdout),
			windowsEncoding
		});
		const decodedStderr = require_spawn_utils.decodeWindowsOutputBuffer({
			buffer: Buffer.from(stderr),
			windowsEncoding
		});
		if (resolvedOptions?.logOutput !== false && require_globals.shouldLogVerbose()) {
			if (decodedStdout.trim()) require_logger.logDebug(decodedStdout.trim());
			if (decodedStderr.trim()) require_logger.logError(decodedStderr.trim());
		}
		return {
		...
		if (err && typeof err === "object") {
			...
			if (errorWithOutput.stdout instanceof Uint8Array) errorWithOutput.stdout = require_spawn_utils.decodeWindowsOutputBuffer({
				buffer: Buffer.from(errorWithOutput.stdout),
				windowsEncoding
			});
			if (errorWithOutput.stderr instanceof Uint8Array) errorWithOutput.stderr = require_spawn_utils.decodeWindowsOutputBuffer({
				buffer: Buffer.from(errorWithOutput.stderr),
				windowsEncoding
			});
		}
		if (resolvedOptions?.logOutput !== false && require_globals.shouldLogVerbose()) require_logger.logError(require_globals.danger(`Command failed: ${command}`));
		throw err;
	}
}
...
//#endregion
Object.defineProperty(exports, "isPlainCommandExitFailure", {
	enumerable: true,
	get: function() {
		return isPlainCommandExitFailure;
	}
});
Object.defineProperty(exports, "releaseChildProcessOutputAfterExit", {
	enumerable: true,
	get: function() {
		return releaseChildProcessOutputAfterExit;
	}
});
Object.defineProperty(exports, "runCommandBuffered", {
	enumerable: true,
	get: function() {
		return runCommandBuffered;
	}
});
Object.defineProperty(exports, "runCommandWithTimeout", {
	enumerable: true,
	get: function() {
		return runCommandWithTimeout;
	}
});
Object.defineProperty(exports, "runExec", {
	enumerable: true,
	get: function() {
		return runExec;
	}
});
Object.defineProperty(exports, "spawnCommand", {
	enumerable: true,
	get: function() {
		return spawnCommand;
	}
