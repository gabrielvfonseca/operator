...
function resolveToolDisplay(params) {
	const name = require_tool_display_common.normalizeToolName(params.name);
	const key = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(name);
	const spec = TOOL_MAP[key];
	const emoji = spec?.emoji ?? FALLBACK.emoji ?? "🧩";
	const title = spec?.title ?? require_tool_display_common.defaultTitle(name);
	const label = spec?.label ?? title;
	const toolDisplayParts = require_tool_display_common.resolveToolVerbAndDetailForArgs({
		toolKey: key,
		args: params.args,
		meta: params.meta,
		spec,
		...
		detailFormatKey: (raw) => require_tool_display_common.formatDetailKey(raw, DETAIL_LABEL_OVERRIDES)
	});
	const { verb } = toolDisplayParts;
	let { detail } = toolDisplayParts;
	if (detail) detail = require_utils.shortenHomeInString(detail);
	return {
		name,
		emoji,
		title,
...
function formatToolDetail(display) {
	return require_tool_display_common.formatToolDetailText(display.detail ? require_redact.redactToolDetail(display.detail) : void 0);
}
...
//#endregion
Object.defineProperty(exports, "formatToolDetail", {
	enumerable: true,
	get: function() {
		return formatToolDetail;
	}
});
Object.defineProperty(exports, "formatToolSummary", {
	enumerable: true,
	get: function() {
		return formatToolSummary;
	}
});
Object.defineProperty(exports, "resolveToolDisplay", {
	enumerable: true,
	get: function() {
