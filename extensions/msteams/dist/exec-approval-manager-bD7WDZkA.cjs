...
function sanitizeOptionalSingleLine(value) {
	...
	return normalized ? require_exec_approval_command_display.sanitizeExecApprovalDisplayText(normalized) : null;
}
function buildExecApprovalPresentation(params) {
	if (!isRecord(params.request)) return null;
	const request = params.request;
	const { commandText, commandPreview } = require_exec_approval_command_display.resolveExecApprovalCommandDisplay(request);
	if (!commandText.trim()) return null;
	return {
		kind: "exec",
		commandText,
		commandPreview,
		warningText: typeof request.warningText === "string" && request.warningText.trim() ? require_exec_approval_command_display.sanitizeExecApprovalWarningText(request.warningText) : null,
...
function buildPluginApprovalPresentation(params) {
	if (!isRecord(params.request)) return null;
	const request = params.request;
	const rawTitle = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(request.title);
	const rawDescription = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(request.description);
	if (!rawTitle || !rawDescription) return null;
	const title = require_exec_approval_command_display.sanitizeExecApprovalDisplayText(rawTitle);
	const description = require_exec_approval_command_display.sanitizeExecApprovalWarningText(rawDescription);
...
function buildSystemAgentApprovalPresentation(params) {
	if (!isRecord(params.request)) return null;
	const request = params.request;
	const title = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(request.title);
	const description = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(request.description);
	if (!title || !description || !/^[a-f0-9]{64}$/.test(request.proposalHash)) return null;
	return {
		kind: "system-agent",
		title: require_exec_approval_command_display.sanitizeExecApprovalDisplayText(title).slice(0, 80),
		description: require_exec_approval_command_display.sanitizeExecApprovalWarningText(description).slice(0, 512),
...
function resolveApprovalTimeoutMs(timeoutMs) {
	return (0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(timeoutMs, 1);
}
...
var ExecApprovalManager = class {
	...
	create(request, timeoutMs, id) {
		const now = Date.now();
		const expiresAtMs = (0, _gabrielvfonseca_normalization_core_number_coercion.resolveExpiresAtMsFromDurationMs)(resolveApprovalTimeoutMs(timeoutMs), { nowMs: now });
		if (expiresAtMs === void 0) throw new Error("approval expiry is unavailable");
		return {
			id: id === null || id === void 0 || id.length === 0 ? (0, node_crypto.randomUUID)() : id,
			request,
			createdAtMs: now,
			expiresAtMs
		};
	...
	*/
	register(record, _timeoutMs) {
		...
		if (persistence) {
			...
			const inserted = require_operator_approval_store.insertOperatorApproval({
				approval: {
					id: record.id,
					kind: this.approvalKind,
					presentation,
	...
	projectLocalRecord(record) {
		...
		return {
			id: record.id,
			resolutionRef: require_state_migrations_cron_run_logs.buildApprovalResolutionRef({
	...
	resolveDetailed(recordId, decision, resolver, localResolvedBy = null, localResolutionSource = "operator") {
		...
		let result;
		try {
			result = require_operator_approval_store.resolveOperatorApproval({
				id: recordId,
				decision,
				resolver,
	...
	forceDenyDetailed(recordId, reason, resolver, status = "denied", localDecision, requireDue = false, localResolvedBy = null) {
		...
		let result;
		try {
			result = require_operator_approval_store.forceDenyOperatorApproval({
				id: recordId,
				status,
				requireDue,
				reason,
	...
	persistStorageCorruptDeny(recordId) {
		...
		const result = require_operator_approval_store.forceDenyOperatorApproval({
			id: recordId,
			status: "denied",
	...
	*/
	...
	consumeAllowOnce(recordId, consumerId = recordId) {
		...
		const persistence = this.options.persistence;
		if (persistence) {
			const result = require_operator_approval_store.consumeOperatorApprovalAllowOnce({
				id: recordId,
				consumerId,
				expectedKind: this.approvalKind,
				runtimeEpoch: persistence.runtimeEpoch,
	...
	*/
	...
	lookupApprovalId(input, opts = {}) {
		...
		const lowerPrefix = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(normalized);
		const matches = [];
		...
		for (const [id, record] of candidates) {
			...
			if ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(id).startsWith(lowerPrefix)) matches.push(id);
		}
		if (matches.length === 1) return {
			kind: "prefix",
			id: (0, _gabrielvfonseca_normalization_core.expectDefined)(matches[0], "matches capture group 0")
		};
		if (matches.length > 1) return {
			kind: "ambiguous",
			ids: matches
...
//#endregion
Object.defineProperty(exports, "EXEC_APPROVAL_RESOLVED_ENTRY_GRACE_MS", {
	enumerable: true,
	get: function() {
		return EXEC_APPROVAL_RESOLVED_ENTRY_GRACE_MS;
	}
});
Object.defineProperty(exports, "ExecApprovalManager", {
	enumerable: true,
	get: function() {
