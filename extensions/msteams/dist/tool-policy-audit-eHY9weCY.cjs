...
const toolPolicyAuditLogger = require_subsystem.createSubsystemLogger("agents/tool-policy");
...
function normalizedToolNames(tools) {
	return require_tool_policy.normalizeToolList(tools.map((tool) => tool.name));
}
...
function removedToolRuleKind(toolName, policy) {
	if (Array.isArray(policy.deny) && policy.deny.length > 0 && !require_tool_policy_match.isToolAllowedByPolicyName(toolName, { deny: policy.deny })) return "deny";
...
function matchedPolicyRuleForTool(params) {
	if (params.ruleKind === "deny" && Array.isArray(params.policy.deny)) return params.policy.deny.find((entry) => !require_tool_policy_match.isToolAllowedByPolicyName(params.toolName, { deny: [entry] }));
}
...
function sanitizeAuditField(value) {
	...
	return `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(sanitized, MAX_AUDIT_FIELD_LENGTH)}...`;
}
...
function auditSandboxToolPolicyBlock(params) {
	const normalizedToolName = require_tool_policy.normalizeToolName(params.toolName);
...
//#endregion
Object.defineProperty(exports, "auditSandboxToolPolicyBlock", {
	enumerable: true,
	get: function() {
		return auditSandboxToolPolicyBlock;
	}
});
Object.defineProperty(exports, "auditToolPolicyFilter", {
	enumerable: true,
	get: function() {
		return auditToolPolicyFilter;
	}
});
Object.defineProperty(exports, "escapeControlCharsVisible", {
	enumerable: true,
	get: function() {
		return escapeControlCharsVisible;
	}
});
