...
function normalizeDeviceMetadataForPolicy(value) {
	...
	if (!trimmed) return "";
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(trimmed.normalize("NFKD").replace(/\p{M}/gu, ""));
}
//#endregion
...
const IOS_SYSTEM_COMMANDS = [require_node_commands.NODE_SYSTEM_NOTIFY_COMMAND];
const SYSTEM_COMMANDS = [
	...require_node_commands.NODE_SYSTEM_RUN_COMMANDS,
	...require_node_commands.NODE_EXEC_APPROVALS_COMMANDS,
	...require_node_commands.NODE_FILE_COMMANDS,
	require_node_commands.NODE_SYSTEM_NOTIFY_COMMAND,
	require_node_commands.NODE_BROWSER_PROXY_COMMAND,
	require_node_commands.NODE_MCP_TOOLS_CALL_COMMAND,
	require_node_commands.NODE_AGENT_CLI_CLAUDE_RUN_COMMAND
];
...
function listDangerousPluginNodeCommands() {
	...
	if (!registry) return [];
	return (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeUniqueStringEntries)([...registry.nodeHostCommands.filter((entry) => entry.command.dangerous === true).map((entry) => entry.command.command), ...registry.nodeInvokePolicies.filter((entry) => entry.policy.dangerous === true).flatMap((entry) => entry.policy.commands)]);
}
function listDefaultPluginNodeCommands(platformId) {
	...
	return (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeUniqueStringEntries)([...policyCommands, ...nodeHostCommands]);
}
...
function hasTalkSurface(node) {
	...
	return (node.caps ?? []).some((capability) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(capability) === "talk") || (node.commands ?? []).some((command) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(command)?.startsWith("talk."));
}
function resolveNodeCommandAllowlistInternal(cfg, node, options) {
	const platformId = normalizePlatformId(node?.platform, node?.deviceFamily);
	const base = filterDesktopHostCommandDefaults({
		platformId,
		commands: (0, _gabrielvfonseca_normalization_core.expectDefined)(PLATFORM_DEFAULTS[platformId], "platform defaults entry at platform id") ?? PLATFORM_DEFAULTS.unknown,
...
//#endregion
Object.defineProperty(exports, "DEFAULT_DANGEROUS_NODE_COMMANDS", {
	enumerable: true,
	get: function() {
		return DEFAULT_DANGEROUS_NODE_COMMANDS;
	}
});
Object.defineProperty(exports, "IOS_WATCH_RELAY_COMMANDS", {
	enumerable: true,
	get: function() {
		return IOS_WATCH_RELAY_COMMANDS;
	}
});
Object.defineProperty(exports, "PLATFORM_DEFAULTS", {
	enumerable: true,
	get: function() {
		return PLATFORM_DEFAULTS;
	}
});
Object.defineProperty(exports, "TALK_PTT_COMMANDS", {
	enumerable: true,
	get: function() {
		return TALK_PTT_COMMANDS;
	}
});
Object.defineProperty(exports, "isForegroundRestrictedPluginNodeCommand", {
	enumerable: true,
	get: function() {
		return isForegroundRestrictedPluginNodeCommand;
	}
});
Object.defineProperty(exports, "isNodeCommandAllowed", {
	enumerable: true,
	get: function() {
		return isNodeCommandAllowed;
	}
});
Object.defineProperty(exports, "listDangerousPluginNodeCommands", {
	enumerable: true,
	get: function() {
		return listDangerousPluginNodeCommands;
	}
});
Object.defineProperty(exports, "normalizeDeclaredNodeCommands", {
	enumerable: true,
	get: function() {
		return normalizeDeclaredNodeCommands;
	}
});
Object.defineProperty(exports, "resolveNodeCommandAllowlist", {
	enumerable: true,
	get: function() {
		return resolveNodeCommandAllowlist;
	}
});
Object.defineProperty(exports, "resolveNodePairingCommandAllowlist", {
	enumerable: true,
	get: function() {
		return resolveNodePairingCommandAllowlist;
	}
});
