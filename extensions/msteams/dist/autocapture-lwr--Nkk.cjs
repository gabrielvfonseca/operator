...
const require_skill_suggestions = require("./skill-suggestions-Da3PH0uQ.cjs");
...
function buildInstructionGroup(params) {
	const skillName = require_curator.normalizeSkillIndexName(params.skillName);
	if (!skillName) return;
	return {
		skillName,
...
function groupDurableInstructionProposals(params) {
	...
	for (const instruction of params.instructions) {
		...
		const existing = matchExistingSkill(instruction, existingSkills) ?? existingSkills.find((skill) => require_curator.normalizeSkillIndexName(skill.name) === inferred.skillName);
...
const log = require_subsystem.createSubsystemLogger("skills/research");
...
const skillCaptureQueue = new require_keyed_async_queue.KeyedAsyncQueue();
...
function fingerprintInstructions(instructions) {
	return require_crypto_digest.sha256Hex(instructions.map((instruction) => compactWhitespace(instruction).toLowerCase()).join("\n"));
}
...
async function runSkillResearchAutoCapture(params) {
	const workshopConfig = require_config.resolveSkillWorkshopConfig(params.config);
	...
	const sessionScope = {
		...
		sessionKey,
		storePath: require_paths.resolveStorePath(params.config?.session?.store, { agentId: params.ctx.agentId })
	};
	await skillCaptureQueue.enqueue(workspaceDir, async () => {
		...
		if (hasUnfailedSkillWorkshopMutationCall(turnMessages)) {
			...
			await require_skill_suggestions.recordSessionSkillCaptureSignals({
				...sessionScope,
				signalHashes
			});
			return;
		}
		const capturedSignalHashes = require_skill_suggestions.readSessionSkillCaptureSignalHashes(sessionScope);
		...
		const existingSkills = require_service.listWritableWorkspaceSkillSummaries(workspaceDir, {
		...
		const manifest = await require_service.listSkillProposals({ workspaceDir });
		...
		if (!workshopConfig.autonomous.enabled) {
			...
			try {
				if (!proposal.existingSkill) {
					if (await require_curator.readWorkspaceSkillFile(require_curator.resolveSkillProposalTarget({
						workspaceDir,
				...
				if (await require_skill_suggestions.recordSessionSkillSuggestion({
					...sessionScope,
					skillName: proposal.skillName,
					signalHash,
		...
		for (const proposal of proposals) {
			...
			const claimedSignalHashes = await require_skill_suggestions.claimSessionSkillCaptureSignals({
				...sessionScope,
				signalHash,
				signalHashes
			});
			...
			try {
				...
				for (const entry of pendingEntries) {
					const inspected = await require_service.inspectSkillProposal(entry.id, { workspaceDir });
				...
				const existingSkill = await require_curator.readWorkspaceSkillFile(existingSkills.find((entry) => entry.name === proposal.skillName)?.filePath ?? require_curator.resolveSkillProposalTarget({
					workspaceDir,
					skillName: proposal.skillName
				}).skillFile);
				const result = autocapturePending ? await require_service.reviseSkillProposal({
					workspaceDir,
					config: params.config,
					proposalId: autocapturePending.record.id,
					content: buildAutoCaptureUpdateContent(require_service.stripProposalFrontmatterForSkill(autocapturePending.content), proposal.content),
					evidence: [autocapturePending.record.evidence, proposal.evidence].filter((value) => Boolean(value)).join("\n")
				}) : existingSkill === null ? await require_service.proposeCreateSkill({
					workspaceDir,
					config: params.config,
				...
				}) : await require_service.proposeUpdateSkill({
					workspaceDir,
					config: params.config,
					agentId: params.ctx.agentId,
					skillName: proposal.skillName,
				...
				await require_skill_suggestions.releaseSessionSkillCaptureSignals({
					...sessionScope,
...
//#endregion
exports.runSkillResearchAutoCapture = runSkillResearchAutoCapture;
