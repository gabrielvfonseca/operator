...
var provider_auth_ref_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	extractEnvVarFromSourceLabel: () => extractEnvVarFromSourceLabel,
	promptSecretRefForSetup: () => promptSecretRefForSetup,
	resolveRefFallbackInput: () => resolveRefFallbackInput
});
const secretResolveLoader = require_lazy_promise.createLazyImportLoader(() => Promise.resolve().then(() => require("./resolve-B9vhODuI.cjs")).then((n) => n.resolve_exports));
...
function resolveDefaultProviderEnvVar(provider, config) {
	return require_provider_env_vars.getProviderEnvVars(provider, {
		...config ? { config } : {},
		includeUntrustedWorkspacePlugins: false
	})?.find((candidate) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(candidate) !== void 0);
}
function resolveDefaultFilePointerId(provider) {
	return `/providers/${require_json_pointer.encodeJsonPointerToken(provider)}/apiKey`;
}
function resolveRefFallbackInput(params) {
	const fallbackEnvVar = params.preferredEnvVar ?? require_provider_env_vars.getProviderEnvVars(params.provider, {
	...
	const value = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)((params.env ?? process.env)[fallbackEnvVar]);
	...
	return {
		ref: {
			source: "env",
			provider: require_ref_contract.resolveDefaultSecretProviderAlias(params.config, "env", { preferFirstProviderForSource: true }),
			id: fallbackEnvVar
		},
		resolvedValue: value
	};
...
async function promptEnvSecretRefForSetup(params) {
	...
	const envCandidate = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeStringifiedOptionalString)(await params.prompter.text({
		...
		validate: (value) => {
			...
			if (!require_types_secrets.isValidEnvSecretRefId(candidate)) return params.copy?.envVarFormatError ?? "Use an env var name like \"OPENAI_API_KEY\" (uppercase letters, numbers, underscores).";
			if (!(0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(env[candidate])) return params.copy?.envVarMissingError?.(candidate) ?? `Environment variable "${candidate}" is missing or empty in this session.`;
		}
	})) ?? "";
	const envVar = envCandidate && require_types_secrets.isValidEnvSecretRefId(envCandidate) ? envCandidate : params.defaultEnvVar;
	if (!envVar) throw new Error(`No valid environment variable name provided for provider "${params.provider}".`);
	const resolvedValue = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(env[envVar]);
	...
	await params.prompter.note(params.copy?.envValidatedMessage?.(envVar) ?? `Validated environment variable ${envVar}. Operator will store a reference, not the key value.`, "Reference validated");
	return {
		ref,
		resolvedValue
	};
...
async function promptProviderSecretRefForSetup(params) {
	...
	const defaultProvider = require_ref_contract.resolveDefaultSecretProviderAlias(params.config, "file", { preferFirstProviderForSource: true });
	...
	const id = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeStringifiedOptionalString)(await params.prompter.text({
		message: idPrompt,
		...
		validate: (value) => {
			...
			if (providerEntry.source === "file" && providerEntry.mode !== "singleValue" && !require_ref_contract.isValidFileSecretRefId(candidate)) return "Use an absolute JSON pointer like \"/providers/openai/apiKey\".";
			if (providerEntry.source === "file" && providerEntry.mode === "singleValue" && candidate !== "value") return "singleValue mode expects id \"value\".";
			if (providerEntry.source === "exec" && !require_ref_contract.isValidExecSecretRefId(candidate)) return require_ref_contract.formatExecSecretRefIdValidationMessage();
		}
	})) || idDefault;
	const ref = {
	...
	try {
		...
		await params.prompter.note(params.copy?.providerValidatedMessage?.(selectedProvider, id, providerEntry.source) ?? `Validated ${providerEntry.source} reference ${selectedProvider}:${id}. Operator will store a reference, not the key value.`, "Reference validated");
		return {
			ref,
			resolvedValue
		};
...
//#endregion
Object.defineProperty(exports, "extractEnvVarFromSourceLabel", {
	enumerable: true,
	get: function() {
		return extractEnvVarFromSourceLabel;
	}
});
Object.defineProperty(exports, "promptSecretRefForSetup", {
	enumerable: true,
	get: function() {
		return promptSecretRefForSetup;
	}
});
Object.defineProperty(exports, "provider_auth_ref_exports", {
	enumerable: true,
	get: function() {
		return provider_auth_ref_exports;
	}
});
Object.defineProperty(exports, "resolveRefFallbackInput", {
	enumerable: true,
	get: function() {
		return resolveRefFallbackInput;
	}
});
