...
const require_node_registry = require("./node-registry-BmdyTqu0.cjs");
...
function createNodeSubscriptionManager() {
	...
	const toPayloadJSON = (payload) => require_node_registry.serializeEventPayload(payload);
...
function isTalkCapableNode(node) {
	return node.caps.some((capability) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(capability) === TALK_CAPABILITY) || node.commands.some((command) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(command)?.startsWith(TALK_COMMAND_PREFIX));
}
//#endregion
...
function createGatewayNodeSessionRuntime(params) {
	const nodeRegistry = new require_node_registry.NodeRegistry({
	...
	const sessionEventSubscribers = require_server_chat_state.createSessionEventSubscriberRegistry();
	const sessionMessageSubscribers = require_server_chat_state.createSessionMessageSubscriberRegistry();
...
//#endregion
exports.createGatewayNodeSessionRuntime = createGatewayNodeSessionRuntime;
