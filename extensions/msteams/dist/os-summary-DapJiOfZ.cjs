...
function resolveDarwinProductVersion() {
	return ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)((0, node_child_process.spawnSync)("sw_vers", ["-productVersion"], { encoding: "utf-8" }).stdout) ?? "") || node_os.default.release();
}
/**
...
function resolveRuntimeOsLabel() {
	...
	const release = node_os.default.release();
	const cacheKey = `${platform}\0${release}\0${node_os.default.arch()}`;
	const cached = cachedRuntimeOsLabelByKey.get(cacheKey);
	if (cached !== void 0) return cached;
	const label = platform === "darwin" ? `macOS ${resolveDarwinProductVersion()}` : `${node_os.default.type()} ${release}`;
...
function resolveOsSummary() {
	const platform = node_os.default.platform();
	const rawRelease = node_os.default.release();
...
//#endregion
Object.defineProperty(exports, "resolveOsSummary", {
	enumerable: true,
	get: function() {
		return resolveOsSummary;
	}
});
Object.defineProperty(exports, "resolveRuntimeOsLabel", {
	enumerable: true,
	get: function() {
		return resolveRuntimeOsLabel;
	}
});
