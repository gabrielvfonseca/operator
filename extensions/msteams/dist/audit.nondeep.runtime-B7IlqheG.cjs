...
const require_audit_extra_async = require("./audit-extra.async-B5HLW22I.cjs");
...
function inferParamBFromIdOrName(text) {
	const matches = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(text).matchAll(/(?:^|[^a-z0-9])[a-z]?(\d+(?:\.\d+)?)b(?=[^a-z0-9]|$)/g);
	let best = null;
...
function resolveAuditModelId(cfg, raw, aliasIndex) {
	...
	return resolved ? require_model_selection_normalize.modelKey(resolved.provider, resolved.model) : raw;
}
...
function collectAuditModelRefs(cfg) {
	...
	add(require_model_input.resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model), "agents.defaults.model.primary");
	for (const fallback of require_model_input.resolveAgentModelFallbackValues(cfg.agents?.defaults?.model)) add(fallback, "agents.defaults.model.fallbacks");
	add(require_model_input.resolveAgentModelPrimaryValue(cfg.agents?.defaults?.imageModel), "agents.defaults.imageModel.primary");
	for (const fallback of require_model_input.resolveAgentModelFallbackValues(cfg.agents?.defaults?.imageModel)) add(fallback, "agents.defaults.imageModel.fallbacks");
...
function resolveToolPolicies$1(params) {
	const globalProviderPolicy = require_provider_tool_policy.resolveProviderToolPolicy({
	...
	return require_agent_tools_policy.resolveConfiguredToolPolicies({
		cfg: params.cfg,
...
function isBrowserEnabled(cfg) {
	...
	return require_manifest_owner_policy.passesManifestOwnerBasePolicy({
		plugin: { id: "browser" },
		normalizedConfig: require_config_activation_shared.normalizePluginsConfigWithResolver(cfg.plugins, (pluginId) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(pluginId) ?? "")
	});
}
...
function collectAttackSurfaceSummaryFindings(cfg) {
	...
	const internalHooksEnabled = require_configured.hasConfiguredInternalHooks(cfg);
	const browserEnabled = isBrowserEnabled(cfg);
	return [{
		checkId: "summary.attack_surface",
		severity: "info",
		title: "Attack surface summary",
		detail: `groups: open=${group.open}, allowlist=${group.allowlist}\ntools.elevated: ${elevated ? "enabled" : "disabled"}\nhooks.webhooks: ${webhooksEnabled ? "enabled" : "disabled"}\nhooks.internal: ${internalHooksEnabled ? "enabled" : "disabled"}\nbrowser control: ${browserEnabled ? "enabled" : "disabled"}\ntrust model: personal assistant (one trusted operator boundary), not hostile multi-tenant on one shared gateway. For multiple users or organizations, run one isolated Gateway cell per tenant: https://docs.operator.ai/gateway/multi-tenant-hosting`
	}];
}
...
function collectSmallModelRiskFindings(params) {
	...
	for (const entry of smallModels) {
		...
		const modelRef = require_model_selection_normalize.parseModelRef(entry.id, "openai", { allowPluginNormalization: false });
		const sandboxMode = require_config.resolveSandboxConfigForAgent(params.cfg, agentId ?? void 0).mode;
		...
		const exposed = [];
		if (isWebSearchEnabled(params.cfg, params.env) && require_tool_policy_match.isToolAllowedByPolicies("web_search", policies)) exposed.push("web_search");
		if (isWebFetchEnabled(params.cfg) && require_tool_policy_match.isToolAllowedByPolicies("web_fetch", policies)) exposed.push("web_fetch");
		if (isBrowserEnabled(params.cfg) && require_tool_policy_match.isToolAllowedByPolicies("browser", policies)) exposed.push("browser");
...
function findGatewayAuthLabelMatchingHooksToken(params) {
	return listActiveGatewaySharedSecrets(params.auth).find((candidate) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(candidate.value) === params.hooksToken)?.label;
}
...
function formatHooksTokenReuseRemediation(reuse) {
	...
	return `Run ${require_command_format.formatCliCommand("openclaw doctor --fix")} to rotate a persisted hooks.token, then update external hook senders to use the new hook token.`;
}
function hasResolvedGatewayHttpAuth(auth) {
	if (auth.mode === "token") return Boolean((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(auth.token));
	if (auth.mode === "password") return Boolean((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(auth.password));
...
function listKnownNodeCommands(cfg) {
	...
	for (const node of [
		...
		const allow = require_node_command_policy.resolveNodeCommandAllowlist(baseCfg, node);
		for (const cmd of allow) {
			const normalized = normalizeNodeCommand(cmd);
			if (normalized) out.add(normalized);
		}
	}
	for (const cmd of require_node_command_policy.resolveNodeCommandAllowlist(baseCfg, { caps: ["talk"] })) {
		const normalized = normalizeNodeCommand(cmd);
		if (normalized) out.add(normalized);
	}
	for (const cmd of require_node_command_policy.DEFAULT_DANGEROUS_NODE_COMMANDS) {
		const normalized = normalizeNodeCommand(cmd);
		if (normalized) out.add(normalized);
	}
	return out;
...
function editDistance(a, b) {
	...
	for (let i = 1; i <= a.length; i++) {
		let prev = (0, _gabrielvfonseca_normalization_core.expectDefined)(dp[0], "dp entry at 0");
		dp[0] = i;
		for (let j = 1; j <= b.length; j++) {
			const temp = dp[j];
			const cost = a[i - 1] === b[j - 1] ? 0 : 1;
			dp[j] = Math.min((0, _gabrielvfonseca_normalization_core.expectDefined)(dp[j], "dp entry at j") + 1, (0, _gabrielvfonseca_normalization_core.expectDefined)(dp[j - 1], "dp entry at j 1") + 1, prev + cost);
			prev = (0, _gabrielvfonseca_normalization_core.expectDefined)(temp, "audit extra.sync temp");
		}
	}
	return (0, _gabrielvfonseca_normalization_core.expectDefined)(dp[b.length], "dp entry at b.length");
}
...
function collectRiskyToolExposureContexts(cfg) {
	...
	for (const context of listAuditAgentToolContexts(cfg)) {
		const sandboxMode = require_config.resolveSandboxConfigForAgent(cfg, context.agentId).mode;
		const policies = require_agent_tools_policy.resolveConfiguredToolPolicies({
			cfg,
			agentTools: context.tools,
			sandboxMode,
			agentId: context.agentId ?? null
		});
		const runtimeTools = ["exec", "process"].filter((tool) => require_tool_policy_match.isToolAllowedByPolicies(tool, policies));
		const fsTools = [
			"read",
			"write",
			"edit",
			"apply_patch"
		].filter((tool) => require_tool_policy_match.isToolAllowedByPolicies(tool, policies));
...
function collectControlPlaneToolExposureContexts(cfg) {
	...
	for (const context of listAuditAgentToolContexts(cfg)) {
		...
		const controlPlaneTools = require_dangerous_tools.GATEWAY_CONTROL_PLANE_TOOLS.filter((tool) => require_tool_policy_match.isToolAllowedByPolicies(tool, policies));
...
function collectSyncedFolderFindings(params) {
	...
	if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) findings.push({
		...
		remediation: `Keep OPERATOR_STATE_DIR on a local-only volume and re-run "${require_command_format.formatCliCommand("openclaw security audit --fix")}".`
	});
	return findings;
}
function collectSecretsInConfigFindings(cfg) {
	const findings = [];
	const password = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(cfg.gateway?.auth?.password) ?? "";
	if (password && !looksLikeEnvRef(password)) findings.push({
		...
		remediation: "Prefer OPERATOR_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk."
	});
...
function collectHooksHardeningFindings(cfg, env = process.env, options = {}) {
	...
	const reusedGatewayAuth = findHooksTokenGatewayAuthReuse({
		hooksToken: token,
		configGatewayAuth: require_auth_resolve.resolveGatewayAuth({
			authConfig: cfg.gateway?.auth,
			tailscaleMode: cfg.gateway?.tailscale?.mode ?? "off",
			env
		}),
		overrideGatewayAuth: options.gatewayAuthOverride ? require_auth_resolve.resolveGatewayAuth({
			authConfig: cfg.gateway?.auth,
	...
	if (((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(cfg.hooks?.path) ?? "") === "/") findings.push({
	...
	const defaultSessionKey = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(cfg.hooks?.defaultSessionKey) ?? "";
...
function collectGatewayHttpSessionKeyOverrideFindings(cfg) {
	...
	findings.push({
		...
		detail: `${enabledEndpoints.join(", ")} accept x-operator-session-key for per-request session routing. Treat API credential holders as trusted principals.`
	});
	return findings;
}
function collectGatewayHttpNoAuthFindings(cfg, env, options = {}) {
	const findings = [];
	const tailscaleMode = cfg.gateway?.tailscale?.mode ?? "off";
	if (hasResolvedGatewayHttpAuth(require_auth_resolve.resolveGatewayAuth({
		authConfig: cfg.gateway?.auth,
		authOverride: options.gatewayAuthOverride,
		tailscaleMode,
		env
...
function collectSandboxDockerNoopFindings(cfg) {
	...
	const hasAnySandboxEnabledAgent = agents.some((entry) => {
		...
		return require_config.resolveSandboxConfigForAgent(cfg, entry.id).mode !== "off";
	});
	...
	for (const entry of agents) {
		...
		if (require_config.resolveSandboxConfigForAgent(cfg, entry.id).mode === "off") configuredPaths.push(`agents.list.${entry.id}.sandbox.docker`);
	}
...
function collectSandboxDangerousConfigFindings(cfg) {
	...
	for (const { source, docker } of configs) {
		...
		for (const bind of binds) {
			...
			const blocked = require_validate_sandbox_security.getBlockedBindReason(bind);
			if (!blocked) continue;
		...
		const normalizedNetwork = require_network_mode.normalizeNetworkMode(network);
		if (require_network_mode.isDangerousNetworkMode(network)) {
		...
		if ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(typeof docker.seccompProfile === "string" ? docker.seccompProfile : void 0) === "unconfined") findings.push({
		...
		if ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(typeof docker.apparmorProfile === "string" ? docker.apparmorProfile : void 0) === "unconfined") findings.push({
...
function collectNodeDangerousAllowCommandFindings(cfg) {
	...
	const allow = new Set((0, _gabrielvfonseca_normalization_core_string_normalization.normalizeUniqueStringEntries)(allowRaw.map(normalizeNodeCommand)));
	if (allow.size === 0) return findings;
	const deny = new Set((cfg.gateway?.nodes?.denyCommands ?? []).map(normalizeNodeCommand));
	const dangerousAllowed = [...require_node_command_policy.DEFAULT_DANGEROUS_NODE_COMMANDS, ...require_node_command_policy.listDangerousPluginNodeCommands()].filter((cmd) => allow.has(cmd) && !deny.has(cmd));
...
function collectLikelyMultiUserSetupFindings(cfg) {
	...
	findings.push({
		...
		detail: "Heuristic signals indicate this gateway may be reachable by multiple users:\n" + signals.map((signal) => `- ${signal}`).join("\n") + `\n${impactLine}\n${riskyContextsDetail}\nOperator's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway. For multiple users or organizations, run one isolated Gateway cell per tenant: https://docs.operator.ai/gateway/multi-tenant-hosting`,
...
async function safeStat(targetPath) {
	try {
		return {
			ok: true,
			isDir: (await node_fs_promises.default.lstat(targetPath)).isDirectory()
		};
	} catch {
		return {
			ok: false,
...
function realpathWithTimeout(p, timeoutMs = 2e3) {
	let timerHandle;
	const realpathPromise = node_fs_promises.default.realpath(p).catch(() => null).then((result) => {
...
async function listWorkspaceSkillMarkdownFiles(workspaceDir, limits = {}) {
	const skillsRoot = node_path.default.join(workspaceDir, "skills");
	...
	for (const _ of Array.from({ length: maxTotalDirVisits })) {
		...
		const dirRealPath = await realpathWithTimeout(dir) ?? node_path.default.resolve(dir);
		if (visitedDirs.has(dirRealPath)) continue;
		visitedDirs.add(dirRealPath);
		const entries = await node_fs_promises.default.readdir(dir, { withFileTypes: true }).catch(() => []);
		for (const entry of entries) {
			...
			if (entry.isSymbolicLink()) {
				const stat = await node_fs_promises.default.stat(fullPath).catch(() => null);
				if (!stat) continue;
				if (stat.isDirectory()) {
					queue.push(fullPath);
					continue;
...
async function collectWorkspaceSkillSymlinkEscapeFindings(params) {
	const findings = [];
	const workspaceDirs = require_workspace_dirs.listAgentWorkspaceDirs(params.cfg);
	if (workspaceDirs.length === 0) return findings;
	const escapedSkillFiles = [];
	const seenSkillPaths = /* @__PURE__ */ new Set();
	for (const workspaceDir of workspaceDirs) {
		const workspacePath = node_path.default.resolve(workspaceDir);
		...
		for (const skillFilePath of skillFilePaths) {
			const canonicalSkillPath = node_path.default.resolve(skillFilePath);
			...
			if ((0, _openclaw_fs_safe_path.isPathInside)(workspaceRealPath, skillRealPath)) continue;
			escapedSkillFiles.push({
				workspaceDir: workspacePath,
				skillFilePath: canonicalSkillPath,
				skillRealPath
...
function resolveAutoDefault(providerId, kind, options) {
	const id = require_registry.normalizeChannelId(providerId) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(providerId);
	if (!id) return false;
	if (typeof options?.autoDefault === "boolean") return options.autoDefault;
	const commandDefaults = require_registry.getLoadedChannelPlugin(id)?.commands ?? (options?.config ? require_read_only_command_defaults.resolveReadOnlyChannelCommandDefaults(id, {
		...options,
...
const loadPluginTrustPolicyDeps = require_lazy_promise.createLazyPromise(() => Promise.all([
	Promise.resolve().then(() => require("./config-DEJMoJiT.cjs")).then((n) => n.config_exports),
	Promise.resolve().then(() => require("./tool-policy-DWNs5HaX.cjs")).then((n) => n.tool_policy_exports),
	Promise.resolve().then(() => require("./tool-policy-match-CCdTHppY.cjs")).then((n) => n.tool_policy_match_exports),
	Promise.resolve().then(() => require("./tool-policy-CvMKC-hp.cjs")).then((n) => n.tool_policy_exports),
	Promise.resolve().then(() => require("./sandbox-tool-policy-DDU5nVeg.cjs")).then((n) => n.sandbox_tool_policy_exports)
...
async function isChannelPluginConfigured(cfg, plugin) {
	...
	for (const accountId of candidates) {
		const inspected = plugin.config.inspectAccount?.(cfg, accountId) ?? await require_read_only_account_inspect.inspectReadOnlyChannelAccount({
			channelId: plugin.id,
			cfg,
			accountId
		});
...
function normalizePluginIdSet(entries) {
	return new Set(entries.map((entry) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(entry)).filter((entry) => Boolean(entry)));
}
function resolveEnabledExtensionPluginIds(params) {
	const normalized = require_config_state.normalizePluginsConfig(params.cfg.plugins);
	...
	for (const [id, entry] of Object.entries(normalized.entries)) {
		const normalizedId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(id);
...
function collectAllowEntries(config) {
	...
	return out.map((entry) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(entry)).filter((entry) => Boolean(entry));
}
...
async function collectPluginsTrustFindings(params) {
	const findings = [];
	const { extensionsDir, pluginDirs } = await require_audit_extra_async.listInstalledPluginDirs({ stateDir: params.stateDir });
	if (pluginDirs.length > 0) {
		const allow = params.cfg.plugins?.allow;
		const allowConfigured = Array.isArray(allow) && allow.length > 0;
		if (allowConfigured) {
			const installedPluginIds = new Set(pluginDirs.map((dir) => node_path.default.basename(dir).toLowerCase()));
			const pluginIndex = require_plugin_registry.loadPluginRegistrySnapshot({
				config: params.cfg,
				stateDir: params.stateDir
			});
			const normalizePluginId = require_plugin_registry.createPluginRegistryIdNormalizer(pluginIndex);
			...
			const phantomEntries = allow.filter((entry) => {
				...
				const canonicalId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(normalizePluginId(entry)) ?? "";
		...
		if (!allowConfigured) {
			const channelPlugins = require_read_only.listReadOnlyChannelPluginsForConfig(params.cfg, { stateDir: params.stateDir });
		...
		if (enabledExtensionPluginIds.length > 0) {
			...
			for (const context of contexts) {
				...
				const broadPolicy = deps.isToolAllowedByPolicies("__operator_plugin_probe__", policies);
	...
	const pluginInstalls = await require_installed_plugin_index_record_reader.loadInstalledPluginIndexInstallRecords({ stateDir: params.stateDir });
	...
	if (npmPluginInstalls.length > 0) {
		...
		for (const [pluginId, record] of npmPluginInstalls) {
			...
			const installedVersion = await require_package_update_utils.readInstalledPackageVersion(record.installPath ?? node_path.default.join(params.stateDir, "extensions", pluginId));
	...
	if (npmHookInstalls.length > 0) {
		...
		for (const [hookId, record] of npmHookInstalls) {
			const recordedVersion = record.resolvedVersion ?? record.version;
			if (!recordedVersion) continue;
			const installedVersion = await require_package_update_utils.readInstalledPackageVersion(record.installPath ?? node_path.default.join(params.stateDir, "hooks", hookId));
			if (!installedVersion || installedVersion === recordedVersion) continue;
...
exports.collectGatewayHttpNoAuthFindings = collectGatewayHttpNoAuthFindings;
exports.collectGatewayHttpSessionKeyOverrideFindings = collectGatewayHttpSessionKeyOverrideFindings;
exports.collectHooksHardeningFindings = collectHooksHardeningFindings;
exports.collectIncludeFilePermFindings = require_audit_extra_async.collectIncludeFilePermFindings;
exports.collectLikelyMultiUserSetupFindings = collectLikelyMultiUserSetupFindings;
exports.collectMinimalProfileOverrideFindings = collectMinimalProfileOverrideFindings;
exports.collectModelHygieneFindings = collectModelHygieneFindings;
exports.collectNodeDangerousAllowCommandFindings = collectNodeDangerousAllowCommandFindings;
exports.collectNodeDenyCommandPatternFindings = collectNodeDenyCommandPatternFindings;
exports.collectPluginsTrustFindings = collectPluginsTrustFindings;
exports.collectSandboxBrowserHashLabelFindings = require_audit_extra_async.collectSandboxBrowserHashLabelFindings;
exports.collectSandboxDangerousConfigFindings = collectSandboxDangerousConfigFindings;
exports.collectSandboxDockerNoopFindings = collectSandboxDockerNoopFindings;
...
exports.collectStateDeepFilesystemFindings = require_audit_extra_async.collectStateDeepFilesystemFindings;
exports.collectSyncedFolderFindings = collectSyncedFolderFindings;
exports.collectWorkspaceSkillSymlinkEscapeFindings = collectWorkspaceSkillSymlinkEscapeFindings;
exports.readConfigSnapshotForAudit = require_audit_extra_async.readConfigSnapshotForAudit;
