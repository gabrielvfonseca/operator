...
function classifyCompactionReason(reason) {
	const text = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(reason);
...
function formatUnknownCompactionReasonDetail(reason) {
	const sanitized = require_ansi.sanitizeForLog((reason ?? "").replace(/\s+/g, " ")).trim().replace(/[^A-Za-z0-9._:@/+~-]+/g, "_").replace(/_+/g, "_").replace(/^_+|_+$/g, "");
	if (!sanitized) return;
	return sanitized.slice(0, MAX_COMPACTION_REASON_DETAIL_CHARS);
}
//#endregion
Object.defineProperty(exports, "DEFERRED_CONTEXT_ENGINE_COMPACTION_REASON", {
	enumerable: true,
	get: function() {
		return DEFERRED_CONTEXT_ENGINE_COMPACTION_REASON;
	}
});
Object.defineProperty(exports, "classifyCompactionReason", {
	enumerable: true,
	get: function() {
		return classifyCompactionReason;
	}
});
Object.defineProperty(exports, "formatUnknownCompactionReasonDetail", {
	enumerable: true,
	get: function() {
		return formatUnknownCompactionReasonDetail;
	}
});
Object.defineProperty(exports, "resolveCompactionFailureReason", {
	enumerable: true,
	get: function() {
		return resolveCompactionFailureReason;
	}
});
