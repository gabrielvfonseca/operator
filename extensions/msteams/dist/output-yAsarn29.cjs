...
var output_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	MIGRATION_CONFLICT_REASON_PHRASES: () => MIGRATION_CONFLICT_REASON_PHRASES,
	assertApplySucceeded: () => assertApplySucceeded,
	assertConflictFreePlan: () => assertConflictFreePlan,
	formatMigrationPreview: () => formatMigrationPreview,
	formatMigrationResult: () => formatMigrationResult,
	writeApplyResult: () => writeApplyResult
});
function formatCount(value, label) {
	return `${value} ${label}${value === 1 ? "" : "s"}`;
}
function formatPlanHeader(plan, heading) {
	const lines = [`${require_theme.theme.heading(heading)} ${plan.providerId}`, `Source: ${plan.source}`];
...
function formatPlanItems(plan, mode) {
	...
	for (const group of ITEM_GROUPS) {
		...
		lines.push("");
		lines.push(require_theme.theme.heading(group.heading));
		for (const item of items) lines.push(formatMigrationItem(item, mode));
	}
	if (other.length > 0) {
		lines.push("");
		lines.push(require_theme.theme.heading("Other:"));
...
function formatPlanWarnings(plan) {
	...
	const lines = ["", require_theme.theme.warn("Warnings:")];
...
function formatMigrationPreview(plan) {
	const safePlan = require_migration.redactMigrationPlan(plan);
	return [
...
function formatMigrationResult(plan) {
	...
	if (safePlan.nextSteps && safePlan.nextSteps.length > 0) {
		lines.push("");
		lines.push(require_theme.theme.heading("Next:"));
...
const REASON_CODE_MESSAGES = {
	...
	already_active: "Plugin is already active in Operator",
	installed: "Plugin is already installed in Operator",
...
function formatItemMessage(item, mode) {
	if (mode === "preview") {
		...
		if (item.kind === "skill" && item.action === "copy") return "Copy Codex skill into Operator";
		if (item.kind === "plugin" && item.action === "install") return "Install Codex plugin into Operator";
...
function formatMigrationItem(item, mode) {
	...
	const messageSuffix = message ? ` ${require_theme.theme.muted(`(${message})`)}` : "";
...
function assertConflictFreePlan(plan, providerId) {
	if (plan.summary.conflicts > 0) throw new Error(`Migration has ${formatCount(plan.summary.conflicts, "conflict")}. Re-run with --overwrite after reviewing operator migrate plan ${providerId}.`);
}
...
function writeApplyResult(runtime, opts, result) {
	if (opts.json) {
		require_runtime.writeRuntimeJson(runtime, require_migration.redactMigrationPlan(result));
		return;
	}
	_clack_prompts.log.message(formatMigrationResult(result).join("\n"));
	if (result.backupPath) runtime.log(`Backup: ${result.backupPath}`);
	else if (!opts.noBackup) runtime.log("Backup: skipped (no existing Operator state found)");
...
//#endregion
Object.defineProperty(exports, "MIGRATION_CONFLICT_REASON_PHRASES", {
	enumerable: true,
	get: function() {
		return MIGRATION_CONFLICT_REASON_PHRASES;
	}
});
Object.defineProperty(exports, "assertApplySucceeded", {
	enumerable: true,
	get: function() {
		return assertApplySucceeded;
	}
});
Object.defineProperty(exports, "assertConflictFreePlan", {
	enumerable: true,
	get: function() {
		return assertConflictFreePlan;
	}
});
Object.defineProperty(exports, "formatMigrationPreview", {
	enumerable: true,
	get: function() {
		return formatMigrationPreview;
	}
});
...
Object.defineProperty(exports, "writeApplyResult", {
	enumerable: true,
	get: function() {
