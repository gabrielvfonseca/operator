...
function withBundledPluginEnablementCompat(params) {
	...
	const allowSet = !bypassAllowlist && Array.isArray(allow) && allow.length > 0 ? new Set(allow.map((pluginId) => require_config_state.normalizePluginId(pluginId)).filter(Boolean)) : void 0;
...
function withBundledPluginVitestCompat(params) {
	...
	if (!Boolean(env.VITEST) || require_manifest_registry.hasExplicitPluginConfig(params.config?.plugins) || params.pluginIds.length === 0) return params.config;
...
//#endregion
Object.defineProperty(exports, "withBundledPluginEnablementCompat", {
	enumerable: true,
	get: function() {
		return withBundledPluginEnablementCompat;
	}
});
Object.defineProperty(exports, "withBundledPluginVitestCompat", {
	enumerable: true,
	get: function() {
