...
const require_deferred = require("./deferred-BZ1hKNtQ.cjs");
...
var WizardSession = class {
	...
	async next() {
		...
		if (!this.stepDeferred) this.stepDeferred = require_deferred.createDeferred();
	...
	cancel() {
		...
		this.abortController.abort(new require_prompts.WizardCancelledError());
		this.currentStep = null;
		for (const [, pending] of this.answerDeferred) pending.deferred.reject(new require_prompts.WizardCancelledError());
	...
	async awaitAnswer(step, validate) {
		...
		this.pushStep(step);
		const deferred = require_deferred.createDeferred();
...
//#endregion
Object.defineProperty(exports, "WizardSession", {
	enumerable: true,
	get: function() {
