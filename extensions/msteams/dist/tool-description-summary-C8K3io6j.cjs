...
function truncateSummary(value, maxLen = 120) {
	...
	const sliced = (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(value, maxLen - 3);
...
function summarizeToolDescriptionText(params) {
	const explicit = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.displaySummary) ?? "";
	if (explicit) return truncateSummary(normalizeSummaryWhitespace(explicit), params.maxLen);
	const raw = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.rawDescription) ?? "";
	if (!raw) return "Tool";
	const paragraphs = (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(raw.split(/\n\s*\n/g));
	for (const paragraph of paragraphs) {
		const lines = (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(paragraph.split("\n"));
...
function describeToolForVerbose(params) {
	...
	const sliced = (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(normalized, maxLen - 3);
	const boundary = sliced.lastIndexOf(" ");
	return `${(boundary >= Math.floor(maxLen / 2) ? sliced.slice(0, boundary) : sliced).trimEnd()}...`;
}
//#endregion
Object.defineProperty(exports, "describeToolForVerbose", {
	enumerable: true,
	get: function() {
		return describeToolForVerbose;
	}
});
Object.defineProperty(exports, "summarizeToolDescriptionText", {
	enumerable: true,
	get: function() {
