...
var remote_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	getRemoteSkillEligibility: () => getRemoteSkillEligibility,
	primeRemoteSkillsCache: () => primeRemoteSkillsCache,
	recordRemoteNodeBins: () => recordRemoteNodeBins,
	recordRemoteNodeInfo: () => recordRemoteNodeInfo,
	refreshRemoteBinsForConnectedNodes: () => refreshRemoteBinsForConnectedNodes,
	refreshRemoteNodeBins: () => refreshRemoteNodeBins,
	removeRemoteNodeInfo: () => removeRemoteNodeInfo,
	setSkillsRemoteRegistry: () => setSkillsRemoteRegistry
});
...
function isMacPlatform(platform, deviceFamily) {
	const platformNorm = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(platform);
	const familyNorm = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(deviceFamily);
...
function buildRemoteProbeSignature(params) {
	return JSON.stringify([
		params.command,
		(0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(params.platform),
		(0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(params.deviceFamily),
...
async function primeRemoteSkillsCache() {
	try {
		const list = await require_node_pairing.listNodePairing();
		let sawMac = false;
		...
		if (sawMac) require_plugin_skills.bumpSkillsSnapshotVersion({ reason: "remote-node" });
	} catch (err) {
...
function recordRemoteNodeInfo(node) {
	...
	require_workspace.recordRemoteSkillNodeInfo({
		nodeId: node.nodeId,
		connId: node.connId,
		displayName: node.displayName,
		commands: node.commands
...
function removeRemoteNodeInfo(nodeId) {
	...
	require_workspace.removeRemoteNodeSkills(nodeId);
	const probeState = remoteNodeProbeStates.get(nodeId);
	if (probeState && !probeState.bins) remoteNodeProbeStates.delete(nodeId);
	if (existing && isMacPlatform(existing.platform, existing.deviceFamily) && supportsSystemRun(existing.commands)) require_plugin_skills.bumpSkillsSnapshotVersion({ reason: "remote-node" });
}
...
function parseBinProbePayload(payloadJSON, payload) {
	...
	try {
		...
		if (Array.isArray(parsed.bins)) return (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(parsed.bins);
		if (parsed.bins && typeof parsed.bins === "object") return Object.entries(parsed.bins).filter(([, resolvedPath]) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(resolvedPath) !== void 0).map(([bin]) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(bin) ?? "").filter(Boolean);
		if (typeof parsed.stdout === "string") return parsed.stdout.split(/\r?\n/).map((line) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(line) ?? "").filter(Boolean);
	} catch {
		return [];
	}
	return [];
...
async function refreshRemoteNodeBinsUncoalesced(params) {
	...
	for (const workspaceDir of workspaceDirs) {
		const entries = require_workspace.loadWorkspaceSkillEntries(workspaceDir, { config: params.cfg });
	...
	if (shouldSkipRemoteNodeProbe({
		...
		if (restoreCachedRemoteNodeBins(params.nodeId)) require_plugin_skills.bumpSkillsSnapshotVersion({ reason: "remote-node" });
		return;
	}
	const logContext = {
		command,
	...
	if (typeof remoteRegistry.checkConnectivity === "function") {
		...
		try {
			...
			if (cleared) require_plugin_skills.bumpSkillsSnapshotVersion({ reason: "remote-node" });
			return;
		}
		if (!connectivity.ok) {
	...
	try {
		...
		await require_node_pairing.updatePairedNodeMetadata(params.nodeId, { bins });
		require_plugin_skills.bumpSkillsSnapshotVersion({ reason: "remote-node" });
	} catch (err) {
		if (!markRemoteNodeProbeFailure({
			nodeId: params.nodeId,
			connId: probeConnId,
...
//#endregion
Object.defineProperty(exports, "getRemoteSkillEligibility", {
	enumerable: true,
	get: function() {
		return getRemoteSkillEligibility;
	}
});
Object.defineProperty(exports, "recordRemoteNodeInfo", {
	enumerable: true,
	get: function() {
		return recordRemoteNodeInfo;
	}
});
Object.defineProperty(exports, "refreshRemoteNodeBins", {
	enumerable: true,
	get: function() {
		return refreshRemoteNodeBins;
	}
});
...
Object.defineProperty(exports, "removeRemoteNodeInfo", {
	enumerable: true,
	get: function() {
		return removeRemoteNodeInfo;
	}
});
