...
async function inspectWindowsGatewayFirewall(params) {
	...
	const runCommandWithTimeout$1 = params.runCommandWithTimeout ?? require_exec.runCommandWithTimeout;
	...
	if (mode === "quick") {
		...
		if (quickJson === null) return {
			...
			details: ["Run `operator gateway status --deep` again, or verify the advertised LAN URL from another device."]
		};
	...
	const [stateJson, rulesJson, netshOutput] = await Promise.all([
		...
		runBestEffortCommand(runCommandWithTimeout$1, [
			require_windows_install_roots.getWindowsSystem32ExePath("netsh.exe"),
			"advfirewall",
			"show",
			"allprofiles"
		], timeoutMs)
	...
	if (stateJson === null || rulesJson === null) return {
		...
		details: ["Run `operator gateway status --deep` from a normal PowerShell session and verify the advertised LAN URL from another device."]
	};
	...
	if (shouldProbeManagedActiveRules(classifyWindowsGatewayFirewallState(state))) {
		...
		if (managedRulesJson !== null) {
			...
			severity: "warning",
			code: "windows_firewall_inspection_failed",
			message: "Operator could not inspect managed Windows Firewall rules for LAN Gateway reachability.",
			details: ["Run `operator gateway status --deep` from a normal PowerShell session and verify Group Policy or administrator-managed allow rules for the Gateway port."]
		};
	}
...
function formatWindowsGatewayFirewallGuidance(params) {
	if ((params.platform ?? process.platform) !== "win32" || params.bind !== "lan") return [];
	return ["Windows firewall: if another device cannot connect to the LAN URL, run `operator gateway status --deep` from this Windows host."];
}
//#endregion
Object.defineProperty(exports, "formatWindowsGatewayFirewallGuidance", {
	enumerable: true,
	get: function() {
		return formatWindowsGatewayFirewallGuidance;
	}
});
Object.defineProperty(exports, "inspectWindowsGatewayFirewall", {
	enumerable: true,
	get: function() {
