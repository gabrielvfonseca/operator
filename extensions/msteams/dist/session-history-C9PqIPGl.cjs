...
async function scanJsonlFile(filePath) {
	...
	try {
		...
		const fh = await node_fs_promises.default.open(filePath, "r");
		try {
			const rl = node_readline.default.createInterface({ input: fh.createReadStream({ encoding: "utf-8" }) });
			let recordCount = 0;
...
function claudeCliSessionTranscriptPath(params) {
	...
	return node_path.default.join(require_claude_cli_project_dir.resolveClaudeCliProjectDirForWorkspace({
		workspaceDir,
...
async function claudeCliSessionTranscriptHasContent(params) {
	...
	const sessionId = normalizeClaudeCliSessionId(params.sessionId);
	require_helpers.cliBackendLog.warn(`claude-cli transcript probe v4 miss (sessionId-deterministic path, grace ${CLAUDE_CLI_TRANSCRIPT_FLUSH_GRACE_MS}ms): sessionId=${sessionId ?? ""} expectedPath=${expectedPath} fileExists=${second.fileExists}`);
	return false;
}
			...
			for (const line of tailText.split(/\r?\n/)) {
				...
				if (role === "assistant") {
					...
					for (const block of blocks) if (isClaudeTranscriptToolUseBlock(block)) {
						const id = require_cli_session_history.resolveToolUseId(block);
...
function extractFallbackTurnText(message) {
	...
	for (const block of content) {
		...
		if (require_cli_session_history.isToolCallBlock(rec) && typeof rec.name === "string") {
...
function formatClaudeCliFallbackPrelude(seed, options) {
	...
	if (seed.summaryText) {
		...
		if (summarySection.length <= remaining) {
			...
			const slice = (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(seed.summaryText, Math.max(0, remaining - 64));
...
function buildClaudeCliFallbackContextPrelude(params) {
	...
	if (!sessionId) return "";
	const seed = require_cli_session_history.readClaudeCliFallbackSeed({
...
function createAcpVisibleTextAccumulator() {
	...
	const resolveNextCandidate = (base, chunk) => {
		...
		if (require_tokens.isSilentReplyText(base, "NO_REPLY") && !chunk.startsWith(base) && startsWithWordChar(chunk)) return chunk;
	...
	return {
		consume(chunk) {
			...
			if (!visibleText) {
				...
				if (require_tokens.isSilentReplyText(trimmedLeadCandidate, "NO_REPLY") || require_tokens.isSilentReplyPrefixText(trimmedLeadCandidate, "NO_REPLY")) {
					pendingSilentPrefix = leadCandidate;
					return null;
				}
				if (require_tokens.startsWithSilentToken(trimmedLeadCandidate, "NO_REPLY")) {
					const stripped = require_tokens.stripLeadingSilentToken(leadCandidate, require_tokens.SILENT_REPLY_TOKEN);
					if (stripped) {
						pendingSilentPrefix = "";
...
if (process.env.VITEST || false) globalThis[Symbol.for("operator.attemptExecutionHelpersTestApi")] = {
...
/** Builds a reseed prompt that carries prior Operator transcript context. */
function buildCliSessionHistoryPrompt(params) {
	...
	const truncationMarker = "[Operator reseed history truncated; older turns dropped]";
	const renderTruncatedSummaryWithTail = (renderedSummary) => {
		...
		const summaryTruncated = (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(renderedSummary, Math.max(0, maxHistoryChars - 56 - separatorBudget - tailBudget)).trimEnd();
		const tailTruncated = tailBudget > 0 ? (0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(tailRaw, -tailBudget).trimStart() : "";
		return [
			truncationMarker,
			summaryTruncated,
			tailTruncated
	...
	if (summaryRendered) if (summaryRendered.length >= maxHistoryChars) renderedHistory = renderTruncatedSummaryWithTail(summaryRendered);
		...
		else if (tailRaw.length > remainingBudget) renderedHistory = `${summaryBlock}${truncationMarker}\n${(0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(tailRaw, -remainingBudget).trimStart()}`;
		else renderedHistory = `${summaryBlock}${tailRaw}`;
	}
	else renderedHistory = tailRaw.length > maxHistoryChars ? `${truncationMarker}\n${(0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(tailRaw, -maxHistoryChars).trimStart()}` : tailRaw;
...
async function readBoundedCliSessionTranscript(filePath, fileSize) {
	if (fileSize <= MAX_CLI_SESSION_HISTORY_FILE_BYTES) return {
		content: await node_fs_promises.default.readFile(filePath, "utf-8"),
		truncated: false
	};
	require_helpers.cliBackendLog.warn(`cli session history truncated to last ${MAX_CLI_SESSION_HISTORY_FILE_BYTES} bytes: ${filePath}`);
	const handle = await node_fs_promises.default.open(filePath, "r");
	try {
...
function isSafeTruncatedCliSessionTail(entries) {
	...
	for (const entry of entries) {
		const node = require_transcript_tree.parseSessionTranscriptTreeEntry(entry);
		if (!node) continue;
...
function parseCliSessionEntries(content) {
	for (const line of content.trim().split("\n")) {
		...
		try {
			...
			require_helpers.cliBackendLog.warn(`cli session history parse failed: ${require_errors.formatErrorMessage(error)}`);
			return;
		}
	}
	return require_session_manager.parseSessionEntries(content);
}
function resolveSafeCliSessionFile(params) {
	...
	const pathOptions = require_paths.resolveSessionFilePathOptions({
		agentId: sessionAgentId ?? defaultAgentId,
		storePath: params.config?.session?.store
	});
	const sessionFile = require_paths.resolveSessionFilePath(params.sessionId, { sessionFile: params.sessionFile }, pathOptions);
	return {
		sessionFile,
		sessionsDir: pathOptions?.sessionsDir ?? node_path.default.dirname(sessionFile)
	};
}
async function loadCliSessionEntries(params) {
	try {
		const { sessionFile, sessionsDir } = resolveSafeCliSessionFile(params);
		const entryStat = await node_fs_promises.default.lstat(sessionFile);
		if (!entryStat.isFile() || entryStat.isSymbolicLink()) return [];
		const realSessionsDir = await safeRealpath(sessionsDir) ?? node_path.default.resolve(sessionsDir);
		const realSessionFile = await safeRealpath(sessionFile);
		if (!realSessionFile || realSessionFile === realSessionsDir || !(0, _openclaw_fs_safe_path.isPathInside)(realSessionsDir, realSessionFile)) return [];
		const stat = await node_fs_promises.default.stat(realSessionFile);
		...
		if (transcript.truncated && !isSafeTruncatedCliSessionTail(rawSessionEntries)) {
			require_helpers.cliBackendLog.warn(`cli session history truncated tail skipped because branch controls are incomplete: ${realSessionFile}`);
			return [];
		}
		...
		return require_transcript_tree.selectSessionTranscriptLeafControlledPath(sessionEntries) ?? sessionEntries;
	} catch (error) {
		if (!isFileNotFoundError(error)) require_helpers.cliBackendLog.warn(`cli session history load failed: ${require_errors.formatErrorMessage(error)}`);
		return [];
	}
}
...
async function hasCliSessionTranscript(params) {
	try {
		...
		const realSessionsDir = await safeRealpath(sessionsDir) ?? node_path.default.resolve(sessionsDir);
		const realSessionFile = await safeRealpath(sessionFile);
		if (!realSessionFile || realSessionFile === realSessionsDir || !(0, _openclaw_fs_safe_path.isPathInside)(realSessionsDir, realSessionFile)) return false;
		return (await node_fs_promises.default.stat(realSessionFile)).isFile();
	} catch {
		return false;
	}
}
...
//#endregion
Object.defineProperty(exports, "buildAgentHookConversationMessages", {
	enumerable: true,
	get: function() {
		return buildAgentHookConversationMessages;
	}
});
Object.defineProperty(exports, "buildClaudeCliFallbackContextPrelude", {
	enumerable: true,
	get: function() {
		return buildClaudeCliFallbackContextPrelude;
	}
});
Object.defineProperty(exports, "buildCliSessionHistoryPrompt", {
	enumerable: true,
	get: function() {
		return buildCliSessionHistoryPrompt;
	}
});
Object.defineProperty(exports, "claudeCliSessionTranscriptHasContent", {
	enumerable: true,
	get: function() {
		return claudeCliSessionTranscriptHasContent;
	}
});
Object.defineProperty(exports, "claudeCliSessionTranscriptHasOrphanedToolUse", {
	enumerable: true,
	get: function() {
		return claudeCliSessionTranscriptHasOrphanedToolUse;
	}
});
Object.defineProperty(exports, "createAcpVisibleTextAccumulator", {
	enumerable: true,
	get: function() {
		return createAcpVisibleTextAccumulator;
	}
});
Object.defineProperty(exports, "hasCliSessionTranscript", {
	enumerable: true,
	get: function() {
		return hasCliSessionTranscript;
	}
});
Object.defineProperty(exports, "loadCliSessionContextEngineMessages", {
	enumerable: true,
	get: function() {
		return loadCliSessionContextEngineMessages;
	}
});
Object.defineProperty(exports, "loadCliSessionHistoryMessages", {
	enumerable: true,
	get: function() {
		return loadCliSessionHistoryMessages;
	}
});
Object.defineProperty(exports, "loadCliSessionReseedMessages", {
	enumerable: true,
	get: function() {
		return loadCliSessionReseedMessages;
	}
});
Object.defineProperty(exports, "resolveAutoCliSessionReseedHistoryChars", {
	enumerable: true,
	get: function() {
		return resolveAutoCliSessionReseedHistoryChars;
	}
});
Object.defineProperty(exports, "resolveFallbackRetryPrompt", {
	enumerable: true,
	get: function() {
		return resolveFallbackRetryPrompt;
	}
});
Object.defineProperty(exports, "sessionFileHasContent", {
	enumerable: true,
	get: function() {
		return sessionFileHasContent;
	}
});
