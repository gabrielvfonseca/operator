...
function resolvePluginWebSearchConfig(config, pluginId) {
	...
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(pluginConfig)) return;
	return (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(pluginConfig.webSearch) ? pluginConfig.webSearch : void 0;
}
//#endregion
//#region src/gateway/model-pricing-cache.ts
var model_pricing_cache_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	getCachedGatewayModelPricing: () => require_model_pricing_config.getCachedGatewayModelPricing,
	startGatewayModelPricingRefresh: () => startGatewayModelPricingRefresh
});
...
const log = require_subsystem.createSubsystemLogger("gateway").child("model-pricing");
...
function listLikeFallbacks(value) {
	...
	return Array.isArray(value.fallbacks) ? value.fallbacks.filter((entry) => typeof entry === "string").map((entry) => require_string_coerce.normalizeOptionalString(entry)).filter((entry) => Boolean(entry)) : [];
}
...
async function readPricingJsonObject(response, source) {
	...
	const buffer = await require_http_body.readResponseWithLimit(response, MAX_PRICING_CATALOG_BYTES, { onOverflow: ({ size }) => /* @__PURE__ */ new Error(`${source} pricing response too large: ${size} bytes`) });
	let payload;
	try {
...
function normalizeExternalPricingSource(value, options) {
	if (!value) return;
	return {
		...value.provider ? { provider: require_model_selection_normalize.normalizeModelRef(value.provider, "placeholder", options).provider } : {},
...
function filterActiveManifestRegistry(params) {
	return {
		...
		plugins: params.registry.plugins.filter((plugin) => require_installed_plugin_index.isInstalledPluginEnabled(params.index, plugin.id, params.config))
	};
}
...
function loadManifestPricingContext(registry, normalizationOptions) {
	...
	for (const row of require_manifest_planner.planManifestModelCatalogRows({ registry }).rows) {
		const pricing = toCachedModelPricing(row.cost);
		if (pricing) catalogPricing.set(require_model_selection_normalize.modelKey(row.provider, row.id), pricing);
	}
	return {
		policies,
		catalogPricing
...
function canonicalizeOpenRouterLookupId(id, options = {
	...
	const provider = require_model_selection_normalize.normalizeModelRef(trimmed.slice(0, slash), "placeholder", {
	...
	const normalizedModel = require_model_selection_normalize.normalizeModelRef(provider, model, {
		allowManifestNormalization: options.allowManifestNormalization,
		allowPluginNormalization: options.allowPluginNormalization,
		manifestPlugins: options.manifestPlugins
	}).model;
	return require_model_selection_normalize.modelKey(provider, normalizedModel);
}
function buildExternalCatalogCandidates(params) {
	const { ref, source, policies } = params;
	const refKey = require_model_selection_normalize.modelKey(ref.provider, ref.model);
	...
	for (const model of applyModelIdTransforms(ref.model, transforms)) {
		const candidate = require_model_selection_normalize.modelKey(provider, model);
	...
	if (sourcePolicy?.passthroughProviderModel && ref.model.includes("/")) {
		const nestedRef = require_model_selection_normalize.parseModelRef(ref.model, require_defaults.DEFAULT_PROVIDER, {
...
function addResolvedModelRef(params) {
	...
	if (!resolved) return;
	const normalized = require_model_selection_normalize.normalizeModelRef(resolved.ref.provider, resolved.ref.model, {
		allowManifestNormalization: params.allowManifestNormalization,
		allowPluginNormalization: params.allowPluginNormalization,
		manifestPlugins: params.manifestPlugins
	});
	params.refs.set(require_model_selection_normalize.modelKey(normalized.provider, normalized.model), normalized);
}
function addModelListLike(params) {
	addResolvedModelRef({
		raw: require_string_coerce.resolvePrimaryStringValue(params.value),
		aliasIndex: params.aliasIndex,
		refs: params.refs,
		allowManifestNormalization: params.allowManifestNormalization,
		allowPluginNormalization: params.allowPluginNormalization,
...
function findConfiguredProviderModel(config, ref, options = {
	...
	return (config.models?.providers?.[ref.provider])?.models?.find((model) => {
		const normalized = require_model_selection_normalize.normalizeModelRef(ref.provider, model.id, {
			allowManifestNormalization: options.allowManifestNormalization,
			allowPluginNormalization: options.allowPluginNormalization,
			manifestPlugins: options.manifestPlugins
		});
		return require_model_selection_normalize.modelKey(normalized.provider, normalized.model) === require_model_selection_normalize.modelKey(ref.provider, ref.model);
	});
}
...
function shouldFetchExternalPricingForRef(params) {
	if (params.seededPricing.has(require_model_selection_normalize.modelKey(params.ref.provider, params.ref.model))) return false;
...
async function fetchOpenRouterPricingCatalog(fetchImpl, signal) {
	...
	for (const entry of entries) {
		const obj = entry;
		const id = require_string_coerce.normalizeOptionalString(obj.id) ?? "";
...
function scheduleRefresh(params) {
	...
	refreshTimer = setTimeout(() => {
		...
		refreshGatewayModelPricingCache(params).catch((error) => {
			...
			log.warn(message);
			if (!params.signal?.aborted) require_model_pricing_config.recordGatewayModelPricingSourceFailure("refresh", message);
		});
	}, CACHE_TTL_MS);
	refreshTimer.unref?.();
}
function collectSeededPricing(params) {
	...
	for (const ref of params.refs) {
		const key = require_model_selection_normalize.modelKey(ref.provider, ref.model);
...
async function refreshGatewayModelPricingCache(params) {
	if (!require_model_pricing_config.isGatewayModelPricingEnabled(params.config)) {
		clearRefreshTimer();
		require_model_pricing_config.clearGatewayModelPricingFailures();
		return;
	}
	if (params.signal?.aborted) return;
	...
	inFlightRefresh = (async () => {
		...
		if (refs.length === 0) {
			if (params.signal?.aborted) return;
			require_model_pricing_config.replaceGatewayModelPricingCache(seededPricing);
			require_model_pricing_config.clearGatewayModelPricingFailures();
			clearRefreshTimer();
			return;
		}
		...
		const [catalogById, litellmCatalog] = await Promise.all([fetchOpenRouterPricingCatalog(fetchImpl, params.signal).then((catalog) => {
			require_model_pricing_config.clearGatewayModelPricingSourceFailure("openrouter");
			return catalog;
		}).catch((error) => {
			const message = formatPricingFetchFailure("OpenRouter", error);
			log.warn(message);
			openRouterFailed = true;
			if (!params.signal?.aborted) require_model_pricing_config.recordGatewayModelPricingSourceFailure("openrouter", message);
			return /* @__PURE__ */ new Map();
		}), fetchLiteLLMPricingCatalog(fetchImpl, params.signal).then((catalog) => {
			require_model_pricing_config.clearGatewayModelPricingSourceFailure("litellm");
			return catalog;
		}).catch((error) => {
			const message = formatPricingFetchFailure("LiteLLM", error);
			log.warn(message);
			litellmFailed = true;
			if (!params.signal?.aborted) require_model_pricing_config.recordGatewayModelPricingSourceFailure("litellm", message);
			return /* @__PURE__ */ new Map();
		})]);
		if (params.signal?.aborted) return;
		...
		for (const ref of refs) {
			...
			if (openRouterPricing && litellmPricing?.tieredPricing) nextPricing.set(require_model_selection_normalize.modelKey(ref.provider, ref.model), {
				...openRouterPricing,
				tieredPricing: litellmPricing.tieredPricing
			});
			else if (openRouterPricing) nextPricing.set(require_model_selection_normalize.modelKey(ref.provider, ref.model), openRouterPricing);
			else if (litellmPricing) nextPricing.set(require_model_selection_normalize.modelKey(ref.provider, ref.model), litellmPricing);
		}
		if (openRouterFailed || litellmFailed) {
			const existingMeta = require_model_pricing_config.getGatewayModelPricingCacheMeta();
			...
			for (const ref of refs) {
				...
				if (!nextPricing.has(key)) {
					const existing = require_model_pricing_config.getCachedGatewayModelPricing({
		...
		if (params.signal?.aborted) return;
		require_model_pricing_config.clearGatewayModelPricingSourceFailure("bootstrap");
		require_model_pricing_config.clearGatewayModelPricingSourceFailure("refresh");
		require_model_pricing_config.replaceGatewayModelPricingCache(nextPricing);
		scheduleRefresh({
			...params,
			fetchImpl
		});
...
function startGatewayModelPricingRefresh(params) {
	...
	queueMicrotask(() => {
		...
		refreshGatewayModelPricingCache({
			...
			log.warn(message);
			if (!abortController.signal.aborted) require_model_pricing_config.recordGatewayModelPricingSourceFailure("bootstrap", message);
		});
	});
	return () => {
		stopped = true;
...
//#endregion
Object.defineProperty(exports, "model_pricing_cache_exports", {
	enumerable: true,
	get: function() {
		return model_pricing_cache_exports;
	}
});
