...
function resolveOutboundTarget(params) {
	return require_targets_session.resolveOutboundTargetWithPlugin({
		plugin: require_channel_resolution.resolveOutboundChannelPlugin({
...
function resolveHeartbeatDeliveryTarget(params) {
	...
	if (rawTarget === "none" || rawTarget === "last") target = rawTarget;
		...
		const normalized = require_channel_resolution.normalizeDeliverableOutboundChannel(rawTarget);
		if (normalized) target = normalized;
		else {
			const explicitTo = heartbeat?.to?.trim();
			if (explicitTo) {
				preparedExplicitPlugin = require_channel_resolution.resolveOutboundChannelPlugin({
					channel: rawTarget,
					cfg,
					allowBootstrap: true
				});
	...
	} else if (rawTarget === void 0 && params.turnSource?.to && params.turnSource.channel && require_message_channel.isDeliverableMessageChannel(params.turnSource.channel)) target = "last";
	if (target === "none") {
		const base = require_targets_session.resolveSessionDeliveryTarget({ entry });
	...
	const resolvedTurnSource = target === "last" ? require_delivery_context_shared.mergeDeliveryContext(params.turnSource, require_delivery_context_shared.deliveryContextFromSession(entry)) : void 0;
	const resolvedTarget = preparedExplicitPlugin && preparedExplicitTo ? require_targets_session.resolveSessionDeliveryTarget({
		entry,
		requestedChannel: target,
		explicitTo: preparedExplicitTo,
		mode: "heartbeat"
	}) : require_targets_session.resolveSessionDeliveryTarget({
		entry,
		requestedChannel: target === "last" ? "last" : target,
		explicitTo: heartbeat?.to,
		mode: "heartbeat",
		turnSourceChannel: resolvedTurnSource?.channel && require_message_channel.isDeliverableMessageChannel(resolvedTurnSource.channel) ? resolvedTurnSource.channel : void 0,
	...
	const plugin = preparedExplicitPlugin ?? require_channel_resolution.resolveOutboundChannelPlugin({
	...
	if (heartbeatAccountId) {
		...
		if (accountIds.length > 0) {
			const normalizedAccountId = require_account_id.normalizeAccountId(heartbeatAccountId);
			if (!new Set(accountIds.map((accountId) => require_account_id.normalizeAccountId(accountId))).has(normalizedAccountId)) return buildNoHeartbeatDeliveryTarget({
	...
	const resolved = require_targets_session.resolveOutboundTargetWithPlugin({
		plugin,
		target: {
			channel: resolvedTarget.channel,
	...
	const sessionChatTypeHint = target === "last" && !heartbeat?.to ? require_chat_type.normalizeChatType(entry?.chatType) : void 0;
	...
	if (plugin?.config.resolveAllowFrom) {
		const explicit = require_targets_session.resolveOutboundTargetWithPlugin({
			plugin,
			target: {
				channel: resolvedTarget.channel,
				to: resolvedTarget.to,
...
async function resolveHeartbeatDeliveryTargetWithSessionRoute(params) {
	...
	else if (targetResolution && require_target_errors.isReservedTargetLiteralError(targetResolution.error)) return buildNoHeartbeatDeliveryTarget({
		reason: "no-target",
		accountId: delivery.accountId,
...
function inferChatTypeFromTarget(params) {
	...
	return (params.plugin ?? require_channel_resolution.resolveOutboundChannelPlugin({ channel: params.channel }))?.messaging?.inferTargetChatType?.({ to }) ?? void 0;
}
...
function resolveHeartbeatSenderId(params) {
	...
	const allowList = require_channel_config_helpers.mapAllowFromEntries(allowFrom).filter((entry) => entry && entry !== "*");
...
function resolveHeartbeatSenderContext(params) {
	...
	const allowFrom = require_channel_config_helpers.mapAllowFromEntries(provider ? require_channel_resolution.resolveOutboundChannelPlugin({
		channel: provider,
		cfg: params.cfg
	...
	return {
		sender: (0, _gabrielvfonseca_normalization_core.expectDefined)(resolveHeartbeatSenderId({
			allowFrom,
...
//#endregion
Object.defineProperty(exports, "resolveHeartbeatDeliveryTarget", {
	enumerable: true,
	get: function() {
		return resolveHeartbeatDeliveryTarget;
	}
});
Object.defineProperty(exports, "resolveHeartbeatDeliveryTargetWithSessionRoute", {
	enumerable: true,
	get: function() {
		return resolveHeartbeatDeliveryTargetWithSessionRoute;
	}
});
Object.defineProperty(exports, "resolveHeartbeatSenderContext", {
	enumerable: true,
	get: function() {
		return resolveHeartbeatSenderContext;
	}
});
Object.defineProperty(exports, "resolveOutboundTarget", {
	enumerable: true,
	get: function() {
