...
function extractStatusCode(err) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(err)) return null;
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(response)) {
...
function extractErrorCode(err) {
	...
	const response = err.response;
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(response)) return null;
	const body = response.body;
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(body)) {
		const error = body.error;
		if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(error) && typeof error.code === "string" && error.code.trim()) return error.code;
	}
	return null;
}
function extractRetryAfterMs(err) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(err)) return null;
	const directMs = require_parse_finite_number.asFiniteNumberInRange(err.retryAfterMs ?? err.retry_after_ms, {
		min: 0,
		max: Number.MAX_SAFE_INTEGER
	});
	if (directMs !== void 0) return directMs;
	const retryAfter = err.retryAfter ?? err.retry_after;
	const retryAfterSeconds = require_parse_finite_number.asFiniteNumberInRange(retryAfter, {
		min: 0,
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(headers)) {
...
function parseNonNegativeRetryAfterSeconds(raw) {
	...
	return require_parse_finite_number.asFiniteNumberInRange(require_parse_finite_number.parseStrictFiniteNumber(trimmed), {
		min: 0,
		max: MAX_SAFE_RETRY_AFTER_SECONDS
	});
}
...
function classifyMSTeamsSendError(err) {
	...
	if (statusCode == null) {
		const networkCode = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(err) && typeof err.code === "string" ? err.code : null;
