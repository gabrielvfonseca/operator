...
function binaryName(token) {
	...
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(cleaned.split(/[/]/).at(-1) ?? cleaned);
}
...
function classifyWorkspacePath(path) {
	...
	for (let index = 0; index < segments.length; index += 1) {
		...
		if (!segment) continue;
		if (segment === ".operator" && segments[index + 1] === "workspace") return "agent";
		if (segment === ".operator" && segments[index + 1] === "sandboxes") return "sandbox";
...
function compactRawCommand(raw, maxLength = 120) {
	const oneLine = require_redact.redactToolPayloadText(raw.replace(/\s*\n\s*/g, " ").replace(/\s{2,}/g, " ").trim());
	if (oneLine.length <= maxLength) return oneLine;
	const half = Math.floor((maxLength - 1) / 2);
	return `${(0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(oneLine, 0, half)}…${(0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(oneLine, -(maxLength - 1 - half))}`;
}
function resolveExecDetail(args, options) {
	...
	if (options?.detailMode !== "explain" && compact && compact !== displaySummary && compact !== summary) return `${displaySummary}${nodeFragment} · ${require_markdown_code.formatInlineCodeSpan(compact)}`;
...
function resolveActionArg(args) {
	...
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(actionRaw) || void 0;
}
...
function coerceDisplayValue(value, opts = {}) {
	...
	if (typeof value === "string") {
		const trimmed = value.trim();
		if (!trimmed) return;
		const rawLine = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(trimmed.split(/\r?\n/)[0]) ?? "";
		if (!rawLine) return;
		const firstLine = require_redact.redactToolPayloadText(rawLine);
		if (firstLine.length > maxStringChars) {
			const half = Math.floor((maxStringChars - 1) / 2);
			return `${(0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(firstLine, 0, half)}…${(0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(firstLine, -(maxStringChars - 1 - half))}`;
		}
		return firstLine;
	}
...
function formatDetailKey(raw, overrides = {}) {
	...
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(last.replace(/_/g, " ").replace(/-/g, " ").replace(/([a-z0-9])([A-Z])/g, "$1 $2")) || (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(last);
}
...
function resolveWriteDetail(toolKey, args) {
	const record = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalObjectRecord)(args);
	if (!record) return;
	const path = resolvePathArg(record) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(record.url);
	if (!path) return;
...
function collectWebSearchQueries(record) {
	...
	for (const key of [
		...
		for (const entry of value) {
			...
			const entryRecord = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalObjectRecord)(entry);
...
function normalizeToolSearchDisplayToolName(toolName) {
	const value = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(toolName);
	if (!value) return;
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(value.match(/^(?:openclaw|mcp|client):[^:]+:(.+)$/s)?.[1]) ?? value;
}
...
function summarizeToolSearchTarget(raw) {
	...
	if (literalMatch?.[1]) return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(literalMatch[1]);
	if (value.match(/\.id\b/)) return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(value.replace(/\.id\b.*/s, ""));
	const namePropertyMatch = value.match(/name\s*:\s*["']([^"']{1,120})["']/s);
	if (namePropertyMatch?.[1]) return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(namePropertyMatch[1]);
...
function parseSimpleToolSearchArgValue(raw) {
	...
	const numeric = require_parse_finite_number.parseStrictFiniteNumber(raw);
...
function summarizeToolSearchCallInput(raw) {
	const value = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(raw)?.replace(/[);\s]+$/g, "").trim();
	if (!value) return;
	...
	if (actionMatch?.[1]) return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(actionMatch[1]);
	const commandMatch = value.match(/command\s*:\s*["']([^"'\n]{1,120})["']/s);
	if (commandMatch?.[1]) return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(commandMatch[1]);
...
function resolveWebFetchDetail(args) {
	...
	if (!url) return;
	const mode = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(record.extractMode);
...
//#endregion
Object.defineProperty(exports, "defaultTitle", {
	enumerable: true,
	get: function() {
		return defaultTitle;
	}
});
Object.defineProperty(exports, "formatDetailKey", {
	enumerable: true,
	get: function() {
		return formatDetailKey;
	}
});
Object.defineProperty(exports, "formatToolDetailText", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "resolveToolSearchCodeDisplayTarget", {
	enumerable: true,
	get: function() {
		return resolveToolSearchCodeDisplayTarget;
	}
});
Object.defineProperty(exports, "resolveToolVerbAndDetailForArgs", {
	enumerable: true,
	get: function() {
		return resolveToolVerbAndDetailForArgs;
	}
});
