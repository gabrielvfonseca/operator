...
/** Formats the Bonjour instance name while preserving user-provided Operator names. */
...
function resolveBonjourCliPath(opts = {}) {
	const envPath = (opts.env ?? process.env).OPERATOR_CLI_PATH?.trim();
	if (envPath) return envPath;
	const statSync = opts.statSync ?? node_fs.default.statSync;
	...
	const siblingCli = node_path.default.join(execDir, "@gabrielvfonseca/operator");
	...
	const distCli = node_path.default.join(cwd, "dist", "index.js");
	if (isFile(distCli)) return distCli;
	const binCli = node_path.default.join(cwd, "bin", "@gabrielvfonseca/operator");
...
async function resolveTailnetDnsHint(opts) {
	const envRaw = (opts?.env ?? process.env).OPERATOR_TAILNET_DNS?.trim();
	const envValue = envRaw && envRaw.length > 0 ? envRaw.replace(/\.$/, "") : "";
	if (envValue) return envValue;
	if (opts?.enabled === false) return;
	const exec = opts?.exec ?? ((command, args) => require_exec.runExec(command, args, {
		timeoutMs: 1500,
		maxBuffer: 2e5
	}));
	try {
		return await require_tailscale.getTailnetHostname(exec);
	} catch {
		return;
	}
}
...
function resolveDiscoveryAdvertiseTimeoutMs(env) {
	const raw = env.OPERATOR_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS?.trim();
...
async function startGatewayDiscovery(params) {
	...
	const localDiscoveryEnabled = mdnsMode !== "off" && !require_env.isTruthyEnvValue(process.env.OPERATOR_DISABLE_BONJOUR) && !process.env.VITEST;
	...
	const sshPort = mdnsMinimal ? void 0 : require_paths.parseTcpPort(process.env.OPERATOR_SSH_PORT) ?? void 0;
	...
	if (params.wideAreaDiscoveryEnabled) {
		const wideAreaDomain = require_widearea_dns.resolveWideAreaDiscoveryDomain({ configDomain: params.wideAreaDiscoveryDomain ?? void 0 });
		if (!wideAreaDomain) {
		...
		const tailnetIPv4 = require_tailnet.pickPrimaryTailnetIPv4();
		if (!tailnetIPv4) params.logDiscovery.warn("discovery.wideArea.enabled is true, but no Tailscale IPv4 address was found; skipping unicast DNS-SD zone update");
			...
			const tailnetIPv6 = require_tailnet.pickPrimaryTailnetIPv6();
			const result = await require_widearea_dns.writeWideAreaGatewayZone({
