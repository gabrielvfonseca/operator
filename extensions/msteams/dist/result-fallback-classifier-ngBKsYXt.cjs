...
require("./agent-runner-failure-copy-Bm9zfCZR.cjs");
...
function hasDeliberateSilentTerminalReply(result) {
	...
	return [result.meta.finalAssistantRawText, result.meta.finalAssistantVisibleText].some((text) => typeof text === "string" && require_tokens.isSilentReplyPayloadText(text));
}
function hasNonTextVisiblePayloadContent(payload) {
	const { text: _text, ...payloadWithoutText } = payload;
	return require_delivery_evidence.hasVisibleAgentPayload({ payloads: [payloadWithoutText] }, {
...
function classifyProviderErrorPayloadReason(errorText, provider) {
	...
	const failoverReason = require_errors.classifyFailoverReason(errorText, { provider });
...
function classifyEmbeddedAgentRunResultForModelFallback(params) {
	...
	if (require_delivery_evidence.hasCommittedOutboundDeliveryEvidence(params.result)) return null;
	...
	if (typeof params.result.meta.finalAssistantVisibleText === "string" && params.result.meta.finalAssistantVisibleText.trim().length > 0 && !require_tokens.isSilentReplyPayloadText(params.result.meta.finalAssistantVisibleText)) return null;
	if (require_delivery_evidence.hasVisibleAgentPayload(params.result, {
		includeErrorPayloads: false,
		includeReasoningPayloads: false
	})) return null;
	...
	if (!require_gpt5_prompt_overlay.isGpt5ModelId(params.model)) return null;
...
//#endregion
Object.defineProperty(exports, "classifyEmbeddedAgentRunResultForModelFallback", {
	enumerable: true,
	get: function() {
		return classifyEmbeddedAgentRunResultForModelFallback;
	}
});
Object.defineProperty(exports, "hasDeliberateSilentTerminalReply", {
	enumerable: true,
	get: function() {
		return hasDeliberateSilentTerminalReply;
	}
});
Object.defineProperty(exports, "mergeEmbeddedAgentRunResultForModelFallbackExhaustion", {
	enumerable: true,
	get: function() {
		return mergeEmbeddedAgentRunResultForModelFallbackExhaustion;
	}
});
