...
function normalizeExactMatch(value) {
	return require_string_coerce.normalizeLowercaseStringOrEmpty(value ?? "");
}
...
async function resolveMSTeamsChannelAllowlist(params) {
	let tokenPromise;
	const getToken = () => {
		tokenPromise ??= require_graph_users.resolveGraphToken(params.cfg);
		return tokenPromise;
	};
	return await require_runtime_api.mapAllowlistResolutionInputs({
		...
		mapInput: async (input) => {
			...
			if (/^[0-9a-fA-F-]{16,}$/.test(team)) teamMatch = {
				...
				const result = await require_graph_users.listTeamsByNameWithPageInfo(token, team);
			...
			let teamChannels;
			try {
				const result = await require_graph_users.listChannelsForTeamWithPageInfo(token, graphTeamId);
				if (result.truncated) return {
					input,
					resolved: false,
...
async function resolveMSTeamsUserAllowlist(params) {
	...
	return await require_runtime_api.mapAllowlistResolutionInputs({
		inputs: params.entries,
		mapInput: async (input) => {
			const query = require_graph_users.normalizeQuery(normalizeMSTeamsUserInput(input));
			if (!query) return {
				input,
				resolved: false
			};
			...
			const result = await require_graph_users.findGraphUsersByExactIdentity({
