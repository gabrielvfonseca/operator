...
function resolveAgainstCwd(filePath, cwd) {
	return (0, node_path.isAbsolute)(filePath) ? (0, node_path.resolve)(filePath) : (0, node_path.resolve)(cwd, filePath);
}
function getCwdRelativePath(filePath, cwd) {
	const resolvedCwd = (0, node_path.resolve)(cwd);
	const relativePath = (0, node_path.relative)(resolvedCwd, resolveAgainstCwd(filePath, resolvedCwd));
	return relativePath === "" || relativePath !== ".." && !relativePath.startsWith(`..${node_path.sep}`) && !(0, node_path.isAbsolute)(relativePath) ? relativePath || "." : void 0;
}
function formatPathRelativeToCwdOrAbsolute(filePath, cwd) {
	const absolutePath = resolveAgainstCwd(filePath, cwd);
	return (getCwdRelativePath(absolutePath, cwd) ?? absolutePath).split(node_path.sep).join("/");
}
//#endregion
...
function createSkillIndexEntry(entry, opts, agentSkillSet) {
	...
	const skillKey = require_frontmatter.resolveSkillKey(entry.skill, entry);
	const source = require_source.resolveSkillSource(entry.skill);
	return {
		entry,
		name,
		...
		source,
		bundled: source === "operator-bundled" || source === "unknown" && opts?.bundledNames?.has(name) === true,
...
function normalizeWorkspaceSkillSupportPath(input) {
	...
	if (node_path.default.posix.isAbsolute(trimmed)) throw new Error("Support file paths must be relative.");
...
async function readWorkspaceSkillFile(filePath) {
	if (!await (0, _openclaw_fs_safe_advanced.pathExists)(filePath)) return null;
	return (await (await (0, _openclaw_fs_safe_root.root)(node_path.default.dirname(filePath))).read(node_path.default.basename(filePath), {
		hardlinks: "reject",
		maxBytes: 1024 * 1024,
		symlinks: "reject"
...
async function readWorkspaceSupportFile(params) {
	...
	if (!await (0, _openclaw_fs_safe_advanced.pathExists)(node_path.default.join(params.skillDir, ...relativePath.split("/")))) return null;
	return (await (await (0, _openclaw_fs_safe_root.root)(params.skillDir)).read(relativePath, {
		hardlinks: "reject",
	...
	try {
		for (const file of supportFiles) {
			await writeWorkspaceFile({
				workspaceDir: params.workspaceDir,
				filePath: node_path.default.join(params.skillDir, ...file.path.split("/")),
...
async function prepareWorkspaceSkillWrite(params) {
	...
	for (const file of params.supportFiles) {
		const filePath = node_path.default.join(params.skillDir, ...file.path.split("/"));
		await resolveWorkspaceSkillWriteTarget({
			workspaceDir: params.workspaceDir,
			filePath,
			symlinkPolicy: params.symlinkPolicy
...
async function writeWorkspaceFile(params) {
	...
	await (await (0, _openclaw_fs_safe_root.root)(target.rootDir)).write(target.relativePath, params.content, {
		encoding: "utf8",
		mkdir: true,
...
async function removeWorkspaceFile(params) {
	const target = await resolveWorkspaceSkillWriteTarget(params);
	await (await (0, _openclaw_fs_safe_root.root)(target.rootDir)).remove(target.relativePath).catch((error) => {
...
async function restoreSupportFilesAfterFailedWrite(params) {
	...
	await Promise.allSettled(params.writtenSupportPaths.toReversed().map(async (relativePath) => {
		const filePath = node_path.default.join(params.skillDir, ...relativePath.split("/"));
...
async function resolveWorkspaceSkillWriteTarget(params) {
	...
	const workspaceDir = node_path.default.resolve(params.workspaceDir);
	const filePath = node_path.default.resolve(params.filePath);
	...
	if (!aliasTarget) return {
		...
		relativePath: node_path.default.relative(workspaceDir, filePath)
	};
	const allowedRoot = params.symlinkPolicy.allowWrites ? require_symlink_targets.findContainingAllowedSkillSymlinkTarget(params.symlinkPolicy.allowedTargetRealPaths, aliasTarget.realTarget) : null;
	...
	return {
		rootDir: allowedRoot,
		relativePath: node_path.default.relative(allowedRoot, aliasTarget.realTarget)
	};
}
async function resolveWorkspaceAliasTarget(params) {
	...
	return (0, _openclaw_fs_safe_path.isPathInside)(workspaceRealPath, realTarget) ? null : { realTarget };
}
async function resolveRealPathThroughExistingAncestors(workspaceDir, filePath) {
	const segments = node_path.default.relative(workspaceDir, filePath).split(node_path.default.sep).filter(Boolean);
	let lexicalCursor = workspaceDir;
	let realCursor = await tryRealpath(workspaceDir) ?? workspaceDir;
	for (const segment of segments) {
		lexicalCursor = node_path.default.join(lexicalCursor, segment);
		realCursor = await tryRealpath(lexicalCursor) ?? node_path.default.join(realCursor, segment);
	}
	return node_path.default.resolve(realCursor);
}
...
function assertInsideWorkspace(workspaceDir, targetPath, label) {
	...
	if (resolvedTarget !== resolvedWorkspaceDir && !(0, _openclaw_fs_safe_path.isPathInside)(resolvedWorkspaceDir, resolvedTarget)) throw new Error(`${label} must stay inside the workspace.`);
}
//#endregion
//#region src/skills/workshop/types.ts
/** Schema id for persisted skill workshop proposal records. */
const SKILL_WORKSHOP_SCHEMA = "operator.skill-workshop.proposal.v1";
const SKILL_WORKSHOP_MANIFEST_SCHEMA = "operator.skill-workshop.proposals-manifest.v1";
const SKILL_WORKSHOP_ROLLBACK_SCHEMA = "operator.skill-workshop.rollback.v1";
...
const PROPOSALS_REL_DIR = node_path.default.join(WORKSHOP_REL_DIR, "proposals");
const TARGET_LOCKS_REL_DIR = node_path.default.join(WORKSHOP_REL_DIR, "locks");
const MANIFEST_REL_PATH = node_path.default.join(WORKSHOP_REL_DIR, "proposals.json");
const MANIFEST_LOCK_REL_PATH = node_path.default.join(TARGET_LOCKS_REL_DIR, "proposals-manifest");
...
const skillWorkshopProcessLocks = new require_keyed_async_queue.KeyedAsyncQueue();
...
function createSkillProposalId(name, now = /* @__PURE__ */ new Date()) {
	...
	const suffix = node_crypto.default.randomUUID().replaceAll("-", "").slice(0, 10);
	return `${normalized.slice(0, 60)}-${date}-${suffix}`;
}
function hashSkillProposalContent(content) {
	return require_crypto_digest.sha256Hex(content);
}
...
function resolveSkillWorkshopStateDir(options = {}) {
	return node_path.default.resolve(options.stateDir ?? require_paths.resolveStateDir(options.env));
}
function resolveProposalDir(proposalId, options = {}) {
	assertProposalId(proposalId);
	return node_path.default.join(resolveSkillWorkshopStateDir(options), proposalRelativeDir(proposalId));
}
function resolveProposalRecordPath(proposalId, options = {}) {
	return node_path.default.join(resolveProposalDir(proposalId, options), PROPOSAL_RECORD_FILE);
}
...
function resolveSkillProposalTarget(params) {
	...
	const skillDir = node_path.default.resolve(params.workspaceDir, "skills", skillKey);
	const skillFile = node_path.default.join(skillDir, "SKILL.md");
...
async function readSkillProposal(proposalId, options = {}) {
	...
	return {
		record,
		content: (await (await (0, _openclaw_fs_safe_root.root)(resolveSkillWorkshopStateDir(options))).read(node_path.default.join(proposalRelativeDir(proposalId), PROPOSAL_DRAFT_FILE), {
			hardlinks: "reject",
...
async function readSkillProposalRecord(proposalId, options = {}) {
	return parseSkillProposalRecord(await (0, _openclaw_fs_safe_json.tryReadJson)(resolveProposalRecordPath(proposalId, options)));
}
...
async function writeSkillProposalFiles(params) {
	const stateRoot = await (0, _openclaw_fs_safe_root.root)(resolveSkillWorkshopStateDir(params.store));
	const relativeDir = proposalRelativeDir(params.record.id);
	await stateRoot.mkdir(relativeDir);
	await stateRoot.write(node_path.default.join(relativeDir, PROPOSAL_DRAFT_FILE), params.content, { encoding: "utf8" });
	for (const file of params.supportFiles ?? []) await stateRoot.write(node_path.default.join(relativeDir, file.path), file.content, {
		encoding: "utf8",
		mkdir: true
	});
	await stateRoot.writeJson(node_path.default.join(relativeDir, PROPOSAL_RECORD_FILE), params.record, { trailingNewline: true });
}
async function replaceSkillProposalDraft(params) {
	...
	for (const file of params.supportFiles ?? []) {
		...
		await stateRoot.write(node_path.default.join(relativeDir, file.path), file.content, {
			encoding: "utf8",
			mkdir: true
		});
	}
	...
	for (const file of params.previousSupportFiles ?? []) {
		const filePath = normalizeWorkspaceSkillSupportPath(file.path);
		if (!nextSupportPaths.has(filePath)) await stateRoot.remove(node_path.default.join(relativeDir, filePath)).catch(() => void 0);
	}
	await refreshSkillProposalManifest(params.store);
}
async function updateSkillProposalRecord(params) {
	assertProposalId(params.record.id);
	await (await (0, _openclaw_fs_safe_root.root)(resolveSkillWorkshopStateDir(params.store))).writeJson(node_path.default.join(proposalRelativeDir(params.record.id), PROPOSAL_RECORD_FILE), params.record, { trailingNewline: true });
	await refreshSkillProposalManifest(params.store);
}
async function withSkillProposalTargetLock(record, fn, options = {}) {
	return await withSkillWorkshopLock(node_path.default.join(resolveSkillWorkshopStateDir(options), TARGET_LOCKS_REL_DIR, `${hashSkillProposalContent(record.target.skillFile)}.target`), fn);
}
async function writeSkillProposalRollback(params) {
	await (await (0, _openclaw_fs_safe_root.root)(resolveSkillWorkshopStateDir(params.store))).writeJson(node_path.default.join(proposalRelativeDir(params.proposalId), PROPOSAL_ROLLBACK_FILE), params.rollback, { trailingNewline: true });
}
...
async function readSkillProposalManifestUnlocked(options = {}) {
	const parsed = parseSkillProposalManifest(await (0, _openclaw_fs_safe_json.tryReadJson)(node_path.default.join(resolveSkillWorkshopStateDir(options), MANIFEST_REL_PATH)));
...
async function refreshSkillProposalManifestUnlocked(options = {}) {
	const stateRoot = await (0, _openclaw_fs_safe_root.root)(resolveSkillWorkshopStateDir(options));
...
async function withSkillProposalManifestLock(options, fn) {
	return await withSkillWorkshopLock(node_path.default.join(resolveSkillWorkshopStateDir(options), MANIFEST_LOCK_REL_PATH), fn);
}
async function withSkillWorkshopLock(lockFile, fn) {
	const lockKey = node_path.default.resolve(lockFile);
	return await skillWorkshopProcessLocks.enqueue(lockKey, async () => {
		await node_fs_promises.default.mkdir(node_path.default.dirname(lockFile), { recursive: true });
		return await require_file_lock.withFileLock(lockFile, SKILL_WORKSHOP_LOCK_OPTIONS, fn);
	});
}
async function readProposalSupportFiles(record, options = {}) {
	...
	for (const file of record.supportFiles ?? []) {
		const filePath = normalizeWorkspaceSkillSupportPath(file.path);
		const content = (await stateRoot.read(node_path.default.join(proposalRelativeDir(record.id), filePath), {
			hardlinks: "reject",
			maxBytes: MAX_WORKSPACE_SKILL_SUPPORT_FILE_BYTES,
			symlinks: "reject"
		})).buffer.toString("utf8");
...
function parseSkillProposalRecord(raw) {
	...
	const record = raw;
	if (record.schema !== "operator.skill-workshop.proposal.v1" || !PROPOSAL_ID_PATTERN.test(record.id) || record.kind !== "create" && record.kind !== "update" || ![
		"pending",
		"applied",
		"rejected",
		"quarantined",
...
function parseSkillProposalManifest(raw) {
	if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
	const manifest = raw;
	if (manifest.schema !== "operator.skill-workshop.proposals-manifest.v1" || typeof manifest.updatedAt !== "string" || !Array.isArray(manifest.proposals)) return null;
	const proposals = manifest.proposals.filter((entry) => {
		return entry && typeof entry === "object" && PROPOSAL_ID_PATTERN.test((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(entry.id) ?? "") && typeof entry.skillName === "string" && typeof entry.skillKey === "string" && typeof entry.updatedAt === "string";
	});
	return {
		...manifest,
		proposals
...
function proposalRelativeDir(proposalId) {
	assertProposalId(proposalId);
	return node_path.default.join(PROPOSALS_REL_DIR, proposalId);
}
//#endregion
//#region src/skills/workshop/curator.ts
var curator_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	getArchivedSkillFiles: () => getArchivedSkillFiles,
	getSkillCuratorDoctorWarning: () => getSkillCuratorDoctorWarning,
	getSkillCuratorStatus: () => getSkillCuratorStatus,
	pinCuratedSkill: () => pinCuratedSkill,
	restoreCuratedSkill: () => restoreCuratedSkill,
	startSkillCuratorMaintenance: () => startSkillCuratorMaintenance,
	unpinCuratedSkill: () => unpinCuratedSkill
});
...
const log = require_subsystem.createSubsystemLogger("skills/curator");
...
function recordSkillUsage(event, options = {}) {
	...
	if (!rawSkillFile || !node_path.default.isAbsolute(rawSkillFile)) {
		log.debug(`skipping skill usage without file identity: ${event.skillName}`);
		return;
	}
	const skillFile = canonicalizePath(node_path.default.resolve(rawSkillFile));
	const skillKey = canonicalSkillKey(event.skillName);
	require_operator_state_db.runOperatorStateWriteTransaction(({ db }) => {
		require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, require_state_migrations_cron_run_logs.getNodeSqliteKysely(db).insertInto("skill_usage").values({
...
function registerSkillUsageTracking(options = {}) {
	return require_diagnostic_events.onTrustedInternalDiagnosticEvent((event, metadata, privateData) => {
...
async function loadCuratedSkills(options = {}) {
	...
	for (const entry of manifest.proposals.toSorted((a, b) => a.id.localeCompare(b.id))) {
		...
		if (record?.status !== "applied" || !record.appliedAt) continue;
...
function detectOverlapCandidates(skills) {
	...
	for (let leftIndex = 0; leftIndex < skills.length; leftIndex += 1) for (let rightIndex = leftIndex + 1; rightIndex < skills.length; rightIndex += 1) {
		...
		if (node_path.default.dirname(node_path.default.dirname(node_path.default.dirname(left.skillFile))) !== node_path.default.dirname(node_path.default.dirname(node_path.default.dirname(right.skillFile)))) continue;
...
function writeSweepAttempt(nowMs, options) {
	require_operator_state_db.runOperatorStateWriteTransaction(({ db }) => {
		require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, require_state_migrations_cron_run_logs.getNodeSqliteKysely(db).insertInto("skill_curator_state").values({
			id: CURATOR_STATE_ID,
...
async function runSkillCuratorSweep(options = {}) {
	...
	try {
		...
		const result = require_operator_state_db.runOperatorStateWriteTransaction(({ db }) => {
			const kysely = require_state_migrations_cron_run_logs.getNodeSqliteKysely(db);
			const lifecycleRows = require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, kysely.selectFrom("skill_lifecycle").selectAll()).rows;
			const usageRows = require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, kysely.selectFrom("skill_usage").select(["skill_file", "last_used_at_ms"])).rows;
			...
			for (const skill of curated) {
				...
				if (!node_fs.default.existsSync(skill.skillFile)) {
					for (const table of ["skill_lifecycle", "skill_usage"]) require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, kysely.deleteFrom(table).where("skill_file", "=", skill.skillFile));
					continue;
				}
				...
				require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, kysely.insertInto("skill_lifecycle").values({
		...
		require_operator_state_db.runOperatorStateWriteTransaction(({ db }) => {
			require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, require_state_migrations_cron_run_logs.getNodeSqliteKysely(db).updateTable("skill_curator_state").set({
...
function getSkillCuratorStatus(options = {}) {
	...
	const state = require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(database.db, kysely.selectFrom("skill_curator_state").selectAll().where("id", "=", CURATOR_STATE_ID));
	const rows = require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, kysely.selectFrom("skill_lifecycle").leftJoin("skill_usage", "skill_usage.skill_file", "skill_lifecycle.skill_file").select([
...
function updateLifecyclePin(skill, pinned, options) {
	...
	const firstSkillFile = require_operator_state_db.runOperatorStateWriteTransaction(({ db }) => {
		const kysely = require_state_migrations_cron_run_logs.getNodeSqliteKysely(db);
		const first = require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(db, kysely.selectFrom("skill_lifecycle").select("skill_file").where("skill_key", "=", skillKey).orderBy("skill_file", "asc"));
		if (!first) return null;
		return require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, kysely.updateTable("skill_lifecycle").set({ pinned: pinned ? 1 : 0 }).where("skill_key", "=", skillKey)).numAffectedRows === 0n ? null : first.skill_file;
	}, options);
...
function restoreCuratedSkill(skill, options = {}) {
	...
	const firstSkillFile = require_operator_state_db.runOperatorStateWriteTransaction(({ db }) => {
		const kysely = require_state_migrations_cron_run_logs.getNodeSqliteKysely(db);
		const first = require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(db, kysely.selectFrom("skill_lifecycle").select("skill_file").where("skill_key", "=", skillKey).where("state", "=", "archived").orderBy("skill_file", "asc"));
		if (!first) return null;
		return require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, kysely.updateTable("skill_lifecycle").set({
			state: "active",
...
function getArchivedSkillFiles(options = {}) {
	try {
		const { database, kysely } = curatorDb(options);
		const rows = require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, kysely.selectFrom("skill_lifecycle").select("skill_file").where("state", "=", "archived").orderBy("skill_file", "asc")).rows;
...
//#endregion
Object.defineProperty(exports, "MAX_SKILL_PROPOSAL_ORIGIN_RUN_IDS", {
	enumerable: true,
	get: function() {
		return MAX_SKILL_PROPOSAL_ORIGIN_RUN_IDS;
	}
});
Object.defineProperty(exports, "SKILL_WORKSHOP_SCHEMA", {
	enumerable: true,
	get: function() {
		return SKILL_WORKSHOP_SCHEMA;
	}
});
Object.defineProperty(exports, "assertInsideWorkspace", {
	enumerable: true,
	get: function() {
		return assertInsideWorkspace;
	}
});
Object.defineProperty(exports, "assertWorkspaceSkillWriteTarget", {
	enumerable: true,
	get: function() {
		return assertWorkspaceSkillWriteTarget;
	}
});
Object.defineProperty(exports, "buildSkillIndexEntries", {
	enumerable: true,
	get: function() {
		return buildSkillIndexEntries;
	}
});
Object.defineProperty(exports, "canonicalizePath", {
	enumerable: true,
	get: function() {
		return canonicalizePath;
	}
});
Object.defineProperty(exports, "createSkillProposalId", {
	enumerable: true,
	get: function() {
		return createSkillProposalId;
	}
});
Object.defineProperty(exports, "createSkillProposalRollback", {
	enumerable: true,
	get: function() {
		return createSkillProposalRollback;
	}
});
Object.defineProperty(exports, "curator_exports", {
	enumerable: true,
	get: function() {
		return curator_exports;
	}
});
Object.defineProperty(exports, "filterPromptVisibleSkillEntries", {
	enumerable: true,
	get: function() {
		return filterPromptVisibleSkillEntries;
	}
});
Object.defineProperty(exports, "filterUserInvocableSkillEntries", {
	enumerable: true,
	get: function() {
		return filterUserInvocableSkillEntries;
	}
});
Object.defineProperty(exports, "formatPathRelativeToCwdOrAbsolute", {
	enumerable: true,
	get: function() {
		return formatPathRelativeToCwdOrAbsolute;
	}
});
Object.defineProperty(exports, "getArchivedSkillFiles", {
	enumerable: true,
	get: function() {
		return getArchivedSkillFiles;
	}
});
Object.defineProperty(exports, "getSkillCuratorStatus", {
	enumerable: true,
	get: function() {
		return getSkillCuratorStatus;
	}
});
Object.defineProperty(exports, "hashSkillProposalContent", {
	enumerable: true,
	get: function() {
		return hashSkillProposalContent;
	}
});
Object.defineProperty(exports, "isLocalPath", {
	enumerable: true,
	get: function() {
		return isLocalPath;
	}
});
Object.defineProperty(exports, "normalizeSkillIndexName", {
	enumerable: true,
	get: function() {
		return normalizeSkillIndexName;
	}
});
Object.defineProperty(exports, "pinCuratedSkill", {
	enumerable: true,
	get: function() {
		return pinCuratedSkill;
	}
});
Object.defineProperty(exports, "prepareSkillProposalSupportFiles", {
	enumerable: true,
	get: function() {
		return prepareSkillProposalSupportFiles;
	}
});
Object.defineProperty(exports, "readProposalSupportFiles", {
	enumerable: true,
	get: function() {
		return readProposalSupportFiles;
	}
});
Object.defineProperty(exports, "readSkillProposal", {
	enumerable: true,
	get: function() {
		return readSkillProposal;
	}
});
Object.defineProperty(exports, "readSkillProposalManifest", {
	enumerable: true,
	get: function() {
		return readSkillProposalManifest;
	}
});
Object.defineProperty(exports, "readSkillProposalRecord", {
	enumerable: true,
	get: function() {
		return readSkillProposalRecord;
	}
});
Object.defineProperty(exports, "readWorkspaceSkillFile", {
	enumerable: true,
	get: function() {
		return readWorkspaceSkillFile;
	}
});
Object.defineProperty(exports, "readWorkspaceSupportFile", {
	enumerable: true,
	get: function() {
		return readWorkspaceSupportFile;
	}
});
Object.defineProperty(exports, "refreshSkillProposalManifest", {
	enumerable: true,
	get: function() {
		return refreshSkillProposalManifest;
	}
});
Object.defineProperty(exports, "replaceSkillProposalDraft", {
	enumerable: true,
	get: function() {
		return replaceSkillProposalDraft;
	}
});
Object.defineProperty(exports, "resolveSkillProposalTarget", {
	enumerable: true,
	get: function() {
		return resolveSkillProposalTarget;
	}
});
Object.defineProperty(exports, "restoreCuratedSkill", {
	enumerable: true,
	get: function() {
		return restoreCuratedSkill;
	}
});
Object.defineProperty(exports, "startSkillCuratorMaintenance", {
	enumerable: true,
	get: function() {
		return startSkillCuratorMaintenance;
	}
});
Object.defineProperty(exports, "unpinCuratedSkill", {
	enumerable: true,
	get: function() {
		return unpinCuratedSkill;
	}
});
Object.defineProperty(exports, "updateSkillProposalRecord", {
	enumerable: true,
	get: function() {
		return updateSkillProposalRecord;
	}
});
Object.defineProperty(exports, "withSkillProposalTargetLock", {
	enumerable: true,
	get: function() {
		return withSkillProposalTargetLock;
	}
});
Object.defineProperty(exports, "writeSkillProposal", {
	enumerable: true,
	get: function() {
		return writeSkillProposal;
	}
});
Object.defineProperty(exports, "writeSkillProposalRollback", {
	enumerable: true,
	get: function() {
		return writeSkillProposalRollback;
	}
});
Object.defineProperty(exports, "writeWorkspaceSkill", {
	enumerable: true,
	get: function() {
		return writeWorkspaceSkill;
	}
});
