...
const require_sandbox_media_paths = require("./sandbox-media-paths-D4S0aDQy.cjs");
...
function isImageExtension(filePath) {
	const ext = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(node_path.default.extname(filePath));
	return IMAGE_EXTENSIONS.has(ext);
}
function normalizeRefForDedupe(raw) {
	return process.platform === "win32" ? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(raw) : raw;
}
function isOperatorCliImageCachePath(filePath) {
	const parts = filePath.replaceAll("\\", "/").split("/");
	return parts.some((part, index) => {
		if (part === ".operator-cli-images") return true;
...
async function sanitizeImagesWithLog(images, label, imageSanitization) {
	const { images: sanitized, dropped } = await require_tool_images.sanitizeImageBlocks(images, label, imageSanitization);
	if (dropped > 0) require_logger.log.warn(`Native image: dropped ${dropped} image(s) after sanitization (${label}).`);
	return sanitized;
}
/**
...
function detectImageReferences(prompt) {
	...
	const addPathRef = (raw) => {
		...
		try {
			(0, _openclaw_fs_safe_advanced.assertNoWindowsNetworkPath)(trimmed, "Image path");
		} catch {
			return;
		}
		const resolved = trimmed.startsWith("~") ? require_home_dir.resolveUserPath(trimmed) : trimmed;
		if (isOperatorCliImageCachePath(resolved)) return;
		seen.add(dedupeKey);
		refs.push({
			raw: trimmed,
			type: "path",
	...
	while ((match = FILE_URL_PATTERN.exec(prompt)) !== null) {
		...
		try {
			const resolved = (0, _openclaw_fs_safe_advanced.safeFileURLToPath)(raw);
			if (isOperatorCliImageCachePath(resolved)) continue;
			seen.add(dedupeKey);
			refs.push({
				raw,
				type: "path",
...
async function loadImageFromRef(ref, workspaceDir, options) {
	try {
		...
		if (!options?.sandbox) targetPath = await require_media_reference.resolveMediaReferenceLocalPath(targetPath);
		if (options?.sandbox) try {
			targetPath = (await require_sandbox_media_paths.resolveSandboxedBridgeMediaPath({
				sandbox: {
			...
			require_logger.log.debug(`Native image: sandbox validation failed for ${ref.resolved}: ${require_errors.formatErrorMessage(err)}`);
			return null;
		}
		else if (!node_path.default.isAbsolute(targetPath)) targetPath = node_path.default.resolve(workspaceDir, targetPath);
		const media = options?.sandbox ? await require_web_media.loadWebMedia(targetPath, {
			maxBytes: options.maxBytes,
			sandboxValidated: true,
			readFile: require_sandbox_media_paths.createSandboxBridgeReadFile({ sandbox: options.sandbox })
		}) : await require_web_media.loadWebMedia(targetPath, options?.workspaceOnly ? {
			maxBytes: options.maxBytes,
			localRoots: options.localRoots ?? [workspaceDir]
		} : options?.maxBytes);
		if (media.kind !== "image") {
			require_logger.log.debug(`Native image: not an image file: ${targetPath} (got ${media.kind})`);
			return null;
		}
		...
		require_logger.log.debug(`Native image: failed to load ${ref.resolved}: ${require_errors.formatErrorMessage(err)}`);
		return null;
	}
}
...
async function detectAndLoadPromptImages(params) {
	...
	require_logger.log.debug(`Native image: detected ${allRefs.length} image refs in prompt`);
	...
	for (const ref of promptRefs) {
		...
		if (image) {
			...
			loadedCount++;
			require_logger.log.debug(`Native image: loaded ${ref.type} ${ref.resolved}`);
		} else skippedCount++;
	}
...
//#endregion
Object.defineProperty(exports, "detectAndLoadPromptImages", {
	enumerable: true,
	get: function() {
		return detectAndLoadPromptImages;
	}
});
Object.defineProperty(exports, "detectImageReferences", {
	enumerable: true,
	get: function() {
		return detectImageReferences;
	}
});
Object.defineProperty(exports, "loadImageFromRef", {
	enumerable: true,
	get: function() {
		return loadImageFromRef;
	}
});
