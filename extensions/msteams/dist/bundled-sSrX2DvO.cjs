...
function resolveBundledMetadataScope(params) {
	const overrideDir = params?.scanDir ? node_path.default.resolve(params.scanDir) : params?.rootDir ? resolveBundledPluginsDirForRoot(params.rootDir) : void 0;
	if (!overrideDir) return params?.rootDir ? { kind: "empty" } : { kind: "default" };
	if (!node_fs.default.existsSync(overrideDir)) return { kind: "empty" };
	return {
		kind: "env",
		env: {
			...process.env,
			OPERATOR_BUNDLED_PLUGINS_DIR: overrideDir,
...
function resolveBundledPluginsDirForRoot(rootDir) {
	return [
		node_path.default.join(rootDir, "extensions"),
		node_path.default.join(rootDir, "dist-runtime", "extensions"),
		node_path.default.join(rootDir, "dist", "extensions")
...
function resolveBundledChannelGeneratedPath(rootDir, entry, pluginDirName, scanDir) {
	return require_bundled_plugin_metadata.resolveBundledPluginGeneratedPath(rootDir, entry, pluginDirName, scanDir);
}
//#endregion
...
*/
const OPERATOR_PACKAGE_ROOT = require_openclaw_root.resolveOperatorPackageRootSync({
	argv1: process.argv[1],
	cwd: process.cwd(),
	moduleUrl: require("url").pathToFileURL(__filename).href.startsWith("file:") ? require("url").pathToFileURL(__filename).href : void 0
}) ?? (require("url").pathToFileURL(__filename).href.startsWith("file:") ? node_path.default.resolve((0, node_url.fileURLToPath)(new URL("../../..", require("url").pathToFileURL(__filename).href))) : process.cwd());
function derivePackageRootFromExtensionsDir(extensionsDir) {
	const parentDir = node_path.default.dirname(extensionsDir);
	const parentBase = node_path.default.basename(parentDir);
	if (parentBase === "dist" || parentBase === "dist-runtime") return node_path.default.dirname(parentDir);
	return parentDir;
}
/**
...
function resolveBundledChannelRootScope(env = process.env) {
	...
	const resolvedPluginsDir = node_path.default.resolve(bundledPluginsDir);
	return {
		packageRoot: node_path.default.basename(resolvedPluginsDir) === "extensions" ? derivePackageRootFromExtensionsDir(resolvedPluginsDir) : resolvedPluginsDir,
...
function normalizeChannelMeta(params) {
	...
	const label = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(next?.label) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(existing?.label) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(next?.selectionLabel) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(existing?.selectionLabel) ?? params.id;
	const selectionLabel = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(next?.selectionLabel) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(existing?.selectionLabel) ?? label;
	const docsPath = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(next?.docsPath) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(existing?.docsPath) ?? `/channels/${params.id}`;
	const blurb = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(next?.blurb) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(existing?.blurb) ?? "";
	return {
...
function hasNativeSourceRequireHook(modulePath) {
	const extension = node_path.default.extname(modulePath).toLowerCase();
...
function isSourceModulePath$1(modulePath) {
	return SOURCE_MODULE_EXTENSIONS.has(node_path.default.extname(modulePath).toLowerCase());
}
...
function loadModule(modulePath) {
	if (!require_plugin_module_loader_cache.isJavaScriptModulePath(modulePath) && !hasNativeSourceRequireHook(modulePath)) {
...
function resolvePluginModuleCandidates(rootDir, specifier) {
	...
	const resolvedPath = node_path.default.resolve(rootDir, normalizedSpecifier);
...
function resolveExistingPluginModulePath(rootDir, specifier) {
	for (const candidate of resolvePluginModuleCandidates(rootDir, specifier)) if (node_fs.default.existsSync(candidate)) return candidate;
	return node_path.default.resolve(rootDir, specifier);
}
/**
...
function isPathInsideCanonicalRoot(rootPath, targetPath) {
	return (0, _openclaw_fs_safe_path.isPathInside)(resolveCanonicalPathOrAbsolute(rootPath), resolveCanonicalPathOrAbsolute(targetPath));
}
function isPackageLocalBundledDistModulePath(params) {
	return [...params.rootScope.pluginsDir ? [node_path.default.join(params.rootScope.pluginsDir, params.metadata.dirName, "dist")] : [], node_path.default.join(params.rootScope.packageRoot, "extensions", params.metadata.dirName, "dist")].some((root) => isPathInsideCanonicalRoot(root, params.modulePath));
}
...
function resolveBundledChannelBoundaryRoot(params) {
	...
	const resolveMatchingRoot = (root) => {
		...
		return (0, _openclaw_fs_safe_path.isPathInside)(canonicalRoot, canonicalModulePath) ? canonicalRoot : null;
	};
	const overrideRoot = params.pluginsDir ? node_path.default.resolve(params.pluginsDir, params.metadata.dirName) : null;
	let boundaryRoot;
	...
	if (overrideBoundaryRoot) boundaryRoot = overrideBoundaryRoot;
		...
		const distBoundaryRoot = resolveMatchingRoot(node_path.default.resolve(params.packageRoot, "dist", "extensions", params.metadata.dirName));
		if (distBoundaryRoot) boundaryRoot = distBoundaryRoot;
		else boundaryRoot = resolveMatchingRoot(node_path.default.resolve(params.packageRoot, "dist-runtime", "extensions", params.metadata.dirName)) ?? resolveCanonicalPathOrAbsolute(node_path.default.resolve(params.packageRoot, "extensions", params.metadata.dirName));
	}
...
function resolveGeneratedBundledChannelModulePath(params) {
	...
	let pluginRoot;
	try {
		packageRoot = node_fs.default.realpathSync.native(params.rootScope.packageRoot);
		pluginRoot = node_fs.default.realpathSync.native(params.metadata.rootDir);
	} catch {
		return null;
	}
	if (!(0, _openclaw_fs_safe_path.isPathInside)(packageRoot, pluginRoot)) return null;
	for (const rawEntry of [params.entry.built, params.entry.source]) {
		if (!rawEntry) continue;
		const candidate = node_path.default.isAbsolute(rawEntry) ? node_path.default.normalize(rawEntry) : node_path.default.resolve(pluginRoot, rawEntry);
		let realCandidate;
		try {
			realCandidate = node_fs.default.realpathSync.native(candidate);
		} catch {
			continue;
		}
		if ((0, _openclaw_fs_safe_path.isPathInside)(pluginRoot, realCandidate)) return realCandidate;
	}
	return null;
}
...
function findMissingModuleCodeInChain(error) {
	...
	while (current && !seen.has(current)) {
		seen.add(current);
		const code = require_errors.extractErrorCode(current);
...
function describeBundledChannelLoadError(error, channelId) {
	...
	if (findMissingModuleCodeInChain(error) !== void 0) return `${detail} (run \`operator doctor --fix\` to install missing bundled runtime dependencies for channel ${channelId})`;
	return detail;
}
...
function shouldIncludeBundledChannelSetupFeatureForConfig(params) {
	...
	if (!require_manifest_owner_policy.passesManifestOwnerBasePolicy({
		plugin: { id: pluginId },
		normalizedConfig: require_config_state.normalizePluginsConfig(params.config.plugins),
...
//#endregion
Object.defineProperty(exports, "getBundledChannelAccountInspector", {
	enumerable: true,
	get: function() {
		return getBundledChannelAccountInspector;
	}
});
Object.defineProperty(exports, "getBundledChannelPlugin", {
	enumerable: true,
	get: function() {
		return getBundledChannelPlugin;
	}
});
Object.defineProperty(exports, "getBundledChannelSecrets", {
	enumerable: true,
	get: function() {
		return getBundledChannelSecrets;
	}
});
Object.defineProperty(exports, "getBundledChannelSetupPlugin", {
	enumerable: true,
	get: function() {
		return getBundledChannelSetupPlugin;
	}
});
Object.defineProperty(exports, "getBundledChannelSetupSecrets", {
	enumerable: true,
	get: function() {
		return getBundledChannelSetupSecrets;
	}
});
Object.defineProperty(exports, "hasBundledChannelPackageSetupFeature", {
	enumerable: true,
	get: function() {
		return hasBundledChannelPackageSetupFeature;
	}
});
Object.defineProperty(exports, "listBundledChannelLegacySessionSurfaces", {
	enumerable: true,
	get: function() {
		return listBundledChannelLegacySessionSurfaces;
	}
});
Object.defineProperty(exports, "listBundledChannelLegacyStateMigrationDetectors", {
	enumerable: true,
	get: function() {
		return listBundledChannelLegacyStateMigrationDetectors;
	}
});
Object.defineProperty(exports, "listBundledChannelSetupPlugins", {
	enumerable: true,
	get: function() {
		return listBundledChannelSetupPlugins;
	}
});
Object.defineProperty(exports, "loadChannelPluginModule", {
	enumerable: true,
	get: function() {
		return loadChannelPluginModule;
	}
});
Object.defineProperty(exports, "normalizeChannelMeta", {
	enumerable: true,
	get: function() {
		return normalizeChannelMeta;
	}
});
Object.defineProperty(exports, "resolveBundledChannelRootScope", {
	enumerable: true,
	get: function() {
		return resolveBundledChannelRootScope;
	}
});
Object.defineProperty(exports, "resolveExistingPluginModulePath", {
	enumerable: true,
	get: function() {
		return resolveExistingPluginModulePath;
	}
});
