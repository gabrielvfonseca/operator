...
function readDirectStatusCode(err) {
	...
	if (typeof candidate === "string") return (0, _gabrielvfonseca_normalization_core_number_coercion.parseStrictNonNegativeInteger)(candidate);
}
...
function readDirectErrorDetails(err) {
	if (!err || typeof err !== "object") return;
	const candidate = err;
	return require_errors$1.extractFailoverSignalDetails(candidate.param, candidate.errorBody, candidate.body, candidate.detail, candidate.error);
}
...
function hasSessionWriteLockContention(err, seen = /* @__PURE__ */ new Set()) {
	if (require_session_write_lock_error.isSessionWriteLockAcquireError(err)) return true;
...
function isEmbeddedAttemptSessionTakeover(err) {
	return Boolean(err && typeof err === "object" && require_errors.readErrorName(err) === "EmbeddedAttemptSessionTakeoverError");
}
...
function hasTimeoutHint(err) {
	...
	if (require_errors.readErrorName(err) === "TimeoutError") return true;
	const message = getErrorMessage(err);
	return Boolean(message && require_sanitize_user_facing_text.isTimeoutErrorMessage(message));
}
/** Return true when an unknown error shape represents a timeout. */
function isTimeoutError(err) {
	if (hasTimeoutHint(err)) return true;
	if (!err || typeof err !== "object") return false;
	if (require_errors.readErrorName(err) !== "AbortError") return false;
	if (hasSessionWriteLockContention(err)) return false;
	const message = getErrorMessage(err);
...
function isSignalTimeoutReason(reason) {
	return require_errors.readErrorName(reason) === "TimeoutError";
}
...
function decideNestedFormatOverride(candidate, inheritedStatus, seen, depth) {
	...
	if (hasDirectMessage && require_errors$1.isUnclassifiedNoBodyHttpSignal({
		...directSignal,
		status: nestedStatus
	})) return true;
	if (hasDirectMessage && (nestedCandidates.length === 0 || require_errors$1.classifyFailoverSignal(directSignal))) return false;
...
function resolveFailoverClassificationFromErrorInternal(err, seen, depth, providerHint) {
	...
	const codeReason = signal.code ? failoverReasonFromClassification(require_errors$1.classifyFailoverSignal({ code: signal.code })) : null;
	const hasExplicitFailoverMetadata = typeof require_errors$1.inferSignalStatus(signal) === "number" || codeReason !== null && codeReason !== "timeout";
	const hasSessionLock = hasSessionWriteLockContention(err);
	const classification = require_errors$1.classifyFailoverSignal(signal);
...
function buildProviderReauthCommand(provider, env = process.env) {
	...
	return require_command_format.formatCliCommand(`openclaw models auth login --provider ${quotePosixShellArg(trimmed)} --force`, env);
}
...
//#endregion
Object.defineProperty(exports, "FailoverError", {
	enumerable: true,
	get: function() {
		return FailoverError;
	}
});
Object.defineProperty(exports, "buildFailoverRemediationHint", {
	enumerable: true,
	get: function() {
		return buildFailoverRemediationHint;
	}
});
Object.defineProperty(exports, "buildProviderReauthCommand", {
	enumerable: true,
	get: function() {
		return buildProviderReauthCommand;
	}
});
Object.defineProperty(exports, "coerceToFailoverError", {
	enumerable: true,
	get: function() {
		return coerceToFailoverError;
	}
});
Object.defineProperty(exports, "describeFailoverError", {
	enumerable: true,
	get: function() {
		return describeFailoverError;
	}
});
Object.defineProperty(exports, "findCliMaxTurnsError", {
	enumerable: true,
	get: function() {
		return findCliMaxTurnsError;
	}
});
Object.defineProperty(exports, "isFailoverError", {
	enumerable: true,
	get: function() {
		return isFailoverError;
	}
});
Object.defineProperty(exports, "isNonProviderRuntimeCoordinationError", {
	enumerable: true,
	get: function() {
		return isNonProviderRuntimeCoordinationError;
	}
});
Object.defineProperty(exports, "isSignalTimeoutReason", {
	enumerable: true,
	get: function() {
		return isSignalTimeoutReason;
	}
});
...
Object.defineProperty(exports, "resolveFailoverReasonFromError", {
	enumerable: true,
	get: function() {
		return resolveFailoverReasonFromError;
	}
});
Object.defineProperty(exports, "resolveFailoverStatus", {
	enumerable: true,
	get: function() {
		return resolveFailoverStatus;
	}
});
Object.defineProperty(exports, "resolveModelFallbackError", {
	enumerable: true,
	get: function() {
		return resolveModelFallbackError;
	}
});
