...
function resolveStoreSessionKey(entries, sessionKey) {
	...
	const lower = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(normalized);
	if (entries.some((entry) => entry.sessionKey === lower)) return lower;
	for (const entry of entries) if ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(entry.sessionKey) === lower) return entry.sessionKey;
	return lower;
}
...
function resolveSessionStorePathForAcp(params) {
	...
	const storePath = require_paths.resolveStorePath(cfg.session?.store, {
...
function rowToAcpSessionMeta(row) {
	const identity = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)((0, _gabrielvfonseca_normalization_core.safeParseJson)(row.identity_json ?? ""));
	const runtimeOptions = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)((0, _gabrielvfonseca_normalization_core.safeParseJson)(row.runtime_options_json ?? ""));
	return {
		backend: row.backend,
		agent: row.agent,
...
function selectAcpSessionRow(db, sessionKey) {
	return require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(db, getAcpSessionKysely(db).selectFrom("acp_sessions").selectAll().where("session_key", "=", sessionKey));
}
...
function readAcpSessionMeta(params) {
	...
	const row = selectAcpSessionRow(require_openclaw_state_db.openOperatorStateDatabase({
		env: params.env,
...
function selectAcpSessionRows(options = {}) {
	...
	return require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, getAcpSessionKysely(database.db).selectFrom("acp_sessions").selectAll().orderBy("last_activity_at", "desc").orderBy("session_key", "asc")).rows;
}
...
function repairAcpSessionMetaKeyForMigration(params) {
	...
	require_openclaw_state_db.runOperatorStateWriteTransaction((database) => {
		...
		for (const candidate of params.candidateSessionKeys ?? []) {
			...
			if (trimmed && trimmed !== sessionKey && (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(trimmed) === normalizedSessionKey) candidateKeys.add(trimmed);
		}
		let row;
		...
		require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, getAcpSessionKysely(database.db).deleteFrom("acp_sessions").where("session_key", "=", row.session_key));
		repaired = true;
	}, {
		env: params.env,
		path: params.databasePath
...
function upsertAcpSessionMetaRow(db, row) {
	require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, getAcpSessionKysely(db).insertInto("acp_sessions").values(row).onConflict((conflict) => conflict.column("session_key").doUpdateSet({
...
function readSessionEntryFromStore(params) {
	...
	try {
		const entries = require_session_accessor.listSessionEntries({
			...agentId ? { agentId } : {},
			storePath,
		...
		return {
			...
			agentId,
			storePath,
			storeSessionKey: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(params.sessionKey),
...
async function listAcpSessionEntries(params) {
	...
	for (const row of rows) {
		...
		let sessionEntries;
		try {
			sessionEntries = require_session_accessor.listSessionEntries({
				...agentId ? { agentId } : {},
				storePath,
				...params.clone === false ? { clone: false } : {}
			});
...
function mergeAcpForReturn(entry, acp) {
	return require_store.mergeSessionEntry(entry, { acp });
}
function sessionStoreUpdateOptions(params) {
	return {
		activeSessionKey: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(params.sessionKey),
...
async function clearLegacyEmbeddedAcpMetadata(params) {
	...
	for (const sessionKey of sessionKeys) await require_session_accessor.patchSessionEntryWithKey({
...
async function upsertAcpSessionMeta(params) {
	...
	require_openclaw_state_db.runOperatorStateWriteTransaction((database) => {
		...
		preparedEntry = require_store.mergeSessionEntry(entry, { updatedAt });
	...
	if (metaToPersist === null) {
		const patched = entry ? await require_session_accessor.patchSessionEntryWithKey({
		...
		require_openclaw_state_db.runOperatorStateWriteTransaction((database) => {
			...
			for (const key of sessionKeysToDelete) require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, getAcpSessionKysely(database.db).deleteFrom("acp_sessions").where("session_key", "=", key));
		}, {
			env: params.env,
			path: params.databasePath
		});
	...
	const persisted = await require_session_accessor.patchSessionEntryWithKey({
		...storeEntry.agentId ? { agentId: storeEntry.agentId } : {},
		storePath: storeEntry.storePath,
		sessionKey: storageSessionKey
	}, (currentEntry) => {
		const next = require_store.mergeSessionEntry(currentEntry, { updatedAt });
		delete next.acp;
		return next;
	}, {
		...sessionStoreUpdateOptions({
	...
	require_openclaw_state_db.runOperatorStateWriteTransaction((database) => {
		...
		if (persisted.sessionKey !== storageSessionKey) require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, getAcpSessionKysely(database.db).deleteFrom("acp_sessions").where("session_key", "=", storageSessionKey));
	}, {
		env: params.env,
		path: params.databasePath
	});
...
//#endregion
Object.defineProperty(exports, "listAcpSessionEntries", {
	enumerable: true,
	get: function() {
		return listAcpSessionEntries;
	}
});
Object.defineProperty(exports, "readAcpSessionEntry", {
	enumerable: true,
	get: function() {
		return readAcpSessionEntry;
	}
});
Object.defineProperty(exports, "readAcpSessionMeta", {
	enumerable: true,
	get: function() {
		return readAcpSessionMeta;
	}
});
Object.defineProperty(exports, "readAcpSessionMetaForEntry", {
	enumerable: true,
	get: function() {
		return readAcpSessionMetaForEntry;
	}
});
Object.defineProperty(exports, "repairAcpSessionMetaKeyForMigration", {
	enumerable: true,
	get: function() {
		return repairAcpSessionMetaKeyForMigration;
	}
});
Object.defineProperty(exports, "upsertAcpSessionMeta", {
	enumerable: true,
	get: function() {
		return upsertAcpSessionMeta;
	}
});
Object.defineProperty(exports, "writeAcpSessionMetaForMigration", {
	enumerable: true,
	get: function() {
		return writeAcpSessionMetaForMigration;
	}
});
