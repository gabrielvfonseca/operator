...
const require_bootstrap_prompt = require("./bootstrap-prompt-BZiXKdcA.cjs");
const require_git_root = require("./git-root-BGq87LF_.cjs");
const require_active_node_context = require("./active-node-context-FDYb96t_.cjs");
...
/**
* Prompt-surface helpers for Operator tool guidance.
*
...
function buildOperatorToolFallbackText(params) {
	if (require_command_registration.isOperatorMainPromptSurface(params.surface)) return [
		"Operator lists the standard tools above. This runtime enables:",
		...
		"- browser: control Operator's dedicated browser",
	...
	return "No Operator tool list is injected for this runtime prompt surface. Use only tools exposed directly by the active backend.";
}
/** Returns whether the main Operator prompt should include workflow hints around the tool list. */
function shouldRenderOperatorToolWorkflowHints(params) {
	return require_command_registration.isOperatorMainPromptSurface(params.surface);
}
...
function resolveAgentPromptSurfaceForSessionKey(sessionKey) {
	if (sessionKey && require_session_key.isAcpSessionKey(sessionKey)) return "acp_backend";
	return sessionKey && require_session_key.isSubagentSessionKey(sessionKey) ? "subagent" : "operator_main";
}
//#endregion
...
function getContextFileBasename(pathValue) {
	...
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(normalizedPath.split("/").pop() ?? normalizedPath);
}
...
function buildExecApprovalPromptGuidance(params) {
	const runtimeChannel = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(params.runtimeChannel);
	if (params.inlineButtonsEnabled || require_native_approval_prompt.hasNativeApprovalPromptRuntimeCapability(params.runtimeCapabilities) || require_native_approval_prompt.isKnownNativeApprovalPromptChannel(runtimeChannel)) return "exec approval-pending: native card/buttons first. Plain /approve only when tool requires chat/manual approval; copy exact \"Reply with:\" command.";
...
function buildAgentBootstrapSystemContext(params) {
	...
	if (params.bootstrapMode === "limited") return [
		"## Bootstrap Pending",
		...require_bootstrap_prompt.buildLimitedBootstrapPromptLines({
	...
	return [
		"## Bootstrap Pending",
		...require_bootstrap_prompt.buildFullBootstrapPromptLines({
...
function formatOwnerDisplayId(ownerId, ownerDisplaySecret) {
	...
	return (hasSecret ? (0, node_crypto.createHmac)("sha256", hasSecret).update(ownerId).digest("hex") : (0, node_crypto.createHash)("sha256").update(ownerId).digest("hex")).slice(0, 12);
}
function buildOwnerIdentityLine(ownerNumbers, ownerDisplay, ownerDisplaySecret) {
	const normalized = (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(ownerNumbers);
...
function normalizeProviderPromptBlock(value) {
	...
	return (0, _gabrielvfonseca_ai_internal_shared.normalizeStructuredPromptSection)(value) || void 0;
}
...
function buildMessagingSection(params) {
	...
	const completionEventGuidance = suppressSilentTokenGuidance ? "- Completion event requesting update: rewrite in normal voice; send. Never forward raw metadata or silent placeholder." : `- Completion event requesting update: rewrite in normal voice; send. Never forward raw metadata or default to ${require_tokens.SILENT_REPLY_TOKEN}.`;
	return [
		"## Messaging",
		...
		params.availableTools.has("message") ? [
			...
			messageToolOnly ? "- Visible `message(send)` content: never repeat in final." : suppressSilentTokenGuidance ? "- Follow turn delivery: private final => visible via `message(send)`; otherwise normal reply once." : `- After visible \`message(send)\`, final ONLY ${require_tokens.SILENT_REPLY_TOKEN}.`,
...
function buildMessageChannelOptions(runtimeChannel) {
	const deliverableChannels = require_message_channel.listDeliverableMessageChannels();
...
function buildAgentSystemPrompt(params) {
	...
	const coreToolSummaries = {
		...
		agents_list: acpSpawnRuntimeEnabled ? "List allowed Operator subagent ids; not ACP ids" : "List allowed subagent ids",
	...
	const availableTools = /* @__PURE__ */ new Set([...visibleTools, ...(0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntriesLower)(params.capabilityToolNames)]);
	...
	const runtimeChannel = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(runtimeInfo?.channel);
	const runtimeChatType = require_chat_type.normalizeChatType(runtimeInfo?.chatType);
	const runtimeCapabilities = runtimeInfo?.capabilities ?? [];
	const runtimeCapabilitiesLower = new Set((0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntriesLower)(runtimeCapabilities));
	...
	const sanitizedWorkspaceDir = require_sanitize_for_prompt.sanitizeForPromptLiteral(params.workspaceDir);
	const sanitizedSandboxContainerWorkspace = sandboxContainerWorkspace ? require_sanitize_for_prompt.sanitizeForPromptLiteral(sandboxContainerWorkspace) : "";
	...
	const workspaceOnlyGuidance = params.fsWorkspaceOnly === true ? "tools.fs.workspaceOnly ON: file-tool scratch/temp/meta stays in workspace, preferably `.operator/tmp/`. If file tools need it later, never exec-write `/tmp`; use workspace path." : "";
	const safetySection = [
		"## Safety",
	...
	const workspaceNotes = (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(params.workspaceNotes);
	if (promptMode === "none") return ["You are a personal assistant running inside Operator.", modelIdentityLine].filter(Boolean).join("\n");
	...
	const lines = [cacheStablePromptPrefix(hashStablePromptInput({
		...
		const lines = [
			...
			...require_promised_work_prompt.buildPromisedWorkPromptSection(),
			...buildOverridablePromptSection({
			...
			...hasOpenClaw ? ["Config, channels, plugins, new agents, model/provider, updates: ask `openclaw`. Never write own config; Operator is system expert."] : ["Config read: `gateway` (`config.get|config.schema.lookup`). Write/restart unavailable; ask human."],
			"",
			...
			params.sandboxInfo?.enabled ? [
				...
				params.sandboxInfo.containerWorkspaceDir ? `Sandbox container workdir: ${require_sanitize_for_prompt.sanitizeForPromptLiteral(params.sandboxInfo.containerWorkspaceDir)}` : "",
				params.sandboxInfo.workspaceDir ? `Sandbox host mount source (file tools bridge only; not valid inside sandbox exec): ${require_sanitize_for_prompt.sanitizeForPromptLiteral(params.sandboxInfo.workspaceDir)}` : "",
				params.sandboxInfo.workspaceAccess ? `Agent workspace access: ${params.sandboxInfo.workspaceAccess}${params.sandboxInfo.agentWorkspaceMount ? ` (mounted at ${require_sanitize_for_prompt.sanitizeForPromptLiteral(params.sandboxInfo.agentWorkspaceMount)})` : ""}` : "",
		...
		if (!isMinimal && silentReplyPromptMode !== "none") lines.push("## Silent Replies", `Nothing to say: entire reply exactly ${require_tokens.SILENT_REPLY_TOKEN}`, `Never append to real response or wrap in Markdown/code.`, "");
		lines.push(_gabrielvfonseca_ai_internal_shared.SYSTEM_PROMPT_CACHE_BOUNDARY);
...
function buildActiveProcessSessionReferenceLines(sessions) {
	...
	return [
		...
		...sessions.map((session) => {
			...
			const cwd = session.cwd ? ` cwd=${require_sanitize_for_prompt.sanitizeForPromptLiteral(session.cwd)}` : "";
			return `- ${session.sessionId} ${session.status}${pid}${cwd} :: ${require_sanitize_for_prompt.sanitizeForPromptLiteral(session.name)}`;
		}),
...
function buildRuntimeLine(runtimeInfo, runtimeChannel, runtimeCapabilities = [], defaultThinkLevel) {
	const normalizedRuntimeCapabilities = (0, _gabrielvfonseca_ai_internal_shared.normalizePromptCapabilityIds)(runtimeCapabilities);
	const { baseSessionKey, runId } = require_session_key.parseCronRunScopeSuffix(runtimeInfo?.sessionKey);
	const stableSessionId = runtimeInfo?.sessionId && runtimeInfo.sessionId !== runId ? runtimeInfo.sessionId : void 0;
	return `Runtime: ${[
		runtimeInfo?.agentId ? `agent=${runtimeInfo.agentId}` : "",
		baseSessionKey ? `session=${require_sanitize_for_prompt.sanitizeForPromptLiteral(baseSessionKey)}` : "",
		stableSessionId ? `sessionId=${require_sanitize_for_prompt.sanitizeForPromptLiteral(stableSessionId)}` : "",
		...
		runtimeInfo?.activeNode ? `active_node=${require_sanitize_for_prompt.sanitizeForPromptLiteral(runtimeInfo.activeNode)}` : "",
...
function buildModelAliasLines(cfg) {
	...
	for (const [keyRaw, entryRaw] of Object.entries(models)) {
		const model = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(keyRaw) ?? "";
		if (!model) continue;
...
function resolveAgentSystemPromptConfig(params) {
	...
	const ownerDisplay = require_io.resolveOwnerDisplaySetting(config);
	const agentSubagents = config && agentId ? require_agent_scope_config.resolveAgentConfig(config, agentId)?.subagents : void 0;
	return {
		...
		ttsHint: config ? require_tts.buildTtsSystemPromptHint(config, agentId) : void 0,
		modelAliasLines: buildModelAliasLines(config),
		fsWorkspaceOnly: require_local_roots.resolveEffectiveToolFsWorkspaceOnly({
			cfg: config,
			agentId
		})
	};
...
function buildSystemPromptParams(params) {
	...
	const userTimezone = require_date_time.resolveUserTimezone(params.config?.agents?.defaults?.userTimezone);
	const userTimeFormat = require_date_time.resolveUserTimeFormat(params.config?.agents?.defaults?.timeFormat);
	const userTime = require_date_time.formatUserTime(/* @__PURE__ */ new Date(), userTimezone, userTimeFormat);
	return {
		runtimeInfo: {
			agentId: params.agentId,
			...params.runtime,
			activeNode: require_active_node_context.formatActiveNodeContextLabel(require_active_node_context.getActiveNodeContext()) ?? params.runtime.activeNode,
			repoRoot
		},
		userTimezone,
		userTime,
...
function resolveRepoRoot(params) {
	...
	if (configured) try {
		const resolved = node_path.default.resolve(configured);
		if (node_fs.default.statSync(resolved).isDirectory()) return resolved;
	} catch {}
	const candidates = (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)([params.workspaceDir ?? "", params.cwd ?? ""]);
	const seen = /* @__PURE__ */ new Set();
	for (const candidate of candidates) {
		const resolved = node_path.default.resolve(candidate);
		if (seen.has(resolved)) continue;
		seen.add(resolved);
		const root = require_git_root.findGitRoot(resolved);
		if (root) return root;
	}
}
//#endregion
Object.defineProperty(exports, "appendModelIdentitySystemPrompt", {
	enumerable: true,
	get: function() {
		return appendModelIdentitySystemPrompt;
	}
});
Object.defineProperty(exports, "buildConfiguredAgentSystemPrompt", {
	enumerable: true,
	get: function() {
		return buildConfiguredAgentSystemPrompt;
	}
});
Object.defineProperty(exports, "buildModelIdentityPromptLine", {
	enumerable: true,
	get: function() {
		return buildModelIdentityPromptLine;
	}
});
Object.defineProperty(exports, "buildSystemPromptParams", {
	enumerable: true,
	get: function() {
		return buildSystemPromptParams;
	}
});
Object.defineProperty(exports, "resolveAgentPromptSurfaceForSessionKey", {
	enumerable: true,
	get: function() {
		return resolveAgentPromptSurfaceForSessionKey;
	}
});
