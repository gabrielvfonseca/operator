...
function formatPairingApproveHint(channelId) {
	return `Approve via: ${require_command_format.formatCliCommand(`openclaw pairing list ${channelId}`)} / ${require_command_format.formatCliCommand(`openclaw pairing approve ${channelId} <code>`)}`;
}
function buildAccountScopedDmSecurityPolicy(params) {
	...
	const defaultBasePath = `channels.${params.channelKey}.accounts.${require_account_id.DEFAULT_ACCOUNT_ID}.`;
	const accountConfig = channelConfig?.accounts?.[resolvedAccountId];
	const defaultAccountConfig = params.inheritSharedDefaultsFromDefaultAccount && resolvedAccountId !== "default" ? channelConfig?.accounts?.[require_account_id.DEFAULT_ACCOUNT_ID] : void 0;
...
//#endregion
Object.defineProperty(exports, "buildAccountScopedDmSecurityPolicy", {
	enumerable: true,
	get: function() {
		return buildAccountScopedDmSecurityPolicy;
	}
});
Object.defineProperty(exports, "resolveChannelDefaultAccountId", {
	enumerable: true,
	get: function() {
