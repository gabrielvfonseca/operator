...
function parseAudioTag(text) {
	const result = require_directive_tags.parseInlineDirectives(text, { stripReplyTags: false });
	return {
		text: result.text,
...
function isBlockedRemoteMediaHostname(hostname) {
	...
	if (strictIp) {
		if ((0, _gabrielvfonseca_net_policy_ip.isIpv4Address)(strictIp)) return (0, _gabrielvfonseca_net_policy_ip.isBlockedSpecialUseIpv4Address)(strictIp);
		if ((0, _gabrielvfonseca_net_policy_ip.isBlockedSpecialUseIpv6Address)(strictIp)) return true;
		const embeddedIpv4 = (0, _gabrielvfonseca_net_policy_ip.extractEmbeddedIpv4FromIpv6)(strictIp);
		return embeddedIpv4 ? (0, _gabrielvfonseca_net_policy_ip.isBlockedSpecialUseIpv4Address)(embeddedIpv4) : false;
	}
	if (normalized.includes(":") && !(0, _gabrielvfonseca_net_policy_ip.parseLooseIpAddress)(normalized)) return true;
	return !(0, _gabrielvfonseca_net_policy_ip.isCanonicalDottedDecimalIPv4)(normalized) && (0, _gabrielvfonseca_net_policy_ip.isLegacyIpv4Literal)(normalized);
}
...
function isValidMedia(candidate, opts) {
	...
	if ((0, _gabrielvfonseca_net_policy_url_protocol.hasHttpUrlPrefix)(candidate)) return isAllowedRemoteMediaUrl(candidate);
...
function isRemoteMarkdownImageMedia(candidate) {
	return (0, _gabrielvfonseca_net_policy_url_protocol.hasHttpUrlPrefix)(candidate) && isValidMedia(candidate);
}
...
function splitMediaFromOutput(raw, options = {}) {
	...
	const fenceSpans = hasFenceMarkers ? require_fences.parseFenceSpans(trimmedRaw) : [];
	...
	for (const line of lines) {
		...
		for (const match of matches) {
			...
			const payload = (0, _gabrielvfonseca_normalization_core.expectDefined)(match[1], "parse regex capture 1");
...
function parseReplyDirectives(raw, options = {}) {
	...
	const replyParsed = require_directive_tags.parseInlineDirectives(text, {
	...
	const isSilent = require_tokens.isSilentReplyPayloadText(text, silentToken);
	if (isSilent) text = "";
	return {
		text,
...
function collectBlockMirrorText(blocks, options = {}) {
	...
	for (const block of blocks) {
		...
		if (block.type === "chart") {
			lines.push(require_payload.renderMessagePresentationChartFallbackText(block));
			continue;
		}
		if (block.type === "table") {
			lines.push(require_payload.renderMessagePresentationTableFallbackText(block));
			continue;
		}
...
function resolveOutboundMirrorText(entry) {
	...
	const presentation = require_payload.normalizeMessagePresentation(entry.payload.presentation);
	...
	const interactive = require_payload.normalizeInteractiveReply(entry.payload.interactive);
...
function createOutboundPayloadPlanEntry(payload, context = {}) {
	if (require_reply_payloads.shouldSuppressReasoningPayload(payload)) return null;
	...
	const normalizedPayload = {
		...payload,
		text: require_reply_payloads.formatBtwTextForExternalDelivery({
			...payload,
			text: parsedText
		}) ?? "",
	...
	if (!require_reply_payloads.isRenderablePayload(normalizedPayload) && !isSilent) return null;
	const hasChannelData = require_payload.hasReplyChannelData(normalizedPayload.channelData);
	return {
		payload: normalizedPayload,
		hasPresentation: require_payload.hasMessagePresentationBlocks(normalizedPayload.presentation),
		hasInteractive: require_payload.hasInteractiveReplyBlocks(normalizedPayload.interactive),
		hasChannelData,
		isSilent
	};
}
...
function createOutboundPayloadPlan(payloads, context = {}) {
	...
	const plan = [];
	for (const entry of prepared) if (!entry.isSilent) plan.push({
		sourceIndex: entry.sourceIndex,
		payload: entry.payload,
		parts: require_reply_payload.resolveSendableOutboundReplyParts(entry.payload),
...
function summarizeOutboundPayloadForTransport(payload) {
	const parts = require_reply_payload.resolveSendableOutboundReplyParts(payload);
...
//#endregion
Object.defineProperty(exports, "createOutboundPayloadPlan", {
	enumerable: true,
	get: function() {
		return createOutboundPayloadPlan;
	}
});
Object.defineProperty(exports, "formatOutboundPayloadLog", {
	enumerable: true,
	get: function() {
		return formatOutboundPayloadLog;
	}
});
Object.defineProperty(exports, "mergeReactionDirectiveChannelData", {
	enumerable: true,
	get: function() {
		return mergeReactionDirectiveChannelData;
	}
});
Object.defineProperty(exports, "normalizeOutboundPayloadsForJson", {
	enumerable: true,
	get: function() {
		return normalizeOutboundPayloadsForJson;
	}
});
Object.defineProperty(exports, "normalizeReplyPayloadsForDelivery", {
	enumerable: true,
	get: function() {
		return normalizeReplyPayloadsForDelivery;
	}
});
Object.defineProperty(exports, "parseReplyDirectives", {
	enumerable: true,
	get: function() {
		return parseReplyDirectives;
	}
});
Object.defineProperty(exports, "projectOutboundPayloadPlanForDelivery", {
	enumerable: true,
	get: function() {
		return projectOutboundPayloadPlanForDelivery;
	}
});
Object.defineProperty(exports, "projectOutboundPayloadPlanForJson", {
	enumerable: true,
	get: function() {
		return projectOutboundPayloadPlanForJson;
	}
});
Object.defineProperty(exports, "projectOutboundPayloadPlanForMirror", {
	enumerable: true,
	get: function() {
		return projectOutboundPayloadPlanForMirror;
	}
});
Object.defineProperty(exports, "projectOutboundPayloadPlanForOutbound", {
	enumerable: true,
	get: function() {
		return projectOutboundPayloadPlanForOutbound;
	}
});
Object.defineProperty(exports, "stripUnsupportedCitationControlMarkers", {
	enumerable: true,
	get: function() {
		return stripUnsupportedCitationControlMarkers;
	}
});
Object.defineProperty(exports, "summarizeOutboundPayloadForTransport", {
	enumerable: true,
	get: function() {
		return summarizeOutboundPayloadForTransport;
	}
});
