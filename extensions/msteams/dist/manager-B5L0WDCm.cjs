...
let _gabrielvfonseca_acp_core_runtime_errors = require("@gabrielvfonseca/acp-core/runtime/errors");
let _gabrielvfonseca_acp_core_runtime_session_identity = require("@gabrielvfonseca/acp-core/runtime/session-identity");
...
function resolveAcpAgentFromSessionKey(sessionKey, fallback = "main") {
	return (0, _gabrielvfonseca_normalization_core_agent_id.normalizeAgentId)(require_session_key.parseAgentSessionKey(sessionKey)?.agentId ?? fallback);
}
/** Builds the stale-session error shown when ACP metadata is missing. */
function resolveMissingMetaError(sessionKey) {
	return new _gabrielvfonseca_acp_core_runtime_errors.AcpRuntimeError("ACP_SESSION_INIT_FAILED", `ACP metadata is missing for ${sessionKey}. Recreate this ACP session with /acp spawn and rebind the thread.`);
}
...
function resolveAcpSessionResolutionError(resolution) {
	...
	return new _gabrielvfonseca_acp_core_runtime_errors.AcpRuntimeError("ACP_SESSION_INIT_FAILED", `Session is not ACP-enabled: ${resolution.sessionKey}`);
}
...
function requireReadySessionMeta(resolution) {
	...
	throw (0, _gabrielvfonseca_normalization_core_error_coercion.toErrorObject)(resolveAcpSessionResolutionError(resolution), "Non-Error thrown");
}
...
function canonicalizeAcpSessionKey(params) {
	...
	const lowered = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(normalized);
	if (lowered === "global" || lowered === "unknown") return lowered;
	const parsed = require_session_key.parseAgentSessionKey(lowered);
	if (parsed) return require_main_session.canonicalizeMainSessionAlias({
		cfg: params.cfg,
		agentId: parsed.agentId,
		sessionKey: lowered
	});
	const mainKey = require_session_key.normalizeMainKey(params.cfg.session?.mainKey);
	if (lowered === "main" || lowered === mainKey) return require_main_session.resolveMainSessionKey(params.cfg);
	return lowered;
}
/** Normalizes session keys for process-local actor maps. */
function normalizeActorKey(sessionKey) {
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(sessionKey);
}
...
function normalizeAcpErrorCode(code) {
	...
	for (const allowed of _gabrielvfonseca_acp_core_runtime_errors.ACP_ERROR_CODES) if (allowed === normalized) return allowed;
	return "ACP_TURN_FAILED";
}
function createUnsupportedControlError(params) {
	return new _gabrielvfonseca_acp_core_runtime_errors.AcpRuntimeError("ACP_BACKEND_UNSUPPORTED_CONTROL", `ACP backend "${params.backend}" does not support ${params.control}.`);
}
...
async function cancelActiveTurn(params) {
	...
	await (0, require_errors$1.errors_exports.withAcpRuntimeErrorBoundary)({
...
function normalizeCancelError(error) {
	return (0, require_errors$1.errors_exports.toAcpRuntimeError)({
		error,
		fallbackCode: "ACP_TURN_FAILED",
		fallbackMessage: "ACP cancel failed before completion."
	});
...
async function prepareFreshManagerRuntimeHandleRetry(params) {
	...
	if (isRecoverableManagerAcpxExitError(params.error.message)) {
		...
		require_globals.logVerbose(`acp-manager: retrying ${params.sessionKey} with a fresh runtime handle after early turn failure: ${params.error.message}`);
		return true;
	}
	...
	if (params.runtime.prepareFreshSession) try {
		...
		require_globals.logVerbose(`acp-manager: failed preparing a fresh persistent session for ${params.sessionKey}: ${require_errors.formatErrorMessage(error)}`);
		return false;
	}
	params.runtimeHandles.clear(params.sessionKey);
	require_globals.logVerbose(`acp-manager: retrying ${params.sessionKey} with a fresh persistent session after missing backend resume target: ${params.error.message}`);
	return true;
}
async function clearPersistedRuntimeResumeState(params) {
	...
	if (!await params.writeSessionMeta({
		...
		mutate: (current, entry) => {
			...
			const base = current;
			if (!base) return null;
			const currentIdentity = (0, _gabrielvfonseca_acp_core_runtime_session_identity.resolveSessionIdentityFromMeta)(base);
		...
		require_globals.logVerbose(`acp-manager: unable to clear persisted runtime resume state for ${params.sessionKey}`);
		return false;
	}
	return true;
}
...
async function tryPrepareFreshManagerRuntimeSession(params) {
	...
	try {
		...
		if (!backend) {
			if (params.missingBackendError) throw (0, _gabrielvfonseca_normalization_core_error_coercion.toErrorObject)(params.missingBackendError, "Non-Error thrown");
			return;
		}
		await backend.runtime.prepareFreshSession?.({ sessionKey: params.sessionKey });
	} catch (error) {
		require_globals.logVerbose(`${params.logPrefix}: unable to prepare fresh session for ${params.sessionKey}: ${require_errors.formatErrorMessage(error)}`);
	}
}
//#endregion
...
async function runManagerCloseSession(params) {
	...
	const currentIdentity = (0, _gabrielvfonseca_acp_core_runtime_session_identity.resolveSessionIdentityFromMeta)(meta);
	const shouldSkipRuntimeClose = input.discardPersistentState && currentIdentity != null && !(0, _gabrielvfonseca_acp_core_runtime_session_identity.identityHasStableSessionId)(currentIdentity);
	...
	if (shouldSkipRuntimeClose) {
		...
		const acpError = (0, require_errors$1.errors_exports.toAcpRuntimeError)({
			error,
			fallbackCode: "ACP_TURN_FAILED",
			fallbackMessage: "ACP close failed before completion."
		});
...
async function reconcileManagerRuntimeSessionIdentifiers(params) {
	...
	if (!runtimeStatus && params.runtime.getStatus) try {
		runtimeStatus = await (0, require_errors$1.errors_exports.withAcpRuntimeErrorBoundary)({
		...
		require_globals.logVerbose(`acp-manager: failed to refresh ACP runtime status for ${params.sessionKey}: ${String(error)}`);
		return {
			handle: params.handle,
			meta: params.meta,
			runtimeStatus
	...
	const now = Date.now();
	const currentIdentity = (0, _gabrielvfonseca_acp_core_runtime_session_identity.resolveSessionIdentityFromMeta)(params.meta);
	const identityAfterEvent = (0, _gabrielvfonseca_acp_core_runtime_session_identity.mergeSessionIdentity)({
		current: currentIdentity,
		incoming: (0, _gabrielvfonseca_acp_core_runtime_session_identity.createIdentityFromHandleEvent)({
			handle: params.handle,
			now
		}),
		now
	}) ?? currentIdentity;
	const nextIdentity = (0, _gabrielvfonseca_acp_core_runtime_session_identity.mergeSessionIdentity)({
		current: identityAfterEvent,
		incoming: (0, _gabrielvfonseca_acp_core_runtime_session_identity.createIdentityFromStatus)({
			status: runtimeStatus,
			now
		}),
		now
	}) ?? identityAfterEvent;
	const handleIdentifiers = (0, _gabrielvfonseca_acp_core_runtime_session_identity.resolveRuntimeHandleIdentifiersFromIdentity)(nextIdentity);
	...
	if (!(!(0, _gabrielvfonseca_acp_core_runtime_session_identity.identityEquals)(currentIdentity, nextIdentity) || hasLegacyAcpIdentityProjection(params.meta))) return {
		handle: nextHandle,
		meta: params.meta,
		runtimeStatus
	};
	...
	if (!(0, _gabrielvfonseca_acp_core_runtime_session_identity.identityEquals)(currentIdentity, nextIdentity)) {
		...
		require_globals.logVerbose(`acp-manager: session identity updated for ${params.sessionKey} (agentSessionId ${currentAgentSessionId} -> ${nextAgentSessionId}, acpxSessionId ${currentAcpxSessionId} -> ${nextAcpxSessionId}, acpxRecordId ${currentAcpxRecordId} -> ${nextAcpxRecordId})`);
	}
	await params.writeSessionMeta({
		cfg: params.cfg,
		sessionKey: params.sessionKey,
...
async function runManagerInitializeSession(params) {
	...
	const agent = (0, _gabrielvfonseca_normalization_core_agent_id.normalizeAgentId)(input.agent);
	const initialRuntimeOptions = require_manager_turn_timeout.validateRuntimeOptionPatch({
	...
	const handle = await (0, require_errors$1.errors_exports.withAcpRuntimeErrorBoundary)({
	...
	const effectiveCwd = (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(handle.cwd) ?? requestedCwd;
	const effectiveRuntimeOptions = require_manager_turn_timeout.normalizeRuntimeOptions({
		...initialRuntimeOptions,
		...effectiveCwd ? { cwd: effectiveCwd } : {}
	});
	const identityNow = Date.now();
	const initializedIdentity = (0, _gabrielvfonseca_acp_core_runtime_session_identity.mergeSessionIdentity)({
		current: void 0,
		incoming: (0, _gabrielvfonseca_acp_core_runtime_session_identity.createIdentityFromEnsure)({
			handle,
			now: identityNow
		}),
		now: identityNow
	...
	if (!(await persistInitializedSessionMeta({
		...
		handle,
		writeSessionMeta: params.writeSessionMeta
	}))?.acp) throw new require_errors$1.errors_exports.AcpRuntimeError("ACP_SESSION_INIT_FAILED", `Could not persist ACP metadata for ${sessionKey}.`);
	params.runtimeHandles.set(sessionKey, {
		...
		mode: input.mode,
		cwd: effectiveCwd,
		configSignature: require_runtime_snapshot.resolveRuntimeConfigCacheKey(input.cfg)
	});
	return {
		runtime,
		handle,
...
async function closeRuntimeAfterInitMetaFailure(params) {
	await params.runtime.close({
		...
		require_globals.logVerbose(`acp-manager: cleanup close failed after metadata write error for ${params.sessionKey}: ${String(closeError)}`);
	});
}
//#endregion
...
function extractConfigOptionKeys(value) {
	...
	return value.map((entry) => {
		if (typeof entry === "string") return (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(entry);
		const record = (0, _gabrielvfonseca_normalization_core_record_coerce.asNullableRecord)(entry);
		return (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(record?.id ?? record?.key);
	}).filter(Boolean);
}
function extractRuntimeStatusConfigOptionKeys(status) {
	const details = (0, _gabrielvfonseca_normalization_core_record_coerce.asNullableRecord)(status?.details);
...
function isOptionalTimeoutConfigKey(key) {
	return OPTIONAL_TIMEOUT_CONFIG_KEYS.has((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(key));
}
function isUnsupportedOptionalTimeoutConfigRejection(key, error) {
	...
	const normalized = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(error instanceof Error ? error.message : typeof error === "string" ? error : String(error));
...
async function resolveManagerRuntimeCapabilities(params) {
	let reported;
	if (params.runtime.getCapabilities) reported = await (0, require_errors$1.errors_exports.withAcpRuntimeErrorBoundary)({
	...
	const normalizedKeys = new Set((reported?.configOptionKeys ?? []).map((entry) => (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(entry)).filter(Boolean));
...
async function applyManagerRuntimeControls(params) {
	const options = require_manager_turn_timeout.resolveRuntimeOptionsFromMeta(params.meta);
	const signature = require_manager_turn_timeout.buildRuntimeControlSignature(options);
	const cached = params.getCachedRuntimeState(params.sessionKey);
	if (cached?.appliedControlSignature === signature) return;
	const needsConfigOptionKeys = require_manager_turn_timeout.buildRuntimeConfigOptionPairs(options).length > 0;
	...
	const configOptions = require_manager_turn_timeout.buildRuntimeConfigOptionPairs(options, capabilities.configOptionKeys);
	const advertisedKeys = new Set((capabilities.configOptionKeys ?? []).map((entry) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(entry)).filter(Boolean));
	await (0, require_errors$1.errors_exports.withAcpRuntimeErrorBoundary)({
		run: async () => {
			if (runtimeMode) {
			...
			if (configOptions.length > 0) {
				...
				for (const [key, value] of configOptions) {
					if (advertisedKeys.size > 0 && !advertisedKeys.has((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(key))) throw new require_errors$1.errors_exports.AcpRuntimeError("ACP_BACKEND_UNSUPPORTED_CONTROL", `ACP backend "${backend}" does not accept config key "${key}".`);
					try {
...
var ManagerRuntimeHandleCache = class {
	...
	async close(params) {
		...
		try {
			...
			require_globals.logVerbose(`acp-manager: cached runtime close failed for ${params.sessionKey}: ${String(error)}`);
		} finally {
	...
	async evictIdle(params) {
		...
		for (const candidate of candidates) await params.actorQueue.run(candidate.actorKey, async () => {
			...
			try {
				...
				require_globals.logVerbose(`acp-manager: idle eviction close failed for ${candidate.state.handle.sessionKey}: ${String(error)}`);
			}
		});
	}
	...
	async isReusable(params) {
		...
		try {
			...
			if (isRuntimeStatusUnavailable(status)) {
				this.clear(params.sessionKey);
				require_globals.logVerbose(`acp-manager: evicting cached runtime handle for ${params.sessionKey} after unhealthy status probe: ${status.summary ?? "status unavailable"}`);
				return false;
			}
			return true;
		} catch (error) {
			this.clear(params.sessionKey);
			require_globals.logVerbose(`acp-manager: evicting cached runtime handle for ${params.sessionKey} after status probe failed: ${String(error)}`);
			return false;
		}
	}
	handleMatchesMeta(params) {
		const identity = (0, _gabrielvfonseca_acp_core_runtime_session_identity.resolveSessionIdentityFromMeta)(params.meta);
		const expectedHandleIds = (0, _gabrielvfonseca_acp_core_runtime_session_identity.resolveRuntimeHandleIdentifiersFromIdentity)(identity);
		...
		return ((0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(params.handle.acpxRecordId) ?? "") === expectedAcpxRecordId;
	}
...
function isRuntimeStatusUnavailable(status) {
	...
	const detailsStatus = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(status.details?.status);
	if (detailsStatus === "dead" || detailsStatus === "no-session") return true;
	const summaryMatch = status.summary?.match(/\bstatus=([^\s]+)/i);
	const summaryStatus = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(summaryMatch?.[1]);
...
async function ensureManagerRuntimeHandle(params) {
	const agent = (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(params.meta.agent) || resolveAcpAgentFromSessionKey(params.sessionKey, "main");
	const mode = params.meta.mode;
	const runtimeOptions = require_manager_turn_timeout.resolveRuntimeOptionsFromMeta(params.meta);
	const cwd = runtimeOptions.cwd ?? (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(params.meta.cwd);
	const model = (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(runtimeOptions.model);
	const thinking = (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(runtimeOptions.thinking);
	const configuredBackend = (params.meta.backend || params.cfg.acp?.backend || "").trim();
	const configSignature = require_runtime_snapshot.resolveRuntimeConfigCacheKey(params.cfg);
	...
	const previousIdentity = (0, _gabrielvfonseca_acp_core_runtime_session_identity.resolveSessionIdentityFromMeta)(previousMeta);
	let identityForEnsure = previousIdentity;
	const persistedResumeSessionId = mode === "persistent" ? (0, _gabrielvfonseca_acp_core_runtime_session_identity.resolveRuntimeResumeSessionId)(previousIdentity) : void 0;
	const shouldPrepareFreshPersistentSession = mode === "persistent" && previousIdentity != null && !(0, _gabrielvfonseca_acp_core_runtime_session_identity.identityHasStableSessionId)(previousIdentity);
	const ensureSession = async (resumeSessionId) => await (0, require_errors$1.errors_exports.withAcpRuntimeErrorBoundary)({
		run: async () => await runtime.ensureSession({
			sessionKey: params.sessionKey,
			agent,
			mode,
	...
	if (persistedResumeSessionId) try {
		...
		require_globals.logVerbose(`acp-manager: resume init failed for ${params.sessionKey}; retrying without persisted ACP session id: ${acpError.message}`);
	...
	const now = Date.now();
	const effectiveCwd = (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(ensured.cwd) ?? cwd;
	const nextRuntimeOptions = require_manager_turn_timeout.normalizeRuntimeOptions({
		...runtimeOptions,
		...effectiveCwd ? { cwd: effectiveCwd } : {}
	});
	const nextIdentity = (0, _gabrielvfonseca_acp_core_runtime_session_identity.mergeSessionIdentity)({
	...
	const nextHandleIdentifiers = (0, _gabrielvfonseca_acp_core_runtime_session_identity.resolveRuntimeHandleIdentifiersFromIdentity)(nextIdentity);
	const nextHandle = {
		...ensured,
	...
	if (previousMeta.backend !== nextMeta.backend || previousMeta.runtimeSessionName !== nextMeta.runtimeSessionName || !(0, _gabrielvfonseca_acp_core_runtime_session_identity.identityEquals)(previousIdentity, nextIdentity) || previousMeta.agent !== nextMeta.agent || previousMeta.cwd !== nextMeta.cwd || !require_manager_turn_timeout.runtimeOptionsEqual(previousMeta.runtimeOptions, nextMeta.runtimeOptions) || hasLegacyAcpIdentityProjection(previousMeta)) await params.writeSessionMeta({
		cfg: params.cfg,
		sessionKey: params.sessionKey,
		mutate: (_current, entry) => {
			if (!entry) return null;
...
async function runSetManagerSessionRuntimeMode(params) {
	...
	const nextOptions = require_manager_turn_timeout.mergeRuntimeOptions({
		current: require_manager_turn_timeout.resolveRuntimeOptionsFromMeta(meta),
...
async function runSetManagerSessionConfigOption(params) {
	...
	const inferredPatch = require_manager_turn_timeout.inferRuntimeOptionPatchFromConfigOption(params.key, params.value);
	...
	const advertisedKeys = new Set((capabilities.configOptionKeys ?? []).map((entry) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(entry)).filter(Boolean));
	const wireKey = require_manager_turn_timeout.resolveRuntimeConfigOptionKey(params.key, capabilities.configOptionKeys);
	if (advertisedKeys.size > 0 && !advertisedKeys.has((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(wireKey))) throw new require_errors$1.errors_exports.AcpRuntimeError("ACP_BACKEND_UNSUPPORTED_CONTROL", `ACP backend "${handle.backend || meta.backend}" does not accept config key "${wireKey}".`);
	await (0, require_errors$1.errors_exports.withAcpRuntimeErrorBoundary)({
...
async function runUpdateManagerSessionRuntimeOptions(params) {
	const nextOptions = require_manager_turn_timeout.mergeRuntimeOptions({
		current: require_manager_turn_timeout.resolveRuntimeOptionsFromMeta(requireReadySessionMeta(params.resolveSession({
			cfg: params.cfg,
			sessionKey: params.sessionKey
		}))),
		patch: params.patch
...
async function persistManagerRuntimeOptions(params) {
	const normalized = require_manager_turn_timeout.normalizeRuntimeOptions(params.options);
...
async function runManagerStartupIdentityReconcile(params) {
	...
	try {
		...
		require_globals.logVerbose(`acp-manager: startup identity scan failed: ${String(error)}`);
		return {
			checked,
			resolved,
			failed: failed + 1
	...
	for (const session of acpSessions) {
		...
		const currentIdentity = (0, _gabrielvfonseca_acp_core_runtime_session_identity.resolveSessionIdentityFromMeta)(session.acp);
		if (!(0, _gabrielvfonseca_acp_core_runtime_session_identity.isSessionIdentityPending)(currentIdentity) || !(0, _gabrielvfonseca_acp_core_runtime_session_identity.identityHasStableSessionId)(currentIdentity)) continue;
		checked += 1;
		try {
			if (await params.withSessionActor(session.sessionKey, async () => {
				...
				return !(0, _gabrielvfonseca_acp_core_runtime_session_identity.isSessionIdentityPending)((0, _gabrielvfonseca_acp_core_runtime_session_identity.resolveSessionIdentityFromMeta)((await params.reconcileRuntimeSessionIdentifiers({
					cfg: params.cfg,
					sessionKey: session.sessionKey,
					runtime,
					handle,
			...
			failed += 1;
			require_globals.logVerbose(`acp-manager: startup identity reconcile failed for ${session.sessionKey}: ${String(error)}`);
		}
	}
	return {
		checked,
...
async function runManagerGetSessionStatus(params) {
	...
	let runtimeStatus;
	if (runtime.getStatus) runtimeStatus = await (0, require_errors$1.errors_exports.withAcpRuntimeErrorBoundary)({
		run: async () => {
			params.throwIfAborted(params.signal);
	...
	const identity = (0, _gabrielvfonseca_acp_core_runtime_session_identity.resolveSessionIdentityFromMeta)(meta);
	return {
		sessionKey: params.sessionKey,
		backend: handle.backend || meta.backend,
		agent: meta.agent,
		...
		state: meta.state,
		mode: meta.mode,
		runtimeOptions: require_manager_turn_timeout.resolveRuntimeOptionsFromMeta(meta),
		capabilities,
		runtimeStatus,
...
const ACP_ACTIVE_TURN_STATE_KEY = Symbol.for("operator.acp.activeTurns");
function getAcpActiveTurnState() {
	return require_global_singleton.resolveGlobalSingleton(ACP_ACTIVE_TURN_STATE_KEY, () => ({ activeTurnKeys: /* @__PURE__ */ new Set() }));
}
...
function resolveBackendCandidatePlan(params) {
	const configuredPrimaryBackend = (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(params.configuredPrimaryBackend);
	const resolvedPrimaryBackend = (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(params.resolvedPrimaryBackend);
	const fallbackBackends = Array.isArray(params.fallbackBackends) ? params.fallbackBackends.map((backend) => (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(backend)).filter((backend) => backend != null) : [];
	return {
...
function summarizeBackgroundTaskText(text) {
	const normalized = (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(text) ?? "ACP background task";
	if (normalized.length <= ACP_BACKGROUND_TASK_TEXT_MAX_LENGTH) return normalized;
	return `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(normalized, ACP_BACKGROUND_TASK_TEXT_MAX_LENGTH - 1)}…`;
}
...
function appendBackgroundTaskProgressSummary(current, chunk) {
	...
	return `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(combined, ACP_BACKGROUND_TASK_PROGRESS_MAX_LENGTH - 1)}…`;
}
...
function resolveBackgroundTaskTerminalResult(progressSummary) {
	const requiredCompletionResult = require_task_completion_contract.resolveRequiredCompletionTerminalResult(progressSummary);
	if (requiredCompletionResult.terminalOutcome) return requiredCompletionResult;
	const normalized = (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(progressSummary)?.replace(/\s+/g, " ").trim();
	if (!normalized) return {};
	const permissionDeniedMatch = normalized.match(/\b(?:write failed:\s*)?permission denied(?: for (?<path>\S+))?\.?/i);
	if (permissionDeniedMatch) {
		const path = (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(permissionDeniedMatch.groups?.path)?.replace(/[.,;:!?]+$/, "");
		return {
...
function resolveBackgroundTaskContext(params) {
	...
	const requesterSessionKey = (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(childEntry?.spawnedBy) ?? (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(childEntry?.parentSessionKey);
	...
	return {
		...
		label: (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(childEntry?.label),
...
function createBackgroundTaskRecord(context, startedAt) {
	try {
		if (!require_task_completion_contract.createRunningTaskRun({
			...
			task: context.task,
			startedAt
		})) require_globals.logVerbose(`acp-manager: failed creating background task for ${context.runId}: persist_failed`);
	} catch (error) {
		require_globals.logVerbose(`acp-manager: failed creating background task for ${context.runId}: ${String(error)}`);
	}
}
function markBackgroundTaskRunning(runId, params) {
	try {
		...
		require_globals.logVerbose(`acp-manager: failed updating background task for ${runId}: ${String(error)}`);
	}
}
...
async function consumeAcpTurnEvents(params) {
	...
	for await (const event of params.events) {
		...
		else if (event.type === "error") streamError = new require_errors$1.errors_exports.AcpRuntimeError(normalizeAcpErrorCode(event.code), (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(event.message) || "ACP turn failed before completion.", event.detailCode ? { detailCode: event.detailCode } : void 0);
...
function errorFromTurnResult(result) {
	return new require_errors$1.errors_exports.AcpRuntimeError(normalizeAcpErrorCode(result.error.code), (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(result.error.message) || "ACP turn failed before completion.", result.error.detailCode ? { detailCode: result.error.detailCode } : void 0);
}
...
async function notifyTerminalResult(params) {
	...
	await params.onEvent?.({
		...
		message: (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(params.result.error.message) || "ACP turn failed before completion.",
...
async function runManagerTurn(params) {
	...
	require_session_state_events.recordSessionHumanDirectMessage({
		sessionKey,
	...
	const recordBackendFailure = async (error) => {
		...
		const errorToRecord = backendAttempts.length > 1 ? new require_errors$1.errors_exports.AcpRuntimeError(error.code, `All ACP backends failed (${backendAttempts.length}): ${failedBackends}`) : error;
		...
		if (taskContext) {
			...
			markBackgroundTaskTerminal(taskContext.runId, {
				...
				error: (0, require_errors$1.errors_exports.formatAcpErrorChain)(errorToRecord),
				progressSummary: taskProgressSummary || null,
				terminalSummary: null
			});
			if (spawnedByWatcher) require_session_state_events.recordSubagentTerminalState({
		...
		await params.setSessionState({
			...
			sessionKey,
			state: "error",
			lastError: (0, require_errors$1.errors_exports.formatAcpErrorChain)(errorToRecord)
		});
		throw errorToRecord;
	};
	...
	try {
		for (const [backendIdx, currentBackend] of candidateBackends.entries()) {
			if (backendIdx > 0) {
				...
				require_globals.logVerbose(`acp-manager: switching backend for ${sessionKey} from ${describeBackendCandidate((0, _gabrielvfonseca_normalization_core.expectDefined)(candidateBackends[backendIdx - 1], "candidate backends entry at backend idx 1"))} to ${describeBackendCandidate(currentBackend)}`);
			}
			for (let attempt = 0; attempt < 2; attempt += 1) {
				...
				try {
					...
					const turnTimeoutMs = require_manager_turn_timeout.resolveTurnTimeoutMs({
						cfg: input.cfg,
						meta
					});
					const sessionMode = meta.mode;
					const turnOutcome = await require_manager_turn_timeout.awaitTurnWithTimeout({
						sessionKey,
						turnPromise,
						...
						onTimeout: async () => {
							...
							await require_manager_turn_timeout.cleanupTimedOutTurn({
								sessionKey,
								activeTurn,
								mode: sessionMode,
					...
					if (!turnOutcome.terminalStatus) throw new require_errors$1.errors_exports.AcpRuntimeError("ACP_TURN_FAILED", "ACP turn ended without a terminal done event.");
					...
					if (!retryFreshHandle && !skipPostTurnCleanup && runtime && handle && meta && meta.mode === "oneshot") try {
						...
						require_globals.logVerbose(`acp-manager: ACP oneshot close failed for ${sessionKey}: ${String(error)}`);
					} finally {
						params.runtimeHandles.clear(sessionKey);
					}
				}
...
const DEFAULT_DEPS = {
	listAcpSessions: require_session_meta.listAcpSessionEntries,
	loadSessionEntry: require_session_meta.readAcpSessionEntry,
	upsertSessionMeta: require_session_meta.upsertAcpSessionMeta,
	getRuntimeBackend: require_registry.getAcpRuntimeBackend,
	requireRuntimeBackend: require_registry.requireAcpRuntimeBackend
};
//#endregion
...
var SessionActorQueue = class {
	constructor() {
		this.queue = new require_keyed_async_queue.KeyedAsyncQueue();
...
var AcpSessionManager = class {
	...
	resolveSession(params) {
		...
		if (require_session_key.isAcpSessionKey(sessionKey)) return {
			kind: "stale",
			sessionKey,
	...
	async initializeSession(input) {
		...
		if (!sessionKey) throw new require_errors$1.errors_exports.AcpRuntimeError("ACP_SESSION_INIT_FAILED", "ACP session key is required.");
	...
	async setSessionRuntimeMode(params) {
		const sessionKey = canonicalizeAcpSessionKey(params);
		if (!sessionKey) throw new require_errors$1.errors_exports.AcpRuntimeError("ACP_SESSION_INIT_FAILED", "ACP session key is required.");
		const runtimeMode = require_manager_turn_timeout.validateRuntimeModeInput(params.runtimeMode);
		await this.evictIdleRuntimeHandles(params.cfg);
		return await this.withSessionActor(sessionKey, async () => {
	...
	async setSessionConfigOption(params) {
		const sessionKey = canonicalizeAcpSessionKey(params);
		if (!sessionKey) throw new require_errors$1.errors_exports.AcpRuntimeError("ACP_SESSION_INIT_FAILED", "ACP session key is required.");
		const normalizedOption = require_manager_turn_timeout.validateRuntimeConfigOptionInput(params.key, params.value);
	...
	async updateSessionRuntimeOptions(params) {
		const sessionKey = canonicalizeAcpSessionKey(params);
		const validatedPatch = require_manager_turn_timeout.validateRuntimeOptionPatch(params.patch);
		if (!sessionKey) throw new require_errors$1.errors_exports.AcpRuntimeError("ACP_SESSION_INIT_FAILED", "ACP session key is required.");
		await this.evictIdleRuntimeHandles(params.cfg);
		return await this.withSessionActor(sessionKey, async () => {
	...
	enforceConcurrentSessionLimit(params) {
		...
		if (activeCount >= limit) throw new require_errors$1.errors_exports.AcpRuntimeError("ACP_SESSION_INIT_FAILED", `ACP max concurrent sessions reached (${activeCount}/${limit}).`);
	}
	...
	async setSessionState(params) {
		await this.writeSessionMeta({
			...
			mutate: (current, entry) => {
				...
				const lastError = (0, _gabrielvfonseca_acp_core_normalize_text.normalizeText)(params.lastError);
	...
	async writeSessionMeta(params) {
		try {
			...
			require_globals.logVerbose(`acp-manager: failed persisting ACP metadata for ${params.sessionKey}: ${String(error)}`);
			return null;
		}
	}
	...
	throwIfAborted(signal) {
		...
		throw new require_errors$1.errors_exports.AcpRuntimeError("ACP_TURN_FAILED", "ACP operation aborted.");
	}
};
//#endregion
//#region src/acp/control-plane/manager.ts
/** Public singleton facade for the ACP session manager control plane. */
var manager_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	AcpSessionManager: () => AcpSessionManager,
	getAcpSessionManager: () => getAcpSessionManager,
	testing: () => testing
});
...
//#endregion
Object.defineProperty(exports, "getAcpSessionManager", {
	enumerable: true,
	get: function() {
		return getAcpSessionManager;
	}
});
Object.defineProperty(exports, "isAcpTurnActive", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "resolveAcpSessionResolutionError", {
	enumerable: true,
	get: function() {
		return resolveAcpSessionResolutionError;
	}
});
