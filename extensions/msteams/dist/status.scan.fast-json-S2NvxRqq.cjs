...
require("./os-summary-DapJiOfZ.cjs");
...
async function executeStatusScanFromOverview(params) {
	const memoryPlugin = require_status_scan_overview.resolveMemoryPluginStatus(params.overview.cfg);
	...
	}), require_status_scan_overview.resolveStatusSummaryFromOverview({ overview: params.overview })]);
...
function resolveDefaultMemoryDatabasePath(agentId) {
	return require_openclaw_agent_db.resolveOperatorAgentSqlitePath({ agentId });
}
...
async function resolveStatusMemoryStatusSnapshot(params) {
	...
	return await require_status_scan_overview.resolveSharedMemoryStatusSnapshot({
		cfg: params.cfg,
		agentStatus: params.agentStatus,
		memoryPlugin: params.memoryPlugin,
		resolveMemoryConfig: require_memory_search.resolveMemorySearchConfig,
...
const STATUS_JSON_CHANNEL_ENV_PREFIXES = require_ids.GENERATED_BUNDLED_CHANNEL_CONFIG_METADATA.filter((entry) => entry.configurable !== false).map((entry) => `${entry.channelId.replace(/[^a-z0-9]+/gi, "_").toUpperCase()}_`);
const STATUS_JSON_CHANNEL_ENV_VARS = new Set(require_ids.GENERATED_BUNDLED_CHANNEL_CONFIG_METADATA.filter((entry) => entry.configurable !== false).flatMap((entry) => entry.channelEnvVars ?? []));
...
function hasExplicitStatusJsonChannelConfig(cfg) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(cfg.channels)) return false;
...
async function scanStatusJsonWithPolicy(opts, runtime, policy) {
	return await executeStatusScanFromOverview({
		overview: await require_status_scan_overview.collectStatusScanOverview({
...
//#endregion
exports.executeStatusScanFromOverview = executeStatusScanFromOverview;
exports.resolveStatusMemoryStatusSnapshot = resolveStatusMemoryStatusSnapshot;
exports.scanStatusJsonFast = scanStatusJsonFast;
exports.scanStatusJsonWithPolicy = scanStatusJsonWithPolicy;
