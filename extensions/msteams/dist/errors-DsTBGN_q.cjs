...
const requireProviderRuntime = resolveNodeRequireFromMeta(require("url").pathToFileURL(__filename).href);
...
*/
const log = require_subsystem.createSubsystemLogger("errors");
...
function isContextOverflowError(errorMessage) {
	...
	const lower = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(errorMessage);
...
function isLikelyContextOverflowError(errorMessage) {
	...
	if (hasRateLimitTpmHint(errorMessage)) return false;
	if (isReasoningConstraintErrorMessage(errorMessage)) return false;
	if (require_sanitize_user_facing_text.isBillingErrorMessage(errorMessage)) return false;
	if (CONTEXT_WINDOW_TOO_SMALL_RE.test(errorMessage)) return false;
	if (require_sanitize_user_facing_text.isRateLimitErrorMessage(errorMessage)) return false;
...
function normalizeFailoverDetailString(value) {
	...
	if (!trimmed) return;
	return trimmed.length > MAX_FAILOVER_DETAIL_CHARS ? (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(trimmed, MAX_FAILOVER_DETAIL_CHARS) : trimmed;
}
...
function inferSignalStatus(signal) {
	...
	return require_assistant_error_format.extractLeadingHttpStatus(stripErrorPrefix(signal.message?.trim() ?? ""))?.code;
}
function isExplicitNoBodyHttpMessage(raw, status) {
	const trimmed = raw?.trim();
	if (!trimmed) return false;
	const candidate = require_assistant_error_format.extractLeadingHttpStatus(trimmed) ? trimmed : stripErrorPrefix(trimmed);
	const leadingStatus = require_assistant_error_format.extractLeadingHttpStatus(candidate);
	if (leadingStatus) {
...
function isHtmlErrorResponse(raw, status) {
	...
	if (!trimmed) return false;
	const candidate = require_assistant_error_format.extractLeadingHttpStatus(trimmed) ? trimmed : stripErrorPrefix(trimmed);
	const inferred = typeof status === "number" && Number.isFinite(status) ? status : require_assistant_error_format.extractLeadingHttpStatus(candidate)?.code;
	if (typeof inferred !== "number" || inferred < 400) return false;
	const rest = require_assistant_error_format.extractLeadingHttpStatus(candidate)?.rest ?? candidate;
...
function isOpenAICodexScopeContext(raw, provider) {
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(provider) === "openai" || /\bopenai\s+codex\b/i.test(raw) || /\bcodex\b.*\bscopes?\b/i.test(raw);
}
function isAuthScopeErrorMessage(raw, status, provider) {
	if (!raw) return false;
	if (!isOpenAICodexScopeContext(raw, provider)) return false;
	const inferred = typeof status === "number" && Number.isFinite(status) ? status : require_assistant_error_format.extractLeadingHttpStatus(raw.trim())?.code;
...
function isSandboxBlockedErrorMessage(raw) {
	return Boolean(require_sanitize_user_facing_text.formatExecDeniedUserMessage(raw)) || SANDBOX_BLOCKED_RE.test(raw);
}
function isSchemaErrorMessage(raw) {
	if (!raw || isReplayInvalidErrorMessage(raw) || isContextOverflowError(raw)) return false;
	return classifyFailoverReason(raw) === "format" || require_sanitize_user_facing_text.matchesFormatErrorPattern(raw);
}
function isTimeoutTransportErrorMessage(raw, status) {
	if (!raw) return false;
	if (require_sanitize_user_facing_text.isTimeoutErrorMessage(raw) || INTERRUPTED_NETWORK_ERROR_RE.test(raw)) return true;
...
function isOAuthRefreshContentionMessage(raw) {
	return /\brefresh_contention\b/i.test(raw) || /\bfile lock timeout\b/i.test(raw) && /(?:\/|\\|^)(?:oauth-refresh|operator-oauth-refresh)[^/\n\\]*?(?:\.lock)?\b/i.test(raw);
}
...
function hasKnownBareLeading402Signal(text) {
	return hasQuotaRefreshWindowSignal(text) || hasExplicit402BillingSignal(text) || require_sanitize_user_facing_text.isRateLimitErrorMessage(text) || hasRetryable402TransientSignal(text);
}
function normalize402Message(raw) {
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(raw)?.replace(LEADING_402_WRAPPER_RE, "").trim() ?? "";
}
function classify402Message(message) {
	...
	if (require_sanitize_user_facing_text.isRateLimitErrorMessage(normalized)) return "rate_limit";
...
function isTransientHttpError(raw) {
	const trimmed = raw.trim();
	if (!trimmed) return false;
	const status = require_assistant_error_format.extractLeadingHttpStatus(trimmed);
...
function classifyFailoverClassificationFromHttpStatus(status, message, messageClassification, explicitStatus, provider, opts) {
	...
	if (status === 402) {
		...
		if (require_assistant_error_format.extractLeadingHttpStatus(message.trim())?.code === 402) {
	...
	if (status === 401 || status === 403) {
		...
		if (message && require_sanitize_user_facing_text.isAuthPermanentErrorMessage(message)) return toReasonClassification("auth_permanent");
...
function classifyFailoverReasonFromErrorType(raw) {
	switch ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(raw)) {
		case "server_error":
...
function hasStructuredBilling429Signal(raw) {
	...
	const leadingStatus = require_assistant_error_format.extractLeadingHttpStatus(raw.trim());
	return Boolean(leadingStatus?.rest && hasBillingApiErrorType(leadingStatus.rest));
}
function hasBillingApiErrorType(raw) {
	const type = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(require_assistant_error_format.parseApiErrorInfo(raw)?.type);
	if (!type) return false;
	return require_sanitize_user_facing_text.isBillingErrorMessage(type) || require_sanitize_user_facing_text.isBillingErrorMessage(type.replaceAll("_", " "));
}
...
function isBilling429MessageForProvider(raw, provider) {
	if (!require_sanitize_user_facing_text.isBillingErrorMessage(raw)) return false;
...
function isOpenRouterProviderReturnedError(raw, provider) {
	return isProvider(provider, "openrouter") && ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(raw)?.includes("provider returned error") ?? false);
}
...
function isExactUnknownNoDetailsError(raw) {
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(raw)?.trim() === "unknown error (no error details in response)";
}
function classifyFailoverClassificationFromMessage(raw, provider, opts) {
	...
	if (require_assistant_error_format.extractLeadingHttpStatus(raw.trim())?.code !== 429 && require_sanitize_user_facing_text.isBillingErrorMessage(raw)) return toReasonClassification("billing");
	if (require_sanitize_user_facing_text.isPeriodicUsageLimitErrorMessage(raw)) return toReasonClassification(require_sanitize_user_facing_text.isBillingErrorMessage(raw) ? "billing" : "rate_limit");
	if (require_sanitize_user_facing_text.isRateLimitErrorMessage(raw)) return toReasonClassification("rate_limit");
	if (require_sanitize_user_facing_text.isOverloadedErrorMessage(raw)) return toReasonClassification("overloaded");
	if (isStructuredServerErrorMessage(raw) && !require_sanitize_user_facing_text.isBillingErrorMessage(raw) && !require_sanitize_user_facing_text.isAuthPermanentErrorMessage(raw) && !require_sanitize_user_facing_text.isAuthErrorMessage(raw)) return toReasonClassification("server_error");
	if (isTransientHttpError(raw)) {
		if (require_assistant_error_format.extractLeadingHttpStatus(raw.trim())?.code === 529) return toReasonClassification("overloaded");
		return toReasonClassification("timeout");
	}
	if (require_assistant_error_format.isGenericProviderInternalError(raw)) return toReasonClassification("timeout");
	if (require_oauth_refresh_failure.classifyOAuthRefreshFailure(raw)?.reason) return toReasonClassification("auth_permanent");
	if (require_sanitize_user_facing_text.isAuthPermanentErrorMessage(raw)) return toReasonClassification("auth_permanent");
	if (require_sanitize_user_facing_text.isAuthErrorMessage(raw)) return toReasonClassification("auth");
	...
	if (require_sanitize_user_facing_text.isServerErrorMessage(raw)) return toReasonClassification("timeout");
	...
	if (require_sanitize_user_facing_text.isTimeoutErrorMessage(raw)) return toReasonClassification("timeout");
...
function classifyProviderRuntimeFailureKind(signal) {
	...
	if (message && require_oauth_refresh_failure.classifyOAuthRefreshFailure(message)) return "auth_refresh";
...
function buildAssistantFailoverSignal(msg, opts) {
	return {
		status: require_assistant_error_format.extractLeadingHttpStatus(msg.errorMessage?.trim() ?? "")?.code,
		code: msg.errorCode,
...
function formatAssistantErrorText(msg, opts) {
	...
	if (unknownTool?.[1]) {
		...
		const rewritten = require_runtime_status.formatSandboxToolPolicyBlockedMessage({
			cfg: opts?.cfg,
	...
	const diskSpaceCopy = require_sanitize_user_facing_text.formatDiskSpaceErrorCopy(raw);
	...
	if (require_sanitize_user_facing_text.isInvalidStreamingEventOrderError(raw)) return "LLM request failed: provider returned an invalid streaming response. Please try again.";
	...
	if (isOpenRouterKeyLimitExceededError(raw, opts?.provider) || isOpenRouterKeyBudgetLimitExceededError(raw, opts?.provider)) return require_sanitize_user_facing_text.formatBillingErrorMessage(opts?.provider, opts?.model ?? msg.model, opts?.authMode);
	if (isBilling429MessageForProvider(raw, opts?.provider)) return require_sanitize_user_facing_text.formatBillingErrorMessage(opts?.provider, opts?.model ?? msg.model, opts?.authMode);
	const transientCopy = require_sanitize_user_facing_text.formatRateLimitOrOverloadedErrorCopy(raw);
	if (transientCopy) return transientCopy;
	if (require_assistant_error_format.isGenericProviderInternalError(raw)) return require_assistant_error_format.formatRawAssistantErrorForUi(raw);
	const transportCopy = require_sanitize_user_facing_text.formatTransportErrorCopy(raw);
	if (transportCopy) return transportCopy;
	if (require_sanitize_user_facing_text.isTimeoutErrorMessage(raw)) return "LLM request timed out.";
	if (require_sanitize_user_facing_text.isBillingErrorMessage(raw)) return require_sanitize_user_facing_text.formatBillingErrorMessage(opts?.provider, opts?.model ?? msg.model, opts?.authMode);
	...
	if (require_sanitize_user_facing_text.isLikelyHttpErrorText(raw) || require_sanitize_user_facing_text.isRawApiErrorPayload(raw)) return require_assistant_error_format.formatRawAssistantErrorForUi(raw);
	if (require_sanitize_user_facing_text.isStreamingJsonParseError(raw)) return "LLM streaming response contained a malformed fragment. Please try again.";
	if (raw.length > 600) log.warn(`Long error truncated: ${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(raw, 200)}`);
	return raw.length > 600 ? `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(raw, 600)}…` : raw;
}
function isRawAssistantErrorPassthrough(params) {
	...
	const parsedMessage = require_assistant_error_format.parseApiErrorInfo(rawError)?.message?.trim();
	const leadingStatusRest = require_assistant_error_format.extractLeadingHttpStatus(rawError)?.rest?.trim();
	...
	return friendlyError === rawError || rawError.length > 600 && friendlyError === `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(rawError, 600)}…` || Boolean(parsedMessage && hasRawDerivedProviderPrefix) || Boolean(leadingStatusRest && friendlyError.startsWith("HTTP "));
}
function formatUserFacingAssistantErrorText(msg, opts) {
	...
	const parsedErrorType = require_assistant_error_format.parseApiErrorInfo(rawError ?? "")?.type?.toLowerCase() ?? "";
...
function isRateLimitAssistantError(msg) {
	...
	return require_sanitize_user_facing_text.isRateLimitErrorMessage(msg.errorMessage ?? "");
}
...
function isBillingAssistantError(msg) {
	if (msg?.stopReason !== "error") return false;
	return require_sanitize_user_facing_text.isBillingErrorMessage(msg.errorMessage ?? "");
}
...
function isJsonApiInternalServerError(raw) {
	if (!raw) return false;
	if (!(0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(raw).includes("\"type\":\"api_error\"")) return false;
	if (require_sanitize_user_facing_text.isBillingErrorMessage(raw) || require_sanitize_user_facing_text.isAuthErrorMessage(raw) || require_sanitize_user_facing_text.isAuthPermanentErrorMessage(raw)) return false;
	return API_ERROR_TRANSIENT_SIGNALS_RE.test(raw);
}
function isStructuredServerErrorMessage(raw) {
	if (!raw) return false;
	const parsedType = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(require_assistant_error_format.parseApiErrorInfo(raw)?.type);
...
function parseImageDimensionError(raw) {
	if (!raw) return null;
	if (!(0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(raw).includes("image dimensions exceed max allowed size")) return null;
...
function isCloudCodeAssistFormatError(raw) {
	return !isImageDimensionErrorMessage(raw) && require_sanitize_user_facing_text.matchesFormatErrorPattern(raw);
}
function isAuthAssistantError(msg) {
	if (msg?.stopReason !== "error") return false;
	return require_sanitize_user_facing_text.isAuthErrorMessage(msg.errorMessage ?? "");
}
...
//#endregion
Object.defineProperty(exports, "AUTH_INVALID_TOKEN_USER_TEXT", {
	enumerable: true,
	get: function() {
		return AUTH_INVALID_TOKEN_USER_TEXT;
	}
});
Object.defineProperty(exports, "GENERIC_ASSISTANT_ERROR_TEXT", {
	enumerable: true,
	get: function() {
		return GENERIC_ASSISTANT_ERROR_TEXT;
	}
});
Object.defineProperty(exports, "classifyAssistantFailoverReason", {
	enumerable: true,
	get: function() {
		return classifyAssistantFailoverReason;
	}
});
Object.defineProperty(exports, "classifyFailoverReason", {
	enumerable: true,
	get: function() {
		return classifyFailoverReason;
	}
});
Object.defineProperty(exports, "classifyFailoverSignal", {
	enumerable: true,
	get: function() {
		return classifyFailoverSignal;
	}
});
Object.defineProperty(exports, "classifyProviderRuntimeFailureKind", {
	enumerable: true,
	get: function() {
		return classifyProviderRuntimeFailureKind;
	}
});
Object.defineProperty(exports, "extractFailoverSignalDetails", {
	enumerable: true,
	get: function() {
		return extractFailoverSignalDetails;
	}
});
Object.defineProperty(exports, "extractObservedOverflowTokenCount", {
	enumerable: true,
	get: function() {
		return extractObservedOverflowTokenCount;
	}
});
Object.defineProperty(exports, "formatAssistantErrorText", {
	enumerable: true,
	get: function() {
		return formatAssistantErrorText;
	}
});
Object.defineProperty(exports, "formatUserFacingAssistantErrorText", {
	enumerable: true,
	get: function() {
		return formatUserFacingAssistantErrorText;
	}
});
Object.defineProperty(exports, "inferSignalStatus", {
	enumerable: true,
	get: function() {
		return inferSignalStatus;
	}
});
Object.defineProperty(exports, "isAuthAssistantError", {
	enumerable: true,
	get: function() {
		return isAuthAssistantError;
	}
});
Object.defineProperty(exports, "isBillingAssistantError", {
	enumerable: true,
	get: function() {
		return isBillingAssistantError;
	}
});
Object.defineProperty(exports, "isCloudCodeAssistFormatError", {
	enumerable: true,
	get: function() {
		return isCloudCodeAssistFormatError;
	}
});
Object.defineProperty(exports, "isCompactionFailureError", {
	enumerable: true,
	get: function() {
		return isCompactionFailureError;
	}
});
...
Object.defineProperty(exports, "isFailoverAssistantError", {
	enumerable: true,
	get: function() {
		return isFailoverAssistantError;
	}
});
Object.defineProperty(exports, "isFailoverErrorMessage", {
	enumerable: true,
	get: function() {
		return isFailoverErrorMessage;
	}
});
Object.defineProperty(exports, "isGenericUnknownStreamErrorMessage", {
	enumerable: true,
	get: function() {
		return isGenericUnknownStreamErrorMessage;
	}
});
Object.defineProperty(exports, "isLikelyContextOverflowError", {
	enumerable: true,
	get: function() {
		return isLikelyContextOverflowError;
	}
});
Object.defineProperty(exports, "isRateLimitAssistantError", {
	enumerable: true,
	get: function() {
		return isRateLimitAssistantError;
	}
});
Object.defineProperty(exports, "isReasoningConstraintErrorMessage", {
	enumerable: true,
	get: function() {
		return isReasoningConstraintErrorMessage;
	}
});
Object.defineProperty(exports, "isTransientHttpError", {
	enumerable: true,
	get: function() {
		return isTransientHttpError;
	}
});
Object.defineProperty(exports, "isUnclassifiedNoBodyHttpSignal", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "parseImageDimensionError", {
	enumerable: true,
	get: function() {
		return parseImageDimensionError;
	}
});
Object.defineProperty(exports, "parseImageSizeError", {
	enumerable: true,
	get: function() {
		return parseImageSizeError;
	}
});
