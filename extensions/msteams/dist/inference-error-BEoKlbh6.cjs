...
/**
* Prompt construction and response parsing for Operator's AI turns.
*
* The assistant carries the conversation (personality included) but can only
* touch the system through Operator's typed command vocabulary; parsing
...
const SYSTEM_AGENT_ASSISTANT_SYSTEM_PROMPT = [
	"You are Operator, the system agent: a small, tidy hermit crab that lives in the config shell.",
	...
	"You are talking to someone setting up or repairing Operator. A real inference turn has already passed before this session can start. Your goals, in order: a workspace, a running gateway, then channels (Discord, Slack, Telegram, WhatsApp, ...) and handing off to their agent (`talk to agent`).",
	...
	"Use the provided Operator docs/source references when the user's request needs behavior, config, or architecture details.",
	"",
	"Config knowledge — the file is ~/.operator/operator.json (JSON5). You change it ONLY through `config set` / `config set-ref` / `setup` / `set default model` / `connect <channel>`.",
	...
	"Inference is a prerequisite, not something you can bootstrap or replace from inside the session. Never change inference-provider credentials, top-level auth (`auth.*`), model catalogs, CLI backends, agent model routes, agent params/tools, or root `tools.*` with `config set` or `config set-ref`. Raw writes under `env.*`, `secrets.*`, `plugins.*`, and `$include` are also refused because they can replace credential resolution or provider activation. Use typed channel/plugin workflows instead. If the user asks to configure or repair provider/auth access, tell them to exit Operator and run `openclaw onboard`, which live-tests a candidate before saving it. Doctor repairs can also change the active inference route; tell the user to exit Operator and run `openclaw doctor --fix`.",
...
const SYSTEM_AGENT_SYSTEM_PROMPT = [
	"You are Operator, the system agent: a small, tidy hermit crab that lives in the config shell.",
	"Personality: warm, competent, concise. Dry humor in small doses. Never corporate. You configure things so the user does not have to.",
	"You are talking to someone setting up or repairing Operator. A real inference turn has already passed before this session can start. Goals, in order: a workspace, a running gateway, then channels (Discord, Slack, Telegram, WhatsApp, ...) and handing off to their agent.",
...
function formatHistory(history) {
	...
	return [
		...
		...history.slice(-12).map((turn) => {
			const text = turn.text.length > HISTORY_TURN_MAX_CHARS ? `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(turn.text, HISTORY_TURN_MAX_CHARS)}…` : turn.text;
			return `${turn.role === "user" ? "User" : "Operator"}: ${text}`;
		}),
		""
	];
}
...
function buildSystemAgentAssistantUserPrompt(params) {
	...
	return [
		...
		`Operator docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
		`Operator source: ${params.overview.references.sourcePath ?? params.overview.references.sourceUrl}`,
...
/** Safe public error for an Operator turn that could not complete with intelligence. */
var SystemAgentInferenceUnavailableError = class extends Error {
	constructor(stage, failures = []) {
		super("Operator could not reach working inference. Run `openclaw onboard` to reconnect and live-test AI, then try again.");
		this.stage = stage;
...
//#endregion
Object.defineProperty(exports, "SYSTEM_AGENT_ASSISTANT_SYSTEM_PROMPT", {
	enumerable: true,
	get: function() {
		return SYSTEM_AGENT_ASSISTANT_SYSTEM_PROMPT;
	}
});
Object.defineProperty(exports, "SYSTEM_AGENT_ASSISTANT_TIMEOUT_MS", {
	enumerable: true,
	get: function() {
		return SYSTEM_AGENT_ASSISTANT_TIMEOUT_MS;
	}
});
Object.defineProperty(exports, "SYSTEM_AGENT_SYSTEM_PROMPT", {
	enumerable: true,
	get: function() {
		return SYSTEM_AGENT_SYSTEM_PROMPT;
	}
});
Object.defineProperty(exports, "SystemAgentInferenceUnavailableError", {
	enumerable: true,
	get: function() {
		return SystemAgentInferenceUnavailableError;
	}
});
Object.defineProperty(exports, "buildSystemAgentAssistantUserPrompt", {
	enumerable: true,
	get: function() {
		return buildSystemAgentAssistantUserPrompt;
	}
});
Object.defineProperty(exports, "isSystemAgentInferenceUnavailableError", {
	enumerable: true,
	get: function() {
		return isSystemAgentInferenceUnavailableError;
	}
});
Object.defineProperty(exports, "parseSystemAgentAssistantPlanText", {
	enumerable: true,
	get: function() {
		return parseSystemAgentAssistantPlanText;
	}
});
