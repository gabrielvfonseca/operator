...
function findHead(db, input) {
	return require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(db, query(db).selectFrom("worker_transcript_commit_heads").selectAll().where("session_id", "=", input.sessionId).where("run_epoch", "=", input.runEpoch));
}
function findCommit(db, input) {
	return require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(db, query(db).selectFrom("worker_transcript_commits").selectAll().where("session_id", "=", input.sessionId).where("run_epoch", "=", input.runEpoch).where("seq", "=", input.seq));
}
...
function insertHead(db, input) {
	...
	require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, query(db).insertInto("worker_transcript_commit_heads").values(head));
}
function insertPendingCommit(db, input) {
	...
	require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, query(db).insertInto("worker_transcript_commits").values(commit));
}
function createWorkerTranscriptCommitStore(options = {}) {
	...
	const complete = (rawInput) => {
		...
		return write((db) => {
			...
			if (require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, query(db).updateTable("worker_transcript_commits").set({
				state: "terminal",
			...
			if (require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, query(db).updateTable("worker_transcript_commit_heads").set({
...
function requestHash(request) {
	return (0, node_crypto.createHash)("sha256").update(require_stable_stringify.stableStringify({
		baseLeafId: request.baseLeafId,
		messages: request.messages
	})).digest("hex");
}
function messageIdempotencyKey(params) {
	return `worker-commit-${(0, node_crypto.createHash)("sha256").update([
		params.sessionId,
		params.runEpoch,
		params.seq,
		params.index
...
async function applyWorkerTranscriptCommit(params) {
	const redactedMessages = params.messages.map((message) => require_session_accessor.redactTranscriptMessage(message, params.config));
	const applied = await require_session_accessor.withTranscriptWriteTransaction(params.target, ({ sessionFile }) => {
		const currentEntry = require_session_accessor.loadSessionEntry(params.target);
		if (!currentEntry || currentEntry.sessionId !== params.sessionId) return {
			ok: false,
			reason: "session-not-attached"
		};
		const manager = require_session_manager.SessionManager.open(sessionFile);
		...
		const freshEntry = require_session_accessor.loadSessionEntry(params.target);
		...
		require_session_accessor.replaceSessionEntrySync(params.target, nextEntry);
		return {
			ok: true,
			messages
		};
	...
	for (const message of applied.messages) {
		...
		await require_session_accessor.publishTranscriptUpdate(params.target, {
...
function createWorkerTranscriptCommitter(options) {
	...
	const sessionOperations = new require_keyed_async_queue.KeyedAsyncQueue();
...
//#endregion
exports.createWorkerTranscriptCommitter = createWorkerTranscriptCommitter;
