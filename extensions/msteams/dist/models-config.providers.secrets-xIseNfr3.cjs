...
function toDiscoveryApiKey(value) {
	...
	if (!trimmed || require_model_auth_markers.isNonSecretApiKeyMarker(trimmed)) return;
	return trimmed;
}
...
function resolveEnvApiKeyVarName(provider, env = process.env, options = {}) {
	const resolved = require_model_auth_env.resolveEnvApiKey(provider, env, options);
	if (!resolved) return;
...
function resolveAwsSdkApiKeyVarName(env = process.env) {
	return require_model_auth_runtime_shared.resolveAwsSdkEnvVarName(env);
}
function resolveEnvAuthEvidenceApiKeyMarker(provider, env) {
	const apiKey = require_model_auth_env.resolveEnvApiKey(provider, env)?.apiKey?.trim();
	if (!apiKey || !require_model_auth_markers.isNonSecretApiKeyMarker(apiKey, { includeEnvVarName: false })) return;
	return apiKey;
}
...
function normalizeHeaderValues(params) {
	...
	for (const [headerName, headerValue] of Object.entries(headers)) {
		const resolvedRef = require_types_secrets.resolveSecretInputRef({
			value: headerValue,
		...
		mutated = true;
		nextHeaders[headerName] = resolvedRef.source === "env" ? require_model_auth_markers.resolveEnvSecretRefHeaderValueMarker(resolvedRef.id) : require_model_auth_markers.resolveNonEnvSecretRefHeaderValueMarker(resolvedRef.source);
	}
	if (!mutated) return {
		headers,
		mutated: false
...
function resolveApiKeyFromCredential(cred, env = process.env) {
	...
	if (cred.type === "api_key") {
		const keyRef = require_types_secrets.coerceSecretRef(cred.keyRef);
		if (keyRef?.id.trim()) {
			...
			return {
				apiKey: require_model_auth_markers.resolveNonEnvSecretRefApiKeyMarker(keyRef.source),
				source: "non-env-ref"
			};
		}
	...
	if (cred.type === "token") {
		const tokenRef = require_types_secrets.coerceSecretRef(cred.tokenRef);
		if (tokenRef?.id.trim()) {
			...
			return {
				apiKey: require_model_auth_markers.resolveNonEnvSecretRefApiKeyMarker(tokenRef.source),
				source: "non-env-ref"
			};
		}
...
function normalizeConfiguredProviderApiKey(params) {
	...
	const configuredApiKeyRef = require_types_secrets.resolveSecretInputRef({
		value: configuredApiKey,
		defaults: params.secretDefaults
	}).ref;
	if (configuredApiKeyRef?.id.trim()) {
		const marker = configuredApiKeyRef.source === "env" ? configuredApiKeyRef.id.trim() : require_model_auth_markers.resolveNonEnvSecretRefApiKeyMarker(configuredApiKeyRef.source);
	...
	if (require_model_auth_markers.isNonSecretApiKeyMarker(normalizedConfiguredApiKey)) params.secretRefManagedProviders?.add(params.providerKey);
...
function resolveMissingProviderApiKey(params) {
	...
	const normalizedApiKey = require_normalize_secret_input.normalizeOptionalSecretInput(params.provider.apiKey);
...
*/
var models_config_providers_secrets_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	createProviderApiKeyResolver: () => createProviderApiKeyResolver,
	createProviderAuthResolver: () => createProviderAuthResolver,
	normalizeApiKeyConfig: () => normalizeApiKeyConfig,
	resolveMissingProviderApiKey: () => resolveMissingProviderApiKey
});
...
function resolveConfigBackedProviderAuth(params) {
	const authProvider = params.authProvider ?? require_provider_auth_aliases.resolveProviderIdForAuth(params.provider, { config: params.config });
	...
	if (apiKey) return require_model_auth_markers.isNonSecretApiKeyMarker(apiKey) ? {
		apiKey,
		discoveryApiKey: toDiscoveryApiKey(apiKey),
		mode: "api_key",
		source: "config"
	} : {
		apiKey: require_model_auth_markers.resolveNonEnvSecretRefApiKeyMarker("file"),
		discoveryApiKey: toDiscoveryApiKey(apiKey),
		mode: "api_key",
		source: "config"
	};
	...
	if (configuredApiKeyRef) {
		...
		return {
			apiKey: require_model_auth_markers.resolveNonEnvSecretRefApiKeyMarker(configuredApiKeyRef.source),
			discoveryApiKey: void 0,
			mode: "api_key",
			source: "config"
		};
	...
	if (require_model_auth_markers.isKnownEnvApiKeyMarker(configuredApiKey)) {
	...
	return require_model_auth_markers.isNonSecretApiKeyMarker(configuredApiKey) ? {
		apiKey: configuredApiKey,
...
//#endregion
Object.defineProperty(exports, "createProviderApiKeyResolver", {
	enumerable: true,
	get: function() {
		return createProviderApiKeyResolver;
	}
});
Object.defineProperty(exports, "createProviderAuthResolver", {
	enumerable: true,
	get: function() {
		return createProviderAuthResolver;
	}
});
Object.defineProperty(exports, "models_config_providers_secrets_exports", {
	enumerable: true,
	get: function() {
		return models_config_providers_secrets_exports;
	}
});
Object.defineProperty(exports, "normalizeConfiguredProviderApiKey", {
	enumerable: true,
	get: function() {
		return normalizeConfiguredProviderApiKey;
	}
});
Object.defineProperty(exports, "normalizeHeaderValues", {
	enumerable: true,
	get: function() {
		return normalizeHeaderValues;
	}
});
Object.defineProperty(exports, "normalizeResolvedEnvApiKey", {
	enumerable: true,
	get: function() {
		return normalizeResolvedEnvApiKey;
	}
});
Object.defineProperty(exports, "resolveApiKeyFromProfiles", {
	enumerable: true,
	get: function() {
		return resolveApiKeyFromProfiles;
	}
});
Object.defineProperty(exports, "resolveMissingProviderApiKey", {
	enumerable: true,
	get: function() {
