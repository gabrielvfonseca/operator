...
function collectExecFilesystemPolicyDriftHits(cfg) {
	...
	for (const context of contexts) {
		const sandbox = require_config.resolveSandboxConfigForAgent(cfg, context.agentId);
		...
		const runtimeTools = RUNTIME_TOOLS.filter((tool) => require_tool_policy_match.isToolAllowedByPolicies(tool, policies));
		if (!runtimeTools.includes("exec")) continue;
		const disabledFilesystemTools = MUTATING_FS_TOOLS.filter((tool) => !require_tool_policy_match.isToolAllowedByPolicies(tool, policies));
...
//#endregion
Object.defineProperty(exports, "collectExecFilesystemPolicyDriftHits", {
	enumerable: true,
	get: function() {
		return collectExecFilesystemPolicyDriftHits;
	}
});
