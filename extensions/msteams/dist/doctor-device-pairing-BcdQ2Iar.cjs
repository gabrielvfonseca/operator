...
function normalizeLocalPairedDevice(device) {
	return {
		...device,
		tokenSummaries: require_device_pairing.summarizeDeviceTokens(device.tokens) ?? []
	};
}
async function loadDoctorPairingSnapshot(params) {
	...
	const local = await require_device_pairing.listDevicePairing();
	return {
...
function resolveApprovedScopes(device) {
	return require_device_auth.normalizeDeviceAuthScopes(device.approvedScopes ?? device.scopes);
}
...
function formatCliArgs(args) {
	return require_command_format.formatCliCommand(args.map(require_quote_cli_arg.quoteCliArg).join(" "));
}
function describeDevice(params) {
	const label = require_safe_text.sanitizeTerminalText(params.displayName?.trim() || "") || require_safe_text.sanitizeTerminalText(params.clientId?.trim() || "");
...
function resolvePendingPairingIssue(pending, paired) {
	...
	const requestedRoles = (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeUniqueSingleOrTrimmedStringList)([pending.roles, pending.role].flat());
	const approvedRoles = require_device_pairing.listApprovedPairedDeviceRoles(paired);
	...
	const requestedScopes = require_device_auth.normalizeDeviceAuthScopes(pending.scopes);
...
function collectPairedRecordIssues(snapshot) {
	...
	for (const device of snapshot.paired) {
		...
		const approvedRoles = require_device_pairing.listApprovedPairedDeviceRoles(device);
		...
		for (const role of approvedRoles) {
			...
			if (token.scopes.length > 0 && !require_operator_scope_compat.roleScopesAllow({
				role,
...
function readJsonFile(filePath) {
	return (0, _openclaw_fs_safe_json.tryReadJsonSync)(filePath);
}
function readLocalIdentity(env = process.env) {
	const identity = readJsonFile(node_path.default.join(require_paths.resolveStateDir(env), "identity", "device.json"));
...
function readLocalDeviceAuthStore(env = process.env) {
	const store = readJsonFile(node_path.default.join(require_paths.resolveStateDir(env), "identity", "device-auth.json"));
...
function collectLocalDeviceAuthIssues(snapshot) {
	...
	const issues = [];
	const approvedRoles = new Set(require_device_pairing.listApprovedPairedDeviceRoles(paired));
	for (const entry of Object.values(store.tokens)) {
		...
		const cachedScopes = require_device_auth.normalizeDeviceAuthScopes(entry.scopes);
		const pairedScopes = require_device_auth.normalizeDeviceAuthScopes(pairedToken.scopes);
...
async function collectLegacyPairingStoreIssues(cfg) {
	...
	const { listLegacyDevicePairingStoreFiles } = await Promise.resolve().then(() => require("./device-pairing-migration-Df_qFYFG.cjs"));
...
//#endregion
exports.collectDevicePairingHealthFindings = collectDevicePairingHealthFindings;
exports.noteDevicePairingHealth = noteDevicePairingHealth;
