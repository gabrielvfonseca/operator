...
async function collectChannelSecurityFindings$1(params) {
	...
	const inspectChannelAccount = async (plugin, cfg, accountId) => {
		...
		return await require_read_only_account_inspect.inspectReadOnlyChannelAccount({
			channelId: plugin.id,
			cfg,
			accountId
		});
	...
	const resolveChannelAuditAccount = async (plugin, accountId) => {
		...
		if (!resolvedAccount) try {
			...
			diagnostics.push(`${plugin.id}:${accountId}: failed to resolve account (${require_errors.formatErrorMessage(error)}).`);
		}
		...
		if (typeof selectedInspection?.enabled !== "boolean" && typeof accountRecord?.enabled !== "boolean" && plugin.config.isEnabled) try {
			...
			enabled = false;
			diagnostics.push(`${plugin.id}:${accountId}: failed to evaluate enabled state (${require_errors.formatErrorMessage(error)}).`);
		}
		...
		if (typeof selectedInspection?.configured !== "boolean" && typeof accountRecord?.configured !== "boolean" && plugin.config.isConfigured) try {
			...
			configured = false;
			diagnostics.push(`${plugin.id}:${accountId}: failed to evaluate configured state (${require_errors.formatErrorMessage(error)}).`);
		}
		return {
			account,
			enabled,
	...
	const warnDmPolicy = async (input) => {
		...
		const { hasWildcard, isMultiUserDm } = await require_dm_allow_state.resolveDmAllowAuditState({
		...
		if (dmScope === "main" && isMultiUserDm) findings.push({
			...
			remediation: "Run: " + require_command_format.formatCliCommand("openclaw config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels) to isolate DM sessions per sender."
		});
	};
	for (const plugin of params.plugins) {
		if (!plugin.security) continue;
		const accountIds = plugin.config.listAccountIds(sourceConfig);
		const orderedAccountIds = (0, _gabrielvfonseca_normalization_core_string_normalization.uniqueStrings)([require_helpers.resolveChannelDefaultAccountId({
			plugin,
			cfg: sourceConfig,
			accountIds
		}), ...accountIds]);
		for (const accountId of orderedAccountIds) {
			...
			if (require_dangerous_name_matching.isDangerousNameMatchingEnabled(accountConfig)) {
...
//#endregion
exports.collectChannelSecurityFindings = collectChannelSecurityFindings;
