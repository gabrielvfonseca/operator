...
const require_task_registry_store = require("./task-registry.store-BKV6yBzt.cjs");
...
function rowToFlowRecord(row) {
	...
	const cancelRequestedAt = require_state_migrations_cron_run_logs.normalizeSqliteNumber(row.cancel_requested_at);
	const requesterOrigin = require_task_registry_store_sqlite.parseDeliveryContextJson(row.requester_origin_json);
	...
	return {
		...
		revision: require_state_migrations_cron_run_logs.normalizeSqliteNumber(row.revision) ?? 0,
		status: parseTaskFlowStatus(row.status),
		notifyPolicy: require_task_registry_store_sqlite.parseTaskNotifyPolicy(row.notify_policy),
		goal: row.goal,
...
function selectFlowRows(db) {
	return require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, getFlowRegistryKysely(db).selectFrom("flow_runs").select([
		"flow_id",
		"sync_mode",
		"shape",
		"owner_key",
...
function upsertFlowRow(db, row) {
	require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, getFlowRegistryKysely(db).insertInto("flow_runs").values(row).onConflict((conflict) => conflict.column("flow_id").doUpdateSet({
...
function saveTaskFlowRegistryStateToSqlite(snapshot) {
	withWriteTransaction(({ db }) => {
		...
		if (flowIds.length === 0) {
			require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, kysely.deleteFrom("flow_runs"));
			return;
		}
...
function deleteTaskFlowRegistryRecordFromSqlite(flowId) {
	withWriteTransaction(({ db }) => {
		require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, getFlowRegistryKysely(db).deleteFrom("flow_runs").where("flow_id", "=", flowId));
	});
}
...
if (process.env.VITEST || false) globalThis[Symbol.for("operator.taskFlowRegistryStoreTestApi")] = { configureTaskFlowRegistryRuntime };
//#endregion
//#region src/tasks/task-flow-registry.ts
const log$1 = require_subsystem.createSubsystemLogger("tasks/task-flow-registry");
...
function normalizeRestoredFlowRecord(record) {
	...
	const controllerId = syncMode === "managed" ? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(record.controllerId) ?? "core/legacy-restored" : void 0;
	return {
		...record,
		syncMode,
		...
		currentStep: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(record.currentStep),
		blockedTaskId: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(record.blockedTaskId),
		blockedSummary: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(record.blockedSummary),
		...record.stateJson !== void 0 ? { stateJson: cloneStructuredValue(record.stateJson) } : {},
		...record.waitJson !== void 0 ? { waitJson: cloneStructuredValue(record.waitJson) } : {},
...
function assertFlowOwnerKey(ownerKey) {
	const normalized = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(ownerKey);
	if (!normalized) throw new Error("Flow ownerKey is required.");
	return normalized;
}
function assertControllerId(controllerId) {
	const normalized = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(controllerId);
...
function resolveFlowBlockedSummary(task) {
	...
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.terminalSummary) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.progressSummary);
}
...
function buildFlowRecord(params) {
	...
	return {
		flowId: node_crypto.default.randomUUID(),
		syncMode,
		...
		goal: params.goal,
		currentStep: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.currentStep),
		blockedTaskId: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.blockedTaskId),
		blockedSummary: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.blockedSummary),
...
function applyFlowPatch(current, patch) {
	const controllerId = patch.controllerId === void 0 ? current.controllerId : (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patch.controllerId);
	...
	return {
		...
		controllerId,
		currentStep: patch.currentStep === void 0 ? current.currentStep : (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patch.currentStep),
		blockedTaskId: patch.blockedTaskId === void 0 ? current.blockedTaskId : (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patch.blockedTaskId),
		blockedSummary: patch.blockedSummary === void 0 ? current.blockedSummary : (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patch.blockedSummary),
...
function createTaskFlowForTask(params) {
	...
	return createFlowRecord({
		...
		goal: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.task.label) ?? (params.task.task.trim() || "Background task"),
		blockedTaskId: terminalFlowStatus === "blocked" ? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.task.taskId) : void 0,
...
function syncFlowFromTaskResult(task) {
	...
	const updated = updateFlowRecordByIdUnchecked(flowId, {
		...
		goal: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.label) ?? (task.task.trim() || "Background task"),
...
if (process.env.VITEST || false) globalThis[Symbol.for("operator.taskFlowRegistryTestApi")] = {
	createFlowRecord,
...
function resolveTaskDisplayTitle(task) {
	return require_task_status.formatTaskStatusTitleText(task.label?.trim() || (task.runtime === "acp" ? "ACP background task" : task.runtime === "subagent" ? "Subagent task" : task.task.trim() || "Background task"));
}
...
function formatTaskTerminalMessage(task, options = {}) {
	...
	const summary = require_task_status.sanitizeTaskStatusText(task.terminalSummary, { errorContext: task.status !== "succeeded" || task.terminalOutcome === "blocked" });
	...
	if (task.status === "lost") {
		const error = require_task_status.sanitizeTaskStatusText(task.error, { errorContext: true });
		const fallbackSummary = require_task_status.sanitizeTaskStatusText(task.terminalSummary, { errorContext: true });
...
function formatTaskBlockedFollowupMessage(task) {
	...
	return `Task needs follow-up: ${resolveTaskDisplayTitle(task)}${resolveTaskRunLabel(task)}. ${require_task_status.sanitizeTaskStatusText(task.terminalSummary, { errorContext: true }) || "Task is blocked and needs follow-up."}`;
}
function formatTaskStateChangeMessage(task, event) {
	const title = resolveTaskDisplayTitle(task);
	if (event.kind === "running") return `Background task started: ${title}.`;
	if (event.kind === "progress") {
		const summary = require_task_status.sanitizeTaskStatusText(event.summary);
...
const TASK_REGISTRY_PROCESS_STATE_KEY = Symbol.for("operator.taskRegistry.state");
...
const log = require_subsystem.createSubsystemLogger("tasks/registry");
...
const TASK_REGISTRY_DELIVERY_RUNTIME_OVERRIDE_KEY = Symbol.for("operator.taskRegistry.deliveryRuntimeOverride");
const TASK_REGISTRY_CONTROL_RUNTIME_OVERRIDE_KEY = Symbol.for("operator.taskRegistry.controlRuntimeOverride");
...
const deliveryRuntimeLoader = require_lazy_promise.createLazyPromiseLoader(() => Promise.resolve().then(() => require("./task-registry-delivery-runtime-uDZZnO09.cjs")), { cacheRejections: true });
const controlRuntimeLoader = require_lazy_promise.createLazyPromiseLoader(() => Promise.resolve().then(() => {
	for (const candidate of TASK_REGISTRY_CONTROL_RUNTIME_CANDIDATES) try {
		return require$1(candidate);
	} catch {}
...
function assertParentFlowLinkAllowed(params) {
	...
	if ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(flow.ownerKey) !== (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.ownerKey)) throw new ParentFlowLinkError("owner_key_mismatch", "Task ownerKey must match parent flow ownerKey.", { flowId });
...
function emitTaskRegistryObserverEvent(createEvent) {
	const observers = require_task_registry_store.getTaskRegistryObservers();
	if (!observers?.onEvent) return;
	try {
		observers.onEvent(createEvent());
	} catch (error) {
...
function persistTaskRegistry() {
	try {
		require_task_registry_store.getTaskRegistryStore().saveSnapshot({
			tasks,
...
function persistTaskUpsert(task, pendingDeliveryState) {
	const store = require_task_registry_store.getTaskRegistryStore();
...
function buildTaskLifecycleTerminalOutcome(params) {
	return require_agent_run_terminal_outcome.buildAgentRunTerminalOutcome({
...
function getTaskRelatedSessionIndexKeys(task) {
	return (0, _gabrielvfonseca_normalization_core_string_normalization.uniqueStrings)([(0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.ownerKey), (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.childSessionKey)].filter(Boolean));
}
function addOwnerKeyIndex(taskId, task) {
	const key = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.ownerKey);
	if (!key) return;
...
function taskRunScopeKey(task) {
	return [
		task.runtime,
		task.scopeKind,
		(0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.ownerKey) ?? "",
		(0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.childSessionKey) ?? ""
	].join("\0");
}
function getTasksByRunScope(params) {
	...
	if (sessionKey) {
		const childMatches = matches.filter((task) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.childSessionKey) === sessionKey);
		if (childMatches.length > 0) return childMatches;
		return matches.filter((task) => task.scopeKind === "session" && (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.ownerKey) === sessionKey);
	}
...
function getPeerTasksForDelivery(task) {
	...
	return getTasksByRunId(task.runId).filter((candidate) => candidate.runtime === task.runtime && candidate.scopeKind === task.scopeKind && ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(candidate.ownerKey) ?? "") === ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.ownerKey) ?? "") && ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(candidate.childSessionKey) ?? "") === ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.childSessionKey) ?? ""));
}
...
function findExistingTaskForCreate(params) {
	...
	const runScopeMatches = runId ? getTasksByRunId(runId).filter((task) => {
		if (task.runtime !== params.runtime || task.scopeKind !== params.scopeKind || ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.ownerKey) ?? "") !== ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.ownerKey) ?? "") || ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.childSessionKey) ?? "") !== ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.childSessionKey) ?? "")) return false;
		if (params.runtime === "acp") return true;
		return ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.parentFlowId) ?? "") === ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.parentFlowId) ?? "");
	}) : [];
	const exact = runId ? runScopeMatches.find((task) => ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.label) ?? "") === ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.label) ?? "") && ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.task) ?? "") === ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.task) ?? "")) : void 0;
...
function mergeExistingTaskForCreate(existing, params) {
	...
	if (params.preferMetadata) {
		if (nextLabel && ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(existing.label) ?? "") !== nextLabel) patch.label = nextLabel;
		const nextTask = params.task.trim();
		if (nextTask && ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(existing.task) ?? "") !== nextTask) patch.task = nextTask;
...
function resolveTaskAgentId(params) {
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.explicitAgentId) ?? require_session_key.parseAgentSessionKey(params.childSessionKey)?.agentId ?? require_session_key.parseAgentSessionKey(params.ownerKey)?.agentId ?? require_session_key.parseAgentSessionKey(params.requesterSessionKey)?.agentId;
}
function resolveTaskRequesterAgentId(params) {
	const explicitRequesterAgentId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.explicitRequesterAgentId);
	return (explicitRequesterAgentId ? (0, _gabrielvfonseca_normalization_core_agent_id.normalizeAgentId)(explicitRequesterAgentId) : void 0) ?? require_session_key.parseAgentSessionKey(params.ownerKey)?.agentId ?? require_session_key.parseAgentSessionKey(params.requesterSessionKey)?.agentId;
}
...
function getLinkedFlowForDelivery(task) {
	...
	const flow = getTaskFlowById(flowId);
	if (!flow) return;
	if ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(flow.ownerKey) !== (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.ownerKey)) return;
	return flow;
}
function resolveTaskDeliveryOwner(task) {
	...
	if (flow) return {
		...
		requesterOrigin: require_delivery_context_shared.normalizeDeliveryContext(flow.requesterOrigin ?? taskDeliveryStates.get(task.taskId)?.requesterOrigin),
		flowId: flow.flowId
	};
	if (task.scopeKind !== "session") return {};
	return {
		sessionKey: task.ownerKey.trim(),
		requesterOrigin: require_delivery_context_shared.normalizeDeliveryContext(taskDeliveryStates.get(task.taskId)?.requesterOrigin)
	};
}
function syncManagedFlowCancellationFromTask(task) {
	...
	if (flow?.syncMode !== "managed" || flow.cancelRequestedAt == null || isTerminalFlowStatus(flow.status)) return;
...
function restoreTaskRegistryOnce() {
	...
	try {
		const restored = require_task_registry_store.getTaskRegistryStore().loadSnapshot();
...
function updateTask(taskId, patch) {
	...
	if (isTerminalTaskStatus(next.status) && typeof next.cleanupAfter !== "number") {
		...
		const cleanupAfter = require_task_retention.resolveTaskCleanupAfter({
			...next,
			createdAt
		});
		Object.assign(next, cleanupAfter === void 0 ? {} : { cleanupAfter });
	}
	const sessionIndexChanged = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(current.ownerKey) !== (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(next.ownerKey) || (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(current.childSessionKey) !== (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(next.childSessionKey);
...
function upsertTaskDeliveryState(state) {
	...
	const next = {
		taskId: state.taskId,
		...state.requesterOrigin ? { requesterOrigin: require_delivery_context_shared.normalizeDeliveryContext(state.requesterOrigin) } : {},
...
function canDeliverTaskToRequesterOrigin(task) {
	const owner = resolveTaskDeliveryOwner(task);
	if (require_completion_delivery_policy.shouldRouteCompletionThroughRequesterSession(owner.sessionKey)) return false;
	return canDeliverToRequesterOrigin(owner.requesterOrigin);
}
function canDeliverToRequesterOrigin(origin) {
	const channel = origin?.channel?.trim();
	const to = origin?.to?.trim();
	return Boolean(channel && to && require_message_channel.isDeliverableMessageChannel(channel));
}
...
function queueTaskSystemEvent(task, text) {
	...
	require_system_events.enqueueSystemEvent(text, {
		sessionKey: ownerKey,
...
function queueBlockedTaskFollowup(task) {
	...
	const ownerKey = owner.sessionKey?.trim();
	if (!ownerKey) return false;
	require_system_events.enqueueSystemEvent(followupText, {
		sessionKey: ownerKey,
...
async function runTaskDeliveryWithIndependentAdmission(taskId, deliver) {
	...
	try {
		...
		if (!admitted && require_gateway_work_admission.isGatewayRestartDraining()) {
			ensureTaskRegistryReady();
			const current = tasks.get(taskId);
...
async function maybeDeliverTaskTerminalUpdateUnderAdmission(taskId) {
	...
	try {
		...
		try {
			...
			const requesterAgentId = require_session_key.parseAgentSessionKey(ownerSessionKey)?.agentId;
...
function ensureListener() {
	...
	listenerStop = require_agent_events.onAgentEvent((evt) => {
		restoreTaskRegistryOnce();
...
function createTaskRecord(params) {
	...
	const now = Date.now();
	const taskId = node_crypto.default.randomUUID();
	...
	const record = normalizeTaskTimestamps({
		taskId,
		runtime: params.runtime,
		taskKind: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.taskKind),
		sourceId: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.sourceId),
		requesterSessionKey,
		ownerKey,
		scopeKind,
		childSessionKey: params.childSessionKey,
		parentFlowId: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.parentFlowId),
		parentTaskId: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.parentTaskId),
		agentId,
		requesterAgentId,
		runId: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.runId),
	...
	if (isTerminalTaskStatus(record.status) && typeof record.cleanupAfter !== "number") {
		const cleanupAfter = require_task_retention.resolveTaskCleanupAfter(record);
...
function hasActiveTaskForChildSessionKey(params) {
	...
	for (const taskId of ids) {
		...
		if (task && isActiveTaskStatus(task.status) && (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(task.childSessionKey) === sessionKey) return true;
	}
	return false;
}
...
function listTasksForOwnerKey(ownerKey) {
	ensureTaskRegistryReady();
	const key = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(ownerKey);
	if (!key) return [];
...
function resetTaskRegistryForTests(opts) {
	...
	require_task_registry_store.resetTaskRegistryRuntimeForTests();
	if (listenerStop) {
		listenerStop();
		listenerStop = null;
	}
	...
	require_task_registry_store.getTaskRegistryStore().close?.();
}
...
if (process.env.VITEST || false) globalThis[Symbol.for("operator.taskRegistryTestApi")] = {
...
//#endregion
Object.defineProperty(exports, "SUBAGENT_KILL_TASK_ERROR", {
	enumerable: true,
	get: function() {
		return SUBAGENT_KILL_TASK_ERROR;
	}
});
Object.defineProperty(exports, "assertTaskCancellationReadyById", {
	enumerable: true,
	get: function() {
		return assertTaskCancellationReadyById;
	}
});
Object.defineProperty(exports, "cancelTaskById", {
	enumerable: true,
	get: function() {
		return cancelTaskById;
	}
});
Object.defineProperty(exports, "createTaskFlowForTask", {
	enumerable: true,
	get: function() {
		return createTaskFlowForTask;
	}
});
Object.defineProperty(exports, "createTaskRecord", {
	enumerable: true,
	get: function() {
		return createTaskRecord;
	}
});
Object.defineProperty(exports, "deleteTaskFlowRecordById", {
	enumerable: true,
	get: function() {
		return deleteTaskFlowRecordById;
	}
});
Object.defineProperty(exports, "deleteTaskRecordById", {
	enumerable: true,
	get: function() {
		return deleteTaskRecordById;
	}
});
Object.defineProperty(exports, "ensureTaskFlowRegistryReady", {
	enumerable: true,
	get: function() {
		return ensureTaskFlowRegistryReady;
	}
});
Object.defineProperty(exports, "ensureTaskRegistryReady", {
	enumerable: true,
	get: function() {
		return ensureTaskRegistryReady;
	}
});
...
Object.defineProperty(exports, "getTaskById", {
	enumerable: true,
	get: function() {
		return getTaskById;
	}
});
Object.defineProperty(exports, "hasActiveTaskForChildSessionKey", {
	enumerable: true,
	get: function() {
		return hasActiveTaskForChildSessionKey;
	}
});
Object.defineProperty(exports, "isChildlessNativeSubagentTask", {
	enumerable: true,
	get: function() {
		return isChildlessNativeSubagentTask;
	}
});
Object.defineProperty(exports, "isProvisionalSubagentKillTask", {
	enumerable: true,
	get: function() {
		return isProvisionalSubagentKillTask;
	}
});
...
Object.defineProperty(exports, "linkTaskToFlowById", {
	enumerable: true,
	get: function() {
		return linkTaskToFlowById;
	}
});
Object.defineProperty(exports, "listFreshTasksForOwnerKey", {
	enumerable: true,
	get: function() {
		return listFreshTasksForOwnerKey;
	}
});
Object.defineProperty(exports, "listTaskFlowRecords", {
	enumerable: true,
	get: function() {
		return listTaskFlowRecords;
	}
});
Object.defineProperty(exports, "listTaskRecords", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "listTasksForAgentId", {
	enumerable: true,
	get: function() {
		return listTasksForAgentId;
	}
});
Object.defineProperty(exports, "listTasksForFlowId", {
	enumerable: true,
	get: function() {
		return listTasksForFlowId;
	}
});
Object.defineProperty(exports, "listTasksForOwnerKey", {
	enumerable: true,
	get: function() {
		return listTasksForOwnerKey;
	}
});
Object.defineProperty(exports, "listTasksForRelatedSessionKey", {
	enumerable: true,
	get: function() {
		return listTasksForRelatedSessionKey;
	}
});
Object.defineProperty(exports, "listTasksForSessionKey", {
	enumerable: true,
	get: function() {
		return listTasksForSessionKey;
	}
});
Object.defineProperty(exports, "markTaskLostById", {
	enumerable: true,
	get: function() {
		return markTaskLostById;
	}
});
Object.defineProperty(exports, "markTaskRunningByRunId", {
	enumerable: true,
	get: function() {
		return markTaskRunningByRunId;
	}
});
Object.defineProperty(exports, "markTaskTerminalById", {
	enumerable: true,
	get: function() {
		return markTaskTerminalById;
	}
});
Object.defineProperty(exports, "maybeDeliverTaskTerminalUpdate", {
	enumerable: true,
	get: function() {
		return maybeDeliverTaskTerminalUpdate;
	}
});
Object.defineProperty(exports, "recordTaskProgressByRunId", {
	enumerable: true,
	get: function() {
		return recordTaskProgressByRunId;
	}
});
Object.defineProperty(exports, "resolveChildlessNativeSubagentTaskDefinition", {
	enumerable: true,
	get: function() {
		return resolveChildlessNativeSubagentTaskDefinition;
	}
});
Object.defineProperty(exports, "resolveTaskForLookupToken", {
	enumerable: true,
	get: function() {
		return resolveTaskForLookupToken;
	}
});
Object.defineProperty(exports, "setTaskCleanupAfterById", {
	enumerable: true,
	get: function() {
		return setTaskCleanupAfterById;
	}
});
Object.defineProperty(exports, "setTaskRunDeliveryStatusByRunId", {
	enumerable: true,
	get: function() {
		return setTaskRunDeliveryStatusByRunId;
	}
});
Object.defineProperty(exports, "updateTaskNotifyPolicyById", {
	enumerable: true,
	get: function() {
