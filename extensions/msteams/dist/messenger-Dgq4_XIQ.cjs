...
function stripHtmlFromTeamsMessage(html) {
	...
	text = text.replace(/<[^>]*>/g, " ");
	text = require_sessions.decodeHtmlEntities(text).replaceAll("\xA0", " ");
...
async function fetchChannelMessage(token, groupId, channelId, messageId, deadline) {
	...
	try {
		return await require_graph_users.fetchGraphJson({
			token,
			path,
			...deadline ? { deadline } : {}
		});
...
async function validateConsentUploadUrl(url, opts) {
	...
	const hostname = require_string_coerce.normalizeLowercaseStringOrEmpty(parsed.hostname);
...
async function uploadToConsentUrl(params) {
	...
	const res = await require_fetch_timeout.fetchWithTimeout(params.url, {
		method: "PUT",
		headers: {
			"User-Agent": require_graph_users.buildUserAgent(),
			"Content-Type": params.contentType ?? "application/octet-stream",
			"Content-Range": `bytes 0-${params.buffer.length - 1}/${params.buffer.length}`
		},
		body: new Uint8Array(params.buffer)
	}, params.timeoutMs ?? require_graph_users.resolveMSTeamsSharePointUploadTimeoutMs(params.buffer.length), fetchFn);
...
async function withPendingUploadLock(options, run) {
	return await require_polls.withMSTeamsSqliteMutationLock(options, PENDING_UPLOAD_LOCK_FILENAME, run);
}
...
function buildTeamsFileInfoCard(file) {
	...
	const fileType = lastDot >= 0 ? require_string_coerce.normalizeLowercaseStringOrEmpty(file.name.slice(lastDot + 1)) : "";
	return {
		contentType: "application/vnd.microsoft.teams.card.file.info",
...
async function getGraphAccessToken(tokenProvider) {
	return await require_graph_users.withMSTeamsRequestDeadline({
...
async function uploadToSharePoint(params) {
	...
	const data = await require_graph_users.withMSTeamsAbortableRequestTimeout({
		label: SHAREPOINT_UPLOAD_TIMEOUT_LABEL,
		timeoutMs: require_graph_users.resolveMSTeamsSharePointUploadTimeoutMs(params.buffer.length),
		work: async (signal) => {
			...
			if (!res.ok) throw await require_oauth_token.createMSTeamsHttpError(res, "SharePoint upload failed");
			return await require_provider_http_errors.readProviderJsonResponse(res, "msteams.graph-upload.uploadSharePointFile");
		}
	});
...
async function getDriveItemProperties(params) {
	...
	const data = await require_graph_users.withMSTeamsAbortableRequestTimeout({
		...
		work: async (signal) => {
			...
			if (!res.ok) throw await require_oauth_token.createMSTeamsHttpError(res, "Get driveItem properties failed");
			return await require_provider_http_errors.readProviderJsonResponse(res, "msteams.graph-upload.getDriveItemProperties");
		}
	});
...
async function getChatMembers(params) {
	const fetchFn = params.fetchFn ?? fetch;
	return await require_graph_users.withMSTeamsAbortableRequestTimeout({
		label: SHAREPOINT_REQUEST_TIMEOUT_LABEL,
		work: async (signal) => {
			const token = await getGraphAccessToken(params.tokenProvider);
			...
			if (!res.ok) throw await require_oauth_token.createMSTeamsHttpError(res, res.status === 403 ? "Get chat members failed; verify Graph chat-member permissions and tenant access policies" : "Get chat members failed");
			return ((await require_provider_http_errors.readProviderJsonResponse(res, "msteams.graph-upload.getChatMembers")).value ?? []).map((member) => ({ aadObjectId: member.userId ?? "" })).filter((member) => member.aadObjectId);
		}
	});
}
/**
...
async function createSharePointSharingLink(params) {
	...
	const data = await require_graph_users.withMSTeamsAbortableRequestTimeout({
		...
		work: async (signal) => {
			...
			if (!res.ok) throw await require_oauth_token.createMSTeamsHttpError(res, "Create SharePoint sharing link failed");
			return await require_provider_http_errors.readProviderJsonResponse(res, "msteams.graph-upload.createSharePointSharingLink");
		}
	});
...
async function getMimeType(url) {
	...
	return await (0, _gabrielvfonseca_media_core_mime.detectMime)({ filePath: url }) ?? "application/octet-stream";
}
/**
...
async function extractFilename(url) {
	if (url.startsWith("data:")) {
		...
		const ext = (0, _gabrielvfonseca_media_core_mime.extensionForMime)(mime) ?? ".bin";
		return `${mime.startsWith("image/") ? "image" : "file"}${ext}`;
	}
	try {
		const pathname = new URL(url).pathname;
		const basename = node_path.default.basename(pathname);
		const existingExt = (0, _gabrielvfonseca_media_core_mime.getFileExtension)(pathname);
		...
		return require_store.extractOriginalFilename(url);
	}
}
/**
...
async function withRevokedProxyFallback(params) {
	try {
		...
		if (!require_errors.isRevokedProxyError(err)) throw err;
		params.onRevokedLog?.();
...
const loadMSTeamsApiModule = require_lazy_runtime.createLazyRuntimeModule(() => import("@microsoft/teams.api"));
...
function normalizeRequiredServiceUrl(ref) {
	...
	return require_graph_users.normalizeBotFrameworkServiceUrl(ref.serviceUrl);
}
function buildSdkConversationReference(source, options) {
	...
	if (options?.serviceUrlBoundary) require_graph_users.validateMSTeamsProactiveServiceUrlBoundary({
...
function sameServiceUrl(left, right) {
	...
	try {
		return require_graph_users.normalizeBotFrameworkServiceUrl(left) === right;
	} catch {
		return false;
	}
}
...
function pushTextMessages(out, text, opts) {
	...
	if (opts.chunkText) {
		for (const chunk of require_runtime.getMSTeamsRuntime().channel.text.chunkMarkdownTextWithMode(text, opts.chunkLimit, opts.chunkMode)) {
			...
			if (!trimmed || require_tokens.isSilentReplyText(trimmed, "NO_REPLY")) continue;
			out.push({ text: trimmed });
		}
		return;
	}
	const trimmed = text.trim();
	if (!trimmed || require_tokens.isSilentReplyText(trimmed, "NO_REPLY")) return;
	out.push({ text: trimmed });
}
...
function renderReplyPayloadsToMessages(replies, options) {
	...
	for (const payload of replies) {
		const reply = require_reply_payload.resolveSendableOutboundReplyParts(payload, { text: require_runtime.getMSTeamsRuntime().channel.text.convertMarkdownTables(payload.text ?? "", tableMode) });
...
async function buildActivity(msg, conversationRef, tokenProvider, sharePointSiteId, mediaMaxBytes, options) {
	...
	if (msg.mediaUrl) {
		...
		if (isLocalPath(msg.mediaUrl)) {
			...
			const media = await require_web_media.loadWebMedia(msg.mediaUrl, maxBytes);
			contentType = media.contentType ?? contentType;
			fileName = media.fileName ?? fileName;
			const conversationType = require_string_coerce.normalizeOptionalLowercaseString(conversationRef.conversation?.conversationType);
...
async function sendMSTeamsMessages(params) {
	...
	const sendWithRetry = async (sendOnce, meta) => {
		...
		for (const attempt of Array.from({ length: retryOptions.maxAttempts }, (_, index) => index + 1)) try {
			...
			await require_sleep.sleep(delayMs);
		}
...
Object.defineProperty(exports, "formatThreadContext", {
	enumerable: true,
	get: function() {
