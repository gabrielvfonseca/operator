...
function scrubDoctorErrorMessage(err) {
	...
	return `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(stripped, ERR_MESSAGE_MAX_LEN - 3)}...`;
}
//#endregion
...
async function runDoctorHealthRepairs(ctx, opts = {}) {
	const checks = (opts.checks ?? require_health_check_registry.listHealthChecks()).map(require_health_check_adapter.normalizeHealthCheck);
	const findings = [];
...
function uniqueDefined(values) {
	return (0, _gabrielvfonseca_normalization_core_string_normalization.uniqueStrings)(values.filter((value) => value !== void 0));
}
//#endregion
exports.runDoctorHealthRepairs = runDoctorHealthRepairs;
