...
const getMessageChannels = () => require_message_channel.listDeliverableMessageChannels();
...
function resolveKnownChannel(value) {
	...
	if (!require_message_channel.isDeliverableMessageChannel(normalized)) return;
...
function resolveAvailableKnownChannel(params) {
	...
	if (!normalized) return;
	return require_channel_resolution.resolveOutboundChannelPlugin({
		channel: normalized,
		cfg: params.cfg,
		allowBootstrap: true
	}) ? normalized : void 0;
...
function listConfiguredOfficialExternalRepairHints(cfg) {
	...
	return Object.keys(channels).filter((channelId) => isConfiguredChannel(cfg, channelId)).map((channelId) => require_official_external_plugin_repair_hints.resolveMissingOfficialExternalChannelPluginRepairHint({
		config: cfg,
		channelId
...
function logChannelSelectionError(params) {
	...
	require_runtime.defaultRuntime.error?.(`[channel-selection] ${params.pluginId}(${params.accountId}) ${params.operation} failed: ${message}`);
}
async function isPluginConfigured(plugin, cfg) {
	...
	for (const accountId of accountIds) {
		...
		if (!(plugin.config.isEnabled ? plugin.config.isEnabled(account, cfg) : require_account_enabled.isAccountEnabled(account))) continue;
...
async function resolveMessageChannelSelection(params) {
	...
	if (normalized) {
		...
		if (!availableExplicit) {
			...
			const repairHint = isConfiguredChannel(params.cfg, normalized) ? require_official_external_plugin_repair_hints.resolveMissingOfficialExternalChannelPluginRepairHint({
				config: params.cfg,
	...
	if (configured.length === 1) return {
		channel: (0, _gabrielvfonseca_normalization_core.expectDefined)(configured[0], "configured entry at 0"),
		configured,
...
//#endregion
Object.defineProperty(exports, "isConfiguredChannel", {
	enumerable: true,
	get: function() {
		return isConfiguredChannel;
	}
});
Object.defineProperty(exports, "listConfiguredMessageChannels", {
	enumerable: true,
	get: function() {
		return listConfiguredMessageChannels;
	}
});
Object.defineProperty(exports, "resolveMessageChannelSelection", {
	enumerable: true,
	get: function() {
		return resolveMessageChannelSelection;
	}
});
