...
function collectEnabledInsecureOrDangerousFlagsFromContracts(cfg, inputs = {}) {
	...
	const collectSandboxDockerDangerousFlags = (docker, pathPrefix) => {
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(docker)) return;
		for (const key of require_config.DANGEROUS_SANDBOX_DOCKER_BOOLEAN_KEYS) if (docker[key] === true) enabledFlags.push(`${pathPrefix}.${key}=true`);
	};
	...
	collectSandboxDockerDangerousFlags((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(cfg.agents?.defaults?.sandbox?.docker) ? cfg.agents?.defaults?.sandbox?.docker : void 0, "agents.defaults.sandbox.docker");
	if (Array.isArray(cfg.agents?.list)) for (const [index, agent] of cfg.agents.list.entries()) collectSandboxDockerDangerousFlags((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(agent?.sandbox?.docker) ? agent.sandbox.docker : void 0, `${getAgentDangerousFlagPathSegment(agent, index)}.sandbox.docker`);
	const pluginEntries = cfg.plugins?.entries;
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(pluginEntries)) return enabledFlags;
	...
	for (const [pluginId, metadata] of configContracts.entries()) {
		...
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(pluginEntry) || !(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(pluginEntry.config)) continue;
...
function collectEnabledInsecureOrDangerousFlagsFromCurrentSnapshot(cfg) {
	const pluginEntries = cfg.plugins?.entries;
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(pluginEntries)) return collectEnabledInsecureOrDangerousFlagsFromContracts(cfg);
...
//#endregion
Object.defineProperty(exports, "collectCoreInsecureOrDangerousFlags", {
	enumerable: true,
	get: function() {
		return collectCoreInsecureOrDangerousFlags;
	}
});
Object.defineProperty(exports, "collectEnabledInsecureOrDangerousFlagsFromContracts", {
	enumerable: true,
	get: function() {
		return collectEnabledInsecureOrDangerousFlagsFromContracts;
	}
});
Object.defineProperty(exports, "collectEnabledInsecureOrDangerousFlagsFromCurrentSnapshot", {
	enumerable: true,
	get: function() {
		return collectEnabledInsecureOrDangerousFlagsFromCurrentSnapshot;
	}
});
