...
var store_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	WorkerSessionAlreadyAttachedError: () => WorkerSessionAlreadyAttachedError,
	createWorkerEnvironmentStore: () => createWorkerEnvironmentStore,
	normalizeWorkerSshEndpoint: () => normalizeWorkerSshEndpoint
});
...
function normalizeBootstrapReceipt(value) {
	...
	return {
		...
		protocolFeatures: (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeSortedUniqueTrimmedStringList)(value.protocolFeatures)
	};
}
...
function normalizeAttachedSessionIds(value) {
	const sessionIds = (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeSortedUniqueTrimmedStringList)(value);
...
function normalizeWorkerSshEndpoint(value) {
	...
	if (!require_ref_contract.isValidSecretRef(value.keyRef)) throw new Error("Worker environment SSH key must be a canonical SecretRef");
	return {
		host,
		port: value.port,
		user,
...
function bootstrapReceiptFrom(row) {
	const { bootstrap_bundle_hash: bundleHash, bootstrap_operator_version: operatorVersion, bootstrap_protocol_features_json: encodedFeatures } = row;
	if (bundleHash === null && operatorVersion === null && encodedFeatures === null) return null;
	if (bundleHash === null || operatorVersion === null || encodedFeatures === null) throw new Error("Worker environment bootstrap receipt is incomplete");
...
function nextGlobalOwnerEpoch(db) {
	const latestEnvironment = require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(db, query(db).selectFrom("worker_environments").select(({ fn }) => fn.max("owner_epoch").as("owner_epoch")));
	const latestTranscriptCommit = require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(db, query(db).selectFrom("worker_transcript_commit_heads").select(({ fn }) => fn.max("run_epoch").as("run_epoch")));
...
function find(db, environmentId) {
	const row = require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(db, query(db).selectFrom("worker_environments").selectAll().where("environment_id", "=", environmentId));
	return row ? fromRow(row) : void 0;
}
function findCredential(db, environmentId) {
	const row = require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(db, query(db).selectFrom("worker_environment_credentials").selectAll().where("environment_id", "=", environmentId));
	return row ? credentialFromRow(row) : void 0;
}
function findCredentialByHash(db, credentialHash) {
	const row = require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(db, query(db).selectFrom("worker_environment_credentials").selectAll().where("credential_hash", "=", credentialHash));
	return row ? credentialFromRow(row) : void 0;
}
...
function update(db, id, state, values) {
	if (require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, query(db).updateTable("worker_environments").set(values).where("environment_id", "=", id).where("state", "=", state)).numAffectedRows !== 1n) throw new Error(`Worker environment ${id} changed during update`);
	return getRequired(db, id);
}
function revokeCredential(db, environmentId) {
	require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, query(db).deleteFrom("worker_environment_credentials").where("environment_id", "=", environmentId));
}
function upsertCredential(db, credential) {
	require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, query(db).insertInto("worker_environment_credentials").values(credential).onConflict((conflict) => conflict.column("environment_id").doUpdateSet({
...
function listRows(db, reconcile) {
	...
	return require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, (reconcile ? filtered.orderBy("provider_id") : filtered).orderBy("created_at_ms").orderBy("environment_id")).rows.map(fromRow);
}
...
function createWorkerEnvironmentStore(options = {}) {
	...
	const read = () => require_openclaw_state_db.openOperatorStateDatabase({ path }).db;
	...
	return {
		createIntent(input) {
			...
			return write((db) => {
				require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, query(db).insertInto("worker_environments").values({
		...
		transition(input) {
			...
			return write((db) => {
				...
				const record = update(db, environmentId, from, {
					...
					bootstrap_operator_version: bootstrapReceipt?.operatorVersion ?? null,
...
//#endregion
Object.defineProperty(exports, "WorkerSessionAlreadyAttachedError", {
	enumerable: true,
	get: function() {
		return WorkerSessionAlreadyAttachedError;
	}
});
Object.defineProperty(exports, "normalizeWorkerSshEndpoint", {
	enumerable: true,
	get: function() {
		return normalizeWorkerSshEndpoint;
	}
