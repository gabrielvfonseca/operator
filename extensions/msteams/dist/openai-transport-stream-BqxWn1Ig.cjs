...
const require_cjk_chars = require("./cjk-chars-BSl3Lc9n.cjs");
...
function resolveAnthropicEphemeralCacheControl(baseUrl, cacheRetention) {
	const retention = cacheRetention ?? (process.env.OPERATOR_CACHE_RETENTION === "long" ? "long" : "short");
...
function applyAnthropicCacheControlToSystem(system, cacheControl) {
	...
	for (const block of system) {
		...
		const split = (0, _gabrielvfonseca_ai_internal_shared.splitSystemPromptCacheBoundary)(record.text);
		if (!split) {
...
function stripAnthropicSystemPromptBoundary(system) {
	...
	for (const block of system) {
		...
		const record = block;
		if (record.type === "text" && typeof record.text === "string") record.text = (0, _gabrielvfonseca_ai_internal_shared.stripSystemPromptCacheBoundary)(record.text);
	}
}
...
function resolveAnthropicPayloadPolicy(input) {
	return {
		allowsServiceTier: require_provider_attribution.resolveProviderRequestCapabilities({
...
function transformTransportMessages(messages, model, normalizeToolCallId, options) {
	...
	const replayable = messages.map((msg) => {
		...
		const modelBoundThinkingReplayMode = (0, _gabrielvfonseca_ai_internal_anthropic.resolveModelBoundThinkingReplayMode)({
			source: {
	...
	return require_session_transcript_repair.repairToolUseResultPairing(replayable, {
...
function sanitizeTransportPayloadText(text) {
	...
	return (0, _gabrielvfonseca_ai_internal_shared.sanitizeSurrogates)(text);
}
...
function createWritableTransportEventStream() {
	const eventStream = (0, require_session_transcript_repair.event_stream_exports.createAssistantMessageEventStream)();
	return {
		eventStream,
		stream: eventStream
	};
...
function normalizeTransportErrorBody(value) {
	...
	if (!text?.trim()) return;
	return require_provider_http_errors.truncateErrorDetail(require_redact.redactSensitiveText(text), 500);
}
...
function resolveOpenAIReasoningEffortMap(model, fallbackMap = {}) {
	const provider = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(model.provider ?? "");
	const id = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(model.id ?? "");
...
require_lazy_promise.createLazyImportLoader(() => import("openclaw/plugin-sdk/llm"));
//#endregion
...
function isGoogleThinkingRequiredModel(modelId) {
	return require_string_coerce.normalizeLowercaseStringOrEmpty(modelId).includes("gemini-2.5-pro");
}
/** @deprecated Google provider-owned stream helper; do not use from third-party plugins. */
function isGoogleGemini25ThinkingBudgetModel(modelId) {
	return /(?:^|\/)gemini-2\.5-/.test(require_string_coerce.normalizeLowercaseStringOrEmpty(modelId));
}
/** @deprecated Google provider-owned stream helper; do not use from third-party plugins. */
function isGoogleGemini3ProModel(modelId) {
	const normalized = require_string_coerce.normalizeLowercaseStringOrEmpty(modelId);
...
function isGemma4Model(modelId) {
	return require_string_coerce.normalizeLowercaseStringOrEmpty(modelId).startsWith("gemma-4");
}
...
const log = require_subsystem.createSubsystemLogger("openai-transport");
...
function resolveCacheRetention(cacheRetention) {
	...
	if (typeof process !== "undefined" && process.env.OPERATOR_CACHE_RETENTION === "long") return "long";
	return "short";
}
function resolvePromptCacheKey(options, cacheRetention) {
	if (cacheRetention === "none") return;
	return (0, _gabrielvfonseca_ai_internal_openai.clampOpenAIPromptCacheKey)(options?.promptCacheKey ?? options?.sessionId);
}
...
function transportPayloadToolName(tool) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(tool)) return;
	const name = readToolPayloadField$1(tool, "name");
	if (typeof name === "string") return name;
	const fn = readToolPayloadField$1(tool, "function");
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(fn)) return;
...
function enforceCodeModeResponsesToolSurface(payload) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(payload) || !Array.isArray(payload.tools)) return;
	payload.tools = payload.tools.filter((tool) => {
		const name = transportPayloadToolName(tool);
		return typeof name === "string" && require_code_mode_control_tools.isCodeModeModelVisibleToolName(name);
	});
}
function assertCodeModeResponsesToolSurface(payload) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(payload) || !Array.isArray(payload.tools)) throw new Error("Code mode payload tool surface violation: expected exec,wait; got no tools");
	...
	if (names.length >= 2 && new Set(names).size === names.length && names.filter((name) => name === "exec").length === 1 && names.filter((name) => name === "wait").length === 1 && names.every(require_code_mode_control_tools.isCodeModeModelVisibleToolName)) return;
...
function buildOpenAIStrictToolDowngradeDiagnosticKey(diagnostics, context) {
	return require_crypto_digest.sha256Hex(JSON.stringify({
...
function resolveOpenAIStrictToolFlagWithDiagnostics(projection, strictSetting, context) {
	const strict = (0, _gabrielvfonseca_ai_internal_openai.resolveOpenAIProjectedToolsStrictToolFlag)(projection, strictSetting);
	if (strictSetting === true && strict === false && log.isEnabled("debug", "any")) {
		const diagnostics = (0, _gabrielvfonseca_ai_internal_openai.findOpenAIStrictToolProjectionDiagnostics)(projection);
...
function isOpenAICodexResponsesModel(model) {
	return OPENAI_CODEX_RESPONSES_PROVIDERS.has(model.provider) && (model.api === "openai-chatgpt-responses" || model.api === "operator-openai-responses-transport");
}
...
function buildOpenAIClientHeaders(model, context, optionHeaders, turnHeaders, sessionId) {
	...
	if (model.provider === "github-copilot") Object.assign(providerHeaders, require_provider_request_config.buildCopilotDynamicHeaders({
	...
	const resolvedHeaders = require_provider_request_config.resolveProviderRequestPolicyConfig({
		provider: model.provider,
		api: model.api,
	...
	if (sessionId && !Object.keys(resolvedHeaders).some((key) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(key) === "session_id") && usesNativeOpenAICodexResponsesBackend(model)) resolvedHeaders.session_id = sessionId;
	return resolvedHeaders;
}
function resolveOpenAISdkTimeoutMs(model) {
	return require_session_transcript_repair.resolveModelRequestTimeoutMs(model, void 0);
}
...
function detectCompat(model) {
	const { defaults } = require_openai_completions_compat.detectOpenAICompletionsCompat(model);
	return {
...
function createOpenAICompletionsClient(model, context, apiKey, optionHeaders, opts) {
	...
	return new openai.default({
		...
		fetch: opts?.fetch ?? require_session_transcript_repair.buildGuardedModelFetch(model),
	...
	return (model, context, options) => {
		...
		(async () => {
			...
			try {
				...
				const baseFetch = require_session_transcript_repair.buildGuardedModelFetch(model);
				...
				if (options?.operatorCodeModeToolSurface === true) {
				...
				firstEventAbort = (0, _gabrielvfonseca_ai_internal_runtime.createFirstStreamEventAbortController)(options?.signal);
				...
				await processOpenAICompletionsStream(responseStream, output, model, stream, {
					...
					emitReasoning,
					firstEventTimeoutMs: (0, _gabrielvfonseca_ai_internal_runtime.getFirstStreamEventTimeoutMs)(options),
					abortFirstEventStream: firstEventAbort.abort,
					onFirstEventTimeout: (0, _gabrielvfonseca_ai_internal_runtime.getFirstStreamEventTimeoutHandler)(options),
...
async function processOpenAICompletionsStream(responseStream, output, model, stream, options) {
	...
	const reasoningTagTextPartitioner = (0, _gabrielvfonseca_ai_internal_runtime.createReasoningTagTextPartitioner)();
	...
	const appendRecoveredToolCall = (toolCall) => {
		...
		const block = {
			type: "toolCall",
			id: `call_${(0, node_crypto.randomUUID)().replaceAll("-", "").slice(0, 24)}`,
			name: toolCall.name,
	...
	const guardedStream = (0, _gabrielvfonseca_ai_internal_runtime.withFirstStreamEventTimeout)(responseStream, {
		provider: model.provider,
		api: model.api,
		model: model.id,
	...
	for await (const rawChunk of guardedStream) {
		...
		if (choice.finish_reason) {
			const finishReasonResult = (0, _gabrielvfonseca_ai_internal_openai.mapOpenAIStopReason)(choice.finish_reason, { allowSingularToolCall: true });
		...
		if (choiceDelta.tool_calls && choiceDelta.tool_calls.length > 0) {
			...
			for (const toolCall of choiceDelta.tool_calls) {
				...
				if (toolCall.function?.arguments) {
					...
					block.arguments = (0, _gabrielvfonseca_ai_internal_runtime.parseStreamingJson)(block.partialArgs);
					pushStreamEvent({
						type: "toolcall_delta",
						contentIndex: toolCallBlockIndices.get(block) ?? -1,
...
function parseDeepSeekDsmlInvokeArguments(body) {
	...
	try {
		...
		if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(parsed) && Object.keys(parsed).length > 0) return parsed;
	} catch {
		return null;
	}
	return null;
...
function estimateOpenAICompletionsInputTokens(payload) {
	...
	if (Array.isArray(payload.tools) && payload.tools.length > 0) try {
		adjustedChars += require_cjk_chars.estimateStringChars(JSON.stringify(payload.tools));
	} catch {
		adjustedChars += 1024;
	}
	if (payload.response_format !== void 0) try {
		adjustedChars += require_cjk_chars.estimateStringChars(JSON.stringify(payload.response_format));
	} catch {
		adjustedChars += 256;
	}
...
function estimateOpenAICompletionsMessagesChars(messages) {
	...
	for (const message of messages) {
		...
		if (record.tool_calls !== void 0) try {
			adjustedChars += require_cjk_chars.estimateStringChars(JSON.stringify(record.tool_calls));
		} catch {
			adjustedChars += 256;
		}
	}
...
function estimateOpenAICompletionsContentChars(value) {
	if (typeof value === "string") return require_cjk_chars.estimateStringChars(value);
	...
	for (const block of value) {
		...
		if (typeof text === "string") {
			adjustedChars += require_cjk_chars.estimateStringChars(text);
			continue;
		}
		try {
			adjustedChars += require_cjk_chars.estimateStringChars(JSON.stringify(block));
		} catch {
			adjustedChars += 256;
		}
	}
...
function convertTools(tools, compat, model) {
	const projection = (0, _gabrielvfonseca_ai_internal_openai.projectOpenAITools)(tools);
	const strict = resolveOpenAIStrictToolFlagWithDiagnostics(projection, require_session_transcript_repair.resolveOpenAIStrictToolSetting(model, {
		transport: "stream",
	...
	return {
		...
		tools: sortTransportToolsByName(projection.tools).map((tool) => {
			const functionTool = {
				...
				parameters: (0, _gabrielvfonseca_ai_internal_openai.normalizeOpenAIStrictToolParameters)(tool.parameters, strict === true, model.compat)
			};
...
function isGoogleOpenAICompatModel(model) {
	const endpointClass = require_openai_completions_compat.detectOpenAICompletionsCompat(model).capabilities.endpointClass;
...
function getReasoningContentReplayModelIdCandidates(modelId) {
	...
	return (0, _gabrielvfonseca_normalization_core_string_normalization.uniqueStrings)(candidates.filter(Boolean));
}
...
function buildOpenAICompletionsParams(model, context, options) {
	const compat = getCompat(model);
	const compatDetection = require_openai_completions_compat.detectOpenAICompletionsCompat(model);
	let messages = (0, _gabrielvfonseca_ai_internal_openai.convertMessages)(model, context.systemPrompt ? {
		...context,
		systemPrompt: (0, _gabrielvfonseca_ai_internal_shared.stripSystemPromptCacheBoundary)(context.systemPrompt)
	} : context, compat);
	...
	if (supportsModelTools(model)) {
		if (context.tools) {
			...
			if (options?.toolChoice) {
				const toolChoice = (0, _gabrielvfonseca_ai_internal_openai.reconcileOpenAICompletionsToolChoice)(options.toolChoice, converted.projection);
	...
	}
	{
		...
		if (maxTokenBudget.clampToModelMaxTokens && clampedMaxTokens !== void 0 && modelMaxTokens !== void 0 && clampedMaxTokens > modelMaxTokens) {
			...
			require_session_transcript_repair.emitModelTransportDebug(log, `[completions] clamp_max_tokens provider=${model.provider} api=${model.api} model=${model.id} requested=${effectiveMaxTokens} output=${clampedMaxTokens} modelMaxTokens=${modelMaxTokens}`);
		}
		if (compatDetection.capabilities.usesExplicitProxyLikeEndpoint && clampedMaxTokens !== void 0 && effectiveContextTokens !== void 0) {
			...
			if (clampedMaxTokens > remainingBudget) {
				...
				require_session_transcript_repair.emitModelTransportDebug(log, `[completions] clamp_max_tokens provider=${model.provider} api=${model.api} model=${model.id} requested=${effectiveMaxTokens} output=${clampedMaxTokens} effectiveContext=${effectiveContextTokens} estimatedInput=${estimatedInputTokens}`);
			}
		}
	...
	const resolvedCompletionsReasoningEffort = completionsReasoningEffort ? (0, _gabrielvfonseca_ai_internal_openai.resolveOpenAIReasoningEffortForModel)({
		model,
		effort: completionsReasoningEffort,
		fallbackMap: compat.reasoningEffortMap
	}) : void 0;
	const omitChatCompletionsToolReasoningEffort = Array.isArray(params.tools) && params.tools.length > 0 && ((0, _gabrielvfonseca_ai_internal_openai.isOpenAIGpt54MiniModel)(model) || (0, _gabrielvfonseca_ai_internal_openai.isOpenAIGpt55Model)(model) && isKnownOpenAICompletionsEndpoint(model));
	const disableChatCompletionsToolReasoning = Array.isArray(params.tools) && params.tools.length > 0 && (0, _gabrielvfonseca_ai_internal_openai.isOpenAIGpt56Model)(model) && isKnownOpenAICompletionsEndpoint(model);
...
function parseTransportChunkUsage(rawUsage, model) {
	...
	(0, _gabrielvfonseca_ai_internal_runtime.calculateCost)(model, usage);
	(0, _gabrielvfonseca_ai_internal_runtime.applyProviderReportedUsageCost)(usage, rawUsage.cost);
	return usage;
}
...
if (process.env.VITEST || false) globalThis.operatorOpenAICompletionsTransportTestApi = completionsTesting;
//#endregion
...
function normalizeLowercaseString(value) {
	const stringValue = (0, _gabrielvfonseca_normalization_core_string_coerce.readStringValue)(value)?.trim().toLowerCase();
	return stringValue ? stringValue : void 0;
}
function normalizeComparableBaseUrl(value) {
	const trimmed = (0, _gabrielvfonseca_normalization_core_string_coerce.readStringValue)(value)?.trim();
	if (!trimmed) return;
...
function resolveBundledOpenAIResponsesEndpointClass(baseUrl) {
	const trimmed = (0, _gabrielvfonseca_normalization_core_string_coerce.readStringValue)(baseUrl)?.trim();
...
function resolveOpenAIResponsesPayloadCapabilities(model) {
	...
	return {
		allowsOpenAIServiceTier: provider === "openai" && (api === "openai-responses" || api === "operator-openai-responses-transport") && endpointClass === "openai-public" || isOpenAIProvider && (api === "openai-chatgpt-responses" || api === "openai-responses" || api === "operator-openai-responses-transport") && endpointClass === "openai",
...
function parsePositiveInteger(value) {
	...
	if (typeof value === "string") return require_parse_finite_number.parseStrictPositiveInteger(value);
}
...
function resolveOpenAIResponsesPayloadPolicy(model, options = {}) {
	...
	const shouldStripDisabledReasoningPayload = isResponsesApi && (!capabilities.usesKnownNativeOpenAIRoute || !(0, _gabrielvfonseca_ai_internal_openai.supportsOpenAIReasoningEffort)(model, "none"));
...
function sanitizeValue(value) {
	...
	if (value.type === "input_image" && typeof value.image_url === "string") {
		const imageUrl = (0, _gabrielvfonseca_media_core_inline_image_data_url.sanitizeInlineImageDataUrl)(value.image_url);
		return imageUrl ? {
			...value,
			image_url: imageUrl
		} : invalidSnakeImage();
...
function summarizeResponsesTools(tools) {
	...
	const mode = require_session_transcript_repair.resolveModelPayloadDebugMode();
...
function stringifyRedactedPayload(value) {
	try {
		...
		const redacted = require_redact.redactSensitiveText(encoded, { mode: "tools" });
		return redacted.length > 8e3 ? `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(redacted, 8e3)}…<truncated>` : redacted;
	} catch {
...
function stringifyRedactedEvent(value) {
	...
	return redacted.length > 2e3 ? `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(redacted, 2e3)}…<truncated>` : redacted;
}
...
function collectResponseFailedIdentifierHashes(value, opts = {}) {
	...
	if (Array.isArray(value)) {
		for (const [index, item] of value.entries()) {
			...
			if (identifierKey && isResponseFailedIdentifierKey(identifierKey) && itemString) {
				out.push(`${path}[${index}]=${require_redact_identifier.redactIdentifier(itemString, { len: 12 })}`);
				continue;
			}
	...
	for (const [key, child] of Object.entries(value)) {
		...
		if (isResponseFailedIdentifierKey(key) && childString) {
			out.push(`${childPath}=${require_redact_identifier.redactIdentifier(childString, { len: 12 })}`);
			continue;
		}
...
function redactResponseFailedDiagnosticValue(value, opts = {}) {
	const key = opts.key ?? "";
	const depth = opts.depth ?? 0;
	if (typeof value === "string" || typeof value === "number") return key && isResponseFailedIdentifierKey(key) ? require_redact_identifier.redactIdentifier(String(value), { len: 12 }) : value;
...
function buildResponsesFailedNoDetailsObservation(event, model, response = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(event.response) ? event.response : void 0) {
	const failureFields = redactResponseFailedDiagnosticValue(buildResponsesFailedFailureFields(response));
	const metadataKeys = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(response?.metadata) ? Object.keys(response.metadata).toSorted() : [];
...
function normalizeResponsesFailedEvent(event, model) {
	const response = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(event.response) ? event.response : void 0;
	const responseId = readResponseFailedString(response, "id") || void 0;
	const error = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(response?.error) ? response.error : void 0;
	if (error) {
	...
	const incompleteReason = readResponseFailedString((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(response?.incomplete_details) ? response.incomplete_details : void 0, "reason");
...
function summarizeResponsesPayload(params) {
	...
	if (require_session_transcript_repair.resolveModelPayloadDebugMode() === "full-redacted") parts.push(`payload=${stringifyRedactedPayload(record)}`);
	return parts.join(" ");
}
...
function shortHash(value) {
	return require_crypto_digest.sha256HexPrefix(value, 16);
}
...
function convertResponsesMessages(model, context, allowedToolCallProviders, options) {
	...
	if ((options?.includeSystemPrompt ?? true) && context.systemPrompt) messages.push(buildResponsesInputMessage(model.reasoning && options?.supportsDeveloperRole !== false ? "developer" : "system", [{
		type: "input_text",
		text: sanitizeTransportPayloadText((0, _gabrielvfonseca_ai_internal_shared.stripSystemPromptCacheBoundary)(context.systemPrompt))
	}]));
	let msgIndex = 0;
	for (const msg of transformedMessages) {
		if (msg.role === "user") if (typeof msg.content === "string") messages.push(buildResponsesInputMessage("user", [{
			...
			const textResult = (0, _gabrielvfonseca_ai_internal_shared.extractToolResultText)(msg.content);
			const sanitizedTextResult = sanitizeTransportPayloadText(textResult);
			const hasText = sanitizedTextResult.trim().length > 0;
			const mediaPlaceholder = (0, _gabrielvfonseca_ai_internal_shared.describeToolResultMediaPlaceholder)(msg.content);
			const hasImages = msg.content.some(_gabrielvfonseca_ai_internal_shared.isImageWithMediaPayload);
			...
			messages.push({
				...
				}] : [], ...msg.content.filter(_gabrielvfonseca_ai_internal_shared.isImageWithMediaPayload).map((item) => ({
					type: "input_image",
					detail: "auto",
					image_url: `data:${item.mimeType};base64,${item.data}`
...
async function processResponsesStream(openaiStream, output, stream, model, options) {
	const resolveToolCallId = (item, fallbackId) => {
		...
		const generatedCallId = `call_${(0, node_crypto.randomUUID)().replaceAll("-", "").slice(0, 24)}`;
		return resolvedItemId ? `${generatedCallId}|${resolvedItemId}` : generatedCallId;
	};
	let currentItem = null;
	let currentBlock = null;
	const streamingToolCalls = (0, _gabrielvfonseca_ai_internal_openai.createResponsesToolCallTracker)();
	let lastTextBlock = null;
	let pendingMessageText = null;
	const streamStartedAt = Date.now();
	let eventCount = 0;
	const eventTypes = /* @__PURE__ */ new Map();
	const sseDebugMode = require_session_transcript_repair.resolveModelSseDebugMode();
	const blockIndex = () => output.content.length - 1;
	...
	const appendCompletedResponseTextItem = (item) => {
		...
		const collapse = (0, _gabrielvfonseca_ai_internal_openai.resolveResponsesMessageSnapshotCollapse)({
			prior: lastTextBlock && {
	...
	const appendCompletedResponseToolCallItem = (item) => {
		const args = (0, _gabrielvfonseca_ai_internal_runtime.parseStreamingJson)(stringifyJsonLike(item.arguments, "{}"));
	...
	const backfillCompletedResponseOutput = (response) => {
		...
		for (const rawItem of response.output) {
			if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(rawItem)) continue;
	...
	const guardedStream = (0, _gabrielvfonseca_ai_internal_runtime.withFirstStreamEventTimeout)(openaiStream, {
		provider: model.provider,
		api: model.api,
		model: model.id,
		timeoutMs: options?.firstEventTimeoutMs ?? 0,
	...
	for await (const rawEvent of guardedStream) {
		...
		if (eventCount === 1) require_session_transcript_repair.emitModelTransportDebug(log, `[responses] first_event provider=${model.provider} api=${model.api} model=${model.id} elapsedMs=${Date.now() - streamStartedAt} type=${type}`);
		if (sseDebugMode === "peek" && eventCount <= 5) require_session_transcript_repair.emitModelTransportDebug(log, `[responses] event_peek provider=${model.provider} api=${model.api} model=${model.id} index=${eventCount} type=${type} event=${stringifyRedactedEvent(event)}`);
		if (type === "response.created") output.responseId = stringifyUnknown(event.response?.id);
			...
			if (item.type === "reasoning") {
				...
				const toolCallState = {
					...
					...(0, _gabrielvfonseca_ai_internal_openai.readResponsesToolCallItemIdentity)(item)
				};
		...
		} else if ((0, _gabrielvfonseca_ai_internal_openai.isResponsesTextDeltaEventType)(type) || type === "response.refusal.delta") {
			...
			if (toolCall) {
				...
				toolCall.block.arguments = (0, _gabrielvfonseca_ai_internal_runtime.parseStreamingJson)(stringifyJsonLike(toolCall.block.partialJson));
				stream.push({
					type: "toolcall_delta",
			...
			if (toolCall) {
				...
				if (doneArguments !== void 0 && (doneArguments.length > 0 || previousPartialJson === "")) {
					...
					toolCall.block.arguments = (0, _gabrielvfonseca_ai_internal_runtime.parseStreamingJson)(doneArguments);
			...
			if (item.type === "reasoning" && currentBlock?.type === "thinking") {
				...
				const finalText = (Array.isArray(item.content) ? item.content : []).map((part) => {
					...
					return (0, _gabrielvfonseca_ai_internal_openai.isResponsesTextContentPartType)(contentPart.type) ? contentPart.text ?? "" : contentPart.refusal ?? "";
				}).join("");
				const phase = item.phase ?? void 0;
				const collapse = pendingMessageText !== null ? (0, _gabrielvfonseca_ai_internal_openai.resolveResponsesMessageSnapshotCollapse)({
					prior: lastTextBlock && {
						text: stringifyUnknown(lastTextBlock.block.text),
						phase: lastTextBlock.phase
					},
				...
				const streamingToolCall = streamingToolCalls.resolve(event, (0, _gabrielvfonseca_ai_internal_openai.readResponsesToolCallItemIdentity)(item));
				...
				const args = (0, _gabrielvfonseca_ai_internal_runtime.parseStreamingJson)(finalPartialJson);
				let toolCallBlock;
				let contentIndex;
			...
			(0, _gabrielvfonseca_ai_internal_runtime.calculateCost)(model, output.usage);
	...
	require_session_transcript_repair.emitModelTransportDebug(log, `[responses] stream_done provider=${model.provider} api=${model.api} model=${model.id} elapsedMs=${Date.now() - streamStartedAt} events=${eventCount} types=${eventTypeSummary} stopReason=${output.stopReason ?? "unset"} contentBlocks=${output.content.length}`);
}
...
function readResponsesOutputMessageText(item) {
	return (Array.isArray(item.content) ? item.content : []).map((part) => {
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(part)) return "";
...
function resolveProviderTransportTurnState(model, params) {
	...
	return require_provider_runtime.resolveProviderTransportTurnStateWithPlugin({
		provider: model.provider,
		modelId: model.id,
...
function createOpenAIResponsesClient(model, context, apiKey, optionHeaders, turnHeaders, sessionId) {
	return new openai.default({
		...
		fetch: require_session_transcript_repair.buildGuardedModelFetch(model),
		...buildOpenAISdkClientOptions(model)
	});
}
	...
	return (model, context, options) => {
		...
		(async () => {
			...
			try {
				...
				require_session_transcript_repair.emitModelTransportDebug(log, `[responses] start provider=${model.provider} api=${model.api} model=${model.id} baseUrl=${require_session_transcript_repair.formatModelTransportDebugBaseUrl(model.baseUrl)} timeoutMs=${safeDebugValue(requestOptions?.timeout)} apiKey=${apiKey ? "present" : "missing"} ${summarizeResponsesPayload(params)}`);
				...
				require_session_transcript_repair.emitModelTransportDebug(log, `[responses] headers provider=${model.provider} api=${model.api} model=${model.id} elapsedMs=${Date.now() - requestStartedAt}`);
				stream.push({
					type: "start",
					partial: output
				});
...
function resolveOpenAIReasoningEffort(options) {
	return (0, _gabrielvfonseca_ai_internal_openai.normalizeOpenAIReasoningEffort)(options?.reasoningEffort ?? options?.reasoning ?? "high");
}
function hasResponsesWebSearchTool(tools) {
	if (!Array.isArray(tools)) return false;
	return tools.some((tool) => {
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(tool)) return false;
		if (tool.type === "web_search") return true;
		if (tool.type === "function" && tool.name === "web_search") return true;
		const fn = tool.function;
		return (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(fn) && fn.name === "web_search";
	});
}
function raiseMinimalReasoningForResponsesWebSearch(params) {
	...
	for (const effort of [
		...
		const resolved = (0, _gabrielvfonseca_ai_internal_openai.resolveOpenAIReasoningEffortForModel)({
			model: params.model,
			effort
		});
...
function buildOpenAICodexResponsesInstructions(context) {
	...
	return sanitizeTransportPayloadText((0, _gabrielvfonseca_ai_internal_shared.stripSystemPromptCacheBoundary)(context.systemPrompt));
}
...
function buildOpenAIResponsesParams(model, context, options, metadata) {
	...
	if (context.tools) {
		const converted = convertResponsesTools(context.tools, model, { strict: require_session_transcript_repair.resolveOpenAIStrictToolSetting(model, { transport: "stream" }) });
		if (converted.tools.length > 0 || converted.projection.inputToolCount === 0 && converted.projection.diagnostics.length === 0) params.tools = converted.tools;
		if (options?.toolChoice) {
			const toolChoice = (0, _gabrielvfonseca_ai_internal_openai.reconcileOpenAIResponsesToolChoice)(options.toolChoice, converted.projection);
			if (toolChoice !== void 0) params.tool_choice = toolChoice;
		}
	}
	if (model.reasoning) {
		if (options?.reasoningEffort || options?.reasoning || options?.reasoningSummary) {
			const resolvedReasoningEffort = (0, _gabrielvfonseca_ai_internal_openai.resolveOpenAIReasoningEffortForModel)({
				model,
			...
			const reasoningEffort = (0, _gabrielvfonseca_ai_internal_openai.resolveOpenAIReasoningEffortForModel)({
				model,
				effort: "none"
			});
	...
	return (model, context, options) => {
		...
		(async () => {
			...
			try {
				...
				await processResponsesStream(responseStream, output, stream, model, {
					firstEventTimeoutMs: (0, _gabrielvfonseca_ai_internal_runtime.getFirstStreamEventTimeoutMs)(options) ?? AZURE_RESPONSES_FIRST_EVENT_TIMEOUT_MS,
					abortFirstEventStream: firstEventAbort.abort,
					onFirstEventTimeout: (0, _gabrielvfonseca_ai_internal_runtime.getFirstStreamEventTimeoutHandler)(options),
					signal: options?.signal,
					authProfileId: responsesOptions?.authProfileId,
...
function resolveAzureDeploymentName(model) {
	return (0, _gabrielvfonseca_ai_internal_openai.resolveAzureDeploymentNameFromMap)({
		modelId: model.id,
...
function createAzureOpenAIClient(model, context, apiKey, optionHeaders, turnHeaders) {
	...
	if ((0, _gabrielvfonseca_ai_internal_openai.isOpenAICompatibleAzureResponsesBaseUrl)(baseURL)) return new openai.default(clientOptions);
...
const responsesTesting = {
	...
	formatModelTransportDebugBaseUrl: require_session_transcript_repair.formatModelTransportDebugBaseUrl,
...
if (process.env.VITEST || false) globalThis.operatorOpenAIResponsesTransportTestApi = responsesTesting;
//#endregion
Object.defineProperty(exports, "applyAnthropicEphemeralCacheControlMarkers", {
	enumerable: true,
	get: function() {
		return applyAnthropicEphemeralCacheControlMarkers;
	}
});
Object.defineProperty(exports, "applyAnthropicPayloadPolicyToParams", {
	enumerable: true,
	get: function() {
		return applyAnthropicPayloadPolicyToParams;
	}
});
Object.defineProperty(exports, "applyOpenAIResponsesPayloadPolicy", {
	enumerable: true,
	get: function() {
		return applyOpenAIResponsesPayloadPolicy;
	}
});
Object.defineProperty(exports, "canonicalizeMaxTokensParam", {
	enumerable: true,
	get: function() {
		return canonicalizeMaxTokensParam;
	}
});
Object.defineProperty(exports, "coerceTransportToolCallArguments", {
	enumerable: true,
	get: function() {
		return coerceTransportToolCallArguments;
	}
});
Object.defineProperty(exports, "createAzureOpenAIResponsesTransportStreamFn", {
	enumerable: true,
	get: function() {
		return createAzureOpenAIResponsesTransportStreamFn;
	}
});
Object.defineProperty(exports, "createDeepSeekV4OpenAICompatibleThinkingWrapper", {
	enumerable: true,
	get: function() {
		return createDeepSeekV4OpenAICompatibleThinkingWrapper;
	}
});
Object.defineProperty(exports, "createEmptyTransportUsage", {
	enumerable: true,
	get: function() {
		return createEmptyTransportUsage;
	}
});
Object.defineProperty(exports, "createGoogleThinkingPayloadWrapper", {
	enumerable: true,
	get: function() {
		return createGoogleThinkingPayloadWrapper;
	}
});
Object.defineProperty(exports, "createOpenAICompletionsTransportStreamFn", {
	enumerable: true,
	get: function() {
		return createOpenAICompletionsTransportStreamFn;
	}
});
Object.defineProperty(exports, "createOpenAIResponsesTransportStreamFn", {
	enumerable: true,
	get: function() {
		return createOpenAIResponsesTransportStreamFn;
	}
});
Object.defineProperty(exports, "createThinkingOnlyFinalTextWrapper", {
	enumerable: true,
	get: function() {
		return createThinkingOnlyFinalTextWrapper;
	}
});
Object.defineProperty(exports, "createWritableTransportEventStream", {
	enumerable: true,
	get: function() {
		return createWritableTransportEventStream;
	}
});
Object.defineProperty(exports, "encodeAssistantTextSignatureV1", {
	enumerable: true,
	get: function() {
		return encodeAssistantTextSignatureV1;
	}
});
Object.defineProperty(exports, "failTransportStream", {
	enumerable: true,
	get: function() {
		return failTransportStream;
	}
});
Object.defineProperty(exports, "finalizeTransportStream", {
	enumerable: true,
	get: function() {
		return finalizeTransportStream;
	}
});
Object.defineProperty(exports, "flattenCompletionMessagesToStringContent", {
	enumerable: true,
	get: function() {
		return flattenCompletionMessagesToStringContent;
	}
});
Object.defineProperty(exports, "hasOnlyAssistantReasoningContent", {
	enumerable: true,
	get: function() {
		return hasOnlyAssistantReasoningContent;
	}
});
Object.defineProperty(exports, "isReasoningOnlyLengthAssistantTurn", {
	enumerable: true,
	get: function() {
		return isReasoningOnlyLengthAssistantTurn;
	}
});
Object.defineProperty(exports, "mergeTransportHeaders", {
	enumerable: true,
	get: function() {
		return mergeTransportHeaders;
	}
});
Object.defineProperty(exports, "resolveAnthropicEphemeralCacheControl", {
	enumerable: true,
	get: function() {
		return resolveAnthropicEphemeralCacheControl;
	}
});
Object.defineProperty(exports, "resolveAnthropicPayloadPolicy", {
	enumerable: true,
	get: function() {
		return resolveAnthropicPayloadPolicy;
	}
});
Object.defineProperty(exports, "resolveMaxTokensParam", {
	enumerable: true,
	get: function() {
		return resolveMaxTokensParam;
	}
});
Object.defineProperty(exports, "resolveOpenAIResponsesPayloadPolicy", {
	enumerable: true,
	get: function() {
		return resolveOpenAIResponsesPayloadPolicy;
	}
});
Object.defineProperty(exports, "sanitizeGoogleThinkingPayload", {
	enumerable: true,
	get: function() {
		return sanitizeGoogleThinkingPayload;
	}
});
Object.defineProperty(exports, "sanitizeNonEmptyTransportPayloadText", {
	enumerable: true,
	get: function() {
		return sanitizeNonEmptyTransportPayloadText;
	}
});
Object.defineProperty(exports, "sanitizeTransportPayloadText", {
	enumerable: true,
	get: function() {
		return sanitizeTransportPayloadText;
	}
});
Object.defineProperty(exports, "streamWithPayloadPatch", {
	enumerable: true,
	get: function() {
		return streamWithPayloadPatch;
	}
});
Object.defineProperty(exports, "stripCompletionMessagesToRoleContent", {
	enumerable: true,
	get: function() {
		return stripCompletionMessagesToRoleContent;
	}
});
Object.defineProperty(exports, "supportsModelTools", {
	enumerable: true,
	get: function() {
		return supportsModelTools;
	}
});
Object.defineProperty(exports, "transformTransportMessages", {
	enumerable: true,
	get: function() {
		return transformTransportMessages;
	}
});
