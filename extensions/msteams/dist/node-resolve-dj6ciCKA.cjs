...
function normalizeNodeKey(value) {
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(value.normalize("NFC")).replace(/(?=\p{M})\p{Emoji_Component}/gu, "").replace(/(?<![\p{L}\p{M}\p{N}])\p{M}+/gu, "").replace(/[^\p{L}\p{M}\p{N}]+/gu, "-").replace(/^-+/, "").replace(/-+$/, "");
}
...
function formatNodeCandidateLabel(node) {
	...
	const clientId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(node.clientId);
	if (clientId) details.push(`client=${clientId}`);
	return `${label} [${details.join(", ")}]`;
}
function isCurrentOperatorClient(clientId) {
	return ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(clientId) ?? "").startsWith("operator-");
}
function isLegacyClawdbotClient(clientId) {
	const normalized = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(clientId) ?? "";
	return normalized.startsWith("clawdbot-") || normalized.startsWith("moldbot-");
}
function pickPreferredLegacyMigrationMatch(matches) {
	const current = matches.filter((match) => isCurrentOperatorClient(match.clientId));
...
function scoreNodeCandidate(node, matchScore) {
	...
	if (isCurrentOperatorClient(node.clientId)) score += 10;
	else if (isLegacyClawdbotClient(node.clientId)) score -= 10;
	return score;
}
function resolveScoredMatches(nodes, query, allowCompactDisplayName) {
	const trimmed = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(query);
	if (!trimmed) return [];
...
function resolveNodeIdFromNodeList(nodes, query, options = {}) {
	const q = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(query) ?? "";
	if (!q) {
...
//#endregion
Object.defineProperty(exports, "resolveNodeFromNodeList", {
	enumerable: true,
	get: function() {
		return resolveNodeFromNodeList;
	}
});
Object.defineProperty(exports, "resolveNodeIdFromNodeList", {
	enumerable: true,
	get: function() {
