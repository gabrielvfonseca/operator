...
function hasAllowFromEntries(list) {
	return Array.isArray(list) && (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(list).length > 0;
}
//#endregion
//#region src/commands/doctor/shared/empty-allowlist-policy.ts
function usesSenderBasedGroupAllowlist(channelName) {
	return require_channel_capabilities.getDoctorChannelCapabilities(channelName).warnOnEmptyGroupSenderAllowlist;
}
function allowsGroupAllowFromFallback(channelName) {
	return require_channel_capabilities.getDoctorChannelCapabilities(channelName).groupAllowFromFallbackToAllowFrom;
}
...
function collectEmptyAllowlistPolicyWarningsForAccount(params) {
	...
	if (params.channelName && (params.shouldSkipDefaultEmptyGroupAllowlistWarning ?? require_channel_doctor.shouldSkipChannelDoctorDefaultEmptyGroupAllowlistWarning)({
...
function scanEmptyAllowlistPolicyWarnings(cfg, params) {
	...
	const checkAccount = (account, prefix, channelName, parent, options = {}) => {
		const accountDm = require_object.asObjectRecord(account.dm);
		const parentDm = require_object.asObjectRecord(parent?.dm);
	...
	for (const [channelName, channelConfig] of Object.entries(channels)) {
		...
		const accountIds = require_channel_capabilities.resolveDoctorChannelAccountIds(channelName, cfg, Object.keys(accounts ?? {}));
		...
		const suppressParentGroupAllowlistWarning = activeAccounts.length > 0 && !hasImplicitActiveAccount && channelConfig.groupPolicy === "allowlist" && activeAccounts.every((account) => {
			...
			if (!require_channel_capabilities.getDoctorChannelCapabilities(channelName).groupAllowFromFallbackToAllowFrom) return false;
			const accountDm = require_object.asObjectRecord(account.dm);
			const parentDm = require_object.asObjectRecord(channelConfig.dm);
...
//#endregion
exports.hasAllowFromEntries = hasAllowFromEntries;
exports.scanEmptyAllowlistPolicyWarnings = scanEmptyAllowlistPolicyWarnings;
