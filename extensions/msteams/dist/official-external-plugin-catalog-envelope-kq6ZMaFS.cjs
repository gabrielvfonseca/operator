...
const OFFICIAL_EXTERNAL_PLUGIN_CATALOG_FEED_PAYLOAD_TYPE = "operator.official-external-plugin-catalog-feed.v1";
...
function verifyOfficialExternalPluginCatalogSignedEnvelope(raw, params) {
	...
	for (const envelopeSignature of envelope.signatures) {
		...
		const normalizedPublicKey = require_ed25519_signature.normalizeEd25519PublicKeyBase64Url(trustedKey.publicKey);
		if (!normalizedPublicKey || trustedSignaturePublicKeys.has(normalizedPublicKey)) continue;
		if (require_ed25519_signature.verifyEd25519SignatureBytes({
...
function parseOfficialExternalPluginCatalogSignedEnvelope(raw) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw) || raw.schemaVersion !== 1) return null;
	...
	const parsedSignatures = signatures.filter((signature) => (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(signature) && typeof signature.keyId === "string" && signature.keyId.trim().length > 0 && signature.algorithm === "ed25519" && typeof signature.signature === "string" && signature.signature.trim().length > 0);
...
function decodeOfficialExternalPluginCatalogEnvelopePayload(payloadBytes) {
	try {
		...
		return require_official_external_plugin_catalog.isOfficialExternalPluginCatalogFeed(raw) ? raw : null;
	} catch {
		return null;
	}
}
//#endregion
exports.verifyOfficialExternalPluginCatalogSignedEnvelope = verifyOfficialExternalPluginCatalogSignedEnvelope;
