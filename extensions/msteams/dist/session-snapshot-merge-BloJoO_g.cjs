...
function anySessionFieldChanged(before, after, fields) {
	return fields.some((field) => !(0, node_util.isDeepStrictEqual)(before[field], after[field]));
}
...
function projectSessionSnapshotChanges(params) {
	...
	if (modelOverrideChanged && !modelOverrideChangedConcurrently) {
		...
		for (const field of MODEL_OVERRIDE_DEPENDENT_FIELDS) {
			...
			if (!(0, node_util.isDeepStrictEqual)(initial[field], next[field])) patchRecord[field] = Object.hasOwn(params.next, field) ? next[field] : void 0;
		}
		if (params.reassertLiveModelSwitchPending) patch.liveModelSwitchPending = true;
	}
	const runtimeModelChanged = !(0, node_util.isDeepStrictEqual)(initial.model, next.model) || !(0, node_util.isDeepStrictEqual)(initial.modelProvider, next.modelProvider);
	const runtimeModelChangedConcurrently = !(0, node_util.isDeepStrictEqual)(current.model, initial.model) || !(0, node_util.isDeepStrictEqual)(current.modelProvider, initial.modelProvider);
	...
	for (const field of fields) {
		...
		if ((0, node_util.isDeepStrictEqual)(initial[field], next[field])) continue;
		if (field === "updatedAt") {
		...
		if (!(0, node_util.isDeepStrictEqual)(current[field], initial[field])) continue;
...
function sessionSnapshotChangesApplied(params) {
	...
	for (const field of fields) if ((params.touchedFields?.includes(field) === true || !(0, node_util.isDeepStrictEqual)(initial[field], next[field])) && !(0, node_util.isDeepStrictEqual)(current[field], next[field])) return false;
	return true;
}
...
function sessionSnapshotTouchedFieldsConflict(params) {
	...
	if (SESSION_MODEL_OVERRIDE_TRANSACTION_FIELDS.some((field) => fields.has(field))) {
		for (const field of MODEL_OVERRIDE_CONFLICT_DEPENDENT_FIELDS) if (!(0, node_util.isDeepStrictEqual)(initial[field], next[field])) fields.add(field);
	}
	return [...fields].some((field) => !(0, node_util.isDeepStrictEqual)(current[field], initial[field]) && !(0, node_util.isDeepStrictEqual)(current[field], next[field]));
}
...
function sessionModelOverrideChangesApplied(params) {
	...
	const current = params.current;
	const initial = params.initial;
	const changedDependentFields = [...MODEL_OVERRIDE_DEPENDENT_FIELDS].filter((field) => !(0, node_util.isDeepStrictEqual)(initial[field], next[field]));
	const modelRouteOverrideChanged = anySessionFieldChanged(initial, next, MODEL_ROUTE_OVERRIDE_FIELDS);
	if ([
		...SESSION_MODEL_OVERRIDE_TRANSACTION_FIELDS,
		...modelRouteOverrideChanged ? MODEL_OVERRIDE_RUNTIME_FIELDS : [],
		...changedDependentFields
	].some((field) => !(0, node_util.isDeepStrictEqual)(current[field], next[field]))) return false;
...
//#endregion
Object.defineProperty(exports, "SESSION_MODEL_OVERRIDE_TRANSACTION_FIELDS", {
	enumerable: true,
	get: function() {
		return SESSION_MODEL_OVERRIDE_TRANSACTION_FIELDS;
	}
});
Object.defineProperty(exports, "adoptPersistedSessionSnapshot", {
	enumerable: true,
	get: function() {
		return adoptPersistedSessionSnapshot;
	}
});
Object.defineProperty(exports, "mergeSessionSnapshotChanges", {
	enumerable: true,
	get: function() {
		return mergeSessionSnapshotChanges;
	}
});
Object.defineProperty(exports, "projectSessionSnapshotChanges", {
	enumerable: true,
	get: function() {
		return projectSessionSnapshotChanges;
	}
});
Object.defineProperty(exports, "sessionModelOverrideChangesApplied", {
	enumerable: true,
	get: function() {
		return sessionModelOverrideChangesApplied;
	}
});
Object.defineProperty(exports, "sessionSnapshotChangesApplied", {
	enumerable: true,
	get: function() {
		return sessionSnapshotChangesApplied;
	}
});
Object.defineProperty(exports, "sessionSnapshotTouchedFieldsConflict", {
	enumerable: true,
	get: function() {
		return sessionSnapshotTouchedFieldsConflict;
	}
});
