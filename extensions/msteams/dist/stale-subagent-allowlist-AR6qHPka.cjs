...
function collectConfiguredSubagentTargetIds(cfg) {
	...
	const defaultAcpAgent = require_session_key.normalizeOptionalAgentId(cfg.acp?.defaultAgent);
	if (defaultAcpAgent) ids.add(defaultAcpAgent);
	for (const entry of cfg.acp?.allowedAgents ?? []) {
		if (entry.trim() === "*") continue;
		const acpAgent = require_session_key.normalizeOptionalAgentId(entry);
		if (acpAgent) ids.add(acpAgent);
	}
	return ids;
}
function collectStaleAllowlistEntries(params) {
	...
	for (const entry of params.allowAgents) {
		...
		const normalizedAgentId = (0, _gabrielvfonseca_normalization_core_agent_id.normalizeAgentId)(trimmed);
...
function filterAllowAgents(params) {
	return params.allowAgents.filter((entry) => {
		const trimmed = entry.trim();
		return !trimmed || trimmed === "*" || !params.staleTargetIds.has((0, _gabrielvfonseca_normalization_core_agent_id.normalizeAgentId)(trimmed));
	});
}
...
//#endregion
exports.collectStaleSubagentAllowlistWarnings = collectStaleSubagentAllowlistWarnings;
exports.maybeRepairStaleSubagentAllowlists = maybeRepairStaleSubagentAllowlists;
exports.scanStaleSubagentAllowlistReferences = scanStaleSubagentAllowlistReferences;
