...
function parseApprovalPresentation(raw) {
	try {
		...
		return require_src.validateApprovalPresentation(value) ? value : null;
	} catch {
		return null;
	}
}
...
function requireApprovalId(value) {
	if (!require_approval_id.isWellFormedApprovalId(value)) throw new Error("operator approval id must be non-empty, well-formed Unicode, and not . or ..");
	return value;
}
...
function stringifyPresentation(presentation) {
	if (!require_src.validateApprovalPresentation(presentation)) throw new Error("operator approval presentation must match the safe protocol schema");
	let raw;
	try {
...
function decodeOperatorApprovalRow(row) {
	...
	if (!presentation || !require_approval_id.isWellFormedApprovalId(row.approval_id) || !require_state_migrations_cron_run_logs.isApprovalResolutionRef(row.resolution_ref) || !reviewerDeviceIds || !audienceSessionKeys || audienceSessionKeys.length > 64 || !OPERATOR_APPROVAL_KINDS.has(kind) || !OPERATOR_APPROVAL_STATUSES.has(status) || !isValidTimestamp(row.created_at_ms) || !isValidTimestamp(row.expires_at_ms) || !isValidTimestamp(row.updated_at_ms) || row.expires_at_ms < row.created_at_ms || row.updated_at_ms < row.created_at_ms || row.resolved_at_ms !== null && (!isValidTimestamp(row.resolved_at_ms) || row.resolved_at_ms < row.created_at_ms || row.resolved_at_ms > row.updated_at_ms) || row.consumed_at_ms !== null && (!isValidTimestamp(row.consumed_at_ms) || row.resolved_at_ms === null || row.consumed_at_ms < row.resolved_at_ms || row.consumed_at_ms > row.updated_at_ms) || row.requested_by_device_token_auth !== 0 && row.requested_by_device_token_auth !== 1 || decision !== null && !OPERATOR_APPROVAL_DECISIONS.has(decision) || terminalReason !== null && !OPERATOR_APPROVAL_TERMINAL_REASONS.has(terminalReason) || resolverKind !== null && !OPERATOR_APPROVAL_RESOLVER_KINDS.has(resolverKind)) return null;
	if (presentation.kind !== kind || row.resolution_ref !== require_state_migrations_cron_run_logs.buildApprovalResolutionRef({
...
function selectOperatorApprovalRow(database, id) {
	...
	return require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(database.db, stateDb.selectFrom("operator_approvals").selectAll().where("approval_id", "=", id));
}
function selectOperatorApprovalRowByLocator(database, locator) {
	const stateDb = require_state_migrations_cron_run_logs.getNodeSqliteKysely(database.db);
	const rows = require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, stateDb.selectFrom("operator_approvals").selectAll().where((eb) => eb.or([eb("approval_id", "=", locator), eb("resolution_ref", "=", locator)])).limit(2)).rows;
	return rows.length === 1 ? rows[0] : void 0;
}
function hasApprovalLocatorNamespaceConflict(params) {
	const stateDb = require_state_migrations_cron_run_logs.getNodeSqliteKysely(params.database.db);
	return require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(params.database.db, stateDb.selectFrom("operator_approvals").select("approval_id").where((eb) => eb.or([eb("approval_id", "=", params.resolutionRef), eb("resolution_ref", "=", params.id)])).where("approval_id", "!=", params.id)) !== void 0;
}
...
function denyCorruptPendingRow(params) {
	...
	const stateDb = require_state_migrations_cron_run_logs.getNodeSqliteKysely(params.database.db);
	require_state_migrations_cron_run_logs.executeSqliteQuerySync(params.database.db, stateDb.updateTable("operator_approvals").set({
		status: "denied",
		decision: "deny",
...
function insertOperatorApproval(params) {
	...
	return require_openclaw_state_db.runOperatorStateWriteTransaction((database) => {
		const stateDb = require_state_migrations_cron_run_logs.getNodeSqliteKysely(database.db);
		require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, stateDb.deleteFrom("operator_approvals").where("status", "!=", "pending").where("resolved_at_ms", "is not", null).where("resolved_at_ms", "<=", input.createdAtMs - OPERATOR_APPROVAL_TERMINAL_RETENTION_MS));
		...
		const source = input.source ?? {};
		const result = require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, stateDb.insertInto("operator_approvals").values({
			approval_id: id,
...
function listPendingOperatorApprovals(params = {}) {
	...
	return require_openclaw_state_db.runOperatorStateWriteTransaction((database) => {
		...
		while (records.length < resultLimit) {
			...
			const rows = require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, query).rows;
...
function resolveOperatorApproval(params) {
	...
	return require_openclaw_state_db.runOperatorStateWriteTransaction((database) => {
		...
		let resolveQuery = require_state_migrations_cron_run_logs.getNodeSqliteKysely(database.db).updateTable("operator_approvals").set({
		...
		const result = require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, resolveQuery);
...
function forceDenyOperatorApproval(params) {
	...
	return require_openclaw_state_db.runOperatorStateWriteTransaction((database) => {
		...
		const auditTimestampMs = clampAuditTimestamp(nowMs, record.createdAtMs);
		let denyQuery = require_state_migrations_cron_run_logs.getNodeSqliteKysely(database.db).updateTable("operator_approvals").set({
		...
		require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, denyQuery);
...
function expireDueOperatorApprovals(params) {
	return require_openclaw_state_db.runOperatorStateWriteTransaction((database) => {
		const nowMs = params.nowMs ?? Date.now();
		const stateDb = require_state_migrations_cron_run_logs.getNodeSqliteKysely(database.db);
		const dueRows = require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, stateDb.selectFrom("operator_approvals").selectAll().where("status", "=", "pending").where("expires_at_ms", "<=", nowMs).orderBy("expires_at_ms", "asc").orderBy("approval_id", "asc")).rows;
		if (dueRows.length === 0) return {
			affected: 0,
			records: []
		};
		const result = require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, stateDb.updateTable("operator_approvals").set({
			status: "expired",
			decision: "deny",
			terminal_reason: "timeout",
...
function closeOrphanedOperatorApprovals(params) {
	...
	return require_openclaw_state_db.runOperatorStateWriteTransaction((database) => {
		const nowMs = params.nowMs ?? Date.now();
		const stateDb = require_state_migrations_cron_run_logs.getNodeSqliteKysely(database.db);
		const orphanRows = require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, stateDb.selectFrom("operator_approvals").selectAll().where("status", "=", "pending").where("runtime_epoch", "!=", runtimeEpoch).orderBy("created_at_ms", "asc").orderBy("approval_id", "asc")).rows;
...
function consumeOperatorApprovalAllowOnce(params) {
	...
	return require_openclaw_state_db.runOperatorStateWriteTransaction((database) => {
		...
		let consumeQuery = require_state_migrations_cron_run_logs.getNodeSqliteKysely(database.db).updateTable("operator_approvals").set({
		...
		const result = require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, consumeQuery);
		row = selectOperatorApprovalRow(database, id);
		if (!row) return { outcome: "not-found" };
		record = requireDecodedRecord(row);
		if (result.numAffectedRows === 1n) return {
...
function pruneTerminalOperatorApprovals(params) {
	...
	return require_openclaw_state_db.runOperatorStateWriteTransaction((database) => {
		...
		const stateDb = require_state_migrations_cron_run_logs.getNodeSqliteKysely(database.db);
		const result = require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, stateDb.deleteFrom("operator_approvals").where("status", "!=", "pending").where("resolved_at_ms", "is not", null).where("resolved_at_ms", "<=", cutoffMs));
		return Number(result.numAffectedRows ?? 0n);
	}, params.databaseOptions);
}
//#endregion
Object.defineProperty(exports, "closeOrphanedOperatorApprovals", {
	enumerable: true,
	get: function() {
		return closeOrphanedOperatorApprovals;
	}
});
Object.defineProperty(exports, "consumeOperatorApprovalAllowOnce", {
	enumerable: true,
	get: function() {
		return consumeOperatorApprovalAllowOnce;
	}
});
Object.defineProperty(exports, "expireDueOperatorApprovals", {
	enumerable: true,
	get: function() {
		return expireDueOperatorApprovals;
	}
});
Object.defineProperty(exports, "forceDenyOperatorApproval", {
	enumerable: true,
	get: function() {
		return forceDenyOperatorApproval;
	}
});
Object.defineProperty(exports, "getOperatorApprovalDetailed", {
	enumerable: true,
	get: function() {
		return getOperatorApprovalDetailed;
	}
});
Object.defineProperty(exports, "getOperatorApprovalDetailedByLocator", {
	enumerable: true,
	get: function() {
		return getOperatorApprovalDetailedByLocator;
	}
});
Object.defineProperty(exports, "insertOperatorApproval", {
	enumerable: true,
	get: function() {
		return insertOperatorApproval;
	}
});
Object.defineProperty(exports, "listPendingOperatorApprovals", {
	enumerable: true,
	get: function() {
		return listPendingOperatorApprovals;
	}
});
Object.defineProperty(exports, "pruneTerminalOperatorApprovals", {
	enumerable: true,
	get: function() {
		return pruneTerminalOperatorApprovals;
	}
});
Object.defineProperty(exports, "resolveOperatorApproval", {
	enumerable: true,
	get: function() {
		return resolveOperatorApproval;
	}
});
