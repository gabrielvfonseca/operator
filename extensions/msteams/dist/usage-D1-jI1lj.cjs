...
require("./provider-usage-ChsIx4Rc.cjs");
...
async function runUsageAgentTasks(tasks) {
	const result = await require_resolve.runTasksWithConcurrency({
		tasks,
...
function resolveSessionUsageFileOrRespond(key, respond, config) {
	const { entry, storePath } = require_session_utils.loadSessionEntry(key);
	...
	let sessionFile;
	try {
		sessionFile = require_paths.resolveSessionFilePath(sessionId, entry, require_paths.resolveSessionFilePathOptions({
			storePath,
			agentId
		}));
	} catch {
		respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, `Invalid session key: ${key}`));
		return null;
	}
	return {
		config,
...
const datePartsToStartMs = (parts, interpretation) => {
	...
	if (interpretation.mode === "time-zone") return require_format_datetime.resolveTimeZoneDayStartMs(formatDateParts(year, monthIndex, day), interpretation.timeZone);
...
const resolveDateInterpretation = (params) => {
	...
	if (params.mode === "specific") {
		...
		if (params.timeZone !== void 0 && params.timeZone !== null) {
			const requestedTimeZone = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.timeZone);
			const timeZone = requestedTimeZone ? require_format_datetime.resolveTimezone(requestedTimeZone) : void 0;
			if (!timeZone) {
			...
			return {
				...
				value: {
					mode: "time-zone",
					timeZone,
					formatDayKey: require_format_datetime.createTimeZoneDayKeyFormatter(timeZone)
				}
			};
		}
		if (utcOffsetMinutes !== void 0) return {
...
function buildStoreBySessionId(store) {
	...
	for (const [sessionId, matches] of matchesBySessionId) {
		const preferredKey = require_session_id_resolution.resolvePreferredSessionKeyForSessionIdMatches(matches, sessionId);
...
function filterSessionStoreByAgent(params) {
	const scopedAgentId = (0, _gabrielvfonseca_normalization_core_agent_id.normalizeAgentId)(params.agentId);
	const scopedStore = {};
	for (const [key, entry] of Object.entries(params.store)) {
		...
		if (require_session_accessor.resolveSessionStoreAgentId(params.config, key) === scopedAgentId) scopedStore[key] = entry;
	}
	return scopedStore;
}
async function discoverAllSessionsForUsage(params) {
	...
	return (await runUsageAgentTasks((requestedAgentId ? [{ id: (0, _gabrielvfonseca_normalization_core_agent_id.normalizeAgentId)(requestedAgentId) }] : require_agent_list.listGatewayAgentsBasic(params.config).agents).map((agent) => async () => {
		const agentId = (0, _gabrielvfonseca_normalization_core_agent_id.normalizeAgentId)(agent.id);
		return (await require_session_cost_usage.discoverAllSessions({
			agentId,
...
function addUniqueSessionIds(target, ids) {
	...
	for (const id of ids) {
		const normalized = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(id);
...
function createEmptySessionCostSummary() {
	return { ...require_session_cost_usage.createEmptyCostUsageTotals() };
}
function mergeSessionUsageInto(target, source) {
	require_session_cost_usage.addCostUsageTotals(target, source);
...
function mergeModelUsage(left, right) {
	...
	for (const entry of [...left ?? [], ...right ?? []]) {
		...
		require_session_cost_usage.addCostUsageTotals(existing.totals, entry.totals);
...
async function loadCostUsageSummaryCached(params) {
	...
	}) : require_session_cost_usage.loadCostUsageSummaryFromCache({
		startMs: params.startMs,
		endMs: params.endMs,
		dayBucket: params.dayBucket,
		config: params.config,
...
async function loadAllAgentCostUsageSummary(params) {
	const summaries = await runUsageAgentTasks(require_agent_list.listGatewayAgentsBasic(params.config).agents.map((agent) => (0, _gabrielvfonseca_normalization_core_agent_id.normalizeAgentId)(agent.id)).map((agentId) => () => require_session_cost_usage.loadCostUsageSummaryFromCache({
		startMs: params.startMs,
		endMs: params.endMs,
		dayBucket: params.dayBucket,
		config: params.config,
	...
	const totals = require_session_cost_usage.createEmptyCostUsageTotals();
	let cacheStatus;
	let updatedAt = 0;
	let days = 0;
	for (const summary of summaries) {
		...
		require_session_cost_usage.addCostUsageTotals(totals, summary.totals);
		...
		for (const day of summary.daily) {
			const entry = dailyByDate.get(day.date) ?? {
				date: day.date,
				...require_session_cost_usage.createEmptyCostUsageTotals()
			};
			require_session_cost_usage.addCostUsageTotals(entry, day);
...
const usageHandlers = {
	"usage.status": async ({ respond }) => {
		respond(true, await require_provider_usage_load.loadProviderUsageSummary(), void 0);
	},
	"usage.cost": async ({ respond, params, context }) => {
		...
		if (!dateInterpretationResolution.ok) {
			respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, dateInterpretationResolution.error));
			return;
		}
		...
		if (!dateRange.ok) {
			respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, dateRange.error));
			return;
		}
	...
	"sessions.usage": async ({ respond, params, context }) => {
		if (!require_src.validateSessionsUsageParams(params)) {
			respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, `invalid sessions.usage params: ${require_validation_errors.formatValidationErrors(require_src.validateSessionsUsageParams.errors)}`));
			return;
		}
		const p = params;
		const dateInterpretationResolution = resolveDateInterpretation({
		...
		const specificKey = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(p.key) ?? null;
		const requestedAgentId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(p.agentId);
		const requestedAllAgents = p.agentScope === "all";
		if (requestedAllAgents && (requestedAgentId || specificKey)) {
			respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, "agentScope=all cannot be combined with key or agentId"));
			return;
		}
		const specificKeyAgentId = specificKey ? require_session_key.parseAgentSessionKey(specificKey)?.agentId : void 0;
		if (requestedAgentId && specificKeyAgentId && (0, _gabrielvfonseca_normalization_core_agent_id.normalizeAgentId)(requestedAgentId) !== specificKeyAgentId) {
		...
		const effectiveAgentId = requestedAllAgents ? void 0 : (0, _gabrielvfonseca_normalization_core_agent_id.normalizeAgentId)(requestedAgentId ?? specificKeyAgentId ?? require_agent_scope_config.resolveDefaultAgentId(config));
		const groupingMode = p.groupBy === "family" || p.includeHistorical === true ? "family" : "instance";
		const { storePath, store } = require_sessions.loadCombinedSessionStoreForGateway(config, effectiveAgentId ? { agentId: effectiveAgentId } : {});
		...
		const mergedEntries = [];
		if (specificKey) {
			const scopedSpecificKey = require_session_accessor.resolveStoredSessionKeyForAgentStore({
				cfg: config,
				agentId: effectiveAgentId ?? require_agent_scope_config.resolveDefaultAgentId(config),
				sessionKey: specificKey
			});
			const scopedParsed = require_session_key.parseAgentSessionKey(scopedSpecificKey);
			const agentIdFromKey = scopedParsed?.agentId ?? effectiveAgentId ?? require_agent_scope_config.resolveDefaultAgentId(config);
			...
			try {
				sessionFile = require_session_cost_usage.resolveExistingUsageSessionFile({
					...
					sessionFile: require_paths.resolveSessionFilePath(sessionId, storeEntry, require_paths.resolveSessionFilePathOptions({
				...
				respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, `Invalid session reference: ${specificKey}`));
				return;
			}
			if (sessionFile) try {
				const stats = node_fs.default.statSync(sessionFile);
		...
		const sessions = [];
		const aggregateTotals = require_session_cost_usage.createEmptyCostUsageTotals();
		...
		for (const [entryIndex, merged] of mergedEntries.entries()) for (const includedSessionId of merged.includedSessionIds ?? [merged.sessionId]) {
			const includedSessionFile = includedSessionId === merged.sessionId ? merged.sessionFile : require_session_cost_usage.resolveExistingUsageSessionFile({
		...
		const agentLoads = await runUsageAgentTasks(Array.from(sessionsByAgent.entries()).map(([agentId, agentSessions]) => async () => ({
			agentSessions,
			loaded: await require_session_cost_usage.loadSessionCostSummariesFromCache({
		...
		for (const { agentSessions, loaded } of agentLoads) {
			...
			for (const [index, summary] of loaded.summaries.entries()) {
				if (!summary) continue;
				const session = (0, _gabrielvfonseca_normalization_core.expectDefined)(agentSessions[index], "agent sessions entry at index");
				const merged = (0, _gabrielvfonseca_normalization_core.expectDefined)(mergedEntries[session.entryIndex], "merged entries entry at session.entry index");
		...
		for (const [entryIndex, merged] of mergedEntries.entries()) {
			...
			if (usage) {
				require_session_cost_usage.addCostUsageTotals(aggregateTotals, usage);
			...
			if (usage) {
				...
				if (usage.modelUsage) for (const entry of usage.modelUsage) {
					...
					const modelExisting = byModelMap.get(modelKey) ?? {
						...
						model: entry.model,
						count: 0,
						totals: require_session_cost_usage.createEmptyCostUsageTotals()
					};
					modelExisting.count += entry.count;
					require_session_cost_usage.addCostUsageTotals(modelExisting.totals, entry.totals);
					...
					require_session_cost_usage.addCostUsageTotals(providerExisting.totals, entry.totals);
				...
				if (agentId) {
					const agentTotals = byAgentMap.get(agentId) ?? require_session_cost_usage.createEmptyCostUsageTotals();
					require_session_cost_usage.addCostUsageTotals(agentTotals, usage);
					byAgentMap.set(agentId, agentTotals);
				}
				if (channel) {
					const channelTotals = byChannelMap.get(channel) ?? require_session_cost_usage.createEmptyCostUsageTotals();
					require_session_cost_usage.addCostUsageTotals(channelTotals, usage);
	...
	"sessions.usage.timeseries": async ({ respond, params, context }) => {
		const key = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params?.key) ?? null;
		if (!key) {
			respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, "key is required for timeseries"));
			return;
		}
		...
		const timeseries = await require_session_cost_usage.loadSessionUsageTimeSeries({
			sessionId,
			sessionEntry: entry,
			sessionFile,
			config,
		...
		if (!timeseries) {
			respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, `No transcript found for session: ${key}`));
			return;
		}
	...
	"sessions.usage.logs": async ({ respond, params, context }) => {
		const key = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params?.key) ?? null;
		if (!key) {
			respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, "key is required for logs"));
			return;
		}
		...
		if (!resolved) return;
		const { config, entry, agentId, sessionId, sessionFile } = resolved;
		respond(true, { logs: await require_session_cost_usage.loadSessionLogs({
			sessionId,
			sessionEntry: entry,
			sessionFile,
			config,
...
exports.usageHandlers = usageHandlers;
