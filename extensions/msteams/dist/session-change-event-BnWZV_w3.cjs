...
function terminalOutcomeFromSnapshot(snapshot) {
	...
	return require_agent_run_terminal_outcome.buildAgentRunTerminalOutcome(snapshot);
}
function shouldPreserveTerminalSnapshot(existing, incoming) {
	...
	return require_agent_run_terminal_outcome.mergeAgentRunTerminalOutcome(existingOutcome, incomingOutcome) === existingOutcome;
}
...
function schedulePendingAgentRunTerminal(pendingRuns, snapshot) {
	...
	}, require_agent_run_terminal_outcome.AGENT_RUN_TERMINAL_RETRY_GRACE_MS);
	timer.unref?.();
...
function ensureAgentRunListener() {
	...
	require_agent_events.onAgentEvent((evt) => {
...
function parseDedupeObservation(entry) {
	...
	if (require_subagent_system_prompt.isNonTerminalAgentRunStatus(status)) return { state: "active" };
	...
	const resultMeta = (0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)((0, _gabrielvfonseca_normalization_core_record_coerce.asOptionalRecord)(payload?.result)?.meta);
	const startedAt = (0, _gabrielvfonseca_normalization_core_number_coercion.asFiniteNumber)(payload?.startedAt);
	const endedAt = (0, _gabrielvfonseca_normalization_core_number_coercion.asFiniteNumber)(payload?.endedAt) ?? entry.ts;
...
function setGatewayDedupeEntry(params) {
	...
	if (existingOutcome && require_agent_run_terminal_outcome.isStickyAgentRunTerminalOutcome(existingOutcome) && !incomingOutcome) return;
	if (existingOutcome && incomingOutcome && require_agent_run_terminal_outcome.isStickyAgentRunTerminalOutcome(existingOutcome)) {
		if (require_agent_run_terminal_outcome.mergeAgentRunTerminalOutcome(existingOutcome, incomingOutcome) === existingOutcome) return;
	}
...
async function waitForAgentJob(params) {
	...
	return await new Promise((resolve) => {
		...
		const timeoutHandle = require_timer_delay.setSafeTimeout(() => {
			if (!params.source) {
...
function emitSessionsChanged(context, payload) {
	...
	const sessionRow = payload.sessionKey ? require_session_utils.loadGatewaySessionRow(payload.sessionKey, payload.sessionKey === "global" && payload.agentId ? { agentId: payload.agentId } : void 0) : null;
	const defaultAgentId = require_agent_scope_config.resolveDefaultAgentId(context.getRuntimeConfig());
...
//#endregion
Object.defineProperty(exports, "emitSessionsChanged", {
	enumerable: true,
	get: function() {
		return emitSessionsChanged;
	}
});
Object.defineProperty(exports, "setGatewayDedupeEntry", {
	enumerable: true,
	get: function() {
		return setGatewayDedupeEntry;
	}
});
Object.defineProperty(exports, "waitForAgentJob", {
	enumerable: true,
	get: function() {
