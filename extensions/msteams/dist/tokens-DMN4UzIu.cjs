...
function getSilentExactRegex(token) {
	...
	const escaped = require_regexp.escapeRegExp(token);
...
const silentIntentTextRe = /^\s*(?:i|i'll|i\s+will|i'm|i\s+am|we|we'll|we\s+will|the\s+assistant|assistant|the\s+bot|bot|operator)\s+(?:(?:will\s+)?(?:stay|remain|keep|be)\s+(?:quiet|silent)(?:\s+(?:here|for\s+now|on\s+this|in\s+this\s+(?:chat|thread|channel|conversation)))?|(?:do\s+not|don't|dont|will\s+not|won't|would\s+not|should\s+not)\s+(?:reply|respond)(?:\s+(?:here|for\s+now|on\s+this|in\s+this\s+(?:chat|thread|channel|conversation)))?|(?:have|has)\s+nothing\s+(?:to|for)\s+(?:say|add|reply|respond))(?:[.!?]+)?\s*$/i;
...
Object.defineProperty(exports, "isInternalFormattingArtifact", {
	enumerable: true,
	get: function() {
		return isInternalFormattingArtifact;
	}
});
Object.defineProperty(exports, "isSilentReplyPayloadText", {
	enumerable: true,
	get: function() {
		return isSilentReplyPayloadText;
	}
});
Object.defineProperty(exports, "isSilentReplyPrefixText", {
	enumerable: true,
	get: function() {
		return isSilentReplyPrefixText;
	}
});
Object.defineProperty(exports, "isSilentReplyText", {
	enumerable: true,
	get: function() {
		return isSilentReplyText;
	}
});
Object.defineProperty(exports, "startsWithSilentToken", {
	enumerable: true,
	get: function() {
		return startsWithSilentToken;
	}
});
Object.defineProperty(exports, "stripLeadingSilentToken", {
	enumerable: true,
	get: function() {
		return stripLeadingSilentToken;
	}
});
Object.defineProperty(exports, "stripSilentToken", {
	enumerable: true,
	get: function() {
		return stripSilentToken;
	}
});
