...
function truncateEvidence(evidence, maxLen = 120) {
	...
	return `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(evidence, maxLen)}…`;
}
...
var scanner_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	clearSkillScanCacheForTest: () => clearSkillScanCacheForTest,
	isScannable: () => isScannable,
	scanDirectoryWithSummary: () => scanDirectoryWithSummary,
	scanSkillContent: () => scanSkillContent,
...
function isScannable(filePath) {
	return SCANNABLE_EXTENSIONS.has(node_path.default.extname(filePath).toLowerCase());
}
...
function findSourceRuleMatch(params) {
	...
	return {
		line: 1,
		evidence: (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(params.source, 120)
	};
}
	...
	for (const rule of LINE_RULES) {
		...
		for (const [i, line] of lines.entries()) {
			...
			if (rule.ruleId === "suspicious-network") {
				const port = Number.parseInt((0, _gabrielvfonseca_normalization_core.expectDefined)(match[1], "scanner regex capture 1"), 10);
...
async function walkDirWithLimit(rootDir, dirPath, candidateLimit, excludeTestFiles, includeHiddenDirectories, includeNestedNodeModulesTestFiles, includeNodeModules) {
	...
	while (stack.length > 0 && files.length < candidateLimit) {
		...
		for (const entry of entries) {
			...
			if (excludeTestFiles && isExcludedTestPath && !(includeNestedNodeModulesTestFiles && pathContainsNodeModulesSegment(node_path.default.relative(rootDir, fullPath)))) continue;
...
async function readDirEntriesWithCache(dirPath) {
	let st;
	try {
		st = await node_fs_promises.default.stat(dirPath);
	} catch (err) {
		if (require_errors.hasErrnoCode(err, "ENOENT")) return [];
		throw err;
	}
	if (!st?.isDirectory()) return [];
	const cached = DIR_ENTRY_CACHE.get(dirPath);
	if (cached && cached.mtimeMs === st.mtimeMs) return cached.entries;
	const dirents = await node_fs_promises.default.readdir(dirPath, { withFileTypes: true });
	const entries = [];
...
async function resolveForcedFiles(params) {
	...
	for (const rawIncludePath of params.includeFiles) {
		const includePath = node_path.default.resolve(params.rootDir, rawIncludePath);
		if (!(0, _openclaw_fs_safe_path.isPathInside)(params.rootDir, includePath)) continue;
		if (!isScannable(includePath)) continue;
		if (seen.has(includePath)) continue;
		let st;
		try {
			st = await node_fs_promises.default.stat(includePath);
		} catch (err) {
			if (require_errors.hasErrnoCode(err, "ENOENT")) continue;
			throw err;
		}
...
async function collectScannableFiles(dirPath, opts) {
	...
	const seen = new Set(forcedFiles.map((f) => node_path.default.resolve(f)));
	const out = [...forcedFiles];
	for (const walkedFile of walked.files) {
		const resolved = node_path.default.resolve(walkedFile);
...
async function scanFileWithCache(params) {
	...
	let st;
	try {
		st = await node_fs_promises.default.stat(filePath);
	} catch (err) {
		if (require_errors.hasErrnoCode(err, "ENOENT")) return {
			scanned: false,
			findings: []
	...
	let source;
	try {
		source = await node_fs_promises.default.readFile(filePath, "utf-8");
	} catch (err) {
		if (require_errors.hasErrnoCode(err, "ENOENT")) return {
			scanned: false,
			findings: []
...
//#endregion
Object.defineProperty(exports, "scanSkillContent", {
	enumerable: true,
	get: function() {
		return scanSkillContent;
	}
});
Object.defineProperty(exports, "scanSource", {
	enumerable: true,
	get: function() {
		return scanSource;
	}
});
Object.defineProperty(exports, "scanner_exports", {
	enumerable: true,
	get: function() {
		return scanner_exports;
	}
});
