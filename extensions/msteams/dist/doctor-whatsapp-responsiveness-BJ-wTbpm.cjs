...
function listLocalTuiProcesses() {
	...
	const ps = (0, node_child_process.spawnSync)("ps", ["-axo", "pid=,command="], {
		encoding: "utf8",
		timeout: 1e3
	});
...
function collectWhatsappResponsivenessHealthFindings(params) {
	...
	return [{
		...
		fixHint: `Close local TUI sessions (${pids}), or run ${require_command_format.formatCliCommand("openclaw doctor --fix")}.`
	}];
}
...
async function terminateLocalTuiProcesses(params) {
	...
	if (graceMs > 0) await require_sleep.sleep(graceMs);
...
if (process.env.VITEST || false) globalThis[Symbol.for("operator.doctorWhatsappResponsivenessTestApi")] = {
...
async function noteWhatsappResponsivenessHealth(params) {
	...
	if ((params.status?.eventLoop)?.degraded === true && tuiProcesses.length > 0) {
		...
		} else warnings.push(`Fix: close those TUI sessions, or run ${require_command_format.formatCliCommand("openclaw doctor --fix")}.`);
	}
	if (warnings.length > 0) require_note.note(warnings.join("\n\n"), "WhatsApp responsiveness");
}
//#endregion
exports.collectWhatsappResponsivenessHealthFindings = collectWhatsappResponsivenessHealthFindings;
exports.noteWhatsappResponsivenessHealth = noteWhatsappResponsivenessHealth;
