...
function suggestOAuthProfileIdForLegacyDefault(params) {
	...
	if (legacyCfg && (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(legacyCfg.provider) === providerKey && legacyCfg.mode !== "oauth") return null;
	const oauthProfiles = require_profile_list.listProfilesForProvider(params.store, providerKey).filter((id) => params.store.profiles[id]?.type === "oauth");
	if (oauthProfiles.length === 0) return null;
	const configuredEmail = legacyCfg?.email?.trim();
	if (configuredEmail) {
		const byEmail = oauthProfiles.find((id) => {
			return require_identity.resolveAuthProfileMetadata({
				cfg: params.cfg,
				store: params.store,
				profileId: id
...
function repairOAuthProfileIdMismatch(params) {
	const legacyProfileId = params.legacyProfileId ?? `${(0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(params.provider)}:default`;
	...
	if ((0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(legacyCfg.provider) !== (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(params.provider)) return {
		config: params.cfg,
		changes: [],
		migrated: false
	};
	...
	const { email: toEmail, displayName: toDisplayName } = require_identity.resolveAuthProfileMetadata({
		cfg: params.cfg,
		store: params.store,
	...
	const nextOrder = (() => {
		...
		if (!order) return;
		const resolvedKey = (0, _gabrielvfonseca_model_catalog_core_provider_id.findNormalizedProviderKey)(order, providerKey);
		if (!resolvedKey) return order;
		const existing = order[resolvedKey];
		if (!Array.isArray(existing)) return order;
		const deduped = require_profile_list.dedupeProfileIds(existing.map((id) => id === legacyProfileId ? toProfileId : id).filter((id) => typeof id === "string" && id.trim().length > 0));
		return {
			...order,
			[resolvedKey]: deduped
		};
...
Object.defineProperty(exports, "repairOAuthProfileIdMismatch", {
	enumerable: true,
	get: function() {
		return repairOAuthProfileIdMismatch;
	}
});
Object.defineProperty(exports, "suggestOAuthProfileIdForLegacyDefault", {
	enumerable: true,
	get: function() {
		return suggestOAuthProfileIdForLegacyDefault;
	}
});
