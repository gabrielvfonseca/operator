...
const require_startup_migration = require("./startup-migration-CeFQ3s5_.cjs");
//#region src/gateway/server-startup-session-migration.ts
const STARTUP_WARNING_ISSUE_CODES = /* @__PURE__ */ new Set([
	"entry_invalid",
...
async function runStartupSessionMigration(params) {
	await require_startup_migration.runSessionStartupMigration(params);
	await runStartupSessionSqliteImport(params);
}
async function runStartupSessionSqliteImport(params) {
	const env = params.env ?? process.env;
	const runDoctorSessionSqlite = params.deps?.runDoctorSessionSqlite ?? (await Promise.resolve().then(() => require("./doctor-session-sqlite-DLuT4my5.cjs"))).runDoctorSessionSqlite;
	let report;
	try {
		...
		if (isSqliteCorruptionError(error)) throw new Error([`session SQLite migration failed during startup because an agent SQLite database could not be opened: ${String(error)}`, "Run \"operator doctor --session-sqlite recover --session-sqlite-all-agents\" to move the corrupt database aside and preserve it for support."].join("\n"), { cause: error });
		throw error;
	}
...
async function restoreFailedStartupSessionSqliteRun(params, report, blockingIssues) {
	...
	if (!restoreSessionSqliteMigrationRun || !writeSessionSqliteMigrationFailureReports) {
		const doctorModule = await Promise.resolve().then(() => require("./doctor-session-sqlite-DLuT4my5.cjs"));
...
function collectStartupBlockingIssues(report) {
	return report.targets.flatMap((target) => target.issues.filter((issue) => !STARTUP_WARNING_ISSUE_CODES.has(issue.code)));
}
...
//#endregion
exports.runStartupSessionMigration = runStartupSessionMigration;
