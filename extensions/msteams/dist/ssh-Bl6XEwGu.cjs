...
function normalizeEndpoint(ssh) {
	...
	const scpTarget = require_scp_host.normalizeScpRemoteHost(`${user}@${bracketedHost}`);
...
async function prepareWorkerSsh(params) {
	...
	const temporaryDir = await node_fs_promises.default.mkdtemp(node_path.default.join(node_os.default.tmpdir(), params.temporaryDirectoryPrefix ?? "operator-worker-ssh-"));
	try {
		const identity = await params.resolveIdentity(params.ssh.keyRef);
		let identityPath;
		if (identity.kind === "path") {
			const resolvedPath = identity.path.trim();
			if (!resolvedPath || !node_path.default.isAbsolute(resolvedPath)) throw new Error("Worker SSH identity path must be absolute");
			identityPath = resolvedPath;
		} else {
			if (!identity.contents.trim()) throw new Error("Worker SSH identity material must be non-empty");
			require_redact.registerSecretValueForRedaction(identity.contents);
			const normalizedContents = normalizeIdentityMaterial(identity.contents);
			if (normalizedContents !== identity.contents) require_redact.registerSecretValueForRedaction(normalizedContents);
			identityPath = node_path.default.join(temporaryDir, "identity");
			await node_fs_promises.default.writeFile(identityPath, normalizedContents, { mode: 384 });
			await node_fs_promises.default.chmod(identityPath, 384);
		}
		const knownHostsPath = node_path.default.join(temporaryDir, "known_hosts");
		await node_fs_promises.default.writeFile(knownHostsPath, knownHosts, { mode: 384 });
		let disposed = false;
		return {
			...endpoint,
			identityPath,
			...
			async dispose() {
				if (disposed) return;
				disposed = true;
				await node_fs_promises.default.rm(temporaryDir, {
					recursive: true,
					force: true
				});
			}
...
//#endregion
Object.defineProperty(exports, "prepareWorkerSsh", {
	enumerable: true,
	get: function() {
		return prepareWorkerSsh;
	}
});
Object.defineProperty(exports, "workerSshCommandOptions", {
	enumerable: true,
	get: function() {
		return workerSshCommandOptions;
	}
});
Object.defineProperty(exports, "workerSshOptions", {
	enumerable: true,
	get: function() {
		return workerSshOptions;
	}
});
Object.defineProperty(exports, "workerSshRemoteCommand", {
	enumerable: true,
	get: function() {
