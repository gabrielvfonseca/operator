...
require("./source-check-bi20wzmV.cjs");
...
function hasChangedInlineOAuthTokenMaterial(params) {
	return INLINE_OAUTH_TOKEN_FIELDS.some((field) => {
		...
		return !(0, node_util.isDeepStrictEqual)(params.credential[field], params.existingCredential[field]);
	});
}
function preserveLegacyOAuthRefsOnSave(params) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(params.existingRaw) || !(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(params.existingRaw.profiles)) return params.payload;
	let nextProfiles;
	for (const [profileId, credential] of Object.entries(params.payload.profiles)) {
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(credential) || credential.oauthRef !== void 0 || credential.type !== "oauth") continue;
		const existingCredential = params.existingRaw.profiles[profileId];
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(existingCredential) || existingCredential.oauthRef === void 0 || existingCredential.type !== "oauth") continue;
...
function publishRuntimeSnapshotsAfterCommit(publish) {
	...
	try {
		...
		require_persisted.log.warn("auth profile store committed but runtime snapshot publication failed", { err });
		return false;
	}
}
const testing = {
...
if (process.env.VITEST || false) globalThis[Symbol.for("operator.authProfileStoreTestApi")] = testing;
...
function isInheritedMainOAuthCredential(params) {
	...
	if (require_path_resolve.resolveAuthStorePath(params.agentDir) === require_path_resolve.resolveAuthStorePath()) return false;
	if (require_persisted.loadPersistedAuthProfileStore(params.agentDir)?.profiles[params.profileId]) return false;
	const mainCredential = require_persisted.loadPersistedAuthProfileStore()?.profiles[params.profileId];
	return mainCredential?.type === "oauth" && ((0, node_util.isDeepStrictEqual)(mainCredential, params.credential) || shouldUseMainOwnerForLocalOAuthCredential({
...
function shouldUseMainOwnerForLocalOAuthCredential(params) {
	...
	if (!require_persisted.isSafeToAdoptMainStoreOAuthIdentity(params.local, params.main)) return false;
	if ((0, node_util.isDeepStrictEqual)(params.local, params.main)) return true;
	const mainExpires = (0, require_number_coercion.number_coercion_exports.asDateTimestampMs)(params.main.expires);
	if (mainExpires === void 0) return false;
	const localExpires = (0, require_number_coercion.number_coercion_exports.asDateTimestampMs)(params.local.expires);
	return localExpires === void 0 || mainExpires >= localExpires;
}
function resolveRuntimeAuthProfileStore(agentDir, options) {
	const mainKey = require_path_resolve.resolveAuthStorePath(void 0);
	const requestedKey = require_path_resolve.resolveAuthStorePath(agentDir);
	const mainStore = require_runtime_snapshots.getRuntimeAuthProfileStoreSnapshot(void 0);
	const requestedStore = require_runtime_snapshots.getRuntimeAuthProfileStoreSnapshot(agentDir);
	if (!agentDir || requestedKey === mainKey) {
		if (!mainStore) return null;
		return mainStore;
	}
	if (mainStore && requestedStore) return require_persisted.mergeAuthProfileStores(mainStore, requestedStore, { preserveBaseRuntimeExternalProfiles: true });
	if (requestedStore) return require_persisted.mergeAuthProfileStores(loadAuthProfileStoreForAgent(void 0, {
		readOnly: true,
		syncExternalCli: false,
		...resolvePersistedLoadOptions(options)
	}), requestedStore, { preserveBaseRuntimeExternalProfiles: true });
	if (mainStore) return require_persisted.mergeAuthProfileStores(mainStore, loadAuthProfileStoreForAgent(agentDir, {
		readOnly: true,
		syncExternalCli: false,
		...resolvePersistedLoadOptions(options)
...
function maybeSyncPersistedExternalCliAuthProfiles(params) {
	if (params.options?.readOnly === true || params.options?.syncExternalCli === false || process.env.OPERATOR_AUTH_STORE_READONLY === "1") return {
		store: params.store,
		cacheable: true
	};
	const synced = require_external_auth.syncPersistedExternalCliAuthProfiles(params.store, {
	...
	const changedProfiles = Object.entries(synced.profiles).filter(([profileId, credential]) => {
		...
		return !(0, node_util.isDeepStrictEqual)(previous, credential);
	});
	...
	let result;
	try {
		result = require_sqlite.runAuthProfileWriteTransaction(params.agentDir, (database) => {
			const latestStore = require_persisted.loadPersistedAuthProfileStore(params.agentDir, {
			...
			for (const [profileId, credential] of changedProfiles) {
				...
				if (!(0, node_util.isDeepStrictEqual)(latest, previous)) {
					require_persisted.log.debug("skipped persisted external cli auth sync for concurrently changed profile", { profileId });
					continue;
				}
		...
		require_persisted.log.warn("skipped persisted external cli auth sync because auth store write failed", { err });
		return {
			store: params.store,
			cacheable: false
		};
...
function shouldKeepProfileInLocalStore(params) {
	...
	if (params.store.runtimeExternalProfileIds?.includes(params.profileId)) {
		if (require_persisted.loadPersistedAuthProfileStore(params.agentDir)?.profiles[params.profileId]) return require_persisted.shouldPersistRuntimeExternalOAuthProfile({
			profileId: params.profileId,
			credential: params.credential,
		...
		if (!runtimeCredential || (0, node_util.isDeepStrictEqual)(runtimeCredential, params.credential)) return false;
	}
	return require_persisted.shouldPersistRuntimeExternalOAuthProfile({
		profileId: params.profileId,
		credential: params.credential,
		profiles: params.externalProfiles()
	});
...
function buildLocalAuthProfileStoreForSave(params) {
	const localStore = require_path_resolve.cloneAuthProfileStore(params.store);
	let externalProfiles;
	const getExternalProfiles = () => externalProfiles ??= require_external_auth.listRuntimeExternalAuthProfiles({
		store: params.store,
		agentDir: params.agentDir
	});
	...
	for (const profileIds of Object.values(require_persisted.loadPersistedAuthProfileState(params.agentDir).order ?? {})) for (const profileId of profileIds) keptOrderProfileIds.add(profileId);
...
function buildAuthProfileStoreWithoutExternalProfiles(params) {
	...
	return stripRuntimeExternalProfileMetadata(require_persisted.mergeAuthProfileStores(loadAuthProfileStoreWithoutExternalProfiles(params.agentDir, params.options), localStore));
}
...
function markRuntimePersistedProfiles(store, persistedStore = store) {
	const profileIds = Object.entries(persistedStore.profiles).flatMap(([profileId, credential]) => (0, node_util.isDeepStrictEqual)(store.profiles[profileId], credential) ? [profileId] : []).toSorted();
	return {
		...store,
...
function runtimeStoreInheritsMainState(store, localStore) {
	...
	return !(0, node_util.isDeepStrictEqual)(state(store), state(localStore));
}
...
function mergeRuntimeExternalProfileReferences(params) {
	...
	const merged = require_path_resolve.cloneAuthProfileStore(params.next);
	...
	for (const profileId of runtimeExternalProfileIds) {
		...
		if (nextCredential) {
			if (mergedRuntimeExternalProfileIds.has(profileId) || existingCredential && (0, node_util.isDeepStrictEqual)(nextCredential, existingCredential)) mergedRuntimeExternalProfileIds.add(profileId);
			continue;
		}
...
function preserveResolvedSecretBackedCredentials(params) {
	const next = require_path_resolve.cloneAuthProfileStore(params.next);
	for (const [profileId, credential] of Object.entries(next.profiles)) {
		const existing = params.existing.profiles[profileId];
		if (credential.type === "api_key" && existing?.type === "api_key" && credential.key === void 0 && existing.key !== void 0 && require_types_secrets.isSecretRef(credential.keyRef) && (0, node_util.isDeepStrictEqual)(credential.keyRef, existing.keyRef)) next.profiles[profileId] = {
			...credential,
			key: existing.key
		};
		else if (credential.type === "token" && existing?.type === "token" && credential.token === void 0 && existing.token !== void 0 && require_types_secrets.isSecretRef(credential.tokenRef) && (0, node_util.isDeepStrictEqual)(credential.tokenRef, existing.tokenRef)) next.profiles[profileId] = {
			...credential,
...
function mergeRuntimeExternalProfileState(params) {
	...
	for (const profileId of existingRuntimeProfileIds) {
		...
		const nextCredential = merged.profiles[profileId];
		if (nextCredential) {
			if (mergedRuntimeProfileIds.has(profileId) || (0, node_util.isDeepStrictEqual)(nextCredential, existingCredential)) {
...
async function updateAuthProfileStoreWithLock(params) {
	...
	let store;
	try {
		store = require_sqlite.runAuthProfileWriteTransaction(params.agentDir, (database) => {
		...
		require_persisted.log.warn(`auth profile store update failed: ${message}`, {
			agentDir: params.agentDir,
			error: message
		});
		return null;
...
function loadAuthProfileStore() {
	const asStore = require_persisted.loadPersistedAuthProfileStore();
	if (asStore) return require_external_auth.overlayExternalAuthProfiles(markRuntimePersistedProfiles(asStore));
	return require_external_auth.overlayExternalAuthProfiles(markRuntimePersistedProfiles({
		version: 1,
		profiles: {}
	}));
}
function loadAuthProfileStoreForAgent(agentDir, options) {
	...
	const asStore = require_persisted.loadPersistedAuthProfileStore(agentDir, resolvePersistedLoadOptions(options));
	...
	const mergedOAuth = require_persisted.mergeOAuthFileIntoStore(store);
...
function loadAuthProfileStoreForRuntime(agentDir, options) {
	...
	if (!agentDir || authPath === mainAuthPath) return setRuntimeLocalProfileMetadata(require_external_auth.overlayExternalAuthProfiles(store, {
		agentDir,
		...externalCli
	}), listRuntimeLocalProfileIds(store));
	const mainStore = loadAuthProfileStoreForAgent(void 0, options);
	const mergedStore = require_persisted.mergeAuthProfileStores(mainStore, store, { preserveBaseRuntimeExternalProfiles: true });
	return setRuntimeLocalProfileMetadata(require_external_auth.overlayExternalAuthProfiles(mergedStore, {
		agentDir,
		...externalCli
...
function ensureAuthProfileStore(agentDir, options) {
	...
	const store = require_external_auth.overlayExternalAuthProfiles(ensureAuthProfileStoreWithoutExternalProfiles(agentDir, options), {
		agentDir,
		...externalCli
	});
...
function ensureAuthProfileStoreWithoutExternalProfiles(agentDir, options) {
	...
	return stripRuntimeExternalProfileMetadata(require_persisted.mergeAuthProfileStores(loadAuthProfileStoreForAgent(void 0, effectiveOptions), store, { preserveBaseRuntimeExternalProfiles: true }));
}
...
function findPersistedAuthProfileCredential(params) {
	const requestedProfile = require_persisted.loadPersistedAuthProfileStore(params.agentDir)?.profiles[params.profileId];
	if (requestedProfile || !params.agentDir) return requestedProfile;
	if (require_path_resolve.resolveAuthStorePath(params.agentDir) === require_path_resolve.resolveAuthStorePath()) return requestedProfile;
	return require_persisted.loadPersistedAuthProfileStore()?.profiles[params.profileId];
}
...
function resolvePersistedAuthProfileOwnerAgentDir(params) {
	...
	const requestedStore = require_persisted.loadPersistedAuthProfileStore(params.agentDir);
	if (require_path_resolve.resolveAuthStorePath(params.agentDir) === require_path_resolve.resolveAuthStorePath()) return;
	const mainStore = require_persisted.loadPersistedAuthProfileStore();
...
function ensureAuthProfileStoreForLocalUpdate(agentDir) {
	...
	return require_persisted.mergeAuthProfileStores(loadAuthProfileStoreForAgent(void 0, {
		readOnly: true,
		syncExternalCli: false
...
function getRuntimeAuthProfileStoreSnapshot(agentDir) {
	return require_runtime_snapshots.getRuntimeAuthProfileStoreSnapshot(agentDir);
}
...
function replaceRuntimeAuthProfileStoreSnapshots(entries) {
	require_runtime_snapshots.replaceRuntimeAuthProfileStoreSnapshots(entries);
}
...
function clearRuntimeAuthProfileStoreSnapshot(agentDir) {
	return require_runtime_snapshots.clearRuntimeAuthProfileStoreSnapshot(agentDir);
}
function saveAuthProfileStoreInTransaction(store, agentDir, options, database, publishFromSuppliedStore = false) {
	const savedAuthPath = require_path_resolve.resolveAuthStorePath(agentDir);
	const mainAuthPath = require_path_resolve.resolveAuthStorePath();
	...
	const existingRaw = require_sqlite.readPersistedAuthProfileStoreRaw(agentDir, database);
	const payload = preserveLegacyOAuthRefsOnSave({
		payload: require_persisted.buildPersistedAuthProfileSecretsStore(localStore),
		existingRaw
	});
	const existingProfiles = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(existingRaw) && (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(existingRaw.profiles) ? existingRaw.profiles : {};
	const changedProfileIds = [.../* @__PURE__ */ new Set([...Object.keys(existingProfiles), ...Object.keys(payload.profiles)])].filter((profileId) => !(0, node_util.isDeepStrictEqual)(existingProfiles[profileId], payload.profiles[profileId]));
	...
	const credentialsChanged = !(0, node_util.isDeepStrictEqual)(existingRaw, payload);
	const statePayload = require_persisted.buildPersistedAuthProfileState(localStore);
	const stateChanged = !(0, node_util.isDeepStrictEqual)(require_sqlite.readPersistedAuthProfileStateRaw(agentDir, database), statePayload);
	const suppliedRuntimeStore = publishFromSuppliedStore ? markRuntimePersistedProfiles(buildRuntimeAuthProfileStoreForSave({
		store,
		agentDir,
		options
	}), localStore) : void 0;
	if (credentialsChanged) require_sqlite.writePersistedAuthProfileStoreRaw(payload, agentDir, database);
	if (stateChanged) require_sqlite.writePersistedAuthProfileStateRaw(statePayload, agentDir, database);
	const publishRuntimeSnapshots = () => {
		const derivedSnapshots = savesMainStore ? require_runtime_snapshots.listRuntimeAuthProfileStoreSnapshots().filter((entry) => require_path_resolve.resolveAuthStorePath(entry.agentDir) !== mainAuthPath) : [];
		if (credentialsChanged || stateChanged) require_runtime_snapshots.noteRuntimeAuthProfileStorePersistedMutation(agentDir, {
			credentialsChanged,
			profileSetChanged,
			stateChanged,
		...
		if (suppliedRuntimeStore) {
			...
			if (existing) require_runtime_snapshots.setRuntimeAuthProfileStoreSnapshot(mergeRuntimeExternalProfileReferences({
				next: suppliedRuntimeStore,
				existing
			}), agentDir);
			if (savesMainStore && (credentialsChanged || stateChanged)) for (const derived of derivedSnapshots) require_runtime_snapshots.setRuntimeAuthProfileStoreSnapshot(mergeRuntimeExternalProfileReferences({
		...
		for (const derived of derivedSnapshots) require_runtime_snapshots.setRuntimeAuthProfileStoreSnapshot(mergeRuntimeExternalProfileReferences({
			next: preserveResolvedSecretBackedCredentials({
				next: loadAuthProfileStoreWithoutExternalProfiles(derived.agentDir),
				existing: derived.store
			}),
...
function saveAuthProfileStore(store, agentDir, options, database) {
	if (database) {
		...
		if (!require_openclaw_agent_db.deferOperatorAgentPostCommitPublication(database, publishAfterCommit)) publishAfterCommit();
		return;
	}
	let publishRuntimeSnapshots;
	require_sqlite.runAuthProfileWriteTransaction(agentDir, (transactionDatabase) => {
...
function rebuildRuntimeAuthProfileStoreSnapshot(agentDir, existing, predecessor) {
	...
	require_runtime_snapshots.setRuntimeAuthProfileStoreSnapshot(mergeRuntimeExternalProfileReferences({
...
Object.defineProperty(exports, "ensureAuthProfileStoreForLocalUpdate", {
	enumerable: true,
	get: function() {
		return ensureAuthProfileStoreForLocalUpdate;
	}
});
Object.defineProperty(exports, "ensureAuthProfileStoreWithoutExternalProfiles", {
	enumerable: true,
	get: function() {
		return ensureAuthProfileStoreWithoutExternalProfiles;
	}
});
Object.defineProperty(exports, "findPersistedAuthProfileCredential", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "loadAuthProfileStoreForRuntime", {
	enumerable: true,
	get: function() {
		return loadAuthProfileStoreForRuntime;
	}
});
Object.defineProperty(exports, "loadAuthProfileStoreForSecretsRuntime", {
	enumerable: true,
	get: function() {
		return loadAuthProfileStoreForSecretsRuntime;
	}
});
Object.defineProperty(exports, "loadAuthProfileStoreWithoutExternalProfiles", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "resolvePersistedAuthProfileOwnerAgentDir", {
	enumerable: true,
	get: function() {
		return resolvePersistedAuthProfileOwnerAgentDir;
	}
});
Object.defineProperty(exports, "saveAuthProfileStore", {
	enumerable: true,
	get: function() {
		return saveAuthProfileStore;
	}
});
Object.defineProperty(exports, "updateAuthProfileStoreWithLock", {
	enumerable: true,
	get: function() {
