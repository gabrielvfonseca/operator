...
function listAdvertisedLanHostCandidates(snapshot) {
	return require_network_interfaces.listExternalInterfaceAddresses(snapshot, "IPv4").filter((entry) => (0, _gabrielvfonseca_net_policy_ip.isRfc1918Ipv4Address)(entry.address)).map((entry, order) => ({
...
async function resolveAdvertisedLanHost(options = {}) {
	const candidates = listAdvertisedLanHostCandidates(require_network_interfaces.safeNetworkInterfaces(options.networkInterfaces));
	if (candidates.length === 0) return null;
	return selectAdvertisedLanHost(candidates, await resolveDefaultRouteHints({
		platform: options.platform ?? process.platform,
		runCommandWithTimeout: options.runCommandWithTimeout ?? require_exec.runCommandWithTimeout,
		timeoutMs: options.timeoutMs ?? DEFAULT_ROUTE_HINT_TIMEOUT_MS
	}));
}
//#endregion
Object.defineProperty(exports, "resolveAdvertisedLanHost", {
	enumerable: true,
	get: function() {
		return resolveAdvertisedLanHost;
	}
});
