...
function resolveConfiguredDockerRuntimeImage(params) {
	const sandboxCfg = require_config.resolveSandboxConfigForAgent(params.config, params.agentId);
...
async function createDockerSandboxBackend(params) {
	return createDockerSandboxBackendHandle({
		containerName: await require_docker.ensureSandboxContainer({
...
function createDockerSandboxBackendHandle(params) {
	return {
		...
		async buildExecSpec({ command, workdir, env, usePty }) {
			return {
				argv: ["docker", ...require_bash_tools_shared.buildDockerExecArgs({
...
function runDockerSandboxShellCommand(params) {
	...
	return require_docker.execDockerRaw(dockerArgs, {
		input: params.stdin,
...
const dockerSandboxBackendManager = {
	async describeRuntime({ entry, config, agentId }) {
		const state = await require_docker.dockerContainerState(entry.containerName);
...
const SANDBOX_PINNED_MUTATION_PYTHON = [
	...
	"    prefix = '.operator-write-' + basename + '.'",
	"    for _ in range(128):",
	...
	return buildPinnedMutationPlan({
		checks: [{
			...
			options: {
				...
				aliasPolicy: _openclaw_fs_safe_advanced.PATH_ALIAS_POLICIES.unlinkTarget
			}
		}],
		args: [
			"remove",
...
function parseSandboxStatMtimeMs(value) {
	const raw = value ?? "0";
	if (/^\d+(?:\.\d+)?$/.test(raw)) return (0, require_number_coercion.number_coercion_exports.asDateTimestampMs)(Number(raw) * 1e3) ?? 0;
	return (0, require_number_coercion.number_coercion_exports.asDateTimestampMs)(Date.parse(raw)) ?? 0;
}
//#endregion
...
function hasMultipleHardlinks(raw) {
	const linkCount = require_parse_finite_number.parseStrictNonNegativeInteger(raw);
...
var RemoteShellSandboxFsBridge = class {
	...
	async readFile(params) {
		const target = this.resolveTarget(params);
		const relativePath = node_path.default.posix.relative(target.mountRootPath, target.containerPath);
		if (relativePath === "" || relativePath === "." || require_path_utils.relativePathEscapesContainerRoot(relativePath)) throw new Error(`Invalid sandbox entry target: ${target.containerPath}`);
		return (await this.runMutation({
			args: [
				"read",
				target.mountRootPath,
				node_path.default.posix.dirname(relativePath) === "." ? "" : node_path.default.posix.dirname(relativePath),
				node_path.default.posix.basename(relativePath)
			],
			signal: params.signal
		})).stdout;
	}
	...
	async mkdirp(params) {
		...
		const relativePath = node_path.default.posix.relative(target.mountRootPath, target.containerPath);
		if (require_path_utils.relativePathEscapesContainerRoot(relativePath)) throw new Error(`Sandbox path escapes allowed mounts; cannot create directories: ${target.containerPath}`);
		await this.runMutation({
			args: [
				"mkdirp",
				target.mountRootPath,
	...
	getMounts() {
		const workspaceRoot = node_path.default.resolve(this.sandbox.workspaceDir);
		const agentRoot = node_path.default.resolve(this.sandbox.agentWorkspaceDir);
		...
		if (this.sandbox.workspaceAccess !== "none" && node_path.default.resolve(this.sandbox.agentWorkspaceDir) !== node_path.default.resolve(this.sandbox.workspaceDir)) mounts.push({
			localRoot: agentRoot,
		...
		if (this.sandbox.workspaceAccess === "rw") mounts.push(...buildRemoteProtectedSkillMounts({
			...
			agentContainerRoot,
			includeAgentMount: node_path.default.resolve(this.sandbox.agentWorkspaceDir) !== node_path.default.resolve(this.sandbox.workspaceDir)
		}));
		return mounts;
	}
	resolveTarget(params) {
		...
		const maybeContainerMount = node_path.default.posix.isAbsolute(inputPosix) ? this.resolveMountByContainerPath(mounts, normalizeContainerPath(inputPosix)) : null;
		...
		const hostCwd = params.cwd ? node_path.default.resolve(params.cwd) : workspaceRoot;
		const hostCandidate = node_path.default.isAbsolute(input) ? node_path.default.resolve(input) : node_path.default.resolve(hostCwd, input);
		...
		if (hostMount) {
			...
			return this.toResolvedPath({
				mount: hostMount,
				containerPath: relative ? node_path.default.posix.join(hostMount.containerRoot, relative) : hostMount.containerRoot
			});
		}
		if (params.cwd) {
			const cwdPosix = params.cwd.replace(/\\/g, "/");
			if (node_path.default.posix.isAbsolute(cwdPosix)) {
				const cwdContainer = normalizeContainerPath(cwdPosix);
				const cwdMount = this.resolveMountByContainerPath(mounts, cwdContainer);
				if (cwdMount) {
					const containerPath = normalizeContainerPath(node_path.default.posix.resolve(cwdContainer, inputPosix));
	...
	toResolvedPath(params) {
		const relative = node_path.default.posix.relative(params.mount.containerRoot, params.containerPath);
		if (require_path_utils.relativePathEscapesContainerRoot(relative)) throw new Error(`Sandbox path escapes allowed mounts; cannot access: ${params.containerPath}`);
		return {
			relativePath: params.mount.source === "workspace" || params.mount.source === "protectedSkill" ? relative === "." ? "" : node_path.default.posix.relative(this.runtime.remoteWorkspaceDir, params.containerPath) : relative === "." ? params.mount.containerRoot : `${params.mount.containerRoot}/${relative}`,
	...
	resolveMountByContainerPath(mounts, containerPath) {
		...
		for (const mount of ordered) if (require_path_utils.isPathInsideContainerRoot(mount.containerRoot, containerPath)) return mount;
		return null;
	}
	resolveMountByLocalPath(mounts, localPath) {
		const ordered = [...mounts].toSorted(compareRemoteMountsByLocalPath);
		for (const mount of ordered) if ((0, _openclaw_fs_safe_path.isPathInside)(mount.localRoot, localPath)) return mount;
		return null;
	}
	...
	findRemoteProtectedSkillRoot(containerPath) {
		...
		for (const root of roots) if (require_path_utils.isPathInsideContainerRoot(root, containerPath)) return root;
		return null;
	}
	getRemoteProtectedSkillRoots() {
		...
		const agentContainerRoot = normalizeContainerPath(this.runtime.remoteAgentWorkspaceDir);
		const roots = [
			node_path.default.posix.join(workspaceContainerRoot, "skills"),
			node_path.default.posix.join(workspaceContainerRoot, ".agents", "skills"),
			node_path.default.posix.join(workspaceContainerRoot, ".operator", "sandbox-skills", "skills")
		];
		if (node_path.default.resolve(this.sandbox.agentWorkspaceDir) !== node_path.default.resolve(this.sandbox.workspaceDir)) roots.push(node_path.default.posix.join(agentContainerRoot, "skills"), node_path.default.posix.join(agentContainerRoot, ".agents", "skills"), node_path.default.posix.join(agentContainerRoot, ".operator", "sandbox-skills", "skills"));
		return roots;
	}
	...
	async resolvePinnedParent(params) {
		const basename = node_path.default.posix.basename(params.containerPath);
		...
		const canonicalParent = await this.resolveCanonicalPath({
			containerPath: normalizeContainerPath(node_path.default.posix.dirname(params.containerPath)),
		...
		const relativeParentPath = node_path.default.posix.relative(mount.containerRoot, canonicalParent);
		if (require_path_utils.relativePathEscapesContainerRoot(relativeParentPath)) throw new Error(`Sandbox path escapes allowed mounts; cannot ${params.action}: ${params.containerPath}`);
		return {
...
function buildRemoteProtectedSkillMounts(params) {
	const materializedSkillsWorkspaceDir = node_path.default.resolve(params.skillsWorkspaceDir ?? require_workspace_mounts.resolveMaterializedSandboxSkillsWorkspaceDir(params.localRoot));
	const mounts = [
		{
			localRoot: node_path.default.join(params.localRoot, "skills"),
			containerRoot: node_path.default.posix.join(params.workspaceContainerRoot, "skills"),
			writable: false,
			source: "protectedSkill",
			allowedRoot: params.localRoot
		},
		{
			localRoot: node_path.default.join(params.localRoot, ".agents", "skills"),
			containerRoot: node_path.default.posix.join(params.workspaceContainerRoot, ".agents", "skills"),
			writable: false,
			source: "protectedSkill",
			allowedRoot: params.localRoot
		},
		{
			localRoot: node_path.default.join(materializedSkillsWorkspaceDir, "skills"),
			containerRoot: node_path.default.posix.join(params.workspaceContainerRoot, ".operator", "sandbox-skills", "skills"),
			writable: false,
			source: "protectedSkill",
	...
	if (params.includeAgentMount) mounts.push({
		localRoot: node_path.default.join(params.localRoot, "skills"),
		containerRoot: node_path.default.posix.join(params.agentContainerRoot, "skills"),
		writable: false,
		source: "protectedSkill",
		allowedRoot: params.localRoot
	}, {
		localRoot: node_path.default.join(params.localRoot, ".agents", "skills"),
		containerRoot: node_path.default.posix.join(params.agentContainerRoot, ".agents", "skills"),
		writable: false,
		source: "protectedSkill",
		allowedRoot: params.localRoot
	}, {
		localRoot: node_path.default.join(materializedSkillsWorkspaceDir, "skills"),
		containerRoot: node_path.default.posix.join(params.agentContainerRoot, ".operator", "sandbox-skills", "skills"),
		writable: false,
		source: "protectedSkill",
		allowedRoot: materializedSkillsWorkspaceDir
	});
	return mounts.filter((mount) => require_workspace_mounts.isExistingWorkspaceSkillMountSource({
...
function normalizeContainerPath(value) {
	const normalized = require_path_utils.normalizeContainerPath(value.trim() || "/");
	return normalized.startsWith("/") ? normalized : `/${normalized}`;
}
function toPosixRelative(root, candidate) {
	return node_path.default.relative(root, candidate).split(node_path.default.sep).filter(Boolean).join(node_path.default.posix.sep);
}
//#endregion
...
function parseSshTarget(raw) {
	...
	if (colonIdx > 0 && colonIdx < hostPart.length - 1) {
		...
		const port = require_parse_finite_number.parseStrictPositiveInteger(hostPart.slice(colonIdx + 1).trim());
...
async function createSshSandboxSessionFromSettings(settings) {
	...
	const configDir = await node_fs_promises.default.mkdtemp(node_path.default.join(resolveSshTmpRoot(), "operator-sandbox-ssh-"));
	try {
		...
		const hostAlias = "operator-sandbox";
		const configPath = node_path.default.join(configDir, "config");
		const lines = [
			`Host ${hostAlias}`,
		...
		await node_fs_promises.default.writeFile(configPath, `${lines.join("\n")}\n`, {
			encoding: "utf8",
			mode: 384
		});
		await node_fs_promises.default.chmod(configPath, 384);
		return {
		...
		await node_fs_promises.default.rm(configDir, {
			recursive: true,
			force: true
		});
		throw error;
...
async function disposeSshSandboxSession(session) {
	await node_fs_promises.default.rm(node_path.default.dirname(session.configPath), {
		recursive: true,
		force: true
	});
}
...
async function runSshSandboxCommand(params) {
	...
	const sshEnv = require_sanitize_env_vars.sanitizeEnvVars(process.env).allowed;
	const result = await require_exec.spawnCommand([executable, ...args], {
		baseEnv: sshEnv,
	...
	if (params.signal?.aborted || result.isCanceled) throw require_abort_signal.createAbortError("Aborted");
	if (result.failed && !require_exec.isPlainCommandExitFailure(result)) throw (0, _gabrielvfonseca_normalization_core_error_coercion.toErrorObject)(result, "SSH command execution failed");
...
async function uploadDirectoryToSshTarget(params) {
	...
	await new Promise((resolve, reject) => {
		const tar = (0, node_child_process.spawn)("tar", [
			"-C",
			params.localDir,
			"-cf",
			"-",
		...
		const ssh = (0, node_child_process.spawn)(sshExecutable, sshArgs, {
			stdio: [
				"pipe",
				"pipe",
				"pipe"
...
async function assertSafeUploadSymlinks(localDir) {
	const rootDir = node_path.default.resolve(localDir);
	await walkDirectory(rootDir);
	async function walkDirectory(currentDir) {
		const entries = await node_fs_promises.default.readdir(currentDir, { withFileTypes: true });
		for (const entry of entries) {
			const entryPath = node_path.default.join(currentDir, entry.name);
			if (entry.isSymbolicLink()) {
				try {
					await (0, _openclaw_fs_safe_advanced.resolveRootPath)({
						absolutePath: entryPath,
						rootPath: rootDir,
						boundaryLabel: "SSH sandbox upload tree"
					});
				} catch (error) {
					const relativePath = node_path.default.relative(rootDir, entryPath).split(node_path.default.sep).join("/");
...
function resolveSshTmpRoot() {
	return node_path.default.resolve(require_tmp_operator_dir.resolvePreferredOperatorTmpDir() ?? node_os.default.tmpdir());
}
function resolveOptionalLocalPath(value) {
	const trimmed = value?.trim();
	return trimmed ? require_home_dir.resolveUserPath(trimmed) : void 0;
}
async function writeSecretMaterial(dir, filename, contents) {
	const pathname = node_path.default.join(dir, filename);
	await node_fs_promises.default.writeFile(pathname, normalizeInlineSshMaterial(contents, filename), {
		encoding: "utf8",
		mode: 384
	});
	await node_fs_promises.default.chmod(pathname, 384);
	return pathname;
}
//#endregion
...
const sshSandboxBackendManager = {
	async describeRuntime({ entry, config, agentId }) {
		const cfg = require_config.resolveSandboxConfigForAgent(config, agentId);
...
var SshSandboxBackendImpl = class {
			...
			buildExecSpec: async ({ command, workdir, env, usePty }) => {
				...
				try {
					...
					return {
						...
						env: require_sanitize_env_vars.sanitizeEnvVars(process.env).allowed,
						stdinMode: "pipe-open",
	...
	async ensureRuntimeInner() {
		...
		try {
			...
			if (this.params.createParams.cfg.workspaceAccess !== "none" && node_path.default.resolve(this.params.createParams.agentWorkspaceDir) !== node_path.default.resolve(this.params.createParams.workspaceDir)) await this.replaceRemoteDirectoryFromLocal(session, this.params.createParams.agentWorkspaceDir, this.params.runtimePaths.remoteAgentWorkspaceDir);
		} finally {
			await disposeSshSandboxSession(session);
		}
	}
...
async function isExistingDirectory(dir) {
	try {
		return (await node_fs_promises.default.stat(dir)).isDirectory();
	} catch {
		return false;
	}
}
function normalizeRemotePath(input) {
	const normalized = node_path.default.posix.normalize(input.replace(/\\/g, "/"));
...
function resolveSshRuntimePaths(workspaceRoot, scopeKey) {
	...
	const runtimeRootDir = node_path.default.posix.join(workspaceRoot, runtimeId);
	return {
		runtimeId,
		runtimeRootDir,
		remoteWorkspaceDir: node_path.default.posix.join(runtimeRootDir, "workspace"),
		remoteAgentWorkspaceDir: node_path.default.posix.join(runtimeRootDir, "agent"),
		remoteSkillsWorkspaceDir: node_path.default.posix.join(runtimeRootDir, "workspace", ".operator", "sandbox-skills")
	};
}
function buildSshSandboxRuntimeId(scopeKey) {
	const trimmed = scopeKey.trim() || "session";
	const safe = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(trimmed).replace(/[^a-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 32);
	const hash = Array.from(trimmed).reduce((acc, char) => (acc * 33 ^ char.charCodeAt(0)) >>> 0, 5381);
	return `operator-ssh-${safe || "session"}-${hash.toString(16).slice(0, 8)}`;
}
//#endregion
...
*/
const SANDBOX_BACKEND_FACTORIES_STATE_KEY = Symbol.for("operator.sandboxBackendFactories");
...
//#endregion
Object.defineProperty(exports, "BROWSER_BRIDGES", {
	enumerable: true,
	get: function() {
		return BROWSER_BRIDGES;
	}
});
Object.defineProperty(exports, "buildPinnedMkdirpPlan", {
	enumerable: true,
	get: function() {
		return buildPinnedMkdirpPlan;
	}
});
Object.defineProperty(exports, "buildPinnedRemovePlan", {
	enumerable: true,
	get: function() {
		return buildPinnedRemovePlan;
	}
});
Object.defineProperty(exports, "buildPinnedRenamePlan", {
	enumerable: true,
	get: function() {
		return buildPinnedRenamePlan;
	}
});
Object.defineProperty(exports, "buildPinnedWritePlan", {
	enumerable: true,
	get: function() {
		return buildPinnedWritePlan;
	}
});
Object.defineProperty(exports, "dockerSandboxBackendManager", {
	enumerable: true,
	get: function() {
		return dockerSandboxBackendManager;
	}
});
Object.defineProperty(exports, "getSandboxBackendManager", {
	enumerable: true,
	get: function() {
		return getSandboxBackendManager;
	}
});
Object.defineProperty(exports, "getSandboxBackendWorkdirResolver", {
	enumerable: true,
	get: function() {
		return getSandboxBackendWorkdirResolver;
	}
});
Object.defineProperty(exports, "parseSandboxStatMtimeMs", {
	enumerable: true,
	get: function() {
		return parseSandboxStatMtimeMs;
	}
});
Object.defineProperty(exports, "parseSandboxStatSize", {
	enumerable: true,
	get: function() {
		return parseSandboxStatSize;
	}
});
Object.defineProperty(exports, "requireSandboxBackendFactory", {
	enumerable: true,
	get: function() {
		return requireSandboxBackendFactory;
	}
});
Object.defineProperty(exports, "runDockerSandboxShellCommand", {
	enumerable: true,
	get: function() {
		return runDockerSandboxShellCommand;
	}
});
Object.defineProperty(exports, "startBrowserBridgeServer", {
	enumerable: true,
	get: function() {
		return startBrowserBridgeServer;
	}
});
Object.defineProperty(exports, "stopCachedBrowserBridge", {
	enumerable: true,
	get: function() {
		return stopCachedBrowserBridge;
	}
});
Object.defineProperty(exports, "stopCachedBrowserBridgesForContainer", {
	enumerable: true,
	get: function() {
		return stopCachedBrowserBridgesForContainer;
	}
});
