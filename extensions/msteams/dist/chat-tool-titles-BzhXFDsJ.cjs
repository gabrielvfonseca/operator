...
function cacheKeyFor(item) {
	return (0, node_crypto.createHash)("sha256").update(`${item.name}\0${item.input}`).digest("hex");
}
function normalizeItems(items) {
	...
	for (const item of items) {
		...
		const input = (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(require_redact.redactToolPayloadText(item.input), TOOL_TITLE_INPUT_MAX_CHARS);
...
function normalizeTitle(raw) {
	...
	return singleLine ? (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(singleLine, TOOL_TITLE_MAX_CHARS) : null;
}
...
function readCachedTitles(agentId, keysByItemId) {
	...
	try {
		const database = require_openclaw_agent_db.openOperatorAgentDatabase({ agentId });
		...
		const rows = require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, kysely.selectFrom("cache_entries").select(["key", "value_json"]).where("scope", "=", TOOL_TITLE_CACHE_SCOPE).where("key", "in", keys)).rows;
		...
		require_globals.logVerbose(`chat-tool-titles: cache read failed: ${String(err)}`);
	}
	return cached;
}
function writeCachedTitles(agentId, entries) {
	...
	try {
		require_openclaw_agent_db.runOperatorAgentWriteTransaction((database) => {
			const kysely = require_state_migrations_cron_run_logs.getNodeSqliteKysely(database.db);
			const now = Date.now();
			for (const [key, title] of entries) require_state_migrations_cron_run_logs.executeSqliteQuerySync(database.db, kysely.insertInto("cache_entries").values({
		...
		require_globals.logVerbose(`chat-tool-titles: cache write failed: ${String(err)}`);
	}
}
async function generateMissingTitles(params) {
	...
	let prepared;
	try {
		prepared = await require_simple_completion_runtime.prepareSimpleCompletionModelForAgent({
			cfg: params.cfg,
		...
		require_globals.logVerbose(`chat-tool-titles: model preparation failed: ${String(err)}`);
		return generated;
	}
	if ("error" in prepared) {
		require_globals.logVerbose(`chat-tool-titles: ${prepared.error}`);
		return generated;
	}
	const controller = new AbortController();
	const timeout = setTimeout(() => controller.abort(), TOOL_TITLES_TIMEOUT_MS);
	try {
		const result = await require_simple_completion_runtime.completeWithPreparedSimpleCompletionModel({
		...
		if (result.stopReason === "error") {
			require_globals.logVerbose(`chat-tool-titles: completion failed: ${result.errorMessage ?? "unknown error"}`);
			return generated;
		}
		...
		require_globals.logVerbose(`chat-tool-titles: completion failed: ${String(err)}`);
		return generated;
	} finally {
...
async function generateToolCallTitles(params) {
	...
	const resolvedRef = require_simple_completion_runtime.resolveUtilityModelRefForAgent({
		cfg: params.cfg,
		agentId: params.agentId,
		primaryProvider: params.sessionPrimaryProvider
	});
	if (!resolvedRef) {
		require_globals.logVerbose("chat-tool-titles: utility routing is disabled or has no derived small-model default; skipping titles");
		return {};
	}
	const strippedRef = require_model_ref_profile.splitTrailingAuthProfile(resolvedRef).model;
	if (!require_model_selection_normalize.parseModelRef(strippedRef, params.sessionPrimaryProvider?.trim() || "openrouter")) {
		require_globals.logVerbose(`chat-tool-titles: utility model ref ${JSON.stringify(resolvedRef)} is malformed; skipping titles`);
		return {};
	}
...
//#endregion
exports.generateToolCallTitles = generateToolCallTitles;
