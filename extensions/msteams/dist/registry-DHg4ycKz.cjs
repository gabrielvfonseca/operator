...
require("./web-provider-public-artifacts.explicit-BBLa7tXl.cjs");
...
function isProviderPlugin(value) {
	return (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value) && typeof value.id === "string" && typeof value.label === "string" && Array.isArray(value.auth);
}
...
function resolveBundledExplicitProviderContractsFromPublicArtifacts(params) {
	const providers = [];
	for (const pluginId of (0, _gabrielvfonseca_normalization_core_string_normalization.sortUniqueStrings)(params.onlyPluginIds)) {
...
function readJsonRecord(filePath) {
	const raw = (0, _openclaw_fs_safe_json.tryReadJsonSync)(filePath);
	return raw && typeof raw === "object" && !Array.isArray(raw) ? raw : void 0;
}
function readBundledCapabilityManifest(pluginDir) {
	if (require_bundled_plugin_metadata.normalizeBundledPluginStringList(require_manifest.getPackageManifestMetadata(readJsonRecord(node_path.default.join(pluginDir, "package.json")))?.extensions).length === 0) return;
	const raw = readJsonRecord(node_path.default.join(pluginDir, require_manifest.PLUGIN_MANIFEST_FILENAME));
	if (!(typeof raw?.id === "string" ? raw.id.trim() : "")) return;
	return raw;
}
function listBundledCapabilityManifests() {
	const scanDir = require_bundled_plugin_metadata.resolveBundledPluginScanDir({
		packageRoot: OPERATOR_PACKAGE_ROOT,
		runningFromBuiltArtifact: RUNNING_FROM_BUILT_ARTIFACT
	});
	if (!scanDir) return [];
	return node_fs.default.readdirSync(scanDir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).map((entry) => readBundledCapabilityManifest(node_path.default.join(scanDir, entry.name))).filter((manifest) => manifest !== void 0).toSorted((left, right) => left.id.localeCompare(right.id));
}
...
function loadScopedCapabilityRuntimeRegistryEntries(params) {
	const discovery = require_discovery.discoverOperatorPlugins({});
	let lastFailure;
	for (let attempt = 0; attempt < 2; attempt += 1) {
		const registry = require_bundled_capability_runtime.loadBundledCapabilityRuntimeRegistry({
		...
		if (!(attempt === 0 && (plugin?.status !== "loaded" || params.loadDeclaredIds(plugin).length === 0))) break;
	}
...
function resolveProviderContractPluginIdsForProviderAlias(providerId) {
	const normalizedProvider = (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(providerId);
	...
	const pluginIds = uniqueStrings(loadProviderContractEntriesForPluginIds(resolveBundledProviderContractPluginIds()).filter((entry) => {
		...
		].some((candidate) => (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(candidate) === normalizedProvider);
...
//#endregion
exports.resolveProviderContractPluginIdsForProviderAlias = resolveProviderContractPluginIdsForProviderAlias;
