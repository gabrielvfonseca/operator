...
const loadResolveManifestContractOwnerPluginId = require_lazy_runtime.createLazyRuntimeNamedExport(() => Promise.resolve().then(() => require("./runtime-web-tools-manifest.runtime-CLYH1ktH.cjs")), "resolveManifestContractOwnerPluginId");
function pushInactiveProviderCredentialWarnings(params) {
	for (const provider of params.selection.providers) {
		...
		for (const path of params.selection.inactivePathsForProvider(provider)) require_runtime_shared.pushInactiveSurfaceWarning({
...
function setResolvedCredentialPath(params) {
	...
	try {
		require_path_utils.setPathExistingStrict(params.resolvedConfig, pathSegments, params.value);
	} catch {}
}
/**
...
async function resolveRuntimeWebProviderSurface(params) {
	...
	if (params.rawProvider && !configuredProvider) {
		...
		require_runtime_shared.pushWarning(params.context, {
			code: params.invalidAutoDetectCode,
			path: params.providerPath,
	...
	return {
		providers,
		configuredProvider,
		enabled: hasConfiguredSurface && !invalidConfiguredProvider && (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(params.toolConfig) || params.toolConfig.enabled !== false),
...
async function resolveRuntimeWebProviderSelection(params) {
	...
	if (params.enabled) {
		...
		if (params.configuredProvider) {
			...
			if (!selectedProvider && unresolvedWithoutFallback.length > 0) failUnresolvedNoFallback((0, _gabrielvfonseca_normalization_core.expectDefined)(unresolvedWithoutFallback[0], "unresolved without fallback entry at 0"));
			if (selectedProvider) {
...
const loadRuntimeWebToolsFallbackProviders = require_lazy_runtime.createLazyRuntimeSurface(() => Promise.resolve().then(() => require("./runtime-web-tools-fallback.runtime-CefeYiAv.cjs")), ({ runtimeWebToolsFallbackProviders }) => runtimeWebToolsFallbackProviders);
const loadRuntimeWebToolsPublicArtifacts = require_lazy_runtime.createLazyRuntimeSurface(() => Promise.resolve().then(() => require("./runtime-web-tools-public-artifacts.runtime-DPWMTH25.cjs")), (mod) => mod);
const loadRuntimeWebToolsManifest = require_lazy_runtime.createLazyRuntimeSurface(() => Promise.resolve().then(() => require("./runtime-web-tools-manifest.runtime-CLYH1ktH.cjs")), (mod) => mod);
function needsRuntimeWebFetchProviderDiscovery(params) {
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(params.fetch) && params.fetch.enabled === false) return false;
	if (params.hasPluginWebFetchConfig) return true;
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(params.fetch)) return false;
	if (params.rawProvider) return true;
	return require_runtime_secret_scan.hasCredentialBearingObjectValue(params.fetch, params.defaults);
}
function hasPluginScopedWebToolConfig(config, key) {
	...
	return Object.values(entries).some((entry) => {
		...
		const pluginConfig = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(entry.config) ? entry.config : void 0;
...
function inferSingleBundledPluginScopedWebToolConfigOwner(config, key) {
	...
	for (const [pluginId, entry] of Object.entries(entries)) {
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(entry) || entry.enabled === false) continue;
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(entry.config) ? entry.config : void 0)?.[key])) continue;
...
function inferExactBundledPluginScopedWebToolConfigOwner(params) {
	...
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(entry) || entry.enabled === false) return;
	return (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(entry.config) ? entry.config : void 0)?.[params.key]) ? params.pluginId : void 0;
}
async function hasCustomWebProviderPluginRisk(params) {
	const installRecords = require_installed_plugin_index_record_reader.loadInstalledPluginIndexInstallRecordsSync({ env: params.env });
...
function readNonEmptyEnvValue(env, names) {
	for (const envVar of names) {
		const value = require_normalize_secret_input.normalizeSecretInput(env[envVar]);
		if (value) return {
			value,
			envVar
		};
...
async function resolveSecretInputWithEnvFallback(params) {
	...
	if (!ref) {
		const configValue = require_normalize_secret_input.normalizeSecretInput(params.value);
	...
	if (params.restrictEnvRefsToEnvVars === true && ref.source === "env" && !params.envVars.includes(ref.id)) unresolvedRefReason = `${params.path} SecretRef env var "${ref.id}" is not allowed.`;
		...
		if (typeof resolvedValue !== "string") unresolvedRefReason = buildUnresolvedReason({
			...
			resolvedFromRef = require_normalize_secret_input.normalizeSecretInput(resolvedValue);
...
async function resolveBundledWebSearchProviders(params) {
	...
	const onlyPluginIds = params.configuredBundledPluginId !== void 0 ? [params.configuredBundledPluginId] : params.onlyPluginIds && params.onlyPluginIds.length > 0 ? (0, _gabrielvfonseca_normalization_core_string_normalization.sortUniqueStrings)(params.onlyPluginIds) : void 0;
	if (onlyPluginIds && onlyPluginIds.length > 0) {
		const bundled = require_web_provider_public_artifacts_explicit.resolveBundledExplicitWebSearchProvidersFromPublicArtifacts({ onlyPluginIds });
...
async function resolveBundledWebFetchProviders(params) {
	...
	if (params.configuredBundledPluginId) {
		const bundled = require_web_provider_public_artifacts_explicit.resolveBundledExplicitWebFetchProvidersFromPublicArtifacts({ onlyPluginIds: [params.configuredBundledPluginId] });
		if (bundled && bundled.length > 0) return bundled;
...
async function resolveRuntimeWebTools(params) {
	...
	const sourceTools = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(params.sourceConfig.tools) ? params.sourceConfig.tools : void 0;
	const sourceWeb = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(sourceTools?.web) ? sourceTools.web : void 0;
	const resolvedTools = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(params.resolvedConfig.tools) ? params.resolvedConfig.tools : void 0;
	const resolvedWeb = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(resolvedTools?.web) ? resolvedTools.web : void 0;
	...
	const legacyXSearchSource = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(sourceWeb?.x_search) ? sourceWeb.x_search : void 0;
	const legacyXSearchResolved = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(resolvedWeb?.x_search) ? resolvedWeb.x_search : void 0;
	...
	const search = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(sourceWeb?.search) ? sourceWeb.search : void 0;
	const fetch = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(sourceWeb?.fetch) ? sourceWeb.fetch : void 0;
	...
	const rawProvider = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(search?.provider);
	...
	if (search || hasPluginWebSearchConfig) {
		const searchSurface = await resolveRuntimeWebProviderSurface({
			...
			sortProviders: require_web_search_providers_shared.sortWebSearchProvidersForAutoDetect,
	...
	const rawFetchProvider = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(fetch?.provider);
	const fetchMetadata = {
		providerSource: "none",
		diagnostics: []
	};
	if (needsRuntimeWebFetchProviderDiscovery({
		...
		const fetchSurface = await resolveRuntimeWebProviderSurface({
			...
			sortProviders: require_web_search_providers_shared.sortWebFetchProvidersForAutoDetect,
...
//#endregion
Object.defineProperty(exports, "resolveRuntimeWebTools", {
	enumerable: true,
	get: function() {
		return resolveRuntimeWebTools;
	}
});
