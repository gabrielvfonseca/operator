...
require("./node-commands-DQ3xnEUk.cjs");
...
function normalizeSystemRunTimeoutMs(value) {
	...
	return timeoutMs > 0 ? (0, _gabrielvfonseca_normalization_core_number_coercion.resolveTimerTimeoutMs)(timeoutMs, 1) : null;
}
const NODE_SKILL_NAME_RE = /^(?!.*--)[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?$/;
//#endregion
//#region src/gateway/node-skill-descriptors.ts
const log = require_subsystem.createSubsystemLogger("gateway/node-skills");
...
function normalizeSystemRunInvokeParams(params) {
	...
	const normalized = {
		...obj,
		runId: normalizeString(obj.runId) || (0, node_crypto.randomUUID)()
	};
	const timeoutMs = normalizeSystemRunTimeoutMs(obj.timeoutMs);
...
const SERIALIZED_EVENT_PAYLOAD = Symbol("operator.serializedEventPayload");
...
var NodeRegistry = class {
	...
	normalizePluginToolDescriptors(params) {
		return require_node_plugin_tool_snapshot.normalizeNodePluginToolDescriptors({
			...params,
			enabled: this.options.nodePluginToolsEnabled,
			registeredDescriptors: require_node_plugin_tool_snapshot.createRegisteredNodePluginToolDescriptorMap(this.options.listRegisteredNodePluginToolCommands?.())
		});
	}
	replaceEffectiveNodePluginTools(node) {
		...
		require_node_plugin_tool_snapshot.replaceConnectedNodePluginTools({
			nodeId: node.nodeId,
	...
	unregister(connId) {
		...
		if (unregistersCurrentNode) {
			...
			require_node_plugin_tool_snapshot.removeConnectedNodePluginTools(nodeId);
	...
	publishActiveNodeContext() {
		...
		require_active_node_context.setActiveNodeContext(active ? { nodeId: active.nodeId } : null);
	}
	...
	async invoke(params) {
		...
		const timeoutMs = (0, _gabrielvfonseca_normalization_core_number_coercion.resolveTimerTimeoutMs)(params.timeoutMs, 3e4, 0);
		const payload = {
			id: requestId,
		...
		const result = new Promise((resolve, reject) => {
			...
			const idleTimeoutMs = (0, _gabrielvfonseca_normalization_core_number_coercion.resolveTimerTimeoutMs)(params.idleTimeoutMs, 0, 0);
	...
	authorizedSystemRunEventExpiresAt(timeoutMs) {
		...
		return (0, _gabrielvfonseca_normalization_core_number_coercion.resolveExpiresAtMsFromDurationMs)((0, _gabrielvfonseca_normalization_core_number_coercion.addTimerTimeoutGraceMs)(timeoutMs, AUTHORIZED_SYSTEM_RUN_EVENT_GRACE_MS)) ?? 0;
	}
	...
	allowsLegacyMacRunIdFallback(params) {
		const node = this.nodesById.get(params.nodeId);
		return node?.connId === params.connId && node.clientId === "operator-macos" && node.platform === "darwin";
	}
	pruneAuthorizedSystemRunEvents(now = Date.now()) {
		for (const [key, event] of this.authorizedSystemRunEvents) if (event.expiresAtMs !== null && !(0, _gabrielvfonseca_normalization_core_number_coercion.isFutureDateTimestampMs)(event.expiresAtMs, { nowMs: now })) this.authorizedSystemRunEvents.delete(key);
	}
...
Object.defineProperty(exports, "serializeEventPayload", {
	enumerable: true,
	get: function() {
