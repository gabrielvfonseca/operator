...
var service_audit_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	...
	readEmbeddedGatewayToken: () => readEmbeddedGatewayToken
});
...
const POSIX_SERVICE_SHELL_WRAPPERS = require_shell_wrapper_resolution.POSIX_SHELL_WRAPPERS;
function isOpaquePosixShellInlineCommand(programArguments) {
	const executable = programArguments[0]?.trim();
	const shellName = executable ? node_path.default.posix.basename(executable).toLowerCase() : "";
	if (!POSIX_SERVICE_SHELL_WRAPPERS.has(shellName)) return false;
	return require_shell_wrapper_resolution.resolveInlineCommandMatch(programArguments, POSIX_SERVICE_INLINE_COMMAND_FLAGS, { allowCombinedC: true }).command !== null;
}
...
async function auditSystemdUnit(env, issues) {
	const unitPath = require_systemd.resolveSystemdUserUnitPath(env);
	let content;
	try {
		content = await node_fs_promises.default.readFile(unitPath, "utf8");
	} catch {
		return;
	}
	...
	const killMode = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(parsed.killMode);
...
async function auditLaunchdPlist(env, issues) {
	const plistPath = require_launchd.resolveLaunchAgentPlistPath(env);
	let content;
	try {
		content = await node_fs_promises.default.readFile(plistPath, "utf8");
	} catch {
		return;
	}
...
function parseGatewayPortArg(value) {
	...
	const port = require_paths.parseTcpPort(raw);
	if (port !== null) return {
		kind: "valid",
		port
	};
...
function auditGatewayToken(command, issues, expectedGatewayToken) {
	...
	const expectedToken = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(expectedGatewayToken);
	if (!expectedToken || serviceToken === expectedToken) return;
	issues.push({
		code: SERVICE_AUDIT_CODES.gatewayTokenMismatch,
		message: "Gateway service OPERATOR_GATEWAY_TOKEN does not match gateway.auth.token in operator.json",
		detail: "service token is stale",
		level: "recommended"
	});
}
function auditManagedServiceEnvironment(command, issues, expectedManagedServiceEnvKeys) {
	const inlineKeys = require_systemd.collectInlineManagedServiceEnvKeys(command, expectedManagedServiceEnvKeys);
...
const SERVICE_PROXY_ENV_KEY_SET = new Set(require_service_path_policy.SERVICE_PROXY_ENV_KEYS.flatMap((key) => {
...
function collectInlineProxyEnvKeys(command) {
	...
	for (const [rawKey, value] of Object.entries(command.environment)) {
		...
		if (!require_systemd.hasInlineEnvironmentSource(readEnvironmentValueSource(command, normalized))) continue;
...
function readEmbeddedGatewayToken(command) {
	if (!command) return;
	if (require_systemd.isEnvironmentFileOnlySource(command.environmentValueSources?.OPERATOR_GATEWAY_TOKEN)) return;
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(command.environment?.OPERATOR_GATEWAY_TOKEN);
}
...
function getEquivalentMinimalPathEntries(entry, platform, normalizedExpected) {
	...
	const normalizedEquivalent = require_service_path_policy.normalizeServicePathEntry(equivalent, platform);
...
function auditGatewayServicePath(command, issues, env, platform, expectedServicePath) {
	...
	const expected = expectedServicePath?.trim() ? (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(expectedServicePath.split(getPathModule(platform).delimiter)) : require_service_path_policy.getMinimalServicePathPartsFromEnv({
		platform,
		env,
		includeMissingUserBinDefaults: false
	});
	const parts = (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(servicePath.split(getPathModule(platform).delimiter));
	const normalizedParts = new Set(parts.map((entry) => require_service_path_policy.normalizeServicePathEntry(entry, platform)));
	const normalizedExpected = new Set(expected.map((entry) => require_service_path_policy.normalizeServicePathEntry(entry, platform)));
	const missing = expected.filter((entry) => {
		const normalized = require_service_path_policy.normalizeServicePathEntry(entry, platform);
		if (normalizedParts.has(normalized)) return false;
		return !getEquivalentMinimalPathEntries(entry, platform, normalizedExpected).some((equivalent) => normalizedParts.has(require_service_path_policy.normalizeServicePathEntry(equivalent, platform)));
	});
	...
	const nonMinimal = parts.filter((entry) => {
		...
		return require_service_path_policy.isNonMinimalServicePathEntry(normalized, platform);
	});
...
async function auditGatewayRuntime(env, command, issues, platform) {
	...
	if (!execPath) return;
	if (require_redact.isBunRuntime(execPath)) {
		issues.push({
	...
	if (!require_redact.isNodeRuntime(execPath)) return;
	if (require_service_path_policy.isVersionManagedNodePath(execPath, platform)) {
		issues.push({
		...
		if (!require_service_path_policy.isSystemNodePath(execPath, env, platform)) {
			if (!await require_service_path_policy.resolveSystemNodePath(env, platform)) issues.push({
...
function checkTokenDrift(params) {
	const serviceToken = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.serviceToken);
	const configToken = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.configToken);
...
function auditGatewayServiceVersion(command, issues) {
	...
	if (!serviceVersion || serviceVersion === require_version.VERSION) return;
	issues.push({
		code: SERVICE_AUDIT_CODES.gatewayServiceVersionMismatch,
		message: `Gateway service was installed by Operator ${serviceVersion}; current CLI is ${require_version.VERSION}.`,
...
//#endregion
Object.defineProperty(exports, "SERVICE_AUDIT_CODES", {
	enumerable: true,
	get: function() {
		return SERVICE_AUDIT_CODES;
	}
});
Object.defineProperty(exports, "auditGatewayServiceConfig", {
	enumerable: true,
	get: function() {
		return auditGatewayServiceConfig;
	}
});
Object.defineProperty(exports, "checkTokenDrift", {
	enumerable: true,
	get: function() {
		return checkTokenDrift;
	}
});
Object.defineProperty(exports, "needsNodeRuntimeMigration", {
	enumerable: true,
	get: function() {
		return needsNodeRuntimeMigration;
	}
});
Object.defineProperty(exports, "readEmbeddedGatewayToken", {
	enumerable: true,
	get: function() {
		return readEmbeddedGatewayToken;
	}
});
Object.defineProperty(exports, "service_audit_exports", {
	enumerable: true,
	get: function() {
		return service_audit_exports;
	}
});
