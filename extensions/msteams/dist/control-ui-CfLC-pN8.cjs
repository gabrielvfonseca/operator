...
function githubApiHeaders(token) {
	const headers = {
		...
		"User-Agent": "Operator-Control-UI",
...
async function readBoundedResponse(response, maxBytes) {
	try {
		return await require_http_body.readResponseWithLimit(response, maxBytes);
	} finally {
		await discardResponse(response);
	}
}
...
async function assertPublicRepositoryUrl(repositoryUrl, fetchImpl, token) {
	...
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(parsed) || parsed.private !== false) throw new ControlUiGitHubError(404, "GitHub repository is not public");
}
...
function previewRepositoryApiUrl(target, value) {
	...
	const base = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value.base) ? value.base : {};
	return requiredString((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(base.repo) ? base.repo : {}, "url");
}
function parseGitHubResponse(target, value) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value)) throw new ControlUiGitHubError(502, "GitHub response was not an object");
	const user = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value.user) ? value.user : {};
	return {
		preview: {
			...target,
...
async function fetchPreview(target, fetchImpl, token) {
	...
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(parsed)) throw new ControlUiGitHubError(502, "GitHub response was not an object");
...
async function gitOutput(cwd, args) {
	try {
		const result = await require_git.runGit(cwd, args);
...
async function resolveSessionPullRequestGitContext(params) {
	const { cfg, entry, storePath, canonicalKey } = require_session_utils.loadSessionEntry(params.sessionKey, { agentId: params.agentId });
	if (!entry?.sessionId || !storePath) return null;
	const agentId = (0, _gabrielvfonseca_normalization_core_agent_id.normalizeAgentId)(require_session_key.parseAgentSessionKey(canonicalKey)?.agentId ?? params.agentId ?? require_session_key.parseAgentSessionKey(params.sessionKey)?.agentId ?? require_agent_scope_config.resolveDefaultAgentId(cfg));
	const root = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(entry.spawnedCwd) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(entry.spawnedWorkspaceDir) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(require_agent_scope_config.resolveAgentWorkspaceDir(cfg, agentId));
	if (!root) return null;
...
async function untrackedFileAdditions(root, filePath) {
	try {
		const abs = node_path.default.resolve(root, filePath);
		const info = await node_fs_promises.default.lstat(abs);
		if (!info.isFile() || info.size === 0 || info.size > MAX_UNTRACKED_STAT_BYTES) return 0;
		const body = await node_fs_promises.default.readFile(abs);
...
async function loadBranchDiffStats(root, defaultBranch) {
	...
	try {
		const result = await require_git.runGit(root, [
			"diff",
			"--shortstat",
			"--no-ext-diff",
			"--no-textconv",
...
function parsePullListItem(value) {
	...
	const base = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value.base) ? value.base : {};
	const baseRepo = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(base.repo) ? base.repo : {};
	const owner = optionalString((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(baseRepo.owner) ? baseRepo.owner : {}, "login");
	const repo = optionalString(baseRepo, "name");
	const head = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value.head) ? value.head : {};
...
async function fetchParentRepo(owner, repo, fetchImpl, token) {
	...
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value) || value.fork !== true || !(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value.parent)) return null;
	const parentLogin = optionalString((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value.parent.owner) ? value.parent.owner : {}, "login");
...
function rollupCheckRuns(value) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value) || !Array.isArray(value.check_runs) || value.check_runs.length === 0) return;
	let passed = 0;
	let failed = 0;
	let skipped = 0;
	let running = 0;
	for (const runValue of value.check_runs) {
		const run = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(runValue) ? runValue : {};
...
async function loadControlUiSessionPullRequests(params, deps = {}) {
	...
	const [branch, snapshot] = await Promise.all([resolveSessionBranch(context), cachedBranchPullRequests(context, deps)]);
	return branch ? {
		...snapshot,
		branch
	} : snapshot;
}
function cachedBranchPullRequests(context, deps) {
...
function createControlUiHandlers(loadGitHubPreview = loadControlUiGitHubPreview, loadSessionPullRequests = loadControlUiSessionPullRequests) {
	return {
		"controlUi.githubPreview": async ({ params, respond }) => {
			...
			if (!target) {
				respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, "invalid controlUi.githubPreview params"));
				return;
			}
			try {
				...
				respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.UNAVAILABLE, "GitHub preview unavailable", { retryable: statusCode === 429 || statusCode === 502 }));
			}
		},
		"controlUi.sessionPullRequests": async ({ params, respond }) => {
			const parsed = parseControlUiSessionPullRequestsParams(params);
			if (!parsed) {
				respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, "invalid controlUi.sessionPullRequests params"));
				return;
			}
			try {
				...
				const statusCode = error instanceof ControlUiGitHubError ? error.statusCode : void 0;
				respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.UNAVAILABLE, "session pull requests unavailable", { retryable: statusCode === 429 || statusCode === 502 }));
			}
		}
	};
}
...
//#endregion
exports.controlUiHandlers = controlUiHandlers;
exports.createControlUiHandlers = createControlUiHandlers;
