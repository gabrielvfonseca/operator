...
* returning control to the caller (typically `triggerOperatorRestart` →
...
function readParentPidFromProc(pid) {
	try {
		const match = (0, node_fs.readFileSync)(`/proc/${pid}/status`, "utf8").match(/^PPid:\s*(\d+)/m);
...
function readParentPidFromPs(pid, spawnTimeoutMs) {
	try {
		const res = (0, node_child_process.spawnSync)("ps", [
			"-o",
			"ppid=",
			"-p",
			String(pid)
		...
		return require_parse_finite_number.parseStrictPositiveInteger(res.stdout.trim()) ?? null;
	} catch {
		return null;
	}
}
...
function parseLsofEntries(stdout) {
	...
	for (const line of stdout.split(/\r?\n/).filter(Boolean)) if (line.startsWith("p")) {
		flush();
		currentPid = require_parse_finite_number.parseStrictPositiveInteger(line.slice(1)) ?? void 0;
		currentCmd = void 0;
...
function readUnixProcessArgsSync(pid, spawnTimeoutMs) {
	if (process.platform === "linux") try {
		const args = require_windows_port_pids.parseProcCmdline((0, node_fs.readFileSync)(`/proc/${pid}/cmdline`, "utf8"));
...
function parsePidsFromLsofOutput(stdout, spawnTimeoutMs, protectedPid) {
	...
	for (const entry of parseLsofEntries(stdout)) {
		...
		if (entry.cmd && (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(entry.cmd).includes("@gabrielvfonseca/operator")) {
			pids.push(entry.pid);
			continue;
		}
		if (verifyGatewayPidByArgvSync(entry.pid, spawnTimeoutMs)) pids.push(entry.pid);
	}
	return (0, _gabrielvfonseca_normalization_core_string_normalization.uniqueValues)(pids);
}
/**
...
function filterVerifiedWindowsGatewayPids(rawPids, protectedPid) {
	...
	return (0, _gabrielvfonseca_normalization_core_string_normalization.uniqueValues)(rawPids).filter((pid) => Number.isFinite(pid) && pid > 0 && !excluded.has(pid)).filter((pid) => {
		const args = require_windows_port_pids.readWindowsProcessArgsSync(pid);
		return args != null && require_windows_port_pids.isGatewayArgv(args, { allowGatewayBinary: true });
	});
}
function filterVerifiedWindowsGatewayPidsResult(rawPids, processArgsResult, protectedPid) {
	const excluded = getExcludedGatewayPidsSync(SPAWN_TIMEOUT_MS, protectedPid);
	const verified = [];
	for (const pid of (0, _gabrielvfonseca_normalization_core_string_normalization.uniqueValues)(rawPids)) {
		...
		if (argsResult.args != null && require_windows_port_pids.isGatewayArgv(argsResult.args, { allowGatewayBinary: true })) verified.push(pid);
	}
	return {
		ok: true,
		pids: verified
...
function findVerifiedWindowsGatewayPidsOnPortSync(port, protectedPid) {
	return filterVerifiedWindowsGatewayPids(require_windows_port_pids.readWindowsListeningPidsOnPortSync(port), protectedPid);
}
function findVerifiedWindowsGatewayPidsOnPortResultSync(port, protectedPid) {
	const result = require_windows_port_pids.readWindowsListeningPidsResultSync(port);
	if (!result.ok) return result;
	return filterVerifiedWindowsGatewayPidsResult(result.pids, (pid) => require_windows_port_pids.readWindowsProcessArgsResultSync(pid), protectedPid);
}
function findGatewayPidsOnPortWithProtectedPidSync(port, spawnTimeoutMs, protectedPid) {
	...
	const res = (0, node_child_process.spawnSync)(require_ports_lsof.resolveLsofCommandSync(), [
		"-nP",
		`-iTCP:${port}`,
		"-sTCP:LISTEN",
		"-Fpc"
...
function pollPortOnceWindows(port) {
	try {
		const result = require_windows_port_pids.readWindowsListeningPidsResultSync(port, POLL_SPAWN_TIMEOUT_MS);
		if (!result.ok) return {
			free: null,
...
function terminateStaleProcessesWindows(pids) {
	const taskkillPath = node_path.default.win32.join(require_windows_install_roots.getWindowsInstallRoots().systemRoot, "System32", "taskkill.exe");
	const killed = [];
...
function cleanStaleGatewayProcessesSync(portOverride, options) {
	try {
		const port = typeof portOverride === "number" && Number.isFinite(portOverride) && portOverride > 0 ? Math.floor(portOverride) : require_paths.resolveGatewayPort(void 0, process.env);
...
//#endregion
Object.defineProperty(exports, "cleanStaleGatewayProcessesSync", {
	enumerable: true,
	get: function() {
		return cleanStaleGatewayProcessesSync;
	}
});
Object.defineProperty(exports, "findGatewayPidsOnPortSync", {
	enumerable: true,
	get: function() {
		return findGatewayPidsOnPortSync;
	}
});
