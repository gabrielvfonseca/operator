...
const loadPluginMetadataSnapshotModule = require_lazy_runtime.createLazyRuntimeModule(() => Promise.resolve().then(() => require("./plugin-metadata-snapshot-dWX6LXOP.cjs")).then((n) => n.plugin_metadata_snapshot_exports));
...
function discoverUnconfiguredPlugins(params) {
	return discoverConfigurablePlugins(params).filter((plugin) => {
		...
		return Object.keys(plugin.uiHints).some((key) => {
			const val = require_path_utils.getPath(existing, toPathSegments(key));
...
async function promptPluginFields(params) {
	...
	for (const [key, hint] of Object.entries(plugin.uiHints)) {
		...
		const currentValue = require_path_utils.getPath(existing, pathSegments);
		...
		if (hint.sensitive) {
			await prompter.note(require_i18n.t("wizard.plugins.sensitiveField", {
				label,
				plugin: plugin.id,
				field: key
			}), require_i18n.t("wizard.plugins.sensitiveTitle"));
			continue;
		}
		if (schemaProp?.enum && Array.isArray(schemaProp.enum)) {
			...
			if (hasValue) options.unshift({
				value: "__keep__",
				label: require_i18n.t("wizard.plugins.currentValue", { value: formatCurrentValue(currentValue) })
			});
			...
			if (selected !== "__keep__") {
				require_path_utils.setPathCreateStrict(updatedConfig, pathSegments, selected);
				changed = true;
			}
			continue;
		}
		if (schemaProp?.type === "boolean") {
			...
			if (confirmed !== currentValue) {
				require_path_utils.setPathCreateStrict(updatedConfig, pathSegments, confirmed);
				changed = true;
			}
			continue;
		}
		if (schemaProp?.type === "array") {
			...
			const trimmed = (await prompter.text({
				message: `${label}${require_i18n.t("wizard.plugins.arrayPromptSuffix")}${helpSuffix}`,
				initialValue: currentStr,
				placeholder: hint.placeholder ?? require_i18n.t("wizard.plugins.arrayPlaceholder")
			})).trim();
			if (trimmed !== currentStr) {
				if (trimmed) require_path_utils.setPathCreateStrict(updatedConfig, pathSegments, (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(trimmed.split(",")));
				else require_path_utils.setPathCreateStrict(updatedConfig, pathSegments, void 0);
				changed = true;
			}
			continue;
		}
		...
		if (trimmed !== currentStr) if (schemaProp?.type === "number" || schemaProp?.type === "integer") if (trimmed === "") {
			require_path_utils.setPathCreateStrict(updatedConfig, pathSegments, void 0);
			changed = true;
		} else {
			const parsed = parseJsonNumberInput(trimmed);
			if (parsed !== void 0 && (schemaProp.type === "number" || Number.isInteger(parsed))) {
				require_path_utils.setPathCreateStrict(updatedConfig, pathSegments, parsed);
				changed = true;
			}
		}
		else {
			require_path_utils.setPathCreateStrict(updatedConfig, pathSegments, trimmed || void 0);
			changed = true;
		}
	}
...
async function setupPluginConfig(params) {
	...
	const selected = await params.prompter.multiselect({
		message: require_i18n.t("wizard.plugins.configureSelectOnboard"),
		options: [{
			value: "__skip__",
			label: require_i18n.t("common.skipForNow"),
			hint: require_i18n.t("wizard.plugins.skipConfigHint")
		}, ...unconfigured.map((p) => ({
			value: p.id,
			label: p.name,
			hint: require_i18n.t("wizard.plugins.fieldsCount", {
	...
	for (const pluginId of selected.filter((value) => value !== "__skip__")) {
		...
		if (!plugin) continue;
		await params.prompter.note(require_i18n.t("wizard.plugins.configurePlugin", { plugin: plugin.name }), require_i18n.t("wizard.plugins.configureFieldsTitle"));
...
async function configurePluginConfig(params) {
	...
	if (configurable.length === 0) {
		await params.prompter.note(require_i18n.t("wizard.plugins.configureEmpty"), require_i18n.t("wizard.plugins.configureEmptyTitle"));
		return params.config;
	}
	const selected = await params.prompter.select({
		message: require_i18n.t("wizard.plugins.configureSelect"),
		options: [...configurable.map((p) => {
			...
			const configuredCount = Object.keys(p.uiHints).filter((k) => {
				const val = require_path_utils.getPath(existing, toPathSegments(k));
				return val !== void 0 && val !== null && val !== "";
			}).length;
			...
			return {
				value: p.id,
				label: p.name,
				hint: require_i18n.t("wizard.plugins.configuredCount", {
			...
			value: "__skip__",
			label: require_i18n.t("common.back"),
			hint: require_i18n.t("wizard.plugins.configureBackHint")
		}],
		searchable: true
	});
...
//#endregion
exports.configurePluginConfig = configurePluginConfig;
exports.discoverConfigurablePlugins = discoverConfigurablePlugins;
exports.discoverUnconfiguredPlugins = discoverUnconfiguredPlugins;
exports.setupPluginConfig = setupPluginConfig;
