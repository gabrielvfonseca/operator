...
function resolveProviderChunkContext(cfg, provider, accountId) {
	const providerKey = provider ? require_message_channel.normalizeMessageChannel(provider) : void 0;
	const providerId = providerKey ? require_registry.normalizeChannelId(providerKey) : null;
	return {
		providerKey,
		providerId,
		textLimit: require_chunk.resolveTextChunkLimit(cfg, providerKey, accountId, { fallbackLimit: providerId ? require_registry.getChannelPlugin(providerId)?.outbound?.textChunkLimit : void 0 })
	};
}
function resolveScopedBlockStreamingCoalesce(config) {
	return config ? require_chunk.resolveChannelStreamingBlockCoalesce(config) : void 0;
}
function resolveProviderBlockStreamingCoalesce(params) {
	...
	const accountCfg = require_account_lookup.resolveAccountEntry(typed.accounts, normalizedAccountId);
...
function resolveBlockStreamingChunking(cfg, provider, accountId) {
	...
	const chunkMode = require_chunk.resolveChunkMode(cfg, providerKey, accountId);
...
function resolveBlockStreamingCoalescing(cfg, provider, accountId, chunking) {
	...
	const providerDefaults = providerId ? require_registry.getChannelPlugin(providerId)?.streaming?.blockStreamingCoalesceDefaults : void 0;
...
//#endregion
Object.defineProperty(exports, "clampPositiveInteger", {
	enumerable: true,
	get: function() {
		return clampPositiveInteger;
	}
});
Object.defineProperty(exports, "resolveBlockStreamingChunking", {
	enumerable: true,
	get: function() {
		return resolveBlockStreamingChunking;
	}
});
Object.defineProperty(exports, "resolveEffectiveBlockStreamingConfig", {
	enumerable: true,
	get: function() {
