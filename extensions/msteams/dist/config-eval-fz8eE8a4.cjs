...
function resolveConfigPath(config, pathStr) {
	...
	for (const part of parts) {
		...
		if (require_prototype_keys.isBlockedObjectKey(part)) return;
		current = current[part];
	}
	return current;
}
function hasBlockedConfigPathSegment(pathStr) {
	return pathStr.split(".").filter(Boolean).some((part) => require_prototype_keys.isBlockedObjectKey(part));
}
...
function hasBinary(bin) {
	...
	const parts = pathEnv.split(node_path.default.delimiter).filter(Boolean);
	const extensions = process.platform === "win32" ? windowsPathExtensions() : [""];
	for (const part of parts) for (const ext of extensions) {
		const candidate = node_path.default.join(part, bin + ext);
		try {
...
//#endregion
Object.defineProperty(exports, "evaluateRuntimeEligibility", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "isConfigPathTruthyWithDefaults", {
	enumerable: true,
	get: function() {
		return isConfigPathTruthyWithDefaults;
	}
});
