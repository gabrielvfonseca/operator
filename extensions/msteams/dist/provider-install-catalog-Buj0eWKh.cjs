...
const require_model_catalog = require("./model-catalog-BgqTA2hC.cjs");
...
var provider_install_catalog_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	resolveDeprecatedProviderInstallCatalogEntry: () => resolveDeprecatedProviderInstallCatalogEntry,
	resolveProviderInstallCatalogEntries: () => resolveProviderInstallCatalogEntries,
	resolveProviderInstallCatalogEntry: () => resolveProviderInstallCatalogEntry
});
...
function resolveInstallInfoFromPackageSource(params) {
	const source = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(params.source) ? params.source : void 0;
	const npm = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(source?.npm) ? source.npm : void 0;
	const clawhub = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(source?.clawhub) ? source.clawhub : void 0;
	const local = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(source?.local) ? source.local : void 0;
	const npmSpec = params.origin === "bundled" || params.origin === "config" ? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(npm?.spec) : void 0;
	const clawhubSpec = params.origin === "bundled" || params.origin === "config" ? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(clawhub?.spec) : void 0;
	const localPath = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(local?.path);
	if (!clawhubSpec && !npmSpec && !localPath) return null;
	const defaultChoice = normalizeDefaultChoice(source?.defaultChoice);
	const expectedIntegrity = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(npm?.expectedIntegrity);
	return {
...
function resolvePreferredInstallsByPluginId(params) {
	...
	const index = require_plugin_registry.loadPluginRegistrySnapshot({
	...
	for (const record of index.plugins) {
		if (record.origin === "workspace" && params.includeUntrustedWorkspacePlugins === false && !require_config_state.resolveEffectiveEnableState({
			id: record.pluginId,
...
function resolveProviderIndexInstallCatalogEntries(params) {
	const entries = [];
	const index = require_model_catalog.loadOperatorProviderIndex();
	for (const provider of Object.values(index.providers)) {
		...
		for (const choice of provider.authChoices ?? []) {
			...
			entries.push({
				...
				origin: "bundled",
				install,
				installSource: require_installed_plugin_index.describePluginInstallSource(install, { expectedPackageName: provider.plugin.package })
			});
		}
	}
	return entries;
...
function resolveOfficialExternalProviderInstallCatalogEntries(params) {
	const entries = [];
	for (const entry of require_official_external_plugin_catalog.listOfficialExternalProviderCatalogEntries()) {
		...
		for (const provider of manifest?.providers ?? []) {
			...
			for (const choice of provider.authChoices ?? []) {
				...
				entries.push({
					...
					origin: "bundled",
					install,
					installSource: require_installed_plugin_index.describePluginInstallSource(install, { expectedPackageName: entry.name })
				});
			}
		}
	}
...
function resolveProviderInstallCatalogEntries(params) {
	...
	const manifestEntries = require_provider_auth_choices.resolveManifestProviderAuthChoices(params).flatMap((choice) => {
		...
		return [{
			...
			installSource: require_installed_plugin_index.describePluginInstallSource(install.install, { expectedPackageName: install.packageName })
		}];
...
//#endregion
Object.defineProperty(exports, "provider_install_catalog_exports", {
	enumerable: true,
	get: function() {
		return provider_install_catalog_exports;
	}
});
Object.defineProperty(exports, "resolveProviderInstallCatalogEntries", {
	enumerable: true,
	get: function() {
		return resolveProviderInstallCatalogEntries;
	}
});
Object.defineProperty(exports, "resolveProviderInstallCatalogEntry", {
	enumerable: true,
	get: function() {
		return resolveProviderInstallCatalogEntry;
	}
});
