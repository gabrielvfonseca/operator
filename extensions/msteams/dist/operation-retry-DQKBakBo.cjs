...
function hasProviderRetryableNetworkCode(value) {
	return require_retryable_network_errors.hasRetryableConnectionErrorCode(value) || PROVIDER_RETRYABLE_DNS_ERROR_CODE_RE.test(value);
}
function hasTransientNetworkSignal(error, message) {
	...
	return hasProviderRetryableNetworkCode(require_errors.formatErrorMessage(cause));
}
function hasTimeoutSignal(error, message) {
	...
	return /\b(?:request timeout|provider timeout|timed out|timeout)\b/i.test(require_errors.formatErrorMessage(cause));
}
...
async function executeProviderOperationWithRetry(params) {
	...
	for (let attemptNumber = 1; attemptNumber <= maxAttempts; attemptNumber += 1) try {
		...
		await (retryOptions.sleep ?? require_src.sleepWithAbort)(delayMs, retryOptions.signal);
	}
	throw lastError;
}
//#endregion
Object.defineProperty(exports, "executeProviderOperationWithRetry", {
	enumerable: true,
	get: function() {
		return executeProviderOperationWithRetry;
	}
});
Object.defineProperty(exports, "providerOperationRetryConfig", {
	enumerable: true,
	get: function() {
		return providerOperationRetryConfig;
	}
});
Object.defineProperty(exports, "resolveTransientProviderAttempts", {
	enumerable: true,
	get: function() {
		return resolveTransientProviderAttempts;
	}
});
Object.defineProperty(exports, "resolveTransientProviderDelayMs", {
	enumerable: true,
	get: function() {
		return resolveTransientProviderDelayMs;
	}
});
Object.defineProperty(exports, "resolveTransientProviderRetryOptions", {
	enumerable: true,
	get: function() {
		return resolveTransientProviderRetryOptions;
	}
});
Object.defineProperty(exports, "shouldRetrySameKeyProviderOperation", {
	enumerable: true,
	get: function() {
