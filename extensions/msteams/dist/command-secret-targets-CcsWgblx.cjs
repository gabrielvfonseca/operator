...
var command_secret_targets_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	getAgentRuntimeCommandSecretTargetIds: () => getAgentRuntimeCommandSecretTargetIds,
	getCapabilityWebFetchCommandSecretTargetIds: () => getCapabilityWebFetchCommandSecretTargetIds,
	getCapabilityWebFetchCommandSecretTargets: () => getCapabilityWebFetchCommandSecretTargets,
	getCapabilityWebSearchCommandSecretTargetIds: () => getCapabilityWebSearchCommandSecretTargetIds,
	getCapabilityWebSearchCommandSecretTargets: () => getCapabilityWebSearchCommandSecretTargets,
	getChannelsCommandSecretTargetIds: () => getChannelsCommandSecretTargetIds,
	getConfiguredChannelsCommandSecretTargetIds: () => getConfiguredChannelsCommandSecretTargetIds,
	getMemoryEmbeddingCommandSecretTargetIds: () => getMemoryEmbeddingCommandSecretTargetIds,
	getModelsCommandSecretTargetIds: () => getModelsCommandSecretTargetIds,
	getQrRemoteCommandSecretTargetIds: () => getQrRemoteCommandSecretTargetIds,
	getScopedChannelsCommandSecretTargets: () => getScopedChannelsCommandSecretTargets,
	getSecurityAuditCommandSecretTargetIds: () => getSecurityAuditCommandSecretTargetIds,
	getStatusCommandSecretTargetIds: () => getStatusCommandSecretTargetIds,
	getTtsCommandSecretTargetIds: () => getTtsCommandSecretTargetIds
});
...
function idsByPrefix(prefixes) {
	return require_target_registry.listSecretTargetRegistryEntries().map((entry) => entry.id).filter((id) => prefixes.some((prefix) => id.startsWith(prefix))).toSorted();
}
...
function getCapabilityWebSearchTargetIds() {
	cachedCapabilityWebSearchTargetIds ??= (0, _gabrielvfonseca_normalization_core_string_normalization.sortUniqueStrings)([...STATIC_CAPABILITY_WEB_SEARCH_TARGET_IDS, ...require_target_registry.listSecretTargetRegistryEntries().map((entry) => entry.id).filter(isPluginWebSearchCredentialTargetId)]);
	return cachedCapabilityWebSearchTargetIds;
}
function getCapabilityWebFetchTargetIds() {
	cachedCapabilityWebFetchTargetIds ??= (0, _gabrielvfonseca_normalization_core_string_normalization.sortUniqueStrings)([...STATIC_CAPABILITY_WEB_FETCH_TARGET_IDS, ...require_target_registry.listSecretTargetRegistryEntries().map((entry) => entry.id).filter(isPluginWebFetchCredentialTargetId)]);
...
function targetIdsForConfigPath(path) {
	return require_target_registry.listSecretTargetRegistryEntries().filter((entry) => pathPatternMatchesConcretePath(entry.pathPattern ?? entry.id, path)).map((entry) => entry.id).toSorted();
}
...
function addConfiguredConfigPathTargets(params) {
	const targetIds = targetIdsForConfigPath(params.path);
	if (targetIds.length === 0) return false;
	if (!require_target_registry.discoverConfigSecretTargetsByIds(params.config, toTargetIdSet(targetIds)).some((target) => target.path === params.path)) return false;
...
function discoverForcedActivePaths(config, targetIds, allowedPaths) {
	...
	for (const target of require_target_registry.discoverConfigSecretTargetsByIds(config, targetIds)) {
...
function discoverConfiguredAllowedPaths(config, targetIds) {
	...
	for (const target of require_target_registry.discoverConfigSecretTargetsByIds(config, targetIds)) allowedPaths.add(target.path);
...
function addConfiguredSearchCredentialTargetIds(params) {
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(scopedConfig) && configuredCredential === scopedConfig.apiKey) addConfigPathTargets({
		...params,
...
function getCapabilityWebSearchSelectedProviderTargetIds(config, providerId) {
	...
	const providers = require_web_search_providers_runtime.resolvePluginWebSearchProviders({ config: withSelectedWebProviderForDiscovery(config, "search", normalizeProviderId(providerId)) }).filter((provider) => provider.id === selectedProviderId);
...
function getCapabilityWebFetchSelectedProviderTargetIds(config, providerId) {
	...
	if (!selectedProviderId) return emptySelectedProviderTargetIds();
	const state = createSelectedProviderTargetState();
	const providers = require_web_fetch_providers_runtime.resolvePluginWebFetchProviders({ config: withSelectedWebProviderForDiscovery(config, "fetch", normalizeProviderId(providerId)) }).filter((provider) => provider.id === selectedProviderId);
...
function getCapabilityWebSearchAutoDetectTargets(config) {
	return getCapabilityWebProviderAutoDetectTargets({
		...
		providers: require_web_search_providers_shared.sortWebSearchProvidersForAutoDetect(require_web_search_providers_runtime.resolvePluginWebSearchProviders({ config })),
...
function getCapabilityWebFetchAutoDetectTargets(config) {
	return getCapabilityWebProviderAutoDetectTargets({
		...
		providers: require_web_search_providers_shared.sortWebFetchProvidersForAutoDetect(require_web_fetch_providers_runtime.resolvePluginWebFetchProviders({ config })),
...
function getAgentRuntimeBaseTargetIds() {
	cachedAgentRuntimeBaseTargetIds ??= [...STATIC_AGENT_RUNTIME_BASE_TARGET_IDS, ...require_target_registry.listSecretTargetRegistryEntries().map((entry) => entry.id).filter(isPluginWebCredentialTargetId).toSorted()];
	return cachedAgentRuntimeBaseTargetIds;
}
function isScopedChannelSecretTargetEntry(params) {
	const channelId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.pluginChannelId);
	if (!channelId) return false;
	const allowedPrefix = `channels.${channelId}.`;
	return params.entry.id.startsWith(allowedPrefix) && params.entry.configFile === "operator.json" && typeof params.entry.pathPattern === "string" && params.entry.pathPattern.startsWith(allowedPrefix) && (params.entry.refPathPattern === void 0 || params.entry.refPathPattern.startsWith(allowedPrefix));
}
function getConfiguredChannelSecretTargetIds(config, env = process.env) {
	...
	if (channels && typeof channels === "object" && !Array.isArray(channels)) for (const channelId of Object.keys(channels)) {
		...
		const contract = require_channel_contract_api.loadChannelSecretContractApi({
			channelId,
			config,
			env
		});
	...
	for (const plugin of require_read_only.listReadOnlyChannelPluginsForConfig(config, {
		env,
...
function pathTargetsScopedChannelAccount(params) {
	...
	return require_account_id.normalizeOptionalAccountId(accountId) === params.accountId;
}
...
function getScopedChannelsCommandSecretTargets(params) {
	...
	const explicitAccountId = require_account_id.normalizeOptionalAccountId(params.accountId);
	const channelPlugin = channel && !explicitAccountId && params.defaultAccountWhenMissing ? require_read_only.listReadOnlyChannelPluginsForConfig(params.config, { includePersistedAuthState: false }).find((plugin) => plugin.id === channel) : void 0;
	const normalizedAccountId = explicitAccountId ?? (channelPlugin ? require_account_id.normalizeOptionalAccountId(require_helpers.resolveChannelDefaultAccountId({
		plugin: channelPlugin,
		cfg: params.config
	})) : void 0);
	if (!channel || !normalizedAccountId) return { targetIds };
	const allowedPaths = /* @__PURE__ */ new Set();
	for (const target of require_target_registry.discoverConfigSecretTargetsByIds(params.config, targetIds)) if (pathTargetsScopedChannelAccount({
...
//#endregion
Object.defineProperty(exports, "command_secret_targets_exports", {
	enumerable: true,
	get: function() {
		return command_secret_targets_exports;
	}
});
Object.defineProperty(exports, "getAgentRuntimeCommandSecretTargetIds", {
	enumerable: true,
	get: function() {
		return getAgentRuntimeCommandSecretTargetIds;
	}
});
Object.defineProperty(exports, "getScopedChannelsCommandSecretTargets", {
	enumerable: true,
	get: function() {
		return getScopedChannelsCommandSecretTargets;
	}
});
