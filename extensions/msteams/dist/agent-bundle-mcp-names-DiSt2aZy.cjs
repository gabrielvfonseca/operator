...
function sanitizeServerName(raw, usedNames) {
	...
	let candidate = base;
	let n = 2;
	while (usedNames.has((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(candidate))) {
		const suffix = `-${n}`;
		candidate = `${base.slice(0, Math.max(1, TOOL_NAME_MAX_PREFIX - suffix.length))}${suffix}`;
		n += 1;
	}
	usedNames.add((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(candidate));
	return candidate;
}
/**
...
function normalizeReservedToolNames(names) {
	return new Set(Array.from(names ?? [], (name) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(name)).filter((name) => Boolean(name)));
}
...
function buildSafeToolName(params) {
	...
	let n = 2;
	while (params.reservedNames.has((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(candidate))) {
		const suffix = `-${n}`;
...
//#endregion
Object.defineProperty(exports, "assignSafeServerNames", {
	enumerable: true,
	get: function() {
		return assignSafeServerNames;
	}
});
Object.defineProperty(exports, "buildSafeToolName", {
	enumerable: true,
	get: function() {
		return buildSafeToolName;
	}
});
Object.defineProperty(exports, "normalizeReservedToolNames", {
	enumerable: true,
	get: function() {
		return normalizeReservedToolNames;
	}
});
Object.defineProperty(exports, "sanitizeServerName", {
	enumerable: true,
	get: function() {
		return sanitizeServerName;
	}
});
