...
function resolveMessageSeq(message) {
	return (0, _gabrielvfonseca_normalization_core_number_coercion.asPositiveSafeInteger)(message?.["__openclaw"]?.seq);
}
function isMessageToolMirrorMessage(message) {
	return message.operatorMessageToolMirror !== void 0;
}
...
function buildSessionHistorySnapshot(params) {
	const projected = require_session_transcript_path.projectChatDisplayMessagesWithState(params.rawMessages, { maxChars: params.maxChars ?? 8e3 });
...
var SessionHistorySseState = class SessionHistorySseState {
	...
	appendInlineMessage(update) {
		...
		const carriedSeq = (0, _gabrielvfonseca_normalization_core_number_coercion.asPositiveSafeInteger)(update.messageSeq);
		...
		const nextMessage = require_session_transcript_readers.attachOperatorTranscriptMeta(update.message, {
			...typeof update.messageId === "string" ? { id: update.messageId } : {},
			...idempotencyKey ? { idempotencyKey } : {},
			seq: this.rawTranscriptSeq
		});
		const hadPendingTurnBoundary = this.turnBoundaryPending;
		const nextProjection = require_session_transcript_path.projectChatDisplayMessagesWithState([nextMessage], {
			maxChars: this.maxChars,
			turnBoundaryPending: hadPendingTurnBoundary
		});
		this.turnBoundaryPending = nextProjection.turnBoundaryPending;
		const projectedMessages = toSessionHistoryMessages(require_session_transcript_path.projectChatDisplayMessages([...this.sentHistory.messages, nextMessage], { maxChars: this.maxChars }));
		if (projectedMessages.length > this.sentHistory.messages.length) {
			...
			if (hadPendingTurnBoundary && !this.turnBoundaryPending && addedMessages[0]) {
				const firstAdded = require_session_transcript_readers.attachOperatorTranscriptMeta(addedMessages[0], { turnBoundary: true });
			...
			if (projectedMessage !== void 0) {
				const emittedMessage = isMessageToolMirrorMessage(projectedMessage) || resolveMessageSeq(projectedMessage) === void 0 ? require_session_transcript_readers.attachOperatorTranscriptMeta(projectedMessage, { seq: this.rawTranscriptSeq }) : projectedMessage;
	...
	async readRawSnapshotAsync() {
		if (this.cursor === void 0 && typeof this.limit === "number") {
			const snapshot = await require_session_transcript_readers.readRecentSessionMessagesWithStatsAsync({
		...
		const snapshot = await require_session_transcript_readers.readSessionMessagesWithSourceAsync({
			agentId: this.target.agentId,
			sessionEntry: this.target.sessionEntry,
			sessionId: this.target.sessionId,
			sessionKey: this.target.sessionKey,
...
function normalizeTranscriptPathForComparison(filePath) {
	return typeof filePath === "string" ? require_session_transcript_path.resolveTranscriptPathForComparison(filePath) : void 0;
}
//#endregion
Object.defineProperty(exports, "SessionHistorySseState", {
	enumerable: true,
	get: function() {
		return SessionHistorySseState;
	}
});
Object.defineProperty(exports, "buildSessionHistorySnapshot", {
	enumerable: true,
	get: function() {
		return buildSessionHistorySnapshot;
	}
});
Object.defineProperty(exports, "resolveSessionHistoryTailReadOptions", {
	enumerable: true,
	get: function() {
		return resolveSessionHistoryTailReadOptions;
	}
});
