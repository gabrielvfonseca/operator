...
var DirectoryCache = class {
	constructor(ttlMs, maxSize = 2e3) {
		...
		this.ttlMs = (0, _gabrielvfonseca_normalization_core_number_coercion.resolveNonNegativeIntegerOption)(ttlMs, 0);
		this.maxSize = Math.max(1, (0, _gabrielvfonseca_normalization_core_number_coercion.resolveNonNegativeIntegerOption)(maxSize, 2e3));
	}
	/**
...
async function maybeResolveIdLikeTarget(params) {
	const target = await require_target_normalization.maybeResolvePluginMessagingTarget({
		...params,
		requireIdLike: true
	});
	if (!target) return;
...
function formatTargetDisplay(params) {
	const plugin = require_registry.getChannelPlugin(params.channel);
	...
	const lowered = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(trimmedTarget);
...
function detectTargetKind(channel, raw, preferred, plugin) {
	...
	const inferredChatType = (plugin ?? require_registry.getChannelPlugin(channel))?.messaging?.inferTargetChatType?.({ to: raw });
...
function normalizeDirectoryEntryId(channel, entry, plugin) {
	return require_target_normalization.normalizeTargetForProvider(channel, entry.id, plugin) ?? entry.id.trim();
}
...
async function listDirectoryEntries(params) {
	const directory = (params.plugin ?? require_registry.getChannelPlugin(params.channel))?.directory;
	if (!directory) return [];
...
async function getDirectoryEntries(params) {
	const signature = require_target_normalization.buildTargetResolverSignature(params.channel, params.plugin);
	const listParams = {
		cfg: params.cfg,
		channel: params.channel,
...
async function resolveMessagingTarget(params) {
	const raw = require_target_normalization.normalizeChannelTargetInput(params.input);
	if (!raw) return {
		ok: false,
		error: /* @__PURE__ */ new Error("Target is required")
	};
	const plugin = params.plugin ?? require_registry.getChannelPlugin(params.channel);
	const providerLabel = plugin?.meta?.label ?? params.channel;
	const hint = plugin?.messaging?.targetResolver?.hint;
	const kind = detectTargetKind(params.channel, raw, params.preferredKind, plugin);
	const normalizedInput = require_target_normalization.resolveNormalizedTargetInput(params.channel, raw, plugin);
	...
	if (normalizedInput && !reservedLiteral && require_target_normalization.looksLikeTargetId({
		channel: params.channel,
	...
	if (match.kind === "ambiguous") {
		...
		return {
			ok: false,
			error: require_target_errors.ambiguousTargetError(providerLabel, raw, hint),
			candidates: match.entries
		};
	}
	if (reservedLiteral) return {
		ok: false,
		error: require_target_errors.reservedTargetLiteralError(providerLabel, reservedLiteral, hint)
	};
	const resolvedFallbackTarget = asResolvedMessagingTarget(await require_target_normalization.maybeResolvePluginMessagingTarget({
		cfg: params.cfg,
		channel: params.channel,
		input: raw,
		accountId: params.accountId,
	...
	return {
		ok: false,
		error: require_target_errors.unknownTargetError(providerLabel, raw, hint)
	};
}
...
async function lookupDirectoryDisplay(params) {
	const normalized = require_target_normalization.normalizeTargetForProvider(params.channel, params.targetId) ?? params.targetId;
...
//#endregion
Object.defineProperty(exports, "formatTargetDisplay", {
	enumerable: true,
	get: function() {
		return formatTargetDisplay;
	}
});
Object.defineProperty(exports, "lookupDirectoryDisplay", {
	enumerable: true,
	get: function() {
		return lookupDirectoryDisplay;
	}
});
Object.defineProperty(exports, "maybeResolveIdLikeTarget", {
	enumerable: true,
	get: function() {
		return maybeResolveIdLikeTarget;
	}
});
Object.defineProperty(exports, "resetDirectoryCache", {
	enumerable: true,
	get: function() {
		return resetDirectoryCache;
	}
});
Object.defineProperty(exports, "resolveChannelTarget", {
	enumerable: true,
	get: function() {
