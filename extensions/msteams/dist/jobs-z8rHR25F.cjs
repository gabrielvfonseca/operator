...
const require_delivery_context = require("./delivery-context-DQy2aKjq.cjs");
...
const require_schedule = require("./schedule-JbL5oWY9.cjs");
...
function resolveLoadedOutboundChannelPlugin(channel) {
	const normalized = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(channel);
	if (!normalized) return;
	return require_registry_loaded.getLoadedChannelPluginForRead(normalized);
}
...
function tryResolveLoadedOutboundTarget(params) {
	return require_targets_session.resolveOutboundTargetWithPlugin({
...
const targetsRuntimeLoader = require_lazy_promise.createLazyImportLoader(() => Promise.resolve().then(() => require("./targets.runtime-CquzOsuQ.cjs")));
...
async function resolveOutboundTargetWithRuntime(params) {
	try {
		...
		return {
			ok: false,
			error: /* @__PURE__ */ new Error(`Invalid delivery target: ${require_errors.formatErrorMessage(err)}`)
		};
	}
}
const channelSelectionRuntimeLoader = require_lazy_promise.createLazyImportLoader(() => Promise.resolve().then(() => require("./channel-selection.runtime-R4EOFWpZ.cjs")));
const deliveryTargetRuntimeLoader = require_lazy_promise.createLazyImportLoader(() => Promise.resolve().then(() => require("./delivery-target.runtime-5mcMorUg.cjs")));
...
function stripSelectedProviderPrefix(params) {
	...
	if (!trimmed) return;
	return require_channel_target_prefix.stripTargetProviderPrefix(trimmed, params.channel).trim() || void 0;
}
...
async function resolveDeliveryTarget(cfg, agentId, jobPayload, options) {
	...
	const mainSessionKey = require_main_session.resolveAgentMainSessionKey({
		cfg,
		agentId
	});
	const storePath = require_paths.resolveStorePath(sessionCfg?.store, { agentId });
	const rawSessionKey = jobPayload.sessionKey?.trim();
	const threadSessionKey = rawSessionKey ? require_session_key$1.resolveCronAgentSessionKey({
		sessionKey: rawSessionKey,
		agentId,
		mainKey: cfg.session?.mainKey,
		cfg
	}) : void 0;
	const storedDeliveryContext = require_delivery_context.resolveCronStoredDeliveryContext({
		cfg,
	...
	const threadEntry = threadSessionKey ? require_session_accessor.loadSessionEntry({
		agentId,
		sessionKey: threadSessionKey,
		storePath
	}) : void 0;
	...
	const preliminary = require_targets_session.resolveSessionDeliveryTarget({
		entry: main,
		requestedChannel,
		explicitTo,
	...
	if (!preliminary.channel) if (preliminary.lastChannel) fallbackChannel = preliminary.lastChannel;
		...
		const detail = require_errors.formatErrorMessage(err);
	...
	const resolved = fallbackChannel ? require_targets_session.resolveSessionDeliveryTarget({
		entry: main,
		requestedChannel,
		explicitTo,
		explicitThreadId: jobPayload.threadId,
	...
	if (mode === "implicit") {
		...
		const resolvedAccountId = require_account_id.normalizeAccountId(accountId);
		const configuredAllowFromRaw = channelPlugin?.config.resolveAllowFrom?.({
			cfg,
			accountId: resolvedAccountId
		});
		const allowFromOverride = (0, _gabrielvfonseca_normalization_core_string_normalization.uniqueStrings)(configuredAllowFromRaw ? mapAllowFromEntries(configuredAllowFromRaw) : []);
	...
	if (!docked.ok) {
		if (!toCandidate || !require_target_errors.isReservedTargetLiteralError(docked.error)) return {
			ok: false,
			channel,
			to: void 0,
			accountId,
	...
	const parserExplicitThreadId = explicitThreadId == null && explicitTo ? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalThreadValue)(require_target_parsing_loaded.resolveExplicitDeliveryTargetCompat({
		channel,
...
function errorBackoffMs(consecutiveErrors, scheduleMs = DEFAULT_ERROR_BACKOFF_SCHEDULE_MS) {
	...
	return (0, _gabrielvfonseca_normalization_core.expectDefined)(scheduleMs[Math.max(0, idx)], "schedule ms entry at math.max(0, idx)") ?? DEFAULT_ERROR_BACKOFF_SCHEDULE_MS[0];
}
...
function resolveStableCronOffsetMs(jobId, staggerMs) {
	...
	const offset = node_crypto.default.createHash("sha256").update(jobId).digest().readUInt32BE(0) % staggerMs;
...
function computeStaggeredCronNextRunAtMs(job, nowMs) {
	if (job.schedule.kind !== "cron") return require_schedule.computeNextRunAtMs(job.schedule, nowMs);
	const staggerMs = require_stagger.resolveCronStaggerMs(job.schedule);
	const offsetMs = resolveStableCronOffsetMs(job.id, staggerMs);
	if (offsetMs <= 0) return require_schedule.computeNextRunAtMs(job.schedule, nowMs);
	let cursorMs = Math.max(0, nowMs - offsetMs);
	for (let attempt = 0; attempt < 4; attempt += 1) {
		const baseNext = require_schedule.computeNextRunAtMs(job.schedule, cursorMs);
...
function computeStaggeredCronPreviousRunAtMs(job, nowMs) {
	...
	const staggerMs = require_stagger.resolveCronStaggerMs(job.schedule);
	const offsetMs = resolveStableCronOffsetMs(job.id, staggerMs);
	if (offsetMs <= 0) return require_schedule.computePreviousRunAtMs(job.schedule, nowMs);
	let cursorMs = Math.max(0, nowMs - offsetMs);
	for (let attempt = 0; attempt < 4; attempt += 1) {
		const basePrevious = require_schedule.computePreviousRunAtMs(job.schedule, cursorMs);
...
function resolveEveryAnchorMs(params) {
	const coerced = require_stagger.coerceFiniteScheduleNumber(params.schedule.anchorMs);
...
function assertSupportedJobSpec(job) {
	...
	if (job.sessionTarget.startsWith("session:")) require_session_target.assertSafeCronSessionTargetId(job.sessionTarget.slice(8));
...
function assertTriggerSupport(job, opts) {
	...
	const minIntervalMs = require_io.resolveCronTriggerMinIntervalMs(opts?.cronConfig);
...
function assertDeliverySupport(job) {
	...
	if (job.delivery.mode === "webhook") {
		const target = require_webhook_url.normalizeHttpWebhookUrl(job.delivery.to);
	...
	if (job.delivery.completionDestination?.mode === "webhook") {
		...
		const target = require_webhook_url.normalizeHttpWebhookUrl(job.delivery.completionDestination.to);
...
function assertFailureDestinationSupport(job) {
	...
	if (failureDestination.mode === "webhook") {
		const target = require_webhook_url.normalizeHttpWebhookUrl(failureDestination.to);
...
function computeJobNextRunAtMs(job, nowMs) {
	...
	if (job.schedule.kind === "every") {
		const everyMsRaw = require_stagger.coerceFiniteScheduleNumber(job.schedule.everyMs);
		...
		const next = require_schedule.computeNextRunAtMs({
			...job.schedule,
			everyMs,
			anchorMs
		}, nowMs);
	...
	if (job.schedule.kind === "at") {
		const atMs = require_stagger.parseAbsoluteTimeMs(job.schedule.at);
...
function createJob(state, input) {
	...
	const id = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(input.id) ?? node_crypto.default.randomUUID();
	const schedule = input.schedule.kind === "every" ? {
		...
		const explicitStaggerMs = require_stagger.normalizeCronStaggerMs(input.schedule.staggerMs);
		if (explicitStaggerMs !== void 0) return {
			...input.schedule,
			staggerMs: explicitStaggerMs
		};
		const defaultStaggerMs = require_stagger.resolveDefaultCronStaggerMs(input.schedule.expr);
	...
	const declarationKey = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(input.declarationKey);
	...
	const displayName = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(input.displayName);
	...
	const ownerAgentId = require_session_key.normalizeOptionalAgentId(input.owner?.agentId);
	const ownerSessionKey = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(input.owner?.sessionKey);
	const job = {
		id,
		...
		agentId: require_session_key.normalizeOptionalAgentId(input.agentId),
		sessionKey: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(input.sessionKey),
		name: require_stagger.normalizeRequiredName(input.name),
		description: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(input.description),
		enabled,
		deleteAfterRun,
		createdAtMs: now,
		updatedAtMs: now,
...
function applyJobPatch(job, patch, opts) {
	if ("name" in patch) job.name = require_stagger.normalizeRequiredName(patch.name);
	if ("description" in patch) job.description = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patch.description);
	if ("displayName" in patch) {
		const displayName = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patch.displayName);
	...
	if (patch.schedule) if (patch.schedule.kind === "cron") {
		const explicitStaggerMs = require_stagger.normalizeCronStaggerMs(patch.schedule.staggerMs);
		if (explicitStaggerMs !== void 0) job.schedule = {
			...
			const defaultStaggerMs = require_stagger.resolveDefaultCronStaggerMs(patch.schedule.expr);
	...
	if (patch.delivery) {
		const implicitMode = require_delivery_plan.resolveCronDeliveryPlan(job).mode;
	...
	if ("agentId" in patch) job.agentId = require_session_key.normalizeOptionalAgentId(patch.agentId);
	if ("sessionKey" in patch) job.sessionKey = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patch.sessionKey);
	assertSupportedJobSpec(job);
	assertTriggerSupport(job, {
...
function mergeCronDelivery(existing, patch, implicitMode) {
	...
	if ("channel" in patch) next.channel = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patch.channel);
	if ("to" in patch) next.to = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patch.to);
	if ("threadId" in patch) next.threadId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalThreadValue)(patch.threadId);
	if ("accountId" in patch) next.accountId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patch.accountId);
	...
	if (hasCompletionDestinationPatch) if (patch.completionDestination == null) next.completionDestination = void 0;
		...
		const to = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patch.completionDestination.to);
	...
	if ("failureDestination" in patch) if (patch.failureDestination == null) next.failureDestination = void 0;
		...
		if (patchFd) {
			if ("channel" in patchFd) {
				const channel = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patchFd.channel) ?? "";
				nextFd.channel = channel ? channel : void 0;
			}
			if ("to" in patchFd) {
				const to = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patchFd.to) ?? "";
				nextFd.to = to ? to : void 0;
			}
			if ("accountId" in patchFd) {
				const accountId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patchFd.accountId) ?? "";
				nextFd.accountId = accountId ? accountId : void 0;
			}
			if ("mode" in patchFd) {
				const mode = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patchFd.mode) ?? "";
...
function mergeCronFailureAlert(existing, patch) {
	...
	if ("mode" in patch) {
		const mode = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patch.mode) ?? "";
		next.mode = mode === "announce" || mode === "webhook" ? mode : void 0;
	}
	if ("accountId" in patch) {
		const accountId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(patch.accountId) ?? "";
...
function resolveJobPayloadTextForMain(job) {
	...
	const text = require_stagger.normalizePayloadToSystemText(job.payload);
	return text.trim() ? text : void 0;
}
//#endregion
Object.defineProperty(exports, "DEFAULT_ERROR_BACKOFF_SCHEDULE_MS", {
	enumerable: true,
	get: function() {
		return DEFAULT_ERROR_BACKOFF_SCHEDULE_MS;
	}
});
Object.defineProperty(exports, "applyDeclarativeJobSpec", {
	enumerable: true,
	get: function() {
		return applyDeclarativeJobSpec;
	}
});
Object.defineProperty(exports, "applyJobPatch", {
	enumerable: true,
	get: function() {
		return applyJobPatch;
	}
});
Object.defineProperty(exports, "assertSupportedJobSpec", {
	enumerable: true,
	get: function() {
		return assertSupportedJobSpec;
	}
});
Object.defineProperty(exports, "computeJobNextRunAtMs", {
	enumerable: true,
	get: function() {
		return computeJobNextRunAtMs;
	}
});
Object.defineProperty(exports, "computeJobPreviousRunAtMs", {
	enumerable: true,
	get: function() {
		return computeJobPreviousRunAtMs;
	}
});
Object.defineProperty(exports, "createJob", {
	enumerable: true,
	get: function() {
		return createJob;
	}
});
Object.defineProperty(exports, "errorBackoffMs", {
	enumerable: true,
	get: function() {
		return errorBackoffMs;
	}
});
Object.defineProperty(exports, "findJobOrThrow", {
	enumerable: true,
	get: function() {
		return findJobOrThrow;
	}
});
Object.defineProperty(exports, "hasScheduledNextRunAtMs", {
	enumerable: true,
	get: function() {
		return hasScheduledNextRunAtMs;
	}
});
Object.defineProperty(exports, "isJobDue", {
	enumerable: true,
	get: function() {
		return isJobDue;
	}
});
Object.defineProperty(exports, "isJobEnabled", {
	enumerable: true,
	get: function() {
		return isJobEnabled;
	}
});
Object.defineProperty(exports, "nextWakeAtMs", {
	enumerable: true,
	get: function() {
		return nextWakeAtMs;
	}
});
Object.defineProperty(exports, "recomputeNextRuns", {
	enumerable: true,
	get: function() {
		return recomputeNextRuns;
	}
});
Object.defineProperty(exports, "recomputeNextRunsForMaintenance", {
	enumerable: true,
	get: function() {
		return recomputeNextRunsForMaintenance;
	}
});
Object.defineProperty(exports, "recordScheduleComputeError", {
	enumerable: true,
	get: function() {
		return recordScheduleComputeError;
	}
});
Object.defineProperty(exports, "resolveDeliveryTarget", {
	enumerable: true,
	get: function() {
		return resolveDeliveryTarget;
	}
});
Object.defineProperty(exports, "resolveJobErrorBackoffUntilMs", {
	enumerable: true,
	get: function() {
		return resolveJobErrorBackoffUntilMs;
	}
});
Object.defineProperty(exports, "resolveJobLastRunStatus", {
	enumerable: true,
	get: function() {
		return resolveJobLastRunStatus;
	}
});
Object.defineProperty(exports, "resolveJobPayloadTextForMain", {
	enumerable: true,
	get: function() {
		return resolveJobPayloadTextForMain;
	}
});
