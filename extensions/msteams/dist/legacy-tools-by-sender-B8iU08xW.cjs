...
function collectLegacyToolsBySenderKeyHits(value, pathParts, hits) {
	...
	const record = require_object.asObjectRecord(value);
	if (!record) return;
	const toolsBySender = require_object.asObjectRecord(record.toolsBySender);
	if (toolsBySender) {
		const path = [...pathParts, "toolsBySender"];
		const pathLabel = require_doctor_config_analysis.formatConfigPath(path);
		for (const rawKey of Object.keys(toolsBySender)) {
			const trimmed = rawKey.trim();
			if (!trimmed || trimmed === "*" || require_config.parseToolsBySenderTypedKey(trimmed)) continue;
			hits.push({
...
function collectLegacyToolsBySenderWarnings(params) {
	...
	const sampleLabel = require_ansi.sanitizeForLog(sample ? `${sample.pathLabel}.${sample.key}` : "toolsBySender");
	return [
...
function maybeRepairLegacyToolsBySenderKeys(cfg) {
	...
	for (const hit of hits) {
		const toolsBySender = require_object.asObjectRecord(require_doctor_config_analysis.resolveConfigPathTarget(next, hit.toolsBySenderPath));
...
//#endregion
exports.collectLegacyToolsBySenderWarnings = collectLegacyToolsBySenderWarnings;
exports.maybeRepairLegacyToolsBySenderKeys = maybeRepairLegacyToolsBySenderKeys;
exports.scanLegacyToolsBySenderKeys = scanLegacyToolsBySenderKeys;
