...
let _gabrielvfonseca_gateway_protocol_client_info = require("@gabrielvfonseca/gateway-protocol/client-info");
let _gabrielvfonseca_gateway_protocol_connect_error_details = require("@gabrielvfonseca/gateway-protocol/connect-error-details");
let _gabrielvfonseca_gateway_protocol_startup_unavailable = require("@gabrielvfonseca/gateway-protocol/startup-unavailable");
let _gabrielvfonseca_gateway_protocol_version = require("@gabrielvfonseca/gateway-protocol/version");
let ws = require("ws");
let _gabrielvfonseca_gateway_protocol_frame_guards = require("@gabrielvfonseca/gateway-protocol/frame-guards");
let _gabrielvfonseca_retry = require("@gabrielvfonseca/retry");
...
function parseGatewayIpAddress(host) {
	const normalized = (0, _gabrielvfonseca_net_policy_ip.normalizeIpAddress)(host);
	return normalized ? (0, _gabrielvfonseca_net_policy_ip.parseCanonicalIpAddress)(normalized) : void 0;
}
//#endregion
...
function shouldRetryGatewayWithDeviceToken(params) {
	...
	const advice = (0, _gabrielvfonseca_gateway_protocol_connect_error_details.readConnectErrorRecoveryAdvice)(params.errorDetails);
	return params.canRetryWithDeviceTokenHint === true || advice.canRetryWithDeviceToken === true || advice.recommendedNextStep === "retry_with_device_token" || (0, _gabrielvfonseca_gateway_protocol_connect_error_details.readConnectErrorDetailCode)(params.errorDetails) === _gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.AUTH_TOKEN_MISMATCH;
}
//#endregion
...
var GatewayProtocolClient = class {
	constructor(opts) {
		...
		this.reconnectSupervisor = new _gabrielvfonseca_retry.RetrySupervisor({
	...
	handleMessage(socket, generation, raw) {
		...
		if ((0, _gabrielvfonseca_gateway_protocol_frame_guards.isGatewayEventFrame)(parsed)) {
		...
		if (!(0, _gabrielvfonseca_gateway_protocol_frame_guards.isGatewayResponseFrame)(parsed)) return;
		this.opts.onActivity?.();
	...
	scheduleReconnect(overrideMs) {
		...
		if (!retry) return;
		(0, _gabrielvfonseca_retry.sleepWithAbort)(retry.delayMs, retry.signal).then(() => this.connect(), () => {});
	}
	closeContext() {
		return {
...
const NON_RECOVERABLE_AUTH_ERRORS = /* @__PURE__ */ new Set([
	_gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.AUTH_TOKEN_MISSING,
	_gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.AUTH_BOOTSTRAP_TOKEN_INVALID,
	_gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.AUTH_PASSWORD_MISSING,
	_gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.AUTH_PASSWORD_MISMATCH,
	_gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.AUTH_RATE_LIMITED,
	_gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.AUTH_DEVICE_TOKEN_MISMATCH,
	_gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.AUTH_SCOPE_MISMATCH,
	_gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.PAIRING_REQUIRED,
	_gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.CONTROL_UI_DEVICE_IDENTITY_REQUIRED,
	_gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.DEVICE_IDENTITY_REQUIRED
]);
function shouldPauseGatewayReconnect(params) {
	const code = (0, _gabrielvfonseca_gateway_protocol_connect_error_details.readConnectErrorDetailCode)(params.details);
	if (!code) return false;
	const pairing = (0, _gabrielvfonseca_gateway_protocol_connect_error_details.readPairingConnectErrorDetails)(params.details);
	if (code === _gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.PAIRING_REQUIRED && (pairing?.pauseReconnect === false || pairing?.recommendedNextStep === "wait_then_retry")) return false;
	if (code === _gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.AUTH_TOKEN_MISMATCH) return params.tokenMismatchIsTerminal === true && !params.deviceTokenRetryPending;
	return NON_RECOVERABLE_AUTH_ERRORS.has(code) || params.protocolMismatchIsTerminal === true && code === _gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.PROTOCOL_MISMATCH || params.clientVersionMismatchIsTerminal === true && code === _gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.CLIENT_VERSION_MISMATCH;
}
//#endregion
//#region packages/gateway-client/src/websocket-data.ts
function rawDataToString(data) {
	if (Array.isArray(data)) return node_buffer.Buffer.concat(data).toString("utf8");
	return data instanceof ArrayBuffer ? node_buffer.Buffer.from(data).toString("utf8") : data.toString("utf8");
}
//#endregion
...
function isLoopbackHost(host) {
	...
	return (0, _gabrielvfonseca_net_policy_ip.isLoopbackIpAddress)(parsed.unbracketedHost);
}
...
var GatewayClientRequestError = class extends GatewayProtocolRequestError {
	constructor(error) {
		super({
			...error,
			message: (0, _gabrielvfonseca_gateway_protocol_connect_error_details.formatConnectErrorMessage)({
...
function resolveGatewayClientConnectChallengeTimeoutMs(opts) {
	return require_timeouts.resolveConnectChallengeTimeoutMs(readConnectChallengeTimeoutOverride(opts), {
		env: opts.env,
...
var GatewayClient = class {
	constructor(opts) {
		...
		this.requestTimeoutMs = typeof opts.requestTimeoutMs === "number" && Number.isFinite(opts.requestTimeoutMs) ? require_timeouts.resolveSafeTimeoutDelayMs(opts.requestTimeoutMs, { minMs: 0 }) : require_timeouts.DEFAULT_GATEWAY_REQUEST_TIMEOUT_MS;
		this.protocol = new GatewayProtocolClient({
			createSocket: (handlers) => this.createSocket(handlers),
			createRequestId: node_crypto.randomUUID,
			...
			onConnectPlanError: (error) => {
				...
				if (this.opts.mode === _gabrielvfonseca_gateway_protocol_client_info.GATEWAY_CLIENT_MODES.PROBE || isGatewayClientStoppedError(error)) this.logDebug(msg);
			...
			handshake: {
				...
				timeoutMessage: (elapsedMs) => `gateway connect challenge timeout (waited ${elapsedMs}ms, limit ${resolveGatewayClientConnectChallengeTimeoutMs(this.opts)}ms)`
			},
			reconnect: {
				initialMs: 1e3,
				multiplier: 2,
	...
	createSocket(handlers) {
		...
		try {
			ws$1 = new ws.WebSocket(url, wsOptions);
		...
		ws$1.on("open", () => {
			...
			if (url.startsWith("wss://") && this.opts.tlsFingerprint) {
				...
				if (tlsError) {
					...
					ws$1.close(1008, tlsError.message);
					return;
				}
			}
			this.transportValidated = true;
		});
		ws$1.on("message", (data) => handlers.message(rawDataToString(data)));
		ws$1.on("close", (code, reason) => {
			const reasonText = reason.toString();
			if (this.ws === ws$1) this.ws = null;
			this.resolvePendingStop(ws$1);
		...
		return {
			isOpen: () => ws$1.readyState === ws.WebSocket.OPEN,
			send: (data) => ws$1.send(data),
			close: (code, reason) => ws$1.close(code, reason)
		};
	}
	stop() {
		this.beginStop();
	...
	async stopAndWait(opts) {
		...
		const timeoutMs = opts?.timeoutMs === void 0 ? STOP_AND_WAIT_TIMEOUT_MS : require_timeouts.resolveSafeTimeoutDelayMs(opts.timeoutMs);
		let timeout = null;
		try {
	...
	beginStop() {
		...
		this.ws = null;
		if (ws$2) {
			const pendingStop = this.createPendingStop(ws$2);
			const forceTerminateTimer = setTimeout(() => {
				try {
					ws$2.terminate();
				} finally {
					this.resolvePendingStop(ws$2);
				}
	...
	createPendingStop(ws$3) {
		if (this.pendingStop?.ws === ws$3) return this.pendingStop;
		let resolve = () => {};
	...
	resolvePendingStop(ws$4) {
		if (this.pendingStop?.ws !== ws$4) return;
	...
	assembleConnectParams(params) {
		...
		return {
			params: {
				minProtocol: this.opts.minProtocol ?? _gabrielvfonseca_gateway_protocol_version.MIN_CLIENT_PROTOCOL_VERSION,
				maxProtocol: this.opts.maxProtocol ?? _gabrielvfonseca_gateway_protocol_version.PROTOCOL_VERSION,
				client: {
					id: this.opts.clientName ?? _gabrielvfonseca_gateway_protocol_client_info.GATEWAY_CLIENT_NAMES.GATEWAY_CLIENT,
					displayName: this.opts.clientDisplayName,
					version: this.opts.clientVersion ?? DEFAULT_CLIENT_VERSION,
					platform,
					deviceFamily: this.opts.deviceFamily,
					mode: this.opts.mode ?? _gabrielvfonseca_gateway_protocol_client_info.GATEWAY_CLIENT_MODES.BACKEND,
	...
	buildDeviceConnectParams(params) {
		...
		const payload = require_device_auth.buildDeviceAuthPayloadV3({
			deviceId: this.opts.deviceIdentity.deviceId,
			clientId: this.opts.clientName ?? _gabrielvfonseca_gateway_protocol_client_info.GATEWAY_CLIENT_NAMES.GATEWAY_CLIENT,
			clientMode: this.opts.mode ?? _gabrielvfonseca_gateway_protocol_client_info.GATEWAY_CLIENT_MODES.BACKEND,
			role,
			scopes,
			signedAtMs,
	...
	handleConnectRequestFailure(error, assembled) {
		...
		if (this.opts.deviceIdentity && assembled.usingStoredDeviceToken && error instanceof GatewayClientRequestError && (0, _gabrielvfonseca_gateway_protocol_connect_error_details.readConnectErrorDetailCode)(error.details) === _gabrielvfonseca_gateway_protocol_connect_error_details.ConnectErrorDetailCodes.AUTH_DEVICE_TOKEN_MISMATCH) {
		...
		const startupRetryAfterMs = (0, _gabrielvfonseca_gateway_protocol_startup_unavailable.resolveGatewayStartupRetryAfterMs)(error);
		...
		if (this.opts.mode === _gabrielvfonseca_gateway_protocol_client_info.GATEWAY_CLIENT_MODES.PROBE || isGatewayClientStoppedError(error)) this.logDebug(message);
	...
	resolveClose(context) {
		...
		const detailCode = context.connectFailure?.error instanceof GatewayClientRequestError ? (0, _gabrielvfonseca_gateway_protocol_connect_error_details.readConnectErrorDetailCode)(context.connectFailure.error.details) : null;
	...
	startTickWatch() {
		...
		const interval = require_timeouts.resolveSafeTimeoutDelayMs(Math.max(this.tickIntervalMs, minInterval));
	...
	async request(method, params, opts) {
		...
		const timeoutMs = opts?.timeoutMs === null ? null : typeof opts?.timeoutMs === "number" && Number.isFinite(opts.timeoutMs) ? require_timeouts.resolveSafeTimeoutDelayMs(opts.timeoutMs, { minMs: 0 }) : expectFinal ? null : this.requestTimeoutMs;
...
Object.defineProperty(exports, "GatewayClientRequestError", {
	enumerable: true,
	get: function() {
		return GatewayClientRequestError;
	}
});
Object.defineProperty(exports, "isGatewayConnectAssemblyError", {
	enumerable: true,
	get: function() {
		return isGatewayConnectAssemblyError;
	}
});
