...
const hookRunnerGlobalStateKey = Symbol.for("operator.plugins.hook-runner-global-state");
function getHookRunnerGlobalState() {
	return require_global_singleton.resolveGlobalSingleton(hookRunnerGlobalStateKey, () => ({
		hookRunner: null,
		registry: null
	}));
}
function collectHookRegistrySources(lastInitialized) {
	...
	const add = (registry) => {
		...
		if (require_runtime.isPluginRegistryRetired(registry)) return;
		seen.add(registry);
		ordered.push(registry);
	};
	const liveRegistries = require_runtime.collectLivePluginRegistries();
...
function composeLiveHookRegistry(lastInitialized) {
	...
	if (lastInitialized && !liveRegistries.includes(lastInitialized)) {
		...
		if (isolatedSourceIndex >= 0) for (const pluginId of (0, _gabrielvfonseca_normalization_core.expectDefined)(hookPluginIdsBySource[isolatedSourceIndex], "isolated hook plugin ids")) claimOwner(pluginId, isolatedSourceIndex);
	}
	...
	sources.forEach((registry, index) => {
		for (const plugin of registry.plugins) if (plugin.status === "loaded" && (0, _gabrielvfonseca_normalization_core.expectDefined)(hookPluginIdsBySource[index], "hook plugin ids by source entry at index").has(plugin.id)) claimOwner(plugin.id, index);
	});
	sources.forEach((registry, index) => {
	...
	sources.forEach((registry, index) => {
		for (const plugin of registry.plugins) if (plugin.status === "loaded" && (0, _gabrielvfonseca_normalization_core.expectDefined)(trustedPolicyPluginIdsBySource[index], "trusted policy plugin ids by source entry at index").has(plugin.id)) claimPolicyOwner(plugin.id, index);
	});
	sources.forEach((registry, index) => {
...
const getFireAndForgetHookState = () => require_global_singleton.resolveGlobalSingleton(Symbol.for("operator.fireAndForgetHookState"), () => ({
	active: 0,
	queue: []
}));
...
function resolveFireAndForgetHookTimeoutMs(value) {
	if (typeof value === "number" && Number.isInteger(value) && value > 0) return (0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(value, DEFAULT_FIRE_AND_FORGET_HOOK_TIMEOUT_MS);
	return (0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(DEFAULT_FIRE_AND_FORGET_HOOK_TIMEOUT_MS, 1);
}
...
function formatHookErrorForLog(err) {
	return (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(replaceLogControlCharacters(require_errors.formatErrorMessage(err)).replace(/\s+/g, " ").trim() || "unknown error", MAX_HOOK_LOG_MESSAGE_LENGTH);
}
/** Run a hook promise without awaiting it, logging rejection safely. */
function fireAndForgetHook(task, label, logger = require_globals.logVerbose) {
	task.catch((err) => {
...
function fireAndForgetBoundedHook(task, label, logger = require_globals.logVerbose, options = {}) {
...
function createHookRunner(registry, options = {}) {
	...
	const acceptPluginReplyPayload = (previous, next) => {
		...
		return require_reply_payload.copyReplyPayloadMetadata(previous, preservesTrustedMediaRefs(previous, clonedPayload) ? {
			...clonedPayload,
	...
	const sanitizeHookError = (error) => {
		return require_errors.formatErrorMessage(error).split("\n")[0]?.trim() || "unknown error";
	};
...
*/
var hook_runner_global_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	getGlobalHookRunner: () => getGlobalHookRunner,
	getGlobalPluginRegistry: () => getGlobalPluginRegistry,
	hasGlobalHooks: () => hasGlobalHooks,
	initializeGlobalHookRunner: () => initializeGlobalHookRunner,
	resetGlobalHookRunner: () => resetGlobalHookRunner,
	runGlobalGatewayStopSafely: () => runGlobalGatewayStopSafely
});
const getLog = () => require_subsystem.createSubsystemLogger("plugins");
/**
...
//#endregion
Object.defineProperty(exports, "fireAndForgetBoundedHook", {
	enumerable: true,
	get: function() {
		return fireAndForgetBoundedHook;
	}
});
Object.defineProperty(exports, "fireAndForgetHook", {
	enumerable: true,
	get: function() {
		return fireAndForgetHook;
	}
});
Object.defineProperty(exports, "getGlobalHookRunner", {
	enumerable: true,
	get: function() {
		return getGlobalHookRunner;
	}
});
Object.defineProperty(exports, "getGlobalHookRunnerRegistry", {
	enumerable: true,
	get: function() {
		return getGlobalHookRunnerRegistry;
	}
});
Object.defineProperty(exports, "getGlobalPluginRegistry", {
	enumerable: true,
	get: function() {
		return getGlobalPluginRegistry;
	}
});
Object.defineProperty(exports, "hasGlobalHooks", {
	enumerable: true,
	get: function() {
		return hasGlobalHooks;
	}
});
Object.defineProperty(exports, "hook_runner_global_exports", {
	enumerable: true,
	get: function() {
		return hook_runner_global_exports;
	}
});
Object.defineProperty(exports, "initializeGlobalHookRunner", {
	enumerable: true,
	get: function() {
		return initializeGlobalHookRunner;
	}
});
Object.defineProperty(exports, "joinPresentTextSegments", {
	enumerable: true,
	get: function() {
		return joinPresentTextSegments;
	}
});
Object.defineProperty(exports, "resolveBlockMessage", {
	enumerable: true,
	get: function() {
