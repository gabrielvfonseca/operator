...
const log = require_subsystem.createSubsystemLogger("agents/auth-profiles");
//#endregion
...
function shouldMirrorRefreshedOAuthCredential(params) {
	...
	const refreshedExpires = (0, _gabrielvfonseca_normalization_core_number_coercion.asDateTimestampMs)(refreshed.expires);
	if (refreshedExpires === void 0) return {
		shouldMirror: false,
		reason: "incoming-not-fresher"
	};
	const existingExpires = (0, _gabrielvfonseca_normalization_core_number_coercion.asDateTimestampMs)(existing.expires);
...
function hasNewerStoredOAuthCredential(existing, incoming) {
	const existingExpires = (0, require_number_coercion.number_coercion_exports.asDateTimestampMs)(existing?.expires);
	const incomingExpires = (0, require_number_coercion.number_coercion_exports.asDateTimestampMs)(incoming.expires);
...
function shouldReplaceStoredOAuthCredential(existing, incoming) {
	if (existing?.type !== "oauth") return true;
...
function hasUsableOAuthCredential(credential, now = Date.now()) {
	return require_credential_state.hasUsableOAuthCredential(credential, { now });
}
...
function isSafeOAuthIdentityTransition(existing, incoming, policy) {
	if (existing?.type !== "oauth") return policy.whenExistingCredentialMissing;
...
function overlayRuntimeExternalOAuthProfiles(store, profiles, options) {
	...
	const next = require_path_resolve.cloneAuthProfileStore(store);
...
function normalizeAuthProfileOrder(raw) {
	...
	const normalized = Object.entries(raw).reduce((acc, [provider, value]) => {
		...
		const providerKey = (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(provider);
		if (!providerKey) return acc;
		const list = (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeTrimmedStringList)(value);
...
function normalizeLastGood(raw) {
	...
	for (const [provider, profileId] of Object.entries(raw)) {
		const providerKey = (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(provider);
		const normalizedProfileId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(profileId);
...
function normalizeUsageStatsEntry(raw) {
	...
	const stats = {
		...
		blockedModel: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(raw.blockedModel),
		...
		cooldownModel: (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(raw.cooldownModel),
...
function loadPersistedAuthProfileState(agentDir, database) {
	return coerceAuthProfileState(require_sqlite.readPersistedAuthProfileStateRaw(agentDir, database));
}
...
function normalizeSecretBackedField(params) {
	...
	const ref = require_types_secrets.coerceSecretRef(value);
	if (ref && !require_types_secrets.coerceSecretRef(params.entry[params.refField])) params.entry[params.refField] = ref;
	delete params.entry[params.valueField];
}
function normalizeCommonCredentialFields(entry) {
	const normalized = { provider: typeof entry.provider === "string" ? (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(entry.provider) : "" };
	const copyToAgents = require_boolean.asBoolean(entry.copyToAgents);
...
function normalizeRawCredentialEntry(raw) {
	...
	if (!("key" in entry) && !require_types_secrets.coerceSecretRef(entry["keyRef"]) && typeof entry["apiKey"] === "string") entry["key"] = entry["apiKey"];
	...
	if (entry.type === "api_key") {
		...
		const keyRef = require_types_secrets.coerceSecretRef(entry.keyRef);
	...
	if (entry.type === "token") {
		...
		const tokenRef = require_types_secrets.coerceSecretRef(entry.tokenRef);
...
function parseCredentialEntry(raw, fallbackProvider) {
	...
	const normalizedProvider = typeof provider === "string" ? (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(provider) : "";
...
function coercePersistedAuthProfileStore(raw) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(raw)) return null;
	const record = raw;
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(record.profiles)) return null;
...
function groupProfileIdsByProvider(profiles) {
	...
	for (const [profileId, credential] of Object.entries(profiles)) {
		const providerKey = (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(credential.provider);
		grouped.set(providerKey, [...grouped.get(providerKey) ?? [], profileId]);
	}
	return grouped;
}
function findOrderEntryKey(order, providerKey) {
	return Object.keys(order ?? {}).find((key) => (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(key) === providerKey);
}
...
function mergeProfileOrderWithOverridePrecedence(params) {
	...
	for (const [providerKey, overrideProfileIds] of groupProfileIdsByProvider(params.overrideProfiles)) {
		...
		for (const provider of Object.keys(mergedOrder)) if (provider !== mergedOrderKey && (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(provider) === providerKey) delete mergedOrder[provider];
		if (overrideOrderKey) {
...
function isLegacyDefaultOAuthProfile(profileId, credential) {
	return profileId === `${(0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(credential.provider)}:default`;
}
...
function findMainStoreOAuthReplacement(params) {
	const providerKey = (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(params.legacyCredential.provider);
	const candidates = Object.entries(params.base.profiles).flatMap(([profileId, credential]) => {
		if (profileId === params.legacyProfileId || credential.type !== "oauth" || (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(credential.provider) !== providerKey) return [];
...
function mergeOAuthFileIntoStore(store) {
	const oauthRaw = require_json_file.loadJsonFile(require_paths.resolveOAuthPath());
...
function loadPersistedAuthProfileStore(agentDir, options) {
	const raw = require_sqlite.readPersistedAuthProfileStoreRaw(agentDir, options?.database);
...
function loadLegacyAuthProfileStore(agentDir) {
	return coerceLegacyAuthStore(require_json_file.loadJsonFile(require_path_resolve.resolveLegacyAuthStorePath(agentDir)));
}
//#endregion
Object.defineProperty(exports, "CLAUDE_CLI_PROFILE_ID", {
	enumerable: true,
	get: function() {
		return CLAUDE_CLI_PROFILE_ID;
	}
});
Object.defineProperty(exports, "CODEX_CLI_PROFILE_ID", {
	enumerable: true,
	get: function() {
		return CODEX_CLI_PROFILE_ID;
	}
});
Object.defineProperty(exports, "EXTERNAL_CLI_SYNC_TTL_MS", {
	enumerable: true,
	get: function() {
		return EXTERNAL_CLI_SYNC_TTL_MS;
	}
});
Object.defineProperty(exports, "LEGACY_OAUTH_REF_PROVIDER", {
	enumerable: true,
	get: function() {
		return LEGACY_OAUTH_REF_PROVIDER;
	}
});
Object.defineProperty(exports, "MINIMAX_CLI_PROFILE_ID", {
	enumerable: true,
	get: function() {
		return MINIMAX_CLI_PROFILE_ID;
	}
});
Object.defineProperty(exports, "OAUTH_REFRESH_CALL_TIMEOUT_MS", {
	enumerable: true,
	get: function() {
		return OAUTH_REFRESH_CALL_TIMEOUT_MS;
	}
});
Object.defineProperty(exports, "OAUTH_REFRESH_LOCK_OPTIONS", {
	enumerable: true,
	get: function() {
		return OAUTH_REFRESH_LOCK_OPTIONS;
	}
});
Object.defineProperty(exports, "OPENAI_CODEX_DEFAULT_PROFILE_ID", {
	enumerable: true,
	get: function() {
		return OPENAI_CODEX_DEFAULT_PROFILE_ID;
	}
});
Object.defineProperty(exports, "applyLegacyAuthStore", {
	enumerable: true,
	get: function() {
		return applyLegacyAuthStore;
	}
});
Object.defineProperty(exports, "areOAuthCredentialsEquivalent", {
	enumerable: true,
	get: function() {
		return areOAuthCredentialsEquivalent;
	}
});
Object.defineProperty(exports, "buildPersistedAuthProfileSecretsStore", {
	enumerable: true,
	get: function() {
		return buildPersistedAuthProfileSecretsStore;
	}
});
Object.defineProperty(exports, "buildPersistedAuthProfileState", {
	enumerable: true,
	get: function() {
		return buildPersistedAuthProfileState;
	}
});
Object.defineProperty(exports, "coerceAuthProfileState", {
	enumerable: true,
	get: function() {
		return coerceAuthProfileState;
	}
});
Object.defineProperty(exports, "coerceLegacyAuthStore", {
	enumerable: true,
	get: function() {
		return coerceLegacyAuthStore;
	}
});
Object.defineProperty(exports, "coercePersistedAuthProfileStore", {
	enumerable: true,
	get: function() {
		return coercePersistedAuthProfileStore;
	}
});
Object.defineProperty(exports, "hasMatchingOAuthIdentity", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "isLegacyOAuthRef", {
	enumerable: true,
	get: function() {
		return isLegacyOAuthRef;
	}
});
Object.defineProperty(exports, "isSafeToAdoptBootstrapOAuthIdentity", {
	enumerable: true,
	get: function() {
		return isSafeToAdoptBootstrapOAuthIdentity;
	}
});
Object.defineProperty(exports, "isSafeToAdoptMainStoreOAuthIdentity", {
	enumerable: true,
	get: function() {
		return isSafeToAdoptMainStoreOAuthIdentity;
	}
});
Object.defineProperty(exports, "isSafeToCopyOAuthIdentity", {
	enumerable: true,
	get: function() {
		return isSafeToCopyOAuthIdentity;
	}
});
Object.defineProperty(exports, "loadLegacyAuthProfileStore", {
	enumerable: true,
	get: function() {
		return loadLegacyAuthProfileStore;
	}
});
Object.defineProperty(exports, "loadPersistedAuthProfileState", {
	enumerable: true,
	get: function() {
		return loadPersistedAuthProfileState;
	}
});
Object.defineProperty(exports, "loadPersistedAuthProfileStore", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "mergeAuthProfileState", {
	enumerable: true,
	get: function() {
		return mergeAuthProfileState;
	}
});
Object.defineProperty(exports, "mergeAuthProfileStores", {
	enumerable: true,
	get: function() {
		return mergeAuthProfileStores;
	}
});
Object.defineProperty(exports, "mergeOAuthFileIntoStore", {
	enumerable: true,
	get: function() {
		return mergeOAuthFileIntoStore;
	}
});
Object.defineProperty(exports, "overlayRuntimeExternalOAuthProfiles", {
	enumerable: true,
	get: function() {
		return overlayRuntimeExternalOAuthProfiles;
	}
});
Object.defineProperty(exports, "shouldBootstrapFromExternalCliCredential", {
	enumerable: true,
	get: function() {
		return shouldBootstrapFromExternalCliCredential;
	}
});
Object.defineProperty(exports, "shouldMirrorRefreshedOAuthCredential", {
	enumerable: true,
	get: function() {
		return shouldMirrorRefreshedOAuthCredential;
	}
});
Object.defineProperty(exports, "shouldPersistRuntimeExternalOAuthProfile", {
	enumerable: true,
	get: function() {
		return shouldPersistRuntimeExternalOAuthProfile;
	}
});
Object.defineProperty(exports, "shouldReplaceStoredOAuthCredential", {
	enumerable: true,
	get: function() {
		return shouldReplaceStoredOAuthCredential;
	}
});
