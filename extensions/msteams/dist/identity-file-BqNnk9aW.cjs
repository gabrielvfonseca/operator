...
function normalizeIdentityValue(value) {
	...
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(normalized.replace(/\s+/g, " "));
}
function normalizeIdentityLabel(label) {
	return (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(label.replace(/[*_`]/g, ""));
}
...
function loadIdentityFromFile(identityPath) {
	try {
		const parsed = parseIdentityMarkdown(node_fs.default.readFileSync(identityPath, "utf-8"));
...
function loadAgentIdentityFromWorkspace(workspace) {
	return loadIdentityFromFile(node_path.default.join(workspace, require_workspace.DEFAULT_IDENTITY_FILENAME));
}
//#endregion
Object.defineProperty(exports, "identityHasValues", {
	enumerable: true,
	get: function() {
		return identityHasValues;
	}
});
Object.defineProperty(exports, "loadAgentIdentityFromWorkspace", {
	enumerable: true,
	get: function() {
		return loadAgentIdentityFromWorkspace;
	}
});
Object.defineProperty(exports, "mergeIdentityMarkdownContent", {
	enumerable: true,
	get: function() {
		return mergeIdentityMarkdownContent;
	}
});
