...
const require_echo_transcript = require("./echo-transcript-DSMSepK_.cjs");
...
async function runMediaCapability(params) {
	try {
		return await require_echo_transcript.runCapability(params);
	} catch (err) {
		if (require_globals.shouldLogVerbose()) require_globals.logVerbose(`Media understanding task failed: ${String(err)}`);
		return;
	}
}
//#endregion
...
function resolveFileExtractionLimits(cfg) {
	...
	return {
		...require_input_files.resolveInputFileLimits({
			...files,
...
function hasSuspiciousBinarySignal(buffer) {
	...
	const signature = (0, _gabrielvfonseca_normalization_core.expectDefined)(sample[2], "sample entry at 2") << 8 | (0, _gabrielvfonseca_normalization_core.expectDefined)(sample[3], "sample entry at 3");
...
function decodeTextSample(buffer) {
	...
	if (utf16Charset === "utf-16be") {
		...
		for (let i = 0; i + 1 < sample.length; i += 2) {
			swapped[i] = (0, _gabrielvfonseca_normalization_core.expectDefined)(sample[i + 1], "UTF-16BE low byte");
			swapped[i + 1] = (0, _gabrielvfonseca_normalization_core.expectDefined)(sample[i], "UTF-16BE high byte");
		}
...
function resolveTextMimeFromName(name) {
	if (!name) return;
	const ext = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(node_path.default.extname(name));
...
async function extractFileContext(params) {
	...
	for (const attachment of attachments) {
		...
		const kind = forcedTextMime ? "document" : require_runner_entries.resolveAttachmentKind(attachment);
		if (!forcedTextMime && (kind === "image" || kind === "video" || kind === "audio")) continue;
		if (!limits.allowUrl && attachment.url && !attachment.path) {
			if (require_globals.shouldLogVerbose()) require_globals.logVerbose(`media: file attachment skipped (url disabled) index=${attachment.index}`);
			continue;
		}
		let bufferResult;
		try {
			...
			if (require_globals.shouldLogVerbose()) require_globals.logVerbose(`media: file attachment skipped (buffer): ${String(err)}`);
			continue;
		}
		const nameHint = bufferResult?.fileName ?? attachment.path ?? attachment.url;
		const forcedTextMimeResolved = forcedTextMime ?? resolveTextMimeFromName(nameHint ?? "");
		const rawMime = bufferResult?.mime ?? attachment.mime;
		const normalizedRawMime = require_input_files.normalizeMimeType(rawMime);
		...
		const mimeType = sanitizeMimeType(textHint ?? require_input_files.normalizeMimeType(rawMime));
		if (textHint && rawMime && !rawMime.startsWith("text/")) require_globals.logVerbose(`media: MIME override from "${rawMime}" to "${textHint}" for index=${attachment.index}`);
		if (!mimeType) {
			if (require_globals.shouldLogVerbose()) require_globals.logVerbose(`media: file attachment skipped (unknown mime) index=${attachment.index}`);
			continue;
		}
		...
		if (!allowedMimes.has(mimeType)) {
			if (require_globals.shouldLogVerbose()) require_globals.logVerbose(`media: file attachment skipped (unsupported mime ${mimeType}) index=${attachment.index}`);
			continue;
		}
		let extracted;
		try {
			...
			extracted = await require_input_files.extractFileContentFromSource({
				source: {
					type: "base64",
			...
			if (require_globals.shouldLogVerbose()) require_globals.logVerbose(`media: file attachment skipped (extract): ${String(err)}`);
			continue;
		}
		...
		blocks.push(require_file_context.renderFileContextBlock({
			filename: bufferResult.fileName,
...
async function applyMediaUnderstanding(params) {
	...
	const originalUserText = [
		...
		ctx.RawBody,
		ctx.Body
	].map((value) => extractMediaUserText(value)).find((value) => value?.trim()) ?? void 0;
	const attachments = require_echo_transcript.normalizeMediaAttachments(ctx);
	const providerRegistry = require_echo_transcript.buildProviderRegistry(params.providers, cfg);
	const cache = require_echo_transcript.createMediaAttachmentCache(attachments, {
		localPathRoots: require_echo_transcript.resolveMediaAttachmentLocalRoots({
			cfg,
			ctx,
	...
	try {
		const results = await (0, p_map.default)(params.processingMode === "audio-only" ? AUDIO_ONLY_CAPABILITY_ORDER : CAPABILITY_ORDER, async (capability) => await runMediaCapability({
			capability,
			cfg,
			ctx,
			attachments: cache,
			...
			concurrency: require_resolve.resolveConcurrency(cfg),
			stopOnError: false
		});
		const outputs = [];
		const decisions = [];
		...
		if (outputs.length > 0) {
			...
			if (audioOutputs.length > 0) {
				...
				if (audioCfg?.echoTranscript && transcript) await require_echo_transcript.sendTranscriptEcho({
					ctx,
					cfg,
					transcript,
		...
		if (outputs.length > 0 || fileContext.blocks.length > 0) require_inbound_context.finalizeInboundContext(ctx, {
...
//#endregion
Object.defineProperty(exports, "applyMediaUnderstanding", {
	enumerable: true,
	get: function() {
		return applyMediaUnderstanding;
	}
});
