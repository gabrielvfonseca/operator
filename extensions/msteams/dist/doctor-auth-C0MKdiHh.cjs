...
function hasStoredCodexOAuthProfile() {
	const store = require_store.ensureAuthProfileStore(void 0, {
...
function hasLegacyCodexTransportOverride(providerOverride) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(providerOverride)) return false;
...
function buildCodexProviderOverrideWarning(providerOverride) {
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(providerOverride)) {
...
if (process.env.VITEST || false) globalThis[Symbol.for("operator.doctorAuthTestApi")] = { legacyCodexProviderOverrideToHealthFinding };
...
function noteLegacyCodexProviderOverride(cfg) {
	...
	require_note.note(buildCodexProviderOverrideWarning(providerOverride), CODEX_OAUTH_WARNING_TITLE);
}
...
function listAuthProfileHealthTargets(cfg) {
	...
	const addTarget = (agentId, agentDir, isDefault) => {
		const key = node_path.default.resolve(agentDir);
	...
	addTarget(defaultAgentId, require_agent_scope_config.resolveDefaultAgentDir(cfg), true);
	for (const agentId of require_agent_scope_config.listAgentIds(cfg)) {
		...
		if (require_source_check.hasLocalAuthProfileStoreSource(agentDir)) addTarget(agentId, agentDir, false);
	}
...
function formatOAuthRefreshFailureDoctorLine(params) {
	const classified = require_oauth_refresh_failure.classifyOAuthRefreshFailure(params.message);
	...
	const commandMarkdown = require_oauth_refresh_failure.formatOAuthRefreshFailureLoginCommandMarkdown(require_oauth_refresh_failure.buildOAuthRefreshFailureLoginCommand(provider, { profileId: provider === rawProvider ? params.profileId : void 0 }));
...
async function resolveAuthIssueHint(issue, cfg, store) {
	...
	if (issue.reasonCode === "malformed_api_key") return "Paste the API key value, not an Operator onboarding command.";
	const providerHint = await require_oauth.formatAuthDoctorHint({
		cfg,
		store,
	...
	return require_provider_auth_recovery_hint.buildProviderAuthRecoveryHint({ provider: issue.provider }).replace(/^Run /, "Re-auth via ");
}
async function formatAuthIssueLine(issue, cfg, store) {
	const remaining = issue.remainingMs !== void 0 ? ` (${require_auth_health.formatRemainingShort(issue.remainingMs)})` : "";
...
function resolveAuthProfileStorePath(target) {
	return require_path_resolve.resolveAuthStorePathForDisplay(target.agentDir);
}
function authProfileIssueToHealthFinding(params) {
	const remaining = params.issue.remainingMs !== void 0 ? ` (${require_auth_health.formatRemainingShort(params.issue.remainingMs)})` : "";
	...
	return {
		...
		fixHint: params.hint ?? (params.issue.status === "expiring" ? "Run `operator doctor --fix` to refresh expiring OAuth profiles, or re-authenticate static tokens." : "Run `operator doctor --fix` to refresh OAuth profiles, or re-authenticate this provider.")
	};
}
...
async function collectAuthProfileHealthFindingsForTarget(params) {
	const store = require_store.ensureAuthProfileStore(params.target.agentDir, {
	...
	for (const profileId of Object.keys(store.usageStats ?? {})) {
		const until = require_usage.resolveProfileUnusableUntilForDisplay(store, profileId);
		if (!until || now >= until) continue;
		const stats = store.usageStats?.[profileId];
		const remaining = require_auth_health.formatRemainingShort(until - now);
	...
	const issues = require_auth_health.buildAuthHealthSummary({
		store,
		cfg: params.cfg,
		warnAfterMs: require_auth_health.DEFAULT_OAUTH_WARN_MS,
		allowKeychainPrompt: params.allowKeychainPrompt
...
async function collectAuthProfileHealthFindings(params) {
	...
	const activeTargets = listAuthProfileHealthTargets(params.cfg).filter((target) => target.isDefault ? require_source_check.hasAnyAuthProfileStoreSource(target.agentDir) || configuredProfiles : require_source_check.hasLocalAuthProfileStoreSource(target.agentDir));
	const findings = [];
...
async function noteAuthProfileHealthForTarget(params) {
	const store = require_store.ensureAuthProfileStore(params.target.agentDir, { allowKeychainPrompt: params.allowKeychainPrompt });
	...
	if (unusable.length > 0) require_note.note(unusable.join("\n"), noteTitle("Auth profile cooldowns"));
	let summary = require_auth_health.buildAuthHealthSummary({
		store,
		cfg: params.cfg,
		warnAfterMs: require_auth_health.DEFAULT_OAUTH_WARN_MS,
		allowKeychainPrompt: params.allowKeychainPrompt
	...
	if (refreshTargets.length > 0 && await params.prompter.confirmAutoFix({
		...
		for (const profile of refreshTargets) try {
			await require_oauth.resolveApiKeyForProfile({
				cfg: params.cfg,
				store,
		...
		if (errors.length > 0) require_note.note(errors.join("\n"), noteTitle("OAuth refresh errors"));
		summary = require_auth_health.buildAuthHealthSummary({
			store: require_store.ensureAuthProfileStore(params.target.agentDir, { allowKeychainPrompt: false }),
			cfg: params.cfg,
			warnAfterMs: require_auth_health.DEFAULT_OAUTH_WARN_MS,
			allowKeychainPrompt: false
		});
...
async function noteAuthProfileHealth(params) {
	...
	require_note.note([...agentsByIssueLine.entries()].toSorted(([left], [right]) => left.localeCompare(right)).map(([line, agentIds]) => agentIds.size === activeTargets.length ? line : `${line} (agents: ${[...agentIds].toSorted().join(", ")})`).join("\n"), "Model auth");
}
//#endregion
exports.collectAuthProfileHealthFindings = collectAuthProfileHealthFindings;
exports.noteAuthProfileHealth = noteAuthProfileHealth;
exports.noteLegacyCodexProviderOverride = noteLegacyCodexProviderOverride;
