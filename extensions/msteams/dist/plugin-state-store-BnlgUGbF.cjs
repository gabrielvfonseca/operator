...
function resolvePluginStateExpiresAtMs(params) {
	...
	const expiresAt = (0, _gabrielvfonseca_normalization_core_number_coercion.resolveExpiresAtMsFromDurationMs)(params.ttlMs, { nowMs: params.now });
...
function wrapPluginStateError(error, operation, fallbackCode, message, pathname = require_openclaw_state_db.resolveOperatorStateSqlitePath(process.env)) {
...
function parseStoredJson(raw, operation) {
	try {
		...
		throw createPluginStateError({
			...
			path: require_openclaw_state_db.resolveOperatorStateSqlitePath(process.env),
			cause: error
		});
	}
}
function rowToEntry(row, operation) {
	const expiresAt = require_state_migrations_cron_run_logs.normalizeSqliteNumber(row.expires_at);
	return {
		key: row.entry_key,
...
function upsertPluginStateEntry(db, row) {
	require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, getPluginStateKysely(db).insertInto("plugin_state_entries").values(row).onConflict((conflict) => conflict.columns([
		"plugin_id",
		"namespace",
		"entry_key"
	]).doUpdateSet({
...
function insertPluginStateEntryIfAbsent(db, row) {
	const result = require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, getPluginStateKysely(db).insertInto("plugin_state_entries").orIgnore().values(row));
	return Number(result.numAffectedRows ?? 0) > 0;
}
function selectPluginStateEntry(db, params) {
	return require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(db, getPluginStateKysely(db).selectFrom("plugin_state_entries").select([
		"plugin_id",
		"namespace",
		"entry_key",
		"value_json",
...
function selectPluginStateEntries(db, params) {
	return require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, getPluginStateKysely(db).selectFrom("plugin_state_entries").select([
		"plugin_id",
		"namespace",
		"entry_key",
		"value_json",
...
function deletePluginStateEntry(db, params) {
	const result = require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, getPluginStateKysely(db).deleteFrom("plugin_state_entries").where("plugin_id", "=", params.pluginId).where("namespace", "=", params.namespace).where("entry_key", "=", params.key));
	return Number(result.numAffectedRows ?? 0);
}
function deleteExpiredPluginStateNamespaceEntries(db, params) {
	require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, getPluginStateKysely(db).deleteFrom("plugin_state_entries").where("plugin_id", "=", params.pluginId).where("namespace", "=", params.namespace).where("expires_at", "is not", null).where("expires_at", "<=", params.now));
}
function countLivePluginStateNamespaceEntries(db, params) {
	return countRow(require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(db, getPluginStateKysely(db).selectFrom("plugin_state_entries").select((eb) => eb.fn.countAll().as("count")).where("plugin_id", "=", params.pluginId).where("namespace", "=", params.namespace).where((eb) => eb.or([eb("expires_at", "is", null), eb("expires_at", ">", params.now)]))));
}
function countLivePluginStateEntries(db, params) {
	return countRow(require_state_migrations_cron_run_logs.executeSqliteQueryTakeFirstSync(db, getPluginStateKysely(db).selectFrom("plugin_state_entries").select((eb) => eb.fn.countAll().as("count")).where("plugin_id", "=", params.pluginId).where((eb) => eb.or([eb("expires_at", "is", null), eb("expires_at", ">", params.now)]))));
}
function deleteOldestPluginStateNamespaceEntries(db, params) {
	const keys = require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, getPluginStateKysely(db).selectFrom("plugin_state_entries").select(["entry_key"]).where("plugin_id", "=", params.pluginId).where("namespace", "=", params.namespace).where("entry_key", "!=", params.protectedKey).where((eb) => eb.or([eb("expires_at", "is", null), eb("expires_at", ">", params.now)])).orderBy("created_at", "asc").orderBy("entry_key", "asc").limit(params.limit)).rows;
...
function sweepExpiredPluginStateEntriesFromDatabase(db, now) {
	const result = require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, getPluginStateKysely(db).deleteFrom("plugin_state_entries").where("expires_at", "is not", null).where("expires_at", "<=", now));
	return Number(result.numAffectedRows ?? 0);
}
function openPluginStateDatabase(operation = "open", options = {}) {
	const pathname = require_openclaw_state_db.resolveOperatorStateSqlitePath(options.env ?? process.env);
	...
	try {
		const database = require_openclaw_state_db.openOperatorStateDatabase(options);
		cachedDatabase = {
			db: database.db,
			path: database.path
		};
...
function runWriteTransaction(operation, write, options = {}) {
	...
	return require_openclaw_state_db.runOperatorStateWriteTransaction(() => {
		return write(store);
	}, options);
}
...
function pluginStateClear(params) {
	try {
		runWriteTransaction("clear", ({ db }) => {
			require_state_migrations_cron_run_logs.executeSqliteQuerySync(db, getPluginStateKysely(db).deleteFrom("plugin_state_entries").where("plugin_id", "=", params.pluginId).where("namespace", "=", params.namespace));
		}, envOptions(params.env));
	} catch (error) {
...
function clearPluginStateDatabaseForTests() {
	...
	require_state_migrations_cron_run_logs.executeSqliteQuerySync(store.db, getPluginStateKysely(store.db).deleteFrom("plugin_state_entries"));
}
...
function probePluginStateStore() {
	const databasePath = require_openclaw_state_db.resolveOperatorStateSqlitePath(process.env);
	const steps = [];
	const wasOpen = cachedDatabase !== null;
	const stateWasOpen = require_openclaw_state_db.isOperatorStateDatabaseOpen();
	...
	try {
		require_state_migrations_cron_run_logs.requireNodeSqlite();
		pushOk("load-sqlite");
	} catch (error) {
	...
	try {
		...
		require_openclaw_state_db.openOperatorStateDatabase().walMaintenance.checkpoint();
		pushOk("checkpoint");
	} catch (error) {
...
if (process.env.VITEST || false) globalThis[Symbol.for("operator.pluginStateSqliteTestApi")] = {
...
if (process.env.VITEST || false) globalThis[Symbol.for("operator.pluginStateStoreTestApi")] = { clearPluginStateStoreForTests };
//#endregion
Object.defineProperty(exports, "MAX_PLUGIN_STATE_ENTRIES_PER_PLUGIN", {
	enumerable: true,
	get: function() {
		return MAX_PLUGIN_STATE_ENTRIES_PER_PLUGIN;
	}
});
Object.defineProperty(exports, "closePluginStateDatabase", {
	enumerable: true,
	get: function() {
		return closePluginStateDatabase;
	}
});
Object.defineProperty(exports, "countPluginStateLiveEntries", {
	enumerable: true,
	get: function() {
		return countPluginStateLiveEntries;
	}
});
Object.defineProperty(exports, "createCorePluginStateSyncKeyedStore", {
	enumerable: true,
	get: function() {
		return createCorePluginStateSyncKeyedStore;
	}
});
Object.defineProperty(exports, "createPluginStateKeyedStore", {
	enumerable: true,
	get: function() {
		return createPluginStateKeyedStore;
	}
});
Object.defineProperty(exports, "createPluginStateSyncKeyedStore", {
	enumerable: true,
	get: function() {
		return createPluginStateSyncKeyedStore;
	}
});
Object.defineProperty(exports, "isPluginStateDatabaseOpen", {
	enumerable: true,
	get: function() {
		return isPluginStateDatabaseOpen;
	}
});
Object.defineProperty(exports, "sweepExpiredPluginStateEntries", {
	enumerable: true,
	get: function() {
		return sweepExpiredPluginStateEntries;
	}
});
