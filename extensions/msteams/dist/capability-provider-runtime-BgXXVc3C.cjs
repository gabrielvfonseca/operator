...
const require_bundled_capability_runtime = require("./bundled-capability-runtime-B5HLLMLn.cjs");
...
function resolveCapabilityPluginIds(params) {
	...
	const contractPlugins = snapshot.plugins.filter((plugin) => require_manifest_contract_eligibility.hasManifestContractValue({
		plugin,
		contract: contractKey,
		value: params.providerId
	}));
	return {
		runtimePluginIds: (0, _gabrielvfonseca_normalization_core_string_normalization.sortUniqueStrings)(contractPlugins.filter((plugin) => require_manifest_contract_eligibility.isManifestPluginAvailableForControlPlane({
			snapshot,
			plugin,
			config: params.cfg
		})).map((plugin) => plugin.id)),
		bundledCompatPluginIds: (0, _gabrielvfonseca_normalization_core_string_normalization.sortUniqueStrings)(contractPlugins.filter((plugin) => plugin.origin === "bundled").map((plugin) => plugin.id))
	};
}
...
function resolveCapabilityProviderSnapshotCacheKey(params) {
	return JSON.stringify({
		key: params.key,
		load: require_loader.resolvePluginRegistryLoadCacheKey(params.loadOptions)
	});
}
function findProviderById(entries, providerId) {
	const normalizedProviderId = require_worker_provider_registry.normalizeCapabilityProviderId(providerId);
	if (!normalizedProviderId) return;
	const providerEntries = entries;
	for (const entry of providerEntries) if (typeof entry.provider.id === "string" && require_worker_provider_registry.normalizeCapabilityProviderId(entry.provider.id) === normalizedProviderId) return entry.provider;
	for (const entry of providerEntries) if ((Array.isArray(entry.provider.aliases) ? entry.provider.aliases : []).some((alias) => typeof alias === "string" && require_worker_provider_registry.normalizeCapabilityProviderId(alias) === normalizedProviderId)) return entry.provider;
}
...
function addModelConfigProviderIds(target, value) {
	for (const ref of require_voice_models.resolveVoiceModelRefs(value)) addStringValue(target, ref.provider);
}
...
function resolveRequestedCapabilityPluginIds(params) {
	...
	return runtimePluginIds.size > 0 ? {
		runtimePluginIds: (0, _gabrielvfonseca_normalization_core_string_normalization.sortUniqueStrings)(runtimePluginIds),
...
function loadCapabilityProviderEntries(params) {
	...
	const coldEntries = (loadedRegistry ? void 0 : require_loader.resolveRuntimePluginRegistry(params.loadOptions))?.[params.key] ?? [];
	...
	const captured = require_bundled_capability_runtime.loadBundledCapabilityRuntimeRegistry({
...
function resolvePluginCapabilityProvider(params) {
	...
	const activeProvider = findProviderById(require_active_runtime_registry.getLoadedRuntimePluginRegistry()?.[params.key] ?? [], params.providerId);
	...
	return findProviderById(require_plugin_cache_primitives.resolveConfigScopedRuntimeCacheValue({
...
function resolveCachedCapabilityProviderEntries(params) {
	return require_plugin_cache_primitives.resolveConfigScopedRuntimeCacheValue({
		cache: capabilityProviderSnapshotCache,
		config: params.cfg,
		key: resolveCapabilityProviderSnapshotCacheKey({
			key: params.key,
...
function resolvePluginCapabilityProviders(params) {
	if (shouldSkipCapabilityResolution(params)) return [];
	const activeProviders = require_active_runtime_registry.getLoadedRuntimePluginRegistry()?.[params.key] ?? [];
...
//#endregion
Object.defineProperty(exports, "loadCapabilityManifestSnapshot", {
	enumerable: true,
	get: function() {
		return loadCapabilityManifestSnapshot;
	}
});
Object.defineProperty(exports, "resolvePluginCapabilityProvider", {
	enumerable: true,
	get: function() {
		return resolvePluginCapabilityProvider;
	}
});
Object.defineProperty(exports, "resolvePluginCapabilityProviders", {
	enumerable: true,
	get: function() {
