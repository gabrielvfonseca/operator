...
const resolveWebLoginProvider = () => require_registry.listChannelPlugins().find((plugin) => [...plugin.gatewayMethods ?? [], ...(plugin.gatewayMethodDescriptors ?? []).map((descriptor) => descriptor.name)].some((method) => WEB_LOGIN_METHODS.has(method))) ?? null;
...
function resolveMissingWebLoginPluginHint(context) {
	...
	const hints = Object.keys(channels).map((channelId) => require_official_external_plugin_repair_hints.resolveMissingOfficialExternalChannelPluginRepairHint({
		config: cfg,
		channelId
	})).filter((hint) => Boolean(hint));
	if (hints.length === 0) return null;
	if (hints.length === 1) return (0, _gabrielvfonseca_normalization_core.expectDefined)(hints[0], "hints entry at 0").repairHint;
	const labels = [...new Set(hints.map((hint) => hint.label))];
	const installCommands = [...new Set(hints.map((hint) => hint.installCommand))];
	const doctorFixCommand = (0, _gabrielvfonseca_normalization_core.expectDefined)(hints[0], "hints entry at 0").doctorFixCommand;
...
function respondProviderUnavailable(params) {
	...
	params.respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, message));
}
function respondProviderUnsupported(respond, providerId) {
	respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, `web login is not supported by provider ${providerId}`));
}
...
const webHandlers = {
	"web.login.start": async ({ params, respond, context }) => {
		if (!require_validation.assertValidParams(params, require_src.validateWebLoginStartParams, "web.login.start", respond)) return;
		try {
	...
	"web.login.wait": async ({ params, respond, context }) => {
		if (!require_validation.assertValidParams(params, require_src.validateWebLoginWaitParams, "web.login.wait", respond)) return;
		try {
			const request = resolveWebLoginRequest({
				rawParams: params,
				respond,
