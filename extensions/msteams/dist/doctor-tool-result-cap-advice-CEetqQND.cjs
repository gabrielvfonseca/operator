...
function collectToolResultCapDoctorIssues(params) {
	...
	const autoCap = require_tool_result_truncation.resolveAutoLiveToolResultMaxChars(params.contextWindowTokens);
	const runtimeCeiling = require_tool_result_truncation.calculateMaxToolResultCharsWithCap(params.contextWindowTokens, Number.MAX_SAFE_INTEGER);
	const effectiveCap = require_tool_result_truncation.calculateMaxToolResultCharsWithCap(params.contextWindowTokens, configuredCap);
	const autoEffectiveCap = require_tool_result_truncation.calculateMaxToolResultCharsWithCap(params.contextWindowTokens, autoCap);
...
function buildToolResultCapDoctorAdvice(params) {
	...
	const effectiveCap = require_tool_result_truncation.calculateMaxToolResultCharsWithCap(params.contextWindowTokens, requestedCap);
	const lines = [];
...
//#endregion
exports.buildToolResultCapDoctorAdvice = buildToolResultCapDoctorAdvice;
exports.collectToolResultCapDoctorIssues = collectToolResultCapDoctorIssues;
exports.toolResultCapDoctorIssueToHealthFinding = toolResultCapDoctorIssueToHealthFinding;
