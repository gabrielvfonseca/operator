...
*
* Opt out per-process by setting `OPERATOR_CHILD_OOM_SCORE_ADJ=0` (also
* accepts `false`/`no`/`off`). Callers may also provide the key via
* `params.env` for per-child overrides.
*/
const CHILD_OOM_SCORE_ADJ_ENV_KEY = "OPERATOR_CHILD_OOM_SCORE_ADJ";
...
function defaultShellAvailable() {
	...
	try {
		cachedShellAvailable = node_fs.default.statSync(OOM_SCORE_WRAP_SHELL).isFile();
	} catch {
...
//#endregion
Object.defineProperty(exports, "prepareOomScoreAdjustedSpawn", {
	enumerable: true,
	get: function() {
		return prepareOomScoreAdjustedSpawn;
	}
});
