...
function resolveHistoryImageContentType(media) {
	const contentType = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(media.contentType);
	if (contentType?.startsWith("image/")) return contentType;
	return (0, _gabrielvfonseca_media_core_mime.mimeTypeFromFilePath)((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(media.path));
}
...
function resolveRecentInboundHistoryImages(params) {
	...
	for (let index = entries.length - 1; index >= 0 && out.length < limit; index -= 1) {
		const entry = (0, _gabrielvfonseca_normalization_core.expectDefined)(entries[index], "entries entry at index");
		...
		for (let mediaIndex = mediaEntries.length - 1; mediaIndex >= 0 && out.length < limit; mediaIndex -= 1) {
			...
			const mediaPath = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(media.path);
			if (!mediaPath || isRemotePath(mediaPath)) continue;
			const contentType = resolveHistoryImageContentType(media);
			if (!contentType?.startsWith("image/")) continue;
			const messageId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(media.messageId) ?? entry.messageId;
...
const agentTurnMediaRuntimeLoader = require_lazy_promise.createLazyImportLoader(() => Promise.resolve().then(() => require("./dispatch-acp-media.runtime-CBqw4hfu.cjs")));
...
async function resolveAgentTurnAttachments(params) {
	...
	if (!require_inbound_media.hasInboundMedia(params.ctx) && !(includeRecentHistoryImages && hasInboundHistoryMedia(params.ctx))) return {
		attachments: [],
		recentHistoryImages: []
	};
	const runtime = params.runtime ?? await loadAgentTurnMediaRuntime();
	const currentAttachments = runtime.normalizeAttachments(params.ctx).map((attachment) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(attachment.path) ? Object.assign({}, attachment, { url: void 0 }) : attachment);
	...
	const resolveImageAttachment = async (attachment) => {
		...
		if (!(0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(attachment.path)) return false;
		try {
			...
			if (runtime.isMediaUnderstandingSkipError(error)) require_globals.logVerbose(`agent-turn-attachments: skipping attachment #${attachment.index + 1} (${error.reason})`);
			else {
				const errorName = error instanceof Error ? error.name : typeof error;
				require_globals.logVerbose(`agent-turn-attachments: failed to read attachment #${attachment.index + 1} (${errorName})`);
			}
			return false;
		}
	};
...
//#endregion
Object.defineProperty(exports, "resolveAgentTurnAttachments", {
	enumerable: true,
	get: function() {
		return resolveAgentTurnAttachments;
	}
});
Object.defineProperty(exports, "resolveInlineAgentImageAttachments", {
	enumerable: true,
	get: function() {
		return resolveInlineAgentImageAttachments;
	}
});
