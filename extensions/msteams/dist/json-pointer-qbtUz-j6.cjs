...
function readJsonPointer(root, pointer, options = {}) {
	...
	for (const token of tokens) {
		if (Array.isArray(current)) {
			const index = require_path_array_index.parseConfigPathArrayIndex(token);
		...
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(current)) return failOrUndefined({
			onMissing,
...
//#endregion
Object.defineProperty(exports, "encodeJsonPointerToken", {
	enumerable: true,
	get: function() {
		return encodeJsonPointerToken;
	}
});
Object.defineProperty(exports, "readJsonPointer", {
	enumerable: true,
	get: function() {
		return readJsonPointer;
	}
});
