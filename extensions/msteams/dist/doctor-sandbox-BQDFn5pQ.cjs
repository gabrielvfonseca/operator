...
function resolveSandboxScript(scriptRel, options = {}) {
	const roots = require_operator_root.resolveOperatorPackageRootsSync({
		cwd: options.cwd ?? process.cwd(),
		argv1: options.argv1 ?? process.argv[1]
	});
	for (const root of roots) {
		const scriptPath = node_path.default.join(root, scriptRel);
		if (node_fs.default.existsSync(scriptPath)) return {
			scriptPath,
			cwd: root
		};
	}
...
if (process.env.VITEST || false) globalThis[Symbol.for("operator.doctorSandboxTestApi")] = { resolveSandboxScript };
async function runSandboxScript(scriptRel, runtime) {
	const script = resolveSandboxScript(scriptRel);
	if (!script) {
		require_note.note(`Unable to locate ${scriptRel}. Run it from the repo root.`, "Sandbox");
		return false;
	}
	runtime.log(`Running ${scriptRel}...`);
	const result = await require_exec.runCommandWithTimeout(["bash", script.scriptPath], {
		timeoutMs: 1200 * 1e3,
		cwd: script.cwd
	});
	if (result.code !== 0) {
...
async function isDockerAvailable() {
	try {
		await require_exec.runExec("docker", [
			"version",
			"--format",
			"{{.Server.Version}}"
		], { timeoutMs: 5e3 });
...
async function runCodexBwrapNamespaceProbe(kind, args) {
	try {
		await require_exec.runExec("unshare", args, { timeoutMs: 5e3 });
		return { ok: true };
	} catch (error) {
...
async function noteCodexBwrapNamespaceWarning(cfg) {
	...
	require_note.note([
		...
		"",
		"Fix the host namespace policy for the Operator service user, then restart the gateway.",
		"Prefer an AppArmor profile that grants the required namespaces to the Operator service process.",
...
async function dockerImageExists(image) {
	try {
		...
		if (require_docker.isDockerDaemonUnavailable(stderr)) return false;
		throw error;
	}
}
function resolveSandboxDockerImage(cfg) {
	const image = cfg.agents?.defaults?.sandbox?.docker?.image?.trim();
	return image ? image : require_constants.DEFAULT_SANDBOX_IMAGE;
}
...
function resolveSandboxBrowserImage(cfg) {
	...
	return image ? image : require_constants.DEFAULT_SANDBOX_BROWSER_IMAGE;
}
...
async function handleMissingSandboxImage(params, runtime, prompter) {
	...
	require_note.note(`Sandbox ${params.kind} image missing: ${params.image}. ${buildHint}`, "Sandbox");
...
async function maybeRepairSandboxImages(cfg, runtime, prompter) {
	...
	if (backend !== "docker") {
		if (sandbox.browser?.enabled) require_note.note(`Sandbox backend "${backend}" selected. Docker browser health checks are skipped; browser sandbox currently requires the docker backend.`, "Sandbox");
		return cfg;
	}
	...
	await handleMissingSandboxImage({
		kind: "base",
		image: dockerImage,
		buildScript: dockerImage === "operator-sandbox-common:bookworm-slim" ? "scripts/sandbox-common-setup.sh" : dockerImage === "operator-sandbox:bookworm-slim" ? "scripts/sandbox-setup.sh" : void 0,
		updateConfig: (image) => {
	...
	if (changes.length > 0) require_note.note(changes.join("\n"), "Doctor changes");
	return next;
}
function formatLegacyRegistryInspectionLine(file) {
	...
	return `- ${file.kind} ${file.source}: ${require_utils.shortenHomePath(sourcePath)} (${status})`;
}
...
function formatLegacyRegistryMigrationLine(result) {
	...
	if (result.status === "quarantined-invalid") {
		const file = require_utils.shortenHomePath(result.source === "sharded" ? result.shardedDir : result.registryPath);
		const quarantine = result.quarantinePath ? ` to ${require_utils.shortenHomePath(result.quarantinePath)}` : "";
...
async function detectLegacySandboxRegistryFileIssues() {
	return (await require_docker.inspectLegacySandboxRegistryFiles()).filter((file) => file.exists);
}
...
${formatLegacyRegistryInspectionLine(file)}`,
		...
		fixHint: `Run ${require_command_format.formatCliCommand("operator doctor --fix")} to migrate valid entries to SQLite.`
	};
}
...
async function maybeRepairSandboxRegistryFiles(prompter) {
	...
	if (!prompter.shouldRepair) {
		require_note.note([
			...
			`Run ${require_command_format.formatCliCommand("operator doctor --fix")} to migrate them to SQLite.`
		].join("\n"), "Sandbox");
		return;
	}
	const results = (await require_docker.migrateLegacySandboxRegistryFiles()).filter((result) => result.status !== "missing").map(formatLegacyRegistryMigrationLine).filter((line) => line.length > 0);
	if (results.length > 0) require_note.note(results.join("\n"), "Doctor changes");
}
...
function noteSandboxScopeWarnings(cfg) {
	...
	for (const agent of agents) {
		...
		if (require_config.resolveSandboxScope({ scope: agentSandbox.scope ?? globalSandbox?.scope }) !== "shared") continue;
		const overrides = [];
...
//#endregion
exports.detectLegacySandboxRegistryFileIssues = detectLegacySandboxRegistryFileIssues;
exports.legacySandboxRegistryInspectionToHealthFinding = legacySandboxRegistryInspectionToHealthFinding;
exports.legacySandboxRegistryInspectionToRepairEffect = legacySandboxRegistryInspectionToRepairEffect;
exports.maybeRepairSandboxImages = maybeRepairSandboxImages;
exports.maybeRepairSandboxRegistryFiles = maybeRepairSandboxRegistryFiles;
