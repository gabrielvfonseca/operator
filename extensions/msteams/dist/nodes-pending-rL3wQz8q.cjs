...
const require_nodes = require("./nodes-DTMZ-I8B.cjs");
...
function pruneExpired(state, nowMs) {
	const validNowMs = (0, _gabrielvfonseca_normalization_core_number_coercion.asDateTimestampMs)(nowMs);
	if (validNowMs === void 0) return false;
	let changed = false;
	for (const [id, item] of state.itemsById) if (item.expiresAtMs !== null && !(0, _gabrielvfonseca_normalization_core_number_coercion.isFutureDateTimestampMs)(item.expiresAtMs, { nowMs: validNowMs })) {
...
function makeBaselineStatusItem(nowMs) {
	return {
		...
		createdAtMs: (0, _gabrielvfonseca_normalization_core_number_coercion.resolveDateTimestampMs)(nowMs),
		expiresAtMs: null
	};
}
function resolvePendingWorkExpiresAtMs(expiresInMs, nowMs) {
	return (0, _gabrielvfonseca_normalization_core_number_coercion.resolveExpiresAtMsFromDurationMs)(typeof expiresInMs === "number" && Number.isFinite(expiresInMs) ? Math.max(1e3, Math.trunc(expiresInMs)) : DEFAULT_PENDING_WORK_TTL_MS, { nowMs }) ?? 0;
}
function enqueueNodePendingWork(params) {
	const nodeId = params.nodeId.trim();
	if (!nodeId) throw new Error("nodeId required");
	const rawNowMs = Date.now();
	const nowMs = (0, _gabrielvfonseca_normalization_core_number_coercion.resolveDateTimestampMs)(rawNowMs);
...
function drainNodePendingWork(nodeId, opts = {}) {
	...
	const nowMs = (0, _gabrielvfonseca_normalization_core_number_coercion.resolveDateTimestampMs)(opts.nowMs ?? Date.now());
...
const nodePendingHandlers = {
	"node.pending.drain": async ({ params, respond, client }) => {
		if (!require_src.validateNodePendingDrainParams(params)) {
			require_nodes_helpers.respondInvalidParams({
				respond,
				method: "node.pending.drain",
				validator: require_src.validateNodePendingDrainParams
			});
			return;
		}
		const nodeId = resolveClientNodeId(client);
		if (!nodeId) {
			respond(false, void 0, require_error_codes.errorShape(require_error_codes.ErrorCodes.INVALID_REQUEST, "node.pending.drain requires a connected device identity"));
			return;
		}
		respond(true, {
			nodeId,
	...
	"node.pending.enqueue": async ({ params, respond, context }) => {
		if (!require_src.validateNodePendingEnqueueParams(params)) {
			require_nodes_helpers.respondInvalidParams({
				respond,
				method: "node.pending.enqueue",
				validator: require_src.validateNodePendingEnqueueParams
			});
			return;
		}
		const p = params;
		await require_nodes_helpers.respondUnavailableOnThrow(respond, async () => {
			...
			if (p.wake !== false && !queued.deduped && !context.nodeRegistry.get(p.nodeId)) {
				...
				const wake = await require_nodes.maybeWakeNodeWithApns(p.nodeId, {
				...
				if (wake.available) {
					const reconnected = await require_nodes.waitForNodeReconnect({
						nodeId: p.nodeId,
						context,
						timeoutMs: require_nodes_wake_state.NODE_WAKE_RECONNECT_WAIT_MS
					});
					context.logGateway.info(`node pending wake stage=wait1 node=${p.nodeId} req=${wakeReqId} reconnected=${reconnected} timeoutMs=${require_nodes_wake_state.NODE_WAKE_RECONNECT_WAIT_MS}`);
				}
				if (!context.nodeRegistry.get(p.nodeId) && wake.available) {
					const retryWake = await require_nodes.maybeWakeNodeWithApns(p.nodeId, {
						force: true,
						wakeReason: "node.pending",
						cfg
					});
					...
					if (retryWake.available) {
						const reconnected = await require_nodes.waitForNodeReconnect({
							nodeId: p.nodeId,
							context,
							timeoutMs: require_nodes_wake_state.NODE_WAKE_RECONNECT_RETRY_WAIT_MS
						});
						context.logGateway.info(`node pending wake stage=wait2 node=${p.nodeId} req=${wakeReqId} reconnected=${reconnected} timeoutMs=${require_nodes_wake_state.NODE_WAKE_RECONNECT_RETRY_WAIT_MS}`);
					}
				}
				if (!context.nodeRegistry.get(p.nodeId)) {
					const nudge = await require_nodes.maybeSendNodeWakeNudge(p.nodeId, { cfg });
...
//#endregion
exports.nodePendingHandlers = nodePendingHandlers;
