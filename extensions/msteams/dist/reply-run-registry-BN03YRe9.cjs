...
function createReplyRunSettleTimer(params) {
	...
	const settleTimer = {
		...
		renew(timeoutMs) {
			...
			}, (0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(timeoutMs, REPLY_RUN_FINALIZATION_SETTLE_TIMEOUT_MS, 1));
			timer.unref?.();
...
function createReplyRunFinalizationLease(params) {
	...
	const lease = {
		...
		beginWork(timeoutMs) {
			...
			workDeadlinesMs.set(workId, Date.now() + (0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(timeoutMs, REPLY_RUN_FINALIZATION_SETTLE_TIMEOUT_MS, 1));
			recordActivity();
			let active = true;
			return () => {
				if (!active) return;
...
var reply_run_registry_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	REPLY_RUN_IDLE_SETTLE_TIMEOUT_MS: () => REPLY_RUN_IDLE_SETTLE_TIMEOUT_MS,
	REPLY_RUN_TERMINAL_SETTLE_TIMEOUT_MS: () => REPLY_RUN_TERMINAL_SETTLE_TIMEOUT_MS,
	ReplyRunAlreadyActiveError: () => ReplyRunAlreadyActiveError,
	ReplyRunFollowupAdmissionBlockedError: () => ReplyRunFollowupAdmissionBlockedError,
	abortActiveReplyRuns: () => abortActiveReplyRuns,
	abortReplyRunBySessionId: () => abortReplyRunBySessionId,
	clearReplyRunForResetBySessionId: () => clearReplyRunForResetBySessionId,
	createReplyOperation: () => createReplyOperation,
	expireStaleReplyOperation: () => expireStaleReplyOperation,
	expireStaleReplyRunBySessionId: () => expireStaleReplyRunBySessionId,
	forceClearReplyRunBySessionId: () => forceClearReplyRunBySessionId,
	getActiveReplyRunCount: () => getActiveReplyRunCount,
	isReplyRunAbortableForCompaction: () => isReplyRunAbortableForCompaction,
	isReplyRunAbortableForSignal: () => isReplyRunAbortableForSignal,
	isReplyRunActiveForSessionId: () => isReplyRunActiveForSessionId,
	isReplyRunEvidenceStale: () => isReplyRunEvidenceStale,
	isReplyRunEvidenceStaleBySessionId: () => isReplyRunEvidenceStaleBySessionId,
	isReplyRunStreamingForSessionId: () => isReplyRunStreamingForSessionId,
	listActiveReplyRunSessionIds: () => listActiveReplyRunSessionIds,
	listActiveReplyRunSessionKeys: () => listActiveReplyRunSessionKeys,
	queueReplyRunMessage: () => queueReplyRunMessage,
	replyRunRegistry: () => replyRunRegistry,
	resolveActiveReplyRunSessionId: () => resolveActiveReplyRunSessionId,
	resolveActiveReplyRunThreadId: () => resolveActiveReplyRunThreadId,
	resolveReplyBackendQueueMessageMismatch: () => resolveReplyBackendQueueMessageMismatch,
	resolveReplyRunPhaseForSessionId: () => resolveReplyRunPhaseForSessionId,
	retainReplyOperationUntilComplete: () => retainReplyOperationUntilComplete,
	runAfterReplyOperationClear: () => runAfterReplyOperationClear,
	waitForReplyBarrierSettlement: () => waitForReplyBarrierSettlement,
	waitForReplyRunEndBySessionId: () => waitForReplyRunEndBySessionId,
	waitForReplyRunFollowupAdmission: () => waitForReplyRunFollowupAdmission
});
...
const replyRunState = require_global_singleton.resolveGlobalSingleton(Symbol.for("operator.replyRunRegistry"), () => ({
...
function createUserAbortError() {
	return require_abort_signal.createAbortError("Reply operation aborted by user");
}
...
function waitForReplyBarrierSettlement(barrier, timeout = REPLY_RUN_IDLE_SETTLE_TIMEOUT_MS) {
	return new Promise((resolve) => {
		...
		if (typeof timeout === "number") schedule((0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(timeout, REPLY_RUN_IDLE_SETTLE_TIMEOUT_MS), finish);
		else {
			const startedAt = Date.now();
			const maxTimeoutMs = (0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(timeout.maxTimeoutMs, REPLY_RUN_IDLE_SETTLE_TIMEOUT_MS);
...
function markReplyRunDiagnosticProgress(params) {
	require_diagnostic_run_activity.markDiagnosticRunProgress({
...
function createReplyOperation(params) {
	...
	const operation = {
		...
		hasOwnedSessionId(candidateSessionId) {
			const normalizedSessionId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(candidateSessionId);
		...
		updateSessionId(nextSessionId) {
			if (result) return;
			const normalizedNextSessionId = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(nextSessionId);
		...
		updateSessionKey(nextSessionKey) {
			const normalizedNextKey = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(nextSessionKey);
		...
		abortForRestart() {
			if (!isReplyOperationAbortable(operation)) return false;
			const phaseBeforeAbort = phase;
			abortWithReason("restart", require_run_termination.createAgentRunRestartAbortError(), { abortedCode: "aborted_for_restart" });
			if (isReplyOperationPreBackendPhase(phaseBeforeAbort) && !retainStateUntilCompleteOperations.has(operation)) clearState();
			else scheduleTerminalSettle();
			return true;
		}
	...
	expireReplyOperationByOperation.set(operation, (reason) => {
		...
		abortInternally(require_abort_signal.createAbortError("Reply operation expired as stale"));
		require_diagnostic_runtime.diagnosticLogger.warn(`reply run stale takeover: forced release sessionKey=${currentSessionKey} reason=${reason} phase=${phase} result=${formatReplyOperationResult(result)} ageMs=${Date.now() - lastActivityAtMs} ranForMs=${Date.now() - startedAtMs}`);
		clearState();
		return true;
	});
	const finalizationLease = createReplyRunFinalizationLease({
		...
		onExpire: () => {
			require_diagnostic_runtime.diagnosticLogger.warn(`reply run finalization settle: forced release sessionKey=${currentSessionKey} phase=${phase} result=${formatReplyOperationResult(result)} ageMs=${Date.now() - lastActivityAtMs} ranForMs=${Date.now() - startedAtMs}`);
	...
	const terminalSettleTimer = createReplyRunSettleTimer({
		...
		onExpire: () => {
			require_diagnostic_runtime.diagnosticLogger.warn(`reply run terminal settle: forced release sessionKey=${currentSessionKey} phase=${phase} result=${formatReplyOperationResult(result)} ageMs=${Date.now() - lastActivityAtMs} ranForMs=${Date.now() - startedAtMs}`);
			clearState();
		}
	});
	...
	if (upstreamAbortSignal) {
		...
		const abortFromUpstream = () => {
			if (result) return;
			const restart = require_run_termination.isAgentRunRestartAbortReason(upstreamAbortSignal.reason);
			const phaseBeforeAbort = phase;
...
function isReplyRunEvidenceStale(operation) {
	...
	return !operation.result && Date.now() - operation.lastActivityAtMs > require_diagnostic_run_activity.resolveRunStaleThresholdMs(activity);
}
...
const replyRunRegistry = {
	...
	waitForIdle(sessionKey, timeoutMs, opts) {
		...
		return new Promise((resolve) => {
			...
			if (typeof timeoutMs === "number" && Number.isFinite(timeoutMs)) waiter.timer = setTimeout(() => waiter.finish(false), (0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(timeoutMs, 100, 100));
			if (opts?.signal) {
...
function queueReplyRunMessage(sessionId, text, options) {
	...
	if (operation?.phase !== "running" || !backend?.queueMessage) return false;
	...
	(options ? backend.queueMessage(text, options) : backend.queueMessage(text)).catch((error) => {
		require_diagnostic_runtime.diagnosticLogger.debug(`queued reply run message rejected: sessionId=${sessionId} error=${String(error)}`);
	});
	return true;
}
...
async function waitForReplyRunFollowupAdmission(sessionKey, timeoutMs, opts) {
	...
	const resolvedTimeoutMs = (0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(timeoutMs, 100, 100);
...
if (process.env.VITEST === "true" || false) globalThis[Symbol.for("operator.replyRunRegistryTestApi")] = replyRunRegistryTestApi;
//#endregion
Object.defineProperty(exports, "REPLY_RUN_IDLE_SETTLE_TIMEOUT_MS", {
	enumerable: true,
	get: function() {
		return REPLY_RUN_IDLE_SETTLE_TIMEOUT_MS;
	}
});
Object.defineProperty(exports, "REPLY_RUN_TERMINAL_SETTLE_TIMEOUT_MS", {
	enumerable: true,
	get: function() {
		return REPLY_RUN_TERMINAL_SETTLE_TIMEOUT_MS;
	}
});
Object.defineProperty(exports, "ReplyRunAlreadyActiveError", {
	enumerable: true,
	get: function() {
		return ReplyRunAlreadyActiveError;
	}
});
Object.defineProperty(exports, "ReplyRunFollowupAdmissionBlockedError", {
	enumerable: true,
	get: function() {
		return ReplyRunFollowupAdmissionBlockedError;
	}
});
Object.defineProperty(exports, "abortActiveReplyRuns", {
	enumerable: true,
	get: function() {
		return abortActiveReplyRuns;
	}
});
Object.defineProperty(exports, "abortReplyRunBySessionId", {
	enumerable: true,
	get: function() {
		return abortReplyRunBySessionId;
	}
});
Object.defineProperty(exports, "beginReplyOperationFinalizationWork", {
	enumerable: true,
	get: function() {
		return beginReplyOperationFinalizationWork;
	}
});
Object.defineProperty(exports, "clearReplyRunForResetBySessionId", {
	enumerable: true,
	get: function() {
		return clearReplyRunForResetBySessionId;
	}
});
Object.defineProperty(exports, "createReplyOperation", {
	enumerable: true,
	get: function() {
		return createReplyOperation;
	}
});
Object.defineProperty(exports, "expireStaleReplyOperation", {
	enumerable: true,
	get: function() {
		return expireStaleReplyOperation;
	}
});
Object.defineProperty(exports, "expireStaleReplyRunBySessionId", {
	enumerable: true,
	get: function() {
		return expireStaleReplyRunBySessionId;
	}
});
Object.defineProperty(exports, "forceClearReplyRunBySessionId", {
	enumerable: true,
	get: function() {
		return forceClearReplyRunBySessionId;
	}
});
Object.defineProperty(exports, "getActiveReplyRunCount", {
	enumerable: true,
	get: function() {
		return getActiveReplyRunCount;
	}
});
Object.defineProperty(exports, "isReplyRunAbortableForCompaction", {
	enumerable: true,
	get: function() {
		return isReplyRunAbortableForCompaction;
	}
});
Object.defineProperty(exports, "isReplyRunAbortableForSignal", {
	enumerable: true,
	get: function() {
		return isReplyRunAbortableForSignal;
	}
});
Object.defineProperty(exports, "isReplyRunActiveForSessionId", {
	enumerable: true,
	get: function() {
		return isReplyRunActiveForSessionId;
	}
});
Object.defineProperty(exports, "isReplyRunEvidenceStale", {
	enumerable: true,
	get: function() {
		return isReplyRunEvidenceStale;
	}
});
Object.defineProperty(exports, "isReplyRunEvidenceStaleBySessionId", {
	enumerable: true,
	get: function() {
		return isReplyRunEvidenceStaleBySessionId;
	}
});
Object.defineProperty(exports, "isReplyRunStreamingForSessionId", {
	enumerable: true,
	get: function() {
		return isReplyRunStreamingForSessionId;
	}
});
Object.defineProperty(exports, "listActiveReplyRunSessionIds", {
	enumerable: true,
	get: function() {
		return listActiveReplyRunSessionIds;
	}
});
Object.defineProperty(exports, "listActiveReplyRunSessionKeys", {
	enumerable: true,
	get: function() {
		return listActiveReplyRunSessionKeys;
	}
});
Object.defineProperty(exports, "queueReplyRunMessage", {
	enumerable: true,
	get: function() {
		return queueReplyRunMessage;
	}
});
Object.defineProperty(exports, "replyRunRegistry", {
	enumerable: true,
	get: function() {
		return replyRunRegistry;
	}
});
Object.defineProperty(exports, "reply_run_registry_exports", {
	enumerable: true,
	get: function() {
		return reply_run_registry_exports;
	}
});
Object.defineProperty(exports, "resolveActiveReplyRunSessionId", {
	enumerable: true,
	get: function() {
		return resolveActiveReplyRunSessionId;
	}
});
Object.defineProperty(exports, "resolveActiveReplyRunThreadId", {
	enumerable: true,
	get: function() {
		return resolveActiveReplyRunThreadId;
	}
});
Object.defineProperty(exports, "resolveReplyBackendQueueMessageMismatch", {
	enumerable: true,
	get: function() {
		return resolveReplyBackendQueueMessageMismatch;
	}
});
Object.defineProperty(exports, "resolveReplyRunPhaseForSessionId", {
	enumerable: true,
	get: function() {
		return resolveReplyRunPhaseForSessionId;
	}
});
Object.defineProperty(exports, "retainReplyOperationUntilComplete", {
	enumerable: true,
	get: function() {
		return retainReplyOperationUntilComplete;
	}
});
Object.defineProperty(exports, "runAfterReplyOperationClear", {
	enumerable: true,
	get: function() {
		return runAfterReplyOperationClear;
	}
});
Object.defineProperty(exports, "waitForReplyBarrierSettlement", {
	enumerable: true,
	get: function() {
		return waitForReplyBarrierSettlement;
	}
});
Object.defineProperty(exports, "waitForReplyRunEndBySessionId", {
	enumerable: true,
	get: function() {
		return waitForReplyRunEndBySessionId;
	}
});
Object.defineProperty(exports, "waitForReplyRunFollowupAdmission", {
	enumerable: true,
	get: function() {
		return waitForReplyRunFollowupAdmission;
	}
});
