...
const require_promised_work_prompt = require("./promised-work-prompt-NTG_g1HH.cjs");
...
typebox_compile = require_rolldown_runtime.__toESM(typebox_compile, 1);
let typebox_format = require("typebox/format");
typebox_format = require_rolldown_runtime.__toESM(typebox_format, 1);
let node_module = require("node:module");
let chalk = require("chalk");
chalk = require_rolldown_runtime.__toESM(chalk, 1);
let node_crypto = require("node:crypto");
let typebox = require("typebox");
typebox = require_rolldown_runtime.__toESM(typebox, 1);
...
typebox_value = require_rolldown_runtime.__toESM(typebox_value, 1);
let node_events = require("node:events");
let proper_lockfile = require("proper-lockfile");
proper_lockfile = require_rolldown_runtime.__toESM(proper_lockfile, 1);
let minimatch = require("minimatch");
let ignore = require("ignore");
ignore = require_rolldown_runtime.__toESM(ignore, 1);
let hosted_git_info = require("hosted-git-info");
hosted_git_info = require_rolldown_runtime.__toESM(hosted_git_info, 1);
...
function normalizePositiveLimit(value, fallback) {
	return (0, _gabrielvfonseca_normalization_core_number_coercion.resolveIntegerOption)(value, fallback, { min: 1 });
}
...
function parseRetryAfterSeconds(valueText, nowMs) {
	...
	if (secondsMatch?.[1]) {
		...
		const unitMilliseconds = (0, ms.default)(`1${unit ?? "s"}`);
		return unitMilliseconds === 1 ? value / 1e3 : value * (unitMilliseconds / 1e3);
	}
	const retryAtMs = (0, _gabrielvfonseca_ai_internal_retry_after.parseRetryAfterHttpDateMs)(valueText, nowMs);
...
function classifyRateLimitWindow(message, nowMs = Date.now()) {
	...
	if (SHORT_WINDOW_RATE_LIMIT_RE.test(raw) || require_assistant_error_format.extractLeadingHttpStatus(raw)?.code === 429) return { kind: "short" };
	return { kind: "unknown" };
}
//#endregion
...
function isRetryableAssistantError(message) {
	...
	const status = require_assistant_error_format.extractLeadingHttpStatus(errorMessage)?.code ?? require_assistant_error_format.extractProviderWrappedHttpStatus(errorMessage)?.code;
...
function sleep(ms, signal) {
	return new Promise((resolve, reject) => {
		...
		const timeout = setTimeout(() => {
			...
			resolve();
		}, (0, require_number_coercion.number_coercion_exports.resolveTimerTimeoutMs)(ms, 0, 0));
		signal?.addEventListener("abort", onAbort, { once: true });
	});
}
//#endregion
//#region src/sessions/user-turn-transcript-runtime-context.ts
const RUNTIME_USER_TURN_TRANSCRIPT_CONTEXT = Symbol.for("operator.runtimeUserTurnTranscriptContext");
const RUNTIME_USER_TURN_TRANSCRIPT_RECORDER = Symbol.for("operator.runtimeUserTurnTranscriptRecorder");
...
const parseFrontmatter = (content) => {
	...
	const extracted = require_frontmatter.extractFrontmatterBlock(normalized);
	if (!extracted) return {
		frontmatter: {},
		body: normalized
	};
	return {
		frontmatter: (0, yaml.parse)(extracted.block) ?? {},
...
function estimateMessagesFromContent(messages) {
	return messages.reduce((total, message) => total + require_session_manager.estimateTokens(message), 0);
}
//#endregion
...
function getProviderLoginHelp() {
	return [
		...
		`  ${(0, node_path.join)(require_config.getDocsPath(), "providers.md")}`,
		`  ${(0, node_path.join)(require_config.getDocsPath(), "models.md")}`
	].join("\n");
}
...
function appendCapturedOutput(current, chunk, maxOutputChars, truncateTail) {
	...
	const nextText = truncateTail ? (0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(combined, overflowChars) : (0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(combined, 0, maxOutputChars);
	return {
		text: nextText,
...
async function execCommand(command, args, cwd, options) {
	return new Promise((resolve) => {
		const proc = require_exec.spawnCommand([command, ...args], {
			buffer: false,
			cwd,
		...
		const releaseOutput = require_exec.releaseChildProcessOutputAfterExit(proc);
		let stdout = {
			text: "",
			truncatedChars: 0
		};
		...
		const killProcess = () => {
			if (!killed) {
				killed = true;
				if (proc.pid) require_kill_tree.killProcessTree(proc.pid, {
					detached: process.platform !== "win32",
...
function getAuthStorageOAuthProviderRegistry(authStorage) {
	...
	if (!registry) {
		registry = new require_oauth.OAuthProviderRegistry();
...
function executeWithConfiguredShell(command) {
	try {
		const { shell, args } = require_shell_utils.getBashShellConfig();
		const result = (0, node_child_process.spawnSync)(shell, [...args, command], {
			encoding: "utf-8",
			timeout: 1e4,
			stdio: [
				"ignore",
...
function acquireLockSyncWithRetry(path) {
	for (let attempt = 1;; attempt++) {
		try {
			return proper_lockfile.default.lockSync(path, { realpath: false });
		} catch (error) {
...
var FileAuthStorageBackend = class {
	constructor(authPath = (0, node_path.join)(require_config.getAgentDir(), "auth.json")) {
		this.authPath = authPath;
	}
	ensureParentDir() {
		const dir = (0, node_path.dirname)(this.authPath);
	...
	ensureFileExists() {
		if (!(0, node_fs.existsSync)(this.authPath)) {
			(0, node_fs.writeFileSync)(this.authPath, "{}", "utf-8");
			(0, node_fs.chmodSync)(this.authPath, 384);
		}
	}
	replaceAuthFileAtomic(content) {
		const dirMode = (0, node_fs.statSync)((0, node_path.dirname)(this.authPath)).mode & 4095;
	...
	withLock(fn) {
		...
		try {
			const { result, next } = fn((0, node_fs.existsSync)(this.authPath) ? (0, node_fs.readFileSync)(this.authPath, "utf-8") : void 0);
	...
	async withLockAsync(fn) {
		...
		const release = await proper_lockfile.default.lock(this.authPath, {
			retries: {
				minTimeout: 100,
				maxTimeout: 1e4,
				randomize: true
		...
		try {
			const { result, next } = await fn((0, node_fs.existsSync)(this.authPath) ? (0, node_fs.readFileSync)(this.authPath, "utf-8") : void 0);
...
var AuthStorage = class AuthStorage {
	...
	static create(authPath) {
		return new AuthStorage(new FileAuthStorageBackend(authPath ?? (0, node_path.join)(require_config.getAgentDir(), "auth.json")));
	}
	...
	*/
	hasAuth(provider) {
		...
		if ((0, _gabrielvfonseca_ai_internal_runtime.getEnvApiKey)(provider)) return true;
	...
	*/
	getAuthStatus(provider) {
		...
		const envKeys = (0, _gabrielvfonseca_ai_internal_runtime.findEnvKeys)(provider);
		if (envKeys?.[0]) return {
			configured: false,
			source: "environment",
			label: envKeys[0]
	...
	*/
	async getApiKey(providerId, options) {
		...
		const envKey = (0, _gabrielvfonseca_ai_internal_runtime.getEnvApiKey)(providerId);
...
function createPrivateTempWriteStream(prefix) {
	...
	return {
		path: filePath,
		stream: (0, node_fs.createWriteStream)(filePath, {
			flags: "wx",
			mode: 384
		})
	};
...
function createPrivateTempFilePath(prefix) {
	...
	return (0, node_path.join)((0, node_os.tmpdir)(), `${prefix}-${id}.log`);
}
//#endregion
...
var OutputAccumulator = class {
	constructor(options = {}) {
		...
		this.tempFilePrefix = options.tempFilePrefix ?? "operator-output";
	...
	snapshot(options = {}) {
		const tailTruncation = require_session_manager.truncateTail(this.getSnapshotText(), {
...
async function executeBashWithOperations(command, cwd, operations, options) {
	const output = new OutputAccumulator({
		tempFilePrefix: "operator-bash",
		transformDecodedText: (text) => require_shell_utils.sanitizeBinaryOutput(text, { ansiMode: "compat" }).replace(/\r/g, "")
	});
	const onData = (data) => {
...
function collectEntriesForBranchSummary(session, oldLeafId, targetId) {
	...
	return require_session_manager.collectEntriesForBranchSummaryFromBranches(session.getBranch(oldLeafId), session.getBranch(targetId));
}
...
async function generateBranchSummary(entries, options) {
	const result = await require_session_manager.generateBranchSummary(entries, {
		runtime: require_session_manager.openClawAgentCoreRuntime,
		...options
	});
	if (result.ok) return result.value;
	if (result.error.code === "aborted") return {
...
function prepareCompaction(pathEntries, settings) {
	return unwrapCompactionResult(require_session_manager.prepareCompaction(pathEntries, settings));
}
...
async function generateSummary(currentMessages, model, reserveTokens, apiKey, headers, signal, customInstructions, previousSummary, thinkingLevel, streamFn) {
	return unwrapCompactionResult(await require_session_manager.generateSummary(currentMessages, model, reserveTokens, apiKey, headers, signal, customInstructions, previousSummary, thinkingLevel, streamFn, require_session_manager.openClawAgentCoreRuntime));
}
...
async function compact(preparation, model, apiKey, headers, customInstructions, signal, thinkingLevel, streamFn) {
	return unwrapCompactionResult(await require_session_manager.compact(preparation, model, apiKey, headers, customInstructions, signal, thinkingLevel, streamFn, require_session_manager.openClawAgentCoreRuntime));
}
//#endregion
...
*/
const log = require_subsystem.createSubsystemLogger("agents/model-registry");
const PercentileCutoffsSchema = typebox.Type.Object({
	p50: typebox.Type.Optional(typebox.Type.Number()),
	p75: typebox.Type.Optional(typebox.Type.Number()),
	p90: typebox.Type.Optional(typebox.Type.Number()),
	p99: typebox.Type.Optional(typebox.Type.Number())
});
const OpenRouterRoutingSchema = typebox.Type.Object({
	allow_fallbacks: typebox.Type.Optional(typebox.Type.Boolean()),
	require_parameters: typebox.Type.Optional(typebox.Type.Boolean()),
	data_collection: typebox.Type.Optional(typebox.Type.Union([typebox.Type.Literal("deny"), typebox.Type.Literal("allow")])),
	zdr: typebox.Type.Optional(typebox.Type.Boolean()),
	enforce_distillable_text: typebox.Type.Optional(typebox.Type.Boolean()),
	order: typebox.Type.Optional(typebox.Type.Array(typebox.Type.String())),
	only: typebox.Type.Optional(typebox.Type.Array(typebox.Type.String())),
	ignore: typebox.Type.Optional(typebox.Type.Array(typebox.Type.String())),
	quantizations: typebox.Type.Optional(typebox.Type.Array(typebox.Type.String())),
	sort: typebox.Type.Optional(typebox.Type.Union([typebox.Type.String(), typebox.Type.Object({
		by: typebox.Type.Optional(typebox.Type.String()),
		partition: typebox.Type.Optional(typebox.Type.Union([typebox.Type.String(), typebox.Type.Null()]))
	})])),
	max_price: typebox.Type.Optional(typebox.Type.Object({
		prompt: typebox.Type.Optional(typebox.Type.Union([typebox.Type.Number(), typebox.Type.String()])),
		completion: typebox.Type.Optional(typebox.Type.Union([typebox.Type.Number(), typebox.Type.String()])),
		image: typebox.Type.Optional(typebox.Type.Union([typebox.Type.Number(), typebox.Type.String()])),
		audio: typebox.Type.Optional(typebox.Type.Union([typebox.Type.Number(), typebox.Type.String()])),
		request: typebox.Type.Optional(typebox.Type.Union([typebox.Type.Number(), typebox.Type.String()]))
	})),
	preferred_min_throughput: typebox.Type.Optional(typebox.Type.Union([typebox.Type.Number(), PercentileCutoffsSchema])),
	preferred_max_latency: typebox.Type.Optional(typebox.Type.Union([typebox.Type.Number(), PercentileCutoffsSchema]))
});
const VercelGatewayRoutingSchema = typebox.Type.Object({
	only: typebox.Type.Optional(typebox.Type.Array(typebox.Type.String())),
	order: typebox.Type.Optional(typebox.Type.Array(typebox.Type.String()))
});
const ThinkingLevelMapValueSchema = typebox.Type.Union([typebox.Type.String(), typebox.Type.Null()]);
const ThinkingLevelMapSchema = typebox.Type.Object({
	off: typebox.Type.Optional(ThinkingLevelMapValueSchema),
	minimal: typebox.Type.Optional(ThinkingLevelMapValueSchema),
	low: typebox.Type.Optional(ThinkingLevelMapValueSchema),
	medium: typebox.Type.Optional(ThinkingLevelMapValueSchema),
	high: typebox.Type.Optional(ThinkingLevelMapValueSchema),
	xhigh: typebox.Type.Optional(ThinkingLevelMapValueSchema),
	max: typebox.Type.Optional(ThinkingLevelMapValueSchema)
});
const OpenAICompletionsCompatSchema = typebox.Type.Object({
	supportsStore: typebox.Type.Optional(typebox.Type.Boolean()),
	supportsDeveloperRole: typebox.Type.Optional(typebox.Type.Boolean()),
	supportsReasoningEffort: typebox.Type.Optional(typebox.Type.Boolean()),
	supportsUsageInStreaming: typebox.Type.Optional(typebox.Type.Boolean()),
	maxTokensField: typebox.Type.Optional(typebox.Type.Union([typebox.Type.Literal("max_completion_tokens"), typebox.Type.Literal("max_tokens")])),
	requiresToolResultName: typebox.Type.Optional(typebox.Type.Boolean()),
	requiresAssistantAfterToolResult: typebox.Type.Optional(typebox.Type.Boolean()),
	requiresThinkingAsText: typebox.Type.Optional(typebox.Type.Boolean()),
	requiresReasoningContentOnAssistantMessages: typebox.Type.Optional(typebox.Type.Boolean()),
	thinkingFormat: typebox.Type.Optional(typebox.Type.Union([
		typebox.Type.Literal("openai"),
		typebox.Type.Literal("openrouter"),
		typebox.Type.Literal("together"),
		typebox.Type.Literal("deepseek"),
		typebox.Type.Literal("zai"),
		typebox.Type.Literal("qwen"),
		typebox.Type.Literal("qwen-chat-template")
	])),
	cacheControlFormat: typebox.Type.Optional(typebox.Type.Literal("anthropic")),
	openRouterRouting: typebox.Type.Optional(OpenRouterRoutingSchema),
	vercelGatewayRouting: typebox.Type.Optional(VercelGatewayRoutingSchema),
	supportsStrictMode: typebox.Type.Optional(typebox.Type.Boolean()),
	supportsLongCacheRetention: typebox.Type.Optional(typebox.Type.Boolean())
});
const OpenAIResponsesCompatSchema = typebox.Type.Object({
	supportsTemperature: typebox.Type.Optional(typebox.Type.Boolean()),
	sendSessionIdHeader: typebox.Type.Optional(typebox.Type.Boolean()),
	supportsLongCacheRetention: typebox.Type.Optional(typebox.Type.Boolean())
});
const AnthropicMessagesCompatSchema = typebox.Type.Object({
	supportsEagerToolInputStreaming: typebox.Type.Optional(typebox.Type.Boolean()),
	supportsLongCacheRetention: typebox.Type.Optional(typebox.Type.Boolean())
});
const ProviderCompatSchema = typebox.Type.Union([
	OpenAICompletionsCompatSchema,
	OpenAIResponsesCompatSchema,
	AnthropicMessagesCompatSchema
]);
const ProviderAuthModeSchema = typebox.Type.Union([
	typebox.Type.Literal("api-key"),
	typebox.Type.Literal("aws-sdk"),
	typebox.Type.Literal("oauth"),
	typebox.Type.Literal("token")
]);
const ModelDefinitionSchema = typebox.Type.Object({
	id: typebox.Type.String({ minLength: 1 }),
	name: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	api: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	baseUrl: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	reasoning: typebox.Type.Optional(typebox.Type.Boolean()),
	thinkingLevelMap: typebox.Type.Optional(ThinkingLevelMapSchema),
	...
	cost: typebox.Type.Optional(typebox.Type.Object({
		input: typebox.Type.Number(),
		output: typebox.Type.Number(),
		cacheRead: typebox.Type.Number(),
		cacheWrite: typebox.Type.Number()
	})),
	contextWindow: typebox.Type.Optional(typebox.Type.Number()),
	maxTokens: typebox.Type.Optional(typebox.Type.Number()),
	params: typebox.Type.Optional(typebox.Type.Record(typebox.Type.String(), typebox.Type.Unknown())),
	headers: typebox.Type.Optional(typebox.Type.Record(typebox.Type.String(), typebox.Type.String())),
	compat: typebox.Type.Optional(ProviderCompatSchema)
});
const ProviderConfigSchema = typebox.Type.Object({
	name: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	baseUrl: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	apiKey: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	auth: typebox.Type.Optional(ProviderAuthModeSchema),
	api: typebox.Type.Optional(typebox.Type.String({ minLength: 1 })),
	headers: typebox.Type.Optional(typebox.Type.Record(typebox.Type.String(), typebox.Type.String())),
	compat: typebox.Type.Optional(ProviderCompatSchema),
	authHeader: typebox.Type.Optional(typebox.Type.Boolean()),
	models: typebox.Type.Optional(typebox.Type.Array(ModelDefinitionSchema))
});
const validateModelsConfig = (0, typebox_compile.Compile)(typebox.Type.Object({
	generatedBy: typebox.Type.Optional(typebox.Type.String()),
	providers: typebox.Type.Record(typebox.Type.String(), ProviderConfigSchema)
}));
...
var ModelRegistry = class ModelRegistry {
	constructor(authStorage, modelsJsonPath, options = {}) {
		...
		this.pluginMetadataSnapshot = require_model_discovery_context.resolveModelPluginMetadataSnapshot({
	...
	static create(authStorage, modelsJsonPath = (0, node_path.join)(require_config.getAgentDir(), "models.json"), options = {}) {
	...
	*/
	refresh() {
		...
		this.loadError = void 0;
		(0, _gabrielvfonseca_ai_providers.resetApiProviders)(_gabrielvfonseca_ai_internal_runtime.defaultApiRegistry);
	...
	loadCustomModels(modelsJsonPath, options = { includePluginCatalogs: true }) {
		if (!(0, node_fs.existsSync)(modelsJsonPath)) return emptyCustomModelsResult();
		try {
			const content = (0, node_fs.readFileSync)(modelsJsonPath, "utf-8");
			const parsed = JSON.parse(stripJsonComments(content));
			if (options.requireGeneratedCatalog === true && !require_plugin_model_catalog.isGeneratedPluginModelCatalog(parsed)) return emptyCustomModelsResult();
			...
			const config = parsed;
			const providers = options.requireGeneratedCatalog === true ? require_plugin_model_catalog.filterGeneratedPluginModelCatalogProviders({
			...
			if (options.includePluginCatalogs !== false) for (const pluginCatalog of require_plugin_model_catalog.listPluginModelCatalogFiles((0, node_path.dirname)(modelsJsonPath))) {
...
function selectAvailableFallbackModel(availableModels) {
	return availableModels.find((model) => model.provider === "openrouter" && model.id === "openrouter/auto") ?? availableModels[0];
}
/**
	...
	for (const pattern of patterns) {
		if (pattern.includes("*") || pattern.includes("?") || pattern.includes("[")) {
			...
			const matchingModels = availableModels.filter((m) => {
				return (0, minimatch.minimatch)(`${m.provider}/${m.id}`, globPattern, { nocase: true }) || (0, minimatch.minimatch)(m.id, globPattern, { nocase: true });
			});
			if (matchingModels.length === 0) {
				console.warn(chalk.default.yellow(`Warning: No models match pattern "${pattern}"`));
				continue;
			}
			for (const model of matchingModels) if (!scopedModels.some((sm) => (0, _gabrielvfonseca_ai_internal_runtime.modelsAreEqual)(sm.model, model))) scopedModels.push({
				model,
				thinkingLevel
			});
			continue;
		...
		if (warning) console.warn(chalk.default.yellow(`Warning: ${warning}`));
		if (!model) {
			console.warn(chalk.default.yellow(`Warning: No models match pattern "${pattern}"`));
			continue;
		}
		if (!scopedModels.some((sm) => (0, _gabrielvfonseca_ai_internal_runtime.modelsAreEqual)(sm.model, model))) scopedModels.push({
			model,
			thinkingLevel
		});
	}
...
async function findInitialModel(options) {
	...
	if (cliProvider && cliModel) {
		...
		if (resolved.error) {
			console.error(chalk.default.red(resolved.error));
			process.exit(1);
		}
...
async function restoreModelFromSession(savedProvider, savedModelId, currentModel, shouldPrintMessages, modelRegistry) {
	...
	if (restoredModel && hasConfiguredAuth) {
		if (shouldPrintMessages) console.log(chalk.default.dim(`Restored model: ${savedProvider}/${savedModelId}`));
		return {
			model: restoredModel,
			fallbackMessage: void 0
		};
	...
	if (shouldPrintMessages) console.error(chalk.default.yellow(`Warning: Could not restore model ${savedProvider}/${savedModelId} (${reason}).`));
	if (currentModel) {
		if (shouldPrintMessages) console.log(chalk.default.dim(`Falling back to: ${currentModel.provider}/${currentModel.id}`));
		return {
			model: currentModel,
	...
	if (availableModels.length > 0) {
		...
		if (shouldPrintMessages) console.log(chalk.default.dim(`Falling back to: ${fallbackModel.provider}/${fallbackModel.id}`));
		return {
			model: fallbackModel,
...
const toPosixPath = (pathValue) => pathValue.split(node_path.sep).join("/");
/** Adds nested ignore-file rules to a matcher using paths relative to the scan root. */
function addIgnoreRules(dir, rootDir, ig = (0, ignore.default)()) {
	const relativeDir = (0, node_path.relative)(rootDir, dir);
	const prefix = relativeDir ? `${toPosixPath(relativeDir)}/` : "";
	for (const filename of IGNORE_FILE_NAMES) {
		const ignorePath = (0, node_path.join)(dir, filename);
		if (!(0, node_fs.existsSync)(ignorePath)) continue;
		try {
			const content = (0, node_fs.readFileSync)(ignorePath, "utf-8");
...
function parseHostedGitUrl(url) {
	...
	for (const candidate of candidates) {
		const info = hosted_git_info.default.fromUrl(candidate);
		if (!info) continue;
...
function getHomeDir() {
	return process.env.HOME || (0, node_os.homedir)();
}
	...
	try {
		...
		for (const entry of entries) {
			...
			if (entry.isSymbolicLink()) try {
				const stats = (0, node_fs.statSync)(fullPath);
				isDir = stats.isDirectory();
				isFile = stats.isFile();
			} catch {
				continue;
			}
			const relPath = toPosixPath((0, node_path.relative)(root, fullPath));
...
function collectSkillEntries(dir, mode, ignoreMatcher, rootDir) {
	const entries = [];
	if (!(0, node_fs.existsSync)(dir)) return entries;
	const root = rootDir ?? dir;
	const ig = addIgnoreRules(dir, root, ignoreMatcher);
	try {
		const dirEntries = (0, node_fs.readdirSync)(dir, { withFileTypes: true });
		for (const entry of dirEntries) {
			...
			let isFile = entry.isFile();
			if (entry.isSymbolicLink()) try {
				isFile = (0, node_fs.statSync)(fullPath).isFile();
			} catch {
				continue;
			}
			const relPath = toPosixPath((0, node_path.relative)(root, fullPath));
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath((0, node_path.relative)(root, fullPath));
			if (mode === "@gabrielvfonseca/operator" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
			if (!isDir) continue;
...
function collectAncestorAgentsSkillDirs(startDir) {
	const skillDirs = [];
	const resolvedStartDir = (0, node_path.resolve)(startDir);
	const gitRepoRoot = findGitRepoRoot(resolvedStartDir);
	let dir = resolvedStartDir;
	while (true) {
		skillDirs.push((0, node_path.join)(dir, ".agents", "skills"));
...
function collectTopLevelAutoResourceEntries(dir, resourceType) {
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath((0, node_path.relative)(dir, fullPath));
			if (ig.ignores(relPath)) continue;
...
function readResourceManifestFile(packageJsonPath) {
	try {
		...
		return JSON.parse(content).operator ?? null;
	} catch {
		return null;
	}
}
	...
	if ((0, node_fs.existsSync)(packageJsonPath)) {
		...
		if (manifest?.extensions?.length) {
			...
			for (const extPath of manifest.extensions) {
				const resolvedExtPath = (0, node_path.resolve)(dir, extPath);
				if ((0, node_fs.existsSync)(resolvedExtPath) && isRealPathWithinRoot(rootDir, resolvedExtPath)) entries.push(resolvedExtPath);
			}
			if (entries.length > 0) return entries;
		}
	}
	const indexTs = (0, node_path.join)(dir, "index.ts");
	const indexJs = (0, node_path.join)(dir, "index.js");
	if ((0, node_fs.existsSync)(indexTs) && isRealPathWithinRoot(rootDir, indexTs)) return [indexTs];
	if ((0, node_fs.existsSync)(indexJs) && isRealPathWithinRoot(rootDir, indexJs)) return [indexJs];
	return null;
}
...
function collectResourceFiles(dir, resourceType) {
	if (resourceType === "skills") return collectSkillEntries(dir, "@gabrielvfonseca/operator");
	if (resourceType === "extensions") return collectAutoExtensionEntries(dir);
	return collectFiles(dir, FILE_PATTERNS[resourceType]);
}
function resolveRealPathIfPossible(path) {
	try {
		return node_fs.realpathSync.native(path);
	} catch {
...
function isPathWithinRoot(root, candidate) {
	...
	return rel === "" || rel !== "" && !rel.startsWith("..") && !(0, node_path.isAbsolute)(rel);
}
function isRealPathWithinRoot(root, candidate) {
	return isPathWithinRoot(resolveRealPathIfPossible((0, node_path.resolve)(root)), resolveRealPathIfPossible(candidate));
}
function getMatchCandidates(filePath, baseDir, includeNames) {
	const name = (0, node_path.basename)(filePath);
	const candidates = [toPosixPath((0, node_path.relative)(baseDir, filePath)), toPosixPath(filePath)];
	if (includeNames) candidates.push(name);
	if (name === "SKILL.md") {
		const parentDir = (0, node_path.dirname)(filePath);
		candidates.push(toPosixPath((0, node_path.relative)(baseDir, parentDir)), toPosixPath(parentDir));
		if (includeNames) candidates.push((0, node_path.basename)(parentDir));
	}
	return candidates;
}
function matchesAnyPattern(filePath, patterns, baseDir) {
	const candidates = getMatchCandidates(filePath, baseDir, true);
	return patterns.some((pattern) => minimatch.minimatch.match(candidates, toPosixPath(pattern)).length > 0);
}
...
var DefaultPackageManager = class {
	...
	async resolve(onMissing) {
		...
		const projectBaseDir = (0, node_path.join)(this.cwd, require_config.CONFIG_DIR_NAME);
	...
	async resolvePackageSources(sources, accumulator, onMissing) {
		for (const { pkg, scope } of sources) {
			...
			if (parsed.type === "npm") {
				...
				if (!(0, node_fs.existsSync)(installedPath) || parsed.pinned && !this.installedNpmMatchesPinnedVersion(parsed, installedPath)) {
					await handleMissing();
					continue;
				}
			...
			if (parsed.type === "git") {
				...
				if (!(0, node_fs.existsSync)(installedPath)) {
					await handleMissing();
					continue;
				}
				metadata.baseDir = installedPath;
	...
	resolveLocalExtensionSource(source, accumulator, filter, metadata, baseDir) {
		...
		if (!(0, node_fs.existsSync)(resolved)) return;
		try {
			const stats = (0, node_fs.statSync)(resolved);
			if (stats.isFile()) {
				metadata.baseDir = (0, node_path.dirname)(resolved);
	...
	parseSource(source) {
		...
		if (require_curator.isLocalPath(source)) return {
			type: "local",
			path: source
		};
	...
	getInstalledNpmVersion(installedPath) {
		const packageJsonPath = (0, node_path.join)(installedPath, "package.json");
	...
	*/
	...
	getNpmInstallPath(source, scope) {
		if (scope === "temporary") return (0, node_path.join)(this.getTemporaryDir("npm"), "node_modules", source.name);
		if (scope === "project") return (0, node_path.join)(this.cwd, require_config.CONFIG_DIR_NAME, "npm", "node_modules", source.name);
		return (0, node_path.join)(this.agentDir, "npm", "node_modules", source.name);
	}
	getGitInstallPath(source, scope) {
		if (scope === "temporary") return this.getTemporaryDir(`git-${source.host}`, source.path);
		if (scope === "project") return (0, node_path.join)(this.cwd, require_config.CONFIG_DIR_NAME, "git", source.host, source.path);
		return (0, node_path.join)(this.agentDir, "git", source.host, source.path);
	}
	getTemporaryDir(prefix, suffix) {
		const hash = (0, node_crypto.createHash)("sha256").update(`${prefix}-${suffix ?? ""}`).digest("hex").slice(0, 8);
		return (0, node_path.join)((0, node_os.tmpdir)(), "operator-resources", prefix, hash, suffix ?? "");
	}
	getBaseDirForScope(scope) {
		if (scope === "project") return (0, node_path.join)(this.cwd, require_config.CONFIG_DIR_NAME);
	...
	resolvePath(input) {
		...
		if (trimmed.startsWith("~/")) return (0, node_path.join)(getHomeDir(), trimmed.slice(2));
		if (trimmed.startsWith("~")) return (0, node_path.join)(getHomeDir(), trimmed.slice(1));
		return (0, node_path.resolve)(this.cwd, trimmed);
	}
	resolvePathFromBase(input, baseDir) {
		...
		if (trimmed.startsWith("~/")) return (0, node_path.join)(getHomeDir(), trimmed.slice(2));
		if (trimmed.startsWith("~")) return (0, node_path.join)(getHomeDir(), trimmed.slice(1));
		return (0, node_path.resolve)(baseDir, trimmed);
	}
	collectPackageResources(packageRoot, accumulator, filter, metadata) {
		...
		let hasAnyDir = false;
		for (const resourceType of RESOURCE_TYPES) if ((0, node_fs.existsSync)((0, node_path.join)(packageRoot, resourceType))) {
	...
	collectDefaultResources(packageRoot, resourceType, target, metadata) {
		...
		if ((0, node_fs.existsSync)((0, node_path.join)(packageRoot, resourceType))) {
			const files = this.collectConventionResourceFiles(packageRoot, resourceType);
	...
	*/
	...
	collectConventionResourceFiles(packageRoot, resourceType) {
		const conventionDir = (0, node_path.join)(packageRoot, resourceType);
		if (!(0, node_fs.existsSync)(conventionDir)) return [];
	...
	readResourceManifest(packageRoot) {
		const packageJsonPath = (0, node_path.join)(packageRoot, "package.json");
	...
	collectFilesFromManifestEntries(entries, root, resourceType) {
		const resolved = entries.filter((entry) => !isOverridePattern(entry)).flatMap((entry) => {
			if (!hasGlobPattern(entry)) return [(0, node_path.resolve)(root, entry)];
			return (0, node_fs.globSync)(entry, { cwd: root }).map((match) => (0, node_path.resolve)(root, match));
		});
	...
	addAutoDiscoveredResources(accumulator, globalSettings, projectSettings, globalBaseDir, projectBaseDir) {
		...
		const userDirs = {
			extensions: (0, node_path.join)(globalBaseDir, "extensions"),
			skills: (0, node_path.join)(globalBaseDir, "skills"),
			prompts: (0, node_path.join)(globalBaseDir, "prompts"),
			themes: (0, node_path.join)(globalBaseDir, "themes")
		};
		const projectDirs = {
			extensions: (0, node_path.join)(projectBaseDir, "extensions"),
			skills: (0, node_path.join)(projectBaseDir, "skills"),
			prompts: (0, node_path.join)(projectBaseDir, "prompts"),
			themes: (0, node_path.join)(projectBaseDir, "themes")
		};
		const userAgentsSkillsDir = (0, node_path.join)(getHomeDir(), ".agents", "skills");
		const projectAgentsSkillDirs = collectAncestorAgentsSkillDirs(this.cwd).filter((dir) => (0, node_path.resolve)(dir) !== (0, node_path.resolve)(userAgentsSkillsDir));
		...
		addResources("skills", collectAutoSkillEntries(projectDirs.skills, "@gabrielvfonseca/operator"), projectMetadata, projectOverrides.skills, projectBaseDir);
		for (const agentsSkillsDir of projectAgentsSkillDirs) {
			const agentsBaseDir = (0, node_path.dirname)(agentsSkillsDir);
		...
		addResources("skills", collectAutoSkillEntries(userDirs.skills, "@gabrielvfonseca/operator"), userMetadata, userOverrides.skills, globalBaseDir);
		const userAgentsBaseDir = (0, node_path.dirname)(userAgentsSkillsDir);
	...
	collectFilesFromPaths(paths, resourceType) {
		...
		for (const p of paths) {
			if (!(0, node_fs.existsSync)(p)) continue;
			try {
				const stats = (0, node_fs.statSync)(p);
	...
	toResolvedPaths(accumulator) {
		const mapToResolved = (entries) => {
			...
			return resolved.filter((entry) => {
				const canonicalPath = require_curator.canonicalizePath(entry.path);
...
function deepMergeSettings(base, overrides) {
	return require_deep_merge.mergeDeep(base, overrides);
}
var FileSettingsStorage = class {
	constructor(cwd, agentDir) {
		this.globalSettingsPath = (0, node_path.join)(agentDir, "settings.json");
		this.projectSettingsPath = (0, node_path.join)(cwd, require_config.CONFIG_DIR_NAME, "settings.json");
	}
	withLock(scope, fn) {
...
var SettingsManager = class SettingsManager {
	...
	static create(cwd, agentDir = require_config.getAgentDir()) {
	...
	getSessionDir() {
		...
		if (sessionDir === "~") return (0, node_os.homedir)();
		if (sessionDir.startsWith("~/")) return (0, node_path.join)((0, node_os.homedir)(), sessionDir.slice(2));
		return sessionDir;
	}
	getDefaultProvider() {
	...
	getImageWidthCells() {
		return (0, _gabrielvfonseca_normalization_core_number_coercion.resolveIntegerOption)(this.settings.terminal?.imageWidthCells, 60, { min: 1 });
	}
...
function decodeHtmlEntities(html) {
	...
	return html.replace(HTML_ENTITY_RE, (entity, hex, decimal, name) => {
		if (hex === void 0 && decimal === void 0) {
			const decodedEntity = (0, entities.decodeHTMLStrict)(entity);
			return decodedEntity === entity && LEGACY_CASE_INSENSITIVE_ENTITY_NAME_RE.test(name ?? "") ? (0, entities.decodeHTMLStrict)(entity.toLowerCase()) : decodedEntity;
		}
...
const highlightJsModule = (0, node_module.createRequire)(require("url").pathToFileURL(__filename).href)("highlight.js");
...
*/
const ColorValueSchema = typebox.Type.Union([typebox.Type.String(), typebox.Type.Integer({
	minimum: 0,
	maximum: 255
})]);
const validateThemeJson = (0, typebox_compile.Compile)(typebox.Type.Object({
	$schema: typebox.Type.Optional(typebox.Type.String()),
	name: typebox.Type.String(),
	vars: typebox.Type.Optional(typebox.Type.Record(typebox.Type.String(), ColorValueSchema)),
	colors: typebox.Type.Object({
	...
	export: typebox.Type.Optional(typebox.Type.Object({
		pageBg: typebox.Type.Optional(ColorValueSchema),
		cardBg: typebox.Type.Optional(ColorValueSchema),
		infoBg: typebox.Type.Optional(ColorValueSchema)
	}))
}));
...
var Theme = class {
	...
	bold(text) {
		return chalk.default.bold(text);
	}
	italic(text) {
		return chalk.default.italic(text);
	}
	underline(text) {
		return chalk.default.underline(text);
	}
	inverse(text) {
		return chalk.default.inverse(text);
	}
	strikethrough(text) {
		return chalk.default.strikethrough(text);
	}
	getFgAnsi(color) {
		const ansi = this.fgColors.get(color);
		if (!ansi) throw new Error(`Unknown theme color: ${color}`);
...
function createTheme(themeJson, mode, sourcePath) {
	const colorMode = mode ?? ((0, _earendil_works_pi_tui.getCapabilities)().trueColor ? "truecolor" : "256color");
...
function loadThemeFromPath(themePath, mode) {
	return createTheme(parseThemeJsonContent(themePath, node_fs.readFileSync(themePath, "utf-8")), mode, themePath);
}
const THEME_KEY = Symbol.for("operator:agent-theme");
...
function keyText(keybinding) {
	return formatKeys((0, _earendil_works_pi_tui.getKeybindings)().getKeys(keybinding));
}
...
function truncateToVisualLines(text, maxVisualLines, width, paddingX = 0) {
	...
	const allVisualLines = new _earendil_works_pi_tui.Text(text, paddingX, 0).render(width);
...
function getTextOutput(result, showImages) {
	...
	let output = textBlocks.map((c) => require_shell_utils.sanitizeBinaryOutput(c.text || "", { ansiMode: "compat" }).replace(/\r/g, "")).join("\n");
	const caps = (0, _earendil_works_pi_tui.getCapabilities)();
	if (imageBlocks.length > 0 && (!caps.images || !showImages)) {
		const imageIndicators = imageBlocks.map((img) => {
			return (0, _earendil_works_pi_tui.imageFallback)(img.mimeType ?? "image/unknown", img.data && img.mimeType ? (0, _earendil_works_pi_tui.getImageDimensions)(img.data, img.mimeType) ?? void 0 : void 0);
		}).join("\n");
...
function appendSessionToolTruncationWarning(text, theme, options) {
	...
	if (options.truncation?.truncated) warnings.push(`${require_session_manager.formatSize(options.truncation.maxBytes ?? 51200)} limit`);
...
*/
const bashSchema = typebox.Type.Object({
	command: typebox.Type.String({ description: "Bash command." }),
	timeout: typebox.Type.Optional(typebox.Type.Number({ description: "Optional timeout seconds; default none." }))
});
function resolveBashTimeoutMs(timeoutSeconds) {
	...
	return (0, _gabrielvfonseca_normalization_core_number_coercion.resolveTimerTimeoutMs)(timeoutSeconds * 1e3, 1);
}
if (process.env.VITEST || false) globalThis[Symbol.for("operator.bashToolTestApi")] = { resolveBashTimeoutMs };
/**
* Create bash operations using Operator runtime's built-in local shell execution backend.
*
* This is useful for extensions that intercept user_bash and still want Operator runtime's
* standard local shell behavior while wrapping or rewriting commands.
*/
function createLocalBashOperations(options) {
	return { exec: (command, cwd, { onData, signal, timeout, env }) => {
		return new Promise((resolve, reject) => {
			const { shell, args } = require_shell_utils.getBashShellConfig(options?.shellPath);
			if (!(0, node_fs.existsSync)(cwd)) {
				reject(/* @__PURE__ */ new Error(`Working directory does not exist: ${cwd}\nCannot execute bash commands.`));
				return;
			}
			const child = require_exec.spawnCommand([
				shell,
				...args,
				command
			], {
				...
				cwd,
				detached: process.platform !== "win32",
				env: env ?? require_shell_utils.getShellEnv(),
				reject: false,
				stdio: [
					"ignore",
					"pipe",
			...
			const releaseOutput = require_exec.releaseChildProcessOutputAfterExit(child);
			let timedOut = false;
			let timeoutHandle;
			const timeoutMs = resolveBashTimeoutMs(timeout);
			if (timeoutMs !== void 0) timeoutHandle = setTimeout(() => {
				timedOut = true;
				if (child.pid) require_shell_utils.killProcessTree(child.pid);
			}, timeoutMs);
...
function resolveSpawnContext(command, cwd, spawnHook) {
	const baseContext = {
		command,
		cwd,
		env: { ...require_shell_utils.getShellEnv() }
	};
	return spawnHook ? spawnHook(baseContext) : baseContext;
}
const BASH_PREVIEW_LINES = 5;
const BASH_UPDATE_THROTTLE_MS = 100;
var BashResultRenderComponent = class extends _earendil_works_pi_tui.Container {
	constructor(..._args) {
		super(..._args);
		this.state = {
			cachedWidth: void 0,
...
function rebuildBashResultRenderComponent(component, result, options, showImages, startedAt, endedAt) {
	...
	if (output) {
		...
		if (options.expanded) component.addChild(new _earendil_works_pi_tui.Text(`\n${styledOutput}`, 0, 0));
			...
			render: (width) => {
				...
				if (state.cachedSkipped && state.cachedSkipped > 0) return [
					"",
					(0, _earendil_works_pi_tui.truncateToWidth)(theme.fg("muted", `... (${state.cachedSkipped} earlier lines,`) + ` ${keyHint("app.tools.expand", "to expand")})`, width, "..."),
					...state.cachedLines ?? []
				];
	...
	if (truncation?.truncated || fullOutputPath) {
		...
		else warnings.push(`Truncated: ${truncation.outputLines} lines shown (${require_session_manager.formatSize(truncation.maxBytes ?? 51200)} limit)`);
		component.addChild(new _earendil_works_pi_tui.Text(`\n${theme.fg("warning", `[${warnings.join(". ")}]`)}`, 0, 0));
	}
	if (startedAt !== void 0) {
		const label = options.isPartial ? "Elapsed" : "Took";
		const endTime = endedAt ?? Date.now();
		component.addChild(new _earendil_works_pi_tui.Text(`\n${theme.fg("muted", `${label} ${require_format_duration.formatDurationSeconds(endTime - startedAt)}`)}`, 0, 0));
	}
}
function createBashToolDefinition(cwd, options) {
	...
	return {
		name: "bash",
		label: "bash",
		description: `Run bash in cwd; stdout+stderr. Returns last ${require_session_manager.DEFAULT_MAX_LINES} lines or ${require_session_manager.DEFAULT_MAX_BYTES / 1024}KB; full truncated output saved temp. Optional timeout seconds.`,
			...
			const formatOutput = (snapshot, emptyText = "(no output)") => {
				...
				if (truncation.truncated) {
					...
					if (truncation.lastLinePartial) {
						const lastLineSize = require_session_manager.formatSize(output.getLastLineBytes());
						text += `\n\n[Showing last ${require_session_manager.formatSize(truncation.outputBytes)} of line ${endLine} (line is ${lastLineSize}). ${formatFullOutputFooter(fullOutputPath)}]`;
					...
					else text += `\n\n[Showing lines ${startLine}-${endLine} of ${truncation.totalLines} (${require_session_manager.formatSize(require_session_manager.DEFAULT_MAX_BYTES)} limit). ${formatFullOutputFooter(fullOutputPath)}]`;
				}
				return {
					text,
					details
		...
		renderCall(args, themeValue, context) {
			...
			const text = context.lastComponent ?? new _earendil_works_pi_tui.Text("", 0, 0);
...
function renderIntraLineDiff(oldContent, newContent) {
	...
	for (const part of (0, diff.diffWords)(oldContent, newContent)) {
...
function levenshteinDistance(left, right) {
	...
	for (let leftIndex = 0; leftIndex < left.length; leftIndex += 1) {
		...
		for (let rightIndex = 0; rightIndex < right.length; rightIndex += 1) {
			...
			current[rightIndex + 1] = Math.min((0, _gabrielvfonseca_normalization_core.expectDefined)(current[rightIndex], "current entry at right index") + 1, (0, _gabrielvfonseca_normalization_core.expectDefined)(previous[rightIndex + 1], "previous entry at right index + 1") + 1, (0, _gabrielvfonseca_normalization_core.expectDefined)(previous[rightIndex], "previous entry at right index") + cost);
		}
		const nextPrevious = current;
		current = previous;
		previous = nextPrevious;
	}
	return (0, _gabrielvfonseca_normalization_core.expectDefined)(previous[right.length], "previous entry at right.length");
}
//#endregion
...
function generateUnifiedPatch(path, oldContent, newContent, contextLines = 4) {
	return (0, diff.createPatch)(path, oldContent, newContent, void 0, void 0, {
...
function generateDiffString(oldContent, newContent, contextLines = 4) {
	const hunks = (0, diff.structuredPatch)("", "", oldContent, newContent, void 0, void 0, { context: contextLines }).hunks;
...
async function computeEditsDiff(path, edits, cwd, operations) {
	const absolutePath = require_path_utils.resolveToCwd(path, cwd);
	try {
		try {
			if (operations) await operations.access(absolutePath);
			else await (0, node_fs_promises.access)(absolutePath, node_fs.constants.R_OK);
		} catch (error) {
			return { error: `Could not edit file: ${path}. ${error instanceof Error && "code" in error ? `Error code: ${String(error.code)}` : String(error)}.` };
		}
		const rawContentResult = operations ? await operations.readFile(absolutePath) : await (0, node_fs_promises.readFile)(absolutePath, "utf-8");
...
*/
const fileMutationQueue = new require_keyed_async_queue.KeyedAsyncQueue();
function getMutationQueueKey(filePath) {
	const resolvedPath = (0, node_path.resolve)(filePath);
	try {
		return node_fs.realpathSync.native(resolvedPath);
	} catch {
		return resolvedPath;
	}
}
...
*/
const replaceEditSchema = typebox.Type.Object({
	oldText: typebox.Type.String({ description: "Exact original text; unique and non-overlapping in this call." }),
	newText: typebox.Type.String({ description: "Replacement text." })
}, { additionalProperties: false });
const editSchema = typebox.Type.Object({
	path: typebox.Type.String({ description: "File path; relative/absolute." }),
	edits: typebox.Type.Array(replaceEditSchema, { description: "Targeted replacements against original file; no overlap/nesting. Merge nearby changes." })
}, { additionalProperties: false });
...
const defaultEditOperations = {
	readFile: (path) => (0, node_fs_promises.readFile)(path),
	writeFile: (path, content) => (0, node_fs_promises.writeFile)(path, content, "utf-8"),
	access: (path) => (0, node_fs_promises.access)(path, node_fs.constants.R_OK | node_fs.constants.W_OK)
};
...
function appendMismatchHint(error, currentContent) {
	const snippet = currentContent.length <= EDIT_MISMATCH_HINT_LIMIT ? currentContent : `${(0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(currentContent, EDIT_MISMATCH_HINT_LIMIT)}\n... (truncated)`;
	const enhanced = new Error(`${error.message}\nCurrent file contents:\n${snippet}`, { cause: error });
	enhanced.stack = error.stack;
	return enhanced;
}
function createEditCallRenderComponent() {
	return Object.assign(new _earendil_works_pi_tui.Box(1, 1, (text) => text), {
		preview: void 0,
...
function getEditCallRenderComponent(state, lastComponent) {
	if (lastComponent instanceof _earendil_works_pi_tui.Box) {
...
function buildEditCallComponent(component, args, theme) {
	...
	component.clear();
	component.addChild(new _earendil_works_pi_tui.Text(formatEditCall(args, theme), 0, 0));
	...
	component.addChild(new _earendil_works_pi_tui.Spacer(1));
	component.addChild(new _earendil_works_pi_tui.Text(body, 0, 0));
	return component;
}
...
function createEditToolDefinition(cwd, options) {
	...
	return {
		...
		async execute(toolCallId, input, signal, onUpdate, ctx) {
			...
			return withFileMutationQueue(absolutePath, async () => {
				...
				try {
					...
					validateNoOpEditTargets(normalizedContent, noOpEdits, realEdits, path);
					if (realEdits.length === 0) return {
						...require_common.textResult(`No changes made to ${path}. The replacement text is identical to the original.`, void 0),
						terminate: true
					};
					const { baseContent, newContent } = applyEditsToNormalizedContent(normalizedContent, realEdits, path);
					...
					if (normalizedError instanceof EditNoChangeError) return {
						...require_common.textResult(`No changes made to ${path}. The replacement produced identical content.`, void 0),
						terminate: true
					};
		...
		renderResult(result, optionsLocal, theme, context) {
			...
			const component = context.lastComponent ?? new _earendil_works_pi_tui.Container();
			component.clear();
			if (!output) return component;
			component.addChild(new _earendil_works_pi_tui.Spacer(1));
			component.addChild(new _earendil_works_pi_tui.Text(output, 1, 0));
			return component;
		}
	};
}
...
*/
const TOOLS_DIR = require_config.getBinDir();
...
function isOfflineModeEnabled() {
	const value = process.env.OPERATOR_OFFLINE;
...
function commandExists(cmd) {
	try {
		const result = (0, node_child_process.spawnSync)(cmd, ["--version"], {
			stdio: "pipe",
			timeout: 5e3
		});
...
function getToolPath(tool) {
	...
	const localPath = (0, node_path.join)(TOOLS_DIR, config.binaryName + ((0, node_os.platform)() === "win32" ? ".exe" : ""));
	if ((0, node_fs.existsSync)(localPath)) return localPath;
...
async function getLatestVersion(repo) {
	const guarded = await require_fetch_guard.fetchWithSsrFGuard({
		...
		init: { headers: { "User-Agent": `${require_config.APP_NAME}-coding-agent` } }
	});
...
async function downloadFile(url, dest, maxBytes) {
	...
	try {
		...
		const fileStream = (0, node_fs.createWriteStream)(dest);
		let downloadCompleted = false;
		try {
			let downloadedBytes = 0;
			const byteCap = new node_stream.Transform({ transform(chunk, _encoding, callback) {
			...
			await (0, node_stream_promises.pipeline)(node_stream.Readable.fromWeb(response.body), byteCap, fileStream);
			downloadCompleted = true;
		} finally {
			if (!downloadCompleted) (0, node_fs.rmSync)(dest, { force: true });
		}
	} finally {
		await guarded.release();
	}
...
function findBinaryRecursively(rootDir, binaryFileName) {
	...
	while (stack.length > 0) {
		...
		const entries = (0, node_fs.readdirSync)(currentDir, { withFileTypes: true });
		for (const entry of entries) {
...
async function downloadTool(tool) {
	...
	const plat = (0, node_os.platform)();
	const architecture = (0, node_os.arch)();
	...
	(0, node_fs.mkdirSync)(TOOLS_DIR, { recursive: true });
	const downloadUrl = `https://github.com/${config.repo}/releases/download/${config.tagPrefix}${version}/${assetName}`;
	const archivePath = (0, node_path.join)(TOOLS_DIR, assetName);
	const binaryExt = plat === "win32" ? ".exe" : "";
	const binaryPath = (0, node_path.join)(TOOLS_DIR, config.binaryName + binaryExt);
	await downloadFile(downloadUrl, archivePath, MAX_ARCHIVE_BYTES);
	const extractDir = (0, node_path.join)(TOOLS_DIR, `extract_tmp_${config.binaryName}_${process.pid}_${(0, node_crypto.randomUUID)()}`);
	(0, node_fs.mkdirSync)(extractDir, { recursive: true });
	try {
		...
		let extractedBinary = [(0, node_path.join)((0, node_path.join)(extractDir, assetName.replace(/\.(tar\.gz|zip)$/, "")), binaryFileName), (0, node_path.join)(extractDir, binaryFileName)].find((candidate) => (0, node_fs.existsSync)(candidate));
		if (!extractedBinary) extractedBinary = findBinaryRecursively(extractDir, binaryFileName) ?? void 0;
		if (extractedBinary) (0, node_fs.renameSync)(extractedBinary, binaryPath);
		else throw new Error(`Binary not found in archive: expected ${binaryFileName} under ${extractDir}`);
		if (plat !== "win32") (0, node_fs.chmodSync)(binaryPath, 493);
	} finally {
...
async function ensureTool(tool, silent = false) {
	...
	if (isOfflineModeEnabled()) {
		if (!silent) console.log(chalk.default.yellow(`${config.name} not found. Offline mode enabled, skipping download.`));
		return;
	}
	if ((0, node_os.platform)() === "android") {
		const pkgName = TERMUX_PACKAGES[tool] ?? tool;
		if (!silent) console.log(chalk.default.yellow(`${config.name} not found. Install with: pkg install ${pkgName}`));
		return;
	}
	if (!silent) console.log(chalk.default.dim(`${config.name} not found. Downloading...`));
	try {
		const path = await downloadTool(tool);
		if (!silent) console.log(chalk.default.dim(`${config.name} installed to ${path}`));
		return path;
	} catch (e) {
		if (!silent) console.log(chalk.default.yellow(`Failed to download ${config.name}: ${e instanceof Error ? e.message : String(e)}`));
		return;
	}
}
const testing = { downloadFile };
if (process.env.VITEST || false) globalThis[Symbol.for("operator.toolsManagerTestApi")] = { testing };
//#endregion
...
function isInsideGitRepository(searchPath) {
	for (let current = searchPath;;) {
		if ((0, node_fs.existsSync)(node_path.default.join(current, ".git"))) return true;
		const parent = node_path.default.dirname(current);
		if (parent === current) return false;
		current = parent;
	}
}
const findSchema = typebox.Type.Object({
	pattern: typebox.Type.String({ description: "File glob, e.g. **/*.ts." }),
	path: typebox.Type.Optional(typebox.Type.String({ description: "Search dir; default cwd." })),
	limit: typebox.Type.Optional(typebox.Type.Number({ description: "Max results; default 1000." }))
});
...
function buildFindResult(params) {
	...
	const truncation = require_session_manager.truncateHead(params.relativized.join("\n"), { maxLines: Number.MAX_SAFE_INTEGER });
	...
	if (truncation.truncated) {
		notices.push(`${require_session_manager.formatSize(require_session_manager.DEFAULT_MAX_BYTES)} limit reached`);
...
function createFindToolDefinition(cwd, options) {
	...
	return {
		name: "find",
		label: "find",
		description: `Find by glob; paths relative to search dir. Respects .gitignore. Caps ${DEFAULT_LIMIT$2} results/${require_session_manager.DEFAULT_MAX_BYTES / 1024}KB.`,
		...
		async execute(toolCallId, { pattern, path: searchDir, limit }, signal, onUpdate, ctx) {
			return new Promise((resolve, reject) => {
				...
				(async () => {
					try {
						const searchPath = require_path_utils.resolveToCwd(searchDir || ".", cwd);
						...
						if (customOps?.glob) {
							...
							const relativized = results.map((p) => {
								...
								return toPosixPath(node_path.default.relative(searchPath, p));
							});
						...
						const child = require_exec.spawnCommand([fdPath, ...args], {
							buffer: false,
							reject: false,
							stdio: [
								"ignore",
						...
						require_exec.releaseChildProcessOutputAfterExit(child);
						const rl = (0, node_readline.createInterface)({ input: child.stdout });
						let stderr = "";
						const lines = [];
						stopChild = () => {
						...
						child.on("close", (code) => {
							...
							for (const rawLine of lines) {
								...
								else relativePath = node_path.default.relative(searchPath, line);
...
const grepSchema = typebox.Type.Object({
	pattern: typebox.Type.String({ description: "Regex/literal pattern." }),
	path: typebox.Type.Optional(typebox.Type.String({ description: "File/dir; default cwd." })),
	glob: typebox.Type.Optional(typebox.Type.String({ description: "File glob, e.g. *.ts." })),
	ignoreCase: typebox.Type.Optional(typebox.Type.Boolean({ description: "Ignore case; default false." })),
	literal: typebox.Type.Optional(typebox.Type.Boolean({ description: "Literal, not regex; default false." })),
	context: typebox.Type.Optional(typebox.Type.Number({ description: "Context lines each side; default 0." })),
	limit: typebox.Type.Optional(typebox.Type.Number({ description: "Max matches; default 100." }))
});
const DEFAULT_LIMIT$1 = 100;
const defaultGrepOperations = {
	isDirectory: (p) => (0, node_fs.statSync)(p).isDirectory(),
	readFile: (p) => (0, node_fs.readFileSync)(p, "utf-8")
};
...
function createGrepToolDefinition(cwd, options) {
	...
	return {
		name: "grep",
		label: "grep",
		description: `Search contents; returns path:line matches. Respects .gitignore. Caps ${DEFAULT_LIMIT$1} matches/${require_session_manager.DEFAULT_MAX_BYTES / 1024}KB; lines cap 500 chars.`,
				...
				(async () => {
					try {
						...
						const formatPath = (filePath) => {
							if (isDirectory) {
								const relative = node_path.default.relative(searchPath, filePath);
						...
						if (settled) return;
						const spawnedChild = require_exec.spawnCommand([rgPath, ...args], {
							buffer: false,
							reject: false,
							stdio: [
								"ignore",
						...
						require_exec.releaseChildProcessOutputAfterExit(spawnedChild);
						child = spawnedChild;
						rl = (0, node_readline.createInterface)({ input: spawnedChild.stdout });
						let stderr = "";
						let matchCount = 0;
						...
						const formatBlock = async (filePath, lineNumber) => {
							...
							for (let current = start; current <= end; current++) {
								...
								const { text: truncatedText, wasTruncated } = require_session_manager.truncateLine(sanitized);
						...
						spawnedChild.on("close", (code) => {
							...
							(async () => {
								...
								for (const match of matches) if (contextValue === 0 && match.lineText !== void 0) {
									...
									const { text: truncatedText, wasTruncated } = require_session_manager.truncateLine(match.lineText.replace(/\r\n/g, "\n").replace(/\r/g, "").replace(/\n$/, ""));
									if (wasTruncated) linesTruncated = true;
								...
								const truncation = require_session_manager.truncateHead(outputLines.join("\n"), { maxLines: Number.MAX_SAFE_INTEGER });
...
*/
const lsSchema = typebox.Type.Object({
	path: typebox.Type.Optional(typebox.Type.String({ description: "Directory; default cwd." })),
	limit: typebox.Type.Optional(typebox.Type.Number({ description: "Max entries; default 500." }))
});
const DEFAULT_LIMIT = 500;
const defaultLsOperations = {
	exists: node_fs.existsSync,
	stat: node_fs.statSync,
	readdir: node_fs.readdirSync
};
...
function createLsToolDefinition(cwd, options) {
	...
	return {
		name: "ls",
		label: "ls",
		description: `List dir alphabetically; / marks dirs; includes dotfiles. Caps ${DEFAULT_LIMIT} entries/${require_session_manager.DEFAULT_MAX_BYTES / 1024}KB.`,
		promptSnippet: "List directory contents",
		parameters: lsSchema,
		async execute(toolCallId, { path, limit }, signal, onUpdate, ctx) {
			if (signal?.aborted) throw new Error("Operation aborted");
			const runListing = async () => {
				try {
					const dirPath = require_path_utils.resolveToCwd(path || ".", cwd);
					...
					for (const entry of entries) {
						...
						const fullPath = node_path.default.join(dirPath, entry);
						let suffix = "";
						try {
					...
					const truncation = require_session_manager.truncateHead(results.join("\n"), { maxLines: Number.MAX_SAFE_INTEGER });
					let output = truncation.content;
					const details = {};
					const notices = [];
					...
					throw (0, _gabrielvfonseca_normalization_core_error_coercion.toErrorObject)(e, "Non-Error rejection");
				}
			};
...
async function normalizeImageForProvider(image) {
	...
	try {
		return {
			image: {
				type: "image",
				data: (await require_media_services.convertImageToPng(Buffer.from(image.data, "base64"))).toString("base64"),
				mimeType: "image/png"
			},
...
async function resizeImage(img) {
	...
	const processor = require_media_services.createImageProcessor();
	try {
...
*/
const readSchema = typebox.Type.Object({
	path: typebox.Type.String({ description: "File path; relative/absolute." }),
	offset: typebox.Type.Optional(typebox.Type.Integer({
		minimum: 1,
		description: "Start line; 1-based."
	})),
	limit: typebox.Type.Optional(typebox.Type.Number({ description: "Max lines." }))
});
...
const defaultReadOperations = {
	...
	decodeText: ({ buffer }) => require_spawn_utils.decodeWindowsTextFileBuffer({ buffer }),
	readFile: (path) => (0, node_fs_promises.readFile)(path),
	access: (path) => (0, node_fs_promises.access)(path, node_fs.constants.R_OK),
...
function getOperatorDocsClassification(absolutePath) {
	const relativePath = (0, node_path.relative)((0, node_path.resolve)((0, node_path.dirname)(require_config.getReadmePath())), (0, node_path.resolve)(absolutePath));
	if (relativePath === "" || relativePath === ".." || relativePath.startsWith(`..${node_path.sep}`) || (0, node_path.isAbsolute)(relativePath)) return;
...
function getCompactReadClassification(args, cwd) {
	const rawPath = str(args?.file_path ?? args?.path);
	if (!rawPath) return;
	const absolutePath = require_path_utils.resolveReadPath(rawPath, cwd);
	const fileName = (0, node_path.basename)(absolutePath);
	if (fileName === "SKILL.md") return {
		kind: "skill",
		label: (0, node_path.basename)((0, node_path.dirname)(absolutePath)) || fileName
	};
	const docsClassification = getOperatorDocsClassification(absolutePath);
	if (docsClassification) return docsClassification;
	if (COMPACT_RESOURCE_FILE_NAMES.has(fileName)) return {
		kind: "resource",
		label: require_curator.formatPathRelativeToCwdOrAbsolute(absolutePath, cwd)
	};
}
async function resolveLocalReadPath(filePath, cwd) {
	const normalizedMediaSource = require_media_reference.normalizeMediaReferenceSource(filePath);
	if (require_media_reference.classifyMediaReferenceSource(normalizedMediaSource).isMediaStoreUrl) return await require_media_reference.resolveMediaReferenceLocalPath(normalizedMediaSource);
	return require_path_utils.resolveReadPath(filePath, cwd);
}
async function resolveReadToolPath(ops, filePath, cwd) {
	return await (ops.resolvePath?.(filePath, cwd) ?? require_path_utils.resolveReadPath(filePath, cwd));
}
...
function formatReadResult(args, result, options, theme, showImages, cwd, isError) {
	...
	if (remaining > 0) text += `${theme.fg("muted", `\n... (${remaining} more lines,`)} ${keyHint("app.tools.expand", "to expand")})`;
	const truncation = result.details?.truncation;
	if (truncation?.truncated) if (truncation.firstLineExceedsLimit) text += `\n${theme.fg("warning", `[First line exceeds ${require_session_manager.formatSize(truncation.maxBytes ?? 51200)} limit]`)}`;
	...
	else text += `\n${theme.fg("warning", `[Truncated: ${truncation.outputLines} lines shown (${require_session_manager.formatSize(truncation.maxBytes ?? 51200)} limit)]`)}`;
	return text;
}
function createReadToolDefinition(cwd, options) {
	...
	return {
		name: "read",
		label: "read",
		description: `Read text/image file (jpg/png/gif/webp/bmp); images attach. Text caps ${require_session_manager.DEFAULT_MAX_LINES} lines or ${require_session_manager.DEFAULT_MAX_BYTES / 1024}KB. Large/full file: continue offset/limit.`,
				...
				(async () => {
					try {
						...
						if (mimeType) {
							...
							if (limit !== void 0) {
								const normalizedLimit = normalizePositiveLimit(limit, require_session_manager.DEFAULT_MAX_LINES);
							...
							const truncation = require_session_manager.truncateHead(selectedContent);
							let outputText;
							if (truncation.firstLineExceedsLimit) {
								const firstLine = allLines.at(startLine);
								if (firstLine === void 0) throw new Error("Requested line is outside the file.");
								outputText = `[Line ${startLineDisplay} is ${require_session_manager.formatSize(Buffer.byteLength(firstLine, "utf-8"))}, exceeds ${require_session_manager.formatSize(require_session_manager.DEFAULT_MAX_BYTES)} limit. Use bash: sed -n '${startLineDisplay}p' ${quotePosixShellArg(path)} | head -c ${require_session_manager.DEFAULT_MAX_BYTES}]`;
								details = { truncation };
								...
								else outputText += `\n\n[Showing lines ${startLineDisplay}-${endLineDisplay} of ${totalFileLines} (${require_session_manager.formatSize(require_session_manager.DEFAULT_MAX_BYTES)} limit). Use offset=${nextOffset} to continue.]`;
								details = { truncation };
						...
						signal?.removeEventListener("abort", onAbort);
						if (!aborted) reject((0, _gabrielvfonseca_normalization_core_error_coercion.toErrorObject)(error, "Non-Error rejection"));
					}
				})();
			});
		},
...
*/
const writeSchema = typebox.Type.Object({
	path: typebox.Type.String({ description: "File path; relative/absolute." }),
	content: typebox.Type.String({ description: "File content." })
});
const defaultWriteOperations = {
	writeFile: (path, content) => (0, node_fs_promises.writeFile)(path, content, "utf-8"),
	mkdir: (dir) => (0, node_fs_promises.mkdir)(dir, { recursive: true }).then(() => {}),
	readFile: (path) => (0, node_fs_promises.readFile)(path),
	statFile: async (path) => {
		try {
			const stat = await (0, node_fs_promises.stat)(path);
			return {
...
var WriteCallRenderComponent = class extends _earendil_works_pi_tui.Text {
	constructor() {
		super("", 0, 0);
	}
};
...
async function didWriteMetadataChange(absolutePath, beforeStat, ops) {
	...
	if (afterStat?.type !== "file") return false;
...
function successfulWriteResult(path, content) {
	return require_common.textResult(`Successfully wrote ${Buffer.byteLength(content, "utf8")} bytes to ${path}`, void 0);
}
...
function createWriteToolDefinition(cwd, options) {
	...
	return {
		...
		async execute(toolCallId, { path, content }, signal, onUpdate, ctx) {
			const absolutePath = require_path_utils.resolveToCwd(path, cwd);
			const dir = (0, node_path.dirname)(absolutePath);
			return withFileMutationQueue(absolutePath, async () => {
				const precheck = await readOriginalWriteState(absolutePath, content, ops);
				if (signal?.aborted) throw new Error("Operation aborted");
				if (precheck.state === "same") return {
					...require_common.textResult(`No changes made to ${path}. The file already has identical content.`, void 0),
					terminate: true
				};
				try {
					await ops.mkdir(dir);
...
var extension_sdk_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	AuthStorage: () => AuthStorage,
	CURRENT_SESSION_VERSION: () => 3,
	DEFAULT_COMPACTION_SETTINGS: () => require_session_manager.DEFAULT_COMPACTION_SETTINGS,
	DEFAULT_MAX_BYTES: () => require_session_manager.DEFAULT_MAX_BYTES,
	DEFAULT_MAX_LINES: () => require_session_manager.DEFAULT_MAX_LINES,
	...
	SessionManager: () => require_session_manager.SessionManager,
	SettingsManager: () => SettingsManager,
	VERSION: () => require_config.VERSION,
	allToolNames: () => allToolNames,
	buildSessionContext: () => require_session_manager.buildSessionContext,
	calculateContextTokens: () => require_session_manager.calculateContextTokens,
	clearApiKeyCache: () => clearApiKeyCache,
	collectEntriesForBranchSummary: () => collectEntriesForBranchSummary,
	compact: () => compact,
	convertToLlm: () => require_session_manager.convertToLlm,
	...
	createSourceInfo: () => require_skill_version.createSourceInfo,
	createSyntheticSourceInfo: () => require_skill_version.createSyntheticSourceInfo,
	...
	estimateContextTokens: () => require_session_manager.estimateContextTokens,
	estimateTokens: () => require_session_manager.estimateTokens,
	executeBashWithOperations: () => executeBashWithOperations,
	findCutPoint: () => require_session_manager.findCutPoint,
	findExactModelReferenceMatch: () => findExactModelReferenceMatch,
	findInitialModel: () => findInitialModel,
	findMostRecentSession: () => require_session_manager.findMostRecentSession,
	findTurnStartIndex: () => require_session_manager.findTurnStartIndex,
	formatSize: () => require_session_manager.formatSize,
	generateBranchSummary: () => generateBranchSummary,
	generateSummary: () => generateSummary,
	getAgentDir: () => require_config.getAgentDir,
	getDefaultSessionDir: () => require_session_manager.getDefaultSessionDir,
	getLastAssistantUsage: () => require_session_manager.getLastAssistantUsage,
	getLatestCompactionEntry: () => require_session_manager.getLatestCompactionEntry,
	...
	loadEntriesFromFile: () => require_session_manager.loadEntriesFromFile,
	migrateSessionEntries: () => require_session_manager.migrateSessionEntries,
	normalizeLoadedFileEntry: () => require_session_manager.normalizeLoadedFileEntry,
	parseModelPattern: () => parseModelPattern,
	parseSessionEntries: () => require_session_manager.parseSessionEntries,
	prepareBranchEntries: () => require_session_manager.prepareBranchEntries,
	...
	serializeConversation: () => require_session_manager.serializeConversation,
	shouldCompact: () => require_session_manager.shouldCompact,
	truncateHead: () => require_session_manager.truncateHead,
	truncateLine: () => require_session_manager.truncateLine,
	truncateTail: () => require_session_manager.truncateTail,
...
const bundledAgentCore = {
	Agent: require_session_manager.Agent,
	bashExecutionToText: require_session_manager.bashExecutionToText,
	buildSessionContext: require_session_manager.buildSessionContext$1,
	calculateContextTokens: require_session_manager.calculateContextTokens,
	collectEntriesForBranchSummaryFromBranches: require_session_manager.collectEntriesForBranchSummaryFromBranches,
	compact: require_session_manager.compact,
	estimateContextTokens: require_session_manager.estimateContextTokens,
	estimateTokens: require_session_manager.estimateTokens,
	findCutPoint: require_session_manager.findCutPoint,
	findTurnStartIndex: require_session_manager.findTurnStartIndex,
	generateBranchSummary: require_session_manager.generateBranchSummary,
	generateSummary: require_session_manager.generateSummary,
	getLastAssistantUsage: require_session_manager.getLastAssistantUsage,
	openClawAgentCoreRuntime: require_session_manager.openClawAgentCoreRuntime,
	prepareBranchEntries: require_session_manager.prepareBranchEntries,
	prepareCompaction: require_session_manager.prepareCompaction,
	runAgentLoop: require_session_manager.runAgentLoop,
	serializeConversation: require_session_manager.serializeConversation,
	shouldCompact: require_session_manager.shouldCompact,
	uuidv7: require_session_manager.uuidv7,
	BRANCH_SUMMARY_PREFIX: require_session_manager.BRANCH_SUMMARY_PREFIX,
	BRANCH_SUMMARY_SUFFIX: require_session_manager.BRANCH_SUMMARY_SUFFIX,
	COMPACTION_SUMMARY_PREFIX: require_session_manager.COMPACTION_SUMMARY_PREFIX,
	COMPACTION_SUMMARY_SUFFIX: require_session_manager.COMPACTION_SUMMARY_SUFFIX,
	DEFAULT_COMPACTION_SETTINGS: require_session_manager.DEFAULT_COMPACTION_SETTINGS
};
const VIRTUAL_MODULES = {
	typebox,
	"typebox/compile": typebox_compile,
	"typebox/format": typebox_format,
	"typebox/value": typebox_value,
	"@sinclair/typebox": typebox,
	"@sinclair/typebox/compile": typebox_compile,
	"@sinclair/typebox/format": typebox_format,
	"@sinclair/typebox/value": typebox_value,
	"openclaw/plugin-sdk/agent-core": bundledAgentCore,
	"@gabrielvfonseca/plugin-sdk/agent-core": bundledAgentCore,
	"openclaw/plugin-sdk/llm": require_session_manager.llm_exports,
	"@gabrielvfonseca/plugin-sdk/llm": require_session_manager.llm_exports,
	"openclaw/plugin-sdk/agent-sessions": extension_sdk_exports,
	"@gabrielvfonseca/plugin-sdk/agent-sessions": extension_sdk_exports
};
...
function resolveExtensionSafeAgentSessionsEntry() {
	const currentDirname = node_path.dirname((0, node_url.fileURLToPath)(require("url").pathToFileURL(__filename).href));
	const jsEntry = node_path.resolve(currentDirname, "..", "extension-sdk.js");
	return node_fs.existsSync(jsEntry) ? jsEntry : node_path.resolve(currentDirname, "..", "extension-sdk.ts");
}
function getExtensionLoaderAliases() {
	if (aliases) return aliases;
	const agentSessionsEntry = resolveExtensionSafeAgentSessionsEntry();
	const typeboxEntry = require$1.resolve("typebox");
	const typeboxCompileEntry = require$1.resolve("typebox/compile");
	const typeboxFormatEntry = require$1.resolve("typebox/format");
	const typeboxValueEntry = require$1.resolve("typebox/value");
	aliases = {
		...require_plugin_module_loader_cache.buildPluginLoaderAliasMap((0, node_url.fileURLToPath)(require("url").pathToFileURL(__filename).href), process.argv[1], require("url").pathToFileURL(__filename).href),
		"openclaw/plugin-sdk/agent-sessions": agentSessionsEntry,
		"@gabrielvfonseca/plugin-sdk/agent-sessions": agentSessionsEntry,
...
function expandPath(p) {
	...
	if (normalized.startsWith("~/")) return node_path.join(node_os.homedir(), normalized.slice(2));
	if (normalized.startsWith("~")) return node_path.join(node_os.homedir(), normalized.slice(1));
	return normalized;
}
function resolvePath(extPath, cwd) {
	const expanded = expandPath(extPath);
	if (node_path.isAbsolute(expanded)) return expanded;
	return node_path.resolve(cwd, expanded);
}
/**
...
function isJavaScriptExtensionPath(extensionPath) {
	switch (node_path.extname(extensionPath).toLowerCase()) {
		case ".cjs":
...
function extensionSourceNeedsJitiAliasResolution(extensionPath) {
	try {
		const source = node_fs.readFileSync(extensionPath, "utf8");
		return EXTENSION_LOADER_ALIAS_IMPORT_PATTERN.test(source) || RELATIVE_EXTENSION_IMPORT_PATTERN.test(source) || node_path.extname(extensionPath).toLowerCase() === ".js" && COMMONJS_EXTENSION_EXPORT_PATTERN.test(source);
	} catch {
		return true;
	}
}
function shouldLoadExtensionWithNativeImport(extensionPath) {
	return !require_config.isBunBinary && isJavaScriptExtensionPath(extensionPath) && !extensionSourceNeedsJitiAliasResolution(extensionPath);
}
async function loadNativeExtensionModule(extensionPath) {
	const url = (0, node_url.pathToFileURL)(extensionPath);
	url.searchParams.set("v", String(++nativeExtensionLoadCounter));
	try {
		const cachedPath = require$1.resolve(extensionPath);
		delete require$1.cache[cachedPath];
	} catch {}
...
async function loadExtensionSourceTransformModule(extensionPath) {
	if (!extensionSourceTransformLoader) {
		require_plugin_module_loader_cache.installOperatorInternalCorePackageNativeResolver({ moduleUrl: require("url").pathToFileURL(__filename).href });
		extensionSourceTransformLoader = (await loadCreateJitiLoaderFactory())(require("url").pathToFileURL(__filename).href, {
			...require_config.isBunBinary ? {
				...require_plugin_module_loader_cache.buildPluginLoaderJitiOptions({}),
				tryNative: false,
				virtualModules: VIRTUAL_MODULES
			} : require_plugin_module_loader_cache.buildPluginLoaderJitiOptions(getExtensionLoaderAliases()),
			moduleCache: false
		});
	}
...
function createExtension(extensionPath, resolvedPath) {
	return {
		path: extensionPath,
		resolvedPath,
		sourceInfo: require_skill_version.createSyntheticSourceInfo(extensionPath, {
			source: extensionPath.startsWith("<") && extensionPath.endsWith(">") ? extensionPath.slice(1, -1).split(":")[0] || "temporary" : "local",
			baseDir: extensionPath.startsWith("<") ? void 0 : node_path.dirname(resolvedPath)
		}),
...
function buildSystemPrompt(options) {
	...
	const readmePath = require_config.getReadmePath();
	const docsPath = require_config.getDocsPath();
	const examplesPath = require_config.getExamplesPath();
	...
	let prompt = `You are an expert coding assistant operating inside Operator's embedded coding agent harness. You help users by reading files, executing commands, editing code, and writing new files.
...
${require_promised_work_prompt.buildPromisedWorkPromptSection().join("\n")}
...
var AgentSessionBase = class {
	...
	async getCompactionRequestAuth(model) {
		if (this.agent.streamFn === _gabrielvfonseca_ai_internal_runtime.streamSimple) return this.getRequiredRequestAuth(model);
		const result = await this.sessionModelRegistry.getApiKeyAndHeaders(model);
		return result.ok ? {
			apiKey: result.apiKey,
			headers: result.headers
	...
	*/
	dispose() {
		...
		this.eventListeners = [];
		(0, _gabrielvfonseca_ai_internal_runtime.cleanupSessionResources)(this.sessionId);
	}
	/** Full agent state */
	get state() {
...
function expandTildePath(input) {
	const trimmed = input.trim();
	if (trimmed === "~") return (0, node_os.homedir)();
	if (trimmed.startsWith("~/")) return (0, node_path.join)((0, node_os.homedir)(), trimmed.slice(2));
	if (trimmed.startsWith("~")) return (0, node_path.join)((0, node_os.homedir)(), trimmed.slice(1));
	return trimmed;
}
//#endregion
...
function loadTemplateFromFile(filePath, sourceInfo) {
	try {
		const { frontmatter, body } = parseFrontmatter((0, node_fs.readFileSync)(filePath, "utf-8"));
		const name = (0, node_path.basename)(filePath).replace(/\.md$/, "");
		let description = frontmatter.description || "";
		if (!description) {
			const firstLine = body.split("\n").find((line) => line.trim());
			if (firstLine) {
				description = (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(firstLine, 60);
...
function loadTemplatesFromDir(dir, getSourceInfo) {
	const templates = [];
	if (!(0, node_fs.existsSync)(dir)) return templates;
	try {
		const entries = (0, node_fs.readdirSync)(dir, { withFileTypes: true });
		for (const entry of entries) {
			const fullPath = (0, node_path.join)(dir, entry.name);
...
function resolvePromptPath(p, cwd) {
	...
	return (0, node_path.isAbsolute)(normalized) ? normalized : (0, node_path.resolve)(cwd, normalized);
}
/**
...
function loadPromptTemplates(options) {
	...
	const templates = [];
	const globalPromptsDir = options.agentDir ? (0, node_path.join)(options.agentDir, "prompts") : resolvedAgentDir;
	const projectPromptsDir = (0, node_path.resolve)(resolvedCwd, require_config.CONFIG_DIR_NAME, "prompts");
	const isUnderPath = (target, root) => {
		const normalizedRoot = (0, node_path.resolve)(root);
		if (target === normalizedRoot) return true;
		const prefix = normalizedRoot.endsWith(node_path.sep) ? normalizedRoot : `${normalizedRoot}${node_path.sep}`;
		return target.startsWith(prefix);
	};
	const getSourceInfo = (resolvedPath) => {
		if (isUnderPath(resolvedPath, globalPromptsDir)) return require_skill_version.createSyntheticSourceInfo(resolvedPath, {
			source: "local",
			scope: "user",
			baseDir: globalPromptsDir
		});
		if (isUnderPath(resolvedPath, projectPromptsDir)) return require_skill_version.createSyntheticSourceInfo(resolvedPath, {
			source: "local",
			scope: "project",
			baseDir: projectPromptsDir
		});
		return require_skill_version.createSyntheticSourceInfo(resolvedPath, {
			source: "local",
			baseDir: (0, node_fs.statSync)(resolvedPath).isDirectory() ? resolvedPath : (0, node_path.dirname)(resolvedPath)
		});
	};
	if (includeDefaults) {
	...
	for (const rawPath of promptPaths) {
		...
		if (!(0, node_fs.existsSync)(resolvedPath)) continue;
		try {
			const stats = (0, node_fs.statSync)(resolvedPath);
...
var AgentSessionPrompting = class extends AgentSessionBase {
	...
	*/
	expandSkillCommand(text) {
		...
		try {
			const body = stripFrontmatter((0, node_fs.readFileSync)(skill.filePath, "utf-8")).trim();
...
var AgentSessionModels = class extends AgentSessionPrompting {
	async emitModelSelect(nextModel, previousModel, source) {
		if ((0, _gabrielvfonseca_ai_internal_runtime.modelsAreEqual)(previousModel, nextModel)) return;
	...
	*/
	...
	async cycleScopedModel(direction) {
		...
		let currentIndex = scopedModels.findIndex((sm) => (0, _gabrielvfonseca_ai_internal_runtime.modelsAreEqual)(sm.model, currentModel));
	...
	async cycleAvailableModel(direction) {
		...
		const currentModel = this.model;
		let currentIndex = availableModels.findIndex((m) => (0, _gabrielvfonseca_ai_internal_runtime.modelsAreEqual)(m, currentModel));
		if (currentIndex === -1) currentIndex = 0;
	...
	*/
	getAvailableThinkingLevels() {
		...
		return (0, _gabrielvfonseca_ai_internal_runtime.getSupportedThinkingLevels)(this.model);
	}
	/**
	...
	*/
	...
	clampThinkingLevel(level) {
		return this.model ? (0, _gabrielvfonseca_ai_internal_runtime.clampThinkingLevel)(this.model, level) : "off";
	}
	/**
...
var AgentSessionInspection = class extends AgentSessionModels {
	...
	*/
	...
	getContextUsage() {
		...
		const latestCompaction = require_session_manager.getLatestCompactionEntry(branchEntries);
		...
		if (latestCompaction) {
			...
			for (const entry of branchEntries.slice(compactionIndex + 1).toReversed()) if (entry.type === "message" && entry.message.role === "assistant") {
				...
				if (assistant.stopReason !== "aborted" && assistant.stopReason !== "error") {
					...
					if (require_session_manager.calculateContextTokens(assistant.usage) > 0) {
		...
		const tokens = estimateFromContent ? estimateMessagesFromContent(this.messages) : require_session_manager.estimateContextTokens(this.messages).tokens;
		return {
			tokens,
			contextWindow,
	...
	/**
	* @deprecated Use the Operator session export command instead.
	* @param outputPath Optional output path (defaults to session directory)
	* @returns Path to exported file
	*/
	async exportToHtml(_outputPath) {
		throw new Error("AgentSession.exportToHtml is deprecated; use the Operator session export command.");
	}
	/**
	...
	*/
	exportToJsonl(outputPath) {
		const filePath = (0, node_path.resolve)(outputPath ?? `session-${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}.jsonl`);
		...
		(0, node_fs.writeFileSync)(filePath, `${lines.join("\n")}\n`);
		return filePath;
	}
	/**
...
var AgentSessionCompaction = class extends AgentSessionInspection {
	...
	*/
	...
	async getAutoCompactionRequestAuth(model) {
		if (this.agent.streamFn !== _gabrielvfonseca_ai_internal_runtime.streamSimple) return this.getCompactionRequestAuth(model);
	...
	async runCompactionWork(options) {
		...
		const preparation = unwrapCoreResult(require_session_manager.prepareCompaction(pathEntries, options.settings));
		if (!preparation) {
			if (isManual) {
		...
		compactionResult ??= unwrapCoreResult(await require_session_manager.compact(preparation, this.model, auth.apiKey, auth.headers, options.customInstructions, options.signal, this.thinkingLevel, this.agent.streamFn));
	...
	*/
	async checkCompaction(assistantMessage, skipAbortedCheck = true) {
		...
		const compactionEntry = require_session_manager.getLatestCompactionEntry(this.sessionManager.getBranch());
		if (compactionEntry !== null && assistantMessage.timestamp <= new Date(compactionEntry.timestamp).getTime()) return false;
		if (sameModel && (0, _gabrielvfonseca_ai_internal_runtime.isContextOverflow)(assistantMessage, contextWindow)) {
		...
		if (assistantMessage.stopReason === "error") {
			const messages = this.agent.state.messages;
			const estimate = require_session_manager.estimateContextTokens(messages);
		...
		} else contextTokens = require_session_manager.calculateContextTokens(assistantMessage.usage);
		if (require_session_manager.shouldCompact(contextTokens, contextWindow, settings)) return await this.runAutoCompaction("threshold", false);
		return false;
	}
	/**
...
var AgentSessionExtensions = class extends AgentSessionCompaction {
	...
	buildExtensionResourcePaths(entries) {
		return entries.map((entry) => {
			...
			const baseDir = entry.extensionPath.startsWith("<") ? void 0 : (0, node_path.dirname)(entry.extensionPath);
			return {
				path: entry.path,
				metadata: {
					source,
	...
	getExtensionSourceLabel(extensionPath) {
		...
		return `extension:${(0, node_path.basename)(extensionPath).replace(/\.(ts|js)$/, "")}`;
	}
	...
	refreshToolRegistry(options) {
		...
		const allCustomTools = [...this.currentExtensionRunner.getAllRegisteredTools(), ...this.customTools.map((definition) => ({
			definition,
			sourceInfo: require_skill_version.createSyntheticSourceInfo(`<sdk:${definition.name}>`, { source: "sdk" })
		...
		const definitionRegistry = new Map(Array.from(this.baseToolDefinitions.entries()).filter(([name]) => isAllowedTool(name)).map(([name, definition]) => [name, {
			definition,
			sourceInfo: require_skill_version.createSyntheticSourceInfo(`<builtin:${name}>`, { source: "builtin" })
		}]));
		...
		const wrappedBuiltInTools = wrapRegisteredTools(Array.from(this.baseToolDefinitions.values()).filter((definition) => isAllowedTool(definition.name)).map((definition) => ({
			definition,
			sourceInfo: require_skill_version.createSyntheticSourceInfo(`<builtin:${definition.name}>`, { source: "builtin" })
		})), runner);
...
var AgentSessionExecution = class extends AgentSessionExtensions {
	...
	*/
	isRetryableError(message) {
		if (message.stopReason !== "error" || !message.errorMessage) return false;
		if ((0, _gabrielvfonseca_ai_internal_runtime.isContextOverflow)(message, this.model?.contextWindow ?? 0)) return false;
...
var AgentSessionTree = class extends AgentSessionExecution {
	...
	*/
	async navigateTree(targetId, options = {}) {
		...
		const { entries: entriesToSummarize, commonAncestorId } = oldLeafId ? require_session_manager.collectEntriesForBranchSummaryFromBranches(this.sessionManager.getBranch(oldLeafId), this.sessionManager.getBranch(targetId)) : {
			entries: [],
			commonAncestorId: null
		};
		...
		try {
			...
			if (options.summarize && entriesToSummarize.length > 0 && !extensionSummary) {
				...
				const result = normalizeBranchSummaryResult(await require_session_manager.generateBranchSummary(entriesToSummarize, {
					model,
					apiKey,
					headers,
					signal: this.branchSummaryAbortController.signal,
...
function createSkillSourceInfo(filePath, baseDir, source) {
	switch (source) {
		case "user": return require_skill_version.createSyntheticSourceInfo(filePath, {
			source: "local",
			scope: "user",
			baseDir
		});
		case "project": return require_skill_version.createSyntheticSourceInfo(filePath, {
			source: "local",
			scope: "project",
			baseDir
		});
		case "path": return require_skill_version.createSyntheticSourceInfo(filePath, {
			source: "local",
			baseDir
		});
		default: return require_skill_version.createSyntheticSourceInfo(filePath, {
			source,
			baseDir
		});
	}
...
function loadSkillFromFile(filePath, source) {
	const diagnostics = [];
	try {
		const rawContent = (0, node_fs.readFileSync)(filePath, "utf-8");
		const { frontmatter } = parseFrontmatter(rawContent);
		const skillDir = (0, node_path.dirname)(filePath);
		const parentDirName = (0, node_path.basename)(skillDir);
		...
		return {
			skill: {
				...
				filePath,
				baseDir: skillDir,
				promptVersion: require_skill_version.computeSkillPromptVersion(rawContent),
				source,
...
function loadSkills(options) {
	...
	const resolvedAgentDir = agentDir ?? require_config.getAgentDir();
	...
	function addSkills(result) {
		...
		for (const skill of result.skills) {
			if (archivedSkillFiles.has(require_curator.canonicalizePath(skill.filePath))) continue;
			const realPath = require_curator.canonicalizePath(skill.filePath);
	...
	if (includeDefaults) {
		addSkills(loadSkillsFromDirInternal((0, node_path.join)(resolvedAgentDir, "skills"), "user", true));
		addSkills(loadSkillsFromDirInternal((0, node_path.resolve)(cwd, require_config.CONFIG_DIR_NAME, "skills"), "project", true));
	}
	const userSkillsDir = (0, node_path.join)(resolvedAgentDir, "skills");
	const projectSkillsDir = (0, node_path.resolve)(cwd, require_config.CONFIG_DIR_NAME, "skills");
	const isUnderPath = (target, root) => {
		const normalizedRoot = (0, node_path.resolve)(root);
		if (target === normalizedRoot) return true;
		const prefix = normalizedRoot.endsWith(node_path.sep) ? normalizedRoot : `${normalizedRoot}${node_path.sep}`;
...
function resolvePromptInput(input, description) {
	if (!input) return;
	if ((0, node_fs.existsSync)(input)) try {
		return (0, node_fs.readFileSync)(input, "utf-8");
	} catch (error) {
		console.error(chalk.default.yellow(`Warning: Could not read ${description} file ${input}: ${String(error)}`));
		return;
	}
	return input;
}
function loadContextFileFromDir(dir) {
	for (const filename of [
		...
		if ((0, node_fs.existsSync)(filePath)) try {
			return {
				path: filePath,
				content: (0, node_fs.readFileSync)(filePath, "utf-8")
			};
		} catch (error) {
			console.error(chalk.default.yellow(`Warning: Could not read ${filePath}: ${String(error)}`));
		}
	}
	return null;
}
function loadProjectContextFiles(options) {
	...
	const root = (0, node_path.resolve)("/");
	while (true) {
		...
		const parentDir = (0, node_path.resolve)(currentDir, "..");
...
var DefaultResourceLoader = class {
	...
	extendResources(paths) {
		...
		for (const entry of skillPaths) this.extensionSkillSourceInfos.set(entry.path, require_skill_version.createSourceInfo(entry.path, entry.metadata));
		for (const entry of promptPaths) this.extensionPromptSourceInfos.set(entry.path, require_skill_version.createSourceInfo(entry.path, entry.metadata));
		for (const entry of themePaths) this.extensionThemeSourceInfos.set(entry.path, require_skill_version.createSourceInfo(entry.path, entry.metadata));
	...
	async reload() {
		...
		const mapSkillPath = (resource) => {
			...
			try {
				if (!(0, node_fs.statSync)(resource.path).isDirectory()) return resource.path;
			} catch {
				return resource.path;
			}
			const skillFile = (0, node_path.join)(resource.path, "SKILL.md");
			if ((0, node_fs.existsSync)(skillFile)) {
		...
		for (const p of this.additionalExtensionPaths) if (require_curator.isLocalPath(p) && !(0, node_fs.existsSync)(p)) extensionsResult.errors.push({
			path: p,
		...
		for (const p of this.additionalSkillPaths) if (require_curator.isLocalPath(p) && !(0, node_fs.existsSync)(p) && !this.skillDiagnostics.some((d) => d.path === p)) this.skillDiagnostics.push({
			type: "error",
			message: "Skill path does not exist",
			path: p
		});
		...
		for (const p of this.additionalPromptTemplatePaths) if (require_curator.isLocalPath(p) && !(0, node_fs.existsSync)(p) && !this.promptDiagnostics.some((d) => d.path === p)) this.promptDiagnostics.push({
			type: "error",
		...
		for (const p of this.additionalThemePaths) if (!(0, node_fs.existsSync)(p) && !this.themeDiagnostics.some((d) => d.path === p)) this.themeDiagnostics.push({
			type: "error",
	...
	findSourceInfoForPath(resourcePath, extraSourceInfos, metadataByPath) {
		...
		const normalizedResourcePath = (0, node_path.resolve)(resourcePath);
		if (extraSourceInfos) for (const [sourcePath, sourceInfo] of extraSourceInfos.entries()) {
			const normalizedSourcePath = (0, node_path.resolve)(sourcePath);
			if (normalizedResourcePath === normalizedSourcePath || normalizedResourcePath.startsWith(`${normalizedSourcePath}${node_path.sep}`)) return {
				...sourceInfo,
				path: resourcePath
			};
		}
		if (metadataByPath) {
			...
			if (exact) return require_skill_version.createSourceInfo(resourcePath, exact);
			for (const [sourcePath, metadata] of metadataByPath.entries()) {
				const normalizedSourcePath = (0, node_path.resolve)(sourcePath);
				if (normalizedResourcePath === normalizedSourcePath || normalizedResourcePath.startsWith(`${normalizedSourcePath}${node_path.sep}`)) return require_skill_version.createSourceInfo(resourcePath, metadata);
			}
		}
	}
	getDefaultSourceInfoForPath(filePath) {
		...
		const normalizedPath = (0, node_path.resolve)(filePath);
		const agentRoots = [
			(0, node_path.join)(this.agentDir, "skills"),
			(0, node_path.join)(this.agentDir, "prompts"),
			(0, node_path.join)(this.agentDir, "themes"),
			(0, node_path.join)(this.agentDir, "extensions")
		];
		const projectRoots = [
			(0, node_path.join)(this.cwd, require_config.CONFIG_DIR_NAME, "skills"),
			(0, node_path.join)(this.cwd, require_config.CONFIG_DIR_NAME, "prompts"),
			(0, node_path.join)(this.cwd, require_config.CONFIG_DIR_NAME, "themes"),
			(0, node_path.join)(this.cwd, require_config.CONFIG_DIR_NAME, "extensions")
		];
		...
		return {
			...
			scope: "temporary",
			origin: "top-level",
			baseDir: (0, node_fs.statSync)(normalizedPath).isDirectory() ? normalizedPath : (0, node_path.resolve)(normalizedPath, "..")
		};
	}
	mergePaths(primary, additional) {
		...
		for (const p of [...primary, ...additional]) {
			...
			const canonicalPath = require_curator.canonicalizePath(resolved);
	...
	resolveResourcePath(p) {
		...
		if (trimmed === "~") expanded = (0, node_os.homedir)();
		else if (trimmed.startsWith("~/")) expanded = (0, node_path.join)((0, node_os.homedir)(), trimmed.slice(2));
		else if (trimmed.startsWith("~")) expanded = (0, node_path.join)((0, node_os.homedir)(), trimmed.slice(1));
		return (0, node_path.resolve)(this.cwd, expanded);
	}
	loadThemes(paths, includeDefaults = true) {
		const themes = [];
		const diagnostics = [];
		if (includeDefaults) {
			const defaultDirs = [(0, node_path.join)(this.agentDir, "themes"), (0, node_path.join)(this.cwd, require_config.CONFIG_DIR_NAME, "themes")];
			for (const dir of defaultDirs) this.loadThemesFromDir(dir, themes, diagnostics);
		}
		for (const p of paths) {
			const resolved = (0, node_path.resolve)(this.cwd, p);
			if (!(0, node_fs.existsSync)(resolved)) {
				diagnostics.push({
					type: "warning",
					message: "theme path does not exist",
					path: resolved
	...
	loadThemesFromDir(dir, themes, diagnostics) {
		...
		try {
			...
			for (const entry of entries) {
				let isFile = entry.isFile();
				if (entry.isSymbolicLink()) try {
					isFile = (0, node_fs.statSync)((0, node_path.join)(dir, entry.name)).isFile();
				} catch {
					continue;
				}
				if (!isFile) continue;
				if (!entry.name.endsWith(".json")) continue;
				this.loadThemeFromFile((0, node_path.join)(dir, entry.name), themes, diagnostics);
			}
		} catch (error) {
	...
	discoverSystemPromptFile() {
		const projectPath = (0, node_path.join)(this.cwd, require_config.CONFIG_DIR_NAME, "SYSTEM.md");
		if ((0, node_fs.existsSync)(projectPath)) return projectPath;
		const globalPath = (0, node_path.join)(this.agentDir, "SYSTEM.md");
		if ((0, node_fs.existsSync)(globalPath)) return globalPath;
	}
	discoverAppendSystemPromptFile() {
		const projectPath = (0, node_path.join)(this.cwd, require_config.CONFIG_DIR_NAME, "APPEND_SYSTEM.md");
		if ((0, node_fs.existsSync)(projectPath)) return projectPath;
		const globalPath = (0, node_path.join)(this.agentDir, "APPEND_SYSTEM.md");
		if ((0, node_fs.existsSync)(globalPath)) return globalPath;
	}
...
function isInstallTelemetryEnabled(settingsManager, telemetryEnv = process.env.OPERATOR_TELEMETRY) {
...
function getDefaultAgentDir() {
	return require_config.getAgentDir();
}
function getAttributionHeaders(model, settingsManager) {
	...
	if (model.provider === "cloudflare-workers-ai" || model.provider === "cloudflare-ai-gateway" || baseUrl.includes("api.cloudflare.com") || baseUrl.includes("gateway.ai.cloudflare.com")) return { "User-Agent": "@gabrielvfonseca/operator" };
}
/**
...
async function createAgentSession(options = {}) {
	...
	const authPath = options.agentDir ? (0, node_path.join)(agentDir, "auth.json") : void 0;
	const modelsPath = options.agentDir ? (0, node_path.join)(agentDir, "models.json") : void 0;
	const authStorage = options.authStorage ?? AuthStorage.create(authPath);
	const modelRegistry = options.modelRegistry ?? ModelRegistry.create(authStorage, modelsPath);
	const settingsManager = options.settingsManager ?? SettingsManager.create(cwd, agentDir);
	const sessionManager = options.sessionManager ?? require_session_manager.SessionManager.create(cwd, require_session_manager.getDefaultSessionDir(cwd, agentDir));
	if (!resourceLoader) {
	...
	const modelThinkingDefault = (model && modelThinkingProvider ? require_thinking.resolveThinkingDefaultForModel({
	...
	else thinkingLevel = (0, _gabrielvfonseca_ai_internal_runtime.clampThinkingLevel)(model, thinkingLevel);
	...
	const convertToLlmWithBlockImages = (messages) => {
		const converted = require_session_manager.convertToLlm(messages);
	...
	const agent = new require_session_manager.Agent({
		initialState: {
			systemPrompt: "",
			model,
			thinkingLevel,
		...
		streamFn: async (modelResult, context, optionsLocal) => {
			...
			return (0, _gabrielvfonseca_ai_internal_runtime.streamSimple)(modelResult, context, {
				...optionsLocal,
				apiKey: auth.apiKey,
...
Object.defineProperty(exports, "ModelRegistry", {
	enumerable: true,
	get: function() {
		return ModelRegistry;
	}
});
Object.defineProperty(exports, "SESSION_TOOL_STDERR_TAIL_BYTES", {
	enumerable: true,
	get: function() {
		return SESSION_TOOL_STDERR_TAIL_BYTES;
	}
});
Object.defineProperty(exports, "SettingsManager", {
	enumerable: true,
	get: function() {
		return SettingsManager;
	}
});
Object.defineProperty(exports, "appendBoundedTextTail", {
	enumerable: true,
	get: function() {
		return appendBoundedTextTail;
	}
});
Object.defineProperty(exports, "attachRuntimeUserTurnTranscriptRecorder", {
	enumerable: true,
	get: function() {
		return attachRuntimeUserTurnTranscriptRecorder;
	}
});
Object.defineProperty(exports, "classifyRateLimitWindow", {
	enumerable: true,
	get: function() {
		return classifyRateLimitWindow;
	}
});
Object.defineProperty(exports, "createAgentSession", {
	enumerable: true,
	get: function() {
		return createAgentSession;
	}
});
Object.defineProperty(exports, "createCodingTools", {
	enumerable: true,
	get: function() {
		return createCodingTools;
	}
});
Object.defineProperty(exports, "createEditTool", {
	enumerable: true,
	get: function() {
		return createEditTool;
	}
});
Object.defineProperty(exports, "createReadTool", {
	enumerable: true,
	get: function() {
		return createReadTool;
	}
});
Object.defineProperty(exports, "createWriteTool", {
	enumerable: true,
	get: function() {
...
Object.defineProperty(exports, "formatFullOutputFooter", {
	enumerable: true,
	get: function() {
		return formatFullOutputFooter;
	}
});
...
Object.defineProperty(exports, "takeRuntimeUserTurnTranscriptContext", {
	enumerable: true,
	get: function() {
		return takeRuntimeUserTurnTranscriptContext;
	}
});
Object.defineProperty(exports, "takeRuntimeUserTurnTranscriptRecorder", {
	enumerable: true,
	get: function() {
		return takeRuntimeUserTurnTranscriptRecorder;
	}
});
Object.defineProperty(exports, "wrapToolDefinition", {
	enumerable: true,
	get: function() {
		return wrapToolDefinition;
	}
});
Object.defineProperty(exports, "writePrivateTempFile", {
	enumerable: true,
	get: function() {
		return writePrivateTempFile;
	}
});
