...
var channel_summary_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({ buildChannelSummary: () => buildChannelSummary });
...
async function listChannelSummaryPlugins(params) {
	const { listReadOnlyChannelPluginsForConfig } = await Promise.resolve().then(() => require("./read-only-MDrE_ZGP.cjs")).then((n) => n.read_only_exports);
...
const buildAccountDetails = (params) => {
	...
	if (require_account_snapshot_fields.hasConfiguredUnavailableCredentialStatus(params.entry.account)) details.push("secret unavailable in this command path");
...
async function buildChannelSummary(cfg, options) {
	...
	for (const plugin of plugins) {
		...
		for (const accountId of resolvedAccountIds) {
			...
			const snapshot = require_account_inspection.buildChannelAccountSnapshot({
				plugin,
				account,
				cfg: effective,
				accountId,
		...
		const status = !anyEnabled ? "disabled" : statusState ? require_status_state.formatChannelStatusState(statusState) : linked !== null ? linked ? "linked" : "not linked" : configured ? "configured" : "not configured";
		const statusColor = status === "linked" || status === "configured" ? require_theme.theme.success : status === "not linked" || status === "auth stabilizing" ? require_theme.theme.error : require_theme.theme.muted;
		let line = `${require_ansi.sanitizeForLog(plugin.meta.label ?? plugin.id).trim() || plugin.id}: ${status}`;
		...
		if (authAgeMs != null && authAgeMs >= 0) line += ` auth ${require_format_relative.formatTimeAgo(authAgeMs)}`;
...
//#endregion
Object.defineProperty(exports, "buildChannelSummary", {
	enumerable: true,
	get: function() {
		return buildChannelSummary;
	}
});
Object.defineProperty(exports, "channel_summary_exports", {
	enumerable: true,
	get: function() {
		return channel_summary_exports;
	}
});
