...
*/
var context_window_guard_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	CONTEXT_WINDOW_HARD_MIN_TOKENS: () => CONTEXT_WINDOW_HARD_MIN_TOKENS,
	evaluateContextWindowGuard: () => evaluateContextWindowGuard,
	formatContextWindowBlockMessage: () => formatContextWindowBlockMessage,
	formatContextWindowWarningMessage: () => formatContextWindowWarningMessage,
...
function resolveContextWindowInfo(params) {
	const fromModelsConfig = (() => {
		...
		const providerEntry = (0, _gabrielvfonseca_model_catalog_core_provider_id.findNormalizedProviderValue)(providers, params.provider);
...
function resolveContextWindowGuardHint(params) {
	const endpoint = require_provider_attribution.resolveProviderEndpoint(params.runtimeBaseUrl ?? void 0);
	return {
...
function formatContextWindowWarningMessage(params) {
	...
	if (params.guard.source === "agentContextTokens") return `${base}; Operator is capped by agents.defaults.contextTokens, so raise that cap if you want to use more of the model context window`;
	if (params.guard.source === "modelsConfig") return `${base}; Operator is using the configured model context limit for this model, so raise contextWindow/contextTokens if it is set too low`;
...
function formatContextWindowBlockMessage(params) {
	...
	if (!resolveContextWindowGuardHint({ runtimeBaseUrl: params.runtimeBaseUrl }).likelySelfHosted) return base;
	if (params.guard.source === "agentContextTokens") return `${base} Operator is capped by agents.defaults.contextTokens. Raise that cap.`;
	if (params.guard.source === "modelsConfig") return `${base} Operator is using the configured model context limit for this model. Raise contextWindow/contextTokens or choose a larger model.`;
	return `${base} This looks like a local model endpoint. Raise the server/model context limit or choose a larger model. Operator local/self-hosted runs work best at ${params.guard.warnBelowTokens}+ tokens.`;
}
...
//#endregion
Object.defineProperty(exports, "CONTEXT_WINDOW_HARD_MIN_TOKENS", {
	enumerable: true,
	get: function() {
		return CONTEXT_WINDOW_HARD_MIN_TOKENS;
	}
});
Object.defineProperty(exports, "context_window_guard_exports", {
	enumerable: true,
	get: function() {
		return context_window_guard_exports;
	}
});
Object.defineProperty(exports, "evaluateContextWindowGuard", {
	enumerable: true,
	get: function() {
		return evaluateContextWindowGuard;
	}
});
Object.defineProperty(exports, "formatContextWindowBlockMessage", {
	enumerable: true,
	get: function() {
		return formatContextWindowBlockMessage;
	}
});
Object.defineProperty(exports, "formatContextWindowWarningMessage", {
	enumerable: true,
	get: function() {
		return formatContextWindowWarningMessage;
	}
});
Object.defineProperty(exports, "resolveContextWindowInfo", {
	enumerable: true,
	get: function() {
