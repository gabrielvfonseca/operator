...
function normalizePluginIdMaybe(value) {
	return typeof value === "string" && value.trim() ? require_config_state.normalizePluginId(value) : void 0;
}
...
function collectToolPolicySources(policy, label, out) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(policy)) return;
	collectListSource({
		out,
		value: policy.allow,
		label: `${label}.allow`
	...
	if ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(policy.byProvider)) for (const [providerId, providerPolicy] of Object.entries(policy.byProvider)) collectToolPolicySources(providerPolicy, `${label}.byProvider.${providerId}`, out);
	collectToolPolicySources((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(policy.sandbox) ? policy.sandbox.tools : void 0, `${label}.sandbox.tools`, out);
	collectToolPolicySources((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(policy.subagents) ? policy.subagents.tools : void 0, `${label}.subagents.tools`, out);
}
function collectToolAllowlistSources(cfg) {
	...
	if (Array.isArray(agentList)) agentList.forEach((agent, index) => {
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(agent)) return;
		collectToolPolicySources(agent.tools, `agents.list[${index}].tools`, sources);
	});
	return sources;
}
function collectSortedSourceLabels(labels) {
	return (0, _gabrielvfonseca_normalization_core_string_normalization.sortUniqueStrings)(labels);
}
...
function collectToolOwners(registry) {
	...
	for (const plugin of registry.plugins) {
		const pluginId = require_config_state.normalizePluginId(plugin.id);
		for (const toolNameRaw of plugin.contracts?.tools ?? []) {
			const toolName = require_tool_policy.normalizeToolName(toolNameRaw);
...
function collectKnownPluginIds(registry) {
	return new Set(registry.plugins.map((plugin) => require_config_state.normalizePluginId(plugin.id)));
}
function collectConfiguredMcpServerNames(cfg) {
	const servers = cfg.mcp?.servers;
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(servers)) return [];
	return Object.entries(servers).filter(([, value]) => (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(value)).map(([name]) => name.trim()).filter(Boolean).toSorted((left, right) => left.localeCompare(right));
}
...
function pickSandboxToolPolicyField(params) {
	const agentValue = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(params.agentPolicy) ? params.agentPolicy[params.key] : void 0;
	if (Array.isArray(agentValue)) return {
		value: agentValue,
		label: `${params.agentLabel}.${params.key}`,
		defined: true
	};
	const globalValue = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(params.globalPolicy) ? params.globalPolicy[params.key] : void 0;
...
function buildEffectiveSandboxToolPolicy(params) {
	...
	return {
		labels,
		dedupeKey: (0, _gabrielvfonseca_normalization_core_string_normalization.uniqueStrings)([...labels, deny.label].filter((label) => Boolean(label))).join("\0"),
		policy,
...
function collectActiveSandboxToolPolicies(cfg, serverNames) {
	...
	if (Array.isArray(agentList)) agentList.forEach((agent, index) => {
		if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(agent)) return;
		const explicitMode = ((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(agent.sandbox) ? agent.sandbox : void 0)?.mode;
		...
		const agentToolsSandbox = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(agentTools?.sandbox) ? agentTools.sandbox : void 0;
		const agentPolicy = (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(agentToolsSandbox?.tools) ? agentToolsSandbox.tools : void 0;
		addPolicy(buildEffectiveSandboxToolPolicy({
			agentPolicy,
...
function buildMcpProbeToolNames(serverNames) {
	...
	return serverNames.map((serverName) => `${require_agent_bundle_mcp_names.sanitizeServerName(serverName, usedNames)}__probe`);
}
function buildMcpToolNamePrefixes(serverNames) {
	const usedNames = /* @__PURE__ */ new Set();
	return serverNames.map((serverName) => require_tool_policy.normalizeToolName(`${require_agent_bundle_mcp_names.sanitizeServerName(serverName, usedNames)}__`)).filter(Boolean);
}
function entriesMatchMcpTool(entries, serverNames, mode) {
	const normalizedEntries = entries.map(require_tool_policy.normalizeToolName).filter(Boolean);
	...
	const patterns = require_glob_pattern.compileGlobPatterns({
		raw: normalizedEntries,
		normalize: require_tool_policy.normalizeToolName
	});
	const probeNames = buildMcpProbeToolNames(serverNames).map(require_tool_policy.normalizeToolName);
	const prefixOrPatternMatches = (prefix, index) => normalizedEntries.some((entry) => entry.length > prefix.length && entry.startsWith(prefix)) || require_glob_pattern.matchesAnyGlobPattern(probeNames[index] ?? "", patterns);
...
function profileToolPolicyBlocksMcp(policy, serverNames) {
	const profilePolicy = require_tool_policy.mergeAlsoAllowPolicy(require_tool_policy.resolveToolProfilePolicy((0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(policy) && typeof policy.profile === "string" ? policy.profile : ""), getList(policy, "alsoAllow"));
...
function nonSandboxToolPoliciesBlockMcp(params) {
	...
	const modelRef = require_primary_model_ref.resolveDoctorPrimaryModelRef(params.cfg, params.agent?.model);
	...
	const agentProviderPolicy = require_provider_tool_policy.resolveProviderToolPolicy({
		byProvider: (0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(agentTools?.byProvider) ? agentTools.byProvider : void 0,
		modelProvider: modelRef.provider,
		modelId: modelRef.model
	});
	const profilePolicy = {
...
function collectPluginToolAllowlistWarnings(params) {
	...
	const wildcardSources = sources.filter((source) => source.entries.some((entry) => require_tool_policy.normalizeToolName(entry) === "*")).map((source) => source.label);
	...
	const exactEntries = sources.flatMap((source) => source.entries.map((entry) => ({
		source: source.label,
		entry: require_tool_policy.normalizeToolName(entry)
	})).filter(({ entry }) => entry && entry !== "*" && entry !== "group:plugins"));
	if (exactEntries.length === 0) return warnings;
	const registry = params.manifestRegistry ?? require_manifest_contract_eligibility.loadManifestMetadataSnapshot({
		config: params.cfg,
	...
	for (const { source, entry } of exactEntries) {
		const pluginId = require_config_state.normalizePluginId(entry);
...
//#endregion
exports.collectPluginToolAllowlistWarnings = collectPluginToolAllowlistWarnings;
