...
function isDefaultOperatorStreamFnForModel(model, streamFn) {
	if (!streamFn || streamFn === _gabrielvfonseca_ai_internal_runtime.streamSimple) return true;
	const api = typeof model.api === "string" ? model.api.trim() : "";
	if (!api) return false;
	const provider = (0, _gabrielvfonseca_ai_internal_runtime.getApiProvider)(api);
...
function resolveOperatorNativeCodexResponsesStreamFn(params) {
	...
	if (!isDefaultOperatorStreamFnForModel(params.model, params.currentStreamFn)) return;
	return params.currentStreamFn ?? _gabrielvfonseca_ai_internal_runtime.streamSimple;
}
function describeEmbeddedAgentStreamStrategy(params) {
	...
	if (resolveOperatorNativeCodexResponsesStreamFn({
		model: params.model,
		currentStreamFn: params.currentStreamFn
	})) return "operator-native-codex-responses";
	if (isDefaultOperatorStreamFnForModel(params.model, params.currentStreamFn)) return require_provider_stream.createBoundaryAwareStreamFnForModel(params.model) ? `boundary-aware:${params.model.api}` : "stream-simple";
	if (hasResolvedRuntimeApiKey(params.resolvedApiKey) && require_provider_stream.createBoundaryAwareStreamFnForModel(params.model)) return `boundary-aware:${params.model.api}`;
...
function resolveEmbeddedAgentStreamFn(params) {
	...
	const currentStreamFn = params.currentStreamFn ?? _gabrielvfonseca_ai_internal_runtime.streamSimple;
	if (params.model.provider === "anthropic-vertex") return require_anthropic_vertex_stream.createAnthropicVertexStreamFnForModel(params.model);
	...
	if (isDefaultOperatorStreamFnForModel(params.model, params.currentStreamFn) || hasResolvedRuntimeApiKey(params.resolvedApiKey) || params.transportAuthAvailable || params.model.api === "anthropic-messages" && params.model.provider !== "anthropic") {
		const boundaryAwareStreamFn = require_provider_stream.createBoundaryAwareStreamFnForModel(params.model);
...
function responseStreamChunkByteLengthUnchecked(chunk) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(chunk)) return utf8JsonByteLength(chunk);
...
function streamContextModelContentFields(policy, streamContext) {
	if (!policy?.anyModelContent || !(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(streamContext)) return;
	const content = {
		...policy.inputMessages && Array.isArray(streamContext.messages) ? { inputMessages: require_diagnostic_llm_content.cloneDiagnosticContentValue(streamContext.messages) } : {},
		...policy.systemPrompt && typeof streamContext.systemPrompt === "string" ? { systemPrompt: streamContext.systemPrompt } : {},
		...policy.toolDefinitions && Array.isArray(streamContext.tools) ? { toolDefinitions: require_diagnostic_llm_content.cloneDiagnosticContentValue(streamContext.tools) } : {}
	};
	return Object.keys(content).length > 0 ? content : void 0;
}
function streamContextModelPromptStats(streamContext) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(streamContext)) return;
...
function normalizedModelCallUsage(rawUsage) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(rawUsage)) return;
	const usage = require_session_accessor.normalizeUsage(rawUsage);
	if (!usage) return;
	const promptTokens = require_session_accessor.derivePromptTokens(usage);
	return {
		...usage,
...
function observeOutputMessageContent(state, chunk) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(chunk)) return;
	let type;
	let message;
	try {
		type = chunk.type;
	...
	if (message !== void 0) {
		...
		if (state.contentCapture?.outputMessages) state.outputMessages = [require_diagnostic_llm_content.cloneDiagnosticContentValue(message)];
	}
}
function observeResultMessageContent(state, startedAt, result) {
	state.timeToFirstByteMs ??= Math.max(0, Date.now() - startedAt);
	observeModelCallUsage(state, result);
	if (state.contentCapture?.outputMessages && state.outputMessages === void 0) state.outputMessages = [require_diagnostic_llm_content.cloneDiagnosticContentValue(result)];
...
function maybeEmitModelCallStreamProgress(eventBase, state) {
	...
	require_diagnostic_run_activity.markDiagnosticRunProgress(progressFields);
...
function boundedTimelineAttribute(value) {
	return (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(value?.trim() ?? "", TIMELINE_ATTRIBUTE_MAX_LENGTH) || void 0;
}
function emitProviderRequestTimelineEvent(eventBase, startedAt, durationMs, ok) {
	...
	require_plugin_metadata_snapshot.emitDiagnosticsTimelineEvent({
...
function modelCallErrorFields(err) {
	const upstreamRequestIdHash = require_diagnostic_error_metadata.diagnosticProviderRequestIdHash(err);
	const failureKind = require_diagnostic_error_metadata.diagnosticErrorFailureKind(err);
	return {
		errorCategory: require_diagnostic_error_metadata.diagnosticErrorCategory(err),
		...failureKind ? {
			failureKind,
...
function dispatchModelCallStartedHook(eventBase) {
	...
	require_hook_runner_global.fireAndForgetBoundedHook(() => hookRunner.runModelCallStarted(event, hookCtx), "model_call_started plugin hook failed");
}
function dispatchModelCallEndedHook(eventBase, fields) {
	...
	const hookCtx = modelCallHookContext(eventBase);
	require_hook_runner_global.fireAndForgetBoundedHook(() => hookRunner.runModelCallEnded(event, hookCtx), "model_call_ended plugin hook failed");
}
...
function withDiagnosticRequestContext(options, trace, state, callId) {
	const traceparent = require_diagnostic_events.formatDiagnosticTraceparent(trace);
...
function createObservedResultFunction(stream, eventBase, startedAt, state) {
	if (!(0, _gabrielvfonseca_normalization_core_record_coerce.isRecord)(stream) || typeof stream.result !== "function") return;
...
function wrapStreamFnWithDiagnosticModelCallEvents(streamFn, ctx) {
	return ((model, streamContext, options) => {
		...
		const trace = require_diagnostic_events.freezeDiagnosticTraceContext(require_diagnostic_events.createChildDiagnosticTraceContext(ctx.trace));
		const eventBase = baseModelCallEvent(ctx, callId, trace, require_diagnostic_events.areDiagnosticsEnabledForProcess() ? streamContextModelPromptStats(streamContext) : void 0);
...
//#endregion
Object.defineProperty(exports, "describeEmbeddedAgentStreamStrategy", {
	enumerable: true,
	get: function() {
		return describeEmbeddedAgentStreamStrategy;
	}
});
Object.defineProperty(exports, "resolveEmbeddedAgentApiKey", {
	enumerable: true,
	get: function() {
		return resolveEmbeddedAgentApiKey;
	}
});
Object.defineProperty(exports, "resolveEmbeddedAgentBaseStreamFn", {
	enumerable: true,
	get: function() {
		return resolveEmbeddedAgentBaseStreamFn;
	}
});
Object.defineProperty(exports, "resolveEmbeddedAgentStreamFn", {
	enumerable: true,
	get: function() {
		return resolveEmbeddedAgentStreamFn;
	}
});
Object.defineProperty(exports, "wrapStreamFnWithDiagnosticModelCallEvents", {
	enumerable: true,
	get: function() {
		return wrapStreamFnWithDiagnosticModelCallEvents;
	}
});
