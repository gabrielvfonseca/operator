...
function normalizeArgValue(value) {
	...
	let text;
	if (typeof value === "string") text = require_string_coerce.normalizeOptionalString(value) ?? "";
	else if (typeof value === "number" || typeof value === "boolean" || typeof value === "bigint") text = require_string_coerce.normalizeOptionalString(String(value)) ?? "";
	else if (typeof value === "symbol") text = require_string_coerce.normalizeOptionalString(value.toString()) ?? "";
	else if (typeof value === "function") text = require_string_coerce.normalizeOptionalString(value.toString()) ?? "";
	else text = JSON.stringify(value);
	return text ? text : void 0;
}
function formatActionArgs(values, params) {
	const action = require_string_coerce.normalizeOptionalLowercaseString(normalizeArgValue(values.action));
...
const BROWSER_SAFE_THINKING_LEVELS = [
	...require_thinking.BASE_THINKING_LEVELS,
	"xhigh",
	"adaptive",
	"max"
];
...
function defineChatCommand(command) {
	...
	return {
		...
		nativeAliases: command.nativeAliases ? require_string_normalization.normalizeStringEntries(command.nativeAliases) : void 0,
		nativeProviders: command.nativeProviders ? require_string_normalization.normalizeStringEntries(command.nativeProviders) : void 0,
...
function registerAlias(commands, key, ...aliases) {
	...
	for (const alias of command.textAliases) {
		const lowered = require_string_coerce.normalizeOptionalLowercaseString(alias);
		if (lowered) existing.add(lowered);
	}
	for (const alias of aliases) {
		const trimmed = alias.trim();
		if (!trimmed) continue;
		const lowered = require_string_coerce.normalizeOptionalLowercaseString(trimmed);
		if (!lowered) continue;
...
function assertCommandRegistry(commands) {
	...
	for (const command of commands) {
		...
		if (command.scope === "text") {
			...
			const nativeKey = require_string_coerce.normalizeOptionalLowercaseString(alias) ?? "";
		...
		for (const alias of command.textAliases) {
			...
			const aliasKey = require_string_coerce.normalizeOptionalLowercaseString(alias) ?? "";
		...
		defineChatCommand({
			...
			args: [{
				...
				choices: ({ cfg, provider, model }) => [
					...
					{
						value: "auto",
						label: require_fast_mode.formatFastModeAutoLabel({ fastAutoOnSeconds: require_fast_mode.resolveFastModeModelAutoOnSeconds({
							cfg,
							provider,
							model
						}) })
...
function buildChatCommands() {
	const commands = [...buildBuiltinChatCommands({ listThinkingLevels: require_thinking.listThinkingLevels }), ...require_registry_loaded.listLoadedChannelPlugins().filter(supportsNativeCommands).map((plugin) => defineDockCommand(plugin))];
	assertCommandRegistry(commands);
	return commands;
}
/** Returns the current command registry, including dynamic dock commands for native surfaces. */
function getChatCommands() {
	const registryVersion = require_runtime_channel_state.getActivePluginChannelRegistryVersionFromState();
...
//#endregion
Object.defineProperty(exports, "getChatCommands", {
	enumerable: true,
	get: function() {
		return getChatCommands;
	}
});
Object.defineProperty(exports, "shouldForwardModelCommandToServer", {
	enumerable: true,
	get: function() {
		return shouldForwardModelCommandToServer;
	}
});
