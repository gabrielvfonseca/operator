...
var web_media_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	LocalMediaAccessError: () => require_local_media_access.LocalMediaAccessError,
	effectiveImageBytesCap: () => effectiveImageBytesCap,
	getDefaultLocalRoots: () => require_local_media_access.getDefaultLocalRoots,
	loadWebMedia: () => loadWebMedia,
	loadWebMediaRaw: () => loadWebMediaRaw,
	optimizeImageBufferForWebMedia: () => optimizeImageBufferForWebMedia,
	optimizeImageToJpeg: () => optimizeImageToJpeg,
	optimizeImageToPng: () => require_media_services.optimizeImageToPng,
	resolveImageCompressionGrid: () => resolveImageCompressionGrid
});
async function resolveMediaStoreUriToPath(mediaUrl) {
	if (!/^media:\/\//i.test(mediaUrl)) return null;
	try {
		return (await require_media_reference.resolveInboundMediaReference(mediaUrl))?.physicalPath ?? null;
	} catch (err) {
		if (err instanceof require_media_reference.MediaReferenceError) throw new require_local_media_access.LocalMediaAccessError(err.code, err.message, { cause: err });
		throw err;
	}
}
async function resolveHostedPluginMediaUrl(mediaUrl) {
	...
	for (const entry of registry?.hostedMediaResolvers ?? []) try {
		...
		if (require_globals.shouldLogVerbose()) require_globals.logVerbose(`Hosted media resolver failed (${entry.pluginId ?? "unknown"}): ${require_errors.formatErrorMessage(err)}`);
	}
	return null;
}
...
function isPathInsideRoot(filePath, root) {
	...
	const relative = node_path.default.relative(node_path.default.resolve(root), node_path.default.resolve(filePath));
...
function resolveLocalMediaFileName(filePath) {
	const fileName = (0, _gabrielvfonseca_media_core_file_name.basenameFromAnyPath)(filePath) || void 0;
	return fileName && isPathInsideRoot(filePath, require_store.getMediaDir()) ? require_store.extractOriginalFilename(fileName) : fileName;
}
...
async function isTrustedGeneratedHostReadHtmlPath(filePath) {
	if (!filePath) return false;
	const info = await (0, node_fs_promises.lstat)(filePath).catch(() => void 0);
	if (!info?.isFile() || info.isSymbolicLink() || info.nlink !== 1) return false;
	const [resolvedFilePath, resolvedTmpRoot] = await Promise.all([(0, node_fs_promises.realpath)(filePath).catch(() => void 0), (0, node_fs_promises.realpath)(require_tmp_operator_dir.resolvePreferredOperatorTmpDir()).catch(() => void 0)]);
...
function isTrustedGeneratedHostReadHtml(params) {
	const sniffedMime = (0, _gabrielvfonseca_media_core_mime.normalizeMimeType)(params.sniffedContentType);
...
function isAllowedHostReadTextAlias(mime, filePath) {
	...
	const ext = (0, _gabrielvfonseca_media_core_mime.getFileExtension)(filePath);
...
function assertHostReadMediaAllowed(params) {
	const declaredMime = (0, _gabrielvfonseca_media_core_mime.normalizeMimeType)((0, _gabrielvfonseca_media_core_mime.mimeTypeFromFilePath)(params.filePath));
	const normalizedMime = (0, _gabrielvfonseca_media_core_mime.normalizeMimeType)(params.contentType);
	if (declaredMime && HOST_READ_DECLARED_TEXT_MIMES.has(declaredMime)) {
		...
		throw new require_local_media_access.LocalMediaAccessError("path-not-allowed", HOST_READ_DECLARED_TEXT_ERROR);
	}
	const sniffedKind = (0, _gabrielvfonseca_media_core_mime.kindFromMime)(params.sniffedContentType);
	...
	if (sniffedMime === "application/x-cfb" && [
		...
		".ppt",
		".xls"
	].includes((0, _gabrielvfonseca_media_core_mime.getFileExtension)(params.filePath) ?? "")) return;
	...
	if (params.kind === "document" && normalizedMime && HOST_READ_ALLOWED_DOCUMENT_MIMES.has(normalizedMime)) throw new require_local_media_access.LocalMediaAccessError("path-not-allowed", `Host-local media sends require buffer-verified media/document types (got fallback ${normalizedMime}).`);
	throw new require_local_media_access.LocalMediaAccessError("path-not-allowed", `Host-local media sends only allow buffer-verified images, audio, video, PDF, Office documents, archives, and validated plain-text documents (got ${sniffedMime ?? normalizedMime ?? "unknown"}).`);
}
function toJpegFileName(fileName) {
	if (!fileName) return;
	const trimmed = (0, _gabrielvfonseca_media_core_file_name.basenameFromAnyPath)(fileName.trim());
	if (!trimmed) return fileName;
	const parsed = node_path.default.parse(trimmed);
	if (!parsed.ext || HEIC_EXT_RE.test(parsed.ext)) return node_path.default.format({
		dir: parsed.dir,
		name: parsed.name || trimmed,
		ext: ".jpg"
	});
	return node_path.default.format({
		dir: parsed.dir,
		name: parsed.name,
		ext: ".jpg"
	});
...
function imageSatisfiesHardDimensionPolicy(buffer, policy) {
	...
	const meta = require_media_services.readImageMetadataFromHeader(buffer);
...
function resolvePreservableOriginalImageContentType(params) {
	...
	const declaredContentType = (0, _gabrielvfonseca_media_core_mime.normalizeMimeType)(params.contentType);
	...
	const meta = require_media_services.readImageMetadataFromHeader(params.buffer);
	if (!meta) return null;
...
function detectPreservableImageMime(buffer) {
	const format = require_media_services.readImageProbeFromHeader(buffer)?.format;
...
function buildDescendingLadder(maxSide, values) {
	...
	const ladder = (0, _gabrielvfonseca_normalization_core_string_normalization.uniqueValues)([
		normalizedMax,
		...values,
	...
	return (0, _gabrielvfonseca_normalization_core_string_normalization.uniqueValues)([
		normalizedMax,
...
function logOptimizedImage(params) {
	if (!require_globals.shouldLogVerbose()) return;
	if (params.optimized.optimizedSize >= params.originalSize) return;
	if (params.optimized.format === "png") {
		require_globals.logVerbose(`Optimized PNG (preserving alpha) from ${formatMb(params.originalSize)}MB to ${formatMb(params.optimized.optimizedSize)}MB (side<=${params.optimized.resizeSide}px)`);
		return;
	}
	require_globals.logVerbose(`Optimized media from ${formatMb(params.originalSize)}MB to ${formatMb(params.optimized.optimizedSize)}MB (side<=${params.optimized.resizeSide}px, q=${params.optimized.quality})`);
}
async function optimizeImageWithFallback(params) {
	const { buffer, cap } = params;
	const grid = resolveImageCompressionGrid(params.imageCompression);
	const optimized = await require_media_services.createImageProcessor().encode(buffer, {
		format: "auto",
		maxBytes: cap,
		opaque: { format: "jpeg" },
	...
	if (optimized.chosen.transparency === "flattened" && require_globals.shouldLogVerbose()) require_globals.logVerbose(`Image transparency flattened to fit ${formatMb(cap, 0)}MB optimization budget`);
	return {
...
async function optimizeImageBufferForWebMedia(params) {
	const baseCap = params.maxBytes ?? (0, _gabrielvfonseca_media_core_constants.maxBytesForKind)("image");
...
async function loadWebMediaInternal(mediaUrlInput, options = {}) {
	...
	if (mediaUrl.startsWith("file://")) try {
		mediaUrl = (0, _openclaw_fs_safe_advanced.safeFileURLToPath)(mediaUrl);
	} catch (err) {
		throw new require_local_media_access.LocalMediaAccessError("invalid-file-url", err.message, { cause: err });
	}
	...
	const clampAndFinalize = async (params) => {
		const cap = maxBytes !== void 0 ? maxBytes : (0, _gabrielvfonseca_media_core_constants.maxBytesForKind)(params.kind ?? "document");
	...
	if ((0, _gabrielvfonseca_net_policy_url_protocol.hasHttpUrlPrefix)(mediaUrl)) {
		const defaultFetchCap = (0, _gabrielvfonseca_media_core_constants.maxBytesForKind)("document");
		const { buffer, contentType, fileName } = await require_fetch.readRemoteMediaBuffer({
			url: mediaUrl,
			fetchImpl,
			requestInit,
			readIdleTimeoutMs,
		...
		return await clampAndFinalize({
			buffer,
			contentType,
			kind: (0, _gabrielvfonseca_media_core_mime.kindFromMime)(contentType),
			fileName
		});
	}
	if (mediaUrl.startsWith("~")) mediaUrl = require_home_dir.resolveUserPath(mediaUrl);
	if (workspaceDir && !node_path.default.isAbsolute(mediaUrl) && !WINDOWS_DRIVE_RE.test(mediaUrl)) mediaUrl = node_path.default.resolve(workspaceDir, mediaUrl);
	try {
		(0, _openclaw_fs_safe_advanced.assertNoWindowsNetworkPath)(mediaUrl, "Local media path");
	} catch (err) {
		throw new require_local_media_access.LocalMediaAccessError("network-path-not-allowed", err.message, { cause: err });
	}
	if ((sandboxValidated || localRoots === "any") && !readFileOverride) throw new require_local_media_access.LocalMediaAccessError("unsafe-bypass", "Refusing localRoots bypass without readFile override. Use sandboxValidated with readFile, or pass explicit localRoots.");
	if (!(sandboxValidated || localRoots === "any")) await require_local_media_access.assertLocalMediaAllowed(mediaUrl, localRoots, { inboundRoots });
	const hostReadDeclaredMime = hostReadCapability ? (0, _gabrielvfonseca_media_core_mime.normalizeMimeType)((0, _gabrielvfonseca_media_core_mime.mimeTypeFromFilePath)(mediaUrl)) : void 0;
	const trustedGeneratedHtmlPath = hostReadDeclaredMime === "text/html" ? await isTrustedGeneratedHostReadHtmlPath(mediaUrl) : false;
	if (hostReadDeclaredMime === "text/html" && !trustedGeneratedHtmlPath) throw new require_local_media_access.LocalMediaAccessError("path-not-allowed", HOST_READ_DECLARED_TEXT_ERROR);
	let data;
	if (readFileOverride) data = await readFileOverride(mediaUrl);
	else try {
		data = (await (0, _openclaw_fs_safe_root.readLocalFileSafely)({ filePath: mediaUrl })).buffer;
	} catch (err) {
		if (err instanceof _openclaw_fs_safe_errors.FsSafeError) {
			if (err.code === "not-found") throw new require_local_media_access.LocalMediaAccessError("not-found", `Local media file not found: ${mediaUrl}`, { cause: err });
			if (err.code === "not-file") throw new require_local_media_access.LocalMediaAccessError("not-file", `Local media path is not a file: ${mediaUrl}`, { cause: err });
			throw new require_local_media_access.LocalMediaAccessError("invalid-path", `Local media path is not safe to read: ${mediaUrl}`, { cause: err });
		}
		throw err;
	}
	const sniffedMime = await (0, _gabrielvfonseca_media_core_mime.detectMime)({ buffer: data });
	...
	if (fileName && !(0, _gabrielvfonseca_media_core_file_name.extnameFromAnyPath)(fileName) && mime) {
		const ext = (0, _gabrielvfonseca_media_core_mime.extensionForMime)(mime);
...
Object.defineProperty(exports, "loadWebMediaRaw", {
	enumerable: true,
	get: function() {
		return loadWebMediaRaw;
	}
});
Object.defineProperty(exports, "web_media_exports", {
	enumerable: true,
	get: function() {
		return web_media_exports;
	}
});
