...
function preparedAgentRuntimeProfileAttemptHasCandidate(params) {
	...
	return (params.attempt.plan.forwardedAuthProfileCandidateIds ?? [params.attempt.profileId]).some((profileId) => !require_usage_state.isProfileInCooldown(params.store, profileId, void 0, params.modelId));
}
...
function resolveProfile(params, profileId, options = {}) {
	...
	const availability = credential ? require_read_only_availability.resolveStoredCredentialReadOnlyAvailability({
		credential,
		cfg: params.config ?? {},
	...
	return {
		...
		cooldown: !options.ignoreCooldown && params.authProfileStore && require_usage_state.isProfileInCooldown(params.authProfileStore, profileId, void 0, params.modelId) ? "active" : "clear"
	};
}
...
function resolvePreparedProviderEntryApiKeyProfileReference(params) {
	...
	if (!require_order.resolveAuthProfileEligibility({
		cfg: params.config,
		store: params.store,
		provider: params.provider,
		profileId: reference.profileId
	}).eligible) throw new Error(`Per-entry apiKey profile "${reference.profileId}" has no usable credentials for ${params.provider}.`);
	if (require_usage_state.isProfileInCooldown(params.store, reference.profileId, void 0, params.modelId)) throw new Error(`Auth profile "${reference.profileId}" is temporarily unavailable for ${params.provider}/${params.modelId}.`);
	return reference;
}
...
function prepareAgentRuntimeAuth(params) {
	...
	if (lockedProfileId) {
		if (!(store ? require_order.resolveAuthProfileEligibility({
			cfg: params.config,
			store,
	...
	const configuredProvider = require_openai_routing.resolveMergedModelProviderConfig(params.config, params.provider);
	...
	const providerHasApiKeySecretRef = harnessAllowsAuthProfileForwarding && Boolean(require_types_secrets.coerceSecretRef(configuredProvider?.apiKey, params.config?.secrets?.defaults));
	...
	const providerHasUsableMarker = providerBinding.kind === "marker" && require_model_auth.hasUsableCustomProviderApiKey(params.config, params.provider, params.env);
	...
	const explicitConfigApiKeyAuth = require_model_auth.shouldPreferExplicitConfigApiKeyAuth(params.config, params.provider);
	...
	} : require_order.resolveAuthProfileOrderWithMetadata({
		cfg: params.config,
		store,
		provider: authProfileSelectionProvider,
	...
	const directSource = (mode, evidence = providerHasUsableMarker ? "runtime" : "provider-config", availability) => require_openai_routing.buildProviderModelAuthDirectSource({
		mode,
		evidence,
		availability
	});
	const directPlanningCandidate = harnessAllowsAuthProfileForwarding ? require_model_auth_env.resolveProviderDirectAuthPlanningEvidence(authProfileSelectionProvider, params.env ?? process.env, {
		config: params.config,
		workspaceDir: params.workspaceDir
	}) : null;
	...
	const resolution = require_openai_routing.resolveOpenAIModelRoutes({
		provider: params.provider,
		modelId: params.modelId,
		api: params.modelApi,
	...
	if (!resolution || resolution.kind === "indeterminate") {
		const sourceDecision = require_openai_routing.selectProviderModelAuthSources({
			provider: authProfileSelectionProvider,
			plan: sourcePlan
		});
		...
		const buildGenericPlan = (attempt, candidateIndex) => {
			...
			return require_auth.buildAgentRuntimeAuthPlan({
				provider: params.provider,
				modelId: params.modelId,
	...
	if (routeAuthDecision.kind === "deferred") {
		const plan = require_auth.buildAgentRuntimeAuthPlan({
			provider: params.provider,
			modelId: params.modelId,
			config: params.config,
			workspaceDir: params.workspaceDir,
...
//#endregion
Object.defineProperty(exports, "agentRuntimeAuthPlanMatchesTarget", {
	enumerable: true,
	get: function() {
		return agentRuntimeAuthPlanMatchesTarget;
	}
});
Object.defineProperty(exports, "canRunPreparedAgentRuntimeAuthAttempt", {
	enumerable: true,
	get: function() {
		return canRunPreparedAgentRuntimeAuthAttempt;
	}
});
Object.defineProperty(exports, "prepareAgentRuntimeAuth", {
	enumerable: true,
	get: function() {
		return prepareAgentRuntimeAuth;
	}
});
Object.defineProperty(exports, "preparedAgentRuntimeProfileAttemptHasCandidate", {
	enumerable: true,
	get: function() {
		return preparedAgentRuntimeProfileAttemptHasCandidate;
	}
});
