...
async function gitOut(cwd, args, okCodes = [0]) {
	try {
		const result = await require_git.runGit(cwd, [
			"-c",
...
function chunkPath(chunk) {
	...
	if (newFile) return (0, _gabrielvfonseca_normalization_core.expectDefined)(newFile[1], "new file capture group 1");
	const oldFile = /^--- a\/(.+)$/m.exec(chunk);
	if (oldFile) return (0, _gabrielvfonseca_normalization_core.expectDefined)(oldFile[1], "old file capture group 1");
	const renameTo = /^rename to (.+)$/m.exec(chunk);
	if (renameTo) return (0, _gabrielvfonseca_normalization_core.expectDefined)(renameTo[1], "rename to capture group 1");
	const header = /^diff --git a\/.+ b\/(.+)$/m.exec(chunk);
	return header ? (0, _gabrielvfonseca_normalization_core.expectDefined)(header[1], "header capture group 1") : null;
}
...
async function isPatchableWorkingTreePath(realRoot, relPath) {
	const abs = node_path.default.resolve(realRoot, relPath);
	try {
		const info = await node_fs_promises.default.lstat(abs);
		if (!info.isFile() || info.nlink !== 1) return false;
		const resolved = await node_fs_promises.default.realpath(abs);
		return resolved === realRoot || resolved.startsWith(realRoot + node_path.default.sep);
	} catch {
		return false;
	}
}
...
async function loadSessionDiff(params) {
	...
	const cwd = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(entry.spawnedCwd) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(entry.spawnedWorkspaceDir) ?? (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(require_agent_scope_config.resolveAgentWorkspaceDir(cfg, agentId));
	if (!cwd) return empty("unknown_session");
	const root = (await gitOut(cwd, ["rev-parse", "--show-toplevel"]))?.trim();
	if (!root) return empty("not_git");
	const realRoot = await node_fs_promises.default.realpath(root).catch(() => root);
...
const sessionsDiffHandlers = { "sessions.diff": async ({ params, respond }) => {
	if (!require_validation.assertValidParams(params, require_src.validateSessionsDiffParams, "sessions.diff", respond)) return;
	respond(true, await loadSessionDiff(params));
} };
//#endregion
exports.loadSessionDiff = loadSessionDiff;
exports.parseNameStatusZ = parseNameStatusZ;
exports.parseNumstatZ = parseNumstatZ;
exports.sessionsDiffHandlers = sessionsDiffHandlers;
exports.splitPatchByFile = splitPatchByFile;
