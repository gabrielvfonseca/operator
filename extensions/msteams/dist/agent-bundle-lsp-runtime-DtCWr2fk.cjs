...
const defaultLspSpawnDependencies = {
	...
	sanitizeHostExecEnv: require_host_env_security.sanitizeHostExecEnv,
	resolveWindowsSpawnProgram: require_windows_spawn.resolveWindowsSpawnProgram,
	materializeWindowsSpawnProgram: require_windows_spawn.materializeWindowsSpawnProgram
};
...
function loadEmbeddedAgentLspConfig(params) {
	const bundleLsp = require_bundle_lsp.loadEnabledBundleLspConfig({
...
const defaultBundleLspRuntimeDependencies = {
	...
	killProcessTree: require_kill_tree.killProcessTree
};
//#endregion
//#region src/agents/agent-bundle-lsp-runtime.ts
var agent_bundle_lsp_runtime_exports = /* @__PURE__ */ require_rolldown_runtime.__exportAll({
	createBundleLspToolRuntime: () => createBundleLspToolRuntime,
	disposeAllBundleLspRuntimes: () => disposeAllBundleLspRuntimes
});
...
function attachLspProcessHandlers(session) {
	...
	session.process.stderr?.on("data", (chunk) => {
		for (const line of chunk.split(/\r?\n/).filter(Boolean)) require_logger.logDebug(`bundle-lsp:${session.serverName}: ${line.trim()}`);
	});
	session.process.stderr?.on("error", (error) => {
		require_logger.logWarn(`bundle-lsp:${session.serverName}: stderr failed: ${String(error)}`);
	});
}
...
function lspAbortError(signal) {
	return signal?.reason instanceof Error ? signal.reason : require_abort_signal.createAbortError("LSP request aborted", { cause: signal?.reason });
}
...
function handleIncomingData(session, chunk) {
	...
	for (const msg of parsed.messages) {
		...
		if ("method" in record && !("id" in record)) require_logger.logDebug(`bundle-lsp:${session.serverName}: notification ${String(record.method)}`);
	}
	if (!parsed.ok) {
...
async function createBundleLspToolRuntime(params) {
	...
	for (const diagnostic of loaded.diagnostics) require_logger.logWarn(`bundle-lsp: ${diagnostic.pluginId}: ${diagnostic.message}`);
	if (Object.keys(loaded.lspServers).length === 0) return {
		tools: [],
		sessions: [],
		dispose: async () => {}
	};
	const reservedNames = new Set(Array.from(params.reservedToolNames ?? [], (name) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(name)).filter(Boolean));
	const sessions = [];
	const tools = [];
	try {
		for (const [serverName, rawServer] of Object.entries(loaded.lspServers)) {
			const launch = require_mcp_stdio.resolveStdioMcpServerLaunchConfig(rawServer);
			if (!launch.ok) {
				require_logger.logWarn(`bundle-lsp: skipped server "${serverName}" because ${launch.reason}.`);
				continue;
			}
			...
			try {
				...
				for (const tool of serverTools) {
					const normalizedName = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(tool.name);
					if (!normalizedName) continue;
					if (reservedNames.has(normalizedName)) {
						require_logger.logWarn(`bundle-lsp: skipped tool "${tool.name}" from server "${serverName}" because the name already exists.`);
						continue;
					}
					reservedNames.add(normalizedName);
					require_tools.setPluginToolMeta(tool, {
						pluginId: "bundle-lsp",
						optional: false
					});
					tools.push(tool);
				}
				require_logger.logDebug(`bundle-lsp: started "${serverName}" (${require_mcp_stdio.describeStdioMcpServerLaunchConfig(launchConfig)}) with ${serverTools.length} tools`);
			} catch (error) {
				if (session) await disposeSession(session);
				require_logger.logWarn(`bundle-lsp: failed to start server "${serverName}" (${require_mcp_stdio.describeStdioMcpServerLaunchConfig(launchConfig)}): ${String(error)}`);
			}
		}
		return {
			tools,
...
//#endregion
Object.defineProperty(exports, "agent_bundle_lsp_runtime_exports", {
	enumerable: true,
	get: function() {
		return agent_bundle_lsp_runtime_exports;
	}
});
Object.defineProperty(exports, "createBundleLspToolRuntime", {
	enumerable: true,
	get: function() {
		return createBundleLspToolRuntime;
	}
});
