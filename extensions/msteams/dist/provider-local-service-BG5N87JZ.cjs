...
*/
const log = require_subsystem.createSubsystemLogger("provider-local-service");
...
const MODEL_PROVIDER_LOCAL_SERVICE_SYMBOL = Symbol.for("operator.modelProviderLocalService");
...
function validateLocalServiceConfig(service, provider) {
	if (!node_path.default.isAbsolute(service.command)) throw new Error(`models.providers.${provider}.localService.command must be an absolute path`);
}
...
function hashStringRecord(record) {
	...
	return (0, node_crypto.createHash)("sha256").update(JSON.stringify(sorted)).digest("hex");
}
...
async function probeHealth(url, headers, signal) {
	throwIfAborted(signal);
	const egressHeaders = require_provider_secret_egress.unwrapHeadersInitSentinelsForProviderEgress(headers, "to probe local model provider health");
...
async function startAndWaitForLocalService(params) {
	...
	managed.process = (0, node_child_process.spawn)(service.command, service.args ?? [], {
		cwd: service.cwd,
		env: service.env ? {
			...process.env,
			...service.env
		...
		detached: require_child_process_tree.shouldDetachChildForProcessTree()
	});
	...
	const readyTimeoutMs = (0, _gabrielvfonseca_normalization_core_number_coercion.resolvePositiveTimerTimeoutMs)(service.readyTimeoutMs, DEFAULT_READY_TIMEOUT_MS);
...
function appendLocalServiceOutputTail(current, chunk, serviceEnv, inheritedEnv, serviceArgs, healthHeaders) {
	let redacted = require_redact.redactSensitiveText(`${current}${chunk.toString()}`, { mode: "tools" });
	for (const value of Object.values(serviceEnv ?? {})) if (value) redacted = redacted.replaceAll(value, "[redacted]");
	for (const [key, value] of Object.entries(inheritedEnv)) if (value && require_redact.isSensitiveFieldKey(key)) redacted = redacted.replaceAll(value, "[redacted]");
...
function scheduleIdleStop(key, managed, service) {
	const idleStopMs = (0, _gabrielvfonseca_normalization_core_number_coercion.clampPositiveTimerTimeoutMs)(service.idleStopMs);
...
function stopManagedService(key, managed, reason) {
	...
	if (child && !hasLocalServiceProcessExited(child)) {
		...
		require_child_process_tree.signalChildProcessTree(child, "SIGTERM");
	}
}
...
//#endregion
Object.defineProperty(exports, "attachModelProviderLocalService", {
	enumerable: true,
	get: function() {
		return attachModelProviderLocalService;
	}
});
Object.defineProperty(exports, "createConfiguredProviderLocalServiceAcquirer", {
	enumerable: true,
	get: function() {
		return createConfiguredProviderLocalServiceAcquirer;
	}
});
Object.defineProperty(exports, "ensureModelProviderLocalService", {
	enumerable: true,
	get: function() {
		return ensureModelProviderLocalService;
	}
});
Object.defineProperty(exports, "getModelProviderLocalService", {
	enumerable: true,
	get: function() {
		return getModelProviderLocalService;
	}
});
