...
function shouldUseFromAsSenderFallback(params) {
	...
	const chatType = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(params.chatType);
...
function resolveProviderFromContext(ctx, cfg) {
	const explicitMessageChannels = [
		...
		ctx.Provider
	].map((value) => require_message_channel.normalizeMessageChannel(value)).filter((value) => Boolean(value));
	const explicitMessageChannel = explicitMessageChannels.find((value) => value !== require_message_channel_core.INTERNAL_MESSAGE_CHANNEL);
	if (!explicitMessageChannel && explicitMessageChannels.includes("webchat")) return {
		providerId: void 0,
		hadResolutionError: false
	};
	const direct = require_registry_normalize.normalizeAnyChannelId(explicitMessageChannel ?? void 0) ?? explicitMessageChannel ?? require_registry_normalize.normalizeAnyChannelId(ctx.Provider) ?? require_registry_normalize.normalizeAnyChannelId(ctx.Surface) ?? require_registry_normalize.normalizeAnyChannelId(ctx.OriginatingChannel);
	if (direct) return {
		providerId: direct,
		hadResolutionError: false
	};
	...
	for (const candidate of candidates) {
		const normalizedCandidateChannel = require_message_channel.normalizeMessageChannel(candidate);
		if (normalizedCandidateChannel === "webchat") return {
			providerId: void 0,
			hadResolutionError: false
		};
		const normalized = require_registry_normalize.normalizeAnyChannelId(normalizedCandidateChannel ?? void 0) ?? normalizedCandidateChannel ?? require_registry_normalize.normalizeAnyChannelId(candidate);
	...
	if (inferred.length === 1) return {
		providerId: (0, _gabrielvfonseca_normalization_core.expectDefined)(inferred[0], "inferred entry at 0").providerId,
		hadResolutionError: (0, _gabrielvfonseca_normalization_core.expectDefined)(inferred[0], "inferred entry at 0").hadResolutionError
	};
	return {
		providerId: void 0,
...
function probeInferredProviders(ctx, cfg) {
	...
	return {
		candidates: require_registry_loaded.listLoadedChannelPlugins().map((plugin) => {
...
function formatAllowFromList(params) {
	...
	return (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(allowFrom);
}
...
function describeAllowFromResolutionError(err) {
	if (err instanceof Error) return ((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(err.name) ?? "") || "Error";
...
function resolveOwnerAllowFromList(params) {
	...
	for (const entry of raw) {
		const trimmed = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(String(entry ?? "")) ?? "";
		if (!trimmed) continue;
		const separatorIndex = trimmed.indexOf(":");
		if (separatorIndex > 0) {
			const channel = require_registry_normalize.normalizeAnyChannelId(trimmed.slice(0, separatorIndex));
			if (channel) {
...
function resolveFallbackAccountConfig(accounts, accountId) {
	...
	const matchKey = Object.keys(accounts).find((key) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(key) === normalizedAccountId);
...
function resolveCommandAuthorization(params) {
	...
	const plugin = providerId ? require_registry_loaded.getLoadedChannelPluginById(providerId) ?? void 0 : void 0;
	const from = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(ctx.From) ?? "";
	const to = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(ctx.To) ?? "";
	...
	const senderIsOwnerByScope = require_message_channel.isInternalMessageChannel(ctx.Provider) && Array.isArray(ctx.GatewayClientScopes) && ctx.GatewayClientScopes.includes("operator.admin");
	...
	const isAuthorizedSender = resolveCommandSenderAuthorization({
		...
		nativeCommandAuthorized: commandAuthorized && require_command_turn_context.isNativeCommandTurn(require_command_turn_context.resolveCommandTurnContext(ctx)) && !requireOwner,
		isOwnerForCommands,
		senderCandidates,
...
//#endregion
Object.defineProperty(exports, "resolveCommandAuthorization", {
	enumerable: true,
	get: function() {
		return resolveCommandAuthorization;
	}
});
Object.defineProperty(exports, "shouldUseFromAsSenderFallback", {
	enumerable: true,
	get: function() {
		return shouldUseFromAsSenderFallback;
	}
});
