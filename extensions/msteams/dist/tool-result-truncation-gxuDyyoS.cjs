...
const toolResultWarningDedupe = {
	promptPressure: require_dedupe.createDedupeCache({
		ttlMs: 0,
		maxSize: TOOL_RESULT_WARNING_DEDUPE_LIMIT
	}),
	sessionRecovery: require_dedupe.createDedupeCache({
		ttlMs: 0,
		maxSize: TOOL_RESULT_WARNING_DEDUPE_LIMIT
	})
};
...
function logToolResultSessionTruncation(params) {
	...
	if (params.aggregateReplacementCount <= 0) {
		require_logger.log.info(message);
		return;
	}
	...
	require_logger.log.warn(`${message}; aggregate tool-result pressure detected; consider /compact or /new if pressure persists`);
}
async function readRuntimeTranscriptFileState(scope) {
	const target = await require_transcript_rewrite.resolveRuntimeTranscriptReadTarget(scope);
	const sqliteMarker = require_sqlite_marker.parseSqliteSessionFileMarker(target.sessionFile);
	if (!sqliteMarker) throw new Error("runtime transcript target is not SQLite-backed");
	return require_transcript_rewrite.createTranscriptFileStateFromPersistedEntries(await require_session_accessor.loadTranscriptEvents({
...
function appendBoundedTruncationSuffix(params) {
	...
	while (true) {
		...
		if (keptText.length === 0) return (0, _gabrielvfonseca_normalization_core_utf16_slice.truncateUtf16Safe)(finalText, params.maxChars);
		const overflow = finalText.length - params.maxChars;
		const nextKeptText = (0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(keptText, 0, Math.max(0, keptText.length - overflow));
		keptText = nextKeptText.length < keptText.length ? nextKeptText : (0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(keptText, 0, -1);
	}
}
/**
...
function hasImportantTail(text) {
	const tail = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)((0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(text, -2e3));
...
function truncateToolResultText(text, maxChars, options = {}) {
	...
	if (hasImportantTail(text) && budget > minKeepChars * 2) {
		...
		if (headBudget > minKeepChars) {
			...
			return appendBoundedTruncationSuffix({
				keptText: (0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(text, 0, headCut) + MIDDLE_OMISSION_MARKER + (0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(text, tailStart),
	...
	return appendBoundedTruncationSuffix({
		keptText: (0, _gabrielvfonseca_normalization_core_utf16_slice.sliceUtf16Safe)(text, 0, cutPoint),
		originalTextLength: text.length,
		maxChars,
		suffixFactory
	});
...
function resolveLiveToolResultMaxChars(params) {
	const cap = require_agent_scope_config.resolveAgentContextLimits(params.cfg, params.agentId)?.toolResultMaxChars ?? resolveAutoLiveToolResultMaxChars(params.contextWindowTokens);
...
function toolResultTextContainsFullOutputFooter(message, fullOutputPath) {
	...
	const footer = require_sessions.formatFullOutputFooter(fullOutputPath);
...
function resolveAggregateElisionMarkers(message) {
	...
	if (!(0, node_fs.existsSync)(spill.fullOutputPath)) return;
...
function getToolResultProjectionKeys(messages, projectionState) {
	...
	return baseKeys.map((baseKey, index) => {
		...
		const sourceIdentity = typeof messageId === "string" && messageId.length > 0 ? `id:${messageId}` : `text:${(0, node_crypto.createHash)("sha256").update(JSON.stringify(getToolResultTextBlocks(message))).digest("base64url")}`;
...
function truncateOversizedToolResultsInExistingSessionManager(params) {
	...
	const rewriteResult = require_transcript_rewrite.rewriteTranscriptEntriesInSessionManager({
		sessionManager,
		replacements: plan.replacements
	});
	if (rewriteResult.changed && params.sessionFile) require_transcript_events.emitSessionTranscriptUpdate({
...
async function truncateOversizedToolResultsInTranscriptState(params) {
	...
	const rewriteResult = require_transcript_rewrite.rewriteTranscriptEntriesInState({
		state,
		replacements: plan.replacements
	});
	if (rewriteResult.changed) {
		await require_transcript_rewrite.persistTranscriptStateMutation({
			sessionFile: params.sessionFile,
			state,
...
function truncateOversizedToolResultsInSessionManager(params) {
	try {
		...
		require_logger.log.warn(`[tool-result-truncation] Failed to truncate: ${errMsg}`);
		return {
			truncated: false,
			truncatedCount: 0,
			reason: errMsg
...
async function truncateOversizedToolResultsInRuntimeTranscript(params) {
	try {
		...
		const rewriteResult = await require_transcript_rewrite.rewriteTranscriptEntriesInRuntimeTranscript({
			scope: params.scope,
...
async function truncateOversizedToolResultsInActiveTarget(params) {
	if (require_sqlite_marker.parseSqliteSessionFileMarker(params.scope.sessionFile)) return await truncateOversizedToolResultsInRuntimeTranscript(params);
...
async function truncateOversizedToolResultsInSession(params) {
	...
	try {
		...
		return await truncateOversizedToolResultsInTranscriptState({
			state: await require_transcript_rewrite.readTranscriptFileState(sessionFile),
			contextWindowTokens,
			maxCharsOverride: params.maxCharsOverride,
			aggregateMaxCharsOverride: params.aggregateMaxCharsOverride,
			protectTrailingToolResults: params.protectTrailingToolResults,
...
//#endregion
Object.defineProperty(exports, "DEFAULT_MAX_LIVE_TOOL_RESULT_CHARS", {
	enumerable: true,
	get: function() {
		return DEFAULT_MAX_LIVE_TOOL_RESULT_CHARS;
	}
});
Object.defineProperty(exports, "calculateMaxToolResultCharsWithCap", {
	enumerable: true,
	get: function() {
		return calculateMaxToolResultCharsWithCap;
	}
});
Object.defineProperty(exports, "estimateToolResultReductionPotential", {
	enumerable: true,
	get: function() {
		return estimateToolResultReductionPotential;
	}
});
Object.defineProperty(exports, "formatContextLimitTruncationNotice", {
	enumerable: true,
	get: function() {
		return formatContextLimitTruncationNotice;
	}
});
Object.defineProperty(exports, "resolveAutoLiveToolResultMaxChars", {
	enumerable: true,
	get: function() {
		return resolveAutoLiveToolResultMaxChars;
	}
});
Object.defineProperty(exports, "resolveLiveToolResultAggregateMaxChars", {
	enumerable: true,
	get: function() {
		return resolveLiveToolResultAggregateMaxChars;
	}
});
Object.defineProperty(exports, "resolveLiveToolResultMaxChars", {
	enumerable: true,
	get: function() {
		return resolveLiveToolResultMaxChars;
	}
});
Object.defineProperty(exports, "sessionLikelyHasOversizedToolResults", {
	enumerable: true,
	get: function() {
		return sessionLikelyHasOversizedToolResults;
	}
});
Object.defineProperty(exports, "toolResultWarningDedupe", {
	enumerable: true,
	get: function() {
		return toolResultWarningDedupe;
	}
});
Object.defineProperty(exports, "truncateOversizedToolResultsInActiveTarget", {
	enumerable: true,
	get: function() {
		return truncateOversizedToolResultsInActiveTarget;
	}
});
Object.defineProperty(exports, "truncateOversizedToolResultsInMessages", {
	enumerable: true,
	get: function() {
		return truncateOversizedToolResultsInMessages;
	}
});
Object.defineProperty(exports, "truncateOversizedToolResultsInSessionManager", {
	enumerable: true,
	get: function() {
		return truncateOversizedToolResultsInSessionManager;
	}
});
Object.defineProperty(exports, "truncateToolResultMessage", {
	enumerable: true,
	get: function() {
		return truncateToolResultMessage;
	}
});
