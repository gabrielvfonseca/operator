...
function resolveControlPlaneRateLimitKey(client) {
	const deviceId = require_control_plane_identity.normalizeControlPlaneIdentityPart(client?.connect?.device?.id, "unknown-device");
	const clientIp = require_control_plane_identity.normalizeControlPlaneIdentityPart(client?.clientIp, "unknown-ip");
	if (deviceId === "unknown-device" && clientIp === "unknown-ip") {
		const connId = require_control_plane_identity.normalizeControlPlaneIdentityPart(client?.connId, "");
...
//#endregion
Object.defineProperty(exports, "consumeControlPlaneWriteBudget", {
	enumerable: true,
	get: function() {
		return consumeControlPlaneWriteBudget;
	}
});
Object.defineProperty(exports, "pruneStaleControlPlaneBuckets", {
	enumerable: true,
	get: function() {
		return pruneStaleControlPlaneBuckets;
	}
});
