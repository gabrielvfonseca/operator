...
const require_dangerous_config_flags = require("./dangerous-config-flags-D5FW8LW-.cjs");
...
const loadAuditDeepModule = require_lazy_runtime.createLazyRuntimeModule(() => Promise.resolve().then(() => require("./audit.deep.runtime-DAbWRbI7.cjs")));
...
function collectDeepProbeFindings(params) {
	...
	if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) findings.push({
		...
		remediation: `Run "${require_command_format.formatCliCommand("openclaw status --all")}" to debug connectivity/auth, then re-run "${require_command_format.formatCliCommand("openclaw security audit --deep")}".`
	});
	if (params.authWarning) findings.push({
		...
		remediation: `Set OPERATOR_GATEWAY_TOKEN/OPERATOR_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${require_command_format.formatCliCommand("openclaw security audit --deep")}".`
	});
	return findings;
}
//#endregion
...
function collectGatewayConfigFindings$1(cfg, sourceConfig, env, options = {}) {
	...
	const controlUiAllowedOrigins = (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(cfg.gateway?.controlUi?.allowedOrigins ?? []);
	...
	const tokenConfiguredFromConfig = require_types_secrets.hasConfiguredSecretInput(sourceConfig.gateway?.auth?.token, sourceConfig.secrets?.defaults);
	const passwordConfiguredFromConfig = require_types_secrets.hasConfiguredSecretInput(sourceConfig.gateway?.auth?.password, sourceConfig.secrets?.defaults);
	const remoteTokenConfigured = require_types_secrets.hasConfiguredSecretInput(sourceConfig.gateway?.remote?.token, sourceConfig.secrets?.defaults);
	...
	const gatewayToolsAllow = new Set(gatewayToolsAllowRaw.map((v) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(v) ?? "").filter(Boolean));
	const reenabledOverHttp = require_dangerous_tools.DEFAULT_GATEWAY_HTTP_TOOL_DENY.filter((name) => gatewayToolsAllow.has(name));
	...
	const enabledDangerousFlags = (options.collectDangerousConfigFlags ?? require_dangerous_config_flags_current.collectCoreInsecureOrDangerousFlags)(cfg);
...
function isStrictLoopbackTrustedProxyEntry(entry) {
	...
	const ipVersion = (0, node_net.isIP)(rawIp.trim());
	const prefix = require_parse_finite_number.parseStrictNonNegativeInteger(rawPrefix);
	if (prefix === void 0) return false;
	if (ipVersion === 4) return rawIp.trim() === "127.0.0.1" && prefix === 32;
	if (ipVersion === 6) return prefix === 128 && (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeLowercaseStringOrEmpty)(rawIp) === "::1";
	return false;
}
//#endregion
//#region src/security/audit.ts
const loadReadOnlyChannelPlugins = require_lazy_runtime.createLazyRuntimeModule(() => Promise.resolve().then(() => require("./read-only-MDrE_ZGP.cjs")).then((n) => n.read_only_exports));
const loadAuditNonDeepModule = require_lazy_runtime.createLazyRuntimeModule(() => Promise.resolve().then(() => require("./audit.nondeep.runtime-B7IlqheG.cjs")));
const loadAuditChannelModule = require_lazy_runtime.createLazyRuntimeModule(() => Promise.resolve().then(() => require("./audit-channel.collect.runtime-DNkxCr_t.cjs")));
const loadPluginMetadataRegistryLoaderModule = require_lazy_runtime.createLazyRuntimeModule(() => Promise.resolve().then(() => require("./metadata-registry-loader-Ccbzh_aX.cjs")).then((n) => n.metadata_registry_loader_exports));
const loadPluginAutoEnableModule = require_lazy_runtime.createLazyRuntimeModule(() => Promise.resolve().then(() => require("./plugin-auto-enable-nYwhgNCn.cjs")).then((n) => n.plugin_auto_enable_exports));
const loadChannelPluginIdsModule = require_lazy_runtime.createLazyRuntimeModule(() => Promise.resolve().then(() => require("./channel-plugin-ids-CD0w6PY3.cjs")).then((n) => n.channel_plugin_ids_exports));
const loadPluginRuntimeModule = require_lazy_runtime.createLazyRuntimeModule(() => Promise.resolve().then(() => require("./runtime-DUfj3X7c.cjs")).then((n) => n.runtime_exports));
const loadGatewayProbeDeps = require_lazy_runtime.createLazyRuntimeModule(() => Promise.all([
	Promise.resolve().then(() => require("./call-CphTnsHC.cjs")).then((n) => n.call_exports),
	Promise.resolve().then(() => require("./probe-auth-bAQnhTXC.cjs")).then((n) => n.probe_auth_exports),
	Promise.resolve().then(() => require("./probe-DBcFqNwO.cjs")).then((n) => n.probe_exports)
...
async function materializeAuditGatewayAuthRefs(params) {
	const materializeParams = {
		...
		hasTokenCandidate: Boolean((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.env.OPERATOR_GATEWAY_TOKEN)),
		hasPasswordCandidate: Boolean((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(params.env.OPERATOR_GATEWAY_PASSWORD))
	};
	if (!require_auth_config_utils.canMaterializeGatewayAuthSecretRefsWithoutExec(materializeParams)) return params.cfg;
	try {
...
function shouldMaterializeHooksGatewayAuthRefs(cfg) {
	return cfg.hooks?.enabled === true && Boolean((0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalString)(cfg.hooks.token));
}
...
function normalizeAllowFromList(list) {
	...
	return (0, _gabrielvfonseca_normalization_core_string_normalization.normalizeStringEntries)(list);
}
async function collectFilesystemFindings(params) {
	const findings = [];
	const stateDirPerms = await (0, _openclaw_fs_safe_permissions.inspectPathPermissions)(params.stateDir, {
		env: params.env,
	...
	if (stateDirPerms.ok) {
		...
		if (stateDirPerms.worldWritable) findings.push({
			...
			detail: `${(0, _openclaw_fs_safe_permissions.formatPermissionDetail)(params.stateDir, stateDirPerms)}; other users can write into your Operator state.`,
			remediation: (0, _openclaw_fs_safe_permissions.formatPermissionRemediation)({
			...
			detail: `${(0, _openclaw_fs_safe_permissions.formatPermissionDetail)(params.stateDir, stateDirPerms)}; group users can write into your Operator state.`,
			remediation: (0, _openclaw_fs_safe_permissions.formatPermissionRemediation)({
				targetPath: params.stateDir,
				perms: stateDirPerms,
				isDir: true,
			...
			detail: `${(0, _openclaw_fs_safe_permissions.formatPermissionDetail)(params.stateDir, stateDirPerms)}; consider restricting to 700.`,
			remediation: (0, _openclaw_fs_safe_permissions.formatPermissionRemediation)({
				targetPath: params.stateDir,
				perms: stateDirPerms,
				isDir: true,
	...
	const configPerms = await (0, _openclaw_fs_safe_permissions.inspectPathPermissions)(params.configPath, {
		env: params.env,
		platform: params.platform,
		exec: params.execIcacls
	});
	if (configPerms.ok) {
		...
		if (configPerms.worldWritable || configPerms.groupWritable) findings.push({
			...
			detail: `${(0, _openclaw_fs_safe_permissions.formatPermissionDetail)(params.configPath, configPerms)}; another user could change gateway/auth/tool policies.`,
			remediation: (0, _openclaw_fs_safe_permissions.formatPermissionRemediation)({
			...
			detail: `${(0, _openclaw_fs_safe_permissions.formatPermissionDetail)(params.configPath, configPerms)}; config can contain tokens and private settings.`,
			remediation: (0, _openclaw_fs_safe_permissions.formatPermissionRemediation)({
				targetPath: params.configPath,
				perms: configPerms,
				isDir: false,
...
function collectGatewayConfigFindings(cfg, sourceConfig, env, options = {}) {
	return collectGatewayConfigFindings$1(cfg, sourceConfig, env, {
		collectDangerousConfigFlags: require_dangerous_config_flags.collectEnabledInsecureOrDangerousFlags,
...
function findClaudeCliBackendConfig(backends) {
	if (!backends) return;
	const directKey = Object.keys(backends).find((key) => (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(key) === "claude-cli");
	if (directKey) return backends[directKey];
	for (const [key, backend] of Object.entries(backends)) {
		const normalizedKey = (0, _gabrielvfonseca_model_catalog_core_provider_id.normalizeProviderId)(key);
		const command = (0, _gabrielvfonseca_normalization_core_string_coerce.normalizeOptionalLowercaseString)(backend.command);
...
function collectYoloExecScopeIds(cfg, approvals) {
	...
	return [{ id: require_session_key.DEFAULT_AGENT_ID }, ...agents.filter((entry) => Boolean(entry) && typeof entry === "object" && typeof entry.id === "string").map((entry) => ({ id: entry.id }))].filter((entry) => {
		const execDefaults = require_exec_defaults.resolveExecDefaults({
			cfg,
			agentId: entry.id === "main" ? void 0 : entry.id
		});
		const resolvedApprovals = require_exec_approvals.resolveExecApprovalsFromFile({
			file: approvals,
			agentId: entry.id === "main" ? void 0 : entry.id,
			overrides: {
		...
		return require_exec_approvals.minSecurity(execDefaults.security, resolvedApprovals.agent.security) === "full" && require_exec_approvals.maxAsk(execDefaults.ask, resolvedApprovals.agent.ask) === "off";
	}).map((entry) => entry.id);
}
function collectExecRuntimeFindings(cfg) {
	...
	const defaultSandboxMode = require_config.resolveSandboxConfigForAgent(cfg).mode;
	...
	const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
	const riskyAgents = agents.filter((entry) => entry && typeof entry === "object" && typeof entry.id === "string" && entry.tools?.exec?.host === "sandbox" && require_config.resolveSandboxConfigForAgent(cfg, entry.id).mode === "off").map((entry) => entry.id).slice(0, 5);
	...
	const effectiveExecScopes = Array.from(new Map([{
		id: require_session_key.DEFAULT_AGENT_ID,
	...
	if (claudePermissionModeHits.length > 0 && yoloExecScopeIds.length > 0) findings.push({
		...
		remediation: "Restrict Operator tools.exec.security/tools.exec.ask, or remove the Claude --permission-mode override."
	});
	...
	const execFilesystemPolicyHits = require_exec_filesystem_policy.collectExecFilesystemPolicyDriftHits(cfg);
	...
	const classifyRiskySafeBinTrustedDir = (entry) => {
		...
		if (!raw) return null;
		if (!node_path.default.isAbsolute(raw)) return "relative path (trust boundary depends on process cwd)";
		const normalized = node_path.default.resolve(raw).replace(/\\/g, "/").toLowerCase();
	...
	const collectRiskyTrustedDirHits = (scopePath, entries) => {
		for (const entry of require_exec_safe_bin_config.normalizeConfiguredTrustedSafeBinDirs(entries)) {
	...
	const globalSafeBins = require_exec_safe_bin_config.normalizeConfiguredSafeBins(globalExec?.safeBins);
	if (globalSafeBins.length > 0) {
		const merged = require_exec_safe_bin_runtime_policy.resolveMergedSafeBinProfileFixtures({ global: globalExec }) ?? {};
		const interpreters = require_exec_safe_bin_runtime_policy.listInterpreterLikeSafeBins(globalSafeBins).filter((bin) => !merged[bin]);
		if (interpreters.length > 0) interpreterHits.push(`- tools.exec.safeBins: ${interpreters.join(", ")}`);
		for (const hit of require_exec_safe_bin_trust.listRiskyConfiguredSafeBins(globalSafeBins)) riskySemanticSafeBinHits.push(`- tools.exec.safeBins: ${hit.bin} (${hit.warning})`);
	}
	for (const entry of agents) {
		if (!entry || typeof entry !== "object" || typeof entry.id !== "string") continue;
		const agentExec = entry.tools?.exec;
		const agentSafeBins = require_exec_safe_bin_config.normalizeConfiguredSafeBins(agentExec?.safeBins);
		if (agentSafeBins.length === 0) continue;
		const merged = require_exec_safe_bin_runtime_policy.resolveMergedSafeBinProfileFixtures({
			global: globalExec,
			local: agentExec
		}) ?? {};
		const interpreters = require_exec_safe_bin_runtime_policy.listInterpreterLikeSafeBins(agentSafeBins).filter((bin) => !merged[bin]);
		if (interpreters.length === 0) {
			for (const hit of require_exec_safe_bin_trust.listRiskyConfiguredSafeBins(agentSafeBins)) riskySemanticSafeBinHits.push(`- agents.list.${entry.id}.tools.exec.safeBins: ${hit.bin} (${hit.warning})`);
			continue;
		}
...
async function readGlobalMcporterRegistrySummary(stateDir) {
	const registryPath = node_path.default.join(stateDir, "skills", "config", "mcporter.json");
	let parsed;
	try {
		parsed = JSON.parse(await node_fs_promises.default.readFile(registryPath, "utf8"));
	} catch {
		return null;
	}
	const mcpServers = (0, _gabrielvfonseca_normalization_core_record_coerce.asNullableRecord)((0, _gabrielvfonseca_normalization_core_record_coerce.asNullableRecord)(parsed)?.mcpServers);
	if (!mcpServers) return null;
	const names = Object.entries(mcpServers).filter(([, value]) => (0, _gabrielvfonseca_normalization_core_record_coerce.asNullableRecord)(value)?.enabled !== false).map(([name]) => name).toSorted();
...
function collectAgentSkillMcpBoundaryScopes(cfg) {
	...
	return [...defaultsHaveSkillAllowlist ? [{
		...
		const sandboxMode = require_config.resolveSandboxConfigForAgent(cfg, candidate.agentId).mode;
		const exec = require_exec_defaults.resolveExecDefaults({
			cfg,
...
function collectOpenExecSurfacePaths(cfg) {
	const channels = (0, _gabrielvfonseca_normalization_core_record_coerce.asNullableRecord)(cfg.channels);
	if (!channels) return [];
	...
	const visit = (value, scope) => {
		...
		for (const [key, nested] of Object.entries(record)) {
			...
			if ((0, _gabrielvfonseca_normalization_core_record_coerce.asNullableRecord)(nested)) visit(nested, `${scope}.${key}`);
		}
	};
...
function collectInterpreterAllowlistHits(params) {
	...
	for (const [agentId, agent] of Object.entries(params.approvals.agents ?? {})) {
		...
		for (const entry of agent.allowlist ?? []) {
			if (!require_extract.isInterpreterLikeAllowlistPattern(entry.pattern)) continue;
...
async function createAuditExecutionContext(opts) {
	...
	const stateDir = opts.stateDir ?? require_paths.resolveStateDir(env);
	const configPath = opts.configPath ?? require_paths.resolveConfigPath(env, stateDir);
	const workspaceDir = opts.workspaceDir ?? require_agent_scope_config.resolveAgentWorkspaceDir(cfg, require_agent_scope_config.resolveDefaultAgentId(cfg));
...
async function runSecurityAudit$1(opts) {
	const findings = [];
