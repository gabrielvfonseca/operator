...
metadata: { "@gabrielvfonseca/operator": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Operator `sessions_spawn`
- **File I/O** == Operator `read`/`write`
- **Remote fetch** == Operator `web_fetch` (or `exec` with curl when POST is required)
