...
const REALTIME_VOICE_CONSULT_SYSTEM_PROMPT = [
  "You are the configured Operator agent receiving delegated requests from a live phone voice bridge.",
...
function resolveVoiceCallConsultSessionKey(call: {
  config: VoiceCallConfig;
  coreSession?: OperatorConfig["session"];
  sessionKey?: string;
  from?: string;
  to?: string;
...
function listRealtimeAgentIds(config: VoiceCallConfig, coreConfig: OperatorConfig): string[] {
...
export async function createVoiceCallRuntime(params: {
  ...
  const cfg = fullConfig ?? (coreConfig as OperatorConfig);
  ...
  const webhookServer = new VoiceCallWebhookServer(
    ...
    provider,
    coreConfig,
    fullConfig ?? (coreConfig as OperatorConfig),
    agentRuntime,
    log,
  );
  if (realtimeProvider) {
