...
Official Voice Call plugin for **Operator**.
...
Docs: `https://docs.operator.ai/plugins/voice-call`
Plugin system: `https://docs.operator.ai/tools/plugin`
...
```bash
PLUGIN_HOME=~/.operator/extensions
mkdir -p "$PLUGIN_HOME"
...
- Runtime accepts canonical config only. If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `operator doctor --fix` to rewrite them.
- advanced webhook, streaming, and tunnel notes: `https://docs.operator.ai/plugins/voice-call`
...
- `realtime.consultThinkingLevel` is optional. When set, it overrides the thinking level used by the model behind realtime `operator_agent_consult` calls.
- `realtime.consultFastMode` is optional. When set, it toggles fast mode for realtime `operator_agent_consult` calls.

## Stale call reaper

See the plugin docs for recommended ranges and production examples:
`https://docs.operator.ai/plugins/voice-call#stale-call-reaper`

## TTS for calls

Voice Call uses the core `messages.tts` configuration for
streaming speech on calls. Override examples and provider caveats live here:
`https://docs.operator.ai/plugins/voice-call#tts-for-calls`
