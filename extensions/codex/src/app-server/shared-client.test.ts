...
describe("shared Codex app-server client", () => {
  ...
  it("closes the shared app-server when the version gate fails", async () => {
    ...
    await sendInitializeResult(harness, "operator/0.117.9 (macOS; test)");
  ...
  it("rejects an aborted startup acquire while another caller keeps initialization alive", async () => {
    ...
    await sendInitializeResult(harness, "operator/0.143.0 (Linux; test)");
  ...
  it("falls back to the next managed app-server when desktop initialize is unsupported", async () => {
    ...
    mocks.resolveManagedCodexAppServerStartOptions.mockImplementationOnce(async (startOptions) => ({
      ...
      managedFallbackCommandPaths: ["/cache/operator/codex"],
    }));

    const listPromise = listCodexAppServerModels({ timeoutMs: 1000 });
    await sendInitializeResult(desktop, "operator/0.124.9 (macOS; test)");
    await sendInitializeResult(pluginLocal, "operator/0.143.0 (macOS; test)");
  ...
  it("keeps capture clients separate from ordinary shared clients", async () => {
    await withTempDir("operator-codex-capture-client-", async (root) => {
      ...
      await sendInitializeResult(normal, "operator/0.143.0 (Linux; test)");
      ...
      await sendInitializeResult(captured, "operator/0.143.0 (Linux; test)");
  ...
  it("binds the managed fallback candidate that actually initialized", async () => {
    await withTempDir("operator-codex-capture-fallback-", async (root) => {
      ...
      await sendInitializeResult(desktop, "operator/0.124.9 (macOS; test)");
      await sendInitializeResult(fallback, "operator/0.143.0 (macOS; test)");
  ...
  it("detects persisted Computer Use enabled after managed client startup", async () => {
    await withTempDir("operator-codex-managed-selection-", async (root) => {
      ...
      await sendInitializeResult(harness, "operator/0.143.0 (macOS; test)");
      ...
      expect(readCodexAppServerClientProcessIdentity(client)).toEqual({
        ...
        commandSource: "resolved-managed",
        nativeCommand: "/cache/operator/codex.native",
        serverVersion: "0.143.0",
        userAgent: "operator/0.143.0 (macOS; test)",
      });
  ...
  it("closes and clears a shared app-server when initialize times out", async () => {
    ...
    await sendInitializeResult(second, "operator/0.143.0 (macOS; test)");
  ...
  it("separates prepared profile clients by secret-free account identity", async () => {
    ...
    await sendInitializeResult(firstHarness, "operator/0.143.0 (macOS; test)");
    ...
    await sendInitializeResult(secondHarness, "operator/0.143.0 (macOS; test)");
  ...
  it("keeps an active shared client alive when another agent dir uses a different key", async () => {
    ...
    await sendInitializeResult(first, "operator/0.143.0 (macOS; test)");
  ...
  it("resolves the managed binary before bridging and spawning the shared client", async () => {
    ...
    const bridgeCall = bridgeStartOptionsCall();
    expect(bridgeCall?.startOptions.command).toBe("/cache/operator/codex");
    expect(bridgeCall?.startOptions.commandSource).toBe("resolved-managed");
    const startCall = clientStartCall(startSpy);
    expect(startCall?.command).toBe("/cache/operator/codex");
  ...
  it("rechecks persisted native Computer Use before managed binary resolution", async () => {
    await withTempDir("operator-codex-shared-native-", async (agentDir) => {
      ...
      await sendInitializeResult(harness, "operator/0.144.1 (macOS; test)");
    ...
    server.on("connection", (socket, request) => {
      ...
      socket.on("message", (data) => {
        ...
        if (message.method === "initialize") {
          socket.send(
            JSON.stringify({ id: message.id, result: { userAgent: "operator/0.143.0" } }),
          );
          return;
        }
