...
} from "@gabrielvfonseca/operator/plugin-sdk/agent-harness";
import { AUTH_PROFILE_RUNTIME_CONTRACT } from "@gabrielvfonseca/operator/plugin-sdk/agent-runtime-test-contracts";
...
describe("Auth profile runtime contract - Codex app-server adapter", () => {
  ...
  beforeEach(async () => {
    resetCodexTestBindingStore();
    vi.useRealTimers();
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-codex-auth-contract-"));
  });
