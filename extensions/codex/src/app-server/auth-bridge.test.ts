...
describe("bridgeCodexAppServerStartOptions", () => {
  it("never overlays persisted profiles onto a supplied runtime store", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-codex-app-server-"));
  ...
  it("uses the native user Codex home for coexistence mode", async () => {
    await withTempDir("operator-codex-user-home-", async (root) => {
  ...
  it("places the ephemeral auth-store override after configured root overrides", async () => {
    await withTempDir("operator-codex-auth-store-", async (agentDir) => {
      const startOptions = createStartOptions({
  ...
  it("does not mistake an option value for the app-server subcommand", async () => {
    await withTempDir("operator-codex-profile-name-", async (agentDir) => {
      const startOptions = createStartOptions({
  ...
  it("preserves custom stdio backend arguments", async () => {
    await withTempDir("operator-codex-custom-backend-", async (agentDir) => {
      const startOptions = createStartOptions({
  ...
  it("fingerprints supplied token stores with the same profile id independently", async () => {
    const resolveKey = async (token: string) =>
      await resolveCodexAppServerAuthAccountCacheKey({
        agentDir: "/tmp/operator-codex-prepared-auth",
        authProfileId: "openai:work",
        authProfileStore: {
          version: 1,
          profiles: {
  ...
  it("applies native Codex CLI OAuth when no Operator auth profile exists", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "operator-codex-app-server-"));
    const agentDir = path.join(root, "agent");
  ...
  it("finds native Codex OAuth in the OS home when Operator uses an isolated home", async () => {
    ...
    const openClawHome = path.join(root, "operator-home");
