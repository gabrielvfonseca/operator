/**
 * Policy promotion for Codex app-server runs that can safely use Operator tool
 * approvals.
 */
import {
 ...
 * requirements allow Operator to handle tool approvals.
 */
export function resolveCodexAppServerForOperatorToolPolicy(params: {
  ...
  const explicitMode =
    ...
    isCodexAppServerPolicyMode(params.env.OPERATOR_CODEX_APP_SERVER_MODE);
  const explicitApprovalPolicy =
    ...
    isConfiguredCodexAppServerApprovalPolicy(
      params.env.OPERATOR_CODEX_APP_SERVER_APPROVAL_POLICY,
    ) ||
