...
export function createToolFactoryHarness(cfg: OperatorPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<OperatorPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
