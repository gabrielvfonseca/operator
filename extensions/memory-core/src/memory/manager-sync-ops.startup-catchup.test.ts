...
import { statSessionEntrySync } from "operator/plugin-sdk/memory-core-host-engine-qmd";
import type {
  MemorySource,
  MemorySyncParams,
...
describe("session startup catch-up", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-session-startup-"));
