...
} from "operator/plugin-sdk/memory-core-host-engine-embeddings";
...
function createMissingLlamaCppProviderError(): Error {
  return new Error(
    [
      ...
      "Install it with: operator plugins install @operator/llama-cpp-provider",
      "Then restart Operator and retry: operator memory status --deep",
    ].join("\n"),
  );
}
