...
const {
  ...
  const chokidarKey = Symbol.for("operator.test.memoryWatchFactory");
  const nativeKey = Symbol.for("operator.test.memoryNativeWatchFactory");
...
const CHOKIDAR_FACTORY_KEY = Symbol.for("operator.test.memoryWatchFactory");
const NATIVE_FACTORY_KEY = Symbol.for("operator.test.memoryNativeWatchFactory");
...
vi.mock("operator/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("operator/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "operator-memory-watch-"));
