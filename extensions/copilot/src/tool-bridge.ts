 ...
 * authoritative `createOperatorCodingTools({...})` call shape).
 *
 * Declared as `Partial<EmbeddedRunAttemptParams>` (imported from the
 * `operator/plugin-sdk/agent-harness-runtime` boundary, *not* from
...
interface CopilotToolBridgeInput {
   ...
   * model, and run-trace fields to `createOperatorCodingTools` so the
  ...
  createOperatorCodingTools?: (opts: unknown) => AnyAgentTool[] | Promise<AnyAgentTool[]>;
  beforeExecute?: (ctx: {
    toolName: string;
    toolCallId: string;
    args: unknown;
...
export async function createCopilotToolBridge(
  ...
  const createOperatorCodingTools =
    input.createOperatorCodingTools ??
    (await import("operator/plugin-sdk/agent-harness")).createOperatorCodingTools;
  ...
  const toolOptions = buildOperatorCodingToolsOptions(
    input,
    {
      ...effectiveToolPlan,
  ...
  try {
    sourceTools = await createOperatorCodingTools(toolOptions);
  } catch (error: unknown) {
    throw createError(
      `[copilot-tool-bridge] createOperatorCodingTools failed: ${toError(error).message}`,
      error,
    );
  }
  ...
  if (!Array.isArray(sourceTools)) {
    throw new Error(
      "[copilot-tool-bridge] createOperatorCodingTools must return an array of tools",
    );
  }
  ...
  return {
    ...
    sdkTools: filteredTools.map((sourceTool) =>
      convertOperatorToolToSdkTool(sourceTool, {
        abortSignal: input.abortSignal,
...
/**
 * Builds the full `createOperatorCodingTools` options bag mirroring the
 * PI in-tree call at `src/agents/pi-embedded-runner/run/attempt.ts:1029-1117`.
 *
 * Why PI parity matters: bridged Operator tools register with the SDK
 * as `overridesBuiltInTool: true, skipPermission: true` (see
 * `convertOperatorToolToSdkTool` below). That means the wrapped-tool
 * enforcement layer
 ...
 */
function buildOperatorCodingToolsOptions(
  input: CopilotToolBridgeInput,
  ...
  const modelCompat =
    ...
    typeof model.compat === "object"
      ? (model.compat as OperatorCodingToolsOptions["modelCompat"])
      : undefined;
...
function convertOperatorToolToSdkTool(
  sourceTool: AnyAgentTool,
  ctx: {
    abortSignal?: AbortSignal;
  ...
  const executeOnce = async (
    ...
    // Operator tools throw for execution failures. Error-shaped details remain
  ...
  return {
    ...
    // Operator owns its bridged tools by design (the harness docs:
    // "Operator still owns ... Operator dynamic tools (bridged)"). The bundled
    // Copilot CLI ships built-in tools whose names (edit, read, write, bash,
    // ...) collide with Operator's coding-tool set. Mark every bridged tool as
    ...
    // same name." Operator's tool layer is the source of truth for these
    // names within a copilot attempt.
    overridesBuiltInTool: true,
    parameters: sourceTool.parameters as Record<string, unknown> | undefined,
    // Bridged Operator tools enforce their own permission/policy decisions
    ...
    // `kind: "custom-tool"` would either short-circuit Operator's richer
    // enforcement (if we allow-all) or block every call (if we
    // reject-all) — neither matches PI parity. The in-tree codex harness
    // takes the same approach: bridged Operator tools are wrapped with
