...
async function convertOperatorToolToSdkToolForTest(
  ...
  const bridge = await createCopilotToolBridge({
    ...
    createOperatorCodingTools: async () => [sourceTool],
    modelId: "gpt-test",
...
describe("createCopilotToolBridge", () => {
  it("returns empty arrays for unsupported providers without calling the seam", async () => {
    const createOperatorCodingTools = vi.fn(async () => [makeTool()]);
    ...
    expect(createOperatorCodingTools).toHaveBeenCalledTimes(0);
  });

  it("allows vetted BYOK providers to expose model tools", async () => {
    const sourceTools = [makeTool()];
    const createOperatorCodingTools = vi.fn(async () => sourceTools);
    ...
    expect(createOperatorCodingTools).toHaveBeenCalledWith(
  ...
  it("forwards supported fields to injected createOperatorCodingTools", async () => {
    ...
    expect(createOperatorCodingTools).toHaveBeenCalledTimes(1);
  ...
  it("returns sdkTools and sourceTools with matching lengths", async () => {
    ...
    const result = await createCopilotToolBridge({
      agentId: "agent-1",
      createOperatorCodingTools: async () => sourceTools,
      modelId: "gpt-4o",
      modelProvider: "github-copilot",
      sessionId: "session-1",
    });
  ...
  it("compacts the Copilot tool surface behind tool_search controls when enabled", async () => {
    const createOperatorCodingTools = vi.fn(async (opts: unknown) => {
  ...
  it("compacts the Copilot tool surface behind code-mode exec/wait when enabled", async () => {
    const createOperatorCodingTools = vi.fn(async () => [
      makeTool({ name: "fake_hidden" }),
      makeTool({ name: "read" }),
    ]);
  ...
  it("throws when createOperatorCodingTools returns a non-array", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createOperatorCodingTools: async () => ({ tools: [] }) as never,
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      }),
    ).rejects.toThrow("createOperatorCodingTools must return an array");
  });

  it("throws when createOperatorCodingTools rejects and includes the cause", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createOperatorCodingTools: async () => {
  ...
  // F6: PI-parity tool context. The bridged Operator tools register
  ...
  describe("PI-parity attempt context (F6)", () => {
    function captureCall() {
      ...
      return {
        createOperatorCodingTools,
        getOpts: () =>
          (createOperatorCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as Record<
            string,
            unknown
          >,
      };
    ...
    it("forwards identity, owner/policy, and channel/routing fields from attemptParams", async () => {
      const { createOperatorCodingTools, getOpts } = captureCall();
    ...
    it("prefers the unscoped toolAuthProfileStore when building Operator tools", async () => {
      const { createOperatorCodingTools, getOpts } = captureCall();
  ...
  describe("sandbox forwarding (PR #86155 [P1])", () => {
    ...
    it("defaults sandbox to undefined and derives spawnWorkspaceDir from workspaceDir when no sandbox is passed (back-compat)", async () => {
      ...
      const opts = (createOperatorCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        sandbox?: unknown;
    ...
    it("forwards an explicit sandbox and spawnWorkspaceDir verbatim to createOperatorCodingTools", async () => {
  ...
  // Operator attempt would suppress. These tests pin the contract.
  describe("tool-surface gating (PR #86155 [P1] round-6)", () => {
    it("short-circuits when attemptParams.disableTools is true and never calls createOperatorCodingTools", async () => {
      const createOperatorCodingTools = vi.fn(async () => [makeTool()]);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
    ...
    it("leaves the tool list unchanged when toolsAllow is undefined", async () => {
      ...
      const createOperatorCodingTools = vi.fn(async () => tools);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
  ...
  describe("tool-name aliases (PR #86155 [P1] round-7)", () => {
    ...
    it("does not discard lean-mode overrides after tool construction", async () => {
      const result = await createCopilotToolBridge({
        ...
        createOperatorCodingTools: async () => [makeTool({ name: "image_generate" })],
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      });
    ...
    it("keeps core tools available for glob allowlists", async () => {
      ...
      const options = (createOperatorCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        toolConstructionPlan?: { includeOperatorTools?: boolean };
      };
      expect(options?.toolConstructionPlan?.includeOperatorTools).toBe(true);
    });
...
describe("createCopilotToolBridge tool conversion", () => {
  it("throws on empty and non-string names", async () => {
    await expect(
      convertOperatorToolToSdkToolForTest(makeTool({ name: "" as never }), {}),
    ).rejects.toThrow("tool name must be a non-empty string");
    await expect(
      convertOperatorToolToSdkToolForTest(makeTool({ name: 42 as never }), {}),
    ).rejects.toThrow("tool name must be a non-empty string");
  });

  it("throws on non-function execute", async () => {
    await expect(
      convertOperatorToolToSdkToolForTest(makeTool({ execute: "nope" as never }), {}),
  ...
  it("preserves name, description, and parameters exactly", async () => {
    ...
    const result = await convertOperatorToolToSdkToolForTest(sourceTool, {});
  ...
  it("sets skipPermission: true so Operator's wrapped-tool internal enforcement handles permission decisions (PI-parity model)", async () => {
    // Per the harness docs: every bridged Operator tool comes from
    // `createOperatorCodingTools`, which already wraps each tool with
    ...
    // for kind: "custom-tool" would either short-circuit Operator's
    // richer enforcement (allow-all) or block every call (reject-all).
    // Setting `skipPermission: true` lets the wrapped execute() run
    // Operator's hook with the right context — mirrors codex
    // (`extensions/codex/src/app-server/dynamic-tools.ts`).
    const result = (await convertOperatorToolToSdkToolForTest(makeTool(), {})) as SdkTool & {
  ...
  it("marks every bridged tool as overridesBuiltInTool so Operator owns names that collide with Copilot CLI built-ins (edit/read/write/bash/...)", async () => {
    ...
    for (const name of ["edit", "read", "write", "bash", "live_echo"]) {
      const result = (await convertOperatorToolToSdkToolForTest(
        makeTool({ name }),
        {},
  ...
  it("returns a failure result when the signal is already aborted", async () => {
    ...
    controller.abort();
    const sourceTool = makeTool();
    const sdkTool = await convertOperatorToolToSdkToolForTest(sourceTool, {
      abortSignal: controller.signal,
    });
  ...
  it("calls beforeExecute with the invocation context before execute", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = await convertOperatorToolToSdkToolForTest(sourceTool, { beforeExecute });
  ...
  it("calls prepareArguments and passes the prepared args and toolCallId to execute", async () => {
    ...
    const sdkTool = await convertOperatorToolToSdkToolForTest(sourceTool, { onToolCompleted });
  ...
  it("converts single text content to an exact textResultForLlm", async () => {
    ...
    const sdkTool = await convertOperatorToolToSdkToolForTest(makeTool({}, sourceResult), {
      onAgentToolResult,
    });
  ...
  it("reports thrown tool failures to the private result observer", async () => {
    ...
    const onAgentToolResult = vi.fn();
    const sdkTool = await convertOperatorToolToSdkToolForTest(
      makeTool({
        execute: vi.fn(async () => {
          throw error;
        }),
  ...
  it("reports returned Operator error results to both tool observers", async () => {
    const onAgentToolResult = vi.fn();
    const onToolCompleted = vi.fn();
    const sourceResult = {
  ...
  it("reports catalog tool failures to the host terminal observer", async () => {
    ...
    await createCopilotToolBridge({
      ...
      createOperatorCodingTools: async (options: unknown) => {
