 ...
 *     use cases.
 *   - Falls back to OPERATOR_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *   4. OPERATOR_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  // "copilot" subdir so it cannot collide with Operator's own files
...
function readEnvTokenFallback(
  ...
  // OPERATOR_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "OPERATOR_GITHUB_TOKEN", value: readString(env.OPERATOR_GITHUB_TOKEN) },
