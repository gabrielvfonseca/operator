...
describe("resolveCopilotAuth - contract-resolved auth (resolvedApiKey + authProfileId)", () => {
  ...
  it("contract resolvedApiKey takes precedence over env fallback", () => {
    const result = resolveCopilotAuth({
      ...
      authProfileId: "p",
      env: {
        OPERATOR_GITHUB_TOKEN: "env-should-be-ignored",
...
describe("resolveCopilotAuth - env var fallbacks", () => {
  ...
  it("OPERATOR_GITHUB_TOKEN takes precedence over GITHUB_TOKEN", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: {
  ...
  it("OPERATOR_GITHUB_TOKEN takes precedence over COPILOT_GITHUB_TOKEN, GH_TOKEN and GITHUB_TOKEN", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      env: {
        OPERATOR_GITHUB_TOKEN: "openclaw-tok",
  ...
  it("explicit auth.useLoggedInUser=true wins over env tokens", () => {
    const result = resolveCopilotAuth({
      agentId: "agent-1",
      auth: { useLoggedInUser: true },
      env: { OPERATOR_GITHUB_TOKEN: "env-tok" } as NodeJS.ProcessEnv,
      homeDir: fakeHomeDir,
    });
    expect(result.authMode).toBe("useLoggedInUser");
  });
