...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: OperatorConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: OperatorConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: OperatorConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "For the usual setup, run Operator on the Mac signed into Messages.",
    ...
    "After Private API setup, run `imsg launch`, then `operator channels status --probe`.",
    "Ensure Operator has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: OperatorConfig; accountId?: string }) =>
