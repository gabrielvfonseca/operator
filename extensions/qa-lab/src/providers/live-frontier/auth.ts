...
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "OPERATOR_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "OPERATOR_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["OPERATOR_LIVE_ANTHROPIC_KEY"],
  gemini: ["OPERATOR_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
...
function qaLiveOpenAiUsesCodexByDefault(cfg: OperatorConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: OperatorConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: OperatorConfig; providerId: string }) {
...
export function assertQaLiveCodexAuthAvailable(params: {
  ...
  throw new Error(
    [
      ...
      "Set OPENAI_API_KEY or OPERATOR_LIVE_OPENAI_KEY for an API-key fallback, or set CODEX_HOME to a logged-in Codex CLI home.",
      "Host Operator OAuth refresh profiles are not copied into QA temp stores.",
    ].join(" "),
  );
}
