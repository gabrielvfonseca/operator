...
import type { QaRunnerCliContribution } from "@gabrielvfonseca/operator/plugin-sdk/qa-runner-runtime";
...
describe("qa cli registration", () => {
  ...
  it.each([
    ...
    await expect(
      program.parseAsync(["node", "@gabrielvfonseca/operator", "qa", "run", flag, ...values]),
  ...
  it("rejects an empty qa run --qa-profile instead of falling back to self-check", async () => {
    await expect(
      program.parseAsync(["node", "@gabrielvfonseca/operator", "qa", "run", "--qa-profile", ""]),
  ...
  it("routes mantis desktop browser smoke flags into the mantis runtime command", async () => {
    ...
    expect(runMantisDesktopBrowserSmokeCommand).toHaveBeenCalledWith({
      browserUrl: "https://operator.ai/docs",
  ...
  it.each([
    ...
    await expect(
      invalidProgram.parseAsync(["node", "@gabrielvfonseca/operator", ...args]),
  ...
  it("routes telegram CLI defaults into the lane runtime", async () => {
    await program.parseAsync(["node", "@gabrielvfonseca/operator", "qa", "telegram"]);
