...
export function registerQaLabCli(program: Command) {
  ...
  qa.command("suite")
    ...
    .option("--runtime-pair <pair>", "Run each scenario under both runtimes, e.g. operator,codex")
    .option(
  ...
  qa.command("jsonl-replay")
    ...
    .option("--runtime-pair <pair>", "Runtime pair label, e.g. operator,codex", "operator,codex")
    .option(
      "--provider-mode <mode>",
  ...
  credentials
    ...
    .option("--site-url <url>", "Override OPERATOR_QA_CONVEX_SITE_URL")
    .option("--endpoint-prefix <path>", "Override OPERATOR_QA_CONVEX_ENDPOINT_PREFIX")
