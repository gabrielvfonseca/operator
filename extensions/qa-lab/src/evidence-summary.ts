...
export function resolveQaEvidenceProfile(params: {
  ...
  const envProfiles = [
    ["OPERATOR_E2E_PROFILE", params.env?.OPERATOR_E2E_PROFILE],
    ["OPERATOR_QA_PROFILE", params.env?.OPERATOR_QA_PROFILE],
  ] as const;
...
function resolveQaEvidenceRunner(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  return params.env?.OPERATOR_QA_RUNNER?.trim() || params.fallback || "host";
}

function resolveQaEvidenceChannelDriver(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  const id =
    params.fallback?.trim() ||
    params.env?.OPERATOR_QA_CHANNEL_DRIVER?.trim() ||
    params.env?.OPERATOR_E2E_CHANNEL_DRIVER?.trim();
  return id ? { id } : undefined;
}

function resolveQaEvidencePackageSource(env: NodeJS.ProcessEnv | undefined) {
  const spec = env?.OPERATOR_QA_PACKAGE_SOURCE?.trim() || undefined;
  const sha = env?.OPERATOR_QA_PACKAGE_SOURCE_SHA?.trim() || undefined;
