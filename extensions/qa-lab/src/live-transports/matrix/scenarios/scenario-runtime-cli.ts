...
import { spawn as startOperatorCliProcess } from "node:child_process";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `operator ${redactMatrixQaCliArgs(args).join(" ")}`;
}
...
function isMatrixQaCliChildProcessGroupRunning(
  child: ReturnType<typeof startOperatorCliProcess>,
): boolean {
...
export function startMatrixQaOperatorCli(params: {
  ...
  const distEntryPath = resolveMatrixQaOperatorCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startOperatorCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
...
export async function runMatrixQaOperatorCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaOperatorCli(params).wait();
}
...
export async function createMatrixQaOperatorCliRuntime(params: {
  accountId: string;
  accessToken: string;
  ...
  const env = {
    ...
    OPERATOR_DISABLE_AUTO_UPDATE: "1",
