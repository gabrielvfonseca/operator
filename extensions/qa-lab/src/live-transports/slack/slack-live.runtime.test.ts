...
describe("Slack live QA runtime helpers", () => {
  ...
  it("resolves env credential payloads", () => {
    expect(
      testing.resolveSlackQaRuntimeEnv({
        OPERATOR_QA_SLACK_CHANNEL_ID: "C123456789",
        OPERATOR_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        OPERATOR_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        OPERATOR_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toEqual({
  ...
  it("rejects malformed Slack channel ids", () => {
    expect(() =>
      testing.resolveSlackQaRuntimeEnv({
        OPERATOR_QA_SLACK_CHANNEL_ID: "qa-channel",
        OPERATOR_QA_SLACK_DRIVER_BOT_TOKEN: "xoxb-driver",
        OPERATOR_QA_SLACK_SUT_BOT_TOKEN: "xoxb-sut",
        OPERATOR_QA_SLACK_SUT_APP_TOKEN: "xapp-sut",
      }),
    ).toThrow("OPERATOR_QA_SLACK channelId must be a Slack id like C123 or U123.");
  });
