...
async function withMockedOpenAiMemoryPlugin<T>(params: {
  ...
  vi.resetModules();
  vi.doMock("operator/plugin-sdk/runtime-env", () => ({
...
describe("memory plugin e2e", () => {
  const { getDbPath, getTmpDir } = installTmpDirHarness({ prefix: "operator-memory-test-" });
  ...
  test("uses provider adapter auth when embedding apiKey is omitted", async () => {
    ...
    vi.resetModules();
    vi.doMock("operator/plugin-sdk/memory-core-host-engine-embeddings", () => ({
    ...
    try {
      ...
      expect(providerOptions.agentDir).toBe("/tmp/operator-agent");
      ...
      vi.doUnmock("operator/plugin-sdk/memory-core-host-engine-embeddings");
      vi.doUnmock("openai");
      vi.doUnmock("./lancedb-runtime.js");
      vi.resetModules();
    }
    ...
    await withMockedOpenAiMemoryPlugin({
      ...
      run: async (dynamicMemoryPlugin) => {
        ...
        try {
          ...
          await program.parseAsync(["node", "operator", "ltm", "list", "--limit", "+03"]);
  ...
  test("escapeMemoryForPrompt strips media attached annotations before escaping", () => {
    expect(
      escapeMemoryForPrompt(
        "User sent image [media attached: /Users/alex/.operator/media/photo.jpg (image/jpeg)] and said hello",
      ),
