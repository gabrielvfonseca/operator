...
function createNativeCommand(
  ...
  opts?: {
    cfg?: OperatorConfig;
    discordConfig?: NonNullable<OperatorConfig["channels"]>["discord"];
  },
  ...
  const baseCfg: OperatorConfig = opts?.cfg ?? {};
  const discordConfig: NonNullable<OperatorConfig["channels"]>["discord"] =
...
function createAllowedGuildAutocompleteConfig(
  commands: NonNullable<OperatorConfig["commands"]>,
): OperatorConfig {
  return {
    commands,
    channels: {
...
describe("createDiscordNativeCommand option wiring", () => {
  ...
  it("returns no autocomplete choices for group DMs outside dm.groupChannels", async () => {
    ...
    } satisfies NonNullable<OperatorConfig["channels"]>["discord"];
    const command = createNativeCommand("think", {
      cfg: {
        commands: {
          allowFrom: {
  ...
  it("truncates Discord command and option descriptions on a UTF-16 boundary", () => {
    ...
    const discordConfig = {} as NonNullable<OperatorConfig["channels"]>["discord"];
    const command = createDiscordNativeCommand({
      command: {
        name: "longdesc",
