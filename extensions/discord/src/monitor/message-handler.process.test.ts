...
import { setReplyPayloadMetadata } from "@gabrielvfonseca/operator/plugin-sdk/reply-payload-testing";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/operator-discord-process-test-sessions.json",
  ),
}));
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/operator-discord-process-test-sessions.json");
    ...
    try {
      ...
      const ctx = await createBaseContext({
        ...
        cfg: {
          ...
          session: { store: "/tmp/operator-discord-process-test-sessions.json" },
        },
...
describe("processDiscordMessage session routing", () => {
  it("carries preflight audio transcript into dispatch context and marks media transcribed", async () => {
    const ctx = await createBaseContext({
      ...
      preparedMedia: [
        {
          path: "/tmp/operator-discord-test/voice.ogg",
  ...
  it("uses prepared media instead of re-downloading after the run queue", async () => {
    ...
    expectRecordFields(requireRecord(getLastDispatchCtx(), "dispatch context"), {
      MediaPath: "/tmp/operator-discord-test/photo.png",
      MediaType: "image/png",
      MediaPaths: ["/tmp/operator-discord-test/photo.png"],
    });
  });
  ...
  it("pins Discord text DM main-route updates to the single configured DM owner", async () => {
    const ctx = await createBaseContext({
      ...
      cfg: {
        messages: { ackReaction: "👀" },
        session: {
          store: "/tmp/operator-discord-process-test-sessions.json",
          dmScope: "main",
        },
      },
