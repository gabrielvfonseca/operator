...
describe("WhatsApp auth dir profile resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["OPERATOR_STATE_DIR", "OPERATOR_OAUTH_DIR"]);
    tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "operator-wa-profile-"));
  ...
  it("resolves the default web auth dir from OPERATOR_STATE_DIR at call time", async () => {
