...
const pluginRuntimeMocks = vi.hoisted(() => {
  ...
  let stateDir = `/tmp/operator-whatsapp-ingress-${Date.now()}-${Math.random()}`;
  ...
  return {
    ...
    reset: () => {
      ...
      stateDir = `/tmp/operator-whatsapp-ingress-${Date.now()}-${Math.random()}`;
    },
  };
});
...
function expectInboxPairingReplyText(
  ...
  expect(text).toContain("Operator: access not configured.");
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    resetWebInboundDedupe();
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "operator-auth-"));
    } else {
      authDir = undefined;
    }
  });
