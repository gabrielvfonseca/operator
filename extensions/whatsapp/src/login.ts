...
import { logInfo } from "@gabrielvfonseca/operator/plugin-sdk/logging-core";
...
import { defaultRuntime, type RuntimeEnv } from "@gabrielvfonseca/operator/plugin-sdk/runtime-env";
...
export async function loginWeb(
  ...
  try {
    ...
    if (result.outcome === "logged-out") {
      runtime.error(
        danger(
          `WhatsApp reported the session is logged out. Cleared cached web session; please rerun ${formatCliCommand("operator channels login")} and scan the QR again.`,
        ),
      );
