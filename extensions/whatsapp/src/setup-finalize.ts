...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: OperatorConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: OperatorConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: OperatorConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: OperatorConfig, accountId: string): Promise<boolean> {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("operator channels login"),
      }),
      "WhatsApp",
    );
  }
