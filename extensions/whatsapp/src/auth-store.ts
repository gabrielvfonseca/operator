...
import { info, success } from "@gabrielvfonseca/operator/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("operator channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
