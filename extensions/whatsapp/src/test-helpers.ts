...
import { formatEnvelopeTimestamp } from "@gabrielvfonseca/operator/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("operator:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("operator:testSourceConfigMock");
...
vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/operator-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("operator:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("operator:lastSocket")];
  if (typeof getter === "function") {
