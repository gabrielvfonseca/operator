...
function resolveVaultAuthMethod() {
  const method = normalizeOptionalString(process.env.OPERATOR_VAULT_AUTH_METHOD) ?? "token";
  if (
    method === "token" ||
    method === "token_file" ||
    method === "jwt" ||
  ...
  throw new Error("OPERATOR_VAULT_AUTH_METHOD must be token, token_file, jwt, or kubernetes.");
}
...
function resolveKvMount() {
  return process.env.OPERATOR_VAULT_KV_MOUNT?.trim().replace(/^\/+|\/+$/gu, "") || "secret";
}

function resolveKvVersion() {
  const raw = process.env.OPERATOR_VAULT_KV_VERSION?.trim();
  if (!raw || raw === "2") {
    return 2;
  }
  if (raw === "1") {
  ...
  throw new Error("OPERATOR_VAULT_KV_VERSION must be 1 or 2.");
}
...
function resolveVaultAuthMount(method) {
  return process.env.OPERATOR_VAULT_AUTH_MOUNT?.trim().replace(/^\/+|\/+$/gu, "") || method;
}

function resolveVaultAuthRole(method) {
  const role = normalizeOptionalString(process.env.OPERATOR_VAULT_AUTH_ROLE);
  if (!role) {
    throw new Error(`OPERATOR_VAULT_AUTH_ROLE is required for ${method} auth.`);
  }
  return role;
}
...
async function resolveVaultJwt(method) {
  const jwtFile =
    normalizeOptionalString(process.env.OPERATOR_VAULT_JWT_FILE) ??
    (method === "kubernetes" ? KUBERNETES_SERVICE_ACCOUNT_TOKEN_PATH : undefined);
  if (!jwtFile) {
    throw new Error("OPERATOR_VAULT_JWT_FILE is required for jwt auth.");
  }
  const jwt = (await readFile(jwtFile, "utf8")).trim();
  if (!jwt) {
    throw new Error("OPERATOR_VAULT_JWT_FILE did not contain a JWT.");
  }
  return jwt;
}
