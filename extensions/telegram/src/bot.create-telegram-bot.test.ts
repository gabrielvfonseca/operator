...
const { resolveTelegramGroupPromptSettings, resolveTelegramScopedGroupConfig } = await import(
  "./group-config-helpers.js"
);
...
function createTelegramBotTestStateDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "operator-telegram-bot-"));
...
describe("createTelegramBot", () => {
  ...
  beforeEach(async () => {
    ...
    process.env.OPERATOR_STATE_DIR = createTelegramBotTestStateDir();
    resetTelegramForumFlagCacheForTest();
    clearPluginInteractiveHandlers();
  ...
  it.each(["stop", "/stop@operator_bot"] as const)(
    ...
    try {
      ...
      await runTelegramMiddlewareChain({
        ctx: {
          ...
          message: {
            chat: { id: -1007, type: "supergroup", title: "Operator Ops" },
            text: "first",
            date: 1736380804,
            message_id: 104,
            from: { id: 42, first_name: "Ada" },
  ...
  it("routes plugin callback_query payloads to plugin handlers without fallback callback_data text", async () => {
    ...
    expect(
      registerPluginInteractiveHandler("operator-code-agent", {
        channel: "telegram",
  ...
  it("ignores group self-authored message updates instead of re-processing bot output", async () => {
    ...
    await handler({
      message: {
        chat: { id: -1001234, type: "supergroup", title: "Operator Ops" },
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "Operator", username: "operator_bot" },
  ...
  it("reloads topic agent overrides between messages without recreating the bot", async () => {
    ...
    const sendTopicMessage = async (chatId: number, messageId: number, text: string) => {
      await handler({
        ...
        me: { username: "operator_bot", has_topics_enabled: true },
        getFile: async () => ({ download: async () => new Uint8Array() }),
      });
    };
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "operator_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("marks explicit Telegram bot-handle mentions in the inbound context", async () => {
    ...
    await dispatchMessage({
      message: {
        chat: { id: 7, type: "group", title: "Test Group" },
        text: "@operator_bot status",
        entities: [{ type: "mention", offset: 0, length: "@operator_bot".length }],
        date: 1736380800,
        message_id: 4,
        from: { id: 9, first_name: "Ada" },
      },
    ...
    expect(payload.BotUsername).toBe("operator_bot");
  });
  ...
  it("honors routed group activation from session store", async () => {
    const storePath = "/tmp/operator-telegram-group-activation.json";
