...
export function shouldShowTelegramDmAccessWarning(cfg: OperatorConfig, accountId: string): boolean {
...
export function buildTelegramDmAccessWarningLines(accountId: string): string[] {
  ...
  return [
    ...
    `  ${formatCliCommand(`operator config set ${configBase}.dmPolicy "allowlist"`)}`,
    `  ${formatCliCommand(`operator config set ${configBase}.allowFrom '["YOUR_USER_ID"]'`)}`,
