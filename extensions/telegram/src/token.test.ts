...
describe("resolveTelegramToken", () => {
  ...
  function createUnknownAccountConfig(): OperatorConfig {
    return {
      channels: {
        telegram: {
  ...
  function expectNoTokenForUnknownAccount(cfg: OperatorConfig) {
  ...
  it.runIf(process.platform !== "win32")("rejects symlinked tokenFile paths", () => {
    ...
    const cfg = { channels: { telegram: { tokenFile: tokenLink } } } as OperatorConfig;
  ...
  // Regression: https://github.com/operator/operator/issues/53876
