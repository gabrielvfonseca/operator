...
import type { ChannelMessageActionAdapter } from "@gabrielvfonseca/operator/plugin-sdk/channel-contract";
import {
...
import { createAccountStatusSink } from "@gabrielvfonseca/operator/plugin-sdk/channel-outbound";
import { createChannelMessageAdapterFromOutbound } from "@gabrielvfonseca/operator/plugin-sdk/channel-outbound";
import {
...
import { createChannelDirectoryAdapter } from "@gabrielvfonseca/operator/plugin-sdk/directory-runtime";
...
import type { RoutePeer } from "@gabrielvfonseca/operator/plugin-sdk/routing";
import {
...
const telegramMessageAdapter = createChannelMessageAdapterFromOutbound<OperatorConfig>({
  id: "telegram",
  live: {
    capabilities: {
      draftPreview: true,
...
function resolveTelegramNativeTopicThreadId(
  ...
  // Keep the chat-scoped canonical id inside Operator state; translate it back
  ...
  base: {
      ...
      inboundFormattingHints: ({ cfg, accountId }) => {
        ...
        if (richMessages) {
          return {
            ...
            rules: [
              "Telegram rich ON (Bot API 10.2 blocks; Operator maps markdown + these HTML islands to typed blocks).",
    ...
    gateway: {
      ...
      logoutAccount: async ({ accountId, cfg }) => {
        ...
        const nextCfg = { ...cfg } as OperatorConfig;
