...
function scanTelegramMalformedGroupsConfig(cfg: OperatorConfig): TelegramMalformedGroupsHit[] {
...
function scanTelegramInvalidAllowFromEntries(cfg: OperatorConfig): TelegramAllowFromInvalidHit[] {
...
function scanTelegramBotEndpointApiRoots(cfg: OperatorConfig): TelegramApiRootBotEndpointHit[] {
...
function formatTelegramAccountConfigPath(cfg: OperatorConfig, accountId: string): string {
  const telegram = asObjectRecord((cfg.channels as Record<string, unknown> | undefined)?.telegram);
  const accounts = asObjectRecord(telegram?.accounts);
...
function maybeRepairTelegramApiRoots(cfg: OperatorConfig): {
...
async function repairTelegramConfig(params: { cfg: OperatorConfig }): Promise<{
  config: OperatorConfig;
  changes: string[];
}> {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: OperatorConfig): Promise<{
  config: OperatorConfig;
  changes: string[];
}> {
