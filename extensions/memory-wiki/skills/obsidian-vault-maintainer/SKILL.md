...
- Start from `operator wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `operator wiki obsidian status` before shelling out, then prefer the dedicated helpers like `operator wiki obsidian search`, `operator wiki obsidian open`, `operator wiki obsidian command`, and `operator wiki obsidian daily`.
