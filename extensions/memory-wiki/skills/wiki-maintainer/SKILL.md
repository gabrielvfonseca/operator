...
description: Maintain the Operator memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `operator wiki ingest`, `operator wiki compile`, and `operator wiki lint` as the default maintenance loop.
- In `bridge` mode, run `operator wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `operator wiki unsafe-local import` only when the user explicitly opted into private local path access.
