...
describe("initializeMemoryWikiVault", () => {
  it("creates the wiki layout and seed files", async () => {
    ...
    await expect(fs.access(path.join(rootDir, ".operator-wiki", "state.json"))).rejects.toThrow(
      /ENOENT/,
    );
    await expect(fs.access(path.join(rootDir, ".operator-wiki", "locks"))).rejects.toThrow(
      /ENOENT/,
    );
  });
