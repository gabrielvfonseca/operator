...
export function clearGoogleOAuthCredentialsCache(): void {
  (
    requireTestApi("operator.google.oauthCredentialsTestApi") as OAuthCredentialsTestApi
  ).clearCredentialsCache();
}

export function setGoogleOAuthCredentialsFs(overrides?: Partial<CredentialFs>): void {
  (requireTestApi("operator.google.oauthCredentialsTestApi") as OAuthCredentialsTestApi).setFs(
    overrides,
  );
}

export function setGoogleOAuthSettingsFs(overrides?: Partial<OAuthSettingsFs>): void {
  (requireTestApi("operator.google.oauthSettingsTestApi") as OAuthSettingsTestApi).setFs(overrides);
}

export function resetGoogleVertexAdcState(): void {
  (requireTestApi("operator.google.vertexAdcTestApi") as VertexAdcTestApi).reset();
}
