...
function usesClaudeCliModelSelection(config: OperatorConfig): boolean {
...
function usesSelectedClaudeCliAuthProfile(config: OperatorConfig): boolean {
  const profiles = config.auth?.profiles ?? {};
...
function collectClaudeCliRuntimeRefsFromConfig(config: OperatorConfig): string[] {
  const refs = new Set<string>(
...
/** Apply Anthropic and Claude CLI defaults to an Operator config object. */
