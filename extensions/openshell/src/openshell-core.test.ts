...
describe("openshell backend manager", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const skillsWorkspaceDir = await makeTempDir("operator-openshell-skills-");
      sandboxMocks.remoteRoot = await makeTempDir("operator-openshell-remote-");
      sandboxMocks.remoteAgentRoot = await makeTempDir("operator-openshell-agent-remote-");
      const materializedDir = path.join(sandboxMocks.remoteRoot, ".operator", "sandbox-skills");
      ...
      const result = await backend.runRemoteShellScript({
        script: 'test -d "$1"',
        args: ["/sandbox/.operator/sandbox-skills"],
      });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      sandboxMocks.remoteRoot = await makeTempDir("operator-openshell-remote-");
      sandboxMocks.remoteAgentRoot = await makeTempDir("operator-openshell-agent-remote-");
      const outsideDir = await makeTempDir("operator-openshell-outside-");
      await fs.symlink(outsideDir, path.join(sandboxMocks.remoteRoot, ".operator"));
      await fs.writeFile(path.join(skillsWorkspaceDir, "SKILL.md"), "# Skill\n", "utf8");
      cliMocks.runOpenShellCli.mockResolvedValue({ code: 0, stdout: "", stderr: "" });
  ...
  it("checks runtime status with config override from Operator config", async () => {
    ...
    const result = await manager.describeRuntime({
      entry: {
        containerName: "operator-session-1234",
        backendId: "openshell",
        runtimeLabel: "operator-session-1234",
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "operator-session-1234",
        config: expectedConfig,
      },
      args: ["sandbox", "get", "operator-session-1234"],
    });
  });
  ...
  it("removes runtimes via openshell sandbox delete", async () => {
    ...
    await manager.removeRuntime({
      entry: {
        containerName: "operator-session-5678",
        backendId: "openshell",
        runtimeLabel: "operator-session-5678",
        sessionKey: "agent:main",
        createdAtMs: 1,
        lastUsedAtMs: 1,
        image: "@gabrielvfonseca/operator",
        configLabelKind: "Source",
      },
      config: {},
    });
    ...
    expect(cliMocks.runOpenShellCli).toHaveBeenCalledWith({
      context: {
        sandboxName: "operator-session-5678",
        config: expectedConfig,
      },
      args: ["sandbox", "delete", "operator-session-5678"],
    });
  });
  ...
  it("preserves a local sandbox skills shadow when mirror sync crosses filesystems", async () => {
    const workspaceDir = await makeTempDir("operator-openshell-workspace-");
    const shadowFile = path.join(workspaceDir, ".operator", "sandbox-skills", "user-note.txt");
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        ...
        await fs.mkdir(path.join(tmpDir, ".operator", "sandbox-skills", "skills"), {
          recursive: true,
        });
        await fs.writeFile(
          path.join(tmpDir, ".operator", "sandbox-skills", "skills", "generated.txt"),
          "generated",
          "utf8",
        );
      }
    ...
    try {
      ...
      await expectPathMissing(
        path.join(workspaceDir, ".operator", "sandbox-skills", "skills", "generated.txt"),
      );
    } finally {
  ...
  it("drops non-directory materialized sandbox skills from mirror downloads", async () => {
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        const tmpDir = expectDefined(args[4], "OpenShell download destination");
        await fs.writeFile(path.join(tmpDir, "from-remote.txt"), "remote", "utf8");
        await fs.mkdir(path.join(tmpDir, ".operator"), { recursive: true });
        await fs.writeFile(path.join(tmpDir, ".operator", "sandbox-skills"), "poison", "utf8");
      }
      return { code: 0, stdout: "", stderr: "" };
    });
    ...
    await expectPathMissing(path.join(workspaceDir, ".operator", "sandbox-skills"));
  });
  ...
  it("restores a local sandbox skills shadow when mirror download has a file parent", async () => {
    ...
    cliMocks.runOpenShellCli.mockImplementation(async ({ args }: { args: string[] }) => {
      if (args[0] === "sandbox" && args[1] === "download") {
        const tmpDir = expectDefined(args[4], "OpenShell download destination");
        await fs.writeFile(path.join(tmpDir, "from-remote.txt"), "remote", "utf8");
        await fs.writeFile(path.join(tmpDir, ".operator"), "poison", "utf8");
      }
      return { code: 0, stdout: "", stderr: "" };
    });
    ...
    await expect(fs.readFile(shadowFile, "utf8")).resolves.toBe("local shadow");
    expect((await fs.stat(path.join(workspaceDir, ".operator"))).isDirectory()).toBe(true);
  });
});
...
async function makeExecutable(params: { name: string; script: string }): Promise<string> {
  const dir = await makeTempDir("operator-openshell-bin-");
...
describe("openshell fs bridges", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const stateDir = await makeTempDir("operator-openshell-remote-pin-");
  ...
  it("writes locally and syncs the file to the remote workspace", async () => {
    const workspaceDir = await makeTempDir("operator-openshell-fs-");
  ...
  it("rejects cross-root mirror renames before the remote backend commit", async () => {
    const workspaceDir = await makeTempDir("operator-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("operator-openshell-agent-fs-");
  ...
  it("reads materialized sandbox skills from the protected skills workspace", async () => {
    ...
    await expect(
      bridge.readFile({
        filePath: "/sandbox/.operator/sandbox-skills/skills/demo/SKILL.md",
      }),
  ...
  it("maps agent mount paths when the sandbox workspace is read-only", async () => {
    const workspaceDir = await makeTempDir("operator-openshell-fs-");
    const agentWorkspaceDir = await makeTempDir("operator-openshell-agent-");
