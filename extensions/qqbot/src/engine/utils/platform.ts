 ...
 * `OPERATOR_HOME`. QQ Bot still checks this tree for legacy state imports and
...
/**
 * Resolve the effective Operator home directory.
 *
 ...
 */
function resolveOperatorHome(): string {
  const raw = process.env.OPERATOR_HOME?.trim();
 ...
 * The base honors `OPERATOR_HOME` (when set) so files written by agents into
 * the Operator-managed media tree are reachable by this plugin even when
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  const allowedRoots = [getOperatorMediaDir(), getQQBotMediaPath()];
