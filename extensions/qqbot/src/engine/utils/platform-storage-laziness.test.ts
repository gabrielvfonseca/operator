...
describe("qqbot storage laziness", () => {
  ...
  it("does not create ~/.operator/qqbot from module imports or read-only probes", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".operator", "qqbot");
  ...
  it("creates storage when qqbot persists runtime state", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".operator", "qqbot");
    const sqlitePath = path.join(stateDir, "state", "operator.sqlite");
