...
async function createStateDir(): Promise<string> {
  const stateDir = await mkdtemp(path.join(tmpdir(), "operator-widget-tool-"));
...
describe("show_widget", () => {
  ...
  it("wraps SVG widgets with the sandbox CSP and SVG layout", async () => {
    ...
    expect(url).toBe(`/__operator__/canvas/documents/${viewId}/index.html`);
    ...
    expect(html).toContain("operator:widget-size");
