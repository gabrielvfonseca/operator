...
important run ids in the public `operator/releases` evidence ledger.
...
```text
full-release-validation operator/operator <run-id> blocking
package-acceptance operator/operator <run-id> blocking
release-checks operator/operator <run-id> blocking
```
...
`ghcr.io/operator/operator-live-media-runner:ubuntu-24.04` container so
...
- Resolve the registry immediately before dispatch:
  ...
  and `npm view operator@beta version dist.tarball dist.integrity --json --prefer-online --cache /tmp/operator-npm-cache-verify-$$`.
- If Peter asks for "latest beta", use `source=npm` with
  `package_spec=operator@beta`, then record the resolved version from `npm view`
  or the workflow summary.
- For reruns, release proof, or comparing one known package, prefer the exact
  immutable spec: `package_spec=operator@YYYY.M.D-beta.N` or
  `package_spec=operator@YYYY.M.D`.
- For stable package proof, use `package_spec=operator@latest` only when the
  question is explicitly the current stable dist-tag; otherwise pin the exact
  version.
- `source=npm` only accepts registry specs for `operator@beta`,
  `operator@latest`, or exact Operator release versions. Do not pass semver
...
- `source=npm`: `operator@beta`, `operator@latest`, or an exact release version.
