...
- On macOS guests, prefer a user-global install plus a stable PATH-visible shim:
  ...
  - make sure `~/.local/bin/operator` exists or `~/.npm-global/bin` is on PATH
  - verify from a brand-new guest shell with `which operator` and `operator --version`

## npm install then update

- Preferred entrypoint: `pnpm test:parallels:npm-update`
- For a macOS-only published release update check, use:
  - `timeout --foreground 75m pnpm test:parallels:npm-update -- --platform macos --package-spec operator@<old-version> --update-target <target-version-or-tag> --json`
...
- For published targets, install the old baseline package first (for example `operator@2026.4.9`), then run the installed guest CLI with the intended channel/tag (for example `operator update --channel beta --yes --json`) and verify `operator --version`, `operator update status --json`, gateway RPC, and an agent turn after the command.
...
- For beta/stable verification, resolve the tag immediately before the run (`npm view operator@beta version dist.tarball` or `npm view operator@latest ...`). Tags can move while a long VM matrix is already running; restart the matrix when the intended prerelease appears after an earlier registry 404/tag-lag check.
...
- For beta 4 npm validation with agent turns, the known-good shape is:
  - `gtimeout --foreground 150m pnpm test:parallels:npm-update -- --beta-validation beta4 --model openai/gpt-5.4 --json`
    Prefer the explicit `beta4` alias over `operator@beta` when validating a specific prerelease number; npm tags can move.
...
- `parallels-macos-smoke.sh --mode fresh --target-package-spec operator@<version>` is an install smoke only. For published old-version -> new-version update coverage on macOS, prefer the npm-update wrapper with `--platform macos`; `parallels-macos-smoke.sh --mode upgrade --target-package-spec ...` installs the target package and does not exercise the baseline CLI's updater.
...
- Optional exact npm-tag baseline on Windows: `bash scripts/e2e/parallels-windows-smoke.sh --mode upgrade --target-package-spec operator@<tag> --json`. That lane installs the published npm tarball as baseline, then runs `operator update --channel dev`.
