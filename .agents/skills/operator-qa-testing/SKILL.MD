...
OPERATOR_NPM_TELEGRAM_PACKAGE_SPEC="operator@beta" \
...
```bash
gh workflow run "NPM Telegram Beta E2E" --repo operator/operator --ref main \
  -f package_spec=operator@YYYY.M.D-beta.N \
  -f package_label=operator@YYYY.M.D-beta.N \
...
```bash
gh api repos/operator/operator/actions/runs/<run-id>/artifacts
```
