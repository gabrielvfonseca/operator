...
const DEFAULT_CHECKOUT = "/Users/steipete/operator";
const DEFAULT_EXPECTED_ORIGIN = "operator/operator";
...
export function classifyActions(
  ...
  const generatedMacProtocolChanged = changedPaths.some((changedPath) =>
    /^apps\/shared\/OperatorKit\/Sources\/OperatorProtocol\//u.test(changedPath),
  );
  const runMacos =
  ...
  const macUiVerification =
    ...
    changedPaths.some((changedPath) =>
      /^(?:apps\/macos\/Sources\/|apps\/shared\/OperatorKit\/Sources\/|apps\/swabble\/Sources\/)/u.test(
        changedPath,
      ),
    );
...
function defaultLockPath(checkout) {
  ...
  return path.join(tmpdir(), `operator-live-updater-${key}.lock`);
}

function defaultStatePath(checkout) {
  return path.join(realpathSync(checkout), ".git", "operator-live-updater-state.json");
}
...
function writeMaintenanceState(statePath, state) {
  ...
  const temporary = path.join(directory, `.operator-live-updater-${process.pid}-${randomUUID()}`);
...
function resolveManagedGatewayCommand(
  ...
  const defaultEnvDir = path.join(stateDir ?? path.join(home, ".operator"), "service-env");
...
function readManagedGatewayLaunchAgent(checkout) {
  ...
  const plistPath = path.join(home, "Library/LaunchAgents/ai.operator.gateway.plist");
  let plistStat;
  try {
  ...
  const configPath =
    typeof plist?.EnvironmentVariables?.OPENCLAW_CONFIG_PATH === "string"
      ...
      : path.join(gatewayCommand.stateDir, "operator.json");
  return {
    configPath,
...
function inspectManagedGatewayDeployment(checkout) {
  ...
  const home = process.env.HOME;
  if (!home || !existsSync(path.join(home, "Library/LaunchAgents/ai.operator.gateway.plist"))) {
    return null;
  }
...
function replaceLaunchAgentEntrypoint(deployment, entrypoint) {
  const temporaryPath = `${deployment.plistPath}.operator-live-updater-${randomUUID()}`;
...
function runBuiltGatewayCli(checkout, args, deployment, options = {}) {
  ...
  const overlayPath = path.join(
    ...
    `.operator-live-updater-config-${randomUUID()}.json`,
  );
  writeFileSync(
    overlayPath,
    `${JSON.stringify({
...
export function prepareGatewaySuspension(
  ...
  const requestId = `operator-live-updater-${randomUUID()}`;
  let result;
  try {
    result = JSON.parse(
...
function runBuildWithPreservedMacApp(runCommand, checkout, sleep = defaultSleep) {
  const appBundle = path.join(checkout, "dist/Operator.app");
  ...
  const preservedBundle = path.join(
    checkout,
    ".git",
    `.operator-live-mac-${process.pid}-${randomUUID()}.app`,
  );
...
function restartGateway(
  ...
  assertExactBuild(checkout, expectedSha);
  if (!deployment) {
    runCommand("pnpm", ["operator", "gateway", "restart"], checkout);
    return startedAtMs;
  }
  if (bootstrap) {
...
function verifyGateway(runCommand, checkout, expectedSha, deployment = null) {
  ...
  runCommand(
    "pnpm",
    ["operator", "gateway", "status", "--deep", "--require-rpc", "--json"],
    checkout,
  );
  runCommand("pnpm", ["operator", "health", "--verbose", "--json"], checkout);
}
...
function isCurrentGatewayLogSource(source, sourceRoot, managedSourceRoots) {
  ...
  while (candidate !== path.dirname(candidate)) {
    ...
    if (existsSync(packagePath) && existsSync(gitPath)) {
      try {
        if (JSON.parse(readFileSync(packagePath, "utf8")).name === "operator") {
...
function readFallbackGatewayLogs(sinceMs) {
  ...
  const directories = new Set(["/tmp/operator", path.join(tmpdir(), "operator")]);
  const contents = [];
  for (const directory of directories) {
    for (const date of dates) {
      const logPath = path.join(directory, `operator-${date}.log`);
...
function defaultVerifyMacTarget(checkout) {
  ...
  const executable = path.join(checkout, "dist/Operator.app/Contents/MacOS/Operator");
