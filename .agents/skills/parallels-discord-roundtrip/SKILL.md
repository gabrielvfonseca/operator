...
```bash
export OPERATOR_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.operator/operator.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env OPERATOR_PARALLELS_DISCORD_TOKEN \
...
- Use the `openclaw` wrapper for guest `message send/read`; `node operator.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/operator-parallels-smoke.*`
...
- host inbound nonce appears in `operator message read` output
