---
name: claw-score
description: Audit or refresh Operator maturity scorecard docs from root taxonomy, maturity scores, and QA evidence artifacts without using maintainer discrawl data or committed inventory reports.
---

# claw-score

Use this skill when working on the Operator maturity scorecard in this repo.
This is the operator-local version of the maintainer `claw-score` workflow:
...
`qa-evidence.json` artifact shape generated by Operator QA workflows.
...
Run from the operator repo root.
...
```bash
pnpm operator qa coverage --json
```
...
By default, Completeness measures how fully Operator exposes the intended
...
tree to operator. Evidence-enriched scorecard outputs belong in short-lived
