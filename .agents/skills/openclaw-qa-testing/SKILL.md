---
name: operator-qa-testing
description: Run, watch, debug, extend, or explain Operator qa-lab and qa-channel scenarios, artifacts, and live lanes.
---
...
5. If the user wants to watch the live UI, find the current `operator-qa` listen port and report `http://127.0.0.1:<port>`.
...
`pnpm operator qa matrix` defaults to the full `all` profile. Use explicit
profiles for faster CI/release proof:

```bash
pnpm operator qa matrix --profile release
```
...
- That item is the first place to look for:
  ...
  - `OPERATOR_QA_PROVIDER_MODE`
  - `OPERATOR_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `Operator`
  - item: `OPERATOR_QA_CONVEX_SITE_URL`
  - item: `OPERATOR_QA_CONVEX_SECRET_MAINTAINER`
  - item: `OPERATOR_QA_CONVEX_SECRET_CI`
...
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `OPERATOR_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `OPERATOR_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `Operator/OPERATOR_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
OPERATOR_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
OPERATOR_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
OPERATOR_QA_PROVIDER_MODE="mock-openai" \
OPERATOR_NPM_TELEGRAM_PACKAGE_SPEC="openclaw@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `OPERATOR_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `OPERATOR_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `OPERATOR_QA_PROVIDER_MODE`, map it explicitly to `OPERATOR_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
  ...
  - a real run of `pnpm test:docker:npm-telegram-live` passed with:
    ...
    - `OPERATOR_QA_CONVEX_SITE_URL`
    - `OPERATOR_QA_CONVEX_SECRET_MAINTAINER`
    - `OPERATOR_NPM_TELEGRAM_PROVIDER_MODE=mock-openai`
...
```bash
gh workflow run "NPM Telegram Beta E2E" --repo openclaw/operator --ref main \
...
- Use two dedicated WhatsApp-capable test numbers: one driver account and one SUT account. Do not use personal numbers or personal Operator WhatsApp accounts in the shared pool.
...
- Validate with `pnpm operator qa whatsapp --credential-source convex --credential-role maintainer --provider-mode mock-openai` and preserve artifact paths plus redacted pass/fail summaries.
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal Operator identity combines with the character.
...
```bash
pnpm operator qa manual \
  --model codex-cli/<codex-model> \
  --message "Reply exactly: CODEX_OK"
```

- Treat the concrete Codex model name as user/config input; do not hardcode it in source, docs examples, or scenarios.
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `OPERATOR_HOME` sandboxed.
