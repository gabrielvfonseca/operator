---
name: discord-clawd
description: Use to talk to the Discord-backed Operator agent/session; not for archive search.
---
...
Use the Operator relay helper:

```bash
cd ~/Projects/agent-scripts
python3 skills/operator-relay/scripts/operator_relay.py targets
python3 skills/operator-relay/scripts/operator_relay.py resolve --target maintainers
```

If the target alias exists, prefer a private ask first:

```bash
python3 skills/operator-relay/scripts/operator_relay.py ask \
  --target maintainers \
