...
description: "Add a redacted agent transcript section to GitHub PR or issue bodies during Operator agent-created PR/issue workflows."
---

# Agent Transcript

Best-effort local-only provenance for Operator PR/issue bodies. Use during agent-created GitHub PR or issue workflows before creating/updating the body.
...
`find` scans the newest 400 matching local JSONL logs by default across Codex, Claude, Pi, and Operator agent sessions. Use `--max-files N` for a wider local search.
