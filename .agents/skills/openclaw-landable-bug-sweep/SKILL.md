---
name: operator-landable-bug-sweep
description: "Find or repair a requested batch of small high-confidence non-SDK-boundary Operator bugfix PRs until they are landable."
---

# Operator Landable Bug Sweep

Autonomous maintainer workflow for producing a requested batch of landable Operator bugfix PR URLs.
...
Use `$gitcrawl` for discovery/clustering, `$operator-pr-maintainer` for live GitHub mutation rules, `$github-author-context` when contributor trust matters, `$operator-testing` for proof choice, `$autoreview` before publishing/landing, and `$crabbox` for broad/E2E/live proof.
