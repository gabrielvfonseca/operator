...
Target release-ops repo environment: `operator/releases`, env `mac-release`.
...
- `operator/operator` is the public product repo. Its GitHub Releases page is
  where macOS assets are ultimately attached.
- `operator/operator` `macos-release.yml` is public handoff validation only.
  It never signs, notarizes, or uploads macOS assets, regardless of
  `preflight_only`.
- `operator/releases` is the restricted release-ops repo. Its macOS workflows
  sign, notarize, validate, and promote assets onto the
  `operator/operator` GitHub release.
...
```bash
gh workflow run macos-release.yml --repo operator/operator \
...
```bash
gh workflow run operator-macos-publish.yml --repo operator/releases --ref main \
  -f tag=vYYYY.M.PATCH \
...
```bash
gh workflow run operator-macos-validate.yml --repo operator/releases --ref main \
  -f tag=vYYYY.M.PATCH \
  -f source_ref=release/YYYY.M.PATCH
```
...
- Release-ops `operator/releases` publish/validate workflows run from their own
...
- `gh release view vYYYY.M.PATCH --repo operator/operator` shows zip, dmg, dSYM zip, not draft, not prerelease.
