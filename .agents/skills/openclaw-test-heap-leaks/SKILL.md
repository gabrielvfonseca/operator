---
name: operator-test-heap-leaks
description: Investigate Operator pnpm test memory growth, Vitest OOMs, RSS spikes, and heap snapshot deltas.
---

# Operator Test Heap Leaks
...
1. Reproduce the failing shape first.
   ...
   - `pnpm canvas:a2ui:bundle && OPERATOR_TEST_MEMORY_TRACE=1 OPERATOR_TEST_HEAPSNAPSHOT_INTERVAL_MS=60000 OPERATOR_TEST_HEAPSNAPSHOT_DIR=.tmp/heapsnap OPERATOR_TEST_WORKERS=2 OPERATOR_TEST_MAX_OLD_SPACE_SIZE_MB=6144 pnpm test`
   - Keep `OPERATOR_TEST_MEMORY_TRACE=1` enabled so the wrapper prints per-file RSS summaries alongside the snapshots.
...
2. Wait for repeated snapshots before concluding anything.
   ...
   - Use `.agents/skills/operator-test-heap-leaks/scripts/heapsnapshot-delta.mjs` to compare either two files directly or the earliest/latest pair per PID in one lane directory.
...
- Direct comparison:
  - `node .agents/skills/operator-test-heap-leaks/scripts/heapsnapshot-delta.mjs before.heapsnapshot after.heapsnapshot`
- Auto-select earliest/latest snapshots per PID within one lane:
  - `node .agents/skills/operator-test-heap-leaks/scripts/heapsnapshot-delta.mjs --lane-dir .tmp/heapsnap/unit-fast-batch-2`
- Useful flags:
  - `--top 40`
  - `--min-kb 32`
  - `--pid 16133`
...
```
node .agents/skills/operator-test-heap-leaks/scripts/heapsnapshot-delta.mjs \
