...
"$GH" workflow run full-release-validation.yml --repo operator/operator --ref "$BRANCH" \
  -f ref="$BRANCH" \
  -f release_profile=beta \
  -f rerun_group=all

"$GH" workflow run operator-npm-release.yml --repo operator/operator --ref "$BRANCH" \
  -f tag="$SHA" \
...
FULL_RELEASE_VALIDATION_RUN_ATTEMPT="$(gh api \
  "repos/operator/operator/actions/runs/${FULL_RELEASE_VALIDATION_RUN_ID}" \
  --jq .run_attempt)"
"$GH" workflow run operator-release-publish.yml --repo operator/operator --ref "$BRANCH" \
  -f tag="$TAG" \
...
- `npm view operator@<version>` shows the exact version, dist-tag `alpha`, tarball, integrity, and publish time.
