...
const excludedHandles = new Set(["operator", "clawsweeper", "claude", "codex", "steipete"]);
...
export function defaultGithubSnapshotPath(base, target, gitCommonDir) {
  ...
  return path.resolve(gitCommonDir, "operator-release-cache", defaultName);
}
...
function closingReferencesIn(text) {
  const references = [];
  for (const match of text.matchAll(
    /\b(?:fix(?:es|ed)?|closes?|closed|resolves?|resolved)\s+(?:(?:operator\/operator)?#\d+)(?:\s*(?:,|and)\s*(?:(?:operator\/operator)?#\d+))*/gi,
  )) {
...
function resolveAssociatedPullRequests(commitHashes, targetTimestamp) {
  ...
  for (let index = 0; index < commitHashes.length; index += commitAssociationQueryBatchSize) {
    ...
    const fields = chunk
      .map(
        (hash, offset) =>
          `c${index + offset}: repository(owner: "operator", name: "operator") {
  ...
  while (pending.length > 0) {
    ...
    const fields = chunk
      .map(
        (item, offset) =>
          `c${offset}: repository(owner: "operator", name: "operator") {
  ...
  while (pending.length > 0) {
    ...
    const fields = chunk
      .map((item, offset) => {
        ...
        return `n${offset}: repository(owner: "operator", name: "operator") {
...
function resolveReferences(numbers) {
  ...
  for (let index = 0; index < numbers.length; index += 40) {
    ...
    const fields = chunk
      .map(
        (number) => `n${number}: repository(owner: "operator", name: "operator") {
