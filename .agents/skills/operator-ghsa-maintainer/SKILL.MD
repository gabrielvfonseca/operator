...
```bash
fork=$(gh api /repos/operator/operator/security-advisories/<GHSA> | jq -r .private_fork.full_name)
...
```bash
gh api -X PATCH /repos/operator/operator/security-advisories/<GHSA> \
