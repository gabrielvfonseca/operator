...
- The scarce resource is Blacksmith runner registrations, not Blacksmith vCPU
  capacity.
- GitHub runner registrations for `operator` currently report a 10,000 per
  5-minute bucket in `actions_runner_registration`. Verify the live bucket
  before each tuning pass because GitHub can change it. The `operator`
...
ghx run list -R operator/operator --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
ghx run list -R operator/clawsweeper --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
...
- **ClawSweeper backlog:** exact-review queue grows while CI is healthy. Tune
  ClawSweeper workers in `operator/clawsweeper`, not Operator CI.
...
ghx pr checks <pr> -R operator/operator --watch --interval 15
```
...
```bash
ghx run list -R operator/operator --limit 20 --json databaseId,status,conclusion,workflowName,event,headBranch,createdAt,updatedAt,url
for repo in operator/clawsweeper operator/clawhub operator/clownfish operator/operator-rtt operator/clawbench; do
