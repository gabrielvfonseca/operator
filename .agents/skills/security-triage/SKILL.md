...
description: "Triage Operator security advisories, drafts, and GHSA reports with shipped-tag and trust-model proof."
---

# Security Triage

Use when reviewing Operator security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/operator/operator/security-advisories/<GHSA>`.
...
```bash
gh api /repos/operator/operator/security-advisories/<GHSA>
gh api /repos/operator/operator/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view operator version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo operator/operator --match title,body,comments -- "<terms>"
gh search prs --repo operator/operator --match title,body,comments -- "<terms>"
```
