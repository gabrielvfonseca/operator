...
- Target npm `extended-stable` and the canonical
  ...
  npm `latest`.
- Cover the core `operator` package and every npm-publishable official plugin
...
if [[ -n "$published_version" ]]; then
  npm view "operator@${published_version}" version \
...
- mode, published `operator@extended-stable` version or approved bootstrap
...
core `operator` selector-repair command. Repair missing or stale official-
