...
- Core `operator` publish is manual `workflow_dispatch`; creating or pushing a tag does not publish by itself.
...
- If an exact PR-head CI run has no active jobs because Blacksmith capacity is
  ...
  `gh api 'repos/operator/operator/contents/.github/workflows/ci.yml?ref=<pr-head-branch>'
--jq .content | base64 --decode | rg -q 'pull_request_number:'`. If absent,
  ...
  `gh workflow run ci.yml --repo operator/operator --ref <pr-head-branch> -f
...
4. Dispatch `full-release-validation.yml` from the same branch with
   ...
   `gh api repos/operator/operator/actions/runs/<run-id> --jq .run_attempt`.
...
7. Independently verify the exact core package, every official plugin package,
   ...
   fails, use the `operator` repair command generated by the core workflow. If
...
- The production Sparkle feed lives at `https://raw.githubusercontent.com/operator/operator/main/appcast.xml`, and the canonical published file is `appcast.xml` on `main` in the `operator` repo.
...
- Stable Windows Hub release closeout requires the signed
  ...
  `operator/operator` GitHub Release. Pass the exact signed
  `operator/operator-windows-node` release tag as `windows_node_tag` to
...
- Website Windows Hub download links should target exact canonical
  `operator/operator/releases/download/vYYYY.M.PATCH/...` assets for the current
...
- To update an existing GitHub Release body, resolve the numeric release id and
  ...
  `gh api repos/operator/operator/releases/tags/vYYYY.M.PATCH --jq .id`, then
  `gh api -X PATCH repos/operator/operator/releases/<id> -F body=@/tmp/notes.md`.
  Do not trust `gh release edit --notes-file` or `--input` JSON if verification
  disagrees; verify with `gh api repos/operator/operator/releases/<id>` because
...
- Post-published beta verification roster:
  ...
  - published npm Telegram proof: dispatch Actions > `NPM Telegram Beta E2E`
    from `main` with `package_spec=operator@<beta-version>` and
...
- Include mac release readiness in preflight by running the public validation
  workflow in `operator/operator` and the release-ops mac preflight in
  `operator/releases` for every release.
...
- Stable npm promotion from `beta` to `latest` uses the restricted release-ops
  `operator/releases/.github/workflows/operator-npm-dist-tags.yml` workflow
...
- Prefer fixing the release-ops workflow token path over any local 1Password
  ...
  repo's `NPM_TOKEN` secret, scoped to the `operator` package with read/write
...
- If the release-ops dist-tag workflow cannot promote because `NPM_TOKEN` is
  ...
  - Promote with a fresh OTP:
    `npm dist-tag add operator@YYYY.M.PATCH latest --otp "$OTP"`.
  - Verify with a cache-bypassed registry read, for example:
    ...
    and `npm view operator@latest version dist.tarball --json --prefer-online`.
...
- `.github/workflows/macos-release.yml` in `operator/operator` is now a
  ...
  artifacts.
- `operator/releases/.github/workflows/operator-macos-validate.yml` is the
...
- Real mac preflight and real mac publish both use
  `operator/releases/.github/workflows/operator-macos-publish.yml`.
...
- Mac publish uses
  `operator/releases/.github/workflows/operator-macos-publish.yml` for
...
- Real release-ops mac publish uploads the packaged `.zip`, `.dmg`, and
  `.dSYM.zip` assets to the existing GitHub release in `operator/operator`
...
20. For stable releases, start `.github/workflows/macos-release.yml` in
    `operator/operator` and wait for the public validation-only run to pass.
21. For stable releases, start
    `operator/releases/.github/workflows/operator-macos-validate.yml` with the
    same tag and wait for the release-ops mac validation lane to pass.
22. For stable releases, start
    `operator/releases/.github/workflows/operator-macos-publish.yml` with
...
24. Start `.github/workflows/operator-release-publish.yml` from the exact pinned
    ...
    `operator/operator-windows-node` tag as `windows_node_tag` and its
...
32. For stable releases, start
    `operator/releases/.github/workflows/operator-macos-publish.yml` for the
