...
normalize_bundle_id() {
  ...
  if [[ -z "$raw" ]]; then
    raw="ai.operatorfoundation.app.test.local"
  fi
  printf '%s\n' "$raw"
}
...
if [[ -z "${bundle_base}" ]]; then
  if [[ "${team_id}" == "${CANONICAL_TEAM_ID}" ]]; then
    bundle_base="ai.operatorfoundation.app"
  else
    bundle_base="ai.operatorfoundation.app.test.${bundle_suffix}"
  fi
fi
...
tmp_file="$(mktemp "${TMPDIR:-/tmp}/operator-ios-configure-signing.XXXXXX")"
cat >"${tmp_file}" <<EOF
...
OPENCLAW_CODE_SIGN_ENTITLEMENTS = Sources/Operator.entitlements
