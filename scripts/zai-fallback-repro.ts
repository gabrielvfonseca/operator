// Zai Fallback Repro script supports Operator repository automation.
...
export async function runZaiFallbackRepro(deps: RunZaiFallbackReproDeps = {}): Promise<number> {
  ...
  const baseDir = await mkdtemp(path.join(os.tmpdir(), "operator-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "operator.json");
  try {
    ...
    const sessionId = env.OPERATOR_ZAI_FALLBACK_SESSION_ID ?? createUuid();
