...
# Operator CLI installer (non-interactive, no onboarding)
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install-cli.sh | bash -s -- [--json] [--prefix <path>] [--version <ver>] [--node-version <ver>] [--onboard]
...
OPENCLAW_EFFECTIVE_HOME="$(resolve_operator_effective_home)"
PREFIX="${OPENCLAW_PREFIX:-${HOME}/.operator}"
...
GIT_DIR="${OPENCLAW_GIT_DIR:-${OPENCLAW_EFFECTIVE_HOME}/operator}"
...
Usage: install-cli.sh [options]
  ...
  --prefix <path>                     Install prefix (default: ~/.operator; use \$OPENCLAW_PREFIX to override)
  --install-method, --method npm|git  Install via npm (default) or from a git checkout
  --npm                               Shortcut for --install-method npm
  --git, --github                     Shortcut for --install-method git
  --git-dir, --dir <path>             Checkout directory (default: ~/operator, or \$OPENCLAW_HOME/operator)
  --version <ver>                     Operator version (default: latest)
...
cleanup_legacy_submodules() {
  local repo_dir="${1:-${OPENCLAW_GIT_DIR:-${OPENCLAW_EFFECTIVE_HOME}/operator}}"
...
resolve_git_operator_ref() {
  ...
  case "$requested" in
    ""|latest)
      resolved_version="$("$(npm_bin)" view "operator" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
install_operator() {
  local requested="${OPENCLAW_VERSION:-latest}"
  if is_operator_source_package_install_spec "$requested"; then
    fail "npm installs do not support Operator GitHub source targets like '${requested}'. Use --install-method git --version main, latest, beta, an exact version, or a built .tgz package."
  fi
  ...
  emit_json "{\"event\":\"step\",\"name\":\"operator\",\"status\":\"start\",\"version\":\"${requested}\"}"
  log "Installing Operator (${requested})..."
  if [[ "$SET_NPM_PREFIX" -eq 1 ]]; then
    fix_npm_prefix_if_needed
  fi

  if [[ "${requested}" == "latest" ]]; then
    if ! env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "operator@latest"; then
      log "npm install operator@latest failed; retrying operator@next"
      emit_json "{\"event\":\"step\",\"name\":\"operator\",\"status\":\"retry\",\"version\":\"next\"}"
      env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "operator@next"
      requested="next"
    fi
  else
    env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "operator@${requested}"
  fi

  mkdir -p "${PREFIX}/bin"
  rm -f "${PREFIX}/bin/operator"
  cat > "${PREFIX}/bin/operator" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec "${PREFIX}/tools/node/bin/node" "$(node_dir)/lib/node_modules/operator/dist/entry.js" "\$@"
EOF
  chmod +x "${PREFIX}/bin/operator"
  emit_json "{\"event\":\"step\",\"name\":\"operator\",\"status\":\"ok\",\"version\":\"${requested}\"}"
}
...
install_operator_from_git() {
  ...
  emit_json "{\"event\":\"step\",\"name\":\"operator\",\"status\":\"start\",\"method\":\"git\",\"repo\":\"${repo_url//\"/\\\"}\"}"
...
EOF
  chmod +x "${PREFIX}/bin/operator"
  emit_json "{\"event\":\"step\",\"name\":\"operator\",\"status\":\"ok\",\"method\":\"git\"}"
}

resolve_operator_version() {
  local version=""
  if [[ -x "${PREFIX}/bin/operator" ]]; then
    version="$("${PREFIX}/bin/operator" --version 2>/dev/null | head -n 1 | tr -d '\r')"
  fi
  echo "$version"
}
...
refresh_gateway_service_if_loaded() {
  local claw="${PREFIX}/bin/operator"
  if [[ ! -x "$claw" ]]; then
    return 0
  fi
...
main() {
  ...
  if [[ "$INSTALL_METHOD" == "git" ]]; then
    install_operator_from_git "$GIT_DIR"
  ...
  if [[ -n "$installed_version" ]]; then
    ...
    log "Operator installed (${installed_version})."
  else
