...
function parseArgs(argv, env = process.env) {
  const opts = parseFlagArgs(
    argv,
    {
      ...
      maxWallMs: readBudgetEnvNumber("OPERATOR_TEST_PERF_MAX_WALL_MS", env),
      baselineWallMs: readBudgetEnvNumber("OPERATOR_TEST_PERF_BASELINE_WALL_MS", env),
      maxRegressionPct: readBudgetEnvNumber("OPERATOR_TEST_PERF_MAX_REGRESSION_PCT", env) ?? 10,
      reportOnly: readBooleanEnv("OPERATOR_TEST_PERF_REPORT_ONLY", env),
    },
    [
