#!/usr/bin/env bash
# ClawDock - Docker helpers for Operator
# Inspired by Simon Willison's "Running Operator in Docker"
# https://til.simonwillison.net/llms/operator-docker
#
# Installation:
...
# Common paths to check for Operator
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/operator"
  "${HOME}/workspace/operator"
  "${HOME}/projects/operator"
  "${HOME}/dev/operator"
  "${HOME}/code/operator"
  "${HOME}/src/operator"
)
...
_clawdock_browser_url_for_gateway_url() {
  ...
  published=$(_clawdock_compose port operator-gateway 18789 2>/dev/null | head -n 1 | tr -d '\r')
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Operator at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/operator"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Operator not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/operator/operator.git ~/operator"
    echo "  cd ~/operator && ./scripts/docker/setup.sh"
    echo ""
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d operator-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart operator-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f operator-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.operator"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show operator.json
  if [[ -f "${config_dir}/operator.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/operator.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/operator.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No operator.json found${_CLR_RESET}"
  fi
  echo ""
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec operator-gateway \
    bash -c 'echo "alias operator=\"./operator.mjs\"" > /tmp/.bashrc_operator && bash --rcfile /tmp/.bashrc_operator'
}
...
clawdock-cli() {
  _clawdock_compose run --rm operator-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build operator-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d operator-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build operator-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPENCLAW_GATEWAY_TOKEN=$token" operator-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" operator-gateway \
    bash -c './operator.mjs config set gateway.remote.token "$TOKEN" && ./operator.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec operator-gateway \
    bash -c "./operator.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart operator-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm --no-deps operator-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec operator-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'operator config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Operator${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (operator alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to operator project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.operator)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'operator channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'operator status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.operator.ai${_CLR_RESET}"
  echo ""
}
