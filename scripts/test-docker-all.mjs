...
// Builds shared Docker images, prepares one Operator npm tarball, assigns lanes
...
const ROOT_DIR = path.resolve(process.env.OPERATOR_DOCKER_E2E_REPO_ROOT || SCRIPT_ROOT_DIR);
...
const DEFAULT_GITHUB_WORKFLOW = "operator-live-and-e2e-checks-reusable.yml";
...
function dockerAllUsage() {
  return [
    ...
    "",
    "Lane selection and scheduler settings are configured with OPERATOR_DOCKER_ALL_* env vars.",
  ].join("\n");
}
...
function resourceLimitEnvName(resource) {
  return `OPERATOR_DOCKER_ALL_${resource.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_LIMIT`;
}
...
function parseSchedulerOptions(env, parallelism) {
  const weightLimit = parsePositiveInt(
    env.OPERATOR_DOCKER_ALL_WEIGHT_LIMIT,
    parallelism,
    "OPERATOR_DOCKER_ALL_WEIGHT_LIMIT",
  );
...
function appendExtension(env, extension) {
  const current = env.OPERATOR_DOCKER_BUILD_EXTENSIONS ?? env.OPERATOR_EXTENSIONS ?? "";
  const tokens = current.split(/\s+/).filter(Boolean);
  if (!tokens.includes(extension)) {
    tokens.push(extension);
  }
  env.OPERATOR_DOCKER_BUILD_EXTENSIONS = tokens.join(" ");
}
...
function githubWorkflowRef(env = process.env) {
  return env.OPERATOR_DOCKER_E2E_WORKFLOW_REF || undefined;
}
...
export function githubWorkflowRerunCommand(laneNames, ref, env = process.env) {
  ...
  const releasePath = env.OPERATOR_DOCKER_ALL_PROFILE === RELEASE_PATH_PROFILE;
  const allowUnreleasedChangelog = env.OPERATOR_DOCKER_E2E_ALLOW_UNRELEASED_CHANGELOG === "true";
  const bareImage = maybeGhcrImage(env.OPERATOR_DOCKER_E2E_BARE_IMAGE);
  const functionalImage = maybeGhcrImage(env.OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE);
  const fields = [
    "gh workflow run",
    shellQuote(env.OPERATOR_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW),
  ...
  if (env.OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC) {
    fields.push(
      "-f",
      `published_upgrade_survivor_baseline=${shellQuote(env.OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC)}`,
    );
  }
  if (env.OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPECS) {
    fields.push(
      "-f",
      `published_upgrade_survivor_baselines=${shellQuote(env.OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPECS)}`,
    );
  }
  if (env.OPERATOR_UPGRADE_SURVIVOR_SCENARIOS) {
    fields.push(
      "-f",
      `published_upgrade_survivor_scenarios=${shellQuote(env.OPERATOR_UPGRADE_SURVIVOR_SCENARIOS)}`,
    );
  }
  if (bareImage) {
...
function buildLaneRerunCommand(name, baseEnv) {
  ...
  const image = poolLane ? e2eImageForLane(poolLane, baseEnv) : baseEnv.OPERATOR_DOCKER_E2E_IMAGE;
  const env = [
    ["OPERATOR_DOCKER_ALL_LANES", name],
    ["OPERATOR_DOCKER_ALL_BUILD", build],
    ["OPERATOR_DOCKER_ALL_PREFLIGHT", "0"],
    ["OPERATOR_SKIP_DOCKER_BUILD", "1"],
    ["OPERATOR_DOCKER_E2E_IMAGE", image || DEFAULT_E2E_FUNCTIONAL_IMAGE],
    ["OPERATOR_DOCKER_E2E_BARE_IMAGE", baseEnv.OPERATOR_DOCKER_E2E_BARE_IMAGE],
    ["OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE", baseEnv.OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE],
    ["OPERATOR_CURRENT_PACKAGE_TGZ", baseEnv.OPERATOR_CURRENT_PACKAGE_TGZ],
    ["OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC", baseEnv.OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPEC],
    ["OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPECS", baseEnv.OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPECS],
    ["OPERATOR_UPGRADE_SURVIVOR_SCENARIOS", baseEnv.OPERATOR_UPGRADE_SURVIVOR_SCENARIOS],
  ];
  if (baseEnv.OPERATOR_DOCKER_ALL_PNPM_COMMAND) {
    env.push(["OPERATOR_DOCKER_ALL_PNPM_COMMAND", baseEnv.OPERATOR_DOCKER_ALL_PNPM_COMMAND]);
  }
  return `${env
...
function withResolvedPnpmCommand(command, env) {
  const pnpmCommand = env.OPERATOR_DOCKER_ALL_PNPM_COMMAND?.trim();
  if (!pnpmCommand) {
    return command;
  }
...
function liveDockerHarnessScriptCommand(script) {
  return `bash -c 'harness="\${OPERATOR_DOCKER_E2E_TRUSTED_HARNESS_DIR:-${LIVE_DOCKER_DEFAULT_HARNESS_DIR}}"; OPERATOR_LIVE_DOCKER_REPO_ROOT="\${OPERATOR_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/${script}"'`;
}
...
export async function writeRunSummary(logDir, summary, env = process.env) {
  ...
  const payload = {
    ...
    allowUnreleasedChangelog:
      env.OPERATOR_DOCKER_E2E_ALLOW_UNRELEASED_CHANGELOG === "true" ? true : undefined,
    packageArtifactName: env.OPERATOR_DOCKER_E2E_PACKAGE_ARTIFACT_NAME || undefined,
    ...
    github: {
      ...
      runUrl:
        env.GITHUB_SERVER_URL && env.GITHUB_REPOSITORY && env.GITHUB_RUN_ID
          ...
          : undefined,
      selectedSha: env.OPERATOR_DOCKER_E2E_SELECTED_SHA || undefined,
...
async function writeFailureIndex(logDir, summary, env) {
  const ref =
    ...
    env.OPERATOR_DOCKER_E2E_SELECTED_SHA ||
    summary.github?.sha ||
    summary.github?.ref ||
    env.GITHUB_SHA ||
    "HEAD";
  ...
  const failureIndex = {
    ...
    version: 1,
    workflow: env.OPERATOR_DOCKER_E2E_WORKFLOW || DEFAULT_GITHUB_WORKFLOW,
  };
...
export function dockerPreflightContainerNames(raw) {
  return raw
    ...
    .filter((name) =>
      /^(?:operator-[a-z0-9-]+-e2e-\d+|operator-openwebui(?:-gateway)?-\d+)$/u.test(name),
    );
}
...
async function recordCleanupSmokeFailure(error, baseEnv, logDir, command, startedAtMs) {
  ...
  return {
    ...
    elapsedSeconds: phaseElapsedSeconds(startedAtMs),
    finishedAt: new Date().toISOString(),
    image: baseEnv.OPERATOR_DOCKER_E2E_IMAGE,
    logFile,
...
async function prepareOperatorPackage(baseEnv, logDir) {
  const existing = baseEnv.OPERATOR_CURRENT_PACKAGE_TGZ;
  if (existing) {
    const packageTgz = path.resolve(existing);
    baseEnv.OPERATOR_CURRENT_PACKAGE_TGZ = packageTgz;
    baseEnv.OPERATOR_BUNDLED_CHANNEL_HOST_BUILD = "0";
    baseEnv.OPERATOR_NPM_ONBOARD_HOST_BUILD = "0";
    console.log(`==> Operator package: ${packageTgz}`);
    return;
  }

  const packDir = path.join(logDir, "operator-package");
  await mkdir(packDir, { recursive: true });
  const packageTgz = path.join(packDir, "operator-current.tgz");
  await runForeground(
    "Prepare Operator package once",
    `node scripts/package-operator-for-docker.mjs --allow-unreleased-changelog --output-dir ${shellQuote(packDir)} --output-name operator-current.tgz`,
    baseEnv,
  );
  ...
  baseEnv.OPERATOR_DOCKER_E2E_ALLOW_UNRELEASED_CHANGELOG = "true";
  baseEnv.OPERATOR_CURRENT_PACKAGE_TGZ = packageTgz;
  baseEnv.OPERATOR_BUNDLED_CHANNEL_HOST_BUILD = "0";
  baseEnv.OPERATOR_NPM_ONBOARD_HOST_BUILD = "0";
  console.log(`==> Operator package: ${baseEnv.OPERATOR_CURRENT_PACKAGE_TGZ}`);
}

function e2eImageForLane(poolLane, baseEnv) {
  if (poolLane.e2eImageKind === "bare") {
    return baseEnv.OPERATOR_DOCKER_E2E_BARE_IMAGE;
  }
  if (poolLane.e2eImageKind === "functional") {
    return baseEnv.OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE;
  }
  return undefined;
}
...
function laneEnv(poolLane, baseEnv, logDir, cacheKey) {
  ...
  env.OPERATOR_DOCKER_ALL_LANE_NAME = name;
  const image = e2eImageForLane(poolLane, baseEnv);
  if (image) {
    env.OPERATOR_DOCKER_E2E_IMAGE = image;
  }
  if (poolLane.e2eImageKind) {
    env.OPERATOR_DOCKER_E2E_IMAGE_KIND = poolLane.e2eImageKind;
  }
  const cacheName = cacheKey || name;
  if (!process.env.OPERATOR_DOCKER_CLI_TOOLS_DIR) {
    env.OPERATOR_DOCKER_CLI_TOOLS_DIR = path.join(logDir, `${cacheName}-cli-tools`);
  }
  if (!process.env.OPERATOR_DOCKER_CACHE_HOME_DIR) {
    env.OPERATOR_DOCKER_CACHE_HOME_DIR = path.join(logDir, `${cacheName}-cache`);
  }
  return env;
}
...
async function runLane(lane, baseEnv, logDir, fallbackTimeoutMs) {
  ...
  await mkdir(env.OPERATOR_DOCKER_CLI_TOOLS_DIR, { recursive: true });
  await mkdir(env.OPERATOR_DOCKER_CACHE_HOME_DIR, { recursive: true });
  await fs.promises.writeFile(
    logFile,
    [
      `==> [${name}] cli tools dir: ${env.OPERATOR_DOCKER_CLI_TOOLS_DIR}`,
      `==> [${name}] cache dir: ${env.OPERATOR_DOCKER_CACHE_HOME_DIR}`,
      ...
      `==> [${name}] e2e image: ${env.OPERATOR_DOCKER_E2E_IMAGE ?? ""}`,
      "",
    ].join("\n"),
  );
  ...
  return {
    ...
    attempts,
    finishedAt: new Date().toISOString(),
    image: env.OPERATOR_DOCKER_E2E_IMAGE,
      ...
      if (running.size === 0) {
        ...
        throw new Error(
          `No Docker lanes fit scheduler limits (${describeDockerSchedulerLimits(
            parallelism,
            options,
          )}): ${blocked}. Tune OPERATOR_DOCKER_ALL_PARALLELISM, OPERATOR_DOCKER_ALL_WEIGHT_LIMIT, or OPERATOR_DOCKER_ALL_<RESOURCE>_LIMIT.`,
        );
      }
...
async function main() {
  ...
  const parallelism = parsePositiveInt(
    process.env.OPERATOR_DOCKER_ALL_PARALLELISM,
    DEFAULT_PARALLELISM,
    "OPERATOR_DOCKER_ALL_PARALLELISM",
  );
  const tailParallelism = parsePositiveInt(
    process.env.OPERATOR_DOCKER_ALL_TAIL_PARALLELISM,
    Math.min(parallelism, DEFAULT_TAIL_PARALLELISM),
    "OPERATOR_DOCKER_ALL_TAIL_PARALLELISM",
  );
  const tailLines = parsePositiveInt(
    process.env.OPERATOR_DOCKER_ALL_FAILURE_TAIL_LINES,
    DEFAULT_FAILURE_TAIL_LINES,
    "OPERATOR_DOCKER_ALL_FAILURE_TAIL_LINES",
  );
  const laneTimeoutMs = parsePositiveInt(
    process.env.OPERATOR_DOCKER_ALL_LANE_TIMEOUT_MS,
    DEFAULT_LANE_TIMEOUT_MS,
    "OPERATOR_DOCKER_ALL_LANE_TIMEOUT_MS",
  );
  const laneStartStaggerMs = parseNonNegativeInt(
    process.env.OPERATOR_DOCKER_ALL_START_STAGGER_MS,
    DEFAULT_LANE_START_STAGGER_MS,
    "OPERATOR_DOCKER_ALL_START_STAGGER_MS",
  );
  const statusIntervalMs = parseNonNegativeInt(
    process.env.OPERATOR_DOCKER_ALL_STATUS_INTERVAL_MS,
    DEFAULT_STATUS_INTERVAL_MS,
    "OPERATOR_DOCKER_ALL_STATUS_INTERVAL_MS",
  );
  const preflightRunTimeoutMs = parsePositiveInt(
    process.env.OPERATOR_DOCKER_ALL_PREFLIGHT_RUN_TIMEOUT_MS,
    DEFAULT_PREFLIGHT_RUN_TIMEOUT_MS,
    "OPERATOR_DOCKER_ALL_PREFLIGHT_RUN_TIMEOUT_MS",
  );
  const failFast = parseBool(process.env.OPERATOR_DOCKER_ALL_FAIL_FAST, true);
  const dryRun = parseBool(process.env.OPERATOR_DOCKER_ALL_DRY_RUN, false);
  const preflightEnabled = parseBool(process.env.OPERATOR_DOCKER_ALL_PREFLIGHT, true);
  const preflightCleanup = parseBool(process.env.OPERATOR_DOCKER_ALL_PREFLIGHT_CLEANUP, true);
  const timingsEnabled = parseBool(process.env.OPERATOR_DOCKER_ALL_TIMINGS, true);
  const buildEnabled = parseBool(process.env.OPERATOR_DOCKER_ALL_BUILD, true);
  const allowFrozenTargetScenarioOmissions = parseBool(
    process.env.OPERATOR_ALLOW_FROZEN_TARGET_SCENARIO_OMISSIONS,
    false,
  );
  const planJson =
    cliOptions.planJson || parseBool(process.env.OPERATOR_DOCKER_ALL_PLAN_JSON, false);
  const planReleaseAll = parseBool(process.env.OPERATOR_DOCKER_ALL_PLAN_RELEASE_ALL, false);
  const profile = parseProfile(process.env.OPERATOR_DOCKER_ALL_PROFILE);
  const releaseProfile = normalizeReleaseProfile(
    process.env.OPERATOR_DOCKER_ALL_RELEASE_PROFILE || process.env.OPERATOR_RELEASE_PROFILE,
  );
  const releaseChunk = process.env.OPERATOR_DOCKER_ALL_CHUNK || process.env.DOCKER_E2E_CHUNK || "";
  const includeOpenWebUI = parseBool(
    process.env.OPERATOR_DOCKER_ALL_INCLUDE_OPENWEBUI ?? process.env.INCLUDE_OPENWEBUI,
    true,
  );
  const selectedLaneNamesRaw =
    process.env.OPERATOR_DOCKER_ALL_LANES || process.env.DOCKER_E2E_LANES || "";
  const selectedLaneNames = parseLaneSelection(selectedLaneNamesRaw);
  if (selectedLaneNamesRaw && selectedLaneNames.length === 0) {
    throw new Error("OPERATOR_DOCKER_ALL_LANES must include at least one lane name");
  }
  const liveMode = parseLiveMode(process.env.OPERATOR_DOCKER_ALL_LIVE_MODE);
  const liveRetries = parseNonNegativeInt(
    process.env.OPERATOR_DOCKER_ALL_LIVE_RETRIES,
    DEFAULT_LIVE_RETRIES,
    "OPERATOR_DOCKER_ALL_LIVE_RETRIES",
  );
  const timingsFile = path.resolve(
    process.env.OPERATOR_DOCKER_ALL_TIMINGS_FILE || DEFAULT_TIMINGS_FILE,
  );
  const runId = process.env.OPERATOR_DOCKER_ALL_RUN_ID || utcStampForPath();
  const logDir = path.resolve(
    process.env.OPERATOR_DOCKER_ALL_LOG_DIR ||
      path.join(ROOT_DIR, ".artifacts/docker-tests", runId),
  );

  const baseEnv = commandEnv({
    OPERATOR_DOCKER_E2E_BARE_IMAGE:
      process.env.OPERATOR_DOCKER_E2E_BARE_IMAGE ||
      process.env.OPERATOR_DOCKER_E2E_IMAGE ||
      DEFAULT_E2E_BARE_IMAGE,
    OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE:
      process.env.OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE ||
      process.env.OPERATOR_DOCKER_E2E_IMAGE ||
      DEFAULT_E2E_FUNCTIONAL_IMAGE,
  });
  baseEnv.OPERATOR_DOCKER_E2E_IMAGE =
    process.env.OPERATOR_DOCKER_E2E_IMAGE || baseEnv.OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE;
  ...
  const { omittedUnsupportedLaneNames, orderedLanes, orderedTailLanes, plan, scheduledLanes } =
    resolveDockerE2ePlan({
      ...
      selectedLaneNames,
      timingStore,
      upgradeSurvivorBaselines: process.env.OPERATOR_UPGRADE_SURVIVOR_BASELINE_SPECS,
      upgradeSurvivorScenarios: process.env.OPERATOR_UPGRADE_SURVIVOR_SCENARIOS,
      upgradeSurvivorTargetRoot: process.env.OPERATOR_UPGRADE_SURVIVOR_TARGET_ROOT,
  ...
  console.log(`==> Docker E2E bare image: ${baseEnv.OPERATOR_DOCKER_E2E_BARE_IMAGE}`);
  console.log(`==> Docker E2E functional image: ${baseEnv.OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE}`);
  if (profile === RELEASE_PATH_PROFILE) {
  ...
  console.log(`==> Live-test bundled plugins: ${baseEnv.OPERATOR_DOCKER_BUILD_EXTENSIONS}`);
  ...
  if (lanesNeedOperatorPackage(scheduledLanes)) {
    await runPhase(phases, "prepare-operator-package", {}, async () => {
      await prepareOperatorPackage(baseEnv, logDir);
    });
  } else {
    console.log("==> Operator package: not needed for selected lanes");
  }
  ...
  if (buildEnabled) {
    ...
    if (lanesNeedE2eImageKind(scheduledLanes, "bare")) {
      buildEntries.push({
        ...
        env: {
          OPERATOR_DOCKER_E2E_IMAGE: baseEnv.OPERATOR_DOCKER_E2E_BARE_IMAGE,
          OPERATOR_DOCKER_E2E_TARGET: "bare",
        },
        label: `shared bare Docker E2E image once: ${baseEnv.OPERATOR_DOCKER_E2E_BARE_IMAGE}`,
        phaseDetails: { image: baseEnv.OPERATOR_DOCKER_E2E_BARE_IMAGE, imageKind: "bare" },
        phases,
      });
    }
    if (lanesNeedE2eImageKind(scheduledLanes, "functional")) {
      buildEntries.push({
        command: "pnpm test:docker:e2e-build",
        env: {
          OPERATOR_DOCKER_E2E_IMAGE: baseEnv.OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE,
          OPERATOR_DOCKER_E2E_TARGET: "functional",
        },
        label: `shared functional Docker E2E image once: ${baseEnv.OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE}`,
        phaseDetails: {
          image: baseEnv.OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE,
  ...
  if (failFast && failures.length > 0) {
    await writeSummary({
      ...
      image: baseEnv.OPERATOR_DOCKER_E2E_IMAGE,
      images: {
        bare: baseEnv.OPERATOR_DOCKER_E2E_BARE_IMAGE,
        functional: baseEnv.OPERATOR_DOCKER_E2E_FUNCTIONAL_IMAGE,
      },
      lanes: allResults,
      omittedUnsupportedLanes,
      phases,
