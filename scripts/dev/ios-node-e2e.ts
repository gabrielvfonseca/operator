// Ios Node E2E script supports Operator repository automation.
...
function usage(): string {
  return [
    ...
    "Or set env: OPERATOR_GATEWAY_URL / OPERATOR_GATEWAY_TOKEN",
    "",
    "Options:",
...
const urlRaw = getArg("--url") ?? process.env.OPERATOR_GATEWAY_URL;
const token = getArg("--token") ?? process.env.OPERATOR_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.OPERATOR_RUN_DANGEROUS === "1";
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      ...
      platform: "dev",
      mode: "cli",
      instanceId: "operator-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "Operator E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",
