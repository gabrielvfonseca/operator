#!/usr/bin/env node
// Test Env Mutation Report script supports Operator repository automation.
...
const DEFAULT_ALLOWED_FILES = new Map([
  ["src/test-utils/operator-test-state.ts", "canonical Operator test state helper"],
  ...
  ["test/setup-operator-runtime.ts", "global runtime-test setup owns process env isolation"],
  [
...
export function renderTestEnvMutationReport(
  ...
  const lines = [
    "Operator test env mutation report",
...
function printHelp(): void {
  process.stdout.write(`Operator test env mutation report
