#!/usr/bin/env bash
# Reset Operator like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.
...
APP_EXECUTABLE_RELATIVE_PATH="Contents/MacOS/Operator"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Operator"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Operator"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Operator"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.operator.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/operator-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
LOCK_HELD=0
WAIT_FOR_LOCK=0
LOG_PATH="${OPENCLAW_RESTART_LOG:-${TMPDIR:-/tmp}/operator-restart-${LOCK_KEY}.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${OPENCLAW_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.operator/disable-launchagent"
ATTACH_ONLY=1
TARGET_ONLY=0
TARGET_APP_BUNDLE="${ROOT_DIR}/dist/Operator.app"
TARGET_EXECUTABLE="${TARGET_APP_BUNDLE}/${APP_EXECUTABLE_RELATIVE_PATH}"
INSTALLED_EXECUTABLE="/Applications/Operator.app/${APP_EXECUTABLE_RELATIVE_PATH}"
STAGED_APP_DIR="${ROOT_DIR}/dist/.operator-replacement-${LOCK_KEY}-$$"
STAGED_APP_BUNDLE="${STAGED_APP_DIR}/Operator.app"
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  --target-only    Restart only this checkout's dist app; fail if another Operator app is active"
      log ""
      log "Env:"
      log "  OPENCLAW_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node operator.mjs daemon install --force --runtime node"
      log "  node operator.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.operator/disable-launchagent"
      log ""
...
kill_all_operator() {
  ...
  [[ -z "$(operator_process_pids)" ]]
}

known_operator_executables() {
  if [[ -n "${APP_BUNDLE}" ]]; then
    printf '%s\n' "${APP_BUNDLE}/${APP_EXECUTABLE_RELATIVE_PATH}"
  fi
  printf '%s\n' \
    "${ROOT_DIR}/dist/Operator.app/${APP_EXECUTABLE_RELATIVE_PATH}" \
    "/Applications/Operator.app/${APP_EXECUTABLE_RELATIVE_PATH}" \
    "${DEBUG_PROCESS_PATTERN}" \
    "${LOCAL_PROCESS_PATTERN}" \
    "${RELEASE_PROCESS_PATTERN}"
}

operator_process_pids() {
  local pattern=""
  while IFS= read -r pattern; do
    [[ -n "${pattern}" ]] || continue
    process_pids_matching "${pattern}"
  done < <(known_operator_executables) | sort -u
}
...
foreign_operator_process_pids() {
  ps axww -o pid=,command= 2>/dev/null \
    | while read -r pid command_line; do
        ...
        if [[ "${executable}" == *"/Operator.app/${APP_EXECUTABLE_RELATIVE_PATH}" \
          || "${executable}" == *"/apps/macos/.build/debug/Operator" \
          || "${executable}" == *"/apps/macos/.build-local/debug/Operator" \
          || "${executable}" == *"/apps/macos/.build/release/Operator" ]]; then
          printf '%s\n' "${pid}"
        fi
      done
}
...
print_managed_operator_supervisor_label() {
  local domain="$1"
  local label="$2"
  local job=""
...
managed_operator_supervisor_labels() {
  local jobs=""
  ...
  while IFS='|' read -r domain label; do
    ...
    print_managed_operator_supervisor_label "${domain}" "${label}" &
...
kill_managed_operator() {
  for _ in {1..10}; do
    local pids=""
    pids="$(managed_operator_process_pids)"
    if [[ -z "${pids}" ]]; then
      return 0
    fi
    while IFS= read -r pid; do
  ...
  remaining_pids="$(managed_operator_process_pids)"
  while IFS= read -r pid; do
    [[ -n "${pid}" ]] || continue
    kill -KILL "${pid}" 2>/dev/null || true
  done <<< "${remaining_pids}"
  sleep 0.3
  [[ -z "$(managed_operator_process_pids)" ]]
}

stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.operator.mac 2>/dev/null || true
}
...
if [[ "$TARGET_ONLY" -eq 1 ]]; then
  if ! managed_supervisors="$(managed_operator_supervisor_labels | sort -u | /usr/bin/paste -sd, -)"; then
    fail "Unable to inspect loaded launchd jobs before target-only restart"
  fi
  if [[ -n "${managed_supervisors}" ]]; then
    fail "Managed Operator app is supervised by launchd job(s): ${managed_supervisors}; stop those jobs before a target-only restart"
  fi
  if [[ -n "$(foreign_operator_process_pids)" ]]; then
    fail "Another Operator app or test process is active; target-only restart deferred"
  fi
  log "==> Keeping managed Operator running while the replacement builds"
else
  stop_launch_agent
  log "==> Killing existing Operator instances"
  if ! kill_all_operator; then
    fail "Operator instances did not exit after cleanup attempts"
  fi
fi
...
install_staged_app() {
  local previous="${ROOT_DIR}/dist/.Operator.app.previous-$$"
  rm -rf "${previous}"
...
choose_app_bundle() {
  ...
  if [[ -d "${ROOT_DIR}/dist/Operator.app" ]]; then
    APP_BUNDLE="$(cd "${ROOT_DIR}/dist/Operator.app" && pwd -P)"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Operator.app missing Sparkle after packaging"
    fi
    return 0
  fi

  if [[ -d "/Applications/Operator.app" ]]; then
    APP_BUNDLE="$(cd "/Applications/Operator.app" && pwd -P)"
    return 0
  fi

  fail "App bundle not found. Set OPENCLAW_APP_BUNDLE to your installed Operator.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -c "cd '${ROOT_DIR}' && node operator.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -c "cd '${ROOT_DIR}' && node operator.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".operator", "operator.json"), "utf8");
...
if [[ "$TARGET_ONLY" -eq 1 ]]; then
  if [[ -n "$(foreign_operator_process_pids)" ]]; then
    fail "Another Operator app or test process appeared during build; target-only restart deferred"
  fi
  log "==> Switching managed installed and exact target Operator instances"
  if ! kill_managed_operator; then
    fail "Managed Operator instances did not exit after cleanup attempts"
  fi
fi
...
if [[ -n "$(process_pids_matching "${APP_BUNDLE}/${APP_EXECUTABLE_RELATIVE_PATH}")" ]]; then
  log "OK: Operator is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -c "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.operator.gateway.plist' | head -n 40 || true"
fi
