// Label Open Issues script supports Operator repository automation.
...
const STATE_FILE_PATH = join(CONFIG_BASE_DIR, "@gabrielvfonseca/operator", STATE_FILE_NAME);
...
function resolveOpenAITimeoutMs(raw = process.env.OPERATOR_LABEL_OPEN_ISSUES_OPENAI_TIMEOUT_MS) {
  return parseStrictIntegerOption({
    fallback: DEFAULT_OPENAI_TIMEOUT_MS,
    label: "OPERATOR_LABEL_OPEN_ISSUES_OPENAI_TIMEOUT_MS",
    min: 1,
    raw,
  });
}
          ...
          input: [
            {
              role: "system",
              content:
                "You classify GitHub issues and pull requests for Operator. Respond with JSON only, no extra text.",
            },
            {
              role: "user",
              content: [
...
async function main() {
  ...
  logHeader("Operator Issue Label Audit");
