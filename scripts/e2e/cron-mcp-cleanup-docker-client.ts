// Cron Mcp Cleanup Docker Client script supports Operator repository automation.
...
export function readCronMcpCleanupProbePidWaitMs(env: NodeJS.ProcessEnv = process.env): number {
  return readPositiveIntEnv("OPERATOR_CRON_MCP_CLEANUP_PID_WAIT_MS", 120_000, env);
}
...
async function runCronCleanupScenario(params: {
  ...
  assert(
    initialArgs === undefined || initialArgs.includes("operator-cron-mcp-cleanup-probe"),
