#!/usr/bin/env bash
# Runs the Operator first-run Docker smoke against the package-installed
# functional E2E image, with only the test harness mounted from the checkout.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
source "$ROOT_DIR/scripts/lib/docker-e2e-image.sh"
IMAGE_NAME="$(docker_e2e_resolve_image "operator-system-agent-first-run-e2e" OPENCLAW_SYSTEM_AGENT_FIRST_RUN_E2E_IMAGE)"
CONTAINER_NAME="operator-system-agent-first-run-e2e-$$"
RUN_LOG="$(mktemp -t operator-system-agent-first-run-log.XXXXXX)"
...
echo "Running in-container Operator first-run smoke..."
...
if [ "$status" -ne 0 ]; then
  echo "Docker Operator first-run smoke failed"
