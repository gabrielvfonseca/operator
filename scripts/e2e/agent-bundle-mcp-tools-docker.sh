#!/usr/bin/env bash
# Verifies embedded Operator bundle MCP tool materialization and tool-policy behavior
# inside the package-installed functional E2E image.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
source "$ROOT_DIR/scripts/lib/docker-e2e-image.sh"
IMAGE_NAME="$(docker_e2e_resolve_image "operator-agent-bundle-mcp-tools-e2e" OPENCLAW_IMAGE)"
CONTAINER_NAME="operator-agent-bundle-mcp-tools-e2e-$$"
RUN_LOG="$(mktemp -t operator-agent-bundle-mcp-tools-log.XXXXXX)"
...
echo "Running in-container Operator bundle MCP tool availability smoke..."
...
if [ "$status" -ne 0 ]; then
  echo "Docker Operator bundle MCP tool availability smoke failed"
