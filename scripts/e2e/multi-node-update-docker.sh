...
# Operator under node-A, registers the gateway service pointing at node-A, then
# switches PATH so node-B comes first and runs `operator update`. Verifies that:
#
...
#
# Requires: Docker, a built operator-current.tgz (or will build one).
set -euo pipefail
...
IMAGE_NAME="$(docker_e2e_resolve_image "operator-multi-node-update-e2e" OPENCLAW_MULTI_NODE_UPDATE_E2E_IMAGE)"
...
echo ""
echo "── Step 2: Install Operator under node-A ──"

# Use node-A to install operator with npm prefix A.
...
echo "Installing Operator package under node-A prefix: $NPM_PREFIX_A"
operator_e2e_install_package "$ARTIFACTS/install-a.log" "Operator package under node-A prefix" "$NPM_PREFIX_A"
echo "Installed. Checking operator location..."

OPENCLAW_A="$(command -v operator)"
echo "operator binary: $OPENCLAW_A"
echo "operator version: $(operator --version 2>/dev/null || echo unknown)"

# Record the package root for node-A install.
PACKAGE_ROOT_A="$NPM_PREFIX_A/lib/node_modules/operator"
...
GATEWAY_UNIT_PATH="/root/.config/systemd/user/operator-gateway.service"
...
if ! operator gateway install --json >"$ARTIFACTS/gateway-install.json" 2>"$ARTIFACTS/gateway-install.err"; then
...
if ! operator gateway status --json \
...
# but operator is NOT installed there.
...
echo "which operator: $(command -v operator)"
echo "process.execPath will be: $(node -e "console.log(process.execPath)")"

echo ""
echo "── Step 6: Run operator update (this is the bug) ──"
...
echo "Running operator update --yes --json..."
UPDATE_EXIT=0
operator update --yes --json \
  --tag /tmp/operator-current.tgz \
...
if [ "$UPDATE_EXIT" -ne 0 ] && ! grep -q "gateway" "$ARTIFACTS/update.err" 2>/dev/null; then
  echo "FAIL: operator update failed before reaching the package install step"
...
if [ "$BAKED_NODE_AFTER" = "$NODE_B" ] && [ "$BAKED_NODE_BEFORE" != "$NODE_B" ]; then
  ...
  echo "   but Operator package is still under node-A prefix ($PACKAGE_ROOT_A)."
...
# Check 2: Is the Operator package installed under node-B npm prefix?
if [ -f "$NPM_PREFIX_B/lib/node_modules/operator/package.json" ]; then
  echo "WARNING: Operator was ALSO installed under node-B prefix (split install)"
else
  echo "OK: Operator is NOT under node-B prefix (expected: only under node-A)"
fi
...
if [ -f "$ARTIFACTS/update.err" ]; then
  if grep -qi "Shell Operator root differs" "$ARTIFACTS/update.err" 2>/dev/null; then
