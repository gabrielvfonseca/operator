...
IMAGE_NAME="$(docker_e2e_resolve_image "operator-docker-e2e-functional:local")"
...
PROJECT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/operator-compose-proof.XXXXXX")"
PROJECT_NAME="operator-compose-proof-$$"
...
cat >"$PROJECT_DIR/config/operator.json" <<EOF
{
  "gateway": {
    "mode": "local",
...
"${COMPOSE[@]}" up -d --no-build operator-gateway
GATEWAY_ID="$("${COMPOSE[@]}" ps -q operator-gateway)"
if [ -z "$GATEWAY_ID" ]; then
  echo "Compose did not create operator-gateway" >&2
  exit 1
fi
...
for _ in $(seq 1 180); do
  ...
  if [ "$health" = "unhealthy" ] || [ "$health" = "exited" ] || [ "$health" = "dead" ]; then
    "${COMPOSE[@]}" logs --no-color operator-gateway >&2
    exit 1
  fi
  sleep 1
done
...
"${COMPOSE[@]}" exec -T operator-gateway node dist/index.js health --token "$TOKEN"
"${COMPOSE[@]}" run -T --no-deps --name "$CLI_NAME" operator-cli health --token "$TOKEN"
GATEWAY_VERSION="$("${COMPOSE[@]}" exec -T operator-gateway node -p "require('./package.json').version")"
