...
function resolveStateDir() {
  if (process.env.OPERATOR_STATE_DIR) {
    return process.env.OPERATOR_STATE_DIR;
  }
  return path.join(process.env.HOME || os.homedir(), ".operator");
}
...
function resolveOperatorEntry() {
  if (process.env.OPERATOR_ENTRY) {
    return process.env.OPERATOR_ENTRY;
  }
  ...
  throw new Error("Missing OPERATOR_ENTRY and dist/index.(m)js");
}

function readPluginsList() {
  const entry = resolveOperatorEntry();
...
async function loadManifestEntries() {
  const explicit = (process.env.OPERATOR_BUNDLED_PLUGIN_SWEEP_IDS || "")
    .split(/[,\s]+/u)
  ...
  return explicit.map((lookup) => {
    ...
    if (!found) {
      throw new Error(
        `OPERATOR_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: ${lookup}. Available: ${available}`,
      );
    }
    return found;
  });
...
async function selectedManifestEntries() {
  ...
  const total = readPositiveIntEnv("OPERATOR_BUNDLED_PLUGIN_SWEEP_TOTAL", 1);
  const index = readNonNegativeIntEnv("OPERATOR_BUNDLED_PLUGIN_SWEEP_INDEX", 0);
  if (index >= total) {
    throw new Error(
      `OPERATOR_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.OPERATOR_BUNDLED_PLUGIN_SWEEP_INDEX}`,
    );
  }
