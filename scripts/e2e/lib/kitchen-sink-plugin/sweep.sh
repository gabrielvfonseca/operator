...
if [[ -z "${KITCHEN_SINK_TMP_DIR:-}" ]]; then
  KITCHEN_SINK_TMP_DIR="$(mktemp -d "/tmp/operator-kitchen-sink.XXXXXX")"
...
cleanup_kitchen_sink_sweep() {
  if [[ -n "${KITCHEN_SINK_CLAWHUB_PID_FILE:-}" && -f "$KITCHEN_SINK_CLAWHUB_PID_FILE" ]]; then
    operator_e2e_stop_process "$(cat "$KITCHEN_SINK_CLAWHUB_PID_FILE" 2>/dev/null || true)"
  fi
...
print_kitchen_sink_log() {
  local log_file="$1"
  local max_bytes
  max_bytes="$(operator_e2e_read_positive_int_env OPENCLAW_DOCKER_E2E_LOG_PRINT_BYTES 65536)" || return $?
  if [ ! -f "$log_file" ]; then
    return 0
  fi
  local log_bytes
...
operator_e2e_read_positive_int_env OPENCLAW_DOCKER_E2E_LOG_PRINT_BYTES 65536 >/dev/null

run_kitchen_sink_operator_logged() {
  local label="$1"
  shift
  local safe_label="${label//[^[:alnum:]._-]/_}"
  local log_file="${KITCHEN_SINK_TMP_DIR}/${safe_label}.log"
  if ! operator_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$OPENCLAW_ENTRY" "$@" >"$log_file" 2>&1; then
...
run_kitchen_sink_operator_capture() {
  local output_file="$1"
  shift
  operator_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$OPENCLAW_ENTRY" "$@" >"$output_file"
}
...
start_kitchen_sink_clawhub_fixture_server() {
  ...
  local wait_attempts
  wait_attempts="$(operator_e2e_read_positive_int_env OPENCLAW_CLAWHUB_FIXTURE_WAIT_ATTEMPTS 600)" || return $?
...
run_success_scenario() {
  ...
  if [ -n "${KITCHEN_SINK_PREINSTALL_SPEC:-}" ]; then
    run_kitchen_sink_operator_logged "kitchen-sink-preinstall-${KITCHEN_SINK_LABEL}" plugins install "$KITCHEN_SINK_PREINSTALL_SPEC" --force
    assert_kitchen_sink_cutover_preinstalled
    install_args+=("--force")
  fi
  run_kitchen_sink_operator_logged "kitchen-sink-install-${KITCHEN_SINK_LABEL}" plugins install "${install_args[@]}" --force
  configure_kitchen_sink_runtime
  run_kitchen_sink_operator_logged "kitchen-sink-enable-${KITCHEN_SINK_LABEL}" plugins enable "$KITCHEN_SINK_ID"
  run_kitchen_sink_operator_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-plugins.json" plugins list --json
  run_kitchen_sink_operator_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-inspect.json" plugins inspect "$KITCHEN_SINK_ID" --runtime --json
  run_kitchen_sink_operator_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-inspect-all.json" plugins inspect --all --runtime --json
  assert_kitchen_sink_installed
  if [ "$KITCHEN_SINK_SOURCE" = "clawhub" ]; then
    run_kitchen_sink_operator_logged "kitchen-sink-uninstall-${KITCHEN_SINK_LABEL}" plugins uninstall "$KITCHEN_SINK_SPEC" --force
  else
    run_kitchen_sink_operator_logged "kitchen-sink-uninstall-${KITCHEN_SINK_LABEL}" plugins uninstall "$KITCHEN_SINK_ID" --force
  fi
  remove_kitchen_sink_channel_config
  run_kitchen_sink_operator_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-uninstalled.json" plugins list --json
  assert_kitchen_sink_removed
}

run_failure_scenario() {
  echo "Testing expected ${KITCHEN_SINK_LABEL} install failure from ${KITCHEN_SINK_SPEC}..."
  run_expect_failure "install-${KITCHEN_SINK_LABEL}" operator_e2e_maybe_timeout "$KITCHEN_SINK_CLI_TIMEOUT" node "$OPENCLAW_ENTRY" plugins install "$KITCHEN_SINK_SPEC" --force
  remove_kitchen_sink_channel_config
  run_kitchen_sink_operator_capture "${KITCHEN_SINK_TMP_DIR}/kitchen-sink-${KITCHEN_SINK_LABEL}-uninstalled.json" plugins list --json
  assert_kitchen_sink_removed
}
