...
function assertOnboardState() {
  ...
  if (!fs.existsSync(configPath)) {
    throw new Error("onboard did not write operator.json");
  }
  ...
  assertOpenAiEnvAuthProfileStore(authStoreText, {
    ...
    rawKeyNeedle: "sk-operator-npm-onboard-e2e",
  });
}
...
function assertMockModelConfig() {
  ...
  if (provider?.agentRuntime?.id !== "@gabrielvfonseca/operator") {
  ...
  if (defaultRuntime !== "@gabrielvfonseca/operator") {
  ...
  if (agent && agentRuntime !== "@gabrielvfonseca/operator") {
...
function assertAgentTurn() {
  ...
  assertAgentReplyContainsMarker(marker, "/tmp/operator-agent.combined");
