...
export OPENAI_API_KEY="sk-operator-release-upgrade-user-journey"
...
if ! operator_e2e_maybe_timeout "${OPENCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g "$BASELINE_SPEC" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
  cat "$BASELINE_INSTALL_LOG" >&2 || true
  exit 1
fi
command -v operator >/dev/null
baseline_root="$(operator_e2e_package_root)"
baseline_entry="$(operator_e2e_package_entrypoint "$baseline_root")"
...
operator plugins install "$plugin_dir" --force >"$PLUGIN_INSTALL_LOG" 2>&1
operator release-upgrade ping >"$PLUGIN_CLI_BEFORE_LOG" 2>&1
...
operator_e2e_install_package "$CANDIDATE_INSTALL_LOG" "candidate Operator package"
...
operator release-upgrade ping >"$PLUGIN_CLI_AFTER_LOG" 2>&1
