...
start_gateway() {
  GATEWAY_PID="$(operator_e2e_start_gateway "$OPENCLAW_ENTRY" 18789 "$GATEWAY_LOG_PATH")"
}

wait_for_gateway() {
  local wait_attempts
  wait_attempts="$(operator_e2e_read_positive_int_env OPENCLAW_ONBOARD_GATEWAY_WAIT_ATTEMPTS 20)" || return $?
  local wait_interval_s="${OPENCLAW_ONBOARD_GATEWAY_WAIT_INTERVAL_S:-1}"
  local saw_listening_log="false"
  for _ in $(seq 1 "$wait_attempts"); do
    if operator_e2e_probe_tcp 127.0.0.1 18789 500 >/dev/null 2>&1; then
      return 0
    fi
...
cleanup_wizard_case() {
  ...
  operator_e2e_stop_process "${wizard_pid:-}"
  stop_gateway "${gw_pid:-}"
...
run_wizard_cmd() {
  ...
  set_isolated_operator_env "$state_ref"
  ...
  operator_e2e_run_script_with_pty "$command" "$log_path" <"$input_fifo" >/dev/null 2>&1 &
  wizard_pid=$!
...
assert_onboard_config() {
  local scenario="$1"
  shift
  operator_e2e_assert_file "$OPENCLAW_CONFIG_PATH"
  node scripts/e2e/lib/onboard/assert-config.mjs "$scenario" "$OPENCLAW_CONFIG_PATH" "$@"
}

set_isolated_operator_env() {
  local state_ref="$1"
  operator_test_state_create "$state_ref" empty
}
...
run_case_local_basic() {
  set_isolated_operator_env local-basic
  operator_e2e_run_logged local-basic node "$OPENCLAW_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
  ...
  operator_e2e_assert_dir "$sessions_dir"
  for file in AGENTS.md BOOTSTRAP.md IDENTITY.md SOUL.md TOOLS.md USER.md; do
    operator_e2e_assert_file "$workspace_dir/$file"
  done
...
run_case_remote_non_interactive() {
  set_isolated_operator_env remote-non-interactive
  # Smoke test non-interactive remote config write.
  operator_e2e_run_logged remote-non-interactive node "$OPENCLAW_ENTRY" onboard --non-interactive --accept-risk \
    --mode remote \
...
run_case_reset() {
  set_isolated_operator_env reset-config
  node scripts/e2e/lib/onboard/write-config.mjs reset "$OPENCLAW_CONFIG_PATH"

  operator_e2e_run_logged reset-config node "$OPENCLAW_ENTRY" onboard \
    --non-interactive \
    --accept-risk \
    --flow quickstart \
    --mode local \
...
run_case_skills() {
  local home_dir
  set_isolated_operator_env skills
  home_dir="$HOME"
