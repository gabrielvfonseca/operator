...
export OPENAI_API_KEY="sk-operator-release-media-memory"
...
media_root="$(mktemp -d /tmp/operator-release-media-memory.XXXXXX)"
...
mock_pid="$(operator_e2e_start_mock_openai "$MOCK_PORT" "$MOCK_OPENAI_LOG")"
...
operator plugins list --json >"$PLUGINS_JSON" 2>"$PLUGINS_STDERR_LOG"
...
operator memory index --force >"$INDEX_LOG" 2>&1
operator memory search "$MEMORY_MARKER" --json >"$SEARCH_BEFORE_JSON" 2>"$SEARCH_BEFORE_STDERR_LOG"
...
operator memory search "$MEMORY_MARKER" --json >"$SEARCH_AFTER_JSON" 2>"$SEARCH_AFTER_STDERR_LOG"
