...
export OPENAI_API_KEY="sk-operator-release-user-journey"
...
cleanup() {
  ...
  operator_e2e_stop_process "${clickclack_pid:-}"
...
start_gateway() {
  local log_path="$1"
  gateway_pid="$(operator_e2e_start_gateway "$entry" "$PORT" "$log_path")"
  operator_e2e_wait_gateway_ready "$gateway_pid" "$log_path" 300 "$PORT"
}
...
operator_e2e_install_package "$INSTALL_LOG"
...
mock_pid="$(operator_e2e_start_mock_openai "$MOCK_PORT" "$OPENAI_LOG")"
...
for _ in $(seq 1 100); do
  if operator_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200 >/dev/null 2>&1; then
    break
  fi
  sleep 0.1
done
operator_e2e_probe_http_status "http://127.0.0.1:$CLICKCLACK_PORT/health" 200
...
operator plugins install "$plugin_a_dir" --force >"$PLUGIN_A_INSTALL_LOG" 2>&1
node scripts/e2e/lib/release-user-journey/assertions.mjs \
  ...
  "$plugin_a_dir"
operator journey-a ping >"$PLUGIN_A_CLI_LOG" 2>&1
...
operator plugins uninstall journey-plugin-a --force >"$PLUGIN_A_UNINSTALL_LOG" 2>&1
node scripts/e2e/lib/release-user-journey/assertions.mjs \
...
operator plugins install "$plugin_b_dir" --force >"$PLUGIN_B_INSTALL_LOG" 2>&1
operator journey-b ping >"$PLUGIN_B_CLI_LOG" 2>&1
...
operator plugins install "$clickclack_plugin_dir" --force >"$CLICKCLACK_PLUGIN_INSTALL_LOG" 2>&1
...
operator channels status --json >"$STATUS_JSON" 2>"$STATUS_ERR"
...
operator plugins inspect journey-plugin-b --runtime --json >"$PLUGIN_B_AFTER_RESTART_JSON" 2>&1
operator channels status --json >"$STATUS_AFTER_RESTART_JSON" 2>"$STATUS_AFTER_RESTART_ERR"
...
operator doctor --repair --non-interactive >"$DOCTOR_LOG" 2>&1
