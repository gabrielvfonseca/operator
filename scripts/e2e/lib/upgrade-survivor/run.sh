...
ARTIFACT_ROOT="$(dirname "${OPENCLAW_UPGRADE_SURVIVOR_SUMMARY_JSON:-/tmp/operator-upgrade-survivor-artifacts/summary.json}")"
...
validate_baseline_package_spec() {
  ...
  echo "OPENCLAW_UPGRADE_SURVIVOR_BASELINE must be operator@latest, operator@beta, operator@alpha, an exact Operator release version, or a bare release version; got: $spec" >&2
  return 1
}
...
normalize_baseline() {
  ...
  case "$raw" in
    operator@*)
      baseline_spec="$raw"
      baseline_version="${raw#operator@}"
      ;;
    *@*)
      echo "OPENCLAW_UPGRADE_SURVIVOR_BASELINE must be operator@<version> or a bare version" >&2
      return 1
      ;;
    *)
      baseline_version="$raw"
      baseline_spec="operator@$raw"
      ;;
  esac
  case "$baseline_version" in
    latest | beta | alpha)
      ...
      ;;
    dev | main | "")
      echo "OPENCLAW_UPGRADE_SURVIVOR_BASELINE must be operator@latest, operator@beta, operator@alpha, operator@<version>, or a bare version" >&2
      return 1
      ;;
    *)
...
cleanup() {
  ...
  if [ -s "$SYSTEMCTL_SHIM_PID_FILE" ]; then
    ...
    if [[ "$shim_pid" =~ ^[0-9]+$ ]] && [ "$shim_pid" -gt 1 ]; then
      operator_e2e_terminate_gateways "$shim_pid"
    fi
  fi
}
...
package_root() {
  printf '%s/lib/node_modules/operator\n' "$npm_config_prefix"
}
...
legacy_runtime_deps_symlink_target() {
  local plugin="$1"
  printf '%s/@operator-upgrade-survivor/%s-runtime-dep\n' "$(dirname "$(package_root)")" "$plugin"
}
...
seed_source_only_plugin_shadow() {
  ...
  local shadow_root="$OPENCLAW_STATE_DIR/extensions/opik-operator"
  mkdir -p "$shadow_root/src"
  cat >"$shadow_root/package.json" <<'JSON'
{
  "name": "@opik/opik-operator",
...
JSON
  cat >"$shadow_root/operator.plugin.json" <<'JSON'
{
  "id": "opik-operator",
  "activation": {
    "onStartup": false
...
legacy_plugin_dependency_probe_paths() {
  ...
  while IFS= read -r plugin_dir; do
    printf '%s\n' \
      ...
      "$plugin_dir/.operator-runtime-deps.json" \
      "$plugin_dir/.operator-runtime-deps-stamp.json" \
      "$plugin_dir/.operator-runtime-deps-copy-upgrade-survivor" \
      "$plugin_dir/.operator-install-stage-upgrade-survivor" \
      "$plugin_dir/.operator-pnpm-store"
...
seed_legacy_plugin_dependency_debris() {
  ...
  for plugin in $(plugin_deps_cleanup_plugins); do
    ...
    found=1
    mkdir -p \
      "$plugin_dir/node_modules/operator-upgrade-survivor-dep" \
      "$plugin_dir/.operator-runtime-deps-copy-upgrade-survivor/node_modules/operator-upgrade-survivor-dep" \
      "$plugin_dir/.operator-install-stage-upgrade-survivor" \
      "$plugin_dir/.operator-pnpm-store" \
      "$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/operator-upgrade-survivor-dep" \
      "$OPENCLAW_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/operator-upgrade-survivor-dep" \
      "$OPENCLAW_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/operator-upgrade-survivor-dep"
    printf '{"name":"operator-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/node_modules/operator-upgrade-survivor-dep/package.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup"}\n' "$plugin" \
      >"$plugin_dir/.operator-runtime-deps.json"
    printf '{"plugin":"%s","scenario":"plugin-deps-cleanup","stale":true}\n' "$plugin" \
      >"$plugin_dir/.operator-runtime-deps-stamp.json"
    printf '{"name":"operator-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$plugin_dir/.operator-runtime-deps-copy-upgrade-survivor/node_modules/operator-upgrade-survivor-dep/package.json"
    printf '{"name":"operator-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$(package_root)/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/operator-upgrade-survivor-dep/package.json"
    printf '{"name":"operator-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$OPENCLAW_STATE_DIR/.local/bundled-plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/operator-upgrade-survivor-dep/package.json"
    printf '{"name":"operator-upgrade-survivor-dep","version":"0.0.0"}\n' \
      >"$OPENCLAW_STATE_DIR/plugin-runtime-deps/$plugin-upgrade-survivor/node_modules/operator-upgrade-survivor-dep/package.json"
...
seed_legacy_runtime_deps_symlink() {
  ...
  printf '{"name":"operator-upgrade-survivor-legacy-runtime-deps","version":"0.0.0"}\n' \
...
install_baseline() {
  ...
  if ! operator_e2e_maybe_timeout "${OPENCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix "$npm_config_prefix" "$baseline_spec" --no-fund --no-audit >"$BASELINE_INSTALL_LOG" 2>&1; then
    echo "baseline npm install failed" >&2
    operator_e2e_print_log "$BASELINE_INSTALL_LOG" >&2
    return 1
  fi
  if ! command -v operator >/dev/null; then
    echo "baseline install did not expose operator on PATH" >&2
    echo "PATH=$PATH" >&2
  ...
  if ! version_output="$(operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" operator --version 2>&1)"; then
    echo "baseline operator --version failed" >&2
    echo "$version_output" >&2
    return 1
  fi
  if [[ "$version_output" != *"$baseline_version"* ]]; then
    echo "baseline operator --version mismatch: expected output to include $baseline_version" >&2
    echo "$version_output" >&2
    return 1
  fi
}
...
seed_state() {
  operator_e2e_eval_test_state_from_b64 "${OPENCLAW_TEST_STATE_FUNCTION_B64:?missing OPENCLAW_TEST_STATE_FUNCTION_B64}"
  if [ "$ROOT_MANAGED_VPS" = "1" ]; then
    if [ "$(id -u)" -ne 0 ]; then
      echo "root-managed VPS survivor mode must run as uid 0" >&2
      return 1
    fi
    rm -rf /root/.operator /root/workspace
    operator_test_state_create /root minimal
  else
    operator_test_state_create "$STATE_HOME_ROOT" minimal
  fi
...
validate_baseline_config() {
  if ! operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" operator config validate >"$BASELINE_CONFIG_VALIDATE_LOG" 2>&1; then
    echo "generated baseline config failed baseline validation" >&2
    operator_e2e_print_log "$BASELINE_CONFIG_VALIDATE_LOG" >&2
    return 1
  fi
}
...
install_update_restart_service_unit() {
  if ! operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" env -u OPENCLAW_GATEWAY_TOKEN -u OPENCLAW_GATEWAY_PASSWORD operator gateway install --force --json >"$BASELINE_SERVICE_INSTALL_JSON" 2>"$BASELINE_SERVICE_INSTALL_ERR"; then
    echo "baseline gateway service install failed" >&2
    operator_e2e_print_log "$BASELINE_SERVICE_INSTALL_ERR" >&2
    operator_e2e_print_log "$BASELINE_SERVICE_INSTALL_JSON" >&2
    return 1
  fi
}
...
update_candidate() {
  ...
  if ! operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${update_env[@]}" operator "${update_args[@]}" >"$UPDATE_JSON" 2>"$UPDATE_ERR"; then
    echo "operator update failed" >&2
    operator_e2e_print_log "$UPDATE_ERR" >&2
    operator_e2e_print_log "$UPDATE_JSON" >&2
    return 1
  fi
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
assert_root_managed_vps_cli_usable() {
  ...
  operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" operator config file >"$ARTIFACT_ROOT/root-vps-config-file.out" 2>"$ARTIFACT_ROOT/root-vps-config-file.err"
  operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" "${root_cli_env[@]}" operator plugins >"$ARTIFACT_ROOT/root-vps-plugins.out" 2>"$ARTIFACT_ROOT/root-vps-plugins.err"
}

run_doctor() {
  if ! operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" operator doctor --fix --non-interactive >"$DOCTOR_LOG" 2>&1; then
    echo "operator doctor failed" >&2
    operator_e2e_print_log "$DOCTOR_LOG" >&2
    return 1
  fi
}

validate_post_doctor_config() {
  if ! operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" operator config validate >>"$DOCTOR_LOG" 2>&1; then
...
start_gateway() {
  ...
  local ready_epoch
  start_epoch="$(node -e "process.stdout.write(String(Date.now()))")"
  env -u OPENCLAW_GATEWAY_TOKEN -u OPENCLAW_GATEWAY_PASSWORD operator gateway --port "$port" --bind loopback --allow-unconfigured >"$GATEWAY_LOG" 2>&1 &
  gateway_pid="$!"
  if [ "$UPDATE_RESTART_MODE" = "auto-auth" ]; then
...
check_gateway_status() {
  ...
  if ! operator_e2e_maybe_timeout "$COMMAND_TIMEOUT" operator gateway status --url "ws://127.0.0.1:$port" --token "$GATEWAY_AUTH_TOKEN_REF" --require-rpc --timeout 30000 --json >"$STATUS_JSON" 2>"$STATUS_ERR"; then
    echo "gateway status failed" >&2
    operator_e2e_print_log "$STATUS_ERR" >&2
    operator_e2e_print_log "$GATEWAY_LOG" >&2
    return 1
  fi
  ...
  if [ "$status_seconds" -gt "$budget" ]; then
    ...
    operator_e2e_print_log "$STATUS_JSON" >&2
    return 1
  fi
