...
operator_e2e_install_package /tmp/operator-install.log "mounted Operator package" /tmp/npm-prefix
...
registry_port_file=/tmp/operator-e2e-registry.port
rm -f "$registry_port_file"
node scripts/e2e/lib/plugin-update/registry-server.mjs "$registry_port_file" >/tmp/operator-e2e-registry.log 2>&1 &
registry_pid=$!
trap 'operator_e2e_stop_process "${registry_pid:-}"' EXIT
...
if [ ! -s "$registry_port_file" ]; then
  ...
  operator_e2e_print_log /tmp/operator-e2e-registry.log
  exit 1
fi
...
plugin_update_timeout_seconds="$(operator_e2e_read_positive_int_env OPENCLAW_PLUGIN_UPDATE_TIMEOUT_SECONDS 180)"

node "$probe" snapshot > /tmp/plugin-update-before.json

set +e
operator_e2e_maybe_timeout "${plugin_update_timeout_seconds}s" node "$entry" plugins update @example/lossless-claw > /tmp/plugin-update-output.log 2>&1
...
if [ "$plugin_update_status" -ne 0 ]; then
  ...
  operator_e2e_print_log /tmp/plugin-update-output.log
