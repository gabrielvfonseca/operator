...
export OPENCLAW_CODEX_MEDIA_PATH_APP_SERVER_LOG="/tmp/operator-codex-media-path-app-server.jsonl"
...
GATEWAY_LOG="/tmp/operator-codex-media-path-gateway.log"
CLIENT_LOG="/tmp/operator-codex-media-path-client.log"
PLUGIN_INSTALL_LOG="/tmp/operator-codex-media-path-plugin-install.log"
PLUGIN_INSPECT_LOG="/tmp/operator-codex-media-path-plugin-inspect.json"
gateway_pid=""
...
dump_debug_logs() {
  ...
  operator_e2e_dump_logs "$PLUGIN_INSTALL_LOG" "$PLUGIN_INSPECT_LOG" "$GATEWAY_LOG" "$CLIENT_LOG" "$OPENCLAW_CODEX_MEDIA_PATH_APP_SERVER_LOG"
}
...
operator plugins install "$PLUGIN_SPEC" --force >"$PLUGIN_INSTALL_LOG" 2>&1
operator plugins inspect codex --runtime --json >"$PLUGIN_INSPECT_LOG"

node scripts/e2e/lib/codex-media-path/write-config.mjs

gateway_pid="$(operator_e2e_start_gateway "$entry" "$PORT" "$GATEWAY_LOG")"
operator_e2e_wait_gateway_ready "$gateway_pid" "$GATEWAY_LOG" 480 "$PORT"

PORT="$PORT" OPENCLAW_GATEWAY_TOKEN="$TOKEN" \
  tsx scripts/e2e/lib/codex-media-path/client.mjs >"$CLIENT_LOG" 2>&1

operator_e2e_print_log "$CLIENT_LOG"
