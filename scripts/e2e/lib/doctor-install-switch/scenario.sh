...
export PATH="/tmp/operator-bin:$PATH"
mkdir -p /tmp/operator-bin
cp scripts/e2e/lib/doctor-install-switch/shims/systemctl /tmp/operator-bin/systemctl
cp scripts/e2e/lib/doctor-install-switch/shims/loginctl /tmp/operator-bin/loginctl
chmod +x /tmp/operator-bin/systemctl /tmp/operator-bin/loginctl
...
(
  cd "$git_root"
  operator_e2e_maybe_timeout "${OPENCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install --omit=optional --no-fund --no-audit >/tmp/operator-git-install.log 2>&1
  git init -q
  git config user.email "docker-e2e@operator.local"
  git config user.name "Operator Docker E2E"
  git add -A --
  git commit -qm "test fixture"
)
npm_log="/tmp/operator-doctor-switch-npm-install.log"
if ! operator_e2e_maybe_timeout "${OPENCLAW_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$package_tgz" >"$npm_log" 2>&1; then
  operator_e2e_print_log "$npm_log"
  exit 1
fi

npm_bin="/tmp/npm-prefix/bin/operator"
npm_root="/tmp/npm-prefix/lib/node_modules/operator"
...
run_flow() {
  ...
  local install_log="/tmp/operator-doctor-switch-${name}-install.log"
  local doctor_log="/tmp/operator-doctor-switch-${name}-doctor.log"
  local command_timeout="${OPENCLAW_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"

  echo "== Flow: $name =="
  operator_test_state_create "switch-${name}" empty
  export USER="testuser"

  if ! operator_e2e_maybe_timeout "$command_timeout" bash -c "$install_cmd" >"$install_log" 2>&1; then
    operator_e2e_print_log "$install_log"
    exit 1
  fi
  rm -f "$HOME/.zshrc" "$HOME/.bashrc" "$HOME/.bash_profile"
  rm -rf "$HOME/.config/fish" "$HOME/.config/powershell"

  unit_path="$HOME/.config/systemd/user/operator-gateway.service"
  if [ ! -f "$unit_path" ]; then
  ...
  if ! operator_e2e_maybe_timeout "$command_timeout" bash -c "$doctor_cmd" >"$doctor_log" 2>&1; then
    operator_e2e_print_log "$doctor_log"
    exit 1
  fi
...
run_cross_state_approval_flow() {
  ...
  local automated_log="/tmp/operator-doctor-switch-${name}-automated.log"
  local direct_log="/tmp/operator-doctor-switch-${name}-direct.log"
  local command_timeout="${OPENCLAW_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  local default_state_dir="$HOME/.operator"
  ...
  local state_database="$custom_state_dir/state/operator.sqlite"
  ...
  if ! operator_e2e_maybe_timeout "$command_timeout" env \
    OPENCLAW_STATE_DIR="$custom_state_dir" \
    OPENCLAW_CONFIG_PATH="$custom_state_dir/operator.json" \
    OPENCLAW_UPDATE_IN_PROGRESS=1 \
    "$npm_bin" doctor --repair --yes --non-interactive >"$automated_log" 2>&1; then
    operator_e2e_print_log "$automated_log"
    exit 1
  fi
  ...
  if ! operator_e2e_maybe_timeout "$command_timeout" env \
    ...
    operator_e2e_print_log "$direct_log"
    exit 1
  fi
...
run_wrapper_flow() {
  ...
  local install_log="/tmp/operator-doctor-switch-${name}-install.log"
  local reinstall_log="/tmp/operator-doctor-switch-${name}-reinstall.log"
  local env_repair_log="/tmp/operator-doctor-switch-${name}-env-repair.log"
  local doctor_log="/tmp/operator-doctor-switch-${name}-doctor.log"
  local clear_log="/tmp/operator-doctor-switch-${name}-clear.log"
  local command_timeout="${OPENCLAW_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  local wrapper="$HOME/.local/bin/operator-wrapper"
  node scripts/e2e/lib/doctor-install-switch/write-wrapper.mjs \
    "$wrapper" \
    "$npm_bin" \
    "$HOME/operator-wrapper-argv.log"

  local unit_path="$HOME/.config/systemd/user/operator-gateway.service"

  if ! operator_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" --force >"$install_log" 2>&1; then
    operator_e2e_print_log "$install_log"
    exit 1
  fi
  ...
  if ! operator_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --force >"$reinstall_log" 2>&1; then
    operator_e2e_print_log "$reinstall_log"
    exit 1
  fi
  assert_exec_arg "$unit_path" 1 "$wrapper"
  assert_exec_arg "$unit_path" 2 "gateway"
  ...
  if ! operator_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
    operator_e2e_print_log "$env_repair_log"
    exit 1
  fi
  assert_exec_arg "$unit_path" 1 "$wrapper"
  assert_env_value "$unit_path" "OPENCLAW_WRAPPER" "$wrapper"

  sed -i "s#^Environment=OPENCLAW_WRAPPER=.*#Environment=OPENCLAW_WRAPPER=/tmp/stale-operator-wrapper#" "$unit_path"
  if ! operator_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
    operator_e2e_print_log "$env_repair_log"
    exit 1
  fi
  ...
  if ! operator_e2e_maybe_timeout "$command_timeout" node "$git_cli" doctor --repair --force --yes >"$doctor_log" 2>&1; then
    operator_e2e_print_log "$doctor_log"
    exit 1
  fi
  ...
  if ! operator_e2e_maybe_timeout "$command_timeout" env OPENCLAW_WRAPPER= "$npm_bin" gateway install --force >"$clear_log" 2>&1; then
    operator_e2e_print_log "$clear_log"
    exit 1
  fi
