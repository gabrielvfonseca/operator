...
const runWithTimeout = async (timeout, command, commandArgs) => {
  const killGrace = parsePositiveNumber(
    process.env.OPERATOR_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS ??
      String(DEFAULT_TIMEOUT_KILL_GRACE_MS),
    "OPERATOR_BUN_GLOBAL_SMOKE_TIMEOUT_KILL_GRACE_MS",
  );
...
if (mode === "assert-image-providers") {
  const raw = process.env.OPERATOR_IMAGE_PROVIDERS_JSON ?? "";
  let parsed;
  try {
...
if (mode === "assert-release-versions") {
  ...
  const rootAiVersion = rootManifest.dependencies?.["@gabrielvfonseca/ai"];
  if (
