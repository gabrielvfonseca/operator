...
operator_plugins_read_positive_int_env() {
...
operator_plugins_read_nonnegative_decimal_env() {
  local name="${1:?missing environment variable name}"
  local fallback="${2:?missing fallback value}"
  local value="${!name-}"
  if [[ -z "${!name+x}" ]]; then
...
operator_plugins_cleanup_fixture_servers() {
  ...
  for pid_file in "${OPENCLAW_PLUGINS_FIXTURE_PID_FILES[@]:-}"; do
    ...
    if [[ "$pid" =~ ^[0-9]+$ ]]; then
      operator_plugins_stop_fixture_process "$pid"
    fi
    rm -f "$pid_file"
  done
}

operator_plugins_signal_fixture_process() {
  local pid="$1"
  local signal="$2"
...
operator_plugins_fixture_process_alive() {
  local pid="$1"
  kill -0 "$pid" >/dev/null 2>&1 || kill -0 -- "-$pid" >/dev/null 2>&1
}

operator_plugins_stop_fixture_process() {
  local pid="$1"
  local _
  local attempts interval
  attempts="$(operator_plugins_read_positive_int_env OPENCLAW_PLUGINS_FIXTURE_STOP_ATTEMPTS 40)" || return $?
  interval="$(operator_plugins_read_nonnegative_decimal_env OPENCLAW_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS 0.25)" || return $?
  if declare -F operator_e2e_stop_process >/dev/null 2>&1; then
    operator_e2e_stop_process "$pid"
    return
  fi
  operator_plugins_signal_fixture_process "$pid" TERM
  for _ in $(seq 1 "$attempts"); do
    ! operator_plugins_fixture_process_alive "$pid" && { wait "$pid" >/dev/null 2>&1 || true; return; }
    sleep "$interval"
  done
  operator_plugins_signal_fixture_process "$pid" KILL
  wait "$pid" >/dev/null 2>&1 || true
}

operator_plugins_print_fixture_log() {
  local log_file="$1"
  ...
  local max_bytes
  max_bytes="$(operator_plugins_read_positive_int_env OPENCLAW_DOCKER_E2E_LOG_PRINT_BYTES 65536)" || return $?
...
operator_plugins_validate_fixture_log_print_bytes() {
  operator_plugins_read_positive_int_env OPENCLAW_DOCKER_E2E_LOG_PRINT_BYTES 65536 >/dev/null
}

operator_plugins_register_fixture_pid_file() {
  local pid_file="$1"
  OPENCLAW_PLUGINS_FIXTURE_PID_FILES+=("$pid_file")
  operator_plugins_install_fixture_cleanup_trap
}
...
operator_plugins_install_fixture_cleanup_trap() {
  ...
  if [[ -n "$existing_trap" && "$existing_trap" != *operator_plugins_fixture_exit_trap* ]]; then
  ...
  trap operator_plugins_fixture_exit_trap EXIT
}

operator_plugins_fixture_exit_trap() {
  local status="$?"
  operator_plugins_cleanup_fixture_servers
...
packageJson.operator.extensions = ["./index.js", " "];
...
start_npm_fixture_registry() {
  ...
  for _ in $(seq 1 100); do
    ...
    if ! kill -0 "$server_pid" 2>/dev/null; then
      operator_plugins_print_fixture_log "$server_log"
      return 1
    fi
    sleep 0.1
  done
