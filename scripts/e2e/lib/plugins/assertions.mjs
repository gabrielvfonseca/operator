...
const scratchRoot = process.env.OPERATOR_PLUGINS_TMP_DIR || os.tmpdir();
...
function readClawHubPreflightLimits() {
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_BODY_MAX_BYTES",
      1024 * 1024,
    ),
    timeoutMs: readPositiveIntEnv("OPERATOR_PLUGINS_E2E_CLAWHUB_PREFLIGHT_TIMEOUT_MS", 30_000),
  };
}
...
function getInstallRecords() {
  ...
  const allowLegacyCompat = process.env.OPERATOR_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
function openClawConfigPath() {
  return path.join(process.env.HOME, ".operator", "operator.json");
}

function readOperatorConfig() {
  const configPath = openClawConfigPath();
  return fs.existsSync(configPath) ? readRequiredOperatorConfig() : {};
}

function readRequiredOperatorConfig() {
  const configPath = openClawConfigPath();
  try {
    return readJson(configPath);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`failed to read Operator config ${configPath}: ${message}`, { cause: error });
  }
}
...
function assertGitPlugin() {
  ...
  const gitRoot = path.join(process.env.HOME, ".operator", "git");
...
function assertNpmPlugin() {
  ...
  if (record.spec !== "@gabrielvfonseca/demo-plugin-npm@0.0.1") {
    throw new Error(`unexpected npm spec: ${record.spec}`);
  }
  if (record.resolvedName !== "@gabrielvfonseca/demo-plugin-npm") {
...
function assertInvalidOperatorExtensionsRejected() {
  const pluginId = "demo-plugin-invalid-metadata";
  for (const expected of ["operator.extensions[1]", "non-empty string"]) {
    assertTextFileIncludes(
      scratchFile("plugins-invalid-operator-extensions.log"),
      expected,
      "malformed metadata install output",
    );
  }

  const list = readJson(scratchFile("plugins-invalid-operator-extensions-list.json"));
  ...
  const managedInstallPath = path.join(process.env.HOME, ".operator", "extensions", pluginId);
...
async function assertClawHubPreflight() {
  ...
  const baseUrl = (
    process.env.OPERATOR_CLAWHUB_URL ||
...
function assertClawHubRemoved() {
  ...
  const configAfterUninstallPath = path.join(process.env.HOME, ".operator", "operator.json");
...
const commands = {
  ...
  "invalid-operator-extensions": assertInvalidOperatorExtensionsRejected,
