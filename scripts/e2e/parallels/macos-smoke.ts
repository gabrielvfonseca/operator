...
// Macos Smoke script supports Operator repository automation.
...
const defaultOptions = (): MacosOptions => ({
  ...
  installUrl: "https://operator.ai/install.sh",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://operator.ai/install.sh
...
class MacosSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await makeTempDir("operator-parallels-macos.");
    ...
    this.tgzDir = await makeTempDir("operator-parallels-macos-tgz.");
    try {
      ...
      if (await this.needsHostTgz()) {
        this.artifact = await packOperator({
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
      this.installMain("operator-main-fresh.tgz"),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    if (this.options.targetPackageSpec) {
      await this.phase("upgrade.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
        this.installMain("operator-main-upgrade.tgz"),
      );
  ...
  private guestOperatorEntryExec(
    args: string[],
    options: { check?: boolean; env?: Record<string, string> } = {},
  ): string {
...
exec node "$entry" ${argv}`,
  ...
  private resetState(): void {
    this.guestSh(String.raw`/usr/bin/pkill -f 'operator.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'operator-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'operator.mjs gateway' >/dev/null 2>&1 || true
...
rm -f /tmp/operator-parallels-macos-gateway.log`);
  ...
  private installLatestRelease(): void {
    this.guestSh(
      `export OPERATOR_NO_ONBOARD=1
curl -fsSL --connect-timeout 10 --max-time 120 --retry 2 --retry-delay 2 ${shellQuote(
        this.options.installUrl,
      )} -o /tmp/operator-install.sh
bash /tmp/operator-install.sh --version ${shellQuote(this.installVersion)}
${guestOperator} --version`,
    );
  }
...
${guestOperator} --version`);
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec([guestOperator, "--version"]);
...
const configPath = path.join(process.env.HOME || ${JSON.stringify(home)}, ".operator", "operator.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
config.update = { ...(config.update || {}), channel: "dev" };
fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + "\\n");
JS
/usr/bin/env NODE_OPTIONS=--max-old-space-size=8192 OPERATOR_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 OPERATOR_DISABLE_BUNDLED_PLUGINS=1 ${guestOperatorEntryRunner} update --channel dev --yes --json
${guestOperatorEntryRunner} --version
${guestOperatorEntryRunner} update status --json`,
    );
  }

  private verifyDevChannelUpdate(): void {
    const status = this.guestOperatorEntryExec(["update", "status", "--json"]);
  ...
  private startManualGatewayIfNeeded(): void {
    ...
    this.guestSh(
      `set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'operator.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'operator-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'operator.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env HOME=${shellQuote(home)} USER=${shellQuote(this.guestUser)} LOGNAME=${shellQuote(this.guestUser)} PATH=${shellQuote(guestPath)} ${shellQuote(
        `${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`,
      )} OPERATOR_HOME=${shellQuote(home)} OPERATOR_STATE_DIR=${shellQuote(`${home}/.operator`)} OPERATOR_CONFIG_PATH=${shellQuote(
        `${home}/.operator/operator.json`,
      )} ${guestOperatorEntryRunner} gateway run --bind loopback --port 18789 --force </dev/null >/tmp/operator-parallels-macos-gateway.log 2>&1 &
sleep 1`,
    );
  }
  ...
  private verifyGateway(): void {
    for (let attempt = 1; attempt <= 8; attempt++) {
      const result = this.guestOperator(
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestExec([guestOperator, "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["gateway", "status", "--deep", "--require-rpc"]
      : ["gateway", "status", "--deep"];
    if (!this.guestOperator(args, false)) {
      throw new Error("gateway status failed");
    }
  }

  private guestOperator(args: string[], check: boolean): boolean {
    const result = run(
      "prlctl",
      [
        "exec",
...
while [ $SECONDS -lt $deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 http://127.0.0.1:18789/ >/tmp/operator-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>Operator Control</title>' /tmp/operator-dashboard-smoke.html >/dev/null &&
      grep -F '<operator-app></operator-app>' /tmp/operator-dashboard-smoke.html >/dev/null; then
      asset_paths="$(
        sed -nE 's/.*<(script|link)[^>]*(src|href)=["'"'"']([^"'"'"']+)["'"'"'].*/\3/p' /tmp/operator-dashboard-smoke.html |
          grep -E '(^|/)assets/' |
          grep -Ev '^(https?:)?//' |
          sort -u
      )"
...
exit 1`);
  ...
  private verifyTurn(): void {
    this.guestOperatorEntryExec(["models", "set", this.auth.modelId]);
...
JSON
${guestOperatorEntryRunner} config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    }
    this.guestOperatorEntryExec([
      "config",
      "set",
    ...
    this.guestOperatorEntryExec(["config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} ${guestOperatorEntryRunner} agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private ensureDiscordGatewayReady(): void {
    ...
    this.verifyGateway();
    const status = this.guestOperatorEntryExec(["channels", "status", "--probe", "--json"]);
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastOperatorVersionFromLog(path.join(this.runDir, `${phaseName}.log`));
  }
