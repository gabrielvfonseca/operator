// Filesystem script supports Operator repository automation.
...
const OPERATOR_VERSION_PATTERN = /Operator\s+([0-9][^\s]*)/gi;
...
export async function extractLastOperatorVersionFromLog(
  logPath: string,
  pattern = OPERATOR_VERSION_PATTERN,
  maxBytes = DEFAULT_TEXT_FILE_TAIL_BYTES,
): Promise<string> {
...
export async function makeTempDir(prefix: string): Promise<string> {
  const root =
    process.env.OPERATOR_PARALLELS_ARTIFACT_ROOT || path.join(repoRoot, ".artifacts", "parallels");
