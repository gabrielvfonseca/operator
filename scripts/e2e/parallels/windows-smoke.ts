...
// Windows Smoke script supports Operator repository automation.
...
const defaultOptions = (): WindowsOptions => ({
  ...
  installUrl: "https://operator.ai/install.ps1",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
  ...
  snapshotHint: "pre-operator-native-e2e-2026-03-12",
  targetPackageSpec: "",
  upgradeFromPackedMain: false,
  vmName: "Windows 11",
});

const windowsPortableGitPathScript = `$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'Operator\\deps') 'portable-git') ''
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-operator-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --provider <openai|anthropic|minimax>
  --model <provider/model>    Override the model used for the agent-turn smoke.
  --api-key-env <var>        Host env var name for provider API key.
  --openai-api-key-env <var> Alias for --api-key-env (backward compatible)
  --install-url <url>        Installer URL for latest release. Default: https://operator.ai/install.ps1
...
class WindowsSmoke extends SmokeRunController<WindowsOptions> {
  ...
  private gatewayRecoveryAfterMs =
    readPositiveIntEnv("OPERATOR_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S", 180) * 1000;
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("operator-parallels-windows.");
    this.phases = new PhaseRunner(this.runDir);
    this.guest = new WindowsGuest(this.options.vmName, this.phases);
    this.tgzDir = await makeTempDir("operator-parallels-windows-tgz.");
    try {
  ...
  private guestPowerShell(
    ...
    return this.guest.powershell(`${windowsOperatorResolver}\n${script}`, options);
  }
...
Invoke-Operator --version
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestPowerShell("Invoke-Operator --version");
...
${this.windowsPluginIsolationScript()}`,
  ...
  private async guestPowerShellBackground(
    ...
    await runWindowsBackgroundPowerShell({
      ...
      label,
      onLaunchRetry: warn,
      script: `${windowsOperatorResolver}\n${script}`,
...
${windowsScopedEnvFunction}
$script:OperatorUpdateExit = 0
Invoke-WithScopedEnv @{ OPERATOR_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'; OPERATOR_DISABLE_BUNDLED_PLUGINS = '1' } {
...
Invoke-Operator update status --json`,
  ...
  private gatewayAction(action: "restart" | "stop"): Promise<void> {
    return this.guestPowerShellBackground(
      ...
      `$ErrorActionPreference = 'Continue'
$PSNativeCommandUseErrorActionPreference = $false
Invoke-Operator gateway ${action}
if ($LASTEXITCODE -ne 0) { throw "gateway ${action} failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyGatewayReachable(): void {
    ...
    while (Date.now() < deadline) {
      const probe = this.guestPowerShell(
        "Invoke-Operator gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json",
      ...
      if (!recoveryTried && Date.now() - start >= this.gatewayRecoveryAfterMs) {
        ...
        this.guestPowerShell("Invoke-Operator gateway start", {
          check: false,
          timeoutMs: 120_000,
        });
        recoveryTried = true;
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestPowerShell("Invoke-Operator gateway status --help", {
      check: false,
    });
    const suffix = help.includes("--require-rpc") ? "--deep --require-rpc" : "--deep";
    this.guestPowerShell(`Invoke-Operator gateway status ${suffix}`);
  }
...
for ($attempt = 1; $attempt -le 2; $attempt++) {
  ...
  $output = Invoke-Operator @args 2>&1
...
if (-not $agentOk) { throw 'openclaw agent finished without OK response' }`,
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastOperatorVersion(this.runDir, phaseName, /Operator\s+([0-9][^\s]*)/gi);
  }
