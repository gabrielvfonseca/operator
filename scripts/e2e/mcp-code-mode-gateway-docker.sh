...
IMAGE_NAME="$(docker_e2e_resolve_image "operator-mcp-code-mode-gateway-e2e" OPENCLAW_IMAGE)"
...
CONTAINER_NAME="operator-mcp-code-mode-e2e-$$"

CLIENT_LOG="$(mktemp -t operator-mcp-code-mode-client-log.XXXXXX)"
...
docker_e2e_run_with_harness \
  ...
  bash -lc "set -euo pipefail
    ...
    mock_pid=\"\$(operator_e2e_start_mock_openai \"$MOCK_PORT\" /tmp/mcp-code-mode-mock-openai.log)\"
    gateway_pid=
    cleanup_inner() {
    ...
    gateway_pid=\"\$(operator_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-code-mode-gateway.log)\"
    operator_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-code-mode-gateway.log 480 $PORT
