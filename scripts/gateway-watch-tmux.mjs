...
const TMUX_CWD_ENV_KEY = "OPERATOR_GATEWAY_WATCH_CWD";
const TMUX_CWD_OPTION_KEY = "@operator.gateway_watch.cwd";
const TMUX_CHILD_ENV_KEYS = [
  "NODE_OPTIONS",
  "OPERATOR_CONFIG_PATH",
  "OPERATOR_DIAGNOSTICS",
  "OPERATOR_DIAGNOSTICS_EVENT_LOOP",
  "OPERATOR_DIAGNOSTICS_TIMELINE_PATH",
  "OPERATOR_GATEWAY_PORT",
  "OPERATOR_GATEWAY_RESTART_TRACE",
  "OPERATOR_GATEWAY_STARTUP_TRACE",
  "OPERATOR_GATEWAY_WATCH_AUTO_DOCTOR",
  "OPERATOR_HOME",
  "OPERATOR_PROFILE",
...
const resolveGatewayWatchPort = ({ args, env }) => {
  ...
  return {
    explicitCli: false,
    port: parsePortValue(env.OPERATOR_GATEWAY_PORT, { allowHost: true }),
  };
};
...
const resolveGatewayWatchProfile = ({ args, env }) => {
  ...
  return env.OPERATOR_PROFILE || null;
};
...
const resolveGatewayWatchBenchmarkArgs = ({ args = [], env = process.env } = {}) => {
  ...
  if (benchmarkFlagSeen) {
    ...
    nextEnv.OPERATOR_TRACE_SYNC_IO ??= "0";
    if (nextEnv.OPERATOR_TRACE_SYNC_IO === "1") {
...
export const buildGatewayWatchTmuxCommand = ({
  ...
  const childEnv = [
    ...
    `OPERATOR_GATEWAY_WATCH_TMUX_CHILD=1`,
    `OPERATOR_GATEWAY_WATCH_SESSION=${sessionName}`,
...
export const runGatewayWatchServiceHandoff = (params = {}) => {
  ...
  const stopEnv = { ...env };
  if (explicitCli) {
    stopEnv.OPERATOR_GATEWAY_PORT = String(watchPort);
  }
...
const shouldAttachTmux = ({ env, stdinIsTTY, stdoutIsTTY }) => {
  const raw = String(env.OPERATOR_GATEWAY_WATCH_ATTACH ?? "").toLowerCase();
...
export const runGatewayWatchTmuxMain = (params = {}) => {
  ...
  if (TMUX_DISABLE_VALUES.has((deps.env.OPERATOR_GATEWAY_WATCH_TMUX ?? "").toLowerCase())) {
  ...
  if (deps.env.OPERATOR_GATEWAY_WATCH_TMUX_CHILD === "1") {
    return runForegroundWatcher({
      args: deps.args,
      cwd: deps.cwd,
      env: deps.env,
