...
export function parseOperatorNpmPrepublishVerifyArgs(
  ...
  if (expectedVersion?.startsWith("-")) {
    throw new Error(`Unknown operator npm prepublish verifier option: ${expectedVersion}`);
  }
...
function main(argv = process.argv.slice(2)): void {
  ...
  try {
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== resolvedExpectedVersion) {
      errors.push(
        `installed operator binary version mismatch: expected ${resolvedExpectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    if (errors.length === 0) {
