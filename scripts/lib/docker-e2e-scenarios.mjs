...
const LIVE_DOCKER_DEFAULT_HARNESS_DIR =
  ...
  process.env.OPERATOR_DOCKER_E2E_REPO_ROOT
    ? ".release-harness"
    : ".";
...
const upgradeSurvivorCommand = "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:upgrade-survivor";
const rootManagedVpsUpgradeCommand =
  "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:root-managed-vps-upgrade";
const updateRestartAuthCommand =
  "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-restart-auth";
const CODEX_HARNESS_API_KEY_ENV = "OPERATOR_LIVE_CODEX_HARNESS_AUTH=api-key";
...
function liveDockerScriptCommand(script, envPrefix = "", options = {}) {
  ...
  const skipBuild = options.skipBuild === false ? "" : "OPERATOR_SKIP_DOCKER_BUILD=1 ";
  return `${prefix}${skipBuild}bash -c 'harness="\${OPERATOR_DOCKER_E2E_TRUSTED_HARNESS_DIR:-${LIVE_DOCKER_DEFAULT_HARNESS_DIR}}"; OPERATOR_LIVE_DOCKER_REPO_ROOT="\${OPERATOR_DOCKER_E2E_REPO_ROOT:-$PWD}" bash "$harness/scripts/${script}"'`;
}
...
function createPackageUpdateMaintenanceLanes() {
  return [
    npmLane("doctor-switch", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:doctor-switch", {
      stateScenario: "empty",
      weight: 3,
    }),
    npmLane(
      "update-channel-switch",
      "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-channel-switch",
      {
    ...
    npmLane("skill-install", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:skill-install", {
    ...
    npmLane(
      ...
      "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:published-upgrade-survivor",
      {
        stateScenario: "upgrade-survivor",
...
const bundledPluginInstallUninstallLanes = Array.from(
  ...
  (_, index) =>
    lane(
      ...
      `OPERATOR_BUNDLED_PLUGIN_SWEEP_TOTAL=${BUNDLED_PLUGIN_INSTALL_UNINSTALL_SHARDS} OPERATOR_BUNDLED_PLUGIN_SWEEP_INDEX=${index} OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:bundled-plugin-install-uninstall`,
      {
        estimateSeconds: 120,
        resources: ["npm"],
        stateScenario: "empty",
...
function livePluginToolLane() {
  return liveLane(
    "live-plugin-tool",
    "OPERATOR_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS=300 OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:live-plugin-tool",
    {
...
function liveOpenAiChatToolsLane() {
  return liveLane(
    "openai-chat-tools",
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-chat-tools",
    {
...
function liveMcpCodeModeGatewayLane() {
  return liveLane(
    ...
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:live-mcp-code-mode-gateway",
    {
...
function kitchenSinkRpcLane() {
  return serviceLane(
    "kitchen-sink-rpc",
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-rpc",
    {
      resources: ["npm"],
      stateScenario: "empty",
      timeoutMs: 25 * 60 * 1000,
...
export const mainLanes = [
  lane(
    ...
    "OPERATOR_SKIP_DOCKER_BUILD=0 pnpm test:docker:selected-plugins",
    {
      e2eImageKind: false,
      estimateSeconds: 600,
      resources: ["docker"],
  ...
  serviceLane("compose-setup", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:compose-setup", {
    stateScenario: "empty",
    timeoutMs: 20 * 60 * 1000,
    weight: 3,
  }),
  npmLane(
    ...
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:package-install",
    {
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
      weight: 3,
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "OPERATOR_IMAGE=operator:local-live-gateway OPERATOR_DOCKER_BUILD_EXTENSIONS=matrix OPERATOR_LIVE_GATEWAY_PROVIDERS=claude-cli,google-gemini-cli",
      { skipBuild: false },
    ),
    {
      providers: ["claude-cli", "google-gemini-cli"],
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-cli-backend-docker.sh",
      "OPERATOR_LIVE_CLI_BACKEND_ADVISORY=1 OPERATOR_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP=1 OPERATOR_LIVE_CLI_BACKEND_MODEL=google-gemini-cli/gemini-3-flash-preview",
    ),
    {
  ...
  liveLane("openwebui", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:openwebui", {
    e2eImageKind: "functional",
    needsLiveImage: false,
    provider: "openai",
    resources: ["service"],
  ...
  serviceLane("onboard", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:onboard", {
    stateScenario: "empty",
    weight: 2,
  }),
  npmLane("codex-on-demand", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:codex-on-demand", {
    resources: ["service"],
    stateScenario: "empty",
    weight: 3,
  }),
  serviceLane(
    "codex-media-path",
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:codex-media-path",
    {
      resources: ["npm"],
      stateScenario: "empty",
      weight: 3,
  ...
  npmLane(
    ...
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "npm-onboard-discord-channel-agent",
    "OPERATOR_NPM_ONBOARD_CHANNEL=discord OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "npm-onboard-slack-channel-agent",
    "OPERATOR_NPM_ONBOARD_CHANNEL=slack OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-onboard-channel-agent",
    { resources: ["service"], stateScenario: "empty", weight: 3 },
  ),
  npmLane(
    "release-user-journey",
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-user-journey",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  npmLane(
    ...
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-typed-onboarding",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  npmLane(
    ...
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-media-memory",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  npmLane(
    ...
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-upgrade-user-journey",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      timeoutMs: 30 * 60 * 1000,
  ...
  npmLane(
    ...
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:release-plugin-marketplace",
    {
      resources: ["npm"],
      stateScenario: "empty",
      timeoutMs: 20 * 60 * 1000,
  ...
  serviceLane("gateway-network", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:gateway-network"),
  ...
  serviceLane(
    ...
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:agents-delete-shared-workspace",
    { stateScenario: "empty" },
  ),
  serviceLane("mcp-channels", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels", {
    resources: ["npm"],
    stateScenario: "empty",
    weight: 3,
  }),
  serviceLane(
    ...
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-code-mode-gateway",
    {
      resources: ["npm"],
      stateScenario: "empty",
      weight: 3,
  ...
  lane(
    ...
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:agent-bundle-mcp-tools",
    {
      stateScenario: "empty",
    },
  ),
  lane("system-agent-rescue", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:system-agent-rescue", {
    stateScenario: "empty",
  }),
  serviceLane(
    "cron-mcp-cleanup",
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:cron-mcp-cleanup",
    { resources: ["npm"], stateScenario: "empty", weight: 3 },
  ),
  ...createPackageUpdateMaintenanceLanes(),
  npmLane("update-migration", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-migration", {
    stateScenario: "upgrade-survivor",
    timeoutMs: 30 * 60 * 1000,
    weight: 3,
  }),
  lane("plugins", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins", {
    resources: ["npm", "service"],
    stateScenario: "empty",
    weight: 6,
  }),
  lane("kitchen-sink-plugin", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin", {
    resources: ["npm"],
    stateScenario: "empty",
    weight: 3,
  }),
  ...
  lane(
    "plugins-offline",
    "OPERATOR_PLUGINS_E2E_CLAWHUB=0 OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
    {
      resources: ["npm", "service"],
      stateScenario: "empty",
      weight: 6,
  ...
  npmLane("plugin-update", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-update", {
    stateScenario: "empty",
  }),
  npmLane(
    "update-corrupt-plugin",
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:update-corrupt-plugin",
    {
      stateScenario: "empty",
      timeoutMs: 30 * 60 * 1000,
      weight: 3,
  ...
  npmLane(
    ...
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugin-lifecycle-matrix",
    {
      stateScenario: "empty",
      timeoutMs: 12 * 60 * 1000,
    },
  ),
  serviceLane("config-reload", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:config-reload", {
    stateScenario: "empty",
  }),
  npmLane("multi-node-update", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:multi-node-update", {
    stateScenario: "empty",
    timeoutMs: 15 * 60 * 1000,
    weight: 3,
  }),
  lane("openai-image-auth", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-image-auth", {
    stateScenario: "empty",
  }),
  lane(
    "system-agent-first-run",
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:system-agent-first-run",
    { stateScenario: "empty" },
  ),
  lane(
    "session-runtime-context",
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:session-runtime-context",
  ),
  lane(
    "plugin-binding-command-escape",
    "OPERATOR_SKIP_DOCKER_BUILD=0 pnpm test:docker:plugin-binding-command-escape",
    {
      e2eImageKind: false,
      resources: ["npm"],
      stateScenario: "empty",
  ...
  lane("commitments-safety", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:commitments-safety", {
    stateScenario: "empty",
  }),
  liveLane("npm-telegram-live", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:npm-telegram-live", {
    e2eImageKind: "bare",
    provider: "openai",
...
export const tailLanes = [
  serviceLane(
    ...
    "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:openai-web-search-minimal",
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "OPERATOR_LIVE_CODEX_HARNESS_AUTH=api-key OPERATOR_LIVE_CODEX_HARNESS_CHAT_IMAGE_PROBE=1 OPERATOR_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 OPERATOR_LIVE_CODEX_HARNESS_MCP_PROBE=0 OPERATOR_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0 OPERATOR_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0",
    ),
    {
      cacheKey: "codex-harness",
      provider: "openai",
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-codex-harness-docker.sh",
      `${CODEX_HARNESS_API_KEY_ENV} OPERATOR_LIVE_CODEX_BIND=1 OPERATOR_LIVE_CODEX_TEST_FILES=src/gateway/gateway-codex-bind.live.test.ts`,
    ),
    {
      cacheKey: "codex-harness",
      provider: "openai",
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "OPERATOR_LIVE_ACP_BIND_AGENT=claude"),
    {
  ...
  liveLane(
    "live-acp-bind-codex",
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "OPERATOR_LIVE_ACP_BIND_AGENT=codex"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      ...
      "OPERATOR_LIVE_ACP_BIND_AGENT=droid OPERATOR_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand("test-live-acp-bind-docker.sh", "OPERATOR_LIVE_ACP_BIND_AGENT=gemini"),
    {
  ...
  liveLane(
    ...
    liveDockerScriptCommand(
      "test-live-acp-bind-docker.sh",
      "OPERATOR_LIVE_ACP_BIND_AGENT=opencode OPERATOR_LIVE_ACP_BIND_REQUIRE_TRANSCRIPT=1",
    ),
    {
...
const primaryReleasePathChunks = {
  core: [
    lane("qr", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:qr"),
    serviceLane("onboard", "OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:onboard", {
      stateScenario: "empty",
      weight: 2,
    }),
...
export function releasePathChunkLanes(chunk, options = {}) {
  ...
  if (!base) {
    throw new Error(
      `OPERATOR_DOCKER_ALL_CHUNK must be one of: ${[
