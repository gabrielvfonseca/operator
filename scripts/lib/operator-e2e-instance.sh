...
operator_e2e_read_positive_int_env() {
...
operator_e2e_read_nonnegative_int_env() {
  local name="${1:?missing environment variable name}"
  local fallback="${2:?missing fallback value}"
  local value="${!name-}"
  if [ -z "${!name+x}" ]; then
...
operator_e2e_resolve_entrypoint() {
  local entry
...
operator_e2e_package_entrypoint() {
...
operator_e2e_install_package() {
  ...
  set +e
  operator_e2e_maybe_timeout "$timeout_value" npm install "${args[@]}" "$package_tgz" --no-fund --no-audit >"$log_file" 2>&1
...
operator_e2e_run_script_with_pty() {
  ...
  if script --version >/dev/null 2>&1; then
    ...
    operator_e2e_maybe_timeout "$timeout_value" node scripts/e2e/lib/run-with-pty.mjs "$log_path" /bin/bash -lc "$command"
  else
    operator_e2e_maybe_timeout "$timeout_value" script -q -F "$log_path" /bin/bash -lc "$command"
  fi
}
...
operator_e2e_process_alive() {
  local pid="${1:-}"
  [ -n "$pid" ] || return 1
  kill -0 "$pid" >/dev/null 2>&1 || kill -0 -- "-$pid" >/dev/null 2>&1
}
operator_e2e_stop_process() {
  local pid="${1:-}" _
  [ -n "$pid" ] || return 0
  operator_e2e_signal_process "$pid" TERM
  for _ in $(seq 1 40); do
    ! operator_e2e_process_alive "$pid" && { wait "$pid" >/dev/null 2>&1 || true; return 0; }
    sleep 0.25
  done
  operator_e2e_signal_process "$pid" KILL
  wait "$pid" >/dev/null 2>&1 || true
}
operator_e2e_terminate_gateways() {
  operator_e2e_stop_process "${1:-}"
}
operator_e2e_start_mock_openai() { operator_e2e_start_tracked_process "$2" env "MOCK_PORT=$1" node scripts/e2e/mock-openai-server.mjs; }
operator_e2e_wait_mock_openai() {
  local port="$1" attempts="${2:-80}" timeout_ms="${3:-400}" _
  local base_url="${4:-http://127.0.0.1:${port}}"
  for _ in $(seq 1 "$attempts"); do
    operator_e2e_probe_http "${base_url}/health" ok "$timeout_ms" && return 0
    sleep 0.1
  done
  operator_e2e_probe_http "${base_url}/health" ok "$timeout_ms"
}
operator_e2e_start_gateway() { operator_e2e_start_tracked_process "$3" node "$1" gateway --port "$2" --bind loopback --allow-unconfigured; }
operator_e2e_exec_gateway() { exec node "$1" gateway --port "$2" --bind "${3:-loopback}" --allow-unconfigured >"$4" 2>&1; }
operator_e2e_gateway_log_port_from_text() {
  sed -nE 's/.*(127\.0\.0\.1|localhost):([0-9]+).*/\2/p' | tail -n 1
}
operator_e2e_wait_gateway_ready() {
    ...
    if [ "$saw_ready_log" != "true" ] && [ -f "$log" ]; then
      ...
      if [ "$log_bytes" -gt "$ready_scan_offset" ]; then
        ...
        if [ -n "$ready_log_lines" ]; then
          saw_ready_log=true
          [ -n "$ready_port" ] || ready_port="$(printf "%s" "$ready_log_lines" | operator_e2e_gateway_log_port_from_text)"
        fi
      fi
    fi
...
operator_e2e_probe_http_status() {
  operator_e2e_probe_http "$1" "${2:-200}" "${3:-400}"
}
operator_e2e_assert_file() { [ -f "$1" ] || { echo "Missing file: $1"; exit 1; }; }
operator_e2e_assert_dir() { [ -d "$1" ] || { echo "Missing dir: $1"; exit 1; }; }
operator_e2e_assert_log_not_contains() {
