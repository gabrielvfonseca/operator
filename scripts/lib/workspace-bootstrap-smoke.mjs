// Verifies installed packages can bootstrap the default Operator workspace files.
...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "operator-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "operator.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".operator", "workspace");
