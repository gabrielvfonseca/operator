// Plugin Npm Release script supports Operator repository automation.
...
export function collectRequiredLatestDependencies(packageJson: PluginPackageJson): {
  ...
  const configured = packageJson.operator?.release?.requireLatestDependencies;
  ...
  if (!Array.isArray(configured)) {
    return {
      dependencies: [],
      errors: ["operator.release.requireLatestDependencies must be an array of package names."],
    };
  }
  ...
  for (const value of configured) {
    if (typeof value !== "string" || !value.trim()) {
      errors.push(
        "operator.release.requireLatestDependencies must contain only non-empty package names.",
      );
      continue;
    }
    ...
    if (seen.has(packageName)) {
      errors.push(
        `operator.release.requireLatestDependencies must not contain duplicate package names; found "${packageName}".`,
      );
      continue;
    }
    ...
    if (typeof version !== "string" || !version.trim()) {
      errors.push(
        `operator.release.requireLatestDependencies must reference package.json dependencies or optionalDependencies; "${packageName}" is not a runtime dependency.`,
      );
      continue;
    }
...
export function collectPublishablePluginPackageErrors(
  ...
  const installNpmSpec = normalizeOptionalString(packageJson.operator?.install?.npmSpec);
  const repositoryUrl =
    typeof packageJson.repository === "string"
      ? packageJson.repository.trim()
      : (packageJson.repository?.url?.trim() ?? "");
  const extensions = packageJson.operator?.extensions ?? [];
  const requiredLatestDependencies = collectRequiredLatestDependencies(packageJson);

  if (!packageName.startsWith("@gabrielvfonseca/")) {
    errors.push(
      `package name must start with "@gabrielvfonseca/"; found "${packageName || "<missing>"}".`,
    );
  }
  ...
  if (repositoryUrl !== OPERATOR_PLUGIN_NPM_REPOSITORY_URL) {
    errors.push(
      `package.json repository.url must be "${OPERATOR_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "${repositoryUrl || "<missing>"}".`,
    );
  }
  if (!packageVersion) {
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("operator.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("operator.extensions must contain only non-empty strings.");
  }
  if (!installNpmSpec) {
    errors.push("operator.install.npmSpec must be a non-empty string for publishable plugins.");
  }
...
export function collectPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    publishable.push({
      ...
      installNpmSpec: normalizeOptionalString(packageJson.operator?.install?.npmSpec),
...
function runNpmView(args: string[]): string {
  const tempDir = mkdtempSync(join(tmpdir(), "operator-plugin-npm-view-"));
