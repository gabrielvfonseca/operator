...
operator_trim_build_metadata_value() {
  local value="${1:-}"
...
operator_is_full_git_commit() {
  [[ "${1:-}" =~ ^[0-9a-fA-F]{40}$ ]]
}

operator_normalize_utc_build_timestamp() {
  local value="${1:-}"
  local LC_ALL=C
...
operator_is_utc_build_timestamp() {
  operator_normalize_utc_build_timestamp "${1:-}" >/dev/null
}

operator_resolve_git_commit() {
  local root_dir="$1"
  local candidate
  local source_name
  for source_name in GIT_COMMIT GIT_SHA; do
    candidate="$(operator_trim_build_metadata_value "${!source_name:-}")"
    [[ -n "${candidate}" ]] && break
  done
  if [[ -n "${candidate}" ]]; then
    if ! operator_is_full_git_commit "${candidate}"; then
  ...
  if [[ -n "${candidate}" ]] && ! operator_is_full_git_commit "${candidate}"; then
  ...
  if [[ -z "${candidate}" ]]; then
    candidate="$(operator_trim_build_metadata_value "${GITHUB_SHA:-}")"
    if [[ -n "${candidate}" ]] && ! operator_is_full_git_commit "${candidate}"; then
...
operator_resolve_build_timestamp() {
  local candidate
  candidate="$(operator_trim_build_metadata_value "${OPENCLAW_BUILD_TIMESTAMP:-}")"
  if [[ -n "${candidate}" ]]; then
    if ! candidate="$(operator_normalize_utc_build_timestamp "${candidate}")"; then
