...
show_usage() {
    cat << EOF
clawlog - Operator Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View Operator logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Operator logs flow through the macOS unified log (subsystem: ai.operator).
