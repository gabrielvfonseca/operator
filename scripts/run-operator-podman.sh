...
# Then:
#   ./scripts/run-operator-podman.sh launch        # Start gateway
#   ./scripts/run-operator-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   operator --container operator dashboard --no-open
#   operator --container operator channels login
#
...
run_podman_detached() {
  operator_host_timeout_cmd "$PODMAN_RUN_TIMEOUT" podman run "$@"
}
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-operator-podman.sh as your normal user so Podman stays rootless."
fi
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v operator >/dev/null 2>&1; then
    echo "Warning: operator not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    operator_CONTAINER="" operator_CONFIG_DIR="$config_dir" \
...
echo "podman exec -it $CONTAINER_NAME operator dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME operator devices approve --latest  # if pairing required"
