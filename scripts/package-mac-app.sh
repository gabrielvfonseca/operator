...
# Build and bundle Operator into a minimal .app we can open.
# Outputs to dist/Operator.app
...
DEFAULT_APP_ROOT="$ROOT_DIR/dist/Operator.app"
...
PRODUCT="Operator"
MLX_TTS_HELPER_PRODUCT="operator-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.operator.mac.debug}"
PKG_VERSION="$(cd "$ROOT_DIR" && node -p "require('./package.json').version" 2>/dev/null || echo "0.0.0")"
BUILD_CONFIG="${BUILD_CONFIG:-debug}"
BUILD_TS="$(operator_resolve_build_timestamp)"
...
if operator_is_full_git_commit "$BUILD_GIT_COMMIT"; then
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/operator/operator/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Operator/Resources/Info.plist"
...
plist_set_string_required "$APP_ROOT/Contents/Info.plist" OperatorBuildTimestamp "$BUILD_TS"
plist_set_string_required "$APP_ROOT/Contents/Info.plist" OperatorGitCommit "$BUILD_GIT_COMMIT"
if [[ "$BUILD_CONFIG" == "release" ]]; then
  EMBEDDED_GIT_COMMIT="$(plist_print_required "$APP_ROOT/Contents/Info.plist" OperatorGitCommit)"
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Operator"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Operator"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Operator"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Operator" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Operator/Resources/Operator.icns" "$APP_ROOT/Contents/Resources/Operator.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Operator/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OperatorKit resources"
...
if [ -d "$OPENCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OperatorKit_OperatorKit.bundle"
  cp -R "$OPENCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OperatorKit_OperatorKit.bundle"
else
  echo "ERROR: OperatorKit resource bundle not found at $OPENCLAWKIT_BUNDLE" >&2
  exit 1
fi

running_packaged_app_pids() {
  command -v pgrep >/dev/null 2>&1 || return 0
  local app_binary="$APP_ROOT/Contents/MacOS/Operator"
  local pid
...
stop_packaged_app_if_running() {
  ...
  echo "⏹  Stopping packaged Operator bundle (${pids[*]})"
  ...
  echo "ERROR: Packaged Operator bundle did not exit: ${pids[*]}" >&2
  return 1
}
