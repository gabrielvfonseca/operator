...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/operator/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Operator Re-Auth" -c "After getting key, run: ssh $SERVER '~/operator/scripts/mobile-reauth.sh'" --id operator-auth
        ;;
    *)
