// Debug Claude Usage script supports Operator repository automation.
...
Options:
  --agent <id>          Operator agent id to inspect (default: main)
...
const loadAuthProfiles = (agentId: string) => {
  const stateRoot = process.env.OPERATOR_STATE_DIR?.trim() || path.join(os.homedir(), ".operator");
...
const resolveFetchTimeoutMs = (raw = process.env.OPERATOR_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS) => {
  return parseStrictIntegerOption({
    fallback: DEFAULT_FETCH_TIMEOUT_MS,
    label: "OPERATOR_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
};
