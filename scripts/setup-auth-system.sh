#!/bin/bash
# Setup Operator Auth Management System
...
echo "=== Operator Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'operator-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Operator message: Send warning via Operator itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Install systemd units
SERVICE_TEMPLATE="$SCRIPT_DIR/systemd/operator-auth-monitor.service"
SYSTEMD_USER_DIR="$HOME/.config/systemd/user"
SERVICE_TARGET="$SYSTEMD_USER_DIR/operator-auth-monitor.service"
TIMER_TARGET="$SYSTEMD_USER_DIR/operator-auth-monitor.timer"
...
SERVICE_TEMP="$(mktemp "$SYSTEMD_USER_DIR/operator-auth-monitor.service.XXXXXX")"
SERVICE_RENDERED=""
cleanup_service_temp() {
    rm -f "$SERVICE_TEMP" "$SERVICE_RENDERED"
}
trap cleanup_service_temp EXIT
SERVICE_RENDERED="$(mktemp "$SYSTEMD_USER_DIR/operator-auth-monitor.service.rendered.XXXXXX")"
...
RENDERED_NTFY_LINE="$(render_environment_line "NOTIFY_NTFY" "operator-alerts" "$NTFY_TOPIC")"
...
trap - EXIT
cp "$SCRIPT_DIR/systemd/operator-auth-monitor.timer" "$TIMER_TARGET"
systemctl --user daemon-reload
systemctl --user enable --now operator-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status operator-auth-monitor.timer"
echo ""
echo "Quick commands:"
