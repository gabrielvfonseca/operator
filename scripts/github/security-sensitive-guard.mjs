...
export const securitySensitiveGuardMarker = "<!-- operator:security-sensitive-guard -->";
...
export function renderSecuritySensitiveAwarenessComment(changes) {
  return [
    ...
    "- If this PR intentionally needs the change, a repository admin or member of `@gabrielvfonseca/operator-secops` must approve the exact head SHA.",
  ].join("\n");
}
...
export function renderAuthorizedSecuritySensitiveComment(override) {
  const lines = [
    ...
    "",
    "This PR includes security-sensitive file changes. A repository admin or member of `@gabrielvfonseca/operator-secops` authorized this exact head SHA with `/allow-security-sensitive-change`.",
    "",
...
export function renderTrustedSecuritySensitiveComment({ actor, headSha, changes }) {
  return [
    ...
    "",
    "This PR includes security-sensitive file changes. The guard is informational because the PR author is a repository admin or a member of `@gabrielvfonseca/operator-secops`.",
    "",
...
export function renderBlockedSecuritySensitiveComment({ headSha, changes }) {
  return [
    ...
    "",
    "Operator does not accept security-sensitive file changes through PRs unless a repository admin or security explicitly authorizes the current head SHA.",
    "",
    "Detected security-sensitive changes:",
    ...renderChangedFileLines(changes),
    "",
    "If this PR intentionally needs these changes, ask a repository admin or member of `@gabrielvfonseca/operator-secops` to comment:",
    "",
    "```text",
...
export function githubApi(token, options = {}) {
  return createGitHubApi(token, {
    ...options,
    userAgent: "operator-security-sensitive-guard",
  });
}
...
async function main() {
  ...
  const trustedCommentAuthors = securitySensitiveGuardCommentAuthors(
    process.env.OPERATOR_SECURITY_SENSITIVE_GUARD_COMMENT_BOTS,
  );
  ...
  const mode = process.env.OPERATOR_SECURITY_SENSITIVE_GUARD_MODE ?? "enforce";
