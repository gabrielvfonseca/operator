...
# Output:
# - dist/Operator.app
# - dist/Operator-<version>.zip
# - dist/Operator-<version>.dmg
...
export BUNDLE_ID="${BUNDLE_ID:-ai.operator.mac}"
...
canonical_sparkle_build() {
  ...
  stderr_file="$(mktemp "${TMPDIR:-/tmp}/operator-sparkle-build.XXXXXX")" || {
...
APP="$ROOT_DIR/dist/Operator.app"
if [[ ! -d "$APP" ]]; then
...
ZIP="$ROOT_DIR/dist/Operator-$VERSION.zip"
DMG="$ROOT_DIR/dist/Operator-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/Operator-$VERSION.notary.zip"
DSYM_ZIP="$ROOT_DIR/dist/Operator-$VERSION.dSYM.zip"
