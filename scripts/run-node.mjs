#!/usr/bin/env node
// Development runner that rebuilds Operator, runs runtime postbuild steps, and
...
const listRequiredBundledPluginMetadataOutputs = (pluginEntries, deps) =>
  pluginEntries.flatMap(({ id, hasManifest, hasPackageJson }) => {
    ...
    if (hasManifest) {
      requiredPaths.push(path.join(builtPluginDir, "operator.plugin.json"));
    }
...
const listRequiredOperatorExtensionAliasOutputs = (deps) => {
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
  ...
  const aliasDir = path.join(distRoot, "extensions", "node_modules", "@gabrielvfonseca/operator");
  return [
...
const listRequiredStaticExtensionAssetOutputs = (deps) => {
  if (deps.env.OPERATOR_RUNTIME_POSTBUILD_STATIC_ASSETS === "0") {
    return [];
  }
  const distRoot = resolveRuntimePostBuildDistRoot(deps);
  const runtimeRoot = resolveRuntimePostBuildRuntimeRoot(deps);
...
export const listRequiredRuntimePostBuildOutputs = (deps) => {
  ...
  return [
    ...
    ...listRequiredOperatorExtensionAliasOutputs(deps),
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.OPERATOR_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.OPERATOR_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (deps) => {
  if (deps.env.OPERATOR_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by OPERATOR_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by OPERATOR_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "OPERATOR_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_CPU_PROF_DIR_ENV = "OPERATOR_RUN_NODE_CPU_PROF_DIR";
const RUN_NODE_CPU_PROF_MAX_FILES_ENV = "OPERATOR_RUN_NODE_CPU_PROF_MAX_FILES";
const RUN_NODE_FILTER_SYNC_IO_STDERR_ENV = "OPERATOR_RUN_NODE_FILTER_SYNC_IO_STDERR";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "OPERATOR_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "OPERATOR_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "OPERATOR_RUN_NODE_BUILD_LOCK_STALE_MS";
const RUN_NODE_SKIP_DTS_BUILD_ENV = "OPERATOR_RUN_NODE_SKIP_DTS_BUILD";
...
const shouldUseRunNodeProgress = (deps) =>
  ...
  deps.env.OPERATOR_RUNNER_PROGRESS !== "0" &&
  deps.env.CI !== "true" &&
  !deps.outputTee;
...
const listRunNodeCpuProfiles = (deps, absoluteProfileDir, commandName) => {
  ...
  const prefix = `operator-${commandName}-`;
  return entries
    .filter(
      (entry) =>
...
const resolveRunNodeCpuProfileArgs = (deps) => {
  ...
  const profileName = `operator-${commandName}-${pid}-${timestamp}.cpuprofile`;
...
const resolveRunNodeDiagnosticArgs = (deps) => {
  ...
  if (deps.env.OPERATOR_TRACE_SYNC_IO === "1") {
...
const runOperator = async (deps) => {
  const diagnosticArgs = resolveRunNodeDiagnosticArgs(deps);
  const useProcessGroup = shouldUseRunNodeChildProcessGroup(deps);
  const nodeProcess = deps.spawn(deps.execPath, [...diagnosticArgs, "operator.mjs", ...deps.args], {
    cwd: deps.cwd,
...
const shouldSkipWatchRuntimeSync = (deps, requirement) =>
  deps.env.OPERATOR_WATCH_MODE === "1" &&
...
const shouldFastPathExistingDistForGatewayClient = (deps) =>
  ...
  deps.env.OPERATOR_FORCE_BUILD !== "1" &&
...
const canUseStampedGatewayClientDist = (deps) => {
  ...
  if (
    ...
    deps.env.OPERATOR_FORCE_RUNTIME_POSTBUILD === "1"
  ) {
    return false;
  }
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.OPERATOR_BUILD_PRIVATE_QA = "1";
    deps.env.OPERATOR_ENABLE_PRIVATE_QA_CLI = "1";
    deps.env.OPERATOR_DISABLE_BUNDLED_PLUGINS ??= "0";
  }
  ...
  try {
    ...
    if (shouldFastPathExistingDistForGatewayClient(deps)) {
      exitCode = await runOperator(deps);
