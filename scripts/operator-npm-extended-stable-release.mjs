...
export async function verifyExtendedStableRegistryReadback({
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    const exactResult = await query(`operator@${expectedVersion}`);
    const extendedStableResult = await query("operator@extended-stable");
  ...
  throw new Error(
    `npm registry did not converge to operator@${expectedVersion} and operator@extended-stable=${expectedVersion} after ${attempts} attempts (exact=${exactVersion}, extended-stable=${extendedStableSelector}).`,
  );
}
...
export function extendedStableSelectorRepairCommand(expectedVersion) {
  ...
  return `npm dist-tag add operator@${parsed.version} extended-stable`;
}
