...
# Operator Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Operator."
...
resolve_operator_effective_home() {
    local operator_home="${OPENCLAW_HOME:-}"
    if [[ -z "$operator_home" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$operator_home" == "~" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$operator_home" == \~/* ]]; then
        echo "${HOME}${operator_home:1}"
        return
    fi
    echo "$operator_home"
}

resolve_operator_user_path() {
    local input="$1"
    local effective_home
    effective_home="$(resolve_operator_effective_home)"
    if [[ "$input" == "~" ]]; then
        echo "$effective_home"
    elif [[ "$input" == \~/* ]]; then
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 Operator Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://operator.ai/install.ps1 | iex"
        exit 1
    fi
...
show_footer_links() {
    local faq_url="https://docs.operator.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_operator_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.operator-* "$npm_root"/operator 2>/dev/null || true
}

extract_operator_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_operator_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/operator" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/operator"|"/usr/local/bin/operator")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/operator/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale operator symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing operator binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Operator package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi

    ui_info "Installing Operator package"
...
install_operator_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Operator npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*operator" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_operator_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_operator_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Operator npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an operator binary already exists"
            if [[ -n "$conflict" ]]; then
...
GIT_DIR_DEFAULT="$(resolve_operator_effective_home)/operator"
...
print_usage() {
    cat <<EOF
Operator installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install.sh | bash -s -- [options]
...
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install.sh | bash -s -- --install-method git --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Operator checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Operator source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_operator_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"operator"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://operator.ai/install.sh | bash"
}
...
fix_npm_permissions() {
    ...
    ui_warn "Avoid sudo npm i -g for future Operator updates; use npm i -g operator@latest so npm keeps using this user prefix instead of a different global prefix."
...
ensure_operator_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/operator" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/operator" ]]; then
        ln -sf "$npm_root/operator/dist/entry.js" "${npm_bin}/operator"
        ui_info "Created operator bin link at ${npm_bin}/operator"
    fi
    return 0
}
...
check_existing_operator() {
    if [[ -n "$(type -P operator 2>/dev/null || true)" ]]; then
        ui_info "Existing Operator installation detected, upgrading"
        return 0
    fi
    return 1
}
...
resolve_git_operator_ref() {
    ...
    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "operator" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_operator_npm_root_candidates() {
    local root=""
...
find_operator_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/operator"
        ...
        local version=""
        version="$(operator_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_operator_npm_root_candidates)
}

warn_duplicate_operator_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_operator_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple Operator global installs detected"
    echo "  Different Node/npm environments can run different Operator versions."

    local active_node active_npm active_operator
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_operator="${OPENCLAW_BIN:-}"
    if [[ -z "$active_operator" ]]; then
        active_operator="$(type -P operator 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active operator: ${INFO}${active_operator:-none}${NC}"
    echo ""
    echo "  Found installs:"
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make operator show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

operator_command_for_user() {
    local claw="${1:-}"
    if [[ -z "$claw" ]]; then
        echo "operator"
        return 0
    ...
    local original_claw=""
    original_claw="$(PATH="$ORIGINAL_PATH" type -P operator 2>/dev/null || true)"
...
warn_operator_not_found() {
    ui_warn "Installed, but operator is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t operator 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named operator; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_operator_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P operator 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/operator" ]]; then
        echo "${npm_bin}/operator"
        return 0
    fi
...
resolve_installed_operator_bin() {
    local installed_bin=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        installed_bin="$HOME/.local/bin/operator"
    elif [[ "$INSTALL_METHOD" == "npm" ]]; then
        local npm_bin=""
        npm_bin="$(npm_global_bin_dir || true)"
        if [[ -n "$npm_bin" ]]; then
            installed_bin="${npm_bin}/operator"
        fi
    fi
...
install_operator_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/operator/operator.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Operator from git checkout: ${repo_dir}"
    else
        ui_info "Installing Operator from GitHub (${repo_url})"
    fi
    ...
    local git_ref
    git_ref="$(resolve_git_operator_ref)"
    if [[ -z "$(git -C "$repo_dir" status --porcelain 2>/dev/null || true)" ]]; then
        ui_info "Using git ref: ${git_ref}"
        checkout_git_operator_ref "$repo_dir" "$git_ref"
    else
    ...
    run_quiet_step "Building Operator" run_pnpm -C "$repo_dir" build
    ...
    cat > "$HOME/.local/bin/operator" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec ${node_bin_quoted} ${entry_path_quoted} "\$@"
EOF
    chmod +x "$HOME/.local/bin/operator"
    ui_success "Operator wrapper installed to \$HOME/.local/bin/operator"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view operator dist-tags.beta 2>/dev/null || true)"
...
is_operator_source_package_install_spec() {
    local value="${1:-}"
    local normalized_value=""
    normalized_value="$(to_lowercase_ascii "$value")"
    normalized_value="${normalized_value#operator@}"

    [[ "$normalized_value" == "main" ]] && return 0
    [[ "$normalized_value" =~ ^github:operator/operator($|[#/]) ]] && return 0

    normalized_value="${normalized_value#git+}"
    [[ "$normalized_value" =~ ^https?://github\.com/operator/operator(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^ssh://git@github\.com[:/]operator/operator(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git://github\.com/operator/operator(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git@github\.com:operator/operator(\.git)?($|[?#]) ]] && return 0
    return 1
}
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:operator/operator#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_operator() {
    ...
    if is_operator_source_package_install_spec "${OPENCLAW_VERSION}"; then
        ui_error "npm installs do not support Operator GitHub source targets like '${OPENCLAW_VERSION}'."
    ...
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Operator v${resolved_version}"
    else
        ui_info "Installing Operator (${OPENCLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${OPENCLAW_VERSION}")"

    if ! install_operator_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_operator_paths
        install_operator_npm "${install_spec}"
    fi

    if [[ "${OPENCLAW_VERSION}" == "latest" && "${package_name}" == "operator" ]]; then
        if ! resolve_operator_bin &> /dev/null; then
            ui_warn "npm install operator@latest failed; retrying operator@next"
            cleanup_npm_operator_paths
            install_operator_npm "operator@next"
        fi
    fi

    ensure_operator_bin_link || true

    ui_success "Operator installed"
}
...
run_doctor() {
    ...
    if [[ -z "$claw" ]]; then
        claw="$(resolve_operator_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (operator not on PATH yet)"
        warn_operator_not_found
        return 0
    fi
    local doctor_exit=0
...
has_operator_config() {
    ...
    if [[ -n "${OPENCLAW_CONFIG_PATH:-}" ]]; then
        local config_path
        config_path="$(resolve_operator_user_path "$OPENCLAW_CONFIG_PATH")"
        [[ -f "$config_path" ]]
        return
    fi
    ...
    if [[ -n "${OPENCLAW_STATE_DIR:-}" ]]; then
        local state_dir
        state_dir="$(resolve_operator_user_path "$OPENCLAW_STATE_DIR")"
        if [[ -f "$state_dir/operator.json" || -f "$state_dir/clawdbot.json" ]]; then
            return 0
        fi
        return 1
    fi

    if [[ -f "$effective_home/.operator/operator.json" ||
        -f "$effective_home/.operator/clawdbot.json" ||
        -f "$effective_home/.clawdbot/operator.json" ||
...
if ! declare -F extract_operator_semver >/dev/null 2>&1; then
...
resolve_operator_version() {
    ...
    local claw="${OPENCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v operator &> /dev/null; then
        claw="$(command -v operator)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_operator_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/operator/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/operator/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$(bounded_probe_output "operator daemon status --json" "$claw" daemon status --json || true)"
...
refresh_gateway_service_if_loaded() {
    ...
    if run_quiet_step "Restarting gateway service" "$claw" gateway restart; then
        ...
        local user_claw
        user_claw="$(operator_command_for_user "$claw")"
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: operator not on PATH yet"
        warn_operator_not_found
        return 1
    fi

    run_quiet_step "Checking Operator version" "$claw" --version || return 1
...
main() {
    ...
    detected_checkout="$(detect_operator_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Operator checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
    ...
    if check_existing_operator; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Operator"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g operator &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g operator 2>/dev/null || true
        ...
        install_operator_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/operator" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/operator"
    ...
    OPENCLAW_BIN="$(resolve_installed_operator_bin || true)"
    warn_duplicate_operator_global_installs || true
    ...
    if has_operator_config; then
        config_present=true
        refresh_gateway_service_if_loaded
    fi

    local installed_version
    installed_version=$(resolve_operator_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 Operator installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 Operator installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        local user_claw
        user_claw="$(operator_command_for_user "${OPENCLAW_BIN:-}")"
        ui_section "Source install details"
        ui_kv "Checkout" "$final_git_dir"
        ui_kv "Wrapper" "$HOME/.local/bin/operator"
        ui_kv "Update command" "${user_claw} update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install.sh | bash -s -- --install-method npm"
    fi
    ...
    if [[ "$config_present" != "true" ]]; then
        if [[ "$NO_ONBOARD" == "1" ]]; then
            ...
            if is_promptable; then
                local claw="${OPENCLAW_BIN:-}"
                if [[ -z "$claw" ]]; then
                    claw="$(resolve_installed_operator_bin || true)"
                fi
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (operator not on PATH yet)"
                    warn_operator_not_found
                    return 0
                fi
                exec </dev/tty
        ...
        if has_controlling_tty || [[ "$NO_ONBOARD" == "1" || "$NO_PROMPT" == "1" ]]; then
            ...
            ui_info "Running operator doctor"
            local doctor_ok=0
            local doctor_exit=0
            if (( ${#doctor_args[@]} )); then
