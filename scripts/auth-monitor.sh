...
#
# Suggested cron: */30 * * * * /path/to/operator/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Operator notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., operator-alerts)
...
STATE_FILE="$HOME/.operator/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Operator if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use operator
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Operator to $NOTIFY_PHONE..."
            operator send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Operator is down. Run on the Operator host: ${SCRIPT_DIR}/mobile-reauth.sh" "urgent"
    exit 1
