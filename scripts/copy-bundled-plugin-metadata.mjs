...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "operator.plugin.json");
    ...
    if (packageJson.operator && "extensions" in packageJson.operator) {
      packageJson.operator = {
        ...packageJson.operator,
        extensions: rewritePackageExtensions(packageJson.operator.extensions),
        ...(typeof packageJson.operator.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.operator.setupEntry) }
          : {}),
      };
    }
