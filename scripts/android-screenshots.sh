...
DEFAULT_SCREENSHOT_AVD="Operator_Screenshots_API36"
...
boot_emulator() {
  ...
  emulator="$(emulator_bin)"
  EMULATOR_LOG="$(mktemp "${TMPDIR:-/tmp}/operator-android-screenshot-emulator.XXXXXX.log")"
...
scene_ready_text() {
  case "$1" in
    ...
    gateway) printf '%s\n' "Connection between this phone and Operator." ;;
    *)
...
"$ADB_BIN" -s "$ADB_SERIAL" shell pm clear ai.operator.app >/dev/null
"$ADB_BIN" -s "$ADB_SERIAL" shell pm grant ai.operator.app android.permission.RECORD_AUDIO >/dev/null
"$ADB_BIN" -s "$ADB_SERIAL" logcat -c >/dev/null 2>&1 || true

for scene in "${SCENES[@]}"; do
  output_path="${OUTPUT_DIR}/operator-${scene}.jpg"
  raw_path="${OUTPUT_DIR}/operator-${scene}.raw.png"
  artifact_path="${ARTIFACT_DIR}/screenshots/operator-${scene}.jpg"
  ui_dump_path="${ARTIFACT_DIR}/ui-dumps/operator-${scene}.xml"
  activity_start_path="${ARTIFACT_DIR}/activity-start/operator-${scene}.txt"
  "$ADB_BIN" -s "$ADB_SERIAL" shell am force-stop ai.operator.app >/dev/null
  "$ADB_BIN" -s "$ADB_SERIAL" shell am start -W \
    -n ai.operator.app/.MainActivity \
    --ez operator.screenshotMode true \
    --es operator.screenshotScene "$scene" >"$activity_start_path"
