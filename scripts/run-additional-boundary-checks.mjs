...
export function parseCliArgs(args, env = process.env) {
  let shardSpec = env.OPERATOR_ADDITIONAL_BOUNDARY_SHARD ?? "";
  let help = false;
  ...
  try {
    ...
    if (cliArgs.help) {
      ...
      const concurrencyRaw =
        process.env.OPERATOR_ADDITIONAL_BOUNDARY_CONCURRENCY ??
        process.env.OPERATOR_EXTENSION_BOUNDARY_CONCURRENCY;
      const concurrencyLabel =
        process.env.OPERATOR_ADDITIONAL_BOUNDARY_CONCURRENCY === undefined
          ? "OPERATOR_EXTENSION_BOUNDARY_CONCURRENCY"
          : "OPERATOR_ADDITIONAL_BOUNDARY_CONCURRENCY";
      const concurrency = resolveConcurrency(concurrencyRaw, 4, concurrencyLabel);
      const checkTimeoutMs = resolvePositiveInteger(
        process.env.OPERATOR_ADDITIONAL_BOUNDARY_TIMEOUT_MS,
        DEFAULT_CHECK_TIMEOUT_MS,
        "OPERATOR_ADDITIONAL_BOUNDARY_TIMEOUT_MS",
      );
      const outputMaxBytes = resolvePositiveInteger(
        process.env.OPERATOR_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES,
        DEFAULT_OUTPUT_MAX_BYTES,
        "OPERATOR_ADDITIONAL_BOUNDARY_OUTPUT_MAX_BYTES",
      );
