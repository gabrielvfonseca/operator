...
export function resolveCompileConcurrency(
  ...
  const raw = env.OPERATOR_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "OPERATOR_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.OPERATOR_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";
