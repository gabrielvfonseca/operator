...
operator_live_acp_bind_append_build_extension() {
...
operator_live_acp_bind_resolve_auth_provider() {
  case "${1:-}" in
    claude) printf '%s\n' "claude-cli" ;;
    codex) printf '%s\n' "codex-cli" ;;
    droid) printf '%s\n' "droid" ;;
...
operator_live_acp_bind_resolve_agent_command() {
  case "${1:-}" in
...
operator_live_acp_bind_load_factory_api_key_from_profile() {
...
operator_live_acp_bind_append_build_extension acpx
OPENCLAW_LIVE_DOCKER_REPO_ROOT="$ROOT_DIR" "$TRUSTED_HARNESS_DIR/scripts/test-live-build-docker.sh"

IFS=',' read -r -a ACP_AGENT_TOKENS <<<"$ACP_AGENT_LIST_RAW"
ACP_AGENTS=()
for token in "${ACP_AGENT_TOKENS[@]}"; do
  agent="$(operator_live_trim "$token")"
  [[ -n "$agent" ]] || continue
  operator_live_acp_bind_resolve_auth_provider "$agent" >/dev/null
  ACP_AGENTS+=("$agent")
done
...
for ACP_AGENT in "${ACP_AGENTS[@]}"; do
  AUTH_PROVIDER="$(operator_live_acp_bind_resolve_auth_provider "$ACP_AGENT")"
  AGENT_COMMAND="$(operator_live_acp_bind_resolve_agent_command "$ACP_AGENT")"
  ...
  if [[ -n "${OPENCLAW_DOCKER_AUTH_DIRS:-}" ]]; then
    ...
    while IFS= read -r auth_dir; do
      [[ -n "$auth_dir" ]] || continue
      AUTH_DIRS+=("$auth_dir")
    done < <(operator_live_collect_auth_dirs_from_csv "$AUTH_PROVIDER")
    while IFS= read -r auth_file; do
      [[ -n "$auth_file" ]] || continue
      AUTH_FILES+=("$auth_file")
    done < <(operator_live_collect_auth_files_from_csv "$AUTH_PROVIDER")
  fi
  ...
  DOCKER_RUN_ARGS=()
  operator_live_init_docker_run_args DOCKER_RUN_ARGS "${OPENCLAW_LIVE_ACP_BIND_DOCKER_RUN_TIMEOUT:-2700s}"
  DOCKER_AUTH_ENV=()
