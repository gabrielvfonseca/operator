...
BASE_IMAGE="${BASE_IMAGE:-operator-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-operator-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=operator.sandbox=1)
NOTE
