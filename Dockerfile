...
ARG OPERATOR_EXTENSIONS=""
ARG OPERATOR_BUNDLED_PLUGIN_DIR=extensions
ARG OPERATOR_DOCKER_BUILD_NODE_OPTIONS="--max-old-space-size=8192"
ARG OPERATOR_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=""
ARG OPERATOR_DOCKER_BUILD_SKIP_DTS=1
ARG OPERATOR_NODE_BOOKWORM_IMAGE="docker.io/library/node:24-bookworm@sha256:8530f76a96d88820d288761f022e318970dda93d01536919fbc16076b7983e63"
ARG OPERATOR_NODE_BOOKWORM_SLIM_IMAGE="docker.io/library/node:24-bookworm-slim@sha256:242549cd46785b480c832479a730f4f2a20865d61ea2e404fdb2a5c3d3b73ecf"
ARG OPERATOR_NODE_BOOKWORM_SLIM_DIGEST="sha256:242549cd46785b480c832479a730f4f2a20865d61ea2e404fdb2a5c3d3b73ecf"
...
ARG OPERATOR_EXTENSIONS
ARG OPERATOR_BUNDLED_PLUGIN_DIR
...
RUN mkdir -p /out/packages "/out/${OPERATOR_BUNDLED_PLUGIN_DIR}" && \
    ...
    node /tmp/docker-plugin-selection.mjs "/tmp/${OPERATOR_BUNDLED_PLUGIN_DIR}" "$OPERATOR_EXTENSIONS" \
      > /out/operator-selected-plugin-dirs && \
    while IFS= read -r ext; do \
      ext_dir="/tmp/${OPERATOR_BUNDLED_PLUGIN_DIR}/$ext"; \
      if [ -f "$ext_dir/package.json" ]; then \
        mkdir -p "/out/${OPERATOR_BUNDLED_PLUGIN_DIR}/$ext" && \
        cp "$ext_dir/package.json" "/out/${OPERATOR_BUNDLED_PLUGIN_DIR}/$ext/package.json"; \
      fi; \
    done < /out/operator-selected-plugin-dirs

# ── Stage 2: Build ──────────────────────────────────────────────
FROM ${OPERATOR_BUN_IMAGE} AS bun-binary
FROM ${OPERATOR_NODE_BOOKWORM_IMAGE} AS build
ARG OPERATOR_BUNDLED_PLUGIN_DIR
ARG OPERATOR_DOCKER_BUILD_NODE_OPTIONS
ARG OPERATOR_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB
ARG OPERATOR_DOCKER_BUILD_SKIP_DTS
...
RUN set -eux; \
    if ! grep -qx 'matrix' /tmp/operator-selected-plugin-dirs; then \
...
ARG GIT_COMMIT=""
ARG OPERATOR_BUILD_TIMESTAMP=""
ENV GIT_COMMIT=${GIT_COMMIT} \
    OPERATOR_BUILD_TIMESTAMP=${OPERATOR_BUILD_TIMESTAMP}
...
RUN set -eu; \
    ...
    if [ -z "$OPERATOR_BUILD_TIMESTAMP" ]; then \
      OPERATOR_BUILD_TIMESTAMP="$(date -u +%Y-%m-%dT%H:%M:%SZ)"; \
      export OPERATOR_BUILD_TIMESTAMP; \
    fi; \
    if grep -qx 'qa-lab' /tmp/operator-selected-plugin-dirs; then \
      export OPERATOR_BUILD_PRIVATE_QA=1 OPERATOR_ENABLE_PRIVATE_QA_CLI=1; \
    fi; \
    OPERATOR_INTERNAL_DOCKER_BUILD_PLUGIN_IDS="$selected_plugin_dirs" OPERATOR_RUN_NODE_SKIP_DTS_BUILD="$OPERATOR_DOCKER_BUILD_SKIP_DTS" OPERATOR_TSDOWN_MAX_OLD_SPACE_MB="$OPERATOR_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB" NODE_OPTIONS="$OPERATOR_DOCKER_BUILD_NODE_OPTIONS" pnpm_config_verify_deps_before_run=false pnpm build:docker; \
    pnpm_config_verify_deps_before_run=false pnpm ui:build
RUN if grep -qx 'qa-lab' /tmp/operator-selected-plugin-dirs; then \
...
RUN --mount=type=cache,id=operator-pnpm-store,target=/root/.local/share/pnpm/store,sharing=locked \
    ...
    if [ -L /app/node_modules/@gabrielvfonseca/ai ]; then \
      ai_runtime_target="$(readlink -f /app/node_modules/@gabrielvfonseca/ai)" && \
      ai_runtime_tmp="$(mktemp -d)" && \
      cp -a "$ai_runtime_target" "$ai_runtime_tmp/ai" && \
      rm /app/node_modules/@gabrielvfonseca/ai && \
      mv "$ai_runtime_tmp/ai" /app/node_modules/@gabrielvfonseca/ai && \
      rmdir "$ai_runtime_tmp"; \
    fi && \
    rm -rf \
      /app/node_modules/operator \
      /app/node_modules/.bin/operator \
      /app/node_modules/.pnpm/operator@*/node_modules/operator && \
    node scripts/check-package-dist-imports.mjs /app

# ── Runtime base image ──────────────────────────────────────────
FROM ${OPERATOR_NODE_BOOKWORM_SLIM_IMAGE} AS base-runtime
ARG OPERATOR_NODE_BOOKWORM_SLIM_DIGEST
LABEL org.opencontainers.image.base.name="docker.io/library/node:24-bookworm-slim" \
  org.opencontainers.image.base.digest="${OPERATOR_NODE_BOOKWORM_SLIM_DIGEST}"
...
# - https://docs.operator.ai/install/docker
LABEL org.opencontainers.image.source="https://github.com/gabrielvfonseca/operator" \
...
RUN --mount=type=cache,id=operator-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=operator-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    apt-get update && \
...
COPY --from=runtime-assets --chown=node:node /app/operator.mjs .
COPY --from=runtime-assets --chown=node:node /app/src/agents/templates ./src/agents/templates
COPY --from=runtime-assets --chown=node:node /app/${OPERATOR_BUNDLED_PLUGIN_DIR} ./${OPERATOR_BUNDLED_PLUGIN_DIR}
...
# Example: docker build --build-arg OPERATOR_IMAGE_APT_PACKAGES="python3 wget" .
# Legacy alias: OPERATOR_DOCKER_APT_PACKAGES is still accepted as a fallback.
ARG OPERATOR_IMAGE_APT_PACKAGES
ARG OPERATOR_DOCKER_APT_PACKAGES=""
RUN --mount=type=cache,id=operator-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=operator-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    packages="${OPERATOR_IMAGE_APT_PACKAGES-$OPERATOR_DOCKER_APT_PACKAGES}"; \
    if [ -n "$packages" ]; then \
      apt-get update && \
...
# Example: docker build --build-arg OPERATOR_IMAGE_PIP_PACKAGES="requests humanize" .
ARG OPERATOR_IMAGE_PIP_PACKAGES=""
RUN --mount=type=cache,id=operator-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=operator-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$OPERATOR_IMAGE_PIP_PACKAGES" ]; then \
...
# Build with: docker build --build-arg OPERATOR_INSTALL_BROWSER=1 ...
# Adds ~300MB but eliminates the 60-90s Playwright install on every container start.
# Must run after node_modules COPY so playwright-core is available.
ARG OPERATOR_INSTALL_BROWSER=""
ENV PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright
RUN --mount=type=cache,id=operator-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=operator-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$OPERATOR_INSTALL_BROWSER" ]; then \
      apt-get update && \
...
# Build with: docker build --build-arg OPERATOR_INSTALL_DOCKER_CLI=1 ...
# Adds ~50MB. Only the CLI is installed — no Docker daemon.
# Required for agents.defaults.sandbox to function in Docker deployments.
ARG OPERATOR_INSTALL_DOCKER_CLI=""
ARG OPERATOR_DOCKER_GPG_FINGERPRINT="9DC858229FC7DD38854AE2D88D81803C0EBFCD88"
RUN --mount=type=cache,id=operator-bookworm-apt-cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,id=operator-bookworm-apt-lists,target=/var/lib/apt,sharing=locked \
    if [ -n "$OPERATOR_INSTALL_DOCKER_CLI" ]; then \
      apt-get update && \
      DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
      ...
      # Update OPERATOR_DOCKER_GPG_FINGERPRINT when Docker rotates release keys.
      curl -fsSL https://download.docker.com/linux/debian/gpg -o /tmp/docker.gpg.asc && \
      expected_fingerprint="$(printf '%s' "$OPERATOR_DOCKER_GPG_FINGERPRINT" | tr '[:lower:]' '[:upper:]' | tr -d '[:space:]')" && \
...
RUN ln -sf /app/operator.mjs /usr/local/bin/operator \
 && chmod 755 /app/operator.mjs
...
# the leaf /home/node/.config/operator inherits the correct parent permissions.
...
RUN install -d -m 0755 -o node -g node /home/node/.config && \
    install -d -m 0700 -o node -g node \
      /home/node/.operator \
      /home/node/.operator/workspace \
      /home/node/.config/operator && \
