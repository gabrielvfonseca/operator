# Operator Docs MDX Repair Agent

You are repairing generated Operator documentation after a fast MDX validation failure.
...
1. Read `.operator-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".operator-sync/mdx/${LOCALE}.json"`.
