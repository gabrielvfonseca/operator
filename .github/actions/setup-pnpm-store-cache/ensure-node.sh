  ...
  case "$requested" in
    *x)
      ...
      if [[ "${requested%%.*}" == "22" ]]; then
        operator_node_version_at_least "$actual" "22.22.3"
      elif [[ "${requested%%.*}" == "24" ]]; then
        operator_node_version_at_least "$actual" "24.15.0"
      elif [[ "${requested%%.*}" == "25" ]]; then
        operator_node_version_at_least "$actual" "25.9.0"
      fi
      ;;
    *.*.*)
      [[ "$actual" == "$requested" ]]
...
operator_node_version_at_least() {
  local actual="$1"
  local minimum="$2"
...
operator_active_node_version() {
  node -p 'process.versions.node' 2>/dev/null || true
}

operator_prepend_node_bin() {
...
operator_find_toolcache_node() {
  ...
  for node_root in ${roots[@]+"${roots[@]}"}; do
    while IFS= read -r candidate; do
      ...
      if operator_node_version_matches "$candidate_version" "$requested_node"; then
        printf '%s\n' "$candidate"
        return 0
      fi
...
operator_resolve_node_download_version() {
  local requested_node="$1"
...
operator_node_download_platform() {
...
operator_download_node() {
  local requested_node="$1"
  local version platform archive_url archive_path install_root temp_root
  version="$(operator_resolve_node_download_version "$requested_node")"
  platform="$(operator_node_download_platform)" || return 1
  temp_root="${RUNNER_TEMP:-/tmp}"
  if command -v cygpath >/dev/null 2>&1; then
    temp_root="$(cygpath -u "$temp_root" 2>/dev/null || printf '%s\n' "$temp_root")"
  fi
  install_root="${temp_root}/operator-node-${version}-${platform}"
  if [[ "$platform" == win-* ]]; then
    ...
    if command -v pwsh >/dev/null 2>&1; then
      ...
      operator_prepend_node_bin "$node_bin_dir" "$ps_bin_dir"
      ...
      operator_prepend_node_bin "$node_bin_dir"
    fi
  else
    ...
    rm -f "$archive_path"
    operator_prepend_node_bin "$install_root/bin"
  fi
}

operator_ensure_node() {
  local requested_node="${1:-}"
  ...
  active_node_version="$(operator_active_node_version)"
  if operator_node_version_matches "$active_node_version" "$requested_node"; then
    echo "Using active Node ${active_node_version} at $(command -v node)"
    return 0
  fi

  node_bin="$(operator_find_toolcache_node "$requested_node" || true)"
  if [[ -n "$node_bin" ]]; then
    echo "Using Node $("$node_bin" -p 'process.versions.node') from $node_bin"
    operator_prepend_node_bin "$(dirname "$node_bin")"
  else
    operator_download_node "$requested_node" || true
  fi
