...
summary: "OpenProse is a markdown-first workflow format for multi-agent AI sessions. In Operator it ships as a plugin with a /prose slash command and a skill pack."
read_when:
  - You want to run or write .prose workflow files
  - You want to enable the OpenProse plugin
  - You need to understand how OpenProse maps to Operator primitives
---

OpenProse is a portable, markdown-first workflow format for orchestrating AI
sessions. In Operator it ships as a plugin that installs an OpenProse skill
...
<Steps>
  <Step title="Enable the plugin">
    ...
    ```bash
    operator plugins enable open-prose
    ```
  ...
  <Step title="Verify">
    ```bash
    operator plugins list | grep prose
    ```
...
`operator plugins install ./extensions/open-prose`
...
## Operator runtime mapping

OpenProse programs map to Operator primitives:

| OpenProse concept         | Operator tool                                   |
| ------------------------- | ----------------------------------------------- |
...
executed. Use Operator tool allowlists and approval gates to control side
...
<CardGroup cols={2}>
  ...
  <Card title="Subagents" href="/tools/subagents" icon="users">
    Operator's native multi-agent coordination layer.
  </Card>
