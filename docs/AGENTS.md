...
- `scripts/docs-sync-publish.mjs` excludes and prunes `docs/internal/**` from the public `operator/docs` publish repo if a page is force-added later.
...
- Foreign-language docs are not maintained in this repo. The generated publish output lives in the separate `operator/docs` repo (often cloned locally as `../operator-docs`).
...
- Pipeline: update English docs here, update `docs/.i18n/glossary.<locale>.json` as needed, then let the publish-repo sync and `scripts/docs-i18n` run in `operator/docs`.
