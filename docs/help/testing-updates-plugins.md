---
summary: "How Operator validates update paths, package migrations, and plugin install/update behavior"
read_when:
  - Changing Operator update, doctor, package acceptance, or plugin install behavior
...
- A user can move from an older published package to the candidate package
  ...
  channel config.
- `operator doctor --fix --non-interactive` owns legacy cleanup and repair
...
- `test:docker:plugins` covers plugin install smoke, local folder installs,
  ...
  and Claude-bundle enable/inspect. Set `OPERATOR_PLUGINS_E2E_CLAWHUB=0` to
...
- `test:docker:plugin-update` validates that an unchanged installed plugin does
  not reinstall or lose install metadata during `operator plugins update`.
...
- `test:docker:published-upgrade-survivor` first installs a published baseline,
  configures it through a baked `operator config set` recipe, updates it to the
...
- `test:docker:update-restart-auth` installs the candidate package, starts a
  ...
  `operator update --yes --json`, and requires the candidate update command to
...
Available scenarios: `base`, `acpx-operator-tools-bridge`, `feishu-channel`,
`bootstrap-persona`, `channel-post-core-restore`, `plugin-deps-cleanup`,
`configured-plugin-installs`, `stale-source-plugin-shadow`, `tilde-log-path`,
and `versioned-runtime-deps`. In aggregate runs, `OPERATOR_UPGRADE_SURVIVOR_SCENARIOS=reported-issues`
...
- `source=trusted-url`: validate an HTTPS tarball with required
  ...
  `OPERATOR_TRUSTED_PACKAGE_TOKEN` secret.
...
`last-stable-4` resolves to the four latest stable npm-published Operator
