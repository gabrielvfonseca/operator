...
Operator has three Vitest suites (unit/integration, e2e, live) plus Docker
...
- Linux VM-backed QA lane: `pnpm operator qa suite --runner multipass --scenario channel-chat-baseline`
...
it("uses a temp workspace", () => {
  const workspace = tempDirs.make("operator-example-");
  // use workspace
});
```
...
```ts
// operator-temp-dir: allow verifies raw fs cleanup behavior
...
- Runtime performance reports: dispatch `Operator Performance` with
...
- Docker live model sweep: `pnpm test:docker:live-models`
  - Each selected model runs a text turn plus a small file-read-style probe.
    ...
    Disable the extra probes with `OPERATOR_LIVE_MODEL_FILE_PROBE=0` or
    `OPERATOR_LIVE_MODEL_IMAGE_PROBE=0` when isolating provider failures.
  - CI coverage: daily `Operator Scheduled Live And E2E Checks` and manual
    `Operator Release Checks` both call the reusable live/E2E workflow with
    `include_live_suites: true`, which includes Docker live model matrix jobs
    sharded by provider.
  - For focused CI reruns, dispatch `Operator Live And E2E Checks (Reusable)`
  ...
  - Add new high-signal provider secrets to `scripts/ci-hydrate-live-auth.sh`
    plus `.github/workflows/operator-live-and-e2e-checks-reusable.yml` and its
...
- Codex app-server harness smoke: `pnpm test:docker:live-codex-harness`
  - Runs gateway agent turns through the plugin-owned Codex app-server
    ...
    sub-agent probe with `OPERATOR_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0` when
    isolating other failures. For a focused sub-agent check, disable the
    other probes:
    `OPERATOR_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 OPERATOR_LIVE_CODEX_HARNESS_MCP_PROBE=0 OPERATOR_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0 OPERATOR_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=1 pnpm test:docker:live-codex-harness`.
    This exits after the sub-agent probe unless
    `OPERATOR_LIVE_CODEX_HARNESS_SUBAGENT_ONLY=0` is set.
- Codex on-demand install smoke: `pnpm test:docker:codex-on-demand`
  - Installs the packaged Operator tarball in Docker, runs OpenAI API-key
...
- Operator rescue command smoke: `pnpm test:live:system-agent-rescue-channel`
  - Opt-in belt-and-suspenders check for the message-channel rescue command
    surface. Exercises `/operator status`, queues a persistent model
    change, replies `/operator yes`, and verifies the audit/config write
    path.
- Operator first-run Docker smoke: `pnpm test:docker:system-agent-first-run`
  - Starts from an empty Operator state dir and first proves the packaged
    `operator setup` CLI fails closed without inference. It then
    ...
    Operator agent/tool/approval proof. The same lane is exposed in QA Lab by
    `pnpm operator qa suite --scenario system-agent-ring-zero-setup`.
- Moonshot/Kimi cost smoke: with `MOONSHOT_API_KEY` set, run
  `operator models list --provider moonshot --json`, then run an isolated
  `operator agent --local --session-id live-kimi-cost --message 'Reply exactly: KIMI_LIVE_OK' --thinking off --json`
...
`transport`. `Operator Release Checks` runs parity plus the reusable Matrix
...
- `pnpm operator qa suite`
  ...
  - When dispatched by `pnpm operator qa run --qa-profile <profile>`, embeds
  ...
  - Supports provider modes `live-frontier`, `mock-openai`, and `aimock`.
    ...
    `mock-openai` lane.
- `pnpm operator qa coverage --match <query>`
...
- `pnpm test:plugins:kitchen-sink-live`
  - Runs the live OpenAI Kitchen Sink plugin gauntlet through QA Lab.
    ...
    profile when the `operator-testbox-env` helper is present.
...
- `pnpm operator qa suite --runner multipass`
...
- `pnpm test:docker:npm-onboard-channel-agent`
  ...
  - Use `OPERATOR_NPM_ONBOARD_CHANNEL=discord` to run the same packaged-install
    lane with Discord.
- `pnpm test:docker:session-runtime-context`
  - Runs a deterministic built-app Docker smoke for embedded runtime context
    transcripts. Verifies hidden Operator runtime context persists as a
    ...
    `operator doctor --fix` rewrites it to the active branch with a backup.
- `pnpm test:docker:npm-telegram-live`
  - Installs an Operator package candidate in Docker, runs installed-package
  ...
  - Emits repeated RTT timing in `qa-evidence.json` by default with
    `OPERATOR_NPM_TELEGRAM_RTT_SAMPLES=20`. Override
    `OPERATOR_NPM_TELEGRAM_RTT_SAMPLES`,
    `OPERATOR_NPM_TELEGRAM_RTT_TIMEOUT_MS`, or
    `OPERATOR_NPM_TELEGRAM_RTT_MAX_FAILURES` to tune the run.
    `OPERATOR_NPM_TELEGRAM_RTT_CHECKS` selects the Telegram QA scenario to
    sample; the supported RTT target is `channel-canary`.
  - Uses the same Telegram env credentials or Convex credential source as
    `pnpm operator qa telegram`. For CI/release automation, set
    `OPERATOR_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex` plus
    `OPERATOR_QA_CONVEX_SITE_URL` and a role secret. If
    `OPERATOR_QA_CONVEX_SITE_URL` and a Convex role secret are present in
  ...
  - The wrapper validates Telegram or Convex credential env on the host
    ...
    `OPERATOR_NPM_TELEGRAM_SKIP_CREDENTIAL_PREFLIGHT=1` only when
    deliberately debugging pre-credential setup.
  - `OPERATOR_NPM_TELEGRAM_CREDENTIAL_ROLE=ci|maintainer` overrides the
    shared `OPERATOR_QA_CREDENTIAL_ROLE` for this lane only. When Convex
...
- GitHub Actions also exposes `Package Acceptance` for side-run product proof
  ...
  normalized `operator-current.tgz` as `package-under-test`, then runs the
...
`source=trusted-url` reads `.github/package-trusted-sources.json` from the trusted workflow ref and does not accept URL credentials or a workflow-input private-network bypass. If the named policy declares bearer auth, configure the fixed `OPERATOR_TRUSTED_PACKAGE_TOKEN` secret.
...
- `pnpm test:docker:plugins`
  - Packs and installs the current Operator build in Docker, starts the
  ...
  - Also installs a known older npm baseline, enables Telegram before
    running `operator update --tag <candidate>`, and verifies the
...
- `pnpm test:parallels:npm-update`
  - Runs the native packaged-install update smoke across Parallels guests.
    ...
    then runs the installed `operator update` command in the same guest and
  ...
  - The OpenAI lane uses `openai/gpt-5.6-luna` for the live agent-turn proof by
    ...
    `OPERATOR_PARALLELS_OPENAI_MODEL` to validate another OpenAI model.
  ...
  - The script writes nested lane logs under
    `/tmp/operator-parallels-npm-update.*`. Inspect `windows-update.log`,
...
- `pnpm operator qa telegram`
  - Runs the Telegram live QA lane against a real private group using the
    driver and SUT bot tokens from env.
  - Requires `OPERATOR_QA_TELEGRAM_GROUP_ID`,
    `OPERATOR_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and
    `OPERATOR_QA_TELEGRAM_SUT_BOT_TOKEN`. The group id must be the numeric
    Telegram chat id.
  - Supports `--credential-source convex` for shared pooled credentials.
    Use env mode by default, or set `OPERATOR_QA_CREDENTIAL_SOURCE=convex`
...
```text
@operator-mantis telegram
@operator-mantis telegram scenario=telegram-status-command
@operator-mantis telegram scenarios=telegram-status-command,channel-canary
```
...
```text
@operator-mantis telegram desktop proof
```
...
- `pnpm operator qa mantis telegram-desktop-builder`
  - Leases or reuses a Crabbox Linux desktop, installs native Telegram
    Desktop, configures Operator with a leased Telegram SUT bot token,
  ...
  - Defaults to `--credential-source convex` so workflows only need the
    ...
    `OPERATOR_QA_TELEGRAM_*` variables as `pnpm operator qa telegram`.
  - Telegram Desktop still needs a user login/profile. The bot token
    configures Operator only. Use `--telegram-profile-archive-env <name>`
...
When `--credential-source convex` (or `OPERATOR_QA_CREDENTIAL_SOURCE=convex`)
...
- `OPERATOR_QA_CONVEX_SITE_URL` (for example `https://your-deployment.convex.site`)
- One secret for the selected role:
  - `OPERATOR_QA_CONVEX_SECRET_MAINTAINER` for `maintainer`
  - `OPERATOR_QA_CONVEX_SECRET_CI` for `ci`
- Credential role selection:
  - CLI: `--credential-role maintainer|ci`
  - Env default: `OPERATOR_QA_CREDENTIAL_ROLE` (defaults to `ci` in CI, `maintainer` otherwise)

Optional env vars:

- `OPERATOR_QA_CREDENTIAL_LEASE_TTL_MS` (default `1200000`)
- `OPERATOR_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS` (default `30000`)
- `OPERATOR_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS` (default `90000`)
- `OPERATOR_QA_CREDENTIAL_HTTP_TIMEOUT_MS` (default `15000`)
- `OPERATOR_QA_CONVEX_ENDPOINT_PREFIX` (default `/qa-credentials/v1`)
- `OPERATOR_QA_CREDENTIAL_OWNER_ID` (optional trace id)
- `OPERATOR_QA_ALLOW_INSECURE_HTTP=1` allows loopback `http://` Convex URLs for local-only development.

`OPERATOR_QA_CONVEX_SITE_URL` should use `https://` in normal operation.

Maintainer admin commands (pool add/remove/list) require
`OPERATOR_QA_CONVEX_SECRET_MAINTAINER` specifically.

CLI helpers for maintainers:

```bash
pnpm operator qa credentials doctor
pnpm operator qa credentials add --kind telegram --payload-file qa/telegram-credential.json
pnpm operator qa credentials list --kind telegram
pnpm operator qa credentials remove --credential-id <credential-id>
```
...
Default endpoint contract (`OPERATOR_QA_CONVEX_SITE_URL` + `/qa-credentials/v1`).
...
plugin manifest, mount as `operator qa <runner>`, and author scenarios under
`qa/scenarios/`.
...
- Compare native opus performance in the `libopus-wasm` benchmark repo, not
  in default Operator install/test loops. Do not set `@discordjs/opus` to
...
<AccordionGroup>
  <Accordion title="Projects, shards, and scoped lanes">
    ...
    - `pnpm test:changed` expands changed git paths into cheap scoped lanes by default: direct test edits, sibling `*.test.ts` files, explicit source mappings, and local import-graph dependents. Config/setup/package edits do not broad-run tests unless you explicitly use `OPERATOR_TEST_CHANGED_BROAD=1 pnpm test:changed`.
    ...
    - Import-light unit tests from agents, commands, plugins, auto-reply helpers, `plugin-sdk`, and similar pure utility areas route through the `unit-fast` lane, which skips `test/setup-operator-runtime.ts`; stateful/runtime-heavy files stay on the existing lanes.
  ...
  <Accordion title="Vitest pool and isolation defaults">
    ...
    - `scripts/run-vitest.mjs` adds `--no-maglev` for Vitest child Node
      ...
      Set `OPERATOR_VITEST_ENABLE_MAGLEV=1` to compare against stock V8
      behavior.
    - `scripts/run-vitest.mjs` terminates explicit non-watch Vitest runs
      after 5 minutes with no stdout or stderr output. Set
      `OPERATOR_VITEST_NO_OUTPUT_TIMEOUT_MS=0` to disable the watchdog for
  ...
  <Accordion title="Fast local iteration">
    ...
    - `pnpm test:changed` routes through cheap scoped lanes by default. Use
      `OPERATOR_TEST_CHANGED_BROAD=1 pnpm test:changed` only when the agent
    ...
    - The base Vitest config marks the projects/config files as
      ...
      wiring changes.
    - The config keeps `OPERATOR_VITEST_FS_MODULE_CACHE` enabled on
      supported hosts; set `OPERATOR_VITEST_FS_MODULE_CACHE_PATH=/abs/path`
...
- Useful overrides:
  - `OPERATOR_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `OPERATOR_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises Operator's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `OPERATOR_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `OPERATOR_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
  - `OPERATOR_E2E_OPENSHELL_CONFIG_HOME=/path/to/config` to expose the registered gateway config to the isolated test
  - `OPERATOR_E2E_OPENSHELL_HOST_IP=172.18.0.1` to override the Docker gateway IP used by the host policy fixture
...
- Default: **enabled** by `pnpm test:live` (sets `OPERATOR_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.operator`.
- Set `OPERATOR_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` defaults to a quieter mode: it keeps `[live] ...` progress output and mutes gateway bootstrap logs/Bonjour chatter. Set `OPERATOR_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `OPERATOR_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `OPERATOR_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `OPERATOR_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Docker live runners keep their own practical caps where needed:
  ...
  `test:docker:live-gateway` defaults to `OPERATOR_LIVE_GATEWAY_SMOKE=1`,
  `OPERATOR_LIVE_GATEWAY_MAX_MODELS=8`,
  `OPERATOR_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `OPERATOR_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Set `OPERATOR_LIVE_MAX_MODELS`
  or the gateway env vars when you explicitly want a smaller cap or larger scan.
- `test:docker:all` builds the live Docker image once via `test:docker:live-build`, packs Operator once as an npm tarball through `scripts/package-operator-for-docker.mjs`, then builds/reuses two `scripts/e2e/Dockerfile` images. The bare image is only the Node/Git runner for install/update/plugin-dependency lanes; those lanes mount the prebuilt tarball. The functional image installs the same tarball into `/app` for built-app functionality lanes. Docker lane definitions live in `scripts/lib/docker-e2e-scenarios.mjs`; planner logic lives in `scripts/lib/docker-e2e-plan.mjs`; `scripts/test-docker-all.mjs` executes the selected plan. The aggregate uses a weighted local scheduler: `OPERATOR_DOCKER_ALL_PARALLELISM` controls process slots, while resource caps keep heavy live, npm-install, and multi-service lanes from all starting at once. If a single lane is heavier than the active caps, the scheduler can still start it when the pool is empty and then keeps it running alone until capacity is available again. Defaults are 10 slots, `OPERATOR_DOCKER_ALL_LIVE_LIMIT=9`, `OPERATOR_DOCKER_ALL_NPM_LIMIT=5`, and `OPERATOR_DOCKER_ALL_SERVICE_LIMIT=7`; tune `OPERATOR_DOCKER_ALL_WEIGHT_LIMIT` or `OPERATOR_DOCKER_ALL_DOCKER_LIMIT` (and other `OPERATOR_DOCKER_ALL_<RESOURCE>_LIMIT` overrides) only when the Docker host has more headroom. The runner performs a Docker preflight by default, removes stale Operator E2E containers, prints status every 30 seconds, stores successful lane timings in `.artifacts/docker-tests/lane-timings.json`, and uses those timings to start longer lanes first on later runs. Use `OPERATOR_DOCKER_ALL_DRY_RUN=1` to print the weighted lane manifest without building or running Docker, or `node scripts/test-docker-all.mjs --plan-json` to print the CI plan for selected lanes, package/image needs, and credentials.
...
- Docker/Bash E2E lanes that install the packed Operator tarball through `scripts/lib/operator-e2e-instance.sh` cap `npm install` at `OPERATOR_E2E_NPM_INSTALL_TIMEOUT` (default `600s`; set `0` to disable the wrapper for debugging).
...
- Npm tarball onboarding/channel/agent smoke: `pnpm test:docker:npm-onboard-channel-agent` installs the packed Operator tarball globally in Docker, configures OpenAI via env-ref onboarding plus Telegram by default, runs doctor, and runs one mocked OpenAI agent turn. Reuse a prebuilt tarball with `OPERATOR_CURRENT_PACKAGE_TGZ=/path/to/operator-*.tgz`, skip the host rebuild with `OPERATOR_NPM_ONBOARD_HOST_BUILD=0`, or switch channel with `OPERATOR_NPM_ONBOARD_CHANNEL=discord` or `OPERATOR_NPM_ONBOARD_CHANNEL=slack`.
...
- Install Smoke CI skips the duplicate direct-npm global update with `OPERATOR_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1`; run the script locally without that env when direct `npm install -g` coverage is needed.
- Agents delete shared workspace CLI smoke: `pnpm test:docker:agents-delete-shared-workspace` (script: `scripts/e2e/agents-delete-shared-workspace-docker.sh`) builds the root Dockerfile image by default, seeds two agents with one workspace in an isolated container home, runs `agents delete --json`, and verifies valid JSON plus retained workspace behavior. Reuse the install-smoke image with `OPERATOR_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE=operator-dockerfile-smoke:local OPERATOR_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD=1`.
...
- Operator bundle MCP tools (real stdio MCP server + embedded Operator profile allow/deny smoke): `pnpm test:docker:agent-bundle-mcp-tools` (script: `scripts/e2e/agent-bundle-mcp-tools-docker.sh`)
...
- Plugins (install/update smoke for local path, `file:`, npm registry with hoisted dependencies, malformed npm package metadata, git moving refs, ClawHub kitchen-sink, marketplace updates, and Claude-bundle enable/inspect): `pnpm test:docker:plugins` (script: `scripts/e2e/plugins-docker.sh`)
  Set `OPERATOR_PLUGINS_E2E_CLAWHUB=0` to skip the ClawHub block, or override the default kitchen-sink package/runtime pair with `OPERATOR_PLUGINS_E2E_CLAWHUB_SPEC` and `OPERATOR_PLUGINS_E2E_CLAWHUB_ID`. Without `OPERATOR_CLAWHUB_URL`/`CLAWHUB_URL`, the test uses a hermetic local ClawHub fixture server.
- Plugin update unchanged smoke: `pnpm test:docker:plugin-update` (script: `scripts/e2e/plugin-update-unchanged-docker.sh`)
- Plugin lifecycle matrix smoke: `pnpm test:docker:plugin-lifecycle-matrix` installs the packed Operator tarball in a bare container, installs an npm plugin, toggles enable/disable, upgrades and downgrades it through a local npm registry, deletes the installed code, then verifies uninstall still removes stale state while logging RSS/CPU metrics for each lifecycle phase.
...
```bash
OPERATOR_DOCKER_E2E_IMAGE=operator-docker-e2e-functional:local pnpm test:docker:e2e-build
OPERATOR_DOCKER_E2E_IMAGE=operator-docker-e2e-functional:local OPERATOR_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels
```

Suite-specific image overrides such as `OPERATOR_GATEWAY_NETWORK_E2E_IMAGE` still win when set. When `OPERATOR_SKIP_DOCKER_BUILD=1` points at a remote shared image, the scripts pull it if it is not already local. The QR and installer Docker tests keep their own Dockerfiles because they validate package/install behavior rather than the shared built-app runtime.
...
outputs such as `.pnpm-store`, `.worktrees`, `__operator_vitest__`, and
...
`OPERATOR_SKIP_CHANNELS=1` so gateway live probes do not start real
...
`OPERATOR_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
...
container, starts a second container that spawns `operator mcp serve`, then
...
embedded Operator bundle MCP runtime, executes the tool, then verifies
...
- `OPERATOR_CONFIG_DIR=...` (default: `~/.operator`) mounted to `/home/node/.operator`
...
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `OPERATOR_LIVE_PROVIDERS` / `OPERATOR_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `OPERATOR_DOCKER_AUTH_DIRS=all`, `OPERATOR_DOCKER_AUTH_DIRS=none`, or a comma list like `OPERATOR_DOCKER_AUTH_DIRS=.claude,.codex`
- `OPERATOR_LIVE_GATEWAY_MODELS=...` / `OPERATOR_LIVE_MODELS=...` to narrow the run
- `OPERATOR_LIVE_GATEWAY_PROVIDERS=...` / `OPERATOR_LIVE_PROVIDERS=...` to filter providers in-container
- `OPERATOR_SKIP_DOCKER_BUILD=1` to reuse an existing `operator:local-live` image for reruns that do not need a rebuild
- `OPERATOR_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `OPERATOR_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `OPERATOR_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
