...
Set `OPERATOR_PLUGIN_LIFECYCLE_TRACE=1` for a phase-by-phase breakdown of plugin metadata, discovery, registry, runtime mirror, config mutation, and refresh work. Writes to stderr, so JSON command output stays parseable.

```bash
OPERATOR_PLUGIN_LIFECYCLE_TRACE=1 operator plugins install tokenjuice --force
```
...
Use this before reaching for a CPU profiler. From a source checkout, measure the built runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm operator ...` also measures source-runner overhead.
...
For one-off profiling through the normal source runner, set `OPERATOR_RUN_NODE_CPU_PROF_DIR`:

```bash
OPERATOR_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm operator status
```
...
```bash
OPERATOR_TRACE_SYNC_IO=1 pnpm operator gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched Gateway child; set `OPERATOR_TRACE_SYNC_IO=1` when you want sync I/O trace output in watch mode too.
...
By default this starts or restarts a tmux session named `operator-gateway-watch-<profile>` (for example `operator-gateway-watch-main`), with a port suffix such as `operator-gateway-watch-dev-19001` added only when `OPERATOR_GATEWAY_PORT` differs from the default port `18789`. It auto-attaches from interactive terminals; non-interactive shells, CI, and agent exec calls stay detached and print attach instructions instead:

```bash
tmux attach -t operator-gateway-watch-main
# Read recent output without attaching
tmux capture-pane -ep -t operator-gateway-watch-main -S -200
```
...
```bash
pnpm operator gateway start
```

When an explicit `--port` or `OPERATOR_GATEWAY_PORT` differs from the installed service's effective port, the wrapper leaves the service running so both Gateways can run side by side.
...
# or
OPERATOR_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Raw mode does not manage the installed service. Run `pnpm operator gateway stop` first when it uses the same port.

Keep tmux management but disable auto-attach:

```bash
OPERATOR_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
Benchmark mode suppresses sync-I/O trace spam by default. Set `OPERATOR_TRACE_SYNC_IO=1` with `--benchmark` to get both CPU profiles and sync-I/O stack traces; in benchmark mode those trace blocks go to `gateway-watch-output.log` under the benchmark directory (filtered from the terminal pane), while normal Gateway logs stay visible.

The tmux wrapper carries common non-secret runtime selectors into the pane, including `OPERATOR_PROFILE`, `OPERATOR_CONFIG_PATH`, `OPERATOR_STATE_DIR`, `OPERATOR_GATEWAY_PORT`, and `OPERATOR_SKIP_CHANNELS`. Put provider credentials in your normal profile/config, or use raw foreground mode for one-off ephemeral secrets.

If the watched Gateway exits during startup, the watcher runs `operator doctor --fix --non-interactive` once and restarts the Gateway child. Set `OPERATOR_GATEWAY_WATCH_AUTO_DOCTOR=0` to see the original startup failure without the dev-only repair pass.
...
The watcher restarts on build-relevant files under `src/`, extension source files, extension `package.json` and `operator.plugin.json` metadata, `tsconfig.json`, `package.json`, and `tsdown.config.ts`. Extension metadata changes restart the gateway without forcing a rebuild; source and config changes still rebuild `dist` first.
...
- **Global `--dev` (profile):** isolates state under `~/.operator-dev` and defaults the gateway port to `19001` (derived ports shift with it).
...
```bash
pnpm gateway:dev
OPERATOR_PROFILE=dev operator tui
```

Without a global install, run the CLI via `pnpm operator ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `OPERATOR_PROFILE=dev`
   - `OPERATOR_STATE_DIR=~/.operator-dev`
   - `OPERATOR_CONFIG_PATH=~/.operator-dev/operator.json`
   - `OPERATOR_GATEWAY_PORT=19001` (browser/canvas ports shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - `pnpm gateway:dev` also sets `OPERATOR_SKIP_CHANNELS=1` to skip channel providers.
...
```bash
OPERATOR_PROFILE=dev operator gateway --dev --reset
```
...
Operator can log the **raw assistant stream** before any filtering/formatting. This is the best way to see whether reasoning is arriving as plain text deltas (or as separate thinking blocks).
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.operator/logs/raw-stream.jsonl
```

Equivalent env vars:

```bash
OPERATOR_RAW_STREAM=1
OPERATOR_RAW_STREAM_PATH=~/.operator/logs/raw-stream.jsonl
```

Default file: `~/.operator/logs/raw-stream.jsonl`
...
- To use the built CLI for other tasks (for example `dashboard --no-open` if your debug session spawns a new auth token), run it from another terminal: `node ./operator.mjs` or an alias like `alias operator-build="node $(pwd)/operator.mjs"`.
