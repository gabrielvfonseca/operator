...
- Prefer CLI surfaces when they exist (example: `operator models status --check`).
...
- `scripts/claude-auth-status.sh [full|json|simple]` - checks Claude Code + Operator auth status.
- `scripts/auth-monitor.sh` - polls status and sends a notification (via Operator send, and/or ntfy.sh) when the token nears expiry. Env: `WARN_HOURS` (default `2`), `NOTIFY_PHONE`, `NOTIFY_NTFY`. Run on a schedule via the bundled `scripts/systemd/operator-auth-monitor.{service,timer}` (every 30 minutes).
...
- `OPERATOR_GH_READ_APP_ID`
- `OPERATOR_GH_READ_PRIVATE_KEY_FILE`

Optional env:

- `OPERATOR_GH_READ_INSTALLATION_ID` when you want to skip repo-based installation lookup
- `OPERATOR_GH_READ_PERMISSIONS` as a comma-separated override for the read permission subset to request
