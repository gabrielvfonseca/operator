---
summary: "Operator code mode: an opt-in compact tool surface backed by QuickJS-WASI and a hidden run-scoped tool catalog"
title: "Code mode"
sidebarTitle: "Code mode"
read_when:
  - You want to enable Operator code mode for an agent run
...
Code mode is an experimental, opt-in Operator agent-runtime feature. When
...
This page documents Operator code mode, not Codex Code Mode. The two features
...
- Operator code mode runs in the generic Operator agent runtime and is
...
- Every catalog-eligible enabled tool (Operator core, plugin, MCP, client) is hidden from
...
- Provider neutral: works for Operator, plugin, MCP, and client tools without
...
```bash
OPERATOR_DEBUG_CODE_MODE=1 \
OPERATOR_DEBUG_MODEL_TRANSPORT=1 \
OPERATOR_DEBUG_MODEL_PAYLOAD=tools \
operator gateway
```
...
`OPERATOR_DEBUG_MODEL_PAYLOAD=full-redacted` for a short debugging session.
...
| Stability           | experimental Operator surface (Codex Code Mode is a separate, stable Codex harness surface) |
| Target surface      | generic Operator agent runs                                                                 |
...
- **Code mode**: the Operator runtime mode that hides catalog-compatible model
...
If code mode is enabled but QuickJS-WASI cannot load, Operator fails closed
...
1. Resolve the agent, model, provider, sandbox, channel, sender, and run
   policy.
2. Build the effective Operator tool list, adding eligible plugin, MCP, and
   client tools.
...
when `tools.codeMode.enabled: true` is configured. Code mode and Operator Tool
...
- `command` is accepted as an exec-compatible alias for hook policies and
  trusted rewrites (the normal Operator shell exec tool also uses a `command`
...
- If `language` is `"typescript"`, Operator transpiles before evaluation.
...
final value is JSON-compatible after Operator's output adapter runs.
...
`wait` exists because nested Operator tools can be slow, interactive, approval
...
2. On suspension, Operator snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot and
   re-registers host callbacks by stable names.
4. Operator delivers nested tool results into the restored VM and drains
...
type ToolCatalogEntry = {
  ...
  label?: string;
  description: string;
  source: "@gabrielvfonseca/operator" | "mcp" | "client";
  sourceName?: string;
};
```

Plugin tools use `source: "@gabrielvfonseca/operator"` with `sourceName` set to the owning
...
const hits = await tools.web_search({ query: "Operator code mode" });
```
...
directory. For each code-mode `exec` call, Operator builds the run-scoped tool
...
catalog for arbitrary enabled Operator, plugin, and client tools; use `MCP`
...
ordinary Operator tools, not through the code-mode bridge.

## Tool catalog

The hidden catalog includes tools after effective policy filtering, in this
order: Operator core tools, bundled plugin tools, external plugin tools, MCP
...
```text
operator:core:message
operator:browser:browser_request
...
Code mode supersedes the Operator Tool Search model surface for runs where it
is active.

When `tools.codeMode.enabled` is true and code mode activates:

- Operator does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- MCP calls use the generated `MCP` namespace and its `$api()` headers instead
  of `tools.call(...)`.
- Nested calls dispatch through the same Operator executor path that Tool
  Search uses.

See [Tool Search](/tools/tool-search) for the Operator compact catalog bridge
that code mode supersedes for active runs.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal Operator
shell `exec` tool is enabled, it is hidden from the model and cataloged like
any other tool.

Inside the guest runtime:

- `tools.call("operator:core:exec", input)` can call the shell exec tool if
  policy allows it.
...
If two tools normalize to the same safe convenience name, Operator omits the
convenience function and requires `tools.call(id, input)`.

## Nested tool execution

Every nested tool call crosses the host bridge and re-enters Operator,
...
- For read-only work, `exec` can set `restartSafe: true`. Operator then rejects
...
- Operator caps the number of concurrently suspended runs per process (64) and
...
Operator loads `quickjs-wasi` as a direct dependency in the owning package; it
...
The runtime executes in a Node.js worker thread, outside Operator's main
...
tool inputs beyond existing Operator trajectory policy.
...
OPERATOR_DEBUG_MODEL_TRANSPORT=1 \
OPERATOR_DEBUG_MODEL_PAYLOAD=tools \
OPERATOR_DEBUG_SSE=events \
operator gateway
```

For payload-shape debugging, use `OPERATOR_DEBUG_MODEL_PAYLOAD=full-redacted`.
...
For stream debugging, use `OPERATOR_DEBUG_SSE=peek` to log the first five
...
- `tools.search`, `tools.describe`, and `tools.call` work for Operator tools
...
5. Send an agent turn with Operator, plugin, MCP, and client test tools.
...
8. In `exec`, call Operator/plugin/client tools through `tools.search`,
