...
Operator currently exposes three user-facing update channels:
...
```bash
gh workflow run operator-npm-release.yml \
...
8. Run `Operator NPM Release` with `preflight_only=true` against the Release SHA/tag. Save the successful `preflight_run_id`. This builds and checks the exact package bytes that include the final changelog.
...
5. Run `pnpm release:generated:check`, `pnpm deps:shrinkwrap:check`, and `OPERATOR_TESTBOX=1 pnpm check:changed`. Push, then verify `origin/main` contains the shipped version and changelog before calling the stable release done.
...
- Plugin version sync updates the publishable `@gabrielvfonseca/ai` runtime package, official plugin package versions, and existing `operator.compat.pluginApi` floors to the Operator release version by default. Treat that field as the plugin SDK/runtime API floor, not just a copy of the package version: for plugin-only releases that intentionally remain compatible with older Operator hosts, keep the floor at the oldest supported host API and document that choice in the plugin release proof.
- Run the manual `Full Release Validation` workflow before release approval to kick off all pre-release test boxes from one entrypoint. It accepts a branch, tag, or full commit SHA, dispatches manual `CI`, and dispatches `Operator Release Checks` for install smoke, package acceptance, cross-OS package checks, QA Lab parity, Matrix, and Telegram lanes. Stable and full runs always include exhaustive live/E2E and Docker release-path soak; `run_release_soak=true` is retained for an explicit beta soak. Package Acceptance provides the canonical package Telegram E2E during candidate validation, avoiding a second concurrent live poller.
...
- Release checks now run in a separate manual workflow: `Operator Release Checks`. It also runs the QA Lab mock parity lane plus the Matrix release profile and Telegram QA lane before release approval. The live lanes use the `qa-live-shared` environment; Telegram also uses Convex CI credential leases. Run the manual `QA-Lab - All Lanes` workflow with `matrix_profile=all` when you want every maintained Matrix scenario; the workflow fans that selection across the transport, media, and E2EE profiles to keep full proof within per-job timeouts.
- Cross-OS install and upgrade runtime validation is part of public `Operator Release Checks` and `Full Release Validation`, which call the reusable workflow `.github/workflows/operator-cross-os-release-checks-reusable.yml` directly. This split is intentional: keep the real npm release path short, deterministic, and artifact-focused, while slower live checks stay in their own lane so they do not stall or block publish.
...
- That workflow runs `OPERATOR_LIVE_TEST=1 OPERATOR_LIVE_CACHE_TEST=1 pnpm test:live:cache` using both `OPENAI_API_KEY` and `ANTHROPIC_API_KEY` workflow secrets.
...
The Docker box lives in `Operator Release Checks` through `operator-live-and-e2e-checks-reusable.yml`, plus the release-mode `install-smoke` workflow. It validates the release candidate through packaged Docker environments instead of only source-level tests.
...
The QA Lab box is also part of `Operator Release Checks`. It is the agentic behavior and channel-level release gate, separate from Vitest and Docker package mechanics.
...
The Package box is the installable-product gate. It is backed by `Package Acceptance` and the resolver `scripts/resolve-operator-package-candidate.mjs`. The resolver normalizes a candidate into the `package-under-test` tarball consumed by Docker E2E, validates the package inventory, records the package version and SHA-256, and keeps the workflow harness ref separate from the package source ref.
...
`Operator Release Publish` is the normal mutating entrypoint. The monthly
...
6. Dispatch `Operator NPM Release` with the release tag, npm dist-tag, and saved `preflight_run_id` after verifying the saved `full_release_validation_run_id` and exact run attempt.
...
gh workflow run operator-release-publish.yml \
  ...
  -f windows_node_installer_digests='{"OperatorCompanion-Setup-x64.exe":"sha256:<approved-x64-sha256>","OperatorCompanion-Setup-arm64.exe":"sha256:<approved-arm64-sha256>"}' \
  -f preflight_run_id=<successful-operator-npm-preflight-run-id> \
  -f full_release_validation_run_id=<successful-full-release-validation-run-id> \
  -f full_release_validation_run_attempt=<successful-full-release-validation-run-attempt> \
  -f npm_dist_tag=beta
...
Use the lower-level `Plugin NPM Release` and `Plugin ClawHub Release` workflows only for focused repair or republish work. `Operator Release Publish` rejects `plugin_publish_scope=selected` when `publish_operator_npm=true` so the core package cannot ship without every publishable official plugin, including `@gabrielvfonseca/diffs-language-pack`. For a selected plugin repair, set `publish_operator_npm=false` with `plugin_publish_scope=selected` and `plugins=@gabrielvfonseca/name`, or dispatch the child workflow directly.
...
gh workflow run plugin-clawhub-new.yml \
  --ref main \
  -f plugins=@gabrielvfonseca/name \
...
release tag plus the parent `Operator Release Publish` run id, attempt, and
...
`Operator NPM Release` accepts these operator-controlled inputs:
...
- `release_publish_run_id`: approved `Operator Release Publish` run id; required when this workflow is dispatched by that parent (bot-actor real-publish calls)
...
`Operator Release Publish` accepts these operator-controlled inputs:
...
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only for focused plugin-only repair work with `publish_operator_npm=false`
- `plugins`: comma-separated `@gabrielvfonseca/*` package names when `plugin_publish_scope=selected`
- `publish_operator_npm`: defaults to `true`; set `false` only when using the workflow as a plugin-only repair orchestrator
...
`Operator Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks require the resolved commit to be reachable from an Operator branch or release tag.
...
- For `Operator NPM Release`, full commit SHA input is allowed only when `preflight_only=true`
- `Operator Release Checks` and `Full Release Validation` are always validation-only
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/gabrielvfonseca/operator/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/gabrielvfonseca/operator/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/operator-npm-release.yml`](https://github.com/gabrielvfonseca/operator/blob/main/.github/workflows/operator-npm-release.yml)
- [`.github/workflows/operator-release-checks.yml`](https://github.com/gabrielvfonseca/operator/blob/main/.github/workflows/operator-release-checks.yml)
- [`.github/workflows/operator-cross-os-release-checks-reusable.yml`](https://github.com/gabrielvfonseca/operator/blob/main/.github/workflows/operator-cross-os-release-checks-reusable.yml)
- [`scripts/resolve-operator-package-candidate.mjs`](https://github.com/gabrielvfonseca/operator/blob/main/scripts/resolve-operator-package-candidate.mjs)
- [`scripts/operator-npm-release-check.ts`](https://github.com/gabrielvfonseca/operator/blob/main/scripts/operator-npm-release-check.ts)
