...
`test/helpers/operator-test-instance.ts`, but the live proof path should inspect
...
SQLite before runtime reads begin. It must not run `operator doctor --fix`
...
`operator doctor --session-sqlite inspect` and
`operator doctor --session-sqlite validate` as diagnostic evidence. Those
...
`operator doctor --session-sqlite validate` is informational only in live mode.
