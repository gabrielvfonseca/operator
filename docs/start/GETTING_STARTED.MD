---
summary: "Get Operator installed and run your first chat in minutes."
read_when:
...
Install Operator, run onboarding, and chat with your AI assistant in about 5
...
/>
  ...
  <Step title="Run onboarding">
    ...
    steps and return later with `operator configure`.
...
```bash
mkdir -p "$HOME/.operator/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.operator/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Operator as a service account or want custom paths:

- `OPERATOR_HOME` — home directory for internal path resolution
- `OPERATOR_STATE_DIR` — override the state directory
- `OPERATOR_CONFIG_PATH` — override the config file path
