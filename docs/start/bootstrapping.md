...
On the first run against a brand-new workspace (default `~/.operator/workspace`),
Operator:
...
For embedded or local-model runs, Operator keeps `BOOTSTRAP.md` out of the
...
host (for example, `user@gateway-host:~/.operator/workspace`).
</Note>
