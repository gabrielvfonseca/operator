...
read_when:
  - Moving Operator runtime data, cache, transcripts, task state, or scratch files into SQLite
...
- Global database: `~/.operator/state/operator.sqlite`
...
- Configuration stays file-backed: `operator.json` remains outside the
  database. Runtime auth profiles move to SQLite; external provider or CLI
  credential files remain owner-managed outside Operator's database.
...
- Raw stream logging uses `OPERATOR_RAW_STREAM=1` plus SQLite diagnostics rows.
  The old pi-mono `PI_RAW_STREAM`, `PI_RAW_STREAM_PATH`, and
  `raw-openai-completions.jsonl` file logger contract is not part of Operator
  runtime or tests.
...
- One global SQLite database owns control-plane state:
  `state/operator.sqlite`.
- One per-agent SQLite database owns data-plane state:
  `agents/<agentId>/agent/operator-agent.sqlite`.
- Config remains file-backed. `operator.json` is not part of this database
  refactor.
...
- Task registry: `clean`. Task and Task Flow runtime rows live in
  `state/operator.sqlite`; unshipped sidecar SQLite importers are deleted.
...
- [x] Keep Kysely generated types aligned after any schema change.
      Files: `src/state/operator-state-schema.sql`,
...
- `operator doctor --fix` owns legacy file-to-database migration. Runtime
...
- The old runtime-owned JSONL transcript streaming helper was deleted. Doctor
  ...
  SQLite rows.
- Codex app-server bindings use the Operator `sessionId` as the canonical
...
former stores now write to `state/operator.sqlite` or the owning
`operator-agent.sqlite` database.
...
- Plugin migrations are data migrations over plugin-owned namespaces, not host
  ...
  `operator-state-schema.sql` unless the host itself is taking ownership of a
  new cross-plugin contract.
- `src/state/operator-agent-db.ts` opens
  `agents/<agentId>/agent/operator-agent.sqlite`, registers the database in the
...
- Per-agent external conversation identity is relational too:
  ...
  `session_conversations` links one Operator session to one or more external
  ...
  Operator session can represent multiple external DM peers without demoting
...
- TUI last-session restore pointers now live in typed shared
  ...
  excludes heartbeat sessions. `operator doctor --fix` strictly validates the
...
- Secret target metadata now talks about stores instead of pretending every
  credential target is a config file. `operator.json` remains the config store;
...
- Workspace bootstrap completion markers now live in typed shared
  ...
  `.operator/workspace-state.json`; runtime no longer reads or rewrites the
  legacy workspace marker, and helper APIs no longer pass around a fake
  `.operator/setup-state` path just to derive storage identity.
- Exec approvals now live in the typed shared SQLite `exec_approvals_config`
  singleton row. Doctor imports legacy `~/.operator/exec-approvals.json`;
  ...
  `state/operator.sqlite` table row; it keeps only the Unix prompt socket on disk
...
- GitHub Copilot compaction no longer writes `operator-compaction-*.json`
  workspace sidecars. The harness calls the SDK history compaction RPC for the
  tracked SDK session, and Operator keeps durable session/transcript state in
  SQLite instead of compatibility marker files.
- The shared Swift runtime (`OperatorKit`) uses the same
  `state/operator.sqlite` rows for device identity and device auth. macOS app
...
- Non-doctor commands do not auto-run legacy config repair. For example,
  `operator update --channel` now fails on invalid legacy config and asks the
...
- APNs runtime reads and writes only `apns_registrations`. Explicit
  `operator doctor --fix` strictly imports the retired
...
- Node-host config now uses a typed singleton row in the shared SQLite database.
  ...
  remains; explicit `operator doctor --fix` strictly imports and removes it
...
- Plugin state now uses the shared `state/operator.sqlite` database. The old
...
- Task and Task Flow runtime tables now live in the shared
  `state/operator.sqlite` database instead of `tasks/runs.sqlite` and
...
- Session target resolution now exposes per-agent database targets, not legacy
  ...
  `operator sessions` enumerate `agent_databases` plus configured agents.
...
- Cache trace, Anthropic payload, raw stream, and diagnostics timeline records
  ...
  `diagnostics.cacheTrace.filePath`, `OPERATOR_CACHE_TRACE_FILE`,
  `OPERATOR_ANTHROPIC_PAYLOAD_LOG_FILE`, and
  `OPERATOR_DIAGNOSTICS_TIMELINE_PATH` JSONL override paths are removed, and
...
- Memory Wiki activity logs now use SQLite plugin state instead of
  `.operator-wiki/log.jsonl`. The Memory Wiki migration provider imports old
  JSONL logs; wiki markdown and user vault content stay file-backed as
  workspace content.
- Memory Wiki no longer creates `.operator-wiki/state.json` or the unused
  `.operator-wiki/locks` directory. The migration provider removes those retired
...
- The macOS companion no longer writes app-local `logs/config-audit.jsonl` or
  `logs/config-health.json` sidecars while editing `operator.json`. The config
...
- Codex app-server conversation bindings now key SQLite plugin state by
  Operator session key or explicit `{agentId, sessionId}` scope. They must not
...
- Zalo hosted outbound media now uses shared SQLite `plugin_blob_entries`
  instead of `operator-zalo-outbound-media` JSON/bin temp sidecars.
...
- ACPX generated wrapper scripts and the isolated Codex home are temporary
  materialization under the Operator temp root, not durable Operator state. The
...
- Gateway media attachments now use the shared `media_blobs` SQLite table as
  ...
  `$OPERATOR_STATE_DIR/media` or config-dir `media` roots; those directories are
  doctor import sources only.
- Shell completion no longer writes `$OPERATOR_STATE_DIR/completions/*` cache
...
- Subagent inline attachments no longer materialize under workspace
  `.operator/attachments/*`. The spawn path prepares SQLite VFS seed entries,
...
- CLI image hydration no longer maintains stable `operator-cli-images` cache
...
- `operator secrets apply` scrubs the config file, env file, and SQLite
...
- User-facing auth docs now describe
  `state/operator.sqlite#table/auth_profile_stores/<agentDir>` instead of
...
- Memory Wiki imported-source sync ledgers now store one SQLite plugin-state row
  per vault/source key instead of rewriting `.operator-wiki/source-sync.json`;
...
- Memory Wiki ChatGPT import-run records now store one SQLite plugin-state row
  per vault/run id instead of writing `.operator-wiki/import-runs/*.json`.
...
- Memory Wiki compiled digests now store SQLite plugin blob rows instead of
  writing `.operator-wiki/cache/agent-digest.json` and
  `.operator-wiki/cache/claims.jsonl`. The migration provider imports old cache
...
- ACPX wrapper scripts and the isolated Codex home are generated in the
  Operator temp root. They are recreated as needed and are not backup or
  migration inputs.
...
- Backup stages the state directory before archiving, copies non-database files,
  ...
  completed backup runs in SQLite with the archive manifest. `operator backup
create` validates the written archive by default; `--no-verify` is the
  explicit fast path.
- `operator backup restore` validates the archive before extraction, reuses the
...
- `operator reset --scope config+creds+sessions` removes per-agent
  `operator-agent.sqlite` databases plus WAL/SHM sidecars, not only legacy
...
- Generated model catalog config is stored in typed global SQLite
  ...
  `ensureOperatorModelCatalog`; there is no `models.json` compatibility API in
...
- QMD's own `index.sqlite` is now a temp runtime materialization backed by the
  ...
  `~/.operator/agents/<agentId>/qmd` sidecar.
- The optional `memory-lancedb` plugin no longer creates
  `~/.operator/memory/lancedb` as an implicit Operator-managed store. It is an
...
`operator doctor --fix` invokes the state migration implementation after
ordinary config preflight and creates a verified backup before import. Runtime
startup and `operator migrate` must not import legacy Operator state files.
...
- Add a single doctor-owned state migration service for file-to-database import.
  Done: `operator doctor --fix` uses the legacy-state migration implementation.
...
Keep shared coordination state in `state/operator.sqlite`:
...
```text
~/.operator/state/operator.sqlite
...
- Verify by opening every DB snapshot and running `PRAGMA integrity_check`.
  `operator backup create` does this archive verification by default;
...
5. Verify every SQLite snapshot's file shape, then open canonical Operator
   ...
   `operator backup create` does this by default; `--no-verify` is only for
...
- Use `{ agentId, sessionKey }` row operations.
  ...
  and the `operator sessions` listing command now read per-agent rows directly
...
8. Backup integration.
   ...
   - Add restore from verified archive snapshots. Done: `operator backup
...
- Remove tests that assert runtime creation of `sessions.json` or transcript
  ...
  Operator test-state helper no longer exposes a legacy `auth-profiles.json`
...
- ad-hoc `operator-state.sqlite` runtime sidecars
- `tasks/runs.sqlite`
...
- Memory Wiki `.operator-wiki/locks/`
- Memory Wiki `.operator-wiki/source-sync.json`
- Memory Wiki `.operator-wiki/import-runs/*.json`
- Memory Wiki `.operator-wiki/cache/agent-digest.json`
- Memory Wiki `.operator-wiki/cache/claims.jsonl`
- ClawHub `.clawhub/lock.json`
- ClawHub `.clawhub/origin.json`
- Browser profile decoration `.operator-profile-decorated`
