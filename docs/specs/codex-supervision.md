...
summary: "Architecture and product boundary for supervising native Codex sessions from Operator."
read_when:
...
Codex supervision lets an Operator operator discover native Codex sessions and,
when safe, create a local branch through the normal Operator Chat surface.
Codex App Server remains the thread and model-loop owner. Operator supplies the
...
- native thread bindings to Operator sessions
...
Codex Desktop -> private stdio App Server -> user Codex home
                                             ^
Operator Codex plugin -> supervision App Server connection
...
Ordinary Operator Codex sessions -> managed agent-home stdio by default
  -> ordinary full harness threads -> Operator Chat and channel delivery
```
...
process-local; a thread unknown to Operator's supervision process is `notLoaded`
...
unbounded `thread/read`. Operator also rejects any serialized item page above
...
`thread/resume`, subscribe the Operator client to live thread requests, or
answer an approval.
...
```text
operator codex sessions [--search <text>] [--host <id>] [--limit <count>] [--cursor <cursor>] [--json] [gateway-options]
operator codex continue <thread-id> [--json] [gateway-options]
operator codex archive <thread-id> --confirm-no-other-runner [--json] [gateway-options]
```
...
2. Otherwise projects bounded user and assistant history through the source's
   ...
   Operator Chat and records a pending harness branch.
3. Stores the pending Codex-only model-lock policy, not a concrete model or
   ...
   returns the Operator `sessionKey`.
...
2. On that same connection, starts the canonical full Codex harness thread with
   `threadSource: "appServer"`, Operator's cwd, policy, config, environment, the
   full Operator harness tool surface, and exactly the model and provider
...
omit Operator model and provider overrides, so Codex restores the persisted pair.
If a separate native Codex control changes the canonical thread, Operator accepts
that native persisted selection. The outer Operator model and fallback chain
...
Deleting or resetting the Operator entry would otherwise discard the native
...
canonical harness thread on the first turn prevents Operator from becoming a
...
source remains eligible for both native and Operator catalogs. The canonical
...
known active Operator binding owner for the exact target or any non-archived
spawned descendant also rejects archive. Operator paginates Codex's experimental
...
`operator doctor --fix` migrates shipped `plugins.entries.codex-supervisor`
...
- audited handoff between Codex Desktop, CLI, and Operator
...
- Later resumes omit Operator model/provider overrides, preserve Codex's
  canonical persisted selection, accept separate native changes to that thread,
  and never substitute the outer Operator model or fallback chain.
