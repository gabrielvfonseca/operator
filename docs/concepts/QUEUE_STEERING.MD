...
When a normal prompt arrives while a session run is already streaming and the queue mode is `steer` (the default, no config needed), Operator tries to send that prompt into the active runtime. Operator and the native Codex app-server harness implement the delivery details differently.
...
Steering does not interrupt a tool call that is already running. Operator checks for queued steering messages at model boundaries:

1. The assistant asks for tool calls.
2. Operator executes the current assistant message's tool-call batch.
3. Operator emits the turn end event.
4. Operator drains queued steering messages.
5. Operator appends those messages as user messages before the next LLM call.
...
The native Codex app-server harness exposes `turn/steer` instead of Operator runtime's internal steering queue. Operator batches queued prompts for the configured quiet window, then sends a single `turn/steer` request with all collected user input in arrival order.

Codex review and manual compaction turns reject same-turn steering. When a runtime cannot accept steering in `steer` mode, Operator waits for the active run to finish before starting the prompt.
...
- With default behavior, the active runtime receives all four messages in arrival order before its next model decision. Operator drains them at the next model boundary; Codex receives them as one batched `turn/steer`.
- With `/queue collect`, Operator does not steer. It waits until the active run ends, then creates a followup turn with compatible queued messages after the debounce window.
- With `/queue interrupt`, Operator aborts the active run and starts the newest message instead of steering.
