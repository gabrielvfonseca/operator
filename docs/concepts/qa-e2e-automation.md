...
The private QA stack exercises Operator in a realistic, channel-shaped way that
a unit test cannot.
...
Every QA flow runs under `pnpm operator qa <subcommand>`. Many have `pnpm qa:*`
...
| `qa parity-report`                                  | Compare two `qa-suite-summary.json` files for a model-axis parity gate, or use `--runtime-axis --token-efficiency` to write Codex-vs-Operator runtime parity and token-efficiency reports.                                                                          |
...
command also needs an Operator root profile, put the root profile before the
QA command:

```bash
pnpm operator --profile work qa run --qa-profile smoke-ci
```
...
```bash
pnpm operator qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`operator.run`, `operator.harness.run`, a latest GenAI semantic-convention
model-call span, `operator.context.assembled`, and `operator.message.delivery`
...
IDs and `operator.content.*` attributes must stay out of the trace. The scenario
...
```bash
pnpm operator qa matrix --provider-mode mock-openai --profile release
```
...
```bash
OPERATOR_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
  pnpm operator qa matrix --provider-mode live-frontier --profile release
```

Plain `pnpm operator qa matrix` runs the full `all` profile and continues after
...
| `e2ee-cli`   | 9         | `operator matrix encryption setup`, recovery-key, multi-account, gateway round-trip, and self-verification commands through the harness. |
...
`OPERATOR_QA_MATRIX_TUWUNEL_IMAGE`; tune negative no-reply assertions with
`OPERATOR_QA_MATRIX_NO_REPLY_WINDOW_MS` (default `8000`, clamped to the active
...
`OPERATOR_QA_MATRIX_DISABLE_FORCE_EXIT=1` only for a direct test harness that
...
E2EE CLI profile also drives `operator matrix encryption setup` and
...
```bash
pnpm operator qa discord
pnpm operator qa slack
pnpm operator qa telegram
pnpm operator qa whatsapp
```
...
```bash
pnpm operator qa mantis slack-desktop-smoke \
  --gateway-setup \
...
Operator Slack gateway running inside the VM on port `38973`; without it, the
...
```bash
pnpm operator qa mantis visual-task \
...
`visual-driver`, captures `visual-task.png`, runs `operator infer image
...
The doctor checks Convex broker env (`OPERATOR_QA_CONVEX_SITE_URL`,
`OPERATOR_QA_CONVEX_ENDPOINT_PREFIX`), validates endpoint settings, reports
only set/missing status for `OPERATOR_QA_CONVEX_SECRET_CI` and
`OPERATOR_QA_CONVEX_SECRET_MAINTAINER`, and verifies admin/list reachability
...
```bash
pnpm operator qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds Operator
...
- `OPERATOR_QA_TELEGRAM_GROUP_ID` - numeric chat id (string).
...
of the `/usage` footer on tool-only replies. Use `pnpm operator qa telegram
...
```bash
OPERATOR_QA_CREDENTIAL_SOURCE=convex \
pnpm test:docker:npm-telegram-live
```

When `OPERATOR_QA_CREDENTIAL_SOURCE=convex` is set, the package live wrapper
...
`OPERATOR_NPM_TELEGRAM_RTT_SAMPLES`, `OPERATOR_NPM_TELEGRAM_RTT_TIMEOUT_MS`,
or `OPERATOR_NPM_TELEGRAM_RTT_MAX_FAILURES` to tune RTT measurement without
...
controlled by the harness and a SUT bot started by the child Operator gateway
...
- `OPERATOR_QA_DISCORD_GUILD_ID`
- `OPERATOR_QA_DISCORD_CHANNEL_ID`
- `OPERATOR_QA_DISCORD_DRIVER_BOT_TOKEN`
- `OPERATOR_QA_DISCORD_SUT_BOT_TOKEN`
- `OPERATOR_QA_DISCORD_SUT_APPLICATION_ID` - must match the SUT bot user id
  returned by Discord (the lane fails fast otherwise).

Optional:

- `OPERATOR_QA_DISCORD_VOICE_CHANNEL_ID` selects the voice/stage channel for
...
- `OPERATOR_QA_SLACK_APPROVAL_CHECKPOINT_DIR` enables visual approval
...
- `OPERATOR_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS` overrides the checkpoint
...
- `slack-codex-approval-exec-native` - opt-in Codex Guardian command approval
  ...
  `operator-codex-app-server`, resolves it, and verifies the Codex turn
...
- `approval-checkpoints/` - only when Mantis sets
  `OPERATOR_QA_SLACK_APPROVAL_CHECKPOINT_DIR`; contains checkpoint JSON,
...
{
  "display_information": {
    "name": "Operator QA Driver",
    "description": "Test driver bot for Operator QA Slack live lane"
  },
  "features": {
    "bot_user": {
      "display_name": "Operator QA Driver",
      "always_online": true
    }
  },
  "oauth_config": {
...
{
  "display_information": {
    "name": "Operator QA SUT",
    "description": "Operator QA SUT connector for Operator"
  },
  "features": {
    "bot_user": {
      "display_name": "Operator QA SUT",
      "always_online": true
    },
    "app_home": {
...
In the QA workspace, create a channel (e.g. `#operator-qa`) and invite both
bots from inside the channel:

```text
/invite @Operator QA Driver
/invite @Operator QA SUT
```
...
`OPERATOR_QA_SLACK_*` variables and pass `--credential-source env`), or seed
...
With `OPERATOR_QA_CONVEX_SITE_URL` and `OPERATOR_QA_CONVEX_SECRET_MAINTAINER`
exported in your shell, register and verify:

```bash
pnpm operator qa credentials add \
  --kind slack \
  --payload-file slack-creds.json \
  --note "QA Slack pool seed"

pnpm operator qa credentials list --kind slack --status all --json
```
...
the harness and a SUT account started by the child Operator gateway through
the bundled WhatsApp plugin.

Required env when `--credential-source env`:

- `OPERATOR_QA_WHATSAPP_DRIVER_PHONE_E164`
- `OPERATOR_QA_WHATSAPP_SUT_PHONE_E164`
- `OPERATOR_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64`
- `OPERATOR_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64`

Optional:

- `OPERATOR_QA_WHATSAPP_GROUP_JID` enables group scenarios such as
...
`@gabrielvfonseca/whatsapp/api.js` package surface instead of reaching into private
...
`--credential-source convex` (or set `OPERATOR_QA_CREDENTIAL_SOURCE=convex`);
...
- `mock-openai` is the scenario-aware Operator mock. It remains the default
...
runs Operator's normal channel plugins against them. `live` is reserved for
...
- how `operator qa <runner>` is mounted beneath the shared `qa` root
...
3. Keep transport-specific mechanics inside the runner plugin or channel
   harness.
4. Mount the runner as `operator qa <runner>` instead of registering a
   competing root command. Runner plugins should declare `qaRunners` in
   `operator.plugin.json` and export a matching `qaRunnerCliRegistrations`
...
or wiring a new transport - run `pnpm operator qa coverage` (add `--json`
for machine-readable output). When choosing focused proof for a touched
behavior or file path, run `pnpm operator qa coverage --match <query>`. The
...
```bash
pnpm operator qa character-eval \
