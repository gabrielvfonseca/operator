---
summary: "Operator capabilities across channels, routing, media, and UX."
read_when:
  - You want a full list of what Operator supports
title: "Features"
---
...
- iMessage, Telegram, and WebChat ship with the core install; every other channel is an
  official plugin installed with `operator plugins install @gabrielvfonseca/<id>` (or on demand
  during `operator onboard` / `operator channels add`)
...
- External plugin channels maintained outside the Operator repo: WeChat, Yuanbao, and Zalo ClawBot
...
<CardGroup cols={2}>
  ...
  <Card title="Plugins" href="/tools/plugin" icon="plug">
    Official and external plugins that extend Operator.
  </Card>
</CardGroup>
