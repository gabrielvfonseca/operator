---
summary: "How Operator separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between Operator, Codex, ACP, or another native agent runtime
...
runtime keys are legacy and ignored. `operator doctor --fix` removes old
...
| Surface                                          | Operator name/config                 | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
makes native app-server features available; `operator doctor --fix` owns
...
That means Operator selects an OpenAI model ref, then asks the Codex
...
Operator session store becomes Codex.
...
3. **Operator explicitly chosen for an OpenAI model** -> keep the model ref as `openai/<model>` and set provider/model runtime policy to `agentRuntime.id: "@gabrielvfonseca/operator"`. A selected `openai` OAuth profile is routed internally through Operator's Codex-auth transport.
4. **Legacy Codex model refs in config** -> repair with `operator doctor --fix` to `openai/<model>`; doctor keeps the Codex auth route by adding provider/model-scoped `agentRuntime.id: "codex"` where the old model ref implied it. Legacy **`codex-cli/*`** model refs repair to the same `openai/<model>` Codex app-server route; Operator no longer keeps a bundled Codex CLI backend.
...
| Surface                     | Operator embedded                              | Codex app-server                                                            |
| --------------------------- | ---------------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | Operator, through the Operator embedded runner | Codex app-server                                                            |
| Canonical thread state      | Operator transcript                            | Codex thread, plus Operator transcript mirror                               |
| Operator dynamic tools      | Native Operator tool loop                      | Bridged through the Codex adapter                                           |
| Native shell and file tools | Operator path                                  | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native Operator context assembly               | Operator projects assembled context into the Codex turn                     |
| Compaction                  | Operator or selected context engine            | Codex-native compaction, with Operator notifications and mirror maintenance |
| Channel delivery            | Operator                                       | Operator                                                                    |

Design rule: if Operator owns the surface, it can provide normal plugin hook
behavior. If the native runtime owns the surface, Operator needs runtime
events or native hooks. If the native runtime owns canonical thread state,
Operator mirrors and projects context rather than rewriting unsupported
internals.

## Runtime selection

Operator resolves an embedded runtime after provider and model resolution, in
this order:
...
and `agents.list[].agentRuntime`. Run `operator doctor --fix` to remove stale
...
never silently routed back to Operator. Only `auto` may route an unmatched
turn to Operator.
...
harness. Explicit Operator runtime config remains an opt-in compatibility
route for `openai/*` agent turns; when paired with a selected `openai` OAuth
profile, Operator routes that path internally through the Codex-auth
...
If `operator doctor` warns that the `codex` plugin is enabled while legacy
Codex model refs remain in config, treat that as legacy route state and run
`operator doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## GitHub Copilot agent runtime

The external `@gabrielvfonseca/copilot` plugin registers an opt-in `copilot` runtime
...
prefix in `extensions/copilot/doctor-contract-api.ts`, which `operator doctor`
...
When a runtime is not Operator, its docs should state which Operator surfaces
it supports:

| Question                               | Why it matters                                                                                    |
| -------------------------------------- | ------------------------------------------------------------------------------------------------- |
| Who owns the model loop?               | Determines where retries, tool continuation, and final answer decisions happen.                   |
| Who owns canonical thread history?     | Determines whether Operator can edit history or only mirror it.                                   |
| Do Operator dynamic tools work?        | Messaging, sessions, cron, and Operator-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around Operator-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume Operator equivalence where the native runtime owns more state.            |
