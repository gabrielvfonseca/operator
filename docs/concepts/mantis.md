...
title: "Mantis"
read_when:
  - Building or running live visual QA for Operator bugs
...
Mantis publishes visual CI evidence and a PR comment for Operator behavior.
...
All commands are `pnpm operator qa mantis <command>`, defined in
`extensions/qa-lab/src/mantis/cli.ts`. Requires `OPERATOR_ENABLE_PRIVATE_QA_CLI=1`
at build/run time (bundled workflows set `OPERATOR_BUILD_PRIVATE_QA=1` and
`OPERATOR_ENABLE_PRIVATE_QA_CLI=1` before building).
...
| `telegram-desktop-builder`      | Lease/reuse a Crabbox desktop, install Telegram Desktop, optionally configure an Operator gateway.                                                        |
...
```bash
pnpm operator qa mantis discord-smoke \
...
Token resolution order: `--token-file` value, then `OPERATOR_QA_DISCORD_MANTIS_BOT_TOKEN`
(override with `--token-env`), then a file named by `OPERATOR_QA_DISCORD_MANTIS_BOT_TOKEN_FILE`
(override with `--token-file-env`). Guild/channel ids come from
`OPERATOR_QA_DISCORD_GUILD_ID` / `OPERATOR_QA_DISCORD_CHANNEL_ID` (override with
`--guild-id` / `--channel-id`) and must be 17-20 digit Discord snowflakes. Set
`OPERATOR_QA_REDACT_PUBLIC_METADATA=1` to replace bot/guild/channel/message ids
and names with `<redacted>` in the published summary and report.

### `run`

```bash
pnpm operator qa mantis run \
  --transport discord \
...
`pnpm operator qa discord --scenario <id> --model openai/gpt-5.4 --alt-model openai/gpt-5.4 --allow-failures`
...
```bash
pnpm operator qa mantis desktop-browser-smoke \
  --output-dir .artifacts/qa-e2e/mantis/desktop-browser
```

Leases or reuses a Crabbox desktop, launches a browser inside the VNC session
pointed at `--browser-url` (default `https://operator.ai`) or a rendered
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome profile archive from that env var before launch (default `OPERATOR_MANTIS_BROWSER_PROFILE_TGZ_B64`); used for logged-in witnesses like Discord Web.
- `--video-duration <seconds>` controls MP4 capture length (default 10s).
- `--keep-lease` (or `OPERATOR_MANTIS_KEEP_VM=1`) keeps a lease this run created open for VNC inspection; failed runs that created a lease also keep it by default.
...
`OPERATOR_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also writes a
Discord Web URL artifact, and `OPERATOR_QA_DISCORD_KEEP_THREADS=1` leaves the
...
`pnpm operator qa slack` inside it, opens Slack Web in the VNC browser,
...
- `OPERATOR_LIVE_OPENAI_KEY` for the remote model lane (if only `OPENAI_API_KEY`
  is set locally, Mantis copies it to `OPERATOR_LIVE_OPENAI_KEY` before
  invoking Crabbox)
...
bot token into the VM as `OPERATOR_MANTIS_SLACK_*` env vars, so GitHub
...
`OPERATOR_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome
profile inside the VM (default `$HOME/.config/operator-mantis/slack-chrome-profile`);
...
optionally restores a user-session archive, configures Operator with the
leased Telegram SUT bot token, starts
`operator gateway run --dev --allow-unconfigured --port 38974`, posts a
driver-bot readiness message to the leased private group, then captures a
screenshot and MP4. A bot token only configures Operator; it never logs
...
`OPERATOR_QA_REDACT_PUBLIC_METADATA=1` for public artifact uploads; it is
...
- `MANTIS_ARTIFACT_R2_BUCKET` (workflows set `operator-crabbox-artifacts`)
- `MANTIS_ARTIFACT_R2_ENDPOINT`
- `MANTIS_ARTIFACT_R2_REGION` (workflows set `auto`)
- `MANTIS_ARTIFACT_R2_PUBLIC_BASE_URL` (workflows set `https://artifacts.operator.ai`)
...
| `Mantis Telegram Live`            | PR comment or manual dispatch                                                              | Runs the bot-API Telegram live QA lane (`operator qa telegram`), writes `mantis-evidence.json` from the QA summary, renders redacted evidence HTML through a Crabbox desktop browser, generates a motion GIF, posts PR evidence. Telegram Web login is not required for this lane.                               |
...
| `Mantis Web UI Chat Proof`        | PR comment or manual dispatch                                                              | Runs the focused Operator Control UI chat Playwright proof against the candidate, verifies the browser sends through the mocked Gateway, captures screenshot/video artifacts, and posts PR evidence. This lane is web chat proof only, not WinUI/native-app or arbitrary visual proof.                           |
...
```text
@operator-mantis discord status reactions
@operator-mantis discord status reactions baseline=origin/main candidate=HEAD
...
@operator-mantis web ui chat
@operator-mantis web-ui-chat candidate=HEAD
```
...
`OPERATOR_MANTIS_CRABBOX_PROVIDER` / `OPERATOR_MANTIS_CRABBOX_CLASS`. GitHub
...
noVNC, Node 22.22.3+, 24.15+, or 25.9+ and pnpm, an Operator checkout, and
...
- `OPERATOR_QA_DISCORD_MANTIS_BOT_TOKEN`
- `OPERATOR_QA_DISCORD_GUILD_ID`
- `OPERATOR_QA_DISCORD_CHANNEL_ID`
- Local `qa mantis run --credential-source env` also requires
  `OPERATOR_QA_DISCORD_DRIVER_BOT_TOKEN`, `OPERATOR_QA_DISCORD_SUT_BOT_TOKEN`,
  and `OPERATOR_QA_DISCORD_SUT_APPLICATION_ID`. GitHub workflows normally use
  `--credential-source convex` and the broker credentials below instead of raw
  Discord bot tokens.
- `OPERATOR_QA_REDACT_PUBLIC_METADATA=1` for public artifact uploads
- `OPERATOR_QA_CONVEX_SITE_URL`, `OPERATOR_QA_CONVEX_SECRET_CI`
- `OPENAI_API_KEY` (or the Telegram Desktop proof-specific
  `OPERATOR_MANTIS_AGENT_OPENAI_API_KEY`)
- `CRABBOX_COORDINATOR` / `CRABBOX_COORDINATOR_TOKEN` (workflows also accept
  `OPERATOR_QA_MANTIS_CRABBOX_COORDINATOR` / `_TOKEN` as a fallback and map
...
ref policy, candidate ref policy, Operator config patch, setup/stimulus steps,
