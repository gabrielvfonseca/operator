---
summary: "Move one Operator session's reply route between linked chat channels"
...
Channel docking is call forwarding for one Operator session. It keeps the same
...
Alice can message Operator on Telegram and Discord:
...
Operator keeps the current session context and changes the reply route:
...
4. Keep the same Operator session, but receive future replies in Discord.
...
Operator generates one `/dock-<channel>` command for every loaded channel plugin
...
Send the dock command from a direct chat with Operator, not from a group chat.
