...
- **Session store**: chat history and routing state in `~/.operator/agents/<agentId>/agent/operator-agent.sqlite`.

Auth profiles are per-agent, read from:

```text
~/.operator/agents/<agentId>/agent/auth-profiles.json
```
...
| -------------------------------- | -------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| Config                           | `~/.operator/operator.json`                                                            | `OPERATOR_CONFIG_PATH`                                                                   |
| State dir                        | `~/.operator`                                                                          | `OPERATOR_STATE_DIR`                                                                     |
| Default agent's workspace        | `~/.operator/workspace` (or `workspace-<profile>` when `OPERATOR_PROFILE` is set)      | `agents.list[].workspace`, then `agents.defaults.workspace`, or `OPERATOR_WORKSPACE_DIR` |
| Other agents' workspace          | `<stateDir>/workspace-<agentId>` (or `<agents.defaults.workspace>/<agentId>` when set) | `agents.list[].workspace`                                                                |
| Agent dir                        | `~/.operator/agents/<agentId>/agent`                                                   | `agents.list[].agentDir`                                                                 |
...
If you configure nothing, Operator runs one agent:

- `agentId` defaults to `main`.
- Sessions key as `agent:main:<mainKey>` (default `mainKey` is `main`).
- Workspace defaults to `~/.operator/workspace` (or `workspace-<profile>` when `OPERATOR_PROFILE` is set to something other than `default`).
- State defaults to `~/.operator/agents/main/agent`.
...
<Steps>
  <Step title="Create each agent workspace">
    ```bash
    operator agents add coding
    operator agents add social
    ```

    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.operator/agents/<agentId>`.
...
The configured path is the parent directory. Operator appends the normalized
agent id, producing paths such as `~/.operator/wiki/support` and
`~/.operator/wiki/marketing`. Agent-scoped CLI and Gateway operations require
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.operator/workspace-alex" },
      { id: "mia", workspace: "~/.operator/workspace-mia" },
    ],
  },
  bindings: [
    {
...
Set `channels.<channel>.defaultAccount` to choose the account used when `accountId` is omitted. When unset, Operator falls back to `default` if present, otherwise the first configured account id (sorted).
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.operator/workspace-main" },
          { id: "coding", workspace: "~/.operator/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.operator/workspace-main" },
          { id: "alerts", workspace: "~/.operator/workspace-alerts" },
        ],
      },
      bindings: [
  ...
  <Accordion title="WhatsApp numbers per agent">
    ...
    ```bash
    operator channels login --channel whatsapp --account personal
    operator channels login --channel whatsapp --account biz
    ```

    `~/.operator/operator.json` (JSON5):
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.operator/workspace-home",
            agentDir: "~/.operator/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.operator/workspace-work",
            agentDir: "~/.operator/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.operator/credentials/whatsapp/personal
              // authDir: "~/.operator/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.operator/credentials/whatsapp/biz
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.operator/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.operator/workspace-opus",
