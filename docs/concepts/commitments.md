...
read_when:
  - You want Operator to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, Operator can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. Operator may check in afterward.
- You say you are exhausted. Operator may ask later whether you slept.
- The agent says it will follow up after something changes. Operator may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: Operator remembers a
...
```bash
operator config set commitments.enabled true
operator config set commitments.maxPerDay 3
```

Equivalent `operator.json`:
...
After an agent reply, Operator may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, Operator stores a commitment with:
...
metadata, and due-commitment heartbeat turns run without Operator tools.

Operator never delivers an inferred commitment immediately after writing it.
...
operator commitments dismiss cm_abc123
```

See [`operator commitments`](/cli/commitments) for the full command reference.
...
Stored commitments are local Operator operational memory in the shared SQLite
state database, not long-term memory. Disable the feature with:

```bash
operator config set commitments.enabled false
```
...
- Check `operator commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`operator commitments`](/cli/commitments)
