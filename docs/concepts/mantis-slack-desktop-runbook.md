...
Linux desktop, VNC rescue, Slack Web, a real Operator gateway, screenshots,
...
- **Mantis artifacts** - owned by the Operator run. Live under
...
- remote logs: `slack-desktop-command.log`, `operator-gateway.log`, `chrome.log`, `ffmpeg.log`
...
- `crabbox.remote_run` - sync, browser launch, Operator install/build or
...
remote status but Mantis copied metadata proving either the Operator gateway
