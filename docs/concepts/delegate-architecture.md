---
summary: "Delegate architecture: running Operator as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Run Operator as a **named delegate**: an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human - it sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate

A delegate is an Operator agent that:
...
Operator's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Operator's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.operator/workspace-delegate",
  tools: {
...
- Cron run history: Operator's shared SQLite state database.
- Session transcripts: `~/.operator/agents/delegate/sessions`.
- Identity provider audit logs (Exchange, Google Workspace).

All delegate actions flow through Operator's session store. For compliance, retain and review these logs.
...
```bash
operator agents add delegate --workspace ~/.operator/workspace-delegate
```

This creates:

- Workspace: `~/.operator/workspace-delegate`
- Agent state: `~/.operator/agents/delegate/agent`
- Sessions: `~/.operator/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.operator/workspace" },
      {
        id: "delegate",
        workspace: "~/.operator/workspace-delegate",
        tools: {
...
~/.operator/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.operator/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.operator/workspace-org",
        agentDir: "~/.operator/agents/org-assistant/agent",
