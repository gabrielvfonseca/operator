---
summary: "OAuth in Operator: token exchange, storage, and multi-account patterns"
read_when:
  - You want to understand Operator OAuth end-to-end
...
Operator supports OAuth ("subscription auth") for providers that offer it,
notably **OpenAI Codex (ChatGPT OAuth)** and **Anthropic Claude CLI reuse**.
For Anthropic, the practical split is:

- **Anthropic API key**: normal Anthropic API billing.
- **Anthropic Claude CLI / subscription auth inside Operator**: Anthropic staff
  told us this usage is allowed again, so Operator treats Claude CLI reuse and
...
Operator stores both OpenAI API-key auth and ChatGPT/Codex OAuth under the
...
`operator doctor --fix`; use `openai:*` profile ids and `auth.order.openai` for
new config.
...
issued for the same user/app. Practical symptom: log in via Operator _and_
via Claude Code / Codex CLI, and one of them randomly gets logged out later.

To reduce that, Operator treats the auth profile store as a **token sink**:

- the runtime reads credentials from one place per agent
- multiple profiles can coexist and route deterministically
- external CLI reuse is provider-specific: once Operator owns a local OAuth
  profile for a provider, the local refresh token is canonical. If that local
  refresh token is rejected, Operator reports the profile for
  ...
  `openai:default`-style profile before Operator owns OAuth for that
  provider; after that, Operator-owned refreshes stay canonical
...
- Legacy compatibility file: `~/.operator/agents/<agentId>/agent/auth.json`
  (static `api_key` entries are scrubbed when discovered)

Legacy import-only file (still supported, but not the main store):

- `~/.operator/credentials/oauth.json` (imported into the auth profile store on first use)

All of the above also respect `$OPERATOR_STATE_DIR` (state dir override). Full reference: [/gateway/configuration-reference#auth-storage](/gateway/configuration-reference#auth-storage)

For static secret refs and runtime snapshot activation behavior, see [Secrets Management](/gateway/secrets).

When a secondary agent has no local auth profile, Operator uses read-through
...
Operator supports Anthropic Claude CLI reuse and `claude -p` as a sanctioned
...
available as a supported token-auth path, but Operator prefers Claude CLI
reuse when it is available.

<Warning>
Anthropic's public Claude Code docs say direct Claude Code use stays within
Claude subscription limits, and Anthropic staff told us Operator-style Claude
CLI usage is allowed again. Operator therefore treats Claude CLI reuse and
`claude -p` usage as sanctioned for this integration unless Anthropic
...
If you want other subscription-style options in Operator, see [OpenAI
...
1. create the token by running `claude setup-token` on any machine with Claude Code, then start Anthropic setup-token or paste-token from Operator
2. Operator stores the resulting Anthropic credential in an auth profile
3. model selection stays on `anthropic/...`
4. existing Anthropic auth profiles remain available for rollback/order control

### OpenAI Codex (ChatGPT OAuth)

OpenAI Codex OAuth is explicitly supported for use outside the Codex CLI, including Operator workflows.
...
`operator models auth list --provider openai` after repair before copying
...
3. try to capture the callback on `http://localhost:1455/auth/callback` (the
   ...
   override with `OPERATOR_OAUTH_CALLBACK_HOST`)
...
Wizard path is `operator onboard` → auth choice `openai`.
...
- externally managed CLI credentials (Claude CLI, narrow Codex CLI bootstrap;
  ...
  spending a copied refresh token. If a managed refresh fails, Operator
...
```bash
operator models auth list --provider <id>
```
