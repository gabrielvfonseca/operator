...
read_when:
  ...
  - You need to restore or clean up an Operator-managed worktree
title: "Managed worktrees"
---

Managed worktrees give an agent task its own git branch and checkout without placing temporary directories inside the source repository. Operator creates them under its state directory, records them in the shared state database, and snapshots their tracked and non-ignored untracked contents before removal.
...
```text
<operator-state-dir>/worktrees/<repo-fingerprint>/<name>
```
...
Only files reported by git as both ignored and untracked are eligible. Tracked files are already present through git and are never copied by this step. Operator does not overwrite destination files or follow symlinked directories, and it preserves copied file modes.

## Run repository setup

If `.operator/worktree-setup.sh` exists in the source repository and is executable, Operator runs it with the new worktree as its current directory. The script receives:

```text
OPERATOR_SOURCE_TREE_PATH=<source checkout>
OPERATOR_WORKTREE_PATH=<managed worktree>
```

A nonzero exit aborts creation and removes the new worktree and branch. This is a repository-local contract; there is no Operator config key for it.
...
```bash
operator worktrees list [--json]
operator worktrees create <repo-root> [--name <name>] [--base-ref <ref>] [--json]
operator worktrees remove <id> [--force] [--json]
operator worktrees restore <id> [--json]
operator worktrees gc [--json]
```
