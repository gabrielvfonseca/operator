...
[Honcho](https://honcho.dev) adds AI-native memory to Operator through an
...
```bash
operator plugins install @honcho-ai/operator-honcho
operator honcho setup
...
Settings live under `plugins.entries["operator-honcho"].config`:
    ...
    entries: {
      "operator-honcho": {
        config: {
          ...
          workspaceId: "@gabrielvfonseca/operator", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `operator honcho setup` detects and
...
During conversation, Honcho tools query the service during Operator's
...
```bash
operator honcho setup                        # Configure API key and migrate files
operator honcho status                       # Check connection status
operator honcho ask <question>               # Query Honcho about the user
operator honcho search <query> [-k N] [-d D] # Semantic search over memory
```
