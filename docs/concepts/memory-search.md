...
Operator uses OpenAI embeddings by default. To use another provider, set it
explicitly:
...
Operator runs two retrieval paths in parallel and merges the results:
...
**No results?** Run `operator memory status` to check the index. If empty, run
`operator memory index --force`.

**Only keyword matches?** Your embedding provider may not be configured. Check
`operator memory status --deep`.
