---
summary: "ClawHub CLI entry points for discovering, installing, publishing, and verifying Operator skills and plugins."
read_when:
  - You want to use ClawHub from the command line
  - You want to install ClawHub skills or plugins through Operator
...
- `operator skills` / `operator plugins` - discover, install, and update
  packages for a local Operator agent or Gateway.
...
operator skills install @owner/<slug> --version <version> --global
operator skills update @owner/<slug>
operator skills update --all --acknowledge-clawhub-risk
...
local path. Full flag reference: [`operator skills`](/cli/skills) and
[`operator plugins`](/cli/plugins).

### Release trust

Operator checks a release's ClawHub trust state before downloading it, for
...
- **Risky** releases (non-clean scan, non-blocking moderation state) print a
  ...
  non-interactively.
- **Official ClawHub publishers/packages and bundled Operator sources** skip
...
- [`operator skills`](/cli/skills) - local skill search, install, update, and
  verification
- [`operator plugins`](/cli/plugins) - plugin search, install, update, and
  inspection
