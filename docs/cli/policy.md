---
summary: "CLI reference for `operator policy` conformance checks"
read_when:
  - You want to check Operator settings against an authored policy.jsonc
...
`operator policy` is provided by the bundled Policy plugin. It is an enterprise
conformance layer over existing Operator settings, not a second configuration
system. You author requirements in `policy.jsonc`; Operator observes the active
...
```bash
operator plugins enable policy
```
...
observed state is existing Operator config or workspace metadata.
...
If an `agentIds` entry is not present in `agents.list[]`, Operator evaluates
...
| `gateway.nodes.denyCommands`            | `gateway.nodes.denyCommands`                   | Require exact node command ids such as `system.run` to be denied in Operator config. |
...
denied by Operator config. A deployment that intentionally allows a privileged
...
`~/.operator/exec-approvals.json` by default, or
`$OPERATOR_STATE_DIR/exec-approvals.json` when `OPERATOR_STATE_DIR` is set.
...
operator policy check --json
operator policy check --severity-min error
```
...
`operator doctor --lint` when the Policy plugin is enabled.

Compare an operator policy file against an authored baseline:

```bash
operator policy compare --baseline official.policy.jsonc
operator policy compare --baseline official.policy.jsonc --policy policy.jsonc --json
```
...
```json
{
  ...
  "evidence": {
    "channels": [
      {
        "id": "telegram",
        "provider": "telegram",
        "source": "oc://operator.config/channels/telegram",
        "enabled": false
      }
    ],
    "mcpServers": [
      {
        "id": "docs",
        "transport": "stdio",
        "source": "oc://operator.config/mcp/servers/docs",
        "command": "npx"
      }
    ],
    "modelProviders": [
      {
        "id": "openai",
        "source": "oc://operator.config/models/providers/openai"
      }
    ],
    "modelRefs": [
      {
        ...
        "provider": "openai",
        "model": "gpt-5.6-sol",
        "source": "oc://operator.config/agents/defaults/model"
      }
    ],
    "network": [
      {
        "id": "browser-private-network",
        "source": "oc://operator.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
        "value": false
      }
    ],
    "gatewayExposure": [
      {
        "id": "gateway-bind",
        "kind": "bind",
        "source": "oc://operator.config/gateway/bind",
        "value": "loopback",
        "nonLoopback": false,
        "explicit": true
      }
    ...
    "agentWorkspace": [
      {
        ...
        "value": "ro",
        "sandboxMode": "all",
        "sandboxModeSource": "oc://operator.config/agents/defaults/sandbox/mode",
        "sandboxEnabled": true,
        "explicit": true
      },
      {
    ...
    "secrets": [
      {
        "id": "vault",
        "kind": "provider",
        "source": "oc://operator.config/secrets/providers/vault",
        "providerSource": "env"
      },
      {
        "id": "oc://operator.config/models/providers/openai/apiKey",
        "kind": "input",
    ...
    "authProfiles": [
      {
        "id": "github",
        "source": "oc://operator.config/auth/profiles/github",
        "validMetadata": true,
        "provider": "github",
        "mode": "token"
      }
...
records the observed Operator state used by the checks, and
...
`operator policy watch` re-runs the check and reports when current evidence no
longer matches `expectedAttestationHash`:

```bash
operator policy watch --json
```
...
| `policy/gateway-node-command-denied`                     | A node command denied by policy is not denied by Operator config.                 |
...
```json
{
  ...
  "source": "policy",
  "path": "operator config",
  "ocPath": "oc://operator.config/mcp/servers/remote",
  "target": "oc://operator.config/mcp/servers/remote",
...
```json
{
  ...
  "source": "policy",
  "path": "operator config",
  "ocPath": "oc://operator.config/agents/defaults/model/fallbacks/#0",
  "target": "oc://operator.config/agents/defaults/model/fallbacks/#0",
...
```json
{
  ...
  "target": "oc://operator.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
...
```json
{
  ...
  "target": "oc://operator.config/gateway/bind",
...
```json
{
  ...
  "severity": "error",
  "message": "Gateway node command 'system.run' is denied by policy but not denied by Operator config.",
  "source": "policy",
  "path": "operator config",
...
```json
{
  ...
  "target": "oc://operator.config/agents/defaults/sandbox/workspaceAccess",
