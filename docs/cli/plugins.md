---
summary: "CLI reference for `operator plugins` (init, build, validate, list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
```bash
operator plugins list [--enabled] [--verbose] [--json]
operator plugins search <query> [--limit <n>] [--json]
operator plugins install <path-or-spec> [--link] [--force] [--pin] [--marketplace <source>]
operator plugins inspect <id> [--runtime] [--json]
operator plugins inspect --all [--runtime] [--json]
operator plugins info <id>                    # alias for inspect
operator plugins enable <id>
operator plugins disable <id>
operator plugins uninstall <id> [--dry-run] [--keep-files] [--force]
operator plugins update <id-or-npm-spec> | --all [--dry-run]
operator plugins registry [--refresh] [--json]
operator plugins doctor
operator plugins init <id> [--name <name>] [--type tool|provider] [--directory <path>]
operator plugins build [--entry <path>] [--check]
operator plugins validate [--entry <path>]
operator plugins marketplace entries [--offline] [--feed-profile <name>] [--json]
operator plugins marketplace list <source> [--json]
operator plugins marketplace refresh [--feed-profile <name>] [--expected-sha256 <sha256>] [--json]
```

For slow install, inspect, uninstall, or registry-refresh investigation, run the
command with `OPERATOR_PLUGIN_LIFECYCLE_TRACE=1`. The trace writes phase timings
...
<Note>
Bundled plugins ship with Operator. Some are enabled by default (for example bundled model providers, bundled speech providers, and the bundled browser plugin); others require `plugins enable`.
...
```bash
operator plugins init stock-quotes --name "Stock Quotes"
cd stock-quotes
...
argument is the plugin id; `--name` sets the display name. Operator uses the
...
`plugin:validate` that build then call `operator plugins build`/`validate`.

`plugins build` imports the built entry, reads its static tool metadata, writes
`operator.plugin.json`, and keeps `package.json`'s `operator.extensions` aligned.
...
```bash
operator plugins init acme-models --name "Acme Models" --type provider
cd acme-models
npm install
npm run build
npm test
...
`operator plugins build`/`validate`; those commands are for the tool
...
```bash
operator plugins search "calendar"                      # search ClawHub plugins
operator plugins install @gabrielvfonseca/<package>            # trusted official catalog
operator plugins install <package>                       # arbitrary npm package
operator plugins install clawhub:<package>                # ClawHub only
operator plugins install npm:<package>                    # npm only
operator plugins install npm-pack:<path.tgz>               # local npm-pack tarball
operator plugins install git:github.com/<owner>/<repo>     # git repo
operator plugins install git:github.com/<owner>/<repo>@<ref>
operator plugins install <path>                            # local path or archive
operator plugins install -l <path>                         # link instead of copy
operator plugins install <plugin>@<marketplace>             # marketplace shorthand
operator plugins install <plugin> --marketplace <name>      # marketplace (explicit)
operator plugins install <package> --force                  # confirm source / overwrite existing
operator plugins install <package> --pin                    # pin resolved npm version
operator plugins install clawhub:<package> --acknowledge-clawhub-risk
operator plugins install <package> --dangerously-force-unsafe-install
```
...
<Warning>
Bare package names install from npm by default during the launch cutover, unless they match a bundled or official plugin id, in which case Operator uses that local/official copy instead of hitting the npm registry. Use `npm:<package>` when you deliberately want an external npm package instead. Use `clawhub:<package>` for ClawHub. Treat plugin installs like running code; prefer pinned versions.
</Warning>

<Warning>
ClawHub packages and Operator's bundled/official catalog are trusted install
...
`bundle-plugin` packages (not skills; use `operator skills search` for those).
...
summary, and an install hint such as `operator plugins install clawhub:<package>`.
...
<AccordionGroup>
  <Accordion title="Config includes and invalid-config repair">
    If your `plugins` section is backed by a single-file `$include`, `plugins install/update/enable/disable/uninstall` write through to that included file and leave `operator.json` untouched. Root includes, include arrays, and includes with sibling overrides fail closed instead of flattening. See [Config includes](/gateway/configuration) for the supported shapes.

    If config is invalid during install, `plugins install` normally fails closed and tells you to run `operator doctor --fix` first. During Gateway startup and hot reload, invalid plugin config fails closed like any other invalid config; `operator doctor --fix` can quarantine the invalid plugin entry. The only documented install-time exception is a narrow bundled-plugin recovery path for plugins that explicitly opt into `operator.install.allowInvalidConfigRecovery`.

  </Accordion>
  <Accordion title="--force confirmation and reinstall vs update">
    `--force` confirms a non-ClawHub source without prompting. It does not bypass `security.installPolicy` or remaining install safety checks. When the plugin or hook pack is already installed, it also reuses the existing target and overwrites it in place. Use it after reviewing an arbitrary npm, local, archive, git, or marketplace source, or when intentionally reinstalling the same id. For routine upgrades of an already tracked npm plugin, prefer `operator plugins update <id-or-npm-spec>`.

    If you run `plugins install` for a plugin id that is already installed, Operator stops and points you at `plugins update <id-or-npm-spec>` for a normal upgrade, or at `plugins install <package> --force` when you genuinely want to overwrite the current install from a different source. Arbitrary sources still show the interactive provenance warning; noninteractive installs must pass `--force` after review. Trusted ClawHub and Operator-catalog sources do not need it. With `--link`, `--force` confirms the source but does not change the linked-path install mode.
  ...
  <Accordion title="--dangerously-force-unsafe-install">
    `--dangerously-force-unsafe-install` is deprecated and is now a no-op. Operator no longer runs built-in install-time dangerous-code blocking for plugin installs.
  ...
  <Accordion title="--acknowledge-clawhub-risk">
    Community ClawHub installs check the selected release's trust record before downloading. If ClawHub disables download for the release, reports malicious scan findings, or puts the release in a blocking moderation state (quarantined, revoked), Operator refuses it outright regardless of this flag. For non-blocking risky scan statuses or moderation states, Operator shows the trust details and asks for confirmation before continuing.

    Use `--acknowledge-clawhub-risk` only after reviewing the ClawHub warning and deciding to continue without an interactive prompt. Pending or stale (not-yet-clean) scan results warn but do not require acknowledgement. Official ClawHub packages and bundled Operator plugin sources bypass this release-trust check entirely.

  </Accordion>
  <Accordion title="Hook packs and npm specs">
    `plugins install` is also the install surface for hook packs that expose `operator.hooks` in `package.json`. Use `operator hooks` for filtered hook visibility and per-hook enablement, not package installation.

    Npm specs are **registry-only** (package name plus optional **exact version** or **dist-tag**). Git/URL/file specs and semver ranges are rejected. Dependency installs run in one managed npm project per plugin with `--ignore-scripts` for safety, even when your shell has global npm install settings. Managed plugin npm projects inherit Operator's package-level npm `overrides`, so host security pins apply to hoisted plugin dependencies too.
    ...
    Raw `@gabrielvfonseca/*` specs that match bundled plugins resolve to the image-owned bundled copy before npm fallback. For example, `operator plugins install @gabrielvfonseca/discord@2026.5.20 --pin` uses the bundled Discord plugin from the current Operator build instead of creating a managed npm override. To force the external npm package, use `operator plugins install npm:@gabrielvfonseca/discord@2026.5.20 --pin`.

    Bare specs and `@latest` stay on the stable track. Operator date-stamped correction versions such as `2026.5.3-1` count as stable for this check. If npm resolves either form to a prerelease, Operator stops and asks you to opt in explicitly with a prerelease tag (`@beta`/`@rc`) or an exact prerelease version (`@1.2.3-beta.4`).

    For npm installs without an exact version (`npm:<package>` or `npm:<package>@latest`), Operator checks the resolved package metadata before install. If the latest stable package requires a newer Operator plugin API or minimum host version, Operator inspects older stable versions and installs the newest compatible release instead. Exact versions and explicit dist-tags stay strict: an incompatible selection fails and asks you to upgrade Operator or choose a compatible version.

    If a bare install spec matches an official plugin id (for example `diffs`), Operator installs the catalog entry directly. To install an npm package with the same name, use an explicit scoped spec (for example `@scope/diffs`).
  ...
  <Accordion title="Git repositories">
    ...
    Git installs clone into a temporary directory, check out the requested ref when present, then use the normal plugin directory installer, so manifest validation, operator install policy, package-manager install work, and install records behave like npm installs. Recorded git installs include the source URL/ref plus the resolved commit so `operator plugins update` can re-resolve the source later.

    After installing from git, use `operator plugins inspect <id> --runtime --json` to verify runtime registrations such as gateway methods and CLI commands. If the plugin registered a CLI root with `api.registerCli`, run that command directly through the Operator root CLI, for example `operator demo-plugin ping`.

  </Accordion>
  <Accordion title="Archives">
    Supported archives: `.zip`, `.tgz`, `.tar.gz`, `.tar`. Native Operator plugin archives must contain a valid `operator.plugin.json` at the extracted plugin root; archives that only contain `package.json` are rejected before Operator writes install records.
...
```bash
operator plugins install clawhub:operator-codex-app-server
operator plugins install clawhub:operator-codex-app-server@1.2.3
```

Bare npm-safe plugin specs install from npm by default during the launch cutover unless they match an official plugin id:

```bash
operator plugins install operator-codex-app-server
```

Use `npm:` to make npm-only resolution explicit:

```bash
operator plugins install npm:operator-codex-app-server
operator plugins install npm:@gabrielvfonseca/discord@2026.5.20
operator plugins install npm:@scope/plugin-name@1.0.1
```

Operator checks the advertised plugin API / minimum gateway compatibility before install. When the selected ClawHub version publishes a ClawPack artifact, Operator downloads the versioned npm-pack `.tgz`, verifies the ClawHub digest header and the artifact digest, then installs it through the normal archive path. Older ClawHub versions without ClawPack metadata still install through the legacy package archive verification path. Recorded installs keep their ClawHub source metadata, artifact kind, npm integrity, npm shasum, tarball name, and ClawPack digest facts for later updates.
Unversioned ClawHub installs keep an unversioned recorded spec so `operator plugins update` can follow newer ClawHub releases; explicit version or tag selectors such as `clawhub:pkg@1.2.3` and `clawhub:pkg@beta` remain pinned to that selector.
...
```bash
operator plugins marketplace list <marketplace-name>
operator plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` to pass the marketplace source explicitly:

```bash
operator plugins install <plugin-name> --marketplace <marketplace-name>
operator plugins install <plugin-name> --marketplace <owner/repo>
operator plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
operator plugins install <plugin-name> --marketplace ./my-marketplace
```
...
<Tabs>
  ...
  <Tab title="Remote marketplace rules">
    For remote marketplaces loaded from GitHub or git, plugin entries must stay inside the cloned marketplace repo. Operator accepts relative path sources from that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path plugin sources from remote manifests.
  </Tab>
</Tabs>

For local paths and archives, Operator auto-detects:

- native Operator plugins (`operator.plugin.json`)
...
`~/.operator/extensions` or `<workspace>/.operator/extensions`; those
...
```bash
operator plugins install -l ./my-plugin
```
...
run `operator plugins enable <plugin-id>` or set
...
run `operator plugins list --enabled --verbose` or
`operator plugins inspect <id>` with a listed plugin id to confirm the plugin
ids and copy trusted ids into `plugins.allow` in `operator.json`. When the
...
so Operator records install provenance.
...
`/app/extensions/synology-chat`. Operator discovers that mounted source overlay
...
- `operator plugins inspect <id> --runtime --json` shows registered hooks and diagnostics from a module-loaded inspection pass. Runtime inspection never installs dependencies; use `operator doctor --fix` to clean legacy dependency state or recover missing downloadable plugins that are referenced by config.
- `operator gateway status --deep --require-rpc` confirms the reachable Gateway URL/profile, service/process hints, config path, and RPC health.
...
Plugin install metadata is machine-managed state, not user config. Installs and updates write it to the shared SQLite state database under the active Operator state directory. The `installed_plugin_index` row stores durable `installRecords` metadata, including records for broken or missing plugin manifests, plus a manifest-derived cold registry cache used by `operator plugins update`, uninstall, diagnostics, and the cold plugin registry.

When Operator sees shipped legacy `plugins.installs` records in config, runtime reads treat them as compatibility input without rewriting `operator.json`. Explicit plugin writes and `operator doctor --fix` move those records into the plugin index and remove the config key when config writes are allowed; if either write fails, the config records are kept so the install metadata is not lost.

## Uninstall

```bash
operator plugins uninstall <id>
operator plugins uninstall <id> --dry-run
operator plugins uninstall <id> --keep-files
operator plugins uninstall <id> --force
```

`uninstall` removes plugin records from `plugins.entries`, the persisted plugin index, plugin allow/deny list entries, and linked `plugins.load.paths` entries when applicable. Unless `--keep-files` is set, uninstall also removes the tracked managed install directory, but only when it resolves inside Operator's plugin extensions root. If the plugin currently owns the `memory` or `contextEngine` slot, that slot resets to its default (`memory-core` for memory, `legacy` for context engine).
...
```bash
operator plugins update <id-or-npm-spec>
operator plugins update --all
operator plugins update <id-or-npm-spec> --dry-run
operator plugins update @gabrielvfonseca/voice-call
operator plugins update @acme/demo
operator plugins update operator-codex-app-server --acknowledge-clawhub-risk
operator plugins update operator-codex-app-server --dangerously-force-unsafe-install
```
...
<AccordionGroup>
  <Accordion title="Resolving plugin id vs npm spec">
    When you pass a plugin id, Operator reuses the recorded install spec for that plugin. That means previously stored dist-tags such as `@beta` and exact pinned versions continue to be used on later `update <id>` runs.

    During `update <id> --dry-run`, exact pinned npm installs stay pinned. If Operator can also resolve the package's registry default line and that default line is newer than the installed pinned version, the dry run reports the pin and prints the explicit `@latest` package update command to follow the registry default line.

    That targeted-update rule differs from the bulk `operator plugins update --all` maintenance path. Bulk updates still respect ordinary tracked install specs, but trusted official Operator plugin records can sync to the current official catalog target instead of staying on a stale exact official package. Use targeted `update <id>` when you intentionally want to keep an exact or tagged official spec untouched.

    For npm installs, you can also pass an explicit npm package spec with a dist-tag or exact version. Operator resolves that package name back to the tracked plugin record, updates that installed plugin, and records the new npm spec for future id-based updates.
  ...
  <Accordion title="--acknowledge-clawhub-risk on update">
    Community ClawHub-backed plugin updates run the same exact-release trust check as installs before downloading the replacement package. Use `--acknowledge-clawhub-risk` for reviewed automation that should continue when the selected ClawHub release has a risky trust warning. Official ClawHub packages and bundled Operator plugin sources bypass this release-trust prompt.
  </Accordion>
</AccordionGroup>
...
```bash
operator plugins inspect <id>
operator plugins inspect <id> --runtime
operator plugins inspect <id> --json
operator plugins inspect --all
```
...
For module-shape failures such as missing `register`/`activate` exports, rerun with `OPERATOR_PLUGIN_LOAD_DEBUG=1` to include a compact export-shape summary in the diagnostic output.

## Registry

```bash
operator plugins registry
operator plugins registry --refresh
operator plugins registry --json
```

The local plugin registry is Operator's persisted cold read model for installed plugin identity, enablement, source metadata, and contribution ownership. Normal startup, provider owner lookup, channel setup classification, and plugin inventory can read it without importing plugin runtime modules.
...
```bash
operator plugins marketplace entries
operator plugins marketplace entries --offline
operator plugins marketplace entries --json
operator plugins marketplace entries --feed-profile <name>
operator plugins marketplace entries --feed-url <url>
operator plugins marketplace list <source>
operator plugins marketplace list <source> --json
operator plugins marketplace refresh
operator plugins marketplace refresh --feed-profile <name>
operator plugins marketplace refresh --feed-url <url>
operator plugins marketplace refresh --expected-sha256 <sha256> --json
```

`plugins marketplace entries` lists entries from the configured Operator marketplace feed. By default it attempts the hosted feed and falls back to the latest accepted snapshot or bundled data. Use `--feed-profile <name>` to read a specific configured profile, `--feed-url <url>` to read an explicit hosted feed URL, and `--offline` to read the latest accepted snapshot without fetching the feed.

`plugins marketplace refresh` refreshes the configured hosted feed snapshot and reports whether Operator accepted hosted data, a hosted snapshot, or bundled fallback data. Use `--expected-sha256` when a caller needs the command to fail unless a fresh hosted payload matches a pinned checksum.
...
Marketplace refresh loads a hosted Operator marketplace feed and persists the
...
refreshes fail if Operator cannot persist the validated snapshot.
