---
summary: "CLI reference for `operator path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
```bash
operator plugins enable oc-path
```
...
Operator state is spread across human-edited markdown, commented JSONC
...
`operator path` gives those callers a stable address instead of a one-off
...
Skip `operator path` for ordinary whole-file edits, rich config migrations, or
...
```bash
operator path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
operator path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
operator path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
operator path resolve 'oc://AGENTS.md/runtime-safety/operator-gateway'
```
...
```bash
operator path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
# Read a leaf
operator path resolve 'oc://gateway.jsonc/version'

# Wildcard search
operator path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
operator path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
operator path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
operator path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
operator path emit ./AGENTS.md
```
...
operator path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
operator path set 'oc://operator.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
operator path set 'oc://operator.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
operator path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
operator path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
operator path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
operator path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
operator path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
operator path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
operator path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
operator path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
operator path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
operator path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
operator path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ operator path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ operator path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ operator path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ operator path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ operator path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ operator path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ operator path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: operator.invoke
```

```bash
$ operator path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ operator path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
```bash
operator path resolve 'oc://AGENTS.md/tools/gh/risk' --human
operator path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
operator path find 'oc://AGENTS.md/tools/**/risk'
operator path find 'oc://session.jsonl/[event=action]/userId'
operator path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
operator path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
operator path set 'oc://gateway.jsonc/version' '2.0'
operator path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ operator path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
operator path emit ./AGENTS.md
operator path emit ./gateway.jsonc --json
```
...
`operator path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__OPERATOR_REDACTED__` (verbatim or as a substring) is refused at write
  time.
