---
summary: "Operator Gateway CLI (`operator gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is Operator's WebSocket server (channels, nodes, sessions, hooks). All subcommands below live under `operator gateway ...`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How Operator advertises and finds gateways.
  </Card>
...
```bash
operator gateway
operator gateway run   # equivalent, explicit form
```

<AccordionGroup>
  <Accordion title="Startup behavior">
    - Refuses to start unless `gateway.mode=local` is set in `~/.operator/operator.json`. Use `--allow-unconfigured` for ad-hoc/dev runs; it bypasses the guard without writing or repairing config.
    - `operator onboard --mode local` and `operator setup` write `gateway.mode=local`. If the config file exists but `gateway.mode` is missing, that is treated as damaged/clobbered config and the Gateway refuses to guess `local` for you — re-run onboarding, set the key manually, or pass `--allow-unconfigured`.
    ...
    - `SIGUSR1` triggers an in-process restart when authorized. `commands.restart` (default: enabled) gates externally-sent `SIGUSR1`; set it to `false` to block manual OS-signal restarts. The agent-facing `gateway` tool is read-only; agents request restart through the human-approved `operator` delegation tool.
...
<ParamField path="--token <token>" type="string">
  Shared token for `connect.params.auth.token`. Defaults to `OPERATOR_GATEWAY_TOKEN` when set.
</ParamField>
...
operator gateway restart --safe
operator gateway restart --safe --skip-deferral
operator gateway restart --force
operator gateway restart --wait 30s
```
...
- `OPERATOR_GATEWAY_STARTUP_TRACE=1` logs phase timings during startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings (installed-index, manifest registry, startup planning, owner-map work).
- `OPERATOR_GATEWAY_RESTART_TRACE=1` logs restart-scoped `restart trace:` lines: signal handling, active-work drain, shutdown phases, next start, ready timing, and memory metrics.
- `OPERATOR_DIAGNOSTICS=timeline` with `OPERATOR_DIAGNOSTICS_TIMELINE_PATH=<path>` writes a best-effort JSONL startup diagnostics timeline for external QA harnesses (equivalent to config `diagnostics.flags: ["timeline"]`; the path is still env-only). Add `OPERATOR_DIAGNOSTICS_EVENT_LOOP=1` to include event-loop samples.
...
```bash
operator gateway health --url ws://127.0.0.1:18789
operator gateway health --port 18789
```
...
<ParamField path="--port <port>" type="number">
  Target a local loopback Gateway on this port. Overrides `OPERATOR_GATEWAY_URL` and `OPERATOR_GATEWAY_PORT` for this call.
</ParamField>
...
```bash
operator gateway usage-cost
operator gateway usage-cost --days 7
operator gateway usage-cost --agent work --json
operator gateway usage-cost --all-agents
operator gateway usage-cost --json
```
...
```bash
operator gateway stability
operator gateway stability --type payload.large
operator gateway stability --bundle latest
operator gateway stability --bundle latest --export
operator gateway stability --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - Fatal Gateway exits, shutdown timeouts, and restart startup failures write the same diagnostic snapshot to `~/.operator/logs/stability/operator-stability-*.json` when the recorder has events. Inspect the newest bundle with `operator gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` apply to bundle output too.
...
operator gateway diagnostics export --output operator-diagnostics.zip
operator gateway diagnostics export --json
```
...
operator gateway status --require-rpc
```
...
operator gateway probe --port 18789
```
...
<AccordionGroup>
  ...
  <Accordion title="Common warning codes">
    ...
    - `multiple_gateways`: distinct gateway identities were reachable, or Operator could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway does not trigger this.
    ...
    - `local_tls_runtime_unavailable`: local Gateway TLS is enabled but Operator could not load the local certificate fingerprint.
...
```bash
operator gateway call status
operator gateway call logs.tail --params '{"limit": 200}'
```
...
Use `--wrapper` when the managed service must start through another executable, for example a secrets manager shim or a run-as helper. The wrapper receives the normal Gateway args and is responsible for eventually exec'ing `operator` or Node with those args.

```bash
cat > ~/.local/bin/operator-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- operator "$@"
EOF
chmod +x ~/.local/bin/operator-doppler

operator gateway install --wrapper ~/.local/bin/operator-doppler --force
operator gateway restart
```

You can also set the wrapper through the environment. `gateway install` validates that the path is an executable file, writes the wrapper into the service `ProgramArguments`, and persists `OPERATOR_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor repairs.

```bash
OPERATOR_WRAPPER="$HOME/.local/bin/operator-doppler" operator gateway install --force
operator doctor
```

To remove a persisted wrapper, clear `OPERATOR_WRAPPER` while reinstalling:
...
<AccordionGroup>
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `OPERATOR_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `OPERATOR_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
`gateway discover` scans for Gateway beacons (`_operator-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (wide-area Bonjour): choose a domain (example: `operator.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
...
```bash
operator gateway discover --timeout 4000
operator gateway discover --json | jq '.beacons[].wsUrl'
```
