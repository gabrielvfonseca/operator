...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Operator Gateway.

`operator acp` speaks ACP over stdio for IDEs and forwards prompts to the Gateway over WebSocket, keeping ACP sessions mapped to Gateway session keys. It is a Gateway-backed ACP bridge, not a full ACP-native editor runtime: it focuses on session routing, prompt delivery, and streaming updates.

If you want an external MCP client to talk directly to Operator channel conversations instead of hosting an ACP harness session, use [`operator mcp serve`](/cli/mcp) instead.

## What this is not

`operator acp` means Operator acts as an ACP server: an IDE or ACP client connects to Operator, and Operator forwards that work into a Gateway session.

This is different from [ACP Agents](/tools/acp-agents), where Operator runs an external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Operator: use `operator acp`
- Operator should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include Operator parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                     |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Operator Gateway or agent instead.                                                                                                                          |
...
# Remote Gateway
operator acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
operator acp --url wss://gateway-host:18789 --token-file ~/.operator/gateway.token

# Attach to an existing session key
operator acp --session agent:main:main

# Attach by label (must already exist)
operator acp --session-label "support inbox"

# Reset the session key before the first prompt
operator acp --session agent:main:main --reset-session
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Operator through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive `operator acp` over stdio with an ACP JSON-RPC client. Cover `initialize`, `session/new`, `session/list` with an absolute `cwd`, `session/resume`, `session/close`, duplicate close, and missing resume.
...
Avoid using `operator gateway call sessions.list` as the only ACP proof. That CLI path may request a fresh-token operator scope upgrade; ACP bridge correctness is proven by ACP stdio frames plus the Gateway `sessions.list` log.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want it to drive an Operator Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `operator acp` over stdio.

Example config (persisted):

```bash
operator config set gateway.remote.url wss://gateway-host:18789
operator config set gateway.remote.token <token>
```
...
```bash
operator acp --session agent:main:main
operator acp --session agent:design:main
operator acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Operator plugin tools or selected built-in tools such as `cron`, enable the gateway-side ACPX MCP bridges instead of trying to pass per-session `mcpServers`. See [ACP Agents](/tools/acp-agents-setup#plugin-tools-mcp-bridge) and [Operator tools MCP bridge](/tools/acp-agents-setup#operator-tools-mcp-bridge).
...
```bash
# One-shot request into your default Operator ACP session
acpx operator exec "Summarize the active Operator session state."

# Persistent named session for follow-up turns
acpx operator sessions ensure --name codex-bridge
acpx operator -s codex-bridge --cwd /path/to/repo \
...
```json
{
  "agents": {
    "@gabrielvfonseca/operator": {
      "command": "env OPERATOR_HIDE_BANNER=1 OPERATOR_SUPPRESS_NOTES=1 operator acp --url ws://127.0.0.1:18789 --token-file ~/.operator/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Operator checkout, use the direct CLI entrypoint instead of the dev runner so the ACP stream stays clean:

```bash
env OPERATOR_HIDE_BANNER=1 OPERATOR_SUPPRESS_NOTES=1 node operator.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client pull contextual information from an Operator agent without scraping a terminal.
...
In Zed, open the Agent panel and select "Operator ACP" to start a thread.
...
- `--token` and `--password` can be visible in local process listings on some systems. Prefer `--token-file`/`--password-file` or environment variables (`OPERATOR_GATEWAY_TOKEN`, `OPERATOR_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`OPERATOR_GATEWAY_*`) then `gateway.auth.*`, falling back to `gateway.remote.*` only when `gateway.auth.*` is unset (a configured-but-unresolved local SecretRef fails closed instead of silently falling back)
...
- `operator acp client` sets `OPERATOR_SHELL=acp-client` on the spawned bridge process, which can be used for context-specific shell/profile rules.
