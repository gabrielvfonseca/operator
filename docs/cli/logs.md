---
summary: "CLI reference for `operator logs` (tail gateway logs via RPC)"
read_when:
...
operator logs --follow --interval 2000
operator logs --limit 500 --max-bytes 500000
operator logs --json
operator logs --plain
operator logs --no-color
operator logs --utc
operator logs --follow --local-time
operator logs --url ws://127.0.0.1:18789 --token "$OPERATOR_GATEWAY_TOKEN"
```

## Fallback and recovery behavior

- If the implicit local loopback Gateway asks for pairing, closes during connect, or times out before `logs.tail` answers, `operator logs` falls back to the configured Gateway file log automatically. Explicit `--url` targets never use this fallback.
