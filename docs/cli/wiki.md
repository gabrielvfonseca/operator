---
summary: "CLI reference for `operator wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, ChatGPT import, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `operator wiki`
title: "Wiki"
---
...
operator wiki okf import ./knowledge-catalog/okf/bundles/ga4
operator wiki compile
operator wiki lint
operator wiki search "alpha"
operator wiki search "who should I ask about Teams?" --mode route-question
...
operator wiki chatgpt import --export ./chatgpt-export --dry-run
operator wiki chatgpt rollback <run-id>
...
operator wiki --agent marketing ingest ./campaign-notes.md
```
...
operator wiki okf import ./bundles/ga4 --json
operator wiki search "BigQuery Table" --mode source-evidence --json
operator wiki get <path-from-json-result>
```
...
Use `wiki search` for wiki-specific ranking and provenance. For one broad shared recall pass, prefer `operator memory search` when the active memory plugin exposes shared search.
...
```bash
operator wiki search "bgroux" --mode find-person
operator wiki search "who knows Teams rollout?" --mode route-question
operator wiki search "maintainer-whois" --mode source-evidence
operator wiki search "strong route Teams" --mode raw-claim --json
```
...
operator wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
```bash
operator wiki chatgpt import --export ./chatgpt-export
operator wiki chatgpt import --export ./conversations.json --dry-run
```
...
`operator wiki` behavior is shaped by:
