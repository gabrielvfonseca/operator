---
summary: "CLI reference for `operator nodes` (status, pairing, invoke, camera/canvas/screen/location/notify)"
read_when:
...
operator nodes status --last-connected 24h
operator nodes list
...
operator nodes rename --node <id|name|ip> --name <displayName>
```

These commands drive the gateway-owned `node.pair.*` store, separate from device pairing (`operator devices approve`) that gates the node's WS `connect` handshake. See [Nodes](/nodes) for how the two relate.
...
```bash
operator nodes invoke --node <id> --command system.which --params '{"bins":["uname"]}'
```
...
```bash
operator nodes notify --node <id> --title "Build" --body "Done" --priority timeSensitive
operator nodes push --node <id> --title "Operator" --environment sandbox
operator nodes location get --node <id> --accuracy precise
operator nodes screen record --node <id> --duration 10s --fps 10 --out ./clip.mp4
```

- `notify` sends a local notification on a node that declares `system.notify`, including macOS, iOS, Android, and direct watchOS nodes. Direct watchOS delivery requires Operator to be active. Requires `--title` or `--body`. Options: `--sound <name>`, `--priority <passive|active|timeSensitive>`, `--delivery <system|overlay|auto>` (default `system`), `--invoke-timeout <ms>` (default `15000`).
- `push` sends an APNs test push to an iOS node. Options: `--title <text>` (default `Operator`), `--body <text>`, `--environment <sandbox|production>` to override the detected APNs environment.
...
Camera and Canvas commands have their own docs: [Camera nodes](/nodes/camera), [Canvas](/platforms/mac/canvas). Canvas is implemented by the bundled experimental Canvas plugin; core keeps `operator nodes canvas` as a compatibility mount point.
