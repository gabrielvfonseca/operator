---
summary: "CLI reference for `operator hooks` (agent hooks)"
read_when:
...
Manage agent hooks (event-driven automations for commands like `/new`, `/reset`, and gateway startup). Bare `operator hooks` is equivalent to `operator hooks list`.
...
```bash
operator hooks list [--eligible] [--json] [-v|--verbose]
```
...
```bash
operator plugins install <package>        # npm by default
operator plugins install npm:<package>    # npm only
operator plugins install <package> --pin  # pin resolved version
operator plugins install <path>           # local directory or archive
operator plugins install -l <path>        # link a local directory instead of copying

operator plugins update <id>
operator plugins update --all
operator plugins update --dry-run
```

Hook packs install through the unified plugins installer/updater; `operator hooks install` / `operator hooks update` still work as deprecated aliases that print a warning and forward to the `plugins` commands.
...
- Bare specs and `@latest` stay on the stable track; if npm resolves to a prerelease, Operator stops and asks you to opt in explicitly (`@beta`, `@rc`, or an exact prerelease version).
...
- Install copies the pack into `~/.operator/hooks/<id>`, enables its hooks under `hooks.internal.entries.*`, and records the install under `hooks.internal.installs`.
- If a stored integrity hash no longer matches the fetched artifact, Operator warns and prompts before continuing; pass global `--yes` to bypass the prompt (for example in CI).
...
| command-logger        | `command`                                         | Logs command events to `~/.operator/logs/commands.log`                                             |
...
Enable any bundled hook with `operator hooks enable <hook-name>`. Full details, config keys, and defaults: [Bundled hooks](/automation/hooks#bundled-hooks).

### command-logger log file

```bash
tail -n 20 ~/.operator/logs/commands.log        # recent commands
cat ~/.operator/logs/commands.log | jq .          # pretty-print
grep '"action":"new"' ~/.operator/logs/commands.log | jq .   # filter by action
```
