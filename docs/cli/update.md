---
summary: "CLI reference for `operator update` (safe-ish source update + gateway auto-restart)"
read_when:
  - You want to update a source checkout safely
  - You are debugging `operator update` output or options
...
Update Operator and switch between stable/extended-stable/beta/dev channels.
...
operator update repair
operator update wizard
operator update --channel extended-stable
operator update --channel beta
operator update --channel dev
operator update --tag beta
operator update --tag main
operator update --dry-run
operator update --no-restart
operator update --yes
operator update --acknowledge-clawhub-risk
operator update --json
operator --update
```

`operator --update` rewrites to `operator update` (useful for shells and
launcher scripts).
...
| `--acknowledge-clawhub-risk`                     | Allow post-update plugin sync to continue past community ClawHub trust warnings without an interactive prompt. Without it, risky community releases are skipped and left unchanged when Operator cannot prompt. Official ClawHub packages and bundled plugin sources bypass this prompt.                                                     |

There is no `--verbose` flag. Use `--dry-run` to preview planned actions,
`--json` for machine-readable results, and `operator update status --json`
...
operator update status --timeout 10
```
...
`operator update` installed the new core package but post-core plugin sync,
managed npm plugin metadata, registry refresh, or doctor repair did not
converge.

```bash
operator update repair
operator update repair --channel beta
operator update repair --acknowledge-clawhub-risk
operator update repair --json
```
...
| `--acknowledge-clawhub-risk`                     | Same behavior as on `operator update`.                                                                                                                                                                                                                              |
| `--no-restart`                                   | Accepted for parity; repair never restarts the Gateway.                                                                                                                                                                                                             |

`update repair` runs `operator doctor --fix`, reloads the repaired config and
...
detached helper and exits, and that helper runs `operator update --yes --json`
...
healthy. If the plist is installed but launchd is not supervising it, Operator
...
`Gateway: restart failed: ...` with a manual `operator gateway restart` hint.
...
- `ok: true`, `result.status: "skipped"`,
  ...
  `operator update --yes --json` outside the live service process.
- `ok: false`, `result.reason: "managed-service-handoff-unavailable"`, and
  `handoff.status: "unavailable"`: Operator could not find a supervising
  service boundary and durable service identity for a safe handoff (for
  example, systemd handoff requires the `OPERATOR_SYSTEMD_UNIT` unit identity,
...
`operator status` and `operator status --all` show an `Update restart` row
...
<Steps>
  ...
  <Step title="Preflight build (dev only)">
    Runs the TypeScript build in a temp worktree. If the tip fails, walks back up to 10 commits to find the newest buildable commit. Set `OPERATOR_UPDATE_PREFLIGHT_LINT=1` to also run lint during this preflight; lint runs in constrained serial mode because user update hosts are often smaller than CI runners.
  </Step>
  <Step title="Rebase">
  ...
  <Step title="Run doctor">
    `operator doctor` runs as the final safe-update check.
  </Step>
...
beta release, Operator falls back to the recorded default/latest spec and
reports a warning. For npm plugins, Operator also falls back when the beta
...
<Warning>
If an exact pinned npm plugin update resolves to an artifact whose integrity differs from the stored install record, `operator update` aborts that plugin artifact update instead of installing it. Reinstall or update the plugin explicitly only after verifying you trust the new artifact.
</Warning>

<Note>
Post-update plugin sync failures that are scoped to a managed plugin and that the sync path can route around (for example an unreachable npm registry for a non-essential plugin) are reported as warnings after the core update succeeds. The JSON result keeps top-level update `status: "ok"` and reports `postUpdate.plugins.status: "warning"` with `operator update repair` and `operator plugins inspect <id> --runtime --json` guidance. Unexpected updater or sync exceptions still fail the update result. Fix the plugin install or update error, then rerun `operator update repair`. When a failed update leaves a managed plugin unusable, Operator disables its runtime entry and resets active slots without changing the operator-authored `plugins.allow` or `plugins.deny` policy.

After the per-plugin sync step, `operator update` runs a mandatory **post-core convergence** pass before the gateway restarts: it repairs missing configured plugin payloads, validates each _active_ tracked install record on disk, and statically verifies its `package.json` is parseable (and any explicitly declared `main` exists). Failures from this pass, and an invalid config snapshot, return `postUpdate.plugins.status: "error"` and flip the top-level update `status` to `"error"`, so `operator update` exits non-zero and the gateway is _not_ restarted with an unverified plugin set. The error includes structured `postUpdate.plugins.warnings[].guidance` lines pointing at `operator update repair` and `operator plugins inspect <id> --runtime --json`. Disabled plugin entries and records that are not trusted-source-linked official sync targets are skipped here (mirroring the `skipDisabledPlugins` policy used by the missing-payload check), so a stale disabled plugin record cannot block an otherwise valid update.
...
version. For default/`latest` intent, Operator does not query plugin
...
For package-manager installs, `operator update` resolves the target package
version before invoking the package manager. npm global installs use a staged
install: Operator installs the new package into a temporary npm prefix,
...
the updater approves only the candidate Operator lifecycle; transitive
dependency scripts remain blocked. Operator then swaps the clean package tree
...
plugin records aligned with the installed Operator build, while leaving full
plugin-command completion rebuilds to explicit
`operator completion --write-state` runs.

## Related

- `operator doctor` (offers to run update first on git checkouts)
