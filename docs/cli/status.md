---
summary: "CLI reference for `operator status` (diagnostics, probes, usage snapshots)"
read_when:
...
Plain `operator status` stays on the fast read-only path and marks memory as
...
`operator status --all`, `operator status --deep`, `operator security audit`,
and `operator memory status --deep`.
...
- Session status output separates `Execution:` from `Runtime:`. `Execution`
  ...
  whether the session is using `Operator Default`, `OpenAI Codex`, a CLI
...
- MiniMax's raw `usage_percent` / `usagePercent` fields are remaining quota,
  so Operator inverts them before display; count-based fields win when
...
- Update info surfaces in the Overview; if an update is available, status
  prints a hint to run `operator update` (see [Updating](/install/updating)).
