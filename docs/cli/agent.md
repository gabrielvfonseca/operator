---
summary: "CLI reference for `operator agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
operator agent --to +15555550123 --message "status update" --deliver
operator agent --agent ops --message "Summarize logs"
operator agent --agent ops --message-file ./task.md
operator agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
operator agent --session-key agent:ops:incident-42 --message "Summarize status"
operator agent --agent ops --session-key incident-42 --message "Summarize status"
...
operator agent --agent ops --message "Run locally" --local
```
...
- Slash commands (for example `/compact`) cannot run through `--message`. The CLI rejects them and points you at the first-class command instead (`operator sessions compact <key>` for compaction).
