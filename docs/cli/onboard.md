---
summary: "CLI reference for `operator onboard` (interactive onboarding)"
read_when:
  - You want to establish inference, then finish setup with Operator
title: "Onboard"
---
...
Operator to configure the rest. `operator setup` reaches this flow on fresh
systems or whenever an onboarding option is present; configured systems use
bare `operator setup` for system-agent chat. `operator setup --baseline` only
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How Operator onboarding fits together.
  </Card>
...
operator onboard --flow quickstart
operator onboard --flow manual
operator onboard --flow import
operator onboard --import-from hermes --import-source ~/.hermes
operator onboard --skip-bootstrap
operator onboard --mode remote --remote-url wss://gateway-host:18789
```
...
- `--flow import`: runs a detected migration provider (for example Hermes via `--import-from hermes`), previews the plan, then applies after confirmation. Import only runs against a fresh Operator setup - reset config, credentials, sessions, and workspace state first if any exist. Use [`operator migrate`](/cli/migrate) for dry-run plans, overwrite mode, reports, and exact mappings.
- `--modern` is a compatibility alias for the Operator conversational setup
  assistant. It uses the same live-inference gate as `operator setup` and
...
Plain `operator onboard` starts the guided flow. It shows the security notice,
...
API-key or token methods use the same live completion path. Operator persists
...
credential. Choose **Skip for now** to exit without starting Operator and
rerun `operator onboard` when you are ready. Workspace and Gateway setup remain
unchanged until Operator starts.

In guided mode, `--workspace <dir>` supplies Operator's proposed workspace
and the isolated inference context. It is not persisted until you approve the
Operator setup proposal. Classic and noninteractive onboarding persist their
workspace through their normal setup flow.

After inference passes, guided onboarding immediately starts Operator with
the verified model. Operator can then configure the workspace, Gateway,
channels, agents, plugins, and other optional features. Inside Operator, use
...
exit Operator and run `operator onboard`; Operator does not open the guided
or classic provider flows.

On a configured install, running `operator onboard` again verifies the current
...
Use `operator onboard --classic` for provider-specific auth, channels, skills,
remote Gateway setup, imports, or full Gateway controls. For conversational
non-inference setup and repair, run `operator setup`; `operator onboard
...
- If the config file exists but fails validation, it starts the classic
  onboarding path with `operator doctor` guidance. Operator needs working
...
```bash
operator onboard --reset
operator onboard --reset --reset-scope full
```
...
```bash
OPERATOR_LOCALE=zh-CN operator onboard
```
...
- `--allow-unconfigured` is a separate `operator gateway run` escape hatch; it does not let onboarding skip `gateway.mode`.

```bash
export OPENAI_API_KEY="your-provider-key"
export OPERATOR_GATEWAY_TOKEN="your-token"
operator onboard --non-interactive \
  --mode local \
  --auth-choice openai-api-key \
  --secret-input-mode ref \
  --gateway-auth token \
  --gateway-token-ref-env OPERATOR_GATEWAY_TOKEN \
  --accept-risk
```
...
- `--install-daemon` starts the managed gateway install path first. Without it, a local gateway must already be running (for example `operator gateway run`).
...
With `--modern`, JSON is a one-shot Operator overview and exits after that
...
- Fastest first chat: `operator dashboard` (Control UI, no channel setup).
