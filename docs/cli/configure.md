---
summary: "CLI reference for `operator configure` (interactive configuration prompts)"
read_when:
...
Use `operator onboard` or `operator setup` for the full guided first-run journey, `operator setup --baseline` for the baseline config/workspace only, and `operator channels add` when you only need channel account setup.

<Tip>
`operator config` with no subcommand opens the same wizard. Use `operator config get|set|unset` for non-interactive edits.
</Tip>
...
operator configure --section model --section channels
operator configure --section gateway --section daemon
```
...
<Note>
`operator configure` requires an interactive terminal (both stdin and stdout must be TTYs). Without one it prints the equivalent non-interactive `operator config get|set|patch|validate` commands and exits with an error instead of partially running.
</Note>
...
Re-running provider auth from configure preserves an existing `agents.defaults.model.primary`, even when the provider's auth step returns a config patch with its own recommended default model. Adding or reauthing a provider makes its models available without taking over your current primary model. Use `operator models auth login --provider <id> --set-default` or `operator models set <model>` to intentionally change the default model.
</Note>
...
`operator configure --section web` picks a web-search provider and configures its credentials. Some providers show provider-specific follow-ups:
