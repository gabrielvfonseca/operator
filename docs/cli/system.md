---
summary: "CLI reference for `operator system` (system events, heartbeat, presence)"
read_when:
...
operator system event --text "Check for urgent follow-ups" --url ws://127.0.0.1:18789 --token "$OPERATOR_GATEWAY_TOKEN"
operator system heartbeat enable
operator system heartbeat last
operator system presence
```
