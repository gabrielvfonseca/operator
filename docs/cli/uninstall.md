---
summary: "CLI reference for `operator uninstall` (remove gateway service + local data)"
read_when:
...
operator uninstall
operator uninstall --service --yes --non-interactive
operator uninstall --state --workspace --yes --non-interactive
operator uninstall --all --yes
operator uninstall --dry-run
```

## Notes

- Run `operator backup create` first for a restorable snapshot before removing
  state or workspaces.
