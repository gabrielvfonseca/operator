---
summary: "CLI reference for `operator security` (audit and fix common security footguns)"
read_when:
...
operator security audit --deep --token <token>
operator security audit --auth password --password <password>
operator security audit --fix
...
Run `operator doctor --fix` to rotate a persisted reused `hooks.token`, then update external hook senders to use the new token.
...
- Warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `operator.browserConfigEpoch`) and recommends `operator sandbox recreate --browser --all`.
...
```bash
operator security audit --json | jq '.summary'
operator security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

With `--fix --json`, output includes both fix actions and the final report:

```bash
operator security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
...
- tightens permissions for state/config and common sensitive files (`credentials/*.json`, `auth-profiles.json`, `operator-agent.sqlite`, and legacy session artifacts)
- also tightens config include files referenced from `operator.json`
