...
- Send media with an optional caption via `operator message send --media`.
- Allow auto-replies from the web inbox to include media alongside text.
- Keep per-type limits sane and predictable.

## CLI Surface

`operator message send --target <dest> --media <path-or-url> [--message <caption>]`
...
- When media is present, the web sender resolves local paths or URLs using the same pipeline as `operator message send`.
- Multiple media entries are sent sequentially if provided.

## Inbound Media To Commands

- When inbound web messages include media, Operator downloads it to a temp file and exposes templating variables:
...
- Media understanding (configured via `tools.media.*` or shared `tools.media.models`) runs before templating and can insert `[Image]`, `[Audio]`, and `[Video]` blocks into `Body`.
  ...
  - If the active primary model already supports vision natively, Operator skips the `[Image]` summary block and passes the original image to the model instead.
