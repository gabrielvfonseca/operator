...
| `consultThinkingLevel`                   | unset                                      | Thinking level override for the agent run behind realtime `operator_agent_consult` calls.                                                                                                                                                                                  |
| `consultFastMode`                        | unset                                      | Fast-mode override for realtime `operator_agent_consult` calls.                                                                                                                                                                                                            |
...
| `realtime.consultRouting`                | -                                          | `provider-direct` preserves the provider's direct reply when it skips `operator_agent_consult`; `force-agent-consult` routes finalized user transcripts through Operator instead.                                                                                          |
| `realtime.instructions`                  | -                                          | Appends provider-facing system instructions to Operator's built-in realtime prompt (voice style/tone); the default `operator_agent_consult` guidance stays.                                                                                                                |
...
- macOS local MLX playback uses the bundled `operator-mlx-tts` helper when present, or an executable on `PATH`. Set `OPERATOR_MLX_TTS_BIN` to point at a custom helper binary during development.
