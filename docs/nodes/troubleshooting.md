...
```bash
operator nodes describe --node <idOrNameOrIp>
operator nodes canvas snapshot --node <idOrNameOrIp>
operator logs --follow
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`operator approvals get --node ...`), not in the gateway pairing record.
...
operator nodes status
operator approvals get --node <idOrNameOrIp>
operator approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
| `ACCESSIBILITY_REQUIRED`               | Grant Accessibility to the current Operator app bundle in macOS System Settings.                                                                                                        |
