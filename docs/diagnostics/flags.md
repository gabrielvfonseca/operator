...
- Flags are case-insensitive strings, resolved from `diagnostics.flags` in
  config plus the `OPERATOR_DIAGNOSTICS` env override, deduped and lowercased.
...
```bash
OPERATOR_DIAGNOSTICS=telegram.http,brave.http
```
...
`OPERATOR_DIAGNOSTICS=0` disables flags from both env and config for that
...
```bash
OPERATOR_DIAGNOSTICS=profiler operator gateway run
```

Enable only reply-dispatch profiler spans:

```bash
OPERATOR_DIAGNOSTICS=reply.profiler operator gateway run
```

Enable only Codex app-server startup/tool/thread profiler spans:

```bash
OPERATOR_DIAGNOSTICS=codex.profiler operator gateway run
```
...
`OPERATOR_DIAGNOSTICS=0` to override every diagnostics flag for that run.
...
OPERATOR_DIAGNOSTICS_TIMELINE_PATH=/tmp/operator-timeline.jsonl \
...
The output path always comes from `OPERATOR_DIAGNOSTICS_TIMELINE_PATH`, even
...
are missing because Operator has not read config yet; subsequent startup spans
are captured normally.

`OPERATOR_DIAGNOSTICS=1`, `=all`, and `=*` also enable the timeline, since they
...
`timeline`: set `OPERATOR_DIAGNOSTICS_EVENT_LOOP=1` (or `on`/`true`/`yes`) on
top of enabling the timeline.

Timeline records use the `operator.diagnostics.v1` envelope and can include
