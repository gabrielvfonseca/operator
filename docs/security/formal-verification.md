---
summary: Machine-checked security models for Operator's highest-risk paths.
...
Operator's formal security models (TLA+/TLC today) give a machine-checked argument that specific highest-risk paths — authorization, session isolation, tool gating, and misconfiguration safety — enforce their intended policy, under explicit stated assumptions.
...
This is **not** a proof that Operator is secure in all respects, and it does not verify the full TypeScript implementation.

## Where the models live

Models are maintained in a separate repo: [vignesh07/operator-formal-models](https://github.com/vignesh07/operator-formal-models).

<Note>
That repository is currently unreachable (GitHub returns "Repository not found" as of this writing). If it is still broken for you, ask in the Operator maintainer channels for the current location before assuming the models were removed.
</Note>
...
```bash
git clone https://github.com/vignesh07/operator-formal-models
cd operator-formal-models
