---
summary: "Use Kilo Gateway's unified API to access many models in Operator"
title: "Kilo Gateway"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Operator
---
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    operator onboard --auth-choice kilocode-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    operator models list --provider kilocode
    ```
  </Step>
</Steps>
...
The default model is `kilocode/kilo/auto`, a provider-owned smart-routing model. Operator does not
publish a task-to-upstream-model mapping for it; routing behind `kilo/auto` is owned by Kilo Gateway.

At startup Operator queries `GET https://api.kilo.ai/api/gateway/models` and merges discovered models
...
`operator models list --provider kilocode` to see the full discovered list.
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path: Operator sanitizes Gemini thought
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, Operator falls back to the static catalog containing `kilocode/kilo/auto`.
    - Confirm your API key is valid and that your Kilo account has the desired models enabled.
    - When Gateway runs as a daemon, ensure `KILOCODE_API_KEY` is available to that process (for example in `~/.operator/.env` or via `env.shellEnv`).
