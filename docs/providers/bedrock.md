---
summary: "Use Amazon Bedrock (Converse API) models with Operator"
read_when:
  - You want to use Amazon Bedrock models with Operator
  - You need AWS credential/region setup for model calls
title: "Amazon Bedrock"
---

Operator can use **Amazon Bedrock** models via its **Bedrock Converse**
...
<Tabs>
  <Tab title="Access keys / env vars">
    **Best for:** developer machines, CI, or hosts where you manage AWS credentials directly.
    ...
    <Tip>
    With env-marker auth (`AWS_ACCESS_KEY_ID`, `AWS_PROFILE`, or `AWS_BEARER_TOKEN_BEDROCK`), Operator auto-enables the implicit Bedrock provider for model discovery without extra config.
    </Tip>
    ...
    **Best for:** EC2 instances with an IAM role attached, using the instance metadata service for authentication.
    ...
    <Steps>
      <Step title="Enable discovery explicitly">
        When using IMDS, Operator cannot detect AWS auth from env markers alone, so you must opt in:

        ```bash
        operator config set plugins.entries.amazon-bedrock.config.discovery.enabled true
        operator config set plugins.entries.amazon-bedrock.config.discovery.region us-east-1
        ```
      </Step>
      <Step title="Optionally add an env marker for auto mode">
        If you also want the env-marker auto-detection path to work (for example, for `operator status` surfaces):
...
Operator can automatically discover Bedrock models that support **streaming**
...
- If `plugins.entries.amazon-bedrock.config.discovery.enabled` is `true`,
  Operator will try discovery even when no AWS env marker is present.
- If `plugins.entries.amazon-bedrock.config.discovery.enabled` is unset,
  Operator only auto-adds the
...
<Note>
For explicit `models.providers["amazon-bedrock"]` entries, Operator can still resolve Bedrock env-marker auth early from AWS env markers such as `AWS_BEARER_TOKEN_BEDROCK` without forcing full runtime auth loading. The actual model-call auth path still uses the AWS SDK default chain.
</Note>
...
<AccordionGroup>
  <Accordion title="Discovery config options">
    ...
    | ------ | ------- | ----------- |
    | `enabled` | auto | In auto mode, Operator only enables the implicit Bedrock provider when it sees a supported AWS env marker. Set `true` to force discovery. |
  ...
  <Accordion title="Context window and max-token limits">
    ...
    status. Operator ships a lookup table of known context windows and output
...
the instance profile, and enables Operator discovery on the EC2 host.
...
<AccordionGroup>
  <Accordion title="Inference profiles">
    Operator discovers **regional and global inference profiles** alongside
    ...
    first in `operator models list` since they generally offer better capacity
    ...
    foundation models in `operator models list`.
  ...
  <Accordion title="Service tier">
    ...
    Fable 5 and Sonnet 5 only support the `default` tier; Operator warns and
  ...
  <Accordion title="Claude Opus 4.7 and 4.8 temperature">
    ...
    4.8. Operator omits `temperature` automatically for any matching Bedrock
  ...
  <Accordion title="Claude Fable 5">
    ...
    Operator applies Fable's 1M context window, 128K output limit, always-on
  ...
  <Accordion title="Claude Mythos 5">
    ...
    required limited-access approval. Operator recognizes the foundation model
    `anthropic.claude-mythos-5` and regional or global inference profiles such
    as `us.anthropic.claude-mythos-5`.

    Operator applies the 1,000,000-token context window, 128,000-token output
  ...
  <Accordion title="Claude Sonnet 5">
    ...
    Operator recognizes the Bedrock foundation model
    ...
    Bedrock keeps adaptive thinking enabled for Sonnet 5. Operator defaults to
  ...
  <Accordion title="Notes and caveats">
    ...
    - Operator surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,
