...
read_when:
  - You want to use Featherless AI with Operator
...
OpenAI-compatible API. Operator installs Featherless as an official external
...
```bash
operator onboard --auth-choice featherless-api-key
```
...
```bash
operator models list --provider featherless
```

## Default model

The plugin uses `Qwen/Qwen3-32B` as the setup default because Featherless
documents native tool calling for the Qwen 3 family. Operator configures its
...
modes and Operator does not embed account-specific plan or request-pricing
rates.
...
Operator deliberately does not copy Featherless's full public model index into
...
- Managed Gateway cannot see the key: put it in `~/.operator/.env` or another
