...
title: "Google (Gemini)"
read_when:
  - You want to use Google Gemini models with Operator
    ...
    **Best for:** standard Gemini API access through Google AI Studio.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        operator onboard --auth-choice gemini-api-key
        ```
      ...
      <Step title="Verify the model is available">
        ```bash
        operator models list --provider google
        ```
      </Step>
    </Steps>
    ...
    **Best for:** signing in with your Google account through Gemini CLI OAuth instead of using a separate API key.
    ...
    <Steps>
      <Step title="Install the Gemini CLI">
        ...
        Operator supports both Homebrew installs and global npm installs, including
    ...
    Gemini 3.1 Pro's Gemini API model id is `gemini-3.1-pro-preview`. Operator accepts the shorter `google/gemini-3.1-pro` as a convenience alias and normalizes it before provider calls.

    **Environment variables:**

    - `OPERATOR_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
    - `OPERATOR_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
...
<Note>
`google/gemini-3-pro-preview` was retired on 2026-03-09; use `google/gemini-3.1-pro-preview` instead. Re-running Gemini API key setup (`operator onboard --auth-choice gemini-api-key` or `operator models auth login --provider google`) rewrites a stale configured default to the current model.
</Note>
...
<Tip>
Gemini 3 models use `thinkingLevel` rather than `thinkingBudget`. Operator maps
...
a fixed Operator level. Gemini 3 and Gemini 3.1 omit a fixed `thinkingLevel` so
Google can choose the level; Gemini 2.5 sends Google's dynamic sentinel
`thinkingBudget: -1`.

Gemma 4 models (for example `gemma-4-26b-a4b-it`) support thinking mode. Operator
...
`thinkingBudget: 0`; Operator strips that value for Gemini 2.5 Pro requests
...
Operator adapts telephony/Meet bridge audio to Gemini's PCM Live API stream and
keeps tool calls on the shared realtime voice contract. Leave `temperature`
unset unless you need sampling changes; Operator omits non-positive values
...
sequential function calling. Operator omits the older `NON_BLOCKING`, function
...
<AccordionGroup>
  <Accordion title="Direct Gemini cache reuse">
    For direct Gemini API runs (`api: "google-generative-ai"`), Operator
    ...
    - Gemini cache-hit usage is normalized into Operator `cacheRead` from
  ...
  <Accordion title="Gemini CLI usage notes">
    When using the `google-gemini-cli` OAuth provider, Operator uses Gemini
    ...
    - `stats.cached` is normalized into Operator `cacheRead`.
    - If `stats.input` is missing, Operator derives input tokens from
  ...
  <Accordion title="Environment and daemon setup">
    ...
    is available to that process (for example, in `~/.operator/.env` or via
    `env.shellEnv`).
  </Accordion>
</AccordionGroup>
