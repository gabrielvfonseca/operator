---
summary: "Use GMI Cloud's OpenAI-compatible API with Operator"
read_when:
  - You want to run Operator with GMI Cloud models
  - You need the GMI provider id, key, or endpoint
title: "GMI Cloud"
---

GMI Cloud is a hosted inference platform for frontier and open-weight models
behind an OpenAI-compatible API. In Operator it is an official external provider
...
primary provider does. Operator owns the provider id, auth profile, aliases,
...
| Package       | `@gabrielvfonseca/gmi-provider`          |
...
```bash
operator onboard --auth-choice gmi-api-key
```
...
```bash
operator models list --provider gmi
```

## Troubleshooting

- `401` or `403`: check that `GMI_API_KEY` is set for the process running
  Operator, or re-run onboarding to store the key in the provider auth profile.
- Unknown model errors: confirm the model exists in your GMI account and use the
  full `gmi/<route-id>` ref shown by `operator models list --provider gmi`.
