---
summary: "Run Operator with Ollama (cloud and local models)"
read_when:
  - You want to run Operator with cloud or local models via Ollama
  - You need Ollama setup and configuration guidance
  - You want Ollama vision models for image understanding
title: "Ollama"
---

Operator talks to Ollama's native API (`/api/chat`), not the OpenAI-compatible
...
<AccordionGroup>
  <Accordion title="Local and LAN hosts">
    Loopback, private-network, `.local`, and bare-hostname Ollama URLs do not need a real bearer token. Operator uses the `ollama-local` marker for these.
  </Accordion>
  ...
  <Tab title="Onboarding (recommended)">
    <Steps>
      <Step title="Run onboarding">
        ...
        On a fresh guided setup, Operator first checks the default or configured
      ...
      <Step title="Verify">
        ```bash
        operator models list --provider ollama
        ```
      </Step>
    </Steps>
    ...
    ```bash
    operator onboard --non-interactive \
      --auth-choice ollama \
  ...
  <Tab title="Manual setup">
    <Steps>
      ...
      <Step title="Set a credential">
        ...
        Or in config: `operator config set models.providers.ollama.apiKey "OLLAMA_API_KEY"`.
      </Step>
      <Step title="Select the model">
        ```bash
        operator models list
        operator models set ollama/gemma4
        ```
...
Operator prompts for the base URL, discovers local models, and checks
...
For cloud-only access without a local daemon, use `operator onboard --auth-choice ollama-cloud` and see [Ollama Cloud](/providers/ollama-cloud) — that path does not need `ollama signin` or a running server:

```bash
operator onboard --auth-choice ollama-cloud
operator models set ollama-cloud/kimi-k2.5:cloud
```

The cloud model list shown during `operator onboard` is populated live from
...
models at setup time, Operator falls back to its hardcoded suggested list so
...
defined, Operator discovers models from `http://127.0.0.1:11434`:
...
| Token limits         | `maxTokens` defaults to Operator's Ollama max-token cap                                                                                                                                                                                                                                       |
...
hand-written `models.json` entry; Operator resolves it live. For signed-in
...
`operator infer audio transcribe` for audio):
...
provider and `/api/tags` is unreachable, Operator records that run as
...
```bash
OPERATOR_LIVE_TEST=1 OPERATOR_LIVE_OLLAMA=1 OPERATOR_LIVE_OLLAMA_WEB_SEARCH=0 \
  pnpm test:live -- extensions/ollama/ollama.live.test.ts
```

For Ollama Cloud, point the same live test at the hosted endpoint (skips
embeddings by default; force with `OPERATOR_LIVE_OLLAMA_EMBEDDINGS=1` since a
cloud key may not authorize `/api/embed`):

```bash
export OLLAMA_API_KEY='<your-ollama-cloud-api-key>'
OPERATOR_LIVE_TEST=1 OPERATOR_LIVE_OLLAMA=1 \
OPERATOR_LIVE_OLLAMA_BASE_URL=https://ollama.com \
OPERATOR_LIVE_OLLAMA_MODEL=glm-5.1:cloud \
OPERATOR_LIVE_OLLAMA_WEB_SEARCH=1 \
pnpm test:live -- extensions/ollama/ollama.live.test.ts
```
...
<Steps>
  ...
  <Step title="Connect the node host">
    ...
    `ollama.models` and `ollama.chat`, check `operator nodes pending` again.
...
```bash
operator config set plugins.entries.ollama.config.nodeInference.enabled false
```

Restart the node (`operator node restart`, or stop/rerun `operator node run`
...
surface may need `operator nodes pending` approval again after reconnect.
...
media-understanding provider, so Operator can route explicit image-description
...
operator infer image describe --file ./photo.jpg --model ollama/qwen2.5vl:7b --json
```
...
that already support native vision. If the call fails, Operator can continue
through `agents.defaults.imageModel.fallbacks`; file/URL preparation errors
fail before fallback is attempted. Use `infer image describe` for Operator's
...
```bash
OPERATOR_LIVE_TEST=1 OPERATOR_LIVE_OLLAMA_IMAGE=1 \
...
Operator rejects image-description requests for models not marked
...
<Tabs>
  <Tab title="Basic (implicit discovery)">
    ...
    <Tip>
    If `OLLAMA_API_KEY` is set, you can omit `apiKey` in the provider entry; Operator fills it in for availability checks.
    </Tip>
...
<AccordionGroup>
  ...
  <Accordion title="LAN Ollama host with manual models">
    ...
    `contextWindow` is Operator's context budget; `params.num_ctx` is sent to
  ...
  <Accordion title="Multiple Ollama hosts">
    ...
    Operator strips the active provider prefix (falling back to a bare
...
prefix, such as `ollama-spark/qwen3:32b`, Operator strips that prefix before
...
# Operator catalog and selected model
operator models list --provider ollama
...
works but Operator does not, check whether the Gateway runs on a different
machine, container, or service account.

## Ollama Web Search

Operator bundles **Ollama Web Search** as a `web_search` provider.
...
Choose it during `operator onboard` or `operator configure --section web`, or set:
...
For a self-hosted host, Operator first tries the local `/api/experimental/web_search`
...
<AccordionGroup>
  <Accordion title="Legacy OpenAI-compatible mode">
    ...
    **Tool calling is not reliable in this mode.** Use it only when a proxy needs OpenAI format and you do not depend on native tool calling.
    ...
    Operator injects `options.num_ctx` by default in this mode so Ollama does
  ...
  <Accordion title="Context windows">
    For auto-discovered models, Operator uses the context window `/api/show`
    reports, including larger `PARAMETER num_ctx` values from custom
    Modelfiles; otherwise it falls back to Operator's default Ollama context
    window.
    ...
    model. `contextWindow` is Operator's own prompt/compaction budget. Native
    ...
    `operator doctor --fix` to copy those into `params.num_ctx`. The
    ...
    top-level request fields instead of nested `options`. Operator only
  ...
  <Accordion title="Thinking control">
    Operator forwards thinking as Ollama expects it: top-level `think`, not
    ...
    ```bash
    operator agent --model ollama/gemma4 --thinking off
    operator agent --model ollama/gemma4 --thinking low
    ```
    ...
    thinking for a specific model. Operator preserves that explicit config
  ...
  <Accordion title="Memory embeddings">
    ...
    loopback origin derived from the configured `baseUrl` use Operator's
  ...
  <Accordion title="Streaming configuration">
    ...
    and `operator agent --thinking off` send top-level `think: false` unless
...
<AccordionGroup>
  <Accordion title="WSL2 crash loop (repeated reboots)">
    ...
    Operator logs a startup warning when it detects WSL2, `ollama.service`
  ...
  <Accordion title="Remote host works with curl but not Operator">
  ...
  <Accordion title="Kimi or GLM returns garbled symbols">
    ...
    ```bash
    operator infer model run --model ollama/kimi-k2.5:cloud --prompt "Reply with exactly: ok" --json
    operator models set ollama/gemma4
    ```
  ...
  <Accordion title="Large-context model is too slow or runs out of memory">
    ...
    `params.num_ctx` is set. Cap both Operator's budget and Ollama's request
    ...
    Lower `contextWindow` if Operator sends too much prompt. Lower
