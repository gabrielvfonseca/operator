---
summary: "Use ElevenLabs speech, Scribe STT, and realtime transcription with Operator"
read_when:
  - You want ElevenLabs text-to-speech in Operator
...
Operator uses ElevenLabs for text-to-speech, batch speech-to-text with Scribe
...
| Capability               | Operator surface                                                     | Default                  |
| ------------------------ | -------------------------------------------------------------------- | ------------------------ |
...
Set `modelId` to `eleven_v3` to use ElevenLabs v3 TTS. Operator keeps
...
returned audio stream instead of waiting for Operator to download the whole
audio file first. `latencyTier` maps to ElevenLabs' `optimize_streaming_latency`
query parameter for models that accept it; Operator omits that parameter for
...
Operator sends multipart audio to ElevenLabs `/v1/speech-to-text` with
