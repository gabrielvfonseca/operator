---
summary: "Use OpenAI via API keys or Codex subscription in Operator"
read_when:
  - You want to use OpenAI models in Operator
  - You want Codex subscription auth instead of API keys
  - You need stricter GPT-5 agent execution behavior
title: "OpenAI"
---

Operator uses one provider id, `openai`, for both direct API-key auth and
...
facts decide whether Operator may select the bundled Codex app-server runtime
...
workflows like Operator.

## Usage and cost tracking

Operator keeps subscription quota and Platform API billing distinct:
...
- Operator never sends `OPENAI_API_KEY` or an `openai` inference profile to organization APIs; those credentials may belong to custom, Azure, or agent-local endpoints.

An explicit Admin key takes precedence over OAuth. Provider-reported history is not merged with Operator's session-derived estimated cost; it can include API activity from other clients and provider-side billing adjustments.
...
| Account without GPT-5.6 access                    | `openai/gpt-5.5`                                                                    | Explicit recovery choice; Operator does not silently downgrade.     |
| Direct API-key billing, explicit Operator runtime | `openai/gpt-5.6` plus provider/model `agentRuntime.id: "@gabrielvfonseca/operator"` | Select a normal `openai` API-key profile.                           |
...
| Authored `openai-completions` adapter                                                                                                                                  | Operator              |
| Custom endpoint                                                                                                                                                        | Operator              |
| Explicit exact official endpoint using HTTP                                                                                                                            | Rejected              |
| Route with an authored provider/model request override                                                                                                                 | Operator              |

An explicit non-default provider/model `agentRuntime.id` remains authoritative.
For example, `agentRuntime.id: "@gabrielvfonseca/operator"` keeps an otherwise Codex-eligible
route on Operator, while `agentRuntime.id: "codex"` requires Codex and fails
...
`operator doctor --fix` migrates legacy `codex/*` and `openai-codex/*` model
...
Operator recognizes the exact `openai/gpt-5.6-sol`,
...
```bash
operator models list --provider openai
```
...
```bash
operator models set openai/gpt-5.5
```

Operator surfaces the upstream access error and does not silently replace a
...
custom endpoints, and request-transport overrides remain on Operator. Plaintext
official HTTP endpoints are rejected. Explicit provider/model runtime config remains
authoritative. Run `operator doctor --fix` to repair stale legacy Codex model
refs, `codex-cli/*` refs, or old runtime session pins that were not set by
explicit runtime config.
</Note>

## Operator feature coverage

| OpenAI capability         | Operator surface                                                                              | Status                                                          |
| ------------------------- | --------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
...
`operator onboard --auth-choice openai-api-key`, a Platform API key set via
...
Operator can use OpenAI, or an OpenAI-compatible embedding endpoint, for
...
`queryInputType` and `documentInputType` under `memorySearch`. Operator
    ...
    **Best for:** direct API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        operator onboard --auth-choice openai-api-key
        ```

        Or pass the key directly:

        ```bash
        operator onboard --openai-api-key "$OPENAI_API_KEY"
        ```
      </Step>
    ...
    | `openai/gpt-5.6` | provider/model `agentRuntime.id: "@gabrielvfonseca/operator"`                  | Operator embedded runtime | Selected `openai` API-key profile |
    | `openai/gpt-5.5` | explicit provider/model `agentRuntime.id`                     | Selected agent runtime    | Selected OpenAI API-key profile   |
    | `openai/*`       | authored Completions, custom, or request override | Operator embedded runtime | Credential type remains unchanged |
    ...
    non-agent OpenAI API surfaces. Run `operator doctor --fix` to migrate older
    ...
    `chat-latest` alias only accepts `medium` text verbosity; Operator forces
    any other requested verbosity to `medium` for this model.

    <Warning>
    Operator does **not** expose `gpt-5.3-codex-spark` on the direct OpenAI
    ...
    **Best for:** using your ChatGPT/Codex subscription with native Codex
    ...
    <Steps>
      <Step title="Run Codex OAuth">
        ```bash
        operator onboard --auth-choice openai
        ```
        ...
        ```bash
        operator models auth login --provider openai --device-code
        ```
      </Step>
      <Step title="Use the canonical OpenAI model route">
        ```bash
        operator config set agents.defaults.model.primary openai/gpt-5.6-sol
        ```
        ...
        Operator installs or repairs the bundled Codex plugin when that runtime
        is chosen.
      </Step>
    ...
    | `openai/gpt-5.6-sol`     | provider/model `agentRuntime.id: "@gabrielvfonseca/operator"`                  | Operator embedded runtime, internal Codex-auth transport | Selected `openai` OAuth profile                    |
    | `openai/gpt-5.5`         | explicit provider/model `agentRuntime.id`                     | Selected agent runtime                                   | Selected OpenAI auth profile                       |
    | `openai/*`               | authored Completions, custom, or request override | Operator embedded runtime                                | Credential requirement remains route-specific      |
    | `openai/*`               | plaintext official HTTP endpoint                  | Rejected                                                 | Credential is not sent                              |
    ...
    Codex GPT refs are legacy Operator routes, not the native Codex runtime
    path; run `operator doctor --fix` to migrate them without upgrading an
    ...
    the auth order under `openai`. Operator tries the subscription first, then
    ...
    browser OAuth (default) or the device-code flow above; Operator manages the
    ...
    operator models auth list --provider openai
    operator config get agents.defaults.model --json
    operator config get models.providers.openai.agentRuntime --json
    ```

    For a specific agent, add `--agent <id>`:

    ```bash
    operator models status --agent <id>
    operator models auth list --agent <id> --provider openai
    ```
    ...
    ```bash
    operator models auth login --provider openai
    operator models status --probe --probe-provider openai
    ```
    ...
    ```bash
    operator models auth login --provider openai --profile-id openai:ritsuko
    operator models auth login --provider openai --profile-id openai:lain
    ```

    Run `operator doctor --fix` to migrate older legacy OpenAI Codex prefix
    ...
    or session state, `operator doctor --fix` rewrites them to `openai/*` with
    the Codex runtime unless Operator is explicitly configured.

    ### Context window cap

    Operator treats model metadata and the runtime context cap as separate
    ...
    Operator uses upstream Codex catalog metadata for `gpt-5.5` when it is
    present. If live Codex discovery omits the `gpt-5.5` row while the account
    is authenticated, Operator synthesizes that OAuth model row so cron,
...
account-based. Operator selects auth in this order:
...
2. The app-server's existing account, such as a local Codex CLI ChatGPT
   sign-in. For the default isolated agent home, Operator bridges that native
...
subscription-style Codex profile is selected, Operator also keeps
...
When that subscription profile is blocked by a Codex usage limit, Operator
...
still accepted. Operator also protects the public OpenAI and OpenAI Codex OAuth
...
`operator infer image edit` when starting from an input file.
...
an `openai` OAuth profile is configured, Operator resolves that stored OAuth
...
also set `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`; Operator
...
```
/tool image_generate model=openai/gpt-image-2 prompt="A polished launch poster for Operator on macOS" size=3840x2160 count=1
```
...
`supportsResolution`. Operator's shared normalization layer converts a
...
Operator adds a shared GPT-5 prompt contribution for GPT-5-family models on
...
project-doc behavior, and Operator disables Codex's built-in personality for
native threads so agent workspace personality files stay authoritative.
Operator contributes only runtime context to native Codex threads: channel
delivery, Operator dynamic tools, ACP delegation, workspace context, and
Operator skills. The heartbeat-guidance text from this same contribution is the
...
checks, and verification on matching Operator-assembled prompts. Channel-
specific reply and silent-message behavior stays in the shared Operator system
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash
    operator config set agents.defaults.promptOverlays.gpt5.personality off
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Speech synthesis (TTS)">
    ...
    `extraBody` is merged into `/audio/speech` request JSON after Operator's
  ...
  <Accordion title="Speech-to-text">
    ...
    Operator's media-understanding transcription surface.
  ...
  <Accordion title="Realtime voice">
    ...
    **GPT-Live (upcoming).** OpenAI's full-duplex `gpt-live-1` and
    ...
    developer API is rolling out to early-access organizations. Operator
    ...
    through a handoff event protocol that Operator's realtime transports do
    ...
    model has emitted audio in that session. Operator currently exposes the
...
generation by overriding the base URL. On the image-generation path, Operator
...
Operator recognizes these Azure host suffixes for the Azure image-generation
route:
...
For image-generation requests on a recognized Azure host, Operator:
...
Operator 2026.4.22 or later. Earlier versions treat any custom
...
routed through the bundled `openai` provider, the `model` field in Operator
...
Operator. If an Azure request fails with a validation error, check the
...
Operator's hidden attribution headers - see the **Native vs OpenAI-compatible
...
The per-model `params` examples below shape Operator's embedded provider
request. Configuring them is authored request behavior, so an otherwise eligible
`auto` route stays on Operator instead of selecting Codex implicitly. The native
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    Operator uses WebSocket-first with SSE fallback (`"auto"`) for `openai/*`.

    In `"auto"` mode, Operator:
  ...
  <Accordion title="Fast mode">
    Operator exposes a shared fast-mode toggle for `openai/*`:

    - **Chat/UI:** `/fast status|auto|on|off`
    - **Config:** `agents.defaults.models["<provider>/<model>"].params.fastMode`

    When enabled, Operator maps fast mode to OpenAI priority processing
  ...
  <Accordion title="Priority processing (service_tier)">
    ...
    If you route either provider through a proxy, Operator leaves
  ...
  <Accordion title="Server-side compaction (Responses API)">
    ...
    OpenAI plugin's Operator stream wrapper auto-enables server-side
    compaction:
    ...
    This applies to the built-in Operator runtime path and to OpenAI provider
  ...
  <Accordion title="Strict-agentic GPT mode">
    For `openai` provider GPT-5-family models run through Operator's embedded
    runtime, Operator already defaults to a stricter execution contract called
    ...
    With `strict-agentic` active, Operator:
    ...
    Operator does not classify assistant prose to decide whether a turn is a
    plan, progress update, or final answer.

    <Note>
    This contract lives entirely in Operator's embedded agent runner. It does
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    Operator treats direct OpenAI, Codex, and Azure OpenAI endpoints
