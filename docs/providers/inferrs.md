---
summary: "Run Operator through inferrs (OpenAI-compatible local server)"
read_when:
  - You want to run Operator against a local inferrs server
  - You are serving Gemma or another model through inferrs
  - You need the exact Operator compat flags for inferrs
title: "Inferrs"
---

[inferrs](https://github.com/ericcurtin/inferrs) serves local models behind an OpenAI-compatible `/v1` API. Operator talks to it through the generic `openai-completions` adapter.

| Property           | Value                                                                |
| ------------------ | -------------------------------------------------------------------- |
| Provider id        | `inferrs` (custom; configure under `models.providers.inferrs`)       |
| Plugin             | none — not a bundled Operator provider plugin                        |
...
<Note>
  `inferrs` is a custom self-hosted OpenAI-compatible backend, not a dedicated Operator provider plugin: you configure it under `models.providers.inferrs` instead of picking an onboarding auth choice. For a bundled plugin with auto-discovery, see [SGLang](/providers/sglang) or [vLLM](/providers/vllm).
</Note>
...
<Steps>
  ...
  <Step title="Add an Operator provider entry">
...
Operator can start `inferrs` itself only when an `inferrs/...` model is selected. Add `localService` to the same provider entry:
...
<AccordionGroup>
  <Accordion title="Why requiresStringContent matters">
    ...
    <Warning>
    If Operator runs fail with:

    ```text
    messages[1].content: invalid type: sequence, expected a string
    ```

    set `compat.requiresStringContent: true` in the model entry. Operator then flattens pure text content parts into plain strings before sending the request.
    </Warning>

  </Accordion>

  <Accordion title="Gemma and tool-schema caveat">
    Some `inferrs` + Gemma combinations accept small direct `/v1/chat/completions` requests but fail on full Operator agent-runtime turns. Try disabling the tool schema surface first:
    ...
    That reduces prompt pressure on stricter local backends. If tiny direct requests still work but normal Operator agent turns keep crashing inside `inferrs`, treat it as an upstream model/server limitation rather than an Operator transport issue.
...
<AccordionGroup>
  ...
  <Accordion title="Direct /v1/chat/completions calls pass but operator infer model run fails">
...
<CardGroup cols={2}>
  <Card title="Local models" href="/gateway/local-models" icon="server">
    Running Operator against local model servers.
  </Card>
