---
summary: "Sign in to GitHub Copilot from Operator using the device flow or non-interactive token import"
read_when:
  - You want to use GitHub Copilot as a model provider
  - You need the `operator models auth login-github-copilot` flow
  - You are choosing between the built-in Copilot provider, Copilot SDK harness, and Copilot Proxy
title: "GitHub Copilot"
---

GitHub Copilot is GitHub's AI coding assistant. It provides access to Copilot
models for your GitHub account and plan. Operator can use Copilot as a model
provider or agent runtime in three different ways.

## Three ways to use Copilot in Operator

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token, then exchange it for
    Copilot API tokens when Operator runs. This is the **default** and simplest path
    ...
    <Steps>
      ...
      <Step title="Set a default model">
        ```bash
        operator models set github-copilot/claude-opus-4.7
        ```
  ...
  <Tab title="Copilot SDK harness plugin (copilot)">
    Install the external `@gabrielvfonseca/copilot` plugin when you want GitHub's
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. Operator talks to
    the proxy's `/v1` endpoint (default `http://localhost:3000/v1`) and uses the
    model list you configure.

    The `copilot-proxy` plugin ships with Operator and is enabled by default.
    Configure the base URL and model ids with:

    ```bash
    operator models auth login --provider copilot-proxy --set-default
    ```
...
endpoints rather than public `github.com`. Operator exposes this as a
first-class auth choice so you do not have to hand-edit URLs.

<Steps>
  <Step title="Pick the Enterprise auth choice">
    In onboarding or `operator models auth`, choose
    **GitHub Copilot (Enterprise / data residency)**. You will be prompted for
    ...
    Operator derives those endpoints from the tenant root automatically.

    ```bash
    operator models auth login --provider github-copilot --method device-enterprise
    ```
...
tenant, or one tenant to another), Operator will not reuse the existing token —
...
`operator models auth login-github-copilot` shortcut, token refresh, embeddings,
...
| ---------------------------------------------------------------------- | --------------- | ---------------------------------------------------- |
| `operator models auth login-github-copilot`                            | `--yes`         | Overwrite an existing auth profile without prompting |
| `operator models auth login --provider github-copilot --method device` | `--set-default` | Also apply the provider's recommended default model  |

```bash
# Skip the re-login confirmation
operator models auth login-github-copilot --yes

# Login and set the default model in one step
operator models auth login --provider github-copilot --method device --set-default
```

## Non-interactive onboarding

The device-login flow requires an interactive TTY. For headless setup, import
an existing GitHub OAuth access token with `operator onboard --non-interactive`:
...
<AccordionGroup>
  ...
  <Accordion title="Live catalog refresh from the Copilot API">
    ...
    Operator refreshes the model catalog on demand from `${baseUrl}/models`
    ...
    churn. Newly published Copilot models become visible without an Operator
  ...
  <Accordion title="Transport selection">
    ...
    models keep the OpenAI Responses transport. Operator selects the correct
    transport based on the model ref.
  </Accordion>

  <Accordion title="Request compatibility">
    Operator sends Copilot IDE-style request headers on Copilot transports
  ...
  <Accordion title="Environment variable resolution order">
    Operator resolves Copilot auth from environment variables in the following
    priority order:
    ...
    When multiple variables are set, Operator uses the highest-priority one.
    The device-login flow (`operator models auth login-github-copilot`) stores
  ...
  <Accordion title="Token storage">
    ...
    token when Operator runs. You do not need to manage the token manually.
  </Accordion>
</AccordionGroup>
...
have logged in, Operator can use it for embeddings without a separate API key.

### Config

Set `memorySearch.provider` explicitly to use GitHub Copilot embeddings. If a
GitHub token is available, Operator discovers available embedding models from
...
1. Operator resolves your GitHub token (from env vars or auth profile).
...
available, Operator skips Copilot and tries the next provider.
