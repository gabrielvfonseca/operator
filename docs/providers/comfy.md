---
summary: "ComfyUI workflow image, video, and music generation setup in Operator"
title: "ComfyUI"
read_when:
  - You want to use local ComfyUI workflows with Operator
  - You want to use Comfy Cloud with image, video, or music workflows
  - You need the bundled comfy plugin config keys
---

Operator ships a bundled `comfy` plugin for workflow-driven ComfyUI runs. The
plugin is entirely workflow-driven: Operator does not map generic `size`,
    ...
    **Best for:** running your own ComfyUI instance on your machine or LAN.
    ...
    <Steps>
      ...
      <Step title="Prepare your workflow JSON">
        Export or create a ComfyUI workflow JSON file. Note the node IDs for the prompt input node and the output node you want Operator to read from.
      </Step>
      ...
      <Step title="Set the default model">
        Point Operator at the `comfy/workflow` model for the capability you configured:
      ...
      <Step title="Verify">
        ```bash
        operator models list --provider comfy
        ```
      </Step>
    </Steps>
    ...
    **Best for:** running workflows on Comfy Cloud without managing local GPU resources.
    ...
    <Steps>
      ...
      <Step title="Set the API key">
        ...
        ```bash
        # Onboarding flag
        operator onboard --comfy-api-key "your-key"
        ...
        # Or inline in config
        operator config set plugins.entries.comfy.config.apiKey "your-key"
        ```
      </Step>
      <Step title="Prepare your workflow JSON">
...
<AccordionGroup>
  ...
  <Accordion title="Video workflows">
    ...
    <Note>
    Operator does not pass input videos into Comfy workflows. Only text prompts and single reference images are supported as inputs.
    </Note>
  ...
  <Accordion title="Backward compatibility">
    ...
    Operator treats that legacy shape as the image workflow config. You do not need to migrate immediately, but the nested `image` / `video` / `music` sections are recommended for new setups. If you only use image generation, the legacy flat config and the new nested `image` section are functionally equivalent.
  ...
  <Accordion title="Live tests">
    ...
    ```bash
    OPERATOR_LIVE_TEST=1 COMFY_LIVE_TEST=1 pnpm test:live -- extensions/comfy/comfy.live.test.ts
    ```
