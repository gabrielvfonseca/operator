---
summary: "Use NovitaAI's OpenAI-compatible API with Operator"
read_when:
  - You want to run Operator with NovitaAI models
...
It ships as a bundled Operator provider (no separate plugin install), so
...
```bash
operator onboard --auth-choice novita-api-key
```
...
```bash
operator models list --provider novita
```
...
- `401`/`403`: verify the key in Novita's key management page and re-run
  `operator onboard --auth-choice novita-api-key` if the stored profile is
  stale.
