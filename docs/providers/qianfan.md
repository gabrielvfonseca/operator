---
summary: "Use Qianfan's unified API to access many models in Operator"
read_when:
...
Qianfan is Baidu's MaaS platform: a unified, OpenAI-compatible API that routes requests to many models behind a single endpoint and API key. Operator ships it as the official external plugin `@gabrielvfonseca/qianfan-provider`.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    operator onboard --auth-choice qianfan-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    operator models list --provider qianfan
    ```
  </Step>
</Steps>
