---
summary: "Use OpenRouter's unified API to access many models in Operator"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in Operator
...
OpenAI-compatible, so Operator talks to it over the same
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run OAuth onboarding">
        ```bash
        operator onboard --auth-choice openrouter-oauth
        ```

        Operator opens OpenRouter's browser sign-in flow (PKCE), exchanges the
        code for an OpenRouter API key, and stores it in the default
        OpenRouter auth profile. On remote/headless hosts, Operator prints the
      ...
      <Step title="(Optional) Switch to a specific model">
        ...
        ```bash
        operator models set openrouter/<provider>/<model>
        ```
  ...
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run API-key onboarding">
        ```bash
        operator onboard --auth-choice openrouter-api-key
        ```
      </Step>
      <Step title="(Optional) Switch to a specific model">
        Onboarding defaults to `openrouter/auto`. Pick a concrete model later:
...
Operator sends image requests to OpenRouter's chat-completions image API with
...
Operator submits text-to-video and image-to-video jobs, polls the returned
...
and also exposes `google/lyria-3-clip-preview`. Operator sends `modalities:
...
Operator sends OpenRouter STT requests as JSON with base64 audio under
`input_audio` (OpenRouter's STT contract), not as multipart OpenAI form
uploads.

## Fusion router

OpenRouter Fusion sends one Operator model ref to several OpenRouter models in
...
`openrouter/fusion`, so the Operator model ref carries both the Operator
provider prefix and the upstream OpenRouter namespace:

```bash
operator models set openrouter/openrouter/fusion
```
...
in normal agent/chat turns to try to force Fusion: Operator turns can include
...
Operator adds a sanitized system-prompt note listing the configured analysis
...
```bash
operator infer model run --local \
...
login flow that issues an OpenRouter API key, so Operator stores the result in
...
```bash
operator models auth login --provider openrouter --method oauth
operator models auth login --provider openrouter --method api-key
```

On verified OpenRouter requests (`https://openrouter.ai/api/v1`), Operator adds
...
<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, Operator
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    Operator sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, Operator maps the selected thinking level
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    V4's required follow-up shape. Operator sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: Operator keeps
  ...
  <Accordion title="Provider routing metadata">
    ...
    Operator forwards that object to OpenRouter as the request `provider`
