---
summary: "Use Vydra image, video, and speech in Operator"
read_when:
  - You want Vydra media generation in Operator
...
Operator uses the same `VYDRA_API_KEY` for all three capabilities.
...
<Steps>
  <Step title="Run interactive onboarding">
    ```bash
    operator onboard --auth-choice vydra-api-key
    ```
...
<AccordionGroup>
  ...
  <Accordion title="Video live tests">
    ...
    ```bash
    export OPERATOR_LIVE_VYDRA_KLING_IMAGE_URL="https://example.com/reference.png"
    ```
