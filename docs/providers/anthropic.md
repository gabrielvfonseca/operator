---
summary: "Use Anthropic Claude via API keys or Claude CLI in Operator"
read_when:
  - You want to use Anthropic models in Operator
  - You want to browse Claude CLI or Claude Desktop sessions across paired computers
title: "Anthropic"
---

Anthropic builds the **Claude** model family. Operator supports two auth routes:
...
Operator detects the available Anthropic credential and selects the matching usage surface:
...
Admin API cost history comes from Anthropic's [Usage and Cost API](https://platform.claude.com/docs/en/manage-claude/usage-cost-api). It is actual provider billing, separate from Operator's session-derived estimated cost.

<Warning>
Operator's Claude CLI backend runs the installed Claude Code CLI in
...
<Tabs>
    ...
    **Best for:** standard API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        operator onboard --anthropic-api-key "$ANTHROPIC_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        operator models list --provider anthropic
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Claude CLI">
    **Best for:** reusing an existing Claude CLI login without a separate API key.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        Operator detects and reuses the existing Claude CLI credentials.
      </Step>
      <Step title="Verify the model is available">
        ```bash
        operator models list --provider anthropic
    ...
    <Warning>
    Claude CLI reuse expects the Operator process to run on the same host as the
    ...
    a provider with Operator-managed OAuth such as
    ...
    **Anthropic setup-token** under **Connect with an API key or token**, or use:
    ...
    ```bash
    operator models auth login --provider anthropic --method setup-token
    ```
    ...
    Operator uses Claude Code's non-interactive `claude -p` path for Claude CLI
    ...
    Operator release. Check `claude auth status`, `/status`, and
    ...
    Claude CLI. Operator also supports subscription-style options from
...
No additional Operator config is required for discovery. The Anthropic plugin
...
`sessions.catalog.continue`. Operator re-resolves the local catalog record,
...
and its local `claude` executable resolves. Operator re-resolves the catalog
...
advertises `agent.cli.claude.run.v1`. Operator never modifies Claude Desktop
...
higher native effort levels. Operator omits manual thinking budgets, custom
...
`/think off` and `/think minimal` map to `low` effort instead. Operator also
...
adaptive-thinking contract. Operator defaults to `high`, maps `/think off` and
...
Claude Opus 4.8 keeps thinking off by default in Operator. When you explicitly
enable adaptive thinking with `/think high|xhigh|max`, Operator sends
...
recovery is to have `claude-opus-4-8` serve that turn. Operator opts into this
...
1. For every direct API-key request to `anthropic/claude-fable-5`, Operator
...
2. Only a safety-classifier decline triggers the fallback. Rate limits,
   ...
   Operator's normal [model failover](/concepts/model-failover).
...
- Anthropic bills per attempt: a decline before output is free, and the rescue
  bills at Claude Opus 4.8 rates (currently half of Fable 5 rates). Operator's
...
- A mid-stream decline additionally bills the already-streamed Fable partial
  ...
  usage but not folded into Operator's per-turn estimate.
...
fallbacks, and the same prompt through Operator returns a normal Opus-served
...
Operator supports Anthropic's prompt caching feature for API-key auth.
...
<AccordionGroup>
  <Accordion title="Fast mode">
    Operator's shared `/fast` toggle sets Anthropic's `service_tier` field for direct API-key traffic to `api.anthropic.com`.
  ...
  <Accordion title="Media understanding (image and PDF)">
    The bundled Anthropic plugin registers image and PDF understanding. Operator
    ...
    When an image or PDF is attached to a conversation, Operator automatically
  ...
  <Accordion title="1M context window">
    ...
    Opus 4.7, Opus 4.6, and Sonnet 4.6. Operator sizes these models
    ...
    these models and Operator no longer sends the retired
    ...
    <Warning>
    Requires long-context access on your Anthropic credential. OAuth/subscription token auth keeps its required Anthropic beta headers, but Operator strips the retired 1M beta header if it remains in older config.
    </Warning>
...
<AccordionGroup>
  ...
  <Accordion title='No API key found for provider "anthropic"'>
    Anthropic auth is **per agent**; new agents do not inherit the main agent's keys. Re-run onboarding for that agent (or configure an API key on the gateway host), then verify with `operator models status`.
  </Accordion>

  <Accordion title='No credentials found for profile "anthropic:default"'>
    Run `operator models status` to see which auth profile is active. Re-run onboarding, or configure an API key for that profile path.
  </Accordion>

  <Accordion title="No available auth profile (all in cooldown)">
    Check `operator models status --json` for `auth.unusableProfiles`. Anthropic rate-limit cooldowns can be model-scoped, so a sibling Anthropic model may still be usable. Add another Anthropic profile or wait for cooldown.
  </Accordion>
</AccordionGroup>
