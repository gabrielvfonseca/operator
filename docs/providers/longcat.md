...
title: "LongCat"
read_when:
  - You want to use LongCat-2.0 with Operator
  - You need the LongCat API key or model limits
---

[LongCat](https://longcat.ai) provides a hosted API for LongCat-2.0, a
reasoning model built for coding and agentic workloads. Operator provides the
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    operator onboard --auth-choice longcat-api-key
    ```
  </Step>
  <Step title="Verify the model">
    ```bash
    operator models list --provider longcat
    ```
  </Step>
</Steps>
...
LongCat exposes binary thinking control. Operator maps enabled thinking levels
to `thinking: { type: "enabled" }` and `/think off` to
`thinking: { type: "disabled" }`. LongCat does not currently document
`reasoning_effort`, so Operator does not send it.

LongCat returns reasoning in `reasoning_content`. Operator preserves that field
...
model through an OpenAI-compatible runtime and use Operator's existing
...
<AccordionGroup>
  <Accordion title="The key works in a shell but not in the Gateway">
    ...
    variable. Put `LONGCAT_API_KEY` in `~/.operator/.env`, configure it through
  ...
  <Accordion title="The model does not appear">
    Run `operator plugins list` and confirm the `longcat` plugin is
    enabled, then run `operator models list --provider longcat`.
  </Accordion>
</AccordionGroup>
