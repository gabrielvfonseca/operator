---
summary: "Use StepFun models with Operator"
read_when:
  - You want StepFun models in Operator
  - You need StepFun setup guidance
title: "StepFun"
---

StepFun ships as an external official plugin (`@gabrielvfonseca/stepfun-provider`) with two provider ids:
...
```bash
operator plugins install @gabrielvfonseca/stepfun-provider
...
<Tabs>
  <Tab title="Standard">
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        operator onboard --auth-choice stepfun-standard-api-key-intl
        ```

        China endpoint:

        ```bash
        operator onboard --auth-choice stepfun-standard-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        operator models list --provider stepfun
        ```
      </Step>
    </Steps>
  ...
  <Tab title="Step Plan">
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        operator onboard --auth-choice stepfun-plan-api-key-intl
        ```

        China endpoint:

        ```bash
        operator onboard --auth-choice stepfun-plan-api-key-cn
        ```
      </Step>
      <Step title="Non-interactive alternative">
        ```bash
      ...
      <Step title="Verify models are available">
        ```bash
        operator models list --provider stepfun-plan
        ```
      </Step>
    </Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Notes">
    - `step-3.7-flash` accepts text and image input through Operator. StepFun's API also supports video, which is not yet a model input modality in Operator.
    ...
    - Use `operator models list` and `operator models set <provider/model>` to inspect or switch models.
