---
summary: "Run Operator through ds4, a local DeepSeek V4 Flash OpenAI-compatible server"
read_when:
  - You want to run Operator against antirez/ds4
  - You want a local DeepSeek V4 Flash backend with tool calls
  - You need the Operator config for ds4-server
title: "ds4"
---

[ds4](https://github.com/antirez/ds4) serves DeepSeek V4 Flash from a local
Metal backend with an OpenAI-compatible `/v1` API. Operator connects to ds4
through the generic `openai-completions` provider family.

ds4 is not a bundled Operator provider plugin. Configure it under
...
<Warning>
Operator agent turns include tool schemas and workspace context. A tiny context
...
<Steps>
  ...
  <Step title="Add the Operator provider config">
    ...
    operator infer model run \
      ...
      --thinking off \
      --prompt "Reply with exactly: operator-ds4-ok" \
      --json
    ```
...
with `--tokens` unless you intentionally want Operator to request less output
than the server default.

## On-demand startup

Operator can start ds4 only when a `ds4/...` model is selected. Add
...
If you run that large context, update both the server flags and Operator model
metadata:
...
Direct HTTP check, bypassing Operator:
...
Operator model routing (same as the Quickstart check):
...
<AccordionGroup>
  ...
  <Accordion title="500 prompt exceeds context">
    The configured `--ctx` is too small for the Operator turn. Raise
  ...
  <Accordion title="The first request is slow">
    ...
    `localService.readyTimeoutMs: 300000` when Operator starts the server on
    demand.
  </Accordion>
</AccordionGroup>
