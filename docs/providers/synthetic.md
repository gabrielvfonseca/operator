---
summary: "Use Synthetic's Anthropic-compatible API in Operator"
read_when:
...
Operator bundles it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    operator onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Operator's Anthropic client appends `/v1` to the base URL automatically, so use
...
`operator models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Operator still appends `/v1` automatically.
