...
read_when:
  ...
  - You are changing plugin behavior across Operator and Codex harness turns
---
...
continuation, and native compaction. Operator owns channel routing, session
files, visible message delivery, Operator dynamic tools, approvals, media
...
Operator compatibility route keeps the normal Operator system prompt even when
it uses Codex-flavored OpenAI auth or transport.

Operator starts and resumes native Codex threads with Codex's built-in
personality disabled (`personality: "none"`) so workspace personality files
and Operator agent identity stay authoritative. Native Codex keeps Codex-owned
base/model instructions and project-doc loading otherwise. Lightweight
Operator runs (for example cron) still suppress project-doc loading.

Operator developer instructions cover Operator runtime concerns: source-channel
delivery, Operator dynamic tools, ACP delegation, adapter context, and the
...
When an Operator session is attached to an existing Codex thread, the next
...
Supervised bindings are the exception. The Operator model picker stays locked,
...
model-difference warning; the outer Operator model and fallback chain never
...
not resume a thread, subscribe Operator to its live events, or answer its
approvals.
...
On the same supervision connection, Operator starts the canonical
...
resumed. The canonical thread has the full Operator harness tool surface;
...
Operator allows archive for a local `idle` or `notLoaded` row only after explicit
...
prove that another process is not using the thread. Operator blocks a known
...
contract: the agent replies normally and Operator posts the final text to the
...
Codex heartbeat turns get `heartbeat_respond` in the searchable Operator tool
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| Operator plugin hooks                 | Operator                 | Product/plugin compatibility across Operator and Codex harnesses.   |
| Codex app-server extension middleware | Operator bundled plugins | Per-turn adapter behavior around Operator dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

Operator does not use project or global Codex `hooks.json` files to route
plugin behavior. For the native tool and permission bridge, Operator injects
...
so Codex's app-server reviewer and Operator's approval bridge handle real
...
controls; they are not exposed as Operator plugin hooks in the v1 contract.

For Operator dynamic tools, Operator executes the tool after Codex asks for
the call, so plugin and middleware behavior runs in the harness adapter. For
Codex-native tools, Codex owns the canonical tool record; Operator can mirror
...
matching app-server approval. If an Operator `before_tool_call` hook returns
`requireApproval` while the native payload sets `operator_approval_mode:
...
request for the same tool use, Operator opens the plugin approval prompt and
maps the decision back to Codex. Codex `PermissionRequest` events are a
separate approval path and can still route through Operator approvals when
...
notifications and Operator adapter state, not native Codex hook commands.
...
debugging. They do not invoke Operator plugin hooks.
...
| Operator channel routing and delivery         | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                                                                                                                                                                                                                                                                                                    |
| Operator dynamic tools                        | Supported                                                                        | Codex asks Operator to execute these tools, so Operator stays in the execution path.                                                                                                                                                                                                                                                                                                                                                                                                |
| Prompt and context plugins                    | Supported                                                                        | Operator projects Operator-specific prompt/context into the Codex turn while leaving Codex-owned base, model, and configured project-doc prompts in the native Codex lane. Operator disables Codex's built-in personality for native threads so agent workspace personality files remain authoritative. Native Codex developer instructions accept only command guidance explicitly scoped to `codex_app_server`; legacy global command hints remain for non-Codex prompt surfaces. |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around Operator-owned dynamic tools.                                                                                                                                                                                                                                                                                                                                                                          |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through Operator after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review.                                                                                                                                                                                                                                                                          |
| App-server trajectory capture                 | Supported                                                                        | Operator records the request it sent to app-server and the app-server notifications it receives.                                                                                                                                                                                                                                                                                                                                                                                    |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but Operator does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. Operator owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms Operator-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | Operator can request native compaction, but does not receive a stable kept/dropped list, token delta, completion summary, or summary payload.   | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | Operator does not let plugins or context engines veto, rewrite, or replace native Codex compaction.                                             | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | Operator can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, Operator only returns explicit allow or deny
...
request, Operator remembers that exact provider/session/tool input/cwd
...
Codex MCP tool approval elicitations route through Operator's plugin approval
...
default `messages.queue.mode: "steer"`, Operator batches steer-mode chat
...
that case, Operator waits for the active run to finish before starting the
...
harness, Operator also calls Codex app-server `feedback/upload` for relevant
...
Operator session ids, Codex thread ids, and local `codex resume <thread-id>`
commands for the threads that were sent.

If you deny or ignore the approval, Operator does not print those Codex ids
...
belongs to Codex app-server. Operator does not run preflight compaction for
Codex turns, replace Codex compaction with context-engine compaction, or fall
back to Operator or public OpenAI summarization when native compaction cannot
be started. Operator keeps a transcript mirror for channel history, search,
...
Operator keeps the request and shared-client lease open until Codex emits the
...
timeout, Operator requests a native turn interrupt. The lease and per-thread
...
period, Operator retires the connection before releasing the fence. Remote
...
compaction is Codex's job; Operator only starts native compaction for manually
requested triggers.

When a context engine requests Codex thread-bootstrap projection, Operator
...
Codex reasoning or plan records when the app-server emits them. Operator
...
not rewrite Codex-native tool result records. It only applies when Operator
writes an Operator-owned session transcript tool result.

## Media and delivery

Operator continues to own media delivery and media provider selection. Image,
...
through the normal Operator delivery path; media generation does not require
the legacy runtime. When Codex emits a native image-generation item with a
`savedPath`, Operator forwards that exact file through the normal reply-media
