...
Operator-owned state stays on the Gateway under `<state-dir>/logbook/`, but
...
- A connected node that exposes `screen.snapshot` or `logbook.snapshot`. The
  ...
  (`operator node host run`) gets the plugin-provided `logbook.snapshot`
...
- The bundled Codex plugin enabled and authenticated. Codex currently provides
  ...
  `operator models auth login --provider openai`; see
...
```bash
operator plugins enable codex
operator plugins enable logbook
```
...
operator plugins inspect logbook --runtime --json
...
1. `operator plugins list --enabled` includes `logbook`.
