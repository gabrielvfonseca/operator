---
summary: "Browse non-archived native Codex sessions and paginated transcripts across Operator nodes"
title: "Supervise Codex sessions"
sidebarTitle: "Codex supervision"
read_when:
  - You want Codex Desktop or CLI sessions to appear in Operator
...
- A stored or idle local session can create a model-locked Operator Chat from
  ...
  supervised binding prevents Operator from substituting another runtime,
...
- Install the official `@gabrielvfonseca/codex` plugin on the Gateway. The Operator
  macOS app can install it when you enable Codex features; CLI installations can
  run `operator plugins install @gabrielvfonseca/codex`.
- Install and sign in to Codex Desktop or the Codex CLI on each computer whose
  sessions you want to list.
- Pair remote computers as Operator nodes. Each computer must opt in locally;
...
Guided `operator onboard` and macOS first-run setup attempt to install and
...
Enable the `codex` plugin and its supervision capability in `operator.json`:
...
sessions visible in both apps without making ordinary Operator turns share
...
Codex clients. Operator does not assume that another client shares the same live
...
The native Operator macOS app reads the same local setting when it advertises
...
`NODE_LIST_FAILED`, compare `operator nodes list` and **Settings > Devices**;
...
```bash
operator codex sessions [--search <text>] [--host <id>] [--limit <count>] [--cursor <cursor>] [--json] [--url <url>] [--token <token>] [--timeout <ms>] [--expect-final]
operator codex continue <thread-id> [--json] [--url <url>] [--token <token>] [--timeout <ms>] [--expect-final]
operator codex archive <thread-id> --confirm-no-other-runner [--json] [--url <url>] [--token <token>] [--timeout <ms>] [--expect-final]
```

`operator codex sessions` options:
...
Operator creates a normal Chat entry, mirrors bounded user and assistant history
...
connection, Operator starts the canonical `appServer`-source full harness thread
...
has the full Operator harness tool surface. This is a visible-history branch, not
...
rather than another Operator model runtime or the ordinary agent-home harness.
...
last turn, Codex emits its normal model-difference warning. Operator uses the
...
Operator omits model and provider overrides. If the canonical thread is changed
through a separate native Codex control, Operator accepts Codex's persisted
selection. Operator never substitutes its outer model or fallback chain.
...
Operator does not subscribe to or answer approval requests while merely listing
...
Operator catalog. The canonical branch is stored as a native Codex thread, but
...
An active row reported by Operator's App Server cannot start a new branch. Wait
...
other Codex client or Operator runner is using that thread or its spawned
descendants. Operator freshly reads the process-local status, proceeds only for
...
Archive is also blocked when Operator knows that an active binding owns the
exact target thread or any non-archived spawned descendant. Operator follows the
...
`operator doctor --fix` moves a retired `codex-supervisor` entry, its endpoint
...
**No sessions appear:** verify that `@gabrielvfonseca/codex` is installed, both the
...
**Old `codex-supervisor` config remains:** run `operator doctor --fix`. Doctor
