---
summary: "Build simple typed agent tools with defineToolPlugin and operator plugins init/build/validate"
title: "Tool plugins"
sidebarTitle: "Tool Plugins"
read_when:
  - You want to build a simple Operator plugin that only adds agent tools
...
manifest metadata Operator needs to discover tools without loading plugin
runtime code.
...
| `package.json`         | Scripts, runtime deps, `operator.extensions: ["./dist/index.js"]` |
| `operator.plugin.json` | Generated manifest metadata for the initial tool                  |

`npm run plugin:build` runs `npm run build` (tsc) then
`operator plugins build --entry ./dist/index.js`. `npm run plugin:validate`
rebuilds and runs `operator plugins validate --entry ./dist/index.js`.
...
`operator plugins init <id>` options:
...
is sent to a model. `operator plugins build` writes the matching
`toolMetadata.<tool>.optional` manifest entry, so Operator can see that the
...
`defineToolPlugin` wraps plain return values into the Operator tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and Operator to keep the original value in `details`.
...
`configSchema` is optional. Omit it and Operator applies a strict empty object
...
Operator reads plugin config from the plugin's entry in the Gateway config. Do
...
Operator must read the plugin manifest before importing plugin runtime code.
`defineToolPlugin` exposes static metadata for this, and
`operator plugins build` writes it into the package. Rerun the generator after
changing plugin id, name, description, config schema, activation, or tool
names:

```bash
npm run build
operator plugins build --entry ./dist/index.js
```
...
`contracts.tools` is the important discovery contract: it tells Operator which
...
`operator plugins build` also aligns `package.json` to the selected runtime
entry:
...
```json
{
  ...
  "peerDependencies": {
    "@gabrielvfonseca/operator": ">=2026.5.17"
  },
...
```bash
npm run build
operator plugins build --entry ./dist/index.js --check
operator plugins validate --entry ./dist/index.js
npm test
```

`plugins validate` checks that:

- `operator.plugin.json` exists and passes the normal manifest loader.
...
- `package.json` points `operator.extensions` at the selected runtime entry.

## Install and inspect locally

From a separate Operator checkout or installed CLI, install the package path:

```bash
operator plugins install ./stock-quotes
operator plugins inspect stock-quotes --runtime
```
...
```bash
npm pack
operator plugins install npm-pack:./operator-plugin-stock-quotes-0.1.0.tgz
operator plugins inspect stock-quotes --runtime --json
```
...
```bash
operator plugins install clawhub:your-org/stock-quotes
```

Bare npm package specs still install from npm during the launch cutover, but
ClawHub is the preferred discovery and distribution surface for Operator
...
`operator plugins build --entry ./dist/index.js` or
`operator plugins validate --entry ./dist/index.js`.
...
### `operator.plugin.json generated metadata is stale`
...
Commit both `operator.plugin.json` and `package.json` changes.

### `package.json operator.extensions must include ./dist/index.js`

The package metadata points at a different runtime entry. Run
`operator plugins build --entry ./dist/index.js` so the generator aligns
...
1. `operator plugins inspect <plugin-id> --runtime`
2. `operator plugins validate --root <plugin-root> --entry ./dist/index.js`
3. `operator.plugin.json` has `contracts.tools` with the expected tool names.
4. `package.json` has `operator.extensions: ["./dist/index.js"]`.
