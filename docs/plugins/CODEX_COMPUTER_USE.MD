---
summary: "Set up Codex Computer Use for Codex-mode Operator agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode Operator agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. Operator
...
Use this page when Operator is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

This is distinct from Operator's built-in [node-backed computer tool](/nodes/computer-use). Use the built-in tool when the same agent contract should control a paired Mac whether the agent runs on the Gateway or another node. Use Codex Computer Use when Codex app-server should own local MCP installation, permissions, and native tool calls.

## Operator.app and Peekaboo

Operator.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want Operator.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode Operator agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an Operator node and exposes mobile
...
Operator-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through Operator's MCP registry instead of the
...
available as a normal Operator MCP server. Use the Codex Computer Use setup on
...
its app prompts for, such as Accessibility and Screen Recording. Operator does
...
Computer Use in and lets Operator install or re-enable it before the turn:
...
With this config, Operator checks Codex app-server before each Codex-mode
turn. If Computer Use is missing but Codex app-server has already discovered
an installable marketplace, Operator asks Codex app-server to install or
re-enable the plugin and reload MCP servers. On macOS, when no matching
marketplace is registered and a standard desktop app bundle exists, Operator
...
installed, Operator falls back to the managed Codex binary installed beside the
...
`appServer.command` config or `OPERATOR_CODEX_APP_SERVER_BIN` still overrides
this managed selection.

Operator serializes native Codex config reads and Computer Use installation
...
`codex` plugin command surface is available. These are Operator chat/runtime
commands, not `operator codex ...` CLI subcommands:
...
Operator uses the same app-server API that Codex itself exposes. The
...
marketplaces. During install, Operator polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds (default 60 seconds).

If multiple known marketplaces contain Computer Use, Operator prefers
...
`computer-use` is registered, Operator tries to add the first standard
...
reconciliation so Operator does not override that selection.
...
| Field                           | Env var                                                        |
| ------------------------------- | -------------------------------------------------------------- |
| `enabled`                       | `OPERATOR_CODEX_COMPUTER_USE`                                  |
| `autoInstall`                   | `OPERATOR_CODEX_COMPUTER_USE_AUTO_INSTALL`                     |
| `marketplaceDiscoveryTimeoutMs` | `OPERATOR_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS` |
| `liveTestTimeoutMs`             | `OPERATOR_CODEX_COMPUTER_USE_LIVE_TEST_TIMEOUT_MS`             |
| `toolCallTimeoutMs`             | `OPERATOR_CODEX_COMPUTER_USE_TOOL_CALL_TIMEOUT_MS`             |
| `healthCheckEnabled`            | `OPERATOR_CODEX_COMPUTER_USE_HEALTH_CHECK_ENABLED`             |
| `healthCheckIntervalMinutes`    | `OPERATOR_CODEX_COMPUTER_USE_HEALTH_CHECK_INTERVAL_MINUTES`    |
| `pluginCacheMode`               | `OPERATOR_CODEX_COMPUTER_USE_PLUGIN_CACHE_MODE`                |
| `strictReadiness`               | `OPERATOR_CODEX_COMPUTER_USE_STRICT_READINESS`                 |
| `autoRepair`                    | `OPERATOR_CODEX_COMPUTER_USE_AUTO_REPAIR`                      |
| `marketplaceSource`             | `OPERATOR_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE`               |
| `marketplacePath`               | `OPERATOR_CODEX_COMPUTER_USE_MARKETPLACE_PATH`                 |
| `marketplaceName`               | `OPERATOR_CODEX_COMPUTER_USE_MARKETPLACE_NAME`                 |
| `pluginName`                    | `OPERATOR_CODEX_COMPUTER_USE_PLUGIN_NAME`                      |
| `mcpServerName`                 | `OPERATOR_CODEX_COMPUTER_USE_MCP_SERVER_NAME`                  |

## What Operator checks

Operator reports a stable setup reason internally and formats the
...
permissions before it can inspect or control apps. If Operator says Computer
...
Operator intentionally fails closed when `computerUse.enabled` is true. A
...
retry in a fresh Operator session. If the host previously ran Computer Use
...
Codex-native tool hook could not reach an active Operator relay through the
local bridge or Gateway fallback. Start a fresh Operator session with `/new`
...
Operator Gateway so old threads and hook registrations are dropped, then
