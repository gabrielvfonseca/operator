---
summary: "Run Operator embedded agent turns through the external GitHub Copilot SDK harness"
...
read_when:
  ...
  - You are wiring an agent to subscription Copilot (github / operator / copilot) and want it to run through the Copilot CLI
---

The external `@gabrielvfonseca/copilot` plugin runs embedded subscription Copilot
agent turns through the GitHub Copilot CLI (`@github/copilot-sdk`) instead of
Operator's built-in harness. The Copilot CLI session owns the low-level
agent loop: native tool execution, native compaction (`infiniteSessions`), and
CLI-managed thread state under `copilotHome`. Operator still owns chat
...
[Side questions (`/btw`)](#side-questions-btw)), and `operator doctor`.
...
- Operator with the `@gabrielvfonseca/copilot` plugin installed.
- If your config uses `plugins.allow`, include `copilot` (the manifest id the
  ...
  `@gabrielvfonseca/copilot` will not match and leaves the plugin blocked, even with
...
- A writable `copilotHome` directory. Defaults to `<agentDir>/copilot` when
  Operator provides an agent directory, otherwise
  `~/.operator/agents/<agentId>/copilot`.

`operator doctor` runs the plugin's [doctor contract](#doctor) for
...
[Quickstart](#quickstart). Without that opt-in, Operator uses its built-in
GitHub Copilot provider and never installs this plugin.

The runtime resolves the SDK in this order:

1. `import("@github/copilot-sdk")` from the installed `@gabrielvfonseca/copilot`
   package.
2. The fallback dir `~/.operator/npm-runtime/copilot/` (legacy on-demand
   install target).
...
through Operator's guarded fetch path so DNS pinning and SSRF policy stay
owned by Operator. Use the native Operator runtime for local Ollama, LM
Studio, or LAN model servers.

## BYOK

Copilot BYOK uses the SDK's session-level custom provider contract. Operator
...
4. **Env-var fallback**, checked in this order (first non-empty value wins,
   ...
   1. `OPERATOR_GITHUB_TOKEN` — harness-specific override; lets you pin a
      token for the Operator harness without disturbing system-wide `gh` /
      Copilot CLI config.
...
Operator's `models.json` / `auth-profiles.json`), or
`~/.operator/agents/<agentId>/copilot` when no agent directory is supplied.
Override with `copilotHome: <path>` on the attempt input for a custom
location (for example, a shared mount for migration).

Live harness tests use `OPERATOR_COPILOT_AGENT_LIVE_TOKEN` for a direct
...
| `reasoningEffort`        | `"low" \| "medium" \| "high" \| "xhigh"`. Maps from Operator's `ThinkLevel` / `ReasoningLevel` resolution in `auto-reply/thinking.ts`.                                                                                                                                                          |
| `infiniteSessionConfig`  | Optional override for the SDK `infiniteSessions` block driven by `harness.compact`. Safe to leave as-is.                                                                                                                                                                                        |
| `hooksConfig`            | Optional native Copilot SDK `SessionHooks` config for tool/MCP, user-prompt, session, and error callbacks. Separate from Operator's portable lifecycle hooks.                                                                                                                                   |
...
Operator plugin hooks need no Copilot-specific attempt configuration. The
...
`before_compaction` and `after_compaction`. Bridged Operator tools run
`before_tool_call` and report `after_tool_call`; `hooksConfig` remains for
native SDK-only callbacks with no portable equivalent.

Nothing else in Operator needs to know about these fields. Other plugins,
...
The Operator-side transcript mirror (below) keeps receiving post-compaction
...
so Operator's `/btw` dispatcher (`src/agents/btw.ts`) falls through to the
...
- `ask_user` uses the same Operator prompt-and-reply path as the Codex
  harness: when the Copilot SDK asks for user input, Operator posts a
...
Permission enforcement for bridged Operator tools happens **inside the tool
...
`createOperatorCodingTools` to every coding tool: loop detection, trusted
...
- `skipPermission: true` — tells the SDK not to fire
  ...
  wrapped `execute()` already performs the richer Operator policy check; an
  SDK-level prompt would either short-circuit Operator's enforcement
  (allow-all) or block every tool call (reject-all) — neither matches PI
  parity.

The in-tree Codex harness uses the same split: bridged Operator tools are
...
`createOperatorCodingTools`: identity (`senderIsOwner`, `memberRoleIds`,
...
the SDK request allows them, and cancels a pending request when the Operator
attempt is aborted.
