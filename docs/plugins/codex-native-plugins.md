---
summary: "Configure native Codex plugins for Codex-mode Operator agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode Operator agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are configuring an existing workspace-directory Codex plugin
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode Operator agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the Operator turn. Plugin calls stay in the native Codex transcript;
Codex app-server owns app-backed MCP execution. Operator does not translate
Codex plugins into synthetic `codex_plugin_*` Operator dynamic tools.
...
`codexPlugins` has no effect on Operator-provider runs, ACP conversation
...
```bash
operator migrate codex --dry-run --verify-plugin-apps
```
...
Operator does not call `plugin/install` or start authentication for a
`workspace-directory` plugin. Install, enable, and authenticate it in Codex
before adding or enabling the Operator policy. Operator keeps apps hidden when
...
Operator reports `marketplace_missing` for each enabled workspace plugin and
...
`enable`/`disable` write only to `~/.operator/operator.json`; they never edit
...
`auth_required`, reauthorize the app in Codex before enabling it in Operator.
For a `workspace-directory` entry, enabling it here changes only Operator
...
| Enabled    | Codex reports the plugin enabled, and Operator config allows it for Codex harness turns.                                           |
...
- During planning, Operator reads source Codex `plugin/read` details and
...
For `workspace-directory` plugins, setup happens outside Operator. Operator
...
disabled, inaccessible, or unauthenticated plugin exposes no apps; Operator
...
Operator reads Codex app inventory through app-server `app/list`, caches it
...
drops it, and Operator rebuilds it from the next `app/list` read.
...
A plugin app is exposed only when Operator can map it back to the configured
...
Operator injects a restrictive `config.apps` patch for the Codex thread:
...
get `open_world_enabled: true`. Operator does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for
  that plugin.
- `false`: Operator returns a deterministic decline.
- `true`: Operator auto-accepts only safe schemas it can map to an approval
  response, such as a boolean approve field.
- `"auto"`: Operator exposes destructive plugin actions to Codex, then
  turns ownership-proven MCP approval elicitations into Operator plugin
  approvals before returning the Codex approval response.
- `"ask"`: Operator uses the same Codex write/destructive gating as
  ...
  admitted app using `"ask"`, Operator selects Codex's human approvals
  reviewer for that app so Codex sends its approval elicitations to
  Operator; other apps and non-app thread approvals keep their configured
  reviewer and policy.
...
| Code                                              | Meaning                                                                                                                              | Fix                                                                                                                    |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| `auth_required`                                   | Migration installed the plugin, but one of its apps still needs authentication. The entry is written disabled until you reauthorize. | Reauthorize the app in Codex, then enable the plugin in Operator.                                                      |
...
| `plugin_detail_unavailable`                       | Operator could not read plugin ownership details.                                                                                    | Inspect the target app-server's `plugin/list` and `plugin/read` responses.                                             |
...
| `app_inventory_missing`, `app_inventory_stale`    | App readiness came from an empty or stale cache.                                                                                     | Operator schedules an async refresh automatically; plugin apps stay excluded until ownership and readiness are known.  |
...
account. Operator can enable an accessible app for the thread even when the
...
`/new` or `/reset`. Operator does not repair or authenticate workspace plugins.
...
`remotePluginId`; Operator keeps owned apps hidden when that selector or the
...
not already active; install, enable, and authenticate it outside Operator.
...
Codex thread bindings keep the app config they started with until Operator
