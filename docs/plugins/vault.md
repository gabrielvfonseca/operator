...
read_when:
  - You want Operator to read API keys from HashiCorp Vault
...
The bundled Vault plugin lets Operator resolve `exec` SecretRefs from
HashiCorp Vault at Gateway startup and reload time. Operator stores Vault
references in config, keeps resolved values in the in-memory secrets snapshot,
and does not write the resolved API keys back to `operator.json`.

Use this when you already run Vault or want model provider keys to live outside
Operator config files. For the SecretRef runtime model, see
...
Enable the bundled plugin before running the `operator vault` commands:

```bash
operator plugins enable vault
```

## Store a provider key in Vault

Operator defaults to KV v2 mounted at `secret`, matching Vault dev-server
examples. For production Vault, set `OPERATOR_VAULT_KV_MOUNT` to your actual KV
mount path before creating SecretRef ids. With the Operator defaults, this
SecretRef id:
...
Use a scoped client token for Operator, not a root token. For the default KV v2
...
that starts Operator. The default auth method reads a Vault client token from
`VAULT_TOKEN`:
...
```bash
export VAULT_ADDR=https://vault.example.com
export OPERATOR_VAULT_AUTH_METHOD=token_file
...
These variables must be present when Operator starts. The Vault plugin forwards
...
other than `auth/kubernetes`. Set `OPERATOR_VAULT_JWT_FILE` only when the service
...
export OPERATOR_VAULT_KV_MOUNT=secret
export OPERATOR_VAULT_KV_VERSION=2
```
...
```bash
operator vault status --provider-alias corp-vault
```

`operator vault status` never prints `VAULT_TOKEN`; it reports only whether the
...
```bash
operator secrets apply --from ./vault-secrets-plan.json --dry-run --allow-exec
operator secrets apply --from ./vault-secrets-plan.json --allow-exec
operator secrets audit --check --allow-exec
operator secrets reload
```

Use `--allow-exec` because the Vault plugin resolves through an Operator-managed
exec SecretRef provider.

If the Gateway is not running yet, start it normally after applying the plan
instead of running `operator secrets reload`.
...
```bash
operator vault setup --openai-id providers/openai/apiKey
operator vault setup --anthropic-id providers/anthropic/apiKey
operator vault setup --openrouter-id providers/openrouter/apiKey
```
...
Bare target paths apply to `operator.json`. Use
`auth-profiles:<agentId>:<path>` for existing `auth-profiles.json` targets.
The target path must be a registered Operator SecretRef target. The setup
command does not create arbitrary named secrets in Operator; Vault remains the
secret store, and Operator stores SecretRefs only on supported config fields.
...
```bash
export OPERATOR_VAULT_KV_VERSION=1
```
...
- one auth source:
  - `VAULT_TOKEN`
  - `OPERATOR_VAULT_AUTH_METHOD=token_file` plus `VAULT_TOKEN_FILE`
  - `OPERATOR_VAULT_AUTH_METHOD=jwt` plus `OPERATOR_VAULT_AUTH_MOUNT`,
    `OPERATOR_VAULT_AUTH_ROLE`, and `OPERATOR_VAULT_JWT_FILE`
  - `OPERATOR_VAULT_AUTH_METHOD=kubernetes` plus `OPERATOR_VAULT_AUTH_ROLE`; optionally
    override `OPERATOR_VAULT_AUTH_MOUNT` or `OPERATOR_VAULT_JWT_FILE`
- optional `VAULT_NAMESPACE`, `OPERATOR_VAULT_KV_MOUNT`, and
  `OPERATOR_VAULT_KV_VERSION`

When using Kubernetes, prefer `OPERATOR_VAULT_AUTH_METHOD=kubernetes`
when Vault has Kubernetes auth configured for the cluster. Use
`OPERATOR_VAULT_AUTH_METHOD=jwt` only when Vault is configured to treat the cluster
...
deployment config. Operator does not require a fixed mount, role, or path: each
Gateway environment can set its own `OPERATOR_VAULT_KV_MOUNT`,
`OPERATOR_VAULT_AUTH_ROLE`, and SecretRef ids. If one shared Gateway must resolve
