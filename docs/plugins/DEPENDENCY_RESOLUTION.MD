---
summary: "How Operator installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged Operator installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

Operator handles plugin dependencies at install/update time only. Runtime
loading never runs a package manager, repairs a dependency tree, or mutates
the Operator package directory.
...
- SDK/core imports are peer or supplied Operator imports.
- Local development plugins bring their own already-installed dependencies.
- npm and git plugins install into Operator-owned package roots.

Operator owns only the plugin lifecycle:
...
Operator uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.operator/npm/projects/<encoded-package>`.
- git packages clone under `~/.operator/git`.
...
```bash
cd ~/.operator/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`operator plugins install npm-pack:<path.tgz>` uses the same per-plugin npm
project root for a local npm-pack tarball: Operator reads the tarball's npm
...
`~/.operator/npm/projects/<encoded-package>` can unblock a temporary
...
`node_modules` beside the plugin package. Operator scans the managed project
...
publishable lockfile during install, and Operator's managed npm project root
supports it through the normal install path. Operator-owned publishable
...
`operator.release.publishToNpm: true`. Third-party plugin packages may also
ship a shrinkwrap; Operator does not require one for community packages, but
...
Operator-owned npm plugin packages can also publish with explicit
...
Local plugins are developer-controlled directories. Operator never runs
...
```bash
operator plugins update <id>
operator plugins install <source>
operator doctor --fix
```

`doctor --fix` cleans legacy Operator-generated dependency state and can
...
Lightweight and core-critical bundled plugins ship as part of Operator. They
...
In source checkouts, Operator treats the repository as a pnpm monorepo.
...
Older Operator versions generated bundled-plugin dependency roots at startup
...
`plugin-runtime-deps` targets, `.operator-runtime-deps*` manifests, generated
...
Older npm installs also used a shared `~/.operator/npm/node_modules` root.
