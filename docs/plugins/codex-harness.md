---
summary: "Run Operator embedded agent turns through the official Codex app-server harness"
title: "Codex harness"
read_when:
  - You want to use the official Codex app-server harness
  - You need Codex harness config examples
  - You want Codex-only deployments to fail instead of falling back to Operator
---

The official `codex` plugin runs embedded OpenAI agent turns through Codex
app-server instead of the built-in Operator harness. Codex owns the
low-level agent session: native thread resume, native tool continuation,
native compaction, and app-server execution. Operator still owns chat
channels, session files, model selection, Operator dynamic tools, approvals,
...
repaired by `operator doctor --fix`.
...
If Codex owns auth before Platform versus ChatGPT routing is known, Operator
...
When no Operator sandbox is active, Operator starts Codex app-server threads
with Codex native code mode enabled (code-mode-only stays off by default), so
native workspace/code capabilities remain available alongside Operator
dynamic tools routed through the app-server `item/tool/call` bridge. An
active Operator sandbox or restricted tool policy disables native code mode
entirely unless you opt into the experimental sandbox exec-server path.

With the default `tools.exec.host: "auto"` and no active Operator sandbox,
...
name or id and keeps Operator's node approval policy in force. If a finite
runtime allowlist disables native Code Mode and leaves the turn without an
execution environment, Operator keeps its policy-filtered `exec` and `process`
tools available instead for direct, unsandboxed execution.

This Codex-native feature is separate from
[Operator code mode](/reference/code-mode), an opt-in QuickJS-WASI runtime
for generic Operator runs with a different `exec` input shape. For the
...
- The official `@gabrielvfonseca/codex` plugin installed. Include `codex` in
...
- Codex app-server `0.143.0` or newer. The plugin manages a compatible
  ...
  startup.
- Codex auth through `operator models auth login --provider openai`, an
...
```bash
operator plugins install @gabrielvfonseca/codex
operator models auth login --provider openai
```
...
The default `appServer.homeScope: "agent"` isolates each Operator agent from
...
that home's native Codex auth, config, plugins, and thread store. Operator does
not inject an Operator auth profile into this app-server.
...
Operator; the fork attaches to the current Operator session and stays
...
`sessionCatalog.enabled: false` to remove it from the Operator sidebar without
...
native configuration; the outer Operator model and fallback chain never replace
...
| Need                                                | Set                                                                                              | Where                              |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ---------------------------------- |
| Enable the harness                                  | `plugins.entries.codex.enabled: true`                                                            | Operator config                    |
| Hide native Codex session discovery                 | `plugins.entries.codex.config.sessionCatalog.enabled: false`                                     | Codex plugin config                |
| Keep an allowlisted plugin install                  | Include `codex` in `plugins.allow`                                                               | Operator config                    |
...
| Sign in with ChatGPT/Codex OAuth                    | `operator models auth login --provider openai`                                                   | CLI auth profile                   |
| Add API-key backup for Codex runs                   | `openai:*` API-key profile listed after subscription auth in `auth.order.openai`                 | CLI auth profile + Operator config |
| Fail closed when Codex is unavailable               | Provider or model `agentRuntime.id: "codex"`                                                     | Operator model/provider config     |
| Use direct OpenAI API traffic                       | Provider or model `agentRuntime.id: "@gabrielvfonseca/operator"` with normal OpenAI auth         | Operator model/provider config     |
...
Operator ignores those local summarizer overrides at runtime, and
`operator doctor --fix` removes them when the agent uses Codex.
...
`agents.defaults.compaction.provider`. `operator doctor --fix` migrates the
...
compaction on the bound thread. Operator does not wait for completion,
impose an Operator timeout, restart the shared app-server, or fall back to a
...
- With runtime unset or `auto`, only an exact official HTTPS Platform Responses
  ...
  implicitly.
- Do not use legacy Codex GPT refs in config; run `operator doctor --fix` to
...
- `agentRuntime.id: "@gabrielvfonseca/operator"` opts a provider or model into the embedded
  Operator runtime when that is intentional.
...
| Direct OpenAI API-key traffic through Operator  | Provider or model `agentRuntime.id: "@gabrielvfonseca/operator"` and normal OpenAI auth                     | `/status` shows Operator runtime        | Use only when Operator is intentional      |
| Legacy config                                   | legacy Codex GPT refs                                                                                       | `operator doctor --fix` rewrites it     | Do not write new config this way           |
...
With Codex forced, Operator fails early if the effective route is not declared
Codex-compatible, the plugin is disabled, the app-server is too old, or the
app-server cannot start.

## App-server policy

By default, the plugin starts Operator's managed Codex binary locally with
...
implicit YOLO posture, Operator selects allowed guardian permissions
instead. When an Operator sandbox is active for the session, Operator
...
Shell access instead goes through Operator sandbox-backed dynamic tools such
as `sandbox_exec` and `sandbox_process` when the normal exec/process tools
are available.

Use normalized Operator exec mode for Codex native auto-review before
...
Operator does not preserve legacy unsafe Codex `approvalPolicy: "never"` or
...
works, but `tools.exec.mode: "auto"` is the normalized Operator surface.
...
supports Operator text commands.
...
- `/codex resume <thread-id>` attaches the current Operator session to an
...
1. Ordered OpenAI auth profiles for the agent, preferably under
   `auth.order.openai`. Run `operator doctor --fix` to migrate older legacy
...
When Operator sees a ChatGPT subscription-style Codex auth profile, it
...
If a subscription profile hits a Codex usage limit, Operator records the
...
When native Codex plugins are configured, Operator installs or refreshes
...
ids, accessibility, and metadata, but Operator owns the per-thread
enablement decision: if policy allows a listed accessible app, Operator
...
installation for unknown ids; Operator only activates marketplace plugins
with `plugin/install` and then refreshes inventory.

### Environment isolation

For local stdio app-server launches, Operator sets `CODEX_HOME` to a
...
`~/.codex` by default. Operator preserves the normal process `HOME`;
...
`appServer.homeScope: "user"`, Operator instead uses the native user Codex
home and its existing account without injecting an Operator auth profile.
...
process. Operator removes `CODEX_HOME` and `HOME` from this list during
...
Codex dynamic tools default to `searchable` loading. Operator normally does
...
Tools marked `catalogMode: "direct-only"`, including the Operator `computer`
tool, use the `operator_direct` namespace instead. Codex treats that namespace
...
Operator's managed `web_search` dynamic tool are mutually exclusive so
managed search cannot bypass native domain restrictions. Operator uses the
managed tool when hosted search is unavailable, explicitly disabled, or
replaced by a selected managed provider. Operator keeps Codex's standalone
...
| -------------------------- | -------------- | ---------------------------------------------------------------------------------------- |
| `codexDynamicToolsLoading` | `"searchable"` | Use `"direct"` to put Operator dynamic tools directly in the initial Codex tool context. |
| `codexDynamicToolsExclude` | `[]`           | Additional Operator dynamic tool names to omit from Codex app-server turns.              |
...
| `homeScope`                                   | `"agent"`                                              | `"agent"` isolates ordinary harness state per Operator agent. `"user"` is an explicit opt-in that shares the native `$CODEX_HOME` or `~/.codex`, uses native auth, and enables owner-only thread management. User scope supports local stdio or Unix transport. For the separate supervision connection, an unset value resolves to `"user"` for stdio or Unix and `"agent"` for WebSocket.     |
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after Operator builds its inherited environment. Operator keeps the selected `CODEX_HOME` and inherited `HOME` for local launches.                                                                                                                                                                           |
| `codeModeOnly`                                | `false`                                                | Opt into Codex's code-mode-only tool surface. Ordinary Operator dynamic tools remain available through nested `tools.*` calls; `operator_direct` tools stay directly model-visible.                                                                                                                                                                                                             |
| `remoteWorkspaceRoot`                         | unset                                                  | Remote Codex app-server workspace root. When set, Operator infers the local workspace root from the resolved Operator workspace, preserves the current cwd suffix under this remote root, and sends only the final app-server cwd to Codex. If the cwd is outside the resolved Operator workspace root, Operator fails closed instead of sending a gateway-local path to the remote app-server. |
| `requestTimeoutMs`                            | `60000`                                                | Timeout for app-server control-plane calls.                                                                                                                                                                                                                                                                                                                                                     |
| `turnCompletionIdleTimeoutMs`                 | `60000`                                                | Quiet window after Codex accepts a turn or after a turn-scoped app-server request while Operator waits for `turn/completed`.                                                                                                                                                                                                                                                                    |
| `postToolRawAssistantCompletionIdleTimeoutMs` | `300000`                                               | Completion-idle and progress guard used after a tool handoff, native tool completion, post-tool raw assistant progress, raw reasoning completion, or reasoning progress while Operator waits for `turn/completed`. Use this for trusted or heavy workloads where post-tool synthesis can legitimately stay quiet longer than the final assistant release budget.                                |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start/resume. Guardian defaults prefer `"workspace-write"` when allowed, otherwise `"read-only"`. When an Operator sandbox is active, `danger-full-access` turns use Codex `workspace-write` with network access derived from the Operator sandbox egress setting.                                                                                     |
| `approvalsReviewer`                           | `"user"` or an allowed guardian reviewer               | Use `"auto_review"` to let Codex review native approval prompts when allowed, otherwise `guardian_subagent` or `user`. `guardian_subagent` remains a legacy alias.                                                                                                                                                                                                                              |
| `serviceTier`                                 | unset                                                  | Optional Codex app-server service tier. `"priority"` enables fast-mode routing, `"flex"` requests flex processing, `null` clears the override, and legacy `"fast"` is accepted as `"priority"`.                                                                                                                                                                                                 |
| `networkProxy`                                | disabled                                               | Opt into Codex permissions-profile networking for app-server commands. Operator defines the selected `permissions.<profile>.network` config and selects it with `default_permissions` instead of sending `sandbox`.                                                                                                                                                                             |
| `experimental.sandboxExecServer`              | `false`                                                | Preview opt-in that registers an Operator sandbox-backed Codex environment with the supported Codex app-server so native Codex execution can run inside the active Operator sandbox.                                                                                                                                                                                                            |

`appServer.networkProxy` is explicit because it changes the Codex sandbox
contract. When enabled, Operator also sets `features.network_proxy.enabled`
and `default_permissions` in the Codex thread config so the generated
permission profile can start Codex managed networking. By default, Operator
generates a collision-resistant `operator-network-<fingerprint>` profile
...
Operator-owned dynamic tool calls are bounded independently from
`appServer.requestTimeoutMs`: Codex `item/tool/call` requests use a 90
second Operator watchdog by default. A positive per-call `timeoutMs`
...
reduced by earlier preparation work. On timeout, Operator aborts the tool
...
After Codex accepts a turn, and after Operator responds to a turn-scoped
...
app-server goes quiet for `appServer.turnCompletionIdleTimeoutMs`, Operator
best-effort interrupts the Codex turn, records a diagnostic timeout, and
releases the Operator session lane so follow-up chat messages are not
...
Tool handoffs use a longer post-tool idle budget: after Operator returns an
...
quiet without `turn/completed`, Operator best-effort interrupts the native
turn and releases the session lane. If another turn watch wins that release
race, Operator still accepts the completed final assistant item once no
...
stuck app-server client and release the Operator session lane; they also
...
- `OPERATOR_CODEX_APP_SERVER_BIN` bypasses the managed binary when
  `appServer.command` is unset.
- `OPERATOR_CODEX_APP_SERVER_ARGS`
- `OPERATOR_CODEX_APP_SERVER_MODE=yolo|guardian`
- `OPERATOR_CODEX_APP_SERVER_APPROVAL_POLICY`
- `OPERATOR_CODEX_APP_SERVER_SANDBOX`

`OPERATOR_CODEX_APP_SERVER_GUARDIAN=1` was removed. Use
`plugins.entries.codex.config.appServer.mode: "guardian"` instead, or
`OPERATOR_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config
...
capabilities in the same Codex thread as the Operator harness turn. Operator
does not translate Codex plugins into synthetic `codex_plugin_*` Operator
dynamic tools.
...
Thread app config is computed when Operator establishes a Codex harness
...
Short version: Operator does not vendor the desktop-control app or execute
...
- Operator dynamic tools are supported. Codex asks Operator to execute
  those tools, so Operator remains in the execution path.
- Codex-native shell, patch, MCP, and native app tools are owned by Codex.
  Operator can observe or block selected native events through the
  supported relay, but it does not rewrite native tool arguments.
- Codex owns native compaction. Operator keeps a transcript mirror for
  channel history, search, `/new`, `/reset`, and future model or harness
  switching, but does not replace Codex compaction with an Operator or
  context-engine summarizer.
- Media generation, media understanding, TTS, approvals, and messaging-tool
  output continue through the matching Operator provider/model settings.
- `tool_result_persist` applies to Operator-owned transcript tool results,
...
**Operator uses the built-in harness instead of Codex:** confirm the effective
...
**Legacy Codex model refs config remains:** run `operator doctor --fix`.
...
`0.143.0-alpha.2` or `0.143.0+custom` are rejected because Operator tests
...
id that Operator no longer has registered. This is a native Codex hook
...
Operator Gateway so old threads are dropped and native hook registrations
are recreated.
...
used for Operator loop detection and its no-policy marker. Required
