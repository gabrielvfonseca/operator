---
summary: "Adds Hugging Face model provider support to Operator."
read_when:
  - You are installing, configuring, or auditing the huggingface plugin
title: "Hugging Face plugin"
---

# Hugging Face plugin

Adds Hugging Face model provider support to Operator.

## Distribution

- Package: `@gabrielvfonseca/huggingface-provider`
- Install route: included in Operator

## Surface

providers: `huggingface`

## Related docs

- [huggingface](/providers/huggingface)
