---
summary: "Adds Microsoft Foundry model provider support to Operator."
read_when:
...
Adds Microsoft Foundry model provider support to Operator.

## Distribution

- Package: `@gabrielvfonseca/microsoft-foundry`
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. Operator refreshes Microsoft Foundry runtime tokens through
...
Operator uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so Operator
...
deployment through onboarding or include model metadata so Operator can verify
