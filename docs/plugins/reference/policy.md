...
- Package: `@gabrielvfonseca/policy`
...
The Policy plugin contributes doctor health checks for policy-managed Operator
...
posture, ingress/channel access posture, data-handling posture, and Operator config secret
provider/auth profile posture.

Policy stores authored requirements in `policy.jsonc`, observes existing
Operator settings and workspace declarations as evidence, and reports drift
through `operator policy check` and `operator doctor --lint`. A clean policy
check emits policy, evidence, findings, and attestation hashes that operators
can record for audit.

`operator policy compare --baseline <file>` compares one policy file to another
