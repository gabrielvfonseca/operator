---
summary: "Bundled `oc-path` plugin: ships the `operator path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`operator path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the Operator repo under
...
- **Local automation**: shell scripts resolve or update one workspace value
  with `operator path … --json` instead of carrying separate markdown, JSONC,
...
operator path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
operator path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
Plugin metadata lives in `extensions/oc-path/operator.plugin.json`:
...
lazily the first time you run `operator path …`, so installs that never use
...
state. Bare `operator path` invocations work immediately on the same host;
the CLI loads the plugin on demand.

Disable with:

```bash
operator plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `operator path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__OPERATOR_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`operator path` CLI reference](/cli/path)
