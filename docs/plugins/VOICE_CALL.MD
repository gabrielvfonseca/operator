...
read_when:
  - You want to place an outbound voice call from Operator
...
Voice calls for Operator via a plugin: outbound notifications, multi-turn
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        operator plugins install @gabrielvfonseca/voice-call
        ```
      </Tab>
...
| `store`                         | unset        | Overrides the default `~/.operator/voice-calls` call-log path.                         |
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    release (`2026.6.0`), so run `operator doctor --fix` to rewrite committed
...
- Voice Call exposes the shared `operator_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal Operator tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `operator_agent_consult`.
...
configured Operator agent without paying a full agent-consult round trip on
...
`operator_agent_consult` still run the full Operator agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call operator_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `operator doctor --fix`; committed config should use `tts.providers.<provider>`.
...
operator voicecall start --to "+15555550123"   # alias for call
operator voicecall continue --call-id <id> --message "Any questions?"
operator voicecall speak --call-id <id> --message "One moment"
operator voicecall dtmf --call-id <id> --digits "ww123456#"
operator voicecall end --call-id <id>
operator voicecall status --call-id <id>
operator voicecall tail
operator voicecall latency                      # summarize turn latency from logs
operator voicecall expose --mode funnel
```
...
Provider signatures are checked against the public URL Operator reconstructs
...
Use `operator logs --follow` for the live phase trace. A healthy Twilio Meet
...
`operator voicecall tail` still shows persisted call records; useful for
...
- `operator logs --follow` shows realtime TwiML served, the realtime bridge started, and the initial greeting queued.
