...
read_when:
  - You are building an Operator plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page covers the **native Operator plugin manifest**, `operator.plugin.json`. For compatible bundle layouts (Codex, Claude, Cursor), see [Plugin bundles](/plugins/bundles).
...
Operator auto-detects those layouts but does not validate them against the `operator.plugin.json` schema below. For a compatible bundle, Operator reads bundle metadata, declared skill roots, Claude command roots, Claude `settings.json` defaults, Claude LSP defaults, and supported hook packs, when the layout matches Operator's runtime expectations.

Every native Operator plugin **must** ship `operator.plugin.json` in the **plugin root**. Operator reads it to validate configuration **without executing plugin code**. A missing or invalid manifest blocks config validation and is treated as a plugin error.
...
`operator.plugin.json` is metadata Operator reads **before loading your plugin code**. Everything in it must be cheap enough to inspect without booting plugin runtime.
...
- QA runner metadata the shared `operator qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`   | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; Operator still reads this during the deprecation window.                                                                                        |
| `providerUsageAuthEnvVars`           | No       | `Record<string, string[]>`   | Usage/billing-only provider credentials. Operator uses these names for usage discovery and secret scrubbing but never for inference auth.                                                                                                                                  |
| `providerAuthAliases`                | No       | `Record<string, string>`     | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                                                                 |
| `channelEnvVars`                     | No       | `Record<string, string[]>`   | Cheap channel env metadata that Operator can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                                                                   |
...
| `qaRunners`                          | No       | `object[]`                   | Cheap QA runner descriptors used by the shared `operator qa` host before plugin runtime loads.                                                                                                                                                                             |
...
The generation provider metadata fields describe static auth signals for providers declared in the matching `contracts.*GenerationProviders` list. Operator reads these fields before provider runtime loads so core tools can decide whether a generation provider is available without importing every provider plugin.
...
`toolMetadata` uses the same `configSignals` and `authSignals` shapes as generation provider metadata, keyed by tool name. `contracts.tools` declares ownership. `toolMetadata` declares cheap availability evidence so Operator can avoid importing a plugin runtime just to have its tool factory return `null`.
...
If a tool has no `toolMetadata`, Operator preserves the existing behavior and loads the owning plugin when the tool contract matches policy. For hot-path tools whose factory depends on auth/config, plugin authors should declare `toolMetadata` instead of making core import runtime to ask.

## providerAuthChoices reference

Each `providerAuthChoices` entry describes one onboarding or auth choice. Operator reads this before provider runtime loads. Provider setup lists use these manifest choices, descriptor-derived setup choices, and install-catalog metadata without loading provider runtime.
...
| `choiceLabel`         | No       | `string`                                                              | User-facing label. If omitted, Operator falls back to `choiceId`.                                         |
...
the exact selected model and returns a config proposal; Operator live-tests that
proposal in isolation and commits it only after success.

## commandAliases reference

Use `commandAliases` when a plugin owns a runtime command name that users may mistakenly put in `plugins.allow` or try to run as a root CLI command. Operator uses this metadata for diagnostics without importing plugin runtime code.
...
the shared `operator qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `operator qa`, for example `matrix`.    |
...
Operator also includes `setup.providers[].envVars` in generic provider auth and env-var lookups. `providerAuthEnvVars` remains supported through a compatibility adapter during the deprecation window, but non-bundled plugins that still use it receive a manifest diagnostic. New plugins should put setup/status env metadata on `setup.providers[].envVars`.
...
Operator can also derive simple setup choices from `setup.providers[].authMethods` when no setup entry is available, or when `setup.requiresRuntime: false` declares setup runtime unnecessary. Explicit `providerAuthChoices` entries stay preferred for custom labels, CLI flags, onboarding scope, and assistant metadata.

Set `requiresRuntime: false` only when those descriptors are sufficient for the setup surface. Operator treats explicit `false` as a descriptor-only contract and will not execute `setup-api` or `operator.setupEntry` for setup lookup. If a descriptor-only plugin still ships one of those setup runtime entries, Operator reports an additive diagnostic and continues ignoring it. Omitted `requiresRuntime` keeps legacy fallback behavior so existing plugins that added descriptors without the flag do not break.
...
Use `contracts` only for static capability ownership metadata that Operator can read without importing the plugin runtime.
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `operator migrate`.                                                                         |
...
| `dangerousFlags`              | No       | `object[]` | Config literals that `operator doctor` should flag as insecure or dangerous when enabled. See below.                                                                                                                                   |
...
`channelConfigs` is plugin manifest metadata, not a new top-level user config section. Users still configure channel instances under `channels.<channel-id>`. Operator reads manifest metadata to decide which plugin owns that configured channel before plugin runtime code executes.
...
Non-bundled plugins that declare `channels[]` should also declare matching `channelConfigs` entries. Without them, Operator can still load the plugin, but cold-path config schema, setup, and Control UI surfaces cannot know the channel-owned option shape until plugin runtime executes.

`channelConfigs.<channel-id>.commands.nativeCommandsAutoEnabled` and `nativeSkillsAutoEnabled` can declare static `auto` defaults for command config checks that run before channel runtime loads. Bundled channels can also publish the same defaults through `package.json#operator.channel.commands` alongside their other package-owned channel catalog metadata.
...
Use `modelSupport` when Operator should infer your provider plugin from shorthand model ids like `gpt-5.6-sol` or `claude-sonnet-4.6` before plugin runtime loads.
...
Operator applies this precedence:
...
Use `modelCatalog` when Operator should know provider model metadata before loading plugin runtime. This is the manifest-owned source for fixed catalog rows, provider aliases, suppression rules, and discovery mode. Runtime refresh still belongs in provider runtime code, but the manifest tells core when runtime is required.
...
| `compat`           | `object`                                                       | Optional compatibility flags matching Operator model config compatibility.  |
...
Do not put runtime-only data in `modelCatalog`. Use `static` only when manifest rows are complete enough for provider-filtered list and picker surfaces to skip registry/runtime discovery. Use `refreshable` when manifest rows are useful listable seeds or supplements but a refresh/cache can add more rows later; refreshable rows are not authoritative by themselves. Use `runtime` when Operator must load provider runtime to know the list.
...
The map key is the integration id. If `providerAlias` is omitted, Operator uses the integration id as the SecretRef provider alias. Provider aliases must match the normal SecretRef provider alias pattern, for example `team-secrets` or `onepassword-work`.

When an operator selects the preset, Operator writes a provider reference like:
...
At startup/reload, Operator resolves that provider by loading current plugin manifest metadata, checking that the owning plugin is installed and active, and materializing the exec command from the manifest. Disabling or removing the plugin revokes the provider for active SecretRefs. Operators who want standalone exec configuration can still write manual `command`/`args` providers directly.

Only `source: "exec"` presets are currently supported. `command` must be `${node}`, and `args[0]` must be a `./` plugin-root-relative resolver script. Operator materializes it at startup/reload to the current Node executable and the absolute in-plugin script path. Node options such as `--require`, `--import`, `--loader`, `--env-file`, `--eval`, and `--print` are not part of the manifest preset contract. Operators who need non-Node commands can configure standalone manual exec providers directly.

Operator derives `trustedDirs` for manifest presets from the plugin root and, for `${node}` presets, the current Node executable directory. Manifest-authored `trustedDirs` are ignored. Other exec provider options such as `timeoutMs`, `noOutputTimeoutMs`, `maxOutputBytes`, `jsonOnly`, `env`, `passEnv`, and `allowInsecurePath` pass through to the normal SecretRef exec provider config.
...
| Field                      | Type               | What it means                                                                                                        |
| -------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the Operator provider id, for example `z-ai` for a `zai` provider. |
...
4. Operator Provider Index preview rows.
...
- if Operator must know it before loading plugin code, put it in `operator.plugin.json`
...
| Field                                                                                      | What it means                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `operator.extensions`                                                                      | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `operator.runtimeExtensions`                                                               | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `operator.setupEntry`                                                                      | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `operator.runtimeSetupEntry`                                                               | Declares the built JavaScript setup entrypoint for installed packages. Requires `setupEntry`, must exist, and must stay inside the plugin package directory.                         |
| `operator.channel`                                                                         | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `operator.channel.commands`                                                                | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `operator.channel.configuredState`                                                         | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `operator.channel.persistedAuthState`                                                      | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `operator.install.clawhubSpec` / `operator.install.npmSpec` / `operator.install.localPath` | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `operator.install.defaultChoice`                                                           | Preferred install path when multiple install sources are available.                                                                                                                  |
| `operator.install.minHostVersion`                                                          | Minimum supported Operator host version, using a semver floor like `>=2026.3.22` or `>=2026.5.1-beta.1`.                                                                             |
| `operator.compat.pluginApi`                                                                | Minimum Operator plugin API range required by this package, using a semver floor like `>=2026.5.27`.                                                                                 |
| `operator.install.expectedIntegrity`                                                       | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `operator.install.allowInvalidConfigRecovery`                                              | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `operator.install.requiredPlatformPackages`                                                | npm package aliases that must materialize when their lockfile platform constraints match the current host.                                                                           |
| `operator.startup.deferConfiguredChannelFullLoadUntilAfterListen`                          | Lets setup-runtime channel surfaces load before listen, then defers the full configured channel plugin until post-listen activation.                                                 |

Manifest metadata decides which provider/channel/setup choices appear in onboarding before runtime loads. `package.json#operator.install` tells onboarding how to fetch or enable that plugin when the user picks one of those choices. Do not move install hints into `operator.plugin.json`.

`operator.install.minHostVersion` is enforced during install and manifest registry loading for non-bundled plugin sources. Invalid values are rejected; newer-but-valid values skip external plugins on older hosts. Bundled source plugins are assumed to be co-versioned with the host checkout.

`operator.install.requiredPlatformPackages` is for npm packages that expose required native binaries through optional, platform-specific aliases. List the bare npm package name for every supported platform alias. During npm install, Operator verifies only the declared alias whose lockfile constraints match the current host. If npm reports success but omits that alias, Operator retries once with a fresh cache and rolls back the install if the alias is still missing.

`operator.compat.pluginApi` is enforced during package install for non-bundled plugin sources. Use it for the Operator plugin SDK/runtime API floor that the package was built against. It can be stricter than `minHostVersion` when a plugin package needs a newer API but still keeps a lower install hint for other flows. Official Operator release sync bumps existing official plugin API floors to the Operator release version by default, but plugin-only releases can keep a lower floor when the package intentionally supports older hosts. Do not use the package version alone as the compatibility contract. `peerDependencies.operator` remains npm package metadata; Operator uses the `operator.compat.pluginApi` contract for install compatibility decisions.
...
Exact npm version pinning already lives in `npmSpec`, for example `"npmSpec": "@wecom/wecom-operator-plugin@1.2.3"`. Official external catalog entries should pair exact specs with `expectedIntegrity` so update flows fail closed if the fetched npm artifact no longer matches the pinned release. Interactive onboarding still offers trusted registry npm specs, including bare package names and dist-tags, for compatibility. Catalog diagnostics can distinguish exact, floating, integrity-pinned, missing-integrity, package-name mismatch, and invalid default-choice sources. They also warn when `expectedIntegrity` is present but there is no valid npm source it can pin. When `expectedIntegrity` is present, install/update flows enforce it; when it is omitted, the registry resolution is recorded without an integrity pin.

Channel plugins should provide `operator.setupEntry` when status, channel list, or SecretRef scans need to identify configured accounts without loading the full runtime. The setup entry should expose channel metadata plus setup-safe config, status, and secrets adapters; keep network clients, gateway listeners, and transport runtimes in the main extension entrypoint.

Runtime entrypoint fields do not override package-boundary checks for source entrypoint fields. For example, `operator.runtimeExtensions` cannot make an escaping `operator.extensions` path loadable.

`operator.install.allowInvalidConfigRecovery` is intentionally narrow. It does not make arbitrary broken configs installable. Today it only allows install flows to recover from specific stale bundled-plugin upgrade failures, such as a missing bundled plugin path or a stale `channels.<id>` entry for that same bundled plugin. Unrelated config errors still block install and send operators to `operator doctor --fix`.

`operator.channel.persistedAuthState` is package metadata for a tiny checker module:
...
`operator.channel.configuredState` follows the same shape for cheap env-only configured checks:
...
Operator discovers plugins from three roots, checked in this order: bundled plugins shipped with Operator, the global install root (`~/.operator/extensions`), and the current workspace root (`<workspace>/.operator/extensions`), plus any explicit `plugins.load.paths` entries.
...
2. **Global install matching a tracked install record** — a plugin installed via `operator plugin install`/`operator plugin update` that Operator's install tracking recognizes for that same id, even when the id also belongs to a bundled plugin
3. **Bundled** — plugins shipped with Operator
...
- To override a bundled plugin, either run `operator plugin install` for that id so the tracked global install outranks the bundled copy, or pin a specific path via `plugins.entries.<id>` so it wins by config-selected precedence.
...
- When extending or forking a bundled plugin with new config keys, update that plugin's `operator.plugin.json` `configSchema` at the same time. Bundled plugin schemas are strict, so adding `plugins.entries.<id>.config.myNewKey` in user config without adding `myNewKey` to `configSchema.properties` will be rejected before the plugin runtime loads.
...
- Unknown `channels.*` keys are **errors**, unless the channel id is declared by a plugin manifest. If the same id also appears in `plugins.allow`, `plugins.entries`, or `plugins.installs` (a plugin that is referenced but not currently discoverable), Operator downgrades this to a **warning** instead.
...
- The manifest is **required for native Operator plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `OperatorPluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.
