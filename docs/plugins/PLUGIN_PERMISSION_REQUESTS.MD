...
| Codex native permission requests | Codex asks before native shell, file, MCP, or app-server actions.        | Codex app-server or native hook approval handling, routed through plugin approvals when Operator owns the prompt. |
| MCP approval elicitations        | A Codex MCP server requests approval for a tool call.                    | MCP approval responses bridged through Operator plugin approvals.                                                 |
...
Operator creates a pending approval with a `plugin:` ID, delivers it to the
...
Operator treats `allow-once` and `allow-always` as approval decisions for the
...
If the hook also returns `params`, Operator applies those parameter changes only
...
- Codex app-server approval requests route through Operator after Codex review.
