...
read_when:
  - You maintain an Operator plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

Operator keeps older plugin contracts wired through named compatibility
...
Operator should not remove a documented plugin contract in the same release
...
- channel `showConfigured` / `showInSetup` metadata while channel packages
  move to `operator.channel.exposure`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `operator plugins registry --refresh`
...
lane when validating the external inspector against Operator plugin
packages. Run it from a clean Operator checkout after the package is built:

```sh
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "pnpm install && pnpm build && npm exec --yes @gabrielvfonseca/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @gabrielvfonseca/plugin-inspector@0.1.0 -- ./extensions/discord --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @gabrielvfonseca/plugin-inspector@0.1.0 -- <clawhub-plugin-dir> --json"
```
