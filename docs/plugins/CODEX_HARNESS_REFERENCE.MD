...
resumes omit Operator model and provider overrides so Codex restores the
...
Operator binding owns the exact target or one of its non-archived spawned
descendants. Operator follows Codex's descendant pagination and fails closed on
...
`operator doctor --fix` to migrate the old entry, endpoint definitions, policy
...
For ordinary harness turns, Operator starts the managed Codex binary shipped
...
enables native Computer Use. If no desktop app bundle is installed, Operator
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after Operator builds its inherited environment.                                                                                                                                                                                                                                                             |
...
| `loopDetectionPreToolUseRelay`                | `true`                                                 | Install the Codex `PreToolUse` subprocess used only for Operator loop detection and its explicit no-policy marker. Set `false` to reduce per-tool process fan-out. Before-tool plugin hooks and trusted-tool policy still install their required relay.                                                                                                                                         |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active Operator sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows Operator sandbox egress.                                                                                                                                                                                       |
...
contract. When enabled, Operator also sets `features.network_proxy.enabled` and
`default_permissions` in the Codex thread config so the generated permission
profile can start Codex-managed networking. Operator generates a
collision-resistant `operator-network-<fingerprint>` profile name from the
...
Operator treats non-loopback WebSocket app-server URLs as remote and requires
...
shorthand before Operator builds app-server start options, and unresolved
structured SecretRefs fail before any token or header is sent. When native
Codex plugins are configured, Operator uses the connected app-server's plugin
...
still the authoritative inventory and metadata source, but Operator policy
...
plugins via `plugin/install` and refreshes inventory. Only connect Operator to
remote app-servers that are trusted to accept Operator-managed plugin installs
...
unattended Operator turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, Operator treats the implicit default as guardian
...
When an Operator sandbox is active, the local Codex app-server process still
runs on the Gateway host. Operator therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the Operator sandbox
backend. Shell access is exposed through Operator sandbox-backed dynamic tools
such as `sandbox_exec` and `sandbox_process` when the normal exec/process tools
are available.

<Note>
On Docker-backed Operator sandbox hosts (`agents.defaults.sandbox.mode` set to
a Docker backend), `operator doctor` probes whether the host allows the
...
Ubuntu/AppArmor hosts. Fix the reported host namespace policy for the Operator
...
The stable default is fail-closed: active Operator sandboxing disables native
...
to try Codex's remote environment support with Operator's sandbox backend.
...
When the flag is on and the current Operator session is sandboxed, Operator
...
Operator-owned environment. If the app-server cannot register the environment,
...
the loopback exec-server unless it is running on the same host, so Operator
...
1. An explicit Operator Codex auth profile for the agent.
2. The app-server's existing account in that agent's Codex home.
3. For local stdio app-server launches only, `CODEX_API_KEY`, then
   `OPENAI_API_KEY`, when no app-server account is present and OpenAI auth is
   still required.

When Operator sees a ChatGPT subscription-style Codex auth profile (OAuth or
...
Stdio app-server launches inherit Operator's process environment by default.
Operator owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's Operator state. That keeps Codex
config, accounts, plugin cache/data, and thread state scoped to the Operator
...
Operator skips its auth-profile bridge for the app-server. Verified owner
...
continuing it in Operator; independent Codex processes do not coordinate
...
unrelated thread not owned by another Operator Chat require
...
In the default agent scope, Operator plugins and Operator skill snapshots
still flow through Operator's own plugin registry and skill loader; personal
Codex `~/.codex` assets do not. If you have useful Codex CLI skills or
plugins from a Codex home that should become part of an isolated Operator
agent, inventory them explicitly:

```bash
operator migrate codex --dry-run
operator migrate apply codex --yes
```
...
Operator removes `CODEX_HOME` and `HOME` from this list during local launch
...
When a finite runtime allowlist disables native Code Mode, Operator sends an
empty execution-environment selection. In that direct, unsandboxed case,
Operator keeps its policy-filtered `exec` and `process` tools as the shell
fallback. Runtime allowlists and `codexDynamicToolsExclude` still apply.

Most remaining Operator integration tools, such as messaging, media, cron,
...
`sessions_spawn` remains available for explicit Operator or ACP delegation.
...
tool, are grouped under `operator_direct`. Operator adds that namespace to
...
Dynamic tool budgets are capped at 600000 ms. On timeout, Operator aborts the
...
releases the Operator session lane so follow-up chat messages are not queued
...
post-tool idle budget: after Operator returns an `item/tool/call` response,
...
Operator best-effort interrupts the native turn and releases the session
...
Operator upgrades the bundled `@openai/codex` version or when a deployment
...
If discovery fails or times out, Operator uses a bundled fallback catalog:
...
The app-server catalog can report `ultra`; Operator reasoning controls currently
...
Operator does not write synthetic Codex project-doc files or depend on Codex
fallback filenames for persona files, because Codex fallbacks only apply when
`AGENTS.md` is missing.

For Operator workspace parity, the Codex harness forwards the other
...
- `SOUL.md`, `IDENTITY.md`, and `USER.md` are forwarded as **turn-scoped**
  ...
  user profile.
- The compact loaded Operator skills list is also forwarded as turn-scoped
...
- `BOOTSTRAP.md`, when present, is forwarded as Operator turn input reference
  context.
...
`OPERATOR_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config is
