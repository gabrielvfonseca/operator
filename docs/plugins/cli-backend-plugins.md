...
read_when:
  ...
  - You need to map a third-party CLI into Operator's text fallback runner
---

CLI backend plugins let Operator call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points Operator at the plugin runtime module              |
| Manifest ownership   | `operator.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ```json package.json
    {
      ...
      "dependencies": {
        "@gabrielvfonseca/operator": "^2026.3.24"
      },
      "devDependencies": {
        "typescript": "^5.9.0"
      }
    ...
    entry is `./src/index.ts`, add `operator.runtimeExtensions` pointing at the
  ...
  <Step title="Declare backend ownership">
    ```json operator.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through Operator",
    ...
    `cliBackends` is the runtime ownership list; it lets Operator auto-load the
...
`CliBackendConfig` describes how Operator should launch and parse the CLI:
...
| `modelAliases`                                            | Map Operator model ids to CLI-native ids                                          |
| `sessionArg` / `sessionArgs`                              | How to pass a session id                                                          |
| `sessionMode`                                             | `always`, `existing`, or `none`                                                   |
| `sessionIdFields`                                         | JSON fields Operator reads from CLI output                                        |
...
| `defaultAuthProfileId`             | Prefer a specific Operator auth profile                                     |
...
| `bundleMcp` / `bundleMcpMode`      | Opt into Operator's loopback MCP tool bridge                                |
| `ownsNativeCompaction`             | Backend owns its own compaction - Operator defers                           |
...
command. Operator hashes the bounded complete installed package tree, including
...
`sideQuestionToolMode: "disabled"`; otherwise Operator fails closed when BTW
...
Operator calls it once with the final fresh or resume argv and fails closed when
...
### `ownsNativeCompaction`: opting out of Operator compaction

If your backend runs an agent that compacts its **own** transcript, set
`ownsNativeCompaction: true` so Operator's safeguard summarizer never runs
...
over-budget session can stay over budget or go stale (Operator no longer
rescues it):
...
CLI backends do not receive Operator tools by default. If the CLI can consume
...
"always-on"` so Operator can fail closed when a caller requires no native
...
```bash
operator plugins inspect acme-cli --runtime --json
operator agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `operator.extensions` and built runtime entries for published packages</Check>
<Check>`operator.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
