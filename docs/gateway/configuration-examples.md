---
summary: "Schema-accurate configuration examples for common Operator setups"
read_when:
  - Learning how to configure Operator
...
Save to `~/.operator/operator.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.operator/agents/main/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.operator/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
