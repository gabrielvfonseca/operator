...
Operator's Gateway exposes an HTTP endpoint for invoking a single tool directly. It is always enabled and uses Gateway auth plus tool policy. Like the OpenAI-compatible `/v1/*` surface, shared-secret bearer auth is treated as trusted operator access for the whole gateway.
...
- `mode="token"` uses `gateway.auth.token` (or `OPERATOR_GATEWAY_TOKEN`).
- `mode="password"` uses `gateway.auth.password` (or `OPERATOR_GATEWAY_PASSWORD`).
...
- Internal same-host callers that bypass the proxy can use `gateway.auth.password` / `OPERATOR_GATEWAY_PASSWORD` as a local direct fallback. Any `Forwarded`, `X-Forwarded-*`, or `X-Real-IP` header evidence keeps the request on the trusted-proxy path instead.
...
- For shared-secret auth modes (`token` and `password`), the endpoint restores the normal full operator defaults even if the caller sends a narrower `x-operator-scopes` header.
- Shared-secret auth also treats direct tool invokes on this endpoint as owner-sender turns.
- Trusted identity-bearing HTTP modes (trusted proxy auth, or `gateway.auth.mode="none"` on a private ingress) honor `x-operator-scopes` when present and otherwise fall back to the normal operator default scope set.
...
| Auth mode                                                                               | Behavior                                                                                                                                                                                                                                                                                                               |
| --------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `token` or `password` + `Authorization: Bearer ...`                                     | Proves possession of the shared gateway operator secret. Ignores narrower `x-operator-scopes`. Restores the full default operator scope set: `operator.admin`, `operator.approvals`, `operator.pairing`, `operator.read`, `operator.talk.secrets`, `operator.write`. Treats direct tool invokes as owner-sender turns. |
| Trusted identity-bearing HTTP (trusted proxy auth, or `mode="none"` on private ingress) | Authenticates an outer trusted identity or deployment boundary. Honors `x-operator-scopes` when present. Falls back to the normal operator default scope set when the header is absent. Only loses owner semantics when the caller explicitly narrows scopes and omits `operator.admin`.                               |
...
- `x-operator-message-channel: <channel>` (example: `slack`, `telegram`)
- `x-operator-account-id: <accountId>` (when multiple accounts exist)
- `x-operator-message-to: <target>` (delivery target for message-tool policy)
- `x-operator-thread-id: <threadId>` (thread context for message-tool policy)
