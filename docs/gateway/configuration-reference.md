---
summary: "Gateway config reference for core Operator keys, defaults, and links to dedicated subsystem references"
...
Field-level reference for `~/.operator/operator.json`: keys, defaults, and links to deeper subsystem pages. For task-oriented setup guidance, see [Configuration](/gateway/configuration). Channel- and plugin-owned command catalogs and deep memory/QMD knobs live on their own pages, not here.

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional; Operator uses safe defaults when omitted.

Code truth beats this page:

- `operator config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in.
...
- `talk.*` (Talk mode)
  - `talk.consultThinkingLevel`: thinking level override for the full Operator agent run behind Control UI Talk realtime consults
  ...
  - `talk.realtime.consultRouting`: Gateway relay fallback for finalized realtime Talk transcripts that skip `operator_agent_consult`
...
- `models.providers.*.localService`: optional on-demand process manager for
  local model servers. Operator probes the configured health endpoint, starts
...
Operator-managed MCP server definitions live under `mcp.servers` and are
consumed by embedded Operator and other runtime adapters. The `operator mcp list`,
...
- `mcp.servers`: named stdio or remote MCP server definitions for runtimes that
  ...
  `type: "http"` is a CLI-native alias that `operator mcp set` and
  `operator doctor --fix` normalize into the canonical `transport` field.
- `mcp.servers.<name>.enabled`: set `false` to keep a saved server definition
  while excluding it from embedded Operator MCP discovery and tool projection.
...
- `mcp.servers.<name>.auth`: set `"oauth"` for HTTP MCP servers that require
  OAuth. Run `operator mcp login <name>` to store tokens under Operator state.
...
- `mcp.servers.<name>.codex`: optional Codex app-server projection controls.
  This block is Operator metadata for Codex app-server threads only; it does not
  affect ACP sessions, generic Codex harness config, or other runtime adapters.
  Non-empty `codex.agents` limits the server to the listed Operator agent ids.
  ...
  `default_tools_approval_mode` for that server. Operator strips the `codex`
...
See [MCP](/cli/mcp#operator-as-an-mcp-client-registry) and
...
- `install.nodeManager`: node installer preference for `metadata.operator.install`
...
- Loaded from package or bundle directories under `~/.operator/extensions` and `<workspace>/.operator/extensions`, plus files or directories listed in `plugins.load.paths`.
...
- Discovery accepts native Operator plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Operator plugin schema when available).
- Channel plugin account/runtime settings live under `channels.<id>` and should be described by the owning plugin's manifest `channelConfigs` metadata, not by a central Operator option registry.
...
It does not enable Codex plugins for Operator provider runs, ACP
...
- `plugins.entries.codex.config.codexPlugins.allow_destructive_actions`:
  ...
  to decline them, `"auto"` to route Codex-required approvals through Operator
...
be installed and enabled, and their owned apps must be accessible; Operator
...
- Enabled Claude bundle plugins can also contribute embedded Operator defaults from `settings.json`; Operator applies those as sanitized agent settings, not as raw Operator config patches.
...
`commitments` controls inferred follow-up memory: Operator can detect check-ins from conversation turns and deliver them through heartbeat runs.
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want Operator to discover `/json/version`; use WS(S)
...
- If an externally managed CDP service is reachable through loopback, set that
  profile's `attachOnly: true`; otherwise Operator treats the loopback port as a
...
- `existing-session` profiles can set `cdpUrl` when Chrome is already running
  ...
  mode Operator passes the endpoint to Chrome MCP instead of using auto-connect;
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or OPERATOR_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `OPERATOR_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `tailscale.preserveFunnel`: when `true` and `tailscale.mode = "serve"`, Operator
...
- `terminal.shell`: optional shell executable. When unset, Operator uses `$SHELL` on Unix and `%ComSpec%` on Windows.
...
- `gateway.push.apns.relay.baseUrl`: base HTTPS URL for the external APNs relay used after relay-backed iOS builds publish registrations to the gateway. Public App Store builds use the hosted Operator relay. Custom relay URLs must match a deliberately separate iOS build/deployment path whose relay URL points at that relay.
- `gateway.push.apns.relay.timeoutMs`: gateway-to-relay send timeout in milliseconds. Defaults to `10000`.
- Relay-backed registrations are delegated to a specific gateway identity. The paired iOS app fetches `gateway.identity.get`, includes that identity in the relay registration, and forwards a registration-scoped send grant to the gateway. Another gateway cannot reuse that stored registration.
- `OPERATOR_APNS_RELAY_BASE_URL` / `OPERATOR_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `OPERATOR_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
- `gateway.handshakeTimeoutMs`: pre-auth Gateway WebSocket handshake timeout in milliseconds. Default: `15000`. `OPERATOR_HANDSHAKE_TIMEOUT_MS` takes precedence when set. Increase this on loaded or low-powered hosts where local clients can connect while startup warmup is still settling.
...
- `gateway.nodes.pairing.sshVerify`: SSH-verified auto-approval for first-time node device pairing (default: enabled). The gateway SSHes back to the pairing host (BatchMode, strict host keys) and approves only on an exact `operator node identity` device-key match. Same eligibility floor as `autoApproveCidrs`; probes are limited to private/CGNAT source addresses unless `cidrs` overrides them. Set `false` to disable, or `{ user, identity, timeoutMs, cidrs }` to tune. See [Node pairing](/gateway/pairing#ssh-verified-device-auto-approval-default).
...
```bash
OPERATOR_CONFIG_PATH=~/.operator/a.json \
OPERATOR_STATE_DIR=~/.operator-a \
operator gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.operator-dev` + port `19001`), `--profile <name>` (uses `~/.operator-<name>`).
...
Cloud workers are opt-in. If `cloudWorkers` is absent, or `profiles` is empty, Operator accepts no new worker creation. Durable records created earlier still reconcile and remain visible; the existing gateway/node projection is unchanged.
...
The bundled `crabbox` provider provisions an SSH-capable lease through the local Crabbox CLI. The inner `settings.provider` selects the Crabbox backend; it is separate from the outer Operator provider id.
...
- `settings.ttl` and `settings.idleTimeout` (required): positive Go duration strings passed to `--ttl` and `--idle-timeout`. These provider-side failsafes are distinct from Operator's stored `lifetime` policy below.
- `settings.binary`: optional absolute Crabbox executable path. Without it, Operator checks the sibling Crabbox checkout, then executable entries on `PATH`, and finally invokes `crabbox` so a missing CLI remains a visible provider error.

Unknown settings are rejected. Crabbox credentials and backend-specific account configuration remain owned by Crabbox; do not place them in `settings`. Operator invokes only the local CLI and makes no provider network calls from this plugin. Provisioning always passes `--keep=true`; Operator owns the external lifecycle and destroys the lease with `crabbox stop`.

<Note>
  Operator resolves Crabbox's lease-local `sshKey` path through the provider-owned secret resolver and pins the authoritative `sshHostKey` returned by `crabbox inspect --json`. AWS admission also requires `providerMetadata.instanceProfileAttached`. Install Crabbox 0.38.1 or newer for this closed inspection contract.
</Note>
...
This foundation installs and verifies the gateway build and provides tunnel start/stop lifecycle, but it does not launch the general Operator CLI. The self-contained worker entry and loop land in the next cloud-worker milestone.
...
<Warning>
  ...
  Its operator must supply the expected `hostKey`; Operator will not learn or accept a key from the first connection.
  Destroying its lease only releases Operator's logical record; it does not stop or clean the host.
</Warning>
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.operator/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-operator-token: <token>`.
Query-string hook tokens are rejected.

Validation and safety notes:

- `hooks.enabled=true` requires a non-empty `hooks.token`.
- `hooks.token` should be distinct from active Gateway shared-secret auth (`gateway.auth.token` / `OPERATOR_GATEWAY_TOKEN` or `gateway.auth.password` / `OPERATOR_GATEWAY_PASSWORD`); startup logs a non-fatal security warning when it detects reuse.
- `operator security audit` flags hook/Gateway auth reuse as a critical finding, including Gateway password auth supplied only at audit time (`--auth password --password <password>`). Run `operator doctor --fix` to rotate a persisted reused `hooks.token`, then update external hook senders to use the new hook token.
...
- `transform` can point to a JS/TS module returning a hook action.
  ...
  - Keep `hooks.transformsDir` under `~/.operator/hooks/transforms`; workspace skill directories are rejected. If `operator doctor` reports this path as invalid, move the transform module into the hooks transforms directory or remove `hooks.transformsDir`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `OPERATOR_SKIP_GMAIL_WATCHER=1` to disable.
  ...
  plugins: {
    entries: {
      canvas: {
        config: {
          host: {
            root: "~/.operator/workspace/canvas",
            liveReload: true,
            // enabled: false, // or OPERATOR_SKIP_CANVAS_HOST=1
          },
        },
      },
    },
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__operator__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__operator__/a2ui/`
...
- Also serves A2UI at `/__operator__/a2ui/`.
...
Writes a unicast DNS-SD zone under `~/.operator/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `operator dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.operator/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${OPERATOR_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `operator.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.operator/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/operator-vault-resolver",
...
- Legacy flat `auth-profiles.json` maps such as `{ "provider": { "apiKey": "..." } }` are not a runtime format; `operator doctor --fix` rewrites them to canonical `provider:default` API-key profiles with a `.legacy-flat.*.bak` backup.
...
- Legacy OAuth imports from `~/.operator/credentials/oauth.json`.
...
[`operator audit`](/cli/audit) or the
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.operator/logs/cache-trace.jsonl",
...
- `stuckSessionAbortMs`: no-progress age threshold in ms before eligible stalled active work may be abort-drained for recovery. When unset, Operator uses the safer extended embedded-run window of at least 5 minutes and 3x `stuckSessionWarnMs`.
...
- `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`: environment toggle for latest experimental GenAI inference span shape, including `{gen_ai.operation.name} {gen_ai.request.model}` span names, `CLIENT` span kind, and `gen_ai.provider.name` instead of legacy `gen_ai.system`. By default spans keep `operator.model.call` and `gen_ai.system` for compatibility; GenAI metrics use bounded semantic attributes.
- `OPERATOR_OTEL_PRELOADED=1`: environment toggle for hosts that already registered a global OpenTelemetry SDK. Operator then skips plugin-owned SDK startup/shutdown while keeping diagnostic listeners active.
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$OPERATOR_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Operator.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `OPERATOR_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `operator doctor --fix` can strip unknown keys).
...
- `webhook`: deprecated legacy fallback webhook URL (http/https) used by `operator doctor --fix` to migrate stored jobs that still have `notify: true`; runtime delivery uses per-job `delivery.mode="webhook"` plus `delivery.to`, or `delivery.completionDestination` when preserving announce delivery.
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `operator.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary. Set `OPERATOR_INCLUDE_ROOTS` (absolute paths) to allow additional roots outside the config directory.
...
- Operator-owned writes that change only one top-level section backed by a single-file include write through to that included file. For example, `plugins install` updates `plugins: { $include: "./plugins.json5" }` in `plugins.json5` and leaves `operator.json` intact.
- Root includes, include arrays, and includes with sibling overrides are read-only for Operator-owned writes; those writes fail closed instead of flattening the config.
