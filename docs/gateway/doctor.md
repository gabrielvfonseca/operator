...
`operator doctor` is the repair and migration tool for Operator. It fixes stale config/state, checks health, and provides actionable repair steps.
...
<Tabs>
  ...
  <Tab title="--lint">
    ```bash
    operator doctor --lint
    operator doctor --lint --json
    ```
...
```bash
cat ~/.operator/operator.json
```

## Read-only lint mode

`operator doctor --lint` is the automation-friendly sibling of
`operator doctor --fix`. They share the same Doctor rule registry, but they do
not select or act on rules in the same way:

| Mode                     | Prompts   | Writes config/state     | Output                 | Use it for                      |
| ------------------------ | --------- | ----------------------- | ---------------------- | ------------------------------- |
| `operator doctor`        | yes       | no                      | friendly health report | a human checking status         |
| `operator doctor --fix`  | sometimes | yes, with repair policy | friendly repair log    | applying approved repairs       |
| `operator doctor --lint` | no        | no                      | structured findings    | CI, preflight, and review gates |
...
```bash
operator doctor --lint
operator doctor --lint --severity-min warning
operator doctor --lint --json
operator doctor --lint --all
operator doctor --lint --only core/doctor/gateway-config --json
```
...
- `--json`, `--severity-min`, `--all`, `--only`, and `--skip` require `--lint`; plain `operator doctor` and `--fix` runs reject them.
...
<AccordionGroup>
  <Accordion title="Health, UI, and updates">
    ...
    - Problem-only skill and plugin notes; healthy inventory stays in `operator skills check` and `operator plugins list`.
  ...
  <Accordion title="Gateway, services, and supervisors">
    ...
    - Channel-specific permission checks live under `operator channels capabilities`; for example, Discord voice channel permissions are audited with `operator channels capabilities --channel discord --target channel:<channel-id>`.
...
The Control UI Dreams scene includes **Backfill**, **Reset**, and **Clear Grounded** actions for the grounded dreaming workflow. These use gateway doctor-style RPC methods but are **not** part of `operator doctor` CLI repair/migration.
...
```bash
operator memory rem-backfill --path ./memory --stage-short-term
```
...
<AccordionGroup>
  ...
  <Accordion title="2. Legacy config key migrations">
    When the config contains a deprecated key with an active migration, other commands refuse to run and ask you to run `operator doctor`. Doctor explains which legacy keys were found, shows the migration it applied, and rewrites `~/.operator/operator.json` with the updated schema. Gateway startup refuses legacy config formats and asks you to run `operator doctor --fix`; it does not rewrite `operator.json` on startup. Cron job store migrations are also handled by `operator doctor --fix`.
    ...
    | `agents.defaults/list[].systemPromptOverride`                                                    | removed (Operator owns the generated system prompt)                        |
  ...
  <Accordion title="2g. Session route cleanup">
    ...
    `operator doctor --fix` can clear auto-created stale state such as `modelOverrideSource: "auto"` model pins, runtime model metadata, pinned harness ids, CLI session bindings, and auto auth-profile overrides when their owning route is no longer configured. Explicit user or legacy session model choices are reported for manual review and left untouched; switch them with `/model ...`, `/new`, or reset the session when that route is no longer intended.

  </Accordion>
  <Accordion title="3. Legacy state migrations (disk layout)">
    Doctor can migrate older on-disk layouts into the current structure:

    - Sessions store + transcripts: from `~/.operator/sessions/` to `~/.operator/agents/<agentId>/sessions/`
    - Agent dir: from `~/.operator/agent/` to `~/.operator/agents/<agentId>/agent/`
    - WhatsApp auth state (Baileys): from legacy `~/.operator/credentials/*.json` (except `oauth.json`) to `~/.operator/credentials/whatsapp/<accountId>/...` (default account id: `default`)

    These migrations are best-effort and idempotent; doctor emits warnings when it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates the legacy sessions + agent dir on startup so history/auth/models land in the per-agent path without a manual doctor run. WhatsApp auth is intentionally only migrated via `operator doctor`. Talk provider/provider-map normalization compares by structural equality, so key-order-only diffs no longer trigger repeat no-op `doctor --fix` changes.
  ...
  <Accordion title="3b. Legacy cron store migrations">
    Doctor also checks the cron job store (`~/.operator/cron/jobs.json` by default, or `cron.store` when overridden) for old job shapes that the scheduler still accepts for compatibility.
    ...
    On Linux, doctor also warns when the user's crontab still invokes legacy `~/.operator/bin/ensure-whatsapp.sh`. That host-local script is not maintained by current Operator and can write false `Gateway inactive` messages to `~/.operator/logs/whatsapp-health.log` when cron cannot reach the systemd user bus. Remove the stale crontab entry with `crontab -e`; use `operator channels status --probe`, `operator doctor`, and `operator gateway status` for current health checks.

  </Accordion>
  <Accordion title="3c. Session lock cleanup">
    Doctor scans every agent session directory for stale write-lock files left behind when a session exited abnormally. For each lock file found it reports: the path, PID, whether the PID is still alive, lock age, and whether it is considered stale (dead PID, malformed owner metadata, older than 30 minutes, or a live PID proven to belong to a non-Operator process). In `--fix` / `--repair` mode it removes locks with dead, orphaned, recycled, malformed-old, or non-Operator owners automatically. Old locks still owned by a live Operator process are reported but left in place so doctor does not cut off an active transcript writer.
  </Accordion>
  <Accordion title="3d. Session transcript branch repair">
    Doctor scans agent session JSONL files for the duplicated branch shape created by the 2026.4.24 prompt transcript rewrite bug: an abandoned user turn with Operator internal runtime context plus an active sibling containing the same visible user prompt. In `--fix` / `--repair` mode, doctor backs up each affected file next to the original and rewrites the transcript to the active branch so gateway history and memory readers no longer see duplicate turns.
  </Accordion>
  <Accordion title="4. State integrity checks (session persistence, routing, and safety)">
    ...
    - **Multiple state dirs**: warns when multiple `~/.operator` folders exist across home directories, or when `OPERATOR_STATE_DIR` points elsewhere (history can split between installs).
    - **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run it on the remote host (the state lives there).
    - **Config file permissions**: warns if `~/.operator/operator.json` is group/world readable and offers to tighten to `600`.
  ...
  <Accordion title="5. Model auth health (OAuth expiry)">
    ...
    When an OAuth refresh fails permanently (for example `refresh_token_reused`, `invalid_grant`, or a provider telling you to sign in again), doctor reports that re-auth is required and prints the exact `operator models auth login --provider ...` command to run.
    ...
    Legacy Codex OAuth profiles whose tokens live in macOS Keychain (older onboarding before the file-based sidecar layout) are repaired only by doctor. Run `operator doctor --fix` once from an interactive terminal to migrate Keychain-backed legacy tokens inline into `auth-profiles.json`; after that, embedded turns (Telegram, cron, sub-agent dispatch) resolve them as canonical OpenAI OAuth profiles.
  ...
  <Accordion title="7b. Plugin install cleanup">
    ...
    Containerized gateway startup has a narrow upgrade exception: when `operator gateway run` starts on a new Operator version, it runs safe state migrations and the existing post-core plugin convergence before readiness, then records a per-version checkpoint. This startup pass can clean stale bundled-plugin records, repair local plugin links, reinstall configured plugin packages when the convergence path requires it, and check active plugin payloads. If startup cannot repair safely, run the same image once with `operator doctor --fix` against the same mounted state/config before restarting the container normally.

  </Accordion>
  <Accordion title="8. Gateway service migrations and cleanup hints">
    Doctor detects legacy gateway services (launchd/systemd/schtasks) and offers to remove them and install the Operator service using the current gateway port. It can also scan for extra gateway-like services and print cleanup hints. Profile-named Operator gateway services are considered first-class and are not flagged as "extra."

    On Linux, if the user-level gateway service is missing but a system-level Operator gateway service exists, doctor does not install a second user-level service automatically. Inspect with `operator gateway status --deep` or `operator doctor --deep`, then remove the duplicate or set `OPERATOR_SERVICE_REPAIR_POLICY=external` when a system supervisor owns the gateway lifecycle.

  </Accordion>
  <Accordion title="8b. Startup Matrix migration">
    When a Matrix channel account has a pending or actionable legacy state migration, doctor (in `--fix` / `--repair` mode) creates a pre-migration snapshot and then runs the best-effort migration steps: legacy Matrix state migration and legacy encrypted-state preparation. Both steps are non-fatal; errors are logged and startup continues. In read-only mode (`operator doctor` without `--fix`) this check is skipped entirely.
  </Accordion>
  <Accordion title="8c. Device pairing and auth drift">
    ...
    - inspect pending requests with `operator devices list`
    - approve the exact request with `operator devices approve <requestId>`
    - rotate a fresh token with `operator devices rotate --device <deviceId> --role <role>`
    - remove and re-approve a stale record with `operator devices remove <deviceId>`
  ...
  <Accordion title="9. Security warnings">
    Doctor emits a Security note only when it finds a warning, such as a provider open to DMs without an allowlist or a dangerously configured policy. Use `operator security audit` for the full security inventory.
  </Accordion>
  ...
  <Accordion title="11. Workspace status (skills, plugins, and TaskFlows)">
    ...
    - **Skills**: lists allowed but unusable skill names; use `operator skills check` for requirement details and full counts.
    - **Plugins**: reports only errored plugin IDs; use `operator plugins list` for loaded, imported, disabled, and bundle-plugin inventory.
  ...
  <Accordion title="11c. Shell completion">
    ...
    - If the shell profile uses a slow dynamic completion pattern (`source <(operator completion ...)`), doctor upgrades it to the faster cached file variant.
  ...
  <Accordion title="12. Gateway auth checks (local token)">
    ...
    - `operator doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
  ...
  <Accordion title="12b. Read-only SecretRef-aware repairs">
    ...
    - `operator doctor --fix` uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
  ...
  <Accordion title="13b. Memory search readiness">
    ...
    Use `operator memory status --deep` to verify embedding readiness at runtime.
  ...
  <Accordion title="15. Supervisor config audit + repair">
    ...
    - `operator doctor` prompts before rewriting supervisor config.
    - `operator doctor --yes` accepts the default repair prompts.
    - `operator doctor --fix` applies recommended fixes without prompts (`--repair` is an alias).
    - `operator doctor --fix --force` overwrites custom supervisor configs.
    - `OPERATOR_SERVICE_REPAIR_POLICY=external` keeps doctor read-only for gateway service lifecycle. It still reports service health and runs non-service repairs, but skips service install/start/restart/bootstrap, supervisor config rewrites, and legacy service cleanup because an external supervisor owns that lifecycle.
    ...
    - Doctor service repairs refuse to rewrite, stop, or restart a gateway service from an older Operator binary when the config was last written by a newer version. See [Gateway troubleshooting](/gateway/troubleshooting#split-brain-installs-and-newer-config-guard).
    - You can always force a full rewrite via `operator gateway install --force`.
