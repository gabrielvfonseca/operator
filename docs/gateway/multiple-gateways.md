---
summary: "Run multiple Operator Gateways on one host (isolation, ports, and profiles)"
read_when:
...
operator --profile rescue onboard
operator --profile rescue gateway install --port 19789
```
...
During `operator --profile rescue onboard`:
...
- Profile/config file
- State directory
- Workspace (default: `~/.operator/workspace-rescue`)
...
# extra gateway
operator --profile ops setup
operator --profile ops gateway --port 19789
```

Named profiles on both sides also work:

```bash
operator --profile main setup
operator --profile main gateway --port 18789
...
operator --profile ops gateway install --port 19789
```
...
| Setting                      | Purpose                              |
| ---------------------------- | ------------------------------------ |
| `OPERATOR_CONFIG_PATH`       | Per-instance config file             |
| `OPERATOR_STATE_DIR`         | Per-instance sessions, creds, caches |
...
`OPERATOR_ALLOW_MULTI_GATEWAY=1` skips the per-config singleton.

## Port mapping (derived)

Base port = `gateway.port` (or `OPERATOR_GATEWAY_PORT` / `--port`).
...
```bash
OPERATOR_CONFIG_PATH=~/.operator/main.json \
OPERATOR_STATE_DIR=~/.operator \
operator gateway --port 18789

OPERATOR_CONFIG_PATH=~/.operator/rescue.json \
OPERATOR_STATE_DIR=~/.operator-rescue \
operator gateway --port 19789
```
...
operator --profile rescue gateway status --deep
operator --profile rescue gateway probe
operator status
operator --profile rescue status
operator --profile rescue browser status
```

- `gateway status --deep` catches stale launchd/systemd/schtasks services from older installs.
- `gateway probe` warning text such as `multiple reachable gateway identities detected` is expected only when you intentionally run more than one isolated gateway, or when Operator cannot prove reachable probe targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is one gateway with multiple transports, even when transport ports differ.
