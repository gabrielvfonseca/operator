---
summary: "Plain-English and technical explanation of npm shrinkwrap in Operator releases"
read_when:
  - You want to know what npm shrinkwrap means in an Operator release
...
Operator source checkouts use `pnpm-lock.yaml`. Published Operator npm packages use `npm-shrinkwrap.json`, npm's publishable dependency lockfile, so package installs use the dependency graph reviewed during release.
...
| --------------------- | ------------------------ | --------------------------------- |
| `pnpm-lock.yaml`      | Operator source checkout | Maintainer dependency graph       |
| `npm-shrinkwrap.json` | Published npm package    | npm install graph for users       |
| `package-lock.json`   | Local npm apps           | Not the Operator publish contract |

For Operator releases this means:
...
Shrinkwrap is not a sandbox. It does not make a dependency safe by itself, and it does not replace host isolation, `operator security audit`, package provenance, or install smoke tests.

Operator is a gateway, plugin host, model router, and agent runtime, so a default install affects startup time, disk use, native package downloads, and supply-chain exposure. Shrinkwrap gives release review a stable boundary: reviewers see transitive dependency movement, validators reject unexpected lockfile drift, and plugin packages carry their own locked dependency graph instead of relying on the root package.
...
Operator package validators require shrinkwrap in new root package tarballs and reject `package-lock.json` for published packages. The plugin npm publish path checks plugin-local shrinkwrap, installs package-local bundled dependencies, then packs or publishes.
...
```bash
npm pack @gabrielvfonseca/discord@<version> --json --pack-destination /tmp/operator-plugin-pack
tar -tf /tmp/operator-plugin-pack/operator-discord-<version>.tgz | grep '^package/npm-shrinkwrap.json$'
tar -tf /tmp/operator-plugin-pack/operator-discord-<version>.tgz | grep '^package/node_modules/'
```
