---
summary: "Pre-flight and rollback checklist before exposing an Operator Gateway beyond loopback"
...
- Gateway host, OS user, and state directory (default `~/.operator`).
...
- Backup location for `~/.operator/operator.json` and credentials.
...
```bash
operator gateway probe --url ws://127.0.0.1:18789 --token "$OPERATOR_GATEWAY_TOKEN"
```
...
Run `operator security audit --deep` after proxy changes. Trusted-proxy
...
1. Re-run `operator security audit --deep`.
...
5. Re-run `operator security audit --deep`.
