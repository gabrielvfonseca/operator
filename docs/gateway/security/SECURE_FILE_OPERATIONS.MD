---
summary: "How Operator handles local file access safely, and why the optional fs-safe Python helper is off by default"
read_when:
...
Operator sets the fs-safe POSIX Python helper to **off** by default:
...
Operator only changes the _default_. An explicit setting always wins:

```bash
# Default Operator behavior: Node-only fs-safe fallbacks.
OPERATOR_FS_SAFE_PYTHON_MODE=off

# Opt into the helper when available, falling back if unavailable.
OPERATOR_FS_SAFE_PYTHON_MODE=auto

# Fail closed if the helper cannot start.
OPERATOR_FS_SAFE_PYTHON_MODE=require

# Optional explicit interpreter path.
OPERATOR_FS_SAFE_PYTHON=/usr/bin/python3
```
...
With the helper off, Operator still gets fs-safe's Node-only guardrails:
...
This covers Operator's normal threat model: trusted gateway code handling untrusted model/plugin/channel path input inside a single trusted operator boundary.
...
That narrows same-UID race windows where another process swaps a parent directory between validation and mutation — defense in depth on hosts where untrusted local processes can modify the same directories Operator operates in.
