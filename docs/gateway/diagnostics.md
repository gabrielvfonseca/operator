...
Operator can build a local diagnostics `.zip` for bug reports: sanitized Gateway
...
2. Operator sends a preamble and asks for one explicit exec approval, which runs
   `operator gateway diagnostics export --json`. Do not approve diagnostics via
   an allow-all rule.
3. After approval, Operator replies with the local bundle path, manifest
   summary, privacy notes, and relevant session ids.

In group chats, an owner can still run `/diagnostics`, but Operator sends the
...
approval also covers an OpenAI feedback upload for the Codex threads Operator
...
approval, the reply lists channels, Operator session ids, Codex thread ids, and
...
Persisted bundles live under `~/.operator/logs/stability/` when events exist.

## Useful options

```bash
operator gateway diagnostics export \
  --output operator-diagnostics.zip \
  --log-lines 5000 \
  --log-bytes 1000000
```
...
| ----------------------- | ----------------------------------------------------------------------------- | -------------------------------------------------- |
| `--output <path>`       | `$OPERATOR_STATE_DIR/logs/support/operator-diagnostics-<timestamp>-<pid>.zip` | Write to a specific zip path (or directory).       |
