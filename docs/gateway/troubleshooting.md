...
- `operator gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `operator doctor` reports no blocking config/service issues.
- `operator channels status --probe` shows live per-account transport status and, where supported, `works` or `audit ok`.
...
- `Update restart` in `operator status` / `operator status --all`. Pending or failed handoffs include the next command to run.
- `plugin load failed: dependency tree corrupted; run operator doctor --fix` under Channels: the channel config still exists, but plugin registration failed before the channel could load.
- Provider 401s after re-auth: `operator doctor --fix` checks for stale per-agent OAuth auth shadows and removes old copies so all agents resolve the current shared profile.
...
<Warning>
For intentional downgrade or emergency recovery only, set `OPERATOR_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1` for the single command. Leave it unset for normal operation.
</Warning>
...
- `Established clients:` in `operator gateway status --deep` or `Gateway clients` in `operator doctor --deep`: active TCP clients connected to the Gateway port, with PIDs and command lines when the OS allows it.
- A client process whose command line points at the newer Operator install or wrapper you rolled back from.

Fix:

1. Stop or restart the stale Operator client process shown by `gateway status --deep`.
2. Restart apps or wrappers that embed Operator: local dashboards, editors, app-server helpers, or long-running `operator logs --follow` shells.
3. Re-run `operator gateway status --deep` or `operator doctor --deep` and confirm the stale client PID is gone.
...
Every skill root is a containment boundary. A symlink under `~/.agents/skills`, `<workspace>/.agents/skills`, `<workspace>/skills`, or `~/.operator/skills` is skipped when its real target resolves outside that root, unless the target is explicitly trusted.
...
operator config get skills.load
```
...
operator config get agents.defaults.models
```
...
Do not assume this is always an Operator configuration issue. The response can come from an upstream security layer such as a CDN, WAF, bot-management rule, or reverse proxy in front of an OpenAI-compatible endpoint.
...
Fix the provider-side filtering first when evidence points to a WAF/CDN block. Prefer a narrowly scoped allow or skip rule for the API path Operator uses, and avoid disabling protection for the whole site.
...
- Operator model runs fail only on normal agent turns.
...
operator infer model run --model <provider/model> --prompt "hi" --json
operator logs --follow
```

Look for:

- Direct tiny calls succeed, but Operator runs fail only on larger prompts.
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0`: the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. Operator retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - Direct tiny requests succeed, but Operator agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds): Operator transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    3. Set `compat.supportsTools: false` for models/backends that cannot handle Operator's tool schema surface reliably.
    ...
    5. If tiny direct requests keep passing while Operator agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
operator status
operator channels status --probe
operator pairing list --channel <channel> [--account <id>]
operator config get channels
operator logs --follow
```
...
If `curl` returns Operator HTML, the Gateway is working and the remaining issue is likely browser cache, an old deep link, or stale tab state. Open `http://127.0.0.1:18789` directly and navigate from the dashboard. If restart does not leave the service running, run `operator gateway start` and recheck `operator gateway status`.
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `operator config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `operator devices list` then `operator devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                               |
...
If `operator devices rotate` / `revoke` / `remove` is denied unexpectedly:

- Paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`.
- `operator devices rotate --scope ...` can only request operator scopes that the caller session already holds.
...
operator logs --follow
operator doctor
operator gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `operator onboard --mode local` / `operator setup` to restamp the expected local-mode config. If you are running Operator via Podman, the default config path is `~/.operator/operator.json`.
    ...
    - `System-level Operator gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `OPERATOR_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `operator doctor --fix` or `operator gateway install --force`, then restart the gateway service.
...
- One or more `*-uncaught_exception.json` bundles in `~/.operator/logs/stability/` with `error.code` set to a transient network code such as `ENETDOWN`, `ENETUNREACH`, `EHOSTUNREACH`, or `ECONNREFUSED`.
...
3. **Add a liveness watchdog** so a future crash burst that gets parked by launchd is caught quickly:
   ...
   state=$(launchctl print gui/$UID/ai.operator.gateway 2>/dev/null | awk -F'= ' '/state =/ {print $2; exit}')
...
This was observed on older installs where both `ai.operator.gateway` and
`ai.operator.node` LaunchAgents were active and each injected
`OPERATOR_LAUNCHD_LABEL`. In that state Operator can detect launchd
...
```bash
for i in 1 2 3 4; do
  ps aux | grep 'operator.*index.js' | grep -v grep | awk '{print $2}'
  sleep 10
done
...
- Both `~/Library/LaunchAgents/ai.operator.gateway.plist` and
  `~/Library/LaunchAgents/ai.operator.node.plist` loaded at the same time on a
...
1. If this host should only run the Gateway service, remove the managed node
   service through Operator. **Skip this step** if you actively rely on the node
...
2. Install a persistent Gateway wrapper that clears the inherited launchd
   markers before starting Operator. Use the supported `--wrapper` option; do
   not edit the generated file under `~/.operator/service-env/`, because service
   reinstall, update, and doctor repair regenerate that file:

   ```bash
   mkdir -p ~/.local/bin
   cat >~/.local/bin/operator-launchd-workaround <<'EOF'
   #!/bin/sh
   set -eu
   unset OPERATOR_LAUNCHD_LABEL LAUNCH_JOB_LABEL LAUNCH_JOB_NAME XPC_SERVICE_NAME || true
   exec operator "$@"
   EOF
   chmod 700 ~/.local/bin/operator-launchd-workaround

   operator gateway install \
     --wrapper ~/.local/bin/operator-launchd-workaround \
     --force
   ```
...
3. Verify that the Gateway is stable and serving RPC, not merely listening:
   ...
   ```bash
   operator gateway status --deep --require-rpc
...
4. After upgrading to a release where the underlying dual-LaunchAgent loop is
   ...
   ```bash
   OPERATOR_WRAPPER= operator gateway install --force
   rm ~/.local/bin/operator-launchd-workaround
   ```
...
operator logs --follow
operator gateway stability --bundle latest
operator gateway diagnostics export
```
...
- `critical memory pressure bundle written` appears shortly before restart → Operator captured a pre-OOM stability bundle. Inspect it with `operator gateway stability --bundle latest`.
- `memory pressure: level=critical ... memoryPressureSnapshot=disabled` appears in gateway logs → Operator detected critical memory pressure, but the pre-OOM stability snapshot is off.
...
- A timestamped `operator.json.rejected.*` file beside the active config.
- A timestamped `operator.json.clobbered.*` file if `doctor --fix` repaired a broken direct edit.
- Operator keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones.

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an Operator-owned write.
    - Gateway startup fails closed instead of rewriting `operator.json`.
    - Hot reload skips invalid external edits and keeps the current runtime config active.
    - Operator-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    - `operator doctor --fix` owns repair. It can remove non-JSON prefixes or restore the last-known-good copy while preserving the rejected payload as `.clobbered.*`.
    - When many repairs happen for one config path, Operator rotates older `.clobbered.*` files so the newest repaired payload is still available.
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an Operator-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an Operator-owned write was rejected because it lost fields or size compared with the last-known-good backup.
    - `Config last-known-good promotion skipped` → the candidate contained redacted secret placeholders such as `***`.

  </Accordion>
  <Accordion title="Fix options">
    1. Run `operator doctor --fix` to let doctor repair prefixed/clobbered config or restore last-known-good.
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `operator config set` or `config.patch`.
    3. Run `operator config validate` before restarting.
...
Use when `operator gateway probe` reaches something, but still prints a warning block.

```bash
operator gateway probe
operator gateway probe --json
operator gateway probe --ssh user@gateway-host
```
...
- `multiple reachable gateway identities detected` → distinct gateways answered, or Operator could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is treated as one gateway with multiple transports, even when transport ports differ.
...
```bash
operator cron status
operator cron list
operator cron runs --id <jobId> --limit 20
operator system heartbeat last
operator logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank, comment, header, fence, or empty-checklist scaffolding, so Operator skips the model call.
...
operator nodes describe --node <idOrNameOrIp>
operator approvals get --node <idOrNameOrIp>
operator logs --follow
operator status
```
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - Stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `operator browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (Operator-managed)](/tools/browser)
...
<AccordionGroup>
  <Accordion title="1. Auth and URL override behavior changed">
    ```bash
    operator gateway status
    operator config get gateway.mode
    operator config get gateway.remote.url
    operator config get gateway.auth.mode
    ```
  ...
  <Accordion title="2. Bind and auth guardrails are stricter">
    ```bash
    operator config get gateway.bind
    operator config get gateway.auth.mode
    operator config get gateway.auth.token
    operator gateway status
    operator logs --follow
    ```
