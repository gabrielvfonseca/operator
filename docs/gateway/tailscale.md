...
Operator can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the Gateway dashboard and WebSocket port. This keeps the gateway bound to loopback while Tailscale provides HTTPS, routing, and (for Serve) identity headers.
...
Status and audit output use **Tailscale exposure** for this Operator Serve/Funnel mode. `off` means Operator is not managing Serve or Funnel; it does not mean the local Tailscale daemon is stopped or logged out.
...
Startup then reports the Service URL as `https://operator.<tailnet-name>.ts.net/` instead of the device hostname. Tailscale Services require the host to be an approved tagged node in your tailnet — configure the tag and approve the Service in Tailscale before enabling this, otherwise `tailscale serve --service=...` fails during gateway startup.
...
Prefer `OPERATOR_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
operator gateway --tailscale serve
operator gateway --tailscale funnel --auth password
```
...
| `token` (default when `OPERATOR_GATEWAY_TOKEN` is set) | Shared token                                                                        |
| `password`                                             | Shared secret via `OPERATOR_GATEWAY_PASSWORD` or config                             |
