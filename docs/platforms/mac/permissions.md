...
- Same path: run the app from a fixed location (for Operator, `dist/Operator.app`).
- Same bundle identifier: Operator's bundle ID is `ai.operator.mac`; changing it creates a new permission identity.
...
Prefer granting Accessibility to Operator.app, Peekaboo.app, or another signed helper with its own bundle identifier instead of a generic `node` binary.
...
Example resets (using Operator's bundle ID, `ai.operator.mac`):

```bash
sudo tccutil reset Accessibility ai.operator.mac
sudo tccutil reset ScreenCapture ai.operator.mac
...
Workaround: move files into the Operator workspace (`~/.operator/workspace`) if you want to avoid per-folder grants.
