...
Voice subsystem is `ai.operator`; each component logs under its own category:
...
- Stream logs while reproducing a sticky overlay:
  ...
  ```bash
  sudo log stream --predicate 'subsystem == "ai.operator" AND category CONTAINS "voicewake"' --level info --style compact
  ```
