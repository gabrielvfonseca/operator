...
The macOS app surfaces Operator skills via the gateway; it does not parse skills locally.
...
- Requirements come from `metadata.operator.requires` in each `SKILL.md`.

## Install actions

- `metadata.operator.install` defines install options (brew/node/go/uv/download).
...
- The app stores keys in `~/.operator/operator.json` under `skills.entries.<skillKey>`.
