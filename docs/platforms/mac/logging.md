---
summary: "Operator logging: rolling diagnostics file log + unified log privacy flags"
read_when:
...
- Location: `~/Library/Logs/Operator/diagnostics.jsonl`.
...
## Enable for Operator (`ai.operator`)

Write the plist to a temp file first, then install it atomically as root:

```bash
cat <<'EOF' >/tmp/ai.operator.plist
...
EOF
sudo install -m 644 -o root -g wheel /tmp/ai.operator.plist /Library/Preferences/Logging/Subsystems/ai.operator.plist
```
...
- Remove the override: `sudo rm /Library/Preferences/Logging/Subsystems/ai.operator.plist`.
