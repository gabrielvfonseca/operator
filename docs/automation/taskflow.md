...
read_when:
  ...
  - You encounter Task Flow or operator tasks flow in release notes or docs
...
| Command                           | Description                                                             |
| --------------------------------- | ----------------------------------------------------------------------- |
| `operator tasks flow list`        | Tracked flows with sync mode, status, revision, controller, task counts |
| `operator tasks flow show <id>`   | Inspect one flow by flow id or owner key, including linked tasks        |
| `operator tasks flow cancel <id>` | Cancel a running flow and its active tasks                              |

Flows are also covered by `operator tasks audit` (stale or broken flow findings) and `operator tasks maintenance` (finalizes stuck cancels, prunes terminal flows after 7 days).
...
Flows coordinate tasks, not replace them. A single flow may drive multiple background tasks over its lifetime. Use `operator tasks` to inspect individual task records and `operator tasks flow` to inspect the orchestrating flow.

## Related

- [Background Tasks](/automation/tasks) - the detached work ledger that flows coordinate
- [CLI: tasks](/cli/tasks) - CLI command reference for `operator tasks flow`
