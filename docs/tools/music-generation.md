...
generated audio is still missing from the reply, Operator sends an
...
<Note>
Not all providers support all parameters. Operator still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, Operator
...
Provider request timeouts are operator configuration only. Operator uses
...
- **Status lookup:** `operator tasks list` or `operator tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** Operator injects an internal completion event back
...
```bash
operator tasks list
operator tasks show <taskId>
operator tasks cancel <taskId>
```
...
```bash
OPERATOR_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
