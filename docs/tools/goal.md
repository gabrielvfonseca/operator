...
read_when:
  - You want Operator to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current Operator session.
...
stale or unknown token snapshot when the goal starts, Operator waits for the
...
Operator usage and model controls.

## Model tools

Operator exposes three goal tools to agent harnesses:
...
Operator keeps the line compact by truncating long objectives. Paused,
...
`/goal` works in command-capable Operator sessions, including the TUI and
...
fresh token snapshot yet. Usage refreshes as Operator records session usage
