...
frontmatter and a markdown body. Operator loads bundled skills plus any local
...
Operator loads from these sources, **highest precedence first**. When the same
...
| 4           | Managed / local skills | `~/.operator/skills`                    |
| 5           | Bundled skills         | shipped with the install                |
| 6 — lowest  | Extra directories      | `skills.load.extraDirs` + plugin skills |

Skill roots support grouped layouts. Operator discovers a skill whenever
...
<Note>
  Codex CLI's native `$CODEX_HOME/skills` directory is **not** an Operator
  skill root. Use `operator migrate plan codex` to inventory those skills, then
  `operator migrate codex` to copy them into your Operator workspace.
</Note>

## Node-hosted skills

A connected headless node can publish skills installed in its active Operator
skills directory (`~/.operator/skills` by default; profile environment overrides
...
| Shared managed | `~/.operator/skills`         | All agents on this machine  |
...
`operator.plugin.json` (paths relative to the plugin root). Plugin skills load
...
`metadata.operator.requires` in its frontmatter, same as any other skill.
...
`operator skills` commands for install and update, or the `clawhub` CLI for
publish and sync.

| Action                             | Command                                                |
| ---------------------------------- | ------------------------------------------------------ |
| Install a skill into the workspace | `operator skills install @owner/<slug>`                |
| Install from a Git repository      | `operator skills install git:owner/repo@ref`           |
| Install a local skill directory    | `operator skills install ./path/to/skill --as my-tool` |
| Install for all local agents       | `operator skills install @owner/<slug> --global`       |
| Update all workspace skills        | `operator skills update --all`                         |
| Update a shared managed skill      | `operator skills update @owner/<slug> --global`        |
| Update all shared managed skills   | `operator skills update --all --global`                |
| Verify a skill's trust envelope    | `operator skills verify @owner/<slug>`                 |
| Print the generated Skill Card     | `operator skills verify @owner/<slug> --card`          |
| Publish / sync via ClawHub CLI     | `clawhub sync --all`                                   |

<AccordionGroup>
  <Accordion title="Install details">
    `operator skills install` installs into the active workspace `skills/`
    directory by default. Add `--global` to install into the shared
    `~/.operator/skills` directory, visible to all local agents unless agent
    ...
    `operator skills update` tracks ClawHub installs only — reinstall Git or
    local sources to refresh them.

  </Accordion>
  <Accordion title="Verification and security scanning">
    `operator skills verify @owner/<slug>` asks ClawHub for the skill's
  ...
  <Accordion title="Private archive installs">
    ...
    `operator.json`. Normal ClawHub installs never need that setting.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Path containment">
    ...
    Managed `~/.operator/skills` and personal `~/.agents/skills` may contain
...
<Note>
  Operator follows the [AgentSkills](https://agentskills.io) spec. Frontmatter
...
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, Operator keeps the skill's instructions out of the agent's normal
...
Operator filters skills at load time using `metadata.operator` (JSON5 object
embedded in the frontmatter, see the parsing note above). A skill with no
`metadata.operator` block is always eligible unless explicitly disabled.
...
<ParamField path="requires.config" type="string[]">
  Each `operator.json` path must be truthy.
</ParamField>
...
<Note>
  ...
  `metadata.operator` is absent, so older installed skills keep their
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, Operator lists each entry so you can
      see all available artifacts.
    - Specs can include `os: ["darwin"|"linux"|"win32"]` to filter by platform.
    - Node installs honor `skills.install.nodeManager` in `operator.json`
  ...
  <Accordion title="Per-installer details">
    - **Homebrew:** Operator does not auto-install Homebrew or translate brew
    ...
    - **Go:** Operator requires Go 1.21 or newer for automatic skill installs.
      If `go` is missing and Homebrew is available, Operator installs Go via
      ...
      dependency always targets a dedicated Operator-managed bin directory
    ...
    - **Download:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`),
      ...
      `targetDir` (default: `~/.operator/tools/<skillKey>`).
  </Accordion>
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience field for skills that declare `metadata.operator.primaryEnv`.
...
<Note>
  ...
  `metadata.operator.skillKey`, use that key under `skills.entries` instead.
  Quote hyphenated names: JSON5 allows quoted keys.
</Note>

## Environment injection

When an agent run starts, Operator:

<Steps>
  <Step title="Reads skill metadata">
    Operator resolves the effective skill list for the agent, applying gating
...
For the bundled `claude-cli` backend, Operator also materializes the same
...
Operator snapshots eligible skills **when a session starts** and reuses that
...
allowlist changes, Operator refreshes the snapshot to keep visible skills
aligned.

<AccordionGroup>
  <Accordion title="Skills watcher">
    By default, Operator watches skill folders and bumps the snapshot when
  ...
  <Accordion title="Remote macOS nodes (Linux gateway)">
    ...
    `system.run` allowed, Operator can treat macOS-only skills as eligible when
    ...
    answering bin probes, Operator clears its cached bin matches.
...
When skills are eligible, Operator injects a compact XML block into the system
...
(`skills.limits.maxSkillsPromptChars`), Operator first preserves as many skill
...
note pointing at `operator skills check` whenever compact formatting or list
