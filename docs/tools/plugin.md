---
summary: "Install, configure, and manage Operator plugins"
read_when:
...
Plugins extend Operator with channels, model providers, agent harnesses, tools,
...
<Steps>
  <Step title="Find the plugin">
    ...
    they match an official plugin id. Raw `@gabrielvfonseca/*` specs that match a
  ...
  <Step title="Install the plugin">
    ...
    reproducible production installs. ClawHub packages and Operator's
  ...
  <Step title="Configure and enable it">
    ...
    before the plugin can load. `operator plugins install` adds the installed
...
| ----------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| ClawHub     | You want Operator-native discovery, scans, version metadata, and install hints | `operator plugins install clawhub:<package>`                   |
| npm         | You need direct npm registry or dist-tag workflows                             | `operator plugins install npm:<package>`                       |
| git         | You need a branch, tag, or commit from a repository                            | `operator plugins install git:github.com/<owner>/<repo>@<ref>` |
...
bare spec installs through npm during the launch cutover. Raw `@gabrielvfonseca/*`
specs that match bundled plugins also resolve to the bundled copy before npm
fallback. Use `npm:@gabrielvfonseca/<plugin>@<version>` to deliberately install the
external npm package instead of the bundled copy. Use `clawhub:`, `npm:`,
`git:`, or `npm-pack:` for deterministic source selection. See
[`operator plugins`](/cli/plugins#install) for the full command contract.

For npm installs, unpinned specs and `@latest` choose the newest stable
package that advertises compatibility with this Operator build. If npm's
current latest release declares a newer `operator.compat.pluginApi` or
`operator.install.minHostVersion` than this build supports, Operator scans
...
later, and only in Operator processes where plugin hooks are loaded, so use
...
compatibility but is a no-op: it does not bypass install policy or Operator's
...
snippet. Run [`operator plugins list --enabled --verbose`](/cli/plugins#list)
or [`operator plugins inspect <id>`](/cli/plugins#inspect) on the listed
plugin id before copying trusted plugins into `operator.json`. The same
...
`plugins.allow` or reinstall from a trusted source so Operator records install
provenance.

Run `operator doctor` or `operator doctor --fix` when config validation
reports stale plugin ids, allowlist/tool mismatches, or legacy bundled plugin
paths.

## Understand plugin formats

Operator recognizes two plugin formats:

| Format                 | How it loads                                                                 | Use when                                                               |
| ---------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Native Operator plugin | `operator.plugin.json` plus a runtime module loaded in process               | You are installing or building Operator-specific runtime capabilities  |
| Compatible bundle      | Codex, Claude, or Cursor plugin layout mapped into Operator plugin inventory | You are reusing compatible skills, commands, hooks, or bundle metadata |

Both formats appear in `operator plugins list`, `operator plugins inspect`,
`operator plugins enable`, and `operator plugins disable`. See
...
Plugin-managed internal hooks show up in `operator hooks list` with
`plugin:<id>`. You cannot enable or disable them through `operator hooks`;
enable or disable the plugin instead.

## Verify the active Gateway

`operator plugins list` and plain `operator plugins inspect` read cold config,
...
sure any manual restart targets the actual `operator gateway run` child that
...
| Symptom                                                        | Check                                                                                                                                      | Fix                                                                                                     |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| Plugin appears in `plugins list` but runtime hooks do not run  | Use `operator plugins inspect <id> --runtime --json` and confirm the active Gateway with `gateway status --deep --require-rpc`             | Restart the live Gateway after install, update, config, or source changes                               |
| Duplicate channel or tool ownership diagnostics appear         | Run `operator plugins list --enabled --verbose`, inspect each suspected plugin with `--runtime --json`, and compare channel/tool ownership | Disable one owner, remove stale installs, or use manifest `preferOver` for intentional replacement      |
...
| Config is invalid during install                               | Read the validation message and run `operator doctor --fix` if it points to stale plugin state                                             | Doctor can quarantine invalid plugin config by disabling the entry and removing the invalid payload     |
| Plugin path is blocked for suspicious ownership or permissions | Inspect the diagnostic before the config error                                                                                             | Fix filesystem ownership/permissions, then run `operator plugins registry --refresh`                    |
| `OPERATOR_NIX_MODE=1` blocks lifecycle commands                | Confirm the install is managed by Nix                                                                                                      | Change plugin selection in the Nix source instead of using plugin mutator commands                      |
| Dependency import fails at runtime                             | Check whether the plugin was installed through npm/git/ClawHub or loaded from a local path                                                 | Run `operator plugins update <id>`, reinstall the source, or install local plugin dependencies yourself |
...
`operator doctor --fix` to remove stale plugin and channel entries. Unknown
...
plugin id. If both plugins are explicitly enabled, Operator keeps that request
...
Operator needs at runtime. Update or reinstall after the publisher ships
...
and validation follows with `plugin present but blocked`, Operator found
plugin files owned by a different Unix user than the process loading them.
Keep the plugin config in place; fix the filesystem ownership or run Operator
as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted Operator config and workspace directories should normally be
owned by uid `1000`:
...
If you intentionally run Operator as root, repair the managed plugin root to
root ownership instead:

```bash
sudo chown -R root:root /path/to/operator-config/npm
```

After fixing ownership, rerun `operator doctor --fix` or
`operator plugins registry --refresh` so the persisted plugin registry
...
```bash
operator config set logging.level trace
...
Operator caches successful plugin tool factory results for repeated
