...
Operator supports **Ollama Web Search** as a bundled `web_search` provider,
...
<Note>
  Operator never auto-selects Ollama Web Search over a higher-priority
...
config examples. If nothing is set, Operator defaults to
...
- If the resolved host is `https://ollama.com`, Operator calls
  `https://ollama.com/api/web_search` directly with the API key as bearer
  auth.
- Otherwise Operator calls the local proxy endpoint
  ...
  the local host.
- Operator warns during setup if Ollama is unreachable or not signed in, but
