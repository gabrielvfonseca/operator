...
summary: "Build, test, and publish custom SKILL.md workspace skills for your Operator agents."
read_when:
...
Operator loads skills from several roots in a defined [precedence order](/tools/skills#loading-order).
...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.operator/workspace/skills/hello-world
    ```
    ...
    ```bash
    mkdir -p ~/.operator/workspace/skills/personal/hello-world
  ...
  <Step title="Verify the skill loaded">
    ...
    Operator watches `SKILL.md` files under skills roots by default. If the
  ...
  <Step title="Test it">
    ```bash
    operator agent --message "give me a greeting"
    ```
...
metadata:
  {
    "@gabrielvfonseca/operator":
      { "requires": { "bins": ["gemini"] }, "primaryEnv": "GEMINI_API_KEY" },
  }
---
```
...
<AccordionGroup>
  <Accordion title="Gating options">
    ...
    | `requires.config` | Each `operator.json` path must be truthy |
  ...
  <Accordion title="Environment and API keys">
    Wire an API key to a skill entry in `operator.json`:
...
operator skills workshop propose-update hello-world \
  --proposal ./PROPOSAL.md \
...
<Steps>
  <Step title="Ensure your SKILL.md is complete">
    Make sure `name`, `description`, and any `metadata.operator` gating fields
...
<Tip>
  ...
  - **Test locally** — use `operator agent --message "..."` before sharing.
