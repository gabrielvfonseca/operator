...
`operator agent` runs a single agent turn from the command line without an
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    operator agent --agent main --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    operator agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    operator agent --session-id abc123 --message "Continue the task"
  ...
  <Step title="Deliver the reply to a channel">
    ...
    operator agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    operator agent --agent ops --message "Generate report" \
...
operator agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
...
<CardGroup cols={2}>
  <Card title="Agent CLI reference" href="/cli/agent" icon="terminal">
    Full `operator agent` flag and option reference.
  </Card>
