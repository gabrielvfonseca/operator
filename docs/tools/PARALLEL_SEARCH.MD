      ...
      parallel: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://api.parallel.ai", // optional; Operator appends /v1/search
          },
        },
      },
    },
...
environment. For a gateway install, put it in `~/.operator/.env`.
...
Cloudflare AI Gateway). Operator normalizes bare hosts by prepending
...
- Both providers return a `session_id`; Operator surfaces it as `sessionId` in
...
- `searchId`, `warnings`, and `usage` from Parallel are passed through when
  present.
- Operator always forwards a resolved result count to Parallel as
  ...
  Operator's generic `web_search` default (5) -- Parallel's own API defaults
  to 10.
