...
<Steps>
  ...
  <Step title="Store the key">
    ...
    gateway install, add it to `~/.operator/.env`), or configure via:
...
Choosing **Kimi** during `operator onboard` or `operator configure --section web`
also prompts for:
...
Operator only returns a Kimi `web_search` result after Moonshot's response
...
grounding (for example "I cannot browse the internet"), Operator returns a
