...
read_when:
  ...
  - Comparing Operator exec approvals with ACPX harness permissions
...
```bash
operator config set tools.exec.mode auto
...
Set ACPX permissions separately from Operator exec approvals:
...
| Use Codex/Operator auto-review before humans  | `tools.exec.mode: "auto"`                                   |
...
Host exec uses the stricter result of Operator config and the host-local approvals file. ACPX harness permissions do not loosen host exec approvals, and host exec approvals do not loosen ACPX harness prompts.
