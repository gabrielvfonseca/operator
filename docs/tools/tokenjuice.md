...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in Operator
...
Today this applies to Operator embedded runs and Operator dynamic tools in the Codex
app-server harness. Tokenjuice hooks Operator's tool-result middleware and
...
```bash
operator plugins install clawhub:@gabrielvfonseca/tokenjuice
```
...
```bash
operator config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
operator plugins disable tokenjuice
```
