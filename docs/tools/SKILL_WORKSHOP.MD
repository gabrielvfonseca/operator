...
Skill Workshop is Operator's governed path for creating and updating workspace
...
```bash
operator skills curator status
operator skills curator pin <skill>
operator skills curator unpin <skill>
operator skills curator restore <skill>
```
...
apply it through the normal approval flow or with `operator skills workshop`.
...
`operator skills workshop apply|reject|quarantine <proposal-id>`. Agents should
...
# Update an existing workspace skill
operator skills workshop propose-update trip-planning --proposal ./PROPOSAL.md
...
Operator detects durable instructions such as “next time,” “remember to,” and reactive corrections
...
are stored in the shared Operator state database; transcript content is not copied
into scan state.

With autonomous capture enabled, Operator can also perform a conservative review after successful,
...
```text
<OPERATOR_STATE_DIR>/skill-workshop/
  proposals.json
...
Default state directory: `~/.operator`.
...
| Proposal does not show in list                 | Check the selected `--agent` workspace and `OPERATOR_STATE_DIR`.                                                                                                                                            |
...
When autonomous capture is enabled, `operator doctor` runs the
...
`operator plugins inspect skill-workshop`; that command now explains that Skill
...
- [Skills CLI](/cli/skills) for `operator skills` commands
