...
read_when:
  - Looking for an overview of Operator's media capabilities
...
Operator generates images, videos, and music, understands inbound media
...
Operator until the shared realtime-voice contract can represent it.
</Note>
...
For async tools, Operator submits the request to the provider, returns a task
...
Operator wakes the agent with the generated media paths so it can tell the
...
Operator sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    other native model types; Operator has no provider contract for those
  ...
  <Accordion title="xAI">
    ...
    not registered in Operator until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
