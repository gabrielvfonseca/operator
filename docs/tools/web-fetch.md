...
`plugins.entries.firecrawl.config.webFetch` via `operator doctor --fix`.
...
- If `provider` is omitted, Operator auto-detects the first ready web-fetch
...
In this mode, Operator still applies hostname-based SSRF checks before sending
