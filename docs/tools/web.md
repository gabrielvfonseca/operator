...
normalized results, cached by query for 15 minutes (configurable). Operator
...
<Steps>
  ...
  <Step title="Configure">
    ```bash
    operator configure --section web
    ```
  ...
  <Step title="Use it">
    ```javascript
    await web_search({ query: "Operator plugin SDK" });
    ```
...
no `provider` is set, Operator checks providers in this order and uses the
...
`operator configure --section web`. Operator does not send managed
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, Operator resolves only the
...
`web_search` tool automatically when Operator web search is enabled and no
...
search and Operator's managed `web_search` dynamic tool are mutually exclusive,
so managed search cannot bypass native domain restrictions. Operator uses the
...
- Set `tools.web.search.provider` to a managed provider such as `brave` to use
  Operator's managed `web_search` instead
...
use the managed `web_search` fallback through Operator's dynamic tool namespace.
Use an explicit managed provider when you need Operator's provider-specific
...
Codex app-server first with `operator models auth login --provider openai`.
The parent agent can use any model or runtime; only the bounded search worker
runs through Codex.

## Network safety

Managed HTTP `web_search` provider calls use Operator's guarded fetch path,
scoped to the current provider's own hostname. For that hostname only,
Operator allows Surge, Clash, and sing-box fake-IP DNS answers in
...
Grok can also reuse an xAI OAuth auth profile from `operator models auth login
...
Operator keeps startup resilient and reports a warning so you can reinstall the
plugin or run `operator doctor --fix` to clean up the stale config.

`web_fetch` fallback provider selection is separate:

- choose it with `tools.web.fetch.provider`
- or omit that field and let Operator auto-detect the first ready web-fetch
...
When you choose **Kimi** during `operator onboard` or
`operator configure --section web`, Operator can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `operator doctor --fix`.
When you choose Grok during `operator onboard` or `operator configure --section web`,
Operator also offers optional `x_search` setup with the same credential right
...
provider, Operator does not show the `x_search` prompt.

### Storing API keys

<Tabs>
  <Tab title="Config file">
    Run `operator configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.operator/.env`.
...
optional structured filters. Operator constructs the built-in xAI `x_search`
