...
read_when:
  ...
  - Debugging why operator is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (Operator-managed)"
---
...
If `operator browser` is missing entirely, or the agent says the browser tool
...
Defaults need both `plugins.entries.browser.enabled` **and** `browser.enabled=true`. Disabling only the plugin removes the `operator browser` CLI, `browser.request` gateway method, agent tool, and control service as one unit; your `browser.*` config stays intact for a replacement.
...
If `operator browser` is unknown after an upgrade, `browser.request` is missing, or the agent reports the browser tool as unavailable, the usual cause is a `plugins.allow` list that omits `browser` and no root `browser` config block exists. Add it:
...
- `chrome`: built-in [Chrome extension](/tools/chrome-extension) profile for
  ...
  desk because it drives tabs through the Operator browser extension instead of
...
Set `browser.defaultProfile: "@gabrielvfonseca/operator"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.operator/operator.json`.
...
but Operator does not automatically attach them to channel replies. To share a
...
understanding model is configured, Operator keeps the normal image result so the
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, Operator pauses new launch
...
- Before each fresh Operator-managed Chrome launch, Operator best-effort disables network prediction, suppressing Chromium's observed speculative preconnect for those denied loads. This is defense in depth, not a policy boundary: a browser reused across a control-service restart and other browser backends may not share the hardening. Playwright routing is still not a network firewall and does not intercept redirect hops, a popup's first request, Service Worker traffic, page code that runs after the bounded guard window, or every background/subresource path. Complete egress isolation requires owner-side isolation or a policy-enforcing proxy.
- In strict SSRF mode, remote CDP endpoint discovery and `/json/version` probes (`cdpUrl`) are checked too.
- Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` environment variables do not automatically proxy the Operator-managed browser. Managed Chrome launches direct by default so provider proxy settings do not weaken browser SSRF checks.
- Operator-managed local CDP readiness probes and DevTools WebSocket connections bypass the managed network proxy for the exact launched loopback endpoint, so `operator browser start` still works when an operator proxy blocks loopback egress.
...
- `POST /start?headless=true` and `operator browser start --headless` request a
  ...
  remote CDP profiles reject the override because Operator does not launch those
  browser processes.
- On Linux hosts without `DISPLAY` or `WAYLAND_DISPLAY`, local managed profiles
  ...
  `operator browser --json status`; trailing `operator browser status --json`
...
- `OPERATOR_BROWSER_HEADLESS=1` forces local managed launches headless for the
  current process. `OPERATOR_BROWSER_HEADLESS=0` forces headed mode for ordinary
...
- For a running local managed profile, status and doctor query Chrome's
  ...
  `operator browser --json status`. A passive status call does not launch Chrome.
...
Operator uses it automatically. Set `browser.executablePath` to override
...
Per-profile `executablePath` only affects local managed profiles that Operator
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, Operator will not launch a local browser.
- For externally managed CDP services on loopback (for example Browserless in
  ...
  without `attachOnly` is treated as a local Operator-managed browser profile.
- `headless` only affects local managed profiles that Operator launches. It does not restart or change existing-session or remote CDP browsers.
...
- local managed profiles: `operator browser stop` stops the browser process that
  Operator launched
- attach-only and remote CDP profiles: `operator browser stop` closes the active
  ...
  though no browser process was launched by Operator
...
Operator preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Operator can
...
- If you set `nodeHost.browserProxy.allowProfiles`, Operator treats it as a least-privilege boundary limiting which profile names the proxy will target.
...
CDP connection URLs over HTTPS and WebSocket. Operator can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let Operator
  discover `/json/version`.

### Browserless Docker on the same host

When Browserless is self-hosted in Docker and Operator runs on the host, treat
...
Operator process. Browserless must also advertise a matching reachable endpoint;
set Browserless `EXTERNAL` to that same public-to-Operator WebSocket base, such
...
an address Operator cannot reach, CDP HTTP can look healthy while the WebSocket
attach still fails.

Do not leave `attachOnly` unset for a loopback Browserless profile. Without
`attachOnly`, Operator treats the loopback port as a local managed browser
profile and may report that the port is in use but not owned by Operator.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). Operator accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** - `http://host[:port]` or `https://host[:port]`.
  Operator calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** - `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. Operator connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** - `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). Operator tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise Operator
  ...
  accepts it, Operator falls back to that root as well. This lets a bare `ws://`
...
`operator browser doctor` uses the same discovery-first, WebSocket-fallback
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-operator-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, Operator
...
- Configure `gateway.auth.token`, `gateway.auth.password`, `OPERATOR_GATEWAY_TOKEN`, or
  `OPERATOR_GATEWAY_PASSWORD` explicitly if you want a stable secret you control
...
Operator supports multiple named profiles (routing configs). Profiles can be:

- **operator-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
...
Operator can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when Operator attaches.
...
```bash
operator browser --browser-profile user start
operator browser --browser-profile user status
operator browser --browser-profile user tabs
operator browser --browser-profile user snapshot --format ai
```
...
- `operator doctor` migrates old extension-based browser config and checks that
...
When `cdpUrl` is set on an existing-session profile, Operator skips
...
- **Dialog visibility** - Managed browser action responses include `blockedByDialog` and `browserState.dialogs.pending` when an action opens a modal dialog; snapshots also include pending dialog state. Respond with `browser dialog --accept/--dismiss --dialog-id <id>` while a dialog is pending. Dialogs handled outside Operator appear under `browserState.dialogs.recent`.
...
When launching locally, Operator picks the first available:
...
control API** plus a matching `operator browser` CLI (snapshots, refs, wait
...
- **CDP startup or readiness failure** means Operator cannot confirm that the browser control plane is healthy.
