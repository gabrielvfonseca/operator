...
Operator supports Grok as a `web_search` provider, using xAI web-grounded
...
lets Operator reuse it as a fallback for the bundled xAI model provider.
...
Choosing **Grok** during `operator onboard` or `operator configure --section
web` lets Operator reuse an existing xAI OAuth profile without prompting for
a separate web-search key. Without OAuth, it falls back to xAI API-key setup.

Operator then offers a follow-up step to enable `x_search` with the same xAI
...
<Steps>
  <Step title="Use xAI OAuth">
    ...
    ```bash
    operator onboard --auth-choice xai-oauth
...
**Credential alternatives:** `operator models auth login --provider xai
...
through an operator proxy or xAI-compatible Responses endpoint. Operator
