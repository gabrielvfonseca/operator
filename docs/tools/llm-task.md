...
`operator.invoke` already has the correct gateway URL/auth context.

For the bundled **embedded** Lobster runner inside Operator, this nested CLI
...
- Lobster steps that do not rely on nested `operator.invoke` calls.
