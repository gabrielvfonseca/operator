---
summary: "Run Operator in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want Operator isolated from your main macOS environment
...
Run Operator in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume). This gives you:
...
1. Install Lume.
2. `lume create operator --os macos --ipsw latest`
3. Complete Setup Assistant, enable Remote Login (SSH).
4. `lume run operator --no-display`
5. SSH in, install Operator, configure channels.
6. Done.
...
```bash
lume create operator --os macos --ipsw latest
```
...
The VM runs in the background; Operator's daemon keeps the gateway running. To check status:

```bash
ssh youruser@192.168.64.X "operator status"
```

## Bonus: iMessage integration

This is the killer feature of running on macOS. Use [iMessage](/channels/imessage) with `imsg` to add Messages to Operator.

Inside the VM:

1. Sign in to Messages.
2. Install `imsg`.
3. Grant Full Disk Access and Automation permission for the process running Operator/`imsg`.
4. Verify RPC support with `imsg rpc --help`.

Add to your Operator config:
