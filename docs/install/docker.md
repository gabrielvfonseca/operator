---
summary: "Optional Docker-based setup and onboarding for Operator"
read_when:
...
<Steps>
  ...
  <Step title="Airgapped rerun">
    ...
    `--offline` verifies `OPERATOR_IMAGE` already exists locally, disables implicit Compose pulls/builds, then runs the normal flow: `.env` sync, permission fixes, onboarding, gateway config sync, Compose startup.

    If `OPERATOR_SANDBOX=1`, offline setup also checks the configured default and per-agent sandbox images on the daemon behind `OPERATOR_DOCKER_SOCKET`, including the browser-contract label on Docker-backed browser images. If a required image is missing or stale, setup exits without changing sandbox config rather than reporting a broken success.
  ...
  <Step title="Complete onboarding">
    ...
    Pre-start onboarding and config writes run through `operator-gateway` directly (with `--no-deps --entrypoint node`), since `operator-cli` shares the gateway's network namespace and only works once the gateway container exists.
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm operator-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ```bash
    # WhatsApp (QR)
    docker compose run --rm operator-cli channels login

    # Telegram
    docker compose run --rm operator-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm operator-cli channels add --channel discord --token "<token>"
    ```
...
docker build \
  ...
  -t operator:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node operator-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node operator-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d operator-gateway
```
...
<Note>
Run `docker compose` from the repo root. If you enabled `OPERATOR_EXTRA_MOUNTS` or `OPERATOR_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`; include it after any `docker-compose.override.yml` you maintain yourself, e.g. `-f docker-compose.yml -f docker-compose.override.yml -f docker-compose.extra.yml`.
</Note>

### Upgrading container images

When you replace the Operator image but keep the same mounted state/config, the
...
same image once with `operator doctor --fix` as the container command, using the
same state/config mounts the gateway uses:

```bash
docker run --rm -v <operator-state>:/home/node/.operator <image> operator doctor --fix
podman run --rm -v <operator-state>:/home/node/.operator <image> operator doctor --fix
```
...
| Variable                                        | Purpose                                                                                                           |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `OPERATOR_IMAGE`                                | Use a remote image instead of building locally                                                                    |
| `OPERATOR_IMAGE_APT_PACKAGES`                   | Install extra apt packages during build (space-separated). Legacy alias: `OPERATOR_DOCKER_APT_PACKAGES`           |
| `OPERATOR_IMAGE_PIP_PACKAGES`                   | Install extra Python packages during build (space-separated)                                                      |
| `OPERATOR_EXTENSIONS`                           | Compile/package supported selected plugins and install their runtime dependencies (comma- or space-separated ids) |
| `OPERATOR_DOCKER_BUILD_NODE_OPTIONS`            | Override the local source-build Node options (default `--max-old-space-size=8192`)                                |
| `OPERATOR_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB` | Override the local source-build tsdown heap in MB                                                                 |
| `OPERATOR_DOCKER_BUILD_SKIP_DTS`                | Skip declaration output during runtime-only local image builds (default `1`)                                      |
| `OPERATOR_INSTALL_BROWSER`                      | Bake Chromium + Xvfb into the image at build time                                                                 |
| `OPERATOR_EXTRA_MOUNTS`                         | Extra host bind mounts (comma-separated `source:target[:opts]`)                                                   |
| `OPERATOR_HOME_VOLUME`                          | Persist `/home/node` in a named Docker volume                                                                     |
| `OPERATOR_SANDBOX`                              | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)                                                            |
| `OPERATOR_SKIP_ONBOARDING`                      | Skip the interactive onboarding step (`1`, `true`, `yes`, `on`)                                                   |
| `OPERATOR_DOCKER_SOCKET`                        | Override the Docker socket path                                                                                   |
| `OPERATOR_DISABLE_BONJOUR`                      | Force Bonjour/mDNS advertising on (`0`) or off (`1`); see [Bonjour / mDNS](#bonjour--mdns)                        |
| `OPERATOR_DISABLE_BUNDLED_SOURCE_OVERLAYS`      | Disable bundled plugin source bind-mount overlays                                                                 |
...
| `OPERATOR_OTEL_PRELOADED`                       | Skip starting a second OpenTelemetry SDK when one is preloaded                                                    |

The official image ships no Homebrew. During onboarding, Operator hides brew-only skill dependency installers in a Linux container without `brew`; provide those dependencies through a custom image or install manually. Use `OPERATOR_IMAGE_APT_PACKAGES` for Debian-packaged dependencies and `OPERATOR_IMAGE_PIP_PACKAGES` for Python dependencies (runs `python3 -m pip install --break-system-packages` at build time, so pin versions and use only indexes you trust).
...
```bash
OPERATOR_DOCKER_BUILD_NODE_OPTIONS=--max-old-space-size=4096 OPERATOR_DOCKER_BUILD_TSDOWN_MAX_OLD_SPACE_MB=4096
```

### Source-built images with selected plugins

`OPERATOR_EXTENSIONS` selects plugin manifest ids from the source checkout;
...
`operator.build.bundledDist: false`, compiles its runtime into the root bundled
...
already part of the root Operator runtime, so the ClickClack image selects only
...
build_gateway_image() {
  ...
  docker buildx build \
    ...
    --build-arg "OPERATOR_BUILD_TIMESTAMP=${BUILD_TIMESTAMP}" \
    --build-arg "OPERATOR_EXTENSIONS=${selected_plugin}" \
    --build-arg OPERATOR_INSTALL_BROWSER= \
    --provenance=mode=max \
    --sbom=true \
    --tag "${REGISTRY}/operator-${gateway}:${SOURCE_SHA}" \
    --push \
    .
}
...
docker buildx imagetools inspect \
  "${REGISTRY}/operator-clickclack:${SOURCE_SHA}"
# Deploy: registry.example.com/fakeco/operator-clickclack@sha256:<manifest-digest>
```
...
separate x86_64 appliance archive containing an Operator npm tarball and pins
the Node, archive, and manifest digests. Build that appliance independently
from the same landed Operator source.

To test bundled plugin source against a packaged image, mount one plugin source directory over its packaged source path, e.g. `OPERATOR_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`. That overrides the matching compiled `/app/dist/extensions/synology-chat` bundle for the same plugin id.
...
```bash
export OPERATOR_EXTENSIONS="diagnostics-otel"
export OTEL_EXPORTER_OTLP_ENDPOINT="http://otel-collector:4318"
export OTEL_SERVICE_NAME="operator-gateway"
./scripts/docker/setup.sh
```

Official prebuilt images already bundle `diagnostics-otel`; install `clawhub:@gabrielvfonseca/diagnostics-otel` yourself only if you removed it. To enable export, allow and enable the `diagnostics-otel` plugin in config, then set `diagnostics.otel.enabled=true` (see the full example in [OpenTelemetry export](/gateway/opentelemetry)). Collector auth headers go through `diagnostics.otel.headers`, not Docker environment variables.

Prometheus metrics reuse the already-published Gateway port. Install `clawhub:@gabrielvfonseca/diagnostics-prometheus`, enable the `diagnostics-prometheus` plugin, then scrape:
...
```bash
docker compose exec operator-gateway node dist/index.js health --token "$OPERATOR_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `OPERATOR_GATEWAY_BIND=lan` so `http://127.0.0.1:18789` on the host works with Docker port publishing.
...
. ./.env
set +a
export OPERATOR_HOME_VOLUME="${OPERATOR_HOME_VOLUME:-operator_home}"
./scripts/docker/setup.sh
```

If `.env` contains values your shell can't source, re-export what you rely on manually first (`OPERATOR_IMAGE`, ports, bind mode, custom paths, `OPERATOR_EXTRA_MOUNTS`, sandbox, skip-onboarding). The generated overlay mounts the home volume for both `operator-gateway` and `operator-cli`; run the remaining commands with that overlay (and `docker-compose.override.yml` first, if you use one):

```bash
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  --entrypoint sh operator-cli -lc \
  'curl -fsSL https://claude.ai/install.sh | bash'
```

The native installer writes `claude` to `/home/node/.local/bin/claude`. Point Operator at that path:

```bash
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  operator-cli config set \
...
```bash
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  --entrypoint /home/node/.local/bin/claude operator-cli auth login
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  --entrypoint /home/node/.local/bin/claude operator-cli auth status --text
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  operator-cli models auth login \
  --provider anthropic --method cli --set-default
docker compose -f docker-compose.yml -f docker-compose.extra.yml run --rm \
  operator-cli models list --provider anthropic
```
...
`OPERATOR_HOME_VOLUME` persists the native install under `/home/node/.local/bin` and `/home/node/.local/share/claude`, plus Claude Code settings/auth under `/home/node/.claude` and `/home/node/.claude.json`. Persisting only `/home/node/.operator` is not enough; if you use `OPERATOR_EXTRA_MOUNTS` instead of a home volume, mount all of those Claude paths into both services.
...
Docker bridge networking usually doesn't forward Bonjour/mDNS multicast (`224.0.0.251:5353`) reliably. When `OPERATOR_DISABLE_BONJOUR` is unset, the bundled Bonjour plugin auto-disables LAN advertising once it detects it's running in a container, so it won't crash-loop retrying multicast the bridge drops. Set `OPERATOR_DISABLE_BONJOUR=1` to force it off regardless of detection, or `0` to force it on (only on host networking, macvlan, or another network where mDNS multicast is known to work).
...
- `operator.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `OPERATOR_GATEWAY_TOKEN`

The auth-profile secret directory stores the local encryption key for OAuth-backed auth profile token material. Keep it with your Docker host state, but separate from `OPERATOR_CONFIG_DIR`.

Installed downloadable plugins store package state under the mounted Operator home, so install records and package roots survive container replacement; gateway startup does not regenerate bundled-plugin dependency trees.
...
<AccordionGroup>
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm operator-cli gateway probe
    docker compose run -T --rm operator-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `operator-cli` uses `network_mode: "service:operator-gateway"` so CLI commands can reach the gateway over `127.0.0.1`. Treat this as a shared trust boundary. The compose config drops `NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on both `operator-gateway` and `operator-cli`.
  </Accordion>

  <Accordion title="Docker Desktop DNS failures in operator-cli">
    Some Docker Desktop setups fail DNS lookups from the shared-network `operator-cli` sidecar after `NET_RAW` is dropped, showing up as `EAI_AGAIN` during npm-backed commands like `operator plugins install`. Keep the default hardened compose file for normal operation. The override below restores default capabilities for the `operator-cli` container only — use it for the one-off command that needs registry access, not as your default invocation:
    ...
    docker compose -f docker-compose.yml -f docker-compose.cli-no-dropped-caps.local.yml run --rm operator-cli plugins install <package>
    ```

    If you already created a long-running `operator-cli` container, recreate it with the same override — `docker compose exec`/`docker exec` can't change Linux capabilities on an already-created container.

  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on `/home/node/.operator`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/operator-config /path/to/operator-workspace
    ```

    The same mismatch can show up as `blocked plugin candidate: suspicious ownership (... uid=1000, expected uid=0 or root)` followed by `plugin present but blocked` — the process uid and the mounted plugin directory owner disagree. Prefer running as the default uid 1000 and fixing the bind mount ownership. Only chown `/path/to/operator-config/npm` to `root:root` if you intentionally run Operator as root long term.
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export OPERATOR_HOME_VOLUME="operator_home"`
    2. **Bake system deps**: `export OPERATOR_IMAGE_APT_PACKAGES="git curl jq"`
    3. **Bake Python deps**: `export OPERATOR_IMAGE_PIP_PACKAGES="requests==2.32.5 humanize==4.14.0"`
    4. **Bake Playwright Chromium**: `export OPERATOR_INSTALL_BROWSER=1`, or use the official `-browser` image tag
    5. **Or install Playwright browsers into a persisted volume**:
       ```bash
       docker compose run --rm operator-cli \
         node /app/node_modules/playwright-core/cli.js install chromium
       ```
    6. **Persist browser downloads**: use `OPERATOR_HOME_VOLUME` or `OPERATOR_EXTRA_MOUNTS`. Operator auto-detects the image's Playwright-managed Chromium on Linux.
...
<AccordionGroup>
  ...
  <Accordion title="Custom tools not found in sandbox">
    Operator runs commands with `sh -lc` (login shell), which sources `/etc/profile` and may reset PATH. Set `docker.env.PATH` to prepend your custom tool paths, or add a script under `/etc/profile.d/` in your Dockerfile.
  </Accordion>
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm operator-cli dashboard --no-open
    docker compose run --rm operator-cli devices list
    docker compose run --rm operator-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm operator-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm operator-cli devices list --url ws://127.0.0.1:18789
    ```
