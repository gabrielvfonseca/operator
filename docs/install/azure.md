---
summary: "Run Operator Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Operator running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Operator Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

Set up an Azure Linux VM with the Azure CLI, apply Network Security Group (NSG) hardening, configure Azure Bastion for SSH access, and install Operator.
...
- Install Operator with the installer script
- Verify the gateway
...
<Steps>
  ...
  <Step title="Install Operator (in the VM shell)">
    ```bash
    curl -fsSL https://operator.ai/install.sh -o /tmp/install.sh
    bash /tmp/install.sh
    rm -f /tmp/install.sh
    ```

    The installer installs Node and dependencies if not already present, installs Operator, and launches onboarding. See [Install](/install) for details.
...
- More detail on Azure deployment with the GitHub Copilot model provider: [Operator on Azure with GitHub Copilot](https://github.com/johnsonshi/operator-azure-github-copilot)
