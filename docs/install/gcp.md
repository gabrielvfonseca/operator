---
summary: "Run Operator Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Operator running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent Operator Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.
...
4. Clone the Operator repository
...
<Steps>
  ...
  <Step title="Create a GCP project">
    ```bash
    gcloud projects create my-operator-project --name="Operator Gateway"
    gcloud config set project my-operator-project
  ...
  <Step title="Create the VM">
    ...
    ```bash
    gcloud compute instances create operator-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    ```bash
    gcloud compute ssh operator-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Configure environment variables">
    ...
    OPERATOR_CONFIG_DIR=/home/$USER/.operator
    OPERATOR_WORKSPACE_DIR=/home/$USER/.operator/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.operator
    ```

    Set `OPERATOR_GATEWAY_TOKEN` to manage the stable gateway token through
    `.env`; otherwise configure `gateway.auth.token` before relying on clients
    across restarts. If neither is set, Operator uses a runtime-only token for
    ...
    **Do not commit this file.** It holds container/runtime env such as
    `OPERATOR_GATEWAY_TOKEN`. Stored provider OAuth/API-key auth lives in the
    mounted `~/.operator/agents/<agentId>/agent/auth-profiles.json`.
    ...
    services:
      operator-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - OPERATOR_GATEWAY_BIND=${OPERATOR_GATEWAY_BIND}
          - OPERATOR_GATEWAY_PORT=${OPERATOR_GATEWAY_PORT}
        ...
        ports:
          ...
          - "127.0.0.1:${OPERATOR_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`OPERATOR_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm operator-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh operator-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop operator-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type operator-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start operator-gateway --zone=us-central1-a
```
...
```bash
gcloud iam service-accounts create operator-deploy \
  --display-name="Operator Deployment"

gcloud projects add-iam-policy-binding my-operator-project \
  --member="serviceAccount:operator-deploy@my-operator-project.iam.gserviceaccount.com" \
