...
read_when:
  - You want to understand `operator.ai/install.sh`
  - You want to automate installs (CI / headless)
  - You want to install from a GitHub checkout
title: "Installer internals"
---

Operator ships three installer scripts, served from `operator.ai`.

| Script                             | Platform             | What it does                                                                                   |
| ---------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------- |
| [`install.sh`](#installsh)         | macOS / Linux / WSL  | Installs Node if needed, installs Operator via npm (default) or git, can run onboarding.       |
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL  | Installs Node + Operator into a local prefix (`~/.operator`) via npm or git. No root required. |
| [`install.ps1`](#installps1)       | Windows (PowerShell) | Installs Node if needed, installs Operator via npm (default) or git, can run onboarding.       |
...
<Tabs>
  <Tab title="install.sh">
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install.sh | bash -s -- --help
    ```

  </Tab>
  <Tab title="install-cli.sh">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install-cli.sh | bash
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install-cli.sh | bash -s -- --help
    ```
  ...
  <Tab title="install.ps1">
    ...
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://operator.ai/install.ps1))) -Tag beta -NoOnboard -DryRun
    ```
...
If run inside an Operator checkout (`package.json` + `pnpm-workspace.yaml`), the script offers:
...
<Tabs>
  ...
  <Tab title="Dry run">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install.sh | bash -s -- --dry-run
    ```
  </Tab>
...
| ------------------------------------------------- | ------------------------------------------------------------------ |
| `OPERATOR_INSTALL_METHOD=git\|npm`                | Install method                                                     |
| `OPERATOR_VERSION=latest\|next\|<semver>\|<spec>` | npm version, dist-tag, or package spec                             |
| `OPERATOR_BETA=0\|1`                              | Use beta if available                                              |
| `OPERATOR_HOME=<path>`                            | Base directory for Operator state and default git/onboarding paths |
| `OPERATOR_GIT_DIR=<path>`                         | Checkout directory                                                 |
| `OPERATOR_GIT_UPDATE=0\|1`                        | Toggle git updates                                                 |
| `OPERATOR_NO_PROMPT=1`                            | Disable prompts                                                    |
| `OPERATOR_VERIFY_INSTALL=1`                       | Run the post-install smoke verify                                  |
| `OPERATOR_NO_ONBOARD=1`                           | Skip onboarding                                                    |
| `OPERATOR_DRY_RUN=1`                              | Dry run mode                                                       |
| `OPERATOR_VERBOSE=1`                              | Debug mode                                                         |
| `OPERATOR_NPM_LOGLEVEL=error\|warn\|notice`       | npm log level (default: `error`, hides npm deprecation noise)      |
...
(default `~/.operator`) and no system Node dependency. Supports npm installs
...
<Steps>
  ...
  <Step title="Refresh loaded gateway service">
    ...
    `operator gateway install --force`, which activates the replacement service,
...
<Tabs>
  ...
  <Tab title="Custom prefix + version">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install-cli.sh | bash -s -- --prefix /opt/operator --version latest
    ```
  </Tab>
  <Tab title="Git install">
    ```bash
  ...
  <Tab title="Run onboarding">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install-cli.sh | bash -s -- --onboard
    ```
  </Tab>
</Tabs>
...
| Flag                                    | Description                                                                     |
| --------------------------------------- | ------------------------------------------------------------------------------- |
| `--prefix <path>`                       | Install prefix (default: `~/.operator`)                                         |
| `--install-method \| --method npm\|git` | Choose install method (default: `npm`)                                          |
| `--npm`                                 | Shortcut for npm method                                                         |
| `--git \| --github`                     | Shortcut for git method                                                         |
...
| `--onboard`                             | Run `operator onboard` after install                                            |
...
| Variable                                    | Description                                                        |
| ------------------------------------------- | ------------------------------------------------------------------ |
| `OPERATOR_PREFIX=<path>`                    | Install prefix                                                     |
| `OPERATOR_INSTALL_METHOD=git\|npm`          | Install method                                                     |
| `OPERATOR_VERSION=<ver>`                    | Operator version or dist-tag                                       |
| `OPERATOR_NODE_VERSION=<ver>`               | Node version                                                       |
| `OPERATOR_HOME=<path>`                      | Base directory for Operator state and default git/onboarding paths |
| `OPERATOR_GIT_DIR=<path>`                   | Git checkout directory for git installs                            |
| `OPERATOR_GIT_UPDATE=0\|1`                  | Toggle git updates for existing checkouts                          |
| `OPERATOR_NO_ONBOARD=1`                     | Skip onboarding                                                    |
| `OPERATOR_NPM_LOGLEVEL=error\|warn\|notice` | npm log level (default: `error`)                                   |
...
<Steps>
  ...
  <Step title="Ensure Node.js 24 by default">
    If missing, attempts install via winget, then Chocolatey, then Scoop. If no package manager is available, the script downloads the official Node.js 24 Windows zip into `%LOCALAPPDATA%\Operator\deps\portable-node` and adds it to the current process and user PATH. Node 22.22.3+, Node 24.15+, and Node 25.9+ are supported; Node 23 is unsupported.
  </Step>
  <Step title="Install Operator">
    ...
    - `git` method: clone/update repo, install/build with pnpm, and install wrapper at `%USERPROFILE%\.local\bin\operator.cmd`. If Git is missing, the script bootstraps user-local MinGit under `%LOCALAPPDATA%\Operator\deps\portable-git` and adds it to the current process and user PATH.

  </Step>
  <Step title="Post-install tasks">
    - Adds needed bin directory to user PATH when possible
    - Refreshes a loaded gateway service best-effort (`operator gateway install --force`, then restart)
    - Runs `operator doctor --non-interactive` on upgrades and git installs (best effort)
...
<Tabs>
  ...
  <Tab title="Git install">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://operator.ai/install.ps1))) -InstallMethod git
    ```
  </Tab>
  <Tab title="GitHub main checkout">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://operator.ai/install.ps1))) -InstallMethod git -Tag main
    ```
  </Tab>
  ...
  <Tab title="Dry run">
    ```powershell
    & ([scriptblock]::Create((iwr -useb https://operator.ai/install.ps1))) -DryRun
    ```
  </Tab>
</Tabs>
...
| `OPERATOR_GIT_DIR=<path>`          | Checkout directory |
| `OPERATOR_NO_ONBOARD=1`            | Skip onboarding    |
| `OPERATOR_GIT_UPDATE=0`            | Disable git pull   |
| `OPERATOR_DRY_RUN=1`               | Dry run mode       |
...
<Tabs>
  <Tab title="install.sh (non-interactive npm)">
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install.sh | bash -s -- --no-prompt --no-onboard
    ```
  </Tab>
  <Tab title="install.sh (non-interactive git)">
    ```bash
    OPERATOR_INSTALL_METHOD=git OPERATOR_NO_PROMPT=1 \
      curl -fsSL --proto '=https' --tlsv1.2 https://operator.ai/install.sh | bash
    ```
  </Tab>
...
<AccordionGroup>
  ...
  <Accordion title='Windows: "operator is not recognized"'>
  ...
  <Accordion title="operator not found after install">
