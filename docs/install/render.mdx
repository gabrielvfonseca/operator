---
summary: "Deploy Operator on Render with Infrastructure-as-Code"
read_when:
  - Deploying Operator to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Operator on [Render](https://render.com) using the repo's `render.yaml` Blueprint. It declares the service, disk, and environment variables in one file.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/operator/operator)
...
The Blueprint defaults to `starter`. To use the free tier, change `plan: free` in your fork's `render.yaml` — note that with no persistent disk, Operator state resets on each deploy.
...
Render redeploys automatically when the connected repo's branch gets a new commit. If you deployed straight from `operator/operator` instead of your own fork, you have no push access to trigger that, so update by running a manual Blueprint sync from the Dashboard, or point the service at your own fork.
...
- **Vertical**: change the plan for more CPU/RAM. Usually sufficient for Operator.
- **Horizontal**: increase instance count (Standard plan and above). Requires sticky sessions or external state management since Operator keeps runtime state on the local disk.
...
Happens on the free tier (no persistent disk). Upgrade to a paid plan, or regularly export a backup with `operator backup create` from the Render shell.
