...
Operator ships four update channels:
...
| `extended-stable` | resolves the public npm `extended-stable` selector, verifies the exact selected package, and installs that exact version. Fails closed with no fallback to `latest`, `beta`, or `dev`. | unsupported: Operator leaves the checkout unchanged and asks you to use a package installation                                                                     |
...
operator update --tag 2026.4.1-beta.1
...
- `--tag main` maps to the npm-compatible spec `github:openclaw/openclaw#main`
  ...
  `curl -fsSL https://operator.ai/install.sh | bash -s -- --install-method git --version main`.
...
- Downgrade protection: if the target version is older than the current
  version, Operator prompts for confirmation (skip with `--yes`).
...
Preview what `operator update` would do without making changes:

```bash
operator update --dry-run
operator update --channel beta --dry-run
operator update --tag 2026.4.1-beta.1 --dry-run
operator update --dry-run --json
```
...
Switching channels with `operator update` also syncs plugin sources:
