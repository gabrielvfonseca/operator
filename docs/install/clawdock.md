---
summary: "ClawDock shell helpers for Docker-based Operator installs"
read_when:
  - You run Operator with Docker often and want shorter day-to-day commands
  - You want a helper layer for dashboard, logs, token setup, and pairing flows
title: "ClawDock"
---

ClawDock is a small shell-helper layer for Docker-based Operator installs.
...
| `clawdock-cli <command>`  | Run Operator CLI commands in Docker           |
...
- The project `.env` next to `docker-compose.yml`: Docker-specific values like image name, ports, and `OPERATOR_GATEWAY_TOKEN`. `clawdock-token` reads the token from here.
- `~/.operator/.env` (mounted into the container): env-backed secrets Operator itself manages, alongside `operator.json` and `agents/<agentId>/agent/auth-profiles.json`.
...
Use `clawdock-show-config` to inspect `operator.json` and both `.env` files quickly; it redacts `.env` values in its printed output.

## Related

<CardGroup cols={2}>
  <Card title="Docker" href="/install/docker" icon="docker">
    Canonical Docker install for Operator.
  </Card>
  ...
  <Card title="Updating" href="/install/updating" icon="arrow-up-right-from-square">
    Updating the Operator package and managed services.
  </Card>
</CardGroup>
