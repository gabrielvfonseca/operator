...
Microsoft Teams ships as a bundled plugin in current Operator releases; no separate install is required in the normal packaged build.

On an older build or a custom install that excludes bundled Teams, install the npm package directly:

```bash
operator plugins install @gabrielvfonseca/msteams
```
...
```bash
operator plugins install ./path/to/local/msteams-plugin
```
...
devtunnel create my-operator-bot --allow-anonymous
devtunnel port create my-operator-bot -p 3978 --protocol auto

# Each dev session:
devtunnel host my-operator-bot
...
**4. Configure Operator** using the credentials from the output:
...
- Talk to Operator via Teams DMs, group chats, or channels.
...
- Do not rely on UPN/display-name matching for allowlists; they can change. Operator disables direct name matching by default; opt in with `channels.msteams.dangerouslyAllowNameMatching: true`.
...
- On startup, Operator resolves team/channel and user allowlist names to IDs (when Graph permissions allow) and logs the mapping. Unresolved names are kept as typed but ignored for routing unless `channels.msteams.dangerouslyAllowNameMatching: true` is set.
...
5. Configure `msteams` in `~/.operator/operator.json` (or env vars) and start the gateway.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `operator-msteams` (must be unique) |
...
For production, Operator supports **federated authentication** as an alternative to client secrets, via `channels.msteams.authType: "federated"`. Two methods:
...
3. At runtime, Operator uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint.
...
**Default:** when `authType` is unset, Operator uses client-secret authentication (`appPassword`). Existing configs keep working unchanged.
...
These auth-related config keys can be set via environment variables instead of `operator.json` (other config keys, such as `groupPolicy` or `historyLimit`, are config-only):
...
Operator exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve verified roster details for a configured conversation.
...
```json5
{
  ...
  description: { short: "Operator in Teams", full: "Operator in Teams" },
...
| Teams environment | Operator config                                             | Proactive `serviceUrl`                             |
| ----------------- | ----------------------------------------------------------- | -------------------------------------------------- |
...
When `replyStyle: "thread"` is in effect and the bot was @mentioned from inside a channel thread, Operator re-attaches the original thread root to the outbound conversation reference (`19:...@thread.tacv2;messageid=<root>`) so the reply lands inside the same thread. This holds for both live (in-turn) sends and proactive sends made after the Bot Framework turn context has expired (e.g., long-running agents, queued tool-call replies via `mcp__operator__message`).
...
By default, Operator only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Per-user sharing is more secure since only chat participants can access the file. Operator requires a successful member lookup for group chats; timeouts, transport failures, empty results, and Graph API denials fail the send instead of widening access to the organization.
...
Uploaded files are stored in a `/OperatorShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Operator sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `operator message poll --channel msteams --target conversation:<id> --poll-question "..." --poll-option "..." --poll-option "..."`.
- Votes are recorded by the gateway in Operator plugin-state SQLite under `state/operator.sqlite`.
- Existing `msteams-polls.json` files are imported by `operator doctor --fix`, not by the running plugin.
- The gateway must stay online to record votes.
- Polls do not auto-post result summaries, and there is no poll-results CLI yet.

## Presentation cards

Send semantic presentation payloads to Teams users or conversations using the `message` tool, CLI, or normal reply delivery. Operator renders them as Teams Adaptive Cards from the generic presentation contract.

The `presentation` parameter accepts semantic blocks. When `presentation` is provided, the message text is optional. Buttons render as Adaptive Card submit or URL actions. Select menus are not native in the Teams renderer, so Operator downgrades them to readable text before delivery.
...
```bash
operator message send --channel msteams \
...
operator message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
operator message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
operator message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send a presentation card to a conversation
operator message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --presentation '{"title":"Hello","blocks":[{"type":"text","text":"Hello"}]}'
```
...
- Proactive messages are only possible **after** a user has interacted, because Operator stores conversation references at that point.
...
- **Ignore** the `groupId` query parameter for Operator routing. It is the Microsoft Entra group ID, not the Bot Framework conversation ID used in incoming Teams activities.
