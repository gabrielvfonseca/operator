...
title: IRC
read_when:
  - You want to connect Operator to IRC channels or DMs
  - You are configuring IRC allowlists, group policy, or mention gating
---

Use IRC when you want Operator in classic channels (`#room`) and direct messages.
...
```bash
operator plugins install @gabrielvfonseca/irc
```

2. Set at least host, nick, and the channels to join in `~/.operator/operator.json`:
...
- IRC uses raw TCP/TLS sockets outside Operator operator-managed forward proxy routing. In deployments that require all egress through that forward proxy, set `channels.irc.enabled=false` unless direct IRC egress is explicitly approved.
- `channels.irc.dmPolicy` defaults to `"pairing"`: unknown DM senders get a pairing code you approve with `operator pairing approve irc <code>`.
...
Even if a channel is allowed (via `groupPolicy` + `groups`) and the sender is allowed, Operator defaults to **mention-gating** in group contexts. The bot counts as mentioned when the message contains the connected bot nick or matches your configured mention patterns.
