---
summary: "Synology Chat webhook setup and Operator config"
read_when:
  - Setting up Synology Chat with Operator
  - Debugging Synology Chat webhook routing
title: "Synology Chat"
---

Synology Chat connects to Operator through a webhook pair: a Synology Chat outgoing webhook posts inbound direct messages to the Gateway, and replies go back through a Synology Chat incoming webhook.
...
```bash
operator plugins install @gabrielvfonseca/synology-chat
```

Local checkout (when running from a git repo):

```bash
operator plugins install ./path/to/local/synology-chat-plugin
```
...
3. Point the outgoing webhook URL to your Operator Gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Operator. Synology Chat appears in the same channel setup list in both flows:
   - Guided: `operator onboard` or `operator channels add`
   - Direct: `operator channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart the Gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- Operator accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
```bash
operator message send --channel synology-chat --target 123456 --message "Hello from Operator"
operator message send --channel synology-chat --target synology-chat:123456 --message "Hello again"
operator message send --channel synology-chat --target synology:123456 --message "Short prefix"
```

Outbound text is chunked at 2000 characters. Media sends are supported by URL-based file delivery: the NAS downloads and attaches the file (max 32 MB). Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before Operator forwards the URL to the NAS webhook.
...
Give each enabled account a distinct `webhookPath`. Operator rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached Operator
- `Rate limit exceeded`:
