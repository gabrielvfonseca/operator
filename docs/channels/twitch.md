...
read_when:
  - Setting up Twitch chat integration for Operator
title: "Twitch"
sidebarTitle: "Twitch"
---

Twitch chat support over Twitch's chat (IRC) interface via the Twurple client. Operator signs in as a Twitch bot account, joins one channel per configured account, and replies in that channel.
...
<Tabs>
  <Tab title="npm registry">
    ```bash
    operator plugins install @gabrielvfonseca/twitch
    ```
  </Tab>
  <Tab title="Local checkout">
    ```bash
    operator plugins install ./path/to/local/twitch-plugin
    ```
  </Tab>
</Tabs>

`plugins install` registers and enables the plugin. Picking Twitch during `operator onboard` or `operator channels add` installs it on demand. Use the bare package name to follow the current release; pin an exact version only for reproducible installs. Requires Operator 2026.4.10 or newer.
...
<Steps>
  ...
  <Step title="Configure the token">
    - Env: `OPERATOR_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "@gabrielvfonseca/operator", // Bot's Twitch account (authenticates)
      accessToken: "oauth:abc123...", // OAuth access token (or use OPERATOR_TWITCH_ACCESS_TOKEN env var)
...
- Tokens work with or without the `oauth:` prefix; Operator normalizes both ways (the setup wizard expects the `oauth:` form).

## Token refresh (optional)

Tokens from [Twitch Token Generator](https://twitchtokengenerator.com/) cannot be refreshed by Operator - regenerate when expired (they last a few hours; no app registration needed).
...
- Operator adds no rate limiting of its own; the Twurple chat client handles Twitch rate limits.
