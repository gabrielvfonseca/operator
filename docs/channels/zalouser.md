...
read_when:
  - Setting up Zalo Personal for Operator
...
- Pin a version: `operator plugins install @gabrielvfonseca/zalouser@<version>`
- From a source checkout: `operator plugins install ./path/to/local/zalouser-plugin`
...
- `operator pairing list zalouser`
- `operator pairing approve zalouser <code>`
...
<Note>
`channels.zalouser.groups.<id>.allow` is a legacy field name; current config uses `enabled`. `operator doctor --fix` migrates `allow` to `enabled` automatically.
</Note>
...
- When a group message is skipped because a mention is required, Operator stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Operator state. Example:
...
Profile names select the saved Zalo login credentials in Operator state. Resolution order:
...
- Operator sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Operator sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `operator channels logout --channel zalouser && operator channels login --channel zalouser`
