...
Goal: let Operator sit in WhatsApp groups, wake up only when pinged, and keep that thread separate from the personal DM session.
...
1. Add your WhatsApp account (the one running Operator) to the group.
2. Say `@operator ...` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
