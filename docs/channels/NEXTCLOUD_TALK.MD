...
Nextcloud Talk is a downloadable channel plugin (`@gabrielvfonseca/nextcloud-talk`) that connects Operator to a self-hosted Nextcloud instance through a Talk webhook bot. Direct messages, rooms, reactions, and markdown messages are supported; media goes out as URLs.

## Install

```bash
operator plugins install @gabrielvfonseca/nextcloud-talk
```
...
```bash
operator plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Operator" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure Operator:
   ...
   ```bash
   operator channels add --channel nextcloud-talk \
...
- With `apiUser`/`apiPassword` and `webhookPublicUrl` set, `operator channels status` probes the bot and warns when the `response` feature is missing.

## Access control (DMs)

- Default: `channels.nextcloud-talk.dmPolicy = "pairing"`. Unknown senders get a pairing code.
- Approve via:
  - `operator pairing list nextcloud-talk`
  - `operator pairing approve nextcloud-talk <CODE>`
