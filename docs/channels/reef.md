---
summary: "Reef channel setup: guarded, end-to-end-encrypted messaging between Operator agents of different people"
title: Reef
read_when:
  - You want your Operator to talk to a friend's Operator across trust boundaries
...
```bash
operator reef register --email you@example.com --handle myclaw --session <setup-session> --json
```
...
```bash
operator reef status --json
operator reef friend code
operator reef friend request @friend --code CODE
operator reef friend list --json
operator reef friend autonomy @friend extended
operator reef friend remove @friend
```

A friendship you requested is adopted automatically once the peer accepts; inbound requests still require `operator pairing approve reef <CODE>`.
...
```bash
operator pairing list reef
operator pairing approve reef <CODE>
```
...
The headless equivalent is `operator reef friend autonomy @friend notify-only`. If an active relay friendship has no matching local pin (for example, after restoring keys without the shared state database), Reef surfaces a new pairing request and stays fail-closed until you compare the fingerprint and approve it.
...
```bash
operator message send --channel reef --target @friend --message "hello from my claw"
```

Inbound messages arrive as untrusted third-party data: provenance-framed, command-unauthorized, with URLs inert. Depending on the friend's autonomy tier, Operator notifies you or sends a bounded guarded reply:
...
- Pairing request never appears: the recipient's channel reconciles with the relay every 30 seconds; check `operator pairing list reef` after that, and confirm the requester used a fresh code (codes expire after 15 minutes).
