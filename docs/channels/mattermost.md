---
summary: "Mattermost bot setup and Operator config"
read_when:
...
<Tabs>
  ...
  <Tab title="Local checkout">
    ```bash
    operator plugins install ./path/to/local/mattermost-plugin
    ```
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Ensure plugin is available">
    Install `@gabrielvfonseca/mattermost` with the command above, then restart the Gateway if it is already running.
  </Step>
  ...
  <Step title="Configure Operator and start the gateway">
    Minimal config:
    ...
    ```bash
    operator channels add --channel mattermost --bot-token <token> --http-url https://chat.example.com
    ```
...
Native slash commands are opt-in. When enabled, Operator registers `oc_*` slash commands on every team the bot is a member of and receives callback POSTs on the gateway HTTP server.
...
<AccordionGroup>
  <Accordion title="Behavior notes">
    ...
    - If `callbackUrl` is omitted, Operator derives `http://<gateway.customBindHost or localhost>:<gateway.port, default 18789><callbackPath>`. Wildcard bind hosts (`0.0.0.0`, `::`) fall back to `localhost`.
    ...
    - Command callbacks are validated with the per-command tokens returned by Mattermost when Operator registers `oc_*` commands.
    - Operator refreshes current Mattermost command registration before accepting each callback, so stale tokens from deleted or regenerated slash commands stop being accepted without a gateway restart.
  ...
  <Accordion title="Reachability requirement">
    ...
    - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Operator.
    - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Operator.
    - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Operator, not `404`.
...
- Approve via:
  - `operator pairing list mattermost`
  - `operator pairing approve mattermost <CODE>`
...
Use these target formats with `operator message send` or cron/webhooks:
...
Operator resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Operator sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Operator sends to a Mattermost DM target and needs to resolve the direct channel first, it retries transient direct-channel creation failures by default.
...
Preview streaming is **on by default** in `partial` mode. Configure via `channels.mattermost.streaming.mode` (legacy scalar/boolean `streaming` values are migrated by `operator doctor --fix`):
...
<AccordionGroup>
  ...
  <Accordion title="Streaming behavior notes">
    - If the stream cannot be finalized in place (for example the post was deleted mid-stream), Operator falls back to sending a fresh final post so the reply is never lost.
...
Buttons come from the semantic `presentation` payload (in normal agent replies and in `message action=send`). Operator renders value buttons as Mattermost interactive buttons, keeps URL buttons visible in the message text, and downgrades select menus to readable text.
...
<AccordionGroup>
  ...
  <Accordion title="Config and reachability">
    ...
    - If `interactions.callbackBaseUrl` is omitted, Operator derives the callback URL from `gateway.customBindHost` + `gateway.port` (default 18789), then falls back to `http://localhost:<port>`. The callback path is `/mattermost/interactions/<accountId>`.
    - Reachability rule: the button callback URL must be reachable from the Mattermost server. `localhost` only works when Mattermost and Operator run on the same host/network namespace.
...
External scripts and webhooks can post buttons directly via the Mattermost REST API instead of going through the agent's `message` tool. Prefer Operator's `message` tool. For direct integrations, import `buildButtonAttachments` from `@gabrielvfonseca/mattermost/api.js`; if posting raw JSON, follow these rules:
...
<Steps>
  <Step title="Derive the secret from the bot token">
    `HMAC-SHA256(key="operator-mattermost-interactions", data=botToken)`, hex-encoded.
  </Step>
...
secret = hmac.new(
    b"operator-mattermost-interactions",
...
The Mattermost plugin includes a directory adapter that resolves channel and user names via the Mattermost API. This enables `#channel-name` and `@username` targets in `operator message send` and cron/webhook deliveries.
...
<AccordionGroup>
  ...
  <Accordion title="Native slash commands fail">
    - `Unauthorized: invalid command token.`: Operator did not accept the callback token. Typical causes:
    ...
    - If `callbackUrl` is omitted and logs warn that the callback resolved to a loopback URL like `http://localhost:18789/...`, that URL is probably only reachable when Mattermost runs on the same host/network namespace as Operator. Set an explicit externally reachable `commands.callbackUrl` instead.
