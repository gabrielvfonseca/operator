...
Operator routes replies **back to the channel where a message came from**. The
...
Operator infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Operator still records inbound session metadata, but it
...
when a guarded path must not create a new Operator session. In that mode,
Operator may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Operator would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.operator/workspace-support" }],
  },
  bindings: [
...
store under `~/.operator/agents/<agentId>/sessions/`. Gateway startup and
`operator doctor --fix` import hot legacy rows/history into SQLite
automatically. Use `operator doctor --session-sqlite inspect
