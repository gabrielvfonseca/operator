...
| ----------------------------------- | --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| Connected but no DM replies         | `operator pairing list whatsapp`                    | Approve sender or switch DM policy/allowlist.                                                                                    |
...
| Random disconnect/relogin loops     | `operator channels status --probe` + logs           | Recent reconnects are flagged even when currently connected; watch logs, restart the gateway, then relink if flapping continues. |
...
| Replies arrive seconds/minutes late | `operator doctor --fix`                             | Doctor stops verified stale local TUI clients when they are degrading the Gateway event loop.                                    |
...
| Symptom                              | Fastest check                                    | Fix                                                                                                                        |
| ------------------------------------ | ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `/start` but no usable reply flow    | `operator pairing list telegram`                 | Approve pairing or change DM policy.                                                                                       |
...
| Polling stalls or reconnects slowly  | `operator logs --follow` for polling diagnostics | Upgrade; if restarts are false positives, tune `pollingStallThresholdMs`. Persistent stalls still point to proxy/DNS/IPv6. |
...
| Upgraded and allowlist blocks you    | `operator security audit` and config allowlists  | Run `operator doctor --fix` or replace `@username` with numeric sender IDs.                                                |
...
| Symptom                                   | Fastest check                                                                                                                | Fix                                                                                                                                                                                                                                                                   |
| ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Bot online but no guild replies           | `operator channels status --probe`                                                                                           | Allow guild/channel and verify message content intent.                                                                                                                                                                                                                |
...
| DM replies missing                        | `operator pairing list discord`                                                                                              | Approve DM pairing or adjust DM policy.                                                                                                                                                                                                                               |
...
| Symptom                                | Fastest check                             | Fix                                                                                                                                                  |
| -------------------------------------- | ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| Socket mode connected but no responses | `operator channels status --probe`        | Verify app token + bot token and required scopes; watch for `botTokenStatus` / `appTokenStatus = configured_unavailable` on SecretRef-backed setups. |
| DMs blocked                            | `operator pairing list slack`             | Approve pairing or relax DM policy.                                                                                                                  |
...
| Symptom                              | Fastest check                                           | Fix                                                                   |
| ------------------------------------ | ------------------------------------------------------- | --------------------------------------------------------------------- |
| `imsg` missing or fails on non-macOS | `operator channels status --probe --channel imessage`   | Run Operator on the Messages Mac or use an SSH wrapper for `cliPath`. |
| Can send but no receive on macOS     | Check macOS privacy permissions for Messages automation | Re-grant TCC permissions and restart channel process.                 |
| DM sender blocked                    | `operator pairing list imessage`                        | Approve pairing or update allowlist.                                  |
...
| Symptom                         | Fastest check                              | Fix                                                      |
| ------------------------------- | ------------------------------------------ | -------------------------------------------------------- |
| Daemon reachable but bot silent | `operator channels status --probe`         | Verify `signal-cli` daemon URL/account and receive mode. |
| DM blocked                      | `operator pairing list signal`             | Approve sender or adjust DM policy.                      |
...
| No inbound messages             | `operator channels status --probe`          | Verify credentials on the QQ Open Platform.                     |
...
| Symptom                             | Fastest check                          | Fix                                                                       |
| ----------------------------------- | -------------------------------------- | ------------------------------------------------------------------------- |
| Logged in but ignores room messages | `operator channels status --probe`     | Check `groupPolicy`, room allowlist, and mention gating.                  |
| DMs do not process                  | `operator pairing list matrix`         | Approve sender or adjust DM policy.                                       |
| Encrypted rooms fail                | `operator matrix verify status`        | Re-verify the device, then check `operator matrix verify backup status`.  |
| Backup restore is pending/broken    | `operator matrix verify backup status` | Run `operator matrix verify backup restore` or rerun with a recovery key. |
| Cross-signing/bootstrap looks wrong | `operator matrix verify bootstrap`     | Repair secret storage, cross-signing, and backup state in one pass.       |
