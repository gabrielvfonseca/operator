...
read_when:
  - You want to connect Operator to QQ
  - You need QQ Bot credential setup
  - You want QQ Bot group or private chat support
title: QQ bot
---

QQ Bot connects to Operator via the official QQ Bot API (WebSocket gateway).
...
```bash
operator plugins install @gabrielvfonseca/qqbot
```
...
```bash
operator channels add --channel qqbot --token "AppID:AppSecret"
```
...
binding. Operator persists the returned credentials under the account's config
scope.
...
- `operator channels add --channel qqbot --token-file ...` sets the AppSecret
...
Run multiple QQ bots under a single Operator instance:
...
```bash
operator channels add --channel qqbot --account bot2 --token "222222222:secret-of-bot-2"
```
...
Old QQBot `toolPolicy` entries are retired. Run `operator doctor --fix` to migrate them to `tools`.
...
| `/bot-version`       | —         | private only | Show the Operator framework version and plugin version                         |
...
- Inbound, outbound, and gateway-bridge media share one payload root under
  `~/.operator/media/qqbot` (honoring `OPERATOR_HOME` when set), so uploads,
...
- If a hot upgrade interrupts the Gateway before it finishes writing
  `operator.json`, the plugin restores the last-known `appId` / `clientSecret`
